...
contract Loan is ReentrancyGuard, Initializable, UUPSUpgradeable, Ownable2StepUpgradeable, RateStorage, LoanStorage {
    ...
    function increaseLoan(
        ...
        AccountFactory accountFactory = AccountFactory(getAccountStorage());
        if (address(accountFactory) != address(0)) {
            try accountFactory.isUserAccount(msg.sender) returns (bool exists) {
                isAccount = exists;
            } catch {
    ...
    * @param tokenId The ID of the loan (NFT) for which the vote is being cast.
    ...
    function _vote(uint256 tokenId, address[] memory pools, uint256[] memory weights) internal virtual returns (bool) {
        ...
        bool isAccount = isUserAccount(msg.sender);
        IVoter voter = _voter;
        // Always use the actual voter contract, not the AccountDiamond
    ...
    function isUserAccount(address owner) public view returns (bool) {
        address accountFactory = getAccountStorage();
        if(accountFactory != address(0)) {
            try AccountFactory(accountFactory).isUserAccount(owner) returns (bool exists) {
                return exists;
            } catch {}
        }
        return false;
