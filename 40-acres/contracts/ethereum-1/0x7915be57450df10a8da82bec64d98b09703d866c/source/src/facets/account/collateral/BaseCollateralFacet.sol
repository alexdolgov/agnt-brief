...
abstract contract BaseCollateralFacet is AccessControl, ICollateralFacet {
    ...
    function addCollateral(uint256 tokenId) public onlyPortfolioManagerMulticall(_portfolioFactory) {
        ...
        // ensure the token has locked collateral and meets minimum requirements
        int128 lockedCollateralInt = IVotingEscrow(address(_votingEscrow)).locked(tokenId).amount;
        require(lockedCollateralInt > 0, "Locked collateral amount must be greater than 0");
        uint256 lockedCollateral = uint256(uint128(lockedCollateralInt));
        require(lockedCollateral > config.getMinimumCollateral(), "Amount below minimum collateral");
