...
contract BnbPricePrediction is Ownable {
    ...
    uint256 public internalBlocks;
    address public adminAddress;
    address public immutable WBNB;
    IOracle public oracle;

    event NextRound(uint256 indexed epoch);
    event Bet(address indexed sender, uint256 indexed currentEpoch, bool deirection, uint256 price);
    event Claim(address indexed sender, uint256 indexed currentEpoch, uint256 price);

    constructor(
        IOracle _oracle,
        address _adminAddress,
        uint256 _internalBlocks,
        address _wbnb
    ) public {
        oracle = _oracle;
        adminAddress = _adminAddress;
        internalBlocks = _internalBlocks;
        WBNB = _wbnb;
        // startFirstRound();
    }
    ...
    function nextRound() external onlyAdmin {
        ...
        round.endPrice = oracle.getPrice(WBNB);
        round.rewardAmount = round.totalAmount.mul(9).div(10);
        if (round.endPrice > round.lockPrice) {
            round.rewardBaseCalAmount = round.bullAmount;
        } else {
            round.rewardBaseCalAmount = round.bearAmount;
        }
        // transfer the treasury bnb to admin
        uint256 burningAmount = round.totalAmount.div(10);
        IWBNB(WBNB).withdraw(burningAmount);
        safeTransferBNB(address(msg.sender), burningAmount);

        // set next epoch info
        currentEpoch = currentEpoch + 1;
        rounds[currentEpoch].startBlock = block.number;
        rounds[currentEpoch].lockPrice = oracle.getPrice(WBNB);
    ...
    /**
     * @dev Start the frist round
     */
    function startFirstRound() external onlyOwner {
        // set the first epoch info
        currentEpoch = 0;
        rounds[currentEpoch].startBlock = block.number;
        rounds[currentEpoch].lockPrice = oracle.getPrice(WBNB);
        rounds[currentEpoch].epoch = currentEpoch;
    ...
    /**
     * @dev Claim the reward you win
     * direction, true: bull, false: bear
     */
    function betDirection(bool direction) external payable {
        require (msg.value > 0, '> 0');
        Round storage round = rounds[currentEpoch];
        IWBNB(WBNB).deposit{value: msg.value}();
        assert(IWBNB(WBNB).transfer(address(this), msg.value));
        round.totalAmount = round.totalAmount.add(msg.value);
        ledger[currentEpoch][msg.sender] = BetInfo(direction, msg.value);
        if (direction) {
            round.bullAmount = round.bullAmount.add(msg.value);
        }
        else {
            round.bearAmount = round.bullAmount.add(msg.value);
        }
        // console.log(rounds[currentEpoch].totalAmount);

        emit Bet(msg.sender, currentEpoch, direction, msg.value);
    }
    ...
    function claimable(uint256 epoch, address user) public view returns(bool res) {
        ...
        return (round.lockPrice < round.endPrice) == betInfo.direction;
    }
    ...
    function claim(uint256 epoch) external {
        ...
        Round storage round = rounds[epoch];
        uint256 reward = betInfo.amount.mul(round.rewardAmount).div(round.rewardBaseCalAmount);
        IWBNB(WBNB).withdraw(reward);
        safeTransferBNB(address(msg.sender), reward);
