...
// File: contracts/strategies/StrategyACryptoSCakeV2TokenTokenV4.sol
...
contract StrategyACryptoSCakeV2TokenTokenV4 {
    ...
    function getName() external pure returns (string memory) {
        return "StrategyACryptoSCakeV2TokenTokenV4";
    }
    ...
    function _convertCakeToWant() internal {
      if(cake != tokenA) {
        ...
        if(_cake > 0 ) {
          IERC20(cake).safeApprove(pancakeSwapRouterV2, 0);
          IERC20(cake).safeApprove(pancakeSwapRouterV2, _cake);

          PancakeSwapRouter(pancakeSwapRouterV2).swapExactTokensForTokens(_cake, uint256(0), cakeToTokenAPath, address(this), now.add(1800));
        }
      }
      uint256 _tokenA = IERC20(tokenA).balanceOf(address(this));
      if(_tokenA > 0 ) {
        //convert tokenA
        IERC20(tokenA).safeApprove(pancakeSwapRouterV2, 0);
        IERC20(tokenA).safeApprove(pancakeSwapRouterV2, _tokenA.div(2));

        address[] memory path = new address[](2);
        path[0] = tokenA;
        path[1] = tokenB;

        PancakeSwapRouter(pancakeSwapRouterV2).swapExactTokensForTokens(_tokenA.div(2), uint256(0), path, address(this), now.add(1800));
        ...
        IERC20(tokenA).safeApprove(pancakeSwapRouterV2, 0);
        IERC20(tokenA).safeApprove(pancakeSwapRouterV2, _tokenA);
        IERC20(tokenB).safeApprove(pancakeSwapRouterV2, 0);
        IERC20(tokenB).safeApprove(pancakeSwapRouterV2, _tokenB);

        PancakeSwapRouter(pancakeSwapRouterV2).addLiquidity(
    ...
    function setCakeToTokenAPath(address[] calldata _cakeToTokenAPath) external {
        require(msg.sender == strategist || msg.sender == governance, "!authorized");
        cakeToTokenAPath = _cakeToTokenAPath;
    }
