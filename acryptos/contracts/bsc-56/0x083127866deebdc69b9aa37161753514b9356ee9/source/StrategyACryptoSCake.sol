...
// File: contracts/yearn/interfaces/yearn/IController.sol
...
// File: contracts/yearn/interfaces/yearn/Token.sol
...
// File: contracts/yearn/contracts/strategies/StrategyACryptoSCake.sol
...
contract StrategyACryptoSCake {
    ...
    address public constant syrup = address(0x009cF7bC57584b7998236eff51b98A168DceA9B0);
    address public constant twt = address(0x4B0F1812e5Df2A09796481Ff14017e6005508003);
    address public constant wbnb = address(0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c);
    address public constant cakeChef = address(0x73feaa1eE314F8c655E354234017bE2193C9E24E);
    address public constant syrupChefTwt = address(0xAfd61Dc94f11A70Ae110dC0E0F2061Af5633061A);
    ...
    function _stakeCakeSyrup() internal {
      uint256 _want = IERC20(want).balanceOf(address(this));
      IERC20(want).safeApprove(cakeChef, 0);
      IERC20(want).safeApprove(cakeChef, _want);
      CakeChef(cakeChef).enterStaking(_want);
      uint256 _syrup = IERC20(syrup).balanceOf(address(this));
      IERC20(syrup).safeApprove(syrupChefTwt, 0);
      IERC20(syrup).safeApprove(syrupChefTwt, _syrup);
      SyrupChefTwt(syrupChefTwt).deposit(_syrup);
    }
    ...
    function withdraw(IERC20 _asset) external returns (uint256 balance) {
        ...
        require(twt != address(_asset), "twt");
    ...
    function _withdrawSome(uint256 _amount) internal returns (uint256) {
      SyrupChefTwt(syrupChefTwt).withdraw(_amount);
    ...
    function _withdrawAll() internal {
      (uint256 _amount,) = SyrupChefTwt(syrupChefTwt).userInfo(address(this));
      SyrupChefTwt(syrupChefTwt).withdraw(_amount);
      _amount = balanceOfStakedWant();
      _amount = _amount.min(IERC20(syrup).balanceOf(address(this)));
      CakeChef(cakeChef).leaveStaking(_amount);
      _convertTwtToCake();
    }

    function _convertTwtToCake() internal {
      uint256 _twt = IERC20(twt).balanceOf(address(this));
      if (_twt > 0) {
        IERC20(twt).safeApprove(pancakeSwapRouter, 0);
        IERC20(twt).safeApprove(pancakeSwapRouter, _twt);
        ...
        PancakeSwapRouter(pancakeSwapRouter).swapExactTokensForTokens(_twt, uint256(0), path, address(this), now.add(1800));
      }
    }
    ...
    function balanceOfTwtPendingTwtInCake() public view returns (uint256) {
      uint256 _twt = IERC20(twt).balanceOf(address(this)).add(SyrupChefTwt(syrupChefTwt).pendingReward(address(this)));
      ...
      return PancakeSwapRouter(pancakeSwapRouter).getAmountsOut(_twt, path)[2];
    }
    ...
    function balanceOf() public view returns (uint256) {
      return balanceOfWant()
        ...
        // .add(balanceOfTwtPendingTwtInCake()); 
    }
