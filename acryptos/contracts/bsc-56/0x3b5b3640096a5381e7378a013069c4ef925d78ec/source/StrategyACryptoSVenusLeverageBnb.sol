...
// File: contracts/strategies/StrategyACryptoSVenusLeverageBnb.sol
...
contract StrategyACryptoSVenusLeverageBnb {
    ...
    function _rebalance(uint withdrawAmount) internal {
      ...
      if(_currentL < _L && _L.sub(_currentL) > targetBorrowLimitHysteresis) {
        ...
        while(_currentL > _L && _currentL.sub(_L) > targetBorrowLimitHysteresis) {
          ...
          if(_dy > _max_dy) _dy = _max_dy;
          if(_dy > _liquidityAvailable) _dy = _liquidityAvailable;
          require(IVBNB(vToken).redeemUnderlying(_dy) == 0, "_rebalance: redeem failed");
    ...
    function _withdrawSome(uint256 _amount) internal returns (uint256) {
      _rebalance(_amount);
      uint _balance = IVBNB(vToken).balanceOfUnderlying(address(this));
      if(_amount > _balance) _amount = _balance;
      require(IVBNB(vToken).redeemUnderlying(_amount) == 0, "_withdrawSome: redeem failed");
    ...
    function borrowLimit() public returns (uint256) {
      return IVBNB(vToken).borrowBalanceCurrent(address(this))
        .mul(1e18).div(IVBNB(vToken).balanceOfUnderlying(address(this)).mul(collateralFactor()).div(1e18));
    }
