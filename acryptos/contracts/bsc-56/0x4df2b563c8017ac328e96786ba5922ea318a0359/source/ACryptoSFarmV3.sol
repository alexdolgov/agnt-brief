...
contract ACryptoSFarmV3 is OwnableUpgradeable {
    ...
    struct AdditionalPoolReward {
        ...
        uint256 rewardPerBlock; // reward tokens given per block
    }
    ...
    function harvest(address _lpToken) public {
        ...
        if(pending > 0) {
            ...
            for (uint256 i = 0; i < additionalPoolRewards[_lpToken].length; ++i) {
                // poolSushiPerBlock = sushiPerBlock.mul(pool.allocPoint).div(totalAllocPoint)
                // poolRewardPerBlock = additionalPoolRewards[_lpToken][i].rewardPerBlock
                // pendingReward = pendingSushi * poolRewardPerBlock / poolSushiPerBlock
                safeAdditionalPoolRewardTransfer(
                    msg.sender,
                    pending.mul(additionalPoolRewards[_lpToken][i].rewardPerBlock).div(sushiPerBlock.mul(pool.allocPoint).div(totalAllocPoint)),
    ...
    function addAdditionalPoolRewards(address _lpToken, AdditionalPoolReward[] memory _additionalPoolRewards) external onlyStrategist {
        for (uint i=0; i < _additionalPoolRewards.length; i++) {
            additionalPoolRewards[_lpToken].push(_additionalPoolRewards[i]);
        }
    }
    ...
    function addAdditionalRewards(AdditionalReward[] memory _additionalRewards) external onlyOwner {
        for (uint i=0; i < _additionalRewards.length; i++) {
            additionalRewards.push(_additionalRewards[i]);
        }
    }
