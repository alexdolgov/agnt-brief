...
// File: contracts/strategies/StrategyACryptoSMdxV2.sol
...
contract StrategyACryptoSMdxV2 {
    ...
    address public constant mdxFarm = address(0xc48FE252Aa631017dF253578B1405ea399728A50);
    ...
    constructor(address _governance, address _strategist, address _controller, address _want, address _tokenA, address _tokenB, uint256 _farmPid, address[] memory _rewardToTokenAPath) public {
        want = _want;
        tokenA = _tokenA;
        tokenB = _tokenB;
        farmPid = _farmPid;
    ...
    function deposit() public {
      ...
      if(want == mdx) {
        uint256 _mdx = IERC20(mdx).balanceOf(address(this)); 
        if (_mdx > 0) {
          uint256 _fee = _mdx.mul(performanceFee).div(FEE_DENOMINATOR);
          uint256 _reward = _mdx.mul(strategistReward).div(FEE_DENOMINATOR);
          IERC20(mdx).safeTransfer(IController(controller).rewards(), _fee);
          IERC20(mdx).safeTransfer(strategist, _reward);
          _stakeWant(false);
        }
      }
    }
    ...
    function _stakeWant(bool _force) internal {
      ...
      if (_want > 0) {
        IERC20(want).safeApprove(mdxFarm, 0);
        IERC20(want).safeApprove(mdxFarm, _want);
      }
      if (_want > 0 || _force) {
        IMdxFarm(mdxFarm).deposit(farmPid, _want);
      }
    }
    ...
    function _withdrawSome(uint256 _amount) internal returns (uint256) {
      if(want == mdx) {
        uint256 _mdx = IERC20(mdx).balanceOf(address(this)); 
        IMdxFarm(mdxFarm).withdraw(farmPid, _amount);
        _mdx = IERC20(mdx).balanceOf(address(this)).sub(_mdx).sub(_amount);
        if (_mdx > 0) {
          uint256 _fee = _mdx.mul(performanceFee).div(FEE_DENOMINATOR);
          uint256 _reward = _mdx.mul(strategistReward).div(FEE_DENOMINATOR);
          IERC20(mdx).safeTransfer(IController(controller).rewards(), _fee);
          ...
          IERC20(want).safeApprove(mdxFarm, 0);
          IERC20(want).safeApprove(mdxFarm, _mdx.sub(_fee).sub(_reward));
          IMdxFarm(mdxFarm).deposit(farmPid, _mdx.sub(_fee).sub(_reward));
        }
      } else {
        IMdxFarm(mdxFarm).withdraw(farmPid, _amount);        
      }
    ...
    function _withdrawAll() internal {
      IMdxFarm(mdxFarm).emergencyWithdraw(farmPid);
    }
    ...
    function _convertRewardsToWant() internal {
      if(mdx != tokenA) {
        uint256 _mdx = IERC20(mdx).balanceOf(address(this));
        if(_mdx > 0 ) {
          IERC20(mdx).safeApprove(mdexRouter, 0);
          IERC20(mdx).safeApprove(mdexRouter, _mdx);

          IMdexRouter(mdexRouter).swapExactTokensForTokens(_mdx, uint256(0), rewardToTokenAPath, address(this), now.add(1800));
        }
      }
      if(want != tokenA) {
    ...
    function balanceOfStakedWant() public view returns (uint256) {
      (uint256 _amount,,) = IMdxFarm(mdxFarm).userInfo(farmPid,address(this));
      return _amount;
    }
    ...
    function harvest() public returns (uint harvesterRewarded) {
      ...
      uint _mdx = IERC20(mdx).balanceOf(address(this)); 
      uint256 _harvesterReward;
      if (_mdx > 0) {
        uint256 _fee = _mdx.mul(performanceFee).div(FEE_DENOMINATOR);
        uint256 _reward = _mdx.mul(strategistReward).div(FEE_DENOMINATOR);
        _harvesterReward = _mdx.mul(harvesterReward).div(FEE_DENOMINATOR);
        IERC20(mdx).safeTransfer(IController(controller).rewards(), _fee);
        IERC20(mdx).safeTransfer(strategist, _reward);
        IERC20(mdx).safeTransfer(msg.sender, _harvesterReward);
      }
...
interface IMdxFarm {
  ...
  // function addMultLP ( address _addLP ) external returns ( bool );
  ...
  // function getMdxBlockReward ( uint256 _lastRewardBlock ) external view returns ( uint256 );
  // function getMultLPAddress ( uint256 _pid ) external view returns ( address );
  // function getMultLPLength (  ) external view returns ( uint256 );
  // function halvingPeriod (  ) external view returns ( uint256 );
  // function isBadAddress ( address account ) external view returns ( bool );
  // function isMultLP ( address _LP ) external view returns ( bool );
  // function massUpdatePools (  ) external;
  // function mdx (  ) external view returns ( address );
  // function mdxPerBlock (  ) external view returns ( uint256 );
  // function multLpChef (  ) external view returns ( address );
  ...
  // function poolCorrespond ( uint256 ) external view returns ( uint256 );
  // function poolInfo ( uint256 ) external view returns ( address lpToken, uint256 allocPoint, uint256 lastRewardBlock, uint256 accMdxPerShare, uint256 accMultLpPerShare, uint256 totalAmount );
  // function poolLength (  ) external view returns ( uint256 );
  // function renounceOwnership (  ) external;
  // function replaceMultLP ( address _multLpToken, address _multLpChef ) external;
  ...
  // function setMdxPerBlock ( uint256 newPerBlock ) external;
  // function setMultLP ( address _multLpToken, address _multLpChef ) external;
  // function setPause (  ) external;
  // function setPoolCorr ( uint256 _pid, uint256 _sid ) external;
  // function startBlock (  ) external view returns ( uint256 );
  // function totalAllocPoint (  ) external view returns ( uint256 );
  // function transferOwnership ( address newOwner ) external;
  // function updatePool ( uint256 _pid ) external;
  function userInfo ( uint256, address ) external view returns ( uint256 amount, uint256 rewardDebt, uint256 multLpRewardDebt );
