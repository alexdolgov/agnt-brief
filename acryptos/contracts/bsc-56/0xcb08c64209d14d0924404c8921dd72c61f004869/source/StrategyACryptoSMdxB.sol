...
// File: contracts/strategies/StrategyACryptoSMdxB.sol
...
contract StrategyACryptoSMdxB {
    ...
    address public constant mdxBFarm = address(0x6aEE12e5Eb987B3bE1BA8e621BE7C4804925bA68);
    ...
    function _stakeWant(bool _force) internal {
      ...
      if (_want > 0) {
        IERC20(want).safeApprove(mdxBFarm, 0);
        IERC20(want).safeApprove(mdxBFarm, _want);
      }
      if (_want > 0 || _force) {
        IMdxBFarm(mdxBFarm).deposit(farmPid, _want);
      }
    }
    ...
    function _withdrawSome(uint256 _amount) internal returns (uint256) {
      if(want == mdx) {
        uint256 _mdx = IERC20(mdx).balanceOf(address(this)); 
        IMdxBFarm(mdxBFarm).withdraw(farmPid, _amount);
        ...
        if (_mdx > 0) {
          ...
          IERC20(want).safeApprove(mdxBFarm, 0);
          IERC20(want).safeApprove(mdxBFarm, _mdx.sub(_fee).sub(_reward));
          IMdxBFarm(mdxBFarm).deposit(farmPid, _mdx.sub(_fee).sub(_reward));
        }
      } else {
        IMdxBFarm(mdxBFarm).withdraw(farmPid, _amount);        
      }
    ...
    function _withdrawAll() internal {
      IMdxBFarm(mdxBFarm).emergencyWithdraw(farmPid);
    }
    ...
    function balanceOfStakedWant() public view returns (uint256) {
      (uint256 _amount,) = IMdxBFarm(mdxBFarm).userInfo(farmPid,address(this));
      return _amount;
    }
    ...
    function harvest() public returns (uint harvesterRewarded) {
      ...
      _stakeWant(true);
      if(enableHarvestMdxSwapFarm) IMdxSwapFarm(mdxSwapFarm).takerWithdraw(); //harvest mdx from swap farm
