...
// File: contracts/strategies/StrategyACryptoSCakeBnbCake.sol
...
contract StrategyACryptoSCakeBnbCake {
    ...
    constructor(address _controller, address _want, uint256 _cakeChefPid) public {
        want = _want;
    ...
    function getName() external pure returns (string memory) {
        return "StrategyACryptoSCakeBnbCake";
    }
    ...
    function withdrawAll() external returns (uint256 balance) {
      ...
      //waste not - send dust cake to rewards
      IERC20(cake).safeTransfer(IController(controller).rewards(),
    ...
    function _convertCakeToWant() internal {
      ...
      if(_cake > 0 ) {
        _convertToken(_cake.div(2),cake,wbnb);
        
        uint256 _wbnb = IERC20(wbnb).balanceOf(address(this));

        _cake = IERC20(cake).balanceOf(address(this));
