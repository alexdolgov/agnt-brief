...
// File: contracts/strategies/StrategyACryptoSHmdxB.sol
...
contract StrategyACryptoSHmdxB {
    using SafeERC20 for IERC20;
    using Address for address;
    using SafeMath for uint256;
    using Math for uint256;

    address public constant mdx = address(0x9C65AB58d8d978DB963e63f2bfB7121627e3a739); //MDX
    address public constant hmdx = address(0xAEE4164c1ee46ed0bbC34790f1a3d1Fc87796668); //HMDX
    address public constant hmdxBFarm = address(0x55c11417C7D9789161A3F8478b8B9057830a6fc9);
    ...
    function deposit() public {
      ...
      if(want == hmdx) {
        uint256 _hmdx = IERC20(hmdx).balanceOf(address(this)); 
        if (_hmdx > 0) {
          uint256 _fee = _hmdx.mul(performanceFee).div(FEE_DENOMINATOR);
          uint256 _reward = _hmdx.mul(strategistReward).div(FEE_DENOMINATOR);
          IERC20(hmdx).safeTransfer(IController(controller).rewards(), _fee);
          IERC20(hmdx).safeTransfer(strategist, _reward);
          _stakeWant(false);
        }
      }
    }
    ...
    function _stakeWant(bool _force) internal {
      ...
      if (_want > 0) {
        IERC20(want).safeApprove(hmdxBFarm, 0);
        IERC20(want).safeApprove(hmdxBFarm, _want);
      }
      if (_want > 0 || _force) {
        IHmdxBFarm(hmdxBFarm).deposit(farmPid, _want);
      }
    }
    ...
    function _withdrawSome(uint256 _amount) internal returns (uint256) {
      IHmdxBFarm(hmdxBFarm).withdraw(farmPid, _amount);
    ...
    function _withdrawAll() internal {
      IHmdxBFarm(hmdxBFarm).emergencyWithdraw(farmPid);
    }

    function _convertHmdxToMdx() internal {
      uint256 _hmdx = IERC20(hmdx).balanceOf(address(this));
      if(_hmdx > 0 ) {
        IERC20(hmdx).safeApprove(mdexRouter, 0);
        IERC20(hmdx).safeApprove(mdexRouter, _hmdx);

        address[] memory path = new address[](2);
        path[0] = hmdx;
        path[1] = mdx;

        IMdexRouter(mdexRouter).swapExactTokensForTokens(_hmdx, uint256(0), path, address(this), now.add(1800));
      }
    }
    ...
    function balanceOfStakedWant() public view returns (uint256) {
      (uint256 _amount,) = IHmdxBFarm(hmdxBFarm).userInfo(farmPid,address(this));
      return _amount;
    }
    ...
    function harvest() public returns (uint harvesterRewarded) {
      ...
      _mdx = IERC20(mdx).balanceOf(address(this)).sub(_mdx); 
...
interface IHmdxBFarm {
  ...
  // function hmdx (  ) external view returns ( address );
  // function hmdxPerBlock (  ) external view returns ( uint256 );
  // function isBadAddress ( address account ) external view returns ( bool );
  // function massUpdatePools (  ) external;
  // function newReward ( uint256 _hmdxAmount, uint256 _newPerBlock, uint256 _startBlock ) external;
  // function owner (  ) external view returns ( address );
  // function pending ( uint256 _pid, address _user ) external view returns ( uint256 );
  // function poolInfo ( uint256 ) external view returns ( address lpToken, uint256 allocPoint, uint256 lastRewardBlock, uint256 accHMDXPerShare, uint256 hmdxAmount );
