...
interface IVault is IVaultEvents {
    ...
    /// @notice Get the address of wrapped native token.
    /// @return wrappedNativeToken Address of the wrapped native token.
    function wrappedNativeToken() external view returns (address wrappedNativeToken);
}
