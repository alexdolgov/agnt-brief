...
contract PMOTTreasureChestGameV3 is Ownable, ReentrancyGuard, IERC721Receiver  {
   ...
   function playGame(uint256 _rarity, uint256 _numOfPlays) public notContract nonReentrant  {       
       ...
       totalBurnedStat = totalBurnedStat.add(burnAmount);
        ...
        uint256 lastPlayResult = 0;
         for(uint i = 0; i< _numOfPlays; i++){
           ...
           uint256 seed  = playResult + lastPlayResult + _numOfPlays;
    ...
    function jackpot() public view returns (uint256) {
        ...
        if(result > 0){
            return  result.sub(pendingTokenPrizesGlobal).mul(jackPotRatio).div(percentageBase);            
        }
        else{
            return 0;
        }
    ...
    function setJackpotMultiplier(uint256 jack) public onlyOwner {
        jackPotRatio = jack;
    }
