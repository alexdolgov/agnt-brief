...
// ============================================================
// FILE: src/library/PauseGuardian.sol
// ============================================================
...
// ============================================================
// FILE: src/OFT/AspidaEscrowOFTAdapter.sol
// ============================================================
...
/**
 * @title Aspida Escrow OFT Adapter
 * @dev Extends AspidaOFTAdapter to include escrow functionality for token management.
 */
contract AspidaEscrowOFTAdapter is AspidaOFTAdapter {
    using SafeERC20 for IERC20;

    address internal escrow_;

    event SetEscrow(address oldEscrow, address newEscrow);

    /**
     * @dev Constructor for AspidaEscrowOFTAdapter.
     ...
     * @param _escrow Initial escrow address where tokens will be held.
     */
    constructor(address _token, address _lzEndpoint, address _escrow) AspidaOFTAdapter(_token, _lzEndpoint) {
        _setEscrowInternal(_escrow);
    }

    /**
     * @dev Internal function to set a new escrow address.
     * @param _escrow The new escrow address to be set.
     * Emits a SetEscrow event on successful update.
     */
    function _setEscrowInternal(address _escrow) internal {
        address _oldEscrow = escrow_;
        require(_escrow != _oldEscrow && _escrow != address(0), "_setEscrowInternal: Invalid escrow address");
        escrow_ = _escrow;
        emit SetEscrow(_oldEscrow, _escrow);
    }

    /**
     * @dev External function callable only by the owner to update the escrow address.
     * Allows the contract owner to update the escrow address securely.
     * @param _escrow The new escrow address to be set.
     */
    function _setEscrow(address _escrow) external onlyOwner {
        _setEscrowInternal(_escrow);
    }

    /**
     * @dev Burns tokens from the sender's specified balance, i.e., pull method.
     * This function is responsible for debiting tokens from a specified address and locking them in escrow.
    ...
    function _debit(
        ...
        // Transfer the specified amount of tokens from the sender's address to the escrow address.
        // This effectively locks the tokens within the contract under control of the escrow.
        innerToken.safeTransferFrom(_from, escrow_, amountSentLD);
    }

    /**
     * @dev Credits tokens to the specified address from the escrow.
     * This function handles the transfer of tokens from the escrow account to a specified recipient address.
     *
    ...
    function _credit(
        ...
        // Unlock the tokens and transfer them to the recipient from the escrow.
        innerToken.safeTransferFrom(escrow_, _to, _amountLD);
        // Return the amount credited, assuming no fees or losses unless specified by an overriding function.
        return _amountLD;
    }

    function escrow() external view virtual returns (address) {
        return escrow_;
    }
}

// ============================================================
// FILE: src/OFT/AspidaEscrowOFTAdapterUpgradeable.sol
// ============================================================
...
import { AspidaEscrowOFTAdapter } from "./AspidaEscrowOFTAdapter.sol";

/**
 * @title Aspida Escrow OFT Adapter Upgradeable Contract
 * @dev Extends AspidaEscrowOFTAdapter to be upgradeable using OpenZeppelin's Initializable pattern.
 */
contract AspidaEscrowOFTAdapterUpgradeable is Initializable, AspidaEscrowOFTAdapter {
    /**
     * @dev Constructor that initializes the contract with necessary addresses.
     * This constructor is only called once during the initial deployment.
     * @param _token Address of the token managed by the escrow.
    ...
    constructor(
        ...
        address _lzEndpoint,
        address _escrow
    ) AspidaEscrowOFTAdapter(_token, _lzEndpoint, _escrow) {
        _disableInitializers();
    }
     ...
     * @param _escrow Address of the escrow to manage the tokens.
     */
    function initialize(address _token, address _escrow) public initializer {
...
// ============================================================
// FILE: src/OFT/AspidaOFTAdapter.sol
// ============================================================
