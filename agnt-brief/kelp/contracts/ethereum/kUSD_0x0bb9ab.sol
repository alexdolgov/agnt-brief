...
interface IRateProvider {
    function kUSDPerToken() external view returns (uint256);
    function tokenPerkUSD() external view returns (uint256);
}
...
contract kUSD is ERC20, Ownable2Step {
    /// DEPENDENCIES ///
    ...
    uint256 public constant MIN_REDEEM = 1 ether;

    /// EVENTS ///

    event Deposit(address indexed staker, uint256 kUSDReceived);
    event Redeemed(address indexed staker, uint256 kUSDBurned);
    ...
    constructor() ERC20("Kernel USD", "kUSD") Ownable2Step() {
        feeAddress = owner();
        mintFee = 10;
        redeemFee = 25;
    }
    ...
    /// @notice                Deposit `_approvedToken` to receive kUSD
    /// @param _approvedToken  Token to deposit
    /// @param _to             Address to send minted kUSD to
    ...
    function deposit(address _approvedToken, address _to, uint256 _amount, bool _receipt) external {
        ...
        uint256 _rate = IRateProvider(token.rateProvider).kUSDPerToken();
    ...
    /// @notice                         Redeem kUSD to receive `_approvedTokenToReceive`
    ...
    /// @param _kUSDToRedeem           Amount of kUSD to redeem
    function redeem(address _approvedTokenToReceive, address _to, uint256 _kUSDToRedeem) external {
        require(approvedToken[_approvedTokenToReceive], "Not token");
        require(_kUSDToRedeem >= MIN_REDEEM, "Less than min redeem");

        Token memory token = tokens[_approvedTokenToReceive];

        uint256 _rate = IRateProvider(token.rateProvider).tokenPerkUSD();

        if (msg.sender != owner() && msg.sender != feeAddress && redeemFee > 0) {
            uint256 _fee = _kUSDToRedeem * redeemFee / FEE_DENOM;
            _kUSDToRedeem -= _fee;
            _transfer(msg.sender, feeAddress, _fee);
        }

        uint256 _toSend = _rate * _kUSDToRedeem / (10 ** decimals());

        require(token.deposited >= _toSend, "Not enough funds to redeem token");

        tokens[_approvedTokenToReceive].deposited -= _toSend;
        _burn(msg.sender, _kUSDToRedeem);

        address _vaultToken = token.vaultToken;

        IERC20(_vaultToken).safeTransfer(_to, _toSend);

        emit Redeemed(_to, _kUSDToRedeem);
    }
