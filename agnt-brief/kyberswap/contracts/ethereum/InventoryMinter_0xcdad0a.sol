...
// ============================================================
// FILE: lib/@uniswap/v3-core/contracts/interfaces/callback/IUniswapV3SwapCallback.sol
// ============================================================
...
// ============================================================
// FILE: lib/@uniswap/v3-core/contracts/libraries/FixedPoint96.sol
// ============================================================
...
// ============================================================
// FILE: lib/@uniswap/v3-core/contracts/libraries/SafeCast.sol
// ============================================================
...
// ============================================================
// FILE: lib/@uniswap/v3-core/contracts/libraries/SqrtPriceMath.sol
// ============================================================
...
// ============================================================
// FILE: lib/@uniswap/v3-core/contracts/libraries/UnsafeMath.sol
// ============================================================
...
// ============================================================
// FILE: lib/@uniswap/v3-periphery/contracts/interfaces/ISwapRouter.sol
// ============================================================
...
// ============================================================
// FILE: lib/@uniswap/v3-periphery/contracts/libraries/LiquidityAmounts.sol
// ============================================================
...
// ============================================================
// FILE: src/governance/InventoryMinter.sol
// ============================================================
...
contract InventoryMinter {
    using OrderLib for OrderLib.Order;
    using SafeERC20 for IERC20;

    event LimitOrder(bytes32 hash);

    address constant router = address(0x1111111254EEB25477B68fb85Ed929f73A960582);
    address constant positions = address(0xC36442b4a4522E871399CD717aBDD847Ab11FE88);
    ...
    bytes32 public immutable ROUTER_DOMAIN_SEPARATOR = _routerDomainSeparator(router);

    uint256 public amount0;
    uint256 public amount1;
    uint256 tokenId;

    OrderLib.Order order0;
    OrderLib.Order order1;

    mapping(bytes32 => bool) approvedHashes;

    constructor(address _token0, address _token1, uint256 _amount0, uint256 _amount1, int24 _tickLower, int24 _tickUpper, uint24 _fee, address _owner, int256 _price) {
        token0 = _token0;
        token1 = _token1;
        amount0 = _amount0;
        amount1 = _amount1;
        ...
        ILegacyERC20(token0).approve(address(positions), type(uint256).max);
        ILegacyERC20(token1).approve(address(positions), type(uint256).max);

        ILegacyERC20(token0).approve(address(router), type(uint256).max);
        ILegacyERC20(token1).approve(address(router), type(uint256).max);
    }

    function getOrder0() external view returns (bytes32 hash, OrderLib.Order memory order) {
        order = order0;
        hash = order.hash(ROUTER_DOMAIN_SEPARATOR);
    }

    function getOrder1() external view returns (bytes32 hash, OrderLib.Order memory order) {
        order = order1;
        hash = order.hash(ROUTER_DOMAIN_SEPARATOR);
    }
    ...
    function cancel() external {
        ...
        IRouter(router).cancelOrder(order0);
        IRouter(router).cancelOrder(order1);
    }
    ...
    function mint() external {
        ...
        (
            ...
            dAmount0,
            dAmount1
        ) = INonfungiblePositionManager(positions).mint(INonfungiblePositionManager.MintParams({
        ...
        if (price > 0) {
            order0 = _openLimitOrder(token0, token1, tokenId, amount0, amount0 * uint256(price));
            order1 = _openLimitOrder(token1, token0, tokenId, amount1, amount1 / uint256(price));
        } else {
            order0 = _openLimitOrder(token0, token1, tokenId, amount0, amount0 / uint256(-price));
            order1 = _openLimitOrder(token1, token0, tokenId, amount1, amount1 * uint256(-price));
        }
    ...
    function _openLimitOrder(address _token0, address _token1, uint256 _tokenId, uint256 _amount0, uint256 _amount1) internal returns (OrderLib.Order memory order) {
        if (_amount0 == 0) {
            return order;
        }

        bytes memory postInteraction = abi.encodePacked(address(this), abi.encode(_token0));

        order = OrderLib.Order({
            salt: uint256(keccak256(abi.encode(address(this)))),
            makerAsset: _token0,
            takerAsset: _token1,
            maker: address(this),
            receiver: address(this),
            allowedSender: address(0),
            makingAmount: _amount0,
            takingAmount: _amount1,
            offsets: (postInteraction.length << 224),
            interactions: postInteraction
        });

        bytes32 hash = order.hash(ROUTER_DOMAIN_SEPARATOR);
        approvedHashes[hash] = true;

        emit LimitOrder(hash);
    }
    ...
    function fillOrderPostInteraction(
        ...
        (address _token0) = abi.decode(interactiveData, (address));
        uint256 dAmount0;
        uint256 dAmount1;
        if (token0 == _token0) {
            (, dAmount0, dAmount1) = INonfungiblePositionManager(positions).increaseLiquidity(INonfungiblePositionManager.IncreaseLiquidityParams({
                tokenId: tokenId,
                amount0Desired: amount0 - makingAmount,
                amount1Desired: amount1 + takingAmount,
                amount0Min: 0,
                amount1Min: amount1 + takingAmount - (price > 0 ? uint256(price) : 0),
                deadline: block.timestamp
            }));
            amount0 = amount0 - makingAmount - dAmount0;
            amount1 = amount1 + takingAmount - dAmount1;
        } else {
            (, dAmount0, dAmount1) = INonfungiblePositionManager(positions).increaseLiquidity(INonfungiblePositionManager.IncreaseLiquidityParams({
                tokenId: tokenId,
                amount0Desired: amount0 + takingAmount,
                amount1Desired: amount1 - makingAmount,
                amount0Min: amount0 + takingAmount - (price < 0 ? uint256(-price) : 0),
                amount1Min: 0,
                deadline: block.timestamp
            }));
            amount0 = amount0 + takingAmount - dAmount0;
            amount1 = amount1 - makingAmount - dAmount1;
        }
    }

    function increaseLiquidity() external {
        (, uint256 dAmount0, uint256 dAmount1) = INonfungiblePositionManager(positions).increaseLiquidity(INonfungiblePositionManager.IncreaseLiquidityParams({
            tokenId: tokenId,
            amount0Desired: amount0,
            amount1Desired: amount1,
            amount0Min: 0,
    ...
    function _routerDomainSeparator(address _router) internal view returns (bytes32) {
        bytes32 hashedName = keccak256(bytes("1inch Aggregation Router"));
        bytes32 hashedVersion = keccak256(bytes("5"));
        bytes32 typeHash = keccak256(
            "EIP712Domain(string name,string version,uint256 chainId,address verifyingContract)"
        );
        return _buildDomainSeparator(_router, typeHash, hashedName, hashedVersion);
    }
    ...
    function _buildDomainSeparator(
        ...
        return keccak256(abi.encode(typeHash, nameHash, versionHash, block.chainid, _router));
    }
}

// ============================================================
// FILE: src/governance/OrderLib.sol
// ============================================================
...
library OrderLib {
    ...
    function hash(Order memory order, bytes32 domainSeparator) internal pure returns(bytes32 result) {
        result = keccak256(abi.encode(_LIMIT_ORDER_TYPEHASH,
            order.salt, order.makerAsset, order.takerAsset,
            order.maker, order.receiver, order.allowedSender,
            order.makingAmount, order.takingAmount, order.offsets,
            keccak256(order.interactions))
        );
        result = keccak256(abi.encodePacked("\x19\x01", domainSeparator, result));
    }
    ...
    function _get(Order memory order, DynamicField field) private pure returns(bytes memory res) {
        res = order.interactions;
        uint256 bitShift = uint256(field) << 5; // field * 32
        uint256 from = uint32((order.offsets << 32) >> bitShift);
        uint256 to = uint32(order.offsets >> bitShift);
        assembly {
            res := add(res, from)
            mstore(res, sub(to, from))
        }
    }

    function makerAssetData(Order memory order) internal pure returns(bytes memory) {
        return _get(order, DynamicField.MakerAssetData);
    }

    function takerAssetData(Order memory order) internal pure returns(bytes memory) {
        return _get(order, DynamicField.TakerAssetData);
    }

    function getMakingAmount(Order memory order) internal pure returns(bytes memory) {
        return _get(order, DynamicField.GetMakingAmount);
    }

    function getTakingAmount(Order memory order) internal pure returns(bytes memory) {
        return _get(order, DynamicField.GetTakingAmount);
    }

    function predicate(Order memory order) internal pure returns(bytes memory) {
        return _get(order, DynamicField.Predicate);
    }

    function permit(Order memory order) internal pure returns(bytes memory) {
        return _get(order, DynamicField.Permit);
    }

    function preInteraction(Order memory order) internal pure returns(bytes memory) {
        return _get(order, DynamicField.PreInteraction);
    }

    function postInteraction(Order memory order) internal pure returns(bytes memory) {
        return _get(order, DynamicField.PostInteraction);
    }
}

// ============================================================
// FILE: src/interfaces/IBurnableERC20.sol
// ============================================================
...
// ============================================================
// FILE: src/interfaces/IERC677Receiver.sol
// ============================================================
...
// ============================================================
// FILE: src/interfaces/ILegacyERC20.sol
// ============================================================
...
// ============================================================
// FILE: src/interfaces/IMintableERC20.sol
// ============================================================
...
// ============================================================
// FILE: src/token/ERC20Blocklist.sol
// ============================================================
...
// ============================================================
// FILE: src/token/ERC20MintBurn.sol
// ============================================================
...
// ============================================================
// FILE: src/token/ERC20Permit.sol
// ============================================================
...
// ============================================================
// FILE: src/token/ERC20Recovery.sol
// ============================================================
