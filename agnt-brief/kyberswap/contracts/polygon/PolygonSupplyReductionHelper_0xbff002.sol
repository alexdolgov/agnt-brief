...
// ============================================================
// FILE: script/scripts/PolygonSupplyReductionHelper.sol
// ============================================================
...
interface IBobSwap {
    ...
    function disableCollateralYield(address token) external;
}

contract PolygonSupplyReductionHelper {
    address constant positionManager = address(0xC36442b4a4522E871399CD717aBDD847Ab11FE88);
    uint256 constant tokenId1 = uint256(412228); // USDC/BOB 0.01%
    ...
    address constant bobSwap = address(0x25E6505297b44f4817538fB2d91b88e1cF841B54);
    ...
    function step1() external {
        ...
        (uint256 amountUSDC1, uint256 amountBOB1) = INonfungiblePositionManager(positionManager).decreaseLiquidity(
            INonfungiblePositionManager.DecreaseLiquidityParams(tokenId1, liquidity1 * 11 / 13, 0, 0, block.timestamp)
        );
        ...
        IBobToken(usdc).approve(bobSwap, amountUSDC1);
        IBobSwap(bobSwap).give(usdc, amountUSDC1);

        IBobSwap(bobSwap).setCollateralFees(usdc, 1 ether, 0);
        IBobSwap(bobSwap).setCollateralFees(usdt, 1 ether, 0);

        IBobSwap(bobSwap).disableCollateralYield(usdc);
        IBobSwap(bobSwap).disableCollateralYield(usdt);
