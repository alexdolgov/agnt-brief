...
// ============================================================
// FILE: src/interface/ISatLayerPoolV2.sol
// ============================================================
...
interface ISatLayerPoolV2 {
    ...
    error WithdrawAttemptTooEarly(); // thrown if staker attempts to withdraw funds before withdrawal window has passed
    error WithdrawTimeNotSet(); // thrown if withdraw time for the asset not set
    ...
    ///@notice Emitted when a staker queues tokens to be withdrawed
    ...
    event WithdrawQueued(uint256 indexed eventId, address indexed withdrawer, address indexed token, uint256 amount);
    ...
    ///@notice queues the withdrawal of a token, adds _amount to currently
    ///@param _token The token to withdraw from the SatLayer Pool
    ///@param _amount amoount of token to be withdrawn, will be added to any pending balances for user
    ///@dev multiple calls to this function for the same _token will reset the withdraw timer
    function queueWithdrawal(address _token, uint256 _amount) external;

    ///@notice finalize withdrawal of a specified amount of a particular supported token previously staked into the SatLayer Pool
    ///@param _token The token to withdraw from the SatLayer Pool
    ///@dev user must have called queueWithdrawal() for the token first
...
import "./interface/ISatLayerPoolV2.sol";

/// @title SatLayer Pool V2
/// @notice A staking pool for liquid restaking token holders which rewards stakers with points from multiple platforms
contract SatLayerPoolV2 is ISatLayerPoolV2, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    using SafeERC20 for IERC20Metadata;

    uint256 WITHDRAW_PERIOD;

    mapping(address => bool) public tokenAllowlist;

    mapping(address => address) public tokenMap;

    // receipt token address => user address => value
    mapping(address => mapping(address => uint256)) public withdrawTimes;
    mapping(address => mapping(address => uint256)) public withdrawAmounts;
    ...
    /**
     * @inheritdoc ISatLayerPoolV2
     */
    function depositFor(address _token, address _for, uint256 _amount) external whenNotPaused nonReentrant {
    ...
    function queueWithdrawal(address _token, uint256 _amount) external {
        ...
        emit WithdrawQueued(++eventId, msg.sender, _token, _amount);
        ...
        // queueing withdrawal from the same wallet resets any existing timer
        withdrawTimes[_token][msg.sender] = block.timestamp + WITHDRAW_PERIOD;
        withdrawAmounts[_token][msg.sender] += _amount;
    }
    ...
    function withdraw(address _token) external {
        ...
        uint256 amount = withdrawAmounts[_token][msg.sender];
