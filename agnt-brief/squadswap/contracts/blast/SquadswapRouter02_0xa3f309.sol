...
// ============================================================
// FILE: contracts/router/interfaces/BlastModes.sol
// ============================================================
...
// ============================================================
// FILE: contracts/router/interfaces/IBlast.sol
// ============================================================
...
library SquadswapLibrary {
                ...
                uint(
                    keccak256(
                        abi.encodePacked(
                            ...
                            hex"58c2c3390cddef0a17aed31cf6b51cc4b11e96866b8b16a613fc7999daefb24e" // init code hash
                        )
                    )
                )
            )
    ...
    function getReserves(
        ...
        (uint reserve0, uint reserve1, ) = ISquadswapPair(
    ...
    function quote(
        ...
        require(
            ...
            "SquadswapLibrary: INSUFFICIENT_LIQUIDITY"
        );
...
contract SquadswapRouter02 is Ownable {
    ...
    function _addLiquidity(
        ...
        (uint reserveA, uint reserveB) = SquadswapLibrary.getReserves(
            factory,
            tokenA,
            tokenB
        );
        if (reserveA == 0 && reserveB == 0) {
            ...
            uint amountBOptimal = SquadswapLibrary.quote(
                amountADesired,
                reserveA,
                reserveB
            );
            if (amountBOptimal <= amountBDesired) {
                require(
                    ...
                    "SquadswapRouter02: INSUFFICIENT_B_AMOUNT"
                );
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint amountAOptimal = SquadswapLibrary.quote(
                    amountBDesired,
                    reserveB,
                    reserveA
                );
                ...
                require(
                    ...
                    "SquadswapRouter02: INSUFFICIENT_A_AMOUNT"
                );
        ...
        returns (uint amountToken, uint amountETH, uint liquidity)
    {
        ...
        if (msg.value > amountETH)
            TransferHelper.safeTransferBNB(msg.sender, msg.value - amountETH);
    }
    ...
    function _swap(
        ...
        for (uint i; i < path.length - 1; i++) {
            ...
            address to = i < path.length - 2
                ? SquadswapLibrary.pairFor(factory, output, path[i + 2])
                : _to;
            ISquadswapPair(SquadswapLibrary.pairFor(factory, input, output))
    ...
    function swapExactTokensForTokens(
        ...
        TransferHelper.safeTransferFrom(
            path[0],
            msg.sender,
            SquadswapLibrary.pairFor(factory, path[0], path[1]),
            amounts[0]
        );
    ...
    function swapTokensForExactTokens(
        ...
        require(
            ...
            "SquadswapRouter02: EXCESSIVE_INPUT_AMOUNT"
        );
        TransferHelper.safeTransferFrom(
            path[0],
            msg.sender,
    ...
    function swapTokensForExactETH(
        ...
        require(
            ...
            "SquadswapRouter02: INVALID_PATH"
        );
        amounts = SquadswapLibrary.getAmountsIn(factory, amountOut, path);
        require(
            amounts[0] <= amountInMax,
    ...
    function _swapSupportingFeeOnTransferTokens(
        ...
        for (uint i; i < path.length - 1; i++) {
            (address input, address output) = (path[i], path[i + 1]);
            (address token0, ) = SquadswapLibrary.sortTokens(input, output);
            ISquadswapPair pair = ISquadswapPair(
                SquadswapLibrary.pairFor(factory, input, output)
            );
            uint amountInput;
            uint amountOutput;
            {
                ...
                amountInput =
                    ...
                    reserveInput;
                amountOutput = SquadswapLibrary.getAmountOut(
                    amountInput,
                    reserveInput,
                    reserveOutput
                );
