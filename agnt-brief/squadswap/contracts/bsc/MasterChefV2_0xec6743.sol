...
contract MasterChefV2 is Ownable, ReentrancyGuard {
    ...
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        poolInfo[_pid] = pool;
        if (poolInfo[_pid].isRegular && boostContract != address(0)) {
            if (_amount > 0) {
                IFarmBooster(boostContract).autoActivatePool(msg.sender, _pid);
            }
            IFarmBooster(boostContract).updateBoostMultiplier(msg.sender, _pid);
        }
