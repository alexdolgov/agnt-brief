...
contract TickLens is ITickLens {
    ...
    {
        // fetch bitmap
        uint256 bitmap = ISquadV3Pool(pool).tickBitmap(tickBitmapIndex);
        ...
        int24 tickSpacing = ISquadV3Pool(pool).tickSpacing();
        populatedTicks = new PopulatedTick[](numberOfPopulatedTicks);
        for (uint256 i = 0; i < 256; i++) {
            if (bitmap & (1 << i) > 0) {
                int24 populatedTick = ((int24(tickBitmapIndex) << 8) + int24(i)) * tickSpacing;
                (uint128 liquidityGross, int128 liquidityNet, , , , , , ) = ISquadV3Pool(pool).ticks(populatedTick);
