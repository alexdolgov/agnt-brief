...
contract CrocQuery {
    ...
    function liquidityToTokens (uint128 curvePrice, uint128 liq)
       ...
       /* @notice Used to check if a proxy contract is installed.
     * @param proxyIdx The index of the proxy contract to check. */
    function checkProxy(uint16 proxyIdx) private view returns (address) {
        // For an array of addresses, each element is 20 bytes (address size)
        // Calculate the slot by adding the index to the array's base slot (1)
        bytes32 slot = bytes32(uint256(1) + proxyIdx);
        
        // Read the slot and convert to address
        uint256 value = CrocSwapDex(dex_).readSlot(uint256(slot));
