...
contract LizardStrategyUsdtStgV5 is Initializable {
    ...
    function withdraw(uint256 _amountUsdt) public nonReentrant {
        ...
        uint256 realAmountUsdt = Math.min(finalAmountUsdt, IERC20(usdt).balanceOf(address(this)));
        IERC20Burnable(lizardUsdt).burn(msg.sender, _amountUsdt*realAmountUsdt/finalAmountUsdt); // burn after read totalSupply

        IERC20(usdt).transfer(
            msg.sender,realAmountUsdt
        );
