...
contract IdleCDOEpochVariant is IdleCDO {
  ...
  /// so here we burn tranche tokens, the equivalent amount of strategy tokens.
  /// @param _amount Amount of tranche tokens to write off
  /// @param _isAA True if the tranche is AA, false if BB
  function writeOffDeposit(uint256 _amount, bool _isAA) external {
    // TODO maybe we can whitelist who can do the writeoff??
    // or maybe only the borrower can do it?
    if (!isEpochRunning || !isWalletAllowed(msg.sender)) {
      revert NotAllowed();
    }

    _skimDonatedAssets();
    // we don't need to call updateAccounting because only moment when there are gains/losses is at stop epoch
    
    // Burn tranche tokens and decrease NAV
    address _tranche = _isAA ? AATranche : BBTranche;

    // TODO this can be simplified 
    // _amount = _amount == 0 ? IERC20Detailed(_tranche).balanceOf(msg.sender) : _amount;
    uint256 _underlyings = _amount * _tranchePrice(_tranche) / ONE_TRANCHE_TOKEN;

    // TODO what about the updateSplitRatio here if we also consider the interest?
    _withdrawOps(_amount, _underlyings, _tranche);

    // Interest is not considered in the write off, so borrower will still need to pay for interest
    // that the written off tranche would have generated in the epoch (if the epoch already started)
    // and send this interest in the contract at the end of the epoch
