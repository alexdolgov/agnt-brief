...
import "../CrossChainMsg/AnzenMsgSenderAppUpg.sol";
...
contract AnzenVotingControllerUpg is AnzenMsgSenderAppUpg, VotingControllerStorageUpg, UUPSUpgradeable {
    ...
    constructor(
        address _veAnzen,
        address _anzenMsgSendEndpoint,
        uint256 initialApproxDestinationGas
    )
        VotingControllerStorageUpg(_veAnzen)
        AnzenMsgSenderAppUpg(_anzenMsgSendEndpoint, initialApproxDestinationGas) // constructor only set immutable variables
        initializer
    ...
    function vote(address[] calldata pools, uint64[] calldata weights) external {
        ...
        if (user != owner && veAnzen.balanceOf(user) == 0) revert Errors.VCZeroVeAnzen(user);

        LockedPosition memory userPosition = _getUserVeAnzenPosition(user);
    ...
    function broadcastResults(uint64 chainId) external payable refundUnusedEth {
        ...
        _broadcastResults(chainId, wTime, anzenPerSec);
    }
     ...
     */
    function addPool(uint64 chainId, address pool, uint64 maxWeight, uint64 minWeight) external onlyHelperAndOwner {
        ...
        _updatePoolWeightConfig(pool, maxWeight, minWeight);
    ...
    /**
     * @notice config max and min weight vote for pool. Can only be done by [governance/owner helper]
     * @custom:gov
     * - weight is in `USER_VOTE_MAX_WEIGHT` (1e18) based
     */
    function updatePoolWeightConfig(address pool, uint64 maxWeight, uint64 minWeight) external onlyHelperAndOwner {
        _updatePoolWeightConfig(pool, maxWeight, minWeight);
    }
    ...
    function forceBroadcastResults(
        uint64 chainId,
        uint128 wTime,
        uint128 forcedAnzenPerSec
    ) external payable onlyOwner refundUnusedEth {
        _broadcastResults(chainId, wTime, forcedAnzenPerSec);
    }

    /**
     * @notice sets new anzenPerSec
     ...
     * race condition
     */
    function setAnzenPerSec(uint128 newAnzenPerSec) external onlyOwner {
        anzenPerSec = newAnzenPerSec;
        emit SetAnzenPerSec(newAnzenPerSec);
    }
    ...
    function getBroadcastResultFee(uint64 chainId) external view returns (uint256) {
        ...
        uint256[] memory totalAnzenAmounts = new uint256[](length);

        return _getSendMessageFee(chainId, abi.encode(uint128(0), pools, totalAnzenAmounts));
    }
    ...
    function _broadcastResults(uint64 chainId, uint128 wTime, uint128 totalAnzenPerSec) internal {
        ...
        for (uint256 i = 0; i < length; ++i) {
            ...
            totalAnzenAmounts[i] = (totalAnzenPerSec * poolVotes * WEEK) / totalVotes;
        }

        if (chainId == block.chainid) {
            address gaugeController = destinationContracts.get(chainId);
            IAGaugeControllerMainchain(gaugeController).updateVotingResults(wTime, pools, totalAnzenAmounts);
        } else {
            _sendMessage(chainId, abi.encode(wTime, pools, totalAnzenAmounts));
        }

        emit BroadcastResults(chainId, wTime, totalAnzenPerSec);
    }

    function _getUserVeAnzenPosition(address user) internal view returns (LockedPosition memory userPosition) {
        if (user == owner) {
            (userPosition.amount, userPosition.expiry) = (
                GOVERNANCE_ANZEN_VOTE,
                WeekMath.getWeekStartTimestamp(uint128(block.timestamp) + MAX_LOCK_TIME)
            );
        } else {
            (userPosition.amount, userPosition.expiry) = veAnzen.positionData(user);
        }
    }
