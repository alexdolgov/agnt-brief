...
import "../../interfaces/IAVeToken.sol";
...
abstract contract VotingControllerStorageUpg is IAVotingController {
    ...
    uint128 public constant GOVERNANCE_ANZEN_VOTE = 10 * (10 ** 6) * (10 ** 18); // 10 mils of ANZEN

    IAVeToken public immutable veAnzen;

    uint128 public deployedWTime;

    uint128 public anzenPerSec;
    ...
    constructor(address _veAnzen) {
        veAnzen = IAVeToken(_veAnzen);
    }
    ...
    function getPoolWeightConfigData(
        address pool
    )
        public
        view
        returns (uint64 maxWeight, uint64 minWeight)
    {
        PoolData storage data = poolData[pool];
        (maxWeight, minWeight) = (data.maxWeight, data.minWeight);
    }
    ...
    function getAllActivePoolsWeightConfig() external view returns (WeightPoolConfig[] memory allPools) {
        address[] memory allPoolAddresses = allActivePools.values();
        allPools = new WeightPoolConfig[](allPoolAddresses.length);
        for (uint i=0; i<allPools.length; i++) {
            address poolAddress = allPoolAddresses[i];
            allPools[i] = WeightPoolConfig(poolAddress, poolData[poolAddress].maxWeight, poolData[poolAddress].minWeight);
        }
        return allPools;
    }
    ...
    function _updatePoolWeightConfig(address pool, uint64 maxWeight, uint64 minWeight) internal {
        if (maxWeight > VeBalanceLib.USER_VOTE_MAX_WEIGHT) revert Errors.VCInvalidWeightConfig();
        if (minWeight > VeBalanceLib.USER_VOTE_MAX_WEIGHT) revert Errors.VCInvalidWeightConfig();
        if (minWeight > maxWeight) revert Errors.VCInvalidWeightConfig();

        poolData[pool].maxWeight = maxWeight;
        poolData[pool].minWeight = minWeight;
        emit PoolWeightConfig(pool, maxWeight, minWeight);
    }
    ...
    function _modifyVoteWeight(
        ...
        // CHECK MAX/MIN VOTE CONFIG
        if (weight > poolData[pool].maxWeight) revert Errors.VCExceededPoolMaxWeight(pool, weight, poolData[pool].maxWeight);
        if (weight < poolData[pool].minWeight) revert Errors.VCBellowPoolMinWeight(pool, weight, poolData[pool].minWeight);
