...
// BananaMakerKashi is MasterApeV2's left hand and kinda a wizard. He can cook up Banana from pretty much anything!
// This contract handles "serving up" rewards for xBanana holders by trading tokens collected from Kashi fees for Banana.
contract BananaMakerKashi is Ownable {
    ...
    function _convertStep(address token0, uint256 amount0) private returns (uint256 bananaOut) {
        if (token0 == banana) {
            IERC20(token0).safeTransfer(bar, amount0);
            bananaOut = amount0;
        } else if (token0 == weth) {
            bananaOut = _swap(token0, banana, amount0, bar);
        } else {
            ...
            bananaOut = _convertStep(bridge, amountOut);
        }
    }
