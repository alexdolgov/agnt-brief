...
contract MasterApeV2 is Ownable {
        ...
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 bananaReward =
                multiplier.mul(bananaPerBlock).mul(pool.allocPoint).div(
                    totalAllocPoint
                );
            accBananaPerShare = accBananaPerShare.add(
                bananaReward.mul(1e12).div(lpSupply)
            );
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        banana.mint(devaddr, bananaReward.div(10));
        banana.mint(address(this), bananaReward);
        pool.accBananaPerShare = pool.accBananaPerShare.add(
            bananaReward.mul(1e12).div(lpSupply)
        );
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            uint256 pending =
                user.amount.mul(pool.accBananaPerShare).div(1e12).sub(
                    user.rewardDebt
                );
