...
import {Math} from "../uniswapv2/libraries/Math.sol";

/*
 * @author Adjusted PancakeSwap Zap contract for Alien Base.
 *
 */
contract AlienbaseZapV1 is Ownable, ReentrancyGuard {
    ...
    // Router interface
    IUniswapV2Router02 public alienbaseRouter;
    ...
    // Address Router
    address private alienbaseRouterAddress;
     ...
     * @param _alienbaseRouter: address of the Router
    ...
    constructor(
        ...
        alienbaseRouterAddress = _alienbaseRouter;
        alienbaseRouter = IUniswapV2Router02(_alienbaseRouter);
        ...
        )
    {
        ...
        if (token0 == _tokenToZap) {
            ...
            swapAmountOut = alienbaseRouter.getAmountOut(swapAmountIn, reserveA, reserveB);
        } else {
            swapTokenOut = token0;
            swapAmountIn = _calculateAmountToSwap(_tokenAmountIn, reserveB, reserveA);
            swapAmountOut = alienbaseRouter.getAmountOut(swapAmountIn, reserveB, reserveA);
        }
    ...
    function _zapIn(
        ...
        uint256[] memory swappedAmounts = alienbaseRouter.swapExactTokensForTokens(
            swapAmountIn,
            _tokenAmountOutMin,
            path,
            address(this),
        ...
        (, , lpTokenReceived) = alienbaseRouter.addLiquidity(
            path[0],
            path[1],
    ...
    function _zapOut(
        ...
        require(amount0 >= MINIMUM_AMOUNT, "AlienbaseRouter: INSUFFICIENT_A_AMOUNT");
        require(amount1 >= MINIMUM_AMOUNT, "AlienbaseRouter: INSUFFICIENT_B_AMOUNT");
        ...
        // Swap tokens
        alienbaseRouter.swapExactTokensForTokens(swapAmountIn, _tokenAmountOutMin, path, address(this), block.timestamp);
    ...
    function _approveTokenIfNeeded(address _token, uint256 _swapAmountIn) private {
        if (IERC20(_token).allowance(address(this), alienbaseRouterAddress) < _swapAmountIn) {
            // Reset to 0
            IERC20(_token).safeApprove(alienbaseRouterAddress, 0);
            // Re-approve
            IERC20(_token).safeApprove(alienbaseRouterAddress, MAX_INT);
        }
    }
    ...
    function _calculateAmountToSwap(
        ...
        uint256 nominator = alienbaseRouter.getAmountOut(halfToken0Amount, _reserve0, _reserve1);
        uint256 denominator = alienbaseRouter.quote(
            halfToken0Amount,
    ...
    function _calculateAmountToSwapForRebalancing(
        ...
        if (sellToken0) {
            ...
            uint256 nominator = alienbaseRouter.getAmountOut(token0AmountToSell, _reserve0, _reserve1);
            uint256 denominator = alienbaseRouter.quote(
                token0AmountToSell,
            ...
            uint256 nominator = alienbaseRouter.getAmountOut(token1AmountToSell, _reserve1, _reserve0);
