...
import {DelegatedOps} from "./utils/DelegatedOps.sol";

/**
    @title Fee Distributor
    @notice `feeTokens` can be deposited into this contract, to be distributed to vote lockers,
            according to their "lock weight", as recorded in the `TokenLocker` contract.
    @dev This would be equivalent to Prisma's FeeDistributor.sol (https://etherscan.io/address/0x62253F7c165e34fd7343b37839bf5186a9E21D4a)
         except for:
            - bumping the Solidity version from 0.8.19 to 0.8.20
            - using OZ's Ownable.sol instead of Prisma's PrismaOwnable.sol
            - using Owner instead of _prismaCore
            - passing in the startTime in the constructor using tokenLocker instead of calling IPrismaCore(prismaCore).startTime()
 */
contract FeeDistributor is Ownable, SystemStart, DelegatedOps {
    ...
    mapping(address token => FeeTokenData) public feeTokenData;
    ...
    constructor(address _owner, ITokenLocker _tokenLocker) Ownable(_owner) SystemStart(_tokenLocker.startTime()) {
