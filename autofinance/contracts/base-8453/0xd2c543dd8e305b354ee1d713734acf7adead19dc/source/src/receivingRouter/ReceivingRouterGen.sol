...
/// @title Receives and routes messages from other senders
contract ReceivingRouterGen is SystemComponent, SecurityBase {
    /// =====================================================
    /// Public vars
    /// =====================================================
    ...
    /// @notice uint64 sourceChainSelector => address[] senders, contract on this chain that forwards data for other
    /// chain
    ...
    event MessageData(
        uint256 messageNonce, address messageOrigin, bytes32 messageType, uint64 sourceChainSelector, bytes message
    );
    ...
    function receiveMessage(uint64 sourceChainSelector, CCUtils.Message memory extMessage) external {
        if (
            !(
                sourceChainSenders[sourceChainSelector][0] == msg.sender
                    || sourceChainSenders[sourceChainSelector][1] == msg.sender
            )
        ) {
            revert Errors.AccessDenied();
        }

        // Intentionally not checking message version

        address origin = extMessage.messageOrigin;
        bytes32 messageType = extMessage.messageType;
        uint256 messageNonce = extMessage.messageNonce;
        bytes memory message = extMessage.message;
        ...
        uint256 sourceVersion = extMessage.version;
        ...
        emit MessageData(messageNonce, origin, messageType, sourceChainSelector, message);
