...
contract Tranche is ERC4626Upgradeable {
    ...
    * @param _owner The address of the owner whose shares are to be migrated.
     */
    function migrate(address _owner) external onlyKeeper{
        ...
        SafeERC20.safeApprove(ERC20(asset()), address(vaultManager.junior()) , assets);
        ITranche(address(vaultManager.junior())).deposit(assets, _owner);
        _burn(_owner, totalShares);
        emit TokenTransferred(_owner, address(0), totalShares, assets);

        emit Migrated(_owner, address(vaultManager.junior()), totalShares, assets);
    ...
    * @param owners The address of the owners whose shares are to be migrated.
     */
    function migrateOnBehalf(address[] calldata owners) external onlyKeeper {
