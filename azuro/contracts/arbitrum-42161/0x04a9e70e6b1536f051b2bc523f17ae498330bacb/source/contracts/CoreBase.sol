...
abstract contract CoreBase is OwnableUpgradeable, ICoreBase {
    ...
    /**
     * @notice Throw in any case.
     */
    modifier forbidden() {
        revert ActionIsForbidden();
        _;
    }
     ...
     */
    function cancelCondition(uint256 conditionId) external forbidden {
     ...
     */
    function stopCondition(uint256 conditionId, bool flag) external forbidden {
        Condition storage condition = _getCondition(conditionId);
    ...
    function resolveAffiliateReward(
        address affiliate,
        bytes calldata data
    ) external virtual override onlyLp forbidden returns (uint256 reward) {
     ...
     */
    function getContributedConditionsCount(
        address affiliate
    ...
    function _resolveCondition(
        ...
        // uint128 affiliatesReward = lp.addReserve(lp.addReserve(
        //       condition.gameId,
        //       lockedReserve,
        //       profitReserve,
        //       condition.leaf
        // );
        //
        // The change is related to the fact that in the future version, lp.addReserve will not return affiliate profit
    ...
    function _resolvePayout(
        uint256 tokenId
    ) internal returns (address, uint128) {
