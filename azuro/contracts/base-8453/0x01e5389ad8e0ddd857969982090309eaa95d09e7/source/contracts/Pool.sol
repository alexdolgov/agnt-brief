...
contract Pool is OwnableUpgradeable, IPool {
    ...
    /**
     * @notice PoolManager: Emits an event with new index data for the specified index.
     * @param indexId The ID of the index being updated.
     * @param data An array of arbitrary binary data associated with the index.
     */
    function updateIndexData(uint256 indexId, bytes calldata data) external onlyPoolManager {
    ...
    function validateTrade(
        ...
        if (!allowedTraders[trader]) revert TraderNotAllowed(trader);
        ReserveData storage reserve = reserves[collateralToken];
