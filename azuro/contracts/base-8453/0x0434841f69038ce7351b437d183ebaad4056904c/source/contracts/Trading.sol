...
contract Trading is OwnableUpgradeable, ITrading {
    ...
    function initialize(
        ...
        tradingFeeRate_.requireLessThanOne();
    ...
    function changeMarketImbalanceFeeSettings(
        ...
        (newMaxMarketImbalanceFeeRate + tradingFeeRate).requireLessThanOne();
     ...
     */
    function changeMaxPayoutThreshold(uint64 newMaxPayoutThreshold) external onlyOwner {
    ...
    /**
     * @notice Owner: Updates the pool contract address.
     * @dev Reverts if the new address is the same as the current one.
     * @param newPool The new pool contract address to be set.
     */
    function changePool(IPool newPool) external onlyOwner {
        if (newPool == pool) revert NoChanges();
        pool = newPool;
     ...
     */
    function changeTradingFeeRate(uint64 newTradingFeeRate) external onlyOwner {
        if (newTradingFeeRate == tradingFeeRate) revert NoChanges();
        (newTradingFeeRate + maxMarketImbalanceFeeRate).requireLessThanOne();
    ...
    function placeMarketOrder(
        ...
        placeLimitOrder(collateralToken, collateralAmount, leverage, isLong, indexPrice, indexId);
    }
    ...
    function placeLimitOrder(
        ...
        pool.validateOrder(indexId, collateralToken, collateralAmount);
        ...
        _tryExecuteOrder(tradeId, trade);
    }
    ...
    function cancelOrder(uint256 tradeId) external {
        ...
        if (msg.sender != trade.trader) revert OnlyTradeOwner(trade.trader);
    ...
    function processTrades(uint256[] calldata tradeIds, address feeReceiver) external {
        for (uint256 i; i < tradeIds.length; ++i) {
            ...
            if (trade.status == TradeStatus.ORDER) _tryExecuteOrder(tradeId, trade);
     ...
     */
    function _getMarketImbalanceFeeRate(Trade storage trade) internal view returns (uint256) {
        if (maxMarketImbalanceFeeRate == 0) return 0;

        Market storage market = _getMarket(trade);
        if (market.longIndex == 0 || market.shortIndex == 0) return 0;

        uint256 marketImbalance = trade.isLong
            ? (market.longIndex).div(market.shortIndex)
            : (market.shortIndex).div(market.longIndex);
    ...
    /**
     * @notice Attempts to convert an open order into a position if index conditions are met.
     * @param tradeId The unique identifier of the trade.
     * @param trade The trade storage reference.
     * @return success True if the order was successfully executed.
     */
    function _tryExecuteOrder(
        uint256 tradeId,
        Trade storage trade
    ) internal updateFunding(trade) returns (bool) {
        if (!_evaluateOrderExecution(trade)) return false;
        ...
        trade.entryFundingRate = trade.isLong
            ? market.cumulativeLongFundingFee
            : market.cumulativeShortFundingFee;

        if (trade.isLong) {
            market.longTokens += leveragedAmount;
            market.longIndex += _getPositionSize(trade);
        } else {
            market.shortTokens += leveragedAmount;
            market.shortIndex += _getPositionSize(trade);
        }

        uint256 tradingFee = leveragedAmount.mul(tradingFeeRate);
        uint256 marketImbalanceFee = leveragedAmount.mul(_getMarketImbalanceFeeRate(trade));
    ...
    function _tryClosePosition(
        ...
        Market storage market = _getMarket(trade);
        if (trade.isLong) {
            market.longTokens -= trade.leveragedAmount;
            market.longIndex -= _getPositionSize(trade);
        } else {
            market.shortTokens -= trade.leveragedAmount;
            market.shortIndex -= _getPositionSize(trade);
        }
    ...
    function _accrueFundingFee(Market storage market, address collateralToken) internal {
        ...
        if (timeDelta == 0 || market.shortTokens == 0 || market.shortTokens == 0) return;

        int256 accumulatedFundingFee = market.fundingRate * timeDelta;
        market.cumulativeLongFundingFee += accumulatedFundingFee;
        market.cumulativeShortFundingFee -= accumulatedFundingFee;

        if (accumulatedFundingFee == 0) return;
        uint256 extraFunding = accumulatedFundingFee > 0
            ? (market.longTokens - market.shortTokens).mul(uint256(accumulatedFundingFee))
            : (market.shortTokens - market.longTokens).mul(uint256(-accumulatedFundingFee));
    ...
    function _updateFundingRate(Trade storage trade, Market storage market) internal {
        ...
        if (market.longIndex == 0 || market.shortIndex == 0) {
            market.fundingRate = 0;
        } else {
            if (market.longIndex > market.shortIndex) {
                market.fundingRate = int256(
                    ((market.longIndex - market.shortIndex) * baseFundingRate) / market.longIndex
                );
            } else {
                market.fundingRate = -int256(
                    ((market.shortIndex - market.longIndex) * baseFundingRate) / market.shortIndex
                );
            }
        }
        ...
        returns (uint256 payout, uint256 indexPrice, int256 fundingPnl, PositionCloseStatus status)
    {
        ...
        int256 remainingCollateral = int256(trade.collateralAmount) + fundingPnl;
        if (
            remainingCollateral <= 0 ||
            int256(payout) <= liquidationThreshold.mul(remainingCollateral)
        ) {
    ...
    function _getFundingPnL(Trade storage trade) internal view returns (int256) {
        ...
        (int256 cumulativeLongFundingFee, int256 cumulativeShortFundingFee) = (
            market.cumulativeLongFundingFee,
            market.cumulativeShortFundingFee
        );
        ...
        // *** For External Calls ***
        int256 timeDelta = int256(block.timestamp - market.fundingFeeAccruedAt);
        if (timeDelta > 0 && market.longIndex > 0 && market.shortIndex > 0) {
            int256 accumulatedFundingFee = market.fundingRate * timeDelta;
            cumulativeLongFundingFee += accumulatedFundingFee;
            cumulativeShortFundingFee -= accumulatedFundingFee;
        }
        // *** For External Calls ***

        int256 fundingFeeDelta = trade.entryFundingRate -
            (trade.isLong ? cumulativeLongFundingFee : cumulativeShortFundingFee);
    ...
    function _getOrder(uint256 tradeId) internal view returns (Trade storage trade) {
        ...
        if (trade.status != TradeStatus.ORDER) revert OrderAlreadyExecuted(tradeId);
    }
    ...
    /**
     * @notice Calculates the position size based on leveraged amount and entry price
     */
    function _getPositionSize(Trade storage trade) internal view returns (uint256) {
        return trade.leveragedAmount.div(trade.entryPrice);
    }
