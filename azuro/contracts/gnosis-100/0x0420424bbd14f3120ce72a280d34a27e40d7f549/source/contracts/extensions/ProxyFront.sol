...
contract ProxyFront is IProxyFront {
    ...
    function withdrawPayouts(WithdrawPayoutData[] calldata data) external {
        for (uint256 i = 0; i < data.length; ++i) {
            WithdrawPayoutData memory withdrawPayoutData = data[i];
            ICoreBase core = ICoreBase(withdrawPayoutData.core);
            ILP lp = ICoreBase(core).lp();
            uint256 payout = lp.withdrawPayout(
                withdrawPayoutData.core,
                withdrawPayoutData.tokenId
            );

            if (
                withdrawPayoutData.withdrawalType != WithdrawalType.TOKEN &&
                payout > 0
            ) {
                address account = withdrawPayoutData.withdrawalType ==
                    WithdrawalType.NATIVE_PREMATCH
                    ? core.azuroBet().ownerOf(withdrawPayoutData.tokenId)
                    : IERC721Upgradeable(withdrawPayoutData.core).ownerOf(
                        withdrawPayoutData.tokenId
                    );
