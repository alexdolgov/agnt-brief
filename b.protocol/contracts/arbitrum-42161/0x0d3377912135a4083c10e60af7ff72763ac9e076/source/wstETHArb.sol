...
contract Redeemer {
    ...
    TroveManager constant troveManager = TroveManager(0x15f74458aE0bFdAA1a96CA1aa779D715Cc1Eefe4);
...
interface CurveLike {
    ...
    function exchange(int128 i, int128 j, uint256 dx, uint256 min_dy) external payable returns(uint);
}
...
contract ETHArb is Usdc2Gem {
    ...
    function swap(uint ethQty, address redeemer, uint minProfit, address wallet) external payable returns(uint) {
        WethLike(WETH).deposit{value: uint(msg.value)}();
        ...
        require(retVal >= minProfit, "min profit");
        ERC20Like(WETH).transfer(wallet, retVal);
...
contract wstETHArb is Usdc2Gem {
    address constant WETH = 0x82aF49447D8a07e3bd95BD0d56f35241523fBab1;
    address constant WSTWETH = 0x5979D7b546E38E414F7E9822514be443A4800529;
    CurveLike constant ETH_WSTETH = CurveLike(0x6eB2dc694eB516B16Dc9FBc678C60052BbdD7d80);
    UniswapReserve constant USDCETH = UniswapReserve(0xC31E54c7a869B9FcBEcc14363CF510d1c41fa443);
    uint160 constant MAX_SQRT_RATIO = 1461446703485210103287273052203988822378723970342;

    constructor() public Usdc2Gem() {
        ERC20Like(WETH).approve(address(USDCETH), uint(-1));
        ERC20Like(WSTWETH).approve(address(ETH_WSTETH), uint(-1));
    }
    ...
    function uniswapV3SwapCallback(
        ...
        // swap USDC to FRAX to VST to wstETH
        usdc2GemRedeemer(USDCAmount, WSTWETH, Redeemer(redeemer));
        uint wstETHQty = ERC20Like(WSTWETH).balanceOf(address(this));

        uint retVal = ETH_WSTETH.exchange(1, 0, wstETHQty, 1);
        WethLike(WETH).deposit{value: uint(retVal)}();
