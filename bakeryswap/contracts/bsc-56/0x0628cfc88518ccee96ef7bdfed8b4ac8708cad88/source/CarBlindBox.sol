...
contract CAR is ERC20Pausable, Ownable {
    constructor(
        string memory name,
...
// File: contracts/car/CarBlindBox.sol
...
contract CarBlindBox is ERC721Holder, ERC721Pausable, Ownable {
    using Address for address;

    uint256 public nextTokenId = 1;
    uint256 private constant ONE_POWER_EIGHTEEN = 1E18;
    CAR public immutable car;
    IERC721Enumerable public immutable carNFT;
    string private _tokenURI;
    event Open(address indexed sender, uint256 tokenId, uint256 carNFTId);
    event Mint(address indexed user, uint256[] tokenIds, uint256 amount);
    ...
    constructor(
        ...
        _tokenURI = tokenURI_;
        car = CAR(_car);
        carNFT = IERC721Enumerable(_carNFT);
    }
    ...
    function mint(uint256 _amount) public returns (uint256[] memory tokenIds) {
        require(_amount >= ONE_POWER_EIGHTEEN, ' amount too small');
        uint256 size = _amount.div(ONE_POWER_EIGHTEEN);
        _amount = size.mul(ONE_POWER_EIGHTEEN);
        car.burnFrom(_msgSender(), _amount);
        tokenIds = new uint256[](size);
        if (size == 1) {
            tokenIds[0] = __mint(_msgSender());
        } else {
            for (uint256 i = 0; i < size; ++i) {
                tokenIds[i] = __mint(_msgSender());
            }
        }
        emit Mint(msg.sender, tokenIds, _amount);
    }

    function open(uint256 tokenId) public {
        require(!address(msg.sender).isContract(), 'only user address can open');
        require(_isApprovedOrOwner(_msgSender(), tokenId), 'caller is not owner nor approved');
        _burn(tokenId);
        uint256 carNFTId =
            carNFT.tokenOfOwnerByIndex(
                address(this),
                RandomGenUtils.randomGen(tokenId, carNFT.balanceOf(address(this)))
            );
        carNFT.safeTransferFrom(address(this), _msgSender(), carNFTId);
        emit Open(_msgSender(), tokenId, carNFTId);
    }
    ...
    function setDefaultTokenURI(string memory tokenURI_) public onlyOwner {
        _tokenURI = tokenURI_;
    }
    ...
    function transferOtherERC20Token(address tokenAddress, uint256 amount) public onlyOwner {
        require(tokenAddress != address(carNFT), 'wrong token address');
        IERC20(tokenAddress).transfer(owner(), amount);
    }
