...
contract BaseMaxChef is ITokenFarm, Constants, Initializable, ReentrancyGuardUpgradeable {
    ...
    bool public bootstrapPhase;
    uint256 public endTimeBootstrapPhase;
    // Info of each pool
    PoolInfo public bsmPoolInfo;
    PoolInfo public blpPoolInfo;
    PoolInfo public lpPoolInfo;
    ...
    mapping(address => LpUserInfo) public lpUserInfo;
    ...
    function setEndTimeBootstrapPhase(uint256 _endTimeBootstrapPhase) external onlyOperator(1) {
        endTimeBootstrapPhase = _endTimeBootstrapPhase;
    }

    function enableBootstrapPhase() external onlyOperator(1) {
    ...
    function setLPToken(IBoringERC20 _LP) external onlyOperator(1) {
    ...
    function setLpPool(IComplexRewarder[] calldata _rewarders, bool _enableCooldown) external onlyOperator(1) {
        require(_rewarders.length <= 10, "set: too many rewarders");
        ...
        lpPoolInfo.rewarders = _rewarders;
        lpPoolInfo.enableCooldown = _enableCooldown;

        emit Set(LP, _rewarders);
    }
    ...
    function depositBsmForVesting(uint256 _amount) external nonReentrant {
        require(!bootstrapPhase , "!bootstrapPhase");
    ...
    function getStakedLp(address _account) external view returns (uint256, uint256) {
        LpUserInfo memory user = lpUserInfo[_account];
        return (user.lpAmount, user.startTimestamp);
    }
    ...
    function emergencyWithdrawBlp() external {
        ...
        if (_amount > 0) {
            if (!checkCooldownWhiteList(msg.sender)) {
                if (bootstrapPhase) {
                    require(
                        endTimeBootstrapPhase < block.timestamp,
                        "didn't pass endTimeBootstrapPhase"
                    );
                } else {
                    require(
    ...
    function _depositLp(uint256 _amount) internal {
        uint256 _pid = 2;
        PoolInfo storage pool = lpPoolInfo;
        LpUserInfo storage user = lpUserInfo[msg.sender];
        ...
        for (uint256 rewarderId = 0; rewarderId < pool.rewarders.length; ++rewarderId) {
            pool.rewarders[rewarderId].onBsmReward(_pid, msg.sender, user.lpAmount);
        }

        if (_amount > 0) {
            pool.totalLp += _amount;
        }
        emit FarmDeposit(msg.sender, LP, _amount);
    }

    //withdraw tokens
    function withdrawLp(uint256 _amount) external nonReentrant {
        uint256 _pid = 2;
        PoolInfo storage pool = lpPoolInfo;
        LpUserInfo storage user = lpUserInfo[msg.sender];

        //this will make sure that user can only withdraw from his pool
        require(user.lpAmount >= _amount, "withdraw: user amount not enough");
        ...
        emit FarmWithdraw(msg.sender, LP, _amount);
    }
    ...
    function emergencyWithdrawLp() external nonReentrant {
        ...
        if (_lpAmount > 0) {
            ...
            LP.safeTransfer(msg.sender, _lpAmount);
            pool.totalLp -= _lpAmount;
            user.lpAmount = 0;
            emit EmergencyWithdraw(msg.sender, LP, _lpAmount);
        }
    }

    // View function to see rewarders for a pool
    function poolRewarders(bool _isBsmPool, bool _isBlpPool, bool _isLpPool) external view returns (address[] memory rewarders) {
    ...
    function harvestMany(bool _bsm, bool _esbsm, bool _blp, bool _lp) public nonReentrant {
        if (_bsm) {
            _depositBsm(0);
        }
        if (_esbsm) {
