...
// File: contracts/utils/BeltDepositor.sol
...
contract BeltBELTDepositorStorage {
    address public BELTAddress;
    address public beltBELTAddress;
    
    uint256 public lockUpPeriod;
    mapping(address => uint256) userDepositTime;
}

contract CCCC is Initializable, OwnableUpgradeable, BeltBELTDepositorStorage {

    using SafeERC20 for IERC20;
    using SafeMath for uint256;

    modifier updateDepositTime() {
        userDepositTime[msg.sender] = now;
        _;
    }

    modifier onlyLockUpPeriodSurpassed() {
        require(userDepositTime[msg.sender].add(lockUpPeriod) <= now);
        _;
    }


    function __BeltBELTDepositor_init(
        address _BELTAddress,
        address _beltBELTAddress
    ) public initializer {
        __Ownable_init();
        __BeltBELTDepositor_init_unchained(_BELTAddress, _beltBELTAddress);
    }

    function __BeltBELTDepositor_init_unchained(
        address _BELTAddress,
        address _beltBELTAddress
    ) internal initializer {
        BELTAddress = _BELTAddress;
        beltBELTAddress = _beltBELTAddress;
        IERC20(BELTAddress).safeApprove(beltBELTAddress, uint256(-1));
        lockUpPeriod = 1 weeks;
    }


    function deposit(uint256 _amount, uint256 _minShares) external updateDepositTime {
        IERC20(BELTAddress).safeTransferFrom(msg.sender, address(this), _amount);
        ISingleStrategyToken2(beltBELTAddress).deposit(
            IERC20(BELTAddress).balanceOf(address(this)),
            _minShares
        );
        IERC20(beltBELTAddress).safeTransfer(
            msg.sender,
            IERC20(beltBELTAddress).balanceOf(address(this))
        );
    }

    function withdraw(uint256 _shares, uint256 _minAmount) external onlyLockUpPeriodSurpassed {
        IERC20(beltBELTAddress).safeTransferFrom(msg.sender, address(this), _shares);
        ISingleStrategyToken2(beltBELTAddress).withdraw(_shares, _minAmount);
        IERC20(BELTAddress).safeTransfer(msg.sender, IERC20(BELTAddress).balanceOf(address(this)));
    }
    ...
    function getUserLockUpEndTime(address userAddr) external view returns (uint256){
        return userDepositTime[userAddr].add(lockUpPeriod);
    }
