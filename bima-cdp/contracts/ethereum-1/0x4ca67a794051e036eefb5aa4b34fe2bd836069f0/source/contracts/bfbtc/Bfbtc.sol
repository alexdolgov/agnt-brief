...
// BitFi Bitcoin (bfBTC)
// Copyright (C) 2024-2026 BitFi Labs
// Contract Version: 3.1.1-Gerneral
contract Bfbtc is Initializable, UUPSUpgradeable, OwnableUpgradeable, PausableUpgradeable, OFTUpgradeable {
    ...
    function _setUnderlyingAsset(IERC20Metadata _underlyingAsset, AggregatorV3Interface _oracle) internal {
        if (address(_underlyingAsset) == address(0)) revert ZeroAddress();
    ...
    function _updateUnderlyingAsset(IERC20Metadata _underlyingAsset, AggregatorV3Interface _oracle) internal {
        IERC20 oldAsset = underlyingAsset;
        if (address(oldAsset) == address(0) || address(oldAsset) == address(_underlyingAsset)) revert InvalidData();

        uint256 oldBalance = oldAsset.balanceOf(address(this));
        if (oldBalance > 0) {
            oldAsset.safeTransfer(owner(), oldBalance);
        }

        _setUnderlyingAsset(_underlyingAsset, _oracle);
    }

    function updateWithConfig(
        IERC20Metadata _underlyingAsset,
        AggregatorV3Interface _oracle,
        uint256 _evmWithdrawFixedFee,
        uint256 _minDepositTokenAmount
    ) external onlyOwner {
        _updateUnderlyingAsset(_underlyingAsset, _oracle);
        _setFeeConfig(FEE_TYPE_EVM, 0, _evmWithdrawFixedFee);
        minDepositTokenAmount = _minDepositTokenAmount;
    }
    ...
    function setBlacklisted(address account, bool isBlacklisted_) external onlyOwner {
    ...
    function depositFor(uint256 amount, uint256 minAmount, address user) external {
        _deposit(amount, minAmount, user);
    }

    function deposit(uint256 amount, uint256 minAmount) external {
        _deposit(amount, minAmount, msg.sender);
    }

    function _deposit(uint256 amount, uint256 minAmount, address user) internal returns (uint256 mintAmount) {
        if (amount < minDepositTokenAmount) revert LessThanMinAmount();
        underlyingAsset.safeTransferFrom(user, underlyingWallet, amount);

        mintAmount = previewDeposit(amount);
        if (mintAmount < minAmount) revert MinimumAmountNotMet();

        _mint(user, mintAmount);

        emit Deposit(user, currentEpoch, amount, mintAmount);
    }
    ...
    function approveWithdraw(uint256 epoch, uint256[] calldata withdrawIds) external onlyMultisig {
    ...
    function nativeWithdraw(uint256 epoch, uint256[] calldata withdrawIds, bytes32[] calldata nativeTxs) external onlyMultisig {
        if (epoch >= currentEpoch) revert InvalidEpoch();
    ...
    function updateEpoch(uint256 newRatio) external whenNotPaused onlyMultisig {
        ...
        ratio[epoch] = newRatio;
        underlyingPrice[epoch] = newPrice;
        emit EpochUpdated(epoch, newRatio, newPrice);
    }
