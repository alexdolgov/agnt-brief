...
import "./StakedBITU.sol";
import "./interfaces/IStakedBITUCooldown.sol";
import "./BITUVault.sol";

/**
 * @title BitUStaking
 * @notice The BitUStaking contract allows users to stake BITU tokens and earn a portion of protocol LST and perpetual yield that is allocated
 ...
 * @dev If cooldown duration is set to zero, the BitUStaking behavior changes to follow ERC4626 standard and disables cooldownShares and cooldownAssets methods. If cooldown duration is greater than zero, the ERC4626 withdrawal and redeem functions are disabled, breaking the ERC4626 standard, and enabling the cooldownShares and the cooldownAssets functions.
 */
contract BitUStaking is IStakedBITUCooldown, StakedBITU {

  mapping(address => UserCooldown) public cooldowns;

  BITUVault public vault;
  ...
  /// @notice Constructor for BitUStaking contract.
  ...
  constructor(IERC20 _asset, address initialRewarder, address owner) StakedBITU(_asset, initialRewarder, owner) {
    vault = new BITUVault(address(this), address(_asset));
  ...
  /// @dev unstake can be called after cooldown have been set to 0, to let accounts to be able to claim remaining assets locked at Vault
  ...
  function cooldownAssets(uint256 assets, address owner) external ensureCooldownOn returns (uint256) {
    ...
    _withdraw(_msgSender(), address(vault), owner, assets, shares);
