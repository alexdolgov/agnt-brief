...
contract SymmioSymbolManager is AccessControlEnumerable, Pausable {
	...
	bytes32 public constant SYMBOL_FCGR_MANAGER_ROLE = keccak256("SYMBOL_FCGR_MANAGER_ROLE");
	...
	constructor(
		...
		address symbol_fgcr_manager,
		DailyLimits memory _limits,
		uint256 _minForceCloseGapRatio,
		uint256 _maxForceCloseGapRatio
	) {
		...
		_grantRole(SYMBOL_ADDER_ROLE, symbol_adder);
		_grantRole(SYMBOL_REMOVER_ROLE, symbol_remover);
		_grantRole(SYMBOL_MODIFIER_ROLE, symbol_modifier);
		_grantRole(SYMBOL_FCGR_MANAGER_ROLE, symbol_fgcr_manager);
	...
	/// @param symbolIds Array of symbol IDs to update
	function activateSymbols(uint256[] memory symbolIds) external whenNotPaused onlyRole(SYMBOL_MODIFIER_ROLE) {
	...
	) external whenNotPaused onlyRole(SYMBOL_FCGR_MANAGER_ROLE) {
