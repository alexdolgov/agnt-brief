...
// File contracts/staking/RewardRouterV4.sol
...
contract RewardRouterV4 is ReentrancyGuard, Governable {
    ...
    function _unstakeBmx(address _account, address _token, uint256 _amount, bool _shouldReduceBnGmx) private {
        require(_amount > 0, "RewardRouter: invalid _amount");
        ...
        if (_shouldReduceBnGmx) {
            ...
            uint256 stakedBnGmx = IRewardTracker(feeBmxTracker).depositBalances(_account, bnBmx);
            if (stakedBnGmx > 0) {
