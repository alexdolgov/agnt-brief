...
abstract contract ExitVaultStorage {
    ...
    function getClaimableAmounts(address _user)
        external
        view
        returns (
    ...
    {
        GmxAStorage storage s = _getStorage();
        UserInfo memory user = s.userInfo[_user];

        // Vested GMX with staked GMX and GLP
        uint256 esGmxToVest;
        uint256 claimedGMX;
        esGmxToVest = user.gmxStream.esGmxVest;
        claimedGMX = user.gmxStream.claimedGmx;
        gmx_vestedGmx = esGmxToVest * (block.timestamp - user.gmxStream.lastClaim) / 365 days;
        gmx_vestedGmx = claimedGMX + gmx_vestedGmx >= esGmxToVest ? esGmxToVest - claimedGMX : gmx_vestedGmx;
        gmx_vestedGmx = gmx_vestedGmx * s.donationPartGmx / HUNDRED_PERCENT;

        esGmxToVest = user.glpStream.esGmxVest;
        claimedGMX = user.glpStream.claimedGmx;
        gmx_vestedGlp = esGmxToVest * (block.timestamp - user.glpStream.lastClaim) / 365 days;
        gmx_vestedGlp = claimedGMX + gmx_vestedGlp >= esGmxToVest ? esGmxToVest - claimedGMX : gmx_vestedGlp;
        gmx_vestedGlp = gmx_vestedGlp * s.donationPartGlp / HUNDRED_PERCENT;

        // GMX rewards for staked GMX
        gmx_stakedGmx = user.gmxStream.shares * s.accumulatedGmxGmxPerShare / 1e18 - user.gmxStream.gmxRewardDebt;

        // WETH rewards for staked GMX. Should be mostly 0.
        weth_stakedGmx = user.gmxStream.shares * s.accumulatedGmxWethPerShare / 1e18 - user.gmxStream.wethRewardDebt;

        // WETH rewards for staked GLP.
        weth_stakedGlp = user.glpStream.shares * s.accumulatedGlpWethPerShare / 1e18 - user.glpStream.wethRewardDebt;
    }
