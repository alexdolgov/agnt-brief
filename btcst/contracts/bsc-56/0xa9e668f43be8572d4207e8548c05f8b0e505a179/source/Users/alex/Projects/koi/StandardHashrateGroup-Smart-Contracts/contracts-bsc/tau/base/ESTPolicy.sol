...
interface IRestorativeElasticToken{
    function restorativeRebase(uint256 epoch,int256 supplyDelta) external returns(uint256);
}
...
contract ESTPolicy is UpgradeableBase{
    ...
    function rebase() external onlyOrchestrator whenNotPaused{
        ...
        uint256 supplyAfterRebase = manToken.totalSupply();
        if (supplyDelta<0){
            ...
            // }
            //above was old rebase method
        }else if (supplyDelta>0){
            //mint supplyDelta for period
            if (address(miningChef)!=address(0)){
                uint256 balance = manToken.balanceOf(address(this));
                uint256 more = uint256(supplyDelta);
                if (balance>0){
                    manToken.burn(balance);
                    more = more.add(balance);
                }
                miningChef.mintMoreDelta(more,minRebaseTimeIntervalSec);
                supplyAfterRebase = manToken.totalSupply();
            }
        }

        IRestorativeElasticToken(address(manToken)).restorativeRebase(epoch, supplyDelta);
