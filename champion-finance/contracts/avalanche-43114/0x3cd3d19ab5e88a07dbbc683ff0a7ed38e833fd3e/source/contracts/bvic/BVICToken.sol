...
contract BVICToken is ERC20, Operator {
    ...
    constructor(address _polWallet, address _wbtceAddress, address _router) ERC20("Bitcoin Victory", "BVIC") {
		require(_polWallet != address(0), "!_polWallet");
		require(_wbtceAddress != address(0), "!_wbtceAddress");
		...
		taxTiersTwaps = [0, 8e7, 9e7, 1.1e8]; // twap with WBTC.e
		taxTiersRates = [2000, 1500, 1000, 0];
		...
		address dexPair = IUniswapV2Factory(_dexRouter.factory()).createPair(address(this), _wbtceAddress);
