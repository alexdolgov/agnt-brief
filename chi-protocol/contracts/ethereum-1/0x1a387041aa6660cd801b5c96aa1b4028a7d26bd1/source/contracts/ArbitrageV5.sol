...
import "contracts/interfaces/IReserveHolderV2.sol";
...
import "contracts/interfaces/IArbitrageV5.sol";
...
contract ArbitrageV5 is IArbitrageV5, ReentrancyGuard, Ownable {
  ...
  function executeArbitrageWithReserveSell(
    uint256 maxChiSpotPrice,
    SwapParams[] calldata swapParams
  ) external returns (uint256) {
    for (uint256 i = 0; i < swapParams.length; i++) {
      reserveHolder.swapReserveForEth(swapParams[i].reserveAsset, swapParams[i].amountIn, swapParams[i].minAmountOut);
    }
