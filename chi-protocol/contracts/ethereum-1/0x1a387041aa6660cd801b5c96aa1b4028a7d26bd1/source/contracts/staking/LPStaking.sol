...
contract LPStaking is ILPStaking, OwnableUpgradeable, StakingWithEpochs {
  ...
  function migrateToV2(uint256 amount, address recipient, address stakingManager) external {
    unstake(amount, address(this));
    stakeToken.approve(stakingManager, amount);
    IStakingManager(stakingManager).stake(address(stakeToken), amount, recipient);
  }

  function migrateLpStaking(uint256 amount, address recipient, address stakingManager) external {
    unstake(amount, address(this));
    stakeToken.approve(stakingManager, amount);
    IStakingManager(stakingManager).stake(address(stakeToken), amount, recipient);
  }
