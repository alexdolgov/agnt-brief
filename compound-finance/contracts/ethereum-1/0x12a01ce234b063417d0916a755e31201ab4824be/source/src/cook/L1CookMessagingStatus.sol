...
import {ConfigEvents, PauserEvents} from "./IMessagingStatus.sol";
...
contract L1CookMessagingStatus is AccessControlEnumerableUpgradeable, ConfigEvents, PauserEvents, OAppSenderUpgradeable {
    ...
    function setIsTransferPausedFor(uint32 eid, bool isPaused) external payable onlyRole(MANAGER_ROLE) {
        ...
        // @dev fix gas limit safe one query when we try to fast pause, same as below
        ...
        _lzSend(
            ...
            // @dev msg.sender will be EoA triggered by offchian system, don't bother the refundAddress. same as below
            payable(msg.sender)
        );
