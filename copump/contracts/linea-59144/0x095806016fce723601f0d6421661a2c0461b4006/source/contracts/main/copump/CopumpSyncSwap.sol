...
contract CopumpSyncSwap is ICopump, Ownable2Step {
    ...
    function sellTokenWithReferrer(
        address tokenAddress,
        uint256 amount,
        uint256 minEthExpected,
        address referrerAddress,
    ...
    function _deployLiquidityPool(
        ...
        // address(0) for native currency instead of ERC20
        ...
        inputs[0] = TokenInput(token0, token0 == listedAddress ? INITIAL_SUPPLY : funding, false);
        inputs[1] = TokenInput(token1, token1 == listedAddress ? INITIAL_SUPPLY : funding, false);
        bytes memory data = abi.encode(address(this));
        // router.addLiquidity{ value: funding }(
        //     poolAddress,
        //     inputs,
        //     data,
        //     0,
    ...
    function _isValidSigner(
        ...
        string memory referrerAddressString = referrerAddress.toHexString();
        bytes memory referrerAddressBytes = bytes(referrerAddressString);
        bytes32 signedMessageHash = referrerAddressBytes.toEthSignedMessageHash();
