...
library OTCLib {
    ...
    function otcSwap(
        ...
        if (self.stableCoinWhitelist[otc.tokenGoingOut] && self.stableCoinWhitelist[otc.tokenComingIn]) {
            ...
            uint256 tokenAamount = otc.tokenGoingOutAmount;
            uint256 tokenBamount = otc.tokenComingInAmount;

            uint256 tokenAdecimals = IERC20Metadata(otc.tokenGoingOut).decimals();
            uint256 tokenBdecimals = IERC20Metadata(otc.tokenComingIn).decimals();

            if (tokenAdecimals != tokenBdecimals) {
                // if decimals are not the same, we need to pad with zeros
                if (tokenAdecimals > tokenBdecimals) {
                    // Pad tokenB amount with zeros
                    tokenBamount = tokenBamount * (10 ** (tokenAdecimals - tokenBdecimals));
                } else {
                    // Pad tokenA amount with zeros
                    tokenAamount = tokenAamount * (10 ** (tokenBdecimals - tokenAdecimals));
                }
            }    
