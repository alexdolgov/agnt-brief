...
library AccessControlLib {
    ...
    uint32 constant ROLE_PAUSE_MANAGER = 0x0100_0000;
    ...
    /// @dev Throws error if sender do not have pause manager role
    function enforceIsPauseManager() internal view {
        if(!isSenderInRole(ROLE_PAUSE_MANAGER)) {
            revert AccessDenied(msg.sender, ROLE_PAUSE_MANAGER);
        }        
    }
}
