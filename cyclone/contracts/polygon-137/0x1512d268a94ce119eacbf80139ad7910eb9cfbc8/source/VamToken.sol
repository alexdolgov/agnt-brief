...
contract VamToken {
    ...
    function burn(uint256 _vamTokenValue) external {
        ...
        s.amToken.transferFrom(address(this), msg.sender, amTokenValue);
    ...
    function updateUserRewardsForToken(address user, address rewardsToken) public {
        if (rewardsToken != address(0) && s.totalSupply > 0 && user != address(0)) {
            ...
            uint256 userApplicableBalance = s.tokenVsRewards[rewardsToken] - _userRewards.lastClaimedContractBalance;
    ...
    function claimRewardsFromController() public {
        ...
        if (address(controller) != address(0)) {
            ...
            uint256 amountReceived = controller.claimRewards(
                    assets,
                    type(uint256).max,
                    address(this),
                    false
