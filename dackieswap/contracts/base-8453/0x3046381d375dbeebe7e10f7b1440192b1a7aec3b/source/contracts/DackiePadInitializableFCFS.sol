...
import "./interfaces/IDackiePadFCFS.sol";
import "./utils/WhiteList.sol";

/**
 * @title DackiePadInitializableFCFS
 */
contract DackiePadInitializableFCFS is IDackiePadFCFS, ReentrancyGuard, Whitelist {
    ...
    struct PoolCharacteristics {
        ...
        uint256 minimumPerUserInLP; // // minimum of tokens per user (if 0, it is ignored)
    ...
    function depositPool(uint256 _amount, uint8 _pid) external override nonReentrant notContract {
        ...
        // Check that raisingAmountPool is less than or equal to the balance of this contract in raiseToken
        require(
            raiseToken.balanceOf(address(this)) <= _poolInformation[_pid].raisingAmountPool,
            "Deposit: Raising amount exceeds contract balance"
        );
        ...
        if (!_poolInformation[_pid].isSpecialSale) {
            ...
            // Check if the pool has a minimum per user
            if (_poolInformation[_pid].minimumPerUserInLP > 0) {
                // Checks whether the minimum has been reached
                require(
                    _amount >= _poolInformation[_pid].minimumPerUserInLP,
                    "Deposit: New amount is too low for the user minimum limit"
                );
            }
     ...
     * @param _minimumPerUserInLP: minimum per user (in LP tokens)
    ...
    function setPool(
        ...
        _poolInformation[_pid].minimumPerUserInLP = _minimumPerUserInLP;
