...
import './interfaces/ICrystalV3Factory.sol';
import "./interfaces/ICrystalV3PoolDeployer.sol";
import './interfaces/ICrystalV3Pool.sol';

/// @title Canonical CrystalSwap V3 factory
/// @notice Deploys CrystalSwap V3 pools and manages ownership and control over pool protocol fees
contract CrystalV3Factory is ICrystalV3Factory {
    /// @inheritdoc ICrystalV3Factory
    ...
    function createPool(
        ...
        pool = ICrystalV3PoolDeployer(poolDeployer).deploy(address(this), token0, token1, fee, tickSpacing);
    ...
    function setFeeProtocol(address pool, uint32 feeProtocol0, uint32 feeProtocol1) external override onlyOwner {
        ICrystalV3Pool(pool).setFeeProtocol(feeProtocol0, feeProtocol1);
    }
    ...
    function collectProtocol(
        ...
        return ICrystalV3Pool(pool).collectProtocol(recipient, amount0Requested, amount1Requested);
    }

    function setLmPool(address pool, address lmPool) external override onlyOwnerOrLmPoolDeployer {
        ICrystalV3Pool(pool).setLmPool(lmPool);
    }
}
