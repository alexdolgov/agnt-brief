...
import './interfaces/IMongSwapV3Factory.sol';
import "./interfaces/IMongSwapV3PoolDeployer.sol";
import './interfaces/IMongSwapV3Pool.sol';

/// @title Canonical MongSwap V3 factory
/// @notice Deploys MongSwap V3 pools and manages ownership and control over pool protocol fees
contract MongSwapV3Factory is IMongSwapV3Factory {
    /// @inheritdoc IMongSwapV3Factory
    ...
    function createPool(
        ...
        pool = IMongSwapV3PoolDeployer(poolDeployer).deploy(address(this), token0, token1, fee, tickSpacing);
    ...
    function setFeeProtocol(address pool, uint32 feeProtocol0, uint32 feeProtocol1) external override onlyOwner {
        IMongSwapV3Pool(pool).setFeeProtocol(feeProtocol0, feeProtocol1);
    }
    ...
    function collectProtocol(
        ...
        return IMongSwapV3Pool(pool).collectProtocol(recipient, amount0Requested, amount1Requested);
    }

    function setLmPool(address pool, address lmPool) external override onlyOwnerOrLmPoolDeployer {
        IMongSwapV3Pool(pool).setLmPool(lmPool);
    }
}
