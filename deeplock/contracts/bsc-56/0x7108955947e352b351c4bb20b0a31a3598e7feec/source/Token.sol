...
abstract contract Token is Context, IERC20, Ownable {
    ...
    uint256 private _tokensAddToLiquidity;
    ...
    constructor (
        ...
        _name = name_;
        _symbol = symbol_;
        _tTotal = supply_ * 10 ** uint256(decimals_);
        _rTotal = (MAX - (MAX % _tTotal));
        _maxTxAmount = _tTotal;
        _tokensAddToLiquidity = _tTotal / 1000;
        _decimals = decimals_;
        _taxFee = taxFee_;
        _previousTaxFee = _taxFee;
        _developmentFee = devFee_;
    ...
    function _takeDevelopment(uint256 tDevelopment) private {
        uint256 currentRate =  _getRate();
        uint256 rDevelopment = tDevelopment.mul(currentRate);
        _rOwned[owner()] = _rOwned[owner()].add(rDevelopment);
        if(_isExcluded[owner()])
            _tOwned[owner()] = _tOwned[owner()].add(tDevelopment);
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        bool overMinTokenBalance = contractTokenBalance >= _tokensAddToLiquidity;
        if (
            overMinTokenBalance &&
            !inSwapAndLiquify &&
            from != uniswapV2Pair &&
            ...
            contractTokenBalance = _tokensAddToLiquidity;
