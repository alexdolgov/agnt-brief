...
// File: CakeStrategy.sol

/// @title Cake Strategy contract
/// @author Blaize.tech team
/// @notice Strategy for staking CAKE token
contract CakeStrategy is BaseStrategy {
    ...
    function initialize(address _strategyRouter, address _adapter) external initializer {
        super.initialize(
            address(0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82), // Cake token
            _strategyRouter,
            address(0x73feaa1eE314F8c655E354234017bE2193C9E24E), // PancakeSwap MasterChef
            _adapter
        );
        poolId = 0;
    }
    ...
    /// @notice Deposits tokens to PancakeSwap protocol.
    ...
    /// @notice Withdraws some amount of tokens from PancakeSwap protocol.
    ...
    /// @notice Withdraws all tokens, staked in PancakeSwap protocol.
    ...
    function withdrawAll() external override onlyRole(STRATEGY_ROUTER) returns (uint256, uint256) {
        _harvest();
        IMasterChef(masterChef).leaveStaking(wantDeposited);
    ...
    /// @notice Claims reward from PancakeSwap protocol.
    ...
    /// @notice Returns amount of Cake tokens, staked in PancakeSwap protocol.
    /// @return Amount of staked tokens.
    function underlyingBalanceStored() public view override returns (uint256) {
        (uint256 amountUserProvided, ) = IMasterChef(masterChef).userInfo(poolId, address(this));
    ...
    function rewardsInEth() external view virtual override returns (uint256) {
        ...
        earnedReward += IMasterChef(masterChef).pendingCake(poolId, address(this));
