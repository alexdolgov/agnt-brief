...
contract DexAdapterCore is Ownable {
    ...
    function getDHVPriceInETH(address _dhvToken) external view virtual returns (uint256) {
        ...
        return IUniswapV2Router01(_router).getAmountsOut(1 ether, path)[1];
    }
...
/// @title PancakeSwap Adapter
/// @author Blaize.tech team
/// @notice Contract for interacting with PancakeSwap router
contract PancakeSwapAdapter is DexAdapterCore {
    using SafeERC20 for IERC20;

    address public apeswapRouter = address(0xcF0feBd3f17CEf5b47b0cD257aCf6025c5BFf3b7);
    address public mdexRouter = address(0x7DAe51BD3E3376B8c7c4900E9107f12Be3AF1bA8);
    address public constant EPS = address(0xA7f552078dcC247C2684336020c03648500C6d9F);

    /// @notice Performs an initial setup.
    /// @notice Sets an addresses of PancakeSwap router, WBNB and BUSD in BSC.
    constructor()
        DexAdapterCore(
    ...
    function _getRouter(address _token) internal view override returns (address) {
        if (_token == address(0x603c7f932ED1fc6575303D8Fb018fDCBb0f39a95)) {
            // Banana Token
            return apeswapRouter;
        }
        if (_token == address(0x9C65AB58d8d978DB963e63f2bfB7121627e3a739)) {
            // Mdx Token
            return mdexRouter;
        }
        return router;
    ...
    function getUnderlyingAmount(
        ...
        if (_tokenToSwap == EPS || _tokenToReceive == EPS) return 0;
        if (_tokenToSwap == _tokenToReceive) return _amount;
        return _getUnderlyingAmount(_amount, _tokenToSwap, _tokenToReceive);
    }
