...
// File: contracts\OptimizerWbnb.sol

contract OptimizerWbnb is OwnableUpgradeable {
    using SafeERC20 for IERC20; 

    address public constant WBNB = address(0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c);
    address public constant CAKE = address(0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82);
    address public constant ALPACA = address(0x8F0528cE5eF7B51152A59745bEfDD91D97091d2F);
    ...
    function optimize(
        ...
        address _adapter,
        address _wbnbStrat,
        uint256 _newWbnbShare
    ) external onlyOwner {
        IDexAdapter adapter = IDexAdapter(_adapter);
        // Prepare shares
        uint256[] memory shares = IClusterToken(_asset).getUnderlyingInCluster();
        shares[7] = _newWbnbShare;

        // Optimize wbnb proportion
        uint256 wbnbOnLockWithdraw = IClusterToken(_asset).clusterLock(WBNB);
        IClusterToken(_asset).optimizeProportion(shares);

        wbnbOnLockWithdraw = wbnbOnLockWithdraw - IClusterToken(_asset).clusterLock(WBNB);
        // Withdraw from strategy
        IStrategyPlugin(_wbnbStrat).withdraw(wbnbOnLockWithdraw);

        // Now we have a lot of WBNB
        IERC20(WBNB).safeApprove(_adapter, IERC20(WBNB).balanceOf(address(this)));

        // Swap part to CAKE
        uint256 swapToCake = IERC20(WBNB).balanceOf(address(this)) / 2;
        uint256 swapToAlpaca = IERC20(WBNB).balanceOf(address(this)) - swapToCake;

        adapter.swapTokenToToken(swapToCake, WBNB, CAKE);

        // Swap part to Alpaca
        adapter.swapTokenToToken(swapToAlpaca, WBNB, ALPACA);
    }

    function postOptimize(
        address _asset,
        address _cakeStrat,
        address _alpacaStrat
    ) external onlyOwner {
        uint256[] memory shares = IClusterToken(_asset).getUnderlyingInCluster();
        // Prepare shares 
        uint256 cakeTotal = (IERC20(CAKE).balanceOf(address(this))) +
                            IClusterToken(_asset).getUnderlyingBalance(CAKE);
        
        shares[0] = (cakeTotal * 10**6) / IERC20(_asset).totalSupply();

        uint256 alpacaTotal = (IERC20(ALPACA).balanceOf(address(this))) +
                              IClusterToken(_asset).getUnderlyingBalance(ALPACA);
        
        shares[5] = (alpacaTotal * 10**6) / IERC20(_asset).totalSupply();

        // Optimize
        IClusterToken(_asset).optimizeProportion(shares);

        // Send tokens to strategies
        cakeTotal = IERC20(CAKE).balanceOf(address(this));
        IERC20(CAKE).safeTransfer(_cakeStrat, cakeTotal);
        IStrategyPlugin(_cakeStrat).deposit(cakeTotal);

        alpacaTotal = IERC20(ALPACA).balanceOf(address(this));
        IERC20(ALPACA).safeTransfer(_alpacaStrat, alpacaTotal);
        IStrategyPlugin(_alpacaStrat).deposit(alpacaTotal);
    }
}