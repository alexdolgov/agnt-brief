...
// File: XvsVenusStrategy.sol

contract XvsVenusStrategy is BaseStrategy {
    using SafeERC20 for IERC20;

    /// @notice Venus vToken pegged to XVS
    ...
    /// @notice Performs initial setup.
    /// @param _strategyRouter Address of StrategyRouter contract.
    /// @param _vToken Venus vToken pegged to XVS.
    /// @param _xvs XVS token address.
    function initialize(
        address _strategyRouter,
        address _vToken,
        address _xvs,
        ...
        super.initialize(
            ...
            _strategyRouter,
            address(0),
            _adapter // BSC cluster adapter
        );

        require(address(want) == _xvs, "Incorrect want token");
        vToken = _vToken;
        require(IVERC20(vToken).underlying() == address(want), "Incorrect underlying");
    }
    ...
    /// @notice Deposits tokens to Venus protocol.
    function deposit(uint256 _amount) external override onlyRole(STRATEGY_ROUTER) {
        if (wantDeposited != 0) {
            _harvest();
        }
        want.safeApprove(vToken, _amount);
        require(IVERC20(vToken).mint(_amount) == 0, "ctoken: mint fail");
        want.safeApprove(vToken, 0);
        wantDeposited += _amount;
        emit Deposited(_amount);
    }

    /// @notice Withdraws certain amount of WANT token from Venus protocol.
    /// @param _amount Amount of tokens to withdraw.
    function withdraw(uint256 _amount) external override onlyRole(STRATEGY_ROUTER) {
        require(_amount > 0 && _amount <= wantDeposited, "Incorrect amount");

        if (_amount == wantDeposited) {
            require(IVERC20(vToken).redeem(IVERC20(vToken).balanceOf(address(this))) == 0, "ctoken: redeem fail");
        } else {
            require(IVERC20(vToken).redeemUnderlying(_amount) == 0, "ctoken: redeemUnderlying fail");
        }

        wantRewardCollected += (want.balanceOf(address(this)) - _amount - wantRewardCollected);
    ...
    /// @notice Withdraws all WANT tokens staked in Venus protocol.
    function withdrawAll() external virtual override onlyRole(STRATEGY_ROUTER) returns (uint256, uint256) {
        _harvest();
        require(IVERC20(vToken).redeem(IVERC20(vToken).balanceOf(address(this))) == 0, "ctoken: redeem fail");
    ...
    function emergencyWithdraw() external override onlyRole(STRATEGIST_ROLE) {
        _harvest();
        IVERC20(vToken).redeem(IVERC20(vToken).balanceOf(address(this)));
    ...
    /// @dev Claims rewards from Venus protocol.
    ...
    /// @notice Returns amount of XVS tokens, staked in Venus protocol.
    /// @return balance Amount of staked tokens.
    function underlyingBalanceStored() public view override returns (uint256 balance) {
        uint256 currentVBalance = IERC20(vToken).balanceOf(address(this));
        if (currentVBalance == 0) {
            balance = 0;
        } else {
            balance = (currentVBalance * IVERC20(vToken).exchangeRateStored()) / (1e18);
        }
    }

    /// @dev Tokens which cannot be withdrawn by sweep()
    /// reward, vToken, XVS (want token)
    ...
    function rewardsInEth() external view override returns (uint256) {
        ...
        tokens[0] = vToken;
        address comp = IVERC20(vToken).comptroller();
        earnedReward += IComptroller(comp).venusAccrued(address(this));

        if (earnedReward > DUST_AMOUNT) {
            return _rewardsInEth(earnedReward);
        }
        return 0;
    }
    ...
    function _harvest() internal override returns (uint256) {
        ...
        address comp = IVERC20(vToken).comptroller();
        IComptroller(comp).claimVenus(address(this), tokens);
