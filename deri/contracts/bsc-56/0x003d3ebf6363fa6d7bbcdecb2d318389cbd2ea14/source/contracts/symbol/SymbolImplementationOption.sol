...
contract SymbolImplementationOption is SymbolStorage, NameVersion {
    ...
    ) NameVersion('SymbolImplementationOption', '3.0.4')
    {
        ...
        minInitialMarginRatio = parameters_[11];
        initialOpenVolume = parameters_[12];
    ...
    external _onlyManager_ returns (ISymbol.SettlementOnTrade memory s)
    {
        ...
        int256 traderInitialMarginPerVolume = SafeMath.max(
            data.curIndexPrice * minInitialMarginRatio / ONE,
            data.initialMarginPerVolume
        );
        _updateOpenVolume(liquidity, data.curIndexPrice, data.delta, p.volume, tradeVolume);
        ...
        s.traderPnl = p.volume * data.theoreticalPrice / ONE - p.cost;
        s.traderInitialMarginRequired = p.volume.abs() * traderInitialMarginPerVolume / ONE;
    ...
    external _onlyManager_ returns (ISymbol.SettlementOnLiquidate memory s)
    {
        ...
        _updateOpenVolume(liquidity, data.curIndexPrice, data.delta, p.volume, -p.volume);
    ...
    // update open volume
    function _updateOpenVolume(int256 liquidity, int256 curIndexPrice, int256 delta, int256 curVolume, int256 tradeVolume) internal {
        ...
        require(curOpenVolume >= 0, 'SymbolImplementationOption._updateOpenVolume: Negative open volume');
        openVolume = curOpenVolume;

        if (deltaOpenVolume > 0) {
            int256 openInterestRatio = curIndexPrice * curOpenVolume / liquidity;
            require(
                minInitialMarginRatio * ONE >= delta.abs() * alpha / ONE * openInterestRatio ||
                initialMarginRatio * ONE >= alpha * openInterestRatio,
                'SymbolImplementationOption.updateOpenVolume: exceed max open interest'
            );
        }
    }
