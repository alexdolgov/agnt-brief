...
import './IERC20.sol';
import './ILTokenOption.sol';
import './IPTokenOption.sol';
import './IEverlastingOptionPricing.sol';
import './IOracleViewer.sol';
import './IVolatilityOracle.sol';
import './ILiquidatorQualifier.sol';
import "./IEverlastingOption.sol";
import './SafeMath.sol';
...
contract EverlastingOption is IEverlastingOption, Migratable {
    ...
    function executeMigrationWithFunding(address source, int256 funding) external _controller_ {
    ...
    function _updateFundingRates(uint256 tradeSymbolId) internal returns (DataSymbol[] memory symbols) {
        ...
        if (curTimestamp > preTimestamp) {
            for (uint256 i = 0; i < symbols.length; i++) {
                ...
                funding += diff * s.tradersNetVolume / ONE;
    ...
    internal view returns (IPTokenOption.Position[] memory positions, int256 margin)
    {
        IPTokenOption pToken = IPTokenOption(_pTokenAddress);
