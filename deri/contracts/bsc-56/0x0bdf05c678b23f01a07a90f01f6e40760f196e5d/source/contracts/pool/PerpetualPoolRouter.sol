...
contract PerpetualPoolRouter is IPerpetualPoolRouter, Migratable {
    ...
    function executeMigration(address sourceRouter) external override _controller_ {
        ...
        for (uint256 i = 0; i < blength; i++) {
            IPerpetualPool.BTokenInfo memory b = IPerpetualPool(sourcePool).getBToken(i);
        ...
        for (uint256 i = 0; i < slength; i++) {
            IPerpetualPool.SymbolInfo memory s = IPerpetualPool(sourcePool).getSymbol(i);
        ...
        (uint256 lastTimestamp, int256 protocolFeeAccrued) = IPerpetualPool(sourcePool).getPoolStateValues();
        IPerpetualPool(_pool).migratePoolStateValues(lastTimestamp, protocolFeeAccrued);
    }
