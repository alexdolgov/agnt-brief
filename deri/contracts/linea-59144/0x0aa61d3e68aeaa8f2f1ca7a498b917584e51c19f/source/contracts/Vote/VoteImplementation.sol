...
contract VoteImplementation is VoteStorage {
    ...
    function getVotePowerOnZkSync(address account) public view returns (uint256) {
        address deri = 0x140D5bc5b62d6cB492B1A475127F50d531023803;
    ...
    function getVotePowerOnLinea(address account) public view returns (uint256) {
        address deri = 0x4aCde18aCDE7F195E6Fb928E15Dc8D83D67c1f3A;
    ...
    function getVotePowersOnZkSync(address[] memory accounts) external view returns (uint256[] memory) {
        uint256[] memory powers = new uint256[](accounts.length);
        for (uint256 i = 0; i < accounts.length; i++) {
            powers[i] = getVotePowerOnZkSync(accounts[i]);
        }
        return powers;
    }

    function getVotePowersOnLinea(address[] memory accounts) external view returns (uint256[] memory) {
        uint256[] memory powers = new uint256[](accounts.length);
        for (uint256 i = 0; i < accounts.length; i++) {
            powers[i] = getVotePowerOnLinea(accounts[i]);
        }
        return powers;
    }
