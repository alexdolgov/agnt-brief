...
contract DexillaExchangeV6 is AccessControl, ReentrancyGuard, Multicall, SelfPermit {
    ...
    function registerThis(address sfsContractAddress) public onlyRole(DEFAULT_ADMIN_ROLE) returns (uint256 tokenId) {
        SfsRegister sfsContract = SfsRegister(sfsContractAddress);
        return sfsContract.register(msg.sender);
    }

    /// @dev Assigns smart contract to existing NFT.
    /// @param sfsContractAddress This address is the address of the SFS contract
    /// @param tokenId This is the tokenId of the NFT
    function assignThis(
        address sfsContractAddress,
        uint256 tokenId
    ) public onlyRole(DEFAULT_ADMIN_ROLE) returns (uint256) {
        SfsRegister sfsContract = SfsRegister(sfsContractAddress);
        return sfsContract.assign(tokenId);
    }
