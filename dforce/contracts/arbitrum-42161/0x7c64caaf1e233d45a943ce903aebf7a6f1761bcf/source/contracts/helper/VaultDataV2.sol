...
contract VaultDataV2 is Ownable {
    ...
    function initialize(IiTokenHelper _priceToken, uint256 _blocksPerYear)
        public
    {
    ...
    function getBalance(IiTokenHelper _iToken, address _account)
        public
        view
        returns (uint256)
    {
    ...
    function getAvailableBalance(
        ...
        IiTokenHelper _underlying =
            IiTokenHelper(address(_iToken.underlying()));
        (bool _success, bytes memory _res) =
            address(_underlying).staticcall(
        ...
        if (_success && _res.length == 32 && abi.decode(_res, (bool))) {
            ...
            (uint256 _collateralFactor, , , , , , ) =
                _controller.markets(_underlying);
            if (
    ...
    function getAssetUSDPrice(
        ...
        uint256 _USDPrice =
            _controller.priceOracle().getUnderlyingPrice(priceToken);
        if (_USDPrice == 0) return 0;

        uint256 _assetUSDPrice =
            _controller.priceOracle().getUnderlyingPrice(_iToken).rdiv(
                _USDPrice
            );
    ...
    function getAccountAvailable(
        ...
        IiTokenHelper[] memory _collateralITokens =
            _controller.getEnteredMarkets(_account);
        ...
        IiTokenHelper[] memory _borrowedITokens =
            _controller.getBorrowedAssets(_account);
    ...
    function getPoolInfo(IiTokenHelper _iMSD)
        public
        returns (uint256 _supplyValue, uint256 _borrowValue)
    {
    ...
    function getPoolsInfo(IiTokenHelper[] calldata _iMSDs)
        external
        returns (
    ...
    function getCollateralData(IiTokenHelper _iMSD)
        external
        returns (
            uint256 _supplyValue,
            uint256 _collateralFactor,
    ...
    function getBorrowData(IiTokenHelper _iMSD)
        external
        returns (
    ...
    function getAccountInfo(IiTokenHelper _iMSD, address _account)
        external
        returns (
            uint256,
            uint256,
    ...
    function getAccountRewardAmount(IiTokenHelper _iMSD, address _account)
        external
        returns (uint256)
    {
        IControllerHelper _controller = _iMSD.controller();
        IRewardDistributorHelper _rewardDistributor =
            _controller.rewardDistributor();
    ...
    function getDistributionSupplyApy(IiTokenHelper _asset)
        external
        returns (uint256)
    {
        ...
        uint256 _assetValue =
            _asset.totalSupply().rmul(_asset.exchangeRateCurrent()).mul(
    ...
    function getDistributionBorrowApy(IiTokenHelper _asset)
        external
        returns (uint256)
    {
        IControllerHelper _controller = _asset.controller();
        ...
        uint256 _assetValue =
            _asset.totalBorrowsCurrent().mul(
                _priceOracle.getUnderlyingPrice(_asset)
            );
    ...
    function rewardTokenSymbol(IiTokenHelper _asset)
        external
        view
        returns (string memory _symbol)
    {
        address _rewardToken =
            address(_asset.controller().rewardDistributor().rewardToken());
        if (_rewardToken != address(0)) {
            (bool _success, bytes memory _res) =
                _rewardToken.staticcall(abi.encodeWithSignature("symbol()"));
            if (_success)
