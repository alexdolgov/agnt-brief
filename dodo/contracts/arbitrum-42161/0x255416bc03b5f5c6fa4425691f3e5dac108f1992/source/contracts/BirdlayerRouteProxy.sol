...
contract BirdlayerRouteProxy is Ownable {
    ...
    // dodo vault address, the receiver of token
    ...
    constructor(address payable weth, address dodoApproveProxy, address vault, address feeReceiver) {
        require(feeReceiver != address(0), "DODORouteProxy: feeReceiver invalid");
        require(vault != address(0), "DODORouteProxy: vault invalid");
    ...
    function updateVault(address vaultAddr) public onlyOwner {
        require(vaultAddr != address(0), "DODORouteProxy: vault invalid");
        _VAULT_ = vaultAddr;
    }
    ...
    function wrap(bytes32 orderId, uint256 amount, address receiver) external payable {
        require(amount > 0, "INVALID_AMOUNT");
        require(msg.value == amount, "ETH_VALUE_WRONG");

        address to = receiver == address(0) ? _VAULT_ : receiver;
    ...
    function unwrap(bytes32 orderId, uint256 amount, address receiver) external {
        require(amount > 0, "INVALID_AMOUNT");

        IDODOApproveProxy(_DODO_APPROVE_PROXY_).claimTokens(_WETH_, msg.sender, address(this), amount);
    ...
    function externalSwap(
        ...
        // distribute toToken
        address to = receiver == address(0) ? _VAULT_ : receiver;
        receiveAmount = _routeWithdraw(toToken, receiveAmount, feeData, expReturnAmount, minReturnAmount, to);
    ...
    function mixSwap(
        ...
        // distribute toToken
        address to = _receiver == address(0) ? _VAULT_ : _receiver;
        receiveAmount = _routeWithdraw(_toToken, receiveAmount, _feeData, _expReturnAmount, _minReturnAmount, to);
    ...
    /// @notice after all swaps, transfer tokens to vault and distribute fees to DODO and broker
    ...
    function _routeWithdraw(
        ...
        if (originToToken == _ETH_ADDRESS_) {
            ...
            payable(receiver).transfer(receiveAmount);
        } else {
            IERC20(toToken).universalTransfer(payable(receiver), receiveAmount);
        }
