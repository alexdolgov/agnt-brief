...
contract TimeLockContract is Ownable2Step, ReentrancyGuard {
    ...
    // `token = ETH_ADDRESS` means ETH
    mapping(address holder => mapping(address token => uint256 balance)) public lockedBalanceOf;
    mapping(address token => mapping(address adapter => bool isWhiteList)) public tokenWhitelist;

    // ============= Events ==============

    event TokenLocked(address indexed account, address token, uint256 amount);
    event TokenWithdrawn(address token, uint256 amount);
    event TransferToAdapter(address indexed holder, address indexed token, uint256 amount);
    ...
    function setAdapterWhitelist(address token, address adapter, bool isWhitelist) external onlyOwner {
        tokenWhitelist[token][adapter] = isWhitelist;
    }

    // ============ For WhiteList ============

    function transferOutByWhitelist(address holder, address token, uint256 amount) external nonReentrant {
        require(tokenWhitelist[token][msg.sender], "Invalid caller");
        require(lockedBalanceOf[holder][token] >= amount, "Insufficient balance");

        lockedBalanceOf[holder][token] -= amount;
        ...
        emit TransferToAdapter(holder, token, amount);
    }
    ...
    receive() external payable {
        ...
        lockedBalanceOf[msg.sender][ETH_ADDRESS] += amount;
        emit TokenLocked(msg.sender, ETH_ADDRESS, amount);
    }
    ...
    function withdrawETH(uint256 amount) external {
        ...
        emit TokenWithdrawn(ETH_ADDRESS, amount);
    }
    ...
    function withdrawERC20(address token, uint256 amount) external {
        ...
        tokenContract.safeTransfer(beneficiary, amount);
