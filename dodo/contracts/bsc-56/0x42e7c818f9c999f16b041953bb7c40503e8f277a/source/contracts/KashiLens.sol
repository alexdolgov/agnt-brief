...
// import "./bentobox/interfaces/IStrategy.sol";
...
contract KashiLens  {
    ...
    function pollKashiPairs(address who, IKashiPair[] calldata pairsIn) external view returns (KashiPairPoll[] memory) {
        ...
        for (uint256 i = 0; i < len; i++) {
            ...
            pairs[i].strategyData = IBentoBox(pair.bentoBox()).strategyData(pair.asset());
        }
...
struct KashiPairPoll {
    ...
    IBentoBox.StrategyData strategyData;
}

interface IStrategyImplem {
...
interface IBentoBox {
    ...
    function strategyData(IBEP20) external view returns (StrategyData memory strategyData_);

    // function strategy(IBEP20) external view returns (IStrategy);
}
