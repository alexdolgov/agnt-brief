...
contract DomfiTrading is IDomfiTrading, Delegatable, Initializable {
    ...
    function openTrade(
    ...
    ) external payable notDone notPaused pairIndexListed(t.pairIndex) {
        ...
        uint256 oracleFeeOffered = msg.value;

        if (oracleFeeOffered < oracleFee) {
            revert InsufficientOracleFee();
        }

        // slither-disable-next-line unused-return
        tradingStorage.handleOracleFee{ value: oracleFeeOffered }(oracleFeeOffered);
    ...
    // slither-disable-next-line cyclomatic-complexity
    function updateTp(uint16 pairIndex, uint8 index, uint192 newTp) external payable notDone {
        address sender = msgSender();
    ...
    function updateSl(uint16 pairIndex, uint8 index, uint192 newSl) external payable notDone {
        address sender = msgSender();
