...
/**
 * @title  SwapAdapterMk4 (UniV2/SushiV2 + optional Curve3 fast-path + Chainlink guards)
 ...
 */
contract SwapAdapterMk4 is Ownable, ReentrancyGuard {
	...
	* @notice Find top-1 and top-2 venues (if any) among enabled routers.
	* @dev
	* - Queries each enabled router for a quote using `_safeQuote`.
	* - Returns the top-1 and top-2 venues with their respective quotes.
	* - If no enabled routers are found, returns (NONE, 0, 0, NONE, 0, 0).
	* - If only one enabled router is found, returns (top-1, NONE, 0, 0).
	* - If both routers are enabled but one fails to quote, returns (top-1, NONE, 0, 0).
	* - If both routers are enabled and both quote successfully, returns (top-1, top-2).
	* 
	* @param  amountIn Input amount
	* @param  path     Swap path
	* @return dex1     Top-1 dex venue and quote
