...
contract StampedeMigrationEngine is Ownable {
    ...
    event BatchTransferToTrumpet(uint upgradedV6, uint transferedTrumpet, uint migratedTrumpetUsers);
    ...
    function dryrun(address[] memory participants) external view returns(uint upgradedV6, uint transferedTrumpet, uint migratedTrumpetUsers) {
        ...
        for(uint256 i = 0; i < participants.length; i++) {
            ...
            (migrateSuccess, balance) = migrateDry(participants[i]);
            
            if (migrateSuccess) {

                upgradedV6++;
                transferedTrumpet += balance;
                migratedTrumpetUsers++;
                
            } else {

                (success, balance) = transferToTrumpetDry(participants[i]);
                if (success) { 
                    migratedTrumpetUsers++;
                    transferedTrumpet += balance;
                }
    ...
    // Batch migrate users to TRUMPET
    function batchProcess(address[] memory participants) onlyOwner external returns(uint upgradedV6, uint transferedTrumpet, uint migratedTrumpetUsers) {
        
        bool success; 
        uint balance;

        for(uint256 i = 0; i < participants.length; i++) {
            if (migrate(participants[i])) {
                upgradedV6++;
            }
            ...
            if (success) { 
                ...
                if (transferToTrumpet(participants[i])) {
                    migratedTrumpetUsers++;
                }
            }
        ...
        emit BatchTransferToTrumpet(upgradedV6, transferedTrumpet, migratedTrumpetUsers);
    }

    ////  User Functions ////

     //@dev migrate account from v5 to v6; DRY RUN
    function migrateDry(address _user)  internal view returns (bool success, uint balance) {  

        (bool eligible, uint deposits,,)  = eligibleForMigration(_user);

        return (eligible, deposits.min(maxBalance));
    }

    //@dev Transfer account to another wallet address; DRY RUN
    function transferToTrumpetDry(address _user)  internal view returns (bool success, uint balance)  {

        StampedeUser memory userData = vault.getUser(_user);

        //Only the owner with balance can migrate to TRUMPET
        return (userData.exists  && userData.current_balance > 0, userData.current_balance);
    }


    //@dev migrate account from v5 to v6
    function migrate(address _user)  internal  returns (bool success) {  
    ...
    function transferToTrumpet(address _user)  internal returns (bool success)  {

        StampedeUser memory userData = vault.getUser(_user);
        StampedeGlobals memory globalsData = vault.getGlobals();

        //Only the owner with balance can migrate to TRUMPET
        if (userData.exists  && userData.current_balance > 0) {
        
            //TVL with claim forgiveness with 5% bonus
            uint tvl = userData.current_balance * 105 / 100;
