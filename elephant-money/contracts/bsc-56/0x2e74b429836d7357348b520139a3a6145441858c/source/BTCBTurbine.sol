...
contract BTCBTurbine is Whitelist, ReentrancyGuard {

    using SafeMath for uint256;
    
    AddressRegistry private registry;

    uint public constant minimumBNBThreshold = 0.2e18;
    ...
    function availableSweep() public view returns (uint btcAmount) {
        ...
        btcAmount = _share * _seconds;  //share times the amount of time elapsed

    }

    function balanceOf() public view returns (uint balance) {
         IERC20 BTCB = IERC20(registry.BTCBAddress());

         balance = BTCB.balanceOf(address(this));
    }

    function estimateBTCAmount(uint wethAmount) public view returns (uint btcbAmount) {
    ...
    function saylor() nonReentrant external {
    ...
    function sweep() nonReentrant public returns (uint btcAmount) {

        //How many tokens are available
        btcAmount = availableSweep();

        //Forward to destinaton
        if (btcAmount >= estimateBTCAmount(minimumBNBThreshold) || msg.sender == owner()) {

            buyback(btcAmount);

            lastSweep = block.timestamp;

            emit Sweep(btcAmount);
        }
        
    }


    function buyBTCB() internal {
        ...
        if (_amount >= minimumBNBThreshold || msg.sender == owner()) {
    ...
    function buyback(uint _btcAmount) internal {
        ...
        path[0] = registry.BTCBAddress();
        path[1] = collateralRouter.WETH();
        path[2] = registry.coreAddress();

        uint[] memory output =  oracle.consultAmountsOut(_btcAmount, path);
        ...
        require(btcb.approve(address(collateralRouter), _btcAmount), "Failed to approve BTCB");

        //buy immediately and store in ELEPHANT Treasury
        collateralRouter.swapExactTokensForTokensSupportingFeeOnTransferTokens(_btcAmount, minimum, path, registry.coreTreasuryAddress(), block.timestamp);
