...
contract BulkNFTMinter is Ownable, ReentrancyGuard {
    ...
    function bulkMint(address to, uint count) nonReentrant external payable returns (uint minted) {
        ...
        require(address(tracker) != address(0), "tracker not set");
...
contract AdministrativeNFTMinter is Whitelist, ReentrancyGuard {
    ...
    event BulkMints(address indexed to, uint amount);
    ...
    function bulkMint(address _to, uint _count) isRunning nonReentrant onlyWhitelisted external returns (uint _minted) {

        require(address(nft) != address(0), "nft not set");
        require(address(tracker) != address(0), "tracker not set");
        require(_count >= 1, "count must be 1 or more");
