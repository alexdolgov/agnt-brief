...
/*
    Elephant Money Bot Relayer

    - Used to fund gas for bots   
...
struct Bot {
    uint total_gas; //partner rewards
    uint last_fund; //last time bot was funded
}

contract BotGasStation is Whitelist {
    using SafeMath for uint256;

    mapping(address => Bot) private bots; //address -> bot
    ...
    uint256 public allowanceMinimum = 0.1e18;
    uint256 public allowanceCap = 2e18;

    event UpdateCollateralRouter(address indexed addr);
    event UpdateAllowanceMinimum(uint256 _minimum);
    event UpdateAllowanceCap(uint256 _cap);
    ...
    constructor() Ownable() {
        ...
        wethToken = IERC20(collateralRouter.WETH());
    ...
    //@dev Update the minimum that the wallet can fall in gas balance
    function updateAllowanceMinimum(uint256 _minimum) public onlyOwner {
        require(_minimum >= 0.1e18, "minimum must be more than 0.1");
        
        allowanceMinimum = _minimum;

        emit UpdateAllowanceMinimum(_minimum);
    }

    //@dev Update the minimum that the wallet can fall in gas balance
    function updateAllowanceCap(uint256 _cap) public onlyOwner {
        require(_cap >= 1e18, "minimum must be more than 1");
        
        allowanceCap = _cap;

        emit UpdateAllowanceCap(_cap);
    }
    ...
    //@dev Funds a wallet if it is in the whitelist
        ...
        onlyWhitelisted
    {
        ...
        uint elapsed = block.timestamp - bots[_bot].last_fund;

        //time restriction means a rogue bot can't drain the system
        require(elapsed > 1 days, "only one fund in a 24 hour period");
        

        if (_bot.balance <= allowanceMinimum){

            uint _amount = estimateWethToCore(allowanceCap);            
            _amount = liquidateCoreToWeth(address(msg.sender), _amount); 
            
            //update bookkeeping
            bots[_bot].last_fund = block.timestamp; 
            bots[_bot].total_gas += _amount; 
    ...
    function estimateWethToCore(uint _wethAmount) public view returns (uint coreAmount) {
         //Convert from collateral to core using oracle
        address[] memory path = new address[](2);
        path[0] = collateralRouter.WETH();
        path[1] = address(coreToken);

        uint[] memory amounts = oracle.consultAmountsOut(_wethAmount, path);
    ...
    function liquidateCoreToWeth(address destination, uint256 _amount) private returns (uint wethAmount) {
        ...
        //approve & swap
        TransferHelper.safeApprove(address(coreToken), address(collateralRouter), _amount, 'BotRelayer: liquidateCore, approve');
        ...
        wethAmount = destination.balance.safeSub(initialBalance);
