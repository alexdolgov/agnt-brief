...
contract SuperCharger is Ownable, ReentrancyGuard {
    ...
    function updateDailyAPR(uint apr) onlyOwner external {
        require(apr >= 137 && apr <= 27500, "Daily APR out of range 137 - 27500 equivalent 0.5 - 100% annual APR");
    ...
    function updateSlippage(uint _slippage) onlyOwner external {
        require(_slippage < 1000 && _slippage >=  500, "slippage < 1000 & slippage >= 500");
    ...
    //A turbine with an administrative escape hatch if needed
    function withdrawUnderlying(uint256 _amount, bool _force) onlyOwner public {
         IERC20 token = IERC20(underlyingAddress);
