...
library AppConfig {
    ...
    bytes32 public constant MAX_LONG_OPEN_INTEREST_USER_CAP = keccak256(abi.encode("MAX_LONG_OPEN_INTEREST_USER_CAP"));
    bytes32 public constant MAX_SHORT_OPEN_INTEREST_USER_CAP = keccak256(abi.encode("MAX_SHORT_OPEN_INTEREST_USER_CAP"));
    ...
    bytes32 public constant ONE_INCH_ROUTER = keccak256(abi.encode("ONE_INCH_ROUTER"));
    ...
    struct SymbolConfig {
        ...
        uint256 maxLongOpenInterestUserCap;
        uint256 maxShortOpenInterestUserCap;
    ...
    function getSymbolConfig(bytes32 symbol) external view returns (SymbolConfig memory) {
        ...
        config.maxLongOpenInterestUserCap = app.getUintValue(
            keccak256(abi.encode(key, symbol, MAX_LONG_OPEN_INTEREST_USER_CAP))
        );
        config.maxShortOpenInterestUserCap = app.getUintValue(
            keccak256(abi.encode(key, symbol, MAX_SHORT_OPEN_INTEREST_USER_CAP))
        );
    ...
    function setSymbolConfig(bytes32 symbol, SymbolConfig memory config) external {
        ...
        app.setUintValue(keccak256(abi.encode(key, symbol, MAX_LONG_OPEN_INTEREST_USER_CAP)), config.maxLongOpenInterestUserCap);
        app.setUintValue(
            keccak256(abi.encode(key, symbol, MAX_SHORT_OPEN_INTEREST_USER_CAP)),
            config.maxShortOpenInterestUserCap
        );
    ...
    function setOneInchRouter(address router) external {
        AppStorage.Props storage app = AppStorage.load();
        app.setAddressValue(keccak256(abi.encode(AppStorage.COMMON_CONFIG, ONE_INCH_ROUTER)), router);
    }

    function getOneInchRouter() external view returns (address) {
        AppStorage.Props storage app = AppStorage.load();
        return app.getAddressValue(keccak256(abi.encode(AppStorage.COMMON_CONFIG, ONE_INCH_ROUTER)));
    }
}
