...
contract StakingAccountFacet is IStakingAccount {
    ...
    function getStakingAccountFeeRewards(
        address account,
        address stakeToken
    ) external view override returns (StakingAccount.FeeRewards memory) {
        return StakingAccount.load(account).getFeeRewards(stakeToken);
    }
}
