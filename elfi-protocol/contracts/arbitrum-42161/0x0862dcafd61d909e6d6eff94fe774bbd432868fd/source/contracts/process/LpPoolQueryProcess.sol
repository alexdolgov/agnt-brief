...
library LpPoolQueryProcess {
    ...
    function getPoolIntValue(
        ...
        if (pool.baseTokenBalance.amount > 0 || pool.baseTokenBalance.unsettledAmount > 0) {
            ...
            value = CalUtils.tokenToUsdInt(
                _getTokenAmount(pool.baseTokenBalance) + unPnl,
            ...
            for (uint256 i; i < stableTokens.length; i++) {
                ...
                value = value.add(
                    CalUtils.tokenToUsdInt(
                        _getTokenAmount(tokenBalance) + unPnl,
    ...
    function getPoolAvailableLiquidityWithPoolValue(
        LpPool.Props storage pool,
        address stableToken
    ) external view returns (uint256) {
        uint256 poolValue = getPoolValue(pool);
        if (poolValue == 0) {
            return 0;
        }
        uint256 stableTokenTokenPrice = OracleProcess.getLatestUsdUintPrice(stableToken, true);
        uint256 stableTokenAmount = CalUtils.usdToToken(
            poolValue,
            TokenUtils.decimals(stableToken),
            stableTokenTokenPrice
        );
        return CalUtils.mulRate(stableTokenAmount, pool.getPoolLiquidityLimit());
    }
    ...
    function getPoolAvailableLiquidity(
        ...
        int256 baseTokenAmount = (pool.baseTokenBalance.amount + pool.baseTokenBalance.unsettledAmount).toInt256();
        ...
        if (stableTokens.length > 0) {
            ...
            for (uint256 i; i < stableTokens.length; i++) {
                LpPool.TokenBalance storage tokenBalance = pool.stableTokenBalances[stableTokens[i]];
                if (_getTokenAmount(tokenBalance) < 0) {
                    int256 tokenUsd = CalUtils.tokenToUsdInt(
                        _getTokenAmount(tokenBalance),
                        TokenUtils.decimals(stableTokens[i]),
                        OracleProcess.getIntOraclePrices(oracles, stableTokens[i], true)
                    );
    ...
    function _getTokenAmount(LpPool.TokenBalance storage tokenBalance) internal view returns (int256) {
        return
            (tokenBalance.amount + tokenBalance.unsettledAmount).toInt256() +
            tokenBalance.fundingSettledAmount -
