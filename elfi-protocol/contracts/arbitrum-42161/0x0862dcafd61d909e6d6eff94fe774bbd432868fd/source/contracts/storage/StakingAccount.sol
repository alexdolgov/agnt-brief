...
library StakingAccount {
    ...
    function getStakeAmount(Props storage self, address stakeToken) external view returns (uint256) {
        return self.stakeTokenBalances[stakeToken].stakeAmount;
    }

    function getStakeUsdAmount(Props storage self) external view returns (uint256) {
        return self.stakeUsdAmount;
    }
