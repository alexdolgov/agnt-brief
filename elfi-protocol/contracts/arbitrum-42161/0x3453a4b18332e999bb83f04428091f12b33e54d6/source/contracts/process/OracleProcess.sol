...
library OracleProcess {
    ...
    function setSingleOraclePrice(OracleParam calldata param) external {
        OraclePrice.Props storage oracle = OraclePrice.load();
        if (param.targetToken == address(0)) {
            oracle.setPrice(param.token, OraclePrice.Data(param.minPrice, param.maxPrice));
        } else {
            oracle.setPrice(param.token, param.targetToken, OraclePrice.Data(param.minPrice, param.maxPrice));
        }
    }

    function getIntOraclePrices(OracleParam[] memory params, address token, bool isMin) external view returns (int256) {
        for (uint256 i; i < params.length; i++) {
            if (params[i].token == token && params[i].targetToken == address(0)) {
