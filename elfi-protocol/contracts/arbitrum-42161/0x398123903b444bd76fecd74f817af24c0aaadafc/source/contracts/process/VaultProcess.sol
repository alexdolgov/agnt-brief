...
library VaultProcess {
    event VaultTransferUpdateEvent(address vault, address token, address receiver, uint256 amount);
    ...
    function transferOut(
        ...
        if (tokenBalance >= amount) {
            ...
            emit VaultTransferUpdateEvent(vault, token, receiver, amount);
            return true;
    ...
    function tryTransferOut(address vault, address token, address receiver, uint256 amount) public returns (uint256) {
        ...
        if (tokenBalance >= amount) {
            ...
            emit VaultTransferUpdateEvent(vault, token, receiver, tokenBalance);
            return tokenBalance;
        }
    }
