...
contract StakeFacet is IStake, ReentrancyGuard {
    ...
    function createMintStakeTokenRequest(MintStakeTokenParams calldata params) external payable override nonReentrant {
        ...
        if (params.stakeToken == address(0) || params.isCollateral) {
        ...
        if (!params.isFromAccount) {
            require(!params.isNativeToken || msg.value == params.requestTokenAmount, "Deposit eth amount error!");
        ...
        (uint256 requestTokenAmount, bool isExecutionFeeFromLpVault) = MintProcess.validateAndDepositMintExecutionFee(
            account,
            params
        );
        if (requestTokenAmount == 0) {
            revert Errors.MintWithAmountZero();
        }

        MintProcess.createMintStakeTokenRequest(params, account, token, requestTokenAmount, isExecutionFeeFromLpVault);
    }
    ...
    function cancelMintStakeToken(uint256 requestId, bytes32 reasonCode) external nonReentrant {
        uint256 startGas = gasleft();
        RoleAccessControl.checkRole(RoleAccessControl.ROLE_KEEPER);
        Mint.Request memory mintRequest = Mint.get(requestId);
        if (mintRequest.account == address(0)) {
    ...
    function executeRedeemStakeToken(
        ...
        ClaimRewardsProcess.claimRewardsFromRedeem(request.account);
    ...
    /// @param requestId The ID of the redeem request
    /// @param reasonCode The reason code for cancellation
    function cancelRedeemStakeToken(uint256 requestId, bytes32 reasonCode) external nonReentrant {
        uint256 startGas = gasleft();
        RoleAccessControl.checkRole(RoleAccessControl.ROLE_KEEPER);
