...
library RedeemProcess {
    ...
    function executeRedeemStakeToken(uint256 requestId, Redeem.Request memory redeemRequest) external {
        ...
        bool isUsdPool = CommonData.getStakeUsdToken() == redeemRequest.stakeToken;
        if (isUsdPool) {
        ...
        MarketProcess.updatePoolBorrowingFeeRate(redeemRequest.stakeToken, !isUsdPool, redeemRequest.redeemToken);
    ...
    function _redeemStakeUsd(Redeem.Request memory params) internal returns (uint256) {
        ...
        (uint256 redeemTokenAmount, uint256 redeemFee) = _executeRedeemStakeUsd(pool, stakeUsdToken, params);
    ...
    function _executeRedeemStakeToken(
        ...
        return cache.redeemTokenAmount - cache.redeemFee;
    }
    ...
    function _executeRedeemStakeUsd(
        ...
        uint256 receiveTokenAmount = redeemTokenAmount - redeemFee;

        if (params.minRedeemAmount > 0 && receiveTokenAmount < params.minRedeemAmount) {
            revert Errors.RedeemStakeTokenTooSmall(receiveTokenAmount);
        }
        ...
        StakeToken(params.stakeToken).transferOut(params.redeemToken, params.receiver, receiveTokenAmount);
        return (redeemTokenAmount, redeemFee);
    }
}
