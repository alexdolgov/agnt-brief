...
library PositionMarginProcess {
    ...
    function updatePositionMargin(uint256 requestId, UpdatePositionMargin.Request memory request) external {
        ...
        if (request.isAdd) {
            ...
            VaultProcess.transferOut(
                ...
                request.marginToken,
                position.isLong ? symbolProps.stakeToken : CommonData.getStakeUsdToken(),
        ...
        // verify liquidation
        if (!position.isCrossMargin && position.isLiquidation(indexPrice)) {
    ...
    function updatePositionLeverage(uint256 requestId, UpdateLeverage.Request memory request) external {
            ...
            if (position.leverage > request.leverage) {
                ...
                if (cache.isCrossMargin) {
                    ...
                    cache.addMarginAmount = CalUtils.usdToToken(
                        ...
                        position.entryMarginTokenPrice
                    );
                ...
                if (position.isCrossMargin) {
                    updateAllPositionFromBalanceMargin(
                        requestId,
                        accountProps.owner,
                        position.marginToken,
                        reduceMarginAmount.toInt256(),
                        position.key
                    );
                }
        ...
        // verify liquidation
        if (
            (!position.isCrossMargin && position.isLiquidation(indexPrice)) ||
            (position.isCrossMargin && accountProps.isCrossLiquidation())
        ) {
            revert Errors.PositionShouldBeLiquidation();
        }
    ...
    function _executeReduceMargin(
        ...
        uint256 maxReduceMarginInUsd = position.initialMarginInUsd -
            CalUtils.divRate(position.qty, symbolConfig.maxLeverage);
        ...
        uint256 reduceMarginAmount = CalUtils.usdToToken(reduceMargin, decimals, position.entryMarginTokenPrice);

        if (reduceMarginAmount >= position.initialMargin) {
            revert Errors.ReduceMarginTooBig();
        }
    ...
    function updateAllPositionFromBalanceMargin(
        ...
        int256 reduceAmount = getFixedReduceTokenAmount(accountProps, token, amount);
        if (reduceAmount == 0) {
            return;
        }
    ...
    function getFixedReduceTokenAmount(
        Account.Props storage accountProps,
        address token,
        int256 amount
    ) internal view returns (int256 fixedAmount) {
        if (amount > 0) {
            return amount;
        }
        uint256 borrowingAmount = accountProps.getBorrowingAmount(token);
        if (borrowingAmount == 0) {
            return 0;
        }
        if (borrowingAmount < (-amount).toUint256()) {
            return -borrowingAmount.toInt256();
        }
        return amount;
    }
}
