...
contract FeeFacet is IFee, ReentrancyGuard {
    /// @dev distributeFeeRewards
    /// @param interval the interval of distribute for minitues
    /// @param oracles OracleProcess.OracleParam[]
    function distributeFeeRewards(
        uint256 interval,
        ...
        RebalanceProcess.rebalanceAllFeeUnsettle();
        FeeRewardsProcess.distributeFeeRewards(interval);
        OracleProcess.clearOraclePrice();
        GasProcess.addLossExecutionFee(startGas);
    }

    function createClaimRewards(address claimUsdToken, uint256 executionFee) external payable override nonReentrant {
    ...
    function executeClaimRewards(
        ...
        OracleProcess.setOraclePrice(oracles);
        ClaimRewardsProcess.claimRewardsFromManual(requestId, request);
        OracleProcess.clearOraclePrice();
    ...
    function cancelClaimRewards(uint256 requestId, bytes32 reasonCode) external nonReentrant {
        uint256 startGas = gasleft();
        RoleAccessControl.checkRole(RoleAccessControl.ROLE_KEEPER);
        ClaimRewards.Request memory request = ClaimRewards.get(requestId);
        if (request.account == address(0)) {
    ...
    function getMarketFeeRewards(bytes32 symbol) external view override returns (AccFeeRewards memory) {
        return FeeQueryProcess.getMarketFeeRewards(symbol);
    }

    function getPoolFeeRewards(address stakeToken) external view override returns (AccFeeRewards memory) {
        return FeeQueryProcess.getPoolFeeRewards(stakeToken);
    }

    function getStakingFeeRewards() external view override returns (AccFeeRewards[] memory) {
        return FeeQueryProcess.getStakingFeeRewards();
    }

    function getDaoFeeRewards() external view override returns (AccFeeRewards[] memory) {
        return FeeQueryProcess.getDaoFeeRewards();
    }
}
