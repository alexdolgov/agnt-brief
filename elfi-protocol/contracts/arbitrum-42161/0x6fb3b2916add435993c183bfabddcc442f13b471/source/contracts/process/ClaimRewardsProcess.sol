...
library ClaimRewardsProcess {
    ...
    function createClaimRewards(address account, address claimUsdToken, uint256 executionFee) external {
        ...
        request.claimSource = ClaimRewards.ClaimSource.MANUAL;
        emit CreateClaimRewardsEvent(requestId, request);
    }

    function claimRewardsFromManual(uint256 requestId, ClaimRewards.Request memory request) external {
        _claimRewards(requestId, request);
    }

    function claimRewardsFromRedeem(address account) external {
        UsdPool.Props storage usdPool = UsdPool.load();

        // default claim usd token
        address token = usdPool.getStableTokens()[0];
        uint256 requestId = UuidCreator.nextId(CLAIM_ID_KEY);

        // if auto claim when redeem, set executionFee to 0
        _claimRewards(requestId, ClaimRewards.Request(account, token, 0, ClaimRewards.ClaimSource.REDEEM));
    }

    function _claimRewards(uint256 requestId, ClaimRewards.Request memory request) internal {
        ...
        for (uint256 i; i < stakeTokens.length; i++) {
            ...
            if (stakingAccount.getStakeAmount(stakeToken) == 0) {
                continue;
            }
        ...
        if (stakingAccount.getStakeUsdAmount() > 0) {
            FeeRewardsProcess.updateAccountFeeRewards(account, CommonData.getStakeUsdToken());
            StakingAccount.FeeRewards storage accountUsdFeeRewards = stakingAccount.getFeeRewards(
                CommonData.getStakeUsdToken()
            );
            uint256 withdrawAmount = CalUtils.usdToToken(
                accountUsdFeeRewards.realisedRewardsTokenAmount,
            ...
            usdPool.totalClaimedRewards += accountUsdFeeRewards.realisedRewardsTokenAmount;
            VaultProcess.transferOut(CommonData.getStakeUsdToken(), request.claimUsdToken, account, withdrawAmount);
            accountUsdFeeRewards.realisedRewardsTokenAmount = 0;
        }
