...
library FeeRewardsProcess {
    ...
    function updateAccountFeeRewards(address account, address stakeToken) public {
        StakingAccount.Props storage stakingAccount = StakingAccount.loadOrCreate(account);
        ...
        uint256 stakeTokens = stakeToken == CommonData.getStakeUsdToken()
            ? stakingAccount.getStakeUsdAmount()
            : stakingAccount.getStakeAmount(stakeToken);
        if (
            stakeTokens > 0 &&
    ...
    function _updateBaseTokenRewardsToLpPool(LpPool.Props storage lpPoolProps) internal {
        ...
        if (baseTokenFeeAmount > 0) {
            lpPoolProps.addBaseToken(baseTokenFeeAmount);
        }
    }
    ...
    function _updateStableTokenRewardsToUsdPool() internal {
        ...
        for (uint256 i; i < stableTokens.length; i++) {
            ...
            if (feeAmount > 0) {
                usdPool.addStableToken(stableTokens[i], feeAmount);
            }
        }
    }
