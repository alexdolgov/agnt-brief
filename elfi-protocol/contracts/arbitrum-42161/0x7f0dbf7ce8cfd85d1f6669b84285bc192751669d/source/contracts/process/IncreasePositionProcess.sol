...
library IncreasePositionProcess {
    ...
    function increasePosition(
    ...
    ) external returns (uint256 increaseQty) {
        ...
        increaseQty = CalUtils.tokenToUsd(
        ...
        /** validate user OI limit */
        if (
            (position.isLong && position.qty + increaseQty > symbolConfig.maxLongOpenInterestUserCap) ||
            (!position.isLong && position.qty + increaseQty > symbolConfig.maxShortOpenInterestUserCap)
        ) {
            revert Errors.UserMaxOILimited(symbolProps.code, position.isCrossMargin, position.isLong);
        }

        if (position.qty == 0) {
            position.marginToken = params.marginToken;
            position.entryPrice = params.indexTokenPrice;
            position.entryMarginTokenPrice = params.marginTokenPrice;
            ...
            position.entryMarginTokenPrice = CalUtils.computeAvgEntryPrice(
                position.qty,
        ...
        // verify liquidation
        if (
            (!position.isCrossMargin && position.isLiquidation(params.indexTokenPrice)) ||
            (params.isCrossMargin && accountProps.isCrossLiquidation())
        ) {
