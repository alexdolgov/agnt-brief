...
library OrderProcess {
    ...
    function createOrderRequest(
        ...
        if (orderMargin == 0 && params.posSide == Order.PositionSide.INCREASE) {
    ...
    function _executeIncreaseOrder(
        ...
        if (!order.isCrossMargin) {
            VaultProcess.transferOut(
                ...
                order.marginToken,
                cache.isLong ? symbolProps.stakeToken : CommonData.getStakeUsdToken(),
                cache.orderMargin
            );
        }
        ...
        uint256 increaseQty = positionProps.increasePosition(
            symbolProps,
        ...
        emit OrderFilledEvent(orderId, order, positionProps, increaseQty, block.timestamp, cache.executionPrice);
    }
    ...
    function _executeDecreaseOrder(
        ...
        emit OrderFilledEvent(orderId, order, position, order.qty, block.timestamp, executionPrice);
    }
