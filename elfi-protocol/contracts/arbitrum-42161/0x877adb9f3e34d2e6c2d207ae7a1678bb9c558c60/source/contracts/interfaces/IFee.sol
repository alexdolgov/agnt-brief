...
interface IFee {
    ...
    struct AccFeeRewards {
        address[] tokens;
        uint256[] feeAmount;
        uint256[] unsettledFeeAmount;
    ...
    function getMarketFeeRewards(bytes32 symbol) external view returns(AccFeeRewards memory);

    function getPoolFeeRewards(address stakeToken) external view returns(AccFeeRewards memory);

    function getStakingFeeRewards() external view returns(AccFeeRewards[] memory);

    function getDaoFeeRewards() external view returns(AccFeeRewards[] memory);
