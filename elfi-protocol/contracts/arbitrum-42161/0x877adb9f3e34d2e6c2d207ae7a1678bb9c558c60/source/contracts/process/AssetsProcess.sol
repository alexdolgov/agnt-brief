...
library AssetsProcess {
    ...
    function withdraw(uint256 requestId, WithdrawParams memory params) public {
        ...
        uint256 amount = _getWithdrawAmount(accountProps, params);
        if (amount == 0) {
            revert Errors.WithdrawWithNoEnoughAmount();
        }
        withdrawToken(requestId, accountProps, params.account, params.account, params.token, amount);
    }
    ...
    function withdrawToken(
        ...
        accountProps.subTokenIgnoreUsedAmount(token, amount, Account.UpdateSource.WITHDRAW);
        VaultProcess.transferOut(IVault(address(this)).getPortfolioVaultAddress(), token, withdrawAddress, amount);
        PositionMarginProcess.updateAllPositionFromBalanceMargin(requestId, account, token, -(amount.toInt256()), "");
    }

    /// @dev Creates a withdraw request
    /// @param token The token to withdraw
    /// @param amount The amount to withdraw
    /// @param executionFee execution fee for keeper
    /// @param isWithdrawMax Whether withdraw 100% percent token
    function createWithdrawRequest(address token, uint256 amount, uint256 executionFee, bool isWithdrawMax) external {
        Account.Props storage accountProps = Account.load(msg.sender);
        if (!isWithdrawMax && accountProps.getTokenAmount(token) < amount) {
            revert Errors.WithdrawWithNoEnoughAmount();
        }
        ...
        request.isWithdrawMax = isWithdrawMax;
    ...
    function executeWithdraw(uint256 requestId, Withdraw.Request memory request) external {
        withdraw(
            requestId,
            WithdrawParams(address(0), request.account, request.token, request.amount, request.isWithdrawMax)
        );
    ...
    /// @dev Get withdrawable token amount
    /// @param accountProps Account.Props
    /// @param token  Token address
    /// @return uint256 The token amount (not in USD)
    function getMaxWithdrawableTokenAmount(
        Account.Props storage accountProps,
        address token
    ) public view returns (uint256) {
        uint256 tokenAmount = accountProps.getTokenAmount(token);
        if (tokenAmount == 0) {
            return 0;
        }
        if (_hasCrossUsed(accountProps)) {
            int256 availableValue = AccountProcess.getCrossAvailableValue(accountProps);
            uint256 tokenPrice = OracleProcess.getLatestUsdUintPrice(token, false);

            uint256 availableAmount = CalUtils.usdToToken(
                availableValue.toUint256(),
                TokenUtils.decimals(token),
                tokenPrice
            );
            return CalUtils.min(tokenAmount, availableAmount);
        } else {
            return tokenAmount;
        }
    }
    ...
    /// Case： hasLiability, hasOrder, hasPosition
    ...
    function _getWithdrawAmount(
        ...
        uint256 maxWithdrawableTokenAmount = getMaxWithdrawableTokenAmount(accountProps, params.token);
        if (params.isWithdrawMax) {
            return maxWithdrawableTokenAmount;
        } else if (params.amount > maxWithdrawableTokenAmount) {
            revert Errors.WithdrawWithNoEnoughAmount();
        }
