...
library PositionQueryProcess {
    ...
    function getPositionUnPnl(
        ...
        int pnlInUsd = position.isLong
            ? position.qty.toInt256().mul(computeIndexPrice.sub(position.entryPrice.toInt256())).div(
                position.entryPrice.toInt256()
            )
            : position.qty.toInt256().mul(position.entryPrice.toInt256().sub(computeIndexPrice)).div(
                position.entryPrice.toInt256()
            );
        int256 marginTokenPrice = OracleProcess.getIntOraclePrices(oracles, position.marginToken, position.isLong);
        return (
    ...
    function isLiquidation(Position.Props storage position, uint256 indexPrice) external view returns (bool) {
    ...
    function getLiquidationPrice(Position.Props storage position) public view returns (uint256) {
