...
library GasProcess {
    ...
    uint256 private constant BASE_GAS_LIMIT = 500_000;
    ...
    function processExecutionFee(PayExecutionFeeParams memory cache) external {
        ...
        uint256 executionFee = (BASE_GAS_LIMIT + usedGas) * tx.gasprice;
        uint256 refundFee;
        uint256 lossFee;
        ...
        VaultProcess.transferOut(cache.from, AppConfig.getChainConfig().wrapperToken, cache.keeper, executionFee);

        if (refundFee > 0) {
            /// @dev Refunds the remaining fee to the user's account
            VaultProcess.transferOut(cache.from, AppConfig.getChainConfig().wrapperToken, cache.account, refundFee);
        }
