...
library Account {
    ...
    function addToken(Props storage self, address token, uint256 amount, UpdateSource source) public {
        ...
        CommonData.load().addTradeTokenCollateral(token, amount);
    ...
    function subToken(Props storage self, address token, uint256 amount, UpdateSource source) public {
        ...
        CommonData.load().subTradeTokenCollateral(token, amount);
    ...
    function subTokenWithLiability(
        ...
        CommonData.load().addTradeTokenLiability(token, liability);
        CommonData.load().subTradeTokenCollateral(token, amount - liability);
        emit AccountTokenUpdateEvent(self.owner, token, preBalance, balance, source);
    }
    ...
    function repayLiability(
        ...
        if (balance.liability > 0 && balance.amount > 0) {
            ...
            CommonData.load().subTradeTokenLiability(token, repayAmount);
            emit AccountTokenUpdateEvent(self.owner, token, preBalance, balance, source);
        }
    }
    ...
    function clearLiability(Props storage self, address token, UpdateSource source) public {
        TokenBalance storage balance = self.tokenBalances[token];
        require(balance.usedAmount >= balance.liability, "usedAmount must bigger than liability");
        TokenBalance memory preBalance = balance;
    ...
    function getBorrowingAmount(Props storage self, address token) external view returns (uint256) {
        TokenBalance storage tokenBalance = self.tokenBalances[token];
        if (tokenBalance.usedAmount - tokenBalance.liability <= tokenBalance.amount) {
            return 0;
        }
        return tokenBalance.usedAmount - tokenBalance.liability - tokenBalance.amount;
    }
}
