...
contract Strategy is ContractGuard {
    ...
    function zapStrategy(address from, uint256 amount, uint256 percentEmpLP) external returns (uint256, uint256) {
        ...
        if (_checkMin(from, amountEmpPair)) {
            zapper.zapTokenToLP(from, amountEmpPair, address(empPair));
        } else {
            IERC20(from).safeTransfer(msg.sender, amountEmpPair);
        }
        if (_checkMin(from, amountEsharePair)) {
            zapper.zapTokenToLP(from, amountEsharePair, address(esharePair));
        } else {
