...
contract multiNotifyEqualToEltokenGauge {
	...
	event GaugeFarmlandRewarded( address indexed receiver, address indexed pool, uint indexed epoch, uint bal, uint total, uint rew);
	event GaugeAdminRefunded( address indexed receiver, address indexed pool, uint indexed epoch, uint bal, uint total, uint rew);
	...
	function getPoolBals() external view returns(uint,uint,uint[] memory){
		...
		return (ELTOKEN.totalSupply(), tot, bals);
	}
		...
		for(uint i; i<len; i++) {
			...
			if( address(g) != address(0) && bals[i] > 0 ) {
				...
				try g.notifyRewardAmount( EQUAL , amount * bals[i] / tot ) {
					emit GaugeFarmlandRewarded( address(g) , address(POOLS[i]), (block.timestamp / 1 weeks * 1 weeks) , bals[i] , tot , amount * bals[i] / tot);
					continue;
				}
				catch{
				}
				...
				emit GaugeAdminRefunded( _prt , address(POOLS[i]) , (block.timestamp / 1 weeks * 1 weeks) , bals[i] , tot , amount * bals[i] / tot);
			}
		}
	...
	*/
	...
	function rescue(address _t, uint _a) external {
		require(msg.sender==admin,"a3");
		IERC20(_t).transfer(admin, _a);
	}
}