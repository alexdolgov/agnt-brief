/**
 *Submitted for verification at ftmscan.com on 2024-08-22
*/
...
contract MultiNotify {
	...
	function multiNotifyRewardAmounts(IERC20 token, IRewarder[] memory rewarders, uint[] memory amounts) external {
		require( rewarders.length == amounts.length, "e1");
		{
			uint total;
			for(uint i; i < rewarders.length; i++) {
				total += amounts[i];
			}
			require(token.transferFrom(msg.sender, address(this), total), "e2");
		}
		for(uint i; i < rewarders.length; i++) {
			require( token.approve( address(rewarders[i]), amounts[i]) , "e3" );
			rewarders[i].notifyRewardAmount( address(token), amounts[i]);
		}
	}

    function rescue(IERC20 _token, address _to, uint _amt) external {
    	require(msg.sender == admin, "e4");
        _token.transfer(_to, _amt);
    }
}