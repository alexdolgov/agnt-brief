...
contract StakingPool is IStakingPool, AccessControlUpgradeable {
    ...
    function initialize(
        ...
        require(_lockingInfo != address(0), "StakingPool: invalid locking info");
    ...
    function bindSequencer(address _signer, uint256 _amount, bytes calldata _signerPubKey) external onlyStakingManager {
        ...
        require(_amount >= lockingInfo.minLock() && _amount <= lockingInfo.maxLock(), "StakingPool: invalid amount");
        ...
        lockingPool.lockWithRewardRecipient(_signer, rewardRecipient, _amount, _signerPubKey);
    ...
    function claimRewards(uint32 _l2GasLimit) external payable onlyStakingManager {
