...
contract FbfChef is OwnableUpgradeSafe {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        //   pending reward = (user.amount * pool.accFbfPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accFbfPerShare` (and `lastRewardTime`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 accFbfPerShare; // Accumulated HOPEs per share, times 1e18. See below.
    ...
    event LogUpdatePool(uint256 indexed pid, uint256 lastRewardTime, uint256 lpSupply, uint256 accFbfPerShare);
    ...
    function initialize(
        ...
        // staking pool
        poolInfo.push(PoolInfo({lpToken: IERC20(_hope), allocPoint: 0, lastRewardTime: _startTime, accFbfPerShare: 0, isStarted: false, startTime: _startTime}));
    ...
    function add(
        ...
        poolInfo.push(PoolInfo({lpToken: _lpToken, allocPoint: _allocPoint, lastRewardTime: _lastRewardTime, accFbfPerShare: 0, isStarted: _isStarted, startTime: _lastRewardTime}));
    ...
    function pendingReward(uint256 _pid, address _user) external view returns (uint256) {
        ...
        uint256 accFbfPerShare = pool.accFbfPerShare;
        ...
        if (now > pool.lastRewardTime && lpSupply != 0) {
            ...
            if (totalAllocPoint > 0) {
                ...
                accFbfPerShare = accFbfPerShare.add(_hopeReward.mul(1e18).div(lpSupply));
            }
        }
        return user.amount.mul(accFbfPerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (totalAllocPoint > 0) {
            uint256 _time = now.sub(pool.lastRewardTime);
            uint256 _hopeReward = _time.mul(rewardPerSecond).mul(pool.allocPoint).div(totalAllocPoint);
            pool.accFbfPerShare = pool.accFbfPerShare.add(_hopeReward.mul(1e18).div(lpSupply));
        }
        pool.lastRewardTime = now;
        emit LogUpdatePool(_pid, pool.lastRewardTime, lpSupply, pool.accFbfPerShare);
    }
    ...
    function _harvestReward(uint256 _pid, address _account) internal {
        ...
        if (user.amount > 0) {
            PoolInfo storage pool = poolInfo[_pid];
            _claimableAmount = user.amount.mul(pool.accFbfPerShare).div(1e18).sub(user.rewardDebt);
            if (_claimableAmount > 0) {
                _topupReserveAndDevFund(_claimableAmount);

                _safeFbfMint(address(this), _claimableAmount);
                ...
                _safeFbfTransfer(_account, _claimableAmount);
            }
        }
        IRewarder _rewarder = rewarder[_pid];
        if (address(_rewarder) != address(0)) {
            _rewarder.onFbfReward(_pid, msg.sender, _account, _claimableAmount, user.amount);
        }
    }
    ...
    function _sendCommission(address _account, uint256 _commission) internal {
        ...
        if (_referrer != address(0)) {
            ...
            _safeFbfTransfer(_referrer, _commission);
            emit Commission(_account, _referrer, _commission);
        } else {
            // or burn
            _safeFbfBurn(_commission);
    ...
    function depositWithRef(
        ...
        user.rewardDebt = user.amount.mul(pool.accFbfPerShare).div(1e18);
    ...
    function _safeFbfTransfer(address _to, uint256 _amount) internal {
    ...
    function _safeFbfMint(address _to, uint256 _amount) internal {
        address _hope = hope;
    ...
    function _safeFbfBurn(uint256 _amount) internal {
        uint256 _hopeBal = IERC20(hope).balanceOf(address(this));
        if (_hopeBal > 0) {
            if (_amount > _hopeBal) {
    ...
    function emergencyWithdraw(uint256 _pid) external lock checkHalving {
        ...
        IRewarder _rewarder = rewarder[_pid];
        if (address(_rewarder) != address(0)) {
            _rewarder.onFbfReward(_pid, msg.sender, msg.sender, 0, 0);
        }
        if (reserveFund != address(0) && now < unfrozenDepositTime(_pid, msg.sender)) {
            uint256 _earlyWithdrawFee = poolEarlyWithdrawFee[_pid];
            if (_earlyWithdrawFee > 0) {
