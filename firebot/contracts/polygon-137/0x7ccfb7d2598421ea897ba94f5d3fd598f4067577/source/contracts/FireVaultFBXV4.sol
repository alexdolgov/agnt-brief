...
interface IFireBotTokenV6 {
	function transferFrom(address sender, address recipient, uint256 amount) external;
    function balanceOf(address account) external view returns(uint256);
}

interface IFireBotItemsV4 {
	function balanceOf(address account, uint256 id) external view returns(uint256);
    function totalSupply(uint256 id) external view returns(uint256);
}

contract FireVaultFBXV4 is ERC20, ERC20Burnable, Ownable, ERC20Permit {
    
    IFireBotTokenV6 public FBX = IFireBotTokenV6(0xD125443F38A69d776177c2B9c041f462936F8218);
    IFireBotItemsV4 public items = IFireBotItemsV4(0x2e14520C30370d114612552616964a3bCeD6176E);
    ...
    function get_fireFBX_price() public view returns (uint256) { // bigint
        ...
        return 1e18 * vault_valuation / (totalSupply() + get_pending_fees());
    }
    ...
    function get_pending_fees() public view returns (uint256) { // bigint
        uint256 elapsed_time = block.timestamp - last_fee_collection;
        return totalSupply() * elapsed_time * daily_fee / 1e18 / 86400;
    }

    function collect_pending_fee() public {
		uint256 to_mint = get_pending_fees();
		last_fee_collection = block.timestamp;
        _mint(vault_address, to_mint);
    }
}