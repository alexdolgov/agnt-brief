...
contract ftYieldWrapperV2 is IftYieldWrapperV2, ERC20, ReentrancyGuardTransient, ProtectedContract {
        ...
        returns (uint256[] memory queueIds)
    {
        ...
        // Use a running value that decrements after each leg so subsequent CB checks
        // see TVL that reflects value already committed to leave.
        uint256 runningPreTvl = _getTvl(token);
            ...
            try strategies[i].withdrawUnderlying(toExit) returns (uint256 got) {
                if (got != 0) {
                    ...
                    uint256 queueId = _protectedTransferWithTvlAndLimiter(
                        positionToken, to, got, runningPreTvl, token, limiterAmount
                    );
                    runningPreTvl =
                        limiterAmount >= runningPreTvl ? 0 : runningPreTvl - limiterAmount;
