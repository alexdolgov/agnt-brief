...
import "./interfaces/IStakedAvantCoin.sol";

contract StakedAvantCoin is SingleAdminAccessControl, ReentrancyGuard, ERC20Permit, ERC4626, IStakedAvantCoin {
  ...
  uint256 private constant MIN_SHARES = 0.00001 ether;
  ...
  /**
   * @param _asset The address of the avantCoin token.
   ...
   * Note that the owner cannot rescue avantCoin tokens because they functionally sit here
   * and belong to stakers but can rescue staked avantCoin as they should never actually
  ...
  function redistributeLockedAmount(address from, address to) external nonReentrant onlyRole(DEFAULT_ADMIN_ROLE) {
    if (hasRole(FULL_RESTRICTED_STAKER_ROLE, from) && !hasRole(FULL_RESTRICTED_STAKER_ROLE, to)) {
      ...
      uint256 avantCoinToVest = previewRedeem(amountToDistribute);
      _burn(from, amountToDistribute);
      // to address of address(0) enables burning
      if (to == address(0)) {
        _updateVestingAmount(avantCoinToVest);
      } else {
  ...
  /**
   * @notice Returns the amount of avantCoin tokens that are vested in the contract.
   */
  ...
  /**
   * @notice Returns the amount of avantCoin tokens that are unvested in the contract.
   */
