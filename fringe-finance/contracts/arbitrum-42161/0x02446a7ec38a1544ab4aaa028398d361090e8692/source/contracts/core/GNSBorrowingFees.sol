...
contract GNSBorrowingFees is Initializable, IGNSBorrowingFees {
    ...
    uint256 private constant P_2 = 1e40;
    ...
    function initialize(IGNSTradingStorage _storageT, IGNSPairInfos _pairInfos) external initializer {
        if (!(address(_storageT) != address(0) && address(_pairInfos) != address(0))) {
            revert WrongParams();
        }
    ...
    function initializeV2(uint48 _windowsDuration) external reinitializer(2) {
        ...
        uint256 storageSlot;
        assembly {
            storageSlot := oiWindowsStorage.slot
        }
    ...
    // Setters
    function setPriceImpactWindowsCount(uint48 _newWindowsCount) external onlyGov {
        PriceImpactUtils.setPriceImpactWindowsCount(_newWindowsCount);
    }

    function setPriceImpactWindowsDuration(uint48 _newWindowsDuration) external onlyGov {
