...
library TradingStorageUtils {
    ...
    function initializeTradingStorage(
        ...
        if (_collaterals.length < 1) revert ITradingStorageUtils.MissingCollaterals();
    ...
    function storeTrade(
        ...
        if (_trade.tradeType == ITradingStorage.TradeType.TRADE) TradingCommonUtils.updateOiTrade(_trade, true, false);
    ...
    function updateTradePosition(
        ...
        if (_isPartialIncrease) {
            s.tradeLiquidationParams[_tradeId.user][_tradeId.index] = _getMultiCollatDiamond().getPairLiquidationParams(
                t.pairIndex
            );
    ...
    function closeTrade(ITradingStorage.Id memory _tradeId, bool _isPnlPositive) external {
        ...
        if (t.tradeType == ITradingStorage.TradeType.TRADE) TradingCommonUtils.updateOiTrade(t, false, _isPnlPositive);
