	...
	IERC1363
{
	...
	/**
	 * @dev Indicates a failure with the token `receiver` due to an invalid retval. Used in transfers.
	 * @param receiver Address to which tokens are being transferred.
	 */
	error ERC1363InvalidReceiverRetVal(address receiver);

	/**
	 * @dev Indicates a failure when the `receiver` is not a contract
	 * @param receiver Address to which tokens are being transferred.
	 */
	...
	/**
	 * @dev Indicates a failure with the token `spender` due to wrong implementation of retval
	 * @param spender Address that may be allowed to operate on tokens without being their owner.
	 */
	error ERC1363InvalidSpenderRetVal(address spender);
	...
	function _checkOnTransferReceived(
		...
		returns (bytes4 retval) {
			if (retval != IERC1363Receiver.onTransferReceived.selector) {
				revert ERC1363InvalidReceiverRetVal(to);
			}
	...
	function _checkOnApprovalReceived(
		...
		returns (bytes4 retval) {
			if (retval != IERC1363Spender.onApprovalReceived.selector) {
				revert ERC1363InvalidSpenderRetVal(spender);
			}
		} catch (bytes memory reason) {
			if (reason.length == 0) {
