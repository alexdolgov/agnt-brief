...
library PriceImpactUtils {
    ...
    function setProtectionCloseFactorBlocks(
        ...
        for (uint256 i = 0; i < _protectionCloseFactorBlocks.length; ++i) {
            s.pairFactors[_pairIndices[i]].protectionCloseFactorBlocks = _protectionCloseFactorBlocks[i];

            emit IPriceImpactUtils.ProtectionCloseFactorBlocksUpdated(_pairIndices[i], _protectionCloseFactorBlocks[i]);
        }
    }
        ...
        )
    {
        ...
        (priceImpactP, priceAfterImpact) = _getTradePriceImpact(
            ...
            _isPnlPositive &&
                ...
                ChainUtils.getBlockNumber() <= _lastPosIncreaseBlock + pairFactors.protectionCloseFactorBlocks
