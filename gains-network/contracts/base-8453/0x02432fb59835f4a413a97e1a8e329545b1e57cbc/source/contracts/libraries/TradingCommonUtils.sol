...
library TradingCommonUtils {
    ...
    function getTradeValuePure(
        ...
        uint256 collateralLiqThreshold = (_collateral * (100 * ConstantsUtils.P_10 - _liqPnlThresholdP)) /
            100 /
            ConstantsUtils.P_10;
    ...
    function getTradeOpeningPriceImpact(
        ...
        (priceImpactP, priceAfterImpact) = _getMultiCollatDiamond().getTradePriceImpact(
            getMarketExecutionPrice(_input.marketPrice, _input.spreadP, trade.long, true, _contractsVersion),
            trade.pairIndex,
            trade.long,
    ...
    /**
     * @dev Adds GNS staking fee to OTC balance to be distributed once OTC is executed
     * @param _collateralIndex collateral index
     * @param _trader trader address
     ...
     */
    function updateOi(ITradingStorage.Trade memory _trade, uint256 _positionSizeCollateral, bool _open) public {
        ...
        _getMultiCollatDiamond().addPriceImpactOpenInterest(_trade.user, _trade.index, _positionSizeCollateral, _open);
    }
    ...
    function updateOiTrade(ITradingStorage.Trade memory _trade, bool _open) external {
        updateOi(_trade, getPositionSizeCollateral(_trade.collateralAmount, _trade.leverage), _open);
    }
