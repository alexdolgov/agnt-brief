...
// File: contracts\GFarmPriceAggregator.sol
...
interface TradingInterfaceV3{
    function openTradeCallback(uint _order, uint _price, uint _spreadP) external;
    function closeTradeCallback(uint _order, uint _price) external;
    function liquidateTradeCallback(uint _order, uint _price) external;
}

contract GFarmPriceAggregator is ChainlinkClient {
    ...
    struct Request{
        uint orderId;
        uint orderType; // 0: open, 1: close, 2: liquidate
        uint pairIndex;
        bool initiated;
    }
    ...
    function setTrading(address _trading) external onlyGov{
        require(_trading != address(0), "INTERFACE_0");
        trading = TradingInterfaceV3(_trading);
    ...
    function setMinAnswers(uint _minAnswers) external onlyGov{
        ...
        emit MinAnswersUpdated(_minAnswers);
    }
    ...
    function addNode(address _a, uint _fee) external onlyGov{
        require(nodes.length < 5, "5_NODES_ALREADY");
    ...
    function addPair(
        ...
        string calldata _to,
        bytes32[5] calldata _jobs,
        uint _spreadP,
        uint _maxOpenInterestToken
    ) external onlyGov{

        require(pairs[_index].jobs[0] == bytes32(0), "PAIR_INDEX_TAKEN");
        Pair storage p = pairs[_index];
        p.from = _from;
        p.to = _to;
        p.jobs = _jobs;
        p.spreadP = _spreadP;
    ...
    function removePair(uint _index) external onlyGov{
        emit PairRemoved(pairs[_index].from, pairs[_index].to);
        delete pairs[_index];
    }

    function pairJobs(uint _pairIndex) external view returns(bytes32[5] memory){
        return pairs[_pairIndex].jobs;
    }

    function updateSpread(uint _index, uint _spreadP) external onlyGov{
        require(pairs[_index].jobs[0] != bytes32(0), "PAIR_NOT_LISTED");
        pairs[_index].spreadP = _spreadP;
    }

    function updateJob(uint _pairIndex, uint _jobIndex, bytes32 _newJob) external onlyGov{
        require(_newJob != bytes32(0), "EMPTY_JOB");
    ...
    function updateMaxOpenInterest(uint _pairIndex, uint _newMaxOpenInterest) external onlyGov{
        require(_newMaxOpenInterest > 0, "MAX_OPEN_INTEREST_0");
    ...
    function fulfill(bytes32 _requestId, uint _price) external recordChainlinkFulfillment(_requestId){
        ...
        if(answers.length == minAnswers){
            uint price = uint(Median.calculate(answers));
            if(r.orderType == 0){
                trading.openTradeCallback(r.orderId, price, pairs[r.pairIndex].spreadP);
            }else if(r.orderType == 1){
                trading.closeTradeCallback(r.orderId, price);
            }else if(r.orderType == 2){
                trading.liquidateTradeCallback(r.orderId, price);
            }
        }
