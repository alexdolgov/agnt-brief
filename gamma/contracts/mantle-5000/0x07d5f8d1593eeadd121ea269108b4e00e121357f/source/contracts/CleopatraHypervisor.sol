...
contract CleopatraHypervisor is IRamsesV2MintCallback, ERC20Permit, ReentrancyGuard {
   ...
   event Deposit(
    ...
    function _zeroBurn(int24 tickLower, int24 tickUpper) internal returns(uint128 liquidity) {
      ...
      if(liquidity > 0) {
        pool.burn(uint256(0), tickLower, tickUpper, 0, veNFTTokenId);
    ...
    function transferVeNFTTokenId(uint256 newVeNFTTokenId) external onlyOwner {
        veNFTTokenId = newVeNFTTokenId;
    }    
