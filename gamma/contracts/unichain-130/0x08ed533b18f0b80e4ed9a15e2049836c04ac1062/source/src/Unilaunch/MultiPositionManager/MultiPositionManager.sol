...
contract MultiPositionManager is IMultiPositionManager, ERC20, ReentrancyGuard, Ownable, SafeCallback, Multicall {
    ...
    function rebalance(
        ...
        _enforceRebalanceParams(params);
        // First call ZERO_BURN to collect fees
    ...
    function _enforceRebalanceParams(IMultiPositionManager.RebalanceParams calldata params) private view {
        ...
        if (isFiftyFifty && params.limitWidth == 0) revert InvalidRebalanceParams();
        ...
        if (factoryContract.requiresLimitPosition(resolvedStrategy)) {
            if (!isFiftyFifty) revert InvalidRebalanceParams();
            if (params.limitWidth == 0) revert InvalidRebalanceParams();
        }
        ...
        if (maxDelta != 0) {
            ...
            if (block.timestamp >= latestTimestamp + twapSeconds) {
                ...
                int24 resolvedCenter = params.center;
                if (resolvedCenter == type(int24).max) {
                    (, int24 slotTick,,) = poolManager.getSlot0(s.poolKey.toId());
                    int24 tickSpacing = s.poolKey.tickSpacing;
                    int24 compressed = slotTick / tickSpacing;
                    if (slotTick < 0 && slotTick % tickSpacing != 0) {
                        compressed--;
                    }
                    resolvedCenter = compressed * tickSpacing;
                }
                int256 delta = int256(resolvedCenter) - int256(twapTick);
                if (delta < 0) {
                    delta = -delta;
                }
