...
/**
 * @title UnilaunchInitialDepositLens
 * @notice Read-only contract for previewing initial deposits to pools for UnilaunchMultiPositionManager
 * @dev Adapted from InitialDepositLens - supports both initialized and uninitialized pools
 *
 * Key constraints (enforced from UnilaunchMultiPositionManager):
 ...
 */
contract UnilaunchInitialDepositLens {
    ...
    struct RebalancePreview {
        ...
        uint128[] liquidities;
        UnilaunchLensRatioUtils.PositionStats[] expectedPositions;
        ...
        returns (UnilaunchLensRatioUtils.PositionStats[] memory stats)
    {
        ...
        stats = new UnilaunchLensRatioUtils.PositionStats[](ranges.length);
        ...
        for (uint256 i = 0; i < ranges.length; i++) {
            ...
            stats[i] = UnilaunchLensRatioUtils.PositionStats({
    ...
    function _calculateSwapParams(
        ...
        uint160 sqrtPriceX96,
        InitialDepositWithSwapParams calldata params
    ) private view returns (UnilaunchLensRatioUtils.SwapParams memory swapParams, uint256 finalAmount0, uint256 finalAmount1) {
    ...
    function _calculateExpectedTotals(uint160 sqrtPriceX96, RebalancePreview memory preview, uint24 limitWidth)
        ...
        pure
    {
        preview.expectedPositions = new UnilaunchLensRatioUtils.PositionStats[](preview.ranges.length);
        ...
        for (uint256 i = 0; i < preview.ranges.length; i++) {
            ...
            preview.expectedPositions[i] = UnilaunchLensRatioUtils.PositionStats({
