...
library RebalanceLogic {
    ...
    function calculateWeightsFromStrategy(
        ...
        for (uint256 i = 0; i < lowerTicks.length; i++) {
            ...
            totalAmount0 += amount0For1e18;
            totalAmount1 += amount1For1e18;
        }
     ...
     * @param weight1 Target weight for token1 (in 1e18)
    ...
    function rebalanceSwap(
        ...
        (uint256 amount0, uint256 amount1) = _burnForSwap(s, poolManager, outMin, totalSupply);
        (amount0, amount1) = _performSwapIfNeeded(s, poolManager, params, aggregator, swapData, minAmountOut, amount0, amount1, address(this));

        return _processRebalanceResult(s, poolManager, params, amount0, amount1, inMin);
    }

    function _burnForSwap(
        SharedStructs.ManagerStorage storage s,
        IPoolManager poolManager,
        uint256[2][] memory outMin,
        uint256 totalSupply
        ...
        if (totalSupply > 0 && (s.basePositionsLength > 0 || s.limitPositionsLength > 0)) {
            return PositionLogic.burnLiquidities(poolManager, s, totalSupply, totalSupply, outMin);
        }
        return (s.currency0.balanceOfSelf(), s.currency1.balanceOfSelf());
    }

    function _performSwapIfNeeded(
        SharedStructs.ManagerStorage storage s,
        IPoolManager poolManager,
        IMultiPositionManager.RebalanceParams calldata params,
        address aggregator,
        ...
        (, bool swapToken0, uint256 swapAmount) = _calculateSwapParams(s, poolManager, params, amount0, amount1);

        if (swapAmount == 0) return (amount0, amount1);

        uint256 amountOut = _executeSwap(s, aggregator, swapData, minAmountOut, swapToken0, swapAmount, manager);
    ...
    function _calculateSwapParams(
        ...
        uint256 amount0,
        uint256 amount1
    ) private view returns (uint160 sqrtPrice, bool swapToken0, uint256 swapAmount) {
        (sqrtPrice, , , ) = poolManager.getSlot0(s.poolKey.toId());
        ...
        uint256 w1;
        {
            ...
            if (w0 == 0 && w1 == 0) {
                (w0, w1) = calculateWeightsFromStrategy(
                    ILiquidityStrategy(params.strategy),
                    params.center,
                    params.tLeft,
                    params.tRight,
                    s.poolKey.tickSpacing,
            ...
            if (w0 + w1 != 1e18) revert InvalidWeightSum();
        }

        (swapToken0, swapAmount) = calculateOptimalSwap(amount0, amount1, sqrtPrice, w0, w1);
    }

    /**
     * @notice Helper function to burn positions and get amounts
     * @dev Separated to avoid stack too deep
     */
    function _burnAndGetAmounts(
        SharedStructs.ManagerStorage storage s,
        IPoolManager poolManager,
        uint256[2][] memory outMin,
        uint256 totalSupply
        ...
        if (totalSupply > 0 && (s.basePositionsLength > 0 || s.limitPositionsLength > 0)) {
            (amount0, amount1) = PositionLogic.burnLiquidities(poolManager, s, totalSupply, totalSupply, outMin);
        } else {
     ...
     */
    function _processRebalanceResult(
        SharedStructs.ManagerStorage storage s,
        IPoolManager poolManager,
        IMultiPositionManager.RebalanceParams calldata params,
        uint256 amount0,
