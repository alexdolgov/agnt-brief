...
import {TokenLaunchFactory} from "../LaunchPad/TokenLaunchFactory.sol";
import {CallOption} from "../LaunchPad/CallOption.sol";
...
contract DelegatedAccount is EIP712 {
        ...
        view
    {
        ...
        if (selector == CallOption.exercise.selector) {
            uint256 tokenAmount = abi.decode(data[4:], (uint256));
            _enforceMaxAmount(permission.maxAmount, tokenAmount);
            return;
        }
        if (selector == TokenLaunchFactory.deployTokenAndLaunchPresale.selector) {
            return;
        }
