...
import {IPool, IPoolV2} from "@src/Interfaces/IPool.sol";
...
/// @dev This is an upgraded contract from Pool.
/// The contact does not inherit from Pool, however, as the methods in the original have not been marked virtual.
/// @custom:oz-upgrades-from Pool
contract PoolV2 is
    Initializable,
    PoolStore,
    ERC20Upgradeable,
    ...
    IPoolV2
{
    ...
    function initializeV2() public virtual reinitializer(2) {
        ...
        // set the exit reserve percentage to 10% and a 5% exit fee by default
        $.exitReservePercentage = 0.1e18;
        $.exitFeePercentage = 0.05e18;
    }
        ...
        returns (uint256 shares)
    {
        ...
        if (assets > maxAssets) {
            revert ERC4626.ERC4626ExceededMaxWithdraw(owner, assets, maxAssets);
        }
        ...
        returns (uint256 assets)
    {
        ...
        if (shares > maxShares) {
            revert ERC4626.ERC4626ExceededMaxRedeem(owner, shares, maxShares);
        }
    ...
    /// @inheritdoc IPool
    function previewDeposit(uint256 assets) public view virtual whenNotPaused returns (uint256) {
        return convertToShares(assets);
    }

    /// @inheritdoc IPool
    function previewMint(uint256 shares) public view virtual whenNotPaused returns (uint256) {
    ...
    function previewWithdraw(uint256 assets) public view returns (uint256) {
        PoolStorage storage $ = _getStorage();
        uint256 fee = assets.mulWadUp($.exitFeePercentage);
        return _convertToSharesUp(assets + fee);
    }
    ...
    function previewRedeem(uint256 shares) public view virtual returns (uint256) {
        ...
        uint256 feePercentage = $.exitFeePercentage;
        uint256 fee = assets.mulDivUp(feePercentage, feePercentage + FixedPointMathLib.WAD);
        return assets - fee;
    }
    ...
    /// @inheritdoc IPoolV2
    function exitReserve() public view virtual returns (uint256) {
        PoolStorage storage $ = _getStorage();
        uint256 totalAssets_ = totalAssets();
        uint256 reserveToUse = totalAssets_.mulWadDown($.exitReservePercentage);
        return reserveToUse;
    }

    /// @inheritdoc IPoolV2
    function maxUnboundedWithdraw(address owner) public view virtual returns (uint256) {
        if (paused()) {
            return 0;
        }
    ...
    /// @inheritdoc IPoolV2
    function maxUnboundedRedeem(address owner) public view virtual returns (uint256) {
        if (paused()) {
            return 0;
        }
    ...
    function maxWithdraw(address owner) public view virtual returns (uint256) {
        PoolStorage storage $ = _getStorage();
        uint256 balanceOfContract = $.asset.balanceOf(address(this));
        uint256 withdrawableAssets = maxUnboundedWithdraw(owner);
        uint256 maxWithdrawWithoutFee = Math.min(balanceOfContract, withdrawableAssets);
        return maxWithdrawWithoutFee.divWadDown(FixedPointMathLib.WAD + $.exitFeePercentage);
    }

    /// @inheritdoc IPool
    function maxRedeem(address owner) public view virtual returns (uint256) {
        PoolStorage storage $ = _getStorage();
        uint256 sharesOfContract = convertToShares($.asset.balanceOf(address(this)));
        uint256 redeemableShares = maxUnboundedRedeem(owner);
        return Math.min(sharesOfContract, redeemableShares);
    }
    ...
    function flushDeposits() external virtual whenNotPaused {
        ...
        uint256 exitReserve_ = exitReserve();
    ...
    /// @inheritdoc IPool
    function claimRewards(uint256[] calldata rewardIdxs) external virtual {
        PoolStorage storage $ = _getStorage();
    ...
    /// @inheritdoc IPool
    function initializeRewardClaims(DelegationPosition[] calldata positions) external virtual {
        PoolStorage storage $ = _getStorage();
    ...
    /// @inheritdoc IPool
    function icnp() external view virtual returns (IICNProtocolMini) {
        PoolStorage storage $ = _getStorage();
        return $.icnp;
    }
    ...
    /// @inheritdoc IPool
    function rewardHandler() external view virtual returns (address) {
        PoolStorage storage $ = _getStorage();
        return address($.rewardHandler);
    }

    /// @inheritdoc IPool
    function portfolioConfig() external view virtual returns (IPool.VaultConfig[] memory) {
        PoolStorage storage $ = _getStorage();
        return $.portfolioConfig;
    }

    /// @inheritdoc IPool
    function scalarNodeIDs() external view virtual returns (uint256[] memory) {
        PoolStorage storage $ = _getStorage();
    ...
    /// @inheritdoc IPool
    function totalPositions() external view virtual returns (uint256) {
        PoolStorage storage $ = _getStorage();
        return $._totalPositions;
    }

    /// @inheritdoc IPool
    function totalOutstandingRewardClaims() external view virtual returns (uint256) {
        PoolStorage storage $ = _getStorage();
        return $._totalOutstandingRewardClaims;
    }

    /// @inheritdoc IPool
    function maxOutstandingRewardClaims() external view virtual returns (uint256) {
        PoolStorage storage $ = _getStorage();
        return $.maxOutstandingRewardClaims;
    }

    /// @inheritdoc IPool
    function minPortfolioDelegationSize() external view virtual returns (uint256) {
        PoolStorage storage $ = _getStorage();
        return $.minPortfolioDelegationSize;
    }

    /// @inheritdoc IPool
    function maxPortfolioDelegationSize() external view virtual returns (uint256) {
        PoolStorage storage $ = _getStorage();
        return $.maxPortfolioDelegationSize;
    }

    /// @inheritdoc IPool
    function triggerInitiateRewardClaimAbs() external view virtual returns (uint256) {
        PoolStorage storage $ = _getStorage();
    ...
    /// @inheritdoc IPool
    function pendingLPRewards() external view virtual returns (uint256) {
        PoolStorage storage $ = _getStorage();
        return $._pendingLPRewards;
    }

    /// @inheritdoc IPool
    function claimedLPRewards() external view virtual returns (uint256) {
        PoolStorage storage $ = _getStorage();
        return $._claimedLPRewards;
    }

    /// @inheritdoc IPool
    function emergencyWithdrawnAssets() external view virtual returns (uint256) {
        PoolStorage storage $ = _getStorage();
        return $._emergencyWithdrawnAssets;
    }

    /// @inheritdoc IPool
    function lastTimeStampCheckpoint() external view virtual returns (uint256) {
        PoolStorage storage $ = _getStorage();
        return $._lastTimeStampCheckpoint;
    }

    /// @inheritdoc IPool
    function delegationThrottleSeconds() external view virtual returns (uint256) {
        PoolStorage storage $ = _getStorage();
        return $.delegationThrottleSeconds;
    }

    /// @inheritdoc IPool
    function treasuryFee() external view virtual returns (uint256) {
        PoolStorage storage $ = _getStorage();
        return $.treasuryFee;
    }

    /// @inheritdoc IPool
    function restakePercentage() external view virtual returns (uint256) {
        PoolStorage storage $ = _getStorage();
        return $.restakePercentage;
    }

    /// @inheritdoc IPoolV2
    function exitReservePercentage() external view virtual returns (uint256) {
        PoolStorage storage $ = _getStorage();
        return $.exitReservePercentage;
    }

    /// @inheritdoc IPoolV2
    function exitFeePercentage() external view virtual returns (uint256) {
        PoolStorage storage $ = _getStorage();
        return $.exitFeePercentage;
    }

    /// @inheritdoc IPool
    function nextNodeIndexToDelegateTo() external view virtual returns (uint256) {
        PoolStorage storage $ = _getStorage();
        return $._nextNodeIndexToDelegateTo;
    }

    /// @inheritdoc IPool
    function totalPortfolioWeight() external view virtual returns (uint256) {
        PoolStorage storage $ = _getStorage();
    ...
    /// @inheritdoc IPool
    function setIcnp(IICNProtocolMini newIcnp) external virtual onlyOwner {
    ...
    /// @inheritdoc IPool
    function setRewardHandler(IRewardHandler newRewardHandler) external virtual onlyOwner {
        PoolStorage storage $ = _getStorage();
    ...
    /// @inheritdoc IPool
    function setPortfolioConfig(VaultConfig[] memory newPortfolioConfig) external virtual onlyOwner {
    ...
    function setScalarNodeIDs(uint256[] memory newScalarNodeIDs) external virtual onlyOwner {
    ...
    /// @inheritdoc IPool
    function setMinPortfolioDelegationSize(uint256 newMinPortfolioDelegationSize) external virtual onlyOwner {
        PoolStorage storage $ = _getStorage();
    ...
    /// @inheritdoc IPool
    function setMaxPortfolioDelegationSize(uint256 newMaxPortfolioDelegationSize) external virtual onlyOwner {
    ...
    /// @inheritdoc IPool
    function setDelegationThrottleSeconds(uint256 newDelegationThrottleSeconds) external virtual onlyOwner {
        PoolStorage storage $ = _getStorage();
    ...
    /// @inheritdoc IPool
    function setTriggerInitiateRewardClaimAbs(uint256 newTriggerInitiateRewardClaimAbs) external virtual onlyOwner {
        PoolStorage storage $ = _getStorage();
    ...
    /// @inheritdoc IPool
    function setMaxOutstandingRewardClaims(uint256 newMaxOutstandingRewardClaims) external virtual onlyOwner {
    ...
    /// @inheritdoc IPool
    function setRestakePercentage(uint256 newRestakePercentage) external virtual onlyOwner {
    ...
    /// @inheritdoc IPool
    function setTreasuryFee(uint256 newTreasuryFee) external virtual onlyOwner {
    ...
    /// @inheritdoc IPoolV2
    function setExitReservePercentage(uint256 newExitReservePercentage) external virtual onlyOwner {
        if (newExitReservePercentage > 1e18) revert InvalidParams();
        PoolStorage storage $ = _getStorage();
        uint256 oldExitReservePercentage = $.exitReservePercentage;
        $.exitReservePercentage = newExitReservePercentage;
        emit ExitReservePercentageUpdated(oldExitReservePercentage, newExitReservePercentage);
    }

    /// @inheritdoc IPoolV2
    function setExitFeePercentage(uint256 newExitFeePercentage) external virtual onlyOwner {
        if (newExitFeePercentage > 1e18) revert InvalidParams();
        PoolStorage storage $ = _getStorage();
        uint256 oldExitFeePercentage = $.exitFeePercentage;
        $.exitFeePercentage = newExitFeePercentage;
        emit ExitFeePercentageUpdated(oldExitFeePercentage, newExitFeePercentage);
    }
    ...
    /// @inheritdoc IPool
    function undelegateCollateral(DelegationPosition[] calldata positions) external virtual onlyOwner {
        PoolStorage storage $ = _getStorage();
        IICNProtocolMini icnp_ = $.icnp;
        uint256 treasuryFee_ = $.treasuryFee;
    ...
    /// @inheritdoc IPool
    function withdrawUnlockedDelegatedTokens(DelegationPosition[] calldata positions) external virtual onlyOwner {
        PoolStorage storage $ = _getStorage();
        IICNProtocolMini icnp_ = $.icnp;
    ...
    /// @inheritdoc IPool
    function emergencyWithdraw() external virtual onlyOwner whenPaused {
        PoolStorage storage $ = _getStorage();
        IERC20 asset_ = $.asset;
