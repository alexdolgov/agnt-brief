...
// File contracts_unflattened/token/AerariumMasterChef.sol
...
/*
    This master chef is based on SUSHI's version with some adjustments:
     ...
     - update of emission rate with upper limit of 6 AERA/block
     - more require checks in general
*/

// Have fun reading it. Hopefully it's still bug-free
contract AerariumMasterChef is Ownable {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of AERA
    ...
    // Info of each pool.
    struct PoolInfo {
        // we have a fixed number of AERA tokens released per block, each pool gets his fraction based on the allocPoint
    ...
    uint256 private constant ACC_AERA_PRECISION = 1e12;

    // distribution percentages: a value of 1000 = 100%
    // 10% percentage of pool rewards that goes to the treasury.
    uint256 public TREASURY_PERCENTAGE = 40;

    // 90% percentage of pool rewards that goes to LP holders.
    uint256 public POOL_PERCENTAGE = 960;

    // distribution percentages: a value of 1000 = 100%
    // @notice Fee on deposits per pool. poolId => fee
    mapping(uint256 => uint256) public depositFee;

    // @notice Exemption from Fee on deposits. user => exemption
    mapping(address => bool) public exempt;

    // @notice Receiver of Deposit Fees
    ...
    // The block number when AERA mining starts.
    ...
    event UpdateEmissionRate(address indexed user, uint256 _aeraPerSec);
    ...
    constructor(
        ...
        require(
            _aeraPerBlock <= 6e18,
            "maximum emission rate of 6 aera per block exceeded"
        );
        aera = _aera;
        treasuryAddress = _treasuryAddress;
        feeReceiver = _treasuryAddress;
    ...
    // Update the given pool's AERA allocation point. Can only be called by the owner.
    ...
    // View function to see pending AERA on frontend.
    function pendingAera(uint256 _pid, address _user)
        external
        view
        returns (uint256 pending)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            // based on the pool weight (allocation points) we calculate the aera rewarded for this specific pool
            uint256 aeraRewards = (blocksSinceLastReward *
                aeraPerBlock *
                pool.allocPoint) / totalAllocPoint;

            // we take parts of the rewards for treasury, these can be subject to change, so we recalculate it
            // a value of 1M = 100%
            uint256 aeraRewardsForPool = (aeraRewards * POOL_PERCENTAGE) / 1000;

            // we calculate the new amount of accumulated aera per LP token
            accAeraPerShare =
                accAeraPerShare +
                ((aeraRewardsForPool * ACC_AERA_PRECISION) / lpSupply);
        }
    ...
    function updatePool(uint256 _pid) public returns (PoolInfo memory pool) {
        ...
        if (block.number > pool.lastRewardBlock) {
            ...
            if (lpSupply > 0) {
                ...
                aera.transfer(
                    treasuryAddress,
                    (aeraRewards * TREASURY_PERCENTAGE) / 1000
                );

                //aera.mint(address(this), aeraRewardsForPool);
    ...
    // @notice Backwards-compatible behaviour
    function deposit(
        uint256 _pid,
        uint256 _amount
    ) public {
    ...
    // Deposit LP tokens to MasterChef for AERA allocation for someone.
    function deposit(
        uint256 _pid,
        uint256 _amt,
        address _to
        ...
        if(depositFee[_pid] > 0 && ( exempt[_to] == false && exempt[msg.sender] == false)) {
        	// @notice Take fee to Treasury
        	// @dev variables are scoped to save gas
        	uint256 _fees = ( _amt * depositFee[_pid] ) / 1e6;
        	lpTokens[_pid].safeTransferFrom(msg.sender, feeReceiver, _fees);
        	_input = _amt - _fees;
        }
        else {_input = _amt;}
    ...
    // Harvest AERA + Secondary Rewards.
    // @notice Backwards-compatible behaviour
    function harvest(uint256 _pid) public { harvest(_pid, msg.sender) ; }

    // Multi-Harvest AERA + Secondary Rewards.
    ...
    /// @param _pid The index of the pool. See `poolInfo`.
    /// @param _to Receiver of AERA rewards.
    function harvest(uint256 _pid, address _to) public {
        ...
        uint256 accumulatedAera = (user.amount * pool.accAeraPerShare) /
            ACC_AERA_PRECISION;
        ...
        if (eligibleAera > 0) {
            safeAeraTransfer(_to, eligibleAera);
        }

        IRewarder _rewarder = rewarder[_pid];
        if (address(_rewarder) != address(0)) {
            _rewarder.onAeraReward(
                _pid,
                msg.sender,
                _to,
                eligibleAera,
    ...
    /// @param _to Receiver of the LP tokens and AERA rewards.
    function withdrawAndHarvest(
        ...
        /*
            ...
            => userInfo.amount * pool.accAeraPerShare / ACC_AERA_PRECISION == userInfo.rewardDebt
    ...
    function emergencyWithdraw(uint256 _pid, address _to) public {
        ...
        IRewarder _rewarder = rewarder[_pid];
        if (address(_rewarder) != address(0)) {
            _rewarder.onAeraReward(_pid, msg.sender, _to, 0, 0);
        }
    ...
    // Safe AERA transfer function, just in case if rounding error causes pool to not have enough AERA.
    ...
    function setDepositFee(uint256 _fee, uint256 _pid) public onlyOwner {
        require(
            _fee <= 1337e2,
            "maximum deposit fee of 13.37% (133700/M)"
        );
        depositFee[_pid] = _fee;
    }

    function setDepositFeeExemption(address _who) public onlyOwner {
        require(
            _who != address(0),
            "Please, do not exempt the dead"
        );
    	exempt[_who] = exempt[_who] == true ? false : true;
    }

    function setfeeReceiver(address _who) public onlyOwner {
        require(
            _who != address(0),
            "Please, do not burn the revenue"
        );
    	feeReceiver = _who;
    }

    function setTreasuryShares(uint256 _tshares) public onlyOwner {
        require(
            _tshares <= 1337e2,
            "maximum treasury share of 13.37% (133700/M)"
        );
    	TREASURY_PERCENTAGE = _tshares;
    	POOL_PERCENTAGE = 1000 - TREASURY_PERCENTAGE;
    }
    ...
    function rescueAERA(uint256 _amt) public onlyOwner {
        ...
        aera.transfer(treasuryAddress, _amt);
    }
}