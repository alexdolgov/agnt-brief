...
//	(C) 2020-9999 543#3017, Guru Network
...
// AERA_L1 : Stake AERA to Claim more AERA
interface L1 {
...
contract Granary
{
	...
	constructor (address _aera, address _l1, address _lp)
	{
		want=IERC20(_aera);
		mc=L1(_l1);
		earn=IERC20(_aera);
		id="8100";//GRAIN#ID, required by Granary Registrar.
		utvl=0x50Dcc6cb1B2d6965c42d98a2b07629c57a6be895;
        lpt = _lp;
		//Approvals
	...
	*/
	function name() public pure returns(string memory){return "Staked AERA";}
	function symbol() public pure returns(string memory){return "AES";}
	...
	*/
	...
	uint256[10] public allnums = [
		0,	//pid		0       unused in this implementation
		2e4,//df		1       config, <= 10% (1e5), default 2%
		8e4,//pf		2       config, <= 10% (1e5), default 8%
		1e4,//wi		3       config, <= 10% (1e5), default 1%
		1e10,//mw		4       config, >= 1, default 0.00000001
	...
	function deposit(uint256 _amt) public rg
	{
		...
		mc.stake(D);
		//Some more fancy math to take care of L1 Deposit Fee
		...
		if(allnums[1]>0){//gas savings
			_fee = eliteness(msg.sender)?
			0 : (_mint.mul(allnums[1])).div(1e6);
		}
		if(_fee>0)//gas savings
		{
			//Deposit Fee Mint, conserves TVL
	...
	{
		//@notice Allowed everyone to earn 'worker incentives' in AES.
	...
	function work(address ben) internal
	{
		...
		allnums[7]=allnums[6];allnums[6]=uint64(block.timestamp);//typecasted to 64-bits, gas savings
	...
	function declareEmergency(uint mode, uint amt) public DAO
	{
		require(!emergency,"Emergency already declared.");
		if(mode==0){mc.exit();}
		if(mode==1){mc.withdraw(amt);}
		if(mode==2){mc.withdraw(amt);mc.getReward();}
		emergency=true;
	}
	...
	function rescue(address tokenAddress, uint256 tokens) public DAO returns (bool success)
	{
		...
		//Main token can only be withdrawn using the EmergencyWithdraw, only by depositors.
	...
	function info() public view returns (uint256, uint256, uint256, uint256, uint256, uint256)
	{
		...
		uint256 roi = aum()*1e18/totalSupply;//ROI since inception: 1e18 === 1x
		uint256 rrr = ((roi-1e18)*(365*86400)*100)/(allnums[6]-allnums[5]);//APY: 1e18 === 1%
		return(
			aum(),
			roi,
			rrr,
			mc.balanceOf(address(this)),//working capital
			mc.totalSupply(),			//global dominance
			mc.earned(address(this))	//pending rewards
		);
	}
	function aum() public view returns (uint256)
	{
		return mc.balanceOf(address(this)) + want.balanceOf(address(this));
	}
	...
	function tvl() public view returns(uint256)
	{
		ITVL tc = ITVL(utvl);
		//uint256 aum = mc.userInfo(allnums[0],address(this)).amount + IERC20(want).balanceOf(address(this));
		return ((tc.p_t_coin_usd(lpt)).mul(aum())).div(1e18);
		//return ((tc.p_lpt_coin_usd(address(want))).mul(aum)).div(1e18);
	}

	//Housekeeping
	function setOwner(address _a) public DAO { dao = _a; }
	function setFeeTaker(address _f) public DAO { treasury = _f; }
	function setTvlArgs(address _tg, address _lp) public DAO { utvl = _tg; lpt = _lp; }

    /**
     * Reference Contracts
     * 
     *  0xFE540D6dbAD8C68928778AaF2Be828efA4b44Fa2  AERA
     *  0x63D73d848b7F5142F11974188E9Fe3F5dE28f88c  Aera_Layer1
     *  0x86905a21757a85b476b24512a6b9e8378daf591a  AERA/USDC
...
/*
	...
	Ans	Simply holding 1 Fractal (AeraNFT) in your wallet ascribes you Eliteness.
