...
contract CLVault is ERC20Upgradeable, ERC721HolderUpgradeable, IUpgradeSource, ControllableInit, CLVaultStorage {
  ...
  function setStrategy(address _strategy) public onlyControllerOrGovernance {
    ...
    if (address(_strategy) != address(strategy())) {
      if (address(strategy()) != address(0)) { // if the original strategy (no underscore) is defined
        IStrategy(strategy()).withdrawAllToVault(true);
      }
  ...
  */
  ...
  function withdrawAll(bool compound) public onlyControllerOrGovernance whenStrategyDefined {
    IStrategy(strategy()).withdrawAllToVault(compound);
  }
  ...
  */
  ...
  function rebalanceCurrentTick(uint256 _posWidth) public onlyControllerOrGovernance {
    ...
    if (currentWeight0 > newWeight0) {
      ...
      uint256 toSwap = IERC20Upgradeable(token0()).balanceOf(address(this)).mul(currentWeight0.sub(newWeight0)).div(currentWeight0);
      if (toSwap > 0) {
        _swap(zeroForOne, toSwap);
      }
    } else {
      bool zeroForOne = false;
      uint256 toSwap = IERC20Upgradeable(token1()).balanceOf(address(this)).mul(currentWeight1.sub(newWeight1)).div(currentWeight1);
      if (toSwap > 0) {
        _swap(zeroForOne, toSwap);
      }
    }
