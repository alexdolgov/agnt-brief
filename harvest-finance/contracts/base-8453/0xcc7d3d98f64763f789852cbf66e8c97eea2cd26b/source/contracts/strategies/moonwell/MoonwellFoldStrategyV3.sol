...
contract MoonwellFoldStrategyV3 is BaseUpgradeableStrategy {
  ...
  */
  ...
  function _computeWantedAmounts() internal returns (uint256, uint256, uint256) {
    address _mToken = mToken();
    uint256 borrowAvail;
    uint256 supplyAvail;
    {
      ...
      uint256 totalSupplied = MTokenInterface(_mToken).totalSupply().mul(MTokenInterface(_mToken).exchangeRateCurrent()).div(1e18);
      if (totalSupplied < supplyCap) {
        supplyAvail = supplyCap.sub(totalSupplied).sub(1);
      } else {
        supplyAvail = 0;
      }
    }
    uint256 wantedSupply;
    uint256 wantedBorrow;
    uint256 wantedCash;
    {
      uint256 currentSupplied = MTokenInterface(_mToken).balanceOfUnderlying(address(this));
      uint256 currentBorrowed = MTokenInterface(_mToken).borrowBalanceCurrent(address(this));
      uint256 currentCash = IERC20(underlying()).balanceOf(address(this));
      if (!fold()) {
        wantedBorrow = 0;
        if (currentCash > supplyAvail) {
          wantedSupply = currentSupplied.add(supplyAvail);
          wantedCash = currentCash.sub(supplyAvail);
        } else {
          wantedSupply = currentSupplied.add(currentCash);
          wantedCash = 0;
        }
      } else if (currentCash > supplyAvail) {
        wantedSupply = currentSupplied.add(supplyAvail);
        if (currentBorrowed >= currentCash.sub(supplyAvail)) {
          wantedBorrow = currentBorrowed.sub(currentCash.sub(supplyAvail));
          wantedCash = 0;
        } else {
          wantedBorrow = 0;
          wantedCash = currentCash.sub(supplyAvail).sub(currentBorrowed);
        }
      } else {
        wantedCash = 0;
        uint256 currentBalance = currentCash.add(currentSupplied).sub(currentBorrowed);
        uint256 _borrowNum = borrowTargetFactorNumerator();
        uint256 maxBorrow = currentBalance.mul(_borrowNum).div(factorDenominator().sub(_borrowNum));
        if (currentBorrowed > maxBorrow) {
          wantedBorrow = maxBorrow;
          wantedSupply = currentSupplied.add(currentCash).sub(currentBorrowed.sub(maxBorrow));
        } else {
          uint256 diff = maxBorrow.sub(currentBorrowed);
          if (diff > borrowAvail) {
            wantedBorrow = currentBorrowed.add(borrowAvail);
            wantedSupply = currentSupplied.add(currentCash).add(borrowAvail);
          } else {
            wantedBorrow = currentBorrowed.add(diff);
            wantedSupply = currentSupplied.add(currentCash).add(diff);
          }
          if (wantedSupply > currentSupplied.add(supplyAvail)) {
            diff = wantedSupply.sub(currentSupplied.add(supplyAvail));
            if (diff > wantedBorrow) {
              wantedBorrow = 0;
              wantedSupply = currentSupplied.add(supplyAvail);
              wantedCash = currentCash.sub(supplyAvail).sub(currentBorrowed);
            } else {
              wantedBorrow = wantedBorrow.sub(diff);
              wantedSupply = wantedSupply.sub(diff);
            }
          }
        }
      }
    }
    return (wantedSupply, wantedBorrow, wantedCash);
  }

  function _canFlashloan(uint256 amount) internal view returns (bool) {
    uint256 balancerBalance = IERC20(underlying()).balanceOf(bVault);
    return (balancerBalance >= amount);
  }
