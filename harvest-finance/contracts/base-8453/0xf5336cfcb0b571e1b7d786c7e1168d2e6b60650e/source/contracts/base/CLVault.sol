...
contract CLVault is ERC20Upgradeable, ERC721HolderUpgradeable, IUpgradeSource, ControllableInit, CLVaultStorage {
  ...
  function posWidth() public view returns(uint256) {
    return _posWidth();
  }

  function targetWidth() public view returns(uint256) {
    return _targetWidth();
  }

  function setTargetWidth(uint256 _target) external onlyGovernance {
    require(_target <= posWidth(), "Target");
  ...
  */
  ...
  function setStrategy(address _strategy) public onlyControllerOrGovernance {
    require(canUpdateStrategy(_strategy), "timelock");
    require(_strategy != address(0), "_strategy 0");
  ...
  */
  ...
  function _withdraw(uint256 numberOfShares, uint256 amount0OutMin, uint256 amount1OutMin, address receiver, address owner) internal returns (uint256, uint256) {
    ...
    require(numberOfShares > 0, "numberOfShares 0");
  ...
  */
  ...
  // function expectedAmount0ForAmount1(uint256 amount1) public view returns (uint256 expectedAmount0) {
  //   (uint256 current0, uint256 current1) = getCurrentTokenAmounts();
  //   expectedAmount0 = amount1.mul(current0).div(current1);
  // }

  function getCurrentTokenWeights() public view returns (uint256 weight0, uint256 weight1) {
    (weight0, weight1) = _getWeightsForTickLimits(tickLower(), tickUpper());
  }

  // function getPositionValueIn0() public view returns (uint256 valueIn0) {
  //   (uint256 amount0, uint256 amount1) = getCurrentTokenAmounts();
  //   (uint256 weight0, uint256 weight1) = getCurrentTokenWeights();
  //   if (weight0 > weight1) {
  //     valueIn0 = amount0.mul(1e18).div(weight0);
  //   } else {
  ...
  //     valueIn0 = amount1.mul(price1In0).div(weight1);
  //   }
  // }

  // function getPositionValueIn1() public view returns (uint256 valueIn1) {
  //   (uint256 amount0, uint256 amount1) = getCurrentTokenAmounts();
  //   (uint256 weight0, uint256 weight1) = getCurrentTokenWeights();
  //   if (weight1 > weight0) {
  //     valueIn1 = amount1.mul(1e18).div(weight1);
  //   } else {
  //     uint256 sqrtPrice = uint256(getSqrtPriceX96());
  //     uint256 price0In1 = sqrtPrice.mul(sqrtPrice).mul(1e18).div(uint(2**(96 * 2)));
  //     valueIn1 = amount0.mul(price0In1).div(weight0);
  //   }
  // }

  function shouldRebalance() public view returns (bool _shouldRebalance) {
    if (posWidth() == targetWidth()) {
      _shouldRebalance = !inRange();
    } else {
      ...
      uint256 maxDiff = targetWidth().mul(uint256(tickSpacing())).div(2);

      _shouldRebalance = diff > maxDiff;
    }
  }
  ...
  function rebalanceCurrentTick(uint256 _posWidth) public onlyControllerOrGovernance {
    ...
    if (currentWeight0 > newWeight0) {
      ...
      uint256 toSwap = IERC20Upgradeable(token0()).balanceOf(address(this)).mul(currentWeight0.sub(newWeight0)).div(1e18);
      _swap(zeroForOne, toSwap, 0);
    } else {
      bool zeroForOne = false;
      uint256 toSwap = IERC20Upgradeable(token1()).balanceOf(address(this)).mul(currentWeight1.sub(newWeight1)).div(1e18);
      _swap(zeroForOne, toSwap, 0);
    }
    ...
    if (_posWidth < targetWidth()) {
      _setTargetWidth(_posWidth);
    }

    emit Rebalanced(oldPosId, posId(), oldLiquidity, underlyingBalanceWithInvestment(), block.timestamp);
  }

  function _createNewPosition(int24 tickLower, int24 tickUpper) internal returns (uint256 tokenId) {
    address _token0 = token0();
    address _token1 = token1();
  ...
  function _swap(bool _zeroForOne, uint256 _amountIn, uint160 _sqrtPriceLimit) internal {
    address _token0 = token0();
    address _token1 = token1();
