...
contract NotifyHelperStateful is Controllable {
  ...
  function notifyPools(uint256 total, uint256 timestamp) public onlyNotifier {
    ...
    IBEP20(rewardToken).safeTransferFrom(msg.sender, address(this), total);
    ...
    for (uint256 i = 0; i < notifications.length; i++) {
      ...
      if (notification.notificationType == NotificationType.TRANSFER) {
        // simple transfer
        IBEP20(rewardToken).safeTransfer(
    ...
    // handle vesting
    if (vestingAmount > 0) {
      IBEP20(rewardToken).safeTransfer(vestingEscrow, vestingAmount);
    }

    // ampliFARM notifications
    if (ampliFARM.checksum > 0) {
      IBEP20(rewardToken).approve(notifyHelperAmpliFARM, ampliFARM.checksum);
    ...
    if (regular.checksum > 0) {
      IBEP20(rewardToken).approve(notifyHelperRegular, regular.checksum);
    ...
    uint256 remainingBalance = IBEP20(rewardToken).balanceOf(address(this));
    if (remainingBalance > 0) {
      IBEP20(rewardToken).safeTransfer(reserve, remainingBalance);
    }
  }
