...
/// @custom:oz-upgrades-from DaoCollateralV0
    ...
    EIP712Upgradeable
{
    ...
    function initializeV1(address _registryContract) public reinitializer(2) {
        ...
        $.swapperEngine = ISwapperEngine(IRegistryContract(_registryContract).getContract(CONTRACT_SWAPPER_ENGINE));
    }
    ...
    function _swapRWAtoStbc(
        ...
        require(
            IERC20($.usd0).approve(address($.swapperEngine), wadRwaQuoteInUSD), "Approval failed"
        );
        ...
        if (wadRwaNotTakenInUSD > 0) {
            require(IERC20($.usd0).approve(address($.swapperEngine), 0), "Approval failed");
    ...
    function swapRWAtoStbcIntent(
        ...
        bytes memory signature = abi.encodePacked(intent.r, intent.s, uint8(intent.v));
