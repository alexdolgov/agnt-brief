...
contract ConvexFraxVault {
    ...
    function initialize(
        ...
        // request the curve lp token from the seed for the initial stake creation
        // IERC20(depositToken).transferFrom(msg.sender, address(this), initAmount);
        /// seed stake 
        bytes32 kekId = IConvexVault(depositVault).stakeLockedCurveLp(initAmount, (_maturityTimestamp - block.timestamp));
        // only we need to add this, but makes it accessible from custodian as depositId(0) to get depositId[0]
