...
contract RewardHYBR is ERC20, Ownable, ReentrancyGuard, Pausable {
    ...
    function redeemFor(uint256 amount, uint8 redeemType, address recipient) external nonReentrant whenNotPaused onlyGaugeOrExempt {
          // Burn rHYBR first        
        if (amount == 0) revert ZeroAmount();
        if (balanceOf(msg.sender) < amount) revert InsufficientBalance();
        _burn(msg.sender, amount);
    ...
    function _redeemToGHYBR(uint256 amount, address recipient) internal {
        ...
        IGHYBR(gHYBR).deposit_for(amount, recipient);
    ...
    modifier onlyGaugeOrExempt() {
        if (exempt.contains(msg.sender) || gaugeManager != address(0) && IGaugeManager(gaugeManager).isGauge(msg.sender)) {
            _;
        } else {
            revert NotGaugeOrExempt();
        }
    }
    ...
    function depostionEmissionsToken(uint256 _amount) external whenNotPaused onlyGaugeOrExempt {
