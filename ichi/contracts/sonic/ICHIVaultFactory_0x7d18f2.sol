// ============================================================
// FILE: @equalizer/v3/contracts/interfaces/IUniswapV3Factory.sol
// ============================================================
...
// ============================================================
// FILE: @equalizer/v3/contracts/interfaces/IUniswapV3Pool.sol
// ============================================================
...
// ============================================================
// FILE: @equalizer/v3/contracts/interfaces/pool/IUniswapV3PoolActions.sol
// ============================================================
...
// ============================================================
// FILE: @equalizer/v3/contracts/interfaces/pool/IUniswapV3PoolDerivedState.sol
// ============================================================
...
// ============================================================
// FILE: @equalizer/v3/contracts/interfaces/pool/IUniswapV3PoolEvents.sol
// ============================================================
...
// ============================================================
// FILE: @equalizer/v3/contracts/interfaces/pool/IUniswapV3PoolImmutables.sol
// ============================================================
...
// ============================================================
// FILE: @equalizer/v3/contracts/interfaces/pool/IUniswapV3PoolState.sol
// ============================================================
...
// IV6 - "deposits must be > 0"
// IV7 - "deposit: deposits too large"
// IV8 - "deposit: to"
// IV9 - "deposit: unexpected burn (1)"
// IV10 - "deposit: unexpected burn (2)"
// IV11 - "withdraw: shares"
// IV12 - "withdraw: to"
// IV13 - "withdraw: min shares"
// IV14 - "rebalance: base position invalid"
// IV15 - "rebalance: limit position invalid"
// IV16 - "deposit: try later"
// IV17 - "deposit: try later"
// IV18 - "currentTick: the pool is locked"
...
contract ICHIVault is IICHIVault, IUniswapV3MintCallback, IUniswapV3SwapCallback, ERC20, ReentrancyGuard, Ownable {
    ...
    // For Equalizer 2Thick the fee is dynamic so we implement a passthrough
    ...
    /// @dev if true then ammFeeRecipient must implement IMultiRewards
    bool public override doNotifyRewards;
    ...
    */
    constructor(
        ...
        require(_pool != NULL_ADDRESS, "IV1");
    ...
    function updateDoNotifyRewards(bool _doNotifyRewards) external override {
        _onlyOwner();
        doNotifyRewards = _doNotifyRewards;
        emit SetDoNotifyRewards(msg.sender, _doNotifyRewards);
    }
    ...
    function validateDepositAndSyncFees(
        ...
        require(allowToken0 || deposit0 == 0, "IV4");
        require(allowToken1 || deposit1 == 0, "IV5");
        require(deposit0 > 0 || deposit1 > 0, "IV6");
        require(deposit0 < deposit0Max && deposit1 < deposit1Max, "IV7");
        require(to != NULL_ADDRESS && to != address(this), "IV8");
    ...
    */
    function withdraw(
        ...
        require(shares > 0, "IV11");
        require(to != NULL_ADDRESS, "IV12");

        uint256 _totalSupply = totalSupply();
        require(shares == _totalSupply || _totalSupply >= shares.add(MIN_SHARES), "IV13");
    ...
    */
    ...
    function _distributeFees(uint256 fees0, uint256 fees1) internal {
        ...
        // NOTE: there should always be enough for fees{0,1}; however if for whatever reason there somehow
        // is not, then use available balance for fees
        ...
        if (ammFee > 0 && ammFeeRecipient != NULL_ADDRESS) {
            bool _doNotifyRewards = doNotifyRewards;

            if (fees0 > 0) {
                uint256 ammFee0 = fees0.mul(ammFee).div(PRECISION);
                if (_doNotifyRewards) {
                    _notifyRewardAmount(token0, ammFee0);
                } else {
                    IERC20(token0).safeTransfer(ammFeeRecipient, ammFee0);
                }
            }
            if (fees1 > 0) {
                uint256 ammFee1 = fees1.mul(ammFee).div(PRECISION);
                if (_doNotifyRewards) {
                    _notifyRewardAmount(token1, ammFee1);
                } else {
                    IERC20(token1).safeTransfer(ammFeeRecipient, ammFee1);
                }
            }
        }
    ...
    */
    ...
    function _notifyRewardAmount(address rewardsToken, uint256 reward) private {
        IERC20(rewardsToken).approve(ammFeeRecipient, reward);
        IMultiRewards(ammFeeRecipient).notifyRewardAmount(rewardsToken, reward);
        IERC20(rewardsToken).approve(ammFeeRecipient, 0);
    }
}
...
import { IICHIVaultFactory } from "../interfaces/IICHIVaultFactory.sol";
import { IUniswapV3Factory } from "@equalizer/v3/contracts/interfaces/IUniswapV3Factory.sol";
import { Ownable } from "@openzeppelin/contracts/access/Ownable.sol";
import { ReentrancyGuard } from "@openzeppelin/contracts/utils/ReentrancyGuard.sol";
...
contract ICHIVaultFactory is IICHIVaultFactory, ReentrancyGuard, Ownable {
    ...
    /**
     @notice Creates an ICHIVault instance for specified tokenA/tokenB/tickSpacing.
    ...
    @param tickSpacing tick spacing of the Uniswap V3 pool.
    ...
    */
    function createICHIVault(
        ...
        // deployer, token0, token1, tickSpacing, allowToken1, allowToken2 -> ichiVault address
        require(
            getICHIVault[genKey(msg.sender, token0, token1, tickSpacing, allowToken0, allowToken1)] == NULL_ADDRESS,
            "IVF.createICHIVault: vault exists"
        );

        address pool = IUniswapV3Factory(uniswapV3Factory).getPool(tokenA, tokenB, tickSpacing);
        ...
        getICHIVault[genKey(msg.sender, token0, token1, tickSpacing, allowToken0, allowToken1)] = ichiVault;
        getICHIVault[genKey(msg.sender, token1, token0, tickSpacing, allowToken1, allowToken0)] = ichiVault;
        allVaults.push(ichiVault);

        emit ICHIVaultCreated(msg.sender, ichiVault, token0, allowToken0, token1, allowToken1, tickSpacing, allVaults.length);
    }
    ...
    */
     ...
     * @param tickSpacing the uniswap v3 tickSpacing
    ...
    function genKey(
        ...
        key = keccak256(abi.encodePacked(deployer, token0, token1, tickSpacing, allowToken0, allowToken1));
    }
}
...
interface IICHIVault {
    ...
    function doNotifyRewards() external view returns(bool);
    ...
    function updateDoNotifyRewards(bool) external;
    ...
    event SetDoNotifyRewards(address sender, bool doNotifyRewards);
...
// ============================================================
// FILE: interfaces/IMultiRewards.sol
// ============================================================
