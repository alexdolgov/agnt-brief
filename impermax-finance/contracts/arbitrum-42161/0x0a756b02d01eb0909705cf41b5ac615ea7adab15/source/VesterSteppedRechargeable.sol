...
// File: contracts\VesterSteppedRechargeable.sol

pragma solidity =0.6.6;



contract VesterSteppedRechargeable is Vester {
	...
	constructor(
		...
		segmentLength = vestingEnd_.sub(vestingBegin_).div(segments);	
	}
	...
	function claim() public virtual override returns (uint amount) {
		require(msg.sender == recipient, "VesterSteppedRechargeable: UNAUTHORIZED");
		uint blockTimestamp = getBlockTimestamp();
		if (blockTimestamp < vestingBegin) return 0;
		uint epochEnd = epochBegin + segmentLength;
		if (blockTimestamp < epochEnd) return 0;
