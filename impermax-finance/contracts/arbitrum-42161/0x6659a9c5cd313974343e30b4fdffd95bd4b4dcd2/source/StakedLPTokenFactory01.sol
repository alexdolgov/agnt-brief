...
contract StakedLPToken01 is IStakedLPToken01, IUniswapV2Pair, PoolToken {
	...
	function mint(address minter) external nonReentrant update returns (uint mintTokens) {
		...
		IStakingRewards(stakingRewards).stake(safe128(mintAmount));
		mintTokens = mintAmount.mul(1e18).div(exchangeRate());
	...
	function redeem(address redeemer) external nonReentrant update returns (uint redeemAmount) {
		...
		_burn(address(this), redeemTokens);
		IStakingRewards(stakingRewards).withdraw(safe128(redeemAmount));
		_safeTransfer(redeemer, redeemAmount);
		emit Redeem(msg.sender, redeemer, redeemAmount, redeemTokens);		
	}
	...
	function reinvest() external nonReentrant update {
		...
		IStakingRewards(stakingRewards).stake(safe128(liquidity));
    ...
    function safe128(uint n) internal pure returns (uint128) {
        require(n < 2**128, "StakedLPToken01: SAFE128");
        return uint128(n);
    }
}
...
contract StakedLPTokenFactory01 is IStakedLPTokenFactory01 {
	...
	function createStakedLPToken(address stakingRewards) external returns (address stakedLPToken) {
		...
		address pair = IStakingRewards(stakingRewards).stakedToken();
		address rewardsToken = IStakingRewards(stakingRewards).rewardToken();
