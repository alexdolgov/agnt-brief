...
interface IRouter {
	function getLendingPool(address underlying) external view returns (address collateral, address borrowableA, address borrowableB);
}


contract FarmingPoolFactory {
	...
	struct FarmingPools {
		address farmingPool0;
		address farmingPool1;
	}	
	mapping(address => FarmingPools) public getFarmingPools;
	address[] public allFarmingPools; // address of the UniswapV2Pair
	...
	function allFarmingPoolsLength() external view returns (uint) {
		return allFarmingPools.length;
	}

	function createFarmingPools(address uniswapV2Pair) external {
		require(getFarmingPools[uniswapV2Pair].farmingPool0 == address(0), "FarmingPoolFactory: PAIR_EXISTS");
		
		(, address borrowable0, address borrowable1) = IRouter(router).getLendingPool(uniswapV2Pair);
		address farmingPool0 = address(new FarmingPool(imx, claimable, borrowable0, vester));
		address farmingPool1 = address(new FarmingPool(imx, claimable, borrowable1, vester));
		
		getFarmingPools[uniswapV2Pair] = FarmingPools({
		    farmingPool0: farmingPool0,
		    farmingPool1: farmingPool1
		});
		allFarmingPools.push(uniswapV2Pair);
	}
}