...
// File: contracts\StakedLPTokenSolidlyStableFenix.sol
...
contract StakedLPTokenSolidlyStableFenix is PoolToken {
	using SafeToken for address;

	bool public constant isStakedLPToken = true;
	string public constant stakedLPTokenType = "SolidlyStable3";
	...
	function _initialize(
		...
		router = _router;
		gauge = ISolidlyVoter(_voter).poolToGauge(underlying);
	...
	function swapExactTokensForTokens(address tokenIn, address tokenOut, uint256 amount) internal {
		...
		IBaseV1Router01(router).swapExactTokensForTokensSimple(amount, 0, tokenIn, tokenOut, hasStablePair(tokenIn, tokenOut), address(this), now);
	}

	function hasStablePair(address token0, address token1) internal view returns(bool) {
		return IBaseV1Router01(router).pairFor(token0, token1, false) == address(0);
	}
...
// File: contracts\StakedLPTokenFactorySolidlyStableFenix.sol

pragma solidity =0.5.16;



contract StakedLPTokenFactorySolidlyStableFenix {
	address public router;
	...
	function createStakedLPToken(address pair) external returns (address stakedLPToken) {
		...
		bytes memory bytecode = type(StakedLPTokenSolidlyStableFenix).creationCode;
		assembly {
			stakedLPToken := create2(0, add(bytecode, 32), mload(bytecode), pair)
		}
		StakedLPTokenSolidlyStableFenix(stakedLPToken)._initialize(pair, router, voter, rewardsToken, bridgeTokens);
