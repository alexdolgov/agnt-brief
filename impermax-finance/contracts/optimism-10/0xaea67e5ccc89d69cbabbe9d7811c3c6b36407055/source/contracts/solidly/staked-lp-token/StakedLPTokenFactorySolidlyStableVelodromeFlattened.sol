...
// File: contracts\StakedLPTokenSolidlyStable3.sol

pragma solidity =0.5.16;

contract StakedLPTokenSolidlyStable3 is PoolToken {
	using SafeToken for address;

	bool public constant isStakedLPToken = true;
	string public constant stakedLPTokenType = "SolidlyStableVelodrome";
	...
	function swapExactTokensForTokens(address tokenIn, address tokenOut, uint256 amount, bool isStable) internal {
	...
	function _getReward() internal returns (uint256 amount) {
		ISolidlyGauge3(gauge).getReward(address(this));
	...
	function _swapWithBestBridge() internal view returns (address bestBridgeToken, uint bestIndex) {
		...
		for (uint i = 0; i < bridgeTokens.length; i++) {
			for (uint j = 0; j < 2; j++) {
				address pair = IBaseV1Factory(pairFactory).getPool(tokens[j], bridgeTokens[i], true);
	...
	function reinvest() external nonReentrant update {
		...
		if (token0 == rewardsToken || token1 == rewardsToken) {
			...
			swapExactTokensForTokens(rewardsToken, bridgeToken, reward.sub(bounty), false);
			if (token0 == bridgeToken || token1 == bridgeToken) { 
				(tokenA, tokenB) = token0 == bridgeToken ? (token0, token1) : (token1, token0);
			}
			else {
				swapExactTokensForTokens(bridgeToken, index == 0 ? token0 : token1, bridgeToken.myBalance(), true);
		...
		swapExactTokensForTokens(tokenA, tokenB, swapAmount, true);
...
contract StakedLPTokenFactorySolidlyStableVelodrome {
	address public router;
	...
	function createStakedLPToken(address pair) external returns (address stakedLPToken) {
		...
		bytes memory bytecode = type(StakedLPTokenSolidlyStable3).creationCode;
		assembly {
			stakedLPToken := create2(0, add(bytecode, 32), mload(bytecode), pair)
		}
		StakedLPTokenSolidlyStable3(stakedLPToken)._initialize(pair, router, voter, rewardsToken, bridgeTokens);
