...
contract SynthetixV3SpotMarketContractGuard is IGuard, TxDataUtils, ITransactionTypes {
  ...
  constructor(
    ...
    require(_slippageAccumulator != address(0), "invalid slippageAcc");
  ...
  function txGuard(
    ...
    if (method == IWrapperModule.wrap.selector) {
      ...
      SynthetixV3Structs.AllowedMarket storage allowedMarket = _validateMarketId(marketId);
      swapData.srcAsset = IAtomicOrderModule(snxV3Core).getUsdToken();
      swapData.dstAsset = allowedMarket.collateralSynth;
      swapData.srcAmount = usdAmount;
      swapData.dstAmount = minSynthAmount;
      swapData.to = _to;
      swapData.poolManagerLogic = _poolManagerLogic;

      _atomicSwapCheck(allowedMarket, swapData);
      ...
      SynthetixV3Structs.AllowedMarket storage allowedMarket = _validateMarketId(marketId);
      swapData.srcAsset = allowedMarket.collateralSynth;
      swapData.dstAsset = IAtomicOrderModule(snxV3Core).getUsdToken();
      swapData.srcAmount = synthAmount;
      swapData.dstAmount = minUsdAmount;
      swapData.to = _to;
      swapData.poolManagerLogic = _poolManagerLogic;
  ...
  function _atomicSwapCheck(
    ...
    require(IHasSupportedAsset(_swapData.poolManagerLogic).isSupportedAsset(_swapData.dstAsset), "unsupported asset");
