...
// File contracts/interfaces/iziswap/IiZiSwapCallback.sol
...
// File contracts/interfaces/iziswap/IiZiSwapFactory.sol
...
// File contracts/interfaces/iziswap/IiZiSwapPool.sol
...
contract SwapRouter is IiZiSwapCallback, ISwapRouter {
    ...
    function pay(
        ...
        if (token == WETH9 && address(this).balance >= value) {
            ...
            safeTransfer(WETH9, recipient, value);
        } else {
    ...
    /// @param data encoded pay token address
    function swapY2XCallback(
        uint256 x,
        uint256 y,
        bytes calldata data
    ...
    function swapAmountInternal(
        ...
        address recipient,
        bytes memory path
    ) private returns (uint256 cost, uint256 acquire, address acquireToken) {
    ...
    function swapAmount(SwapAmountParams calldata params, uint128 amount, address recipient, bool receiveNative)
        ...
        payable
        override
        returns (uint256 cost, uint256 acquire, address acquireToken) 
    {
        (cost, acquire, acquireToken) = swapAmountInternal(
            amount, 
            receiveNative ? address(this): recipient, 
            params.path
        );
        require(acquire >= params.minAcquired, 'Too much requested in swapAmount');
        if (receiveNative) {
            if (acquireToken == WETH9) {
                uint256 all = IERC20(WETH9).balanceOf(address(this));
                if (all > 0) {
                    IWETH9(WETH9).withdraw(all);
                }
                safeTransferETH(recipient);
            } else {
                safeTransfer(acquireToken, recipient, acquire);
            }
        }
        emit Swap(acquireToken, acquire);
    }
