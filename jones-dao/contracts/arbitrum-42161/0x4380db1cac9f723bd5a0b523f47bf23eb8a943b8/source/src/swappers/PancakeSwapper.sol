...
import {IV3CakeSwapRouter} from "src/interfaces/swap/IV3CakeSwapRouter.sol";
...
contract PancakeSwapper is UpgradeableGovernable, IUniswapV3Swapper, ITokenSwapper {
    using SafeMath for uint256;
    using MulDivMathLib for uint256;
    using SafeERC20 for IERC20;

    // Cake V3 Router
    IV3CakeSwapRouter private V3_ROUTER;

    //Uni V3 Factory
    address private constant PANCAKE_V3_FACTORY = 0x0BFbCF9fa4f9C56B0F40a671Ad40E0805A091865;

    // Uni V3 Pool Code Hash
    bytes32 private constant PANCAKE_POOL_INIT_CODE_HASH =
        0x6ce8eb472fa82df5469c6ab6d485f17c3ad13c8cd7af59b3d4a8026c5ce0f7e2;

    uint256 private constant arbID = 42161;
    ...
    function initialize(address _factory) external initializer {
        ...
        if (chainId == arbID) {
            V3_ROUTER = IV3CakeSwapRouter(0xFE6508f0015C778Bdcc1fB5465bA5ebE224C9912);
        } else {
            V3_ROUTER = IV3CakeSwapRouter(0x13f4EA83D0bd40E75C8222255bc855a974568Dd4);
        }
    }
        ...
        returns (uint256 amountOut)
    {
        ...
        IV3CakeSwapRouter.ExactInputParams memory params;
        ...
        returns (uint256)
    {
        ...
        for (uint256 i; i < length;) {
            IPancakeV3Pool pool = PancakeLibrary.getPool(
                PANCAKE_V3_FACTORY, _pairs[i].from, _pairs[i].to, _pairs[i].fee, PANCAKE_POOL_INIT_CODE_HASH
            );
    ...
    function _checkManipulation(address pool) private view returns (uint256 spot) {
        spot = PancakeLibrary.getSpot(pool);

        uint256 mean = PancakeLibrary.getPrice(pool, ticksMin);
