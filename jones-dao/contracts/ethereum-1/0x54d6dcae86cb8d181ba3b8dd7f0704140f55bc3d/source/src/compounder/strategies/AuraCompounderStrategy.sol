...
contract AuraCompounderStrategy is IStrategy, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function withdraw(address user, uint256 amount, bool tokenized) external nonReentrant returns (uint256) {
        ...
        (uint256 noTokenizedPortion, uint256 tokenizedPortion) = vaultsCurrentPosition();

        if (tokenized) {
            if (tokenizedPortion < amount) {
                LOCKER.processExpiredLocks(false);
                (, tokenizedPortion) = vaultsCurrentPosition();
            ...
            if (noTokenizedPortion < amount) {
                LOCKER.processExpiredLocks(false);

                (noTokenizedPortion,) = vaultsCurrentPosition();
