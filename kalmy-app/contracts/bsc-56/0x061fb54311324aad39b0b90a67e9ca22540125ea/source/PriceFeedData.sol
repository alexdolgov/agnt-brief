...
* Chainlink oracle.
*/
contract PriceFeedData is Ownable {
    using SafeMath for uint256;

    address public immutable kalm;
    // kalm oracle address
    address public kalmOracle; 

    mapping (address => address) public assetChainlink;
    ...
    function lpPriceInUsd(address lp) external view returns (uint256) {
        return _getLpPrice(lp);
    }

    /* ========== MUTATIVE FUNCTION ========== */

    function setAssetSource(address[] memory _asset, address[] memory _chainlink) external onlyOwner {
        require(_asset.length == _chainlink.length);
        for (uint i; i < _asset.length; i++) {
            assetChainlink[_asset[i]] = _chainlink[i];
        }
    }

    function setKalmPriceOracle(address _oracle) external onlyOwner {
        kalmOracle = _oracle;
    }
    ...
    function _kalmPrice() internal view returns (uint256) {
        uint256 price = IKalmOraclePrice(kalmOracle).kalmPrice();
        return price;
    }

    function _otherTokenPrice(address asset) internal view returns (uint256) {
        address chainlink = assetChainlink[asset];
    ...
    function _fairAssetReserve(address lp) internal view returns(uint256 fairReserveKalm, uint256 fairReserveOther)
    {
      ...
      uint256 p = _kalmPrice().mul(1e18).div(_otherTokenPrice(other));

      fairReserveKalm = Math.sqrt(k.div(p)).mul(1e9);
      fairReserveOther = Math.sqrt(k.mul(p)).div(1e9);
    }

    function _getLpPrice(address lp) internal view returns(uint256)
    {
      IPancakeswapV2Pair pair = IPancakeswapV2Pair(lp);
      (uint256 fairReserveKalm, uint256 fairReserveOther) = _fairAssetReserve(lp);

      uint totalSupply = pair.totalSupply();
      uint256 totalOtherPrice = fairReserveOther.mul(_otherTokenPrice(lp));
