...
// File: contracts/StrategyAddTwoSidesOptimalWombatV2.sol
...
contract StrategyAddTwoSidesOptimalWombatV2 is Ownable, ReentrancyGuard, Strategy {
    ...
    constructor(IWombatRouterV2 _router, address _goblin, IWombatPool _wombatPool) public {
        router = _router;
        ...
        nonReentrant 
    {
        ...
        pool[0] = address(wombatPool);
        if (ethBalance > 0) {
            router.swapExactNativeForTokens.value(ethBalance)(path, pool, 0, address(this), now); // ETH to farming tokens
        }
