    ...
    ReentrancyGuardUpgradeable
{
    ...
    event DepositOwnerUpdated(
        address indexed oldDepositOwner,
        address indexed asset,
        uint256 indexed depositId,
        address newDepositOwner
    );
    ...
    /// @notice Locks the given deposit for the provided time period.
    /// @dev Can only be called by the governance. It is here to recover from
    ///      the situation when the depositor wallet was compromised and the
    ///      situation has to be mitigated right away.
    /// @param depositOwner The owner of the deposit.
    /// @param asset Address of the asset to be locked.
    /// @param depositId ID of the deposit.
    ...
    function lock(
        ...
        DepositInfo memory selectedDeposit = deposits[depositOwner][asset][
            depositId
        ];
        uint256 depositedAmount = selectedDeposit.balance;
        ...
        emit DepositLocked(depositOwner, asset, depositId, lockPeriod);

        mezoPortal.lock(asset, selectedDeposit.mezoDepositId, lockPeriod);
    }

    /// @notice Updates the owner of the deposit.
    /// @dev Can only be called by the governance. It is here to recover from
    ///      the situation when the depositor wallet was compromised and the
    ///      deposit owner has to be updated.
    /// @param depositOwner The current owner of the deposit.
    /// @param asset Address of the asset to be locked.
    /// @param depositId ID of the deposit.
    /// @param newDepositOwner The new deposit owner.
    function updateDepositOwner(
        address depositOwner,
        address asset,
        uint256 depositId,
        address newDepositOwner
        ...
        uint256 depositedAmount = selectedDeposit.balance;
        uint256 mezoDepositId = selectedDeposit.mezoDepositId;
        ...
        // Delete the existing deposit info, the owner is going to change.
        delete deposits[depositOwner][asset][depositId];

        // Register the new deposit info.
        deposits[newDepositOwner][asset][depositId] = DepositInfo({
            balance: depositedAmount,
            mezoDepositId: mezoDepositId
        });

        emit DepositOwnerUpdated(
            depositOwner,
            asset,
            depositId,
            newDepositOwner
