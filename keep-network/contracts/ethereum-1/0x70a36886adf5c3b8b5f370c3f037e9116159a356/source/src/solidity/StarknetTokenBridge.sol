    ...
    Roles
{
    ...
    event TokenHaltSignalled(address indexed sender, address indexed token);
    event TokenHaltCompleted(address indexed sender, address indexed token);
    uint256 constant N_DEPOSIT_PAYLOAD_ARGS = 5;
    uint256 constant DEPOSIT_MESSAGE_FIXED_SIZE = 1;
    address public immutable cleareanceAddress;

    constructor(address _cleareanceAddress) Roles(false) {
        require(_cleareanceAddress != address(0), "INVALID_CLEAREANCE_ADDRESS");
        cleareanceAddress = _cleareanceAddress;
    }

    function identify() external pure virtual returns (string memory) {
        return "StarkWare_StarknetTokenBridge_2.0_6-halt";
    }
    ...
    */
    ...
    modifier notHalted(address token) {
        HaltState _status = haltState()[token];
        require(_status == HaltState.TokenActive, "TOKEN_HALTED");
        _;
    }
    ...
    function signalTokenHalt(address token) external payable onlyAppGovernor {
        require(cleareanceAddress != address(0), "CLEAREANCE_ADDRESS_NOT_SET");
        require(tokenSettings()[token].tokenStatus == TokenStatus.Active, "TOKEN_NOT_ACTIVE");
        HaltState _status = haltState()[token];
        // We allow signal on both Active and Signalled state,
        // So that in case the previous signal message was not handled on L2 we can send it again.
        require(_status != HaltState.HaltCompleted, "TOKEN_HALT_COMPLETED");
        setHaltState(token, HaltState.HaltSignalled);
        sendHaltMessage(token);
        emit TokenHaltSignalled(msg.sender, token);
    }

    function completeTokenHalt(
        address token,
        uint256 amount,
        address recipient
    ) external onlyAppGovernor {
        HaltState _status = haltState()[token];
        require(_status == HaltState.HaltSignalled, "TOKEN_HALT_NOT_SIGNALLED");
        require(recipient == cleareanceAddress, "CLEARANCE_ADDRESS_MISMATCH");
        setHaltState(token, HaltState.HaltCompleted);
        consumeHaltMessage(token, amount);
        transferOutFunds(token, amount, cleareanceAddress);
        emit TokenHaltCompleted(msg.sender, token);
    }
    ...
    function addressMessagePayload(address _address) private pure returns (uint256[] memory) {
        uint256[] memory payload = new uint256[](1);
        payload[0] = uint256(uint160(_address));
        return payload;
    }
    ...
    function sendHaltMessage(address token) internal returns (bytes32) {
        require(l2TokenBridge() != 0, "L2_BRIDGE_NOT_SET");
        Fees.checkFee(msg.value);

        (bytes32 haltMsgHash, ) = messagingContract().sendMessageToL2{value: msg.value}(
            l2TokenBridge(),
            HANDLE_TOKEN_HALT_SELECTOR,
            addressMessagePayload(token)
        );
        return haltMsgHash;
    }
    ...
    function consumeHaltMessage(address token, uint256 amount) internal {
        require(l2TokenBridge() != 0, "L2_BRIDGE_NOT_SET");
        uint256[] memory payload = new uint256[](4);
        payload[0] = HALT_MESSAGE;
