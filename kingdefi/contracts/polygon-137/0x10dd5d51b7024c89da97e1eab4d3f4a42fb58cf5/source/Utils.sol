...
contract Utils{
    ...
    function token0(uint256 _pid) view public returns(address){
        ...
        address token = factory.getPair(tokenLP.token0(), address(0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270));
        return token;
    }
    ...
    function token1(uint256 _pid) view public returns(address){
        (address tokenAddr, , , , , , , ,) = kingDeFiFarm.poolInfo(_pid);
        IPancakeLPToken tokenLP = IPancakeLPToken(tokenAddr);
        address token = factory.getPair(tokenLP.token1(), address(0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270));
        return token;
    }
    ...
    function tvl(uint256 _pid) view public returns (uint256) {
        if(_pid == 0){
            return (prezzoMatic() * reverseToken0(_pid) / 10**18) + (prezzoKRWinUSDC() * reverseToken1(_pid) / 10**18);
        }
    ...
    function apr(uint256 _pid) view public returns(uint256){
        uint256 blockPerYear = (365 * 24 * 60 * 60) / 2;
        uint256 busdPerYear = blockPerYear * kingDeFiFarm.getKrwPerBlock(_pid) * prezzoKRWinUSDC();
    ...
    function tvlTotal(uint256 _pid, address account) view public returns (uint256) {
        if(_pid == 0){
            return (prezzoMatic() * reverseToken0Total(_pid, account) / 10**18) + (prezzoKRWinUSDC() * reverseToken1Total(_pid, account) / 10**18);
        }
    ...
    function totalPending(address account) view external returns(uint256){
        uint256 tt = 0;
        for(uint256 i = 0; i < kingDeFiFarm.poolLength(); i++){
            tt = tt + calculatePending(i, account);
        }
        return tt;
    }
    ...
    function dollariPerUnita(uint256 _pid) view public returns(uint256){
        if(_pid == 0){
            return (((1000000000000000000 * reserve0(_pid)) / totalSupply(_pid) * prezzoMatic()) / 10**18) + (((1000000000000000000 * reserve1(_pid)) / totalSupply(_pid) * prezzoKRWinUSDC()) / 10**18);
        }
    ...
    function calculatePending(uint256 _pid, address account) view public returns(uint256){
        ( , , , , , uint256 t ,uint256 krwPerBlock, uint256 lastRewardBlock, uint256 rewardsPerShare) = kingDeFiFarm.poolInfo(_pid);
        (uint256 amount, uint256 rwMinus) = kingDeFiFarm.userInfo(_pid, account);
        uint256 rewardToDeliver = krwPerBlock.mul(block.number.sub(lastRewardBlock));
        uint256 rw = rewardsPerShare.add((rewardToDeliver.mul(1e18)).div(t));
        return ((amount.mul(rw)).div(1e18)).sub(rwMinus);
    }
}