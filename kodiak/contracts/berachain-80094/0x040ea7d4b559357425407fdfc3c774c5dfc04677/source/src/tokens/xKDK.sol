 ...
 * xKDK may be allocated to Usages contracts (e.g. staking, governance)
 */
contract XKodiakToken is Ownable, ReentrancyGuard, ERC20("Kodiak escrowed token", "xKDK"), IXKdkToken {
    ...
    address public forfeitAddress;

    EnumerableSet.AddressSet private _whitelisters; // addresses allowed to whitelist/unwhitelist
    EnumerableSet.AddressSet private _transferWhitelistFrom; // addresses allowed to send xKDK
    EnumerableSet.AddressSet private _transferWhitelistTo; // addresses allowed to receive xKDK
    ...
    uint256 public constant MAX_DEALLOCATION_FEE = 5000; // 50%
    ...
    uint256 public redeemRewardsAdjustment = 0; // 0% by default
    ...
    constructor() {
        _transferWhitelistFrom.add(address(this));
        _transferWhitelistTo.add(address(this));
        forfeitAddress = owner();
    }
    ...
    event UpdateTransferWhitelistFrom(address account, bool add);
    event UpdateTransferWhitelistTo(address account, bool add);
    ...
    event ForfeitAddressSet(address forfeitAddress);
    /***********************************************/
    /****************** MODIFIERS ******************/
    /***********************************************/
    ...
    /// @dev returns length of transferWhitelistFrom array
    function transferWhitelistFromLength() external view returns (uint256) {
        return _transferWhitelistFrom.length();
    }

    /// @dev returns transferWhitelistFrom array item's address for "index"
    function transferWhitelistFrom(uint256 index) external view returns (address) {
        return _transferWhitelistFrom.at(index);
    }

    /// @dev returns if "account" is allowed to send xKDK
    function isTransferWhitelistedFrom(address account) external override view returns (bool) {
        return _transferWhitelistFrom.contains(account);
    }

    /// @dev returns length of transferWhitelistTo array
    function transferWhitelistToLength() external view returns (uint256) {
        return _transferWhitelistTo.length();
    }

    /// @dev returns transferWhitelistTo array item's address for "index"
    function transferWhitelistTo(uint256 index) external view returns (address) {
        return _transferWhitelistTo.at(index);
    }

    /// @dev returns if "account" is allowed to receive xKDK
    function isTransferWhitelistedTo(address account) external override view returns (bool) {
        return _transferWhitelistTo.contains(account);
    }
    ...
    /// @dev Adds or removes addresses from both from and to transfer whitelists
    function updateTransferWhitelist(address account, bool add) external onlyWhiteLister {
        _updateTransferWhitelistFrom(account, add);
        _updateTransferWhitelistTo(account, add);
    }

    /// @dev Adds or removes addresses from the transferWhitelistFrom
    function updateTransferWhitelistFrom(address account, bool add) public onlyWhiteLister {
        _updateTransferWhitelistFrom(account, add);
    }

    /// @dev Adds or removes addresses from the transferWhitelistTo
    function updateTransferWhitelistTo(address account, bool add) public onlyWhiteLister {
        _updateTransferWhitelistTo(account, add);
    }

    /// @dev Sets the forfeit address
    function setForfeitAddress(address _forfeitAddress) external onlyOwner {
        forfeitAddress = _forfeitAddress;
        emit ForfeitAddressSet(_forfeitAddress);
    }
    ...
    /// @dev Convert caller's "amount" of KDK to xKDK AND allocate it to "usage" contract in one call
    function convertAndAllocate(uint256 amount, IXKdkTokenUsage usage, bytes calldata usageData) external kdkActive nonReentrant {
        _convert(amount, msg.sender);
        _approveUsage(usage, amount);
        _allocate(msg.sender, address(usage), amount);
        usage.allocate(msg.sender, amount, usageData);
    }
    ...
    /// @notice KDK excess is automatically sent to forfeit address
    function _finalizeRedeem(address userAddress, uint256 xKdkAmount, uint256 kdkAmount) internal {
        ...
        // Sends KDK excess to forfeitAddress
        kdkToken.safeTransfer(forfeitAddress, kdkExcess);
    ...
    function _deallocate(address userAddress, address usageAddress, uint256 amount) internal {
        ...
        kdkToken.safeTransfer(forfeitAddress, deallocationFeeAmount);
    ...
    function _beforeTokenTransfer(address from, address to, uint256 /*amount*/) internal view override {
        require(from == address(0) || _transferWhitelistFrom.contains(from) || _transferWhitelistTo.contains(to), "transfer: not allowed");
    }
    ...
    function _updateTransferWhitelistFrom(address account, bool add) internal {
        require(account != address(this), "updateTransferWhitelistFrom: Cannot remove xKDK from whitelist");

        if(add) _transferWhitelistFrom.add(account);
        else _transferWhitelistFrom.remove(account);

        emit UpdateTransferWhitelistFrom(account, add);
    }

    function _updateTransferWhitelistTo(address account, bool add) internal {
        require(account != address(this), "updateTransferWhitelistTo: Cannot remove xKDK from whitelist");

        if(add) _transferWhitelistTo.add(account);
        else _transferWhitelistTo.remove(account);

        emit UpdateTransferWhitelistTo(account, add);
    }

    function _approveUsage(IXKdkTokenUsage usage, uint256 amount) internal {
