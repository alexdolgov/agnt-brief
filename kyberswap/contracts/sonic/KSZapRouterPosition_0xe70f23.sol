...
import {IKSZapValidatorActions} from
  'contracts/interfaces/zap/validators/IKSZapValidatorActions.sol';
...
contract KSZapRouterPosition is IKSZapRouterPosition, IZapDexEnum, KSRescueV2, ReentrancyGuard {
    ...
    returns (bytes memory zapResults)
  {
    ...
    if (_exe.validator != address(0)) {
      ...
      initialData =
        IKSZapValidatorActions(_exe.validator).prepareValidationData(dexType, _desc.zapInfo);
    }
    ...
    if (_exe.validator != address(0)) {
      bool isValid = IKSZapValidatorActions(_exe.validator).validateData(
