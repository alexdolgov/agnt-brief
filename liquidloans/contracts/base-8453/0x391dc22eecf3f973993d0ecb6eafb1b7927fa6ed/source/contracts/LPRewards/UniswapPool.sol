...
/*
 * On deployment a new Uniswap pool will be created for the pair USDL/ETH and its token will be set here.

 * Essentially the way it works is:

 * - Liquidity providers add funds to the Uniswap pool, and get Uniswap LP tokens in exchange
 * - Liquidity providers stake those Uniswap LP tokens into Uniswappool rewards contract
 ...
 * - Liquidity providers can unstake Uniswap LP tokens to exit the program (i.e., stop earning rewards) when they want

 * Funds for rewards will only be added once, on deployment of LOAN token,
 ...
 * in order to avoid getting LOAN tokens locked.
 * That also means that the start time for the program will be the event that occurs first:
 * either LOAN token contract is deployed, and therefore LOAN tokens are minted to Uniswappool contract,
 * or first liquidity provider stakes Uniswap LP tokens into it.
 */
contract Uniswappool is LPTokenWrapper, Ownable, CheckContract, IUniswappool {
    string constant public NAME = "Uniswappool";
    ...
    event UniswaptokenAddressChanged(address _UniswaptokenAddress);
        ...
        onlyOwner
    {
        ...
        checkContract(_UniswaptokenAddress);

        Uniswaptoken = IERC20(_UniswaptokenAddress);
        loanToken = ILOANToken(_loanTokenAddress);
        duration = _duration;

        _notifyRewardAmount(loanToken.getLpRewardsEntitlement(), _duration);

        emit LOANTokenAddressChanged(_loanTokenAddress);
        emit UniswaptokenAddressChanged(_UniswaptokenAddress);
    ...
    function lastTimeRewardApplicable() public view override returns (uint256) {
        return LiquidLoansMath._min(block.timestamp, periodFinish);
    }
    ...
    function stake(uint256 amount) public override {
        ...
        require(address(Uniswaptoken) != address(0), "Liquidity Pool Token has not been set yet");
    ...
    function _notifyRewardAmount(uint256 _reward, uint256 _duration) internal {
        assert(_reward > 0);
        assert(_reward == loanToken.balanceOf(address(this)));
