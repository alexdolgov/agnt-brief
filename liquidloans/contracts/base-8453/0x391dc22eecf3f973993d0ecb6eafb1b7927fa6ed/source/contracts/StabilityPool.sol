 ...
 *
 * The formula for a depositor's accumulated ETH gain is derived in the math proof.
 *
 ...
 *
 * Please see the implementation spec in the math proof document, which closely follows on from the compounded deposit / ETH gain derivations.
 *
 *
...
contract StabilityPool is LiquidLoansBase, Ownable, CheckContract, IStabilityPool {
    ...
    */
    function provideToSP(uint _amount, address _frontEndTag) external override {
        ...
        emit ETHGainWithdrawn(msg.sender, depositorETHGain, USDLLoss); // USDL Loss required for event log
    ...
    * - Updates snapshots for deposit and tagged front end stake */
    function withdrawETHGainToVault(address _upperHint, address _lowerHint) external override {
        uint initialDeposit = deposits[msg.sender].initialValue;
        _requireUserHasDeposit(initialDeposit);
        ...
        /* Emit events before transferring ETH gain to Vault.
         This lets the event log make more sense (i.e. so it appears that first the ETH gain is withdrawn
        and then it is deposited into the Vault, not the other way around). */
        emit ETHGainWithdrawn(msg.sender, depositorETHGain, USDLLoss);
        emit UserDepositChanged(msg.sender, compoundedUSDLDeposit);

        ETH = ETH.sub(depositorETHGain);
        emit StabilityPoolETHBalanceUpdated(ETH);
        emit PulseSent(msg.sender, depositorETHGain);

        borrowerOperations.moveETHGainToVault{ value: depositorETHGain }(msg.sender, _upperHint, _lowerHint);
    }
    ...
    */
    function offset(uint _debtToOffset, uint _collToAdd) external override {
        ...
        _updateRewardSumAndProduct(ETHGainPerUnitStaked, USDLLossPerUnitStaked);  // updates S and P
    ...
    function _computeRewardsPerUnitStaked(
        ...
        internal
        returns (uint ETHGainPerUnitStaked, uint USDLLossPerUnitStaked)
    {
        /*
        * Compute the USDL and ETH rewards. Uses a "feedback" error correction, to keep
        ...
        */
        ...
        ETHGainPerUnitStaked = ETHNumerator.div(_totalUSDLDeposits);
        lastETHError_Offset = ETHNumerator.sub(ETHGainPerUnitStaked.mul(_totalUSDLDeposits));

        return (ETHGainPerUnitStaked, USDLLossPerUnitStaked);
    }

    // Update the Stability Pool reward sum S and product P
    function _updateRewardSumAndProduct(uint _ETHGainPerUnitStaked, uint _USDLLossPerUnitStaked) internal {
        uint currentP = P;
        uint newP;
    ...
    */
    ...
    // --- Sender functions for USDL deposit, ETH gains and LOAN gains ---
    ...
    function _requireUserHasVault(address _depositor) internal view {
        require(vaultManager.getVaultStatus(_depositor) == 1, "StabilityPool: caller must have an active vault to withdraw ETHGain to");
    }
