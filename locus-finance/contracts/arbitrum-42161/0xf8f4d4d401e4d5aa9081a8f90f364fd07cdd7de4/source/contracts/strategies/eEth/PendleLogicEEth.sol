...
import {PendleGeneralInfo} from "./PendleGeneralEEth.sol";
...
contract PendleLogicEEth {
    ...
    function addLiquidity(uint256 amount, OffChainData calldata _offChainData) external returns(uint256 lpExpected, uint256 ytExpected, IPendleRouter.TokenInput memory inputData) {
        ...
        uint256 amountOut = _swapWethToEEthViaRamses(amount, msg.sender);
        inputData = IPendleRouter.TokenInput(WETH, amountOut, WETH, address(0), swapData);
    ...
    function _swapWethToEEthViaRamses(uint256 amount, address rec) internal returns(uint256 amountOut) {
        ...
        params.amountOutMinimum = 0;//assetToWantRate(amount) * pendleGeneralInfo.slippage / 10000;
        params.sqrtPriceLimitX96 = 0;
        amountOut = ISwapRouter(pendleGeneralInfo.extraDex).exactInputSingle(params);
    }
}