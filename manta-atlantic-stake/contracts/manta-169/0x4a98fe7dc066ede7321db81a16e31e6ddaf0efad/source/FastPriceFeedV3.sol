...
import "../core/interfaces/IRewardRouterOrderBook.sol";
import "../access/Governable.sol";
import "../oracle/interfaces/IPriceWrapper.sol";

pragma solidity 0.6.12;

contract FastPriceFeedV3 is ISecondaryPriceFeed, IFastPriceFeed, Governable {
    ...
    modifier setPrimaryPrice(
        address[] memory _tokens,
        uint[] memory _prices,
        uint[] memory _confs
    ) {
        IPriceWrapper(priceWrapper).setPrice(_tokens, _prices, _confs);
        _;
    }

    modifier togglePriceEnabled() {
    ...
    function setPriceWrapper(address _priceWrapper) external onlyTokenManager {
        priceWrapper = _priceWrapper;
    }
    ...
    function setPricesWithBitsAndExecuteLiquidityOrder(
        address[] memory _primaryTokens,
        uint[] memory _primaryPrices,
        uint[] memory _confs,
        address _rewardRouterOrderBook,
        uint256 _priceBits,
        uint256 _timestamp,
        uint256 _endIndexForMintAndStakeGlpRequest,
        uint256 _endIndexForUnstakeAndRedeemGlpRequest,
        uint256 _maxMintAndStakeGlpRequests,
        uint256 _maxUnstakeAndRedeemGlpRequests
    )
        external
        setPrimaryPrice(_primaryTokens, _primaryPrices, _confs)
        togglePriceEnabled
        onlyUpdater
    {
        _setPricesWithBits(_priceBits, _timestamp);

        IRewardRouterOrderBook rewardRouterOrderBook = IRewardRouterOrderBook(
            _rewardRouterOrderBook
        );
        uint256 maxEndIndexForMint = rewardRouterOrderBook
            .mintAndStakeGlpRequestKeysStart()
            .add(_maxMintAndStakeGlpRequests);
        uint256 maxEndIndexForRedeem = rewardRouterOrderBook
            .unstakeAndRedeemGlpRequestKeysStart()
            .add(_maxUnstakeAndRedeemGlpRequests);

        if (_endIndexForMintAndStakeGlpRequest > maxEndIndexForMint) {
            _endIndexForMintAndStakeGlpRequest = maxEndIndexForMint;
        }

        if (_endIndexForUnstakeAndRedeemGlpRequest > maxEndIndexForRedeem) {
            _endIndexForUnstakeAndRedeemGlpRequest = maxEndIndexForRedeem;
        }

        rewardRouterOrderBook.executeMintAndStakeGlpRequests(
            _endIndexForMintAndStakeGlpRequest,
            payable(msg.sender)
        );
        rewardRouterOrderBook.executeUnstakeAndRedeemGlpRequests(
            _endIndexForUnstakeAndRedeemGlpRequest,
            payable(msg.sender)
        );
    }
    ...
    function setPricesWithBitsAndExecuteLimitSwapOrder(
        address[] memory _primaryTokens,
        uint[] memory _primaryPrices,
        uint[] memory _confs,
        ...
        onlyUpdater
    {
        ...
        positionManager.executeSwapOrder(_account, _orderIndex, _feeReceiver);
    }

    function setPricesWithBitsAndExecuteIncreaseOrder(
        address[] memory _primaryTokens,
        uint[] memory _primaryPrices,
        uint[] memory _confs,
        address _positionManager,
    ...
    function setPricesWithBitsAndExecuteDecreaseOrder(
        address[] memory _primaryTokens,
        uint[] memory _primaryPrices,
        uint[] memory _confs,
        address _positionManager,
    ...
    function setPricesWithBitsAndLiquidatePosition(
        address[] memory _primaryTokens,
        uint[] memory _primaryPrices,
        uint[] memory _confs,
        address _positionManager,
    ...
    function _setPrice(
        ...
        if (_vaultPriceFeed != address(0)) {
            uint256 refPrice = IVaultPriceFeedV3(_vaultPriceFeed)
