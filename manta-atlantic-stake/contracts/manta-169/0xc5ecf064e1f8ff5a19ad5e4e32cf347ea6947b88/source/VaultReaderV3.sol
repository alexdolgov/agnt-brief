...
contract VaultReaderV3 {
    ...
    function getVaultTokenInfoV5(address _vault, address _positionManager, address _positionRouter, address _weth, address[] memory _tokens) public view returns (uint256[] memory) {
        ...
        for (uint256 i = 0; i < _tokens.length; i++) {
            ...
            amounts[i * propsLength + 5] = vault.maxUsdgAmounts(token);
            ...
            amounts[i * propsLength + 9] = positionRouter.maxGlobalShortSizes(token);
            amounts[i * propsLength + 10] = positionRouter.maxGlobalLongSizes(token);
            amounts[i * propsLength + 11] = vault.guaranteedUsd(token);
        }
