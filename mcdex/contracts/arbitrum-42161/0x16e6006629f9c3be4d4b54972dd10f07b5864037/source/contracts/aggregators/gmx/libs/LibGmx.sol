...
library LibGmx {
    ...
    function getPrExecutionFee(ProjectConfigs memory projectConfigs) internal view returns (uint256) {
        return IGmxPositionRouter(projectConfigs.positionRouter).minExecutionFee();
    }

    function getObExecutionFee(ProjectConfigs memory projectConfigs) internal view returns (uint256) {
