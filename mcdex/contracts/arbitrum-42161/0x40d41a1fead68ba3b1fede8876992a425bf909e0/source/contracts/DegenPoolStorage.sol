...
contract DegenPoolStorage is Initializable, ContextUpgradeable, ReentrancyGuardUpgradeable, IDegenPoolStorage {
    ...
    function _collectFeeToDistributor(uint8 tokenId, address trader, uint96 wadAmount) internal {
        emit CollectedFeeToDistributor(tokenId, wadAmount);
    ...
    // @dev remove wadAmount from token.spotLiquidity outside this function
    function _collectFeeToMlp(uint8 tokenId, uint96 wadAmount) internal {
        emit CollectedFeeToMlp(tokenId, wadAmount);
        Asset storage collateral = _storage.assets[tokenId];
        collateral.spotLiquidity += wadAmount;
    }
