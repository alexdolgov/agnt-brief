...
contract Repurchase is Ownable {
    ...
    address public constant HMDX = 0xAEE4164c1ee46ed0bbC34790f1a3d1Fc87796668;
    address public constant MDX_USDT = 0xe1cBe92b5375ee6AfE1B22b555D257B4357F6C68;
    address public constant HMDX_USDT = 0xE16699481Cb5DC79CA491F8437182d73eb0dd09D;
    ...
    constructor (uint256 _amount, address _emergencyAddress, address _factory) {
    ...
    function swapMDX() external onlyCaller returns (uint256 amountOut){
        ...
        amountOut = IMdexFactory(factory).getAmountOut(amountIn, reserve0, reserve1, USDT, MDX);
        IERC20(USDT).safeTransfer(MDX_USDT, amountIn);
        IMdexPair(MDX_USDT).swap(0, amountOut, destroyAddress, new bytes(0));
    }

    function swapHMDX() external onlyCaller returns (uint256 amountOut){
        require(IERC20(USDT).balanceOf(address(this)) >= amountIn, "Insufficient contract balance");
        (uint256 reserve0, uint256 reserve1,) = IMdexPair(HMDX_USDT).getReserves();
        amountOut = IMdexFactory(factory).getAmountOut(amountIn, reserve0, reserve1, USDT, HMDX);
        IERC20(USDT).safeTransfer(HMDX_USDT, amountIn);
        IMdexPair(HMDX_USDT).swap(0, amountOut, destroyAddress, new bytes(0));
    }
