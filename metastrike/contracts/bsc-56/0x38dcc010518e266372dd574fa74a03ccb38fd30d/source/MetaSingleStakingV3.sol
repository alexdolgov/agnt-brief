...
interface MetastrikeCore {
    function ownerOf (uint256 _tokenId) external view returns (address);
    function ownedBy (address _owner) external view returns (uint256[] memory);
}

contract MetaSingleStakingV3 is IStakingRewards, RewardsDistributionRecipient, ReentrancyGuard, Pausable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    /* ========== STATE VARIABLES ========== */

    MetastrikeCore public core;
    ...
    function checkIsValidEarlyNFTOwner(address _owner) public view returns (uint256, bool) {
        uint256[] memory tokenIds = core.ownedBy(_owner);
        uint256 i = 0;
        bool isValidOwner = false;
        uint256 validTokenId = 0;
        ...
        return (validTokenId, isValidOwner);
    }
    ...
    function stake(uint256 amount) external nonReentrant notPaused updateReward(msg.sender) {
        ...
        (, bool isValidOwner) = checkIsValidEarlyNFTOwner(msg.sender);
        require(isValidOwner == true, "Invalid early holder");
    ...
    function setMetastrikeCore(address _core, uint256 _fromTokenId, uint256 _toTokenId) external onlyOwner {
        require(_core != address(0), "Invalid address");
        require(_fromTokenId <= _toTokenId, "Invalid tokenIds");
        core = MetastrikeCore(_core);
