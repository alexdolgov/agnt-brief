...
contract EtherFiViewer is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function EigenPod_withdrawableRestakedExecutionLayerGwei(address[] memory _etherFiNodes) external view returns (uint256[] memory _withdrawableRestakedExecutionLayerGwei) {
        _withdrawableRestakedExecutionLayerGwei = new uint256[](_etherFiNodes.length);
        for (uint256 i = 0; i < _etherFiNodes.length; i++) {
            IEigenPod eigenPod = IEigenPod(IEtherFiNode(_etherFiNodes[i]).eigenPod());
            _withdrawableRestakedExecutionLayerGwei[i] = eigenPod.withdrawableRestakedExecutionLayerGwei();
        }
    }
