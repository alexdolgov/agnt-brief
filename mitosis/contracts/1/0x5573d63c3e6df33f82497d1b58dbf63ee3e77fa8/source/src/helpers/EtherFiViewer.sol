...
contract EtherFiViewer is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function EtherFiNodesManager_splitBalanceInExecutionLayer(address[] memory _nodes) external view returns (uint256[] memory _withdrawalSafe, uint256[] memory _eigenPod, uint256[] memory _delayedWithdrawalRouter) {
        _withdrawalSafe = new uint256[](_nodes.length);
        _eigenPod = new uint256[](_nodes.length);
        _delayedWithdrawalRouter = new uint256[](_nodes.length);

        for (uint256 i = 0; i < _nodes.length; i++) {
            (_withdrawalSafe[i], _eigenPod[i], _delayedWithdrawalRouter[i]) = IEtherFiNode(_nodes[i]).splitBalanceInExecutionLayer();
        }
    }
    ...
    function EtherFiNodesManager_numAssociatedValidators(uint256[] memory _validatorIds) external view returns (uint256[] memory _numAssociatedValidators) {
        _numAssociatedValidators = new uint256[](_validatorIds.length);

        for (uint256 i = 0; i < _validatorIds.length; i++) {
            _numAssociatedValidators[i] = _getEtherFiNode(_validatorIds[i]).numAssociatedValidators();
        }
    }
