...
contract Cap is ICap, Router, Ownable2StepUpgradeable, CapStorageV1 {
  ...
  uint32 public constant MAX_REMOTE_DOMAIN = 20;
  ...
  /// @dev prevent enroll remote chain more than MAX_REMOTE_DOMAIN
  function _enrollRemoteRouter(uint32 domain, bytes32 router) internal override {
    if (_routers.uint32Keys().length + 1 > MAX_REMOTE_DOMAIN) {
      revert Error.MaxRemoteRouterReached(MAX_REMOTE_DOMAIN);
    }
    super._enrollRemoteRouter(domain, router);
  }
