  ...
  UUPSUpgradeable
{
  ...
  /// @notice Role for verifying signatures (keccak256("SIGNATURE_VERIFIER_ROLE"))
  bytes32 public constant SIGNATURE_VERIFIER_ROLE =
    0xd1f27dfd2772d9685a1b40c84a8cac99c3e88131d670de3826b5718bbc6f736d;
  ...
  // NOTE: basically, we can make this function to be called by anyone. but we restricted access to the operator role. for security reason.
  function claimWithdrawFromLista() external onlyRole(OPERATOR_ROLE) nonReentrant {
    uint256 totalClaimed = 0;
    uint256 requestCount = 0;

    uint256 requests = staking.getUserWithdrawalRequests(address(this)).length;
    uint256 totalClaimCount = 0;
    uint256 nextClaimIndex = 0;

    for (; requestCount < requests; requestCount++) {
      (bool isClaimable, uint256 amount) =
        staking.getUserRequestStatus(address(this), nextClaimIndex);
      ...
      if (!isClaimable) {
        // leave this index as unclaimable
        nextClaimIndex++;
        continue;
      }

      staking.claimWithdraw(nextClaimIndex);
      totalClaimed += amount;
      totalClaimCount++;
    }

    emit WithdrawalClaimed(requestCount, totalClaimCount, totalClaimed);
  }
