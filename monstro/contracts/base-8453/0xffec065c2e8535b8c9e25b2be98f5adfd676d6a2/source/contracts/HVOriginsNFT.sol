...
/**
 * @title HVOriginsNFT
 * @dev ERC721-compatible NFT contract with 3750 implicit tokens, holder tracking, batch burning, 
 ...
 */
contract HVOriginsNFT {
    // Immutable deployer address, set at construction
    address public immutable owner;

    // Fixed total supply of 3750 tokens (IDs 1–3750)
    uint256 public constant TOTAL_SUPPLY = 3750;

    // Private storage for token metadata
    string private _name;        // Token name (e.g., "Heroes and Villains: Origins")
    string private _symbol;      // Token symbol (e.g., "HVONFT")
    string private _baseTokenURI;// Base URI for token metadata (e.g., "https://meta.monstro.fun/hv/")
     ...
     * @param name_ Token name (e.g., "Heroes and Villains: Origins")
     * @param symbol_ Token symbol (e.g., "HVONFT")
     * @param initialBaseURI Base URI (e.g., "https://meta.monstro.fun/hv/")
     */
    constructor(string memory name_, string memory symbol_, string memory initialBaseURI) {
        ...
        _balances[msg.sender] = TOTAL_SUPPLY;  // Implicitly assign all 3750 tokens to deployer
    ...
    /**
     * @dev Returns the fixed total supply (3750).
     */
