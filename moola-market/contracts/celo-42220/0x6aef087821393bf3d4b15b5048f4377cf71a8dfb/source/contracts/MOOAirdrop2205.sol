...
contract MOOAirdrop2205 is Ownable {
    ...
    function claim(uint256 _index, address _account, uint256 _amount, bytes32 _merkleRoot, bytes32[] calldata _merkleProof) external {
        require(allowedMerkleRoots[_merkleRoot], 'MOOAirdrop2205: Invalid merkleRoot.');

        uint claimed = claimedAmount[_account];
        require(claimed < _amount, 'MOOAirdrop2205: Drop already claimed.');

        // Verify the merkle proof.
        bytes32 node = keccak256(abi.encodePacked(_index, _account, _amount));
        require(MerkleProof.verify(_merkleProof, _merkleRoot, node), 'MOOAirdrop2205: Invalid proof.');

        // Mark it claimed and send the token.
        claimedAmount[_account] = _amount;
        require(IERC20(token).transfer(_account, _amount - claimed), 'MOOAirdrop2205: Transfer failed.');
    ...
    function addRoot(bytes32 _newRoot) public onlyOwner() {
        require(!allowedMerkleRoots[_newRoot], 'MOOAirdrop2205: merkleRoot already exists.');
    ...
    function recover(address _to, bytes calldata _data) external onlyOwner() returns(bool, bytes memory) {
        if (_to == token) {
            require(block.timestamp > timeout, 'MOOAirdrop2205: not timed out yet.');
        }
