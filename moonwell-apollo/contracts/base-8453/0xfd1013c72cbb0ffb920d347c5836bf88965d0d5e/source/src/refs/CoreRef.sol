...
abstract contract CoreRef is Pausable {
    ...
    function pause() external hasAnyOfThreeRoles(Roles.ADMIN, Roles.GOVERNOR_DAO_ROLE, Roles.GUARDIAN) {
        _pause();
    }

    /// @notice set pausable methods to unpaused
    /// callable by admin or token governance
    function unpause() external hasAnyOfThreeRoles(Roles.ADMIN, Roles.GOVERNOR_DAO_ROLE, Roles.GUARDIAN) {
        _unpause();
    }
        ...
        returns (bytes[] memory returnData)
    {
        ...
        for (uint256 i; i < calls.length;) {
            ...
            require(target != address(0), "CoreRef: target cannot be address(0)");
