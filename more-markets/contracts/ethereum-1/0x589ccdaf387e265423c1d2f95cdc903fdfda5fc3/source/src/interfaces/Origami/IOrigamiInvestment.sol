...
interface IOrigamiInvestment is IERC20Metadata, IERC20Permit {
    ...
    /**
     * @notice The borrow and lend token of the investment
     */
    function borrowLend() external view returns (address);

    /**
     * @notice The supplied balance of the investment
     */
    function suppliedBalance() external view returns (uint256);

    /**
     * @notice The debt balance of the investment
     */
    function debtBalance() external view returns (uint256);

    /**
     * @notice The manager address
     */
    function manager() external view returns (address);

    /**
     * @notice The debt token to reserve token oracle address
     */
    function debtTokenToReserveTokenOracle() external view returns (address);
     ...
     * @notice The borrow and lend token of the investment
     */
    function assetsAndLiabilities(PriceType)
        external
        view
        returns (uint256 assets, uint256 liabilities, uint256 ratio);
    ...
    function latestPrice(PriceType priceType, Rounding rounding) external view returns (uint256);
}