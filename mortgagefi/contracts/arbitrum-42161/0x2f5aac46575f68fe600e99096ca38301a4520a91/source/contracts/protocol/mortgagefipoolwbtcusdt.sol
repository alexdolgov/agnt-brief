...
contract mortgagefipoolusdtwbtc is ERC20, Multicall, AccessControl {
    ...
    function createContract(uint256 size, uint256 duration, uint256 maxPaymentPerMonth, address ref) giveYield() external {
        IConversion(conversionVault).doUpdate();
        // duration in years
        ...
        uint256 stables = totalSupply + unclaimed + buffer + inExitLine - stablesInContracts;//
