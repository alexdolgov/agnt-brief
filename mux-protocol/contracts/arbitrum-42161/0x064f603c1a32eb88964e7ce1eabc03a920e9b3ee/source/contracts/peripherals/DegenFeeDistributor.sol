...
contract DegenFeeDistributor is Initializable, IDistributor {
    ...
    function initialize(address degenPool_, address referralManager_, address referralTiers_) external initializer {
     ...
     */
    function updateRewards(uint8 tokenId, address tokenAddress, address trader, uint256 rawAmount) external override {
        tokenId; // not used
        ...
        uint256 rawAmountToTrader = (rawAmount * discountRate) / 1e5;
        if (trader == address(0)) {
            // this should never happen, but just in case
            rawAmountToTrader = 0;
        }
        uint256 rawAmountToReferrer = (rawAmount * rebateRate) / 1e5;
        ...
        if (rawAmountToPool > 0) {
            IERC20Upgradeable(tokenAddress).safeTransfer(address(degenPool), rawAmountToPool);
        }
