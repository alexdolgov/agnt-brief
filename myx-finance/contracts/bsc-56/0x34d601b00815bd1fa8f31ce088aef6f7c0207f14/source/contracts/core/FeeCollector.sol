...
contract FeeCollector is IFeeCollector, ReentrancyGuardUpgradeable, Upgradeable {
    ...
    function updateRouterAddress(address newAddress) external onlyPoolAdmin {
        revert("deprecated");
    }
    ...
    function claimReferralFeeDelegate(address user) external override nonReentrant onlyRouter returns (uint256) {
        uint256 claimableReferralFee = referralFee[user];
        if (claimableReferralFee > 0) {
            referralFee[user] = 0;
            pool.transferTokenTo(pledgeAddress, user, claimableReferralFee);
        }
        emit ClaimedReferralsTradingFee(user, pledgeAddress, claimableReferralFee);
        return claimableReferralFee;
    }
    ...
    function claimUserTradingFeeDelegate(address user) external override nonReentrant onlyRouter returns (uint256) {
        uint256 claimableUserTradingFee = userTradingFee[user];
        if (claimableUserTradingFee > 0) {
            userTradingFee[user] = 0;
            pool.transferTokenTo(pledgeAddress, user, claimableUserTradingFee);
        }
        emit ClaimedUserTradingFee(user, pledgeAddress, claimableUserTradingFee);
        return claimableUserTradingFee;
    }
    ...
    function distributeTradingFee(
        ...
        if (referralOwner != address(0)) {
            ...
            referralUserAmount = surplusFee.mulPercentage(Math.min(referralUserRatio, Math.min(referralsRatio, maxReferralsRatio)));
