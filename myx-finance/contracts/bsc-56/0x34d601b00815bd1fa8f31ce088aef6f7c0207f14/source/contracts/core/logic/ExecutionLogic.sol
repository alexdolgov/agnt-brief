...
contract ExecutionLogic is IExecutionLogic {
    ...
    function executeIncreaseOrder(
        ...
        if (orderSize > 0) {
            (executionSize) = positionManager.exposureAmountChecker(
                pairIndex,
                order.isLong,
                orderSize,
                executionPrice
            ...
            bool makerOnly = orderManager.orderMakerOnly(order.orderId);
            if (makerOnly) {
                uint256 maxMakerSize = getMaxAvailableMakerSize(pairIndex, true, order.isLong);
                executionSize = Math.min(executionSize, maxMakerSize);
            }
        ...
        emit ExecuteOrderV2(
            ...
            TradingHelper.packExecuteEventFlags(true, order.isLong, false, closeOrder)
        );
    }
    ...
    function executeDecreaseOrder(
        ...
        bool makerOnly = orderManager.orderMakerOnly(order.orderId);
        if (makerOnly) {
            uint256 maxMakerSize = getMaxAvailableMakerSize(pairIndex, false, order.isLong);
            executionSize = Math.min(executionSize, maxMakerSize);
        }
        ...
        if (_needADL) {
            ...
            emit ExecuteOrderV2(
                ...
                0,
                TradingHelper.packExecuteEventFlags(false, order.isLong, _needADL, false)
            );
        ...
        emit ExecuteOrderV2(
            ...
            orderNetworkFee.paymentType,
            orderNetworkFee.networkFeeAmount,
            TradingHelper.packExecuteEventFlags(false, order.isLong, _needADL, closeOrder)
        );
    ...
    function getMaxAvailableMakerSize(uint256 pairIndex, bool isIncrease, bool isLong) private view returns (uint256) {
        uint256 longTracker = positionManager.longTracker(pairIndex);
        uint256 shortTracker = positionManager.shortTracker(pairIndex);

        if ((isIncrease && isLong) || (!isIncrease && !isLong)) {
            return shortTracker > longTracker ? shortTracker - longTracker : 0;
        } else {
            return longTracker > shortTracker ? longTracker - shortTracker : 0;
        }
    }
}
