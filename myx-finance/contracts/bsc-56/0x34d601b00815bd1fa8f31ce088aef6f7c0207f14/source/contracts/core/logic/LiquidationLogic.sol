    ...
    function _executeLiquidationOrder(
        ...
        if (needADL) {
            ...
            emit ExecuteOrderV2(
                ...
                0,
                TradingHelper.packExecuteEventFlags(false, order.isLong, needADL, false)
            );
            return;
        }
        ...
        position = positionManager.getPosition(order.account, pairIndex, order.isLong);
        if (position.positionAmount == 0) {
        ...
        emit ExecuteOrderV2(
            ...
            TradingHelper.packExecuteEventFlags(false, order.isLong, needADL, true)
        );
    }
