...
contract Facet is Ownable, Upgradeable {
    ...
    struct HyperVaultData {
        address token;
        string symbol;
        uint256 amount;
    }

    function getHyperVaultData() external view returns (HyperVaultData[] memory) {
        address[] memory tokens = IHyperVault(hyperVault).getAcceptableAssets();
        ...
        for (uint256 i = 0; i < tokens.length; i++) {
            ...
            data.amount = IHyperVault(hyperVault).hyperVaults(tokens[i]);
            data.token = tokens[i];
        }
