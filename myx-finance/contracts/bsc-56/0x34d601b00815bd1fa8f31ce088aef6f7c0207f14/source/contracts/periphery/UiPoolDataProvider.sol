...
contract UiPoolDataProvider is IUiPoolDataProvider {
    ...
    function getPairsData(
        ...
        ITrustedForwarder trustedForwarder,
        ...
        for (uint256 i = 0; i < pairIndexes.length; i++) {
            ...
            IMarket.OperationStatus memory operationStatus = router.getOperationStatus(pairIndex);
            ...
            if (pairData.lpPrice > 0) {
                uint256 indexAvailable = vault.indexTotalAmount > vault.indexReservedAmount ? vault.indexTotalAmount - vault.indexReservedAmount : 0;
                uint256 stableAvailable = vault.stableTotalAmount > vault.stableReservedAmount ? vault.stableTotalAmount - vault.stableReservedAmount : 0;
            ...
            pairData.gasFee = trustedForwarder.pledgeFee();
        }
        return pairsData;
    }
    ...
    function getHyperVaultData(
        IHyperVault hyperVault
    ) external view returns (HyperVaultData[] memory) {
        address[] memory tokens = hyperVault.getAcceptableAssets();
        HyperVaultData[] memory hyperVaultData = new HyperVaultData[](tokens.length);

        for (uint256 i = 0; i < tokens.length; i++) {
            HyperVaultData memory data = hyperVaultData[i];
            address hyperToken = hyperVault.getHyperPair(tokens[i]).hyperToken;
            data.symbol = IERC20Metadata(tokens[i]).symbol();
            data.hyperSymbol = IERC20Metadata(hyperToken).symbol();
            data.amount = hyperVault.hyperVaults(tokens[i]);
            data.token = tokens[i];
            data.hyperToken = hyperToken;
        }
        return hyperVaultData;
    }
}
