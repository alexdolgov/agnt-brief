...
contract Pool is IPool, Upgradeable {
    ...
    modifier onlyRouter() {
        require(isAllowedRouters[msg.sender], "not allowed router");
        _;
    }
    ...
    function _swapInUni(uint256 _pairIndex, address _tokenOut, uint256 _expectAmountOut) private {
        ...
        if (IERC20(tokenIn).allowance(address(this), spotSwap) < amountInMaximum) {
            IERC20(tokenIn).forceApprove(spotSwap, type(uint256).max);
        }

        (uint256 outAmount, uint256 useAmountIn) = ISpotSwap(spotSwap).swap(tokenIn, tokenOut, amountInMaximum, expectAmountOut);
        Vault storage vault = vaults[_pairIndex];
        if (tokenOut == pair.indexToken) {
            vault.indexTotalAmount += outAmount;
        } else if (tokenOut == pair.stableToken) {
            vault.stableTotalAmount += outAmount;
        }
        emit PairSwap(_pairIndex, tokenIn, useAmountIn, tokenOut, outAmount);
    }
