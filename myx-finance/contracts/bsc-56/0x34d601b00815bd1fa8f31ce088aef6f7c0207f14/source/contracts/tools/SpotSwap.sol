...
contract SpotSwap is ISpotSwap, Upgradeable {
    ...
    function swap(address tokenIn, address tokenOut, uint256 amountIn, uint256 amountOut) external returns (uint256 outAmount, uint256 useAmountIn) {
        ...
        useAmountIn = IUniSwapV3Router(swapRouter).exactOutput(
        ...
        if (useAmountIn < amountIn) {
            IERC20(tokenIn).safeTransfer(msg.sender, amountIn - useAmountIn);
        }
    }
}
