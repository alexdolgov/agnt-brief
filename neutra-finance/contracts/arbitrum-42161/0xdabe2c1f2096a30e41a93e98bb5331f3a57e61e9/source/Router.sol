...
contract Router is ReentrancyGuard, Governable {
    ...
    event ConfirmAndBuy(uint256 wantPendingAmount, uint256 mintAmount, uint256 value);
    ...
    event LastTotalValue(uint256 totalValue);
    ...
    */
    function executePositionsBeforeDealGlp(uint256 _amount, bytes[] calldata _params, bool _isWithdraw) external payable onlyHandler {
        if (!_isWithdraw) {
            ...
            lastTotalValue = IStrategyVault(strategyVault).totalValue();
            ...
            emit LastTotalValue(lastTotalValue);
    ...
    */
    function confirmAndBuy(uint256 _wantAmount, address _recipient) external onlyHandler returns (uint256) {
        ...
        uint256 mintAmount = totalSupply == 0 ? value * (10 ** decimals) / PRICE_PRECISION : value * totalSupply / lastTotalValue;
        ...
        emit ConfirmAndBuy(pendingAmountsWant, mintAmount, value);
