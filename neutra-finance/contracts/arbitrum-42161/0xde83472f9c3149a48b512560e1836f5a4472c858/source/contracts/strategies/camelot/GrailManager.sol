...
contract GrailManager is INFTHandler, Initializable, UUPSUpgradeable {
    ...
    function initialize(address _manager, address _strategy, GrailManagerConfig memory _config) external initializer {
    ...
    function getPendingRewards() public view returns (uint256, uint256) {
        ...
        uint256 xGrailRewards = pending.mul(pool.xGrailRewardsShare()).div(_TOTAL_REWARDS_SHARES);
    ...
    function onNFTHarvest(
        ...
        require(_operator == address(this), "caller is not the nft previous owner");
    ...
    function onNFTWithdraw(address _operator, uint256 _tokenId, uint256 /*lpAmount*/) external override returns (bool) {
        require(msg.sender == address(pool), "unexpected nft");
        require(_operator == address(this), "NFTHandler: caller is not the nft previous owner");
