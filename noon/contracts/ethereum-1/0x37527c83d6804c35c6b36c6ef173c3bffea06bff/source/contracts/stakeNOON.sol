    ...
    IStakeNOON
{
    ...
    /// @dev Percentage of VIP claim amount that gets staked (1-100). 0 = 100% (backward compat).
    uint256 public vipStakePercentage;
    ...
    function claimAndStake(uint256 amount, bytes32[] calldata proof) external nonReentrant returns (uint256) {
        ...
        // Apply stake percentage: only a portion is staked, the rest goes to wallet
        // vipStakePercentage == 0 means 100% (backward compatible after upgrade)
        uint256 pct = vipStakePercentage == 0 ? 100 : vipStakePercentage;
        uint256 stakeAmount = (additionalAmount * pct) / 100;
    ...
    /**
     * @dev Sets the percentage of VIP claim amount that gets staked.
     *      The remainder is sent directly to the user's wallet.
     * @param _percentage Percentage to stake (1-100). 100 = stake everything.
     */
    function setVipStakePercentage(uint256 _percentage) external onlyOwner {
        if (_percentage == 0 || _percentage > 100) revert InvalidPercentage();
        vipStakePercentage = _percentage;
        emit VipStakePercentageUpdated(_percentage);
    }
