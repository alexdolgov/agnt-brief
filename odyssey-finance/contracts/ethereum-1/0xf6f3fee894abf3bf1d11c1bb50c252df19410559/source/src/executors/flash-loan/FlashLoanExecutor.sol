 ...
 * @notice Abstract implementation to perform flash loan on behalf of the smart account
 */
abstract contract FlashLoanExecutor is IERC3156FlashBorrower {
    ...
    function start(
        ...
        _revertIfNotAuthorized(_smartAccount);
    ...
    function onFlashLoan(
        ...
        ) = abi.decode(data_, (address, address, SwapParams, Execution[]));
        ...
        // 2. Execute logic
        _executeOnBehalf(_smartAccount, _executions);
        ...
        if (_excess > 0) {
            IERC20(token_).safeTransfer(_smartAccount, _excess);
        }
