...
contract FuelRouterDT is IFuelRouterDT, FuelRouter {
    ...
    function routeFuelForPrimarySale(
        ...
        external
        override(FuelRouter, IFuelRouterDT)
        onlyEventImplementation
        returns (uint256 _totalFuelValue, uint256 _totalFuelTokens)
    {
        bool isEnabled = registry.economicsFactory().isIntegratorEnabled(integratorId);
        if (!isEnabled) return (0, 0);

        (_totalFuelValue, _totalFuelTokens) = _routePrimarySale(_ticketActions);
    }
    ...
    function _routePrimarySale(
        IEventImplementation.TicketAction[] calldata _ticketActions
    ) internal returns (uint256 _totalFuel, uint256 _protocolFuel) {
        uint256 _totalFee = _calculateProductFeePrimary(_ticketActions);
        uint256 _protocolFee = _calculateBaseFee(_ticketActions.length);
        uint256 _treasuryFee = _totalFee - _protocolFee;
        ...
        return (_totalFuel, _protocolFuel);
    }
