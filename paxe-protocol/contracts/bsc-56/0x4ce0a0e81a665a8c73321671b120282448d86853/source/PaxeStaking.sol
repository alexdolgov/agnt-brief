...
contract PaxeStaking is Ownable, ReentrancyGuard, IErrors, IEvents {
    ...
    uint256 constant REFERRAL_REWARDS_THRESHOLD = 2000 ether;
    uint256 constant DEPOSIT_FEE = 0.01 ether;
    uint256 constant MAGNITUDE = 1e8;
    uint256[5] public REFERRAL_GENERATION_REWARDS = [
        50e6,
        30e6,
        20e6,
        10e6,
    ...
    uint256[5] public rates = [300e6, 800e6, 1800e6, 3000e6, 4000e6];
    ...
    function _handleReferralRewards(uint256 pPAXERewards) internal {
        ...
        for (uint256 i = 0; i < 5; i++) {
            ...
            if (!_isReferralRewardsEligible(_referrer)) {
                _referee = _referrer;
                continue;
            }

            // calculate rewards based on generation and go to next generation
            uint256 rewards = (pPAXERewards * REFERRAL_GENERATION_REWARDS[i]) /
                MAGNITUDE;
    ...
    function _isReferralRewardsEligible(
        ...
        for (uint256 i = 0; i < userDeposit.length; i++) {
            ...
            if (
                ...
                activeDeposits += userDeposit[i].value;
            }
            if (activeDeposits >= REFERRAL_REWARDS_THRESHOLD) return true;
        }
        return false;
    }
