...
interface IAccessHub {
    ...
    function xRam() external view returns (IXPhar xRam);

    /// @notice P33 contract
    function r33() external view returns (IP33 r33);
    ...
    function autoVault() external view returns (IAutoVault autoVault);
    ...
    function rescueAndWipeAutoVault(address _token) external;

    /// @notice sets the legacy router address for LP unwrapping in AutoVault
    /// @param _legacyRouter the legacy router address
    function setLegacyRouterAutoVault(address _legacyRouter) external;
}
