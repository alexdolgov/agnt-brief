...
import {IDLMMFlashLoanCallback} from "./interfaces/IDLMMFlashLoanCallback.sol";
import {IDLMMPool} from "./interfaces/IDLMMPool.sol";
import {DLMMToken, IDLMMToken} from "./DLMMToken.sol";
...
/// @title Liquidity Bin Pair
///
/// @notice The Liquidity Bin Pair contract is the core contract of the DLMM protocol
contract DLMMPool is DLMMToken, ReentrancyGuardUpgradeable, Clone, IDLMMPool {
    ...
    /// @dev Constructor for the Liquidity Bin Pair contract that sets the Liquidity Bin Factory
    /// @param factory_ The Liquidity Bin Factory
    constructor(IDLMMFactory factory_) {
        _factory = factory_;
    ...
    /// @notice Initialize the Liquidity Bin Pair fee parameters and active id
    /// @dev Can only be called by the Liquidity Bin Factory
    ...
    /// @param activeId The active id of the Liquidity Bin Pair
    function initialize(
        uint16 baseFactor,
        uint16 filterPeriod,
        uint16 decayPeriod,
    ...
    /// @notice Returns the Liquidity Bin Factory
    /// @return factory The Liquidity Bin Factory
    function getFactory() external view override returns (IDLMMFactory factory) {
        return _factory;
    }

    /// @notice Returns the token X of the Liquidity Bin Pair
    /// @return tokenX The address of the token X
    function getTokenX() external pure override returns (IERC20 tokenX) {
        return _tokenX();
    }

    /// @notice Returns the token Y of the Liquidity Bin Pair
    /// @return tokenY The address of the token Y
    function getTokenY() external pure override returns (IERC20 tokenY) {
        return _tokenY();
    }

    /// @notice Returns the bin step of the Liquidity Bin Pair
    ...
    /// @return binStep The bin step of the Liquidity Bin Pair, in 10_000th
    function getBinStep() external pure override returns (uint16) {
        return _binStep();
    }

    /// @notice Returns the reserves of the Liquidity Bin Pair
    ...
    /// @notice Returns the active id of the Liquidity Bin Pair
    /// @dev The active id is the id of the bin that is currently being used for swaps.
    /// The price of the active bin is the price of the Liquidity Bin Pair and can be calculated as follows:
    /// `price = (1 + binStep / 10_000) ^ (activeId - 2^23)`
    /// @return activeId The active id of the Liquidity Bin Pair
    ...
    /// @notice Returns the protocol fees of the Liquidity Bin Pair
    ...
    /// @notice Returns the static fee parameters of the Liquidity Bin Pair
    ...
    /// @notice Gets the hooks parameters of the Liquidity Bin Pair
    /// @return The hooks parameters of the Liquidity Bin Pair
    function getLBHooksParameters() external view override returns (bytes32) {
        return _hooksParameters;
    }

    /// @notice Returns the variable fee parameters of the Liquidity Bin Pair
    ...
    /// @notice Returns the oracle parameters of the Liquidity Bin Pair
    ...
    /// @notice Returns the cumulative values of the Liquidity Bin Pair at a given timestamp
    ...
    /// @return cumulativeId The cumulative id of the Liquidity Bin Pair at the given timestamp
    /// @return cumulativeVolatility The cumulative volatility of the Liquidity Bin Pair at the given timestamp
    /// @return cumulativeBinCrossed The cumulative bin crossed of the Liquidity Bin Pair at the given timestamp
    ...
    function flashLoan(IDLMMFlashLoanCallback receiver, bytes32 amounts, bytes calldata data) external override {
        _nonReentrantBefore();
        ...
        (bool success, bytes memory rData) = address(receiver).call(
            abi.encodeWithSelector(
                IDLMMFlashLoanCallback.LBFlashLoanCallback.selector,
                msg.sender,
                _tokenX(),
                _tokenY(),
                amounts,
    ...
    /// It will mint Liquidity Bin (LB) tokens for each bin where the user adds liquidity.
    /// This function will not transfer the tokens from the caller, it is expected that the tokens have already been
    /// transferred to this contract through another contract, most likely the router.
    ...
    /// @notice Burn Liquidity Bin (LB) tokens and withdraw tokens from the pool.
        ...
        onlyFactory
    {
        ...
        IDLMMHooks hooks = IDLMMHooks(Hooks.getHooks(hooksParameters));
        ...
        override(DLMMToken, IDLMMToken)
    {
        ...
        DLMMToken.batchTransferFrom(from, to, ids, amounts);
