...
contract OrdererAvax is IOrderer, IOrdererV2, UUPSUpgradeable, ERC165Upgradeable {
    ...
    function externalSwap(ExternalSwapV2 calldata _info) external override onlyRole(ORDERING_EXECUTOR_ROLE) {
        ...
        require(
            IAccessControl(registry).hasRole(EXCHANGE_FACTORY_ROLE, _info.swapTarget),
            "OrdererAvax: INVALID_TARGET"
        );
        require(_info.swapData.length != 0, "OrdererAvax: INVALID_DATA");
    ...
    function _validatedOrder(
        ...
        internal
        view
        returns (uint lastOrderId, AssetDetails storage orderSellAsset, AssetDetails storage orderBuyAsset)
    {
