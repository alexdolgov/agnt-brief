...
pragma solidity ^0.8.0;
contract PUSDToken is ERC20Upgradeable,OwnableUpgradeable {

    bool public isUseBlackList;

    mapping(address => bool) public blackList;

    event SetBlackList(bool isUseWhiteList,address[] addrs,bool[] valids);
    ...
    function mint(address user_,uint256 amount) public onlyOwner {
        _mint(user_, amount);
    }
    
    function burnFrom(address user_,uint256 amount) public onlyOwner {
        _burn(user_, amount);
    }
    ...
    function setBlackList(bool isUseBlackList_,address[] calldata addrs_,bool[] calldata valids_) external onlyOwner {
        isUseBlackList = isUseBlackList_;
        if(isUseBlackList == true){
            require(addrs_.length == valids_.length, "size");
            for(uint i = 0; i < addrs_.length; i++) {
                blackList[ addrs_[i] ] = valids_[i];
            }
        }
        emit SetBlackList(isUseBlackList_,addrs_,valids_);
    }
