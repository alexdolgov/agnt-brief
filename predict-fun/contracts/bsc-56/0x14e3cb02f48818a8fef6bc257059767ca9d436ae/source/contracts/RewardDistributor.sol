 ...
 */
contract RewardDistributor is Pausable, ReentrancyGuard, AccessControl {
    ...
    // Total amount claimed by all users (sum of all claims)
    uint256 public totalAmountClaimedByAllUsers;
     ...
     */
    constructor(address _rewardToken, address _owner, address _operator) {
    ...
    function claim(uint256 amount, bytes32[] calldata merkleProof) external whenNotPaused nonReentrant {
        ...
        // Update total claimed by all users
        totalAmountClaimedByAllUsers += adjustedAmount;
     ...
     */
    function withdrawToken(address currency, uint256 amount) external onlyRole(OPERATOR_ROLE) {
