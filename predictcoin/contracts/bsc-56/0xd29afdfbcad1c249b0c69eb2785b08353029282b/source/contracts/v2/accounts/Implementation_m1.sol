...
contract Constants is Variables {
    // BxdIndex Address.
    address internal immutable bxdIndex;
    // Connnectors Address.
    address public immutable connectorsM1;

    constructor(address _bxdIndex, address _connectors) {
        connectorsM1 = _connectors;
        bxdIndex = _bxdIndex;
    }
}

contract BxdImplementationM1 is Constants {

    constructor(address _bxdIndex, address _connectors) Constants(_bxdIndex, _connectors) {}
    ...
    */
    ...
    external
    payable 
    returns (bytes32) // Dummy return to fix bxdIndex buildWithCast function
    {   
        uint256 _length = _targetNames.length;
        require(_auth[msg.sender] || msg.sender == bxdIndex, "1: permission-denied");
