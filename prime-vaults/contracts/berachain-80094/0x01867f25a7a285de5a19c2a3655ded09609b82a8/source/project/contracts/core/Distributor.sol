...
contract Distributor is PrimeAuth, ReentrancyGuard, IBeforeUpdateHook {
    ...
    /// @notice account -> reward token -> amount of reward per token paid
    mapping(address => mapping(address => uint256)) public userRewardPerTokenPaid;

    /// @notice account -> reward token -> accumulated rewards
    ...
    event RewardPaid(address indexed account, address indexed rewardsToken, uint256 reward, address to);
    ...
    event ThirdPartyCompoundAllowed(address indexed account, bool allowed);
    ...
    event RewardUpdated(address indexed account, address indexed rewardsToken, uint256 earned);
    ...
    /**
     * @notice Set reward rate and duration for a reward token
     * @param _rewardsToken The reward token address
     * @param _rewardRate The new reward rate (tokens per second)
     * @param _rewardsDuration The new rewards duration
     * @dev Starts a new reward period immediately
     * @dev Callable by OPERATOR_ROLE
     */
    function setReward(address _rewardsToken, uint256 _rewardRate, uint256 _rewardsDuration) external onlyOperator updateReward(address(0)) {
        if (_rewardsDuration == 0) revert Distributor__ZeroDuration();

        rewardData[_rewardsToken].rewardsDuration = _rewardsDuration;
        rewardData[_rewardsToken].rewardRate = _rewardRate;
        ...
        emit RewardAdded(_rewardRate * _rewardsDuration);
    }
    ...
    function updateRewardForAccount(address account) public {
        for (uint256 i = 0; i < rewardTokens.length; i++) {
            ...
            uint256 newEarned = earned(account, token);
            rewards[account][token] = newEarned;
            userRewardPerTokenPaid[account][token] = rewardData[token].rewardPerTokenStored;

            emit RewardUpdated(account, token, newEarned);
        }
    }

    function updateRewardForAccountBatch(address[] memory accounts) public {
        for (uint256 j = 0; j < accounts.length; j++) {
            updateRewardForAccount(accounts[j]);
        }
    }
