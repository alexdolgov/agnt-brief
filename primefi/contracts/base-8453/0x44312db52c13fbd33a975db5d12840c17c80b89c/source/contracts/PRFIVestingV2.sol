    ...
    IPRFIVesting
{
    ...
    function migrateVesting(address beneficiary) internal {
        PRFIVestingV2Storage storage $ = _getPRFIVestingV2Storage();
        require(address($.oldPrfiVesting) != address(0), MigrationNotAllowed());
        IPRFIVesting.VestingSchedule memory oldSchedule = $.oldPrfiVesting
            .vestingSchedules(beneficiary);
        if (oldSchedule.totalAmount == 0) {
            return; // No schedule to migrate
        }
        VestingSchedule storage newSchedule = $.vestingSchedules[
            beneficiary
            ];
            ...
            $.totalVestingAmount += oldSchedule.totalAmount;
            ...
            emit TokensClaimed(beneficiary, oldSchedule.claimedAmount);
    }
    ...
    function claimTokens() external nonReentrant {
        ...
        if (schedule.totalAmount == 0) {
            if (address($.oldPrfiVesting) != address(0)) {
                IPRFIVesting.VestingSchedule memory oldSchedule = $.oldPrfiVesting
                    .vestingSchedules(msg.sender);
                if (oldSchedule.totalAmount != 0) {
                    // migrate caller's schedule from V1
                    migrateVesting(msg.sender);
                }
            }
        }

        // Refresh reference in case it was migrated
        schedule = $.vestingSchedules[msg.sender];
