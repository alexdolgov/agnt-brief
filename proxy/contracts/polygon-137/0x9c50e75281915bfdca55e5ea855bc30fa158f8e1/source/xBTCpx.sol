...
contract xBTCpx is Ownable, ERC20Burnable {
    ...
    constructor(uint256 _stakingStartDate) ERC20("Multiply BTCpx", "xBTCpx") {
    ...
    function updateTreasury(address _daoWallet) external onlyOwner {
    ...
    function unstake(uint256 _stakeId) public updateReward(_msgSender()) {
        ...
        IERC20(BTCpx).safeTransferFrom(daoWallet, _msgSender(), _stakeAmount);
        emit Unstaked(
            _msgSender(),
            userStake[_msgSender()],
            _stakeAmount,
    ...
    function getTVLInUsd() external view returns (uint256) {
        return (totalSupply().mul(getBtcPriceInUsd())).div(1 * 10**(18));
    }
