...
// File contracts/oracles/MaigUniOracle.sol
...
contract MaiGUniOracle is IPriceSource {
    ...
    constructor(address _pool, address _feed0, address _feed1) public {
        uint256 dec0 = uint256(IPriceSource(IGUniPool(_pool).token0()).decimals());
        require(dec0 <= 18, "token0-dec-gt-18");
        UNIT_0 = 10 ** dec0;
        TO_WAD_0 = 10 ** (18 - dec0);
        uint256 dec1 = uint256(IPriceSource(IGUniPool(_pool).token1()).decimals());
        require(dec1 <= 18, "token1-dec-gt-18");
        UNIT_1 = 10 ** dec1;
        TO_WAD_1 = 10 ** (18 - dec1);
        uint256 decOracle0 = uint256(IPriceSource(_feed0).decimals());
        require(decOracle0 <= 18, "oracle0-dec-gt-18");
        TO_WAD_ORACLE_0 = 10 ** (18 - decOracle0);
        uint256 decOracle1 = uint256(IPriceSource(_feed1).decimals());
    ...
    function getTokenType() external pure override returns (IPriceSource.TokenType) {
        return IPriceSource.TokenType.Complex;
    }
    ...
    function _getWADPrice(bool isToken0)
        ...
        returns (uint256)
    {
        int256 price = EAC(isToken0 ? priceFeed0 : priceFeed1).latestAnswer();
