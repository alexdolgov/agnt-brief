...
import {ISlidingFeePlugin} from '@cryptoalgebra/integral-base-plugin/contracts/interfaces/plugins/ISlidingFeePlugin.sol';
import '@cryptoalgebra/integral-core/contracts/interfaces/IAlgebraFactory.sol';
import './VolatilityOraclePlugin.sol';


abstract contract SlidingFeePlugin is ISlidingFeePlugin, VolatilityOraclePlugin {
  struct FeeFactors {
  ...
  function setPriceChangeFactor(uint16 newPriceChangeFactor) external override {
    if(!IAlgebraFactory(factory).hasRoleOrOwner(ALGEBRA_BASE_PLUGIN_MANAGER, msg.sender)) revert NotAllowed();
