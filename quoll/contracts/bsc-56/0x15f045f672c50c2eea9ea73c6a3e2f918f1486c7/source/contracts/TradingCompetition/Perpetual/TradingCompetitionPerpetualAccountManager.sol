...
contract TradingCompetitionPerpetualAccountManager is ITradingCompetitionPerpetualAccountManager, MultiAccount {
    ...
    /// @notice Get the weights length for the prize
    function getWeightsLength() external view returns(uint256 length) {
        return _weightsLength();
    }

    function _weightsLength() internal view returns(uint256 length) {
        return tc_data.prize.weights.length;
    }
    ...
    function isAccountValid(address user, uint256 start, uint256 size) external view returns(bool) {
        ...
        // 1 correct quote is sufficient to be valid
    ...
    function _getPnLOfUser(address user) internal view returns(int256 amount, bool isValid) {
        address account = _getAccountOf(user);
        if(account == address(0)) return(0, false);

        uint quoteslength = _getQuotesLength(account);

        if(quoteslength == 0) return (0,false);

        uint256[] memory quoteIds = new uint256[](quoteslength);
        quoteIds = _getQuoteIdOf(account, 0, quoteslength);
        ...
        bool _isValid = false;
        int256 tmp_amnt = 0;
        for(uint256 i = 0; i < quoteslength; i++) {
            (tmp_amnt, _isValid) = _processQuoteWithPnL(quoteIds[i], startTimestamp, endTimestamp);
            if(_isValid){
                // if there's a valid quote set isValid to true and increase amount
                if(!isValid) isValid = _isValid;
                amount += tmp_amnt;
            }
        }
    }
    ...
    function _getQuotesLength(address account) internal view returns(uint256 length) {
        bytes memory _callData = abi.encodeWithSignature(
    ...
    function _getQuoteIdOf(address account, uint256 start, uint256 quoteslength) internal view returns(uint256[] memory quoteIds) {
        bytes memory _callData = abi.encodeWithSignature(
            "quoteIdsOf(address,uint256,uint256)",
            account,
            start,
        ...
        quoteIds = abi.decode(_resultData, (uint256[])); 
    }
    ...
    function _verifyQuote(Quote memory q, uint256 startTimestamp, uint256 endTimestamp) internal view returns(bool) {
        ...
        // quote must be opened and closed/liquidated between competition period and must be a valid pair
        if(q.quoteStatus == QuoteStatus.CLOSED || q.quoteStatus == QuoteStatus.LIQUIDATED){
            ...
            return _isValidPair[q.symbolId];
        }
