...
contract ThenaBasePlugin is SlidingFeePlugin, FarmingProxyPlugin, DynamicFeesPlugin, Pausable {
    ...
    function beforeModifyPosition(address, address, int24, int24, int128, bytes calldata) external override onlyPool whenNotPaused returns (bytes4, uint24) {
    ...
    function beforeSwap(address, address, bool zeroToOne, int256, uint160, bool, bytes calldata) external override onlyPool whenNotPaused returns (bytes4, uint24, uint24) {
        uint16 newFee = 0;
        if(feeType) {
    ...
    /// @inheritdoc IThenaBasePlugin
    /// @dev This hook is unused 
    function beforeFlash(address, address, uint256, uint256, bytes calldata) external override onlyPool whenNotPaused returns (bytes4) {
        _updatePluginConfigInPool(defaultPluginConfig); // should not be called, reset config
    ...
    function setFeeType(bool _feeType) external {
        if(msg.sender != pluginFactory) revert NotAllowed();
        emit FeeTypeUpdated(feeType, _feeType);
