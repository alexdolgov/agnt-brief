...
contract TokenizedStrategy {
    ...
    function requireManagement(address _sender) public view {
        if (_sender != _strategyStorage().management) {
            revert Errors.NotManagement();
        }
    }
    ...
    function requireKeeperOrManagement(address _sender) public view {
        StrategyData storage S = _strategyStorage();
        if (_sender != S.keeper && _sender != S.management) {
    ...
    function requireEmergencyAuthorized(address _sender) public view {
        StrategyData storage S = _strategyStorage();
        if (_sender != S.emergencyAdmin && _sender != S.management) {
            revert Errors.NotEmergencyAuthorized();
        }
    }
    ...
    function initialize(
        ...
        if (_performanceFeeRecipient == address(this)) {
            revert Errors.SelfMint();
        }
    ...
    function deposit(uint256 assets, address receiver) external nonReentrant returns (uint256 shares) {
        ...
        if (assets > _maxDeposit(S, receiver)) {
            revert Errors.DepositMoreThanMax();
        }
    ...
    function mint(uint256 shares, address receiver) external nonReentrant returns (uint256 assets) {
        ...
        if (shares > _maxMint(S, receiver)) {
            revert Errors.MintMoreThanMax();
        }
        // Check for rounding error.
        assets = _convertToAssets(S, shares, Math.Rounding.Up);
        if (assets == 0) {
        ...
        returns (uint256 shares)
    {
        // Get the storage slot for all following calls.
        StrategyData storage S = _strategyStorage();
        if (assets > _maxWithdraw(S, owner)) {
            revert Errors.WithdrawMoreThanMax();
        }
        // Check for rounding error or 0 value.
        shares = _convertToShares(S, assets, Math.Rounding.Up);
        if (shares == 0) {
            revert Errors.ZeroShares();
        }
        ...
        returns (uint256)
    {
        // Get the storage slot for all following calls.
        StrategyData storage S = _strategyStorage();
        if (shares > _maxRedeem(S, owner)) {
            revert Errors.RedeemMoreThanMax();
        }
        uint256 assets;
        // Check for rounding error or 0 value.
        ...
        if (idle < assets) {
            ...
            if (idle < assets) {
                ...
                if (maxLoss < MAX_BPS) {
                    ...
                    if (loss > (assets * maxLoss) / MAX_BPS) {
                        revert Errors.TooMuchLoss();
                    }
                }
    ...
    function emergencyWithdraw(uint256 amount) external nonReentrant onlyEmergencyAuthorized {
        ...
        if (!_strategyStorage().shutdown) {
            revert Errors.NotShutdown();
        }
    ...
    function acceptManagement() external {
        StrategyData storage S = _strategyStorage();
        if (msg.sender != S.pendingManagement) {
            revert Errors.NotPending();
        }
    ...
    function setEmergencyAdmin(address _emergencyAdmin) external onlyManagement {
        if (_strategyStorage().emergencyAdmin != _emergencyAdmin) {
    ...
    function setPerformanceFee(uint16 _performanceFee) external onlyManagement {
        if (_performanceFee > MAX_FEE) {
            revert Errors.ExceedsMaxFee();
        }
    ...
    function setProfitMaxUnlockTime(uint256 _profitMaxUnlockTime) external onlyManagement {
        ...
        if (_profitMaxUnlockTime > SECONDS_PER_YEAR) {
    ...
    function _mint(StrategyData storage S, address account, uint256 amount) internal {
        if (account == address(0)) {
            revert Errors.CannotMintToAddressZero();
        }
    ...
    function _burn(StrategyData storage S, address account, uint256 amount) internal {
        if (account == address(0)) {
            revert Errors.CannotBurnToAddressZero();
        }
        ...
        external
    {
        if (deadline < block.timestamp) {
            revert Errors.PermitDeadlineExpired();
        }
