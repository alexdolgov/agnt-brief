	...
	Ownable
{
	...
	function fetchSpot(address _tokenIn, address _tokenOut, uint256 _amountIn) public view returns (uint256 amountOut) {
		return UniV3PoolMath.getQuoteAtTick(currentTick(), UniV3PoolMath.uint128Safe(_amountIn), _tokenIn, _tokenOut);
	}
	...
	function _amountsForLiquidity(
		...
		(uint160 sqrtRatioX96, , , ) = _queryPoolSlot0();
		return
		...
		returns (uint160 sqrtPriceX96, int24 tick, uint16 observationIndex, bool unlocked)
	{
		try IUniswapV3Pool(pool).slot0() returns (
			...
			try ICLPool(pool).slot0() returns (
				uint160 sqrtPriceX96_,
				int24 tick_,
				uint16 observationIndex_,
				uint16,
				...
				revert UniV3TokenizedLp_failedToQueryPool();
			}
		}
	}
