...
/// @author Radiant
contract UniV3PoolHelperTemp is IPoolHelper, Initializable, OwnableUpgradeable {
	...
	function initializePool(address lptoken_) external onlyOwner {
		if (address(tokenizedLpToken) != address(0)) revert AlreadyInitialized();
		tokenizedLpToken = UniV3TokenizedLp(lptoken_);
	 ...
	 * The amounts sent will initialize the pool at the computed `sqrtPriceX96`, and will be
	 * deployed in full range to the pool. The liquidity ownership will remain locked in this contract.
	 * The init price is trivial as it will be adjusted to the external oracle in the `autoRebalance` call.
	 * It is recommended to call `autoRebalance` after this function has been called.
	 */
	function initializePool(InitializeParams memory params) public onlyOwner {
...

		{
			...
			int24 baseLower_ = UniV3PoolMath.roundTick(UniV3PoolMath.MIN_TICK, tickSpacing);
			int24 baseUpper_ = UniV3PoolMath.roundTick(UniV3PoolMath.MAX_TICK, tickSpacing);
			...
			(uint256 amount0, uint256 amount1) = rdntAddr == token0
				? (initRdntBal, initWeth9Bal)
				: (initWeth9Bal, initRdntBal);
		...
		tokenizedLpToken.setApprovedRebalancer(address(this), false);
		tokenizedLpToken.setApprovedRebalancer(msg.sender, true);
		tokenizedLpToken.setFeeRecipient(msg.sender);
	...
	/**
	 * @notice Gets needed WETH9 (ESTIMATE) for adding LP.
	 * Does not consider pending fees to be collected.
	 * @param lpAmount LP amount
	 * @return weth9Amount WETH9 amount
	 */
	function quoteWETH(uint256 lpAmount) public view returns (uint256 weth9Amount) {
		uint256 lpPriceInWeth9 = getLpPrice(lpAmount);
		weth9Amount = (lpPriceInWeth9 * lpAmount) / PRECISION;
	}

	/**
	 * @notice Zap WETH9 into LP
	 * @param amount of WETH9
	 ...
	 * @return weth9Managed WETH9 amount
	 * @return lpTokenSupply LP token supply
	 */
	function getReserves() public view returns (uint256 rdntManaged, uint256 weth9Managed, uint256 lpTokenSupply) {
		lpTokenSupply = tokenizedLpToken.totalSupply();
		// tokenizedLpToken.getTotalAmounts() has checks to ensure spot sqrtPriceX96 has not
		// been manipulated in this block.
		(uint256 total0, uint256 total1) = tokenizedLpToken.getTotalAmounts();
		(weth9Managed, rdntManaged) = weth9Addr == token0 ? (total0, total1) : (total1, total0);
	}

	/**
	 * @notice Returns LP price in WETH9
	 * @dev Return value decimals must be 8
	 * This method does NOT account accumulated fees to maintain it as a "view" method.
	 * @return priceInWeth9 LP price in WETH9 with 8 decimals precision
	 */
	...
	/**
	 * @notice Zap WETH9 and RDNt into LP
	 * @param _weth9Amt amount of WETH9
	 * @param _rdntAmt amount of RDNT
	 * @return liquidity LP token amount
	 */
	function zapTokens(uint256 _weth9Amt, uint256 _rdntAmt) public onlyLockZap returns (uint256 liquidity) {
		IERC20(weth9Addr).safeTransferFrom(msg.sender, address(this), _weth9Amt);
		IERC20(rdntAddr).safeTransferFrom(msg.sender, address(this), _rdntAmt);
		uint256 token0Amt = token0 == weth9Addr ? _weth9Amt : _rdntAmt;
		uint256 token1Amt = token1 == weth9Addr ? _weth9Amt : _rdntAmt;
		liquidity = tokenizedLpToken.deposit(token0Amt, token1Amt, msg.sender);
	}

	/**
	 * @notice Returns proportional amount of WETH9 for given RDNT amount per defined `WETH9_CL_RANGE` and `RDNT_CL_RANGE` ratios.
	 * @param tokenAmount amount of RDNT
	 * @return optimalWETH9Amount WETH9 amount
	 */
	function quoteFromToken(uint256 tokenAmount) public view returns (uint256 optimalWETH9Amount) {
		uint256 wethForRdnt = tokenizedLpToken.fetchOracle(rdntAddr, weth9Addr, tokenAmount);
		optimalWETH9Amount = (wethForRdnt * WETH9_CL_RANGE) / RDNT_CL_RANGE;
	}
	...
	/**
	 * @notice Returns RDNT price in WETH9 truncated to 8 decimals
	 * @return priceInWeth9 price of RDNT
	 */
	function getPrice() public view returns (uint256 priceInWeth9) {
		return tokenizedLpToken.fetchOracle(rdntAddr, weth9Addr, PRECISION) / 1e10;
	}
	...
	/**
	 * @notice Swaps WETH9 to RDNT
	 * @dev This method is called by the {Compounder.sol} and pushes the `_weth9Amount` to this address
	 * before the call. In this case the `payer` is this address.
	 * @param _weth9Amount the amount of RDNT to sell
	 ...
	 */
	function swapWethToRdnt(uint256 _weth9Amount, uint256 _minAmountOut) external returns (uint256) {
		if (_weth9Amount == 0) revert AmountZero();
		bool zeroForOne = weth9Addr == token0;
		address payer = IERC20(weth9Addr).balanceOf(address(this)) >= _weth9Amount ? address(this) : msg.sender;
	...
	/**
	 * @notice Provides a quote of how much RDNT one would receive for a given amount of WETH9
	 * @param _weth9Amount WETH9 amount to be sold
	 * @return quoted RDNT amount to be received
	 */
	function quoteWethToRdnt(uint256 _weth9Amount) external view returns (uint256) {
		return tokenizedLpToken.fetchSpot(weth9Addr, rdntAddr, _weth9Amount);
	}
