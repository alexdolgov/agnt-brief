...
contract BountyManager is Initializable, OwnableUpgradeable, PausableUpgradeable, RecoverERC20 {
	...
	// Bounty types enum maintains backward compatibility with previous bounty types
	enum BountyTypes {
		ALL, // 0
		MFD, // 1
		OBSOLETE, // 2
	...
	// Array of available Bounty functions to run. See _getMfdBounty, _getAutoCompoundBounty.
	...
	function initialize(
		...
		bounties[uint256(BountyTypes.MFD)] = _getMfdBounty; // 1
		bounties[uint256(BountyTypes.OBSOLETE)] = _emptyBounty; // 2
		bounties[uint256(BountyTypes.AUTOCOMPOUND)] = _getAutoCompoundBounty; // 3
		bountyCount = 3;
	 ...
	 * @param _actionType which of the BountyTypes (above) to run.
	 * @return bounty in RDNT to be paid to Hunter (via vesting)
	 * @return actionType which bounty ran
	 */
	function claim(address _user, BountyTypes _actionType) public returns (uint256, BountyTypes) {
	...
	function executeBounty(
		address _user,
		bool _execute,
		BountyTypes _actionType
	) public whenNotPaused isWhitelisted returns (uint256 bounty, BountyTypes actionType) {
	...
	/**
	 * @notice Given a user and actionType, execute that bounty
	 * @param _user address
	 * @param _execute whether to execute this txn, or just quote what its execution would return
	 * @param _actionTypeIndex, which of the BountyTypes (above) to run.
	...
	function _getAvailableBounty(
		address _user,
		bool _execute,
		BountyTypes _actionTypeIndex
	) internal returns (address incentivizer, uint256 totalBounty, bool issueBaseBounty, BountyTypes actionType) {
		if (_actionTypeIndex != BountyTypes.ALL) {
			// execute bounty w/ given params
			(incentivizer, totalBounty, issueBaseBounty) = bounties[uint256(_actionTypeIndex)](_user, _execute);
	...
	/**
	 * @dev Required for backwards compatibility with old bounty types 
	 * @return address zero
	 * @return zero amount
	 * @return false
	 */
	function _emptyBounty(
		address ,
		bool
	) internal pure returns (address, uint256, bool) {
	...
	function setBounties() external onlyOwner {
		bounties[uint256(BountyTypes.MFD)] = _getMfdBounty;
		bounties[uint256(BountyTypes.OBSOLETE)] = _emptyBounty;
		bounties[uint256(BountyTypes.AUTOCOMPOUND)] = _getAutoCompoundBounty;
		emit BountiesSet();
	}
