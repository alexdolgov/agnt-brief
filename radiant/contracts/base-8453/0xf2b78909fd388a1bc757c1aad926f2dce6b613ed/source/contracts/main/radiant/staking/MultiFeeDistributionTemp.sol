...
/// @title MultiFeeDistributionTemp
/// @author Radiant
contract MultiFeeDistributionTemp is
	...
	RecoverERC20
{
	...
	event AddressesUpdated(IMiddleFeeDistribution _middleFeeDistribution, address indexed _treasury);
	...
	/**
	 * @notice Set MFD and Treasury.
	 * @param middleFeeDistribution_ address
	 * @param treasury_ address
	 */
	function setAddresses(IMiddleFeeDistribution middleFeeDistribution_, address treasury_) external onlyOwner {
		...
		emit AddressesUpdated(middleFeeDistribution_, treasury_);
	}
	...
	function resetUserStake(address user) onlyOwner external {
		emit Withdrawn(user, _balances[user].locked, 0, 0, 0, true);
		lockedSupply = lockedSupply - _balances[user].locked;
		lockedSupplyWithMultiplier = lockedSupplyWithMultiplier - _balances[user].lockedWithMultiplier;
		delete _balances[user];
		delete _userLocks[user];
	}
