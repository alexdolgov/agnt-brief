...
import { IRizLendingPoolAddressesProvider } from "./RizLendingPoolAddressesProvider.sol";
...
contract RizLendingPool is VersionedInitializable, IRizLendingPool, RizLendingPoolStorage {
    ...
    function finalizeTransfer(
        ...
        if (msg.sender != _reserves[asset].aTokenAddress) {
            revert(Errors.LP_CALLER_MUST_BE_AN_ATOKEN);
        }
     ...
     *
     */
    function repay(address asset, uint256 amount, DataTypes.InterestRateMode rateMode, address onBehalfOf)
        external
        virtual
        whenNotPausedOrShutdown
        returns (uint256)
    {
        ...
        uint256 paybackAmount = rateMode == DataTypes.InterestRateMode.STABLE ? stableDebt : variableDebt;
        ...
        if (rateMode == DataTypes.InterestRateMode.STABLE) {
    ...
    function liquidationCall(
        ...
        if (!success) {
            revert(Errors.LP_LIQUIDATION_CALL_FAILED);
        }
    ...
    function _addReserveToList(address asset) internal {
        ...
        if (reservesCount > _maxNumberOfReserves) {
            revert(Errors.LP_NO_MORE_RESERVES_ALLOWED);
        }
