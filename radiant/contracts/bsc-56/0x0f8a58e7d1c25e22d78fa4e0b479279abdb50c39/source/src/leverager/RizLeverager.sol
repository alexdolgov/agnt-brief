...
contract RizLeverager is OwnableUpgradeable {
     ...
     */
    function initialize(IOracleRouter _oracleRouter, RizLockZap _lockZap, uint256 _feePercent, address _treasury)
        external
        initializer
    {
    ...
    function loop(
        ...
        if (!isBorrow) {
            ...
            // Scale up fee, so that it is rounded up and it's not 0
            fee = (amount * feePercent + RATIO_DIVISOR - 1) / RATIO_DIVISOR;
            if (fee > 0) {
    ...
    {
        if (msg.sender != borrower && msg.sender != address(lendingPool)) revert Errors.NotAuthorized();
