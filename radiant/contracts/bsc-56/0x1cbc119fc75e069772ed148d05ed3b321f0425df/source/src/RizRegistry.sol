...
import { RizRegistryDeploy } from "./libraries/RizRegistryDeploy.sol";
...
contract RizRegistry is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function getAddressProviderByMarketId(string memory marketId_) external view returns (address) {
    ...
    function addPool(string memory marketId_, address proxyAdmin) external onlyOwner {
        // Check `marketId_` is not empty or used
        if (
            bytes(marketId_).length == 0 || _marketIdToAddressProvider[marketId_] != address(0)
                || proxyAdmin == address(0)
        ) {
            revert Errors.InvalidId(0);
        }
        ...
        addressProvider.setRizRegistry(address(this));
        ...
        address badDebtManager =
            RizRegistryDeploy.deployBDM(proxyAdmin, lendingPool, owner(), implementations[BAD_DEBT_MANAGER]);
        addressProvider.setBadDebtManager(address(badDebtManager));
    ...
    function setMarketId(address addressProvider, string memory marketId_) external onlyOwner {
        if (_isValidAddressProvider[addressProvider] == false) {
            revert Errors.NoAddressProvider();
        }
        RizLendingPoolAddressesProvider(addressProvider).setMarketId(marketId_);
        _marketIdToAddressProvider[marketId_] = addressProvider;
    }
    ...
    /// @param marketId_ The market id
    function _registerPool(address addressProvider_, string memory marketId_) internal {
        uint256 providersCount = _addressesProvidersList.length;
    ...
    /// @param input The input data for the reserve initialization
    /// @param lendingPoolAddressesProvider The address of the LP address provider
    /// @param initAmounts array of amounts to deposit into the reserve
    function initReserve(
        ...
        if (inputLength != initAmounts.length) revert Errors.InvalidLength();
        ...
        for (uint256 i; i < inputLength;) {
            ...
            IERC20(asset).safeTransferFrom(msg.sender, address(this), initAmounts[i]);
            IERC20(asset).approve(address(lp), initAmounts[i]);
            RizLendingPool(lp).deposit(address(asset), initAmounts[i], msg.sender, 0);
    ...
    /// @param rzTokenAddr The address of the interest bearing token of the reserve
    /// @param newTreasury The new treasury address of the reserve
    function setTreasuryAddress(address rzTokenAddr, address newTreasury) external onlyOwner {
        // Make sure new address is valid
        if (newTreasury == address(0)) {
            revert Errors.AddressZero();
        }

        if (AToken(rzTokenAddr).RESERVE_TREASURY_ADDRESS() != newTreasury) {
            AToken(rzTokenAddr).setTreasuryAddress(newTreasury);
