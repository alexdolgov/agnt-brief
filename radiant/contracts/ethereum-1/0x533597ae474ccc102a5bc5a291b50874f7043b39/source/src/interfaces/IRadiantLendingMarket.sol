...
/// @dev This interface is used for factorizing IRadiantLendingMarketStaticTyping and IRadiantLendingMarket.
/// @dev Consider using the IRadiantLendingMarket interface instead of this one.
interface IRadiantLendingMarketBase {
...
/// @dev This interface is inherited by RadiantLendingMarket so that function signatures are checked by the
/// compiler.
/// @dev Consider using the IRadiantLendingMarket interface instead of this one.
interface IRadiantLendingMarketStaticTyping is IRadiantLendingMarketBase {
    ...
    function idToMarketParams(Id id)
        external
        view
        returns (address loanToken, address collateralToken, address oracle, address irm, uint256 ltv, uint256 lltv);
}

/// @title IRadiantLendingMarket
/// @author Radiant Capital
/// @dev Use this interface for LendingMarket to have access to all the functions with the appropriate
/// function signatures.
interface IRadiantLendingMarket is IRadiantLendingMarketBase {
    /// @notice The state of the position of `user` on the market corresponding to `id`.
