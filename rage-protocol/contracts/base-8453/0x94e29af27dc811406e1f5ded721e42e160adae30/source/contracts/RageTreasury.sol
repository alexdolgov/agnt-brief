 ...
 * RageTreasury
 *
 * Central treasury that holds RAGE tokens on behalf of multiple other contracts
 ...
 */
contract RageTreasury is ReentrancyGuard {
  // constant
  ...
  mapping(address => RageStructs.TreasuryContract) private CONTRACTS_MAP; // mapping for all registered contracts
  RageStructs.TreasuryState private STATE;
  ...
  // getOwners
  function getOwners() internal pure returns (address, address) {
    // return RAGE_CHAOS_ENGINE.getOwners();
    address owner1 = 0x035A12973C7d6dE9E3D3D450899913F07f68edee;
    address owner2 = 0xCBe5a4103d4C7Ed5D73d9942101473C1Bc0A8020;
  ...
  function getUnallocated() internal view returns (uint256) {
    ...
    for (uint256 i = 0; i < contracts.length; i++) {
        totalTransfered += CONTRACTS_MAP[contracts[i]].transferred;
    }

    uint256 rageBalance = getRageBalance();
    uint256 totalAvailable = STATE.totalAttributed - totalTransfered;
  ...
  // returns the full TreasuryContract struct for every registered contract
  function getRegisteredContracts() external view returns (RageStructs.TreasuryContract[] memory) {
    uint256 count = contracts.length;
    RageStructs.TreasuryContract[] memory result = new RageStructs.TreasuryContract[](count);
  ...
  function getState() external view returns (RageStructs.TreasuryGetState memory) {
    ...
    return RageStructs.TreasuryGetState({
        version: VERSION,
