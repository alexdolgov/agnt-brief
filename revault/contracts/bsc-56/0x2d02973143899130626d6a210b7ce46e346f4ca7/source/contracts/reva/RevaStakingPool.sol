...
contract RevaStakingPool is OwnableUpgradeable, ReentrancyGuard {
    ...
    mapping (uint => PoolMigrations[]) public poolMigrationsInfo;
    // pid => userAddress => time
    mapping (uint => mapping (address => uint)) public userLastPoolInteraction;
    ...
    function userTimeRemaining(uint _pid, address _user) public view returns (uint) {
        PoolMigrations[] memory migrations = poolMigrationsInfo[_pid];
        PoolInfo memory pool = poolInfo[_pid];
        UserPoolInfo memory user = userPoolInfo[_pid][_user];
        uint lastInteraction = userLastPoolInteraction[_pid][_user];
        uint numOfMigrations = migrations.length;
        if (numOfMigrations == 0) {
            if (block.timestamp >= user.timeDeposited.add(pool.timeLocked)) return 0;
            else return user.timeDeposited.add(pool.timeLocked).sub(block.timestamp);
        }
        uint lockTime;
        for (uint i = 0; i < numOfMigrations; i++) {
           PoolMigrations memory migration = migrations[i];
           if (lastInteraction <= migration.migratedTime) {
               lockTime = migration.prevLockTime;
               break;
           }
        }
        if (lockTime == 0) lockTime = pool.timeLocked;
        if (block.timestamp >= user.timeDeposited.add(lockTime)) return 0;
        else return user.timeDeposited.add(lockTime).sub(block.timestamp);
    }
    ...
    function deposit(uint _pid, uint _amount) external nonReentrant {
        ...
        if (_amount > 0) {
            ...
            userLastPoolInteraction[_pid][msg.sender] = block.timestamp;
        }
    ...
    function withdraw(uint _pid, uint _amount) external nonReentrant {
        ...
        require(userTimeRemaining(_pid, msg.sender) == 0, "time locked");
    ...
    function withdrawEarly(uint _pid, uint _amount) external nonReentrant {
        ...
        require(userTimeRemaining(_pid, msg.sender) > 0, "Not early");
    ...
    // to fix previous bug, restoring users last lock time
    function setUserLastPoolInteraction(
        uint _pid,
        address _user,
        uint _lastInteraction
    ) external {
        require(msg.sender == admin || msg.sender == owner(), "perms");
        userLastPoolInteraction[_pid][_user] = _lastInteraction;
    }
    ...
    function changeTimeLocked(uint _pid, uint _timeLocked) external onlyOwner {
        PoolInfo storage pool = poolInfo[_pid];
        PoolMigrations[] storage migrations = poolMigrationsInfo[_pid];
        require(migrations.length <= 20, "MAX");
        migrations.push(PoolMigrations({
            migratedTime: block.timestamp,
            prevLockTime: pool.timeLocked
        }));
        pool.timeLocked = _timeLocked;
    }
