...
contract NFTCavePoolTriple is Ownable,ReentrancyGuard, IERC721Receiver {
   ...
   struct NftPoolInfo{
    ...
    constructor(        
        ...
        address _nftOne, //MUST be PMOT IF included in Set
        address _nftTwo,
        address _nftThree,
    ...
    function deposit(uint256 _amount) public {
        ...
        if(_amount > 0) {
            NFTInterface pmotNFT = NFTInterface(pool.nftOne);
            NFTInterface mythicNFT = NFTInterface(pool.nftTwo);
            NFTInterface nftThree = NFTInterface(pool.nftThree);

             uint256[] memory nftThreeTokenIds = nftThree.getUserNftTokens(msg.sender);
            uint256[] memory nftThreeTokensToTransfer = new uint256[](nftThreeTokenIds.length);
            uint256 nftThreeCounter = 0;
            for(uint256 i = 0; i < nftThreeTokenIds.length; i++){
                if(nftThreeTokenIds[i] > 900 && nftThreeTokenIds[i] <= 1500 ){
                    nftThreeTokensToTransfer[nftThreeCounter] = nftThreeTokenIds[i];
                    nftThreeCounter = nftThreeCounter + 1;
                }
            }
            
            require(nftThreeCounter >= _amount, "deposit: Not enough NFTs");
            ...
            for(uint256 i = 0; i < mythicsTokens.length; i++){
                if(mythicsTokens[i] > 900 && mythicsTokens[i] <= 1500 ){
            ...
            if(pool.nftOne == pmotNFTaddress ){
               pmotTokens = pmotNFT.getUserNftTokensForRarity(1,msg.sender);
               require(pmotTokens.length >= _amount, "deposit: Not enough NFTs");
                ...
                for(uint256 i = 0; i < pmotTokens.length; i++){
                    if(pmotTokens[i] > 900 && pmotTokens[i] <= 1500 ){
            ...
            for(uint256 t = 0; t < _amount; t++){                                
                ...
                nftThree.safeTransferFrom(address(msg.sender), address(this), nftThreeTokensToTransfer[t]); 
                user.nftThreeTokenIds.push(nftThreeTokensToTransfer[t]);
                if(pool.nftOne == pmotNFTaddress ){
    ...
    function getuserStakedToken3(address user) external view returns (uint256[] memory){
    return userInfo[user].nftThreeTokenIds;
    }
    ...
    function withdraw(uint256 _amount) public {
        ...
        if(_amount > 0) {
            ...
            for(uint256 ti = user.amountStaked; ti > toPopCounter; ti--){
                ...
                nftThree.safeTransferFrom(address(this), address(msg.sender), user.nftThreeTokenIds[ti-1]);                       
                user.pmotUltraRaretokenIds.pop();
                user.mythicUltraRaretokenIds.pop();
                user.nftThreeTokenIds.pop();
            }
