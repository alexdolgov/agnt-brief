...
contract MarginCalculator is Ownable {
    ...
    function getExpiredPayoutRate(address _otoken) external view returns (uint256) {
        ...
        // because physically settled vaults are not liquidatable, there is a chance they can become undercollateralised
        ...
        if (isPhysicallySettled && cashValueInCollateral.isGreaterThan(FPI.fromScaledUint(1, 0))){
    ...
    /**
     * @notice return the strike payment for physically settled options
     * @param _otoken oToken address
     * @param _amount amount of the oTokens to calculate the payment for, always represented in 1e8
     * @return how much strikeAsset is needed to exercise the otokens (strike asset decimals)
     */
     ...
     * @param _vaultType vault type (0 for max loss/spread and 1 for naked margin vault, 2 for physially settled)
     ...
     * @param _vault theoretical vault that needs to be checked
     * @param _vaultType vault type (0 for spread/max loss, 1 for naked margin, 2 for physially settled)
     ...
     * @param _vault vault struct
     * @param _vaultType vault type, 0 for max loss/spreads and 1 for naked margin vault, 2 for physially settled
        ...
        returns (bool)
    {
        ...
        if (_vaultDetails.hasShort) {
            ...
            if (_vaultDetails.vaultType == 0 || _vaultDetails.vaultType == 2) {
    ...
    function _getCashValue(
        ...
        if (_isPhysicallySettled) {
            ...
            if (_isPut) revert("MarginCalculator: physically settled put not supported");
