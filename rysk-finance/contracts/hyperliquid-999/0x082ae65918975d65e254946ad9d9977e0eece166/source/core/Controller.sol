...
contract Controller is Initializable, OwnableUpgradeSafe, ReentrancyGuardUpgradeSafe {
    ...
    /// @dev mapping to whitelist addresses that can call the Controller (e.g., OptionRegistry)
    mapping(address => bool) public authorizedCallers;

    /// @notice emits an event when an authorized caller is added or removed
    ...
    function donate(address _asset, uint256 _amount) external {
        require(authorizedCallers[msg.sender], "C38");
    ...
    /**
     * @notice set an address as an authorized caller
     * @dev can only be called by owner
     * @param _caller address to authorize or deauthorize
    ...
    function setAuthorizedCaller(address _caller, bool _isAuthorized) external onlyOwner {
        require(_caller != address(0), "Controller: Invalid caller address");
     ...
     * @dev can only be called when the system is not fully paused and by authorized callers only
    ...
    function operate(Actions.ActionArgs[] memory _actions) external nonReentrant notFullyPaused {
        ...
        require(authorizedCallers[msg.sender], "C38"); // Controller: Caller not authorized
