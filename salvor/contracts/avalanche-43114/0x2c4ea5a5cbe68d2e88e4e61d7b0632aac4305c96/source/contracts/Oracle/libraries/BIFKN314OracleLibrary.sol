...
library BIFKN314OracleLibrary {
     ...
     * @return tokenPriceCumulative Reflects the price of token in native currency
     * @return nativePriceCumulative Reflects the price of native currency in token
    ...
    function currentCumulativePrices(
        ...
        view
        returns (
            uint256 tokenPriceCumulative,
            uint256 nativePriceCumulative,
            uint32 blockTimestamp
        )
    {
        blockTimestamp = currentBlockTimestamp();
        tokenPriceCumulative = IERC314(token).price0CumulativeLast();
        nativePriceCumulative = IERC314(token).price1CumulativeLast();
        ...
        if (blockTimestampLast != blockTimestamp) {
            ...
            tokenPriceCumulative +=
                ((nativeReserve * Q112) / tokenReserve) *
                timeElapsed;
            nativePriceCumulative +=
                ((tokenReserve * Q112) / nativeReserve) *
                timeElapsed;
        }
    }
}
