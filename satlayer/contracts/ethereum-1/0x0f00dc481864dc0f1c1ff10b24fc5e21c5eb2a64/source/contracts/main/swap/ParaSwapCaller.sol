...
contract ParaSwapCaller {
    ...
    function _decodeBalancerV2Tokens(bytes memory balancerData)
        internal
        pure
        returns (address srcToken, address destToken)
    {
        ...
        assembly ("memory-safe") {
            ...
            if eq(srcToken, 0) { srcToken := ETH }
            if eq(destToken, 0) { destToken := ETH }
        }
        return (srcToken, destToken);
    }

    function analysisPayload(bytes calldata _swapData)
        public
        pure
        returns (uint256 amountIn_, address tokenIn_, address tokenOut_)
    {
        // Match the selector
        require(_swapData.length >= 4, "ParaSwapCaller: Invalid calldata length");
        bytes4 selector = bytes4(_swapData[:4]);
        if (selector == IParaSwapV62.swapExactAmountIn.selector) {
            // Decode the swapData
            (, GenericData memory d_,,,) = abi.decode(_swapData[4:], (address, GenericData, uint256, bytes, bytes));
            ...
            // Decode the swapData
            (AugustusRFQData memory data_, OrderInfo[] memory order_,) =
                abi.decode(_swapData[4:], (AugustusRFQData, OrderInfo[], bytes));
            ...
            // Decode the swapData
            (BalancerV2Data memory d_,,, bytes memory data_) =
                abi.decode(_swapData[4:], (BalancerV2Data, uint256, bytes, bytes));
            ...
            (CurveV1Data memory curveV1Data_,,) = abi.decode(_swapData[4:], (CurveV1Data, uint256, bytes));
            ...
            (CurveV2Data memory curveV2Data_,,) = abi.decode(_swapData[4:], (CurveV2Data, uint256, bytes));
            ...
            (UniswapV2Data memory uniswapV2Data_,,) = abi.decode(_swapData[4:], (UniswapV2Data, uint256, bytes));
            ...
            (UniswapV3Data memory uniswapV3Data_,,) = abi.decode(_swapData[4:], (UniswapV3Data, uint256, bytes));
            ...
            (MakerPSMData memory makerPSMData_,) = abi.decode(_swapData[4:], (MakerPSMData, bytes));
    ...
    function _executeSwap(
        ...
        (uint256 amountIn_, address tokenIn_, address tokenOut_) = analysisPayload(_swapData);
        bool toETH = tokenOut_ == address(ETH);
        uint256 amountBefore_ = toETH ? address(this).balance : IERC20(_dstToken).balanceOf(address(this));
        require(amountIn_ <= _amount, "ParaSwapCaller: Amount in exceeds maximum");
        ...
        (bool success_,) = PARASWAP_AUGUSTUS_PROXY_V6.call{value: _srcToken == address(ETH) ? _amount : 0}(_swapData);
        require(success_, "ParaSwapCaller: Swap execution failed");
        uint256 amountAfter_ = toETH ? address(this).balance : IERC20(_dstToken).balanceOf(address(this));
        require(amountAfter_ - amountBefore_ >= _swapGetMin, "ParaSwapCaller: Insufficient output amount");
        return (amountAfter_ - amountBefore_, amountIn_);
    }
}
