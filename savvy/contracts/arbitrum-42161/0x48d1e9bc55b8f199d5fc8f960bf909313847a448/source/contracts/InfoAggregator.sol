...
contract InfoAggregator is IInfoAggregator, Ownable2StepUpgradeable {
  ...
  /// @inheritdoc IInfoAggregator
  function isSavvyPositionManager(
    address addr
  ) external view override returns (bool) {
    return savvyPositionManagers.contains(addr);
  }
  ...
  function getWithdrawableAmount(
    ...
    for (uint256 i = 0; i < length; i++) {
      ...
      (, address[] memory depositedTokens) = savvyPositionManager.accounts(
        owner_
      );
  ...
  function getBorrowableAmount(
    ...
    for (uint256 i = 0; i < length; i++) {
      ...
      values[i] = SavvyPosition(savvyPositionManagers.at(i), borrowableAmount);
    }
  ...
  function getUserDepositedAmount(address user_) public view returns (uint256) {
    uint256 totalAmount = 0;
    uint256 length = savvyPositionManagers.length();
  ...
  function getAvailableCreditToken(
    ...
    for (uint256 i = 0; i < length; i++) {
      ...
      credits[i] = DebtInfo(address(savvyPositionManager), availableCredit);
    }
  ...
  function getFullUserHarvestedYield(
    ISavvyPositionManager savvyPositionManager,
    address account_,
    address yieldToken
  ) external view returns (uint256) {
    return
      InfoAggregatorUtils._getFullUserHarvestedYield(
        savvyPositionManager,
        account_,
        yieldToken
      );
  ...
  function _addSupportTokens(
    ...
    Checker.checkArgument(length > 0, "empty support token information array");
