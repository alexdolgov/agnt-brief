...
library InfoAggregatorUtils {
  ...
  function _getPoolsInfo(
    ...
    for (uint256 i = 0; i < state.numOfSavvyPositionManagers; i++) {
      ...
      for (uint256 j = 0; j < supportedYieldTokens.length; j++) {
        ...
        state.userHarvestedYield = _getFullUserHarvestedYield(
          savvyPositionManager,
          account_,
          yieldToken
        );
  ...
  function _getFullUserHarvestedYield(
    ...
    ) = savvyPositionManager.positions(account_, yieldToken);

    ISavvyPositionManager.YieldTokenParams memory yieldTokenParams = (
      savvyPositionManager.yieldStrategyManager()
    ...
    uint256 unrealizedCredit = ((currentAccruedWeight - lastAccruedWeight) *
      shares) / FIXED_POINT_SCALAR;

    return unrealizedCredit + harvestedYield;
  }
