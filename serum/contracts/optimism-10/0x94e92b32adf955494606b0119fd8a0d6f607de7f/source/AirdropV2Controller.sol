...
contract AirdropV2Controller is Context, IAirdrop, IAirdropController {
    ...
    modifier onlyGovernance() {
        require(_msgSender() == _governance, "AirdropController: not governance");
        _;
    }
    ...
    function recycle(uint256 id, address recipient) external override onlyValid(id) onlyGovernance {
        ...
        IERC20(airdrop.token).safeTransfer(recipient, amount);
        _assignedBalances[airdrop.token] -= amount;
    ...
    function getUnclaimed(uint256 id) external view override onlyValid(id) returns (uint256) {
        return _getUnclaimed(id);
    }

    function isExpired(uint256 id) external view override onlyValid(id) returns (bool) {
        return _isExpired(id);
    }

    function isClaimed(uint256 id, uint256 index) external view override onlyValid(id) returns (bool) {
        return _isClaimed(id, index);
    }

    function isAllClaimed(uint256 id) external view override onlyValid(id) returns (bool) {
        return _isAllClaimed(id);
    }
