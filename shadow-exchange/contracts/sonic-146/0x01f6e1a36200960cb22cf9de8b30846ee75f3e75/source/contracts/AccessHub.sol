...
contract AccessHub is IAccessHub0, Initializable, AccessControlEnumerableUpgradeable {
    ...
    /// @inheritdoc IAccessHub0
    function migrateStates() external onlyRole(PROTOCOL_OPERATOR) {
        AccessHubStorage.AccessHubState storage oldStorage = AccessHubStorage.getOriginalStorage();
        AccessHubStorage.AccessHubState storage $ = AccessHubStorage.getStorage();

        require(oldStorage.timelock != address(0));

        $.timelock = oldStorage.timelock;
        $.treasury = oldStorage.treasury;
        $.clGaugeFactory = oldStorage.clGaugeFactory;
        $.gaugeFactory = oldStorage.gaugeFactory;
        $.feeDistributorFactory = oldStorage.feeDistributorFactory;
        $.voter = oldStorage.voter;
        $.minter = oldStorage.minter;
        $.launcherPlugin = oldStorage.launcherPlugin;
        $.xShadow = oldStorage.xShadow;
        $.x33 = oldStorage.x33;
        $.shadowV3PoolFactory = oldStorage.shadowV3PoolFactory;
        $.poolFactory = oldStorage.poolFactory;
        $.feeRecipientFactory = oldStorage.feeRecipientFactory;
        $.feeCollector = oldStorage.feeCollector;
        $.voteModule = oldStorage.voteModule;
        $.nfpManager = oldStorage.nfpManager;

        delete oldStorage.timelock;
        delete oldStorage.treasury;
        delete oldStorage.clGaugeFactory;
        delete oldStorage.gaugeFactory;
        delete oldStorage.feeDistributorFactory;
        delete oldStorage.voter;
        delete oldStorage.minter;
        delete oldStorage.launcherPlugin;
        delete oldStorage.xShadow;
        delete oldStorage.x33;
        delete oldStorage.shadowV3PoolFactory;
        delete oldStorage.poolFactory;
        delete oldStorage.feeRecipientFactory;
        delete oldStorage.feeCollector;
        delete oldStorage.voteModule;
        delete oldStorage.nfpManager;
    }
        ...
        onlyRole(SWAP_FEE_SETTER)
    {
        ...
        for (uint256 i; i < _pools.length; ++i) {
            _shadowV3PoolFactory.setPoolFeeProtocol(_pools[i], _feeProtocol[i]);
        }
    }
        ...
        onlyRole(SWAP_FEE_SETTER)
    {
        ...
        IPairFactory _poolFactory = AccessHubStorage.getStorage().poolFactory;
    ...
    function killGauge(address[] calldata _pairs) external onlyRole(PROTOCOL_OPERATOR) {
        ...
        for (uint256 i; i < _pairs.length; ++i) {
            ...
            /// @dev kill the gauge
            _voter.killGauge(_voter.gaugeForPool(pair));
            /// @dev set the new fees in the pair to 95/5
            _shadowV3PoolFactory.setPoolFeeProtocol(pair, 5);
        }
    }
    ...
    function reviveGauge(address[] calldata _pairs) external onlyRole(PROTOCOL_OPERATOR) {
        ...
        for (uint256 i; i < _pairs.length; ++i) {
            ...
            _shadowV3PoolFactory.setPoolFeeProtocol(pair, _shadowV3PoolFactory.feeProtocol());
        }
    }
    ...
    function setLegacyFeeRecipient(address _pair, address _feeRecipient) external onlyRole(PROTOCOL_OPERATOR) {
        AccessHubStorage.getStorage().poolFactory.setFeeRecipient(_pair, _feeRecipient);
    }
    ...
    function rescueFromX33(address _token, uint256 _amount) external onlyRole(PROTOCOL_OPERATOR) {
        AccessHubStorage.getStorage().x33.rescue(_token, _amount);
    ...
    fallback(bytes calldata _calldata) external returns (bytes memory) {
        ...
        for (uint256 i = 0; i < length; i++) {
            (bool success, bytes memory returnData) = expansionPacks.at(i).delegatecall(_calldata);
...
contract AccessHubExpansionPack1 is IAccessHub1, AccessHubExpansionPack {
    ...
    function voter() external view returns (IVoter) {
        return AccessHubStorage.getStorage().voter;
    }

    /// @notice weekly emissions minter
    function minter() external view returns (IMarbleMinter) {
        return AccessHubStorage.getStorage().minter;
    }

    /// @notice launchpad plugin for augmenting feeshare
    function launcherPlugin() external view returns (ILauncherPlugin) {
        return AccessHubStorage.getStorage().launcherPlugin;
    }

    /// @notice xShadow contract
    function xShadow() external view returns (IXShadow) {
        return AccessHubStorage.getStorage().xShadow;
    }

    /// @notice X33 contract
    function x33() external view returns (IX33) {
        return AccessHubStorage.getStorage().x33;
    }

    /// @notice CL V3 factory
    function shadowV3PoolFactory() external view returns (IShadowV3Factory) {
        return AccessHubStorage.getStorage().shadowV3PoolFactory;
    }

    /// @notice legacy pair factory
    function poolFactory() external view returns (IPairFactory) {
        return AccessHubStorage.getStorage().poolFactory;
    }

    /// @notice legacy fees holder contract
    function feeRecipientFactory() external view returns (IFeeRecipientFactory) {
        return AccessHubStorage.getStorage().feeRecipientFactory;
    }

    /// @notice fee collector contract
    function feeCollector() external view returns (IFeeCollector) {
        return AccessHubStorage.getStorage().feeCollector;
    }

    /// @notice voteModule contract
    function voteModule() external view returns (IVoteModule) {
        return AccessHubStorage.getStorage().voteModule;
    }

    /// @notice NFPManager contract
    function nfpManager() external view returns (INonfungiblePositionManager) {
        return AccessHubStorage.getStorage().nfpManager;
    }
    ...
    /// @inheritdoc IAccessHub1
    function pokeVotesOnBehalfOf(address[] calldata _users) external timelocked {
        IVoter _voter = AccessHubStorage.getStorage().voter;
