...
contract PoolGuard is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    using SafeERC20 for IERC20;
    ...
    IERC20 public constant GUARD = IERC20(0xF606bd19b1E61574ED625d9ea96C841D4E247A32);
    ...
    function usableCollateralBalance() public view returns (uint256) {
        uint256 _balance = GUARD.balanceOf(address(this));
        // uint256 _nativeBalance =  address(this).balance;
        uint256 _totalBalance = _balance;
        return
    ...
    function mint(
        ...
        GUARD.safeTransferFrom(_sender, address(this), _ethIn);

        if (_yTokenOutTwap > 0 && _guardSwapIn > 0) {
            GUARD.safeIncreaseAllowance(
                address(swapStrategy),
                _guardSwapIn
            );
            swapStrategy.execute(_guardSwapIn, _yTokenOutTwap);
        }
    ...
    function redeem(
        ...
        if (isTokenTradingActive) {
            require(
                _minEthOut <= _guardOut && _minYTokenOut <= _yTokenOutSpot,
                "Pool::redeem: >slippage"
            );
        } else {
            require(_minEthOut <= _guardOut, "Pool::redeem: >slippage");
        }
        ...
        if (_guardOut > 0) {
            userInfo[_sender].ethBalance =
                userInfo[_sender].ethBalance +
                _guardOut;
            unclaimedEth = unclaimedEth + _guardOut;
        }
        ...
        emit Redeem(_sender, _xTokenIn, _guardOut, _yTokenOutSpot, _fee);
    }
    ...
    function collect() external nonReentrant {
        ...
        if (userInfo[_sender].ethBalance > 0) {
            ...
            // _sendEth = true;
            GUARD.transfer(_sender, _ethAmount);
        }
    ...
    function recollateralize(uint256 _amount) external payable {
        ...
        GUARD.safeTransferFrom(msg.sender, address(this), _amount);
    ...
    function transferToTreasury(uint256 _amount) internal {
        ...
        if (_amount > 0) {
            GUARD.safeTransfer(treasury, _amount);
        }
    }
