...
contract ProtocolLens is BasicLens {
    string public constant mainSymbol = "METIS";
    string public constant mainCTokenSymbol = "sbMETIS";
    ...
    function getCTokenInfo(CToken cToken, Comptroller comptroller) public returns (CTokenInfo memory) {
        address underlyingAssetAddress;
        uint256 underlyingDecimals;
        ...
        if (isNative) {
            ...
            cTokenInfo.underlyingSymbol = EIP20Interface(cErc20.underlying()).symbol();
        }
        ...
        cTokenInfo.poolBalance =
            isNative ? contractAddress.balance : IERC20(underlyingAssetAddress).balanceOf(contractAddress);
        ...
        (, cTokenInfo.collateralFactor,) = comptroller.markets(contractAddress);
        ...
        JumpRateModelV4 interestRateModel = JumpRateModelV4(address(cToken.interestRateModel()));
        ...
        cTokenInfo.isMintPaused = comptroller.mintGuardianPaused(contractAddress);
        cTokenInfo.isBorrowPaused = comptroller.borrowGuardianPaused(contractAddress);
    ...
    // returns AccountInfo
    function getAccountInfo(CToken cToken, address payable account) public returns (AccountInfo memory) {
        ...
        if (isNative) {
            ...
            CErc20 cErc20 = CErc20(address(cToken));
            underlyingAssetAddress = cErc20.underlying();
            accountInfo.allowance = EIP20Interface(underlyingAssetAddress).allowance(account, contractAddress);
        }
        ...
        accountInfo.myRealTokenBalance = isNative ? account.balance : IERC20(underlyingAssetAddress).balanceOf(account);
    ...
    function getOraclePrice(CToken cToken, Comptroller comptroller) public view returns (uint256) {
    ...
    function cTokenMetaDataList(Comptroller comptroller) external returns (CTokenMetaData[] memory) {
    ...
    function cTokenMetaDataListAuth(address payable account, Comptroller comptroller)
        external
