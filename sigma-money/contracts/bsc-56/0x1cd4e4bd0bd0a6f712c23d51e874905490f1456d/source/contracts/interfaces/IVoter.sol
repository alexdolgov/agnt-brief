...
interface IVoter {
    ...
    /// @param _pool pool's address  
    /// @return _gauge address of the new gauge
    function createVeFunderGauge(address _receiver, uint256 _maxEmission, address _pool) external returns (address _gauge);

    /// @notice sets preallocation percentage for a SigmaGauge
    /// @param _gauge address of the SigmaGauge
    /// @param _preallocationBps new preallocation percentage using BASIS
    function setSigmaGaugePreallocation(address _gauge, uint256 _preallocationBps) external;

    /// @notice returns the preallocation percentage for a SigmaGauge
    /// @param _gauge address of the gauge
    /// @return _preallocationBps preallocation percentage using BASIS
    function sigmaGaugePreallocation(address _gauge) external view returns (uint256 _preallocationBps);
