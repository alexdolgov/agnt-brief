...
interface IGaugeEmission {
    ...
    /// @dev error for if token is not set correctly
    ...
    event GaugeActive(bool _isGaugeActive);
    event EmissionsMultiplierUpdated(uint256 _emissionsMultiplier);
    event EmergencyWithdraw(address indexed token, address indexed to, uint256 amount);

    /// @notice initializes the GaugeEmission proxy
    /// @param _accessHub the access control center address
    function initialize(address _accessHub) external;
