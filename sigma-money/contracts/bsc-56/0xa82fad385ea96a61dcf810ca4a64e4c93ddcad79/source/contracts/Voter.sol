...
contract Voter is IVoter, ReentrancyGuard, Initializable {
    ...
    function _claimablePerPeriod(
        ...
        uint256 totalAllocation = preallocationAmount + baseAllocation;
        
        // Handle VeFunderGauge maxEmission limit
        if (isVeFunderGauge[gauge]) {
            uint256 emissionRemaining = IVeFunderGauge(gauge).maxEmission() - IVeFunderGauge(gauge).emission();
            if (totalAllocation > emissionRemaining) {
                return emissionRemaining;
            }
        }
        
        // Add redistributed amount from capped VeFunderGauges
        uint256 redistributedAmount = _getRedistributedAmountForPool(pool, period, totalAllocation);
        
        return totalAllocation + redistributedAmount;
    }
    ...
    /// @dev calculates redistributed amount for a specific pool from capped VeFunderGauges
    function _getRedistributedAmountForPool(
        address pool, 
        uint256 period, 
        ...
        // VeFunderGauges don't receive redistributed amounts
        if (isVeFunderGauge[gauge]) {
            return 0;
        }
        
        // Calculate total excess from all VeFunderGauges
        uint256 totalExcess = _getTotalExcessFromVeFunderGauges(period);
        if (totalExcess == 0) {
            return 0;
        }
        
        // Calculate eligible votes (excluding VeFunderGauges)
        uint256 eligibleVotes = _getEligibleVotesForRedistribution(period);
        if (eligibleVotes == 0) {
            return 0;
        }
        
        // Redistribute proportionally based on votes
        uint256 poolVotes = poolTotalVotesPerPeriod[pool][period];
        return (totalExcess * poolVotes) / eligibleVotes;
    }

    /// @dev calculates total excess from all VeFunderGauges that exceed maxEmission
    function _getTotalExcessFromVeFunderGauges(uint256 period) internal view returns (uint256) {
        ...
        for (uint256 i = 0; i < gaugesLength; i++) {
            address gauge = gauges.at(i);
            if (isVeFunderGauge[gauge] && isAlive[gauge]) {
                address pool = poolForGauge[gauge];
                
                // Calculate what this VeFunderGauge would get without cap
                uint256 numerator = (remainingReward * poolTotalVotesPerPeriod[pool][period]) * 1e18;
                uint256 uncappedAllocation = numerator / totalVotesPerPeriod[period] / 1e18;
                
                uint256 emissionRemaining = IVeFunderGauge(gauge).maxEmission() - IVeFunderGauge(gauge).emission();
                if (uncappedAllocation > emissionRemaining) {
                    totalExcess += uncappedAllocation - emissionRemaining;
                }
            }
        }
    ...
    /// @dev calculates eligible votes for redistribution (excluding VeFunderGauges)
    function _getEligibleVotesForRedistribution(uint256 period) internal view returns (uint256) {
        uint256 eligibleVotes = 0;
        uint256 gaugesLength = gauges.length();
        ...
        for (uint256 i = 0; i < gaugesLength; i++) {
            ...
            if (!isVeFunderGauge[gauge] && isAlive[gauge]) {
                address pool = poolForGauge[gauge];
                eligibleVotes += poolTotalVotesPerPeriod[pool][period];
            }
        }
