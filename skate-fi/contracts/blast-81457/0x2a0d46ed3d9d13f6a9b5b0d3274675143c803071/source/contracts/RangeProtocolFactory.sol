...
import {IThrusterPoolFactory} from "./thruster/interfaces/IThrusterPoolFactory.sol";
...
contract RangeProtocolFactory is IRangeProtocolFactory, Ownable {
    ...
    constructor(address _thrusterPoolFactory, address _deployer) Ownable() {
        factory = _thrusterPoolFactory;
        deployer = _deployer;
    ...
    // @param tokenA one of the tokens in the thruster pair
    // @param tokenB the other token in the thruster pair
    // @param fee fee tier of the thruster pair
    ...
    function createVault(
        ...
        address pool = IThrusterPoolFactory(factory).getPool(tokenA, tokenB, fee);
    ...
    function _createVault(
        ...
        int24 tickSpacing = IThrusterPoolFactory(factory).feeAmountTickSpacing(fee);
        vault = address(
            new ERC1967Proxy(
                implementation,
