 ...
 * @notice RangeProtocolVault is fungible vault shares contract that accepts thruster pool tokens for liquidity
 * provision to the corresponding thruster pool. This contract is configurable to work with any thruster
 ...
 * active position in the thruster pool) and burning involves removing liquidity from the thruster pool along with
 * the vault's fee.
 *
 * The manager of the contract can remove liquidity from thruster pool and deposit into a newer take range to maximise
 * the profit by keeping liquidity out of the pool under high volatility periods.
 *
 * Part of the fee earned from thruster position is paid to manager as performance fee and fee is charged on the LP's
    ...
    RangeProtocolVaultStorage
{
     ...
     * @param _pool address of the thruster pool associated with vault
     * @param _tickSpacing tick spacing of the thruster pool
    ...
    function initialize(
        ...
        ) = abi.decode(data, (address, string, string, address));
        ...
        state.pool = IThrusterPool(_pool);
        ...
        // Managing fee is 0%, performanceFee is 10% and otherFee is 0% at the time vault initialization.
        VaultLib.updateFees(state, 0, 1000, 0);
    }
    ...
    /// @notice uniswapV3MintCallback Thruster callback fn, called back on pool.mint
    ...
    /// @notice uniswapV3SwapCallback Thruster callback fn, called back on pool.swap
    ...
    /**
     * @notice mint mints range vault shares, fractional shares of a Thruster position/strategy
    ...
    /**
     * @notice burn burns range vault shares (shares of a Thruster position) and receive underlying
    ...
    /**
     * @notice removeLiquidity removes liquidity from thruster pool and receives underlying tokens
    ...
    /**
     * @dev Mars@RangeProtocol
     * @notice addLiquidity allows manager to add liquidity into thruster pool into newer tick ranges.
    ...
    /**
     * @dev pullFeeFromPool pulls accrued fee from thruster pool that position has accrued since
     * last collection.
     */
    ...
    function setOtherFeeRecipient(address newOtherFeeRecipient) external override onlyManager {
    ...
    /**
     * @notice getPositionID returns the position id of the vault in thruster pool
     * @return positionID position id of the vault in thruster pool
     */
     ...
     * includes current liquidity invested in thruster position, current fees earned
