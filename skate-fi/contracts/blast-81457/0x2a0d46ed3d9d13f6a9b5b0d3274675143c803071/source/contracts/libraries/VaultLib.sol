...
import {FullMath} from "../thruster/FullMath.sol";
...
library VaultLib {
    ...
    /// Performance fee cannot be set more than 20% of the fee earned from thruster pool.
    ...
    /**
     * @notice _withdraw internal function to withdraw liquidity from thruster pool
     * @param liquidity liquidity to remove from the thruster pool
     */
    function _withdraw(
        DataTypes.State storage state,
        uint128 liquidity
     ...
     * @param feeGrowthInsideLast last time the fee was realized for the vault in thruster pool
     */
    function _feesEarned(
        ...
        (IThrusterPool pool, int24 lowerTick, int24 upperTick) = (
            state.pool,
            state.lowerTick,
            state.upperTick
        );
    ...
    /**
     * @notice _applyPerformanceAndOtherFee applies the performance fee to the fees earned from thruster pool.
    ...
    /**
     * @notice _netPerformanceAndOtherFees computes the fee share for manager as performance fee from the fee earned from thruster pool.
     * @param rawFee0 fee earned in token0 from thruster pool.
     * @param rawFee1 fee earned in token1 from thruster pool.
