...
contract RewardManager is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
  ...
  modifier onlyMigrationEnabled() {
    require(IxGF(xGF).migrationEnabled(), "Migration is disabled");
    _;
  }

  modifier onlyMigrationDisable() {
    require(!IxGF(xGF).migrationEnabled(), "Not allowed while migration is enabled");
    _;
  }
  ...
  /// @param _user The address to claim rewards for
  function claim(address _user) external nonReentrant onlyLive onlyXGF onlyMigrationDisable returns (uint256) {
    return 0;
  }
  ...
  function transferToMigrator() external onlyMigrationEnabled {
    address xGFMigrator = IxGF(xGF).xGFMigrator();
    require(msg.sender == xGFMigrator, "not from migrator");

    uint256 balance = rewardToken.myBalance();
    rewardToken.safeTransfer(xGFMigrator, balance);
    emit LogMigrated(xGFMigrator, balance);
  }
}
