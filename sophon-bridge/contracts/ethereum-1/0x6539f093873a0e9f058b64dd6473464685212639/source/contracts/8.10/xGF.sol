...
contract xGF is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
  ...
  event LogMigrated(address indexed to, uint256 amount);
  ...
  modifier onlyMigrationDisable() {
    require(!migrationEnabled, "Not allowed while migration is enabled");
    _;
  }
  ...
  function createLock(uint256 _amount, uint256 _unlockTime) external onlyEOAorWhitelisted nonReentrant onlyMigrationDisable {
  ...
  /// @param _amount The amount that user wishes to deposit
  function depositFor(address _for, uint256 _amount) external nonReentrant onlyMigrationDisable {
  ...
  function adjustLock(uint256 _amount, uint256 _newUnlockTime) external onlyEOAorWhitelisted nonReentrant onlyMigrationDisable {
  ...
  function withdraw() external nonReentrant onlyMigrationDisable {
  ...
  function earlyWithdraw(uint256 _amount) external nonReentrant onlyMigrationDisable {
    LockedBalance memory _lock = locks[msg.sender];
  ...
  function setGFMigrator(address _xGFMigrator) external onlyOwner {
    require(_xGFMigrator != address(0), "can not set to zero address");
    xGFMigrator = _xGFMigrator;
  }

  function transferToMigrator() external onlyMigrationEnabled {
    require(msg.sender == xGFMigrator, "not from migrator");

    uint256 migrateBalance = token.myBalance() - accumRedistribute;
    
    token.safeTransfer(xGFMigrator, migrateBalance);
    emit LogMigrated(xGFMigrator, migrateBalance);
  }
}
