...
import {CurvePoolUtil} from "core-v2/src/libraries/CurvePoolUtil.sol";
import {ICurvePool} from "core-v2/src/interfaces/ICurvePool.sol";
import {IStableSwapNG} from "core-v2/src/interfaces/IStableSwapNG.sol";
import {ICurveNGPool} from "core-v2/src/interfaces/ICurveNGPool.sol";
...
contract RouterUtil {
    ...
    function spotExchangeRate(address _curvePool, uint256 _i, uint256 _j) public view returns (uint256) {
        if (_i == 0 && _j == 1) {
            return CurvePoolUtil.CURVE_UNIT.mulDiv(CurvePoolUtil.CURVE_UNIT, ICurvePool(_curvePool).last_prices());
        } else if (_i == 1 && _j == 0) {
     ...
     * @return The spot exchange rate of _i in terms of _j
     */
    function spotExchangeRateSNG(address _curvePool, int128 _i, int128 _j) public view returns (uint256) {
        ...
        if (_i == 0 && _j == 1) {
            last_prices = stored_rates[0].mulDiv(CurvePoolUtil.CURVE_UNIT, stored_rates[1])
                .mulDiv(CurvePoolUtil.CURVE_UNIT, last_prices);
            return last_prices;
        } else if (_i == 1 && _j == 0) {
     ...
     */
    function convertIBTToYTSpot(uint256 _inputIBTAmount, address _curvePool) public view returns (uint256) {
        ...
        uint256 ptRateInUnderlying = IPrincipalToken(pt).convertToUnderlying(10 ** IPrincipalToken(pt).decimals());
        uint256 ptRate = RayMath.toRay(ptRateInUnderlying, IERC20Metadata(IPrincipalToken(pt).underlying()).decimals()); // Ray

        uint256 ptInUnderlyingRay = spotExchangeRate(_curvePool, 1, 0).mulDiv(ibtRate, CurvePoolUtil.CURVE_UNIT);
     ...
     * @return The upper bound for search interval in root finding algorithms
     */
    function convertIBTToYTSpotSNG(uint256 _inputIBTAmount, address _curvePool) public view returns (uint256) {
        // The spot exchange rate between IBT and YT is evaluated using the tokenization equation without fees.
        // This equation reads: ptRate = 1 PT + 1 YT .
        ...
        uint256 ptInUnderlyingRay = spotExchangeRateSNG(_curvePool, 1, 0).mulDiv(ibtRate, CurvePoolUtil.CURVE_UNIT);
        if (ptInUnderlyingRay > ptRate) {
            revert PoolLiquidityError();
        }
        uint256 ytInUnderlyingRay = ptRate - ptInUnderlyingRay;
     ...
     */
    function previewFlashSwapIBTToExactYT(address _curvePool, uint256 _outputYTAmount)
        public
        view
        returns (uint256 inputIBTAmount, uint256 borrowedIBTAmount)
    {
        // Tokens
        address pt = ICurvePool(_curvePool).coins(1);
        ...
        borrowedIBTAmount = (_outputYTAmount * ptRate * Constants.UNIT)
        .ceilDiv(ibtRate * (Constants.UNIT - IPrincipalToken(pt).getTokenizationFee()));
        if (swapPTForIBT > borrowedIBTAmount) {
            revert PoolLiquidityError();
        }
        inputIBTAmount = borrowedIBTAmount + _getFlashFee(pt, ibt, borrowedIBTAmount) - swapPTForIBT;
    }
     ...
     * @return borrowedIBTAmount the quantity of IBT borrowed to execute that swap
     */
    function previewFlashSwapIBTToExactYTSNG(address _curvePool, uint256 _outputYTAmount)
        public
        view
        returns (uint256 inputIBTAmount, uint256 borrowedIBTAmount)
    {
     ...
     * @return borrowedIBTAmount The quantity of IBT borrowed to execute that swap.
     */
    function previewFlashSwapExactIBTToYT(address _curvePool, uint256 _inputIBTAmount)
        public
        view
        returns (uint256 ytAmount, uint256 borrowedIBTAmount)
    {
        // initial guesses
        address pt = ICurvePool(_curvePool).coins(1);
        ...
        for (uint256 i = 0; i < Constants.MAX_ITERATIONS_SECANT; ++i) {
            if (_delta(x0, x1).mulDiv(ibtUnit, Math.max(x0, x1)) < ibtUnit / Constants.PRECISION_DIVISOR) {
                break;
            }
            ...
            // x0, x1 = x1, x2
            uint256 x2 =
                (x1.toInt256() - ((answer1 * (x1.toInt256() - x0.toInt256())) / (answer1 - answer0))).toUint256();
            x0 = x1;
            x1 = x2;
        }
     ...
     * @return borrowedIBTAmount The quantity of IBT borrowed to execute that swap.
     */
    function previewFlashSwapExactIBTToYTSNG(address _curvePool, uint256 _inputIBTAmount)
        public
        view
        returns (uint256 ytAmount, uint256 borrowedIBTAmount)
    {
        ...
        uint256 resInputIBTAmount;
        (resInputIBTAmount, borrowedIBTAmount) = previewFlashSwapIBTToExactYTSNG(_curvePool, ytAmount);
     ...
     */
    function previewFlashSwapExactYTToIBT(address _curvePool, uint256 inputYTAmount)
        public
        view
     ...
     * @return The amount of IBT borrowed to execute that swap.
     */
    function previewFlashSwapExactYTToIBTSNG(address _curvePool, uint256 inputYTAmount)
        public
        view
        returns (uint256, uint256)
    {
     ...
     */
    function previewAddLiquidityWithAsset(address _curvePool, uint256 _assets)
        public
        view
     ...
     * @return minMintAmount amount of lp tokens received
     */
    function previewAddLiquidityWithAssetSNG(address _curvePool, uint256 _assets)
        public
        view
        returns (uint256 minMintAmount)
    {
     ...
     * @return minMintAmount amount of lp tokens received
     */
    function previewAddLiquidityWithIBT(address _curvePool, uint256 _ibts) public view returns (uint256 minMintAmount) {
        address pt = ICurvePool(_curvePool).coins(1);
        uint256 ibtToDepositInPT = CurvePoolUtil.calcIBTsToTokenizeForCurvePool(_ibts, _curvePool, pt);
     ...
     * @return minMintAmount amount of lp tokens received
     */
    function previewAddLiquidityWithIBTSNG(address _curvePool, uint256 _ibts)
        public
        view
        returns (uint256 minMintAmount)
    {
     ...
     */
    function previewRemoveLiquidityForAsset(address _curvePool, uint256 _lpAmount)
        public
        view
        returns (uint256 assets)
    {
        uint256[2] memory minAmounts = CurvePoolUtil.previewRemoveLiquidity(_curvePool, _lpAmount);
        assets = IERC4626(ICurvePool(_curvePool).coins(0)).previewRedeem(minAmounts[0])
     ...
     * @return assets amount of asset received
     */
    function previewRemoveLiquidityForAssetSNG(address _curvePool, uint256 _lpAmount)
        public
        view
        returns (uint256 assets)
    {
        uint256[] memory minAmounts = CurvePoolUtil.previewRemoveLiquiditySNG(_curvePool, _lpAmount);
        assets = IERC4626(ICurvePool(_curvePool).coins(0)).previewRedeem(minAmounts[0])
            + IPrincipalToken(ICurvePool(_curvePool).coins(1)).previewRedeem(minAmounts[1]);
    }
     ...
     */
    function previewRemoveLiquidityForIBT(address _curvePool, uint256 _lpAmount) public view returns (uint256 ibts) {
        uint256[2] memory minAmounts = CurvePoolUtil.previewRemoveLiquidity(_curvePool, _lpAmount);
        ibts = minAmounts[0] + IPrincipalToken(ICurvePool(_curvePool).coins(1)).previewRedeemForIBT(minAmounts[1]);
    }
     ...
     * @return ibts amount of ibt received
     */
    function previewRemoveLiquidityForIBTSNG(address _curvePool, uint256 _lpAmount) public view returns (uint256 ibts) {
        uint256[] memory minAmounts = CurvePoolUtil.previewRemoveLiquiditySNG(_curvePool, _lpAmount);
        ibts = minAmounts[0] + IPrincipalToken(ICurvePool(_curvePool).coins(1)).previewRedeemForIBT(minAmounts[1]);
    }
     ...
     */
    function previewRemoveLiquidity(address _curvePool, uint256 _lpAmount)
        public
        view
     ...
     * @return minAmounts array of length two cointaining the amount of ibt and pt received after withdrawing from the curve pool
     */
    function previewRemoveLiquiditySNG(address _curvePool, uint256 _lpAmount)
        public
        view
     ...
     */
    function previewRemoveLiquidityOneCoin(address _curvePool, uint256 _lpAmount, uint256 _i)
        public
        view
     ...
     * @return minAmount amount of token at index _i after withdrawing from the curve pool
     */
    function previewRemoveLiquidityOneCoinSNG(address _curvePool, uint256 _lpAmount, int128 _i)
        public
        view
        returns (uint256 minAmount)
    {
     ...
     * @return borrowedIBTAmount the quantity of IBT borrowed to execute that swap
     */
    function previewNGFlashSwapIBTToExactYT(address _curvePool, uint256 _outputYTAmount)
        public
        view
        returns (uint256 inputIBTAmount, uint256 borrowedIBTAmount)
    {
     ...
     * @return borrowedIBTAmount The quantity of IBT borrowed to execute that swap.
     */
    function previewNGFlashSwapExactIBTToYT(address _curvePool, uint256 _inputIBTAmount)
        public
        view
        returns (uint256 ytAmount, uint256 borrowedIBTAmount)
    {
     ...
     * @return The amount of IBT borrowed to execute that swap.
     */
    function previewNGFlashSwapExactYTToIBT(address _curvePool, uint256 inputYTAmount)
        public
        view
        returns (uint256, uint256)
    {
    ...
    function previewNGAddLiquidityWithAsset(address _curvePool, uint256 _assets)
        public
        view
        returns (uint256 minMintAmount)
    {
    ...
    function previewNGAddLiquidityWithIBT(address _curvePool, uint256 _ibts)
        public
        view
        returns (uint256 minMintAmount)
    {
    ...
    function previewNGAddLiquidity(address _curvePool, uint256[2] memory _amounts)
        public
        view
        returns (uint256 minMintAmount)
    {
    ...
    function previewNGRemoveLiquidityForAsset(address _curvePool, uint256 _lpAmount)
        public
        view
        returns (uint256 assets)
    {
        uint256[2] memory minAmounts = CurvePoolUtil.previewRemoveLiquidityNG(_curvePool, _lpAmount);
        assets = IERC4626(ICurveNGPool(_curvePool).coins(0)).previewRedeem(minAmounts[0])
            + IPrincipalToken(ICurveNGPool(_curvePool).coins(1)).previewRedeem(minAmounts[1]);
    }

    function previewNGRemoveLiquidityForIBT(address _curvePool, uint256 _lpAmount) public view returns (uint256 ibts) {
        uint256[2] memory minAmounts = CurvePoolUtil.previewRemoveLiquidityNG(_curvePool, _lpAmount);
        ibts = minAmounts[0] + IPrincipalToken(ICurvePool(_curvePool).coins(1)).previewRedeemForIBT(minAmounts[1]);
    }

    function previewNGRemoveLiquidity(address _curvePool, uint256 _lpAmount)
        public
        view
        returns (uint256[2] memory minAmounts)
    {
    ...
    function previewNGRemoveLiquidityOneCoin(address _curvePool, uint256 _lpAmount, uint256 _i)
        public
        view
        returns (uint256 minAmount)
    {
     ...
     */
    function _getFlashFee(address _pt, address _ibt, uint256 _borrowedIBTAmount) internal view returns (uint256) {
