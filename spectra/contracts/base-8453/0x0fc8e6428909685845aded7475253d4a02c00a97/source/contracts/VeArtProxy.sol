...
contract VeArtProxy is IVeArtProxy {
    ...
    function tokenURI (uint256 _tokenId) external view returns (string memory output) {
        ...
        string memory attributes = string(abi.encodePacked(
	        '{ "trait_type": "Unlock Date", "value": "',
	            ...
	            '"}, ',
	        '{ "trait_type": "veSPECTRA Balance", "value": "',
	            readableBalance,
	            '"}, ',
	        '{ "trait_type": "Locked SPECTRA", "value": "',
	            readableAmount, '"}'
        ));
		    ...
		    bytes(
                string(
                    abi.encodePacked(
                        ...
                        '", "background_color": "121a26", "description": "Spectra is a permissionless DeFi protocol that enables users to create, trade, and manage yield derivatives. Spectra NFTs vote on token emissions and receive bribes and fees generated by the protocol.", "image": "data:image/svg+xml;base64,',
    ...
    /// @dev Calculates the number of digits before the "decimal point" in an NFT's veSPECTRA balance.
    ///      Input expressed in 1e18 format.
    function numBalanceDigits (uint256 _balanceOf) internal pure returns (int256 digitCount) {
        uint256 convertedveSPECTRAvalue = _balanceOf / 1e18;
        while (convertedveSPECTRAvalue != 0) {
            convertedveSPECTRAvalue /= 10;
            digitCount++;
        }
    }
    ...
    /// @dev Determines the number of lines in the SVG. NFTs with less than 10 veSPECTRA balance have zero lines.
