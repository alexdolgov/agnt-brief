...
library BalancerLPSharePricing {
    ...
    function priceBptCPMM(
        ...
        **********************************************************************************************/
        ...
        for (uint256 i = 0; i < weights.length; i++) {
            prod = prod.mulDown(
                FixedPoint.powDown(
                    underlyingPrices[i].divDown(weights[i]),
                    weights[i]
                )
            );
    ...
    function priceBpt2CLP(
        ...
        **********************************************************************************************/
        ...
        if (px.divDown(py) <= sqrtAlpha.mulUp(sqrtAlpha)) {
            ...
            bptPrice = invariantDivSupply.mulDown(py).mulDown(
                sqrtBeta - sqrtAlpha
            );
        } else {
    ...
    function priceBpt3CLP(
        ...
        term = (underlyingPrices[0] + underlyingPrices[1] + underlyingPrices[2])
            .mulUp(cbrtAlpha);
    ...
    function relativeEquilibriumPrices3CLP(
        ...
        if (pYZ < alpha.mulDown(pXZ).mulDown(pXZ)) {
            ...
            else if (pXZ > alphaInv.mulDown(pYZ))
    ...
    function priceBptECLP(
        ...
        **********************************************************************************************/
        (int256 px, int256 py) = (
            underlyingPrices[0].toInt256(),
            underlyingPrices[1].toInt256()
        );
        ...
        if (pxIny < params.alpha) {
            ...
            bptPrice = scalarProdDown(IECLP.Vector2(px, py), vec)
                .toUint256()
                .mulDown(invariantDivSupply);
        }
    }
    ...
    function scalarProdDown(
        IECLP.Vector2 memory t1,
        IECLP.Vector2 memory t2
    ...
    function mulAinv(
        IECLP.Params memory params,
        IECLP.Vector2 memory t
    ) internal pure returns (IECLP.Vector2 memory tp) {
        tp.x =
            t.x.mulDownMag(params.lambda).mulDownMag(params.c) +
            t.y.mulDownMag(params.s);
        tp.y =
            -t.x.mulDownMag(params.lambda).mulDownMag(params.s) +
            t.y.mulDownMag(params.c);
    }
    ...
    function mulA(
        IECLP.Params memory params,
        IECLP.Vector2 memory tp
    ) internal pure returns (IECLP.Vector2 memory t) {
        t.x =
    ...
    function zeta(
        IECLP.Params memory params,
        int256 px
    ) internal pure returns (int256 pxc) {
        IECLP.Vector2 memory nd = mulA(
            params,
            IECLP.Vector2(-SignedFixedPoint.ONE, px)
        );
    ...
    function tau(
        IECLP.Params memory params,
        int256 px
    ) internal pure returns (IECLP.Vector2 memory tpp) {
    ...
    function eta(int256 pxc) internal pure returns (IECLP.Vector2 memory tpp) {
        int256 z = FixedPoint
            .powDown(
                FixedPoint.ONE + (pxc.mulDownMag(pxc).toUint256()),
                ONEHALF
            )
            .toInt256();
        tpp = eta(pxc, z);
