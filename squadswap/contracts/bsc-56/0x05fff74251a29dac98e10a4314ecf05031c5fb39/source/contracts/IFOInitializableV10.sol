...
contract IFOInitializableV10 is IIFOV10, ReentrancyGuardTransient, Ownable {
    ...
    struct PoolCharacteristics {
        ...
        bool distributeOfferingToken; // whether to distribute offering tokens from contract (true) or track only (false)
    ...
    function depositPool(
        ...
        // Verify tokens were deposited properly only if distribution is enabled for this pool
        if (_poolInformation[_pid].distributeOfferingToken) {
            if (
                IERC20(addresses[2]).balanceOf(address(this)) <
                totalTokensOffered
            ) {
        ...
        if (offeringTokenAmount > 0) {
            if (100 - _poolInformation[_pid].vestingConfig.percentage > 0) {
                uint256 amount = (offeringTokenAmount *
                    (100 - _poolInformation[_pid].vestingConfig.percentage)) /
                    100;

                // Transfer the tokens at TGE only if distribution is enabled for this pool
                if (_poolInformation[_pid].distributeOfferingToken) {
            ...
            if (_poolInformation[_pid].vestingConfig.percentage > 0) {
                uint256 amount = (offeringTokenAmount *
                    _poolInformation[_pid].vestingConfig.percentage) / 100;

                // Create VestingSchedule object only if distribution is enabled for this pool
                if (_poolInformation[_pid].distributeOfferingToken) {
    ...
    function recoverWrongTokens(
        ...
        if (
            _tokenAddress == addresses[0] ||
            _tokenAddress == addresses[1] ||
            _tokenAddress == addresses[2]
        ) {
            revert InvalidToken();
        }
     ...
     * @param _pid Pool id
     * @param _distributeOfferingToken Whether to distribute offering tokens from contract (true) or track only (false)
    ...
    function setPool(
        ...
        poolInfo.distributeOfferingToken = _distributeOfferingToken;
        ...
        poolInfo.vestingConfig.slicePeriodSeconds = _vestingConfig
            .slicePeriodSeconds;
    ...
    function updateStartAndEndTimestamps(
        ...
        emit NewStartAndEndTimestamps(
            _startAndEndTimestamps[0],
            _startAndEndTimestamps[1]
        );
    }
    ...
    /**
     * @notice External view function to see user deposit amount and claim status
     * @param _user User address
     * @param _pid Pool id
     * @return amountPool Amount of LP tokens deposited by user
     * @return claimedPool Whether user has claimed tokens
     */
    function viewUserDeposit(
        address _user,
        uint8 _pid
    ) external view returns (uint256 amountPool, bool claimedPool) {
        return (
            _userInfo[_user][_pid].amountPool,
    ...
    /**
     * @notice Get offering amount for a single user in a specific pool
     * @param _user User address
     * @param _pid Pool id
     * @return Offering amount for the user
     */
    function getUserOfferingAmount(
        address _user,
        uint8 _pid
    ) external view returns (uint256) {
        if (_poolInformation[_pid].raisingAmountPool == 0) {
            return 0;
        }
        (
            uint256 offeringAmount,
        ...
        ) = _calculateOfferingAndRefundingAmountsPool(_user, _pid);
        return offeringAmount;
    }

    /**
     * @notice Get offering amounts for multiple users in batch (for off-chain distribution)
     * @param _users Array of user addresses
     * @param _pid Pool id
     * @return users Array of user addresses that have deposits (filters out zero deposits)
     * @return amounts Array of offering amounts corresponding to users
     */
    function getBatchUserOfferingAmounts(
        ...
        // First pass: count users with deposits
        uint256 count = 0;
        for (uint256 i = 0; i < _users.length; i++) {
            if (_userInfo[_users[i]][_pid].amountPool > 0) {
                count++;
            }
        }
        ...
        for (uint256 i = 0; i < _users.length; i++) {
            if (_userInfo[_users[i]][_pid].amountPool > 0) {
                users[index] = _users[i];
                (
                    uint256 offeringAmount,
                    ,

                ) = _calculateOfferingAndRefundingAmountsPool(_users[i], _pid);
                amounts[index] = offeringAmount;
                index++;
            }
        }
    ...
    function release(bytes32 _vestingScheduleId) external nonReentrant {
        // Check validation
        IFOLibV10.IsVestingInitializedPreCheck(
        ...
        vestingSchedulesTotalAmount =
            vestingSchedulesTotalAmount -
            vestedAmount;
        IERC20(addresses[2]).safeTransfer(
    ...
    function _computeReleasableAmount(
        ...
        if (
            block.timestamp <
            vestingStartTime +
                _poolInformation[_vestingSchedule.pid].vestingConfig.cliff
        ) {
            return 0;
        } else if (
            block.timestamp >=
            ...
            uint256 vestedAmount = (_vestingSchedule.amountTotal *
                vestedSeconds) /
                _poolInformation[_vestingSchedule.pid].vestingConfig.duration;
    ...
    function _calculateOfferingAndRefundingAmountsPool(
        ...
        if (
            ...
            userOfferingAmount =
                (_poolInformation[_pid].offeringAmountPool * allocation) /
                1e12;

            // Calculate the payAmount
            uint256 payAmount = (_poolInformation[_pid].raisingAmountPool *
                allocation) / 1e12;
    ...
    function _getUserAllocationPool(
        ...
        if (_poolInformation[_pid].totalAmountPool > 0) {
            return
                (_userInfo[_user][_pid].amountPool * 1e12) /
                _poolInformation[_pid].totalAmountPool;
        } else {
            return 0;
        }
