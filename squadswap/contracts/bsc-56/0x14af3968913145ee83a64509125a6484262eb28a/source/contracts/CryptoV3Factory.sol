...
/// @title Canonical CryptoSwap V3 factory
/// @notice Deploys CryptoSwap V3 pools and manages ownership and control over pool protocol fees
contract CryptoV3Factory is ICryptoV3Factory {

    /// @inheritdoc ICryptoV3Factory
    ...
    function createPool(
        ...
        pool = ICryptoV3PoolDeployer(poolDeployer).deploy(address(this), token0, token1, fee, tickSpacing);
    ...
    function setFeeProtocol(address pool, uint32 feeProtocol0, uint32 feeProtocol1) external override onlyOwner {
        ICryptoV3Pool(pool).setFeeProtocol(feeProtocol0, feeProtocol1);
    }
    ...
    function collectProtocol(
        ...
        return ICryptoV3Pool(pool).collectProtocol(feeManager, amount0Requested, amount1Requested);
    }

    function setLmPool(address pool, address lmPool) external override onlyOwnerOrLmPoolDeployer {
        ICryptoV3Pool(pool).setLmPool(lmPool);
    }
