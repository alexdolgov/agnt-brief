    ...
    * @notice Allows the owner to recover tokens sent to the contract by mistake
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.timestamp > lastRewardTimestamp && stakedTokenSupply != 0) {
            ...
            uint256 adjustedTokenPerShare = accTokenPerShare + (squadReward * PRECISION_FACTOR) / stakedTokenSupply;
    ...
    function _updatePool() internal {
        ...
        uint256 multiplier = _getMultiplier(lastRewardTimestamp, block.timestamp);
        uint256 squadReward = multiplier * rewardPerSecond;
        accTokenPerShare = accTokenPerShare + (squadReward * PRECISION_FACTOR) / stakedTokenSupply;
        lastRewardTimestamp = block.timestamp;
    }
