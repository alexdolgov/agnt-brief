...
// ============================================================
// FILE: contracts/libs/utils/TransferHelper.sol
// ============================================================
...
// ============================================================
// FILE: contracts/router/libraries/SquadswapLibrary.sol
// ============================================================
...
library SquadswapLibrary {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint160(uint(keccak256(abi.encodePacked(
                ...
                hex'666b17e0f0313ce8c608a4761ae7fd0e1c936c7c63eb833ac540370647c0efdb' // init code hash
            )))));
    }

    // fetches and sorts the reserves for a pair
    function getReserves(address factory, address tokenA, address tokenB) internal view returns (uint reserveA, uint reserveB, uint fee) {
        (address token0,) = sortTokens(tokenA, tokenB);
        (uint reserve0, uint reserve1, , uint _fee) = ISquadswapPair(pairFor(factory, tokenA, tokenB)).getReserves();
        fee = _fee;
    ...
    function getAmountIn(uint amountOut, uint reserveIn, uint reserveOut, uint fee) internal pure returns (uint amountIn) {
        ...
        uint denominator = (reserveOut - amountOut) * (FEE_DENOMINATOR - fee);
    ...
    function getAmountsOut(address factory, uint amountIn, address[] memory path) internal view returns (uint[] memory amounts) {
        ...
        for (uint i; i < path.length - 1; i++) {
            (uint reserveIn, uint reserveOut, uint fee) = getReserves(factory, path[i], path[i + 1]);
    ...
    function getAmountsIn(address factory, uint amountOut, address[] memory path) internal view returns (uint[] memory amounts) {
        ...
        for (uint i = path.length - 1; i > 0; i--) {
            (uint reserveIn, uint reserveOut, uint fee) = getReserves(factory, path[i - 1], path[i]);
...
// ============================================================
// FILE: contracts/router/SquadswapRouter02.sol
// ============================================================
...
contract SquadswapRouter02 is Ownable {
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            (uint reserve0, uint reserve1, , uint fee) = pair.getReserves();
            (uint reserveInput, uint reserveOutput) = input == token0 ? (reserve0, reserve1) : (reserve1, reserve0);
            amountInput = IERC20(input).balanceOf(address(pair)) - reserveInput;
            amountOutput = SquadswapLibrary.getAmountOut(amountInput, reserveInput, reserveOutput, fee);
            }
    ...
    {
        return SquadswapLibrary.getAmountOut(amountIn, reserveIn, reserveOut, fee);
    }
    ...
    {
        return SquadswapLibrary.getAmountIn(amountOut, reserveIn, reserveOut, fee);
    }
