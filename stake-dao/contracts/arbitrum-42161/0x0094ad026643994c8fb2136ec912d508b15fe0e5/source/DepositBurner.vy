...
struct SwapData:
    ...
    multiplier: DynArray[uint256, N_COINS_MAX]  # decimals multiplier
...
def _get_min_lp(_swap_data: SwapData, _amounts: DynArray[uint256, N_COINS_MAX]) -> uint256:
    min_lp: uint256 = _amounts[0] * _swap_data.multiplier[0]
    if _swap_data.implementation in Implementation.CONST:
        for i in range(1, N_COINS_MAX):
            if i == len(_swap_data.multiplier):
                break
            min_lp += _amounts[i] * _swap_data.multiplier[i]
    elif _swap_data.implementation in Implementation.ORACLE:
        min_lp += _amounts[1] * _swap_data.multiplier[1] * ONE / _swap_data.pool.price_oracle()
    elif _swap_data.implementation in Implementation.ORACLE_NUM:
        for i in range(1, N_COINS_MAX):
            if i == len(_swap_data.multiplier):
                break
            min_lp += _amounts[i] * _swap_data.multiplier[i] * ONE / _swap_data.pool.price_oracle(i - 1)
...
def _add_liquidity(
    ...
    if len(_swap_data.coins) == 2:
        ...
        if _swap_data.implementation in Implementation.UNDERLYING:
            Pool2(_swap_data.pool.address).add_liquidity(amounts, _min_lp, True, value=_eth_amount)
        else:
            Pool2(_swap_data.pool.address).add_liquidity(amounts, _min_lp, value=_eth_amount)
        return

    if len(_swap_data.coins) == 3:
        amounts: uint256[3] = [_amounts[0], _amounts[1], _amounts[2]]
        if _swap_data.implementation in Implementation.UNDERLYING:
            Pool3(_swap_data.pool.address).add_liquidity(amounts, _min_lp, True, value=_eth_amount)
        else:
            Pool3(_swap_data.pool.address).add_liquidity(amounts, _min_lp, value=_eth_amount)
        return

    if len(_swap_data.coins) == 4:
        amounts: uint256[4] = [_amounts[0], _amounts[1], _amounts[2], _amounts[3]]
        if _swap_data.implementation in Implementation.UNDERLYING:
            Pool4(_swap_data.pool.address).add_liquidity(amounts, _min_lp, True, value=_eth_amount)
        else:
            Pool4(_swap_data.pool.address).add_liquidity(amounts, _min_lp, value=_eth_amount)
        return

    if len(_swap_data.coins) == 5:
        amounts: uint256[5] = [_amounts[0], _amounts[1], _amounts[2], _amounts[3], _amounts[4]]
        if _swap_data.implementation in Implementation.UNDERLYING:
            Pool5(_swap_data.pool.address).add_liquidity(amounts, _min_lp, True, value=_eth_amount)
        else:
            Pool5(_swap_data.pool.address).add_liquidity(amounts, _min_lp, value=_eth_amount)
        return
