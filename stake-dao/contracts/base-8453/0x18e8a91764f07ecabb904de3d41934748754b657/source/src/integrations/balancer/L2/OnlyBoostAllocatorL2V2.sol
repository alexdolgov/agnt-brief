...
/// @dev Minimal interface for L2 Curve/Balancer gauge (inflation_rate is a week-indexed mapping)
interface ICurveGaugeL2 {
    function working_supply() external view returns (uint256);
    function inflation_rate(uint256 week) external view returns (uint256);
    function totalSupply() external view returns (uint256);
}

/// @title OnlyBoostAllocatorL2V2
/// @author Stake DAO
/// @custom:github @stake-dao
/// @custom:contact contact@stakedao.org

/// @notice L2 variant of OnlyBoostAllocator that uses quadratic optimization (v2.0 formula)
///         adapted for L2 gauges where inflation_rate is a per-gauge week-indexed mapping
///         instead of a global rate requiring gauge controller weight lookup.
contract OnlyBoostAllocatorL2V2 is Allocator {
    ...
    /// @notice Address of the Boost Delegation contract (e.g., BalancerBoostProviderAdapter)
    address public immutable BOOST_PROVIDER;

    /// @notice Address of the Sidecar Boost Holder contract (e.g., Aura Voter Proxy)
    address public immutable SIDECAR_BOOST_HOLDER;

    /// @notice Address of the Sidecar Factory contract (e.g., AuraSidecarFactoryL2)
    ISidecarFactory public immutable SIDECAR_FACTORY;

    /// @notice Protocol controller that manages permissions.
    IProtocolController public immutable PROTOCOL_CONTROLLER;

    /// @notice Accountant for fee lookup (direct reference, no protocol controller indirection).
    address public immutable ACCOUNTANT;

    /// @notice One week in seconds, used as the inflation_rate mapping key divisor.
    ...
    /// @notice Initializes the OnlyBoostAllocatorL2V2 contract
    ...
    /// @param _boostProvider Address of the boost provider contract (BalancerBoostProviderAdapter)
    /// @param _sidecarBoostHolder Address of the sidecar boost holder (e.g., Aura Voter Proxy)
    /// @param _accountant Address of the accountant for fee lookup
    constructor(
        address _locker,
        address _gateway,
    ...
    struct OptimalBalanceParams {
        ...
        uint256 R; // Actual gauge emission rate (per-gauge on L2, already accounts for gauge share)
        uint256 B; // Gauge total supply
        uint256 V; // veBAL total supply (via boost provider)
    }
    ...
    function _getOptimalLockerBalance(address gauge, address sidecar, uint256 balanceOfLocker)
        ...
        returns (uint256)
    {
        // 1. Fetch sidecar (Aura) gauge balance - if zero, nothing to optimize
        ...
        // 3. Get per-gauge inflation rate (L2: week-indexed mapping, already per-gauge)
        uint256 actualGaugeRate = ICurveGaugeL2(gauge).inflation_rate(block.timestamp / WEEK);
        if (actualGaugeRate == 0) return type(uint256).max;
        ...
        OptimalBalanceParams memory p = OptimalBalanceParams({
            ...
            W: ICurveGaugeL2(gauge).working_supply(),
            R: actualGaugeRate,
            B: ICurveGaugeL2(gauge).totalSupply(),
