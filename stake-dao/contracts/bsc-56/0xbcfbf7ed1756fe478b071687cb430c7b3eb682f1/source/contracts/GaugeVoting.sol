...
contract GaugeVoting is Ownable, Pausable {
    ...
    address constant NO_NEED_MASTERCHEF = address(1);

    /// @dev v4FakeAddress => chainId => v4PoolInfo
    mapping(address => mapping(uint256 => V4PoolInfo)) public gaugeAddressTov4PoolInfo;
    ...
    function addGauge(
        ...
        _addGauge(gauge_addr, gauge_type, _weight, _pid, _masterChef, _chainId, _boostMultiplier, _maxVoteCap);
    }

    /// @notice Add gauge for v4 `gauge_poolId`, `gauge_poolManager` of type `gauge_type` with weight `weight`
    /// @param gauge_poolId Gauge v4 poolId
    /// @param gauge_poolManager Gauge v4 pool manager
    /// @param gauge_type Gauge type
    /// @param _weight Gauge weight
    /// @param _chainId the gauge's chainId
    /// @param _boostMultiplier The boost for weight
    ...
    function addGaugeV4(
        ...
        address gauge_addr = getGaugeV4Address(gauge_poolId, _chainId);
        require(gauge_addr != address(0), "Invalid gauge parameter");

        // store mapping for gauge_addr => v4GaugeInfo
        V4PoolInfo memory poolInfo;
        poolInfo.poolId = gauge_poolId;
        poolInfo.poolManager = gauge_poolManager;
        gaugeAddressTov4PoolInfo[gauge_addr][_chainId] = poolInfo;

        _addGauge(gauge_addr, gauge_type, _weight, 0, NO_NEED_MASTERCHEF, _chainId, _boostMultiplier, _maxVoteCap);
    }
    ...
    function updateGaugeInfo(
        ...
        bytes32 gauge_hash = _getHash(gauge_addr, _chainId);
    ...
    function voteForGaugeWeights(
        ...
        bytes32 gauge_hash = _getHash(_gauge_addr, _chainId);
        ...
        if (inCap) {
            if (info.maxVoteCap >= 100 && bias > 0) {
                ...
                for (uint256 i = 0; i < gaugeCount; i++) {
                    ...
                    bytes32 gauge_hash1 = _getHash(info1.pairAddress, info1.chainId);
    ...
    /// @notice Get address of gauge v4 by poolId
    /// @param _poolId Pool id
    /// @param _chainId chain id
    /// @return Address of gauge
    function getGaugeV4Address(bytes32 _poolId, uint256 _chainId) public view returns (address) {
        return address(uint160(uint256(keccak256(abi.encodePacked(_poolId, _chainId)))));
    }
    ...
    function _addGauge(
        ...
        if (gauge_weight > 0) {
            ...
            gaugeTypePointsSum[gauge_type][nextTime].bias =
                (gauge_weight * _boostMultiplier) /
                BOOST_PRECISION +
                oldTypeSum;
            gaugeTypeSumLastScheduled[gauge_type] = nextTime;
            gaugePointsTotal[nextTime] = oldTotal + (typeWeight * gauge_weight * _boostMultiplier) / BOOST_PRECISION;
            totalLastScheduled = nextTime;

            gaugePointsWeight[gauge_hash][nextTime].bias = (gauge_weight * _boostMultiplier) / BOOST_PRECISION;
        }
        ...
        emit NewGauge(gauge_hash, gauge_type, gauge_weight, _pid, _masterChef, _chainId, _boostMultiplier, _maxVoteCap);
    }
    ...
    /// @notice Output the hash string for pack gauge_addr and chainId
    function _getHash(address gauge_addr, uint256 _chainId) internal view returns (bytes32 gauge_hash) {
        gauge_hash = keccak256(abi.encodePacked(gauge_addr, _chainId));
    }
}
