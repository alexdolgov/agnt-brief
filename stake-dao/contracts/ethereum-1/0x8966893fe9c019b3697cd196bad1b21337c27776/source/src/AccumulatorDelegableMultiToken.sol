...
abstract contract AccumulatorDelegableMultiToken is AccumulatorBase {
    ...
    function _shareWithDelegation(address _token) internal returns (uint256 delegationShare) {
        ...
        return _shareWithDelegationAmount(_token, amount);
    }

    /// @notice Shares a specific amount of rewards with the delegation contract
    /// @param _token The token to share with delegation
    /// @param _amount The amount to calculate delegation share from
    /// @return delegationShare The amount shared with delegation
    function _shareWithDelegationAmount(address _token, uint256 _amount) internal returns (uint256 delegationShare) {
        ...
        delegationShare = _amount * bpsDelegated / DENOMINATOR;
