...
contract PausableUntil {
    ...
    event Paused(uint256 duration, uint256 pausedUntil);
    /// @notice Emitted when resumed by the `resume` call
    event Resumed(uint256 resumedAt);

    error ZeroPauseDuration();
    error PausedExpected();
    error ResumedExpected();
    error PauseUntilMustBeInFuture();
    error DurationExtensionRequired();
    ...
    function _pauseFor(uint256 _duration) internal {
        ...
        uint256 currentPauseUntil = RESUME_SINCE_TIMESTAMP_POSITION.getStorageUint256();
        if (resumeSince <= currentPauseUntil) revert DurationExtensionRequired();
    ...
    function _setPausedState(uint256 _resumeSince) internal {
        ...
        uint256 pausedUntil = _resumeSince == PAUSE_INFINITELY ? PAUSE_INFINITELY : _resumeSince - 1;
        
        if (_resumeSince == PAUSE_INFINITELY) {
            emit Paused(PAUSE_INFINITELY, pausedUntil);
        } else {
            emit Paused(_resumeSince - block.timestamp, pausedUntil);
        }
    }
}
