    ...
    UUPSUpgradeable
{
    ...
    function createVaultStrategyAndRegisterJob(
        ...
        tokenId = IStrategyRegistry(strategyRegistry).createStrategy(
            params.strategyCreator,
            params.name,
            params.execBundle,
            params.maxGasCost,
            params.maxGasPerAction
        );
        newVault = IVaultRegistry(vaultRegistry).createVault(
        ...
        IDynamicJobs(newVault).registerJob(
            ...
            params.targetAddresses,
            params.jobName,
            params.ipfsForJobDetails
        );
    }
    ...
    function _deposit(
        ...
        IVaultRegistry.VaultState state = IVaultRegistry(vaultRegistry)
            .getVaultDetails(vaultAddress)
            .state;
        require(
            state == IVaultRegistry.VaultState.PendingThreshold ||
                state == IVaultRegistry.VaultState.Active,
            "Invalid Vault State"
        );
