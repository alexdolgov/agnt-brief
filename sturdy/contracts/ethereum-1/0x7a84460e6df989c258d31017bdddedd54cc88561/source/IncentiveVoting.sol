...
/**
    @title Sturdy Incentive Voting
    @notice Users with STRDY balances locked in `STRDYLocker` may register their
            lock weights in this contract, and use this weight to vote on where
            new STRDY emissions will be released in the following week.
...
contract IncentiveVoting is DelegatedOps, SystemStart {
    ...
    ISTRDYLocker public immutable tokenLocker;
    ...
    event AccountWeightRegistered(
        ...
        ISTRDYLocker.LockData[] registeredLockData
    );
    ...
    constructor(uint256 _startTime, ISTRDYLocker _tokenLocker, address _vault) SystemStart(_startTime) {
    ...
    function clearRegisteredWeight(address account) external returns (bool) {
        ...
        if (length > 0 || frozenWeight > 0) {
            ...
            emit AccountWeightRegistered(account, week, 0, new ISTRDYLocker.LockData[](0));
        }
    ...
    function unfreeze(address account, bool keepVote) external returns (bool) {
        ...
        if (frozenWeight > 0) {
            ...
            ISTRDYLocker.LockData[] memory lockData = new ISTRDYLocker.LockData[](1);
            lockData[0] = ISTRDYLocker.LockData({ amount: amount, weeksToUnlock: MAX_LOCK_WEEKS });
    ...
    function _registerAccountWeight(address account, uint256 minWeeks) internal returns (uint256) {
        ...
        // get updated account lock weights and store locally
        (ISTRDYLocker.LockData[] memory lockData, uint256 frozen) = tokenLocker.getAccountActiveLocks(
            account,
            minWeeks
        );
