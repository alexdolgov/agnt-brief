...
/// @title ETHRSETHPendlePTOracle
/// @author Jason (Sturdy) https://github.com/iris112
/// @notice  An oracle for ETH/rsETH Pendle PT

contract ETHRSETHPendlePTOracle {
    using PendlePtOracleLib for IPMarket;

    address private constant REDSTONE_RSETH_ETH_PRICE = 0xA736eAe8805dDeFFba40cAB8c99bCB309dEaBd9B;
    ...
    function getPrices() external view returns (bool _isBadData, uint256 _priceLow, uint256 _priceHigh) {
        (, int256 _answer, , uint256 _updatedAt, ) = AggregatorV3Interface(REDSTONE_RSETH_ETH_PRICE).latestRoundData();
        // If data is stale or negative, set bad data to true and return
        if (_answer <= 0 || (block.timestamp - _updatedAt > MAX_ORACLE_DELAY)) {
            revert REDSTONE_BAD_PRICE();
        }
        uint256 rate = 1e18 * uint256(_answer) / 1e8;       // rsETH/ETH, redstone price decimal is 8
