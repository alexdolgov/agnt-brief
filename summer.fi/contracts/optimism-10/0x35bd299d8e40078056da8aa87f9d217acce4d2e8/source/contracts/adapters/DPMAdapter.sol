...
contract DPMAdapter is ISecurityAdapter {
    ...
    constructor(ServiceRegistry _serviceRegistry, IAccountGuard _accountGuard) {
        require(address(_serviceRegistry) != address(0), "dpm-adapter/invalid-service-registry");
        require(address(_accountGuard) != address(0), "dpm-adapter/invalid-account-guard");
        self = address(this);
        botAddress = _serviceRegistry.getRegisteredService(AUTOMATION_BOT_KEY);
        require(botAddress != address(0), "dpm-adapter/invalid-bot-address");
