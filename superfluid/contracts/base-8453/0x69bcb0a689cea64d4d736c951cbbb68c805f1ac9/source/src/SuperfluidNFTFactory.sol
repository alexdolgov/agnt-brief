...
contract SuperfluidNFTFactory is ISuperfluidNFTFactory, OwnableUpgradeable {
    ...
    /// @notice The base domain URI for token metadata (passed to clones on deploy)
     ...
     * @param _domainUri The base domain URI for token metadata
     */
    function initialize(
        address _owner,
        address _superfluidNFTImpl,
        address _signer,
        address _treasury,
    ...
    function deploySuperfluidNFT(
        ...
        SuperfluidNFT(newCollection).initialize(
            owner(), name, symbol, mintStartDate, mintEndDate, mintPrice, maxTokensPerUser, signer, treasury, domainUri
        );
