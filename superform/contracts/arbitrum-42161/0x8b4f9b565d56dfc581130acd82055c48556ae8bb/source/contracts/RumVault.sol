...
contract RumVault is IRumVault, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable, ERC20BurnableUpgradeable {
    ...
    modifier InvalidID(uint256 positionId, address user) {
        require(positionId < positionInfo[user].length, "RUM: positionID !valid");
        _;
    }

    modifier onlyBurner() {
        require(burner[msg.sender], "!allowed to burn");
        _;
    }
    ...
    function updateDebtAdjustment() external onlyOwner {
        ...
        require(block.timestamp - debtAdjustmentValues.time > leverageParams.debtAdjustmentInterval, "Invalid time for DA");
    ...
    function requestOpenPosition(uint256 _amount, uint16 _leverage) external payable whenNotPaused returns (uint256) {
        ...
        require(_amount > 0, "RUM: amount is zero");
    ...
    function fulfillOpenCancellation(uint256 orderId) public onlyKeeper returns (bool) {
        DepositRecord storage dr = depositRecord[orderId];
        require(dr.isOrderCompleted == false, "RUM: already fulfilled");
    ...
    function _requestClosePositionWithGmToken(uint32 _positionID, address gmToken) internal InvalidID(_positionID, msg.sender) {
        PositionInfo storage _positionInfo = positionInfo[msg.sender][_positionID];
        require(!_positionInfo.isLiquidated, "RUM: liquidated");
        require(msg.sender == _positionInfo.user, "RUM: !allowed to close");
        require(!inCloseProcess[msg.sender][_positionID], "RUM: request already ongoing");
        require(msg.value >= keeperInfo.keeperFee + feeConfiguration.hlpFee, "RUM: fee !enough");
    ...
    function requestClosePosition(uint32 _positionID) external payable InvalidID(_positionID, msg.sender) nonReentrant {
        ...
        if (_positionInfo.position > (1_000_000 * 1e18) || msg.sender == keeperInfo.keeper) {
            _requestClosePositionWithGmToken(_positionID, 0x70d95587d40A2caf56bd97485aB3Eec10Bee6336);
            emit ManualClosePosition(msg.sender, _positionID, _positionInfo.position, block.timestamp);
            return;
        }
    ...
    function fulfillCloseCancellation(uint256 orderId) public onlyKeeper returns (bool) {
        ...
        require(wr.isOrderCompleted == false, "RUM: already fulfilled");
    ...
    function fulfillClosePosition(uint256 _orderId, uint256 _returnedUSDC) public onlyKeeper returns (bool) {
        ...
        require(inCloseProcess[wr.user][wr.positionID], "Rum: request !ongoing");
    ...
    function requestLiquidatePosition(address _user, uint256 _positionID) external payable nonReentrant {
        ...
        require(!_positionInfo.isLiquidated, "RUM: liquidated");
        require(_positionInfo.user != address(0), "RUM: liquidation request !exist");
        ...
        require(currentDTVWithDA >= (leverageParams.DTVLimit * leverageParams.DTVSlippage) / 1000, "Liquidation threshold !reached yet");
