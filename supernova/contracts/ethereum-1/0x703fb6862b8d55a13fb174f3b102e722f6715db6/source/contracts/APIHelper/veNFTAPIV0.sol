...
contract veNFTAPIV0 {
    ...
    function _getNFTFromId(uint256 id, address _owner) internal view returns(veNFT memory venft){
        ...
        uint _totalPoolVotes = address(voter) != address(0) ? voter.poolVoteLength(id): 0;
        ...
        venft.rebase_amount = address(rewardDisitributor) == address(0) ? 0:rewardDisitributor.claimable(id);
        venft.lockEnd = _lockedBalance.end;
        venft.vote_ts = address(voter) == address (0) ? 0: voter.lastVotedTimestamp(id);
        venft.votes = votes;
