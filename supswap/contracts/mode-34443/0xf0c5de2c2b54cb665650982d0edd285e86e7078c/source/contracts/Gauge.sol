...
contract Gauge is Initializable {
    ...
    constructor(
        ...
        if (
            address(_nftManager) == address(0) ||
            address(_weth) == address(0) ||
            address(_referralHandler) == address(0) ||
    ...
    function buyExactTokens(
        ...
        if (!accessRegistry.isAccessible(_recipient)) {
            revert Gauge__Unauthorized();
        }
        ...
        if (fee > 0) {
            referralHandler.handleFees{value: fee}(
                _recipient,
                address(token),
                cost - fee
            );
    ...
    function buyExactEth(address _recipient, uint256 _minAmountOut) public payable {
        ...
        // Add MIN_BUY check here, before adjusting for available tokens
    ...
    function _launchPool() internal returns (address, uint256) {
        if (!gaugeActive()) revert Gauge__GaugeClosed();
        ...
        // Make approvals
        token.approve(address(poolHelper), type(uint256).max);
        ...
        uint256 sqrtPriceX96 = (Math.sqrt((amount1 * 1e18) / amount0) *
            2 ** 96) / 1e9;

        (address _pool, uint256 _tokenId) = poolHelper.createPoolAndMint(
        ...
        if (weth.balanceOf(address(this)) > 0)
            weth.transfer(lpReceiver, weth.balanceOf(address(this)));
        if (token.balanceOf(address(this)) > 0)
            token.transfer(lpReceiver, token.balanceOf(address(this)));

        emit PoolLaunched(address(token), _pool, _tokenId);

        return (_pool, _tokenId);
    }
    ...
    function quoteBuyExactTokens(
        ...
        uint256 fee = (rawCost * feeParameters.buyFee) /
            (FRACTION_DENOMINATOR - feeParameters.buyFee);
    ...
    function quoteBuyExactEth(
        ...
        uint256 fee = (_amount * feeParameters.buyFee) /
            (FRACTION_DENOMINATOR);
    ...
    function quoteSellExactEth(
        ...
        uint256 fee = (_amount * feeParameters.sellFee) /
            (FRACTION_DENOMINATOR);
