...
contract RoleManager is Governance2Step {
    ...
    function _isPositionHolder(bytes32 _positionId) internal view virtual {
        require(msg.sender == governance || msg.sender == getPositionHolder(_positionId), "!allowed");
    }

    // Encoded name so that it can be held as a constant.
    bytes32 internal constant _name_ = bytes32(abi.encodePacked("Swell Vault Role Manager"));
        ...
        Governance2Step(_governance)
    {
        ...
        _positions[DADDY] = Position({ holder: _daddy, roles: uint96(Roles.ALL) });

        // Setup default roles for Brain.
        _positions[BRAIN] = Position({
            holder: _brain,
            roles: uint96(
                Roles.REPORTING_MANAGER | Roles.DEBT_MANAGER | Roles.QUEUE_MANAGER | Roles.DEPOSIT_LIMIT_MANAGER
                    | Roles.DEBT_PURCHASER
            )
        });

        // Security can set the max debt for strategies to have.
        _positions[SECURITY] = Position({ holder: _security, roles: uint96(Roles.MAX_DEBT_MANAGER) });

        // The keeper can process reports.
        _positions[KEEPER] = Position({ holder: _keeper, roles: uint96(Roles.REPORTING_MANAGER) });

        // Debt allocators manage debt and also need to process reports.
        _positions[DEBT_ALLOCATOR].roles = uint96(Roles.REPORTING_MANAGER | Roles.DEBT_MANAGER);

        // The strategy manager can add and remove strategies.
        _positions[STRATEGY_MANAGER] = Position({
            holder: _strategyManager,
            roles: uint96(Roles.ADD_STRATEGY_MANAGER | Roles.REVOKE_STRATEGY_MANAGER)
        });
    ...
    function newVault(
        ...
        returns (address)
    {
        return _newVault(_asset, _category, _depositLimit, defaultProfitMaxUnlock);
    }
    ...
    function newVault(
        ...
        returns (address)
    {
        return _newVault(_asset, _category, _depositLimit, _profitMaxUnlockTime);
    }
        ...
        returns (address _vault)
    {
        ...
        string memory _name = string(abi.encodePacked(ERC20(_asset).symbol(), "-", _categoryString, " Swell Vault"));
        string memory _symbol = string(abi.encodePacked("swell", ERC20(_asset).symbol(), "-", _categoryString));

        // Deploy through the registry so it is automatically endorsed.
        _vault = Registry(getPositionHolder(REGISTRY)).newEndorsedVault(
            _asset, _name, _symbol, address(this), _profitMaxUnlockTime
        );
        ...
        if (_assetToVault[_asset][_apiVersion][_category] != address(0)) {
            revert AlreadyDeployed(_assetToVault[_asset][_apiVersion][_category]);
        }
        ...
        vaultConfig[_vault] =
            VaultConfig({ asset: _asset, category: _category, debtAllocator: _debtAllocator, index: vaults.length });
    ...
    function _deployAllocator(address _vault) internal virtual returns (address _debtAllocator) {
        ...
        if (factory != address(0)) {
            ...
            _debtAllocator = DebtAllocatorFactory(factory).newDebtAllocator(_vault);
        } else {
    ...
    function _sanctify(address _vault, address _debtAllocator) internal virtual {
        ...
        _setRole(_vault, Position(_debtAllocator, _positions[DEBT_ALLOCATOR].roles));
    }
    ...
    function updateKeeper(address _vault, address _keeper) external virtual onlyPositionHolder(BRAIN) {
        ...
        if (_keeper != defaultKeeper && IVault(_vault).roles(defaultKeeper) != 0) {
    ...
    function removeVault(address _vault) external virtual onlyPositionHolder(BRAIN) {
        ...
        delete _assetToVault[config.asset][IVault(_vault).apiVersion()][config.category];
    ...
    function setPositionRoles(bytes32 _position, uint256 _newRoles) external virtual onlyGovernance {
        ...
        require(_position != DEBT_ALLOCATOR && _position != KEEPER, "cannot update");
