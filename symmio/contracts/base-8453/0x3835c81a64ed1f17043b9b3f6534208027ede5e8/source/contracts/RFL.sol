	...
	PausableUpgradeable
{
	...
	bytes32 public constant TRANSFER_PAUSE_MANAGER_ROLE = keccak256("TRANSFER_PAUSE_MANAGER_ROLE");
	...
	event CollateralLocked(uint256 indexed tokenId, uint256 increasedAmount, uint256 totalLocked);

	/// @notice Emitted when `amount` collateral is removed from `tokenId`.
	event CollateralUnlocked(uint256 indexed tokenId, uint256 decreasedAmount, uint256 totalLocked);
	...
	event ReferralOpenTimestampUpdated(uint256 oldTimestamp, uint256 newTimestamp);
	 ...
	 */
	function initialize(address _admin, address _minter, address _collateral, uint256 _referralOpenTimestamp) public initializer {
	 ...
	 */
	function isCodeActiveReferrer(string memory code) public view returns (bool) {
	 ...
	 */
	function getIndexOfTokenForUser(address user, uint256 tokenId) public view returns (uint256) {
	 ...
	 */
	function safeMint(string memory code) external whenNotPaused returns (uint256) {
	 ...
	 */
	function safeMintFor(address user, string memory code) external onlyRole(MINTER_ROLE) whenNotPaused returns (uint256) {
	 ...
	 * @param code            Unique referral code for the new NFT.
	 */
	function safeMintWithReferrer(uint256 referrerTokenId, string memory code) external whenNotPaused returns (uint256 tokenId) {
	 ...
	 */
	function setReferrerTokenId(uint256 tokenId, uint256 referrerTokenId) external whenNotPaused onlyNftOwner(tokenId) {
		_setReferer(tokenId, referrerTokenId);
	}
	 ...
	 */
	function setReferrerCode(uint256 tokenId, string memory referrerCode) external whenNotPaused onlyNftOwner(tokenId) {
	 ...
	 */
	function setTokenInUse(address user, uint256 tokenId) external whenNotPaused onlyNftOwner(tokenId) {
	 ...
	 */
	function increaseLockedCollateral(uint256 tokenId, uint256 amount) external whenNotPaused onlyNftOwner(tokenId) nonReentrant {
	 ...
	 */
	function decreaseLockedCollateral(uint256 tokenId, uint256 amount) external whenNotPaused onlyNftOwner(tokenId) nonReentrant {
		if (amount == 0) revert ZeroAmount();
		if (lockedTokens[tokenId] < amount) revert InsufficientLockedCollateral();
	 ...
	 */
	function ogMint(address user, string memory code) public onlyRole(MINTER_ROLE) returns (uint256 tokenId) {
		tokenId = __safeMint(user, code);
		isOg[tokenId] = true;
	}
	 ...
	 */
	function setReferralOpenTimestamp(uint256 newReferralOpenTimestamp) external onlyRole(SETTER_ROLE) {
		emit ReferralOpenTimestampUpdated(referralOpenTimestamp, newReferralOpenTimestamp);
	 ...
	 */
	function __safeMint(address user, string memory code) internal returns (uint256 tokenId) {
		...
		// Auto-select token on first mint
		if (balanceOf(user) == 1) _setTokenInUse(user, tokenId);
	}
	...
	function _setReferer(uint256 tokenId, uint256 referrerTokenId) internal {
		...
		if (!isTokenActiveReferrer(referrerTokenId)) {
			if (!isOg[referrerTokenId] && block.timestamp < referralOpenTimestamp) revert NonOgReferralNotYetAllowed();
	 ...
	 */
	function transferInterceptor(address from, address to, uint256 tokenId) internal {
		if (isTransferPaused && from != address(0) && to != address(0)) revert TransfersIsPaused();
		...
		if (to != address(0)) {
			...
			// === Burning ===
			if (lockedTokens[tokenId] > 0) revert CantBurnTokenWithLockedBalance();
	...
	function supportsInterface(
		bytes4 interfaceId
	) public view override(AccessControlEnumerableUpgradeable, ERC721EnumerableUpgradeable, ERC721Upgradeable) returns (bool) {
	...
	function _update(
		...
		uint256 tokenId,
		address auth
	) internal override(ERC721EnumerableUpgradeable, ERC721Upgradeable) whenNotPaused returns (address) {
