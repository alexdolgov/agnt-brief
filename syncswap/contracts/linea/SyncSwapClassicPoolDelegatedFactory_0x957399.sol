// ============================================================
// FILE: contracts/flatten_deployment_linea/classic_factory_delegate.sol
// ============================================================
...
// File contracts/pool/classic-delegate/SyncSwapClassicPoolDelegated.sol
...
contract SyncSwapClassicPoolDelegated is ERC20Permit2, PoolFlashLoans {
    ...
    constructor() {
        ...
        if (_success0 && _success1) {
            _initialize(
                string(abi.encodePacked("SyncSwap Classic LP")),
                string(abi.encodePacked(_symbol0, "/", _symbol1, "-C"))
            );
        } else {
            _initialize("SLP", "SLP");
        }
            ...
            if (_feeOn) {
                ...
                if (_invariant > _invariantLast) {
                    ...
                    if (_liquidity != 0) {
                        ...
                        try
                            IFeeRecipient(_feeRecipient).notifyFees(1, address(this), _liquidity, _protocolFee, abi.encode(2))
                        {} catch {}
    ...
    function _getAmountOut(
        ...
        if (_amountIn != 0) {
            uint _amountInWithFee = Math.mulUnsafeFirst(_amountIn, MAX_FEE - _swapFee);
            _feeIn = Math.mulDivUnsafeFirstLast(_amountIn, _swapFee, MAX_FEE);

            if (_token0In) {
                _dy = Math.mulDiv(_amountInWithFee, _reserve1, Math.mulUnsafeFirst(MAX_FEE, _reserve0) + _amountInWithFee);
            } else {
                _dy = Math.mulDiv(_amountInWithFee, _reserve0, Math.mulUnsafeFirst(MAX_FEE, _reserve1) + _amountInWithFee);
            }
        }
    }
...
// File contracts/pool/classic-delegate/SyncSwapClassicPoolDelegatedFactory.sol

// Original license: SPDX_License_Identifier: AGPL-3.0-or-later

pragma solidity ^0.8.0;
contract SyncSwapClassicPoolDelegatedFactory is BasePoolFactory {
    ...
    function _createPool(address token0, address token1) internal override returns (address pool) {
        ...
        pool = address(new SyncSwapClassicPoolDelegated{salt: salt}()); // this will prevent duplicated pools.
