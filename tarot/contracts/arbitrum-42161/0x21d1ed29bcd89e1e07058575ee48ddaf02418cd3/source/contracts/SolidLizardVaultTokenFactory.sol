...
import "./SolidLizardVaultToken.sol";
import "./interfaces/ISolidLizardVaultTokenFactory.sol";

contract SolidLizardVaultTokenFactory is Ownable, ISolidLizardVaultTokenFactory {
        ...
        returns (address vaultToken)
    {
        ...
        bytes memory bytecode = type(SolidLizardVaultToken).creationCode;
        assembly {
            vaultToken := create2(0, add(bytecode, 32), mload(bytecode), _underlying)
        }
        SolidLizardVaultToken(vaultToken)._initialize(
            _underlying,
            optiSwap,
            router,
            voter,
