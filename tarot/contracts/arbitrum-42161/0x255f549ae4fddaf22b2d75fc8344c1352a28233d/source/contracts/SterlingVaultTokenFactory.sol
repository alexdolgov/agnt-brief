...
import "./SterlingVaultToken.sol";
import "./interfaces/ISterlingVaultTokenFactory.sol";

contract SterlingVaultTokenFactory is Ownable, ISterlingVaultTokenFactory {
        ...
        returns (address vaultToken)
    {
        ...
        bytes memory bytecode = type(SterlingVaultToken).creationCode;
        assembly {
            vaultToken := create2(0, add(bytecode, 32), mload(bytecode), _underlying)
        }
        SterlingVaultToken(vaultToken)._initialize(
            _underlying,
            optiSwap,
            router,
            voter,
