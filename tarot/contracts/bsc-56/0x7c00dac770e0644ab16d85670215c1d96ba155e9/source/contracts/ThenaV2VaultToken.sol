...
import "./interfaces/IThenaV2VaultToken.sol";
...
contract ThenaV2VaultToken is IThenaV2VaultToken, PoolToken {
    ...
    uint256 public REINVEST_BOUNTY = 0.03e18;
    ...
    function _update() internal {
        uint256 _totalBalance = IThenaGaugeV2(gauge).balanceOf(address(this));
    ...
    function mint(address minter) external nonReentrant update returns (uint256 mintTokens) {
        ...
        uint256 _totalBalanceBefore = IThenaGaugeV2(gauge).balanceOf(address(this));
        IThenaGaugeV2(gauge).deposit(mintAmount);
        uint256 _totalBalanceAfter = IThenaGaugeV2(gauge).balanceOf(address(this));
    ...
    function redeem(address redeemer) external nonReentrant update returns (uint256 redeemAmount) {
        ...
        if (IThenaGaugeV2(gauge).emergency()) {
            IThenaGaugeV2(gauge).emergencyWithdrawAmount(redeemAmount);
        } else {
            IThenaGaugeV2(gauge).withdraw(redeemAmount);
        }
    ...
    function reinvest() external nonReentrant update {
        ...
        IThenaGaugeV2(gauge).deposit(liquidity);
