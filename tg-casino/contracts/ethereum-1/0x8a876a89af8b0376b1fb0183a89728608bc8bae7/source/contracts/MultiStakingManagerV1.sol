...
contract MultiStakingManagerV1 is
    Initializable,
    OwnableUpgradeable,
    PausableUpgradeable,
    ReentrancyGuardUpgradeable
{
    ...
    function initialize(
        address _stakeToken,
        uint256[][4] calldata _poolConfig,
        bool[] calldata _poolUpdateStakedTime
    ) public initializer {
        __Ownable_init_unchained();
        __Pausable_init_unchained();
        __ReentrancyGuard_init_unchained();
        stakeToken = IERC20Upgradeable(_stakeToken);
        for (uint256 i; i < _poolConfig[0].length; i++) {
            _createPool(
                _poolConfig[0][i],
                _poolConfig[1][i],
                _poolConfig[2][i],
                _poolConfig[3][i],
                _poolUpdateStakedTime[i]
            );
        }
        _pause();
    }
