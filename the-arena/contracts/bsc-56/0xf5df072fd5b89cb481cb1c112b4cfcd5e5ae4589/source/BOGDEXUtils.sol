...
contract BOGDEXUtils is BOGAuth, IBOGDEXUtils {
    ...
    function sortTokens(address tokenA, address tokenB) public pure override returns (address token0, address token1) {
        require(tokenA != tokenB, 'BOGDEXUtils: IDENTICAL_ADDRESSES');
        (token0, token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), 'BOGDEXUtils: ZERO_ADDRESS');
    }
    ...
    function getAmountOut(uint256 dexID, uint amountIn, uint reserveIn, uint reserveOut) public view override returns (uint amountOut) {
        require(amountIn > 0, 'BOGDEXUtils: INSUFFICIENT_INPUT_AMOUNT');
        require(reserveIn > 0 && reserveOut > 0, 'BOGDEXUtils: INSUFFICIENT_LIQUIDITY');
    ...
    function getAmountIn(uint256 dexID, uint amountOut, uint reserveIn, uint reserveOut) public view override returns (uint amountIn) {
        require(amountOut > 0, 'BOGDEXUtils: INSUFFICIENT_OUTPUT_AMOUNT');
        require(reserveIn > 0 && reserveOut > 0, 'BOGDEXUtils: INSUFFICIENT_LIQUIDITY');
    ...
    function getAmountsOut(uint amountIn, address[] memory path, uint256[] memory dexPath) public view override returns (uint[] memory amounts) {
        require(path.length >= 2 && dexPath.length == path.length-1, 'BOGDEXUtils: INVALID_PATH');
