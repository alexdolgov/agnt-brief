...
/// @title Provides quotes for swaps, identical to the standard v3-periphery Quoter except getPool directly from the pool factory
/// @notice Allows getting the expected amount out or amount in for a given swap without executing the swap
/// @dev These functions are not gas efficient and should _not_ be called on chain. Instead, optimistically execute
/// the swap and check the amounts in the callback.
contract QuoterV1 is IQuoter, IUniswapV3SwapCallback, PeripheryImmutableState {
    ...
    function getPool(address tokenA, address tokenB, uint24 fee) private view returns (IUniswapV3Pool pool) {
        address poolAddress = IUniswapV3Factory(factory).getPool(tokenA, tokenB, fee);
        require(poolAddress != address(0), "NULL_A");
