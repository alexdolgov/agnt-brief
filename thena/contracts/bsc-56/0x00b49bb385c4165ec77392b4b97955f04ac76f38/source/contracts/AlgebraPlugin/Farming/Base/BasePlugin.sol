...
abstract contract BasePlugin is IThenaBasePlugin, Timestamp {
    ...
    /// @notice Pause state of the plugin
    bool public paused;
    ...
    /// @inheritdoc IThenaBasePlugin
    function handlePluginFee(uint256, uint256) external view override returns (bytes4) {
        return IThenaBasePlugin.handlePluginFee.selector;
    }

    /// @inheritdoc IThenaBasePlugin
    function beforeInitialize(address, uint160) external virtual override returns (bytes4) {
    ...
    /// @inheritdoc IThenaBasePlugin
    function beforeModifyPosition(address, address, int24, int24, int128, bytes calldata) external virtual override returns (bytes4, uint24) {
        return (IThenaBasePlugin.beforeModifyPosition.selector, 0);
    }

    /// @inheritdoc IThenaBasePlugin
    function afterModifyPosition(address, address, int24, int24, int128, uint256, uint256, bytes calldata) external virtual override returns (bytes4) {
        return IThenaBasePlugin.afterModifyPosition.selector;
    }

    /// @inheritdoc IThenaBasePlugin
    function beforeSwap(address, address, bool, int256, uint160, bool, bytes calldata) external virtual override returns (bytes4, uint24, uint24) {
        return (IThenaBasePlugin.beforeSwap.selector, 0, 0);
    }

    /// @inheritdoc IThenaBasePlugin
    function afterSwap(address, address, bool, int256, uint160, int256, int256, bytes calldata) external virtual override returns (bytes4) {
        return IThenaBasePlugin.afterSwap.selector;
    }

    /// @inheritdoc IThenaBasePlugin
    function beforeFlash(address, address, uint256, uint256, bytes calldata) external virtual override returns (bytes4) {
        return IThenaBasePlugin.beforeFlash.selector;
    }

    /// @inheritdoc IThenaBasePlugin
    function afterFlash(address, address, uint256, uint256, uint256, uint256, bytes calldata) external virtual override returns (bytes4) {
    ...
    /// @notice Check if the plugin is paused
    /// @dev revert if the plugin is paused
    function _whenNotPaused() internal view {
        if(paused) revert PluginIsPaused();    
    }
