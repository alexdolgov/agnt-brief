...
contract FarmingCenter is IFarmingCenter, IPositionFollower, Multicall {
    ...
    /// @dev Combine collectRewards() and claimReward() in 1 call
    function collectAndClaimRewards(address to, IncentiveKey calldata key, uint256 tokenId) external isApprovedOrOwner(tokenId) returns(uint256 reward, uint256 bonusReward) {
        (reward, bonusReward) = eternalFarming.collectRewards(key, tokenId, nonfungiblePositionManager.ownerOf(tokenId));
        if(reward > 0) eternalFarming.claimRewardFrom(key.rewardToken, msg.sender, to, reward);
        if(bonusReward > 0) eternalFarming.claimRewardFrom(key.bonusRewardToken, msg.sender, to, bonusReward);
    }
