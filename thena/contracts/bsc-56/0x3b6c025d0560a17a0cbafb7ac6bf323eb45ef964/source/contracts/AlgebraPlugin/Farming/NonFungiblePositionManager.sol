...
import '@cryptoalgebra/integral-periphery/contracts/interfaces/INonfungibleTokenPositionDescriptor.sol';
...
import '@cryptoalgebra/integral-periphery/contracts/base/ERC721Permit.sol';
...
import '@cryptoalgebra/integral-periphery/contracts/base/PoolInitializer.sol';
    ...
    SelfPermit
{
    ...
    /// @dev verify the pool exists and get the pool Id
    function _verifyPoolAndGetPoolId(address _poolToCheck) internal returns(uint80 _poolId) {
        // compute address with input pool and make sure sender is plugin
        PoolAddress.PoolKey memory key = PoolAddress.PoolKey({deployer: address(0), token0: IAlgebraPool(_poolToCheck).token0(), token1: IAlgebraPool(_poolToCheck).token1()});
        address _pool = PoolAddress.computeAddress(poolDeployer,key);
        require(_pool == _poolToCheck, 'pool mismatch');
        _poolId = _cachePoolKey(address(_pool),key);
    }
