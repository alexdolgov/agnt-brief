...
abstract contract AlgebraRouter is IAlgebraRouter,PeripheryImmutableState,PeripheryPaymentsWithFee,SelfPermit {
    ...
    function exactInputSingle(ExactInputSingleParams calldata params) external payable override returns (uint256 amountOut) {
        amountOut = exactInputInternal(
            params.amountIn == Constants.CONTRACT_BALANCE ? IERC20(params.tokenIn).balanceOf(address(this)) : params.amountIn ,
            params.recipient,
    ...
    function exactInput(ExactInputParams memory params) external payable override returns (uint256 amountOut) {
        ...
        if(params.amountIn == Constants.CONTRACT_BALANCE){
            ...
            params.amountIn = IERC20(tokenA).balanceOf(address(this));
        }
    ...
    function exactOutputInternal(uint256 amountOut,address recipient,uint160 limitSqrtPrice,SwapCallbackData memory data) private returns (uint256 amountIn) {
        if (recipient == address(0)) recipient = address(this); // allow swapping to the router address with address 0
    ...
    /// @inheritdoc IAlgebraRouter
    function exactOutputSingle(ExactOutputSingleParams calldata params) external payable override returns (uint256 amountIn, uint256 extra_in) {
        ...
        extra_in = params.amountInMaximum - amountIn;
    }

    /// @inheritdoc IAlgebraRouter
    function exactOutput(ExactOutputParams calldata params) external payable override returns (uint256 amountIn, uint256 extra_in, address tokenOut) {
        ...
        // since this address has amountInMax, we need to send back any remaining token
        while (true) {
