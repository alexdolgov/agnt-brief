...
import "./interfaces/IThroneV3Pool.sol";
...
contract MasterChefV3 is INonfungiblePositionManagerStruct, Multicall, Ownable, ReentrancyGuard, Enumerable {
    ...
    /// @notice Address of THRONE contract.
    IThrone public immutable THRONE;
    ...
    /// @notice Address of the ThroneVault.
    ThroneVault public throneVault;

    uint256 public thronePerSecond;
    ...
    event AddPool(uint256 indexed pid, uint256 allocPoint, IThroneV3Pool indexed v3Pool, ILMPool indexed lmPool);
    ...
    event SetThronePerBlock(uint256 prevThronePerSecond, uint256 thronePerSecond);
    ...
    /// @param _THRONE The THRONE token contract address.
    /// @param _nonfungiblePositionManager the NFT position manager contract address.
    constructor(address _THRONE, INonfungiblePositionManager _nonfungiblePositionManager, address _WETH) {
        THRONE = IThrone(_THRONE);
        nonfungiblePositionManager = _nonfungiblePositionManager;
        WETH = _WETH;
        throneVault = new ThroneVault(_THRONE);
    }

    /// @notice Returns the throne per second.
    /// @param _pid The pool pid.
    /// @return poolThronePerSecond Throne reward per second.
    function getThronePerSecondByPid(uint256 _pid) public view returns (uint256 poolThronePerSecond) {
        if (totalAllocPoint > 0) {
            poolThronePerSecond = (thronePerSecond * poolInfo[_pid].allocPoint) / totalAllocPoint;
        }
    }

    /// @notice Returns the throne per second. This is for liquidity mining pool.
    /// @param _v3Pool Address of the V3 pool.
    /// @return poolThronePerSecond Throne reward per second.
    function getThronePerSecond(address _v3Pool) public view returns (uint256 poolThronePerSecond) {
        if (totalAllocPoint > 0) {
            poolThronePerSecond = (thronePerSecond * poolInfo[v3PoolAddressPid[_v3Pool]].allocPoint) / totalAllocPoint;
        }
    }

    /// @notice View function for checking pending THRONE rewards.
    /// @dev The pending throne amount is throned on the last state in LMPool. The actual amount will happen whenever liquidity changes or harvest.
    /// @param _tokenId Token Id of NFT.
    /// @return reward Pending reward.
    function pendingThrone(uint256 _tokenId) external view returns (uint256 reward) {
    ...
    function setThronePerSecond(uint256 _thronePerSecond) external onlyOwner {
        massUpdatePools();
        uint256 prevThronePerSecond = thronePerSecond;
        thronePerSecond = _thronePerSecond;
        emit SetThronePerBlock(prevThronePerSecond, thronePerSecond);
    }
    ...
    function add(uint256 _allocPoint, IThroneV3Pool _v3Pool, bool _withUpdate) external onlyOwner {
    ...
    /// @notice Update the given pool's THRONE allocation point. Can only be called by the owner.
    ...
    function onERC721Received(
        ...
        if (msg.sender != address(nonfungiblePositionManager)) revert NotThroneNFT();
    ...
    /// @notice harvest throne from pool.
    /// @param _tokenId Token Id of NFT.
    /// @param _to Address to.
    /// @return reward Throne reward.
    ...
    function harvestOperation(
        ...
        if (reward > 0) {
            if (_to != address(0)) {
                ...
                _safeTransferThroneFromVault(_to, reward);
    ...
    /// @notice Update throne reward for all the liquidity mining pool.
    ...
    /// @notice Update throne reward for the liquidity mining pool.
    ...
    function updatePool(uint256 _pid) internal {
        ...
        if (pool.allocPoint != 0 && address(LMPool) != address(0)) {
            ...
            if (currentTimeStamp > lastRewardTimestamp) {
                ...
                uint256 throneReward = (multiplier * thronePerSecond) * (pool.allocPoint / totalAllocPoint);
                THRONE.mint(address(throneVault), throneReward);
    ...
    /// @notice Safe Transfer THRONE.
    /// @param _to The THRONE receiver address.
    /// @param _amount Transfer THRONE amounts.
    function _safeTransferThroneFromVault(address _to, uint256 _amount) internal {
        if (_amount > 0) {
            uint256 balance = THRONE.balanceOf(address(throneVault));
            if (balance < _amount) {
                _amount = balance;
            }

            throneVault.safeTransferThrone(_to, _amount);
        }
    }
