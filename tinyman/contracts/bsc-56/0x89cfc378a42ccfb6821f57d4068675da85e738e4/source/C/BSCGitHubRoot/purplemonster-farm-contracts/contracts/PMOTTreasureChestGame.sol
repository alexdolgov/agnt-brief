...
contract PMOTTreasureChestGame is Ownable, ReentrancyGuard, IERC721Receiver  {
    using SafeMath for uint256;
    using Address for address;

    IERC20 _purpleMonsterToken;
    IPurpleMonNFT _purpleMonsterNFT;
    ...
    struct GameResult
    {
        ...
        uint256 prizeAmount;
        uint256 prizeIsNft;
        uint256 rarityLevelPlayed;
    }

    //mapping player to Game Number to Game Results
    mapping(address => mapping (uint256 => GameResult)) public playerHistory;
    ...
    struct PendingPrizes{
        uint256 tokenCount;
        uint256 commonNftCount;
        uint256 rareNftCount;
        uint256 ultraRareNftCount;
    }
    ...
    event Claim(uint256 tokenCount, uint256[] tokenIds);
   ...
   function playGame(uint256 _rarity, uint256 _numOfPlays, string[] memory data) public notContract nonReentrant  {
       require(_numOfPlays < 11 && _numOfPlays > 0, "Min 1, Max 10 plays at once");
       require(_rarity >=0 && _rarity <= 2, "Invalid Rarity Provided");
       require(_numOfPlays == data.length, "num of plays does not match data length");
       
       uint256 gamePlayCost = getTotalCost(_rarity, _numOfPlays);
       ...
       _purpleMonsterToken.transferFrom(msg.sender, burnAddress, burnAmount);
       _purpleMonsterToken.transferFrom(msg.sender, address(this), gamePlayCost.sub(burnAmount));
       
       totalBurnedStat = totalBurnedStat.add(burnAmount);

       GameResult[] memory _gameResults = new GameResult[](_numOfPlays);
       uint256 costPerPlay = gamePlayCost.div(_numOfPlays);
       uint256[] storage playersGames = playersGameNumberList[msg.sender];

       for(uint i = 0; i< _numOfPlays; i++){
           
           uint256 playResult = _generateRandomNumber(msg.sender, data[i]);                   
           ...
           /*
                 ...
                 Tokens:    2x ~10%
                            ...
                            jackpot ~0.01%
           */
           playersGames.push(gameNumber);

           PendingPrizes storage pendingPrizes = playerPendingPrizes[msg.sender];
           GameResult storage gameResult = gameHistory[gameNumber];
           ...
           if(playResult == 0){
               ...
               gameResult.prizeIsNft = 1;
               if(_rarity == 0){
                    pendingPrizes.commonNftCount = pendingPrizes.commonNftCount.add(1);
               }
               else if(_rarity == 1){
                    pendingPrizes.rareNftCount = pendingPrizes.rareNftCount.add(1);
               }else if (_rarity == 2){
                    pendingPrizes.ultraRareNftCount = pendingPrizes.ultraRareNftCount.add(1);
               }
               ...
               uint256 jPotPrize =  jackpot();                     
               gameResult.prizeAmount = jPotPrize;               
           }
            
            if(gameResult.prizeIsNft == 0){                
           ...
           _gameResults[i] = gameResult; 

            playerHistory[msg.sender][gameNumber] = gameResult;
            
            gameNumber = gameNumber + 1;         
           
            
       } 
       emit AllGamesPlayed(_rarity, _numOfPlays,  _gameResults);      
   }
    ...
    function claim() public notContract nonReentrant {
            ...
            PendingPrizes storage pendingPrizesForPlayer =  playerPendingPrizes[msg.sender];
             if(pendingPrizesForPlayer.tokenCount > 0){
                 _purpleMonsterToken.transfer(msg.sender, pendingPrizesForPlayer.tokenCount);
            ...
            emit Claim(wonTokenCount, wonTokenIds);
    }

    function jackpot() public view returns (uint256) {
        return _purpleMonsterToken.balanceOf(address(this)).sub(pendingTokenPrizesGlobal).mul(750).div(percentageBase);
    }

    function getTotalCost(uint256 rarity, uint256 numOfPlays) public view returns (uint256){
        uint256 cost = 0;
        if(rarity == 0){
    ...
    function setBurnRate(uint256 _newBurnRate) public onlyOwner {
        burnPercentage = _newBurnRate.div(percentageBase);
   ...
   function _generateRandomNumber(address sender, string memory seed) private view returns (uint256) {        
        uint256 convertedAddress = uint256(uint160(address(sender)));
        uint256 randomHash = uint256(keccak256(abi.encodePacked(blockhash(block.number - 1), block.timestamp + convertedAddress, seed))).mod(
                            1000
                        );                    
        return randomHash;
    }
