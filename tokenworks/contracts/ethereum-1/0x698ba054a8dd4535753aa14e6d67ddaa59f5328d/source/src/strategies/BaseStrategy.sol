    ...
    ERC20
{
    ...
    function _afterTokenTransfer(
        ...
        // Allow whitelisted distributors to send tokens freely on mainnet
        if (block.chainid == 1) {
            if (
                IGlobalDistributor(GLOBAL_DISTRIBUTION_HANDLER)
                    .isGlobalDistributor(from) || isDistributor[from]
            ) {
                return;
            }
        }
