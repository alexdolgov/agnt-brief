...
contract RipeStrategy is RecursiveStrategy {
    ...
    /// @notice Sets the TWAP increment amount
    /// @dev Can only be called by the owner
    /// @param _twapIncrement The new TWAP increment amount in wei
    function setTwapIncrement(uint256 _twapIncrement) external onlyOwner {
    ...
    /// @notice Allows anyone to manually add fees to the contract
    function addFeesManual() external payable nonReentrant {
