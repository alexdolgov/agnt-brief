...
interface IDynamicFeeModule is ICustomFeeModule {
    ...
    event DynamicFeeConfigured(address indexed pool, uint24 indexed feeCap, uint64 indexed scalingFactor);
    ...
    /// @notice Sets fee cap and scaling factor for multiple pools
    /// @dev Must be called by the current fee manager
    /// @dev Each pool must exist
    /// @dev A zero scaling factor uses module defaults, matching getFee behavior
    /// @param _pools The pool addresses to configure
    /// @param _feeCaps The fee caps to set for each pool
    /// @param _scalingFactors The scaling factors to set for each pool
    function configureDynamicFees(
        address[] calldata _pools,
        uint24[] calldata _feeCaps,
