...
contract CLPool is ICLPool {
    ...
    function collectFees() external override lock onlyGauge returns (uint128 amount0, uint128 amount1) {
        // Returned amounts must equal what is actually transferred so the gauge's
        // fees0/fees1 accumulators stay in lockstep with the gauge's token balance.
        // We leave 1 wei in each slot to keep it warm (avoids the SSTORE-cleared refund quirk).
        uint128 pending0 = gaugeFees.token0;
        uint128 pending1 = gaugeFees.token1;
        if (pending0 > 1) {
            amount0 = pending0 - 1;
            gaugeFees.token0 = 1;
