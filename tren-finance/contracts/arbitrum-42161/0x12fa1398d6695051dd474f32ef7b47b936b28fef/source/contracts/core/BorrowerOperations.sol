    ...
    IBorrowerOperations
{
        ...
        internal
    {
        ...
        // When the adjustment is a debt repayment, check it's a valid amount, that the caller
        // has enough debt tokens and meets minimum net debt amount
    ...
    function _closeTrenBox(address _asset, address _borrower) internal {
        ...
        address recipient = _borrower;
        if (msg.sender == flashLoanAddress) {
            recipient = flashLoanAddress;
        }
        if (IAdminContract(adminContract).compoundedCollaterals(_asset)) {
            uint256 safetyTransferColl = SafetyTransfer.decimalsCorrection(_asset, coll);
            ISwapManager(swapManager).unstake(_asset, safetyTransferColl);
            uint256 diff = ITrenBoxManager(trenBoxManager).getTrenBoxColl(_asset, _borrower) - coll;
            uint256 safetyTransferDiff = SafetyTransfer.decimalsCorrection(_asset, diff);
            ISwapManager(swapManager).unstake(_asset, safetyTransferDiff);

            uint256 totalToSend = coll + diff;
            bytes memory data = abi.encode(_asset, totalToSend);
            ITrenBoxStorage(trenBoxStorage).update(data);

            // Not to get stack too deep
            address asset = _asset;
            IERC20(asset).safeTransfer(
                recipient, SafetyTransfer.decimalsCorrection(asset, totalToSend)
            );
        } else {
            ITrenBoxStorage(trenBoxStorage).sendCollateral(_asset, recipient, coll);
        }
    ...
    function _trenBoxStorageAddColl(address _asset, uint256 _amount) internal {
        uint256 amount = SafetyTransfer.decimalsCorrection(_asset, _amount);
        if (IAdminContract(adminContract).compoundedCollaterals(_asset)) {
            IERC20(_asset).safeTransferFrom(msg.sender, swapManager, amount);
            ISwapManager(swapManager).stake(_asset, amount);
        } else {
            IERC20(_asset).safeTransferFrom(msg.sender, trenBoxStorage, amount);
        }
