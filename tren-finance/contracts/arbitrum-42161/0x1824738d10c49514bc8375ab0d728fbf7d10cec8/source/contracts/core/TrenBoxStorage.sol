    ...
    ConfigurableAddresses
{
    ...
    modifier onlyBorrowerOperationsOrSwapManager() {
        if (msg.sender != borrowerOperations && msg.sender != swapManager) {
            revert TrenBoxStorage__BorrowerOperationsOrSwapManagerOnly();
        }
        _;
    }
    ...
    function increaseActiveCollateral(
        ...
        external
        override
        onlyBorrowerOperationsOrSwapManager
    {
    ...
    function update(bytes memory data) external override {
        if (msg.sender != borrowerOperations) {
            revert TrenBoxStorage__BorrowerOperationsOnly();
        }

        (address _collateral, uint256 _amount) = abi.decode(data, (address, uint256));
        _updateActiveCollateral(_collateral, _amount, false);
    }
        ...
        onlyAuthorizedProtocolContracts
    {
        ...
        if (IAdminContract(adminContract).compoundedCollaterals(_collateral)) {
            ISwapManager(swapManager).unstake(_collateral, safetyTransferAmount);
        }
