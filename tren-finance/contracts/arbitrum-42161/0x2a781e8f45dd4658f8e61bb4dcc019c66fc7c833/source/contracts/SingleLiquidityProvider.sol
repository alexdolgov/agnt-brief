...
/// liquidity in Uniswap pool via Gamma private Vault
    ...
    ERC20
{
    ...
    /// @notice The address of the private vault
     ...
     * @param _vault The address of Gamma private vault
    ...
    /// @dev Disabled renounceOwnership. Not intended to be callable in this contract.
    ...
    function withdrawRequest(uint256 amount) external nonReentrant {
        ...
        requests[requestId] =
            Request({ amountToWithdraw: correctedAmount, timestamp: block.timestamp + 600 });
    ...
    function withdraw(uint256 requestId) external nonReentrant {
        ...
        if (block.timestamp < withdrawReq.timestamp) revert LockedTimeNotPassed();
    ...
    function claimRewards() external nonReentrant {
        if (address(trenToken) == address(0)) revert TrenTokenNotSetYet();
    ...
    function _removeLiquidity(uint256 amount, uint256 shares) private {
        ...
        if (stableAmount > amount1Out) {
            ...
            IERC20Metadata(debtToken).safeTransfer(address(this), correctedDiff);
