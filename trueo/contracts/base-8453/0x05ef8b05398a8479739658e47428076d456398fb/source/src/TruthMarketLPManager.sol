    ...
    function removeMarketLiquidity(address market) external onlyRole(Roles.OPERATOR_ROLE) {
        ...
        for (uint256 i = tokenIds.length; i > 0; i--) {
            ...
            if (deposits[tokenId].liquidity > 0) {
                ...
                // Get token addresses
                address yesToken = ITruthMarket(market).yesToken();
                address noToken = ITruthMarket(market).noToken();

                if (deposits[tokenId].token0 == yesToken) {
                    totalYesAmount += amount0;
                } else if (deposits[tokenId].token0 == noToken) {
                    totalNoAmount += amount0;
                }

                if (deposits[tokenId].token1 == yesToken) {
                    totalYesAmount += amount1;
                } else if (deposits[tokenId].token1 == noToken) {
                    totalNoAmount += amount1;
                }
            }
        }
        delete marketTokenIds[market];
        
        // Try to redeem tokens if market is finalized
        ...
        if (status == MarketStatus.Finalized) {
            ...
            if (winningPosition == 1) { // YES won
                IERC20(truthMarket.yesToken()).approve(market, totalYesAmount);
                truthMarket.redeem(totalYesAmount);
            } else if (winningPosition == 2) { // NO won
                IERC20(truthMarket.noToken()).approve(market, totalNoAmount);
                truthMarket.redeem(totalNoAmount);
            } else if (winningPosition == 3) { // CANCELED
                IERC20(truthMarket.yesToken()).approve(market, type(uint256).max);
                IERC20(truthMarket.noToken()).approve(market, type(uint256).max);
                truthMarket.withdrawFromCanceledMarket();
            }
        }
        
        emit MarketLiquidityRemoved(market, totalYesAmount, totalNoAmount);
    }
