...
library LRound {
    struct TicketStorage {
        ...
        mapping(uint8 => uint256) _categoryClaimedTicketsNumber;
        mapping(address => bool) _userClaimed;
        mapping(address => mapping(uint8 => uint256[])) _winnerCategoryTickets;
        mapping(address => mapping(uint8 => bool)) _ticketsClaimed;
    }
    ...
    function getAdminAllowToClaimTickets() public pure returns (uint256) {
        return 0xEFF100;
    }
    ...
    function ifNotClaimedAndClosed(LUtil.RoundStatus status) public pure {
        require(
            status != LUtil.RoundStatus.CLAIMED_AND_CLOSED,
            "ROUND: not claimed and closed"
        );
    }

    function isClosed(LUtil.RoundStatus status) public pure {
        require(status == LUtil.RoundStatus.CLOSED, "ROUND: not funded");
    }
    ...
    function _approveWinnersPay(
        ...
        if (tickets._ticketsPool[uint8(winnerCategory) + 1].length > 0) {
            if (winnerCategory == LUtil.WinnerCategory.JACKPOT) {
                lottery.resetJackpot(roundIndex);
                if (
        ...
        for (uint256 index = startIndex; index < stopIndex + 1; index++) {
            ...
            if (coincidenceCount > 1) {
                tickets
                    ...
                    .push(ticketKey);
                tickets
                ._winnerCategoryTickets[tickets._ticketOwners[ticketKey]][
                    LLottery.getTicketLength() + 1 - coincidenceCount
                ].push(ticketKey);
    ...
    function _isCategoryClaimed(
        TicketStorage storage tickets,
        LUtil.WinnerCategory category
    ) private view returns (bool) {
        return
            getCategoryTicketsClaimed(tickets, category) ==
            getCategoryTicketsCount(tickets, category);
    }

    function isClaimed(TicketStorage storage tickets)
        public
        view
        returns (bool)
    {
        for (uint256 i = 0; i < getPoolLength() - 1; i++) {
            if (!_isCategoryClaimed(tickets, LUtil.WinnerCategory(i)))
                return false;
        }
        return true;
    }

    function getCategoryTicketsClaimed(
        TicketStorage storage tickets,
        LUtil.WinnerCategory category
    ) public view returns (uint256) {
        return tickets._categoryClaimedTicketsNumber[uint8(category)];
    }

    function isCategoryClaimedByUser(
        TicketStorage storage tickets,
        address user,
        LUtil.WinnerCategory category
    ) public view returns (bool) {
        return tickets._ticketsClaimed[user][uint8(category) + 1];
    }

    function isUserClaimed(TicketStorage storage tickets, address user)
        external
        view
        returns (bool)
    {
        return tickets._userClaimed[user];
    }

    function userHasWinningTickets(TicketStorage storage tickets, address user)
        public
        view
        returns (bool)
    {
        for (uint8 i = 0; i < getPoolLength() - 1; i++) {
            if (tickets._winnerCategoryTickets[user][i + 1].length > 0)
                return true;
        }
        return false;
    }
    ...
    function getUserClaimableAmount(
        ...
        for (uint8 i = 0; i < getPoolLength() - 1; i++) {
            if (getCategoryTicketsCount(tickets, LUtil.WinnerCategory(i)) > 0) {
                claimAmount +=
                    (wallets[i].amount / tickets._ticketsPool[i + 1].length) *
                    tickets._winnerCategoryTickets[user][i + 1].length;
            }
        }
    }

    function _claimByCategory(
        TicketStorage storage tickets,
        LUtil.PrizeWallet[] storage wallets,
        LUtil.WinnerCategory category,
        address user,
        ...
        if (_isCategoryClaimed(tickets, category))
            tickets._ticketsClaimed[user][uint8(category) + 1] = true;
        if (
            getCategoryTicketsCount(tickets, category) > 0 &&
            !_isCategoryClaimed(tickets, category) &&
            !isCategoryClaimedByUser(tickets, user, category)
        ) {
            tickets._ticketsClaimed[user][uint8(category) + 1] = true;
            if (
                tickets
                ._winnerCategoryTickets[user][uint8(category) + 1].length > 0
            ) {
                uint256[] memory ticketKeys = tickets._winnerCategoryTickets[
                    user
                ][uint8(category) + 1];
                uint256 claimAmount = (wallets[uint8(category)].amount /
                    tickets._ticketsPool[uint8(category) + 1].length) *
                    ticketKeys.length;
                tickets._categoryClaimedTicketsNumber[
                    uint8(category)
                ] += ticketKeys.length;
                IRound(roundAddress).updateCategoryBalance(
                    category,
                    claimAmount
                );
                ...
                IEventEmitter(eventEmitterAddress).emitClaim(
                    roundAddress,
                    category,
                    user,
                    claimAmount,
    ...
    function claim(
        ...
        require(!tickets._userClaimed[user], "already claimed");
        tickets._userClaimed[user] = true;
        for (uint256 i = 0; i < getPoolLength() - 1; i++) {
            _claimByCategory(
                tickets,
                wallets,
                LUtil.WinnerCategory(i),
                user,
                lotteryAddress,
                roundAddress,
                eventEmitterAddress
    ...
    function _claimByAdminByCategory(
        TicketStorage storage tickets,
        LUtil.PrizeWallet[] storage wallets,
        LUtil.WinnerCategory category,
        address lotteryAddress,
        ...
        if (!_isCategoryClaimed(tickets, category)) {
            uint256 amount = wallets[uint8(category)].amount;
            if (LUtil.WinnerCategory.JACKPOT == category) {
                uint256 currentAmount = _getToken(lotteryAddress).balanceOf(
                    wallets[uint8(category)].wallet
                );
                if (amount > currentAmount) {
                    amount -= _getLottery(lotteryAddress)
                        .getJackpotRequireMin();
                }
            }
            _safeTransferFrom(
                lotteryAddress,
            ...
            IEventEmitter(eventEmitterAddress).emitClaimByAdmin(
                roundAddress,
                category,
                amount
            );
