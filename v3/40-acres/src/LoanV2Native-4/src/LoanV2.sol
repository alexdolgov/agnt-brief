    ...
    LoanStorage
{
     ...
     */
    function increaseLoan(uint256 tokenId, uint256 amount) public {
        require(amount > .01e6);
        require(_ve.ownerOf(tokenId) == address(this));
    ...
    function _handleZeroBalance(
        ...
        if (
            loan.zeroBalanceOption == ZeroBalanceOption.InvestToVault ||
            wasActiveLoan
        ) {
            remaining -= _payZeroBalanceFee(
                loan.borrower,
                tokenId,
                remaining,
                totalRewards,
        ...
        IERC20 asset = loan.preferredToken == address(0)
            ? _asset
            : IERC20(loan.preferredToken);
        remaining -= _payZeroBalanceFee(
            loan.borrower,
            tokenId,
            remaining,
        ...
        if (tokenId == getManagedNft()) {
            ICommunityRewards(loan.borrower).notifyRewardAmount(
                address(asset),
                remaining
            );
        }
    ...
    function claim(
        ...
        if (
            loan.borrower == address(0) || _ve.ownerOf(tokenId) != address(this)
        ) {
            return 0;
        }
        ...
        if (
            loan.balance == 0 &&
            loan.zeroBalanceOption == ZeroBalanceOption.DoNothing
        ) {
            return 0;
        }
        ...
        if (rewardsAmount == 0 && loan.balance == 0) {
            rewardToken = loan.preferredToken == address(0)
                ? address(_asset)
                : loan.preferredToken;
        ...
        if (
            loan.balance == 0 &&
            (!userUsesPayoffToken(loan.borrower) ||
                getUserPayoffToken(loan.borrower) == 0)
        ) {
            _handleZeroBalanceClaim(
                loan,
                rewardsAmount,
                allocations[0],
                aeroAmount
    ...
    function _processRewards(
        ...
        for (uint256 i = 0; i < flattenedTokens.length; i++) {
            ...
            if (
                token.allowance(address(this), odosRouter()) < type(uint256).max
            ) {
     ...
     */
    function odosRouter() public pure virtual returns (address) {
    ...
    function _handleActiveLoanClaim(
        ...
        uint256 feeEligibleAmount = _calculateFeeEligibleAmount(
            loan,
            totalRewards
        );

        // Process fees
        remaining -= _processFees(
            loan,
            tokenId,
            totalRewards,
            feeEligibleAmount
     ...
     */
    function _calculateFeeEligibleAmount(
        LoanInfo storage loan,
        uint256 amount
        ...
        if (
            userUsesPayoffToken(loan.borrower) &&
            getUserPayoffToken(loan.borrower) != 0 &&
            getUserPayoffToken(loan.borrower) != loan.tokenId
        ) {
            uint256 payoffTokenLoanBalance = _loanDetails[
                getUserPayoffToken(loan.borrower)
            ].balance;
    ...
    function _increaseNft(
        ...
        uint256 tokenToIncrease = (userIncreasesManagedToken(loan.borrower) ||
            loan.optInCommunityRewards)
            ? managedNft
            : loan.tokenId;
     ...
     */
    function _recordDepositOnManagedNft(
        uint256 tokenId,
        uint256 amount,
        address owner
    ) internal {
        ...
        if (managedNftAddress != address(0)) {
            ICommunityRewards(managedNftAddress).deposit(
                tokenId,
                amount,
                owner
            );
    ...
    function getMaxLoan(
        ...
        return
            LoanUtils.getMaxLoanByRewardsRate(
                tokenId,
                address(_ve),
                getRewardsRate(),
                _multiplier,
     ...
     */
    function setManagedNft(uint256 tokenId) public override onlyOwner {
        LoanInfo storage loan = _loanDetails[tokenId];
        require(loan.borrower != address(0));
     ...
     */
    function setTopUp(uint256 tokenId, bool enable) public {
        LoanInfo storage loan = _loanDetails[tokenId];
        require(loan.borrower == msg.sender);
        loan.topUp = enable;
    }
     ...
     */
    function setPreferredToken(uint256 tokenId, address preferredToken) public {
        LoanInfo storage loan = _loanDetails[tokenId];
        require(loan.borrower == msg.sender);
        require(isApprovedToken(preferredToken));
    ...
    function _vote(
        ...
        bool isActive = ProtocolTimeLibrary.epochStart(loan.voteTimestamp) >
            ProtocolTimeLibrary.epochStart(block.timestamp) - 14 days;
    ...
    function _getLockedAmount(
        ...
        IVotingEscrow.LockedBalance memory lockedBalance = _ve.locked(tokenId);
        if (
            lockedBalance.end < ProtocolTimeLibrary.epochStart(block.timestamp)
        ) {
            return 0;
        }
     ...
     */
    function confirmUsdcPrice() internal view virtual returns (bool) {
        (
            ,
    ...
    function _withinVotingWindow() internal view returns (bool) {
        return
            block.timestamp >=
            ProtocolTimeLibrary.epochVoteEnd(block.timestamp) - 1 hours;
    }
