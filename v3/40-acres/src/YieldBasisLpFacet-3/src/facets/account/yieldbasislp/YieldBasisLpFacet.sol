...
contract YieldBasisLpFacet is AccessControl, ICollateralFacet {
    ...
    function deposit(uint256 amount) external onlyPortfolioManagerMulticall(_portfolioFactory) {
        ...
        // depositedAssetValue stored in underlying (e.g. BTC) units via LP pricePerShare()
        ERC4626CollateralManager.addCollateral(_config(), address(_lpToken), address(_lpToken), amount);
    ...
    function withdraw(uint256 amount) external onlyPortfolioManagerMulticall(_portfolioFactory) {
        ...
        if (sharesToRemove > 0) {
            ERC4626CollateralManager.removeCollateral(_config(), address(_lpToken), address(_lpToken), sharesToRemove);
        }
    ...
    function getTotalLockedCollateral() external view override returns (uint256) {
        return ERC4626CollateralManager.getTotalCollateralValue(address(_lpToken), address(_lpToken));
    }
    ...
    function getMaxLoan() external view override returns (uint256 maxLoan, uint256 maxLoanIgnoreSupply) {
        return ERC4626CollateralManager.getMaxLoan(_config(), address(_lpToken), address(_lpToken));
    }

    function enforceCollateralRequirements() external view override returns (bool success) {
        return ERC4626CollateralManager.enforceCollateralRequirements(_config(), address(_lpToken), address(_lpToken));
    }

    function getLTVRatio() external view override returns (uint256) {
        return ERC4626CollateralManager.getLTVRatio(_config(), address(_lpToken), address(_lpToken));
    }
}
