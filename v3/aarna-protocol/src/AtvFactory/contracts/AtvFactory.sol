  ...
  function createAToken(
    ...
    require(
      pooldata._underlyingTokensProportion.length == pooldata._pendleMarketPalace.length &&
        pooldata._pendleMarketPalace.length == pooldata._moonwell.length &&
        pooldata._moonwell.length == pooldata._aaveToken.length &&
    ...
    require(address(_rebalContract) != address(0), "AF: zero addr");
