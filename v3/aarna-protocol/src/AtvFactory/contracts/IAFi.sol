...
interface IAFi {
  ...
  function _supplyMoonwell(address tok, uint amount) external;

  function _supplyMorpho(address token, uint256 amount) external; 

  function _supplyAave(address tok, uint amount) external;

  function _withdrawAave(address tok, uint amount) external;

  function _withdrawMoonwell(address tok, uint amount) external;

  function _withdrawMorpho(address tok, uint amount) external;
  ...
  function isOTokenWhitelisted(address oToken) external view returns (bool, bool);
  ...
  function getHelperTokens() external view returns (address[] memory uTokensArray);
  function getIfLandingPoolSame() external view returns (bool);
}