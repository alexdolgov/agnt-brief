...
contract AtvWrappedBoosterTL is ERC4626, OwnableDelayModuleV2, Pausable{
    ...
    uint256 internal constant MAX_PLATFORM_FEE = 200;
    uint256 internal constant UNIT_NAV = 1e27;
    uint256 internal constant FEE_DIV = 10000;
   
    address internal platformWallet;
    uint256 internal depositFee; // Deposit fee in basis points (e.g., 100 = 1%)
    address internal controller;
    address internal atvStorage;
    address internal atvOracle;
    address private immutable UNDERLYING;

    uint256 internal deadlineDelay = 1 hours;
    ...
    event VaultMigrated(address from, address to);
    ...
    function updatePlatformWalletAndFee(
        address _platformWallet, 
        uint256 _withdrawalFee,
        uint256 _depositFee
    ) external onlyOwner {
        platformWallet = _platformWallet;
        _chkLte(_withdrawalFee, MAX_PLATFORM_FEE);
        _chkLte(_depositFee, MAX_PLATFORM_FEE); 
        platformFee = _withdrawalFee;
    ...
    function migrateVault(address _vault) external onlyOwner whenPaused {
        ...
        emit VaultMigrated(oldVault, _vault);
    }
    ...
    function updateatvStorageAndOracle(address _atvStorage, address _atvOracle, address _controller) external onlyOwner {
        atvStorage = _atvStorage; 
        atvOracle = _atvOracle;
    ...
    function adjustExtraVaultTokens(address to) external onlyOwner{
        ...
        _safeTransfer(address(ATV_VAULT), to, extra);
    }
    ...
    function withdraw(
        ...
        bytes[] memory pendleWithdrawData,
        uint slippageBasisPoints
        ...
        // Call helper function that handles everything
        return _processWithdrawal(shares, assets, receiver, pendleWithdrawData, slippageBasisPoints, minAmountOut, deadline, minimumReturnAmount);
    }
    ...
    function _processWithdrawal(
        ...
        if(pF > 0 && platformWallet != address(0)){
            ...
            _safeTransfer(UNDERLYING, platformWallet, pF);
        }

        _safeTransfer(UNDERLYING, receiver, vaultTokensReceived);    
    ...
    function _convertToShares(uint256 assets, Math.Rounding rounding) internal view virtual override returns (uint256) {
        ...
        assets = assets - ((assets * depositFee) / FEE_DIV); // Apply deposit fee
    ...
    function previewMint(uint256 shares) public view virtual override returns (uint256) {
        // Reverse the deposit fee calculation
        return (_convertToAssets(shares, Math.Rounding.Ceil) * FEE_DIV) / (FEE_DIV - depositFee);
    }
