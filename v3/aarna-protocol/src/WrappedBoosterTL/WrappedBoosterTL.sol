...
// File contracts/Ownable2StepV2.sol
...
// File contracts/OwnableDelayModuleV2.sol
...
// File contracts/WrappedBoosterTL.sol
...
contract WrappedBoosterTL is ERC4626, OwnableDelayModuleV2, Pausable {
    ...
    uint256 public platformFee; // 100 for 1%
    bool public isInternalTransfer;
    ...
    constructor(address token, ERC20 _underlyingToken)
    ...
    function checkBool(bool status) internal pure{
        require(!status, "WAtv02");
    }
    ...
    function freezeRewardsForAFiToken() external onlyOwner {
        checkBool(frozen[asset()].isFrozen);
        frozen[asset()].isFrozen = true;
        frozen[asset()].freezingDate = block.timestamp;

        emit AFiTokenFrozen(asset(), frozen[asset()].freezingDate);
    }

    function _stake(uint256 amount, address token, address owner) internal whenNotPaused{
        checkBool(stakePause);
        require(amount > 0, "WAtv04");
        checkBool(frozen[token].isFrozen);
        ...
        stakingDetails[owner][numStakes[owner]] = Stake(
            amount,
            block.timestamp,
            token,
            _stakeNAV,
        ...
        numStakes[owner]++;
        totalStakedByUser[owner] += amount;
        emit Staked(owner, token, amount, numStakes[owner]);
    }
    ...
    function deposit(uint256 assets, address receiver) public virtual override returns (uint256) {
        ...
        uint256 shares = previewDeposit(assets);
        _deposit(_msgSender(), receiver, assets, shares);
        _stake(assets, address(asset()), msg.sender);
        return shares;
    }

    function withdrawById(uint256 stakeId) public returns (uint256) {
        isInternalTransfer = true;
        uint256 assets = stakingDetails[msg.sender][stakeId].amount;
        ...
        uint256 shares = previewWithdraw(assets);
        _unstake(stakeId);
        shares -= (shares * platformFee) / 10000;
    ...
    function rewardTransfer(address to, uint256 amt) internal {
        rewardToken.safeTransfer(to, amt);
    }

    function _unstake(uint256 stakeIndex) internal {
        checkBool(!stakingDetails[msg.sender][stakeIndex].active);
        ...
        if (reward > 0) {
            rewardTransfer(msg.sender, reward);
    ...
    function accruedReward(address user, uint256 stakeIndex) public view returns (uint256, uint256, uint256, uint256) {
        ...
        if (durationIndex > 0) {
            ...
            reward = (((_yieldGenerated * durationFactors[durationIndex - 1]) * (10 ** 18)) / (100 * 10000 * (10 ** 18)));
        }
        if (durationIndex == 0) {
    ...
    function claimPendingReward(uint256 stakeId) external {
        ...
        checkBool(!pendingReward[stakeId][msg.sender]);
        require(EndNAV[userStake.token] > 0, "WAtv15");
        ...
        uint256 reward = (((_yieldGenerated * userStake.durationRate) * (10 ** 18)) / (100 * 10000 * (10 ** 18)));
        userStake.rewards = reward;
    ...
    // -------------------
    // Transfer override with conditional ID handling
    // -------------------
    function transfer(address to, uint256 shares) public override(ERC20, IERC20) returns (bool) {
        address owner = _msgSender();
        trasnferValidation(owner, to, shares);
        _stake(shares, address(asset()), to);
        _transfer(owner, to, shares);
        return true;
    }

    function trasnferValidation(address from, address to, uint256 value) internal {
        require(to != address(0), "WAtv16");
        uint256 remaining = value;
        uint256 stakeCount = numStakes[from];

        // Deduct from ID-based shares first
        if (stakeCount > 0) {
            if(!isInternalTransfer){

                for (uint256 i = 0; i <= stakeCount && remaining > 0; i++) {
                    uint256 balance = stakingDetails[from][i].amount;

                    if(stakingDetails[from][i].active && !stakingDetails[from][i].claimed){
                        if (balance >= remaining) {
                            uint256 stakeBalance = stakingDetails[from][i].amount;
                            stakeBalance -= remaining;
                            if (stakeBalance == 0) {
                                _unstake(i);
                                stakingDetails[from][i].active = false;
                            }else{
                                stakingDetails[from][i].amount -= remaining;
                                (
                                    uint256 reward,
                                    ,
                                    uint256 uNAV,
                                    uint256 rate
                                ) = accruedReward(from, i);
      
                                totalStakedByUser[from] -= remaining;
                                if(reward > 0){
                                    uint256 _yieldGenerated = ((uNAV - stakingDetails[from][i].stakeNAV) * value);
                                    uint256 miniReward = (((_yieldGenerated * rate) * (10 ** 18)) / (100 * 10000 * (10 ** 18)));
                                    if (miniReward > 0) {
                                        rewardTransfer(from, miniReward);
                                        totalRewardsDistributed += miniReward;
                                    }
                                }
                            }
                            remaining = 0;
    ...
    function transferFrom(address from, address to, uint256 value) public virtual override(ERC20, IERC20) returns (bool) {
        trasnferValidation(from, to, value);
        _stake(value, address(asset()), to);
        address spender = _msgSender();
        _spendAllowance(from, spender, value);
        _transfer(from, to, value);
        return true;
