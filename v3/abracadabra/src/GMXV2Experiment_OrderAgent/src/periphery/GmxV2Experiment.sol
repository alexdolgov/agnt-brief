...
contract GMXV2Experiment_OrderAgent is OperatableV2 {
    ...
    function createOrder(address market, address user, GmRouterOrderParams memory params) external payable onlyOperators returns (address order) {
        order = LibClone.clone(orderImplementation);
        params.inputToken.safeTransferFrom(msg.sender, address(order), params.inputAmount);
        GMXV2Experiment_RouterOrder(order).init{value: msg.value}(market, user, params);
    }
}
