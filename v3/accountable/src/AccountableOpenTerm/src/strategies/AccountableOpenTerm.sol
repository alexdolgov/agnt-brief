...
contract AccountableOpenTerm is UUPSUpgradeable, ReentrancyGuardUpgradeable, AccountableStrategy, OpenTermStorage {
    ...
    function initialize(OpenTermInitParams memory params) external initializer {
        ...
        IERC20(params.asset).forceApprove(vault, type(uint256).max);
        IERC20(params.asset).forceApprove(params.feeManager, type(uint256).max);
    }
    ...
    /// @dev Overridden deposit hook with extra validation logic and balances update
    ...
    /// @dev Overridden mint hook with extra validation logic and balances update
    ...
    /// @dev Overridden onRequestRedeem hook for allocating a redemption request
    ...
    {
        if (controller != managerFeeRecipient && controller != investmentManager) {
    ...
    /// @dev Overridden onCancelRedeemRequest hook for updating batches upon cancellation
    ...
    /// @dev Overridden maxDeposit hook
