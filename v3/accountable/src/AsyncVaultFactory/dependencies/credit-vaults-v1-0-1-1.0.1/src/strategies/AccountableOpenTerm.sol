...
contract AccountableOpenTerm is UUPSUpgradeable, AccountableStrategy, OpenTermStorage {
        ...
        returns (uint256 maxAssets)
    {
        ...
        (uint256 scaleFactor_,) = _previewAccruedInterest();
        uint256 totalAssets = IAccountableVault(vault).totalAssets();
        uint256 principalAssets = _loan.outstandingPrincipal.mulDiv(scaleFactor_, PRECISION, Math.Rounding.Ceil);
    ...
    function _previewAccruedInterest() internal view returns (uint256 scaleFactor_, uint256 scaleFactorNet_) {
        scaleFactor_ = _scaleFactor;
        scaleFactorNet_ = _scaleFactorNet;

        uint256 accruedAt_ = _accruedAt;
        if (block.timestamp <= accruedAt_) return (scaleFactor_, scaleFactorNet_);

        uint256 timeDelta = block.timestamp - accruedAt_;
        if (timeDelta == 0) return (scaleFactor_, scaleFactorNet_);
        ...
        scaleFactor_ += baseInterest + delinquencyFee;
        scaleFactorNet_ += baseNetInterest + delinquencyFee;
    }

    /// @notice Accrues interest based time delta since last accrual
    function _accrueInterest() internal {
        (uint256 scaleFactor_, uint256 scaleFactorNet_) = _previewAccruedInterest();

        if (scaleFactor_ == _scaleFactor) return;

        _scaleFactor = scaleFactor_;
        _scaleFactorNet = scaleFactorNet_;
    ...
    /// @dev Returns the share price at current moment, using virtual interest accrual
    ///      Share price required to use decimals precision stored as PRECISION in the vault
    ...
    function _sharePrice(address) internal view override returns (uint256) {
        ...
        (, uint256 scaleFactorNet_) = _previewAccruedInterest();
        return scaleFactorNet_;
    }
