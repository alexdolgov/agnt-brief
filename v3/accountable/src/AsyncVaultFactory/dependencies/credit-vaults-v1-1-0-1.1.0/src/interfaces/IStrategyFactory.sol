...
/// @notice Parameters for creating a yield loan with an associated vault
struct YieldFactoryParams {
    /// @notice The asset token address for the strategy
    address asset;
    /// @notice The investment manager address
    address investmentManager;
...
interface IYieldStrategyFactory is IStrategyFactoryBase {
    /// @notice Emitted when a new yield loan is created
    /// @param proxy The address of the created loan proxy contract
    /// @param vault The address of the associated vault contract
    /// @param manager The investment manager address
    event YieldStrategyCreated(address indexed proxy, address indexed vault, address indexed manager);

    /// @notice Creates a new yield loan with an associated vault
    /// @param params The parameters for creating a yield loan with an associated vault
    function createYieldStrategy(YieldFactoryParams memory params) external returns (address, address);
}
