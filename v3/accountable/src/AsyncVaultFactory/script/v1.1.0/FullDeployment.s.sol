...
/**
 * @title FullDeployment for Credit Vaults v1.1.0
 * @notice Uses Foundry profile v1-1-0 with isolated output directory (out/v1-1-0).
 *         This ensures artifacts don't conflict with other versions.
 */
...
import "@credit-vaults-v1-1-0/factory/AsyncVaultFactory.sol";
import "@credit-vaults-v1-1-0/factory/RewardsFactory.sol";
...
import "@credit-vaults-v1-1-0/strategies/AccountableFixedTerm.sol";
import "@credit-vaults-v1-1-0/strategies/AccountableOpenTerm.sol";
import "@credit-vaults-v1-1-0/strategies/AccountableYield.sol";
import "@credit-vaults-v1-1-0/publisher/DVNPublisher.sol";
import "@credit-vaults-v1-1-0/batcher/AtomicBatcher.sol";
...
contract FullDeployment is Script {
    ...
    AccountableYield public yieldImpl;
    DVNPublisher public dvnPublisherImpl;
    FixedTermFactory public fixedTermFactory;
    OpenTermFactory public openTermFactory;
    YieldStrategyFactory public yieldStrategyFactory;
    DVNPublisherFactory public dvnPublisherFactory;
    AtomicBatcher public atomicBatcher;
    ...
    // - YieldStrategyFactory: owner
    // - DVNPublisherFactory: owner
    ...
    function run() external {
        ...
        // Step 7: Deploy and configure YieldStrategyFactory
        deployYieldStrategyFactory();

        // Step 8: Deploy and configure DVNPublisherFactory
        deployDVNPublisherFactory();

        // Step 9: Deploy AtomicBatcher (standalone)
        deployAtomicBatcher();

        // Step 10: Transfer ownership to multisig (if different from deployer)
        transferOwnership();
    ...
    function deployFeeManager() internal {
        ...
        // Use explicit artifact path to avoid conflict with v1.0.x FeeManager
        proxy = Upgrades.deployUUPSProxy(
            "credit-vaults-v1-1-0-1.1.0/src/modules/FeeManager.sol:FeeManager",
            initData
        );
    ...
    function deployFactories() internal {
        console.log(
            "=== Step 2: Deploying AsyncVaultFactory and RewardsFactory ==="
        );
        asyncVaultFactory = new AsyncVaultFactory();
        console.log(
            "AsyncVaultFactory deployed at:",
    ...
    function deployGlobalRegistry() internal {
        ...
        // Use explicit artifact path to avoid conflict with v1.0.x GlobalRegistry
        proxy = Upgrades.deployUUPSProxy(
            "credit-vaults-v1-1-0-1.1.0/src/modules/GlobalRegistry.sol:GlobalRegistry",
    ...
    function deployStrategyImplementations() internal {
        ...
        console.log(
            "AccountableFixedTerm implementation deployed at:",
            address(fixedTermImpl)
        );
        waitForBlockTime(2);

        openTermImpl = new AccountableOpenTerm();
        console.log(
            "AccountableOpenTerm implementation deployed at:",
            address(openTermImpl)
        );
        waitForBlockTime(2);

        yieldImpl = new AccountableYield();
        console.log(
            "AccountableYield implementation deployed at:",
            address(yieldImpl)
        );
        waitForBlockTime(2);

        dvnPublisherImpl = new DVNPublisher();
        console.log(
            "DVNPublisher implementation deployed at:",
            address(dvnPublisherImpl)
        );
        console.log("");
    }
    ...
    function deployFixedTermFactory() internal {
        console.log(
            "=== Step 5: Deploying and Configuring FixedTermFactory ==="
        );
    ...
    function deployOpenTermFactory() internal {
        console.log(
            "=== Step 6: Deploying and Configuring OpenTermFactory ==="
        );
    ...
    function deployYieldStrategyFactory() internal {
        console.log(
            "=== Step 7: Deploying and Configuring YieldStrategyFactory ==="
        );
        yieldStrategyFactory = new YieldStrategyFactory(
            address(globals), // globals - global registry reference
            deployer // owner - deployer initially, transferred later
        );
        console.log("YieldStrategyFactory deployed at:", address(yieldStrategyFactory));
        waitForBlockTime(3);

        console.log("Setting StrategyImplementation...");
        yieldStrategyFactory.setStrategyImplementation(address(yieldImpl));
        console.log("StrategyImplementation set to:", address(yieldImpl));
        waitForBlockTime(2);

        console.log("Setting FeeManager...");
        yieldStrategyFactory.setFeeManager(address(feeManager));
        console.log("FeeManager set to:", address(feeManager));
        console.log("");
    }

    function deployDVNPublisherFactory() internal {
        console.log(
            "=== Step 8: Deploying and Configuring DVNPublisherFactory ==="
        );
        dvnPublisherFactory = new DVNPublisherFactory(
            deployer, // owner - deployer initially, transferred later
            address(dvnPublisherImpl) // implementation
        );
        console.log("DVNPublisherFactory deployed at:", address(dvnPublisherFactory));
        console.log("");
    }

    function deployAtomicBatcher() internal {
        console.log("=== Step 9: Deploying AtomicBatcher (Standalone) ===");
        atomicBatcher = new AtomicBatcher();
        console.log("AtomicBatcher deployed at:", address(atomicBatcher));
        console.log("");
    }
    ...
    function transferOwnership() internal {
        ...
        if (accountableSafeWallet == deployer) {
            console.log("=== Step 10: Ownership Transfer ===");
        ...
        console.log(
            "=== Step 10: Transferring Ownership to Accountable Safe Wallet ==="
        );
        console.log(
            "Transferring ownership from deployer to:",
            accountableSafeWallet
        );
        console.log("");
        ...
        // Transfer YieldStrategyFactory ownership
        console.log("Transferring YieldStrategyFactory ownership...");
        yieldStrategyFactory.transferOwnership(accountableSafeWallet);
        console.log("  > YieldStrategyFactory ownership transfer initiated");
        waitForBlockTime(2);

        // Transfer DVNPublisherFactory ownership
        console.log("Transferring DVNPublisherFactory ownership...");
        dvnPublisherFactory.transferOwnership(accountableSafeWallet);
        console.log("  > DVNPublisherFactory ownership transfer initiated");

        console.log("");
        console.log(
            "IMPORTANT: Multisig must accept ownership for all contracts"
        );
        console.log("");
    }

    /// @notice Returns the implementation address behind an ERC1967 proxy
    ...
    function printDeploymentSummary() internal view {
        ...
        console.log("  FeeManager (impl):", getImplementation(address(feeManager)));
        console.log("  AsyncVaultFactory:", address(asyncVaultFactory));
        console.log("  RewardsFactory:", address(rewardsFactory));
        console.log("  GlobalRegistry (proxy):", address(globals));
        console.log("  GlobalRegistry (impl):", getImplementation(address(globals)));
        console.log("");
        console.log("Strategy Implementations:");
        console.log("  AccountableFixedTerm (impl):", address(fixedTermImpl));
        console.log("  AccountableOpenTerm (impl):", address(openTermImpl));
        console.log("  AccountableYield (impl):", address(yieldImpl));
        console.log("  DVNPublisher (impl):", address(dvnPublisherImpl));
        console.log("");
        console.log("Strategy Factories:");
        console.log("  FixedTermFactory:", address(fixedTermFactory));
        console.log("  OpenTermFactory:", address(openTermFactory));
        console.log("  YieldStrategyFactory:", address(yieldStrategyFactory));
        console.log("  DVNPublisherFactory:", address(dvnPublisherFactory));
        console.log("");
        console.log("Utilities:");
        console.log("  AtomicBatcher:", address(atomicBatcher));
        console.log("");
    ...
    function getProtocolMultisig() internal view returns (address) {
        ...
        if (block.chainid == 4114) {
            ...
            require(
                multisig != address(0),
                "PROTOCOL_MULTISIG_CITREA not set in .env"
            );
            return multisig;
        }
        if (block.chainid == 31337) {
        ...
        if (block.chainid == 11155111) {
            ...
            require(
                multisig != address(0),
                "PROTOCOL_MULTISIG_SEPOLIA not set in .env"
            );
            return multisig;
        }
        if (block.chainid == 143) {
            ...
            require(
                multisig != address(0),
                "PROTOCOL_MULTISIG_MONAD not set in .env"
            );
            return multisig;
        }
        if (block.chainid == 42161) {
            ...
            require(
                multisig != address(0),
                "PROTOCOL_MULTISIG_ARBITRUM not set in .env"
            );
            return multisig;
        }
        if (block.chainid == 1) {
            ...
            require(
                multisig != address(0),
                "PROTOCOL_MULTISIG_ETHEREUM not set in .env"
            );
            return multisig;
        }
        if (block.chainid == 60808) {
            ...
            require(
                multisig != address(0),
                "PROTOCOL_MULTISIG_BOB not set in .env"
            );
            return multisig;
        }
    ...
    function getNetworkName() internal view returns (string memory) {
        ...
        return
            string(
                abi.encodePacked("Unknown (", vm.toString(block.chainid), ")")
            );
    }
