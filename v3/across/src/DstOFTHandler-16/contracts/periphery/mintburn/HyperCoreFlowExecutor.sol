...
contract HyperCoreFlowExecutor is AccessControlUpgradeable, AuthorizedFundedFlow, HyperCoreFlowRoles {
    ...
    function _getExistingCoreTokenInfo(
        ...
        // TODO: requirement changed to name length + weiDecimals length to check the non-sponsored swap flow
        require(
            bytes(coreTokenInfo.tokenInfo.name).length != 0 && coreTokenInfo.tokenInfo.weiDecimals != 0,
    ...
    function _initiateSwapFlow(CommonFlowParams memory params, uint256 maxUserSlippageBps) internal {
        ...
        uint256 estSlippagePpm;
        {
            ...
            uint64 approxExecutionPriceX1e8 = _getApproxRealizedPrice(finalTokenInfo, finalCoreTokenInfo);
    ...
    function _setCoreTokenInfo(
        ...
        // TODO: commented for unsponsored swap flow testing
    ...
    function _getApproxRealizedPrice(
        ...
        uint256 spotPxRaw = HyperCoreLib.spotPx(finalTokenInfo.assetIndex);
        ...
        uint256 spotX1e8 = spotPxRaw * (10 ** finalCoreTokenInfo.tokenInfo.szDecimals);
