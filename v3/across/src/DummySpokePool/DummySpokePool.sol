...
contract DummySpokePool is ReentrancyGuard {
    using SafeERC20 for IERC20;

    function mockFill(address token, uint256 amount, address multicallHandler, bytes memory message) external nonReentrant {
        IERC20(token).safeTransferFrom(msg.sender, address(this), amount);
        IERC20(token).safeTransfer(multicallHandler, amount);
        AcrossMessageHandler(multicallHandler).handleV3AcrossMessage(token, amount, msg.sender, message);
    }
}
