...
contract HyperliquidDepositHandler is AcrossMessageHandler, ReentrancyGuard, Ownable {
     ...
     * @param token The address of the token to deposit.
     * @param evmAmount The amount of tokens on Hypercore to send to user.
     * @param user The address of the user on Hypercore to send the tokens to.
     */
    function sweepCoreFundsToUser(address token, uint256 evmAmount, address user) external onlyOwner {
        ...
        (, uint64 amountCoreToReceive) = HyperCoreLib.maximumEVMSendAmountToAmounts(
            evmAmount,
            decimalDiff
        );
        HyperCoreLib.transferERC20CoreToCore(tokenIndex, user, amountCoreToReceive);
    }
    ...
    function _depositToHypercore(address token, uint256 evmAmount, address user) internal {
        // uint64 tokenIndex = PrecompileLib.getTokenIndex(token);
        // int8 decimalDiff = PrecompileLib.tokenInfo(uint32(tokenIndex)).evmExtraWeiDecimals;
        HyperCoreLib.transferERC20EVMToCore(token, 360, user, evmAmount, -2);
    }
