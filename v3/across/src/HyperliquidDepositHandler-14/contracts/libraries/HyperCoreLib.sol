...
library HyperCoreLib {
    ...
    /**
     * @notice Activates a core account for a given user.
     * @dev Reverts if the user already exists on HyperCore.
     * @dev Reverts if the user does not have enough balance on Hypercore to activate the account.
     * @param user The address of the user to activate
     * @param fundingTokenIndex The HyperCore index id of the token to fund the activation fee with.
     * @param evmActivationFee The activation fee in EVM units
     * @param decimalDiff The decimal difference of evmDecimals - coreDecimals for the funding token
     */
    function activateCoreAccount(
        address user, 
        uint64 fundingTokenIndex, 
        uint256 evmActivationFee,
        int8 decimalDiff
    ) internal {
        if (coreUserExists(user)) {
            revert CoreUserAlreadyExists();
        }
        (, uint64 coreActivationFee) = maximumEVMSendAmountToAmounts(evmActivationFee, decimalDiff);
        if (spotBalance(msg.sender, fundingTokenIndex) < coreActivationFee + 1) {
            revert InsufficientCoreBalanceToActivateAccount();
        }
        // Transfer 1 wei from this contract's core account on HyperCore to the user's account on HyperCore.
        // The core activation fee will be deducted from the caller's core account.
        transferERC20CoreToCore(fundingTokenIndex, user, 1);
    }
