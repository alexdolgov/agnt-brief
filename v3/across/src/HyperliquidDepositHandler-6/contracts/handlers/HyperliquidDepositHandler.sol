...
contract HyperliquidDepositHandler is AcrossMessageHandler, ReentrancyGuard, Ownable {
    ...
    function _depositToHypercore(address token, uint256 evmAmount, address user) internal {
        ...
        // wei to the user's account, so we pull the activation fee + 1 wei from the donation box. If the
        // donation box lacks funds, then the caller of this function will end up paying for the user's
        // activation fee.
        if (!userExists) {
            if (IERC20(token).balanceOf(address(donationBox)) >= amountRequiredToActivate) {
                // To activate an account, we must pay the activation from this contract's core account and then send 1
