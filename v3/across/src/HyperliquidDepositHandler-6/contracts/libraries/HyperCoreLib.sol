...
library HyperCoreLib {
     ...
     * @dev Reverts if the user has zero balance on Hypercore to activate the account. Does not actually
     * check that the user's core balance has enough to pay the activation fee, that is left as a responsibility of the caller.
     ...
     */
    function activateCoreAccount(address user, uint64 fundingTokenIndex) internal {
        if (coreUserExists(user)) {
            revert CoreUserAlreadyExists();
        }
        if (spotBalance(address(this), fundingTokenIndex) < 1) {
        ...
        // The core activation fee + 1 wei will be deducted from the caller's core account.
