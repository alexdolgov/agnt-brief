...
contract AddyExchange is Ownable {
    using SafeERC20 for IERC20;
    using SafeMath for uint256;

    address public addy_old;
    address public feeDist;
    address public addy_new;
    mapping (address => bool) public hasExchangedStaked;

    constructor(address _addy_old, address _addy_new, address _feeDist) public {
        addy_old = _addy_old;
        addy_new = _addy_new;
        feeDist = _feeDist;
    }
    ...
    function exchangeTokens() external {
        ...
        require(!hasExchangedStaked[msg.sender], "already exchanged");

        (uint256 vestedAmount, uint256 penaltyAmount) = IMultiFeeDistribution(feeDist).withdrawableBalance(msg.sender);
        uint256 unlockedBalance = IMultiFeeDistribution(feeDist).unlockedBalance(msg.sender);
        require(unlockedBalance == 0 && vestedAmount == 0, "withdraw all unlocked and vested tokens first");
        
        hasExchangedStaked[msg.sender] = true;

        uint256 amount = IERC20(addy_old).balanceOf(msg.sender);
        //reverts if amount > sender"s balance, so I don"t need to check if amount <= sender"s balance
        IERC20(addy_old).safeTransferFrom(msg.sender, address(this), amount);
        
        uint256 total = IMultiFeeDistribution(feeDist).totalBalance(msg.sender).add(amount);
        //reverts if amount > the contract"s balance, so I don"t need to check if amount <= contract"s balance
        IERC20(addy_new).safeTransfer(msg.sender, total);

        emit Exchanged(msg.sender, total);
    }

    /* ========== VIEW FUNCTIONS ========== */

    function getBalance() public view returns (uint256) {
        return IERC20(addy_new).balanceOf(address(this));
    }

    function getAmountConvertible(address user) public view returns (uint256) {
        if(hasExchangedStaked[user]) return 0;
        return IERC20(addy_old).balanceOf(user) + IMultiFeeDistribution(feeDist).totalBalance(user);
    }

    //Events

    event Exchanged(address indexed from, uint256 amount);
}
