...
contract ApeStrategy is BaseStrategy {

    address public BANANA = 0x5d47bAbA0d66083C52009271faF3F50DCc01023C;
    address public WMATIC = 0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270;
    address public tokenA;
    address public tokenB;

    address public MASTER_CHEF = 0x54aff400858Dcac39797a81894D9920f16972D1D;
    address public APE_ROUTER = 0xC0788A3aD43d79aa53B09c2EaCc313A787d1d607;
        ...
        )
    {
        ...
        reward_a_path[0] = harvestedToken;
    ...
    function harvest() public override onlyHumanOrWhitelisted {
        ...
        if (_harvested_balance > 0) {
            ...
            if(tokenB == BANANA) {
                //Swap 1/2 of BANANA for WMATIC
                _swapUniswapWithPath(reward_a_path, afterFeeAmount.div(2));
            }
            else {
                //Swap all BANANA for tokenA
    ...
    function _getReward() internal {
        IApeChef(rewards).harvest(poolId, address(this));
    }
    ...
    function _withdrawSome(uint256 _amount)
        ...
        returns (uint256)
    {
        IApeChef(rewards).withdraw(poolId, _amount, address(this));
        return _amount;
    }
    ...
    function deposit() public override {
        ...
        if (_want > 0) {
            ...
            IApeChef(rewards).deposit(poolId, _want, address(this));
        }
    }
    ...
    function emergencyWithdraw() public onlyOwner {
        IApeChef(rewards).emergencyWithdraw(poolId, address(this));
...
contract ApeStrategyDeployer is Ownable {
    ...
    function deploy(address lp, address tokenA, address tokenB, uint256 _poolId) public onlyOwner {
        address strategy = address(new ApeStrategy(lp, tokenA, tokenB, _poolId, FUND));
        address jar = address(new GenericVault(IStrategy(strategy), proxy_minter, FUND));
        ApeStrategy(strategy).setJar(jar);
    ...
    event Deployed(address indexed lp, address indexed strategy, address indexed jar);
}