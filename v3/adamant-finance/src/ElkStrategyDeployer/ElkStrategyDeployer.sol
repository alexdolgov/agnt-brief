...
//Deploys strategies for Elk/Other pairs
contract ElkStrategyDeployer is Ownable {
    ...
    function deploy(address rewards, address lp, address tokenA) public onlyOwner {
        require(finalized == false, "This deployer contract is not allowed to deploy any more vaults");

        address strategy = address(new StrategyElkPair(rewards, lp, tokenA, FUND));
        address jar = address(new GenericVault(IStrategy(strategy), proxy_minter, FUND));
        StrategyElkPair(strategy).setJar(jar);
