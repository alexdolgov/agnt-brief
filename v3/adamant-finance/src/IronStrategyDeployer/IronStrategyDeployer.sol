...
abstract contract BaseStrategyIron is BaseStrategyMasterChef {
    ...
    // Uniswap swap paths
    address[] public TITAN_WMATIC_path;
    ...
    {
        TITAN_WMATIC_path = new address[](2);
        TITAN_WMATIC_path[0] = TITAN;
        TITAN_WMATIC_path[1] = WMATIC;
    }
    ...
    function swapTitanToWmaticAndDistributeFee(uint256 feeAmount) internal {
        if(feeAmount > 0) {
            _swapUniswapWithPath(TITAN_WMATIC_path, feeAmount);
...
contract StrategyIronUsdc is BaseStrategyIron {
    ...
    address public constant USDC_IRON_LP = 0x85dE135fF062Df790A5f20B79120f17D3da63b2d;

    // Uniswap swap paths
    address[] public IRON_USDC_path;
        ...
        )
    {
        ...
        IRON_USDC_path = new address[](2);
        IRON_USDC_path[0] = IRON;
        IRON_USDC_path[1] = usdc;
    }

    // **** State Mutations ****

    function harvest() public override onlyHumanOrWhitelisted {
        //Transfer any harvestedToken that may already be in the contract to the fee dist fund
        ...
        if (_harvested_balance > 0) {
            ...
            swapTitanToWmaticAndDistributeFee(feeAmount);

            //Swap remaining TITAN for IRON
            _swapUniswapWithPath(TITAN_IRON_path, afterFeeAmount);
            //Then swap 1/2 of IRON for USDC
            uint256 _ironBalance = IERC20(IRON).balanceOf(address(this));
            _swapUniswapWithPath(IRON_USDC_path, _ironBalance.div(2));
        }

        uint256 usdcBalance = IERC20(usdc).balanceOf(address(this));
        uint256 ironBalance = IERC20(IRON).balanceOf(address(this));
        if (usdcBalance > 0 && ironBalance > 0) {
            IERC20(usdc).safeApprove(currentRouter, 0);
            IERC20(usdc).safeApprove(currentRouter, usdcBalance);
            IERC20(IRON).safeApprove(currentRouter, 0);
            IERC20(IRON).safeApprove(currentRouter, ironBalance);

            IUniswapRouterV2(currentRouter).addLiquidity(
                usdc, IRON,
                usdcBalance, ironBalance,
                0, 0,
                address(this),
                now + 60
            );
...
contract IronStrategyDeployer is Ownable {
