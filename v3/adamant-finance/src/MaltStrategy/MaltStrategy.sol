...
interface ILiquidityMine {
    ...
    //this withdraws rewards
    function withdraw(uint256 rewardAmount, uint256 maltAmount) external;

    // **** Views ****

    function balanceOfBonded(address account) external view returns (uint256);

    function earned(address account) external view returns (uint256 rewardEarned, uint256 maltEarned);
}
interface IERCFund {
...
contract MaltStrategy is BaseStrategy {

    uint256 public constant keepMax = 10000;

    address public QUICKSWAP_ROUTER = 0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff; //Quickswap router
    address public BONDING_CONTRACT = 0x539618aA29c95c28C0B04aBB9025815C014A9DB9;
    address public rewards;

    address public MALT_DAI_LP = 0x3b7e9EA4bF300525CcbbC81C792b2bCe7A13A2d6;

    address public WMATIC = 0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270;
    address public MALT = 0x1c40Ac03AacAF5F85808674E526E9c26309DB92F;
    address public DAI = 0x8f3Cf7ad23Cd3CaDbD9735AFf958023239c6A063;

    // Uniswap swap paths
    address[] public daiWmaticPath;
    address[] public daiMaltPath;
    ...
    {
        rewards = BONDING_CONTRACT;

        daiWmaticPath = new address[](2);
        daiWmaticPath[0] = DAI;
        daiWmaticPath[1] = WMATIC;

        daiMaltPath = new address[](2);
        daiMaltPath[0] = DAI;
        daiMaltPath[1] = MALT;
    }
    ...
    function swapRewardToWmaticAndDistributeFee(uint256 feeAmount) internal {
        if(feeAmount > 0) {
            _swapUniswapWithPath(daiWmaticPath, feeAmount);
    ...
    function harvest() public override onlyHumanOrWhitelisted {
        //Transfer any harvestedToken (DAI) and WMATIC that may already be in the contract to the fee dist fund
        ...
        if (_harvested_balance > 0) {
            ...
            _swapUniswapWithPath(daiMaltPath, afterFeeAmount.div(2));
        }

        uint256 harvestedTokenBalance = IERC20(DAI).balanceOf(address(this));
        uint256 otherBalance = IERC20(MALT).balanceOf(address(this));
        if (harvestedTokenBalance > 0 && otherBalance > 0) {
            ...
            IERC20(MALT).safeApprove(currentRouter, 0);
            IERC20(MALT).safeApprove(currentRouter, otherBalance);
    ...
    function deposit() public override {
        ...
        if (_want > 0) {
            ...
            ILiquidityMine(rewards).bond(_want);
        }
    }
    ...
    function _withdrawSome(uint256 _amount)
        ...
        returns (uint256)
    {
        ILiquidityMine(rewards).unbond(_amount);
        return _amount;
    }
    
    function _getReward() internal {
        (uint256 rewardEarned, uint256 maltEarned) = ILiquidityMine(rewards).earned(address(this));
        ILiquidityMine(rewards).withdraw(rewardEarned, maltEarned);
    }
    ...
    function balanceOfPool() public override view returns (uint256) {
        return ILiquidityMine(rewards).balanceOfBonded(address(this));
    }
    ...
    //Withdraw all funds from the contract and return it to the vault
    function emergencyExit() public onlyOwner {
        ILiquidityMine(rewards).unbond(balanceOfPool());
        uint256 _want = IERC20(want).balanceOf(address(this));
        if (_want > 0) {
