...
contract PolyYeldStrategyDeployer is Ownable {
    ...
    address YELD = 0xd0f3121A190d85dE0AB6131f2bCEcdbfcfB38891;
    address MASTER_CHEF = 0x2DC11B394BD0f1CC6AC0a269cfe3CC0b333601B4;

    //just route all trades thru QS router
    ...
    function deploy(address lp, address otherToken, uint256 _poolId) public onlyOwner {
        address strategy = address(new MasterChefReferralStrategy(MASTER_CHEF, lp, otherToken, _poolId, FUND, YELD, QUICKSWAP_ROUTER));
        address jar = address(new GenericVault(IStrategy(strategy), proxy_minter, FUND));
        MasterChefReferralStrategy(strategy).setJar(jar);
