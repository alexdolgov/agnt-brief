...
//Copied from Quickswap's Staking Rewards contract
contract MeshStakingRewards is IStakingRewards, RewardsDistributionRecipient, ReentrancyGuard, Ownable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    address public constant MESH = 0x82362Ec182Db3Cf7829014Bc61E9BE8a2E82868a;
    address public constant ADMESH = 0x791e974FD87c8d0BCAa9aa253ff4591A5957127c;

    /* ========== STATE VARIABLES ========== */

    IERC20 public rewardsToken; //actually isn't used
    ...
    constructor() public {
        rewardsToken = IERC20(MESH);
        stakingToken = IERC20(ADMESH);
    ...
    //for compatibility with vault ABI
    ...
    function stakeFor(address beneficiary, uint256 amount) external nonReentrant updateReward(beneficiary) {
        require(amount > 0, "Cannot stake 0");
        _totalSupply = _totalSupply.add(amount);
    ...
    function notifyRewardAmount(uint256 reward, uint256 rewardsDuration) external override onlyRewardsDistribution updateReward(address(0)) {
        ...
        //Check above not applicable, since minter contract mints MESH rewards to the fee dist contract
    ...
    function setRewardsDistribution(address _address) external onlyOwner {
        rewardsDistribution = _address;
    }
