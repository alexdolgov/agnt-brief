...
interface IPolyZapChef {
    function getHarvestUntil(uint256 _pid, address _user) external view returns (uint256);
}

abstract contract StrategyPolyZapBase is BaseStrategyMasterChef {

    //Tokens
    address public constant pzap = 0xeb2778f74E5ee038E67AA6c77f0F0451ABd748FD;
    address public constant WMATIC = 0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270;
    address public otherToken;
    //Other
    address public pzapRouter = 0x4aAEC1FA8247F85Dc3Df20F4e03FEAFdCB087Ae9;
    address public pzapChef = 0xB93C082bCfCCf5BAeA0E0f0c556668E25A41B896;
    
    uint256 public constant keep = 200;
    uint256 public constant keepMax = 10000;
    uint256 public depositCutoff = 3600;

    // Uniswap swap paths
    address[] public reward_other_path;
    address[] public reward_wmatic_path; //Since calc doesn't support pzap, convert the fee into WMATIC and notify the vault
        ...
        )
    {
        ...
        reward_other_path[0] = pzap;
        reward_other_path[1] = _otherToken;
        
        reward_wmatic_path = new address[](2);
        reward_wmatic_path[0] = pzap;
        reward_wmatic_path[1] = WMATIC;
    }

    function getDiff() public view returns (uint256) {
        uint256 nextDepositTime = IPolyZapChef(rewards).getHarvestUntil(poolId, address(this));
        if(nextDepositTime > now) {
            return nextDepositTime - now;
        }
        return 0;
    }

    function deposit() public override {
        //Stop automatically depositing funds if there are less than X seconds before the next harvest
        if(getDiff() < depositCutoff) {
            return;
        }
    ...
    function harvest() public override {
        ...
        if (harvestedTokenBalance > 0 && otherBalance > 0) {
            ...
            IERC20(otherToken).safeTransfer(strategist, IERC20(otherToken).balanceOf(address(this)));
...
contract StrategyPolyZap is StrategyPolyZapBase {
...
contract PolyZapStrategyDeployer is Ownable {
    ...
    function deploy(address lp, address tokenA, uint256 _poolId, string memory _pair_name) public onlyOwner {
        address strategy = address(new StrategyPolyZap(lp, tokenA, _poolId, FUND, _pair_name));
        address jar = address(new GenericVault(IStrategy(strategy), proxy_minter, FUND));

        StrategyPolyZap(strategy).setJar(jar);
