...
abstract contract QiDaoStrategyBase is BaseStrategyMasterChef {
    ...
    address public qiChef = 0x574Fe4E8120C4Da1741b5Fd45584de7A5b521F0F;
    
    uint256 public constant keep = 200;
    uint256 public constant keepMax = 10000;
    bool emissionsEnabled = false;

    // Uniswap swap paths
    address[] public miMatic_other_path;
    address[] public qi_miMatic_path;
    address[] public miMatic_usdc_path;
    address[] public qi_usdc_path;
        ...
        )
    {
        ...
        miMatic_other_path = new address[](2);
        miMatic_other_path[0] = miMatic;
        miMatic_other_path[1] = _otherToken;
        
        qi_miMatic_path = new address[](2);
        qi_miMatic_path[0] = QI;
        qi_miMatic_path[1] = miMatic;
        
        qi_usdc_path = new address[](3);
        qi_usdc_path[0] = QI;
        qi_usdc_path[1] = miMatic;
        qi_usdc_path[2] = usdc;

        miMatic_usdc_path = new address[](2);
        miMatic_usdc_path[0] = miMatic;
        miMatic_usdc_path[1] = usdc;
    }
    ...
    function harvest() public override {
        ...
        if (_harvested_balance > 0) {
            uint256 feeAmount = _harvested_balance.mul(keep).div(keepMax);
            uint256 afterFeeAmount = _harvested_balance.sub(feeAmount);
            IERC20(harvestedToken).safeTransfer(strategist, feeAmount);

            //Swap remainder for otherToken
            if(otherToken == usdc) {
                //need to swap qi for miMatic before swapping to usdc
                _swapUniswapWithPath(qi_miMatic_path, afterFeeAmount);
                uint256 _miMaticBal = IERC20(miMatic).balanceOf(address(this));
                _swapUniswapWithPath(miMatic_usdc_path, _miMaticBal.div(2));
            }
            else {
                _swapUniswapWithPath(qi_miMatic_path, afterFeeAmount.div(2));
            }
        }
        
        //only miMatic - X pairs
        uint256 miMaticBalance = IERC20(miMatic).balanceOf(address(this));
        uint256 otherBalance = IERC20(otherToken).balanceOf(address(this));
        if (miMaticBalance > 0 && otherBalance > 0) {
            IERC20(miMatic).safeApprove(currentRouter, 0);
            IERC20(miMatic).safeApprove(currentRouter, miMaticBalance);
...
contract QiDaoStrategy is QiDaoStrategyBase {
...
contract QiDaoStrategyDeployer is Ownable {
    ...
    function deploy(address lp, address tokenA, uint256 _poolId, string memory _pair_name) public onlyOwner {
        address strategy = address(new QiDaoStrategy(lp, tokenA, _poolId, FUND, _pair_name));
        address jar = address(new GenericVault(IStrategy(strategy), proxy_minter, FUND));

        GenericVault(jar).setPoolDepositFee(50); //0.5% deposit fee for QiDao pools
        QiDaoStrategy(strategy).setJar(jar);
