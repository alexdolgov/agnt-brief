...
abstract contract BaseStrategyMasterChef is BaseStrategy {
    ...
    constructor(
        ...
        public
        BaseStrategy(_want, _strategist)
    {
        poolId = _poolId;
        rewards = _rewards;
    }
...
abstract contract StrategyDarkBuildRebasingUSDBase is BaseStrategyMasterChef {

    // Token addresses for MATIC
    address public constant darkBuild = 0x0e59D50adD2d90f5111aca875baE0a72D95B4762;
    address public constant rdUSD = 0x9F001a9c830c6eD1e656A35163d3B029FffA5505;
    
    uint256 public constant keepFXS = 200; //2% dev fee
    uint256 public constant keepFXSmax = 10000;

    // Uniswap swap paths
    address[] public db_rdusd_path;
    ...
    {
        db_rdusd_path = new address[](2);
        db_rdusd_path[0] = darkBuild;
        db_rdusd_path[1] = rdUSD;
    }
    ...
    function harvest() public override {
        ...
        //Swap 1/2 of DB for rdUSD
        uint256 _DB_balance = IERC20(darkBuild).balanceOf(address(this));
        if (_DB_balance > 0) {
            _swapUniswapWithPathForFeeOnTransferTokens(db_rdusd_path, _DB_balance.div(2));
        }
        
        // Add liquidity for pair
        uint256 _darkBuild = IERC20(darkBuild).balanceOf(address(this));
        uint256 _rdUSD = IERC20(rdUSD).balanceOf(address(this));
        if (_darkBuild > 0 && _rdUSD > 0) {
            IERC20(darkBuild).safeApprove(currentRouter, 0);
            IERC20(darkBuild).safeApprove(currentRouter, _darkBuild);
            IERC20(rdUSD).safeApprove(currentRouter, 0);
            IERC20(rdUSD).safeApprove(currentRouter, _rdUSD);
            ...
            // Donates DUST
            IERC20(darkBuild).safeTransfer(
                strategist,
                IERC20(darkBuild).balanceOf(address(this))
            );
            IERC20(rdUSD).safeTransfer(
                strategist,
                IERC20(rdUSD).balanceOf(address(this))
            );
        }
...
contract StrategyDarkBuildRebasingUSD is StrategyDarkBuildRebasingUSDBase {
    // Token addresses
    address public DB_MASTER = 0x5eFe33090ebc78E09c03Dc3DC46Ef305880fc398;
    address public DB_rdUSD_LP = 0x5A35B8768f60341680D78d0C12450234552E390B;
    uint256 public DB_rdUSD_POOL_ID = 1;

    constructor(address _strategist)
        public
        StrategyDarkBuildRebasingUSDBase(
            DB_MASTER,
            DB_rdUSD_LP,
            _strategist,
            DB_rdUSD_POOL_ID
