...
contract StrategyComethPair is BaseStrategyRewardPairNonReentrant {
...
//Deploys strategies for Cometh/Other pairs
contract StrategyDeployerComethPair is Ownable {
    ...
    function deploy(address rewards, address lp, address otherToken, string memory _pair_name) public onlyOwner {
        require(finalized == false, "This deployer contract is not allowed to deploy any more vaults");

        address strategy = address(new StrategyComethPair(rewards, lp, otherToken, FUND, _pair_name));
        address jar = address(new GenericVault(IStrategy(strategy), proxy_minter, FUND));
        StrategyComethPair(strategy).setJar(jar);
