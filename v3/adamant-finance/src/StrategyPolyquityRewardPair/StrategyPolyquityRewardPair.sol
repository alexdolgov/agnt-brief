...
contract StrategyPolyquityRewardPair is BaseStrategyPolyquityRewards {
    ...
    address public SUSHI_ROUTER = 0x1b02dA8Cb0d097eB8D57A175b88c7D8b47997506; //Quickswap router

    // Uniswap swap paths
    address[] public reward_other_path;
    address[] public reward_matic_path;

    constructor(address rewards, address lp, address _otherToken, address strategist)
        public
    ...
    function harvest() public override onlyHumanOrWhitelisted {
        ...
        //This will leave some leftover PYQ in the contract until the transfer fee goes to 0
        uint256 remainingHarvested = IERC20(harvestedToken).balanceOf(address(this));
        if (remainingHarvested > 0) {
            _swapUniswapWithPathForFeeOnTransferTokens(reward_other_path, remainingHarvested.div(2));
        }
        ...
        if (harvestedTokenBalance > 0 && otherBalance > 0) {
            IERC20(harvestedToken).safeApprove(rewards, 0);
            IERC20(harvestedToken).safeApprove(rewards, harvestedTokenBalance);
            IERC20(otherToken).safeApprove(rewards, 0);
            IERC20(otherToken).safeApprove(rewards, otherBalance);

            //Reward pool contract is whitelisted and can add liquidity without being taxed
            IPolyquityRewards(rewards).addLiquidity(
