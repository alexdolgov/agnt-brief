...
//Deploys strategies for Quick/Other pairs
contract QuickStrategyDeployer is Ownable {
    ...
    function deploy(address rewards, address lp, address tokenA, string memory _pair_name) public onlyOwner {
        require(finalized == false, "This deployer contract is not allowed to deploy any more vaults");

        address strategy = address(new StrategyQuickPair(rewards, lp, tokenA, FUND, _pair_name));
        address jar = address(new GenericVault(IStrategy(strategy), proxy_minter, FUND));
        StrategyQuickPair(strategy).setJar(jar);
