...
abstract contract BaseStrategySingleAssetWhale is BaseStrategyMasterChef {

    address public WHALE_MASTER_CHEF = 0x34bc3D36845d8A7cA6964261FbD28737d0d6510f;

    // Token addresses for MATIC
    address public constant krill = 0x05089C9EBFFa4F0AcA269e32056b1b36B37ED71b;
    address public constant usdc = 0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174;
    
    uint256 public constant keepFXS = 200; //2% performance fee distributed to ADDY stakers
    uint256 public constant keepFXSmax = 10000;

    // Uniswap swap paths
    address[] public usdc_krill_path;
    address[] public usdc_want_path;
    ...
    {
        usdc_krill_path = new address[](2);
        usdc_krill_path[0] = krill;
        usdc_krill_path[1] = usdc;

        usdc_want_path = new address[](2);
        usdc_want_path[0] = usdc;
        usdc_want_path[1] = want;
    }
    ...
    function harvest() public override {
        ...
        if(getReward) {
            ...
            //Swap KRILL for USDC
            uint256 _krillBalance = IERC20(krill).balanceOf(address(this));
            if (_krillBalance > 0) {
                _swapUniswapWithPath(usdc_krill_path, _krillBalance);
            }
            //Swap USDC for token being staked (if the token being staked isn't USDC)
            uint256 _usdcBalance = IERC20(usdc).balanceOf(address(this));
            if (_usdcBalance > 0 && want != usdc) {
                _swapUniswapWithPath(usdc_want_path, _usdcBalance);
            }
...
contract StrategySingleAssetWhale is BaseStrategySingleAssetWhale {

    string private pair_name;
    address public FUND = 0xC9E908D24f13c18A0ab803A4ACF469B37c1761e8;

    constructor(address _want, uint256 _poolId, string memory _pair_name)
        public
        BaseStrategySingleAssetWhale(
            _want,
            FUND,
            _poolId
        )
    ...
    function getName() external override pure returns (string memory) {
        return "StrategySingleAssetWhale";
    }
