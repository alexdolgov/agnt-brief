...
abstract contract StrategyFraxFarmBase is StrategyStakingRewardsBase {
    ...
    address public USDC_maUSDC_QUICKSWAP_STAKING_CONTRACT = 0x68910d18332fFDc1D11caEA4fE93C94Ccd540732;

    // Token addresses for MATIC
    address public quick = 0x831753DD7087CaC61aB5644b308642cc1c33Dc13;
    address public usdc = 0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174;
    address public mausdc = 0x9719d867A500Ef117cC201206B8ab51e794d3F82;

    // LP Token addresses
    address public USDC_maUSDC_LP = 0x7295304b10740BA8e037826787d3e9386FD99925;
    ...
    address[] public usdc_mausdc_path;
        ...
        )
    {
        ...
        usdc_mausdc_path = new address[](2);
        usdc_mausdc_path[0] = usdc;
        usdc_mausdc_path[1] = mausdc;
    }

    // **** State Mutations ****

    function harvest() public override {
        //prevent smart contracts from calling harvest()
        ...
        //Swap 1/2 of USDC for maUSDC
        uint256 _usdcBalance = IERC20(usdc).balanceOf(address(this));
        if (_usdcBalance > 0) {
            _swapUniswapWithPath(usdc_mausdc_path, _usdcBalance.div(2));
        }
        
        // Add liquidity for FXS/FRAX
        uint256 _usdc = IERC20(usdc).balanceOf(address(this));
        uint256 _mausdc = IERC20(mausdc).balanceOf(address(this));
        if (_usdc > 0 && _mausdc > 0) {
            IERC20(usdc).safeApprove(currentRouter, 0);
            IERC20(usdc).safeApprove(currentRouter, _usdc);
            IERC20(mausdc).safeApprove(currentRouter, 0);
            IERC20(mausdc).safeApprove(currentRouter, _mausdc);
            ...
            IERC20(mausdc).safeTransfer(
                strategist,
                IERC20(mausdc).balanceOf(address(this))
            );
        }
    ...
    function salvage(address recipient, address token, uint256 amount) public onlyOwner {
        //should be no tokens here in Matic
        IERC20(token).safeTransfer(recipient, amount);
    }

    uint256 public lockPeriod = 1 days;
    uint256 public lockTimeStamp = 0;
    address public newAddress;

    function migrate(address newStakingContract) public onlyOwner {
        if(lockTimeStamp < now && lockTimeStamp != 0 && newAddress != address(0)) {
            //Collect all rewards from the old staking contract and convert to LP
            harvest();
            //Withdraw all tokens from the old staking contract
            _withdrawSome(balanceOfPool());
            //Set staking contract to the new address and stake in it
            ...
            lockTimeStamp = now + lockPeriod;
            newAddress = newStakingContract;
        }
    }
}

contract StrategyUSDCmaUSDC is StrategyFraxFarmBase {

    constructor(address _strategist)
        public
        StrategyFraxFarmBase(
            USDC_maUSDC_QUICKSWAP_STAKING_CONTRACT,
            USDC_maUSDC_LP,
            _strategist
        )
    {}
