...
abstract contract BaseStrategyWaultChef is BaseStrategy {
    ...
    constructor(
        ...
        public
        BaseStrategy(_want, _strategist, WEXpoly, WAULT_ROUTER)
    {
        poolId = _poolId;
        rewards = waultChef;
    }
...
contract StrategyWaultOtherPair is BaseStrategyWaultChef {
...
contract WaultOtherStrategyDeployer is Ownable {
    ...
    function deploy(address lp, address tokenA, address tokenB, uint256 _poolId) public onlyOwner {
        ...
        address strategy = address(new StrategyWaultOtherPair(lp, tokenA, tokenB, _poolId, FUND));
        address jar = address(new GenericVault(IStrategy(strategy), proxy_minter, FUND));
        StrategyWaultOtherPair(strategy).setJar(jar);
