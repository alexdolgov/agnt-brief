...
abstract contract StrategyWhaleBase is BaseStrategyMasterChef {
    ...
    function harvest() public override {
        ...
        //Swap 1/2 KRILL for USDC
        uint256 _krillBalance = IERC20(krill).balanceOf(address(this));
        if (_krillBalance > 0) {
            _swapUniswapWithPath(usdc_krill_path, _krillBalance.div(2));
        }
        
        // Add liquidity for KRILL/USDC
        uint256 _usdc = IERC20(usdc).balanceOf(address(this));
        uint256 _krill = IERC20(krill).balanceOf(address(this));
        if (_usdc > 0 && _krill > 0) {
            ...
            IERC20(krill).safeApprove(currentRouter, _krill);
            ...
            IERC20(krill).safeTransfer(
                strategist,
                IERC20(krill).balanceOf(address(this))
            );
        }
...
contract StrategyWhale is StrategyWhaleBase {
    // Token addresses
    address public WHALE_MASTER_CHEF = 0x34bc3D36845d8A7cA6964261FbD28737d0d6510f;
    address public WHALE_USDC_LP = 0x6405Ebc22cB0899FC21f414085Ac4044B4721a0d;
