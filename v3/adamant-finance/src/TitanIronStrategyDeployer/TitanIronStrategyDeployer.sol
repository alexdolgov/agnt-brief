...
contract StrategyTitanIron is BaseStrategyIron {

    string private pair_name;
    address public constant IRON = 0xD86b5923F3AD7b585eD81B448170ae026c65ae9a;
    address public constant TITAN_IRON = 0x35c1895DAC1e2432b320e2927b4F71a0D995602F;
        ...
        )
    {
        ...
        //titan/iron staking uses a different masterchef contract
        masterChef = 0xb444d596273C66Ac269C33c30Fbb245F4ba8A79d;
        rewards = 0xb444d596273C66Ac269C33c30Fbb245F4ba8A79d;
    }
    ...
    function harvest() public override onlyHumanOrWhitelisted {
        ...
        if (_harvested_balance > 0) {
            ...
            //Swap 1/2 of remaining TITAN for IRON
            _swapUniswapWithPath(TITAN_IRON_path, afterFeeAmount.div(2));
        }

        uint256 ironBalance = IERC20(IRON).balanceOf(address(this));
        uint256 titanBalance = IERC20(TITAN).balanceOf(address(this));
        if (ironBalance > 0 && titanBalance > 0) {
            ...
            IERC20(TITAN).safeApprove(currentRouter, titanBalance);

            IUniswapRouterV2(currentRouter).addLiquidity(
                IRON, TITAN,
                ironBalance, titanBalance,
                0, 0,
                address(this),
                now + 60
            );
...
contract TitanIronStrategyDeployer is Ownable {
    ...
    function deploy(string memory _pair_name) public onlyOwner {
        ...
        address strategy = address(new StrategyTitanIron(FUND, _pair_name));
        address jar = address(new GenericVault(IStrategy(strategy), proxy_minter, FUND));
        StrategyTitanIron(strategy).setJar(jar);
