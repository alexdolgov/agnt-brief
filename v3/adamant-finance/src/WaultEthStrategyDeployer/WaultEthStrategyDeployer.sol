...
contract WaultEthStrategyDeployer is Ownable {
    ...
    function deploy(address lp, address tokenA, uint256 _poolId) public onlyOwner {
        ...
        address strategy = address(new StrategyWaultEthPair(lp, tokenA, _poolId, FUND));
        address jar = address(new GenericVault(IStrategy(strategy), proxy_minter, FUND));
        StrategyWaultEthPair(strategy).setJar(jar);
