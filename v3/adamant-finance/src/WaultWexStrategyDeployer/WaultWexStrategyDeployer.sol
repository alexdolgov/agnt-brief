...
//Deploys strategies for WEXpoly/Other pairs
contract WaultWexStrategyDeployer is Ownable {
    ...
    function deploy(address lp, address tokenA, uint256 _poolId) public onlyOwner {

        address strategy = address(new StrategyWaultWexPair(lp, tokenA, _poolId, FUND));
        address jar = address(new GenericVault(IStrategy(strategy), proxy_minter, FUND));
        StrategyWaultWexPair(strategy).setJar(jar);
