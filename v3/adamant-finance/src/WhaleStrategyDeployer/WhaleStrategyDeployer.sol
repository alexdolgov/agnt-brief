...
abstract contract BaseStrategyWhale is BaseStrategyMasterChef {

    //Tokens
    address public constant krill = 0x05089C9EBFFa4F0AcA269e32056b1b36B37ED71b;
    ...
    address public masterChef = 0x0c23DCc118313ceB45a029CE0A4AB744eA4928ef;
    
    uint256 public constant keep = 200;
    uint256 public constant keepMax = 10000;
    bool public mintingEnabled = false;

    // Uniswap swap paths
    address[] public krill_quick_path;
    address[] public quick_krill_path;
    ...
    {
        otherToken = _otherToken; //either QUICK or USDC

        krill_quick_path = new address[](2);
        krill_quick_path[0] = krill;
        krill_quick_path[1] = QUICK;
        
        quick_krill_path = new address[](2);
        quick_krill_path[0] = QUICK;
        quick_krill_path[1] = krill;
    ...
    function harvest() public override {
        ...
        if (_harvested_balance > 0) {
            //Swap all Krill to Quick, then distribute fee
            _swapUniswapWithPath(krill_quick_path, _harvested_balance);
            uint256 quickBalance = IERC20(QUICK).balanceOf(address(this));
            uint256 feeAmount = quickBalance.mul(IERCFund(strategist).getFee()).div(keepMax);
            uint256 afterFeeAmount = quickBalance.sub(feeAmount);
            ...
            if(otherToken == usdc) {
                //Swap 1/2 back to Krill, 1/2 back to USDC
                uint256 half = afterFeeAmount.div(2);
                _swapUniswapWithPath(quick_krill_path, half);
                _swapUniswapWithPath(quick_usdc_path, half);
            }
            else {
                //Swap 1/2 back to Krill
                _swapUniswapWithPath(quick_krill_path, afterFeeAmount.div(2));
            }
        }
        
        //only krill - X pairs
        uint256 krillBalance = IERC20(krill).balanceOf(address(this));
        uint256 otherBalance = IERC20(otherToken).balanceOf(address(this));
        if (krillBalance > 0 && otherBalance > 0) {
            IERC20(krill).safeApprove(currentRouter, 0);
            IERC20(krill).safeApprove(currentRouter, krillBalance);
    ...
    //needs to be disabled while waiting on timelock
    function toggleAddyMinting(bool _enabled) external onlyOwner {
        mintingEnabled = _enabled;
    }
}


contract StrategyWhale2 is BaseStrategyWhale {

    string private pair_name;

    constructor(address lp, address tokenA, uint256 _poolId, address strategist, string memory _pair_name)
        public
        BaseStrategyWhale(
            lp,
            tokenA,
...
contract WhaleStrategyDeployer is Ownable {
    ...
    function deploy(address lp, address tokenA, uint256 _poolId, string memory _pair_name) public onlyOwner {
        address strategy = address(new StrategyWhale2(lp, tokenA, _poolId, FUND, _pair_name));
        address jar = address(new GenericVault(IStrategy(strategy), proxy_minter, FUND));

        StrategyWhale2(strategy).setJar(jar);
