...
//Deploys strategies for Quick/Other pairs that stake in the Dragon's Lair
contract dQuickStrategyDeployer is Ownable {
    ...
    constructor(address _proxy_minter)
        public
    {
        proxy_minter = _proxy_minter;
    }

    function deploy(address rewards, address lp, string memory _pair_name) public onlyOwner {
        address strategy = address(new StrategyDragonLair(rewards, lp, FUND, _pair_name));
        address jar = address(new DragonVault(IStrategy(strategy), proxy_minter, FUND));
        StrategyDragonLair(strategy).setJar(jar);
