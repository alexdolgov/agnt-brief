        ...
        returns (CalldataReader)
    {
        ...
        unchecked {
            for (; clipboardCount != 0; --clipboardCount) {
                ...
                // Requirements: check that the result index is within the results bounds
                ...
                // Requirements: check that the copy offset is within the result bounds
                require(copyOffset + WORD_SIZE <= result.length, CopyOffsetOutOfBounds());

                uint256 pasteOffset = clipboard & MASK_16_BIT;
                // Requirements: check that the paste offset is within the data bounds
