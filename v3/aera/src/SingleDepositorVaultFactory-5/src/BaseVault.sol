...
import { IMerkleRootProvider } from "./interfaces/IMerkleRootProvider.sol";
...
abstract contract BaseVault is IBaseVault, Pausable, CallbackHandler, ReentrancyGuardTransient, Auth {
    ...
    /// @notice Merkle root or provider address (if first 12 bytes are zero)
    bytes32 public root;
    ...
    /// @dev Must be called in all contracts that inherit from `BaseVault`
    ...
    function _initialize(address guardianAddress, IVaultHook hooksAddress, bytes32 root_) internal virtual {
    ...
    function _setRoot(bytes32 root_) internal {
        require(root_ != bytes32(0), Aera__ZeroAddressMerkleRoot());

        // If it's a IMerkleRootProvider address, validate by calling merkleRoot()
        if (!_isMerkleRoot(root_)) {
            IMerkleRootProvider(address(uint160(uint256(root_)))).merkleRoot();
        }
    ...
    function _getRoot() internal view returns (bytes32) {
        bytes32 rootValue = root;
        return _isMerkleRoot(rootValue)
            ? rootValue
            : IMerkleRootProvider(address(uint160(uint256(rootValue)))).merkleRoot();
    }
