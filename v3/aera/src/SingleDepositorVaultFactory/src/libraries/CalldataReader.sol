...
/// @notice No functions where changed, only added new functions
/// @author https://github.com/aera-finance
...
library CalldataReaderLib {
    ...
    function readBytes(CalldataReader self, uint256 length)
        internal
        pure
    ...
    function readBytesToMemory(CalldataReader self) internal pure returns (CalldataReader, bytes memory data) {
        ...
        assembly ("memory-safe") {
            callDataLength := shr(240, calldataload(self))
            self := add(self, 2)
        }
        ...
        assembly ("memory-safe") {
            calldatacopy(add(data, 32), self, callDataLength)
            self := add(self, callDataLength)
        }
