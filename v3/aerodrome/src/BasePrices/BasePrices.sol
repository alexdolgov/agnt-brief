...
import "IAeroPair.sol";
import {IERC20Metadata} from "IERC20Metadata.sol";
import "Sqrt.sol";
import "Address.sol";
import {Clones} from "Clones.sol";
import {IPoolFactory} from "IPoolFactory.sol";
import {IBasePrices} from "IBasePrices.sol";
...
contract BasePrices is IBasePrices {
    ...
    /// @param _factoryV2 Address of the factory contract for Aero pairs
    ...
    function _stableRate(IERC20Metadata t0, IERC20Metadata t1, int256 dec_diff) internal view returns (uint256 rate) {
        ...
        // newOut in t1
        try IAeroPair(currentPair).getAmountOut((10**t0_dec), address(t0)) returns (uint256 result) {
            newOut = result;
        } catch {
            return 0;
        }
        ...
        returns (uint256 srcBalance, uint256 dstBalance)
    {
        ...
        if (!Address.isContract(pairAddress)) {
            ...
            (uint256 reserve0, uint256 reserve1,) = IAeroPair(pairAddress).getReserves();
