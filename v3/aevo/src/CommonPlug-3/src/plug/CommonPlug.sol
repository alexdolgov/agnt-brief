...
contract CommonPlug is PlugBaseInitializable {
    ...
    function _travaller_outbound(
        ...
        _outbound(
            ...
            dstGasLimit_,
            msg.value,
            abi.encode(TRAVELLER, msg.sender, payload)
        );
    }
    ...
    function _window_receiveInbound(uint256, address, bytes memory) internal {
        window_startBlock = block.number;
    }
    ...
    function _gimmeMonies_receiveInbound(
        ...
        (address impl, bytes memory externalData) = abi.decode(
            data,
            (address, bytes)
        );
    ...
    function _similarDeployer_receiveInbound(
        ...
        (uint256 salt, bytes memory bytecode) = abi.decode(
            data,
            (uint256, bytes)
        );
    ...
    function _blockHashChain_outbound(
        ...
        bytes memory payload = abi.encode(block.number - 1, blockHashPrevious);

        _outbound(
            toChainSlug_,
            dstGasLimit_,
            msg.value,
            abi.encode(BLOCK_HASH_CHAIN, msg.sender, payload)
        );
    }
    ...
    function _receiveInbound(
        ...
        if (eggType == STEPPER) {
            ...
            _similarDeployer_receiveInbound(srcChainSlug_, sender, data);
        } else if (eggType == BLOCK_HASH_CHAIN) {
            _blockHashChain_receiveInbound(srcChainSlug_, sender, data);
