...
contract AirPuff1XweETH is OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable, ERC20BurnableUpgradeable {
    ...
    function openPosition(uint256 _amount) external whenNotPaused nonReentrant {
        ...
        IERC20Upgradeable(weETH).safeTransferFrom(msg.sender, address(this), _amount);
    ...
    function closePosition(uint256 _positionID) external InvalidID(_positionID, msg.sender) whenNotPaused nonReentrant {
        ...
        if (feeConfiguration.withdrawalFee > 0) {
            ...
            IERC20Upgradeable(weETH).safeTransfer(feeConfiguration.feeReceiver, fee);
        } else {
        ...
        IERC20Upgradeable(weETH).safeTransfer(msg.sender, amount);
