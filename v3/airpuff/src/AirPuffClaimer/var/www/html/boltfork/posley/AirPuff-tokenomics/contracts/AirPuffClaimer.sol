...
contract AirPuffClaimer is PausableUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    event WithdrawnToken(address token, uint256 amount, uint256 timestamp);
    ...
    function withdrawToken(address _token, uint256 _amount) external onlyOwner {
        ...
        emit WithdrawnToken(_token, _amount, block.timestamp);
    }
    ...
    function claim(uint256 _campaignID) public whenNotPaused nonReentrant {
        ClaimableInfo storage claimable = claimableInfo[_campaignID];
