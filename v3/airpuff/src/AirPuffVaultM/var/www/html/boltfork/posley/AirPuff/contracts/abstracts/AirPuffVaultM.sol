...
abstract contract AirPuffVaultM is IVault, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable, ERC20BurnableUpgradeable {
    ...
    uint256 public liquidatorRewardsPercent;
    ...
    event Withdraw(address indexed user, uint256 positionID, uint256 amount, uint256 leverage, uint256 repayAmount, uint256 time, uint256 userTokensBurned, uint256 totalInterests, uint256 originalInterests);
    ...
    event SetLiquidatorRewardPercentage(uint256 liquidatorRewardsPercent);

    /** ----------- Change onlyOwner functions ------------- */

    function setLiquidatorRewardPercentage(uint256 _liquidatorRewardsPercent) external onlyOwner {
        require(_liquidatorRewardsPercent <= MAX_BPS, "AirPuff: invalid liquidator reward percentage");
        liquidatorRewardsPercent = _liquidatorRewardsPercent;
        emit SetLiquidatorRewardPercentage(_liquidatorRewardsPercent);
    }
    ...
    function closePosition(uint256 _positionID) external InvalidID(_positionID, msg.sender) nonReentrant notTrxOrigin {
        ...
        if (amountOut < _userInfo.leverageAmount) {
            ...
            } else if (amountOut < _userInfo.leverageAmount + closeData.totalInterests && amountOut >= _userInfo.leverageAmount) {
                lendingRepayAmount = _userInfo.leverageAmount;
                closeData.totalInterests = amountOut - lendingRepayAmount;
                IERC20Upgradeable(strategyAddresses.WETH).safeTransfer(feeConfiguration.lendingFeeReceiver, closeData.totalInterests);
            } else {
                lendingRepayAmount = _userInfo.leverageAmount;
                closeData.toLeverageUser = amountOut - lendingRepayAmount - closeData.totalInterests;
                IERC20Upgradeable(strategyAddresses.WETH).safeTransfer(feeConfiguration.lendingFeeReceiver, closeData.totalInterests);
        ...
        emit Withdraw(_userInfo.user, positionID, closeData.toLeverageUser, _userInfo.leverage, lendingRepayAmount, block.timestamp, userTokensBurned, closeData.totalInterests, originalInterests);
    }
    ...
    function liquidatePosition(uint256 _positionID, address _user) external InvalidID(_positionID, _user) nonReentrant {
        ...
        if (amountOut < _userInfo.leverageAmount) {
                ...
                lendingRepayAmount = _userInfo.leverageAmount;
                amountOut = amountOut - lendingRepayAmount - closeData.totalInterests;
                IERC20Upgradeable(strategyAddresses.WETH).safeTransfer(feeConfiguration.lendingFeeReceiver, closeData.totalInterests);

                liquidatorReward = amountOut * liquidatorRewardsPercent  / MAX_BPS;
                IERC20Upgradeable(strategyAddresses.WETH).safeTransfer(_userInfo.liquidator, liquidatorReward);
                amountOut = amountOut - liquidatorReward;

                IERC20Upgradeable(strategyAddresses.WETH).safeTransfer(_userInfo.user, amountOut);
        }
