...
contract StakingMETH is ProtocolEvents, BareVaultUpgradable, LockupUpgradable, ReentrancyGuardUpgradeable {
    // errors
    error DepositAmountTooSmall(address receiver, uint256 amount, uint256 minStake);
    error UnacceptedNativeTokenTransfer();
    ...
    // --------- Initialize ---------- //
    /// @notice METHStaking Init params
    struct Init {
        address admin;
        address operator;
        address pauser;
    ...
    // reinitializer(2) indicate bounce upgrade once for testing
    function initialize(Init calldata init) external reinitializer(2) {
    ...
    function deposit(uint256 assets) public payable override nonReentrant returns (uint256) {
        if (msg.value != 0) {
            revert UnacceptedNativeTokenTransfer();
        }
