...
contract sVECVesting is Ownable {
    /// DEPENDENCIES ///
    ...
    /// @notice VEC token
    IERC20 internal constant VEC = IERC20(0x1BB9b64927e0C5e207C9DB4093b3738Eef5D8447);
    /// @notice sVEC Claim Token
    IERC20 internal constant sVEC = IERC20(0x66d5c66E7C83E0682d947176534242c9f19b3365);
    /// @notice Stake VEC for sVEC
    IStaking internal constant staking = IStaking(0xFdC28cd1BFEBF3033870C0344B4E0beE639be9b1);

    /// @notice Tracks address term info
    mapping(address => Term) public terms;
    /// @notice Tracks address change
    mapping(address => address) public walletChange;
    /// @notice Bool if address is paused
    ...
    function pushWalletChange(address _newAddress) external {
        require(terms[msg.sender].totalIndexAdjustedCanClaim != 0, "No wallet to change");
        require(!paused[msg.sender], "Paused");
    ...
    function pullWalletChange(address _oldAddress) external {
        ...
        require(terms[msg.sender].totalIndexAdjustedCanClaim == 0, "Wallet already exists");
        require(!paused[_oldAddress], "Paused");
    ...
    function redeemableFor(address _address) public view returns (uint256 _canRedeem) {
        Term memory info = terms[_address];
        uint256 totalReedemable = (info.totalIndexAdjustedCanClaim * percentVested(_address)) / 1e9;

        _canRedeem = fromIndexAdjusted(totalReedemable - info.indexAdjustedClaimed);
    }
    ...
    function setTerms(address _address, uint256 _vecToStakeAndVest, uint256 _vestLength) external onlyOwner {
        require(terms[_address].totalIndexAdjustedCanClaim == 0, "Address already exists");
    ...
    /// @notice Pause `_toPause`
    function pause(address _toPause) external onlyOwner {
        paused[_toPause] = true;
    }

    /// @notice Unpause `_toUnpause`
    function unpause(address _toUnpause) external onlyOwner {
        paused[_toUnpause] = false;
    }
}
