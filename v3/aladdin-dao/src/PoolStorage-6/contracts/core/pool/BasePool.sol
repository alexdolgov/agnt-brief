...
abstract contract BasePool is TickLogic, PositionLogic {
  ...
  function _redeem(uint256 rawDebts) internal returns (uint256 rawColls) {
    (uint256 cachedCollIndex, uint256 cachedDebtIndex) = _updateCollAndDebtIndex();
    ...
    while (debtShare > 0) {
      if (!hasDebt) {
        ...
        // If the tick won't move, break the loop
        if (!_tickWillMove(tick, collShareRedeemed, debtShareToRedeem)) break;

        _liquidateTick(tick, collShareRedeemed, debtShareToRedeem, price, false);
