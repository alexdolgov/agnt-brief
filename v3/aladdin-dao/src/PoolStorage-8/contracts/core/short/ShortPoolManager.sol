...
contract ShortPoolManager is ProtocolFees, FlashLoans, AssetManagement, IShortPoolManager {
  ...
  function updateRateProvider(address token, address provider) external onlyRole(OPERATOR_ROLE) {
  ...
  function updatePoolMinDebt(address pool, uint64 minDebt) external onlyRole(OPERATOR_ROLE) onlyRegisteredPool(pool) {
    _updatePoolMinDebt(pool, minDebt);
  }
