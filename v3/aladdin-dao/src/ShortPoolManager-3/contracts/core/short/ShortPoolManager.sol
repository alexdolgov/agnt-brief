...
contract ShortPoolManager is ProtocolFees, FlashLoans, AssetManagement, IShortPoolManager {
  ...
  function getMinDebt(address pool) external view returns (uint256 minDebt) {
    minDebt = _getPoolMinDebt(pool);
  }
  ...
  function operate(
    ...
    if (newDebt > 0) {
      _handleBorrow(pool, debtToken, uint256(newDebt), uint256(newRawDebt), vars.poolBorrowFeeRatio);
    } else if (newDebt < 0) {
      _handleRepay(pool, debtToken, uint256(-newDebt), uint256(-newRawDebt), vars.poolRepayFeeRatio);
    }
  ...
  function redeem(
    ...
    ILongPoolManager(counterparty).repay(IShortPool(pool).counterparty(), pool, rawDebts);
  ...
  function rebalance(
    ...
    _checkDebtValues(pool, maxRawDebts);
  ...
  function killPool(address pool) external onlyRegisteredPool(pool) onlyRole(POOL_KILLER_ROLE) {
    ...
    uint256 shortfall = ILongPoolManager(counterparty).liquidateShortPool(IShortPool(pool).counterparty(), pool, rawColls, rawDebts);
  ...
  function reduceDebt(
    ...
    ILongPoolManager(counterparty).repay(IShortPool(pool).counterparty(), pool, rawAmount);
  ...
  /// @param pool The address of fx pool.
  /// @return minDebt The minimum debt.
  function _getPoolMinDebt(address pool) internal view returns (uint256 minDebt) {
    minDebt = poolInfo[pool].debtData.decodeUint(DEBT_MIN_OFFSET, DEBT_MIN_BITS);
  }

  /// @dev Internal function to check the debt values.
  /// @param pool The address of fx pool.
  /// @param debts The debts to check.
  function _checkDebtValues(address pool, uint256 debts) internal view {
    uint256 minDebt = _getPoolMinDebt(pool);
    if (debts < minDebt) {
  ...
  function _handleRepay(
    ...
    unchecked {
      ...
      ILongPoolManager(counterparty).repay(IShortPool(pool).counterparty(), pool, rawDebtAmount);
    }
  }
  ...
  function _afterRebalanceOrLiquidate(
    ...
    ILongPoolManager(counterparty).repay(IShortPool(pool).counterparty(), pool, op.rawDebts);
