...
contract DCBLiqLocker is Initializable, OwnableUpgradeable, IStaking {
  ...
  function addLiquidityAndLock(
    ...
    IUniswapV2Pair pair = IUniswapV2Pair(poolInfo[_pid].inputToken);
  ...
  /**
   *
   * @dev claim accumulated TOKEN reward from all poolInfo
   *
   * Beware of gas fee!
   *
   */
  ...
  /**
   *
   * @dev get length of the poolInfo
   *
   * @return {uint256} length of the poolInfo
   *
   */
  ...
  /**
   *
   * @dev get all poolInfo info
   *
   * @return {Pool[]} length of the poolInfo
   *
   */
  ...
  function payout(uint16 _pid, address _addr) public view override returns (uint256 reward) {
    ...
    if (to > from) {
      reward = (to.sub(from)).mul(user.totalInvested).mul(pool.apy).div(1000).div(365 days);
  ...
  function _claim(uint16 _pid, address _user) internal returns (bool) {
    ...
    if (amount > 0) {
      _safeTOKENTransfer(pool.rewardToken, _user, amount);
