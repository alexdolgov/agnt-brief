...
library SmartRouterHelper {
    ...
    function getStableAmountsIn(
        ...
        for (uint256 i = length - 1; i > 0; i--) {
            ...
            amounts[last] = IStableSwapFactory(stableSwapFactory).get_dx(swapContract, k, j, amounts[i], type(uint256).max);
        }
    }
    ...
    bytes32 internal constant V2_INIT_CODE_HASH = 0x7ede5bbb7d245103c4a6d59bfd62246fbc488e93f95f23a19d9d76f0d91bd0d0; //V2 ALB
    ...
    /// @param deployer The Alien Base V3 deployer contract address
    ...
    function getPool(
        ...
        return IUniswapV3Pool(computeAddress(deployer, getPoolKey(tokenA, tokenB, fee)));
    }

    /// @notice Returns the address of a valid Alien Base V3 Pool
    /// @param deployer The contract address of the Alien Base V3 deployer
    ...
    {
        pool = IUniswapV3Pool(computeAddress(deployer, poolKey));
