...
abstract contract MultiToken is Context, IERC20, IERC20Metadata {
     ...
     * @return exact transferred amount.
     */
    function _transfer(address from, address to, uint amount) internal virtual returns (uint);
     ...
     * @return exact transferred amount.
     */
    function _subTransfer(address from, address to, uint amount, uint index) internal virtual returns (uint);
    ...
    function transferFrom(address from, address to, uint amount) public virtual override returns (bool) {
        ...
        uint virtualAmountChange = _transfer(from, to, amount);
        _spendAllowance(from, spender, virtualAmountChange);
        return true;
    }
    ...
    function subTransferFrom(address from, address to, uint amount, uint index) public virtual returns (bool) {
        address spender = _msgSender();
        uint virtualAmountChange = _subTransfer(from, to, amount, index);
        _spendAllowance(from, spender, virtualAmountChange);
        return true;
    }
