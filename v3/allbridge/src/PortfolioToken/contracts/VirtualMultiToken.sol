...
abstract contract VirtualMultiToken is MultiToken {
     ...
     * @return exact transferred virtual amount.
     */
    function _transfer(address from, address to, uint virtualAmount) internal virtual override returns (uint) {
        ...
        uint balanceBefore = balanceOf(from);
        ...
        uint balanceAfter = balanceOf(from);
     ...
     * @return exact transferred virtual amount.
     */
    function _subTransfer(address from, address to, uint virtualAmount, uint index) internal virtual override returns (uint) {
        require(to != address(0), "ERC20: transfer to the zero address");
        require(virtualAmount > 0, "ERC20: transfer zero amount");
