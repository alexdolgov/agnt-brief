  ...
  ReentrancyGuardUpgradeable
{
  ...
  event LogSetMaxCrossChainAmount(
    uint256 _oldMaxCrossChainAmount,
    uint256 _newMaxCrossChainAmount
  );
  ...
  function _fairLaunchHarvest() internal {
    ...
    (bool _success, ) = address(fairLaunch).call(
      abi.encodeWithSelector(0xddc63262, fairLaunchPoolId)
    );
    if (_success)
      emit LogFairLaunchHarvest(address(this), token.myBalance() - _before);
  }
  ...
  function forwardToken() external nonReentrant {
    _fairLaunchHarvest();
    uint256 _forwardAmount = token.myBalance() > maxCrossChainAmount
      ? maxCrossChainAmount
      : token.myBalance();
  ...
  function setAnyswapConfig(IAnyswapV4Router _anyswapRouter, address _anyToken)
    external
    onlyOwner
  {
  ...
  function setDestinationConfig(address _destination, uint64 _destChainId)
    external
    onlyOwner
  {
    destination = _destination;
  ...
  function setMaxCrossChainAmount(uint256 _newMaxCrossChainAmount)
    external
    onlyOwner
  {
    ...
    emit LogSetMaxCrossChainAmount(
      _oldMaxCrossChainAmount,
      maxCrossChainAmount
    );
  }
}
