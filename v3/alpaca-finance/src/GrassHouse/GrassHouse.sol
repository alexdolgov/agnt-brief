  ...
  OwnableUpgradeable
{
  ...
  event LogKilled();
  event LogSetWhitelistedCheckpointCallers(
    ...
    returns (uint256)
  {
    ...
    int128 _bias = _point.bias -
      _point.slope *
      SafeCastUpgradeable.toInt128(int256(_timestamp - _point.timestamp));
    if (_bias < 0) {
      return 0;
    }
    ...
    for (uint256 _i = 0; _i < 52; _i++) {
      ...
      if (block.timestamp < _nextWeekCursor) {
        if (_deltaSinceLastTimestamp == 0 && block.timestamp == _timeCursor) {
          ...
          tokensPerWeek[_thisWeekCursor] =
            tokensPerWeek[_thisWeekCursor] +
            ((_toDistribute * (block.timestamp - _timeCursor)) /
        ...
        if (_deltaSinceLastTimestamp == 0 && _nextWeekCursor == _timeCursor) {
          ...
          tokensPerWeek[_thisWeekCursor] =
            tokensPerWeek[_thisWeekCursor] +
            ((_toDistribute * (_nextWeekCursor - _timeCursor)) /
              _deltaSinceLastTimestamp);
        }
      }
  ...
  function checkpointToken() external nonReentrant {
    require(
      msg.sender == owner() ||
        ...
        (canCheckpointToken &&
          (block.timestamp > lastTokenTimestamp + TOKEN_CHECKPOINT_DEADLINE)),
      "!allow"
    );
    _checkpointToken();
  }
    ...
    for (uint256 _i = 0; _i < 52; _i++) {
      if (_weekCursor > _roundedTimestamp) {
        ...
        if (_weekCursor > _point.timestamp) {
          _timeDelta = SafeCastUpgradeable.toInt128(
            int256(_weekCursor - _point.timestamp)
          );
        }
  ...
  function _claim(address _user, uint256 _maxClaimTimestamp)
    internal
    returns (uint256)
  {
    ...
    Point memory _userPoint = IxALPACA(xALPACA).userPointHistory(
      _user,
      _userEpoch
    );
    ...
    Point memory _prevUserPoint = Point({
      bias: 0,
      slope: 0,
      timestamp: 0,
      blockNumber: 0
    ...
    for (uint256 _i = 0; _i < 52; _i++) {
      ...
      if (
        _userWeekCursor >= _userPoint.timestamp && _userEpoch <= _maxUserEpoch
      ) {
        ...
        int128 _timeDelta = SafeCastUpgradeable.toInt128(
          int256(_userWeekCursor - _prevUserPoint.timestamp)
        );
        uint256 _balanceOf = SafeCastUpgradeable.toUint256(
          Math128.max(
            _prevUserPoint.bias - _timeDelta * _prevUserPoint.slope,
            0
          )
        );
    ...
    returns (uint256)
  {
    ...
    if (
      canCheckpointToken &&
      (block.timestamp > _lastTokenTimestamp + TOKEN_CHECKPOINT_DEADLINE)
    ) {
      _checkpointToken();
  ...
  function claimMany(address[] calldata _users)
    external
    nonReentrant
    onlyLive
    returns (bool)
  ...
  function setCanCheckpointToken(bool _newCanCheckpointToken)
    external
    onlyOwner
  {
  ...
  function _timestampToFloorWeek(uint256 _timestamp)
    internal
    pure
    returns (uint256)
  {
  ...
  function injectReward(uint256 _timestamp, uint256 _amount)
    external
    onlyOwner
  {
    rewardToken.safeTransferFrom(msg.sender, address(this), _amount);
  ...
  function setWhitelistedCheckpointCallers(
