...
contract ProxyToken is IProxyToken, ERC20Upgradeable, OwnableUpgradeable {
  ...
  modifier onlyTimelock() {
    require(
      timelock == msg.sender,
      "proxyToken::onlyTimelock:: msg.sender not timelock"
    );
    _;
  }
  ...
  function setOkHolders(address[] memory _okHolders, bool _isOk)
    external
    override
    onlyOwner
  {
  ...
  function transfer(address to, uint256 amount) public override returns (bool) {
    require(
      ...
      "proxyToken::transfer:: unapproved holder on msg.sender"
    );
  ...
  function transferFrom(
    ...
    require(
      okHolders[from],
      "proxyToken::transferFrom:: unapproved holder in from"
    );
    require(
      okHolders[to],
      "proxyToken::transferFrom:: unapproved holder in to"
    );
