...
contract MasterChef is Ownable {
    ...
    function updateRoar(Roar _roar) external onlyOwner {
        roar = _roar;
    }
    ...
    function updateRewardFirstBlock(uint256 _blockNumber) public onlyOwner {
    ...
    function pendingKwik(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 roarReward = multiplier
                .mul(roarPerBlock)
                .mul(pool.allocPoint)
                .div(totalAllocPoint);
            accKwikPerShare = accKwikPerShare.add(
                roarReward.mul(1e12).div(lpSupply)
            );
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 kwikReward = multiplier
            .mul(roarPerBlock)
            .mul(pool.allocPoint)
            .div(totalAllocPoint);
        pool.accKwikPerShare = pool.accKwikPerShare.add(
            kwikReward.mul(1e12).div(lpSupply)
        );
        pool.lastRewardBlock = block.number;
        if (address(roar) != address(0)) {
            bool hasRole = roar.hasRole(roar.MINTER_ROLE(), address(this));
            if (hasRole) {
                roar.mint(address(this), kwikReward);
            }
        }
    }
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        updatePool(_pid);
        uint256 pending = user.amount.mul(pool.accKwikPerShare).div(1e12).sub(
            user.rewardDebt
        );
        safeKwikTransfer(msg.sender, pending);
    ...
    function getAllPoolUsers(uint256 _pid)
        public
        view
        returns (address[] memory)
    {
    ...
    function getAllPoolUserInfos(uint256 _pid)
        public
        view
        returns (address[] memory, uint256[] memory)
    {
        address[] memory userAddresses = new address[](
            poolUsers[_pid].length()
        );
        uint256[] memory amounts = new uint256[](poolUsers[_pid].length());
        for (uint256 i = 0; i < poolUsers[_pid].length(); i++) {
            userAddresses[i] = poolUsers[_pid].at(i);
            amounts[i] = userInfo[_pid][poolUsers[_pid].at(i)].amount;
        }
