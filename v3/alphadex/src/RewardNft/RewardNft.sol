...
// File contracts/rewardnft.sol

pragma solidity ^0.8.4;

contract RewardNft is ERC721, ERC721Enumerable, AccessControl {
    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");
    string public baseURI;
    uint256 public maxSupply;
    uint256 public maxPerTrx;
    ...
    function setMaxPerTrx(uint256 _maxPerTrx)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        require(_maxPerTrx > 0 && _maxPerTrx <= maxSupply, "Invalid input");
        maxPerTrx = _maxPerTrx;
    }
    ...
    function mint(address _to, uint256 _numberOfTokens)
        public
        onlyRole(MINTER_ROLE)
    {
        require(
            _numberOfTokens <= maxPerTrx,
            "ERC721: no of tokens exceeds max per trx"
        );
        require(
            totalSupply() + _numberOfTokens <= maxSupply,
            "ERC721: minting would exceed total supply"
        );
