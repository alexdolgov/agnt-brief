...
abstract contract VaultCoreV1 is
    ...
    IVaultCoreV1
{
    /// @dev Pause the protocol with Ingress.setProtocolPause to stop deposit, withdraw, borrow, claim
    /// @dev If updating the users in batches, also:
    /// @dev Pause validateRepay, validateRebalance, validateCommit with Ingress.setFunctionsPause
    function _updatePrecision() internal {
        uint256 debtIndex = debtToken.interestIndex();
        require(debtIndex < 1e27, "ALREADY_UPDATED");
    ...
    function updateUserPrecision(address[] calldata users) public {
        require(IIngress(ingressControl).hasRole(keccak256("ALPHA"), msg.sender), "NOT_ALPHA");

        // Update the precision of the interest indexes and snapshot indexes
        _updatePrecision();
        ...
        for (uint i; i < users.length; i++) {
            // Passing the already stored balance, while setting the user index.
            ...
            debtToken.setBalance(users[i], debtToken.balanceStored(users[i]), debtToken.userIndex(users[i]) * 1e21);

            (uint debt, , ) = debtToken.balanceOfDetails(users[i]);
            totalDebt += debt;
            totalSupply += supplyToken.balanceOf(users[i]);
        }

        // Calculate debt difference (in basis points)
        uint256 debtDifference = totalDebt > debtToken.totalSupply()
            ? totalDebt - debtToken.totalSupply()
            : debtToken.totalSupply() - totalDebt;
        debtDifference = (debtDifference * 1e4) / debtToken.totalSupply();

        // Calculate supply difference (in basis points)
        uint256 supplyDifference = totalSupply > supplyToken.totalSupply()
            ? totalSupply - supplyToken.totalSupply()
            : supplyToken.totalSupply() - totalSupply;
        supplyDifference = (supplyDifference * 1e4) / supplyToken.totalSupply();

        // Assert the percentage difference is below threshold of 1 basis points
        assert(debtDifference == 0); // Allow up to 1 basis points (0.01%) difference
        assert(supplyDifference == 0); // Allow up to 1 basis points (0.01%) difference
    }
