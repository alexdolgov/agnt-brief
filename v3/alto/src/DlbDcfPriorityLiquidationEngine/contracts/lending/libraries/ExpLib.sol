...
int256 constant MATH_PRECISION_INT = int256(MATH_PRECISION);
...
library ExpLib {
    ...
    /// @dev This upper bound corresponds to: ln(type(int256).max / 1e36) (scaled by MATH_PRECISION_INT, floored).
    ...
    function wExp(int256 x) internal pure returns (int256) {
        unchecked {
            ...
            int256 expR = MATH_PRECISION_INT + r + (r * r) / MATH_PRECISION_INT / 2;
