...
/* - 2026-01-16: Removed `SetName` and `SetSymbol` events     */
/*                                                            */
/*.•°:°.´+˚.*°.˚:*.´•*.+°.•°:´*.´•*.•°.•°:°.´:•˚°.*°.˚:*.´+°.•*/
...
library EventsLib {
    ...
    /// @notice Emitted `protocolFee` is set to `newProtocolFee`.
    event SetProtocolFee(address indexed caller, uint256 newProtocolFee);

    /// @notice Emitted when a new `newProtocolFeeRecipient` is set.
    ...
    /// @param protocolFeeShares The shares minted to the protocol fee recipient.
    event AccrueInterest(uint256 newTotalAssets, uint256 feeShares, uint256 protocolFeeShares);
