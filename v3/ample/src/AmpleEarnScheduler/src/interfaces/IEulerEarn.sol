...
/* - 2026-01-16: Removed setName and setSymbol functions        */
/*                                                              */
/*.•°:°.´+˚.*°.˚:*.´•*.+°.•°:´*.´•*.•°.•°:°.´:•˚°.*°.˚:*.´+°.•* */
...
interface IEulerEarnBase {
    ...
    /// @dev The protocol fee is calculated as a percentage of the fee shares that would otherwise go to the fee recipient.
    /// When fees are accrued, a portion of the fee shares equal to `protocolFee / WAD` is minted to the protocol fee
    /// recipient, and the remaining fee shares are minted to the fee recipient. If `fee()` is 0, then no fee shares
    /// are generated, so the protocol fee will have no effect regardless of its value.
    ...
    /// @notice Sets `protocolFee` to `newProtocolFee`.
    function setProtocolFee(uint256 newProtocolFee) external;

    /// @notice Sets `protocolFeeRecipient` to `newProtocolFeeRecipient`.
