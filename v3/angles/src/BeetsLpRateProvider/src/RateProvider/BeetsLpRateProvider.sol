...
import {IRateProvider} from "../../lib/vaults/src/RateProvider/IRateProvider.sol";
...
contract BeetsLpRateProvider is IRateProvider {
    ...
    function rate(address token) external view returns (uint256) {
        if (token == BPT) {
            (, bytes memory data) = BPT.staticcall(abi.encodeWithSelector(bytes4(keccak256("getRate()"))));
