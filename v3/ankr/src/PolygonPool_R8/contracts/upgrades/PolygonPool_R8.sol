...
contract PolygonPool_R8 is PausableUpgradeable, ReentrancyGuardUpgradeable, OwnableUpgradeable, IGlobalPool_R2 {
    ...
    // @dev deprecated - PolClaimPending is emitted instead
    event MaticClaimPending(address indexed claimer, uint256 amount);
    event PolClaimPending(address indexed claimer, uint256 amount);
    ...
    event PolygonTokenAddressChanged(address oldToken, address newToken);
    ...
    /// @custom:oz-renamed-from _pendingMaticClaimGap
    uint256 private _pendingPolClaimGap;
    /// @custom:oz-renamed-from _maticToken
    IERC20Upgradeable private _polygonToken;
    ...
    function initialize(address operator, address polygonToken, address ankrToken, address feeCollector) public initializer {
        __Pausable_init();
        ...
        _notary = operator;
        _minimumStake = 1e18;
        _polygonToken = IERC20Upgradeable(polygonToken);
    ...
    function _stake(address staker, uint256 amount, bool isRebasing) internal nonReentrant {
        ...
        require(_polygonToken.transferFrom(staker, address(this), amount), "failed to receive POL");
    ...
    function getPendingStakes() external view returns (address[] memory, uint256[] memory) {
    ...
    function getRawPendingStakes() external view returns (address[] memory, uint256[] memory) {
    ...
    function claimToIntermediary(address payable intermediary, uint256 threshold) public onlyOperator {
        ...
        require(_polygonToken.transfer(intermediary, total), "POL not sent");
        /* decrease arrays */
    ...
    // @dev deprecated - use pendingPolClaimsOf
    function pendingMaticClaimsOf(address claimer) external view returns (uint256) {
        uint256 claimsTotal;
        for (uint256 i = _pendingPolClaimGap; i < _pendingClaimers.length; i++) {
    ...
    function pendingPolClaimsOf(address claimer) external view returns (uint256) {
        uint256 claimsTotal;
        for (uint256 i = _pendingPolClaimGap; i < _pendingClaimers.length; i++) {
            if (_pendingClaimers[i] == claimer) {
                claimsTotal += _pendingClaimAmounts[i];
    ...
    function getPendingClaims() external view returns (address[] memory, uint256[] memory) {
        address[] memory addresses = new address[](_pendingClaimers.length.sub(_pendingPolClaimGap));
        uint256[] memory amounts = new uint256[](_pendingClaimers.length.sub(_pendingPolClaimGap));
        uint256 j = 0;
        for (uint256 i = _pendingPolClaimGap; i < _pendingClaimers.length; i++) {
    ...
    function getRawPendingClaims() external view returns (address[] memory, uint256[] memory) {
    ...
    function _unstake(
        ...
        emit PolClaimPending(recipient, amount);
    ...
    function serveClaims(uint256 amountToUse, address payable residueAddress, uint256 minThreshold) public onlyOperator payable {
        address[] memory claimers = new address[](_pendingClaimers.length.sub(_pendingPolClaimGap));
        uint256[] memory amounts = new uint256[](_pendingClaimers.length.sub(_pendingPolClaimGap));
        uint256 availableAmount = _polygonToken.balanceOf(address(this));
        availableAmount = availableAmount.sub(_getTotalPendingStakes());
        require(amountToUse <= availableAmount, "not enough POL tokens to serve claims");
        if (amountToUse > 0) {
        ...
        uint256 gaps = 0;
        uint256 i = 0;
        for (i = _pendingPolClaimGap; i < _pendingClaimers.length && gasleft() > _SERVE_GAS_LEFT; i++) {
            ...
            require(_polygonToken.transfer(wallet, amount), "cannot send POL to claimer");
        ...
        _pendingPolClaimGap = _pendingPolClaimGap.add(gaps);
        uint256 missing = 0;
        for (i = _pendingPolClaimGap; i < _pendingClaimers.length; i++) {
            missing = missing.add(_pendingClaimAmounts[i]);
        }
        /* Send event with results */
        if (availableAmount > 0) {
            require(_polygonToken.transfer(residueAddress, availableAmount), "cannot send POL to residue");
        }
        /* decrease arrays */
    ...
    function updatePolygonTokenAddress(address newAddress) external onlyOwner {
        require(newAddress != address(0), "PolygonPool: polygon token address must not be zero");
        emit PolygonTokenAddressChanged(address(_polygonToken), newAddress);
        _polygonToken = IERC20Upgradeable(newAddress);
    }
}
