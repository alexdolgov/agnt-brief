...
contract ValidatorManager_R1 is OwnableUpgradeable, IValidatorManager, ReentrancyGuardUpgradeable {
    ...
    struct RewardPoolData {
        ...
        uint64 extraCommissionStashed; // extra commission of provider
        uint16 extraCommission;
    }
    ...
    function publishPendingRewards(address[] calldata providers, uint64[] calldata rewards) external onlyConsensus {
        ...
        for (uint256 i = 0; i < providers.length; i++) {
            RewardPoolData storage pool = _pools[providers[i]];
            uint64 prevReward = pool.pendingReward + pool.extraCommissionStashed;

            require(prevReward <= rewards[i], "ValidatorManager: reward cannot be decreased");

            uint64 newReward = rewards[i] - prevReward;
            uint64 fee = newReward * pool.extraCommission / MAX_COMMISSION + pool.extraCommissionStashed;
            uint64 rewardWithoutFee = rewards[i] - fee;

            pool.extraCommissionStashed = fee;
            pool.pendingReward = rewardWithoutFee;

            emit PendingRewardPublished(providers[i], rewardWithoutFee, fee);
        }
    }
    ...
    function setExtraCommission(address provider, uint16 commission) external onlyConsensus {
        require(commission <= MAX_COMMISSION, "ValidatorManager: commission too big");
        emit ExtraCommissionChanged(provider, _pools[provider].extraCommission, commission);
        _pools[provider].extraCommission = commission;
    }
}
