...
contract aMATICb_R8 is OwnableUpgradeable, ERC20BondBase, IinternetBond_R3 {
    ...
    uint256 private _ratio;  // deleted
    ...
    function _bondsToShares(uint256 amount) internal view returns (uint256) {
        return safeCeilMultiplyAndDivide(amount, ratio(), 1e18);
    }

    function _sharesToBonds(uint256 amount) internal view returns (uint256) {
        return safeFloorMultiplyAndDivide(amount, 1e18, ratio());
    }
    ...
    function setInternetBondRatioFeed(address newValue) external onlyOwner {
        require(newValue != address(0), "aMATICb: address can not be null");
