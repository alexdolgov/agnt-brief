...
import "../../interfaces/IAVotingEscrowMainchain.sol";
...
contract VotingEscrowAnzenMainchain is VotingEscrowTokenBase, IAVotingEscrowMainchain, AnzenMsgSenderAppUpg {
    ...
    // to ask for their veAnzen balance at any wTime
    ...
    ) initializer AnzenMsgSenderAppUpg(_anzenMsgSendEndpoint, initialApproxDestinationGas) {
        anzen = _anzen;
     ...
     * @param additionalAmountToLock anzen amount to be pulled in from user to lock.
    ...
    function increaseLockPosition(
        ...
        if (additionalAmountToLock > 0) {
            anzen.safeTransferFrom(user, address(this), additionalAmountToLock);
        }
    ...
    /**
     * @notice Withdraws an expired lock position, returns locked ANZEN back to user
     * @dev reverts if position is not expired, or if no locked ANZEN to withdraw
    ...
    function withdraw() external returns (uint128 amount) {
        ...
        anzen.safeTransfer(user, amount);
     ...
     */
    function totalSupplyCurrent() public virtual override(IAVeToken, VotingEscrowTokenBase) returns (uint128) {
