...
abstract contract StarknetTokenStorage {
    ...
    // string internal constant HALT_STATE_TAG = "STARKNET_TOKEN_BRIDGE_HALT_STATE";
    string internal constant CLEAREANCE_ADDRESS_TAG = "STARKNET_TOKEN_BRIDGE_CLEAREANCE_ADDRESS";

    // Web3.keccak(text="HALT_STATE_TAG").
    bytes32 constant HALT_STATE_SLOT =
        0x3017c368677450501824dff2d9eb0a5a2992d9c9eae7ad957ca4d956433aa873;
    ...
    enum HaltState {
        // TokenActive is the default state, this must remain first.
        TokenActive,
        HaltSignalled,
        HaltCompleted
    }
    ...
    function haltState() internal pure returns (mapping(address => HaltState) storage _haltState) {
        assembly {
            _haltState.slot := HALT_STATE_SLOT
        }
    }
    ...
    function setHaltState(address token, HaltState state) internal {
        haltState()[token] = state;
    }
}
