...
contract IFOInitializable is IIFOV2, ReentrancyGuard, Ownable, Whitelist {
     ...
     * @param _maxBufferTimes: maximum buffer of blocks from the current block number
    ...
    function initialize(
        ...
        endTime = _endTime;
        MAX_BUFFER_BLOCKS = _maxBufferTimes;
    ...
    function updateStartAndEndTimes(uint256 _startTime, uint256 _endTime) external onlyOwner {
        require(_endTime < (block.timestamp + MAX_BUFFER_BLOCKS), "Operations: EndTime too far");
        require(block.timestamp < startTime, "Operations: IFO has started");
