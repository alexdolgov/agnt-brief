...
import {
    IAerodromeStandardModulePrivateFees
} from "../interfaces/IAerodromeStandardModulePrivateFees.sol";
import {IArrakisLPModule} from "../interfaces/IArrakisLPModule.sol";
import {
    IArrakisLPModuleID
} from "../interfaces/IArrakisLPModuleID.sol";
...
import {UnderlyingAerodrome} from "../libraries/UnderlyingAerodrome.sol";
...
/// @notice Aerodrome CL module that collects LP trading fees in token0/token1.
/// @dev Mirrors UniswapV3StandardModule with Aerodrome-specific interface adaptations:
///   - slot0() returns 6 values (no feeProtocol)
///   - ticks() returns 10 values (extra stakedLiquidityNet and rewardGrowthOutsideX128)
///   - Factory uses int24 tickSpacing instead of uint24 fee
abstract contract AerodromeStandardModule is
    ReentrancyGuardUpgradeable,
    PausableUpgradeable,
    IArrakisLPModule,
    IArrakisLPModuleID,
    IAerodromeStandardModulePrivateFees
{
    ...
    /// for initializing the Aerodrome CL standard module.
    /// @param tickSpacing_ tick spacing of the Aerodrome CL pool.
    /// @param oracle_ address of the oracle used by the module.
    /// @param maxSlippage_ allowed to manager for rebalancing the inventory using swap.
    ...
    function initialize(
        ...
        // Aerodrome CL doesn't support native coin
        if (
        ...
        pool = IUniswapV3Factory(factory)
            .getPool(address(token0), address(token1), tickSpacing_);
    ...
    /// @inheritdoc IAerodromeStandardModulePrivateFees
    function approve(
        address spender_,
        ...
        for (uint256 i; i < length; i++) {
            ...
            uint256 allowance = IERC20Metadata(token)
                .allowance(address(this), spender_);
    ...
    function setPool(
        ...
        address pool_ = IUniswapV3Factory(factory)
            .getPool(address(token0), address(token1), tickSpacing_);
    ...
    /// @param proportion_ proportion of overall position to withdraw.
        ...
        returns (uint256 amount0, uint256 amount1)
    {
        ...
        for (uint256 i; i < length; i++) {
            ...
            bytes32 positionId = UnderlyingAerodrome.getPositionId(
    ...
    function getRanges() public view returns (Range[] memory ranges) {
        ranges = _ranges;
    }
        ...
        returns (uint256 amount0, uint256 amount1)
    {
        ...
        (amount0, amount1,,) = UnderlyingAerodrome.totalUnderlyingWithFees(
            UnderlyingPayloadV3({
                ranges: _ranges,
                pool: pool,
                self: address(this),
    ...
    function totalUnderlyingAtPrice(
        ...
        (amount0, amount1,,) =
            UnderlyingAerodrome.totalUnderlyingAtPriceWithFees(
                UnderlyingPayloadV3({
                    ranges: _ranges,
                    pool: pool,
                    self: address(this),
    ...
    function validateRebalance(
        ...
        IERC20Metadata _token0 = token0;
        IERC20Metadata _token1 = token1;
        // Aerodrome CL: slot0 returns 6 values (no feeProtocol)
    ...
    function _managerBalance(
        ...
        (,, uint256 fee0, uint256 fee1) = UnderlyingAerodrome.totalUnderlyingWithFees(
            UnderlyingPayloadV3({
                ranges: _ranges,
                pool: pool,
                self: address(this),
        ...
        if (isToken0_) {
            return FullMath.mulDiv(fee0, managerFeePIPS, PIPS);
        } else {
            return FullMath.mulDiv(fee1, managerFeePIPS, PIPS);
        }
    }
        ...
        for (uint256 i; i < length; i++) {
            ...
            if (_activeRanges[positionId]) {
                ...
                if (burn.liquidity == liquidity) {
                    (, uint256 index) =
                        UnderlyingAerodrome.rangeExists(_ranges, burn.range);
        ...
        if (zeroForOne_) {
            ...
            if (
                FullMath.mulDiv(
                    ...
                    < FullMath.mulDiv(price, PIPS - maxSlippage, PIPS)
            ) revert ExpectedMinReturnTooLow();
        } else {
            uint256 price = oracle.getPrice1();
    ...
    function _checkRebalanceSlippage(
        Rebalance calldata rebalance_,
        uint256 amount0Minted,
        uint256 amount1Minted,
        uint256 amount0Burned,
