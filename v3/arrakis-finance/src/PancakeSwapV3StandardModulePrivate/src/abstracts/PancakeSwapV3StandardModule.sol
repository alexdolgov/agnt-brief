    ...
    IERC721Receiver
{
        ...
        if (params_.swapPayload.amountIn > 0) {
            ...
            if (
                params_.swapPayload.router == address(metaVault)
                    ...
                    || params_.swapPayload.router == address(token0)
                    || params_.swapPayload.router == address(token1)
            ) {
                revert WrongRouter();
            }
        ...
        returns (uint256 amount0, uint256 amount1)
    {
        ...
        return _totalUnderlying(sqrtPriceX96, tick);
    }
    ...
    function totalUnderlyingAtPrice(
        ...
        return _totalUnderlying(priceX96_, tick);
    }
    ...
    function validateRebalance(
        ...
        if (oraclePrice == 0) revert SwapNotSupported();
        ...
        )
    {
...

        {
            ...
            if (liquidity == 0) {
                ...
                return (0, 0, 0, 0, cakeAmountCollected);
            }
    ...
    function _mint(
        ...
        if (params_.recipient != address(this)) {
            revert RecipientNotModule();
        }
    ...
    function _checkMinReturn(
        ...
        if (zeroForOne_) {
            uint256 oraclePrice = oracle.getPrice0();
            ...
            if (
                FullMath.mulDiv(
                    ...
                    < FullMath.mulDivRoundingUp(
                        oraclePrice, PIPS - maxSlippage, PIPS
                    )
            ) revert ExpectedMinReturnTooLow();
        } else {
            uint256 oraclePrice = oracle.getPrice1();
