...
} from "../interfaces/IUniswapV3StandardModule.sol";
...
import {
    IUniswapV3PoolVariant
} from "../interfaces/IUniswapV3PoolVariant.sol";
import {
    IUniswapV3FactoryVariant
} from "../interfaces/IUniswapV3FactoryVariant.sol";
import {
    PIPS,
    BASE,
    NATIVE_COIN,
    ...
    IUniswapV3StandardModule
{
    ...
    function initialize(
        ...
        pool = IUniswapV3FactoryVariant(factory)
            .getPool(address(token0), address(token1), fee_);
    ...
    function setPool(
        ...
        address pool_ = IUniswapV3FactoryVariant(factory)
            .getPool(address(token0), address(token1), fee_);
    ...
    function totalUnderlyingAtPrice(
        ...
        (amount0, amount1,,) =
            UnderlyingV3.totalUnderlyingAtPriceWithFees(
                UnderlyingPayloadV3({
                    ranges: _ranges,
                    pool: pool,
                    self: address(this),
    ...
    function _managerBalance(
        ...
        (,, uint256 fee0, uint256 fee1) = UnderlyingV3.totalUnderlyingWithFees(
            UnderlyingPayloadV3({
                ranges: _ranges,
                pool: pool,
                self: address(this),
