    ...
    AccessControlUpgradeable
{
    ...
    function getUserWithdrawRequest(
        ...
        require(
            _nonce < userWithdrawRequestsNonce[msg.sender].length(),
            "AMTWithdrawalManager: INVALID_INDEX"
        );
        return
            withdrawRequests[userWithdrawRequestsNonce[msg.sender].at(_nonce)];
    }

    function getUserWithdrawRequestLength() external view returns (uint256) {
        return userWithdrawRequestsNonce[msg.sender].length();
    }
