...
contract IchiSwapXCLPAdapter is IAdapter, Ownable2Step {
    ...
    bool allowPeg; // if true, allow pegCoin to be added; else allow stableCoin to be added
    uint256 decimalDiff;
    ...
    function addLiquidity(uint256 _amountPeg, uint256 _amountStable, uint256 _minAmountLP) external override onlyAMO onlyCalm {
        // 0 - input validation
        // Note: only add one side of the pair is allowed
        bool _allowPeg = allowPeg;
        bool _pegIsZero = pegIsZero;
        if (allowPeg) {
            require(_amountStable == 0, "Only one side of the pair is allowed");
            require(IERC20(pegCoin).balanceOf(address(this)) >= _amountPeg, "No Enough PegCoin");
        } else {
            require(_amountPeg == 0, "Only one side of the pair is allowed");
        ...
        if (_pegIsZero) {
            ...
            if (_allowPeg) {
                ...
                amount0 = _amountStable;
                amount1 = 0;
            }
        }
        uint balOfLPBefore = IERC20(vault).balanceOf(address(this));
        if (_allowPeg) {
            IERC20(pegCoin).safeIncreaseAllowance(vault, _amountPeg);
        } else {
            IERC20(stableCoin).safeIncreaseAllowance(vault, _amountStable);
        }
        IichiVault(vault).deposit(amount0, amount1, address(this));
    ...
    function _redeemLiquidity(uint256 _amountLP, uint256, uint256) internal {
