...
contract Vesting  {
    ...
    uint256 public LINEAR = 2 * 86400 * 365;
    uint256 public NOT_CLAIMABLE_TIME = 86400 * 7;
    ...
    constructor() {
        owner = msg.sender;
        token = address(0xd1DA314f7db27DbE3c61aC172bE13d86e67688B7);
    }

    bool initialized = false;

    // init distribution
    function init(address[] memory who, uint[] memory amounts) external onlyOwner {
    ...
    function claim() external returns(uint) {
        ...
        require(block.timestamp >= startTimestamp + NOT_CLAIMABLE_TIME, 'claim later');
        
        uint256 dt = block.timestamp - _timestamp;
        if (block.timestamp >= startTimestamp + LINEAR) {
            dt = startTimestamp + LINEAR - _timestamp;
        }
        require(dt > 0);
    ...
    function withdrawAll(address _token) external onlyOwner {
        IERC20(_token).safeTransfer(owner, IERC20(token).balanceOf(address(this)));
    }
