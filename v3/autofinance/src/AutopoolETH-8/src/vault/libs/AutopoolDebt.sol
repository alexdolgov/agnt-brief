...
library AutopoolDebt {
    ...
    function processRebalance(
        ...
        updates.newDebt = _calculateDebt(updates.startingDebt, result.totalDebtIncrease, result.totalDebtDecrease);

        $.assetBreakdown.totalIdle = updates.newIdle;
        $.assetBreakdown.totalDebt = updates.newDebt;
        $.assetBreakdown.totalDebtMin =
            _calculateDebt($.assetBreakdown.totalDebtMin, result.totalMinDebtIncrease, result.totalMinDebtDecrease);
        $.assetBreakdown.totalDebtMax =
            _calculateDebt($.assetBreakdown.totalDebtMax, result.totalMaxDebtIncrease, result.totalMaxDebtDecrease);
    }
    ...
    function updateDebtReporting(
        ...
        changes.newDebt = _calculateDebt(changes.startingDebt, result.totalDebtIncrease, result.totalDebtDecrease);
    ...
    // In the case that our debt decrease is more than we'd expect, set to zero to avoid underflow
    function _calculateDebt(
        uint256 autopoolStoredDebt,
        uint256 debtIncrease,
        uint256 debtDecrease
    ) private returns (uint256 newDebtValue) {
        if (autopoolStoredDebt + debtIncrease > debtDecrease) {
            newDebtValue = autopoolStoredDebt + debtIncrease - debtDecrease;
        } else {
            newDebtValue = 0;
        }
    }
