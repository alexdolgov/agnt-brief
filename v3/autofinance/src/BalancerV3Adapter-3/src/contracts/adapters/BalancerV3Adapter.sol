...
contract BalancerV3Adapter {
    ICompositeLiquidityRouter public compositeRouter;

    IRouter public router;

    IAllowanceTransfer public permit2;

    IVaultExplorer public vaultExplorer;
    ...
    function addLiquidity(
        ...
        if (isBoosted) {
            compositeRouter.addLiquidityUnbalancedToERC4626Pool(pool, amountsIn, minAmountOut, wethIsEth, userData);
        } else {
    ...
    function removeLiquidity(
        ...
        if (isBoosted) {
            compositeRouter.removeLiquidityProportionalFromERC4626Pool(
                pool, burnAmountLP, minAmountsOut, wethIsEth, userData
            );
        } else {
