...
import {
    ...
    PoolSwapParams,
    HooksConfig
} from "src/external/balancer/v3/VaultTypes.sol";
...
interface IVaultExplorer {
    ...
    function getTokenDelta(
        IERC20 token
    ) external view returns (int256 tokenDelta);
     ...
     */
    function getAddLiquidityCalledFlag(
        address pool
    ) external view returns (bool liquidityAdded);
    ...
    function getCurrentLiveBalances(
        address pool
    ) external view returns (uint256[] memory balancesLiveScaled18);
    ...
    function getPoolConfig(
        address pool
    ) external view returns (PoolConfig memory poolConfig);
    ...
    function totalSupply(
        address token
    ) external view returns (uint256 tokenTotalSupply);
     ...
     */
    function isPoolPaused(
        address pool
    ) external view returns (bool poolPaused);
    ...
    function getStaticSwapFeePercentage(
        address pool
    ) external view returns (uint256 swapFeePercentage);
    ...
    function getPoolRoleAccounts(
        address pool
    ) external view returns (PoolRoleAccounts memory roleAccounts);
    ...
    function isPoolInRecoveryMode(
        address pool
    ) external view returns (bool inRecoveryMode);
     ...
     */
    function getBufferTotalShares(
        IERC4626 wrappedToken
    ) external view returns (uint256 bufferShares);
