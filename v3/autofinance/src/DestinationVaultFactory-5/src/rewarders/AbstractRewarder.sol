...
abstract contract AbstractRewarder is IBaseRewarder, SecurityBase {
    ...
    constructor(
        ...
        AutopilotErrors.verifyNotZero(_rewardToken, "_rewardToken");
        AutopilotErrors.verifyNotZero(_duration, "_duration");
        AutopilotErrors.verifyNotZero(_newRewardRatio, "_newRewardRatio");
        AutopilotErrors.verifyNotZero(_rewardRole, "_rewardRole");

        systemRegistry = _systemRegistry;
        if (!systemRegistry.isRewardToken(_rewardToken)) {
            revert AutopilotErrors.InvalidParam("_rewardToken");
        }
    ...
    function notifyRewardAmount(
        ...
        if (rewardRate <= 0) revert AutopilotErrors.ZeroAmount();
    ...
    function setTokeLockDuration(
        ...
        if (_tokeLockDuration > 0) {
            AutopilotErrors.verifyNotZero(address(systemRegistry.accToke()), "accToke");
    ...
    function addToWhitelist(
        ...
        AutopilotErrors.verifyNotZero(wallet, "wallet");
    ...
    function _getReward(address account, address recipient) internal {
        AutopilotErrors.verifyNotZero(account, "account");
        AutopilotErrors.verifyNotZero(recipient, "recipient");
    ...
    function recover(address token, address recipient) external override hasRole(Roles.TOKEN_RECOVERY_MANAGER) {
        ...
        AutopilotErrors.verifyNotZero(recipient, "recipient");
        if (recipient == address(this)) revert AutopilotErrors.InvalidAddress(recipient);

        if (!canTokenBeRecovered(token)) revert AutopilotErrors.AssetNotAllowed(token);
        //slither-disable-next-line timestamp
