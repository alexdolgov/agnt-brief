...
contract ERC4626NonLPSpotEthOracle is SystemComponent, SecurityBase, ISpotPriceOracle {
    ...
    /// @param pools LP Pool to lookup price in
    ...
    function registerPool(
        ...
        emit TokenLookupPoolRegistered(baseAsset, pools);

        tokenLookupPools[baseAsset] = LookupInfo({ pools: pools, remapAsset: mapLookup == address(0) ? 0 : 1 });
    ...
    function _getSpotPrice(
        ...
        // Avg spot price of the vaults base asset in terms of the requested quote
        for (uint256 i = 0; i < poolLen;) {
            price += _getSpotPriceInQuote(rootOracle, baseAsset, lookupInfo.pools[i], requestedQuoteToken);
            unchecked {
                ++i;
            }
        }
