...
abstract contract LSTCalculatorBase is ILSTStats, BaseStatsCalculator {
    ...
    uint256 public constant APR_FILTER_INIT_INTERVAL_IN_SEC = 2 * 24 * 60 * 60; // 2 days
    ...
    function current() external returns (LSTStatsData memory) {
        ...
        // feeApr is 0 until the filter is initialized so we'll just call 0 current
        return LSTStatsData({
            lastSnapshotTimestamp: baseAprFilterInitialized ? lastSnapshotTimestamp : block.timestamp,
            baseApr: baseApr,
