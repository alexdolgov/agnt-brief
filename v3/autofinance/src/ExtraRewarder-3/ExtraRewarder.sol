...
library SSTORE2 {
    ...
    function predictDeterministicAddress(
        bytes32 salt,
        address deployer
    ) internal pure returns (address pointer) {
        /// @solidity memory-safe-assembly
        assembly {
...
library Errors {
    ...
    function verifySystemsMatch(
        ...
        address registry1 = abi.decode(
            component1.functionStaticCall(abi.encodeCall(ISystemComponent.getSystemRegistry, ())), (address)
        );
        address registry2 = abi.decode(
            component2.functionStaticCall(abi.encodeCall(ISystemComponent.getSystemRegistry, ())), (address)
        );
...
abstract contract AbstractRewarder is IBaseRewarder, SecurityBase {
     ...
     */
    function _withdrawAbstractRewarder(
        address account,
        uint256 amount
    ) internal {
        Errors.verifyNotZero(account, "account");
     ...
     */
    function _stakeAbstractRewarder(
        address account,
        uint256 amount
    ) internal {
        Errors.verifyNotZero(account, "account");
...
// src/rewarders/ExtraRewarder.sol
...
contract ExtraRewarder is AbstractRewarder, IExtraRewarder, ReentrancyGuard {
    ...
    constructor(
        ...
        AbstractRewarder(_systemRegistry, _rewardToken, _newRewardRatio, _duration, Roles.EXTRA_REWARD_MANAGER)
    {
