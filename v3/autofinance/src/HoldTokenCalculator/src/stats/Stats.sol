...
library Stats {
    ...
    /// @notice Generate an id for a stat calc representing a "dex" calc holding token
    /// @dev For IBTs that we hold directly
    /// @param tokenAddress address of the token
    function generateHoldTokenIdentifier(
        address tokenAddress
    ) internal pure returns (bytes32) {
        return keccak256(abi.encode("hold-erc20", tokenAddress));
    }
