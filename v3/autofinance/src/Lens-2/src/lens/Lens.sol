...
contract Lens is SystemComponent {
    ...
    function getUserRewardInfo(
        ...
        for (uint256 i = 0; i < nAutoPools; ++i) {
            ...
            uint256 extraRewardLength = mainRewarder.extraRewardsLength();

            rewardTokens[i] = new RewardToken[](extraRewardLength + 1);
            earnedAmounts[i] = new TokenAmount[](extraRewardLength + 1);

            rewardTokens[i][extraRewardLength] = RewardToken({ tokenAddress: mainRewarder.rewardToken() });
            earnedAmounts[i][extraRewardLength] = TokenAmount({ amount: mainRewarder.earned(wallet) });

            // Add the Extra Rewarder tokens and amounts
            for (uint256 k = 0; k < extraRewardLength; ++k) {
    ...
    function _getDestinations(
        ...
        _populateDestUnderlyingTokenInfo(autoPool, destinations);
    }

    function _populateDestUnderlyingTokenInfo(address autopool, DestinationVault[] memory destinations) private {
        address strategy = address(0);
        try IAutopool(autopool).autoPoolStrategy() returns (IAutopoolStrategy addr) {
            strategy = address(addr);
        } catch { }
        if (strategy == address(0) || strategy == 0x000000000000000000000000000000000000dEaD) {
            // Version of autopool that doesn't have strategy or stats
            // This new version probably works fine for both styles but want to keep the existing working the way it was

            IRootPriceOracle rootOracle = systemRegistry.rootPriceOracle();
            address baseAsset = IAutopool(autopool).asset();

            for (uint256 i = 0; i < destinations.length; ++i) {
                // We know underlying tokens and underlying token reserves match in order
                // slither-disable-next-line unused-return
                (, uint256[] memory balances) = IDestinationVault(destinations[i].vaultAddress).underlyingReserves();

                for (uint256 t = 0; t < destinations[i].underlyingTokens.length; ++t) {
                    // slither-disable-start divide-before-multiply
                    uint256 totalReserveTokenValue = balances[t]
                        * rootOracle.getPriceInQuote(destinations[i].underlyingTokens[t].tokenAddress, baseAsset)
                        / (10 ** IERC20Metadata(destinations[i].underlyingTokens[t].tokenAddress).decimals());

                    destinations[i].underlyingTokenValueHeld[t] = UnderlyingTokenValueHeld({
                        valueHeldInEth: totalReserveTokenValue * destinations[i].autoPoolOwnsShares
            ...
            for (uint256 i = 0; i < destinations.length; ++i) {
                if (destinations[i].underlyingTokens.length == destinations[i].reservesInEth.length) {
                    for (uint256 t = 0; t < destinations[i].underlyingTokens.length; ++t) {
                        destinations[i].underlyingTokenValueHeld[t] = UnderlyingTokenValueHeld({
