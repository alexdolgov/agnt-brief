...
contract LiquidationRow is ILiquidationRow, ReentrancyGuard, SystemComponent, SecurityBase {
    ...
    constructor(
        ...
        AutopilotErrors.verifyNotZero(address(destinationVaultRegistry), "destinationVaultRegistry");
    ...
    function addToWhitelist(
        ...
        AutopilotErrors.verifyNotZero(swapper, "swapper");
        if (!whitelistedSwappers.add(swapper)) revert AutopilotErrors.ItemExists();
    ...
    function removeFromWhitelist(
        ...
        if (!whitelistedSwappers.remove(swapper)) revert AutopilotErrors.ItemNotFound();
    ...
    function claimsVaultRewards(
        ...
        if (vaults.length == 0) revert AutopilotErrors.InvalidParam("vaults");
    ...
    function _prepareForLiquidation(
        ...
        for (uint256 i = 0; i < length; ++i) {
            ...
            if (!tokenVaults[fromToken].remove(vaultAddress)) revert AutopilotErrors.ItemNotFound();
        }
        ...
        if (tokenVaults[fromToken].length() == 0) {
            if (!rewardTokens.remove(fromToken)) revert AutopilotErrors.ItemNotFound();
        }
    ...
    function _increaseBalance(address tokenAddress, address vaultAddress, uint256 balance) internal {
        AutopilotErrors.verifyNotZero(balance, "balance");
        ...
        if (newTotalBalance > balanceOfToken) {
             ...
             */
            revert AutopilotErrors.InsufficientBalance(tokenAddress);
        }

        // if currentBalance is 0, then the vault is not yet added to the token vaults list
        if (currentBalance == 0) {
            if (!tokenVaults[tokenAddress].add(vaultAddress)) revert AutopilotErrors.ItemExists();

            if (totalBalance == 0) {
                if (!rewardTokens.add(tokenAddress)) revert AutopilotErrors.ItemExists();
            }
        }
    ...
    function _setPriceMarginBps(
        ...
        if (_priceMarginBps > MAX_PCT / 10) revert AutopilotErrors.InvalidParam("priceMarginBps");
