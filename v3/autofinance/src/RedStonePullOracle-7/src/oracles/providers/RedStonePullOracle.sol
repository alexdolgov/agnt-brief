...
contract RedStonePullOracle is PrimaryProdDataServiceConsumerBase, SystemComponent, SecurityBase {
    ...
    constructor(
        ...
        AutopilotErrors.verifyNotZero(_customOracle, "customOracle");
        ...
        if (address(_systemRegistry) != customOracleSystemRegistry) {
            revert AutopilotErrors.SystemMismatch(address(_systemRegistry), customOracleSystemRegistry);
        }
    ...
    function updatePriceWithFeedId(
        ...
        for (uint256 i = 0; i < len; ++i) {
            ...
            // Validate the price
            AutopilotErrors.verifyNotZero(values[i], "baseToken price");
    ...
    function registerFeedId(
        ...
        AutopilotErrors.verifyNotZero(feedId, "feedId");
        AutopilotErrors.verifyNotZero(address(tokenAddress), "tokenAddress");
    ...
    function _initializeSignerAddressToIndex(
        ...
        for (uint256 i = 0; i < signerAddresses.length; ++i) {
            ...
            AutopilotErrors.verifyNotZero(signerAddress, "signerAddress");
