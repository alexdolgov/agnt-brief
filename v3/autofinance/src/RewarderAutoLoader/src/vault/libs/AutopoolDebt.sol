...
library AutopoolDebt {
    ...
    struct RebalanceOutParams {
        ...
        address receiver;
        /// The "out" destination vaults
        address[] destinationsOut;
        /// The amounts of tokenOut that will be withdrawn
        uint256[] amountsOut;
        /// The underlying for destinationsOut
        address[] tokensOut;
        IERC20 _baseAsset;
    }
    ...
    function flashRebalance(
        ...
        result = _handleRebalanceOut(
            AutopoolDebt.RebalanceOutParams({
                ...
                destinationsOut: args.rebalanceParams.destinationsOut,
                amountsOut: args.rebalanceParams.amountsOut,
                tokensOut: args.rebalanceParams.tokensOut,
        ...
        uint256 destInLen = args.rebalanceParams.destinationsIn.length;

        FlashResultInfo memory flashResultInfo;
        flashResultInfo.tokenInBalanceBefore = new uint256[](destInLen);
        flashResultInfo.tokenInBalanceAfter = new uint256[](destInLen);

        // get "before" counts
        for (uint256 i = 0; i < destInLen;) {
            flashResultInfo.tokenInBalanceBefore[i] = IERC20(args.rebalanceParams.tokensIn[i]).balanceOf(address(this));
        ...
        bytes32 flashResult = args.receiver.onFlashLoan(
            msg.sender, args.rebalanceParams.tokensIn, args.rebalanceParams.amountsIn, 0, data
        );
        if (flashResult != keccak256("ERC3156FlashBorrower.onFlashLoan")) {
            revert AutopilotErrors.FlashLoanFailed();
        }

        for (uint256 i = 0; i < destInLen;) {
            address token = args.rebalanceParams.tokensIn[i];
            uint256 amount = args.rebalanceParams.amountsIn[i];

            uint256 tokenInBalanceAfter = IERC20(token).balanceOf(address(this));

            if (tokenInBalanceAfter < flashResultInfo.tokenInBalanceBefore[i] + amount) {
                revert AutopilotErrors.FlashLoanInvalidBalance(token, amount);
            }

            flashResultInfo.tokenInBalanceAfter[i] = tokenInBalanceAfter;
        ...
        for (uint256 i = 0; i < destInLen;) {
            address destinationIn = args.rebalanceParams.destinationsIn[i];
            if (args.rebalanceParams.destinationsIn[i] != address(this)) {
                IdleDebtUpdates memory inDebtResult = _handleRebalanceIn(
                    $.destinationInfo[destinationIn],
                    IDestinationVault(destinationIn),
                    args.rebalanceParams.tokensIn[i],
                    flashResultInfo.tokenInBalanceAfter[i]
                );

                // If prices aren't safe, revert
                if (!inDebtResult.pricesWereSafe) {
                    revert InvalidPrices();
                }

                // Update IdleDebtUpdates struct
                ...
                result.totalIdleIncrease +=
                    flashResultInfo.tokenInBalanceAfter[i] - flashResultInfo.tokenInBalanceBefore[i];
            }
            unchecked {
                ++i;
            }
    ...
    function validateRebalanceParams(AutopoolState storage $, ProcessRebalanceParams memory args) private view {
        ...
        uint256 outLen = args.rebalanceParams.destinationsOut.length;
        uint256 inLen = args.rebalanceParams.destinationsIn.length;

        // Check array lengths
        if (outLen != args.rebalanceParams.tokensOut.length || outLen != args.rebalanceParams.amountsOut.length) {
            revert ArrayLengthMismatch();
        }

        if (inLen != args.rebalanceParams.tokensIn.length || inLen != args.rebalanceParams.amountsIn.length) {
            revert ArrayLengthMismatch();
        }

        // Validations for destinations out
        for (uint256 i = 0; i < outLen;) {
            address destinationOut = args.rebalanceParams.destinationsOut[i];
            address tokenOut = args.rebalanceParams.tokensOut[i];
            uint256 amountOut = args.rebalanceParams.amountsOut[i];

            ensureNoZeros(destinationOut, tokenOut, amountOut);
            ensureDestinationRegistered(autopool, destinationOut);

            // if the out destination is the Autopool then the out token must be the baseAsset
            // if the out dest is not the Autopool then the out token must match the destinations underlying token
            if (destinationOut == autopool) {
                if (tokenOut != baseAsset) {
                    revert RebalanceDestinationUnderlyerMismatch(
                        args.rebalanceParams.destinationsOut[i], args.rebalanceParams.tokensOut[i], baseAsset
                    );
                }
                if (amountOut > $.assetBreakdown.totalIdle) {
                    revert InsufficientAssets(tokenOut);
                }
            } else {
                IDestinationVault outDest = IDestinationVault(destinationOut);
                if (tokenOut != outDest.underlying()) {
                    revert RebalanceDestinationUnderlyerMismatch(destinationOut, outDest.underlying(), tokenOut);
                }
                if (amountOut > outDest.balanceOf(autopool)) {
                    revert InsufficientAssets(tokenOut);
                }
            }
        ...
        // Validations for destinations in
        for (uint256 i = 0; i < inLen;) {
            address destinationIn = args.rebalanceParams.destinationsIn[i];
            address tokenIn = args.rebalanceParams.tokensIn[i];
            uint256 amountIn = args.rebalanceParams.amountsIn[i];

            ensureNoZeros(destinationIn, tokenIn, amountIn);
            ensureDestinationRegistered(autopool, destinationIn);

            // when a vault is shutdown, rebalancing can only pull assets from destinations back to the vault
            if ($.shutdown && destinationIn != autopool) {
                revert OnlyRebalanceToIdleAvailable();
            }

            // if the in destination is the Autopool then the in token must be the baseAsset
            // if the in dest is not the Autopool then the in token must match the destinations underlying token
            if (destinationIn == autopool) {
                if (tokenIn != baseAsset) {
                    revert RebalanceDestinationUnderlyerMismatch(destinationIn, tokenIn, baseAsset);
                }
            } else {
                IDestinationVault inDest = IDestinationVault(destinationIn);
                if (tokenIn != inDest.underlying()) {
                    revert RebalanceDestinationUnderlyerMismatch(destinationIn, inDest.underlying(), tokenIn);
                }
            }
        ...
        // Next we loop through all destinations for in and out arrays to make sure they are unique. We do not cross
        // check between arrays because we may have the same destination in and out. This can happen when we have
        // skewed dex pools, we may want to withdraw from a position and then put some back in to the same destination

        // Loop through destinations out to make sure they are all unique
        for (uint256 i = 0; i < outLen;) {
            address currentDestOuterLoop = args.rebalanceParams.destinationsOut[i];
            // Can always start at i + 1 because we are comparing the same arrays
            for (uint256 j = i + 1; j < outLen;) {
                if (currentDestOuterLoop == args.rebalanceParams.destinationsOut[j]) {
        ...
        // Then loop through destinations in to make sure that they are all unique
        for (uint256 i = 0; i < inLen;) {
            address currentDestOuterLoop = args.rebalanceParams.destinationsIn[i];
            // Can always start at i + 1 because we are comparing the same arrays
            for (uint256 j = i + 1; j < inLen;) {
                if (currentDestOuterLoop == args.rebalanceParams.destinationsIn[j]) {
                    revert RebalanceDestinationsMatch();
                }
                unchecked {
                    ++j;
    ...
    function ensureNoZeros(address destination, address token, uint256 amount) private pure {
    ...
    /**
     * @notice Perform withdraw and debt info update for the "out" destinations during a rebalance
     * @dev This "out" function performs more validations and handles idle as opposed to "in" which does not
     *  debtDecrease The previous amount of debt the destinations out accounted for in totalDebt
     *  debtIncrease The current amount of debt the destinations out should account for in totalDebt
     *  idleDecrease Amount of baseAsset that was sent from the vault. > 0 only when tokenOut == baseAsset
     *  idleIncrease Amount of baseAsset that was claimed from Destination Vaults
     * @param params Rebalance out params
     * @param $ The state of the autopool, used to update information about the destinations out
    ...
    function _handleRebalanceOut(
        ...
        uint256 len = params.destinationsOut.length;
        IERC20 baseAssetLocal = params._baseAsset;

        // Handle decrease (shares going "Out", cashing in shares and sending underlying back to swapper)
        // If the tokenOut is _asset we assume they are taking idle
        // which is already in the contract
        for (uint256 i = 0; i < len;) {
            address currentTokenOut = params.tokensOut[i];
            uint256 amountOut = params.amountsOut[i];

            if (currentTokenOut != address(baseAssetLocal)) {
                IDestinationVault dvOut = IDestinationVault(params.destinationsOut[i]);
                ...
                uint256 beforeBaseAssetBal = baseAssetLocal.balanceOf(address(this));

                // Withdraw underlying from the destination vault
                // Shares are sent directly to the flashRebalance receiver
                // slither-disable-next-line unused-return
                dvOut.withdrawUnderlying(amountOut, params.receiver);

                // Get a debt info snapshot for current destination vault
                IdleDebtUpdates memory assetChangeLocal = _recalculateDestInfo(
                    $.destinationInfo[address(dvOut)], dvOut, originalShareBal, originalShareBal - amountOut
                );

                // If we have an unsafe price, update for all destinations
                if (assetChangeLocal.pricesWereSafe == false) {
                    revert InvalidPrices();
                }

                // Update total asset changes with this destinations values. Ignoring idle increase and decrease here
                // as it is not accounted for in `_recalculateDestInfo`
                assetChange.totalDebtIncrease += assetChangeLocal.totalDebtIncrease;
                assetChange.totalDebtDecrease += assetChangeLocal.totalDebtDecrease;
                assetChange.totalMinDebtIncrease += assetChangeLocal.totalMinDebtIncrease;
                assetChange.totalMinDebtDecrease += assetChangeLocal.totalMinDebtDecrease;
                assetChange.totalMaxDebtIncrease += assetChangeLocal.totalMaxDebtIncrease;
                assetChange.totalMaxDebtDecrease += assetChangeLocal.totalMaxDebtDecrease;

                // Capture any rewards we may have claimed as part of withdrawing
                assetChange.totalIdleIncrease += baseAssetLocal.balanceOf(address(this)) - beforeBaseAssetBal;
            } else {
                // Working with idle baseAsset which should be in the vault already
                // Just send it out
                IERC20(params.tokensOut[i]).safeTransfer(params.receiver, amountOut);
                assetChange.totalIdleDecrease += amountOut;
            }
        ...
        // If we get here with no revert, it means all prices were safe
