...
interface IAutopool is IERC4626, IERC20Permit {
    ...
    /// @notice Returns the asset amount you must send during intialization
    function BASE_ASSET_INIT_DEPOSIT() external view returns (uint256);

    function initialize(string memory symbol, string memory name, bytes memory extraData) external;
