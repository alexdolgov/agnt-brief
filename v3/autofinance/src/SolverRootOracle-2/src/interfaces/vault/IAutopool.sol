...
interface IAutopool is IERC4626, IERC20Permit {
    ...
    /// @notice The reason a vault was shutdown
    ...
    /// @notice How is the value for total assets being used
    ...
    /// @param profitUnlockRate profit shares unlock rate/sec. Rate when calculated is denominated in MAX_BPS_PROFIT.
    ...
    /// @param rebalanceFeeHighWaterMarkEnabled Returns whether the fee nav/share high water mark is enabled
    ...
    /// @notice Reorder the withdrawal queue according to the provided instructions
    function reorderWithdrawalQueue(
        IAutopool.ReorderInstruction[] memory instructions
    ) external;
    ...
    /// @notice Add or remove a destination from the allowed direct deposit list
    /// @param destinations Destination to add or remove
    function toggleAllowedDirectDeposits(
        address[] memory destinations
    ) external;

    /// @notice Add or remove a destination from the allowed direct withdrawal list
    /// @param destinations Destination to add or remove
    function toggleAllowedDirectWithdrawals(
        address[] memory destinations
    ) external;
    ...
    /// @notice Get strategy hooks configured on the Autopool
