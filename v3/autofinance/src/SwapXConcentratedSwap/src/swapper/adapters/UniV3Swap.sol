...
contract UniV3Swap is BaseAdapter {
    ...
    function _decodePath(
        bytes memory path
    ) internal pure virtual returns (address sellAddress, address buyAddress) {
        bool hasMultiplePools = _hasMultiplePools(path);
        (sellAddress, buyAddress) = _decodeFirstPool(path);

        while (hasMultiplePools) {
            path = _skipToken(path);
            hasMultiplePools = _hasMultiplePools(path);

            // We can only determine the last token in the path when there are no more pools left
            if (!hasMultiplePools) {
                // slither-disable-next-line unused-return
                (, address tokenOut) = _decodeFirstPool(path);
                buyAddress = tokenOut;
            }
        }
