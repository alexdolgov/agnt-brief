...
contract Lottery is OwnableUpgradeable {
    ...
    function buy(uint256 _price, uint8[4] memory _numbers) external {
        ...
        jaws.safeTransferFrom(address(msg.sender), address(this), _price);
        _burnToken(burnAmt);
        burnTotal += burnAmt;
    ...
    function  multiBuy(uint256 _price, uint8[4][] memory _numbers) external {
        ...
        require(!drawingPhase, 'drawing, can not buy now');
        require (_price >= minPrice, 'price must above minPrice');
        uint256 burnAmt = _price.mul(_numbers.length).mul(burnPerTxn).div(100);
        ...
        totalAmount = totalAmount.sub(burnAmt);
        jaws.safeTransferFrom(address(msg.sender), address(this), totalPrice);
        _burnToken(burnAmt);
        burnTotal += burnAmt;
    ...
    function claimReward(uint256 _tokenId) external {
        ...
        if(reward>0) {
            jaws.safeTransfer(address(msg.sender), reward);
        }
    ...
    function  multiClaim(uint256[] memory _tickets) external {
        ...
        if(totalReward>0) {
            jaws.safeTransfer(address(msg.sender), totalReward);
        }
    ...
    function _burnToken(uint256 _amount) private {
        IERC20(jaws).transfer(BURN_ADDRESS, _amount);
        burnTotal += _amount;
    }
    ...
    function adminWithdraw(uint256 _amount) public onlyAdmin {
        jaws.safeTransfer(address(msg.sender), _amount);
    ...
    // Set the max number for one ticket
    ...
    function topUp(uint256 _amount) external onlyOwner {
        jaws.safeTransferFrom(address(msg.sender), address(this), _amount);
