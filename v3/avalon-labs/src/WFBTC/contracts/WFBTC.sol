...
contract WFBTC is IWFBTC, OFTUpgradeable, AccessControlUpgradeable, PausableUpgradeable {
    ...
    function initialize(string memory _name, string memory _symbol, address _delegate) public reinitializer(2) {
    ...
    function burn(address user, uint256 amount) public virtual onlyRole(BURN_ROLE) {
        if (!isBlackListed[user]) revert NotInBlackList();
        _burn(user, amount);
    }

    function burnFromWrapper(address user, uint256 amount) public virtual onlyRole(BURN_ROLE) {
        _burn(user, amount);
    }
