...
contract PairInfos is Initializable, IPairInfos {
    ...
    mapping(uint => PairRolloverParams) public pairRolloverFeePerBlock;
    ...
    function updatePairStorage(address _newPairStorage) external onlyGov{

        require(_newPairStorage != address(0), "ZERO_ADDR");

        pairsStorage = IPairStorage(_newPairStorage);
        emit PairStorageUpdated(_newPairStorage);
    }
    /**
    ...
    /**
     * @notice Sets the rollover fee per block for a trading pair by the keeper
     * @param pairIndex The index of the trading pair
     * @param _longRolloverFeePerBlockP The long rollover fee per block
     * @param _shortRolloverFeePerBlockP The short rollover fee per block
     */
    function setRolloverFeePerBlockPByKeeper(uint pairIndex, uint _longRolloverFeePerBlockP, uint _shortRolloverFeePerBlockP) external onlyKeeper {
        require(_longRolloverFeePerBlockP <= 25e6, "TOO_HIGH"); // ≈ 100% per day, 43200 blocks per day, 1800 per hour, 0.01% per hour is
        require(_shortRolloverFeePerBlockP <= 25e6, "TOO_HIGH");

        _storeAccRolloverFees(pairIndex);
        pairRolloverFeePerBlock[pairIndex].longRolloverFeePerBlockP = _longRolloverFeePerBlockP;
        pairRolloverFeePerBlock[pairIndex].shortRolloverFeePerBlockP = _shortRolloverFeePerBlockP;

        emit RolloverFeePerBlockPUpdated(pairIndex, _longRolloverFeePerBlockP, _shortRolloverFeePerBlockP);
    }
     ...
     * @param _longRolloverFeePerBlockP Long fee value to set.
     * @param _shortRolloverFeePerBlockP Short fee value to set.
     */
    function setRolloverFeePerBlockP(uint pairIndex, uint _longRolloverFeePerBlockP, uint _shortRolloverFeePerBlockP) external onlyGov {
        require(_longRolloverFeePerBlockP <= 25e6 && _shortRolloverFeePerBlockP <= 25e6 , "TOO_HIGH"); // ≈ 100% per day, 43200 blocks per day, 1800 per hour, 0.01% per hour is
    ...
    function getTradeValue(
        ...
        uint tier = storageT.openTradesInfo(_trade.trader, _trade.pairIndex, _trade.index).lossProtection;
        ...
        uint lossProtection = pairsStorage.lossProtectionMultiplier(_trade.pairIndex, tier);
    ...
    function getTradePriceImpact(
        ...
        (uint posSpreadCap, uint negSpreadCap) = pairsStorage.spreadCaps(pairIndex, isPnl);
        
        int priceImpactSpread = getPriceImpactSpread(pairIndex, long, tradeOpenInterest, isPnl);
        int skewImpactSpread = getSkewImpactSpread(pairIndex, long, tradeOpenInterest, isPnl);
        int dynamicSpread =  (int(pairsStorage.pairSpreadP(pairIndex, isPnl)) + priceImpactSpread + skewImpactSpread);
        
        dynamicSpread = dynamicSpread < -int(posSpreadCap / 10) ? -int(posSpreadCap / 10) : (dynamicSpread > int(negSpreadCap / 10) ? int(negSpreadCap / 10) : dynamicSpread);
    ...
    function getPriceImpactSpread(uint256 _pairIndex, bool _isBuy, uint256 _leveragePosition, bool isPnl) public view returns(int256 spread){
        
        uint b = pairsStorage.pairPriceImpactMultiplier(_pairIndex, isPnl);
    ...
    function getSkewImpactSpread(uint _pairIndex, bool _isBuy, uint _leveragePosition, bool isPnl) public view returns(int256 spread){
        ...
        int skewParam = pairsStorage.pairSkewImpactMultiplier(_pairIndex, isPnl);
   ...
   /** @notice Calculate and return the loss protection tier for a given trade based on current skew
     ...
     */
    function lossProtectionTier(ITradingStorage.Trade memory _trade, bool isPnl) external view override returns (uint) {

        if(isPnl) return 0; // No loss protection
     ...
     *  @return PairRolloverParams[] and PairRolloverFees[] arrays containing trading pair parameters and rollover fees
     */
    function getPairInfos(
        uint[] calldata indices
    ) external view returns (PairRolloverParams[] memory, PairRolloverFees[] memory) {
        PairRolloverParams[] memory params = new PairRolloverParams[](indices.length);
        PairRolloverFees[] memory rolloverFees = new PairRolloverFees[](indices.length);

        for (uint i; i < indices.length;) {
            uint index = indices[i];

            params[i] = pairRolloverFeePerBlock[index];
   ...
   /** @notice Get the one percent depth above the current price for the given pair index
     ...
     */
    function getRolloverFeePerBlockP(uint pairIndex) external view returns (uint, uint) {
        return (
            pairRolloverFeePerBlock[pairIndex].longRolloverFeePerBlockP,
            pairRolloverFeePerBlock[pairIndex].shortRolloverFeePerBlockP
        );
    }
    ...
    function getBlendedUtilizationRatio(uint _pairIndex, uint _longOI, uint _shortOI) public view returns(uint256){
        ...
        return pairUtilization;
    }

    /**
     * @notice Retrieves the blended skew ratio
     * @param _pairIndex The index of the trading pair
     * @return Utlization Ratio in 1e2 precision. 100 means 100% skew
     */
    ...
    function getPendingAccRolloverFees(uint pairIndex) public view returns (uint valueLong, uint valueShort) {
        ...
        uint256 blendedUtilizationRatio = openInterestUSDCLong + openInterestUSDCShort > 0 ? getBlendedUtilizationRatio(pairIndex,openInterestUSDCLong, openInterestUSDCShort) : 0;

        if (openInterestUSDCLong > 0) {
            uint skewRatio = getBlendedSkew(pairIndex, true, openInterestUSDCLong, openInterestUSDCShort);
            uint longMultiplier = blendedUtilizationRatio < pairsStorage.pairUtililizationThreshold(pairIndex) 
                                  || skewRatio <= pairsStorage.pairSkewThreshold(pairIndex)
                                  ? pairsStorage.pairBorrowFeesMultiplier(pairIndex)
                                  : _PRECISION*(blendedUtilizationRatio*skewRatio)/(_PRECISION*1e2 - blendedUtilizationRatio*skewRatio);
        ...
        if (openInterestUSDCShort > 0) {
            ...
            uint shortMultiplier =  blendedUtilizationRatio < pairsStorage.pairUtililizationThreshold(pairIndex) 
                                    || skewRatio <= pairsStorage.pairSkewThreshold(pairIndex)
                                    ? pairsStorage.pairBorrowFeesMultiplier(pairIndex)
                                    : _PRECISION*(blendedUtilizationRatio*skewRatio)/(_PRECISION*1e2 - blendedUtilizationRatio*skewRatio);
   ...
   /**
    ...
    function getTradeRolloverFee(
        ...
        if (!t.openedAfterUpdate || 
            pairsStorage.getPosType(trader, pairIndex, index)
        ){
            return 0;
        }

        (uint pendingLong, uint pendingShort) = getPendingAccRolloverFees(pairIndex);
        uint borrowFee = getTradeRolloverFeePure(t.rollover, long ? pendingLong : pendingShort, collateral, leverage);
        (uint minBorrowFee, uint maxBorrowFee) = minMaxRollOverFee(trader, pairIndex, index, collateral, leverage, long);
        return borrowFee >= maxBorrowFee ? maxBorrowFee : borrowFee >= minBorrowFee ? borrowFee : minBorrowFee;
    }

    /**
    @notice Compute the returns minimum and maximum roll over Fee for a trade based on its open Timestamp
    ...
    */
    function minMaxRollOverFee(
        ...
        uint leverage,
        bool isBuy
    ) public view returns (uint minBorrowFee, uint maxBorrowFee) {

        uint openTimestamp = storageT.openTrades(trader, pairIndex, index).positionSizeUSDC;
        if(openTimestamp ==  0) return (0, 0);

        uint timeSinceOpen = block.timestamp - openTimestamp;

        minBorrowFee = pairsStorage.minBorrowFee(pairIndex, isBuy) * timeSinceOpen * collateral.mul(leverage) / (_PRECISION * 365 days); 
        maxBorrowFee = pairsStorage.maxBorrowFee(pairIndex, isBuy) * timeSinceOpen * collateral.mul(leverage) / (_PRECISION * 365 days); 
    }
    ...
    function getTradeRolloverFeePure(
        ...
        uint leverage
            ...
            if(accRolloverFeesPerOi > endAccRolloverFeesPerOi ){
                return 0;
            }
    ...
    /**
     * @notice Retrieves the risk manager address
     * @return The risk manager address
     */
    ...
    function resetTradeInitialAccess(address trader, uint pairIndex, uint index) external override onlyCallbacks {
        delete tradeInitialAccFees[trader][pairIndex][index];
    }
