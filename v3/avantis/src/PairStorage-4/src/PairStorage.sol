...
contract PairStorage is Initializable, IPairStorage {
    ...
    mapping(uint => uint[2]) public groupOIs; // NOTE: To be taken care of while migration
    ...
    modifier onlyGov() {
        require(msg.sender == address(0xE4cc1B667217C1e9D9e827aEf83a90f6C6Fd797D), "GOV_ONLY");
        _;
    }
    ...
    function updatePairParams(uint _pairIndex, PairParams calldata _params) external onlyGov{
        ...
        params.param_1 = _params.param_1;
        params.param_2 = _params.param_2;
        params.param_3 = _params.param_3;
        params.param_4 = _params.param_4;
        params.param_5 = _params.param_5;
        params.param_6 = _params.param_6;
        params.param_7 = _params.param_7;
        params.param_8 = _params.param_8;
    ...
    function updateAdditionalPairParams(uint _pairIndex, AdditionalPairParams calldata _params) external onlyGov{
        ...
        params.param_9 = _params.param_9;
        params.param_10 = _params.param_10;
        params.param_11 = _params.param_11;
        params.param_12 = _params.param_12;
    ...
    function pairMinLeverage(uint _pairIndex) external view override returns (uint) {
        return pairs[_pairIndex].leverages.minLeverage;
    }
    ...
    function pairMaxLeverage(uint _pairIndex) external view override returns (uint) {
        return pairs[_pairIndex].leverages.maxLeverage;
    }
    ...
    function pairMaxOI(uint _pairIndex) external view override returns (uint) {
        return (pairs[_pairIndex].values.groupOpenInterestPercentageP * groupMaxOI(_pairIndex)) / 100;
    }
   ...
   /** 
    ...
    function pairMaxLongOI(uint _pairIndex) external view override returns(uint){
        ...
        uint pairMaxOi = pairs[_pairIndex].values.groupOpenInterestPercentageP * groupMaxOI(_pairIndex) / 100;
        return pairMaxOi * pairs[_pairIndex].values.maxLongOiP / 100;
    }
   ...
   /** 
    ...
    function pairMaxShortOI(uint _pairIndex) external view override returns(uint){
        ...
        uint pairMaxOi = pairs[_pairIndex].values.groupOpenInterestPercentageP * groupMaxOI(_pairIndex) / 100;
        return pairMaxOi * pairs[_pairIndex].values.maxShortOiP / 100;
    }
    ...
    function guaranteedSlEnabled(uint _pairIndex) external view override returns (bool) {
        ...
        if(groupIndex == 2 || groupIndex == 3) return false; // Disabled for Forex and commodities
        return true;
    }
    ...
    function maxWalletOI(uint _pairIndex) external view override returns (uint) {
        return (storageT.maxOpenInterest() * pairs[_pairIndex].values.maxWalletOIP) / 100;
    }
     ...
     */
    function spreadCaps(uint _pairIndex) external view override returns(uint,uint){

        return (pairParams[_pairIndex].posSpreadCap, pairParams[_pairIndex].negSpreadCap);
    }  
    ...
    * @notice Resets Position type
    ...
    /**
    * @notice Syncs the group OI between new and old pairStorage contract
    * @param _pairIndex The index of the trading pair.
    * @param _newLongOI The new Long OI
    * @param _newShortOI The new Short OI
    * @dev Only to be called before migration
     */
    function syncGroupOI(uint _pairIndex, uint _newLongOI, uint _newShortOI) external {

        address oldPairStorage = address(0x92Ed158d5e423CFdc9eed5Bd7328FFF7CeD6fF94);
        require(msg.sender == storageT.gov() || msg.sender == oldPairStorage , "GOV_ONLY");
    ...
    /**
    *@notice Syncs the current order ID between new and old pairStorage contract
    */
    function syncOrderId(uint _orderId) external {

        address oldPairStorage = address(0x92Ed158d5e423CFdc9eed5Bd7328FFF7CeD6fF94);
        require(msg.sender == storageT.gov() || msg.sender == oldPairStorage , "GOV_ONLY");

        currentOrderId = _orderId + 1;
    }
