...
contract PairStorage is Initializable, IPairStorage {
    ...
    uint private constant _MAX_LEVERAGE = 1000 * _PRECISION;
    ...
    function updatePair(
        ...
        p.groupOpenInterestPecentage = _pair.groupOpenInterestPecentage;
        p.maxWalletOI = _pair.maxWalletOI;
        p.feed = _pair.feed;
    ...
    function updateGroupOI(uint _pairIndex, uint _amount, bool _long, bool _increase) external override {
        ...
        address newPairStorage = 0x5db3772136e5557EFE028Db05EE95C84D76faEC4;
        (bool success, bytes memory data ) = newPairStorage.call(abi.encodeWithSignature("syncGroupOI(uint256,uint256,uint256)",_pairIndex, oi[0], oi[1]));
        require(success, "SYNCOI_FAILED");
    }
    ...
    function pairJob(uint _pairIndex) external override returns (string memory, string memory, bytes32, address, uint) {
        ...
        address newPairStorage = 0x5db3772136e5557EFE028Db05EE95C84D76faEC4;
        (bool success, bytes memory data ) = newPairStorage.call(abi.encodeWithSignature("syncOrderId(uint256)",currentOrderId));
        require(success, "SYNCORDERID_FAILED");
    ...
    function maxWalletOI(uint _pairIndex) external view override returns (uint) {
        return (storageT.maxOpenInterest() * pairs[_pairIndex].maxWalletOI) / 100;
    }
    ...
    function resetGroupOIs() external onlyGov{
        ...
        for(uint j = 0; j< pairsCount; j++){

            uint openInterestUSDCLong = storageT.openInterestUSDC(j, 0);
            uint openInterestUSDCShort = storageT.openInterestUSDC(j, 1);
            groupOIs[pairs[j].groupIndex][0] +=  openInterestUSDCLong;
            groupOIs[pairs[j].groupIndex][1] +=  openInterestUSDCShort;
        }
    }

    /**
    * @notice Gets the Treshold to close the trade
    */
    function openCloseThreshold(uint _pairIndex, uint256 _leveragePos) external override returns(uint256){

        uint256 timer;
        uint256 minTimer = 3*60; // 3 mins
        if(_leveragePos > 250000e6) timer = 15*60;
        else if(_leveragePos > 50000e6) timer = 15*60;
        else if(_leveragePos > 5000e6) timer = 10*60;
        else timer = minTimer;
        ...
        if (_pairIndex == 2) return 0;
        if (_pairIndex == 34 || _pairIndex == 35) return timer;
        if(groupIndex == 0 || groupIndex == 2 || groupIndex == 3 ) return 0;
        return  timer ;
    }

    // Function to update listing mapping
    function enlistPair(string memory _from, string memory _to) external onlyGov{
        require(!isPairListed[_from][_to], "PAIR_ALREADY_LISTED");
        isPairListed[_from][_to] = true;
    }
