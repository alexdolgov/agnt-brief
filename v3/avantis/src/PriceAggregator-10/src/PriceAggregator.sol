...
contract PriceAggregator is Initializable, IPriceAggregator {
    ...
    function fulfill(uint orderId, bytes[] calldata priceUpdateData) external payable override onlyTrading {
        ...
        if (r.initiated) {
            ...
            if(useBackupOnly && r.orderType == OrderType.LIMIT_CLOSE && backupFeed.feedId != address(0)){
                ...
                if(
                    r.orderType == OrderType.UPDATE_MARGIN || 
                    r.orderType == OrderType.UPDATE_SL     ||
                    r.orderType == OrderType.LIMIT_CLOSE   ||
                    r.orderType == OrderType.LIMIT_CLOSE_PNL 
                ){
                    pythPrice = pyth.getPriceNoOlderThan(r.job, 30); // 30 seconds
                }else{
                    pythPrice = pyth.getPriceNoOlderThan(r.job, 10); // 10 seconds
                }
   ...
   /**
    ...
    function getPriceNoOlderThan(IPriceAggregator.OrderType _type) private {
