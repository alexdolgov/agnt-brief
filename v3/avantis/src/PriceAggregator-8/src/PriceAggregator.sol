        ...
        if (r.initiated) {
            ...
            if(useBackupOnly && r.orderType == OrderType.LIMIT_CLOSE && backupFeed.feedId != address(0)){
                ...
                if(_priceSourcing == PriceSourcing.PYTH_CORE){
                    ...
                    require(priceUpdateData.length > 0, "NO_UPDATE_DATA");
                    (bytes memory payload, PythLazerStructs.Update memory update) = pythLazer.verifyAndParseUpdate{ value: verification_fee }(priceUpdateData[0]);
                    (uint64 _timestamp,,uint8 feedsLen,) = PythLazerLib.parsePayloadHeader(payload);

                    require(feedsLen == 1,"INVALID_PAYLOAD_LENGTH");
                    require(_timestamp > block.timestamp - 60, "INVALID_TIMESTAMP");
                    int16 exponent = update.feeds[0]._exponent;
                    if (exponent > 0) {
                        price = uint64(update.feeds[0]._price) * _PRECISION * 10 ** uint32(uint16(exponent));
                        conf = update.feeds[0]._confidence * _PRECISION * 10 ** uint32(uint16(exponent));
                    } else {
                        uint32 absExponent = uint32(uint16(-exponent));
                        price = (uint64(update.feeds[0]._price) * _PRECISION) / 10 ** absExponent;
                        conf = (update.feeds[0]._confidence * _PRECISION) / 10 ** absExponent;
                    }
                }
