...
contract Referral is PausableUpgradeable, IReferral {
    ...
    * @notice Helper method used only during migration 
    */
    function migrateCodes(bytes32[] memory _codes, address[] memory _owners, uint[] memory ids) external onlyGovOrHandler{
       
        for(uint i; i< _codes.length; i++){
            require(_codes[i] != bytes32(0), "Referral: invalid _code");
            require(codeOwners[_codes[i]] == address(0), "Referral: code already exists");
            require(codes[_owners[i]] == bytes32(0),"Referral: referrer already registered");

            codeOwners[_codes[i]] = _owners[i];
            codes[_owners[i]] =  _codes[i];
            referrerTiers[_owners[i]] = ids[i];
            isPrivate[_codes[i]]  = false;
            
            emit RegisterCode(_owners[i], _codes[i]);
        }
    ...
    * @notice Helper method to set traderReferral Codes
    */
    function migrateTraderReferrals(address[] memory _traders, bytes32[] memory _codes) external onlyGovOrHandler{
        
        for(uint i; i< _traders.length; i++){
            traderReferralCodes[_traders[i]] = _codes[i];
            emit SetTraderReferralCode(_traders[i], _codes[i]);
        }
    }
