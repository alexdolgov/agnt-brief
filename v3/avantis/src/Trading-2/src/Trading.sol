...
error Revert_PositionRollOngoing();
error Revert_GovOnly();
error Revert_OnlyOperator();
error Revert_ZeroAddress();
...
contract Trading is PausableUpgradeable, Delegatable {
    ...
    modifier onlyOperator() {
        if(!isOperator[msg.sender]){
            revert Revert_OnlyOperator();
        }
        _;
    }
    
    modifier whenMarketNotPaused(uint _pairIndex) {
        if(pairInfos.pairsStorage().isPositionRollOngoing(_pairIndex)){
            revert Revert_PositionRollOngoing();
        }
        _;
    }
    ...
    */
    ...
    ) external payable whenNotPaused whenMarketNotPaused(_pairIndex) returns(uint orderId){
    ...
    function openTrade(
        ...
        uint _slippageP
    ) external payable whenNotPaused whenMarketNotPaused(t.pairIndex) returns(uint orderId) {
    ...
    function updateOpenLimitOrder(
        ...
        uint _tp,
        uint _sl
    ) external whenNotPaused whenMarketNotPaused(_pairIndex) {
     ...
     * @param _index The index of the order
     */
    function cancelOpenLimitOrder(uint _pairIndex, uint _index) external whenNotPaused whenMarketNotPaused(_pairIndex) {
        _cancelLimitOrder(
            __msgSender(),
            _pairIndex,
            _index,
    ...
    /**
     * @notice Cancels an open limit order during position roll
     * @param _trader Trader who's limit order keeper wants to cancel
     * @param _pairIndex Pair index of trade that we want to cancel
     * @param _index Index of trade that we want to cancel
    */
    function cancelRollLimitOrders(
        address _trader,
        uint _pairIndex,
        uint _index
    ) external {
        if(!(storageT.positionRollkeepers(msg.sender) && pairInfos.pairsStorage().isPositionRollOngoing(_pairIndex))){
            revert("NOT_PERMITTED");
        }
        _cancelLimitOrder(
            _trader,
            _pairIndex,
    ...
    function _cancelLimitOrder(
        ...
        if(_userInit && block.number - o.block < limitOrdersTimelock){
            revert("LIMIT_TIMELOCK");
        }
        storageT.transferUSDC(address(storageT), _trader, o.positionSize + o.executionFee);
        storageT.unregisterOpenLimitOrder(_trader, _pairIndex, _index);
        emit OpenLimitCanceled(_trader, _pairIndex, _index, block.timestamp, o.positionSize);
    }
    ...
    function updateTpAndSl(
        ...
        bytes[] calldata priceUpdateData,
        IPriceAggregator.PriceSourcing _priceSourcing
    ) external payable whenNotPaused whenMarketNotPaused(_pairIndex){
    ...
    function updateSl(
        ...
        payable
        whenNotPaused
        whenMarketNotPaused(_pairIndex)
    {
        if (priceUpdateData.length != 0) _updateSl(_pairIndex, _index, _newSl, priceUpdateData, _priceSourcing);
    }
     ...
     */
     function executeLimitOrder(
        ...
        IPriceAggregator.PriceSourcing _priceSourcing,
        int spreadP
    ) external payable whenNotPaused onlyOperator whenMarketNotPaused(_pairIndex) { 
    ...
    function _updateSl(uint _pairIndex, uint _index, uint _newSl, bytes[] calldata priceUpdateData, IPriceAggregator.PriceSourcing _priceSourcing) internal {
        ...
        if (_newSl == 0 || !aggregator.pairsStorage().guaranteedSlEnabled(_pairIndex)) {
            _newSl = storageT.updateSl(__msgSender(), _pairIndex, _index, _newSl);
    ...
    /**
    * @notice emitting OpenLimitPlaced event due to stack too deep
    * @param __msgSender trader address
    * @param _t Trade details
    * @param _index trade index
    * @param _type order type
    * @param _slippageP slippage to be applied
    */
    function emitOpenLimitPlaced(
        address __msgSender,
        ITradingStorage.Trade calldata _t,
        uint _index,
        IExecute.OpenLimitOrderType _type,
        uint _slippageP
    ) internal {
