...
contract TradingCallbacks is Initializable, ICallbacks {
    ...
    uint public zfpLiqFeeP;
    uint public zfpLiqTotalFeeP;
   ...
   /**
     ...
     * @param _zfpLiqFeeP ZFP Liquidator reward percentage of Liquidation Fees
     */
    function setFeeP(
        uint _liqFeeP, 
        uint _liqTotalFeeP, 
     ...
     */
    function openTradeMarketCallback(uint orderId, uint price, int spreadP, bool isPnl) external override onlyPriceAggregator {
        require(spreadP > -1e10 && spreadP < int(2e10), "INVALID_SPREAD");
        ...
        // crypto only
        t.openPrice = _marketExecutionPrice(int(price), spreadP, t.buy);
     ...
     * @param isPnl Bool indicating if IsPnl based Order.
     */
    function executeLimitOpenOrderCallback(uint orderId, uint price, int spreadP, bool isPnl) external override onlyPriceAggregator {
        require(spreadP > -1e10 && spreadP < int(2e10), "INVALID_SPREAD");
        ...
        if (price != 0 && storageT.hasOpenLimitOrder(n.trader, n.pairIndex, n.index)) {
                ...
                && storageT.withinExposureLimits(o.trader, o.pairIndex, o.positionSize.mul(o.leverage), o.buy))
            {
                ...
                price = _marketExecutionPrice(int(price), spreadP, o.buy);
                ...
                if(!(o.buy ? price  > o.price + (o.price * o.slippageP) / 100 / _PRECISION : price < o.price - (o.price * o.slippageP) / 100 / _PRECISION)) {
                    ...
                    //Non matching with def to avoid more vars
                    emit LimitExecuted(
                        orderId,
                        n.index,
                        finalTrade,
    ...
    function executeLimitCloseOrderCallback(uint orderId, uint price, bool isPnl) external override onlyPriceAggregator {
        ...
        if (price != 0 && t.leverage > 0) {
            ...
            if (o.orderType == ITradingStorage.LimitOrder.LIQ) {
                ...
                uint liquidatorFeeP = isPnl ? zfpLiqFeeP : liqFeeP;
                uint totalLiquidationP = isPnl ? zfpLiqTotalFeeP : liqTotalFeeP;

                values.reward = (t.buy ? price <= liqPrice : price >= liqPrice) ? (t.initialPosToken * liquidatorFeeP) / 100 : 0;
                values.levPosToken = values.reward * ( totalLiquidationP - liquidatorFeeP) / liquidatorFeeP;
                
                require(t.buy ? price <=liqPrice : price >= liqPrice, "INVALID_LIQ");
            } else {
    ...
    function updateSlCallback(uint orderId, uint price, bool isPnl) external override onlyPriceAggregator {
        ...
        if (
            ...
            o.newSl = storageT.updateSl(o.trader, o.pairIndex, o.index, o.newSl);
            emit SlUpdated(orderId, o.trader, o.pairIndex, o.index, o.newSl);
        }else {
            revert("SL_UPDATE_FAILED");
        }
    ...
    function _registerTrade(ITradingStorage.Trade memory _trade, bool isPnl) private returns (ITradingStorage.Trade memory) {
        ...
        _trade.sl = pairsStored.correctSl(_trade.openPrice, _trade.leverage, _trade.sl, _trade.buy, _trade.pairIndex, isPnl);
    ...
    function _unregisterTrade(
        ...
        ICallbacks.feeValues memory feeValues;
        {
            (
                feeValues.feeAfterRebate,
                feeValues.referrerRebate,
                feeValues.gasFees
            ) = storageT.applyReferralAndPnlFee(
                _trade.trader,
                _lpFeeToken,
            ...
            int pnl;
            (usdcSentToTrader, pnl, feeValues.totalFees) = pairInfos.getTradeValue(
                _trade,
                _collateral,
                _percentProfit,
                feeValues.feeAfterRebate + _feeAmountToken
            );

            uint vaultAllocation = ((feeValues.feeAfterRebate - feeValues.referrerRebate) * (100 - vaultFeeP)) / 100;
            uint govFees = (feeValues.feeAfterRebate - feeValues.referrerRebate - vaultAllocation) / 2;

            storageT.incrementClosingFees(
                feeValues.feeAfterRebate - feeValues.referrerRebate - vaultAllocation - govFees,
                govFees
            );

            storageT.vaultManager().sendUSDCToTrader(address(storageT), feeValues.feeAfterRebate - feeValues.referrerRebate - vaultAllocation);
            ...
            if (pnl >= 0 ){
                ...
                if(pnl + int(feeValues.gasFees) < 0) {
                    storageT.vaultManager().allocateRewards(uint(-pnl) - feeValues.gasFees, true);
                    storageT.vaultManager().allocateGasFees(feeValues.gasFees);
                }
                else {
                    storageT.vaultManager().allocateGasFees(uint(-pnl));
                }
            }
            
            if (feeValues.totalFees > (_feeAmountToken + (feeValues.feeAfterRebate - vaultAllocation))) {
                storageT.vaultManager().allocateRewards(feeValues.totalFees - _feeAmountToken - (feeValues.feeAfterRebate - vaultAllocation), false);
            }
            if (feeValues.referrerRebate > 0) storageT.vaultManager().sendReferrerRebateToStorage(feeValues.referrerRebate);
        }
    ...
    function _marketExecutionPrice(int _price, int _spreadP, bool _long) private pure returns (uint) {
        int priceDiff = (_price * _spreadP) / 100 / int(_PRECISION);
        return _long ? uint(_price + priceDiff) : uint(_price - priceDiff);
    }
}
