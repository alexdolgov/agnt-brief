...
contract TradingStorage is Initializable, ITradingStorage {
    ...
    function setSuiteContracts(
        ...
        _trading = IPausable(_trading);
        _callbacks = ICallbacks(_callbacks);
    ...
    function updateTp(address _trader, uint _pairIndex, uint _index, uint _newTp) external override onlyTrading {
        ...
        IPairStorage pairsStored = priceAggregator.pairsStorage();   
        _newTp = pairsStored.correctTp(t.openPrice, t.leverage, _newTp, t.buy, t.pairIndex);
        t.tp = _newTp;
...
/**
    ...
    * @param _buy Long/Short
    ...
    function claimFees(uint256 _devAmount, uint _govAmount) external onlyGov {
    ...
    function decrementFees(uint256 _devAmount, uint _govAmount) external onlyGov {
