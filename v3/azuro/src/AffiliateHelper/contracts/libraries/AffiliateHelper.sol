...
library AffiliateHelper {
    ...
    function updateContribution(
        ...
        for (uint256 i = 0; i < 2; i++) {
            ...
            if (newProfit > oldProfit)
                affiliateProfits[i] += uint128(newProfit - oldProfit);
            else if (newProfit < oldProfit) {
                assert(affiliateProfits[i] >= oldProfit - newProfit);
                affiliateProfits[i] -= uint128(oldProfit - newProfit);
            }
        }
    ...
    /**
     * @notice Get total contribution to profit of beneficial affiliates for each outcome of condition `conditionId`.
     */
    function getAffiliatedProfit(
        AffiliatedProfits storage _affiliatedProfits,
        uint256 conditionId
    ) public view returns (uint128[2] memory) {
        return _affiliatedProfits.map[conditionId];
    }
}
