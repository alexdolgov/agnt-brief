    ...
    IAffiliate
{
    ...
    function initialize(address lp_, address core_) external initializer {
     ...
     * @param  maxBetShare_ maximum percent of sandbox liquidity allowed to allocate for one express bet
     */
    function setParams(uint128 margin_, uint64 maxBetShare_)
        external
        onlyOwner
    {
        if (margin_ > FixedMath.ONE) revert IncorrectMargin();
        margin = margin_;
        if (maxBetShare_ > FixedMath.ONE) revert IncorrectMaxBetShare();
        maxBetShare = maxBetShare_;

        emit ParamsUpdated(margin_, maxBetShare_);
    }
    ...
    function putBet(
        ...
        for (uint256 i = 0; i < length; i++) {
            ...
            uint64 adjustedOdds = CoreTools
                .marginAdjustedOdds(odds, _calcMargin(margin, odds));
        ...
        uint128 payout = expressOdds.mul(amount).toUint128();
        _lockLiquidity(payout - amount);
    ...
    function _shiftOdds(
        ...
        if (divider == 0) revert OddsTooSmall();
    ...
    /**
     * @notice Lock requested amount of liquidity for express payout
     */
    function _lockLiquidity(uint128 lockAmount) internal {
        if (
            lockAmount >
            maxBetShare.mul(lp.getLockedLiquidityLimit(address(this)))
        ) {
            revert LargeBet();
        }
        lp.changeLockedLiquidity(0, lockAmount.toInt128());
    }
