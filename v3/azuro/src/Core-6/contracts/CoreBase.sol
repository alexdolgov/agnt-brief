...
abstract contract CoreBase is OwnableUpgradeable, ICoreBase {
     ...
     * @param  account AzuroBet token owner
    ...
    {
        if (azuroBet.ownerOf(tokenId) != account) revert OnlyBetOwner();
        ...
        if (_isConditionResolved(condition)) {
            if (bet.outcome == condition.outcomeWin) return (true, bet.payout);
            else return (true, 0);
        }
    ...
    /**
     * @notice Resolve AzuroBet token `tokenId` payout for `account`.
     * @param  account AzuroBet token owner
     * @param  tokenId AzuroBet token ID
        ...
        returns (uint128)
    {
        ...
        if (!accepted) azuroBet.burn(tokenId);
        else if (amount > 0) bets[tokenId].isPaid = true;
