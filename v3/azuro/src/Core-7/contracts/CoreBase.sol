...
abstract contract CoreBase is OwnableUpgradeable, ICoreBase {
    ...
    AffiliateHelper.Contributions private contributions;
    AffiliateHelper.ContributedConditionIds private contributedConditionIds;
    AffiliateHelper.AffiliatedProfits private affiliatedProfits;
    ...
    function cancelByOracle(uint256 oracleConditionId) external onlyOracle {
        _cancel(oracleConditionIds[msg.sender][oracleConditionId]);
    }

    /**
     * @notice Oracle: Change the current condition `oracleConditionId` odds.
     */
    function changeOdds(uint256 oracleConditionId, uint64[2] calldata newOdds)
        external
        onlyOracle
    {
        uint256 conditionId = oracleConditionIds[msg.sender][oracleConditionId];
     ...
     * @param  affiliate_ address indicated as an affiliate when placing bets
     * @param  start index of the first element to check reward in an array of all conditions contributed by
     *         the affiliate that are not rewarded yet
     * @param  count number of elements of the array to check. Set to 0 to check the array until the end
     * @return contribution amount
     */
    function resolveAffiliateReward(
        address affiliate_,
        uint256 start,
        uint256 count
    ) external override onlyLp returns (uint128) {
        uint256[] storage conditionIds = contributedConditionIds.map[
            affiliate_
        ];
        ...
        uint256 end = (count != 0 && start + count < conditionIds.length)
            ? start + count
            : conditionIds.length;
        while (start < end) {
            ...
            if (state == ICondition.ConditionState.RESOLVED) {
                ...
                if (profit > 0) {
                    contribution = contributions.map[affiliate_][conditionId];
                    outcomeWinIndex = condition.outcomeWin ==
                    ...
                    if (contribution.totalNetBets > payout) {
                        reward += uint128(
                            ((contribution.totalNetBets - payout) * profit) /
                                affiliatedProfits.map[conditionId][
                                    outcomeWinIndex
                                ]
                        );
                    }
                ...
                (, bool gameIsCanceled) = lp.getGameInfo(condition.gameId);
                if (
                    !gameIsCanceled &&
            ...
            delete contributions.map[affiliate_][conditionId];
        ...
        returns (bool, uint128)
    {
        ...
        uint128 balance = uint128(azuroBet.balanceOf(account, tokenId));
        ...
        if (state == ConditionState.RESOLVED) {
            if (condition.outcomeWin == condition.outcomes[outcomeIndex])
                return (
                    true,
                    uint128(azuroBet.balancePayoutOf(account, tokenId))
                );
            else return (true, 0);
        }
        if (
    ...
    /**
     * @notice Indicate condition `conditionId` with oracle ID `oracleConditionId` as canceled.
     * @notice Set oracleConditionId to zero if the function is not called by an oracle.
     * @param  conditionId the match or condition ID
     */
    function _cancel(uint256 conditionId) internal {
        Condition storage condition = _getCondition(conditionId);
        ConditionState state = condition.state;
        if (
    ...
    function _createCondition(
        ...
        uint64 margin
    ) internal onlyOracle returns (uint256 conditionId) {
        if (outcomes[0] == outcomes[1]) revert SameOutcomes();
        if (margin > FixedMath.ONE) revert IncorrectMargin();
        if (oracleConditionIds[msg.sender][oracleConditionId] != 0)
            revert ConditionAlreadyCreated();

        conditionId = azuroBet.newConditionId();
        uint64 gameStart = lp.addCondition(gameId, conditionId);

        if (gameStart <= block.timestamp) revert GameAlreadyStarted();
        ...
        oracleConditionIds[msg.sender][oracleConditionId] = conditionId;

        emit ConditionCreated(gameId, oracleConditionId, conditionId);
    }

    /**
     * @notice Indicate outcome `outcomeWin` as happened in oracle's condition `oracleConditionId`.
     * @param  oracleConditionId the match or condition ID in oracle's internal numbering
     * @param  outcomeWin ID of happened condition's outcome
     */
    function _resolveCondition(uint256 oracleConditionId, uint64 outcomeWin)
        internal
        onlyOracle
    {
        uint256 conditionId = oracleConditionIds[msg.sender][oracleConditionId];
        Condition storage condition = _getCondition(conditionId);
        {
            ...
            if (gameIsCanceled || condition.state != ConditionState.CREATED)
                revert ConditionAlreadyResolved();
        ...
        condition.profit = uint128(
            Math.diffOrZero(profitReserve, lockedReserve)
        );
        bool affiliated = AffiliateHelper.getAffiliatedProfit(
            affiliatedProfits,
            conditionId
        )[outcomeIndex] > 0; // Only to avoid StackTooDeep error
        lp.addReserve(
            condition.gameId,
            lockedReserve,
            profitReserve,
        ...
        emit ConditionResolved(
            ...
            outcomeWin,
            int128(profitReserve) - int128(lockedReserve)
        );
    }
    ...
    function _changeFunds(
        ...
        lp.changeLockedLiquidity(
            condition.gameId,
            int128(_calcReserve(reinforcement, newFunds)) -
                int128(_calcReserve(reinforcement, funds))
        );
    ...
    function _resolvePayout(address account, uint256 tokenId)
        ...
        returns (uint128)
    {
        if (azuroBet.balanceOf(account, tokenId) == 0) revert ZeroBalance();
    ...
    function _calcOdds(
        ...
        return uint64(CoreTools.marginAdjustedOdds(odds, condition.margin));
    }
