...
contract LP is OwnableUpgradeable, ILP {
    ...
    uint256 constant PERCENT1 = 10000000000; // 1%
    uint256 constant PERCENT99 = 990000000000; //99%

    uint256 public betFeeStarted;

    mapping(uint256 conditionId => uint128 amount)
        public betFeeConditionChargeback; // rejected/canceled bets, canceled conditions
    mapping(uint256 conditionId => uint128 amount) public betFeeCondition; // collected bet fee for active condition
    uint128 public affiliateRewards; // prepared bet fee for claim
    ...
    function claimAffiliateRewards() external onlyOwner {
        uint128 _rewards = affiliateRewards;
        if (_rewards > 0) {
            _withdraw(msg.sender, _rewards);
            affiliateRewards = 0;
        }
        emit AffiliateRewardsClaimed(_rewards);
    }
    ...
    /**
     * @notice Start getting bet fee
     */
    function startBetFee() external onlyOwner {
        if (betFeeStarted == 0) betFeeStarted = block.timestamp;
    }
    ...
    function betOrder(
        ...
        (
            ...
            uint128 totalAmount
        ) = _getOrderConditionBetFeeAmounts(order);

        _takeBetFee(conditionIds, feeAmounts);
    ...
    /**
     * @notice Increase/decrease amounts and odds by percent, used in core
     * increase result additionly rounded up
     */
    function changeByPercent(
        bool isIncrease,
        uint256 amount
    ) external view override isCore(msg.sender) returns (uint128 result) {
        result = (
            // not started or started in this block
            _isBetFeeNotStarted()
                ? amount
                : isIncrease
                    ? _roundUp(amount.div(PERCENT99))
                    : amount.mul(PERCENT99)
        ).toUint128();
    }

    /**
     * @notice Reject exact amount (for exact bets) of condition's fee for chargeback, used in core._rejectConditionBets
     */
    function rejectFee(
        ...
        if (_isBetFeeNotStarted()) return;
        if (timestamp <= betFeeStarted) return; // bet made before fee started
        _rejectFee(
            conditionId,
            uint128(reducedAmount.div(PERCENT99)) - reducedAmount
        );
    }

    /**
     * @notice Reject exact subbet amount (for exact combo bets) of condition's fee for chargeback, used in core.rejectComboBets
     */
    function rejectFeeComboPart(
        ...
        if (_isBetFeeNotStarted()) return;
        if (timestamp <= betFeeStarted) return; // bet made before fee started
        _rejectFee(conditionId, _getComboPartFee(betAmount, count, element));
    }

    /**
     * @notice Prepare all condition's fee for chargeback, used in core.cancelConditions
     */
    function rejectConditionFee(
        uint256 conditionId
    ) external override isCore(msg.sender) {
        if (_isBetFeeNotStarted()) return;
        uint128 amountFee = betFeeCondition[conditionId];
        if (amountFee > 0) _rejectFee(conditionId, amountFee);
    }

    /**
     * @notice Preaper condition's fee for claiming, used in core._resolveCondition
     */
    function releaseFee(
        uint256 conditionId
    ) external override isCore(msg.sender) {
        if (_isBetFeeNotStarted()) return;
        uint128 conditionFee = betFeeCondition[conditionId];

        if (conditionFee > 0) {
            betFeeCondition[conditionId] = 0;
            affiliateRewards += conditionFee;
        }
    }
    ...
    function viewPayout(
        address core,
        uint256 tokenId
    ) external view isCore(core) returns (uint128 payout) {
        payout = IBetting(core).viewPayout(tokenId);

        // combo loss
        if (payout == 0) return 0;

        (, uint128 betAmount) = ICore(core).getBetAmountCondition(tokenId);
        (uint64 timestemp, IOrder.ComboPartOdds[] memory comboParts) = IBetting(
            core
        ).getBetData(tokenId);

        // If bet fee started and bet fee charged, for rejected cases needs to return fee
        if (betFeeStarted > 0 && timestemp > betFeeStarted) {
            if (comboParts.length == 0) {
                // ordinary bet
                if (payout == betAmount) {
                    payout = uint128(payout.div(PERCENT99));
                }
            } else {
                // combo bet
                (uint128 amount, , ) = _viewComboChargeBack(
                    core,
                    betAmount,
                    comboParts
                );
    ...
    function _withdrawPayout(
        ...
        if (amount > 0) {
            (, uint128 betAmount) = ICore(core).getBetAmountCondition(tokenId);
            (uint64 timestemp, ComboPartOdds[] memory comboParts) = IBetting(
                core
            ).getBetData(tokenId);
            ...
            if (betFeeStarted > 0 && timestemp > betFeeStarted) {
                if (comboParts.length == 0) {
                    // ordinary bet reject
                    if (amount == betAmount) {
                        uint128 restoredAmount = uint128(amount.div(PERCENT99));
                        _returnFee(core, tokenId, restoredAmount - amount);
                        amount = restoredAmount;
                    }
                } else {
                    // combo bet rejected (partially)
                    (
                        uint128 result,
                        uint256[] memory conditionIds,
                        uint128[] memory subFeeAmounts
                    ) = _viewComboChargeBack(core, betAmount, comboParts);
                    for (uint256 i; i < conditionIds.length; ++i) {
                        _returnFeeByCondition(
                            conditionIds[i],
                            subFeeAmounts[i]
                        );
                    }
    ...
    /**
     * @notice Check if bet fee not start, if `betFeeStarted` have time value it is started for next block time and further
     */
    function _isBetFeeNotStarted() internal view returns (bool) {
        return (betFeeStarted == 0 || betFeeStarted == block.timestamp);
    }
    ...
    /**
     * @notice Take fee from the bet
     */
    function _takeBetFee(
        uint256[] memory conditionIds,
        uint128[] memory amounts
    ) internal {
        if (_isBetFeeNotStarted()) return;
        for (uint256 i; i < conditionIds.length; ++i) {
            betFeeCondition[conditionIds[i]] += amounts[i];
        }
    }

    function _viewComboChargeBack(
        address core,
        uint128 betAmount,
        IOrder.ComboPartOdds[] memory parts
    )
        internal
        view
        returns (
    ...
    {
        uint128 amount;
        conditionIds = new uint256[](parts.length);
        subFeeAmounts = new uint128[](parts.length);
        for (uint256 i; i < parts.length; ++i) {
            if (
                parts[i].odds == FixedMath.ONE ||
                IBetting(core).isConditionCanceled(parts[i].conditionId)
            ) {
                amount = _getComboPartFee(betAmount, parts.length, i);
                conditionIds[i] = parts[i].conditionId;
                subFeeAmounts[i] = amount;
                result += amount;
            }
        }
    }
    ...
    /**
     * @notice Divide combo amount by parts and last part can be differ,
     *         example 7/3 is 2,2,3
     */
    function _getComboPartFee(
        uint128 betAmount,
        uint256 count,
        uint256 element
    ) internal pure returns (uint128 amountFee) {
        // restore original bet amount and calculate bet 1% fee
        uint256 originalAmountFee = betAmount.div(PERCENT99).mul(PERCENT1);
        //get fee part amount
        uint128 amountPart = uint128(originalAmountFee / count);
        return
            _getElementOfParts(originalAmountFee, amountPart, count, element);
    }

    function _getElementOfParts(
        uint256 originalAmountFee,
        uint256 amountPart,
        uint256 count,
        uint256 element
        ...
        return
            uint128(
                element < count - 1
                    ? amountPart
                    : originalAmountFee - amountPart * element
            ); // last part got the rest
    }

    function _rejectFee(uint256 conditionId, uint128 amountFee) internal {
        betFeeCondition[conditionId] -= amountFee;
        betFeeConditionChargeback[conditionId] += amountFee;
    }
    ...
    function _returnFee(
        ...
        (uint256 conditionId, ) = ICore(core).getBetAmountCondition(tokenId);
        _returnFeeByCondition(conditionId, amount);
    }

    function _returnFeeByCondition(
        uint256 conditionId,
        uint128 amount
    ) internal {
        if (betFeeConditionChargeback[conditionId] < amount)
            revert InsufficientReward();
        betFeeConditionChargeback[conditionId] -= amount;
    }

    /**
     * @notice Rounding value up accordingly to accuracy,
     *         example 1262626262626 -> 1270000000000
     */
    function _roundUp(uint256 input) internal pure returns (uint256 output) {
        output = (input / 10000000000) * 10000000000;
        return output < input ? output + 10000000000 : output;
    }

    function _getOrderConditionBetFeeAmounts(
        ...
        )
    {
        ...
        if (order.betType == BetType.COMBO) {
            ClientComboBetData memory combodata = abi.decode(
                order.clientBetData,
                (ClientComboBetData)
            );

            ComboPart[] memory comboParts = combodata.comboParts;

            totalAmount = combodata.amount;
            uint128 reducedAmount = uint128((combodata.amount).mul(PERCENT1));
            uint128 amountPart = uint128(reducedAmount / comboParts.length);
            conditionIds = new uint256[](comboParts.length);
            feeAmounts = new uint128[](comboParts.length);

            for (uint256 i; i < conditionIds.length; ++i) {
                _amount = _getElementOfParts(
                    reducedAmount,
                    amountPart,
                    comboParts.length,
                    i
                );
                conditionIds[i] = comboParts[i].conditionId;
                feeAmounts[i] = _amount;
            }
            return (conditionIds, feeAmounts, totalAmount);
        }
        ...
        conditionIds = new uint256[](betdata.bets.length);
        feeAmounts = new uint128[](betdata.bets.length);
        for (uint256 i; i < betdata.bets.length; ++i) {
            conditionIds[i] = betdata.bets[i].conditionId;
            totalAmount += betdata.bets[i].amount;
            _amount = uint128(uint256(betdata.bets[i].amount).mul(PERCENT1));
            feeAmounts[i] = _amount;
        }
        return (conditionIds, feeAmounts, totalAmount);
    }
