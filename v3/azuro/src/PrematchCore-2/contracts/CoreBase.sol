...
abstract contract CoreBase is OwnableUpgradeable, ICoreBase {
    ...
    function _createCondition(
        ...
        uint256 conditionId_ = conditionId; // Avoid StackTooDeepError without changing the order of function parameters.
        if (conditionId_ == 0) revert IncorrectConditionId();
        ...
        for (uint256 i = 0; i < length; ++i) {
            ...
            if (outcomeNumbers[conditionId_][outcome] != 0)
                revert DuplicateOutcomes(outcome);
            outcomeNumbers[conditionId_][outcome] = i + 1;
        }
        ...
        emit ConditionCreated(gameId, conditionId_, outcomes);
        emit CreatedConditionMetadata(conditionId_, data);
    }
