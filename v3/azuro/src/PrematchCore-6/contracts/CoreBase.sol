...
abstract contract CoreBase is OwnableUpgradeable, ICoreBase {
    ...
    /**
     * @notice See {ICoreBase-changeConditionSettings}.
     */
    function changeConditionSettings(
        uint256 conditionId,
        uint64 newMargin,
        uint128 newReinforcement
    ) external restricted(this.changeConditionSettings.selector) {
        Condition storage condition = _getCondition(conditionId);
        ...
        emit ConditionSettingsChanged(conditionId, newMargin, newReinforcement);
    }
    ...
    function _createCondition(
        ...
        emit ConditionCreated(condition);
    }
    ...
    function _applyOdds(
        ...
        for (uint256 i = 0; i < length; ++i) {
            uint256 virtualFund = (fund * FixedMath.ONE.div(odds[i])) /
                normalizer;
            if (virtualFund >= maxVirtualFund) revert CoreTools.IncorrectOdds();

            condition.virtualFunds[i] = virtualFund.toUint128();
        }
    }
