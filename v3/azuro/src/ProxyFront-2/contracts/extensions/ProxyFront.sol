...
contract ProxyFront is IProxyFront {
    ...
    function withdrawPayouts(WithdrawPayoutData[] calldata data) external {
        for (uint256 i = 0; i < data.length; ++i) {
            ...
            if (data[i].withdrawalType != WithdrawalType.TOKEN && payout > 0) {
                address account = data[i].withdrawalType ==
                    WithdrawalType.NATIVE_PREMATCH
                    ? core.azuroBet().ownerOf(data[i].tokenId)
                    : IERC721Upgradeable(data[i].core).ownerOf(data[i].tokenId);
