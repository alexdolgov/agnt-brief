...
contract Trading is OwnableUpgradeable, ITrading {
    ...
    function placeLimitOrder(
        ...
        trade.leveragedAmount = collateralAmount.mul(leverage);
        trade.isLong = isLong;
    ...
    function _getMarketImbalanceFeeRate(Trade storage trade) internal view returns (uint256) {
        ...
        Market storage market = _getMarket(trade);
        if (market.totalLong == 0 || market.totalShort == 0) return 0;

        uint256 marketImbalance = trade.isLong
            ? (market.totalLong).div(market.totalShort)
            : (market.totalShort).div(market.totalLong);
    ...
    function _tryResolveOrder(
        ...
        uint256 tradingFee = trade.leveragedAmount.mul(tradingFeeRate);
        uint256 marketImbalanceFee = trade.leveragedAmount.mul(_getMarketImbalanceFeeRate(trade));
