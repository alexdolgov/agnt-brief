...
contract Trading is OwnableUpgradeable, ITrading {
    ...
    // Temporary workaround. This variable will be removed once all trades have been migrated to the next version.
    uint256 public holdingFeeFromTradeId;
    ...
    modifier updateHolding(address collateralToken) {
        _accrueHoldingFee(collateralToken);
        _;
    }
    ...
    function initializeV2() external reinitializer(2) {
        holdingFeeFromTradeId = nextTradeId;
    }
    ...
    /**
     * @notice Owner: updates the holding fee rate for a specific collateral token.
     * @param token The collateral token for which the holding fee rate is being updated.
     * @param newHoldingFeeRate The new holding fee rate to set (per second, fixed-point, < 1.0).
     */
    function changeHoldingFeeRate(
        address token,
        uint64 newHoldingFeeRate
    ) external onlyOwner updateHolding(token) {
        if (newHoldingFeeRate == collateralTokens[token].holdingFeeRate) revert NoChanges();
        newHoldingFeeRate.requireLessThanOne();

        collateralTokens[token].holdingFeeRate = newHoldingFeeRate;

        emit HoldingFeeRateChanged(token, newHoldingFeeRate);
    }
    ...
    function placeMarketOrder(
        ...
        uint256 indexPrice = priceFeed.getPrice(indexId);
    ...
    function canProcessTrades(
        ...
        for (uint256 i; i < length; ++i) {
            ...
            try priceFeed.getPrice(trade.indexId) {} catch {
                result[i] = false;
                continue;
            }
            if (trade.status == TradeStatus.ORDER) {
                ...
                (, , , , PositionCloseStatus status) = _evaluatePositionClose(
                    trade,
                    _isHoldingFeeEnabled(tradeId)
                );
    ...
    function _getMarketImbalanceFeeRate(
        ...
        uint256 virtualFund = pool.getVirtualFund(indexId);
        uint256 totalLong = market.totalLong + virtualFund;
        uint256 totalShort = market.totalShort + virtualFund;
        if (maxMarketImbalanceFeeRate == 0 || isLong ? totalShort == 0 : totalLong == 0) return 0;

        uint256 marketImbalance = isLong
            ? (totalLong + leveragedAmount).div(totalShort)
            : (totalShort + leveragedAmount).div(totalLong);
    ...
    function getHoldingFee(uint256 tradeId) external view returns (uint256) {
        return _getHoldingFee(_getPosition(tradeId), _isHoldingFeeEnabled(tradeId));
    }
    ...
    function getLiquidationPrice(
        ...
        }
        {
            ...
            int256 sumCollateralFunding = int256(trade.collateralAmount);
    ...
    function getMarketImbalanceFeeRate(
        ...
        return
            _getMarketImbalanceFeeRate(
                ...
                collateralAmount.mul(leverage).mul(FixedMath.ONE - tradingFeeRate),
                isLong
            );
    }

    /**
     * @notice Calculates the additional fee rate based on the current market imbalance and funding fee rate.
     */
    function getFeeRates(
        ...
        bool isLong,
        uint256 indexId
    ) external view returns (uint256 marketImbalanceFeeRate, int256 fundingRate) {
        uint256 leveragedAmount = collateralAmount.mul(leverage).mul(
            FixedMath.ONE - tradingFeeRate
        );

        Market storage market = markets[indexId][collateralToken];
        marketImbalanceFeeRate = _getMarketImbalanceFeeRate(
            indexId,
            market,
            leveragedAmount,
            isLong
        );
        leveragedAmount -= leveragedAmount.mul(marketImbalanceFeeRate);

        uint256 totalLong = market.totalLong;
        uint256 totalShort = market.totalShort;
        if (isLong) {
            totalLong += leveragedAmount;
        } else {
            totalShort += leveragedAmount;
        }
        ...
        if (totalLong == 0 || totalShort == 0) {
            ...
            if (totalLong > totalShort) {
                fundingRate = int256(((totalLong - totalShort) * baseFundingRate) / totalLong);
            } else {
                fundingRate = -int256(((totalShort - totalLong) * baseFundingRate) / totalShort);
            }
        }
    }
    ...
    function getPayout(uint256 tradeId) external view returns (uint256 payout) {
        (payout, , , , ) = _getPayout(_getPosition(tradeId), _isHoldingFeeEnabled(tradeId), false);
    }
    ...
    function _tryResolveOrder(
        ...
        Trade storage trade,
        bool orderValidated
    ) internal updateFunding(trade) updateHolding(trade.collateralToken) {
        ...
        uint256 marketImbalanceFee = leveragedAmount.mul(
            _getMarketImbalanceFeeRate(trade.indexId, market, leveragedAmount, trade.isLong)
        );
        ...
        collateralTokens[trade.collateralToken].totalPositionsSize += leveragedAmount;
        if (trade.isLong) market.totalLong += leveragedAmount;
        else market.totalShort += leveragedAmount;

        uint256 maxPayout = _getMaxPayout(trade);
        trade.maxPayout = maxPayout;

        trade.entryHoldingRate = _getCurrentHoldingRate(trade.collateralToken);
    ...
    function _tryClosePosition(
        ...
        address feeReceiver
    ) internal updateFunding(trade) updateHolding(trade.collateralToken) returns (bool) {
        (
            uint256 payout,
            uint256 indexPrice,
            int256 fundingPnl,
        ...
        ) = _evaluatePositionClose(trade, _isHoldingFeeEnabled(tradeId));
        if (!requestedByTrader && status == PositionCloseStatus.PENDING) return false;

        trade.status = TradeStatus.CLOSED;
        // Temporary workaround. This condition will be removed once all trades have been migrated to the next version.
        if (_isHoldingFeeEnabled(tradeId)) {
            collateralTokens[trade.collateralToken].totalPositionsSize -= trade.leveragedAmount;
        }
        Market storage market = _getMarket(trade);
        if (trade.isLong) {
        ...
        emit ExtraFeesAccrued(tradeId, fundingPnl, holdingFee);
    ...
    function _accrueHoldingFee(address collateralToken) internal {
        CollateralToken storage token = collateralTokens[collateralToken];

        if (token.holdingFeeAccruedAt == 0) {
            token.holdingFeeAccruedAt = block.timestamp;
            return;
        }

        uint256 timeDelta = block.timestamp - token.holdingFeeAccruedAt;
        if (timeDelta == 0 || token.holdingFeeRate == 0) {
            token.holdingFeeAccruedAt = block.timestamp;
            return;
        }

        token.cumulativeHoldingRate += uint256(token.holdingFeeRate) * timeDelta;
        token.holdingFeeAccruedAt = block.timestamp;
    }
    ...
    function _evaluateOrderExecution(Trade storage trade) internal view returns (bool ok) {
        uint256 price = priceFeed.getPrice(trade.indexId);
        ok =
        ...
        )
    {
        ...
        (payout, indexPrice, fundingPnl, holdingFee, maxPayoutReached) = _getPayout(
            trade,
            holdingFeeEnabled,
            true
        );
        ...
        if (
            payout <= liquidationThreshold.mul(trade.collateralAmount) ||
    ...
    function _getCurrentHoldingRate(address collateralToken) internal view returns (uint256) {
        CollateralToken storage token = collateralTokens[collateralToken];

        if (token.holdingFeeAccruedAt == 0) {
            return token.cumulativeHoldingRate;
        }
        ...
        return token.cumulativeHoldingRate + uint256(token.holdingFeeRate) * timeDelta;
    }

    function _getHoldingFee(
        Trade storage trade,
        bool holdingFeeEnabled
    ) internal view returns (uint256) {
        if (!holdingFeeEnabled) return 0;

        uint256 currentRate = _getCurrentHoldingRate(trade.collateralToken);
        if (currentRate <= trade.entryHoldingRate) return 0;

        uint256 deltaRate = currentRate - trade.entryHoldingRate;
        return trade.leveragedAmount.mul(deltaRate);
    }
    ...
    {
        try priceFeed.getPrice(trade.indexId) returns (uint256 indexPrice_) {
            indexPrice = indexPrice_;
        } catch (bytes memory data) {
            if (bytes4(data) == IPriceFeed.OutdatedPrice.selector) {
                if (revertOutdatedPrice) revert IPriceFeed.OutdatedPrice(trade.indexId);
                (indexPrice, ) = priceFeed.oraclePrices(trade.indexId);
            } else {
                assembly {
        ...
        holdingFee = _getHoldingFee(trade, holdingFeeEnabled);
        fundingPnL = _getFundingPnL(trade);

        rawPayout += fundingPnL - int256(holdingFee);
    ...
    function _isHoldingFeeEnabled(uint256 tradeId) internal view returns (bool) {
        return tradeId >= holdingFeeFromTradeId;
    }
