...
contract BridgeAssist is AccessControl, Pausable, EIP712 {
    ...
    uint256 public dailyFulfillLimit;
    ...
    event ChainAdded(string chain, uint256 exchangeRateFromPow);
    event ChainRemoved(string chain);
    event RelayerSet(address indexed relayer);
    event RelayerConsensusThresholdSet(uint256 indexed threshold);
    event FeeSet(uint256 indexed feeSend, uint256 indexed feeFulfill);
    event FeeWalletSet(address indexed feeWallet);
    event LimitPerSendSet(uint256 indexed limitPerSend);
    event Withdrawn(
        IERC20 indexed token,
        address indexed to,
        uint256 amount
    );
    event DailyFulfillLimitSet(uint256 dailyFulfillLimit, uint256 dayLength);
    ...
    constructor(
        ...
        require(dailyFulfillLimit_ != 0, 'Daily fulfill limit is zero');
        require(dayLength_ != 0, 'Day length is zero');
        ...
        dailyFulfillLimit = dailyFulfillLimit_;
        ...
        whenNotPaused
    {
        ...
        require(bridgedToday + amount < dailyFulfillLimit, 'Daily limit');
        bridgedToday += amount;
        ...
        onlyRole(MANAGER_ROLE)
    {
        ...
        for (uint256 i; i < chains.length; ) {
            ...
            emit ChainAdded(chains[i], exchangeRatesFromPow[i]);
            unchecked {
                ++i;
            }
        }
    ...
    /**
     * @notice Allows to set daily fulfill limit
     * @param dailyFulfillLimit_ - daily fulfill limit
     * @param dayLength_ - period length in seconds (can be != 1 day)
     */
    function setDailyFulfillLimit(
        uint256 dailyFulfillLimit_,
        uint256 dayLength_
    ) external onlyRole(MANAGER_ROLE) {
        require(dailyFulfillLimit_ != 0, 'Daily fulfill limit is zero');
        require(
            dailyFulfillLimit_ != dailyFulfillLimit || dayLength_ != dayLength,
            'Moot state change'
        );
        require(dayLength_ != 0, 'Day length is zero');
        ...
        dailyFulfillLimit = dailyFulfillLimit_;
        dayLength = dayLength_;
        emit DailyFulfillLimitSet(dailyFulfillLimit_, dayLength_);
    }
    ...
    /**
     * @notice Allows to reset daily bridged limit
     */
    function resetDailyLimit() external onlyRole(MANAGER_ROLE) {
        lastDay = 0;
        bridgedToday = 0;
