...
// File: contracts/babyDogeDoggies.sol

pragma solidity ^0.6.0;

contract BabyDogeDoggies is ERC721, Ownable {
    using Counters for Counters.Counter;

    // Map the number of tokens per doggieId
    mapping(uint8 => uint256) public doggieCount;

    // Map the number of tokens burnt per doggieId
    mapping(uint8 => uint256) public doggieBurnCount;

    // Used for generating the tokenId of new NFT minted
    Counters.Counter private _tokenIds;

    // Map the doggieId for each tokenId
    mapping(uint256 => uint8) private doggieIds;

    // Map the doggieName for a tokenId
    mapping(uint8 => string) private doggieNames;
    ...
    /**
     * @dev Get doggieId for a specific tokenId.
     */
    function getdoggieId(uint256 _tokenId) external view returns (uint8) {
        return doggieIds[_tokenId];
    }

    /**
     * @dev Get the associated doggieName for a specific doggieId.
     */
    function getdoggieName(uint8 _doggieId)
        external
        view
        returns (string memory)
    {
        return doggieNames[_doggieId];
    }

    /**
     * @dev Get the associated doggieName for a unique tokenId.
     */
    function getdoggieNameOfTokenId(uint256 _tokenId)
        external
        view
        returns (string memory)
    {
        uint8 doggieId = doggieIds[_tokenId];
        return doggieNames[doggieId];
    }
    ...
    function mint(
        ...
        _tokenIds.increment();
        doggieIds[newId] = _doggieId;
        doggieCount[_doggieId] = doggieCount[_doggieId].add(1);
        _mint(_to, newId);
    ...
    /**
     * @dev Set a unique name for each doggieId. It is supposed to be called once.
     */
    function setdoggieName(uint8 _doggieId, string calldata _name)
        external
        onlyOwner
    {
        doggieNames[_doggieId] = _name;
    }
    ...
    function burn(uint256 _tokenId) external onlyOwner {
        uint8 doggieIdBurnt = doggieIds[_tokenId];
        doggieCount[doggieIdBurnt] = doggieCount[doggieIdBurnt].sub(1);
        doggieBurnCount[doggieIdBurnt] = doggieBurnCount[doggieIdBurnt].add(1);
        _burn(_tokenId);
    }
}

// File: contracts/doggieFactoryV2.sol

pragma solidity ^0.6.12;

contract DoggieFactoryV2 is Ownable {
    using SafeMath for uint256;
    using SafeBEP20 for IBEP20;

    BabyDogeDoggies public babyDogeDoggies;
    ...
    uint8 private constant numberdoggieIds = 10;

    // number of previous series (i.e. different visuals)
    uint8 private constant previousNumberdoggieIds = 5;

    // Map the token number to URI
    mapping(uint8 => string) private doggieIdURIs;
    ...
    function mintNFT(uint8 _doggieId) external {
        ...
        require(_doggieId >= previousNumberdoggieIds, "doggieId too low");
        // Check that the _doggieId is within boundary:
        ...
        string memory tokenURI = doggieIdURIs[_doggieId];

        uint256 tokenId =
            babyDogeDoggies.mint(address(_msgSender()), tokenURI, _doggieId);

        emit doggieMint(_msgSender(), tokenId, _doggieId);
    }
     ...
     * Assign tokenURI to look for each doggieId in the mint function
     * Only the owner can set it.
     */
    function setdoggieJson(
        string calldata _doggieId5Json,
        string calldata _doggieId6Json,
        string calldata _doggieId7Json,
        string calldata _doggieId8Json,
        string calldata _doggieId9Json
    ) external onlyOwner {
        doggieIdURIs[5] = string(abi.encodePacked(ipfsHash, _doggieId5Json));
        doggieIdURIs[6] = string(abi.encodePacked(ipfsHash, _doggieId6Json));
        doggieIdURIs[7] = string(abi.encodePacked(ipfsHash, _doggieId7Json));
        doggieIdURIs[8] = string(abi.encodePacked(ipfsHash, _doggieId8Json));
        doggieIdURIs[9] = string(abi.encodePacked(ipfsHash, _doggieId9Json));
    }
     ...
     * Only the owner can set it.
     */
    function setdoggieNames(
        string calldata _doggieId5,
        string calldata _doggieId6,
        string calldata _doggieId7,
        string calldata _doggieId8,
        string calldata _doggieId9
    ) external onlyOwner {
        babyDogeDoggies.setdoggieName(5, _doggieId5);
        babyDogeDoggies.setdoggieName(6, _doggieId6);
        babyDogeDoggies.setdoggieName(7, _doggieId7);
        babyDogeDoggies.setdoggieName(8, _doggieId8);
        babyDogeDoggies.setdoggieName(9, _doggieId9);
    }
...
// File: contracts/doggieMintingStation.sol
...
contract DoggieMintingStation is AccessControl {
    ...
    constructor(BabyDogeDoggies _babyDogeDoggies) public {
        babyDogeDoggies = _babyDogeDoggies;
    ...
    function mintCollectible(
        ...
        uint256 tokenId =
            babyDogeDoggies.mint(_tokenReceiver, _tokenURI, _doggieId);
        return tokenId;
    }
     ...
     */
    function setdoggieName(uint8 _doggieId, string calldata _doggieName)
        external
        onlyOwner
    {
        babyDogeDoggies.setdoggieName(_doggieId, _doggieName);
    }
...
contract DoggieFactoryV3 is Ownable {
    using SafeMath for uint256;
    using SafeBEP20 for IBEP20;

    DoggieFactoryV2 public doggieFactoryV2;
    ...
    constructor(
        DoggieFactoryV2 _doggieFactoryV2,
    ...
    function mintNFT(uint8 _doggieId) external {
        ...
        bool hasClaimedV2 = doggieFactoryV2.hasClaimed(senderAddress);
    ...
    function canMint(address userAddress) external view returns (bool) {
        if (
            ...
            (doggieFactoryV2.hasClaimed(userAddress))
        ) {
            return false;
        } else {
            return true;
