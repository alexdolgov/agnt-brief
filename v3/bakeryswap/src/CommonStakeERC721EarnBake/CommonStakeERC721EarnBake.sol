...
interface ICommonStakeERC721EarnBake {
    ...
    event EmergencyUnstakeAllFromBake(address indexed user);
    ...
    function emergencyUnstakeAllFromBake() external;
    ...
    PausableUpgradeable
{
    ...
    IBakeryMaster public constant bakeryMaster = IBakeryMaster(0x20eC291bB8459b6145317E7126532CE7EcE5056f);
    mapping(uint256 => bool) private _mintPowers;
    mapping(address => UserInfo) private _userInfoMap;
    mapping(address => EnumerableSetUpgradeable.UintSet) private _stakingTokens;
    ...
    function initialize(
        ...
        erc721 = IERC721Upgradeable(_erc721);
    }
    ...
    function pendingBake(address _user) external view override returns (uint256) {
        ...
        if (totalStakingPower != 0) {
            uint256 totalPendingBake = bakeryMaster.pendingBake(address(this), address(this));
    ...
    function updateStaking() public override {
        ...
        bakeryMaster.deposit(address(this), 0);
    ...
    function _harvest(UserInfo storage userInfo) internal {
        ...
        if (userInfo.stakingPower != 0) {
            uint256 pending =
                userInfo.stakingPower.mul(accBakePerShare).div(accBakePerShareMultiple).sub(userInfo.rewardDebt);
            if (pending != 0) {
    ...
    function stake(uint256 _tokenId) public override whenNotPaused {
        ...
        if (!_mintPowers[_tokenId]) {
            _mint(address(this), ONE_POWER_EIGHTEEN);
        }

        erc721.safeTransferFrom(_msgSender(), address(this), _tokenId);
        userInfo.stakingPower = userInfo.stakingPower.add(ONE_POWER_EIGHTEEN);
        _stakingTokens[_msgSender()].add(_tokenId);
        _approveToMasterIfNecessary(ONE_POWER_EIGHTEEN);
        bakeryMaster.deposit(address(this), ONE_POWER_EIGHTEEN);
        totalStakingPower = totalStakingPower.add(ONE_POWER_EIGHTEEN);
        userInfo.rewardDebt = userInfo.stakingPower.mul(accBakePerShare).div(accBakePerShareMultiple);
        emit Stake(_msgSender(), _tokenId, ONE_POWER_EIGHTEEN);
    }
    ...
    function unstake(uint256 _tokenId) public override {
        ...
        UserInfo storage userInfo = _userInfoMap[_msgSender()];
        _harvest(userInfo);
        userInfo.stakingPower = userInfo.stakingPower.sub(ONE_POWER_EIGHTEEN);
        _stakingTokens[_msgSender()].remove(_tokenId);
        erc721.safeTransferFrom(address(this), _msgSender(), _tokenId);
        bakeryMaster.withdraw(address(this), ONE_POWER_EIGHTEEN);
        totalStakingPower = totalStakingPower.sub(ONE_POWER_EIGHTEEN);
        userInfo.rewardDebt = userInfo.stakingPower.mul(accBakePerShare).div(accBakePerShareMultiple);
        emit Unstake(_msgSender(), _tokenId, ONE_POWER_EIGHTEEN);
    }
    ...
    function unstakeAll() external override {
        EnumerableSetUpgradeable.UintSet storage stakingTokens = _stakingTokens[_msgSender()];
    ...
    function emergencyUnstake(uint256 tokenId) external override {
        require(_stakingTokens[_msgSender()].contains(tokenId), 'EMERGENCY UNSTAKE FORBIDDEN');
        UserInfo storage userInfo = _userInfoMap[_msgSender()];
        userInfo.stakingPower = userInfo.stakingPower.sub(ONE_POWER_EIGHTEEN);
        _stakingTokens[_msgSender()].remove(tokenId);
        erc721.safeTransferFrom(address(this), _msgSender(), tokenId);
        totalStakingPower = totalStakingPower.sub(ONE_POWER_EIGHTEEN);
        userInfo.rewardDebt = userInfo.stakingPower.mul(accBakePerShare).div(accBakePerShareMultiple);
        emit EmergencyUnstake(_msgSender(), tokenId, ONE_POWER_EIGHTEEN);
    }

    function emergencyUnstakeAllFromBake() external override onlyOwner whenPaused {
        bakeryMaster.emergencyWithdraw(address(this));
        emit EmergencyUnstakeAllFromBake(_msgSender());
    }

    function safeBakeTransfer(address _to, uint256 _amount) internal {
        uint256 bakeBal = IERC20Upgradeable(bakeryToken).balanceOf(address(this));
        if (_amount > bakeBal) {
            IERC20Upgradeable(bakeryToken).transfer(_to, bakeBal);
        } else {
            IERC20Upgradeable(bakeryToken).transfer(_to, _amount);
        }
    }
