...
// File: contracts/StakingRewardsGas.sol
...
contract StakingRewardsGas is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    receive() external payable {
        assert(msg.sender == address(wrappedGasToken)); // only accept ETH via fallback from the WETH contract
    }
    ...
    function getReward() public nonReentrant updateReward(msg.sender) {
        ...
        if (reward > 0) {
            ...
            if (isContract(msg.sender)) {
                wrappedGasToken.safeTransfer(msg.sender, reward);
            } else {
                IWETH(address(wrappedGasToken)).withdraw(reward);
    ...
    function notifyRewardAmount(uint256 reward) external onlyOwner updateReward(address(0)) {
        ...
        uint balance = wrappedGasToken.balanceOf(address(this));
    ...
    function setwrappedGasToken(address token) external onlyOwner {
        wrappedGasToken = IERC20(token);
    }
