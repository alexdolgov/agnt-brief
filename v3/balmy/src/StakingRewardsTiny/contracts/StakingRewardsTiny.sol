...
contract StakingRewardsTiny is IStakingRewards, Ownable, ReentrancyGuard {
    ...
    function rewardPerToken() public view returns (uint256) {
        ...
        return
            rewardPerTokenStored.add(
                lastTimeRewardApplicable().sub(lastUpdateTime).mul(1e18).mul(rewardAmount).div(rewardsDuration).div(_totalSupply)
            );
    }
    ...
    function notifyRewardAmount(uint256 reward) external onlyOwner updateReward(address(0)) {
        if (block.timestamp >= periodFinish) {
            ...
            // require(false, "I don't know what happens on this branch");
        ...
        require(rewardAmount <= balance, "Provided reward too high");
