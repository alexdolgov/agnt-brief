...
contract EuroBet is Ownable {
    ...
    function settleMatchResult(
        ...
        if (msg.sender != owner() || now > endBettingTime.add(48 hours)) {
    ...
    function buyTicket(
        ...
        uint32 _betOdd,
        uint256 _betAmount
    ) public returns (uint256 _ticketIndex) {
...
contract EuroBetFactory {
    ...
    function createEuroBet(
        string memory _matchName,
        uint256 _startBettingTime,
        uint256 _endBettingTime,
        string[] memory _betDescriptions,
        uint32[] memory _odds1x2,
        uint32[] memory _oddsHandicap,
        uint32[] memory _oddsOverUnder,
        uint256[] memory _maxBudgets
    ) external returns (EuroBet bet) {
        bet = new EuroBet();
        bet.addMatchInfo(_matchName, _startBettingTime, _endBettingTime, _betDescriptions[0], _odds1x2, _betDescriptions[1], _oddsHandicap, _betDescriptions[2], _oddsOverUnder, _maxBudgets, fund);
