...
contract PollyToken is ERC20("Panda Token", "PNDA"), Ownable, Authorizable {
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "Polly::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "Polly::delegateBySig: invalid nonce");
        require(now <= expiry, "Polly::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "Polly::getPriorVotes: not yet determined");
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "Polly::_writeCheckpoint: block number exceeds 32 bits");
...
// File: browser/PollyMasterFarmer.sol

pragma solidity 0.6.12;


interface IMigratorToPollySwap {
    // Perform LP token migration from legacy UniswapV2 to PollySwap.
    // Take the current LP token address and return the new LP token address.
    // Migrator should have full access to the caller's LP token.
    // Return the new LP token address.
    //
    // XXX Migrator must have allowance access to UniswapV2 LP tokens.
    // PollySwap must mint EXACTLY the same amount of PollySwap LP tokens or
    // else something bad will happen. Traditional UniswapV2 does not
    // do that so be careful!
    function migrate(IERC20 token) external returns (IERC20);
}

// PollyMasterFarmer is the master of Polly. He can make Polly and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once Polly is sufficiently
// distributed and the community can show to govern itself.
//
contract PollyMasterFarmer is Ownable, Authorizable {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of Pollys
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accPollyPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accPollyPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. Pollys to distribute per block.
        uint256 lastRewardBlock;  // Last block number that Pollys distribution occurs.
        uint256 accPollyPerShare; // Accumulated Pollys per share, times 1e12. See below.
    }

    // The Polly TOKEN!
    PollyToken public Polly;
    ...
    // Polly tokens created per block.
    uint256 public REWARD_PER_BLOCK;
    // Bonus muliplier for early Polly makers.
    ...
    // The block number when Polly mining starts.
    ...
    IMigratorToPollySwap public migrator;
    ...
    event SendPollyReward(address indexed user, uint256 indexed pid, uint256 amount, uint256 lockAmount);
    ...
    function add(uint256 _allocPoint, IERC20 _lpToken, bool _withUpdate) public onlyOwner {
        require(poolId1[address(_lpToken)] == 0, "PollyMasterFarmer::add: lp is already in pool");
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    // Update the given pool's Polly allocation point. Can only be called by the owner.
    ...
    function setMigrator(IMigratorToPollySwap _migrator) public onlyOwner {
        migrator = _migrator;
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 PollyForDev;
        uint256 PollyForFarmer;
		uint256 PollyForLP;
		uint256 PollyForCom;
		uint256 PollyForFounders;
        (PollyForDev, PollyForFarmer, PollyForLP, PollyForCom, PollyForFounders) = getPoolReward(pool.lastRewardBlock, block.number, pool.allocPoint);
        Polly.mint(address(this), PollyForFarmer);
        pool.accPollyPerShare = pool.accPollyPerShare.add(PollyForFarmer.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
		if (PollyForCom > 0) {
            Polly.mint(comfundaddr, PollyForCom);
            ...
            if (block.number <= FINISH_BONUS_AT_BLOCK) {
                Polly.lock(address(comfundaddr), PollyForCom.mul(75).div(100));
            }
        }
		if (PollyForDev > 0) {
            Polly.mint(devaddr, PollyForDev);
			//Polly rewards to go to the dev treasury to support technical bounties, hiring, infrastructure costs and other on-going fees.
			if (block.number <= FINISH_BONUS_AT_BLOCK) {
                Polly.lock(address(devaddr), PollyForDev.mul(50).div(100));
            }
        }
        if (PollyForFounders > 0) {
            Polly.mint(founderaddr, PollyForFounders);
			//Polly rewards for the founding dev team members directly. Unlocks at the same pace as user tokens.
			if (block.number <= FINISH_BONUS_AT_BLOCK) {
                Polly.lock(address(founderaddr), PollyForFounders.mul(75).div(100));
            }
        }
        if (PollyForLP > 0) {
            Polly.mint(liquidityaddr, PollyForLP);
			//Polly rewards for Bao holders which will be dripped out over time in staking contract on mainnet. It will be dripped out over many years based on staking of Bao, lockedBao, BaoNFTs and other franchise tokens.
			if (block.number <= FINISH_BONUS_AT_BLOCK) {
                Polly.lock(address(liquidityaddr), PollyForLP.mul(50).div(100));
            }
        }
    ...
    function getPoolReward(uint256 _from, uint256 _to, uint256 _allocPoint) public view returns (uint256 forDev, uint256 forFarmer, uint256 forLP, uint256 forCom, uint256 forFounders) {
        ...
        uint256 PollyCanMint = Polly.cap().sub(Polly.totalSupply());

        if (PollyCanMint < amount) {
            forDev = 0;
			forFarmer = PollyCanMint;
			forLP = 0;
			forCom = 0;
			forFounders = 0;
        }
    ...
    // View function to see pending Pollys on frontend.
    function pendingReward(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accPollyPerShare = pool.accPollyPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply > 0) {
            uint256 PollyForFarmer;
            (, PollyForFarmer, , ,) = getPoolReward(pool.lastRewardBlock, block.number, pool.allocPoint);
            accPollyPerShare = accPollyPerShare.add(PollyForFarmer.mul(1e12).div(lpSupply));

        }
        return user.amount.mul(accPollyPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function _harvest(uint256 _pid) internal {
        ...
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accPollyPerShare).div(1e12).sub(user.rewardDebt);
            uint256 masterBal = Polly.balanceOf(address(this));
            ...
            if(pending > 0) {
                Polly.transfer(msg.sender, pending);
                uint256 lockAmount = 0;
                if (user.rewardDebtAtBlock <= FINISH_BONUS_AT_BLOCK) {
                    lockAmount = pending.mul(PERCENT_LOCK_BONUS_REWARD).div(100);
                    Polly.lock(msg.sender, lockAmount);
                }

                user.rewardDebtAtBlock = block.number;

                emit SendPollyReward(msg.sender, _pid, pending, lockAmount);
            }

            user.rewardDebt = user.amount.mul(pool.accPollyPerShare).div(1e12);
        }
    }
    ...
    // Deposit LP tokens to PollyMasterFarmer for $Polly allocation.
    function deposit(uint256 _pid, uint256 _amount, address _ref) public {
        require(_amount > 0, "PollyMasterFarmer::deposit: amount must be greater than 0");
        ...
        devr.rewardDebt = devr.amount.mul(pool.accPollyPerShare).div(1e12);
  ...
  // Withdraw LP tokens from PollyMasterFarmer.
    function withdraw(uint256 _pid, uint256 _amount, address _ref) public {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        UserGlobalInfo storage refer = userGlobalInfo[_ref];
        UserGlobalInfo storage current = userGlobalInfo[msg.sender];
        require(user.amount >= _amount, "PollyMasterFarmer::withdraw: not good");
        if(_ref != address(0)){
                refer.referrals[msg.sender] = refer.referrals[msg.sender] - _amount;
                refer.globalRefAmount = refer.globalRefAmount - _amount;
            }
    ...
    // Safe Polly transfer function, just in case if rounding error causes pool to not have enough Pollys.
    function safePollyTransfer(address _to, uint256 _amount) internal {
        uint256 PollyBal = Polly.balanceOf(address(this));
        if (_amount > PollyBal) {
            Polly.transfer(_to, PollyBal);
        } else {
            Polly.transfer(_to, _amount);
        }
    }
