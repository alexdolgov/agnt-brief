...
contract MasterChefV2 is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. BSWAP to distribute per block.
        uint256 lastRewardTime; // Last block number that BSWAP distribution occurs.
        uint256 accPerShare; // Accumulated BSWAP per share, times 1e12. See below.
        uint256 totalDeposit;
    }

    BswapToken public bswap;

    // tokens created per sec.
    uint256 public bswapPerSec;
    ...
    // The block time when BSWAP mining starts.
    uint256 public startTime = type(uint256).max;

    // Bonus muliplier for early BSWAP makers.
    ...
    constructor(address _treasury, BswapToken _bswap, uint256 _rewardPerSec) public {
        require(_treasury != address(0) && address(_bswap) != address(0), "Zero address");

        treasury = _treasury;
        bswap = _bswap;
        bswapPerSec = _rewardPerSec;
    }
    ...
    // Update the given pool's BSWAP allocation point. Can only be called by the owner.
    ...
    function updatePool(uint256 _pid) public {
        ...
        bswap.mint(address(this), reward);

        uint256 teamReward = reward.mul(teamRewardPercent).div(10000);
        bswap.mint(treasury, teamReward);
    ...
    function safeRewardTransfer(address _to, uint256 _amount) internal {
        uint256 balance = bswap.balanceOf(address(this));
        if (_amount > balance) {
            bswap.transfer(_to, balance);
        } else {
            bswap.transfer(_to, _amount);
        }
    }
}