...
contract BPLPManager is ReentrancyGuard, Governable, IGlpManager {
    using SafeMath for uint256;
    ...
    function setShortsTrackerAveragePriceWeight(uint256 _shortsTrackerAveragePriceWeight) external override onlyGov {
        require(shortsTrackerAveragePriceWeight <= BASIS_POINTS_DIVISOR, "BPLPManager: invalid weight");
    ...
    function setCooldownDuration(uint256 _cooldownDuration) external override onlyGov {
        require(_cooldownDuration <= MAX_COOLDOWN_DURATION, "BPLPManager: invalid _cooldownDuration");
        cooldownDuration = _cooldownDuration;
    }
    ...
    function addLiquidity(
        ...
        if (inPrivateMode) {
            revert("BPLPManager: action not enabled");
        }
    ...
    function _addLiquidity(
        ...
        require(_amount > 0, "BPLPManager: invalid _amount");
        ...
        require(usdgAmount >= _minUsdg, "BPLPManager: insufficient USDG output");

        uint256 mintAmount = aumInUsdg == 0 ? usdgAmount : usdgAmount.mul(glpSupply).div(aumInUsdg);
        require(mintAmount >= _minGlp, "BPLPManager: insufficient GLP output");
    ...
    function _removeLiquidity(
        ...
        require(_glpAmount > 0, "BPLPManager: invalid _glpAmount");
        require(
            lastAddedAt[_account].add(cooldownDuration) <= block.timestamp,
            "BPLPManager: cooldown duration not yet passed"
        );
        ...
        require(amountOut >= _minOut, "BPLPManager: insufficient output");
    ...
    function _validateHandler() private view {
        require(isHandler[msg.sender], "BPLPManager: forbidden");
    }
}