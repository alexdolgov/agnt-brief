 ...
 */
contract WombatRouter02 is OwnableUpgradeable, IWombatRouter {
    ...
    function initialize(IWNative _wNative) external initializer{
        __Ownable_init();
    ...
    function addLiquidityNative(
        ...
        if (amount > 0) {
            IERC20(address(wNative)).safeTransferFrom(msg.sender, address(this), amount);
        }
        return pool.deposit(address(wNative), msg.value + amount, minimumLiquidity, to, deadline, shouldStake);
    }
