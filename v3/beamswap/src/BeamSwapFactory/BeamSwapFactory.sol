...
// File contracts/uniswapv2/BeamSwapFactory.sol

pragma solidity =0.5.16;
contract BeamSwapFactory is IBeamSwapFactory {
    bytes32 public constant INIT_CODE_PAIR_HASH =
        keccak256(abi.encodePacked(type(BeamSwapPair).creationCode));
    ...
    {
        require(tokenA != tokenB, "BeamSwap: IDENTICAL_ADDRESSES");
        (address token0, address token1) = tokenA < tokenB
            ? (tokenA, tokenB)
            : (tokenB, tokenA);
        require(token0 != address(0), "BeamSwap: ZERO_ADDRESS");
        require(getPair[token0][token1] == address(0), "BeamSwap: PAIR_EXISTS"); // single check is sufficient
        bytes memory bytecode = type(BeamSwapPair).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(token0, token1));
        assembly {
            pair := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        IBeamSwapPair(pair).initialize(token0, token1);
    ...
    function setFeeTo(address _feeTo) external {
        require(msg.sender == feeToSetter, "BeamSwap: FORBIDDEN");
        feeTo = _feeTo;
    }
