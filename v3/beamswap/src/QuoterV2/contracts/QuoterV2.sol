...
contract QuoterV2 is IQuoterV2, IBeamswapV3SwapCallback, PeripheryImmutableState {
    using Path for bytes;
    using SafeCast for uint256;
    using PoolTicksCounter for IBeamswapV3Pool;
    ...
    function getPool(
        ...
        address tokenB,
        uint24 fee
    ) private view returns (IBeamswapV3Pool) {
        return IBeamswapV3Pool(PoolAddress.computeAddress(deployer, PoolAddress.getPoolKey(tokenA, tokenB, fee)));
    }

    /// @inheritdoc IBeamswapV3SwapCallback
    function beamswapV3SwapCallback(
        ...
        IBeamswapV3Pool pool = getPool(tokenIn, tokenOut, fee);
        ...
        )
    {
        ...
        IBeamswapV3Pool pool = getPool(params.tokenIn, params.tokenOut, params.fee);
