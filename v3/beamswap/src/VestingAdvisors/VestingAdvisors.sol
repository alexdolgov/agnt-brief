...
// File contracts/vestings/VestingAdvisors.sol
...
contract VestingAdvisors is Ownable {
    ...
    uint256 private constant RELEASEAMOUNT = 9000000 ether;
    uint256 private immutable endDay;
    uint256 public epoch;
    address private beneficiary; // team multisig to claim tokens
    IERC20 private immutable glintToken;
    event Claimed(uint256 amount, uint256 epoch);

    constructor(address _glintTokenAddress, uint256 _startTime) {
        glintToken = IERC20(_glintTokenAddress);
        startTime = _startTime; //tge + 182 days
        endDay = _startTime.add(300 days);
    ...
    function claimTokens() external {
        ...
        if (block.timestamp > endDay) {
            ...
            if (currentEpoch > epoch) {
                ...
                glintToken.safeTransfer(
                    beneficiary,
                    RELEASEAMOUNT.mul(multiplier)
                );
                epoch = epoch.add(multiplier);
            } else {
                glintToken.safeTransfer(beneficiary, RELEASEAMOUNT);
                epoch = epoch.add(1);
            }
    ...
    function claimDust() public {
        require(block.timestamp >= endDay, "Vesting not yet finished");
        glintToken.safeTransfer(
            beneficiary,
            glintToken.balanceOf(address(this))
        );
    }
