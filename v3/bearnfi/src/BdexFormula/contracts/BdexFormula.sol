...
contract BdexFormula is IBdexFormula {
    ...
    function getReserveAndWeights(address pair, address tokenA) public override view returns (
        ...
        if (tokenA == token0) {
            (tokenB, reserveA, reserveB, tokenWeightA, tokenWeightB) = (IBdexPair(pair).token1(), reserve0, reserve1, tokenWeight0, tokenWeight1);
        } else {
    ...
    function getAmountOut(
        ...
        // validate input
        require(amountIn > 0, 'bDexFormula: INSUFFICIENT_INPUT_AMOUNT');
        require(reserveIn > 0 && reserveOut > 0, "bDexFormula: INSUFFICIENT_LIQUIDITY");
    ...
    function getAmountIn(
        ...
        // validate input
        require(amountOut > 0, 'bDexFormula: INSUFFICIENT_OUTPUT_AMOUNT');
        require(reserveIn > 0 && reserveOut > 0, "bDexFormula: INSUFFICIENT_LIQUIDITY");
        // special case for equal weights
        if (tokenWeightIn == tokenWeightOut) {
    ...
    function getAmountsOut(address tokenIn, address tokenOut, uint amountIn, address[] calldata path) external override view returns (uint[] memory amounts) {
        require(path.length > 0, 'bDexFormula: INVALID_PATH');
        ...
        require(currentTokenIn == tokenOut, 'bDexFormula: INVALID_TOKEN_OUT_PATH');
    }
    ...
    function getAmountsIn(address tokenIn, address tokenOut, uint amountOut, address[] calldata path) external override view returns (uint[] memory amounts) {
        ...
        require(currentTokenIn == tokenIn, 'bDexFormula: INVALID_TOKEN_IN_PATH');
    }
    ...
    function getWeightsAndSwapFee(address pair) public view returns (uint32 tokenWeight0, uint32 tokenWeight1, uint32 swapFee) {
        try IBdexPair(pair).getTokenWeights() returns (uint32 _tokenWeight0, uint32 _tokenWeight1) {
            return (_tokenWeight0, _tokenWeight1, IBdexPair(pair).getSwapFee());
    ...
    function getFactoryWeightsAndSwapFee(address factory, address pair) public view returns (uint32 tokenWeight0, uint32 tokenWeight1, uint32 swapFee) {
        return IBdexFactory(factory).getWeightsAndSwapFee(pair);
    }
    ...
    function sortTokens(address tokenA, address tokenB) public pure override returns (address token0, address token1) {
        require(tokenA != tokenB, 'bDexFormula: IDENTICAL_ADDRESSES');
        (token0, token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), 'bDexFormula: ZERO_ADDRESS');
    }
    ...
    function quote(uint amountA, uint reserveA, uint reserveB) external pure override returns (uint amountB) {
        require(amountA > 0, 'bDexFormula: INSUFFICIENT_AMOUNT');
        require(reserveA > 0 && reserveB > 0, 'bDexFormula: INSUFFICIENT_LIQUIDITY');
