...
contract DaoFundTradeRouter {
    ...
    address public constant bdoBusd8020 = address(0x257Dd90481a39E3eD42B518fBA3bEb390D9137Df);
    ...
    function buyBDO(address _inputToken, uint256 _amount) public {
        ...
        _path[0] = _inputToken;
        _path[1] = bdo;
        pancakeSwapToken(_path, _inputToken, bdo, _amount);
    }

    function concurrentJobs(
        uint256 _busdAmountToBuy,
        uint256 _wbnbAmountToBuy,
        uint256 _busdAmountToBuy8020
    ) external {
        if (_busdAmountToBuy > 0) {
            buyBDO(busd, _busdAmountToBuy);
        }
        if (_wbnbAmountToBuy > 0) {
            buyBDO(wbnb, _wbnbAmountToBuy);
        }
        if (_busdAmountToBuy8020 > 0) {
            vswapSwapToken(bdoBusd8020, busd, bdo, _busdAmountToBuy8020);
        }
    }
