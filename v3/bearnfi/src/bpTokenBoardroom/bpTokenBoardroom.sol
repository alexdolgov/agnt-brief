...
contract bpTokenBoardroom is IbpTokenBoardroom, ContractGuard {
    ...
    modifier onlyMainboardroom() {
        require(mainBoardroom == msg.sender, "bpTokenBoardroom: caller is not the main boardroom");
        _;
    }

    modifier updateReward(address _token, address _director) {
    ...
    function earned(address _token, address _director) public view override returns (uint256) {
    ...
    function claimReward(address _token, address _director) external override updateReward(_token, _director) onlyMainboardroom {
    ...
    function allocateSeignioragePegToken(address _token, uint256 _amount) external override onlyOneBlock onlyMainboardroom {
