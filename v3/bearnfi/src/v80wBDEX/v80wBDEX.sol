...
contract v80wBDEX is ERC20UpgradeSafe, ITokenLocker {
    ...
    function initialize(address _bdex, uint256 startReleaseBlock_) public initializer {
        __ERC20_init("80-weeks vesting bDex.Fi", "v80wBDEX");
        bdex = _bdex;
        _startReleaseBlock = startReleaseBlock_;
        _endReleaseBlock = startReleaseBlock_.add(BLOCKS_PER_WEEK * 80);
        governance = msg.sender;
    }

    modifier onlyGovernance() {
        require(msg.sender == governance, "v80wBDEX: !governance");
        _;
    }
    ...
    function lock(address _account, uint256 _amount) external override {
        require(block.number <= _endReleaseBlock, "v80wBDEX: no more lock");
        require(_account != address(0), "v80wBDEX: no lock to address(0)");
        require(_amount > 0, "v80wBDEX: zero lock");
    ...
    function unlock(uint256 _amount) public override {
        require(block.number > _startReleaseBlock, "v80wBDEX: still locked");
    ...
    function claimUnlocked() public override {
        require(block.number > _startReleaseBlock, "v80wBDEX: still locked");
        require(_locks[msg.sender] > _released[msg.sender], "v80wBDEX: no locked");
    ...
    function governanceRecoverUnsupported(
        ...
        require(_token != bdex || IERC20(bdex).balanceOf(address(this)).sub(_amount) >= _totalLock, "v80wBDEX: Not enough locked amount left");
