...
contract xBDEX is ERC20UpgradeSafe, ITokenLocker {
    ...
    function initialize(
        ...
        __ERC20_init("Locked bDex.Fi", "xBDEX");
        require(endReleaseBlock_ > startReleaseBlock_, "xBDEX: endReleaseBlock_ is before startReleaseBlock_");
        bdex = _bdex;
    ...
    modifier onlyGovernance() {
        require(msg.sender == governance, "xBDEX: !governance");
        _;
    }
    ...
    function editLocker(uint256 startReleaseBlock_, uint256 endReleaseBlock_) external onlyGovernance {
        require(_startReleaseBlock > block.number && startReleaseBlock_ > block.number, "xBDEX: late");
        require(endReleaseBlock_ > startReleaseBlock_ && endReleaseBlock_ <= startReleaseBlock_.add(BLOCKS_PER_WEEK * 80), "xBDEX: invalid _endReleaseBlock");
        _startReleaseBlock = startReleaseBlock_;
        _endReleaseBlock = endReleaseBlock_;
        emit EditLocker(startReleaseBlock_, endReleaseBlock_);
    }
    ...
    function lock(address _account, uint256 _amount) external override {
        require(block.number <= _endReleaseBlock, "xBDEX: no more lock");
        require(_account != address(0), "xBDEX: no lock to address(0)");
        require(_amount > 0, "xBDEX: zero lock");

        IERC20(bdex).safeTransferFrom(msg.sender, address(this), _amount);
    ...
    function unlock(uint256 _amount) public override {
        require(block.number > _startReleaseBlock, "xBDEX: still locked");
    ...
    function claimUnlocked() public override {
        require(block.number > _startReleaseBlock, "xBDEX: still locked");
        require(_locks[msg.sender] > _released[msg.sender], "xBDEX: no locked");

        uint256 _amount = canUnlockAmount(msg.sender);
        IERC20(bdex).safeTransfer(msg.sender, _amount);
    ...
    function governanceRecoverUnsupported(
        ...
        require(_token != bdex || IERC20(bdex).balanceOf(address(this)).sub(_amount) >= _totalLock, "xBDEX: Not enough locked amount left");
