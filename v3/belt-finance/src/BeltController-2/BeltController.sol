...
// File contracts/bsc/interfaces/IStrategy.sol
...
interface IStrategy {
    ...
    function earn(uint256 _minAmount) external;
    function setbuyBackRate(uint256 _buyBackRate, uint256 _buyBackPoolRate) external;
    ...
    function setBuyBackAddress(address newAddr) external;
...
// File contracts/bsc/utils/BeltController.sol
...
contract BeltController is Initializable, OwnableUpgradeable, BeltControllerStorage {
    ...
    function setVoidStrategyTokenAddress(address addr, address _voidStrategyTokenAddress) public onlyPolicyController onlyMultiStrategyToken(addr) {
        IMultiStrategyToken(addr).setVoidStrategyTokenAddress(_voidStrategyTokenAddress);
    }
    ...
    function addStrategy(address addr, address strategyAddress) public onlyPolicyController onlyMultiStrategyToken(addr) {
        IMultiStrategyToken(addr).addStrategy(strategyAddress);
        emit StrategyAdded(strategyAddress);
    }
    
    function removeStrategy(address addr, address strategyAddress) public onlyPolicyController onlyMultiStrategyToken(addr) {
        IMultiStrategyToken(addr).removeStrategy(strategyAddress);
    ...
    function earnStrategy(address addr, uint _minAmount) public onlyPolicyController onlyStrategy(addr) {
        IStrategy(addr).earn(_minAmount);
    }
    ...
    function strategySetBuyBackRate(address addr, uint256 _buyBackRate, uint256 _buyBackPoolRate) public onlyPolicyController onlyStrategy(addr) {
        IStrategy(addr).setbuyBackRate(_buyBackRate, _buyBackPoolRate);
    }
    ...
    function strategySetBuyBackPoolAddress(address addr, address _buyBackPoolAddress) public onlyPolicyController onlyStrategy(addr) {
        IStrategy(addr).setbuyBackPoolAddress(_buyBackPoolAddress);
    }
    ...
    function leverage(address addr, uint256 _amount) public onlyPolicyController onlyStrategy(addr) {
        ILeverageStrategy(addr).leverage(_amount);
    }

    function deleverage(address addr, uint256 _amount) public onlyPolicyController onlyStrategy(addr) {
        ILeverageStrategy(addr).deleverage(_amount);
    }

    function deleverageAll(address addr, uint256 _redeemFeeAmount) public onlyPolicyController onlyStrategy(addr) {
        ILeverageStrategy(addr).deleverageAll(_redeemFeeAmount);
    }
