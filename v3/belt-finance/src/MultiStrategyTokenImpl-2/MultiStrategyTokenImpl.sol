...
contract MultiStrategyTokenImpl is MultiStrategyTokenStorage {
        ...
        returns (uint256)
    {
        ...
        if(withdrawActive[voidStrategyTokenAddress]) {
            strategyToWithdraw = voidStrategyTokenAddress;
            strategyAvailableAmount = getBalanceOfOneStrategy(strategyToWithdraw);
        } else {
            strategyAvailableAmount = 0;
        }
