...
// File: contracts/utils/NativeDepositor.sol
...
contract NativeDepositor {
    ...
    address public beltNative;

    constructor(address _wrappedNativeAddress, address _beltNative) public {
        wrappedNativeAddress = _wrappedNativeAddress;
        beltNative = _beltNative;
        IERC20(wrappedNativeAddress).safeApprove(beltNative, uint256(-1));
    }

    function depositBNB(uint256 _minShares) external payable {
        Wrapped(wrappedNativeAddress).deposit{value: msg.value}();
        IMultiStrategyToken(beltNative).deposit(
            IERC20(wrappedNativeAddress).balanceOf(address(this)),
            _minShares
        );
        IERC20(beltNative).safeTransfer(
            msg.sender,
            IERC20(beltNative).balanceOf(address(this))
        );
    }

    function withdrawBNB(uint256 _shares, uint256 _minAmount) external {
        IERC20(beltNative).safeTransferFrom(msg.sender, address(this), _shares);
        IMultiStrategyToken(beltNative).withdraw(_shares, _minAmount);
        Wrapped(wrappedNativeAddress).withdraw(IERC20(wrappedNativeAddress).balanceOf(address(this)));
