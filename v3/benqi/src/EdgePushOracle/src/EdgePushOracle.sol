...
contract EdgePushOracle is OwnableUpgradeable, UUPSUpgradeable {
    ...
    address public trustedSender; // Address authorized to post updates
    ...
    event TrustedSenderUpdated(address indexed previousSender, address indexed newSender); // Event emitted when trusted sender is updated
    ...
    /**
     * @notice Owner can set the trusted sender who is authorized to post updates
     * @param _trustedSender Address of the trusted sender
     */
    function setTrustedSender(address _trustedSender) external onlyOwner {
        address previousSender = trustedSender;
        trustedSender = _trustedSender;
        emit TrustedSenderUpdated(previousSender, _trustedSender);
    }

    // ============ Update Posting Function ============

    /**
     * @notice Only the trusted sender can submit a report signed by multiple trusted oracles
    ...
    function postUpdate(bytes memory report, bytes[] memory signatures) external {
        // Ensure msg.sender is the trusted sender or the owner if no trusted sender is set
        require(msg.sender == trustedSender, "Sender not authorized");
