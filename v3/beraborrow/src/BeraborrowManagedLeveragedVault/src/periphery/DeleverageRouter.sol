...
contract DeleverageRouter is IDeleverageRouter {
    ...
    constructor(address _borrowerOperations, address _nect, address _collVaultRouter) {
        if (_borrowerOperations == address(0) || _nect == address(0) || _collVaultRouter == address(0)) {
            revert ZeroAddress();
        }
        ...
        returns (uint256 nectReceived)
    {
        ...
        ICollVaultRouter.RedeemToOneParams memory _params = ICollVaultRouter.RedeemToOneParams({
            ...
            collVault: IInfraredCollateralVault(collVault),
