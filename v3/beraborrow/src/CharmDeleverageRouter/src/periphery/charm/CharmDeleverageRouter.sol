...
import {DeleverageRouter, IDenManager, IInfraredCollateralVault, IERC20, SafeERC20} from "src/periphery/DeleverageRouter.sol";
 ...
 * @dev Assumes the collateral asset is an Alpha Pro Vault (Charm Finance) and at least one of the tokens in the pair is NECT
 */
contract CharmDeleverageRouter is DeleverageRouter {
    using SafeERC20 for IERC20;

    IOBRouter public constant obRouter = IOBRouter(0xFd88aD4849BA0F729D6fF4bC27Ff948Ab1Ac3dE7);

    constructor(address _borrowerOperations, address _nect, address _collVaultRouter) DeleverageRouter(_borrowerOperations, _nect, _collVaultRouter) {}
        ...
        returns (uint256 nectReceived)
    {
        ...
        (
            uint256 amountIn,
            address tokenIn
        ) = _unwrapCharmVault(_denManager, collVaultAmount, collVaultParams.swapCalldatas[1]);
        ...
        nectReceived = obRouter.swap(tokenInfo, p.pathDefinition, p.executor, p.referralCode);
    ...
    function _unwrapCharmVault(
        IDenManager _denManager,
        uint256 collVaultAmount,
        bytes memory payload
    ) internal returns (uint256 amountIn, address tokenIn) {
        IInfraredCollateralVault collVault = IInfraredCollateralVault(_denManager.collateralToken());
        IAlphaProVault collVaultAsset = IAlphaProVault(collVault.asset());

        uint256 prevBalance = collVaultAsset.balanceOf(address(this));
        
        collVault.redeem(collVaultAmount, address(this), address(this));

        uint256 postBalance = collVaultAsset.balanceOf(address(this));

        // Search for non-NECT token
        address token0 = collVaultAsset.token0();
        tokenIn = token0 == address(nect) ? collVaultAsset.token1() : token0;

        prevBalance = IERC20(tokenIn).balanceOf(address(this));

        (uint256 minToken0Amount, uint256 minToken1Amount) = abi.decode(payload, (uint256, uint256));
        IAlphaProVault(collVaultAsset).withdraw(postBalance - prevBalance, minToken0Amount, minToken1Amount, address(this));

        amountIn = IERC20(tokenIn).balanceOf(address(this)) - prevBalance;
    }
}