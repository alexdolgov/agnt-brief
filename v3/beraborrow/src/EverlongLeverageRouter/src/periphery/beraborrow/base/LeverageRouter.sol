...
import {ILeverageRouter} from "src/interfaces/periphery/beraborrow/base/ILeverageRouter.sol";
...
contract LeverageRouter is ILeverageRouter {
    ...
    constructor(address _borrowerOperations, address _nect, address _priceFeed, address _core, address _everlongCore, address[] memory _initialSwapRouters) {
        if (_borrowerOperations == address(0) || _nect == address(0) || _priceFeed == address(0) || _core == address(0) || _everlongCore == address(0)) {
            revert ZeroAddress();
        }
    ...
    function _processFlashLoan(
        ...
        uint256 prevStuckedNect = nect.balanceOf(address(this)) - nectFlashLoaned;
        ...
        uint256 collVaultShares =
            _swapAndWrap(IDenManager(denManager), account, params, asset, collVault);

        /// @dev All NECT remaining is because it is stucked, which means we can productively use it to payback part of the flashloan
        uint256 additionalStuckedNect = nect.balanceOf(address(this)) - prevStuckedNect;
        uint256 totalNectToPayback = nectFlashLoaned - additionalStuckedNect;
        if (action == Action.OpenDen) {
            _openDen(denManager, account, collVault, params.denParams, collVaultShares, totalNectToPayback);
            ...
            _increaseCollateral(denManager, account, collVault, params.denParams, collVaultShares, totalNectToPayback);
    ...
    /// @dev Swaps Nectar into a collateral asset and deposits the collateral asset into the collVault
    function _swapAndWrap(
        ...
        uint256 collAssetReceived = _swap(denManager, account, params.nectToColl, address(nect), asset, params.flashloanNectAmount) + params.marginCollAmount;
