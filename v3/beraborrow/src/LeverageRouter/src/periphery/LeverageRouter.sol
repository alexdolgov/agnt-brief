...
contract LeverageRouter is ILeverageRouter {
        ...
        nonReentrant
    {
        ...
        if (
            !nect.flashLoan(
                IERC3156FlashBorrower(address(this)), address(nect), denLoopingParams.flashloanNectAmount, data
            )
        ) {
    ...
    function calculateDebtAmount(
        ...
        m.marginInCollVault = margin != 0 ? IInfraredCollateralVault(collVault).previewDeposit(margin) : 0;
    ...
    function _processFlashLoan(
        ...
        uint256 collVaultShares =
            _swapAndWrap(params.nectToColl, asset, collVault, nectFlashLoaned, params.marginCollAmount);
    ...
    function _swap(DexAggregatorParams memory params, address tokenIn, address tokenOut, uint256 amount)
        private
    ...
    ) private returns (uint256 collVaultSharesMinted) {
        uint256 collAssetReceived = _swap(params, address(nect), asset, nectAmount) + marginAmount;
