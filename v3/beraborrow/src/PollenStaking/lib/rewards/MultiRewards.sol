 ...
 * @notice Based out of the following Infrared Finance implementation:
 ...
 *         Added permissioned logic for changing the gas limit on reward transferring.
 *         Added balance checks before and after withdraw hook.
 */
abstract contract MultiRewards is ReentrancyGuard, Pausable, IMultiRewards {
    ...
    /**
     * @notice The address of the MetaBeraborrowCore contract
     * @dev This contract is used to manage the core functionality of the protocol
     */
    ...
    /**
     * @notice The gas limit for transferring rewards
     */
    uint256 public rewardGasLimit = 200000;
     ...
     * @param _metaBeraborrowCore address The address of the MetaBeraborrowCore contract.
     */
    constructor(address _stakingToken, address _metaBeraborrowCore) {
    ...
    /**
     * @notice Hook called in the stake function after transferring staking token in
    ...
    function withdraw(
        ...
        _withdrawInternal(amount, msg.sender);
    }

    function _withdrawInternal(
        uint256 amount,
        address user
    ) internal virtual updateReward(user) {
        ...
        uint256 stakingTokenAmount = stakingToken.balanceOf(address(this)) - balanceBefore;

        stakingToken.safeTransfer(msg.sender, stakingTokenAmount);
        emit Withdrawn(user, stakingTokenAmount);
    }
    ...
    function getRewardForUser(
        ...
        uint256 gasLimit = rewardGasLimit;
    ...
    /// @inheritdoc IMultiRewards
    function setRewardGasLimit(uint256 _rewardGasLimit) external {
        if (msg.sender != metaBeraborrowCore.owner())
            revert Errors.Unauthorized(msg.sender);

        rewardGasLimit = _rewardGasLimit;
    }
