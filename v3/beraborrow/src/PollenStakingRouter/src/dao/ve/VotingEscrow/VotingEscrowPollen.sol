...
contract VotingEscrowPollen is VotingEscrowTokenBase, IVotingEscrowPollen {
    ...
    function _calculateWeightedHistoryPenalty(
        ...
        for (uint256 i = startIndex; i < historyLength; i++) {
            ...
            uint128 checkpointAmount = checkpoint.value.bias * MAX_LOCK_TIME / position.expiry;
            
            if (checkpointAmount == 0) continue;

            uint128 penaltyRatio = _calculatePenaltyRatio(checkpoint.lastUpdateTimestamp, position.expiry);
        ...
        uint128 finalPenalty = totalWeightedPenalty / totalAmount;
