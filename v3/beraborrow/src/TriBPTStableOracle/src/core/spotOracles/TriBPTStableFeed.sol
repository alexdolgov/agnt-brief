 ...
 */
contract TriBPTStableOracle is ISpotOracle {
    ...
    constructor(
        ...
        if (PRICE_FEED.fetchPrice(_tokens[0]) == 0 || PRICE_FEED.fetchPrice(_tokens[1]) == 0 || PRICE_FEED.fetchPrice(_tokens[2]) == 0) {
            revert NoFeed();
        }
        ...
        tokens[2] = TokenInfo({token: _tokens[2], decimals: IAsset(_tokens[2]).decimals()});
    }
    ...
    function _calculatePrice() internal view returns (uint256) {
        TokenInfo[3] storage tokenInfo = tokens;

        uint256 min = type(uint256).max;

        // 18 decimals
        uint256[3] memory prices = _getTokenPrices([tokenInfo[0].token, tokenInfo[1].token, tokenInfo[2].token]);
        ...
        try BEX_POOL.getTokenRate(address(tokenInfo[2].token)) returns (uint256 rate) {
            prices[2] = prices[2].divWad(rate);
        } catch {}
    ...
    function _getTokenPrices(address[3] memory _tokens) internal view returns (uint256[3] memory prices) {
        prices[0] = PRICE_FEED.fetchPrice(_tokens[0]);
        prices[1] = PRICE_FEED.fetchPrice(_tokens[1]);
        prices[2] = PRICE_FEED.fetchPrice(_tokens[2]);
    }
