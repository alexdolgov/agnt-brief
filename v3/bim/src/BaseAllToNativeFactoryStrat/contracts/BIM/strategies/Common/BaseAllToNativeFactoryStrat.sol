...
import "../../interfaces/bim/IStrategyFactory.sol";
...
abstract contract BaseAllToNativeFactoryStrat is OwnableUpgradeable, PausableUpgradeable {
    ...
    function _chargeFees(address callFeeRecipient) internal {
        IFeeConfig.FeeCategory memory fees = bimFeeConfig().getFees(address(this));
    ...
    function _swap(address tokenFrom, address tokenTo, uint amount) internal {
        if (tokenFrom != tokenTo) {
            ...
            IBimSwapper(swapper).swap(tokenFrom, tokenTo, amount);
        }
    }
    ...
    function bimFeeConfig() public view returns (IFeeConfig) {
        return IFeeConfig(factory.bimFeeConfig());
    }

    function bimFeeRecipient() public view returns (address) {
        return factory.bimFeeRecipient();
    }

    function getAllFees() external view returns (IFeeConfig.AllFees memory) {
        return IFeeConfig.AllFees(bimFeeConfig().getFees(address(this)), depositFee(), withdrawFee());
    }
