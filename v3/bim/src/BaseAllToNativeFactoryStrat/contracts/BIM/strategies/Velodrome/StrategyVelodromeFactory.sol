...
contract StrategyVelodromeFactory is BaseAllToNativeFactoryStrat {
    ...
    function _swapNativeToWant() internal override {
        ...
        if (stable) {
            address factory = ISolidlyRouter(solidlyRouter).defaultFactory();
            uint256 lp0Decimals = 10**IERC20Extended(lpToken0).decimals();
            uint256 lp1Decimals = 10**IERC20Extended(lpToken1).decimals();
            uint256 out0 = IBimSwapper(swapper).getAmountOut(native, lpToken0, lp0Amt) * 1e18 / lp0Decimals;
            uint256 out1 = IBimSwapper(swapper).getAmountOut(native, lpToken1, lp1Amt) * 1e18 / lp1Decimals;
            (uint256 amountA, uint256 amountB,) = ISolidlyRouter(solidlyRouter).quoteAddLiquidity(lpToken0, lpToken1, stable, factory, out0, out1);
        ...
        if (lpToken0 != native) {
            IBimSwapper(swapper).swap(native, lpToken0, lp0Amt);
        }

        if (lpToken1 != native) {
            IBimSwapper(swapper).swap(native, lpToken1, lp1Amt);
        }
