...
// Minimal proxy pattern for creating new BIM strategies
contract StrategyFactory is Ownable {
    ...
    /// @notice The bim fee recipient
    address public bimFeeRecipient;

    /// @notice The bim fee config
    address public bimFeeConfig;
    ...
    event SetBimFeeRecipient(address bimFeeRecipient);
    event SetBimFeeConfig(address bimFeeConfig);
    ...
    constructor(
        address _native,
        address _keeper,
        address _bimFeeRecipient,
        address _bimFeeConfig
    ) Ownable(msg.sender) {
        native = _native;
        keeper = _keeper;
        bimFeeRecipient = _bimFeeRecipient;
        bimFeeConfig = _bimFeeConfig;
    }

    /** @notice Creates a new BIM Strategy as a proxy of the template instance
    * @param _strategyName The name of the strategy
    * @return A reference to the new proxied BIM Strategy
   */
    function createStrategy(string calldata _strategyName) external returns (address) {

        // Create a new BIM Strategy as a proxy of the template instance
   ...
   */
    ...
    /**
     * @notice set the bim fee recipient address
   * @param _bimFeeRecipient The new bim fee recipient address
   */
    function setBimFeeRecipient(address _bimFeeRecipient) external onlyOwner {
        bimFeeRecipient = _bimFeeRecipient;
        emit SetBimFeeRecipient(_bimFeeRecipient);
    }

    /**
     * @notice set the bim fee config address
   * @param _bimFeeConfig The new bim fee config address
   */
    function setBimFeeConfig(address _bimFeeConfig) external onlyOwner {
        bimFeeConfig = _bimFeeConfig;
        emit SetBimFeeConfig(_bimFeeConfig);
    }
