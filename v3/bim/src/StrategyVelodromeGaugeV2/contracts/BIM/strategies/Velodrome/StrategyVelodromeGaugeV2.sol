...
import "../../interfaces/bim/IBimSwapper.sol";
...
contract StrategyVelodromeGaugeV2 is StratFeeManagerInitializable {
    ...
    event ChargedFees(uint256 callFees, uint256 bimFees, uint256 strategistFees);
    event SetBimSwapper(address newSwapper);
    ...
    function chargeFees(address callFeeRecipient) internal {
        ...
        (address router,,,,) = IBimSwapper(bimSwapper).swapInfo(output, native);
        if (router != address(0)) {
            IBimSwapper(bimSwapper).swap(output, native, toNative);
        } else {
        ...
        uint256 bimFeeAmount = nativeBal * fees.bim / DIVISOR;
        IERC20(native).safeTransfer(bimFeeRecipient, bimFeeAmount);

        uint256 strategistFeeAmount = nativeBal * fees.strategist / DIVISOR;
        IERC20(native).safeTransfer(strategist, strategistFeeAmount);

        emit ChargedFees(callFeeAmount, bimFeeAmount, strategistFeeAmount);
    }
    ...
    function addLiquidity() internal {
        ...
        if (lpToken0 != output) {
            (address router,,,,) = IBimSwapper(bimSwapper).swapInfo(output, lpToken0);
            if (router != address(0)) {
                IBimSwapper(bimSwapper).swap(output, lpToken0, lp0Amt);
            } else {
        ...
        if (lpToken1 != output) {
            (address router,,,,) = IBimSwapper(bimSwapper).swapInfo(output, lpToken1);
            if (router != address(0)) {
                IBimSwapper(bimSwapper).swap(output, lpToken1, lp1Amt);
            } else {
    ...
    function _giveAllowances() internal {
        ...
        IERC20(output).safeApprove(bimSwapper, type(uint).max);
    ...
    function _removeAllowances() internal {
        ...
        IERC20(output).safeApprove(bimSwapper, 0);
    ...
    function setBimSwapper(address _bimSwapper) external onlyManager {
        IERC20(output).safeApprove(bimSwapper, 0);
        bimSwapper = _bimSwapper;
        if (!paused()) IERC20(output).safeApprove(bimSwapper, type(uint).max);
        emit SetBimSwapper(_bimSwapper);
    }
}