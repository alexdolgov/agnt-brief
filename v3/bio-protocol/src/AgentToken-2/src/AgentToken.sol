...
contract AgentToken is ContextUpgradeable, IAgentToken, Ownable2StepUpgradeable {
    ...
    function _addInitialLiquidity(address lpOwner) internal {
        ...
        address pairAddr = IUniswapV2Factory(_uniswapRouter.factory()).getPair(address(this), pairToken);
    ...
    function addLiquidityPool(address newLiquidityPool_) public onlyOwnerOrFactory {
        ...
        if (!_liquidityPools.add(newLiquidityPool_)) revert LiquidityPoolAlreadyExists();
    ...
    function removeLiquidityPool(address removedLiquidityPool_) external onlyOwnerOrFactory {
        ...
        if (!_liquidityPools.remove(removedLiquidityPool_)) revert LiquidityPoolDoesNotExist();
    ...
    function setProjectTaxRecipient(address projectTaxRecipient_) external onlyOwnerOrFactory {
        if (projectTaxRecipient_ == address(0)) revert InvalidAddress();
        ...
        onlyOwnerOrFactory
    {
        ...
        _tokenHasTax = (newProjectBuyTaxBasisPoints_ + newProjectSellTaxBasisPoints_) > 0;
            ...
            if (_tokenHasTax && applyTax_ && !_autoSwapInProgress) {
                ...
                if (isLiquidityPool(to_) && totalSellTaxBasisPoints() > 0) {
                    if (projectSellTaxBasisPoints > 0) {
                        ...
                        projectTaxPaid += uint128(projectTax);
                        tax += projectTax;
                    }
                }
                // on buy
    ...
    function withdrawTax() external onlyOwnerOrFactory {
        uint256 projectTaxPaid_ = projectTaxPaid;
        if (projectTaxPaid_ != 0) {
            projectTaxPaid = 0;
            IERC20(address(this)).safeTransfer(projectTaxRecipient, projectTaxPaid_);
            emit TaxWithdrawn(projectTaxRecipient, projectTaxPaid_);
        } else {
            revert NoTaxToWithdraw();
        }
    }

    function mint(address account, uint256 amount) external onlyOwnerOrFactory {
