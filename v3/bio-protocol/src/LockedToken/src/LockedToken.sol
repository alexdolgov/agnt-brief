 ...
 *      Shareholders are flagged and cannot unlock without admin approval.
 */
contract LockedToken is Initializable, ERC20Upgradeable, ERC20PermitUpgradeable, ERC20VotesUpgradeable, ReentrancyGuardTransient {
    ...
    /// @notice Mapping of addresses marked as shareholders (cannot unlock without admin approval)
    ...
    event ShareholderSet(address indexed account, bool status);
    ...
    modifier notShareholder(address account) {
        if (isShareholder[account]) revert Shareholder();
        _;
    }
     ...
     * @dev Shareholders cannot call this function
     */
    function requestUnlock() external notShareholder(msg.sender) {
     ...
     */
    function unlock() external nonReentrant notShareholder(msg.sender) {
    ...
    /**
     * @notice Set or unset shareholder status (factory only)
     * @param account The account to modify
     * @param status True to mark as shareholder, false to unmark
     */
    function setShareholder(address account, bool status) external onlyFactory {
        isShareholder[account] = status;

        // If setting as shareholder, cancel any pending unlock
        if (status && locks[account].unlockRequestTime != 0) {
            delete locks[account].unlockRequestTime;
        }

        emit ShareholderSet(account, status);
    }

    /**
     * @notice Admin override to unlock shareholder tokens (factory only)
     * @dev Used when shareholder status is approved for exit off-chain
     * @param account The shareholder account to unlock
     */
    function adminUnlock(address account) external onlyFactory nonReentrant {
        if (!isShareholder[account]) revert NotShareholder();

        uint256 amount = balanceOf(account);
        if (amount == 0) revert ZeroAmount();

        // Clear any pending unlock request
        delete locks[account].unlockRequestTime;
    ...
    function canUnlockNow(address account) external view returns (bool) {
        ...
        return userLock.unlockRequestTime != 0 && block.timestamp >= userLock.unlockRequestTime && !isShareholder[account];
    }
