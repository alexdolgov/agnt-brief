...
import "./interfaces/IStakedBITU.sol";

/**
 * @title StakedBITU
 * @notice The StakedBITU contract allows users to stake BITU tokens and earn a portion of protocol LST and perpetual yield that is allocated
 * to stakers by the BitU DAO governance voted yield distribution algorithm.  The algorithm seeks to balance the stability of the protocol by funding
 * the protocol's insurance fund, DAO activities, and rewarding stakers with a portion of the protocol's yield.
 */
contract StakedBITU is SingleAdminAccessControl, ReentrancyGuard, ERC20Permit, ERC4626, IStakedBITU {
  ...
  /**
   * @notice Constructor for StakedBITU contract.
   * @param _asset The address of the BITU token.
   ...
   * Note that the owner cannot rescue BITU tokens because they functionally sit here
   * and belong to stakers but can rescue staked BITU as they should never actually
  ...
  /**
   * @notice Returns the amount of BITU tokens that are vested in the contract.
   */
  ...
  /**
   * @notice Returns the amount of BITU tokens that are unvested in the contract.
   */
  ...
  function _deposit(address caller, address receiver, uint256 assets, uint256 shares)
    ...
    notZero(shares)
  {
    if (hasRole(SOFT_RESTRICTED_STAKER_ROLE, caller) || hasRole(SOFT_RESTRICTED_STAKER_ROLE, receiver) || hasRole(FULL_RESTRICTED_STAKER_ROLE, caller)) {
      revert OperationNotAllowed();
    }
