...
interface IStakedBITU {
  // Events //
  /// @notice Event emitted when the rewards are received
  event RewardsReceived(uint256 indexed amount, uint256 newVestingBITUAmount);
  ...
  error InvalidAmount();
  /// @notice Error emitted when owner attempts to rescue BITU tokens.
  error InvalidToken();
