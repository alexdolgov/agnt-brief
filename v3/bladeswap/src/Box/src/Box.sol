...
contract Box is AccessControl {
    ...
    event Draw(address indexed addr, uint256 timestamp, bytes32 data);
    ...
    function addReward(Token token) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function draw(bytes32 data) external {
        emit Draw(msg.sender, block.timestamp, data);
    }

    function getClaimNonceUsed(
        address a,
        uint256 n
    ) external view returns (bool) {
        return _epochClaimed[n][a];
    }
