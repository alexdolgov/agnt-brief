...
contract SwapAuxillaryFacet is VaultStorage, IFacet, Common {
    ...
    function initializeFacet() external {
        ...
        _setFunction(SwapAuxillaryFacet.getPoolBalance.selector, thisImplementation);
        _setFunction(SwapAuxillaryFacet.getGaugeBalance.selector, thisImplementation);
        _setFunction(SwapAuxillaryFacet.emissionStarted.selector, thisImplementation);
    }
    ...
    function getPoolBalance(address pool, Token t) external view returns (uint256) {
        return t.fromScaledBalance(_poolBalances()[IPool(pool)][t].poolHalf());
    }

    function getGaugeBalance(address pool, Token t) external view returns (uint256) {
        return t.fromScaledBalance(_poolBalances()[IPool(pool)][t].gaugeHalf());
    }
