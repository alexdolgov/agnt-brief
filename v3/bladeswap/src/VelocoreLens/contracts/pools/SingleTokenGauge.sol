...
contract SingleTokenGauge is Pool, IGauge {
    ...
    constructor(
        ...
        Token stake_,
        IBribe bribe
    ) Pool(vault_, address(this), msg.sender) {
        ...
        if (newEmissions > 0) {
            ...
            if (totalStakes > 0) {
                unchecked {
                    ...
                    emissionPerStake1e9 += uint128(
                        (newEmissions * 1e9) / totalStakes
                    );
                }
            }
        }
        ...
        returns (int128[] memory deltaGauge, int128[] memory deltaPool)
    {
        ...
        unchecked {
            ...
            uint256 claimed = ((emissionPerStake1e9 -
                stakerInformation[user].emissionPerStake1e9AtLastClaim) *
        ...
        if (stakeIndex != type(uint256).max) {
            stakerInformation[user].staked = (int256(
                uint256(stakerInformation[user].staked)
            ) + amounts.u(stakeIndex)).toUint256().toUint128();
