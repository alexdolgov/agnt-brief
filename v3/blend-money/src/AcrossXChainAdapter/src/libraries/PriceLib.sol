...
library PriceLib {
    ...
    /// @dev Uses the larger of the two decimals as the target; only the amount with fewer
    ///      decimals is scaled up. When equal, neither is scaled.
    ...
    /// @custom:reverts InvalidDecimalScale when |decimals1 - decimals2| > MAX_DECIMAL_DELTA (77)
    /// @custom:reverts NormalizationOverflow when amount * 10^delta overflows uint256
    function normalize(uint256 amount1, uint8 decimals1, uint256 amount2, uint8 decimals2)
        internal
        pure
        returns (uint256 norm1, uint256 norm2)
    {
        uint8 delta = decimals1 >= decimals2 ? decimals1 - decimals2 : decimals2 - decimals1;
        require(delta <= MAX_DECIMAL_DELTA, InvalidDecimalScale());

        if (decimals1 < decimals2) {
            uint256 scale = 10 ** uint256(delta);
            require(amount1 <= (type(uint256).max / scale), NormalizationOverflow());
            norm1 = amount1 * scale;
            norm2 = amount2;
        } else if (decimals2 < decimals1) {
            uint256 scale = 10 ** uint256(delta);
            require(amount2 <= (type(uint256).max / scale), NormalizationOverflow());
            norm1 = amount1;
            norm2 = amount2 * scale;
        } else {
            norm1 = amount1;
