 ...
 *      Follows CEI. Emits no events; relies on CCTP contract events for observability.
 *
 ...
 */
contract CCTPXChainAdapter is IXChainAdapterController {
    ...
    /**
     * @notice Parameters for a CCTP deposit operation.
     * @dev Encoded in the `data` parameter of `execute()`. Fee-less bridging: maxFee is always 0.
     */
    struct CCTPDepositParams {
        /**
         * @notice Amount of USDC to bridge to the destination chain
         * @dev Must be greater than zero. Actual amount burned may differ from balance if dust exists.
         */
        uint256 amount;
    }
     ...
     * @dev Expects `data` to be abi-encoded `CCTPDepositParams`. Validates chain, USDC, addresses, balance.
     *      Reads the local CCTP minter burn limit and splits `params.amount` into multiple burn messages
     ...
     * @param data ABI-encoded `CCTPDepositParams`
     ...
     * @custom:reverts InvalidBalance when contract balance is insufficient
     ...
     * @custom:events This function emits no events; observability is via the CCTP TokenMessenger contract events.
    ...
    function execute(uint256 chainId, address _sender, address _recipient, IERC20 _inputToken, bytes calldata data)
        ...
        override
    {
        CCTPDepositParams memory params = abi.decode(data, (CCTPDepositParams));
        ...
        require(_inputToken.balanceOf(address(this)) >= params.amount, InvalidBalance());
        ...
        _inputToken.forceApprove(address(TOKEN_MESSENGER), params.amount);
