...
///
/// @dev Restricted to pairs where at least one token is RAISE_TOKEN (WETH) to prevent
///      tax-free transfer tunnels via disposable junk-token pairs (BOAR-17 fix).
///      removeLiquidity sends tokens from the exempt LPManager to msg.sender with zero tax —
///      this is intentional: tax-free LP removal is the whole purpose of the LPManager.
contract BoardwalkLPManager {
    ...
    /// @dev At least one of tokenA/tokenB must be RAISE_TOKEN (BOAR-17 fix)
