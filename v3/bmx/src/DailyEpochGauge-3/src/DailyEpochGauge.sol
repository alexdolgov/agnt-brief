...
contract DailyEpochGauge is Ownable2Step {
    ...
    /// @notice TEST-ONLY: Directly credit the current day bucket (no N+2 scheduling)
    /// @dev Owner-only; intended for mainnet testing and will be removed for production
    function addRewardsTest(PoolId poolId, uint256 amount) external onlyOwner {
        uint32 dayNow = TimeLibrary.dayCurrent();
        dayBuckets[poolId][dayNow] += amount;
        emit RewardsAdded(poolId, amount);
    }
