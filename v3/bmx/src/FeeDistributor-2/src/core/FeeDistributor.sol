...
contract FeeDistributor is Timelocked, Initializable {
    ...
    function retryPendingFees() external {
        ...
        if (lpAmount > 0) {
            pendingLpFees = 0;
            try ILPStaking(lpStaking).notifyFees(lpAmount) {}
            catch {
                pendingLpFees += lpAmount;
                emit FeeForwardFailed("LPStaking", lpAmount);
            }
        }
        uint256 bwAmount = pendingBoardwalkFees;
        if (bwAmount > 0) {
            pendingBoardwalkFees = 0;
            try IBoardwalkFeeCollector(feeCollector).receiveFees(token, bwAmount) {}
            catch {
                pendingBoardwalkFees += bwAmount;
                emit FeeForwardFailed("FeeCollector", bwAmount);
            }
        }
    }
    ...
    function claimIntegratorFees() external {
        ...
        uint256 amount = integratorAccrued - integratorClaimed;
        if (amount == 0) revert NothingToClaimYet();
    ...
    ///      Atomically updates tax exemption on the BoardwalkToken so the new collector
    ///      can use swapExactTokensForTokens without triggering the transfer tax (BOAR-15 fix).
    ...
    function setFeeCollector(
        ...
        // Update tax exemption atomically (BOAR-15 fix)
