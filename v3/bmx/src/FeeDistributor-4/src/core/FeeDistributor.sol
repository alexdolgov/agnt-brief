...
/// @title FeeDistributor - Routes token tax to LP staking, Boardwalk, issuer, and referrer
...
///         Issuer claims as raise token with rate limit. Referrer claims in native token.
contract FeeDistributor is Timelocked, Initializable {
    ...
    address[] public issuerRecipients; // Issuer fee recipients (up to 3)
    uint256[] public issuerSplits; // Each split as BPS of issuer share (sum = 10000)
    string[] public issuerLabels;
    ...
    event TaxReceived(
        uint256 amount, uint256 lpShare, uint256 boardwalkShare, uint256 issuerShare, uint256 referrerShare
    );
    event IssuerClaimed(
    ...
    function initialize(
        ...
        if (p.issuerRecipients.length != p.issuerLabels.length) revert ArrayLengthMismatch();
        ...
        // Validate fee BPS
        uint256 _totalFeeBps = p.issuerBps + p.boardwalkBps + p.lpIncentiveBps + p.referrerBps;
        ...
        for (uint256 i = 0; i < p.issuerRecipients.length;) {
            ...
            issuerLabels.push(p.issuerLabels[i]);
            unchecked {
                ++i;
            }
        }
    ...
    function onTaxReceived(
        ...
        uint256 issuerShare = amount - lpShare - boardwalkShare - referrerShare;

        // Forward LP fees to LPStaking
        if (lpShare > 0) {
            try ILPStaking(lpStaking).notifyFees(lpShare) {}
            catch {
                pendingLpFees += lpShare;
                emit FeeForwardFailed("lpStaking", lpShare);
            }
        }

        // Forward Boardwalk fees to FeeCollector
        if (boardwalkShare > 0) {
            try IBoardwalkFeeCollector(feeCollector).receiveFees(token, boardwalkShare) {}
            catch {
                pendingBoardwalkFees += boardwalkShare;
                emit FeeForwardFailed("feeCollector", boardwalkShare);
            }
        }
        ...
        emit TaxReceived(amount, lpShare, boardwalkShare, issuerShare, referrerShare);
    }
    ...
    function retryPendingFees() external {
        ...
        if (lpAmount > 0) {
            pendingLpFees = 0;
            ILPStaking(lpStaking).notifyFees(lpAmount);
        }
        uint256 bwAmount = pendingBoardwalkFees;
        if (bwAmount > 0) {
            pendingBoardwalkFees = 0;
            IBoardwalkFeeCollector(feeCollector).receiveFees(token, bwAmount);
        }
    }
    ...
    /// @param recipientIdx Index of the issuer recipient (0-2)
    ...
    function claimAsRaiseToken(
        ...
        // Swap tokens to raise token via native Boardwalk LP
    ...
    function claimReferrerFees() external {
        ...
        uint256 amount = referrerAccrued - referrerClaimed;
    ...
    /// @notice Signal an issuer recipient address change (only current recipient can signal)
    ...
    /// @notice Execute an issuer recipient address change (anyone can call after delay)
    ...
    /// @notice Cancel a pending issuer address change (only current recipient can cancel)
    ...
    /// @notice Signal a referrer address change (timelocked)
    ...
    /// @notice Execute referrer address change after timelock delay
    ...
    /// @notice Cancel a pending referrer address change
    ...
    /// @notice Get the number of issuer recipients
