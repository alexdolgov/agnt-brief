...
contract GovernanceVoter is Ownable2Step, Timelocked {
    ...
    ///         Prevents budget duplication across epochs (BOAR-14 fix).
    ...
    constructor(
        ...
        // type(uint256).max = "never ineligible" sentinel; no real epoch will match
    ...
    // ============ Peer Initialization (BOAR-18/16/2 fix) ============

    /// @notice Wire peer contracts after deployment. Can only be called once.
    /// @dev Validates bidirectional wiring: both peers must point back to this contract.
    ///      Deploy order: GovernanceVoter → LPLocker(this) → ParticipationDistributor(this) → initializePeers()
    ...
    /// @notice Finalize an epoch after it ends. Batched: re-validates prior epoch's voters before
    ///         determining the winner. Call repeatedly with maxBatch until complete.
    ///         Sets finalizationInProgress=true (blocks staking via RewardRouterV5) until the
    ///         last batch completes.
    ///
    /// @dev Advance voting: finalize(N) uses epoch N-1's votes to determine epoch N's winner.
    ///      Budget = current WETH balance minus already-accounted funds from prior epochs.
    ///      Sequential: epoch N-1 must be finalized AND executed before epoch N can finalize.
    ///
    ...
    function finalize(
        ...
        if (!finalizationInProgress) {
            // Strictly sequential: previous epoch must be finalized AND executed (BOAR-20/14/5 fix)
            if (epoch > 0) {
            ...
            // Per-epoch budget: only new revenue since last finalization (BOAR-14 fix, CEI pattern)
        ...
        // Determine winner from prior epoch's votes (advance voting model)
        uint8 winner;
        bool quorumMet;
        if (epoch == 0) {
            // No prior votes exist — default to treasury
            ...
            /// @dev Quorum uses min(snapshotTotalWeight, liveSupply) to protect against supply decreases.
            ...
            uint256 quorumBase =
                votingEpoch.snapshotTotalWeight < liveSupply ? votingEpoch.snapshotTotalWeight : liveSupply;

            // BOAR-9 fix: zero supply or zero total vote weight → no quorum
            ...
            if (quorumMet) {
                ...
                for (uint8 i = 0; i < NUM_OPTIONS;) {
                    // BOAR-8 fix: skip ineligible options during winner selection
                ...
                // BOAR-9 fix: if all options ineligible or zero weight, default to treasury
                if (winner > 0) {
    ...
    /// @dev forceMarkExecuted is intentionally non-sequential — it's the emergency escape hatch
    ///      for when normal execution fails (e.g. a swap repeatedly reverts). The accountedBudget
    ///      arithmetic is safe because each epoch's budget is fixed at finalization time (which IS
    ///      sequential). The execution/force-execution order doesn't affect the accounting.
    ...
    function executeSetFallbackTreasury(
        ...
        _execute(ACTION_SET_FALLBACK_TREASURY, keccak256(abi.encode(_fallbackTreasury)));
        if (_fallbackTreasury == address(0)) revert ZeroAddress();
    ...
    /// @dev Validate voters from a specific epoch. Under advance voting, finalize(N) validates
    ///      epoch N-1's voters by reading their current SBF_BMX balance. If finalization is
    ///      delayed, voters' balances may have changed since they voted. This is the same
    ///      accepted tradeoff as BOAR-10 (voter weight can only decrease via unstaking).
    ...
    // WETH is unwrapped before the swap and ETH is sent via msg.value.
    ...
    // Uses native ETH for the WETH half of the LP position (v4 pools use ETH, not WETH).
    // SWEEP recovers excess ETH sent to PositionManager that wasn't consumed by the mint.
    function _executeBuyBurnLp(
        ...
        // Transfer BMX to router; ETH goes via msg.value
