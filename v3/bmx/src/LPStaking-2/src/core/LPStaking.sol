...
contract LPStaking is ReentrancyGuardTransient, Initializable {
    ...
    function stake(
        ...
        // Settle pending rewards BEFORE updating MP.
        if (_lpStaked > 0) {
            uint256 pending = _pendingReward(user, _accRewardPerWeight);
            if (pending > 0) {
                rewardToken.safeTransfer(msg.sender, pending);
                emit Claimed(msg.sender, pending);
            }
            user.rewardDebt = (_lpStaked + user.multiplierPoints) * _accRewardPerWeight / PRECISION;
        }
        ...
        user.rewardDebt = newWeight * _accRewardPerWeight / PRECISION;
    ...
    function withdraw(
        ...
        // Settle pending BEFORE MP update.
        uint256 pending = _pendingReward(user, _accRewardPerWeight);
        if (pending > 0) {
            rewardToken.safeTransfer(msg.sender, pending);
            emit Claimed(msg.sender, pending);
    ...
    function claim() external nonReentrant returns (uint256 pendingAmount) {
        ...
        // Calculate pending BEFORE MP update.
        pendingAmount = _pendingReward(user, _accRewardPerWeight);

        if (pendingAmount > 0) {
            user.rewardDebt = (user.lpStaked + user.multiplierPoints) * _accRewardPerWeight / PRECISION;
    ...
    function _distributeRewards() internal {
        ...
        if (totalWeight == 0) {
            // No stakers - ALL rewards for this period are LOST
            lastRewardUpdate = currentTime;
            return;
        }

        // Calculate vesting rewards for elapsed period
        uint256 vestingReward = _calculateVestingReward(lastUpdate, currentTime);

        // Calculate fee rewards for elapsed period within current epoch
    ...
    ///      MP is lazily evaluated: it only enters totalWeight when a user interacts (stake/withdraw/claim).
    ///      Active users who claim frequently will crystallize MP faster, capturing a slightly larger
    ///      share of rewards vs passive users. This is an inherent property of lazy evaluation in
    ///      MasterChef-style contracts and is consistent with the GMX/Morphex staking model (BOAR-7).
    ...
    function _simulatePendingRewards(
        ...
        if (totalWeight > 0) {
            uint256 vestingReward = _calculateVestingReward(lastRewardUpdate, block.timestamp);
            uint256 feeReward = _calculateFeeReward(lastRewardUpdate, block.timestamp);
            uint256 totalReward = vestingReward + feeReward;
            if (totalReward > 0) {
                simAccReward += totalReward * PRECISION / totalWeight;
            }
        }
