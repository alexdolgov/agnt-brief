...
contract LPStaking is ReentrancyGuardTransient, Initializable {
    ...
    uint256 public vestingAllocation; // Total tokens allocated for vesting over 3 years
    ...
    function stake(
        ...
        if (_lpStaked > 0) {
            ...
            // Set debt with current weight.
            user.rewardDebt = (_lpStaked + user.multiplierPoints) * accRewardPerWeight / PRECISION;
        }
        ...
        user.rewardDebt = newWeight * accRewardPerWeight / PRECISION;
    ...
    function claim() external nonReentrant returns (uint256 pendingAmount) {
        ...
        pendingAmount = _pendingReward(user);

        if (pendingAmount > 0) {
            // Set debt with old weight.
            user.rewardDebt = (user.lpStaked + user.multiplierPoints) * accRewardPerWeight / PRECISION;
    ...
    function _pendingReward(
        ...
        uint256 accumulated = weight * accRewardPerWeight / PRECISION;
