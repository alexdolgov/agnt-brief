...
contract LaunchFactory is Ownable2Step, Timelocked {
    ...
    uint256 private constant MIN_ADVANCED_DURATION = 10 minutes; // TODO: revert to 2 days before prod
    ...
    function _setVestingConfig(
        ...
        for (uint256 i = 0; i < len - 1;) {
            vestingAmounts[i] = issuerVestingTokens * config.vestingPercents[i] / AllocationLib.BPS_DENOMINATOR;
    ...
    function _validateConfig(
        ...
        if (config.path == LaunchPath.EXPRESS) {
            ...
            // BOAR-6 fix: Advanced launches with presalePercent < 5000 MUST have vesting recipients
            // to avoid silently under-minting supply (issuer vesting tokens only mint if vesting configured)
    ...
    /// @notice Execute Express graduation threshold change
    ...
    /// @notice Execute Advanced graduation threshold change
