...
contract PresaleManager is Initializable {
    ...
    /// @param labels Array of labels for each allocation
    function setVestingConfig(
        ...
        if (recipients.length != labels.length) revert ArrayLengthMismatch();

        for (uint256 i = 0; i < recipients.length;) {
            if (recipients[i] == address(0)) revert ZeroAddress();
            _vestingRecipients.push(recipients[i]);
            _vestingAmounts.push(amounts[i]);
            _vestingLabels.push(labels[i]);
            unchecked {
                ++i;
            }
        }
    ...
    function contribute(
        ...
        uint256 bonusMultiplier = 10000 + (1000 * (presaleDuration - timeElapsed) / presaleDuration);

        // Calculate weighted contribution
        uint256 weightedAmount = amount * bonusMultiplier / 10000;
    ...
    function seedLiquidity() external {
        ...
        // Allocation math must match LaunchFactory._setVestingConfig()
        uint256 presaleTokens = TOTAL_SUPPLY * presalePercent / 10000;
        uint256 liquidityTokens = presaleTokens;
        uint256 vestingTotal = TOTAL_SUPPLY - presaleTokens - liquidityTokens;
        uint256 lpIncentiveTokens = vestingTotal * 20 / 100; // 20% of vesting to LP staking
        ...
        IERC20(token).approve(router, liquidityTokens);
        raiseToken.approve(router, totalRaised);
        ...
        (,, uint256 lpTokens) = IDEXRouter(router)
            .addLiquidity(
                token,
                address(raiseToken),
                liquidityTokens,
                totalRaised,
                0, // min token (first LP, no slippage concern)
                0, // min raise token
                address(this),
                block.timestamp
            );
        ...
        if (vestingStream != address(0) && issuerVestingTokens > 0 && _vestingRecipients.length > 0) {
            IVestingStream(vestingStream)
                .initialize(token, liquiditySeedTime, _vestingRecipients, _vestingAmounts, _vestingLabels);
        }
