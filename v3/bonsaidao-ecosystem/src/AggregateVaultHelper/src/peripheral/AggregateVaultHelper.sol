...
contract AggregateVaultHelper is AggregateVaultStorage, AggregateVaultViews, BaseHandler, GlobalACL {
    ...
    function setVaultReturnBipsEscrow(uint256 _newBips) external onlyDelegateCall onlyConfigurator {
        IFeeEscrow(getVaultState().withdrawalFeeEscrow).setVaultReturnBips(_newBips);
        IFeeEscrow(getVaultState().depositFeeEscrow).setVaultReturnBips(_newBips);
    }
    ...
    function _getPendingFees(
        ...
        uint256 oneToken = vault.token == TOKEN_USDC_NATIVE ? 1e6 : 1e18;
        (bytes memory ret) = _getStorage().feeHelper.delegateCall(
            abi.encodeCall(
                ...
                (
                    currentBalance,
                    vaultSupply,
                    vaultSupply == 0 ? oneToken : oneToken * currentBalance / vaultSupply,
                    vault.feeWatermarkPPS,
                    vault.epochDelta,
        ...
        (,,, totalVaultFee) = abi.decode(ret, (uint256, uint256, uint256, uint256));
    }
