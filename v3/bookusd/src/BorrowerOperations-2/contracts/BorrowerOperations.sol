...
contract BorrowerOperations is LiquityBase, Ownable, CheckContract, IBorrowerOperations {
    ...
    function openTrove(
        ...
        require(
            borrowCap == 0 || currentDebt.add(vars.netDebt) <= borrowCap,
            "BorrowerOperations: Borrow cap exceeded"
        );
        ...
        if (msg.sender != owner()) {
            require(
                maxBorrowPerWallet == 0 || userDebt.add(vars.netDebt) <= maxBorrowPerWallet,
                "BorrowerOperations: Max borrow per wallet exceeded"
            );
        }
    ...
    function _adjustTrove(
        ...
        if (_isDebtIncrease) {
            ...
            uint256 currentDebt = activePool.getLUSDDebt();
            require(
                borrowCap == 0 || currentDebt.add(vars.netDebtChange) <= borrowCap,
                "BorrowerOperations: Borrow cap exceeded"
            );

            // --- Enforce max borrow per wallet ---
            uint256 userDebt = vars.debt;
            if (msg.sender != owner()) {
                require(
                    maxBorrowPerWallet == 0 || userDebt.add(vars.netDebtChange) <= maxBorrowPerWallet,
                    "BorrowerOperations: Max borrow per wallet exceeded"
                );
            }
        }
