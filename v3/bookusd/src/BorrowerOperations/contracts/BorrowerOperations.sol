...
contract BorrowerOperations is LiquityBase, Ownable, CheckContract, IBorrowerOperations {
    ...
    // --- Borrow cap state ---
    uint256 public borrowCap = 100_000_000e18;
    event BorrowCapChanged(uint256 newCap);

    // --- Max borrow per wallet state ---
    uint256 public maxBorrowPerWallet = 10_000e18;
    ...
    function setAddresses(
        ...
        // Approve contracts that need permission to transfer tokens
    ...
    function setBorrowCap(uint256 _cap) external override onlyOwner {
        borrowCap = _cap;
        emit BorrowCapChanged(_cap);
    }

    function setMaxBorrowPerWallet(uint256 _max) external override onlyOwner {
        maxBorrowPerWallet = _max;
        emit MaxBorrowPerWalletChanged(_max);
    }
    ...
    function openTrove(
        ...
        require(block.timestamp >= startTime, "BorrowerOperations: Trove creation not yet enabled");
        // transfer BOOK in

        require(book.transferFrom(msg.sender, address(this), _collateralAmount), "Could not transfer token");
        ...
        // --- Enforce borrow cap ---
        uint256 currentDebt = activePool.getLUSDDebt();
        require(borrowCap == 0 || currentDebt.add(vars.netDebt) <= borrowCap, "BorrowerOperations: Borrow cap exceeded");

        // --- Enforce max borrow per wallet ---
        uint256 userDebt = troveManager.getTroveDebt(msg.sender);
        if (msg.sender != owner()) {
            require(maxBorrowPerWallet == 0 || userDebt.add(vars.netDebt) <= maxBorrowPerWallet, "BorrowerOperations: Max borrow per wallet exceeded");
        }
    ...
    function addColl(uint _collateralAmount, address _upperHint, address _lowerHint) external override {
        ...
        _adjustTrove(msg.sender, 0, 0, false, _upperHint, _lowerHint, 0, _collateralAmount);
    }
    ...
    function moveETHGainToTrove(
        ...
        require(book.transferFrom(msg.sender, address(this), _bookAmount), "Could not transfer token");
        _adjustTrove(_borrower, 0, 0, false, _upperHint, _lowerHint, 0, _bookAmount);
    }
    ...
    function _adjustTrove(
        ...
        _requireSingularCollChange(_collWithdrawal, _bookAmount);
        _requireNonZeroAdjustment(_collWithdrawal, _LUSDChange, _bookAmount);
        _requireTroveisActive(contractsCache.troveManager, _borrower);

        // Confirm the operation is either a borrower adjusting their own trove, or a pure ETH transfer from the Stability Pool to a trove
        assert(msg.sender == _borrower || (msg.sender == stabilityPoolAddress && _bookAmount > 0 && _LUSDChange == 0));

        contractsCache.troveManager.applyPendingRewards(_borrower);

        // Get the collChange based on whether or not ETH was sent in the transaction
        (vars.collChange, vars.isCollIncrease) = _getCollChange(_bookAmount, _collWithdrawal);

        vars.netDebtChange = _LUSDChange;

        // --- Enforce borrow cap and per-wallet cap for debt increases ---
        _requireBorrowCaps(_LUSDChange, _isDebtIncrease, isRecoveryMode, _borrower, contractsCache.troveManager, contractsCache.activePool);
    ...
    function _activePoolAddColl(IActivePool _activePool, uint _amount) internal {
        bool success = _activePool.receiveTokens(_amount);
        require(success, "BorrowerOps: Sending BOOK to ActivePool failed");
    }
    ...
    function _requireSingularCollChange(uint _collWithdrawal, uint _bookAmount) internal pure {
        require(_bookAmount == 0 || _collWithdrawal == 0, "BorrowerOperations: Cannot withdraw and add coll");
    }
    ...
    function _requireNonZeroAdjustment(uint _collWithdrawal, uint _LUSDChange, uint _bookAmount) internal pure {
        require(
            _bookAmount != 0 || _collWithdrawal != 0 || _LUSDChange != 0,
    ...
    function _requireBorrowCaps(
        ...
        if (_isDebtIncrease) {
            uint256 currentDebt = _activePool.getLUSDDebt();
            uint256 totalNewDebt = currentDebt.add(_LUSDChange);
            uint256 userDebt = _troveManager.getTroveDebt(_borrower);
            uint256 userNewDebt = userDebt.add(_LUSDChange);
            if (!_isRecoveryMode) {
                uint fee = _troveManager.getBorrowingFee(_LUSDChange);
                totalNewDebt = totalNewDebt.add(fee);
                userNewDebt = userNewDebt.add(fee);
            }
            require(borrowCap == 0 || totalNewDebt <= borrowCap, "BorrowerOperations: Borrow cap exceeded");
            if (_borrower != owner()) {
                require(maxBorrowPerWallet == 0 || userNewDebt <= maxBorrowPerWallet, "BorrowerOperations: Max borrow per wallet exceeded");
            }
        }
    }
}
