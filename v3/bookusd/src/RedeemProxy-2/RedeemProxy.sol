...
// File contracts/Interfaces/IBOOKToken.sol
...
interface ILQTYStaking {
    ...
    event BOOKAddressSet(address _book);
...
interface IStabilityPool {
    ...
    event BOOKAddressChanged(address _newBookAddress);
...
// File contracts/RedeemProxy.sol
...
contract RedeemProxy {
    ...
    IBudPool public constant budPool = IBudPool(0x1220952d4D7543A913F3aa1209Ec4BE6372cac36);

    uint32 public twapDuration;

    address public owner;

    uint256 public minBudPrice = 99e16; // 0.99 BUD price in 18 decimals
    ...
    event WhitelistAddressAdded(address indexed user, uint256 timestamp);
    event WhitelistAddressRemoved(address indexed user, uint256 timestamp);

    event UpdateMinBudPrice(uint256 newMinBudPrice);
    ...
    function redeemCollateral(
        ...
        require(isEligible(msg.sender), "RedeemProxy: Cannot redeem when BUD price is above minimum");
    ...
    function isEligible(address _addr) public view returns (bool) {
        if (getBudTwap() <= minBudPrice) {
            return true; // If BUD price is below or equal to min, anyone can redeem
        }

        return checkWhitelist(_addr);
    }

    // TWAP for BUD token - 60 second

    function getBudTwap() public view returns (uint256) {
        uint32 _twapDuration = 60;
        uint32[] memory secondsAgo = new uint32[](2);
        secondsAgo[0] = _twapDuration;
        secondsAgo[1] = 0;

        (int56[] memory tickCumulatives, ) = budPool.observe(secondsAgo);
        int24 currentTwap = int24((tickCumulatives[1] - tickCumulatives[0]) / _twapDuration);

        uint256 priceU18 = OracleLibrary.getQuoteAtTick(
            currentTwap,
    ...
    // BUD PRICE ALLOWED MANAGEMENT

    function setMinBudPrice(uint256 _minBudPrice) external {
        require(msg.sender == owner, "RedeemProxy: Only owner can set min BUD price");
        require(_minBudPrice >= 99e16, "RedeemProxy: min BUD price must be greater than 99 cents");
        minBudPrice = _minBudPrice;
        emit UpdateMinBudPrice(_minBudPrice);
    }

    // OWNER MANAGEMENT

    function setOwner(address _newOwner) external {
        require(msg.sender == owner, "RedeemProxy: Only owner can set new owner");
        require(_newOwner != address(0), "RedeemProxy: New owner cannot be zero address");
        owner = _newOwner;
        emit UpdateOwner(_newOwner);
    }

    function renounceOwnership() external {
        require(msg.sender == owner, "RedeemProxy: Only owner can renounce ownership");
        owner = address(0);
    ...
    function addToWhitelist(address[] calldata toAddAddresses) external {
        require(msg.sender == owner, "RedeemProxy: Only owner can add to whitelist");
        for (uint i = 0; i < toAddAddresses.length; i++) {
            if (whitelist[toAddAddresses[i]] != true) {
                whitelist[toAddAddresses[i]] = true;
                emit WhitelistAddressAdded(toAddAddresses[i], block.timestamp);
            }
        }
    }

    function removeFromWhitelist(address[] calldata toRemoveAddresses) external {
        require(msg.sender == owner, "RedeemProxy: Only owner can remove from whitelist");
        for (uint i = 0; i < toRemoveAddresses.length; i++) {
            if (whitelist[toRemoveAddresses[i]] == true) {
                delete whitelist[toRemoveAddresses[i]];
                emit WhitelistAddressRemoved(toRemoveAddresses[i], block.timestamp);
            }
        }
    }
