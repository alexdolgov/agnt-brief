...
contract BracketVaultV2 is RebasingToken, AccessControlUpgradeable, PausableUpgradeable, IBracketVaultV2 {
    ...
    bytes32 private constant MANAGER_FEE_CLAIMER_ROLE = keccak256("MANAGER_FEE_CLAIMER_ROLE");
    bytes32 private constant BRACKET_FEE_CLAIMER_ROLE = keccak256("BRACKET_FEE_CLAIMER_ROLE");
    bytes32 private constant NAV_UPDATER_ROLE = keccak256("NAV_UPDATER_ROLE");

    uint48 private constant LOCK_FREQUENCY = 7 days;

    mapping(bytes32 => WithdrawalStatus) private withdrawals;
    ...
    function updateNav(uint256 newNav, uint256 managerPerformanceFee, uint256 managerTvlFee, uint256 brktTvlFee) external onlyRole(NAV_UPDATER_ROLE) onlyLocked {
    ...
    function claimManagerFees() external onlyRole(MANAGER_FEE_CLAIMER_ROLE) {
    ...
    function switchPause() external onlyRole(DEFAULT_ADMIN_ROLE) {
        if (paused()) {
            _unpause();
        } else {
            _pause();
