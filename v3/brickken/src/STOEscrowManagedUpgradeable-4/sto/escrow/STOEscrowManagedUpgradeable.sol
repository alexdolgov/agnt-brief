...
contract STOEscrowManagedUpgradeable is STOEscrowUpgradeable, AccessControlEnumerableUpgradeable {
    ...
    function initialize(
        ...
        } else if (!hasRole(AccessControlUpgradeable.DEFAULT_ADMIN_ROLE, _msgSender()))
        ...
        for(uint256 i = 1; i <= issuanceIndex; i++) {
            if(issuances[i].raisedAmount > 0) issuances[i].paymentTokensCollected = issuances[i].raisedAmount * 
                10 ** paymentToken.decimals() /
                1e18;

            if(issuances[i].status == IssuanceStatuses.WITHDRAWN) issuances[i].partialWithdraw = issuances[i].raisedAmount;
            if(issuances[i].status == IssuanceStatuses.ROLLBACK) paymentTokensToRefund[i] = address(paymentToken);
        }
    }
    ...
    function setPaymentToken(
        ...
        _setPaymentToken(newPaymentToken, newPaymentTokenOracle, newTwapInterval);
    ...
    function setPriceAndSwapManager(address newPriceAndSwapManager) external onlyRole(AccessControlUpgradeable.DEFAULT_ADMIN_ROLE) {
    ...
    function changeWithdrawalFee(uint256 newFee) external onlyRole(AccessControlUpgradeable.DEFAULT_ADMIN_ROLE) {
    ...
    function newOffering(Issuance calldata newIssuance) external onlyRole(Roles.ESCROW_NEW_OFFERING_ROLE) {
    ...
    function finalizeIssuance(address withdrawTo) external onlyRole(Roles.ESCROW_OFFERING_FINALIZER_ROLE) {
    ...
    function partialWithdraw(uint256 amount, address withdrawTo) external onlyRole(Roles.ESCROW_WITHDRAW_ROLE) {
        ...
        if (isEnded(issuanceIndexCached))
            revert Errors.IssuanceEnded(caller, issuances[issuanceIndexCached].endDate);

        if (!isSuccess(issuanceIndexCached)) revert Errors.IssuanceNotSuccess(caller);
    ...
    /// @param amount is in paymentToken units, so if paymentToken is USDC, this amount goes with 6 decimals
    /// @param to the user that should receive the corresponding tokens from this offchain ticket.
    function offchainReporting(uint256 amount, address to) external onlyRole(Roles.ESCROW_OFFCHAIN_REPORTER_ROLE) {
        _offchainReporting(amount, to);
        emit OffchainReport(amount,to);
    }
