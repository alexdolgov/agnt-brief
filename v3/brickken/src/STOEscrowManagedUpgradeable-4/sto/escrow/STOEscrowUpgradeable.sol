...
import { IERC20MetadataUpgradeable, IERC20Upgradeable } from "@openzeppelin/contracts-upgradeable@4.9.3/token/ERC20/extensions/IERC20MetadataUpgradeable.sol";
    ...
    ReentrancyGuardUpgradeable
{
    ...
    mapping(address tokenAddress => ERC20Token config) public tokenERC20Whitelist;
    ...
    mapping(uint256 issuanceIndex => mapping(address investorAddress => Investor info)) public investors;
    ...
    mapping(uint256 issuanceIndex => address paymentTokenForRefund) public paymentTokensToRefund;
    ...
    event ChangePaymentToken(address indexed newPaymentTokenAddress, address indexed newPaymentTokenOracle);
    ...
    /// @param amount amount of paymentTokens being reported
    ...
    function buyToken(
        ...
        Investor storage investor = investors[issuanceIndex][caller];
        if(investor.redeemed) revert Errors.TokensAlreadyReedemed(caller);

        if (tokenUsed != address(paymentTokenCached)) {
            bool isBalanceEnough = IERC20Upgradeable(tokenUsed).balanceOf(caller) >= amountOfTokens;
            if(!isBalanceEnough) revert Errors.InsufficientBalance(caller, tokenUsed, amountOfTokens);
            ...
            uint256 previewBalance = paymentTokenCached.balanceOf(address(this));
        ...
        uint256 usdPriceOfPaymentToken = priceAndSwapManager.getUSDPriceOfPaymentToken(
            paymentTokenOracle,
            twapInterval
        );
        ...
        if(maxAmount > 0 && actualAmount > maxAmount) {     
            ...
            SafeERC20Upgradeable.safeTransfer(paymentTokenCached, caller, actualAmount-maxAmount);
        ...
        // Avoid stack too deep error
        {
            ...
            uint256 newAmountInPaymentToken = investor.amountInPaymentToken + actualAmount;
            ...
            issuanceCached.paymentTokensCollected = issuanceCached.paymentTokensCollected + actualAmount;
    ...
    function getEstimationSTOToken(
        ...
        return priceAndSwapManager.getEstimationSTOToken(
            caller,
    ...
    function amountAvailable(
        ...
        if (isActive(issuanceIndexQueried) && !isHardcapReached(issuanceIndexQueried)) {
            Issuance memory issuanceCached = issuances[issuanceIndexQueried];
            return
                issuanceCached.hardCap - issuanceCached.raisedAmount > issuanceCached.maxTicket
                    ? issuanceCached.maxTicket
    ...
    function getPriceInPaymentToken(
        ...
        return priceAndSwapManager.getPriceInPaymentToken(
            IPriceAndSwapManager.PoolDetails(
                tokenAddress,
                address(paymentToken),
                router,
    ...
    function getUSDPriceOfPaymentToken() public view returns (uint256) {
        return priceAndSwapManager.getUSDPriceOfPaymentToken(
            paymentTokenOracle,
            twapInterval
        );
    }
    ...
    function _setPriceAndSwapManager(
        address newPriceAndSwapManager
    ) internal {
        if(!newPriceAndSwapManager.isContract()) revert Errors.NotContractAddress();
    ...
    function _changeTreasuryAddress(
    ...
    function _newOffering(Issuance calldata newIssuance) internal {
        ...
        if (
            ...
            !(isWithdrawn(issuanceIndexCached) || isRollback(issuanceIndexCached))
        ...
        if (
            ...
            (stoRelatedTokenCached.supplyCap() - stoRelatedTokenCached.totalSupply())
        ) revert Errors.SupplyCapExceeded();

        if (
            newIssuance.issuanceAmount + stoRelatedTokenCached.totalSupply() > type(uint224).max
    ...
    function _checkValidStatus(
        ...
        if (!stoRelatedToken.hasRole(Roles.TOKEN_WHITELIST_ROLE, caller) && !stoRelatedToken.whitelist_OLD_SLOT(caller))
            revert Errors.UserIsNotWhitelisted(caller);

        if (issuanceIndexCached == 0) revert Errors.IssuanceNotStarted(caller);

        if (!isStarted(issuanceIndexCached)) revert Errors.IssuanceNotStarted(caller);

        if (isEnded(issuanceIndexCached))
            revert Errors.IssuanceEnded(caller, endDate);

        if (!tokenERC20Whitelist[tokenUsed].status)
            revert Errors.TokenIsNotWhitelisted(tokenUsed, caller);

        if (
            raisedAmount == hardCap
        ) revert Errors.HardCapRaised();
    }
    ...
    /// @param caller Address of the investor
    ...
    function _validateInputs(uint256 amount, address caller, uint256 usdPriceOfPaymentToken, uint256 paymentTokenScale) internal view returns(uint256){
        Issuance storage issuanceCached = issuances[issuanceIndex];
        ...
        if ( // alllow quantities less than minTicket only if what's left is minor than minTicket
            (usdValue < minTicket) &&
            (hardCap - raisedAmount) >= minTicket
        ) revert Errors.InsufficientAmount(caller, usdValue, minTicket);

        uint256 maxTicket = issuanceCached.maxTicket;

        if (usdValue > maxTicket)
            revert Errors.AmountExceeded(caller, usdValue, maxTicket);
        ...
        if(isInvestor(issuanceIndex, caller)) {
            uint256 newAmountInPaymentToken = investors[issuanceIndex][caller].amountInPaymentToken + amount;
    ...
    function _redeemToken(uint256 index) internal {
        ...
        if (isRedeemed(index, caller)) revert Errors.RedeemedAlready(caller, index);

        if (!isInvestor(index, caller)) revert Errors.NotInvestor(caller, index);

        if (
            !isWithdrawn(index) && issuances[index].partialWithdraw == 0
        ) revert Errors.IssuanceNotWithdrawn(issuer);

        uint256 amountInSTO = investors[index][caller].amountInSTO;

        investors[index][caller].redeemed = true;

        stoRelatedToken.mint(caller, amountInSTO);

        emit Redeemed(caller, index, amountInSTO);
    }
    ...
    function _refundToken(uint256 index) internal {
        ...
        if (isRefunded(index, caller)) revert Errors.RefundedAlready(caller, index);

        if (!isInvestor(index, caller)) revert Errors.NotInvestor(caller, index);

        if (!isRollback(index)) revert Errors.IssuanceNotInRollback(index);

        uint256 amountInPaymentToken = investors[index][caller].amountInPaymentToken;
    ...
    function _partialWithdraw(uint256 amountToWithdraw, address withdrawTo) internal {
       ...
       issuances[issuanceIndexCached].partialWithdraw = partialWithdrawn + amountToWithdraw;

        // Withdraw
        uint256 fee = amountToWithdraw.mulDiv(
            withdrawalFee,
            MAX_FEE_LIMIT,
            MathUpgradeable.Rounding.Up
        );
        ...
        SafeERC20Upgradeable.safeTransfer(paymentToken, withdrawTo, amountToWithdraw - fee);

        emit Withdrawn(issuer, issuanceIndexCached, fee, amountToWithdraw - fee);
    }
    ...
    function _rollBack() internal {
        uint256 issuanceIndexCached = issuanceIndex;
        if (isRollback(issuanceIndexCached)) revert Errors.IssuanceWasRollbacked(issuer);
    ...
    function _offchainReporting(uint256 offchainAmount, address toAssign) internal {
        Issuance storage issuanceCached = issuances[issuanceIndex];
        Investor storage investor = investors[issuanceIndex][toAssign];
