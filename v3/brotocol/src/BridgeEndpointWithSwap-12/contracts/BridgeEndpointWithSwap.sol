    ...
    onlyApprovedToken(tokenOut)
  {
    ...
    bytes32 orderHash = _hashTypedDataV4(
      keccak256(
        abi.encode(
          keccak256(
            "Order(address,address,address,uint256,uint256,bytes,bytes,bytes,bytes32)"
          ),
          target,
          tokenIn,
          tokenOut,
    ...
    if (success) {
      ...
      if (registry.burnable(tokenOut)) {
        IBurnable(tokenOut).burnFrom(address(this), swapOut);
      } else {
        ERC20(tokenOut).transferFromFixed(address(this), pegInAddress, swapOut);
      }
      ...
      if (tokenInBalance1 - tokenInBalance0 > 0) {
        IBurnable(tokenIn).burnFrom(
          address(this),
          tokenInBalance1 - tokenInBalance0
        );
      }
    } else {
      // Emit error reason for debugging
      emit SwapTargetError(target, reason);
      IBurnable(tokenIn).burnFrom(address(this), amountIn);
      emit TransferToSwapEvent(
        orderHash,
        salt,
        target,
