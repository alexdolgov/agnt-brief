  ...
  Allowlistable
{
  ...
  /**
   * @dev Updates an existing order or creates a new one
  ...
  function updateOrder(
    ...
    _createOrUpdateOrder(orderHash, recipient, token, amount, salt, sent);
  }
    ...
    notWatchlist(recipient)
  {
    ...
    if (registry.burnable(token)) {
      ...
      _createOrUpdateOrder(orderHash, recipient, token, amount, salt, false);
    }
  }
  ...
  /**
   * @dev Internal function to create or update an order
   * @param orderHash The hash of the order
   * @param recipient The recipient address
   * @param token The token address
   * @param amount The amount of tokens
