    ...
    ERC20("Genesis NFT Pool", "GNFT-BSCS")
{
    ...
    struct UserInfo {
        ...
        mapping(uint256 => uint256) amountPerNftId; // How many staked tokens the user has provided
        uint256 lastStakingBlock;
        mapping(ERC20 => uint256) rewardDebt; // Reward debt
        mapping(ERC20 => mapping(uint256 => uint256)) rewardDebtPerNftId; // Reward debt per nft id
   ...
   //// extra state for genesis nft staking
    ...
    // nft address to nft id to genesis nft details
    mapping(address => mapping(uint256 => GenesisNFT)) public nftIdToDetail;
    ...
    mapping(uint256 => uint256) public tokenIdStakedAmount;
  ...
  //// end extra info for genesis nft staking
    ...
    {   
        ...
        GenesisNFT storage nft = nftIdToDetail[_nftAddress][_nftId];
        if (nft.nftId == _nftId) {
            nft.owner = _newOwner;
        }
    }
    ...
    {   
        ...
        GenesisNFT storage newNft = nftIdToDetail[_tokenAddress][_nftId];
    ...
    function deposit(uint256 _nftId, address _nftAddress) external nonReentrant {
        ...
        // if user ever transferred/sold the nft to other user then now update the owner
        if (IERC721(_nftAddress).ownerOf(_nftId) != nft.owner) {
        ...
        //if (user.amount > 0) {
            ...
            //for (uint256 i = 0; i < rewardTokens.length; i++) {
                ...
                // if (pending > 0) {
                //     //// transferring reward token to user
                //     IERC20(rewardTokens[i]).safeTransfer(address(msg.sender),pending);
                // }

            //}
        //}

        if (user.amountPerNftId[_nftId] > 0) {
            uint256 pendingNft;
            for (uint256 i = 0; i < rewardTokens.length; i++) {
                pendingNft = user
                .amountPerNftId[_nftId]
                .mul(accTokenPerShare[rewardTokens[i]])
                .div(PRECISION_FACTOR[rewardTokens[i]])
                .sub(user.rewardDebtPerNftId[rewardTokens[i]][_nftId]);

                if (pendingNft > 0) {
                    //// transferring reward token to user
                    IERC20(rewardTokens[i]).safeTransfer(address(msg.sender),pendingNft);
                }
            }
        }
        ...
        if (_amount > 0) {
            ...
            user.amountPerNftId[_nftId] = user.amountPerNftId[_nftId].add(_amount);
        ...
        for (uint256 i = 0; i < rewardTokens.length; i++) {
            ...
            user.rewardDebtPerNftId[rewardTokens[i]][_nftId] = user
            .amountPerNftId[_nftId]
            .mul(accTokenPerShare[rewardTokens[i]])
            .div(PRECISION_FACTOR[rewardTokens[i]]);
        }
     ...
     */
    function withdraw(uint256 _nftId, address _nftAddress, bool harvestOnly) external nonReentrant {
        require(unStakingBlock <= block.number, "Unstaking has not started");
        
        UserInfo storage user = userInfo[msg.sender];
        GenesisNFT storage nft = nftIdToDetail[_nftAddress][_nftId];
        
        require(nft.isStaked == true, "NFT token is not in the pool");
        require(nft.owner == msg.sender, "You are not the owner");
        
        uint256 _amount = nft.price;
        
        // updating reward variables of the given pool to be up-to-date.
        _updatePool();
        
        uint256 pending;
        for (uint256 i = 0; i < rewardTokens.length; i++) {
            uint256 amtPerNft = user.amountPerNftId[_nftId];
            pending = 
                amtPerNft
                .mul(accTokenPerShare[rewardTokens[i]])
                .div(PRECISION_FACTOR[rewardTokens[i]])
        ...
        if (_amount > 0 && harvestOnly == false) {
            user.amount = user.amount.sub(_amount);

            user.amountPerNftId[_nftId] = user.amountPerNftId[_nftId].sub(_amount);
            //burn LP token 
     ...
     */
    function emergencyWithdraw(uint256 _nftId, address _nftAddress) external nonReentrant {
        UserInfo storage user = userInfo[msg.sender];
        ...
        require(nft.nftId == _nftId,"emergencyWithdraw: Invalid NFT");
        
        user.amount = 0;
        for (uint256 i = 0; i < rewardTokens.length; i++) {
            user.rewardDebt[rewardTokens[i]] = 0;
            user.rewardDebtPerNftId[rewardTokens[i]][_nftId] = 0;
        }
    ...
    function emergencyRewardWithdraw(uint256 _amount) external onlyOwner {
        for (uint256 i = 0; i < rewardTokens.length; i++) {
            IERC20(rewardTokens[i]).safeTransfer(address(msg.sender), _amount);
        }
    }
     ...
     * @return Pending reward for a given user by NFT token ID
     */
    function pendingRewardByNftID(address _user, address _nftAddress, uint256 _nftId)
        external
        view
        returns (uint256[] memory, ERC20[] memory)
    {
        ...
        if (block.number > lastRewardBlock && stakedTokenSupply != 0) {
            ...
            for (uint256 i = 0; i < rewardTokens.length; i++) {
                ...
                uint256 pendingByNftId = user.amountPerNftId[_nftId];

                if (nft.isStaked == false) {
                   userPendingRewards[i] = 0;
                } else {
                  userPendingRewards[i] = 
                  pendingByNftId
