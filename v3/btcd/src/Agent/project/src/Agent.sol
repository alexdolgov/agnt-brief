...
contract Agent is UsingLZ {
    ...
    /// @notice Emitted when a deposit is fully processed by the Agent.
    /// @param requestId The originating request ID.
    /// @param assets Amount of asset received from the Router.
    /// @param staked Amount of vetToken minted by the gateway.
    /// @param shares Amount of sVetToken staked into the vault.
    ...
    /// @notice Emitted when a redeem is fully processed by the Agent (instant path or claimUnstake).
    /// @param requestId The originating request ID.
    /// @param shares Amount of sVetToken redeemed from the vault.
    /// @param unstaked Amount of vetToken reclaimed.
    /// @param assets Amount of underlying asset returned to the Router.
    ...
    function handleDepositRequest(
        ...
        emit DepositRequestProcessed(requestId_, assets_, _peggedAmount, _shares);
    }
    ...
    function handleRedeemRequest(
        ...
        if (_instantAllowed) {
            ...
            emit RedeemRequestProcessed(requestId_, shares_, _unstaked, _assets);
        } else {
    ...
    function claimUnstake(uint256 requestId_) external payable nonReentrant {
        ...
        emit RedeemRequestProcessed(requestId_, _r.shares, _unstaked, _assets);
    }
    ...
    function _receive(
        ...
        if (_msgType == MSG_DEPOSIT_REQUEST) {
            (_failed, _reason) = _tryHandleDepositRequest(_requestId, tokenIn_, amountIn_, msg_, ethValue_);
        } else if (_msgType == MSG_REDEEM_REQUEST) {
            (_failed, _reason) = _tryHandleRedeemRequest(_requestId, tokenIn_, amountIn_, msg_, ethValue_);
        } else {
    ...
    function _tryHandleDepositRequest(
        ...
        bytes memory msg_,
        uint256 ethValue_
    ) private returns (bool _failed, bytes memory _reason) {
