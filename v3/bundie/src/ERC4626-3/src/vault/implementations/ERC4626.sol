...
contract ERC4626 is Vault {
    ...
    function withdraw(
        ...
        // Check vault redeem limits before attempting
        _checkMaxRedeem(vault, actualShares, address(this));
        ...
        try IERC4626(vault).redeem(actualShares, receiver, address(this)) returns (uint256 _assets) {
    ...
    /// @notice Check if redeem amount exceeds vault's maximum redeem limit
    /// @param vault ERC4626 vault address
    /// @param shares Intended redeem amount
    /// @param owner Owner of shares being redeemed
    function _checkMaxRedeem(address vault, uint256 shares, address owner) internal {
        try IERC4626(vault).maxRedeem(owner) returns (uint256 maxShares) {
            require(shares <= maxShares, Errors.WithdrawFailed());
        } catch (bytes memory lowLevelData) {
            emit Events.ErrorLogBytes(lowLevelData);
            revert Errors.ExternalCallFailed();
        }
