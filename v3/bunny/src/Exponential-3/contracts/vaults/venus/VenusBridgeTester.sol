...
contract VenusBridgeTester is Whitelist, Exponential, IVaultVenusBridge {
    ...
    uint public override performanceFeeBp;
    ...
    function setFeeTreasury(address _treasury) external onlyOwner {
        require(_treasury != address(0), 'VaultVenusBridge: invalid address');
        feeTreasury = _treasury;
    }

    function setPerformanceFeeBp(uint _performanceFeeBp) external onlyOwner {
        require(_performanceFeeBp <= 10000, 'VaultVenusBridge: invalid performanceFeeBp');
        performanceFeeBp = _performanceFeeBp;
    }

    function migrateTo(address payable target) external override onlyOwner {
    ...
    function deposit(address vault, uint amount) external override payable onlyOwner {
        MarketInfo storage market = markets[vault];
        market.available = market.available.add(msg.value > 0 ? msg.value : amount);
    }

    function withdraw(address account, uint amount) external override onlyOwner {
        MarketInfo storage market = markets[msg.sender];
    ...
    function harvest() public override updateAvailable(msg.sender) onlyOwner {
        ...
        if (xvsBalance > 0) {
            ...
            if (feeTreasury != address(0)) {
                uint performanceFee = xvsBalance.mul(performanceFeeBp).div(10000);
                xvsBalance = xvsBalance.sub(performanceFee);
                XVS.safeTransfer(feeTreasury, performanceFee);
            }
    ...
    function mint(uint amount) external override updateAvailable(msg.sender) onlyOwner {
        MarketInfo memory market = markets[msg.sender];
        if (market.token == WBNB) {
    ...
    function redeemUnderlying(uint amount) external override updateAvailable(msg.sender) onlyOwner {
        MarketInfo memory market = markets[msg.sender];
        IVToken vToken = IVToken(market.vToken);
        vToken.redeemUnderlying(amount);
    }

    function redeemAll() external override updateAvailable(msg.sender) onlyOwner {
        MarketInfo memory market = markets[msg.sender];
        IVToken vToken = IVToken(market.vToken);
        vToken.redeem(market.vTokenAmount);
    }

    function borrow(uint amount) external override updateAvailable(msg.sender) onlyOwner {
        MarketInfo memory market = markets[msg.sender];
        IVToken vToken = IVToken(market.vToken);
        vToken.borrow(amount);
    }

    function repayBorrow(uint amount) external override updateAvailable(msg.sender) onlyOwner {
        MarketInfo memory market = markets[msg.sender];
