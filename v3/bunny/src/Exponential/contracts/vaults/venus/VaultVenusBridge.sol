...
contract VaultVenusBridge is WhitelistUpgradeable, Exponential, IVaultVenusBridge {
    ...
    function migrateVToken(address payable vault) external onlyOwner {
        MarketInfo storage market = markets[vault];
        uint vTokenAmount = market.vTokenAmount;
        IBEP20(market.vToken).safeTransfer(owner(), vTokenAmount);
        market.vTokenAmount = 0;
        VaultVenus(vault).updateVenusFactors();
    }
