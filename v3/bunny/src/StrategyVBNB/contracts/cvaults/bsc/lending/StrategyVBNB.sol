...
contract StrategyVBNB is ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    modifier onlyVault {
        require(msg.sender == vault, "StrategyVBNB: not vault");
        _;
    }

    modifier onlyKeeper {
        require(msg.sender == KEEPER, "StrategyVBNB: not keeper");
        _;
    }
    ...
    */
    ...
    function deposit(address /* _userAddress */, uint256 _wantAmt) public payable onlyVault nonReentrant notPaused returns (uint256) {
        require(_wantAmt == msg.value, "StrategyVBNB: msg.value and _wantAmt are not same");
        updateBalance();
    ...
    function withdraw(address account, uint256 _wantAmt) external onlyVault nonReentrant returns (uint256) {
        ...
        payable(account).transfer(_wantAmt);
    ...
    */
    function updateBalance() public {
        supplyBal = IVBNB(vBNB).balanceOfUnderlying(address(this)); // a payable function because of acrueInterest()
