...
contract BunnyParkCakeBPPoolV2 is Ownable {
    using SafeMath for uint256;
    ...
    // Info of each pool.
    struct PoolInfo {
        uint256 supply;         // add all user deposited cake in this pool
    ...
    event Deposit(address indexed user, uint256 amount, uint256 havestAmount);
    event Withdraw(address indexed user, uint256 amount, uint256 havestAmount);
    ...
    constructor(IMasterChef _CAKE_MASTER_CHEF, IBEP20 _cake, IBPToken _bp, uint256 _startBlock, address _cakeRecipientAddress) public {
        CAKE_MASTER_CHEF = _CAKE_MASTER_CHEF;
        cake = _cake;
        bpToken = _bp;
        startBlock = _startBlock;
        poolInfo.lastRewardBlock = _startBlock;
        _safeApprove(address(_cake), address(_CAKE_MASTER_CHEF), uint(~0));
        cakeRecipientAddress = _cakeRecipientAddress;
    }

    // View function to see pending BPs on frontend.
    function pendingReward(address _user) external view returns (uint256) {
        return pendingBPByBlockNumber(_user, block.number);
    }

    function pendingBPByBlockNumber(address _user, uint256 currentBlock) public view returns (uint256){
        PoolInfo memory pool = poolInfo;
        UserInfo memory user = userInfo[_user];
        uint256 accBPPerShare = pool.accBPPerShare;
        if (currentBlock > pool.lastRewardBlock && pool.supply != 0) {
            uint256 BPReward = getBPRewardByBlockNumber(pool.lastRewardBlock, currentBlock);
            //calculate acc BP rate per lp
            accBPPerShare = accBPPerShare.add(BPReward.mul(1e12).div(pool.supply));
        }
    ...
    function updatePool() public {
        ...
        if (poolInfo.supply == 0) {
            poolInfo.lastRewardBlock = block.number;
            return;
        }

        //calculate lastRewardBlock to this block's reward
        uint256 BPReward = getBPReward(poolInfo.lastRewardBlock);
        if (BPReward <= 0) {
            return;
        }
        ...
        poolInfo.accBPPerShare = poolInfo.accBPPerShare.add(BPReward.mul(1e12).div(poolInfo.supply));
        poolInfo.lastRewardBlock = block.number;
    }

    // Deposit cake to BunnyMasterChef for BP allocation.
    // user approve cake first
    function deposit(uint256 _amount) public lock {
        UserInfo storage user = userInfo[msg.sender];
        updatePool();
        uint256 pending = 0;
        if (user.amount > 0) {
            pending = user.amount.mul(poolInfo.accBPPerShare).div(1e12).sub(user.rewardDebt);
            if (pending > 0) {
        ...
        user.rewardDebt = user.amount.mul(poolInfo.accBPPerShare).div(1e12);
        CAKE_MASTER_CHEF.enterStaking(_amount);

        poolInfo.supply = poolInfo.supply.add(_amount);
        emit Deposit(msg.sender, _amount, pending);
    }

    // Withdraw cake from MasterChef.only havest BP if input _amount is 0
    function withdraw(uint256 _amount) public lock{
        UserInfo storage user = userInfo[msg.sender];
        require(user.amount >= _amount, "BunnyMasterChef: amount not right");
        updatePool();
        uint256 pending = user.amount.mul(poolInfo.accBPPerShare).div(1e12).sub(user.rewardDebt);
        if (pending > 0) {
            safeBPTransfer(msg.sender, pending);
        }
        if (_amount > 0) {
        ...
        poolInfo.supply = poolInfo.supply.sub(_amount);
        user.rewardDebt = user.amount.mul(poolInfo.accBPPerShare).div(1e12);
    ...
    function balanceOf() public view returns (uint256){
        return poolInfo.supply;
    }

    //harvest CAKE 
    function harvestAllCake() public {
        CAKE_MASTER_CHEF.leaveStaking(0);
        cake.transfer(cakeRecipientAddress, cake.balanceOf(address(this)));
    }
    ...
    function getBPRewardByBlockNumber(uint256 _lastRewardBlock, uint256 currentBlock) public view returns (uint256) {
        require(_lastRewardBlock <= currentBlock, "BunnyMasterChef: must less than the current block number");
    ...
    function _safeApprove(address token, address to, uint value) internal {
        ...
        require(success && (data.length == 0 || abi.decode(data, (bool))), 'BunnyMasterChef: APPROVE_FAILED');
    }
