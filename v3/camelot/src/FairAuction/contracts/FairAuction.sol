...
contract FairAuction is Ownable, ReentrancyGuard {
  ...
  constructor(IERC20 projectToken, IERC20 saleToken, IERC20 lpToken, uint256 startTime, uint256 endTime, address treasury_, uint256 maxToDistribute, uint256 minToRaise, uint256 maxToRaise, uint256 capPerWallet) {
...
//  }
  ...
  modifier isSaleActive() {
    require(hasStarted() && !hasEnded() && PROJECT_TOKEN.balanceOf(address(this)) >= MAX_PROJECT_TOKENS_TO_DISTRIBUTE, "isActive: sale is not active");
    _;
  }
  ...
  modifier isClaimable(){
    ...
    require(LP_TOKEN.totalSupply() > 0 || forceClaimable, "isClaimable: no LP tokens");
    _;
  }
 ...
 * @dev Purchase an allocation for the sale for a value of "amount" SALE_TOKEN, referred by "referralAddress"
  ...
  function burnUnsoldTokens() external onlyOwner {
    ...
    PROJECT_TOKEN.transfer(0x000000000000000000000000000000000000dEaD, MAX_PROJECT_TOKENS_TO_DISTRIBUTE.sub(totalSold));
  }
