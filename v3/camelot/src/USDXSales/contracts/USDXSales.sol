...
contract USDXSales is Ownable2Step, AccessControl, Pausable, ReentrancyGuard, IUSDXSales {
  ...
  /// @notice Maximum amount of USDX that can be minted per block.
  /// @dev This value is used to limit minting within a single block for security and rate limiting.
  uint256 public maxMintPerBlock;

  /// @notice Tracks the total amount of USDX minted in each block.
  /// @dev Mapping key is the block number, and value is the amount of USDX minted in that block.
  ...
  constructor(IUSDX _usdx, address[] memory _assets, address[] memory _custodians, address _owner, uint256 _maxMintPerBlock) {
  ...
  /// @inheritdoc IUSDXSales
  function setMaxMintPerBlock(uint256 _maxMintPerBlock) external override onlyOwner {
    _setMaxMintPerBlock(_maxMintPerBlock);
  }
  ...
  function _buy(address _collateralAsset, uint256 _collateralAmount, address _custodianAddress) internal {
    ...
    // Check and update minted amount for the current block
    uint256 currentBlock = block.number;
    uint256 newMintedAmount = mintedPerBlock[currentBlock] + _usdxAmount;
    require(newMintedAmount <= maxMintPerBlock, Errors.MAX_MINT_PER_BLOCK_EXCEEDED);

    mintedPerBlock[currentBlock] = newMintedAmount;
  ...
  function _setMaxMintPerBlock(uint256 _maxMintPerBlock) internal {
    emit MaxMintPerBlockUpdated(maxMintPerBlock, _maxMintPerBlock);
