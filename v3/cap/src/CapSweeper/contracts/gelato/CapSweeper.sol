...
contract CapSweeper is ICapSweeper, Access, CapSweeperStorageUtils, UUPSUpgradeable {
    ...
    function initialize(address _accessControl, address _cusd, uint256 _sweepInterval, uint256 _minSweepAmount) external initializer {
    ...
    function checker() external view returns (bool canExec, bytes memory execPayload) {
        ...
        for (uint256 i; i < assets.length; ++i) {
            ...
            uint256 assetBal = IERC20Metadata(assets[i]).balanceOf($.cusd) * 1e18 / (10**IERC20Metadata(assets[i]).decimals());
