...
contract StakingAnyToken is Ownable, Pausable {
    ...
    // the penalty in deposit, a number between 0 and 10000
    uint16 public penalty;

    // the penalty in interest, a number between 0 and 100
    uint8 public penaltyInInterest;

    // the fee in deposit on unstake, a number between 0 and 10000
    ...
    struct Stake {
        ...
        uint256 penalty;
        // penalty in interest charged, if any
        uint256 penaltyInInterest;
    ...
    struct StakeView {
        ...
        // the version on closing
        uint32 finishedVersion;
        // interest at minimum stake time, only available before minimum stake time
        uint256 interestAtMinStakeTime;
        // interest realtime
        uint256 interestRealTime;
        // penalty realtime
        uint256 penaltyRealTime;
        // penalty in interest realtime
        uint256 penaltyInInterestRealTime;
        // decimals of apy
        int8 apyDecimals;
    ...
    struct StakeTokenInfo {
        ...
        // penalties in interest are collected and stored by the contract
        uint256 collectedPenaltyInInterest;
    ...
    event Withdraw(address indexed user, uint32 indexed index, uint64 interestSeconds, uint256 interest, uint256 fee);
    event WithdrawInterest(address indexed user, uint32 indexed index, uint64 interestSeconds, uint256 interest);
    event EmergencyWithdraw(address indexed user, uint32 indexed index, uint64 interestSeconds, uint256 interest, uint256 penalty, uint256 penaltyInInterest, uint256 fee);
    event WithdrawWithoutReward(address indexed user, uint32 indexed index, uint256 fee);
    ...
    constructor(uint256 _minimumStakeTimeSeconds, uint16 _penalty, uint8 _penaltyInInterest, uint16 _unstakeFee) Ownable() {
        require(_penalty<=10000, "Penalty must be less than 10000");
        require(_penaltyInInterest<=100, "Penalty in interest must be less than 100");
        require(_unstakeFee<=10000, "Unstake fee must be less than 10000");
        minimumStakeTime = _minimumStakeTimeSeconds;
        penalty = _penalty;
        penaltyInInterest = _penaltyInInterest;
    ...
    function modifyPenalty(uint16 _newVal) external onlyOwner {
        penalty = _newVal;
    }
    function modifyPenaltyInInterest(uint8 _newVal) external onlyOwner {
        penaltyInInterest = _newVal;
    }
    function modifyUnstakeFee(uint16 _newVal) external onlyOwner {
        unstakeFee = _newVal;
    }
    ...
    function queryUserStakeSummary(address _user, address _stakeToken, address _rewardToken) public view returns (uint256 historyTotal, uint256 validWithObsoleted, uint256 validTotal) {
        Stake[] storage userStakes = stakesOfOwner[_user];
        for (uint256 i = 0; i < userStakes.length; i++) {
            Stake storage stake = userStakes[i];
            if (stake.stakeToken != _stakeToken || stake.rewardToken != _rewardToken) {
                continue;
            }
    ...
    function queryUserStakeByIndex(address _user, uint32 _inx) external view returns (StakeView memory) {
        Stake[] storage userStakes = stakesOfOwner[_user];
        require(_inx < userStakes.length, "Index is out of bounds");
        return queryUserStakeByStake(userStakes[_inx], _inx);
    }

    function queryUserStakeByStake(Stake storage _stake, uint32 _inx) private view returns (StakeView memory) {
        uint256 interestRealTime = 0;
        uint256 penaltyRealTime = 0;
        uint256 penaltyInInterestRealTime = 0;
        uint256 interestAtMinStakeTime = 0;
        uint32 apy;
        int8 apyDecimals; 
        if (_stake.finishedTimestamp != 0) {
            (,,,,, apy, apyDecimals) = calculateInterestByStake(_stake, false, false);
        } else {
            (interestRealTime, penaltyRealTime, penaltyInInterestRealTime,, interestAtMinStakeTime, apy, apyDecimals) = calculateInterestByStake(_stake, block.timestamp - _stake.startTimestamp < minimumStakeTime, true);
        }

        return StakeView({
            index: _inx,
            version: _stake.version,
            stakeToken: _stake.stakeToken,
            rewardToken: _stake.rewardToken,
            startTimestamp: _stake.startTimestamp,
            amount: _stake.amount,
            interest: _stake.interest,
            penalty: _stake.penalty,
            penaltyInInterest: _stake.penaltyInInterest,
            lastWithdrawalTimestamp: _stake.lastWithdrawalTimestamp,
            finishedTimestamp: _stake.finishedTimestamp,
            finishedVersion: _stake.finishedVersion,
            interestAtMinStakeTime: interestAtMinStakeTime,
            interestRealTime: interestRealTime,
            penaltyRealTime: penaltyRealTime,
            penaltyInInterestRealTime: penaltyInInterestRealTime,
            apy: apy,
    ...
    function queryUserStake(address _user, bool _skipFinished) public view returns (StakeView[] memory) {
        ...
        for (uint32 i = 0; retStakeInx < ret.length ; i++) {
            ...
            ret[retStakeInx] = queryUserStakeByStake(stake, i);
            retStakeInx += 1;
        }
        return ret;
    }
    ...
    function withdrawStakeWithoutReward(uint32 _arrayIndex) external whenWithdrawNotLocked {
        ...
        uint256 theFee = stake.amount * unstakeFee / 10000;

        if (theFee > 0) {
            // transfer the fee amount to the contract owner
            IERC20(stake.stakeToken).transfer(owner(), theFee);
        }

        // transfer the amount from the contract itself
        IERC20(stake.stakeToken).transfer(msg.sender, stake.amount);

        emit WithdrawWithoutReward(msg.sender, _arrayIndex, theFee);
    }

    
    function calculateInterestByStake(Stake storage _stake, bool _processPenalty, bool _calcInterestAtMinStakeTime) private view returns (
                uint256 retInterest,
                uint256 retPenalty,
                uint256 retPenaltyInInterest,
                uint64 retInterestSeconds,
                uint256 retInterestAtMinStakeTime,
                uint32 apy,
                int8 apyDecimals) {
        apy = 0;
        ...
        if (_stake.finishedTimestamp == 0) {
            ...
            // calculate penalty = stake amount * penalty / 10000
            // calculate penaltyInInterest = interest * penaltyInInterest / 100
            if (_processPenalty) {
                retPenaltyInInterest = retInterest * penaltyInInterest / 100;
                retPenalty = _stake.amount * penalty / 10000;
                retInterest -= retPenaltyInInterest;
                if (_calcInterestAtMinStakeTime) {
                    retInterestAtMinStakeTime = interestPerYear * minimumStakeTime / 31536000;
                }
            }
        }
    }
    ...
    function withdrawStake(uint32 _arrayIndex) external whenWithdrawNotLocked {
        ...
        // get the interest
        (uint256 interest,,, uint64 interestSeconds,,,) = calculateInterestByStake(stake, false, false);
        ...
        IERC20(stake.stakeToken).transfer(msg.sender, stake.amount - theFee);

        // transfer the amount from the contract itself
        emit Withdraw(msg.sender, _arrayIndex, interestSeconds, interest, theFee);
    }
    ...
    function emergencyWithdrawStake(uint32 _arrayIndex) external whenWithdrawNotLocked {
        ...
        // get the interest
        (uint256 interest, uint256 thePenalty, uint256 thePenaltyInInterest, uint64 interestSeconds,,,) = calculateInterestByStake(stake, true, false);
        ...
        stake.penaltyInInterest += thePenaltyInInterest;
        ...
        stakeTokenInfo.collectedPenaltyInInterest += thePenaltyInInterest;
        ...
        if (thePenalty + theFee > 0) {
            // transfer the penalty amount and the fee amount to the contract owner
            IERC20(stake.stakeToken).transfer(owner(), thePenalty + theFee);
        }

        // transfer remaining
        IERC20(stake.stakeToken).transfer(msg.sender, stake.amount - thePenalty - theFee);
        
        emit EmergencyWithdraw(msg.sender, _arrayIndex, interestSeconds, interest, thePenalty, thePenaltyInInterest, theFee);
    }
}