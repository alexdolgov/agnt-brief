// SPDX-License-Identifier: GPL-3.0
/*                            ******@@@@@@@@@**@*                               
                        ***@@@@@@@@@@@@@@@@@@@@@@**                             
                     *@@@@@@**@@@@@@@@@@@@@@@@@*@@@*                            
                  *@@@@@@@@@@@@@@@@@@@*@@@@@@@@@@@*@**                          
                 *@@@@@@@@@@@@@@@@@@*@@@@@@@@@@@@@@@@@*                         
                **@@@@@@@@@@@@@@@@@*@@@@@@@@@@@@@@@@@@@**                       
                **@@@@@@@@@@@@@@@*@@@@@@@@@@@@@@@@@@@@@@@*                      
                **@@@@@@@@@@@@@@@@*************************                    
                **@@@@@@@@***********************************                   
                 *@@@***********************&@@@@@@@@@@@@@@@****,    ******@@@@*
           *********************@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@************* 
      ***@@@@@@@@@@@@@@@*****@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@****@@*********      
   **@@@@@**********************@@@@*****************#@@@@**********            
  *@@******************************************************                     
 *@************************************                                         
 @*******************************                                               
 *@*************************                                                    
   ********************* 
   
    /$$$$$                                               /$$$$$$$   /$$$$$$   /$$$$$$ 
   |__  $$                                              | $$__  $$ /$$__  $$ /$$__  $$
      | $$  /$$$$$$  /$$$$$$$   /$$$$$$   /$$$$$$$      | $$  \ $$| $$  \ $$| $$  \ $$
      | $$ /$$__  $$| $$__  $$ /$$__  $$ /$$_____/      | $$  | $$| $$$$$$$$| $$  | $$
 /$$  | $$| $$  \ $$| $$  \ $$| $$$$$$$$|  $$$$$$       | $$  | $$| $$__  $$| $$  | $$
| $$  | $$| $$  | $$| $$  | $$| $$_____/ \____  $$      | $$  | $$| $$  | $$| $$  | $$
|  $$$$$$/|  $$$$$$/| $$  | $$|  $$$$$$$ /$$$$$$$/      | $$$$$$$/| $$  | $$|  $$$$$$/
 \______/  \______/ |__/  |__/ \_______/|_______/       |_______/ |__/  |__/ \______/                                      
*/
pragma solidity ^0.8.2;

import {MerkleProof} from "@openzeppelin/contracts/utils/cryptography/MerkleProof.sol";
import {Ownable} from "@openzeppelin/contracts/access/Ownable.sol";
import {ReentrancyGuard} from "@openzeppelin/contracts/security/ReentrancyGuard.sol";

/// @title JonesGuardedLaunch
/// @notice ETH claimable by Jones DAO Guarded Launch participants
/// @author Jones DAO
contract JonesGuardedLaunch is Ownable, ReentrancyGuard {
    // Merkle Tree Root
    bytes32 public immutable merkleRoot;

    // Mapping of addresses who have claimed their ETH
    mapping(address => bool) public hasClaimed;

    /// Thrown if address has already claimed
    error AlreadyClaimed();
    /// Thrown if address/amount are not part of Merkle tree
    error NotInMerkle();

    /// @param _merkleRoot of claimees
    constructor(bytes32 _merkleRoot) {
        merkleRoot = _merkleRoot; // Update root
    }

    /// Emitted after a successful claim
    /// @param to recipient of claim
    /// @param amount of eth claimed
    event Claim(address indexed to, uint256 amount);

    /// Allows claiming guarded launch deposits if address is part of merkle tree
    /// @param to address of claimee
    /// @param amount of eth owed to claimee
    /// @param proof merkle proof to prove address and amount are in tree
    function claim(
        address to,
        uint256 amount,
        bytes32[] calldata proof
    ) external nonReentrant {
        // Throw if address has already claimed guarded launch
        if (hasClaimed[to]) revert AlreadyClaimed();

        // Verify merkle proof, or revert if not in tree
        bytes32 leaf = keccak256(abi.encodePacked(to, amount));
        bool isValidLeaf = MerkleProof.verify(proof, merkleRoot, leaf);
        if (!isValidLeaf) revert NotInMerkle();

        // Set address to claimed
        hasClaimed[to] = true;

        // Send ETH to address
        payable(to).transfer(amount);

        // Emit claim event
        emit Claim(to, amount);
    }

    // workaround for receiving ether
    receive() external payable {
        require(msg.value >= 0);
    }

    // Multisig can withdraw funds if something goes wrong
    function emergencyWithdraw() external onlyOwner {
        uint256 ethBalance = payable(address(this)).balance;
        payable(msg.sender).transfer(ethBalance);
    }
}
