...
contract Vault {
    ...
    error Vault__notEnoughDebt();
    error Vault__invalidMaxAmount();

    uint256 internal constant ONE = 1e18;
    uint256 internal constant LIQUIDATION_BONUS = 1000;
    uint256 internal constant REDEEM_FEE = 100; // 1%, 0.99 USD redeem value
    ...
    function _repay(uint256 pos, uint256 amount) internal {
    ...
    function claimFees(uint256 pos) external {
        ...
        if(positionManager.ownerOf(pos) != address(this)){
            revert Vault__invalidPositionAccess();
        }
    ...
    function redeemForFees(uint256 pos, uint256 maxAmount) external {
        if(positionManager.ownerOf(pos) != address(this)){
            revert Vault__invalidPositionAccess();
        }
        ...
        (uint256 amount0, uint256 amount1) = positionManager.collect(collect);

        (address token0, address token1,) = getPositionInfo(pos);
        IOracle oracle = oracle();
        uint256 priceA = oracle.price(token0);
        uint256 priceB = oracle.price(token1);
        ...
        uint256 totalValue = 0;
        {
            ...
            uint256 priceA = oracle.price(token0);
            uint256 priceB = oracle.price(token1);
            totalValue = (amount0*priceA/10**token0Info.decimals) + (amount1*priceB/10**token1Info.decimals);
        }

        uint256 redeemValue = Math.mulDiv(totalValue, BASIS_POINT + REDEEM_FEE, BASIS_POINT);

        if(redeemValue > maxAmount){
            revert Vault__invalidMaxAmount();
        }

        _accrueInterest(pos);
        (uint256 totalDebt,) = _getDebtAndInterest(pos);

        // This can lead to user paying less than redeemValue
        if(redeemValue > totalDebt){
            revert Vault__notEnoughDebt();
        }

        _repay(pos, redeemValue);
    ...
    function _getPrices(address tokenA, address tokenB, uint8 decimalsA, uint8 decimalsB) internal view returns (uint256 priceA, uint256 priceB) {
        IOracle oracle = oracle();
        uint256 _priceA = oracle.price(tokenA);
        uint256 _priceB = oracle.price(tokenB);
