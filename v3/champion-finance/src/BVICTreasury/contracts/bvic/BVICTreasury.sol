...
contract BVICTreasury is ContractGuard {
    ...
    function initialize(
        ...
        mainTokenPriceOne = 10**STABLE_DECIMALS; // This is to allow a PEG of 1 MainToken per WBTC.e
        mainTokenPriceRebase = 8*10**(STABLE_DECIMALS - 1); // 0.8 WBTC.e
    ...
    function setMainTokenPriceRebase(uint256 _value) external onlyOperator {
        uint256 maxMainTokenPriceRebase = 8*10**(STABLE_DECIMALS - 1); // 0.8 WBTC.e
        uint256 minMainTokenPriceRebase = 6*10**(STABLE_DECIMALS - 1); // 0.6 WBTC.e
