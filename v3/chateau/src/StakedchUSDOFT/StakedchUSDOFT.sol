...
pragma solidity =0.8.20 >=0.8.0 ^0.8.0 ^0.8.1 ^0.8.20;
...
// contracts/interfaces/ICHTDefinitions.sol

interface ICHTDefinitions {
    /// @notice Event emitted when a new CHT token is minted (inflation) by the owner
    event Mint(address indexed to, uint256 amount);

    /// @notice This error is returned if the zero address is used
    error ZeroAddressException();
    /// @notice This error is returned if mint is called within MINT_WAIT_PERIOD of last mint
    ...
    /// @notice This error is returned if minting is attempted on a non-designated chain
    error MintingNotAllowedOnThisChain();
} 
...
// lib/devtools/packages/oft-evm/contracts/OFT.sol
...
// contracts/OFT/OFTOwnable2Step.sol
...
abstract contract OFTOwnable2Step is OFT {
    ...
    // @dev custom error for unauthorized account
    error OwnableUnauthorizedAccount(address account);

    event OwnershipTransferStarted(address indexed previousOwner, address indexed newOwner);

    // @dev custom error for no renounce ownership
    // error CantRenounceOwnership(); // Commented out - already declared in ICHTDefinitions.sol
    ...
    constructor(
        ...
        // Transfer ownership from CREATE2 factory to delegate
    ...
    function renounceOwnership() public virtual view override onlyOwner {
        revert ICHTDefinitions.CantRenounceOwnership();
    }
}

// contracts/OFT/chUSDOFT.sol

/**
 * @title chUSDOFT
 */
contract chUSDOFT is OFTOwnable2Step, RateLimiter {
    ...
    /**
     * @dev Constructor to initialize the chUSDOFT contract.
...
// contracts/OFT/StakedchUSDOFT.sol

/**
 * @title StakedchUSDOFT
 */
contract StakedchUSDOFT is chUSDOFT {
    ...
    /**
     * @dev Constructor to initialize the StakedchUSDOFT contract.
     * @param _rateLimitConfigs An array of RateLimitConfig structures defining the rate limits.
     * @param _name The name of the token.
     * @param _symbol The symbol of the token.
     * @param _lzEndpoint Address of the LZ endpoint.
    ...
    constructor(
        ...
        address _lzEndpoint,
        address _delegate
    ) chUSDOFT(_rateLimitConfigs, _name, _symbol, _lzEndpoint, _delegate) {}
