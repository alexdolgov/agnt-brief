...
import "./interfaces/IStakedchUSD.sol";

/**
 * @title StakedchUSD
 * @notice The StakedchUSD contract allows users to stake chUSD tokens and earn a portion of protocol LST and perpetual yield that is allocated
 * to stakers by the Chateau DAO governance voted yield distribution algorithm.  The algorithm seeks to balance the stability of the protocol by funding
 * the protocol's insurance fund, DAO activities, and rewarding stakers with a portion of the protocol's yield.
 */
contract StakedchUSD is SingleAdminAccessControl, ReentrancyGuard, ERC20Permit, ERC4626, IStakedchUSD {
    ...
    uint256 private constant VESTING_PERIOD = 5 days;
    /// @notice Minimum non-zero shares amount to prevent donation attack
    uint256 private constant MIN_SHARES = 1 ether;
    /// @notice The maximum amount of assets that can be deposited into the contract
    uint256 private _maxDeposit = type(uint256).max; // todo: add setter
    /// @notice The maximum amount of shares that can be minted into the contract
    uint256 private _maxMint = type(uint256).max; // todo: add setter
    ...
    /**
         * @notice Constructor for StakedchUSD contract.
    * @param _asset The address of the chUSD token.
    ...
    /**
     * @notice Allows the owner (DEFAULT_ADMIN_ROLE) to set the maximum amount of assets that can be deposited into the contract.
     * @param newMaxDeposit The maximum amount of assets that can be deposited into the contract.
     */
    function setMaxDeposit(uint256 newMaxDeposit) external onlyRole(DEFAULT_ADMIN_ROLE) {
        if (newMaxDeposit == 0) revert InvalidAmount();
        _maxDeposit = newMaxDeposit;
    }

    /**
     * @notice Allows the owner (DEFAULT_ADMIN_ROLE) to set the maximum amount of shares that can be minted into the contract.
     * @param newMaxMint The maximum amount of shares that can be minted into the contract.
     */
    function setMaxMint(uint256 newMaxMint) external onlyRole(DEFAULT_ADMIN_ROLE) {
        if (newMaxMint == 0) revert InvalidAmount();
        _maxMint = newMaxMint;
    }
     ...
     * Note that the owner can retrieve chUSD tokens in the case that the NAV is reduced 
     * and therefore the schUSD backing is reduced.
     * @param token The token to be retrieved.
     * @param amount The amount of tokens to be retrieved.
     * @param to Where to send retrieved tokens
     */
    function retrieveTokens(address token, uint256 amount, address to)
        external
        nonReentrant
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        // add a constant percentage cap to the drawdown in the case of DEFAULT_ADMIN_ROLE compromise
        if (amount > totalAssets()) revert InvalidAmount();
    ...
    /**
     * @notice Returns the amount of chUSD tokens that are vested in the contract.
     */
    ...
    /**
     * @notice Returns the amount of chUSD tokens that are unvested in the contract.
     */
