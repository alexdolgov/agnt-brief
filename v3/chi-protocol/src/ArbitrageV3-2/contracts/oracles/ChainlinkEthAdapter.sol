...
contract ChainlinkEthAdapter is IChainlinkEthAdapter {
  ...
  function peek() external view returns (uint256 price) {
    ...
    uint256 ethOracleDecimals = AggregatorV3Interface(ExternalContractAddresses.ETH_USD_CHAINLINK_FEED).decimals();
    uint256 priceInUsd = Math.mulDiv(uint256(priceInEth), uint256(ethPriceInUsd), 10 ** (uint256(ethOracleDecimals)));
    return priceInUsd;
  }
