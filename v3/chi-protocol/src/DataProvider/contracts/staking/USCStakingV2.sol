...
contract USCStakingV2 is IUSCStaking, OwnableUpgradeable, StakingWithEpochs {
  ...
  function initializeUscRewardBoots() external reinitializer(4) {
    EpochData storage epoch = epochs[currentEpoch - 1];
    epoch.cumulativeRewardsPerShare[RewardToken.USC] += Math.mulDiv(3250 ether, 1e18, epoch.shares);
  }

  function migrateToV2(uint256 amount, address recipient, address stUscAddress) external {
    unstake(amount, address(this));
    stakeToken.approve(stUscAddress, amount);
    ISTUSC(stUscAddress).stake(amount, recipient);
  }
