...
/**
 * @title StrategyDefault contract
 * @author Naturelab
 */
contract StrategyDefault is IStrategy, OwnableUpgradeable {
    ...
    // The address of multisig contract used to manage assets.
    address public safeWallet;

    // Records the total net assets of the current strategy.
    ...
    function updateSafeWallet(address _newSafeWallet) external onlyOwner {
        if (_newSafeWallet.code.length == 0 || _newSafeWallet == address(0)) revert Errors.UnSupportedOperation();
        emit UpdateSafeWallet(safeWallet, _newSafeWallet);
        safeWallet = _newSafeWallet;
    }
    ...
    function transferToSafeWallet(address _token, uint256 _amount) external onlyRebalancer {
        if (safeWallet == address(0)) revert Errors.UnSupportedOperation();
        IERC20(_token).safeTransfer(safeWallet, _amount);

        emit TransferToSafeWallet(_token, _amount);
    }
