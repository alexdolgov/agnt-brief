...
contract RedeemOperator is IRedeemOperator, Constants, Ownable {
    ...
    function confirmWithdrawal(address[] memory _users, uint256 _totalGasTokenAmount) external onlyOperator {
        ...
        for (uint256 i = 0; i < _users.length; ++i) {
            if (!_pendingWithdrawers.contains(_users[i])) revert CustomErrors.InvalidWithdrawalUser();
        ...
        if (lastExchangePrice == 0) revert CustomErrors.UnSupportedOperation();
    ...
    function refundWithdraw(address[] memory _users) external onlyOperator {
        for (uint256 i = 0; i < _users.length; ++i) {
            if (!_pendingWithdrawers.contains(_users[i])) revert CustomErrors.InvalidWithdrawalUser();
            IERC20(vault).safeTransfer(_users[i], _withdrawalRequest[_users[i]]);
            _pendingWithdrawers.remove(_users[i]);
            delete _withdrawalRequest[_users[i]];
        }
    }
