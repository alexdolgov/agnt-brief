...
contract YieldTokenAggregator is OwnableUpgradeable, UUPSUpgradeable {
        ...
        returns (uint80 roundId)
    {
        ...
        // query the fee percentage
        uint256 feePct = ytoken.managementFee();

        // calculated as principle plus interest (less fee)
        uint256 balance = _principal + FixedPointMathLib.mulDivDown(_interest, (HUNDRED_PCT - feePct), HUNDRED_PCT);
        ...
        if (_interest > 0 && feePct > 0) ytoken.processFees(_interest, answer);
