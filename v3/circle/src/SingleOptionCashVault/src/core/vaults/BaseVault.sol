...
contract BaseVault is ERC1155TokenReceiver, OwnableUpgradeable, ReentrancyGuardUpgradeable, UUPSUpgradeable {
    ...
    function closeRound() external nonReentrant {
        ...
        // if nextRound is uninitialized, price per share will be 0, in this case initialize it on the fly
        if (pricePerShare[nextRound] < PLACEHOLDER_UINT) _initRounds(1, nextRound);
    ...
    function initRounds(uint256 _numRounds, uint32 _startFromRound) external {
        _initRounds(_numRounds, _startFromRound);
    }
     ...
     * @param _startFromRound is the round number from which to start initializing the maps
     */
    function _initRounds(uint256 _numRounds, uint32 _startFromRound) internal {
        unchecked {
            uint256 i;
