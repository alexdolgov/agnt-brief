...
contract PoolFactory is OwnableUpgradeable {
  ...
  ///@notice Event emitted when new cpool contract address is set
  ///@param newCpoolAddress Address of the new Cpool contract
  event CpoolAddressChanged(address newCpoolAddress);
  ...
  /// @notice Function is used to change the CPOOL address
  /// @param _cpool The address of the new CPOOL contract
  ///@dev Only called by the owner
  function changeCpoolAddress(address _cpool) external onlyOwner notZeroAddress(_cpool) {
    require(address(cpool) != _cpool, 'AIS');
    cpool = IERC20Upgradeable(_cpool);
     emit CpoolAddressChanged(_cpool);
  }
