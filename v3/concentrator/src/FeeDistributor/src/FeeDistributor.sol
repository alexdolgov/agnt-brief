...
contract FeeDistributor is Ownable2Step, ReentrancyGuard {
    ...
    function claim() external nonReentrant whenNotKilled returns (uint256 amount) {
        ...
        amount = _claim(msg.sender, msg.sender, 0);
    }
        ...
        returns (uint256 amount)
    {
        ...
        amount = _claim(user, receiver, 0);
    }

    /**
     * @notice Claim rewards starting from a specific epoch, skipping earlier ones
     * @dev Power-user entrypoint. Skipped epochs are permanently forfeited.
     *      Use this only if you know the skipped range contains no rewards for you.
     *      Reverts if startEpoch is before the user's natural start epoch.
     * @param startEpoch Epoch timestamp to start claiming from
     * @return amount Total tokens claimed
     */
    function claimWithPermanentEpochSkip(uint256 startEpoch)
        external
        nonReentrant
        whenNotKilled
        returns (uint256 amount)
    {
        require(msg.sender != address(0), ZeroAddress());
        amount = _claim(msg.sender, msg.sender, startEpoch);
    }
     ...
     * @return amount Total tokens claimed
     */
    function claimWithPermanentEpochSkip(address user, address receiver, uint256 startEpoch)
        external
        onlyOperator
        nonReentrant
        whenNotKilled
        returns (uint256 amount)
    {
        require(user != address(0), ZeroAddress());
        require(receiver != address(0), ZeroAddress());
        amount = _claim(user, receiver, startEpoch);
    }
     ...
     * @param user Address to claim for
     * @param receiver Address to receive the tokens
     * @param startEpoch_ Epoch to start iterating from (0 = use default from checkpoints)
     * @return amount Tokens claimed
     */
    function _claim(address user, address receiver, uint256 startEpoch_) internal returns (uint256 amount) {
        if (startEpoch_ != 0) {
            require(startEpoch_ == _roundDownToEpoch(startEpoch_), StartEpochNotAligned());
            require(startEpoch_ >= _getStartEpoch(user), StartEpochTooEarly());
        }
        ...
        (amount, lastProcessedEpoch) = _claimable(user, startEpoch_);
     ...
     *      Pass startEpoch to skip known-empty epochs (e.g., from dust-staking griefing).
     *      Use lastProcessedEpoch to detect if more epochs remain to query.
     * @param user Address to check
     * @param startEpoch Epoch to start iterating from (0 = use default from checkpoints)
    ...
    {
        (amount, lastProcessedEpoch) = _claimable(user, startEpoch);
    }
     ...
     * @return lastProcessedEpoch The last epoch that was processed (0 if none)
     */
    function _claimable(address user, uint256 startEpoch_)
        internal
        view
        returns (uint256 amount, uint256 lastProcessedEpoch)
    {
        uint256 startEpoch = startEpoch_ == 0 ? _getStartEpoch(user) : startEpoch_;
    ...
    function _checkpointToken() internal {
        // No checkpoint activity before distribution starts
