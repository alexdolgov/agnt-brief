...
# https://github.com/curvefi/curve-dao-contracts/blob/master/contracts/gauges/GaugeLiquidityV4.vy
...
# reward token -> killed (cannot be re-added)
reward_killed: public(HashMap[address, bool])
...
def add_reward(_reward_token: address, _distributor: address):
    ...
    assert self.reward_killed[_reward_token] == False  # dev: reward token killed
...
@external
def recover_reward_token(_token: address) -> uint256:
    """
    @notice Kill a reward token distribution and recover remaining balance
    @dev Users with unclaimed rewards will have their claims revert until tokens are replenished.
         Token cannot be re-added as reward after recovery if integral was > 0.
    @param _token The reward token to recover
    @return uint256 The amount of tokens recovered
    """
    assert msg.sender == self.admin

    assert _token != self.SDT
    assert _token != self.staking_token

    if self.reward_data[_token].integral > 0:
        self.reward_killed[_token] = True

    self.reward_data[_token].distributor = empty(address)
    self.reward_data[_token].rate = 0
    self.reward_data[_token].period_finish = 0
    self.reward_data[_token].last_update = 0
    self.reward_data[_token].integral = 0
