...
contract OnlyBoostAllocator is Allocator {
    ...
    /// @param _boostProvider Address of the boost provider contract
    ...
    function getOptimalLockerBalance(address gauge) public view returns (uint256 balanceOfLocker) {
        // 1. Get the balance of veBoost on Stake DAO and the sidecar boost holder
        uint256 veBoostOfLocker = IBalanceProvider(BOOST_PROVIDER).balanceOf(LOCKER);
        uint256 veBoostOfSidecar = IBalanceProvider(BOOST_PROVIDER).balanceOf(SIDECAR_BOOST_HOLDER);

        // 2. Get the balance of the liquidity gauge held by the sidecar boost holder
        uint256 balanceOfSidecar = IBalanceProvider(gauge).balanceOf(SIDECAR_BOOST_HOLDER);

        // 3. If there is no balance or no veBoost on the sidecar, return 0
        if (balanceOfSidecar == 0 || veBoostOfSidecar == 0) return 0;
        ...
        balanceOfLocker = balanceOfSidecar.mulDiv(veBoostOfLocker, veBoostOfSidecar);
    }
