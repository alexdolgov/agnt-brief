...
contract OnlyBoostAllocator is Allocator {
    ...
    mapping(address => ManualWeights) public manualWeights;
    ...
    event ManualWeightsSet(address indexed gauge, uint256 lockerWeight);
        ...
        returns (Allocation memory alloc)
    {
        ...
        ManualWeights memory manualConfig = manualWeights[gauge];
        if (manualConfig.enabled) {
            alloc.amounts[1] = amount.mulDiv(manualConfig.lockerWeight, WEIGHT_SCALE);
        ...
        returns (Allocation memory alloc)
    {
        ...
        if (manualConfig.enabled) {
            ...
            uint256 desiredLocker = finalTotal.mulDiv(manualConfig.lockerWeight, WEIGHT_SCALE);
            ...
            // 6b.iv. Withdraw from locker up to its allowable delta.
            alloc.amounts[1] = Math.min(lockerWithdraw, remaining);
            remaining -= alloc.amounts[1];

            // 6b.v. Withdraw from sidecar up to its allowable delta.
            alloc.amounts[0] = Math.min(sidecarWithdraw, remaining);
            remaining -= alloc.amounts[0];

            // 6b.vi. If locker had limited capacity, exhaust remaining demand from the sidecar.
            if (remaining > 0) {
                uint256 extraSidecar =
                    balanceOfSidecar > alloc.amounts[0] ? Math.min(balanceOfSidecar - alloc.amounts[0], remaining) : 0;
                alloc.amounts[0] += extraSidecar;
                remaining -= extraSidecar;
            }

            // 6b.vii. If sidecar was also constrained, satisfy the remainder from the locker.
            if (remaining > 0) {
                uint256 extraLocker =
                    balanceOfLocker > alloc.amounts[1] ? Math.min(balanceOfLocker - alloc.amounts[1], remaining) : 0;
                alloc.amounts[1] += extraLocker;
            }
        ...
        returns (Allocation memory alloc)
    {
        ...
        if (lockerOnly[gauge]) {
            ...
            ManualWeights memory manualConfig = manualWeights[gauge];
            if (manualConfig.enabled) {
                if (manualConfig.lockerWeight > WEIGHT_SCALE) revert InvalidGaugeWeights();
                alloc.amounts[1] = totalBalance.mulDiv(manualConfig.lockerWeight, WEIGHT_SCALE);
    ...
    function _setGaugeWeights(address gauge, uint256 lockerWeight) private {
        ...
        // 2. Ensure the gauge has a sidecar, meaning manual splits are applicable.
        require(sidecar != address(0), InvalidGauge());

        // 3. Guard against invalid weights (greater than 100%).
        require(lockerWeight <= WEIGHT_SCALE, InvalidGaugeWeights());

        // 4. Persist configuration and emit the update event.
        manualWeights[gauge] = ManualWeights({lockerWeight: uint128(lockerWeight), enabled: true});

        emit ManualWeightsSet(gauge, lockerWeight);
    }
}
