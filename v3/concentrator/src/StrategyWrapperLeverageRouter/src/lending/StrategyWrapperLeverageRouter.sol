...
import {IMorpho, MarketParams, Authorization, Signature, Id, MarketParamsLib} from "@shared/src/interfaces/IMorpho.sol";
...
contract StrategyWrapperLeverageRouter is IMorphoFlashLoanCallback, ReentrancyGuardTransient {
    ...
    function leverage(
        ...
        _validateInputsNoAuth(wrapper, marketParams);
        _leverage(wrapper, marketParams, initialShares, flashLoanAmount, minSharesOut, borrowAmount, swapData);
    }

    /// @notice Leverage with signature authorization (for EOAs)
    /// @param wrapper The StrategyWrapper contract for this market
    /// @param marketParams The Morpho market parameters
    /// @param initialShares Amount of RewardVault shares to deposit initially (can be 0)
    /// @param flashLoanAmount Amount of loan token to flashloan for swapping
    ...
    /// @param borrowAmount Amount to borrow to repay flashloan
    /// @param swapData Calldata for Enso swap (loan token → RewardVault shares)
    /// @param authOn Authorization to enable this contract
    /// @param sigOn Signature for authOn
    /// @param authOff Authorization to disable this contract
    /// @param sigOff Signature for authOff
    function leverageWithSig(
        IStrategyWrapper wrapper,
        MarketParams calldata marketParams,
        uint256 initialShares,
        uint256 flashLoanAmount,
        ...
        uint256 borrowAmount,
        bytes calldata swapData,
        Authorization calldata authOn,
        Signature calldata sigOn,
        Authorization calldata authOff,
        Signature calldata sigOff
    ) external nonReentrant {
        _validateInputsWithAuth(wrapper, marketParams, authOn, authOff);

        // Set authorization via signature
        MORPHO.setAuthorizationWithSig(authOn, sigOn);

        _leverage(wrapper, marketParams, initialShares, flashLoanAmount, minSharesOut, borrowAmount, swapData);

        // Revoke authorization
        MORPHO.setAuthorizationWithSig(authOff, sigOff);
    }
    ...
    /// @notice Deleverage with signature authorization (for EOAs)
    /// @param wrapper The StrategyWrapper contract for this market
    /// @param marketParams The Morpho market parameters
    /// @param flashLoanAmount Amount of loan token to flashloan
    /// @param debtToRepay Amount of debt to repay on Morpho
    ...
    function _validateInputsNoAuth(IStrategyWrapper wrapper, MarketParams calldata marketParams) internal pure {
        require(address(wrapper) != address(0), ZeroAddress());
        require(marketParams.collateralToken == address(wrapper), InvalidWrapper());
    }

    function _validateInputsWithAuth(
        IStrategyWrapper wrapper,
        MarketParams calldata marketParams,
        Authorization calldata authOn,
        Authorization calldata authOff
        ...
        require(
            authOn.authorizer == msg.sender && authOn.authorized == address(this) && authOn.isAuthorized == true,
            InvalidAuthorization()
        );
        require(
            authOff.authorizer == msg.sender && authOff.authorized == address(this) && authOff.isAuthorized == false,
            InvalidAuthorization()
        );
    }
}
