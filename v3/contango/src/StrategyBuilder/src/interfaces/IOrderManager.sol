...
interface IOrderManager is IOrderManagerEvents, IOrderManagerErrors, IContangoErrors {
    ...
    function executeWithFeesAndTokenPrice(
        OrderId orderId,
        ExecutionParams calldata execParams,
        FeeParams memory feeParams,
        uint256 cashflowTokenPriceInNativeToken
    ) external payable returns (PositionId positionId_, Trade memory trade_, uint256 keeperReward_);
