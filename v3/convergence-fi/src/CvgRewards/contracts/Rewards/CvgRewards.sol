...
contract CvgRewards is Ownable2StepUpgradeable {
    ...
    function initialize(ICvgControlTower _cvgControlTower) external initializer {
        ...
        require(address(_gaugeController) != address(0), "GAUGE_CONTROLLER_ZERO");
        gaugeController = _gaugeController;

        uint256 firstCycleStart = (block.timestamp / 1 weeks) * 1 weeks;
        firstCycleStartTimestamp = firstCycleStart;
        lastUpdatedTimestamp = firstCycleStart;
        cvgRewardsConfig = CvgRewardsConfig({maxChunkCheckpoint: 50, maxChunkDistribute: 50});
    ...
    function _distributeCvgRewards() internal {
        ...
        (uint256[] memory gaugeWeights, uint256 totalWeight) = gaugeController.get_gauge_weights(addresses);
        for (uint256 i; i < gaugeWeights.length; ) {
            /// @dev compute the amount of CVG to distribute in the gauge
            cvgDistributed = totalWeight != 0 ? (stakingInflation * gaugeWeights[i]) / totalWeight : 0;
