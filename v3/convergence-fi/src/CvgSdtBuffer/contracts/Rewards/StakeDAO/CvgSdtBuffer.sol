...
contract CvgSdtBuffer is Ownable2StepUpgradeable {
    ...
    function pullRewards(address processor) external returns (ICommonStruct.TokenAmount[] memory) {
        ...
        address sdtRewardDistributor = cvgControlTower.sdtRewardDistributor();
        ...
        if (sdtAmount != 0) {
            ...
            ///@dev transfers all Sdt to the sdtRewardDistributor
            _sdt.transfer(sdtRewardDistributor, sdtAmount);
        }
        ...
        if (sdFrax3CrvAmount != 0) {
            ...
            _sdFrax3Crv.transferFrom(veSdtMultisig, sdtRewardDistributor, sdFrax3CrvAmount);
        }
        /// @dev else reduces the length of the array to not return some useless 0 TokenAmount structs
        else {
            // solhint-disable-next-line no-inline-assembly
        ...
        if (cvgSdtAmount != 0) {
            ...
            ///@dev transfers all CvgSdt to the sdtRewardDistributor
            _cvgSdt.transfer(sdtRewardDistributor, cvgSdtAmount);
        }
        /// @dev else reduces the length of the array to not return some useless 0 TokenAmount structs
        else {
            // solhint-disable-next-line no-inline-assembly
