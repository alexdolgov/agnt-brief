...
contract SdtStakingPositionService is Ownable2StepUpgradeable {
    ...
    ISdtRewardDistributor public sdtRewardDistributor;
    ...
    mapping(uint256 => NextClaimableCycles) private _nextClaims; // tokenId => lastCycleClaimed
  ...
  =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=--=-=-=-= */
    ...
    function initialize(
        ...
        /// @dev Else the stakingAsset is CvgSdt, we don't create the contract from the CloneFactory but in the classic way
            ...
            require(_cvgSdtBuffer != address(0), "CVGSDT_BUFFER_ZERO");
        ...
        ISdtRewardDistributor _sdtRewardDistributor = ISdtRewardDistributor(_cvgControlTower.sdtRewardDistributor());
        require(address(_sdtRewardDistributor) != address(0), "SDT_REWARD_RECEIVER_ZERO");
    ...
    function deposit(uint256 tokenId, uint256 amount, address operator) external {
        ...
        address receiver = msg.sender == cvgControlTower.sdtUtilities() ? operator : msg.sender;
        uint256 _tokenId;
     ...
     * @param _minCvgSdtAmountOut  Minimum amount of cvgSDT to receive in case of a pool exchange
     * @param _isConvert     If true, converts all SDT into CvgSDT.
     */
    function claimCvgSdtRewards(
        uint256 _tokenId,
        uint256 _minCvgSdtAmountOut,
        bool _isConvert
        ...
        sdtRewardDistributor.claimCvgSdtSimple(msg.sender, cvgClaimable, tokenAmounts, _minCvgSdtAmountOut, _isConvert);
    ...
    function claimCvgSdtMultiple(
        ...
        /// @dev Only the SdtRewardDistributor can claim this function.
        require(msg.sender == address(sdtRewardDistributor), "NOT_SDT_REWARD_RECEIVER");
    ...
    function _claimCvgSdtRewards(
        ...
        uint128 nextClaimableCvg = _nextClaims[tokenId].nextClaimableCvg;
        uint128 nextClaimableSdt = _nextClaims[tokenId].nextClaimableSdt;
        uint128 actualCycle = stakingCycle;
        uint256 lengthHistory = _stakingHistoryByToken[tokenId].length;

        /// @dev If never claimed on this token
        if (nextClaimableSdt == 0) {
            /// @dev Set the lastClaimed as the first action.
            nextClaimableSdt = uint128(_stakingHistoryByToken[tokenId][0]);
        }
        require(actualCycle > nextClaimableSdt, "ALL_SDT_CLAIMED_FOR_NOW");
        ...
        for (; nextClaimableSdt < actualCycle; ) {
            ...
            if (tokenStaked != 0) {
                ...
                if (_cycleInfo[nextClaimableSdt].isSdtProcessed) {
                    ...
                    newLastClaimSdt = nextClaimableSdt;
                }
            }
        ...
        require(_cvgClaimable != 0 || isSdtRewards, "NO_REWARDS_CLAIMABLE");

        /// @dev If last sdt cycle claimed is the last cycle, we can setup the next cycle to claim as the actual cycle
        if (newLastClaimSdt == actualCycle - 1) {
            _nextClaims[tokenId].nextClaimableSdt = actualCycle;
        }
        /// @dev Else, we don't know yet if the last cycle has been distributed, so we put the last cycle as the next Sdt Cycle to claim.
        else {
            _nextClaims[tokenId].nextClaimableSdt = actualCycle - 1;
        }
    ...
    function processSdtRewards() external {
        ...
        for (uint256 i; i < _rewardAssets.length; ) {
            ...
            _sdtRewardsByCycle[_cvgStakingCycle][erc20Id] = ICommonStruct.TokenAmount({
                token: _token,
                amount: _sdtRewardsByCycle[_cvgStakingCycle][erc20Id].amount + _rewardAssets[i].amount
            });
        ...
        for (; nextClaimableSdt < actualCycle; ) {
            ...
            if (tokenStaked != 0) {
                ...
                if (nextClaimableCvg <= nextClaimableSdt) {
                    /// @dev Computes the staking share of the Staking Position compared to the total Staked.
                    ///      By multiplying this share by the total CVG distributed for the cycle, we get the claimable amount.
                    uint256 cvgClaimableAmount = (tokenStaked * _cycleInfo[nextClaimableSdt].cvgRewardsAmount) /
                        totalStaked;
                    /// @dev increments the total amount in CVG to mint to the user
    ...
    function getClaimableCyclesAndAmounts(uint256 tokenId) external view returns (ClaimableCyclesAndAmounts[] memory) {
        ...
        ClaimableCyclesAndAmounts[] memory claimableCyclesAndAmounts = new ClaimableCyclesAndAmounts[](
            actualCycle - nextClaimableSdt
        );
        uint256 counter;
        uint256 maxLengthRewards = numberOfSdtRewards;
        for (; nextClaimableSdt < actualCycle; ) {
            uint256 amountStaked = _stakedAmountEligibleAtCycle(nextClaimableSdt, tokenId, lengthHistory);
            uint256 totalStaked = _cycleInfo[nextClaimableSdt].totalStaked;
            ...
            if (amountStaked != 0) {
                ...
                if (nextClaimableCvg <= nextClaimableSdt) {
                    /// @dev Computes the staking share of the Staking Position compared to the total Staked.
                    ///      By multiplying this share by the total CVG distributed for the cycle, we get the claimable amount.
                    cvgAmount = (amountStaked * _cycleInfo[nextClaimableSdt].cvgRewardsAmount) / totalStaked;
                }
                ...
                if (_cycleInfo[nextClaimableSdt].isSdtProcessed) {
                    ...
                    for (uint256 x; x < maxLengthRewards; ) {
                        ICommonStruct.TokenAmount memory rewardAsset = _sdtRewardsByCycle[nextClaimableSdt][x + 1];
                        if (rewardAsset.amount != 0) {
                ...
                claimableCyclesAndAmounts[counter++] = ClaimableCyclesAndAmounts({
                    cycleClaimable: nextClaimableSdt,
    ...
    /**
     *  @notice Get StakeDao rewards that have been processed for a cycleId.
