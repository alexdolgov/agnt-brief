...
contract SdtStakingViewer {
    ...
    function getGlobalViewCvgSdtStaking(
        ...
        address _stakingAsset = address(_stakingContract.stakingAsset());

        return
            ISdtStruct.CvgSdtGlobalView({
                cvgSdt: _stakingAsset,
    ...
    function getGlobalViewSdAssetStaking(
        ...
        for (uint256 i; i < _stakingContracts.length; ) {
            ...
            address asset = address(sdAsset.operator().token());

            _sdAssets[i] = ISdtStruct.SdAssetGlobalView({
                gaugeAsset: address(gaugeAsset),
                sdAsset: address(sdAsset),
                asset: asset,
     ...
     */
    function getProcessableRewardsOnSdtStakingContracts(
        ISdtStakingPositionService[] calldata sdtStaking
    ) external view returns (ProcessableRewards[] memory) {
        ...
        for (uint256 i; i < sdAssetGauges.length; ) {
            ...
            for (uint256 j; j < rewardCount; ) {
                ...
                processableData[j] = ProcessableData({rewardToken: rewardToken, processableAmount: processableAmount});
