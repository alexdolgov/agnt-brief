...
// File contracts/interfaces/external/IAlpacaBEP20.sol
...
// File contracts/protocol/integration/wrap/AlpacaLendingAdapter.sol
...
/**
 * @title AlpacaLendingAdapter
 * @author Cook Finance
 *
 * Wrap adapter for Alpaca Lending Pool that returns data for wrap/unwraps of tokens .
 */
contract AlpacaLendingAdapter {
    using SafeMath for uint256;
        ...
        returns(uint256) 
    {
        ...
        IAlpacaBEP20 iAlpacaToken = IAlpacaBEP20(_wrappedToken);

        uint256 totalSupply = iAlpacaToken.totalSupply();
        uint256 totalToken = iAlpacaToken.totalToken();

        if (totalSupply == 0 || totalToken == 0) {
            return _wrappedAmount;
        }

        uint256 underlyingTokenAmount = (_wrappedAmount.mul(totalToken).sub(1)).div(totalSupply).add(1);
    ...
    function validTokenPair(address _underlyingToken, address _wrappedToken) internal view returns(bool) {
        address unwrappedToken = IAlpacaBEP20(_wrappedToken).token();
        return _underlyingToken == ETH_TOKEN_ADDRESS ? unwrappedToken == WETH : unwrappedToken == _underlyingToken;
    }
}