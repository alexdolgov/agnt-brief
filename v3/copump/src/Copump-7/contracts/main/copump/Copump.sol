...
contract Copump is ICopump, Ownable2Step {
    ...
    uint256 public constant FUND_THRESHOLD = 3.5 ether;
    // Maximum supply for tokens
    uint256 public constant MAX_SUPPLY = 650_000 * DECIMALS;
    // Initial supply that will be minted for a new liquidity pool.
    uint256 public constant INITIAL_SUPPLY = 350_000 * DECIMALS;
    ...
    uint256 public constant INITIAL_PRICE = 1e10;
    // Growth rate (k), scaled to avoid precision loss
    uint256 public constant SCALED_K = 13 * 1e12;
    ...
    function setTokenCreatorFeePercent(
        ...
        require(newTokenCreatorFeePercent <= 100, "Percent exceeds 100%");
    ...
    function buyToken(
        ...
        _distributeFee(tokenInfo.creatorAddress, fee);
    ...
    function calculateMarketCap(uint256 supply) public pure returns (uint256) {
    ...
    // Divide and distribute fee between owner and token creator
    function _distributeFee(address creatorAddress, uint256 fee) private {
        uint256 creatorFee = tokenCreatorFeePercent * fee / 100;
        uint256 ownerFee = fee - creatorFee;
