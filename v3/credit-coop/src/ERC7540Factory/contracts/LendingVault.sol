...
contract LendingVault is ILendingVault, IERC7540, ERC4626, Pausable, AccessControl, ReentrancyGuard {
    ...
    function migrateLiquidStrategy(address _newLiquidStrategy) external {
        _onlyVaultManager();
        _cannotBeZeroAddress(_newLiquidStrategy);
        _onlyNativeAsset(_newLiquidStrategy);

        uint256 withdrawn = IERC20(underlyingAsset).balanceOf(address(this));
        uint256 balanceBefore = IERC20(underlyingAsset).balanceOf(address(this));

        // Withdraw all assets from the old liquid strategy
        liquidStrategy.withdraw(liquidStrategy.totalAssets());
        withdrawn = IERC20(underlyingAsset).balanceOf(address(this)) - balanceBefore;
    ...
    /**
     * @notice - The total amount of underlying tokens the LendingVault holds or has claims to.
     * @return - Total amount of underlying tokens the LendingVault holds or has deployed to lines of credit.
     */
    function totalAssets() public view override returns (uint256) {
        return totalLiquidAssets() + CreditStrategy(creditStrategy).totalCreditDeployed();
    }

    /**
     * @notice - The total amount of underlying tokens the Vault holds or has claims to.
     * @param interest - Amount of interest to add to the total assets.
     * @return         - The total amount of underlying tokens the Vault holds or has deployed to lines of credit PLUS the interest.
     */
    function totalAssetsPlusUnclaimedInterest(uint256 interest) public view returns (uint256) {
        return totalLiquidAssets() + CreditStrategy(creditStrategy).totalCreditDeployed() + interest;
    }
    ...
    /**
     * @notice  - The value of a single vault share expressed in terms of underlying tokens that the vault holds or has claim to, including assets deployed to lines of credit.
    ...
    /**
     * @notice  - The value of a single vault share expressed in terms of underlying tokens that the vault holds or has claim to, including assets deployed to lines of credit, as well as a given amount of interest.
     * @param interest - The amount of interest to add to the total assets.
     * @return         - The value of a single vault share expressed in terms of underlying tokens.
     */
    function sharePricePlusUnclaimedInterest(uint256 interest) public view returns (uint256) {
        // Get the total supply of shares.
        if (totalSupply() != 0) {
            // Calculate the exchange rate by dividing the total holdings by the share supply.
            return totalAssetsPlusUnclaimedInterest(interest).mulDiv(baseUnit, totalSupply());
        }
     ...
     */
    function processDepositRequest(address controller, uint256 assets) external nonReentrant returns (bool) {
        _onlyOperator();
        return _processDepositRequest(controller, assets);
    }
     ...
     * @return              - True if the deposit request was successfully processed.
     */
    function batchProcessDepositRequest(address[] memory controllers, uint256[] memory assets)
        external
        nonReentrant
        returns (bool)
    {
        ...
        for (uint256 x = 0; x < len; x++) {
            _processDepositRequest(controllers[x], assets[x]);
        }
    ...
    function _processDepositRequest(address controller, uint256 assets) internal returns (bool) {
        _cannotExceedDepositRequest(controller, assets);

        pendingDepositRequest[REQUEST_ID][controller] -= assets;
        claimableDepositRequest[REQUEST_ID][controller] += assets;
        emit ProcessDepositRequest(controller, REQUEST_ID, assets);
        return true;
    }
    ...
    function _processRedeemRequest(address controller, uint256 shares, uint256 minAssets, uint256 maxAssets) internal returns (bool) {
        ...
        if (assetsToWithdraw < minAssets || assetsToWithdraw > maxAssets) {
            revert RequestOutsideLimits(controller, assetsToWithdraw, minAssets, maxAssets);
        }
        ...
        emit ProcessRedeemRequest(controller, REQUEST_ID, shares);
