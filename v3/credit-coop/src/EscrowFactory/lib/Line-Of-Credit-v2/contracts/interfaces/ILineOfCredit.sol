...
interface ILineOfCredit is IInterestRateCredit {
    ...
    function available(uint256 id) external returns (uint256, uint256);
    function tokenContract() external returns (ICreditPositionToken);
    function interestAccrued(uint256 id) external returns (uint256);
