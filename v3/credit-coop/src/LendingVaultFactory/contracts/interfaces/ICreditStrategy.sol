...
interface ICreditStrategy {
    ...
    event CreditDeployed(address indexed line, uint256 indexed tokenId, uint256 amount);
    event ReduceCreditDeployed(address indexed line, uint256 indexed tokenId, uint256 indexed amount);
    event CreditImpaired(
        address indexed line, uint256 indexed tokenId, uint256 initialDeposit, uint256 unpaidPrincipal
    );
    ...
    error AlreadyImpaired();
    error CannotLendToSelf();
    error CannotImpairCredit();
    error AssetNotSupported();
    error CallerNotLine();
    error NoWithdrawableAssets();
    error NonExistentCreditPosition();
    ...
    function creditDeployedToPositions(address line, uint256 tokenId) external returns (uint256);

    function creditTokenIdToLine(uint256 tokenId) external returns (address);

    function impairments(address line, uint256 tokenId) external returns (bool);

    function totalCreditDeployed() external returns (uint256);
    ...
    function impairCredit(address line, uint256 tokenId) external;
