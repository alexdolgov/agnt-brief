...
abstract contract LineOfCredit is ILineOfCredit, InterestRateCredit, MutualConsent, ReentrancyGuard {
    ...
    /**
     * @notice - returns total outstanding debt for all positions with same token as first position
     * @return total - total outstanding debt including principal and accrued interest
     */
    function totalOutstandingDebt() external view returns (uint256 total) {
        uint256 lenIds = ids.length;
        if (lenIds == 0) return total;

        address baseToken = credits[ids[0]].token; // Get the token address of the first position

        for (uint256 i = 0; i < lenIds; ++i) {
            uint256 id = ids[i];
            Credit storage credit = credits[id];
            if (id > 0 && credit.isOpen && credit.token == baseToken) { // Check if token matches baseToken
                total += 
                    credit.principal + credit.interestAccrued + 
                    _getAccruedInterest(id, credit.principal, credit.deposit);
            }
        }
    }
