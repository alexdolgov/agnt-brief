...
abstract contract LineOfCredit is ILineOfCredit, InterestRateCredit, MutualConsent, ReentrancyGuard {
    ...
    function addCredit(
        ...
        _tryIncrementCreditDeployed(lender, tokenId, amount);
    }
     ...
     */
    function _increaseCredit(uint256 tokenId, uint256 amount, uint256 deadline) internal {
        ...
        if (fees.originationFee != 0) {
            ...
            uint256 remainingTimeToLive = credits[tokenId].deadline - block.timestamp;
            proratedOriginationFee = proratedOriginationFee
                + FeesLib._calculateOriginationFee(fees, credits[tokenId].deposit, deadline - remainingTimeToLive);
        }
    ...
    function refinanceCredit(
        ...
        _increaseCredit(tokenId, amount, deadline);
     ...
     */
    function _tryIncrementCreditDeployed(address lender, uint256 tokenId, uint256 amount) internal {
        // Skip if not a contract or contract does not support the interface
        if (
            lender.code.length == 0
            || !_safeSupportsInterface(lender, ICreditStrategyLike.incrementCreditDeployed.selector)
        ) {
            return;
        }
        ...
        (bool success, ) = lender.call(
            abi.encodeWithSelector(
                ICreditStrategyLike.incrementCreditDeployed.selector,
                tokenId,
                amount
            )
        );
