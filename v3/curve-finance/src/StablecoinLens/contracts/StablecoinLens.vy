...
def _circulating_supply() -> uint256:
    ...
    controller -> monetary policy -> peg keepers.

    @custom:warning This function is exposed for access but should not be used in
    sensitive applications or by third-party contracts, as it is prone to
    manipulation. Specifically, the crvUSD circulating supply can be artificially
    inflated or deflated, leading to inaccurate supply readings. Use with caution!
    """
