...
library TickMath {
    /// @dev Thrown when price is out of range
    error PriceOutOfRange();

    /// @dev Thrown when tick is out of range
    ...
    /// @return price A Fixed point Q64.96 number representing the sqrt of the ratio of the two
    /// assets (token1/token0)
    /// at the given tick
    ...
    /// @dev Throws in case price < MIN_SQRT_RATIO, as MIN_SQRT_RATIO is the lowest value
    /// getRatioAtTick may
    /// ever return.
    ...
    function getTickAtSqrtRatio(uint160 price) internal pure returns (int24 tick) {
        unchecked {
            ...
            tick =
                tickLow == tickHi ? tickLow : getSqrtRatioAtTick(tickHi) <= price ? tickHi : tickLow;
        }
    }
}
