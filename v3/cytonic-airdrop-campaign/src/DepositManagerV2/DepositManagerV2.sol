    ...
    AccessControlUpgradeable
{
    ...
    function withdraw(
        ...
        uint checkpointIndex = stakeTokenWithdrawCheckpoints[stakeToken]
            .length - 1;

        // toFinalizeCheckpointIndexs length should smaller than 10
        require(
            toFinalizeCheckpointIndexs[stakeToken].length < 10,
            "too many checkpoint to finalize"
        );

        // update toFinalizeCheckpointIndexs
        toFinalizeCheckpointIndexs[stakeToken].push(checkpointIndex);
    ...
    function finalizeWithdraw(
        ...
        // loop to check checkpointIndex is in toFinalizeCheckpointIndexs
        uint inArrayIndex = 0;
        bool found = false;
        for (
            uint i = 0;
            ...
            if (toFinalizeCheckpointIndexs[stakeToken][i] == checkpointIndex) {
                found = true;
                inArrayIndex = i;
                break;
            }
        ...
        require(found, "checkpointIndex not in toFinalizeCheckpointIndexs");
    ...
    // batch get stakeToken's accumulatedEquivalentTokenAmounts - cancelledEquivalentTokenAmounts
    function batchGetAccumulatedEquivalentTokenAmounts(
        address[] memory stakeTokens
    ) public view returns (uint[] memory) {
        uint[] memory amounts = new uint[](stakeTokens.length);

        for (uint i = 0; i < stakeTokens.length; ++i) {
            // check token is registered
            require(
                acceptTokenInfos[stakeTokens[i]].registered,
                "token not registered"
            );
            
            amounts[i] =
                accumulatedEquivalentTokenAmounts[stakeTokens[i]] -
                cancelledEquivalentTokenAmounts[stakeTokens[i]];
        }
