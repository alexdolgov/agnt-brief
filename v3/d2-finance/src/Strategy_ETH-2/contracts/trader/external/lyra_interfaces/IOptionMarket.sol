...
interface IOptionMarket {
    ...
    function getBoardAndStrikeDetails(uint256 boardId)
        external
        view
        returns (OptionBoard memory, Strike[] memory, uint256[] memory, uint256);
    ...
    function getSettlementParameters(uint256 strikeId)
        external
        view
        returns (uint256 strikePrice, uint256 priceAtExpiry, uint256 strikeToBaseReturned);
    ...
    error TradeIterationsHasRemainder(
        address thrower, uint256 iterations, uint256 expectedAmount, uint256 tradeAmount, uint256 totalAmount
    );
