...
/// versions.
contract DackieV3LmPoolDeployerV2 {
    struct Parameters {
        address pool;
        address masterChef;
    ...
    function deploy(IDackieV3PoolWithLMPool pool) external onlyOwner returns (IDackieV3LmPoolV2 lmPool) {
        ...
        lmPool = new DackieV3LmPoolV2{salt: keccak256(abi.encode(address(pool), masterChef, block.timestamp))}();
