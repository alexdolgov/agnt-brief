...
import './interfaces/INineMMV3Factory.sol';
import "./interfaces/INineMMV3PoolDeployer.sol";
import './interfaces/INineMMV3Pool.sol';

/// @title Canonical NineMM V3 factory
/// @notice Deploys NineMM V3 pools and manages ownership and control over pool protocol fees
contract NineMMV3Factory is INineMMV3Factory {
    /// @inheritdoc INineMMV3Factory
    ...
    function createPool(
        ...
        pool = INineMMV3PoolDeployer(poolDeployer).deploy(address(this), token0, token1, fee, tickSpacing);
    ...
    function setFeeProtocol(address pool, uint32 feeProtocol0, uint32 feeProtocol1) external override onlyOwner {
        INineMMV3Pool(pool).setFeeProtocol(feeProtocol0, feeProtocol1);
    }
    ...
    function collectProtocol(
        ...
        return INineMMV3Pool(pool).collectProtocol(recipient, amount0Requested, amount1Requested);
    }

    function setLmPool(address pool, address lmPool) external override onlyOwnerOrLmPoolDeployer {
        INineMMV3Pool(pool).setLmPool(lmPool);
    }
}
