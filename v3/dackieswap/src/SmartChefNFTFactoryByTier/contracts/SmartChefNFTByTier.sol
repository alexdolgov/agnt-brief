...
import "./interfaces/IIDackieTier.sol";


contract SmartChefNFTByTier is Ownable, ReentrancyGuard {
    ...
    function deposit(uint256[] memory _tokenIds) external nonReentrant {
        require(address(iDackieTier) != address(0), "iDackieTier address not set");
        ...
        uint256 totalAmountAfterDeposit = user.amount + _calculateTotalRarity(_tokenIds) * BASE_FACTOR;
        if (userTier == 1) require(totalAmountAfterDeposit <= 50 * BASE_FACTOR, "Tier 1 users can deposit up to 5 tokens");
        else if (userTier == 2) require(totalAmountAfterDeposit <= 150 * BASE_FACTOR, "Tier 2 users can deposit up to 15 tokens");
        else if (userTier == 3) require(totalAmountAfterDeposit <= 300 * BASE_FACTOR, "Tier 3 users can deposit up to 30 tokens");
        else if (userTier == 4) require(totalAmountAfterDeposit <= 500 * BASE_FACTOR, "Tier 4 users can deposit up to 50 tokens");
