...
/**
 * @title SmartStakingDackieUSD
 * @dev A smart contract for staking tokens and earning rewards.
 */
contract SmartStakingDackieUSD is Ownable, ReentrancyGuard, Pausable {
    ...
    // Annual Percentage Rate (APR) for staking rewards
    uint256 public constant APR = 195; // 19.5% APR
    ...
    function deposit(uint256 _amount) external nonReentrant whenNotPaused {
        ...
        if (user.amount > 0) {
            uint256 pending = _calculatePendingReward(user);
            if (pending > 0) {
    ...
    /**
     * @notice Calculate pending reward for a user based on the fixed APR.
     * @param user: user information
     * @return Pending reward for the user
     */
    function _calculatePendingReward(UserInfo storage user) internal view returns (uint256) {
        uint256 timeStaked = block.timestamp - user.lastDepositTime;
        uint256 annualReward = (user.amount * APR) / 1000;
        uint256 pending = (annualReward * timeStaked) / 365 days;
