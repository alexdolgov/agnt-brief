...
contract MockDeBridgeGate is DeBridgeGate {
    ...
    /// @dev Override the values that will be returned by the `getNativeInfo`
    /// function for the specified token.
    function overrideGetNativeInfo(
        address _token,
        uint256 _nativeChainId,
        bytes memory _nativeAddress
    ) external {
        getNativeInfo[_token] = TokenInfo({
            nativeChainId: _nativeChainId,
            nativeAddress: _nativeAddress
        });
    }

    /// @dev Override the values that will be returned by the `getDebridge`
    /// function for the specified debridgeId.
    function overrideGetDebridge(
        bytes32 _debridgeId,
        DebridgeInfo memory _debridgeInfo
    ) external {
        getDebridge[_debridgeId] = _debridgeInfo;
    }
    ...
    function increaseCollectedFees(
        ...
        getDebridgeFeeInfo[_debridgeId].collectedFees += _increaseAmount;
    }
