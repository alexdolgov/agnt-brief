...
// File: contracts/interface/IPangolinRouter.sol
...
contract superPangolinPairV2 is superTokenDebugV2 {
    ...
    constructor(address multiSignature,address origin0,address origin1,address _stakeToken,address _dsOracle,address payable _FeePool,uint256 _poolId)
        ...
        IERC20(LPToken0).safeApprove(address(pangolinRouter),uint256(-1));
        IERC20(LPToken1).safeApprove(address(pangolinRouter),uint256(-1));
    }
    ...
    function compound() external{
        ...
        if (balance0>0 && balance1>0){
            ...
            IPangolinRouter(pangolinRouter).addLiquidity(LPToken0, LPToken1, balance0, balance1,0,0, address(this), block.timestamp+30);
    ...
    function swapPair(address token,uint256 sellLimit)internal{
        ...
        swapPangolin(token,LPToken0,balance/2);
        swapPangolin(token,LPToken1,balance/2);
    }
    function swapPangolin(address token0,address token1,uint256 balance)internal{
        if(token0 == token1){
            return;
        }
        uint256 minOut = getSwapMinAmountOut(token0,token1,balance);
        address[] memory path = getSwapRouterPathInfo(token0,token1);
        if (token0 == address(0)){
            IPangolinRouter(pangolinRouter).swapExactAVAXForTokens{value : balance}(minOut,path,address(this),block.timestamp+30);
        }else{
            if (token1 == address(0)){
                IPangolinRouter(pangolinRouter).swapExactTokensForAVAX(balance,minOut,path,address(this),block.timestamp+30);
            }else{
                IPangolinRouter(pangolinRouter).swapExactTokensForTokens(balance,minOut,path,address(this),block.timestamp+30);
            }
        }
    }
