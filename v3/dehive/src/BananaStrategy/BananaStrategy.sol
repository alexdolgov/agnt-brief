...
// File: BananaStrategy.sol

/// @title Banana Strategy contract
/// @author Blaize.tech team
/// @notice Strategy for staking BANANA token
contract BananaStrategy is BaseStrategy {
    using SafeERC20 for IERC20;
    ...
    /// @param _strategyRouter Address of StrategyRouter contract.
    /// @param _adapter Address of dex-adapter.
    function initialize(address _strategyRouter, address _adapter) external initializer {
        super.initialize(
            address(0x603c7f932ED1fc6575303D8Fb018fDCBb0f39a95), // Banana token
            _strategyRouter,
            address(0x5c8D727b265DBAfaba67E050f2f739cAeEB4A6F9), // ApeSwap MasterApe
            _adapter
        );
        poolId = 0;
    }
    ...
    /// @notice Deposits tokens to ApeSwap protocol.
    ...
    function deposit(uint256 _amount) external override onlyRole(STRATEGY_ROUTER) {
        ...
        IMasterApe(masterChef).enterStaking(_amount);
        wantDeposited += _amount;
        emit Deposited(_amount);
    }

    /// @notice Withdraws some amount of tokens from ApeSwap protocol.
    ...
    function withdraw(uint256 _amount) external override onlyRole(STRATEGY_ROUTER) {
        ...
        IMasterApe(masterChef).leaveStaking(_amount);
    ...
    /// @notice Withdraws all tokens, staked in ApeSwap protocol.
    ...
    function withdrawAll() external override onlyRole(STRATEGY_ROUTER) returns (uint256, uint256) {
        _harvest();
        IMasterApe(masterChef).leaveStaking(wantDeposited);
    ...
    function emergencyWithdraw() external override onlyRole(STRATEGIST_ROLE) {
        IMasterApe(masterChef).emergencyWithdraw(poolId);
        want.safeTransfer(_msgSender(), want.balanceOf(address(this)));
        wantDeposited = 0;
        wantRewardCollected = 0;
    }

    /// @notice Claims reward from ApeSwap protocol.
    ...
    /// @notice Returns amount of Banana tokens, staked in ApeSwap protocol.
    /// @return Amount of staked tokens.
    function underlyingBalanceStored() public view override returns (uint256) {
        (uint256 amountUserProvided, ) = IMasterApe(masterChef).userInfo(poolId, address(this));
        return amountUserProvided;
    }
    ...
    function rewardsInEth(uint256 _share, uint256 _total) external view virtual override returns (uint256) {
        ...
        earnedReward += (IMasterApe(masterChef).pendingCake(poolId, address(this)) * _share) / _total;
    ...
    function _harvest() internal override returns (uint256) {
        ...
        IMasterApe(masterChef).enterStaking(0);
    ...
    /// @param _rewardAmount Amount of reward, collected reward.
