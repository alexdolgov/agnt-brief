...
interface IElkRouter {
    ...
    function swapTokensForExactxDAI(
        uint256 amountOut,
        uint256 amountInMax,
        address[] calldata path,
        address to,
    ...
    function swapxDAIForExactTokens(
        uint256 amountOut,
        address[] calldata path,
        address to,
        uint256 deadline
    ...
    function removeLiquidityxDAISupportingFeeOnTransferTokens(
        ...
        address to,
        uint256 deadline
    ) external returns (uint256 amountxDAI);
...
contract DexAdapterCore is Ownable {
    ...
    function swapETHToUnderlying(address underlying, uint256 underlyingAmount) external payable virtual {
        _swapETHToUnderlying(underlying, underlyingAmount);
    }
    ...
    function swapUnderlyingsToETH(uint256[] memory underlyingAmounts, address[] memory underlyings) external virtual {
        for (uint256 i = 0; i < underlyings.length; i++) {
            ...
            _swapUnderlyingToETH(underlyingAmounts[i], underlyings[i]);
        }
    }
    ...
    function getUnderlyingAmount(
        ...
        return _getUnderlyingAmount(_amount, _tokenToSwap, _tokenToReceive);
    }
    ...
    function getTokensPrices(address[] memory _tokens) external view virtual returns (uint256[] memory) {
        ...
        for (uint256 i = 0; i < _tokens.length; i++) {
            if (_tokens[i] == WETH) {
                ...
                prices[i] = _getTokenPrice(_tokens[i]);
            }
        }
        return prices;
    }
    ...
    function getEthAmountWithSlippage(uint256 _amount, address _tokenToSwap) external view virtual returns (uint256) {
        ...
        return _getEthAmountWithSlippage(_amount, _tokenToSwap);
    }
    ...
    function _swapUnderlyingToETH(uint256 underlyingAmount, address underlying) internal {
        IERC20(underlying).safeTransferFrom(msg.sender, address(this), underlyingAmount);
        if (underlying == WETH) {
            IWETH(WETH).withdraw(underlyingAmount);
            Address.sendValue(payable(msg.sender), underlyingAmount);
        } else {
            uint256 balance = IERC20(underlying).balanceOf(address(this));
            address[] memory path = getPath(PathType.TOKEN_TO_ETH, underlying, WETH);
            address _router = _getRouter(underlying);

            IERC20(underlying).safeApprove(_router, 0);
            IERC20(underlying).safeApprove(_router, balance);
            IUniswapV2Router01(_router).swapExactTokensForETH(balance, 0, path, msg.sender, block.timestamp + 100);
        }
    }

    function _swapETHToUnderlying(address underlying, uint256 underlyingAmount) internal virtual {
        if (underlying == WETH) {
    ...
    function _getTokenPrice(address _token) internal view virtual returns (uint256) {
        address[] memory path = getPath(PathType.TOKEN_TO_ETH, _token, WETH);
        address _router = _getRouter(_token);
        uint256 price = IUniswapV2Router01(_router).getAmountsOut(10**IERC20Metadata(_token).decimals(), path)[path.length - 1];
        return price;
    }

    function _getEthAmountWithSlippage(uint256 _amount, address _tokenToSwap) internal view virtual returns (uint256) {
...
/// @title DEX Adapter xDai
/// @author Blaize.tech team
/// @notice Contract for interacting with ElkRouter
contract DexAdapterxDai is DexAdapterCore {
    using SafeERC20 for IERC20;
    using Address for address;
    ...
    constructor()
        DexAdapterCore(
            ...
            address(0x44fA8E6f47987339850636F88629646662444217) // DAI
        )
    {}
    ...
    /// @notice Swaps an amount of ETH to underlying token and sends it to the sender.
    /// @param underlying Underlying token to be bought for ETH.
    function swapETHToUnderlying(address underlying, uint256 underlyingAmount) external payable virtual override {
        if (underlying == address(0xE1C110E1B1b4A1deD0cAf3E42BfBdbB7b5d7cE1C)) {
            // ELC token
            address[] memory path = getPath(PathType.ETH_TO_TOKEN, WETH, underlying);
            address _router = _getRouter(underlying);
            IElkRouter(_router).swapExactxDAIForTokens{value: msg.value}(underlyingAmount, path, msg.sender, block.timestamp + 100);
        } else if (
            underlying == address(0xC45b3C1c24d5F54E7a2cF288ac668c74Dd507a84) ||
            underlying == address(0x63e62989D9EB2d37dfDB1F93A22f063635b07d51)
            // SYMM token || MIVA token
        ) {
            address _router = _getRouter(underlying);
            IERC20(WETH).safeApprove(_router, 0);
            IERC20(WETH).safeApprove(_router, msg.value);
            IWETH(WETH).deposit{value: msg.value}();
            IBPool(_router).swapExactAmountOut(WETH, msg.value, underlying, underlyingAmount, type(uint256).max);
    ...
    /// @param underlyingAmounts Amount of each underlying token to be swaped.
    /// @param underlyings Addresses of underlying tokens to be swaped.
    function swapUnderlyingsToETH(uint256[] memory underlyingAmounts, address[] memory underlyings) external virtual override {
        for (uint256 i = 0; i < underlyings.length; i++) {
            if (underlyingAmounts[i] == 0) {
                continue;
            }
            if (underlyings[i] == address(0xE1C110E1B1b4A1deD0cAf3E42BfBdbB7b5d7cE1C)) {
                // ELC token
                IERC20(underlyings[i]).safeTransferFrom(msg.sender, address(this), underlyingAmounts[i]);
                uint256 balance = IERC20(underlyings[i]).balanceOf(address(this));
                ...
                IElkRouter(_router).swapExactTokensForxDAI(balance, 0, path, msg.sender, block.timestamp + 100);
            } else if (
                underlyings[i] == address(0xC45b3C1c24d5F54E7a2cF288ac668c74Dd507a84) ||
                underlyings[i] == address(0x63e62989D9EB2d37dfDB1F93A22f063635b07d51)
                // SYMM token || MIVA token
            ) {
                IERC20(underlyings[i]).safeTransferFrom(msg.sender, address(this), underlyingAmounts[i]);
                uint256 balance = IERC20(underlyings[i]).balanceOf(address(this));
                ...
                IERC20(underlyings[i]).safeApprove(_router, 0);
                IERC20(underlyings[i]).safeApprove(_router, balance);
                IBPool(_router).swapExactAmountIn(underlyings[i], balance, WETH, 0, type(uint256).max);
    ...
    function getEthAmountWithSlippage(uint256 _amount, address _tokenToSwap) external view virtual override returns (uint256) {
        if (_tokenToSwap == WETH) {
            return _amount;
        }
        if (
            _tokenToSwap == address(0xC45b3C1c24d5F54E7a2cF288ac668c74Dd507a84) ||
            _tokenToSwap == address(0x63e62989D9EB2d37dfDB1F93A22f063635b07d51)
            // SYMM token || MIVA token
        ) {
            address _router = _getRouter(_tokenToSwap);
            uint256 xDaiRequired = IBPool(_router).calcInGivenOut(
                IBPool(_router).getBalance(WETH),
                IBPool(_router).getDenormalizedWeight(WETH),
                IBPool(_router).getBalance(_tokenToSwap),
                IBPool(_router).getDenormalizedWeight(_tokenToSwap),
                _amount,
                IBPool(_router).getSwapFee()
            );
            return (xDaiRequired);
        }
        return _getEthAmountWithSlippage(_amount, _tokenToSwap);
    ...
    function getTokensPrices(address[] memory _tokens) external view virtual override returns (uint256[] memory) {
        uint256[] memory prices = new uint256[](_tokens.length);
        for (uint256 i = 0; i < _tokens.length; i++) {
            if (_tokens[i] == WETH) {
                prices[i] = 1 ether;
            ...
            if (
                _tokens[i] == address(0xC45b3C1c24d5F54E7a2cF288ac668c74Dd507a84) ||
                _tokens[i] == address(0x63e62989D9EB2d37dfDB1F93A22f063635b07d51)
                // SYMM token || MIVA token
            ) {
                address _router = _getRouter(_tokens[i]);
                prices[i] = IBPool(_router).calcOutGivenIn(
                    IBPool(_router).getBalance(_tokens[i]),
                    IBPool(_router).getDenormalizedWeight(_tokens[i]),
                    IBPool(_router).getBalance(WETH),
                    IBPool(_router).getDenormalizedWeight(WETH),
                    10**IERC20Metadata(_tokens[i]).decimals(),
                    IBPool(_router).getSwapFee()
                );
            } else {
                prices[i] = _getTokenPrice(_tokens[i]);
    ...
    function getUnderlyingAmount(
        ...
        if (
            (_tokenToSwap == address(0xC45b3C1c24d5F54E7a2cF288ac668c74Dd507a84) ||
                _tokenToSwap == address(0x63e62989D9EB2d37dfDB1F93A22f063635b07d51)) ||
            _tokenToReceive == address(0x63e62989D9EB2d37dfDB1F93A22f063635b07d51) ||
            _tokenToReceive == address(0xC45b3C1c24d5F54E7a2cF288ac668c74Dd507a84)
            // SYMM token || MIVA token
        ) {
            if (_tokenToSwap == WETH || _tokenToReceive == WETH) {
                address _router = _getRouter(_tokenToSwap == WETH ? _tokenToReceive : _tokenToSwap);
                uint256 underlyingAmount = IBPool(_router).calcOutGivenIn(
                    IBPool(_router).getBalance(_tokenToSwap),
                    IBPool(_router).getDenormalizedWeight(_tokenToSwap),
                    IBPool(_router).getBalance(_tokenToReceive),
                    IBPool(_router).getDenormalizedWeight(_tokenToReceive),
                    _amount,
                    IBPool(_router).getSwapFee()
                );
    ...
    ///@notice Withdraw all xDai stucked.
    function withdrawRemainderXDai() external onlyOwner {
        Address.sendValue(payable(_msgSender()), address(this).balance);
    }

    ///@notice Withdraw all amount of stucked ERC-20 token.
    ///@param _token Stucked token to withdraw.
    function withdrawRemainder(address _token) external onlyOwner {
...
// File: HoneySwapAdapter.sol

/// @title HoneySwap Adapter for xDai cluster
/// @author Blaize.tech team
/// @notice Contract for interacting with HoneySwap router
contract HoneySwapAdapter is DexAdapterxDai {
    using SafeERC20 for IERC20;

    address public constant mivaPool = address(0xBf511aD7C4426980e27f6b84A2e3899C565B5c2b);
    address public constant symmPool = address(0xcd65F38b298310fA2c8b1844ea187016f913B0C8);
    address public constant elkRouter = address(0xe5759714998e8B50A33c7333C04C2d02e5dcE77f);
    ...
    /// @notice Sets an addresses of HoneySwap router, DAI and WXDAI in xDai.
    constructor() {}
    ...
    function _getRouter(address _token) internal view override returns (address) {
        if (_token == address(0xE2e73A1c69ecF83F464EFCE6A5be353a37cA09b2)) {
            //Link Token
            return sushiswapRouter;
        }
        if (_token == address(0xE1C110E1B1b4A1deD0cAf3E42BfBdbB7b5d7cE1C)) {
            // Elk Token
            return elkRouter;
        }
        if (_token == address(0xC45b3C1c24d5F54E7a2cF288ac668c74Dd507a84)) {
            //Symm Token
            return symmPool;
        }
        if (_token == address(0x63e62989D9EB2d37dfDB1F93A22f063635b07d51)) {
            // Miva Token
            return mivaPool;
        }
        return router;
