...
contract Optimizer is OwnableUpgradeable {
    using SafeERC20 for IERC20; 

    address constant public FISH = address(0x3a3Df212b7AA91Aa0402B9035b098891d276572B);
    address constant public QUICK = address(0x831753DD7087CaC61aB5644b308642cc1c33Dc13);
    address constant public QI = address(0x580A84C73811E1839F75d86d75d88cCa0c241fF4);
    ...
    function optimize(
        address _asset,
        address _adapter,
        uint256 _newFishShare
    ) external onlyOwner {
        IDexAdapter adapter = IDexAdapter(_adapter);

        // Prepare shares
        uint256[] memory shares = IClusterToken(_asset).getUnderlyingInCluster();
        shares[3] = _newFishShare;

        // approve some quick
        IERC20(QUICK).safeApprove(
            _asset, 
            IERC20(QUICK).balanceOf(address(this))
        );

        // Optimize fish proportion and get quick
        ...
        amounts[0] = IERC20(FISH).balanceOf(address(this));

        IERC20(FISH).safeApprove(_adapter, amounts[0]);
        adapter.swapUnderlyingsToETH(amounts, tokens);

        // Swap matic to qi
        adapter.swapETHToUnderlying{value: address(this).balance}(QI, 0);

        // Prepare shares 
        uint256 qiTotal = IERC20(QI).balanceOf(address(this)) +
                          IERC20(QI).balanceOf(_asset);
    
        shares[5] = (qiTotal * 10**6) / IERC20(_asset).totalSupply();

        // Approve qi
        IERC20(QI).safeApprove(_asset, IERC20(QI).balanceOf(address(this)));
