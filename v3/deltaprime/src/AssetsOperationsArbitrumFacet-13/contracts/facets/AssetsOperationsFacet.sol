    ...
    **/
    function fund(bytes32 _fundedAsset, uint256 _amount) public virtual noBorrowInTheSameBlock nonReentrant {
        ...
        if(tokenManager.isGmxMarketWhitelisted(address(token))) {
            ...
            if(benchmark.exists) {
                (uint256 gmTokenPrice, uint256 longTokenPrice, uint256 shortTokenPrice) = _getGmxTokenPrices(address(token));
                GmxV2FeesLib._sweepFees(address(token), gmTokenPrice, longTokenPrice, shortTokenPrice);    
            } 
        }
        ...
        _createOrUpdateBenchmarkIfGmToken(address(token), _amount);

        emit Funded(msg.sender, _fundedAsset, _amount, block.timestamp);
    }

    function _getGmxTokenPrices(address gmMarket) internal returns (uint256 gmTokenPrice, uint256 longTokenPrice, uint256 shortTokenPrice) {
        ...
        if (shortToken == longToken) {
            prices[2] = 0; // to avoid double counting of tokenPriceUSD in GMX Plus Markets
        }
        return (prices[0], prices[1], prices[2]);
    }

    function _createOrUpdateBenchmarkIfGmToken(address _token, uint256 _amount) internal {
         ITokenManager tokenManager = DeploymentConstants.getTokenManager();
        ...
        if (tokenManager.isGmxMarketWhitelisted(_token) ) {
            ...
            address gmxReader = DeploymentConstants.getGmxReaderAddress();
            address dataStore = DeploymentConstants.getGmxDataStoreAddress();
            IGmxReader.MarketProps memory marketProps = IGmxReader(gmxReader).getMarket(dataStore, _token);
            address longToken = marketProps.longToken;
            address shortToken = marketProps.shortToken;
            require(longToken != address(0) && shortToken != address(0), "GMX market tokens not found");
            ...
            uint256 longTokenAmount = (_amount * IERC20(longToken).balanceOf(_token)) / totalGmSupply;
            uint256 shortTokenAmount = (_amount * IERC20(shortToken).balanceOf(_token)) / totalGmSupply;
            bytes32[] memory symbols = new bytes32[](3);
            symbols[0] = tokenManager.tokenAddressToSymbol(_token);
            symbols[1] = tokenManager.tokenAddressToSymbol(longToken);
            symbols[2] = tokenManager.tokenAddressToSymbol(shortToken);
            uint256[] memory prices = getPrices(symbols);

            if(tokenManager.isGmxPlusMarket(_token)) {
                shortTokenAmount = 0; // to avoid double counting of underlying token amounts
                prices[2] = 0; // to avoid double counting of tokenPriceUSD 
            }
            ...
            GmxV2FeesLib._createOrUpdatePositionBenchmark(_token, _amount, positionDetails);    
