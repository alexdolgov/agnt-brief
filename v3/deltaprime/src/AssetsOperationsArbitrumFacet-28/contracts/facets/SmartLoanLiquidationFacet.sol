...
contract SmartLoanLiquidationFacet is ReentrancyGuardKeccak, SolvencyMethods {
    ...
    // To handle dust amounts when liquidating. 0.1 USD with 18 decimals
    ...
    **/
    function unsafeLiquidateLoan(bytes32[] memory assetsToRepay, uint256[] memory amountsToRepay, uint256 _liquidationBonusPercent) external onlyWhitelistedLiquidators accountNotFrozen nonReentrant {
        liquidate(
            LiquidationConfig({
    ...
    * @param _liquidationBonusPercent per mille bonus for liquidator. Must be lower than or equal to  getMaxLiquidationBonus()
    **/
    function liquidateLoan(bytes32[] memory assetsToRepay, uint256[] memory amountsToRepay, uint256 _liquidationBonusPercent) external onlyWhitelistedLiquidators accountNotFrozen nonReentrant {
        liquidate(
            LiquidationConfig({
                assetsToRepay : assetsToRepay,
                amountsToRepay : amountsToRepay,
    ...
    **/
    function liquidate(LiquidationConfig memory config) internal {
        ...
        if (healingLoan) {
            ...
            require(_getTotalValueWithPrices(cachedPrices.ownedAssetsPrices, cachedPrices.stakedPositionsPrices) <= DUST_THRESHOLD_USD, "Healing a loan must end up with 0 total value");
        } else {
