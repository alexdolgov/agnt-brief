...
contract AssetsOperationsFacet is ReentrancyGuardKeccak, OnlyOwnerOrInsolvent {
    ...
    **/
    function fund(bytes32 _fundedAsset, uint256 _amount) public virtual noBorrowInTheSameBlock nonReentrant {
        ...
        ITokenManager tokenManager = DeploymentConstants.getTokenManager();
        GmxV2FeesLib.GmxTokenPrices memory gmxTokenPrices;
        if(tokenManager.isGmxMarketWhitelisted(address(token))) {
            ...
            if(benchmark.exists) {
                ...
                GmxV2FeesLib._sweepFees(address(token), gmxTokenPrices);    
            } 
        }
    ...
    function _getGmxTokenPrices(address gmMarket) internal returns (GmxV2FeesLib.GmxTokenPrices memory){ 
    ...
    function _createOrUpdateBenchmarkIfGmToken(address _token, uint256 _amount, GmxV2FeesLib.GmxTokenPrices memory gmxTokenPrices) internal {
         ITokenManager tokenManager = DeploymentConstants.getTokenManager();
        ...
        if (tokenManager.isGmxMarketWhitelisted(_token) ) {
            ...
            GmxV2FeesLib._createOrUpdatePositionBenchmark(_token, positionDetails);    
