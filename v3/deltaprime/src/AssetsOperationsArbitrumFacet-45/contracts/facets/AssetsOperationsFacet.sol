...
//        emit Withdrawn(msg.sender, "GLP", _amount, block.timestamp);
    ...
    **/
    function borrow(bytes32 _asset, uint256 _amount) external onlyOwner remainsSolvent noBorrowInTheSameBlock nonReentrant {
     ...
     **/
    function repay(bytes32 _asset, uint256 _amount) public payable  noBorrowInTheSameBlock nonReentrant {
    ...
    function withdrawUnsupportedToken(address token) external nonReentrant onlyOwner noBorrowInTheSameBlock remainsSolvent {
        ITokenManager tokenManager = DeploymentConstants.getTokenManager();
    ...
    * @dev _repayAmount and __borrowAmount can be used to control the slippage.
     ...
     */
    function swapDebt(bytes32 _fromAsset, bytes32 _toAsset, uint256 _repayAmount, uint256 _borrowAmount, address[] calldata _path, address[] calldata _adapters) external onlyOwner remainsSolvent noBorrowInTheSameBlock nonReentrant {
