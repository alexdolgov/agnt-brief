...
contract AssetsOperationsFacet is ReentrancyGuardKeccak, PrimeAccountModifiers, GmxV2FeesHelper, GlvHelper {
    ...
    **/
    function fund(bytes32 _fundedAsset, uint256 _amount) public virtual onlyOwner noBorrowInTheSameBlock nonReentrant {
     ...
     *      being funded in, then extend the benchmark additively (M-12 fix: do NOT overwrite,
     *      which would let a dust fund() call erase an unrealized-gain cost basis).
     */
    function _createOrUpdateBenchmarkIfGmOrGlvToken(address _token, uint256 _amount, GmxTokenPrices memory gmxTokenPrices) internal {
        ...
        if (tokenManager.isGlvTokenWhitelisted(_token)) {
            ...
            longTokenAmount = (_amount * IERC20(unified.longToken).balanceOf(_token)) / totalGmSupply;
            shortTokenAmount = (_amount * IERC20(unified.shortToken).balanceOf(_token)) / totalGmSupply;
