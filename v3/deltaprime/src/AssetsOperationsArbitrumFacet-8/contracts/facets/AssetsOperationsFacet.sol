...
contract AssetsOperationsFacet is ReentrancyGuardKeccak, OnlyOwnerOrInsolvent, GmxV2FeesHelper, GlvHelper {
    ...
    **/
    ...
    function _createOrUpdateBenchmarkIfGmOrGlvToken(address _token, uint256 _amount, GmxTokenPrices memory gmxTokenPrices) internal {
        ...
        if (tokenManager.isGlvTokenWhitelisted(_token)) {
            ...
            uint256 glvTokenBalance = IERC20(_token).balanceOf(address(this));
            (longTokenAmount, shortTokenAmount) = _getGlvLongAndShortTokenAmounts(_token, glvTokenBalance);
