...
contract CTKN is ERC20, Ownable {
    ...
    /**
     * @dev Owner-only batch mint function
     * @param recipients Array of addresses to mint tokens to
     * @param amounts Array of token amounts to mint (in wei, accounting for decimals)
     */
    function batchMint(address[] calldata recipients, uint256[] calldata amounts) public onlyOwner {
        require(recipients.length == amounts.length, "CTKN: recipients and amounts length mismatch");
        require(recipients.length > 0, "CTKN: empty arrays");
