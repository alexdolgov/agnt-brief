...
abstract contract GmxV2FeesHelper is DiamondMethodsAccess, GmxGlvUnifiedHelper {
    ...
    /// @dev keep in sync with the mirror at SolvencyFacetProd._computeDeductibleFeesInGmTokens
    /// (DPSC-340 fix). Both must use the same percentage or solvency calc and fee-sweep diverge.
     ...
     *
     *      This is the fix for Digisec M-12 / HackenProof DPSC-184 cluster: with the old
     *      overwrite behavior, a user holding a GM position with unrealized gains could call
     *      fund() with a dust amount of GM tokens and have the benchmark rewritten to
     *      `totalBalance * currentPrice`, erasing the prior cost basis and effectively
     *      "resetting" the performance fee accrual to zero.
     *
