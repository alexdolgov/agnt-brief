...
abstract contract GmxV2CallbacksFacet is IDepositCallbackReceiver, IWithdrawalCallbackReceiver, IGasFeeCallbackReceiver, ReentrancyGuardKeccak, GmxV2FeesHelper, GlvHelper {
    ...
    function _updatePositionBenchmark(address market) internal {
        ...
        if (tokenManager.isGlvTokenWhitelisted(market)) {
            (longTokenAmount, shortTokenAmount) = _getLongAndShortTokenAmounts(market, gmTokenAmount);
        } else {
