...
import {GmxUnifiedStruct} from "./GmxUnifiedStruct.sol";

contract GlvHelper is ReentrancyGuardKeccak, OnlyOwnerOrInsolvent, GmxUnifiedStruct {
    ...
    function _getLongAndShortTokenAmounts(address glvToken, uint256 glvAmount) internal view returns (uint256 longTokenAmount, uint256 shortTokenAmount) {
    ...
    function _getLongAndShortTokenAmountsWithCachedPrices(address glvToken, uint256 glvAmount, GmxTokenPrices memory glvTokenPrices) internal view returns (uint256 longTokenAmount, uint256 shortTokenAmount) {
