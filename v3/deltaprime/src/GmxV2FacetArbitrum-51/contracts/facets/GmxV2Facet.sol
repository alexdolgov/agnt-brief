...
abstract contract GmxV2Facet is GmxV2FacetCommon {
    ...
    function _deposit(
        ...
        GmxV2FeesLib.GmxTokenPrice memory gmxTokenPrice;
        // Fetch prices once at the beginning
        (gmxTokenPrice.gmTokenPrice, gmxTokenPrice.longTokenPrice, gmxTokenPrice.shortTokenPrice) = _getGmxTokenPrices(gmToken);
        ...
        {    
            ...
            if(benchmark.exists) {
                GmxV2FeesLib._sweepFees(gmToken, gmxTokenPrice.gmTokenPrice, gmxTokenPrice.longTokenPrice, gmxTokenPrice.shortTokenPrice);    
            } 
        }
        {
        ...
        _updatePositionBenchmark(gmToken, minGmAmount, gmxTokenPrice);
    ...
    function sweepFeesAndUpdateBenchMark(address gmToken) external nonReentrant onlyWhitelistedLiquidators remainsSolvent returns (uint256 gmTokensInFees) {
        ...
        gmTokensInFees = GmxV2FeesLib._sweepFees(gmToken, gmTokenPrice, longTokenPrice, shortTokenPrice);
        GmxV2FeesLib._updateBenchmark(gmToken, gmTokenPrice, longTokenPrice, shortTokenPrice);

    }


    function _updatePositionBenchmark(address market, uint256 gmTokenAmount, GmxV2FeesLib.GmxTokenPrice memory gmxTokenPrice) internal{
        ...
        GmxV2FeesLib.GmxPositionDetails memory positionDetails = GmxV2FeesLib.GmxPositionDetails({
            ...
            gmTokenPriceUsd: gmxTokenPrice.gmTokenPrice,
            longTokenPriceUsd: gmxTokenPrice.longTokenPrice,
            shortTokenPriceUsd: gmxTokenPrice.shortTokenPrice,
    ...
    function initiateGmxFeesBenchMark(address gmToken) external onlyWhitelistedLiquidators nonReentrant {
        ...
        (uint256 longTokenAmount, uint256 shortTokenAmount, address longToken, address shortToken) = _getUnderlyingTokenDetails(gmToken, gmTokenAmount);
        (uint256 gmTokenPrice, uint256 longTokenPrice, uint256 shortTokenPrice)  = _getGmxTokenPrices(gmToken);
        GmxV2FeesLib.GmxPositionDetails memory positionDetails = GmxV2FeesLib.GmxPositionDetails({
            underlyingLongTokenAmount: longTokenAmount,
            underlyingShortTokenAmount: shortTokenAmount,
            gmTokenPriceUsd: gmTokenPrice,
        ...
        GmxV2FeesLib._createOrUpdatePositionBenchmark(gmToken, gmTokenAmount, positionDetails);       
    }

    function _getGmxTokenPrices(address gmMarket) internal view returns (uint256 gmTokenPrice, uint256 longTokenPrice, uint256 shortTokenPrice) {
    ...
    function getGmPerformance(address gmToken) external view returns (uint256) {
        (uint256 gmTokenPrice, uint256 longTokenPrice, uint256 shortTokenPrice)  = _getGmxTokenPrices(gmToken);
        return GmxV2FeesLib._getGmAnnualisedPerformance(gmToken, gmTokenPrice, longTokenPrice, shortTokenPrice);
    }
        ...
        onlyOwnerNoStaySolventOrInsolventPayable
    {
        ...
        _updatePositionBenchmark(gmToken, gmAmount, gmxTokenPrice);
