...
contract HealthMeterFacetProd is ArbitrumProdDataServiceConsumerBase {
    ...
    function _getTotalTraderJoeV2Weighted() internal view returns (uint256) {
        ...
        if (ownedTraderJoeV2Bins.length > 0) {
            for (uint256 i; i < ownedTraderJoeV2Bins.length; i++) {
                ...
                {
                    uint256 debtCoverageX = DeploymentConstants.getTokenManager().tieredDebtCoverage(DiamondStorageLib.getPrimeLeverageTier(), address(binInfo.pair.getTokenX()));
                    uint256 debtCoverageY = DeploymentConstants.getTokenManager().tieredDebtCoverage(DiamondStorageLib.getPrimeLeverageTier(), address(binInfo.pair.getTokenY()));
    ...
    **/
    ...
    function getHealthMeter() public view returns (uint256) {
        ...
        uint256 weightedCollateralPlus = assetsPrices[0].price * address(this).balance * tokenManager.tieredDebtCoverage(DiamondStorageLib.getPrimeLeverageTier(),tokenManager.getAssetAddress(nativeTokenSymbol, true)) / (10 ** 26);
        ...
        for (uint256 i = 0; i < assetsPrices.length; i++) {
            ...
            uint256 debtCoverage;
            {
                ...
                debtCoverage = tokenManager.tieredDebtCoverage(DiamondStorageLib.getPrimeLeverageTier(), address(token));
            }
