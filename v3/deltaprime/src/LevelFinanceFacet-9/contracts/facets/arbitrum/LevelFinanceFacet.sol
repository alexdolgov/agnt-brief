...
contract LevelFinanceFacet is ReentrancyGuardKeccak, OnlyOwnerOrInsolvent {
    ...
    function exitLevelFinanceAndHarvestRewardsForOwner() external payable onlyWhitelistedLiquidators{
        ...
        uint256 jnrTrancheBalance = levelJnrBalance();
        uint256 mzeTrancheBalance = levelMzeBalance();
        uint256 snrTrancheBalance = levelSnrBalance();

        if(jnrTrancheBalance > 0){
            ...
            farmingContract.harvest(2, DiamondStorageLib.contractOwner());
            DiamondStorageLib.removeStakedPosition("stkdJnrLLP");
        }

        if(mzeTrancheBalance > 0){
            orderManager.placeRemoveLiquidityOrder{value: msg.value}(
                LEVEL_MEZZANINE_LLP,
                0xaf88d065e77c8cC2239327C5EDb3A432268e5831,
                mzeTrancheBalance,
            ...
            farmingContract.harvest(1, DiamondStorageLib.contractOwner());
            DiamondStorageLib.removeStakedPosition("stkdMzeLLP");
        }
