  ...
  function executeOperation(
...

    {
      ...
      for (uint256 i = 0; i < assets.length; i++) {
        amountsDelivered[i] = amounts[i] - IERC20Metadata(assets[i]).balanceOf(lep.loan);
      }
...
//                  );
  ...
  function executeFlashloan(
    ...
    for (uint256 i = 0; i < _args.assets.length; i++) {
      amountsDelivered[i] = _args.amounts[i] - IERC20Metadata(_args.assets[i]).balanceOf(_args.loanAddress);
    }
