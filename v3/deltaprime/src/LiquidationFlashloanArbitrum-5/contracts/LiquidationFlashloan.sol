...
contract LiquidationFlashloan is FlashLoanReceiverBase, Ownable {
  ...
  function executeOperation(
    ...
    uint256[] calldata amountsDelivered = getAmounts();
    uint256[] calldata premiums = getPremiums();

    {
      for (uint32 i = 0; i < assets.length; i++) {
        if(amountsDelivered[i] > 0){
          assets[i].safeApprove(lep.loan, 0);
          assets[i].safeApprove(lep.loan, amountsDelivered[i]);
        }
      }

      // Only liquidate
      liquidateLoan(_params, lep);
    }
    ...
    ) = getSurplusDeficitAssets(assets, premiums, amountsDelivered, lep.tokenManager);
...
//                  );
    ...
    for (uint32 i = 0; i < assets.length; i++) {
      ...
      assets[i].safeApprove(address(POOL), amountsDelivered[i] + premiums[i]);
    }
    return true;
  }
  ...
  function executeFlashloan(
    ...
    bytes memory enrichedParams = bytes.concat(
      ...
      encoded,
      abi.encodePacked(_args.amounts),
      _args.params
    );

    uint256[] memory amountsDelivered = new uint256[](_args.assets.length);
    for (uint256 i = 0; i < _args.assets.length; i++) {
      uint256 balance = IERC20Metadata(_args.assets[i]).balanceOf(_args.loanAddress);
      amountsDelivered[i] = balance > _args.amounts[i] ? 0 : _args.amounts[i] - balance;
    }
  ...
  function getLiqEnrichedParams(
    ...
    uint256[] memory _amountsToRepay = new uint256[](5);
    assembly {
    ...
    bytes memory encodedOffers = new bytes(length);
    bytes memory encodedAmountsToRepay = new bytes(32*6); // 5 array elements + 1 length
    for (uint256 i = 0; i < length; ++i) {
      // Read length bytes from _enrichedParams + 124 bytes offset
      encodedOffers[i] = _enrichedParams[124 + i];
    }
    for (uint256 i = 0; i < 32*6; ++i) {
      encodedAmountsToRepay[i] = _enrichedParams[124 + length + i];
    }
    _offers = abi.decode(encodedOffers, (IYieldYakRouter.FormattedOffer[]));
    _amountsToRepay = abi.decode(encodedAmountsToRepay, (uint256[]));
    return
    LiqEnrichedParams({
      loan: _loan,
