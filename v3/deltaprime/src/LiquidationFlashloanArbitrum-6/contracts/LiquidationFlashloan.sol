...
contract LiquidationFlashloan is FlashLoanReceiverBase, Ownable {
  ...
  function executeOperation(
    ...
    {
      uint256[] memory amountsToRepay = new uint256[](assets.length);
      for (uint256 i = 0; i < assets.length; i++) {
        amountsToRepay[i] = amountsDelivered[i] + IERC20Metadata(assets[i]).balanceOf(lep.loan);
      }
      ...
      // Only liquidate
      liquidateLoan(_params, lep, amountsToRepay);
    }
