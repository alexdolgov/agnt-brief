...
contract LiquidationFlashloan is FlashLoanReceiverBase, Ownable {
  ...
  function executeOperation(
    ...
    {
      uint256[] memory amountsDelivered = new uint256[](assets.length);
      for (uint256 i = 0; i < assets.length; i++) {
        uint256 balance = IERC20Metadata(assets[i]).balanceOf(lep.loan);
        amountsDelivered[i] = balance > amounts[i] ? 0 : amounts[i] - balance;
      }
