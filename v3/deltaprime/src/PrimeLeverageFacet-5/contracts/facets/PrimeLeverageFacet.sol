...
contract PrimeLeverageFacet is  IPrimeLeverageFacet,ReentrancyGuardKeccak, DiamondMethodsAccess, PrimeAccountModifiers {
    ...
    **/
    ...
    function unstakePrime(uint256 amount) external onlyOwner nonReentrant {
        ...
        // L-02 fix: a PREMIUM account with accrued PRIME debt must keep enough staked PRIME
        // to cover that debt, otherwise an unstake could leave the protocol with a PRIME-debt
        // that exceeds available stake even though the USD-denominated stake ratio is satisfied.
