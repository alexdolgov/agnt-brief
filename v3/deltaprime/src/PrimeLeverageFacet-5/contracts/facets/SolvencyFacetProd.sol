...
abstract contract SolvencyFacetProd is RedstoneConsumerNumericBase, DiamondHelper {
    ...
    struct CachedPrices {
        ...
        // Kept in its own slot so _getTotalAssetsValueBase / _getTWVOwnedAssets do not sum them.
    ...
    **/
    function getAllPricesForLiquidation(bytes32[] memory assetsToRepay) public view returns (CachedPrices memory result) {
        ...
        // Populate ownedAssetsPrices struct — sized to owned assets only so the
        // _getTotalAssetsValueBase / _getTWVOwnedAssets loops do not double-count
        // any TJv2 bin token that is also an owned asset.
        ...
        // resolve tokenX / tokenY for non-owned TJv2 bin tokens without leaking
        // into the owned-asset sum.
    ...
    **/
     ...
     * @dev DPSC-340 fix: for GM/GLV tokens the solvency path previously used the raw ERC20
     *      balance, while the view path (SmartLoanViewFacet._getGmTokenBalanceAfterFeesWithPrices)
     *      already netted out the 10% unrealized-performance fee. That divergence lets a PA
     *      borrow into the "ghost zone" between net-of-fee TWV and raw TWV and also blocks
     *      sweepFeesAndUpdateBenchMark() via its own remainsSolvent modifier, leaving the
     *      fee uncollectable.
     *
     *      This helper consolidates the correct accounting: for any owned asset that has an
     *      active GMX V2 benchmark (created via GMX deposit callback or additive fund()),
     *      return balance - feeInGmTokens. If prices for the underlying long/short tokens
     *      are not available in `ownedAssetsPrices` (edge case: the PA holds only the GM
     *      token), the helper conservatively returns the raw balance — never over-deducts.
     */
    function _getSolvencyBalance(
        ...
        // Locate long/short prices in the already-fetched owned-asset price array.
        // If not present, skip fee deduction (fail-open: never over-deduct collateral).
        // We use type(uint256).max as the "not-found" sentinel rather than a separate
        // bool to keep the bytecode compact while still distinguishing absent prices
        // from a (theoretical) genuine zero.
        uint256 longPrice = _findPriceForToken(benchmark.longTokenAddress, ownedAssetsPrices, tokenManager);
        if (longPrice == type(uint256).max) return rawBalance;

        // GmxPlus markets store shortTokenAddress == longTokenAddress (or address(0));
        // in those cases the short side contributes 0 and no lookup is needed.
        uint256 shortPrice;
        if (benchmark.shortTokenAddress != address(0) && benchmark.shortTokenAddress != benchmark.longTokenAddress) {
            ...
            if (shortPrice == type(uint256).max) return rawBalance;
        }

        uint256 feeInGmTokens = _computeDeductibleFeesInGmTokens(token, tokenPriceUsd, longPrice, shortPrice, benchmark, rawBalance);
        return feeInGmTokens >= rawBalance ? 0 : rawBalance - feeInGmTokens;
    }

    /// @dev Mirror of `GmxV2FeesHelper._getDeductibleFeesInGmTokens`, scoped to this facet so
    ///      SolvencyFacetProd doesn't need to take a cross-facet dependency on the fees lib.
    function _computeDeductibleFeesInGmTokens(
        address gmMarket,
    ...
    ) internal view returns (uint256 feeInGmTokens) {
        uint256 currentValueUsd = (currentBalance * gmTokenPriceUsd) / 1e8;
        uint256 longValueUsd = _gmUnderlyingValueUsd(benchmark.underlyingLongTokenAmount, longTokenPriceUsd, benchmark.longTokenAddress);
        uint256 shortValueUsd = _gmUnderlyingValueUsd(benchmark.underlyingShortTokenAmount, shortTokenPriceUsd, benchmark.shortTokenAddress);
        uint256 underlyingValueUsd = longValueUsd + shortValueUsd;
        if (currentValueUsd <= underlyingValueUsd) {
            return 0;
        }
        uint256 gmPerformanceUsd = currentValueUsd - underlyingValueUsd;
        // KEEP IN SYNC with GmxV2FeesHelper.FEE_PERCENTAGE (1e17 of 1e18 = 10%).
        // This is a deliberate mirror to avoid a cross-facet dependency from
        // SolvencyFacetProd into GmxV2FeesHelper; both values must be updated together.
        // `gmPerformanceUsd / 10` is the bytecode-cheap form of `(gmPerformanceUsd * 1e17) / 1e18`.
        uint256 gmPerformanceFeeUsd = gmPerformanceUsd / 10;
        feeInGmTokens = (gmPerformanceFeeUsd * 1e8) / gmTokenPriceUsd;
    }

    function _gmUnderlyingValueUsd(uint256 amount, uint256 priceUsd, address tokenAddr) internal view returns (uint256) {
    ...
    /// @dev Returns the price for `token` from `prices`, or type(uint256).max if not present.
    /// The sentinel-based return avoids a (uint256, bool) tuple at every call site, which
    /// shaves bytecode versus the two-value form.
    ...
    **/
    function _getTotalAssetsValueBase(AssetPrice[] memory ownedAssetsPrices) public virtual view returns (uint256) {
        if (ownedAssetsPrices.length > 0) {
            ...
            for (uint256 i = 0; i < ownedAssetsPrices.length; i++) {
                IERC20Metadata token = IERC20Metadata(tokenManager.getAssetAddress(ownedAssetsPrices[i].asset, true));
                // DPSC-340 fix: deduct pending GMX V2 performance fee from GM/GLV token balance.
