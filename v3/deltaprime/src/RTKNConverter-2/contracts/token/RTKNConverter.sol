...
contract RTKNConverter is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    error InvalidUSDCBalanceAfterSwap();
    ...
    function _executeParaSwapSwap(
        ...
        uint256 initialUSDCBalance = usdcToken.balanceOf(address(this));
        
        // Record initial target token balance
        ...
        // Verify USDC balance after swap
        uint256 finalUSDCBalance = usdcToken.balanceOf(address(this));
        uint256 expectedUSDCBalance = initialUSDCBalance - usdcAmount;
        if (finalUSDCBalance != expectedUSDCBalance) revert InvalidUSDCBalanceAfterSwap();
        
        // Calculate received amount of target token
