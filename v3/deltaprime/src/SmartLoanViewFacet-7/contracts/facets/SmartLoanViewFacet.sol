...
// Last deployed from commit: 932ac864b406e918dbbc1c3171233dbe7c466cbc;
...
contract SmartLoanViewFacet is SolvencyMethods, GmxV2FeesHelper {
    ...
    **/
    ...
    function _getGmTokenBalanceAfterFees(address gmToken) internal view returns (uint256) {
        ...
        if (tokenManager.isGmxPlusMarket(gmToken)) {
            // For GM plus markets, long and short tokens are the same
            // Only request prices for unique symbols to avoid duplicate requests
            tokenSymbols = new bytes32[](2);
            tokenSymbols[0] = tokenManager.tokenAddressToSymbol(gmToken);
            tokenSymbols[1] = tokenManager.tokenAddressToSymbol(longToken); // same as shortToken
            
            uint256[] memory prices = getPrices(tokenSymbols);
            
            // Create 3-element array for _getDeductibleFeesInGmTokens call
            tokenPrices = new uint256[](3);
            tokenPrices[0] = prices[0]; // gmToken price
            tokenPrices[1] = prices[1]; // longToken price
            tokenPrices[2] = 0; // shortToken price set to 0 to avoid double counting
        } else {
            // For regular GM markets, long and short tokens are different
            tokenSymbols = new bytes32[](3);
            tokenSymbols[0] = tokenManager.tokenAddressToSymbol(gmToken);
        ...
        uint256 feeInGmTokens = _getDeductibleFeesInGmTokens(gmToken, tokenPrices[0], tokenPrices[1], tokenPrices[2]);
