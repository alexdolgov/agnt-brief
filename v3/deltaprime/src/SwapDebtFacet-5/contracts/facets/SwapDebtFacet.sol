...
contract SwapDebtFacet is ReentrancyGuardKeccak, OnlyOwnerOrInsolvent, ParaSwapHelper {
     ...
     * @param receivedRepayTokenAmount The total amount received after swap
     */
  ...
  /**
     ...
     */    
     function swapDebtParaSwap(bytes32 _fromAsset, bytes32 _toAsset, uint256 _repayAmount, uint256 _borrowAmount, bytes4 selector, bytes calldata data) external onlyOwner remainsSolvent nonReentrant {
        ...
        InitialTokenAmounts memory ITA = InitialTokenAmounts({
            fromTokenAmount: fromToken.balanceOf(address(this)),
            toTokenAmount: toToken.balanceOf(address(this))
        });
        ...
        _processRepay(tokenManager, fromAssetPool, address(fromToken), _repayAmount, fromToken.balanceOf(address(this)) - ITA.fromTokenAmount);

        {
            uint256 currentToAssetAmount = toToken.balanceOf(address(this));
            if (currentToAssetAmount > ITA.toTokenAmount) {
                _increaseExposure(tokenManager, address(toToken), currentToAssetAmount - ITA.toTokenAmount);
            } else {
                _decreaseExposure(tokenManager, address(toToken), ITA.toTokenAmount - currentToAssetAmount);
            }
        }
