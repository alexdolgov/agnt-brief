...
// Last deployed from commit: 314bfd56b3227d132238b059e73464e7b574d925;
...
contract TokenManager is OwnableUpgradeable {
    ...
    mapping(address => bool) private isUnsupportedWithdrawalAsset;
    ...
    * Sets the address of the vPrimeController contract
    ...
    function addUnsupportedWithdrawableAssets(address[] memory assets) external onlyOwner {
        for (uint256 i = 0; i < assets.length; i++) {
            ...
            require(!isUnsupportedWithdrawalAsset[assets[i]], "Asset already added");
            
            unsupportedWithdrawableAssets.push(assets[i]);
            isUnsupportedWithdrawalAsset[assets[i]] = true;
    ...
    function removeunsupportedWithdrawableAssets(address[] memory assets) external onlyOwner {
        for (uint256 i = 0; i < assets.length; i++) {
            require(isUnsupportedWithdrawalAsset[assets[i]], "Asset not in list");
            ...
            isUnsupportedWithdrawalAsset[assets[i]] = false;
