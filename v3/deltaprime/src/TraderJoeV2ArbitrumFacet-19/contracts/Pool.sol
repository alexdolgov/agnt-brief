...
contract Pool is PendingOwnableUpgradeable, ReentrancyGuardUpgradeable, IERC20, ProxyConnector {
    ...
    function setTokenManager(ITokenManager _tokenManager) external onlyOwner  {
     ...
     **/
    function deposit(uint256 _amount) external  nonReentrant virtual {
     ...
     **/
    function borrow(uint256 _amount) external virtual canBorrow nonReentrant  {
        if (_amount > IERC20(tokenAddress).balanceOf(address(this)))
            revert InsufficientPoolFunds();
