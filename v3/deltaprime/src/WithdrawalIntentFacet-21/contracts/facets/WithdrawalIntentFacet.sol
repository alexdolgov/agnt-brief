...
contract WithdrawalIntentFacet is IWithdrawalIntentFacet, ReentrancyGuardKeccak, SolvencyMethods {
    ...
    function _getGmxTokenPrices(address gmToken) internal returns (uint256 gmTokenPrice, uint256 longTokenPrice, uint256 shortTokenPrice) {
        ...
        return (tokenPrices[0], tokenPrices[1], tokenPrices[2]);
    }
    ...
    function executeWithdrawalIntent(bytes32 _asset, uint256[] calldata intentIndices) external onlyOwner noBorrowInTheSameBlock nonReentrant canRepayDebtFully remainsSolvent {
        ...
        if (tokenManager.isGmxMarketWhitelisted(tokenAddress) ) {
            (gmTokenPrice, longTokenPrice, shortTokenPrice) = _getGmxTokenPrices(tokenAddress);
            uint256 feeInGmTokens = GmxV2FeesLib._sweepFees(tokenAddress, gmTokenPrice, longTokenPrice, shortTokenPrice);
        ...
        GmxV2FeesLib._updateBenchmark(tokenAddress, gmTokenPrice, longTokenPrice, shortTokenPrice);
