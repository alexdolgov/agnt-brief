...
contract WithdrawalIntentFacet is IWithdrawalIntentFacet, ReentrancyGuardKeccak, GmxV2FeesHelper, GlvHelper {
    ...
    /**
     * @dev L-04 fix: allow whitelisted liquidators to cancel a PA's withdrawal intents
     * once an insolvency snapshot is in place. This is operationally needed so that
     * intents whose underlying token amounts are no longer available cannot remain as
     * stale obligations blocking liquidation accounting. Owners retain unrestricted
     * cancel rights as before.
     */
