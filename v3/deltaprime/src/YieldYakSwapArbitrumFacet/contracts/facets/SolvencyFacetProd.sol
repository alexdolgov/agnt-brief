...
abstract contract SolvencyFacetProd is RedstoneConsumerNumericBase, DiamondHelper {
    ...
    **/
    function _getTWVOwnedAssets(AssetPrice[] memory ownedAssetsPrices) internal virtual view returns (uint256) {
        ...
        uint256 weightedValueOfTokens = ownedAssetsPrices[0].price * (address(this).balance - msg.value) * tokenManager.tieredDebtCoverage(DiamondStorageLib.getPrimeLeverageTier(),tokenManager.getAssetAddress(nativeTokenSymbol, true)) / (10 ** 26);
