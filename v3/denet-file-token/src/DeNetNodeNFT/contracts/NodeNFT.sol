...
contract DeNetNodeNFT is SimpleMetaData, PoSAdmin, IDeNetNodeNFT {    
    ...
    /**
        ...
        example: nodeId = 10, gastoken required 10 * 1e18.
    */
    function isAddressHaveGastokenToBeNode(address nodeAddress, uint nodeId) public view returns (bool) {
        ...
        return gastokenBalance + rentGasTokenBalance >= nodeId * 1e18;
    }
    ...
    */
    ...
    function restoreFromOld(address oldNFT) external onlyOwner {
        if (oldNFT != address(0)) {
            IDeNetNodeNFT oldContract = IDeNetNodeNFT(oldNFT);
            ISimpleINFT oldOwner = ISimpleINFT(oldNFT);

            uint nodesCount = oldContract.totalSupply();
            
            for (uint _tmpId = 1; _tmpId <= nodesCount; _tmpId++) {
                address _tmpOwner = oldOwner.ownerOf(_tmpId);
                _mint(_tmpOwner, nextNodeID);
                nodeByAddress[_tmpOwner] = nextNodeID;
                nextNodeID += 1;
            }
        }
    }
    ...
    */
    ...
    function getNodesRow(uint _startId, uint _endId) external override view returns(DeNetNode[] memory) {
        DeNetNode[] memory  _returns;
        for (uint i = _startId; i < _endId; i++) {
            _returns[i % (_endId - _startId)] = nodeInfo(i);
        }
        return _returns;
    }
}