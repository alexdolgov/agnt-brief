...
PRECISION_MUL: constant(uint256[N_COINS]) = [10**0, 10**0,10**0]
...
dao: public(address) 
team_address: public(address)
send_reward_to_dao: public(bool)

@external
def __init__(
    _owner: address,
    _coins: address[N_COINS],
    _c_tokens: address[N_COINS],
    _pool_token: address,
    _A: uint256,
    _fee: uint256,
    _admin_fee: uint256,
    ...
    _dao: address,
    _team_address: address,
    _send_reward_to_dao: bool
):
    """
    @notice Contract constructor
    @param _owner Contract owner address
    @param _coins Addresses of ERC20 conracts of coins
    @param _c_tokens Addresses of cToken of coins
    ...
    self.dao = _dao
    self.team_address = _team_address
    self.send_reward_to_dao = _send_reward_to_dao
...
def _donate_dao():
    ...
    for i in range(N_COINS - 1):
        ...
        if cERC20(self.c_tokens[index]).balanceOf(self) * cERC20(self.c_tokens[index]).exchangeRateStored() / PRECISION > self.balances[index]:
            ...
            x: uint256 = xp[index] + (dx * rates[index] / PRECISION)
            y: uint256 = self.get_y(index, 0, x, xp)
            if xp[0] > y + 1: 
                dy: uint256 = (xp[0] - (y + 1)) * PRECISION / rates[0]
                # write
                self.balances[0] -= dy
                self.balances[index] += dx
...
def remove_liquidity_imbalance(amounts: uint256[N_COINS], max_burn_amount: uint256, donate: bool):
    ...
    for i in range(N_COINS):
        if amounts[i] != 0:
            ...
            ok: uint256 = cERC20(self.c_tokens[i]).redeemUnderlying(amounts[i])
            if ok > 0:
                raise "Could not redeem coin"
...
def remove_liquidity_one_coin(_token_amount: uint256, i: int128, min_amount: uint256, donate: bool):
    ...
    ok: uint256 = cERC20(self.c_tokens[i]).redeemUnderlying(dy)
    if ok > 0:
        raise "Could not redeem coin"
...
@external
def set_reward_info(new_team_address: address, new_dao: address, new_send_reward_to_dao: bool):
    assert msg.sender == self.owner
    self.team_address = new_team_address
    self.dao = new_dao
