...
library SafeERC20 {
    ...
    /**
     * @dev Imitates a Solidity high-level call (GatewayIndex.e. a regular function call to a contract), relaxing the requirement
...
// File contracts/ichain/gateway/GatewayIndex.sol
...
// File contracts/ichain/gateway/IGateway.sol
...
// File contracts/ichain/liqclaim/ILiqClaim.sol
...
// File contracts/ichain/swapper/ISwapper.sol
...
// File contracts/ichain/token/IDToken.sol
...
// File contracts/ichain/token/IIOU.sol
...
// File contracts/ichain/vault/IVault.sol
...
// File contracts/library/Bytes32.sol
...
// File contracts/library/Bytes32Map.sol
...
// File contracts/utils/IImplementation.sol
...
// File contracts/ichain/gateway/GatewayHelper.sol
...
library GatewayHelper {
    ...
    {
        s.cumulativePnlOnGateway = gatewayStates.getInt(GatewayIndex.S_CUMULATIVEPNLONGATEWAY);
        s.liquidityTime = gatewayStates.getUint(GatewayIndex.S_LIQUIDITYTIME);
        s.totalLiquidity = gatewayStates.getUint(GatewayIndex.S_TOTALLIQUIDITY);
        s.cumulativeTimePerLiquidity = gatewayStates.getInt(GatewayIndex.S_CUMULATIVETIMEPERLIQUIDITY);
        s.gatewayRequestId = gatewayStates.getUint(GatewayIndex.S_GATEWAYREQUESTID);
        s.dChainExecutionFeePerRequest = gatewayStates.getUint(GatewayIndex.S_DCHAINEXECUTIONFEEPERREQUEST);
        s.totalIChainExecutionFee = gatewayStates.getUint(GatewayIndex.S_TOTALICHAINEXECUTIONFEE);
        s.cumulativeCollectedProtocolFee = gatewayStates.getUint(GatewayIndex.S_CUMULATIVECOLLECTEDPROTOCOLFEE);
    }
    ...
    {
        s.vault = bTokenStates[bToken].getAddress(GatewayIndex.B_VAULT);
        s.oracleId = bTokenStates[bToken].getBytes32(GatewayIndex.B_ORACLEID);
        s.collateralFactor = bTokenStates[bToken].getUint(GatewayIndex.B_COLLATERALFACTOR);
    }
    ...
    {
        s.requestId = dTokenStates[lTokenId].getUint(GatewayIndex.D_REQUESTID);
        s.bToken = dTokenStates[lTokenId].getAddress(GatewayIndex.D_BTOKEN);
        s.bAmount = IVault(bTokenStates[s.bToken].getAddress(GatewayIndex.B_VAULT)).getBalance(lTokenId);
        s.b0Amount = dTokenStates[lTokenId].getInt(GatewayIndex.D_B0AMOUNT);
        s.lastCumulativePnlOnEngine = dTokenStates[lTokenId].getInt(GatewayIndex.D_LASTCUMULATIVEPNLONENGINE);
        s.liquidity = dTokenStates[lTokenId].getUint(GatewayIndex.D_LIQUIDITY);
        s.cumulativeTime = dTokenStates[lTokenId].getUint(GatewayIndex.D_CUMULATIVETIME);
        s.lastCumulativeTimePerLiquidity = dTokenStates[lTokenId].getUint(GatewayIndex.D_LASTCUMULATIVETIMEPERLIQUIDITY);
        s.lastRequestIChainExecutionFee = dTokenStates[lTokenId].getUint(GatewayIndex.D_LASTREQUESTICHAINEXECUTIONFEE);
        s.cumulativeUnusedIChainExecutionFee = dTokenStates[lTokenId].getUint(GatewayIndex.D_CUMULATIVEUNUSEDICHAINEXECUTIONFEE);
    }
    ...
    {
        s.requestId = dTokenStates[pTokenId].getUint(GatewayIndex.D_REQUESTID);
        s.bToken = dTokenStates[pTokenId].getAddress(GatewayIndex.D_BTOKEN);
        s.bAmount = IVault(bTokenStates[s.bToken].getAddress(GatewayIndex.B_VAULT)).getBalance(pTokenId);
        s.b0Amount = dTokenStates[pTokenId].getInt(GatewayIndex.D_B0AMOUNT);
        s.lastCumulativePnlOnEngine = dTokenStates[pTokenId].getInt(GatewayIndex.D_LASTCUMULATIVEPNLONENGINE);
        s.singlePosition = dTokenStates[pTokenId].getBool(GatewayIndex.D_SINGLEPOSITION);
        s.lastRequestIChainExecutionFee = dTokenStates[pTokenId].getUint(GatewayIndex.D_LASTREQUESTICHAINEXECUTIONFEE);
        s.cumulativeUnusedIChainExecutionFee = dTokenStates[pTokenId].getUint(GatewayIndex.D_CUMULATIVEUNUSEDICHAINEXECUTIONFEE);
    }
    ...
    {
        uint256 liquidityTime = gatewayStates.getUint(GatewayIndex.S_LIQUIDITYTIME);
        uint256 totalLiquidity = gatewayStates.getUint(GatewayIndex.S_TOTALLIQUIDITY);
        cumulativeTimePerLiquidity = gatewayStates.getUint(GatewayIndex.S_CUMULATIVETIMEPERLIQUIDITY);
        uint256 liquidity = dTokenStates[lTokenId].getUint(GatewayIndex.D_LIQUIDITY);
        cumulativeTime = dTokenStates[lTokenId].getUint(GatewayIndex.D_CUMULATIVETIME);
        uint256 lastCumulativeTimePerLiquidity = dTokenStates[lTokenId].getUint(GatewayIndex.D_LASTCUMULATIVETIMEPERLIQUIDITY);
    ...
    {
        fees = new uint256[](5);
        fees[0] = executionFees[GatewayIndex.ACTION_REQUESTADDLIQUIDITY];
        fees[1] = executionFees[GatewayIndex.ACTION_REQUESTREMOVELIQUIDITY];
        fees[2] = executionFees[GatewayIndex.ACTION_REQUESTREMOVEMARGIN];
        fees[3] = executionFees[GatewayIndex.ACTION_REQUESTTRADE];
        fees[4] = executionFees[GatewayIndex.ACTION_REQUESTTRADEANDREMOVEMARGIN];
    }
    ...
    {
        if (bTokenStates[bToken].getAddress(GatewayIndex.B_VAULT) != address(0)) {
        ...
        bTokenStates[bToken].set(GatewayIndex.B_VAULT, vault);
        bTokenStates[bToken].set(GatewayIndex.B_ORACLEID, oracleId);
        bTokenStates[bToken].set(GatewayIndex.B_COLLATERALFACTOR, collateralFactor);
    ...
    ) external
    {
        ...
        if (IVault(bTokenStates[bToken].getAddress(GatewayIndex.B_VAULT)).stTotalAmount() != 0) {
            revert CannotDelBToken();
        }

        bTokenStates[bToken].del(GatewayIndex.B_VAULT);
        bTokenStates[bToken].del(GatewayIndex.B_ORACLEID);
        bTokenStates[bToken].del(GatewayIndex.B_COLLATERALFACTOR);
    ...
    {
        gatewayStates.set(GatewayIndex.S_DCHAINEXECUTIONFEEPERREQUEST, dChainExecutionFeePerRequest);
    }
    ...
    {
        tokenETH.transferOut(to, tokenETH.balanceOfThis() - gatewayStates.getUint(GatewayIndex.S_TOTALICHAINEXECUTIONFEE));
    }
    ...
    ) external
    {
        ...
        uint256 cumulativeUnusedIChainExecutionFee = dTokenStates[dTokenId].getUint(GatewayIndex.D_CUMULATIVEUNUSEDICHAINEXECUTIONFEE);
        if (cumulativeUnusedIChainExecutionFee > 0) {
            uint256 totalIChainExecutionFee = gatewayStates.getUint(GatewayIndex.S_TOTALICHAINEXECUTIONFEE);
            totalIChainExecutionFee -= cumulativeUnusedIChainExecutionFee;
            gatewayStates.set(GatewayIndex.S_TOTALICHAINEXECUTIONFEE, totalIChainExecutionFee);

            dTokenStates[dTokenId].del(GatewayIndex.D_CUMULATIVEUNUSEDICHAINEXECUTIONFEE);
    ...
    function finishCollectProtocolFee(
        ...
        uint256 cumulativeCollectedProtocolFeeOnGateway = gatewayStates.getUint(GatewayIndex.S_CUMULATIVECOLLECTEDPROTOCOLFEE);
        if (cumulativeCollectedProtocolFeeOnEngine > cumulativeCollectedProtocolFeeOnGateway) {
            ...
            if (amount > 0) {
                ...
                gatewayStates.set(GatewayIndex.S_CUMULATIVECOLLECTEDPROTOCOLFEE, cumulativeCollectedProtocolFeeOnGateway);
...
// File contracts/utils/ISwitchOracle.sol
...
// File contracts/ichain/gateway/GatewayImplementation.sol
...
contract GatewayImplementation is GatewayStorage {
    ...
    function requestAddLiquidity(uint256 lTokenId, address bToken, uint256 bAmount) external payable {
        ...
        uint256 ethAmount = _receiveExecutionFee(lTokenId, _executionFees[GatewayIndex.ACTION_REQUESTADDLIQUIDITY]);
        if (bToken == tokenETH) {
            bAmount = ethAmount;
        }
        if (bAmount == 0) {
    ...
    function requestRemoveLiquidity(uint256 lTokenId, address bToken, uint256 bAmount) external payable {
        ...
        _receiveExecutionFee(lTokenId, _executionFees[GatewayIndex.ACTION_REQUESTREMOVELIQUIDITY]);
        if (bAmount == 0) {
            revert InvalidBAmount();
        }
    ...
    function requestAddMargin(uint256 pTokenId, address bToken, uint256 bAmount, bool singlePosition) public payable returns (uint256) {
        if (pTokenId == 0) {
            ...
            if (singlePosition) {
                _dTokenStates[pTokenId].set(GatewayIndex.D_SINGLEPOSITION, true);
            }
        } else {
    ...
    function requestRemoveMargin(uint256 pTokenId, address bToken, uint256 bAmount) external payable {
        ...
        _receiveExecutionFee(pTokenId, _executionFees[GatewayIndex.ACTION_REQUESTREMOVEMARGIN]);
        if (bAmount == 0) {
            revert InvalidBAmount();
        }
    ...
    function requestTrade(uint256 pTokenId, bytes32 symbolId, int256[] calldata tradeParams) public payable {
        ...
        _receiveExecutionFee(pTokenId, _executionFees[GatewayIndex.ACTION_REQUESTTRADE]);

        Data memory data = _getData(msg.sender, pTokenId, _dTokenStates[pTokenId].getAddress(GatewayIndex.D_BTOKEN));
        _getExParams(data);
    ...
    function requestLiquidate(uint256 pTokenId) external {
        Data memory data = _getData(pToken.ownerOf(pTokenId), pTokenId, _dTokenStates[pTokenId].getAddress(GatewayIndex.D_BTOKEN));
        _getExParams(data);
        uint256 realMoneyMargin = _getDTokenLiquidity(data);
    ...
    function requestAddMarginAndTrade(
        ...
        if (bToken == tokenETH) {
            uint256 executionFee = _executionFees[GatewayIndex.ACTION_REQUESTTRADE];
    ...
    function requestTradeAndRemoveMargin(
        ...
        _receiveExecutionFee(pTokenId, _executionFees[GatewayIndex.ACTION_REQUESTTRADEANDREMOVEMARGIN]);
        if (bAmount == 0) {
            revert InvalidBAmount();
        }
    ...
    function finishUpdateLiquidity(bytes memory eventData, bytes memory signature) external _reentryLock_ {
        ...
        Data memory data = _getData(lToken.ownerOf(v.lTokenId), v.lTokenId, _dTokenStates[v.lTokenId].getAddress(GatewayIndex.D_BTOKEN));
    ...
    function finishRemoveMargin(bytes memory eventData, bytes memory signature) external _reentryLock_ {
        ...
        // Cumulate unsettled PNL to b0Amount
        Data memory data = _getData(pToken.ownerOf(v.pTokenId), v.pTokenId, _dTokenStates[v.pTokenId].getAddress(GatewayIndex.D_BTOKEN));
        int256 diff = v.cumulativePnlOnEngine.minusUnchecked(data.lastCumulativePnlOnEngine);
        data.b0Amount += diff.rescaleDown(18, decimalsB0);
        data.lastCumulativePnlOnEngine = v.cumulativePnlOnEngine;
    ...
    function finishLiquidate(bytes memory eventData, bytes memory signature) external _reentryLock_ {
        ...
        {
            uint256 lastRequestIChainExecutionFee = _dTokenStates[v.pTokenId].getUint(GatewayIndex.D_LASTREQUESTICHAINEXECUTIONFEE);
            uint256 cumulativeUnusedIChainExecutionFee = _dTokenStates[v.pTokenId].getUint(GatewayIndex.D_CUMULATIVEUNUSEDICHAINEXECUTIONFEE);
            _dTokenStates[v.pTokenId].del(GatewayIndex.D_LASTREQUESTICHAINEXECUTIONFEE);
            _dTokenStates[v.pTokenId].del(GatewayIndex.D_CUMULATIVEUNUSEDICHAINEXECUTIONFEE);

            uint256 totalIChainExecutionFee = _gatewayStates.getUint(GatewayIndex.S_TOTALICHAINEXECUTIONFEE);
            totalIChainExecutionFee -= lastRequestIChainExecutionFee + cumulativeUnusedIChainExecutionFee;
            _gatewayStates.set(GatewayIndex.S_TOTALICHAINEXECUTIONFEE, totalIChainExecutionFee);
        }
    ...
    function _getData(address account, uint256 dTokenId, address bToken) internal view returns (Data memory data) {
        ...
        data.cumulativePnlOnGateway = _gatewayStates.getInt(GatewayIndex.S_CUMULATIVEPNLONGATEWAY);
        data.vault = _bTokenStates[bToken].getAddress(GatewayIndex.B_VAULT);

        data.b0Amount = _dTokenStates[dTokenId].getInt(GatewayIndex.D_B0AMOUNT);
        data.lastCumulativePnlOnEngine = _dTokenStates[dTokenId].getInt(GatewayIndex.D_LASTCUMULATIVEPNLONENGINE);

        _checkBTokenConsistency(dTokenId, bToken);
    }

    function _saveData(Data memory data) internal {
        _gatewayStates.set(GatewayIndex.S_CUMULATIVEPNLONGATEWAY, data.cumulativePnlOnGateway);
        _dTokenStates[data.dTokenId].set(GatewayIndex.D_BTOKEN, data.bToken);
        _dTokenStates[data.dTokenId].set(GatewayIndex.D_B0AMOUNT, data.b0Amount);
        _dTokenStates[data.dTokenId].set(GatewayIndex.D_LASTCUMULATIVEPNLONENGINE, data.lastCumulativePnlOnEngine);
    }
    ...
    function _checkRequestId(uint256 dTokenId, uint256 requestId) internal {
        ...
        if (_dTokenStates[dTokenId].getUint(GatewayIndex.D_REQUESTID) != uint256(userRequestId)) {
            revert InvalidRequestId();
        } else {
            // increment requestId so that callback can only be executed once
            _dTokenStates[dTokenId].set(GatewayIndex.D_REQUESTID, uint256(userRequestId + 1));
        }
    }
    ...
    function _incrementRequestId(uint256 dTokenId) internal returns (uint256) {
        uint128 gatewayRequestId = uint128(_gatewayStates.getUint(GatewayIndex.S_GATEWAYREQUESTID));
        gatewayRequestId += 1;
        _gatewayStates.set(GatewayIndex.S_GATEWAYREQUESTID, uint256(gatewayRequestId));

        uint128 userRequestId = uint128(_dTokenStates[dTokenId].getUint(GatewayIndex.D_REQUESTID));
        userRequestId += 1;
        _dTokenStates[dTokenId].set(GatewayIndex.D_REQUESTID, uint256(userRequestId));
    ...
    function _checkBTokenInitialized(address bToken) internal view {
        if (_bTokenStates[bToken].getAddress(GatewayIndex.B_VAULT) == address(0)) {
            revert InvalidBToken();
        }
    }

    function _checkBTokenConsistency(uint256 dTokenId, address bToken) internal view {
        address preBToken = _dTokenStates[dTokenId].getAddress(GatewayIndex.D_BTOKEN);
        if (preBToken != address(0) && preBToken != bToken) {
            uint256 stAmount = IVault(_bTokenStates[preBToken].getAddress(GatewayIndex.B_VAULT)).stAmounts(dTokenId);
            if (stAmount != 0) {
                revert InvalidBToken();
            }
        }
    ...
    function _receiveExecutionFee(uint256 dTokenId, uint256 executionFee) internal returns (uint256) {
        uint256 dChainExecutionFee = _gatewayStates.getUint(GatewayIndex.S_DCHAINEXECUTIONFEEPERREQUEST);
        if (msg.value < executionFee) {
            revert InsufficientExecutionFee();
        }
        uint256 iChainExecutionFee = executionFee - dChainExecutionFee;

        uint256 totalIChainExecutionFee = _gatewayStates.getUint(GatewayIndex.S_TOTALICHAINEXECUTIONFEE) + iChainExecutionFee;
        _gatewayStates.set(GatewayIndex.S_TOTALICHAINEXECUTIONFEE,  totalIChainExecutionFee);

        uint256 lastRequestIChainExecutionFee = _dTokenStates[dTokenId].getUint(GatewayIndex.D_LASTREQUESTICHAINEXECUTIONFEE);
        uint256 cumulativeUnusedIChainExecutionFee = _dTokenStates[dTokenId].getUint(GatewayIndex.D_CUMULATIVEUNUSEDICHAINEXECUTIONFEE);
        cumulativeUnusedIChainExecutionFee += lastRequestIChainExecutionFee;
        lastRequestIChainExecutionFee = iChainExecutionFee;
        _dTokenStates[dTokenId].set(GatewayIndex.D_LASTREQUESTICHAINEXECUTIONFEE, lastRequestIChainExecutionFee);
        _dTokenStates[dTokenId].set(GatewayIndex.D_CUMULATIVEUNUSEDICHAINEXECUTIONFEE, cumulativeUnusedIChainExecutionFee);
    ...
    function _transferLastRequestIChainExecutionFee(uint256 dTokenId, address to) internal {
        ...
        if (lastRequestIChainExecutionFee > 0) {
            ...
            _dTokenStates[dTokenId].del(GatewayIndex.D_LASTREQUESTICHAINEXECUTIONFEE);
    ...
    function _getBPrice(address bToken) internal view returns (uint256 bPrice) {
        if (bToken == tokenB0) {
            ...
            bPrice = oracle.getValue(_bTokenStates[bToken].getBytes32(GatewayIndex.B_ORACLEID)).itou().rescale(decimalsB, decimalsB0);
            if (bPrice == 0) {
    ...
    function _getExParams(Data memory data) internal view {
        data.collateralFactor = _bTokenStates[data.bToken].getUint(GatewayIndex.B_COLLATERALFACTOR);
    ...
    function _updateLiquidity(uint256 lTokenId, uint256 newLiquidity, uint256 newTotalLiquidity) internal {
        ...
        _gatewayStates.set(GatewayIndex.S_LIQUIDITYTIME, block.timestamp);
        _gatewayStates.set(GatewayIndex.S_TOTALLIQUIDITY, newTotalLiquidity);
        _gatewayStates.set(GatewayIndex.S_CUMULATIVETIMEPERLIQUIDITY, cumulativeTimePerLiquidity);
        _dTokenStates[lTokenId].set(GatewayIndex.D_LIQUIDITY, newLiquidity);
        _dTokenStates[lTokenId].set(GatewayIndex.D_CUMULATIVETIME, cumulativeTime);
        _dTokenStates[lTokenId].set(GatewayIndex.D_LASTCUMULATIVETIMEPERLIQUIDITY, cumulativeTimePerLiquidity);
    }
