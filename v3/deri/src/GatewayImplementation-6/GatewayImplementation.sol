...
contract GatewayImplementation is GatewayStorage {
    ...
    {
        uint256 liquidityTime = _gatewayStates.getUint(GatewayIndex.S_LIQUIDITYTIME);
        uint256 totalLiquidity = _gatewayStates.getUint(GatewayIndex.S_TOTALLIQUIDITY);
        cumulativeTimePerLiquidity = _gatewayStates.getUint(GatewayIndex.S_CUMULATIVETIMEPERLIQUIDITY);
        uint256 liquidity = _dTokenStates[lTokenId].getUint(GatewayIndex.D_LIQUIDITY);
        cumulativeTime = _dTokenStates[lTokenId].getUint(GatewayIndex.D_CUMULATIVETIME);
        uint256 lastCumulativeTimePerLiquidity = _dTokenStates[lTokenId].getUint(GatewayIndex.D_LASTCUMULATIVETIMEPERLIQUIDITY);
