...
// File contracts/reward/RewardVaultStorage.sol
...
interface IDToken is IERC721 {
    ...
    function BASE_TOKENID() external view returns (uint256);

}


// File contracts/reward/RewardVaultImplementation.sol
...
contract RewardVaultImplementation is RewardVaultStorage {
    ...
    event SetRewardPerSecond(address indexed gateway, uint256 newRewardPerSecond);
    ...
    event AddGateway(address indexed gateway);
    ...
    //  ========== ADMIN ==============
    // Initialize new pool
    function initialize(address _gateway, uint256 rewardPerSecond) external _onlyAdmin_ {
        require(!authorizedPool[_gateway], "reward: initialized");
        IGateway.GatewayParam memory p = IGateway(_gateway).getGatewayParam();
        uint256 totalMinted = uint256(IDToken(p.lToken).totalMinted());
        uint256 BASE_TOKENID = IDToken(p.lToken).BASE_TOKENID();

        for (uint256 tokenId = BASE_TOKENID + 1; tokenId <= BASE_TOKENID + totalMinted; tokenId++) {
            UserInfo storage user = userInfo[_gateway][tokenId];
            (, uint256 cumulativeTime) = IGateway(_gateway).getCumulativeTime(tokenId);
            user.lastCumulativeTime = cumulativeTime;
        }
        
        authorizedPool[_gateway] = true;
        gateways.push(_gateway);
        rewardPerSeconds[_gateway] = rewardPerSecond;

        emit AddGateway(_gateway);
    }
    ...
    function setRewardPerSecond(
        ...
        require(authorizedPool[_gateway], "reward: not initialized");
        require(rewardPerSeconds[_gateway] != newRewardPerSecond, "reward: unchanged rewardPerSecond");
        IGateway.GatewayParam memory p = IGateway(_gateway).getGatewayParam();
        uint256 totalMinted = uint256(IDToken(p.lToken).totalMinted());
        uint256 BASE_TOKENID = IDToken(p.lToken).BASE_TOKENID();
        uint256 rewardPerSecond = rewardPerSeconds[_gateway];

        for (uint256 tokenId = BASE_TOKENID + 1; tokenId <= BASE_TOKENID + totalMinted; tokenId++) {
            UserInfo storage user = userInfo[_gateway][tokenId];
            (, uint256 cumulativeTime) = IGateway(_gateway).getCumulativeTime(tokenId);
            uint256 rewardAmount = (cumulativeTime - user.lastCumulativeTime) * rewardPerSecond / UONE;
        
            user.unclaimed += rewardAmount;
            user.lastCumulativeTime = cumulativeTime;
        }

        rewardPerSeconds[_gateway] = newRewardPerSecond;
        emit SetRewardPerSecond(_gateway, newRewardPerSecond);
    }

    function claim(address _gateway, uint256 lTokenId) public _reentryLock_ {
        _checkLTokenIdOwner(_gateway, lTokenId, msg.sender);
        require(authorizedPool[_gateway], "reward: invalid gateway");
        UserInfo storage user = userInfo[_gateway][lTokenId];
        uint256 rewardPerSecond = rewardPerSeconds[_gateway];
        (, uint256 cumulativeTime) = IGateway(_gateway).getCumulativeTime(lTokenId);

        uint256 rewardAmount = (cumulativeTime - user.lastCumulativeTime) * rewardPerSecond / UONE + user.unclaimed;
        user.lastCumulativeTime = cumulativeTime;
        user.unclaimed = 0;
        RewardToken.safeTransfer(msg.sender, rewardAmount);
        emit Claim(_gateway, lTokenId, rewardAmount);
    }

    function pending(address _gateway, uint256 lTokenId) public view returns (uint256 rewardAmount) {
        UserInfo memory user = userInfo[_gateway][lTokenId];
        (, uint256 cumulativeTime) = IGateway(_gateway).getCumulativeTime(lTokenId);
        uint256 rewardPerSecond = rewardPerSeconds[_gateway];
        rewardAmount = (cumulativeTime - user.lastCumulativeTime) * rewardPerSecond / UONE + user.unclaimed;
    }


    function claim(address _gateway, uint256[] memory lTokenIds) external {
        for (uint256 i = 0; i < lTokenIds.length; i++) {
            uint256 lTokenId = lTokenIds[i];
            claim(_gateway, lTokenId);
        }
    }

    function pending(address _gateway, uint256[] memory lTokenIds) external view returns (uint256 rewardAmount) {
        for (uint256 i = 0; i < lTokenIds.length; i++) {
            uint256 lTokenId = lTokenIds[i];
            rewardAmount += pending(_gateway, lTokenId);
        }
    }

    function totalUnclaimed(address _gateway) external view returns (uint256 totalAmount) {
        IGateway.GatewayParam memory p = IGateway(_gateway).getGatewayParam();
        uint256 totalMinted = uint256(IDToken(p.lToken).totalMinted());
        uint256 BASE_TOKENID = IDToken(p.lToken).BASE_TOKENID();
        uint256 rewardPerSecond = rewardPerSeconds[_gateway];
        for (uint256 tokenId = BASE_TOKENID + 1; tokenId <= BASE_TOKENID + totalMinted; tokenId++) {
            UserInfo storage user = userInfo[_gateway][tokenId];
            (, uint256 cumulativeTime) = IGateway(_gateway).getCumulativeTime(tokenId);
            totalAmount += (cumulativeTime - user.lastCumulativeTime) * rewardPerSecond / UONE + user.unclaimed;
        }
    }


    function _checkLTokenIdOwner(address _gateway, uint256 lTokenId, address owner) internal view {
        IGateway.GatewayParam memory p = IGateway(_gateway).getGatewayParam();
        require(IDToken(p.lToken).ownerOf(lTokenId) == owner, "invalid tokenId");
    }
    
    function emergencyWithdraw(address to) external _onlyAdmin_ {
...
interface IGateway {
    ...
    function getCumulativeTime(uint256 lTokenId) external view returns (uint256 cumulativeTimePerLiquidity, uint256 cumulativeTime);
