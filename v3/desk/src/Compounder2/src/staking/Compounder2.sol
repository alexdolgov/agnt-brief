...
contract Compounder2 is NonblockingLzAppUpgradeable {
  ...
  error Compounder2_NotWhitelisted();
  ...
  modifier onlyWhitelisted() {
    if (!whitelisted[msg.sender]) revert Compounder2_NotWhitelisted();
    _;
  }
  ...
  function compoundFor(
    ...
    _claimAll(forUser, pools, rewarders, startEpochTimestamp, type(uint256).max);
    _compoundOrTransfer(forUser, true);
  }
