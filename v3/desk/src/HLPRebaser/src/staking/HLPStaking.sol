...
import { IDLP } from "src/tokens/interfaces/IDLP.sol";
import { IHLP } from "src/tokens/interfaces/IHLP.sol";
...
contract HLPStaking is ISurgeStaking, OwnableUpgradeable {
  ...
  modifier whenNotPaused() {
    if (paused) revert HLPStaking_ContractPaused();
    _;
  }
  ...
  function delegate(bool allow) external whenNotPaused {
  ...
  function moveStake(address from, address to) external onlyOwner whenNotPaused {
  ...
  function depositSurge(address to, uint256 amount) external whenNotPaused {
  ...
  function _deposit(address to, uint256 amount, bool isSurge) internal {
    ...
    if (isMigrated) revert HLPStaking_AlreadyMigrated();
  ...
  function _withdraw(uint256 amount) internal {
    uint256 amountInput = amount;
    if (surgeRewarder == address(0)) revert HLPStaking_SurgeRewarderNotSet();
    if (isMigrated) {
      amount = getOriginalAmount(amount);
    }
    ...
    if (!isMigrated && userTokenAmount[msg.sender] < userSurgeAmount[msg.sender]) {
    ...
    IERC20Upgradeable(stakingToken).safeTransfer(msg.sender, amountInput);

    emit LogWithdraw(msg.sender, amountInput);
  }

  function harvest(address[] memory _rewarders) external whenNotPaused {
    _harvestFor(msg.sender, msg.sender, _rewarders);
  }

  function harvestToCompounder(address user, address[] memory _rewarders) external whenNotPaused {
  ...
  function calculateTotalShare(address /*rewarder*/) external view returns (uint256) {
    if (isMigrated) {
      address hlpAddress = IDLP(stakingToken).asset();
      uint256 dlpBalance = IERC20Upgradeable(stakingToken).balanceOf(address(this));
      return getOriginalAmount(dlpBalance);
    } else {
  ...
  function setRebaser(address _rebaser) external onlyOwner {
    if (_rebaser == address(0)) revert HLPStaking_InvalidRebaser();
    emit LogSetRebaser(rebaser, _rebaser);
    rebaser = _rebaser;
  }

  function migrateToDLP(address dlpAddress, uint256 rebaseIndex) external {
    if (msg.sender != rebaser) revert HLPStaking_NotRebaser();
    if (stakingToken == dlpAddress) revert HLPStaking_AlreadyMigrated();

    uint256 totalBalanceOfHlp = IHLP(stakingToken).balanceOf(address(this));
    // approve HLP to be spent by DLP
    IERC20Upgradeable(stakingToken).safeApprove(dlpAddress, totalBalanceOfHlp);
    // deposit HLP to DLP
    IDLP(dlpAddress).deposit(totalBalanceOfHlp, address(this));

    stakingToken = dlpAddress;
    isMigrated = true;
    dlpRebaseIndex = rebaseIndex;
    emit LogMigrateToDLP(dlpAddress, rebaseIndex);
  }

  function getRebasedUserTokenAmount(address user) external view returns (uint256) {
    if (isMigrated) {
      return Math.mulDiv(userTokenAmount[user], dlpRebaseIndex, 1e18);
    }
    return userTokenAmount[user];
  }
  ...
  function getOriginalAmount(uint256 amount) public view returns (uint256) {
    if (isMigrated) {
      return Math.mulDiv(amount, 1e18, dlpRebaseIndex);
    }
    return amount;
  }
