...
/// @title TLFDXVester - Vesting contract for TLFDX tokens.
contract TLFDXVester is OwnableUpgradeable, ReentrancyGuardUpgradeable, ILHMXVester {
  ...
  /// @param _tlfdxAddress The address of the TLFDX token contract.
  ...
  /// @dev Claims TLFDX tokens for the sender.
  /// @param amount The amount of TLFDX tokens to claim.
  function claimFor(uint256 amount) external nonReentrant {
    ...
    // Interaction
    // Transfer TLFDX tokens from the user to address(0xdead)
  ...
  /// @dev Allow to set FDX staking after deployment because TLFDXVester is deployed before FDXStaking.
  ...
  function _getUnlockAmount(address account) internal view returns (uint256) {
    // The total unlock amount is calculated based on the elapsed time since TLFDX deployment,
    // starting from 6 months after deployment and divided into 18 equal monthly unlock periods.
    // The unlock amount is determined by multiplying the total TLFDX amount by the elapsed months
    ...
    // Calculate the unlock amount by dividing the total TLFDX amount by 18 and multiplying
