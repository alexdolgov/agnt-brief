...
interface IHMXStaking {
  ...
  function dp() external returns (DragonPoint);

  function reinvestDp(address user) external;

  function withdrawAsMigrator(address user, address stakingToken, uint256 amount) external;

  function dragonPointRewarder() external returns (IRewarder);
}
