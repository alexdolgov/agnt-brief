...
contract Oracle {
    ...
    function consult(address token, uint amountIn) external view returns (uint amountOut) {
        ...
        if (token == info.token0) {
            ...
            priceDecimal = IUniswapV2Pair(info.token1).decimals();
          
        } else {
            require(token == info.token1, "ExampleOracleSimple: INVALID_TOKEN");
            amountOut = info.price1Average.mul(amountIn).decode144();

            priceDecimal = IUniswapV2Pair(info.token0).decimals();
        }

        uint decimalDelta = uint(18).sub(priceDecimal);
        if (decimalDelta > 0) {
            amountOut =  amountOut.mul(10**decimalDelta);
        }
    }
