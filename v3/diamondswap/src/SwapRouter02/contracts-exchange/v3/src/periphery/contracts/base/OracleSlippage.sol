...
import "@exchange-v3/periphery/contracts/base/PeripheryImmutableState.sol";
import "@exchange-v3/periphery/contracts/base/BlockTimestamp.sol";
...
import "@exchange-v3/periphery/contracts/libraries/OracleLibrary.sol";
...
abstract contract OracleSlippage is IOracleSlippage, PeripheryImmutableState, BlockTimestamp {
    ...
    function getBlockStartingAndCurrentTick(IDiamondSwapV3Pool pool)
        internal
        view
    ...
    function getPoolAddress(address tokenA, address tokenB, uint24 fee)
        ...
        view
        virtual
        returns (IDiamondSwapV3Pool pool)
    {
        pool = IDiamondSwapV3Pool(PoolAddress.computeAddress(factory, PoolAddress.getPoolKey(tokenA, tokenB, fee)));
    }
        ...
        returns (int256 syntheticAverageTick, int256 syntheticCurrentTick)
    {
        ...
        for (uint256 i = 0; i < numPools; i++) {
            ...
            IDiamondSwapV3Pool pool = getPoolAddress(tokenIn, tokenOut, fee);
            ...
            if (secondsAgo == 0) {
                ...
                (, currentTick,,,,,) = IDiamondSwapV3Pool(pool).slot0();
            }
