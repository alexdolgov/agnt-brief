...
contract RingAdapter is IDODOAdapter {
    ...
    function sellBase(address to, address pool, bytes memory data) external override {
        ...
        // Wrap ERC20 Token to FewToken
        address token = IFewWrappedToken(baseToken).token();
        uint256 amount = IERC20(token).balanceOf(address(this));
        IERC20(token).approve(baseToken, amount);
        IFewWrappedToken(baseToken).wrapTo(amount, pool);
        ...
        // Unwrap FewToken to ERC20 Token
        IERC20(quoteToken).approve(quoteToken, receiveQuoteAmount);
        IFewWrappedToken(quoteToken).unwrapTo(receiveQuoteAmount, to);
    }
    ...
    function sellQuote(address to, address pool, bytes memory data) external override {
        ...
        // Wrap ERC20 Token to FewToken
        address token = IFewWrappedToken(quoteToken).token();
        uint256 amount = IERC20(token).balanceOf(address(this));
        IERC20(token).approve(quoteToken, amount);
        IFewWrappedToken(quoteToken).wrapTo(amount, pool);
        ...
        // Unwrap FewToken to ERC20 Token
        IERC20(baseToken).approve(baseToken, receiveBaseAmount);
        IFewWrappedToken(baseToken).unwrapTo(receiveBaseAmount, to);
    }
}