...
contract UniV4Adapter is IDODOAdapter, IUnlockCallback {
    ...
    function sellBase(address to, address /*pool*/, bytes memory data) external {
        (address tokenIn, , , , ) = abi.decode(data, (address, address, uint24, int24, uint160));

        uint256 amountIn;
        if (tokenIn == address(0)) {
            amountIn = IERC20(_WETH_).balanceOf(address(this));
            IWETH(_WETH_).withdraw(amountIn);
        } else {
            amountIn = IERC20(tokenIn).balanceOf(address(this));
        }

        bytes memory unlockData = abi.encode(data, amountIn, to);
    ...
    function _uniV4Swap(
        ...
        int256 amountSpecified,
        uint160 sqrtX96
    ) internal returns (uint128 actualAmountIn, uint128 amountOut) {
    ...
    function unlockCallback(bytes calldata data) external returns (bytes memory) {
        ...
        (bytes memory poolData, uint256 amountIn, address to) = abi.decode(data, (bytes, uint256, address));
        (address tokenIn, address tokenOut, uint24 fee, int24 tickSpacing, uint160 sqrtX96) 
            = abi.decode(poolData, (address, address, uint24, int24, uint160));
        ...
        (uint128 actualAmountIn, uint128 amountOut) = _uniV4Swap(
            poolKey,
            zeroForOne,
            -int256(amountIn),
            sqrtX96
        ...
        _settle(Currency.wrap(tokenIn), actualAmountIn);

        if (tokenOut == address(0)) { 
            _take(Currency.wrap(tokenOut), address(this), amountOut); 
            IWETH(_WETH_).deposit{value: amountOut}(); 
            SafeERC20.safeTransfer(IERC20(_WETH_), to, amountOut); 
        } else { 
            _take(Currency.wrap(tokenOut), to, amountOut); 
        }

        // check if use all from tokem
        address fromToken = tokenIn == address(0) ? _WETH_ : tokenIn;
