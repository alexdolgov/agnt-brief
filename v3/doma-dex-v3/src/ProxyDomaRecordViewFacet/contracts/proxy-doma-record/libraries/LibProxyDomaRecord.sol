...
library LibProxyDomaRecord {
    ...
    struct ProxyDomaRecordStorage {
        ...
        /**
         * @dev Per-destination-chain bridge fee configuration.
         */
        mapping(string => BridgeConfig) bridgeConfigs;
        /**
         * @dev Mapping of tokenId to timestamp when tokenization was requested.
         */
        mapping(uint256 => uint256) tokenizationRequestedAt;
        /**
         * @dev Mapping of tokenId to timestamp when claim ownership was requested.
         */
        mapping(uint256 => uint256) claimRequestedAt;
        /**
         * @dev Mapping of tokenId to timestamp when detokenization was requested.
         */
        mapping(uint256 => uint256) detokenizationRequestedAt;
    }

    struct BridgeConfig {
        uint256 feeUSDCents;
        uint256 feeDestNative;
        uint8 feeDestNativeDecimals;
        AggregatorV3Interface destOracle;
    }
    ...
    /**
     * @notice Cooldown period for cross-chain operations to prevent duplicate requests.
     */
    uint256 public constant OPERATION_COOLDOWN = 10 minutes;
    ...
    /**
     * @notice Sets the timestamp when tokenization was requested for a given tokenId.
     * @param tokenId The ID of the token for which tokenization was requested.
     * @param timestamp The timestamp to set for the tokenization request.
     */
    function _setTokenizationRequestedAt(uint256 tokenId, uint256 timestamp) internal {
        ProxyDomaRecordStorage storage ds = proxyDomaRecordStorage();
        ds.tokenizationRequestedAt[tokenId] = timestamp;
    }

    /**
     * @notice Gets the timestamp when tokenization was requested for a given tokenId.
     * @param tokenId The ID of the token for which tokenization was requested.
     * @return The timestamp when tokenization was requested.
     */
    function _tokenizationRequestedAt(uint256 tokenId) internal view returns (uint256) {
        ProxyDomaRecordStorage storage ds = proxyDomaRecordStorage();
        return ds.tokenizationRequestedAt[tokenId];
    }

    /**
     * @notice Sets the timestamp when claim ownership was requested for a given tokenId.
     * @param tokenId The ID of the token for which claim was requested.
     * @param timestamp The timestamp to set for the claim request.
     */
    function _setClaimRequestedAt(uint256 tokenId, uint256 timestamp) internal {
        ProxyDomaRecordStorage storage ds = proxyDomaRecordStorage();
        ds.claimRequestedAt[tokenId] = timestamp;
    }

    /**
     * @notice Gets the timestamp when claim ownership was requested for a given tokenId.
     * @param tokenId The ID of the token for which claim was requested.
     * @return The timestamp when claim was requested.
     */
    function _claimRequestedAt(uint256 tokenId) internal view returns (uint256) {
        ProxyDomaRecordStorage storage ds = proxyDomaRecordStorage();
        return ds.claimRequestedAt[tokenId];
    }

    /**
     * @notice Sets the timestamp when detokenization was requested for a given tokenId.
     * @param tokenId The ID of the token for which detokenization was requested.
     * @param timestamp The timestamp to set for the detokenization request.
     */
    function _setDetokenizationRequestedAt(uint256 tokenId, uint256 timestamp) internal {
        ProxyDomaRecordStorage storage ds = proxyDomaRecordStorage();
        ds.detokenizationRequestedAt[tokenId] = timestamp;
    }

    /**
     * @notice Gets the timestamp when detokenization was requested for a given tokenId.
     * @param tokenId The ID of the token for which detokenization was requested.
     * @return The timestamp when detokenization was requested.
     */
    function _detokenizationRequestedAt(uint256 tokenId) internal view returns (uint256) {
        ProxyDomaRecordStorage storage ds = proxyDomaRecordStorage();
        return ds.detokenizationRequestedAt[tokenId];
    }
}
