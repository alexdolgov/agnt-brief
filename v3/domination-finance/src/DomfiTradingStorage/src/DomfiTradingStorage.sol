...
contract DomfiTradingStorage is IDomfiTradingStorage, Initializable {
    ...
    uint256 public oracleFees; // PRECISION_18 (Wei)
    ...
    function handleOpeningFees(
        ...
        uint256 oiLong =
            (openInterest[pairIndex][uint256(OpenInterest.LONG)] * latestPrice) / PRECISION_18
                / 1e12;
    ...
    function handleOracleFee(uint256 amount) external payable onlyTrading returns (uint256) {
        oracleFees += amount;
        return oracleFees;
    }
    ...
    function claimOracleFees(uint256 amount) external onlyRegisteryOrGov {
        if (amount > oracleFees) {
            revert InsufficientOracleFeeBalance();
        }
        ...
        SafeTransferLib.safeTransferETH(registry.dev(), amount);
    }
