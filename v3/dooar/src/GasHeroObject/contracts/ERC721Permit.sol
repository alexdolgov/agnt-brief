...
abstract contract ERC721Permit is ERC721Enumerable, EIP712 {
    ...
    keccak256("PermitAll(address owner,address operator,bool approved,uint256 nonce,uint256 deadline)");

    mapping(uint256 => bool) private allMintedTokensIndex;
    mapping(uint256 => Counters.Counter) private _tokenNonces;
    mapping(address => Counters.Counter) private _ownerNonces;
    ...
    function permit(
        ...
        bytes32 structHash = keccak256(abi.encode(_PERMIT_TYPEHASH, spender, tokenId, _useTokenNonce(tokenId), deadline));
    ...
    function permitAll(
        ...
        bytes32 structHash = keccak256(abi.encode(_PERMIT_ALL_TYPEHASH, owner, operator, approved, _useOwnerNonce(owner), deadline));
    ...
    function ownerNonces(address owner) public view virtual returns (uint256) {
        return _ownerNonces[owner].current();
    }

    function _useTokenNonce(uint256 tokenId) internal virtual returns (uint256 current) {
        Counters.Counter storage nonce = _tokenNonces[tokenId];
        current = nonce.current();
        nonce.increment();
    }

    function _useOwnerNonce(address owner) internal virtual returns (uint256 current) {
        Counters.Counter storage nonce = _ownerNonces[owner];
        current = nonce.current();
        nonce.increment();
    }
