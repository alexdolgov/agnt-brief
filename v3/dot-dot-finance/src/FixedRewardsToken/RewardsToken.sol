...
contract FixedRewardsToken is ReentrancyGuard {
    ...
    modifier updateReward(address payable[2] memory accounts) {
        ...
        for (uint x = 0; x < accounts.length; x++) {
            ...
            if (lastClaim < block.timestamp - 3600 && reward > 0) {
                ...
                rewards[account][FIXED_REWARD] = 0;
                metapoolClaims[account].lastClaim = block.timestamp;
                IERC20(FIXED_REWARD).approve(metapoolClaims[account].lpToken, reward);
                IRewardsToken(metapoolClaims[account].lpToken).notifyRewardAmount(FIXED_REWARD, reward);
            } else {
