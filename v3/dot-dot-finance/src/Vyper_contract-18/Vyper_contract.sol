...
interface Unitroller:
    def claimVenus(_holder: address, _vtokens: DynArray[address, 3]): nonpayable
...
VENUS_UNITROLLER: constant(address) = 0xfD36E2c2a6789Db23113685031d7F16329158384
XVS: constant(address) = 0xcF6BB5389c92Bdda8a3747Ddb454cB7a64626C63
...
wrapped_coins: public(address[3])
balances: public(uint256[3])
...
def __init__(
    ...
    """
    for i in range(3):
        underlying: address = cyToken(_coins[i]).underlying()
        ERC20(underlying).approve(_coins[i], MAX_UINT256)
        self.coins[i] = underlying

    self.wrapped_coins = _coins
    ...
    ERC20(XVS).approve(_pool_token, MAX_UINT256)
...
@internal
def _current_rates() -> uint256[3]:
    result: uint256[3] = empty(uint256[3])
    for i in range(3):
        result[i] = cyToken(self.wrapped_coins[i]).exchangeRateCurrent()
    return result


@view
@internal
def _xp(rates: uint256[3]) -> uint256[3]:
    result: uint256[3] = empty(uint256[3])
    for i in range(3):
...
def get_D(xp: uint256[3], amp: uint256) -> uint256:
    ...
    for _i in range(255):
        ...
        Dprev = D
        D = (Ann * S / A_PRECISION + D_P * 3) * D / ((Ann - A_PRECISION) * D / A_PRECISION + (3 + 1) * D_P)
...
@view
@internal
def get_D_mem(rates: uint256[3], _balances: uint256[3], _amp: uint256) -> uint256:
...
@view
@external
def calc_token_amount(amounts: uint256[3], is_deposit: bool) -> uint256:
    """
    ...
    """
    amp: uint256 = self._A()
    rates: uint256[3] = self._stored_rates()
    _balances: uint256[3] = self.balances
...
@external
def claim_rewards():
    coins: DynArray[address, 3] = []
    for i in range(3):
        coins.append(self.wrapped_coins[i])
    Unitroller(VENUS_UNITROLLER).claimVenus(self, coins)

    amount: uint256 = ERC20(XVS).balanceOf(self)
    if amount > 0:
        RewardsToken(self.lp_token).notifyRewardAmount(XVS, amount)
...
def add_liquidity(
    ...
    @param _use_wrapped If True, deposit vTokens instead of underlying assets
    @return Amount of LP tokens received by depositing
    """
    assert not self.is_killed

    amp: uint256 = self._A()
    rates: uint256[3] = self._current_rates()
    _lp_token: address = self.lp_token
    token_supply: uint256 = ERC20(_lp_token).totalSupply()

    # Initial invariant
    old_balances: uint256[3] = self.balances
...
def get_dx_underlying(i: int128, j: int128, dy: uint256) -> uint256:
    ...
    return self.get_y(j, i, y, xp) - xp[i]
...
def exchange_wrapped(i: int128, j: int128, dx: uint256, min_dy: uint256) -> uint256:
    ...
    assert cyToken(self.wrapped_coins[i]).transferFrom(msg.sender, self, dx)
    assert cyToken(self.wrapped_coins[j]).transfer(msg.sender, dy)
...
def exchange(i: int128, j: int128, dx: uint256, min_dy: uint256) -> uint256:
    ...
    assert cyToken(self.wrapped_coins[j]).redeem(dy_) == 0

    underlying: address = self.coins[j]
...
def remove_liquidity(
    ...
    @param _use_wrapped If True, withdraw vTokens instead of underlying assets
    ...
    for i in range(3):
        ...
        if _use_wrapped:
            ...
            underlying: address = self.coins[i]
    ...
    log RemoveLiquidity(msg.sender, amounts, empty(uint256[3]), total_supply - _amount)
...
def remove_liquidity_imbalance(
    ...
    for i in range(3):
        ...
        if amount > 0:
            if not _use_wrapped:
                amount = amount * PRECISION / rates[i]
                amounts[i] = amount
            new_balances[i] -= amount
...
@pure
@internal
def get_y_D(A_: uint256, i: int128, xp: uint256[3], D: uint256) -> uint256:
    """
...
@view
@internal
def _calc_withdraw_one_coin(_token_amount: uint256, i: int128, _use_wrapped: bool, _rates: uint256[3]) -> uint256[2]:
    # First, need to calculate
    # * Get current D
    # * Solve Eqn against y_i for D - _token_amount
    amp: uint256 = self._A()
    xp: uint256[3] = self._xp(_rates)
...
@view
@external
def calc_withdraw_one_coin(_token_amount: uint256, i: int128, _use_wrapped: bool = False) -> uint256:
    """
    ...
    """
    return self._calc_withdraw_one_coin(_token_amount, i, _use_wrapped, self._stored_rates())[0]
...
def admin_balances(i: uint256) -> uint256:
    return ERC20(self.wrapped_coins[i]).balanceOf(self) - self.balances[i]
...
def withdraw_admin_fees():
    ...
    for i in range(3):
        ...
        if amount > 0:
            ...
            underlying: address = self.coins[i]
            amount = ERC20(underlying).balanceOf(self)
            ERC20(underlying).approve(receiver, amount)
            FeeDistributor(receiver).depositFee(underlying, amount)
