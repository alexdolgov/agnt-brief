    ...
    **/
    function swapTokenForToken(uint256 _id, uint256 _amount) public nonReentrant {
        ...
        if(fee > 0){
            ...
            require(toSend > 0, 'invalidly low amount');
            require(IERC20(_pair.t2).allowance(msg.sender, address(this)) >= toSend, 'you do not have enough balance');
            require(IERC20(_pair.t2).balanceOf(msg.sender) >= toSend, 'you do not have enough balance');
    ...
    **/
    function swapEthForToken(uint256 _id, uint256 _amount) public payable nonReentrant {
        ...
        if(fee > 0){
            ...
            TransferHelper.safeTransfer(address(_pair.t1), owner(), feeToOwner1);
            require(toSend > 0, 'invalidly low amount');
            require(msg.value >= toSend, 'you do not have MATIC balance');
