...
contract DSFUserHelper {
     ...
     */
    function _estimate3CRV(address token, uint256 amount) internal view returns (uint256) {
        ICurvePool pool3crv = ICurvePool(CURVE_3POOL);

        uint256[3] memory depositAmounts;
        if (token == DAI) depositAmounts[0] = amount;
        else if (token == USDC) depositAmounts[1] = amount;
        else if (token == USDT) depositAmounts[2] = amount;

        uint256 raw3CRV = pool3crv.calc_token_amount(depositAmounts, true);
    ...
    function estimateAllStablecoins() external view returns (
        ...
        estUSDT = estimateStablecoinDepositValue(USDT, 1e6);   // 1 USDT (6 decimals)
        estUSDC = estimateStablecoinDepositValue(USDC, 1e6);   // 1 USDC (6 decimals)
        estDAI  = estimateStablecoinDepositValue(DAI,  1e18);  // 1 DAI  (18 decimals)
    }
