 ...
 *       - Oracle pricing uses Chainlink Feed Registry; staleness is checked via `oracleStalenessSeconds`
 */
contract DSFanalytics is Ownable, ReentrancyGuard, AccessControl {
    using SafeERC20 for IERC20;

    /// @notice Chainlink Feed Registry used for TOKEN/USD and TOKEN/ETH lookups
    ...
    /// @notice Max allowed staleness for oracle price data
    ...
    event FeedRegistryUpdated(address indexed oldReg, address indexed newReg);
    event OracleStalenessUpdated(uint32 oldVal, uint32 newVal);

    /**
     * @notice Deploys DSFanalytics and binds DSF core + RewardManager
     * @dev    - `dsf_` and `rm_` must be deployed contracts
     ...
     */
    constructor(address dsf_, address rm_) Ownable(msg.sender) {
    ...
    /**
     * @notice Updates Chainlink Feed Registry address
     * @dev    `reg` must be a deployed contract
     * @param  reg Feed Registry address (Chainlink)
     */
    function setFeedRegistry(address reg) external onlyAdmin {
        require(reg != address(0) && reg.code.length > 0, "RM: bad registry");
        address old = address(feedRegistry);
        feedRegistry = IFeedRegistry(reg);
        emit FeedRegistryUpdated(old, reg);
    }

    /**
     * @notice Updates oracle staleness threshold
     * @dev    Protects UI/bots from stale price reads
     * @param  s Max staleness in seconds (min 60)
     */
    function setOracleStalenessSeconds(uint32 s) external onlyAdmin {
        require(s >= 60, "RM: too low");
        uint32 old = oracleStalenessSeconds;
        oracleStalenessSeconds = s;
        emit OracleStalenessUpdated(old, s);
    }
        ...
        )
    {
        ...
        if (adapter == address(0) || adapter.code.length == 0) {
            ...
            tokenValuesUsdt1e18 = new uint256[](0);
            return (adapter, address(0), tokens, balances, decimals, 0, tokenValuesUsdt1e18);
        }

        pool = IStrategyPoolAdapter(adapter).pool();
        tokens = IStrategyPoolAdapter(adapter).tokens();
        balances = IStrategyPoolAdapter(adapter).balances();
        decimals = IStrategyPoolAdapter(adapter).decimals();

        // опционально
        try IStrategyPoolAdapter(adapter).tvlUsdt1e18() returns (uint256 tvl) { tvlUsdt1e18 = tvl; } catch {}
        try IStrategyPoolAdapter(adapter).tokenValuesUsdt1e18() returns (uint256[] memory vals) { tokenValuesUsdt1e18 = vals; }
        catch { tokenValuesUsdt1e18 = new uint256[](0); }
    }
        ...
        ) 
    {
        ...
        for (uint256 i = 0; i < n; i++) {
            ...
            if (adapter != address(0) && adapter.code.length > 0) {
                ...
                try IStrategyPoolAdapter(adapter).tvlUsdt1e18() returns (uint256 tvl) {
                    poolTvlUsdt1e18[j] = tvl;
                } catch {}
            }
            j++;
        }
    ...
    /**
     * @notice Returns token USD price (1e18) via Chainlink Feed Registry (best-effort)
     * @dev    `ok` is true only if updatedAt is within staleness threshold
     * @param  token Token address (use WETH/0 for ETH pricing)
     * @param  asEth If true, forces token to be treated as ETH (uses ETH/USD feed)
     * @return price1e18 USD price scaled to 1e18 (0 if unavailable)
     * @return updatedAt Feed timestamp (0 if unavailable)
     * @return ok True if price is present and not stale
     */
    function getTokenUsdPrice1e18(address token, bool asEth)
        external
        view
        returns (uint256 price1e18, uint256 updatedAt, bool ok)
    {
        (price1e18, updatedAt) = _tryTokenUsdPrice1e18_CL(token, asEth);
        if (price1e18 == 0 || updatedAt == 0) return (price1e18, updatedAt, false);

        ok = (block.timestamp - updatedAt) <= oracleStalenessSeconds;
    }
     ...
     * @param  asEth  Flags to force ETH pricing per token
     * @return prices1e18 USD prices (1e18)
     * @return updatedAts Feed timestamps
     * @return oks True if price present & not stale
     */
    function getTokensUsdPrices1e18(address[] calldata tokens, bool[] calldata asEth)
        external
        view
        returns (uint256[] memory prices1e18, uint256[] memory updatedAts, bool[] memory oks)
    {
        require(tokens.length == asEth.length, "RM: len mismatch");
        ...
        for (uint256 i; i < tokens.length; i++) {
            (uint256 p, uint256 u) = _tryTokenUsdPrice1e18_CL(tokens[i], asEth[i]);
            prices1e18[i] = p;
            updatedAts[i] = u;
            oks[i] = (p != 0 && u != 0 && (block.timestamp - u) <= oracleStalenessSeconds);
        }
    }
    ...
    /**
     * @notice Reads Chainlink Feed Registry price for base/quote and normalizes to 1e18
     * @dev    Returns (0, updatedAt) on missing/invalid feed
     * @param  base      Base asset address in Feed Registry
     * @param  quote     Quote asset address in Feed Registry
    ...
    /**
     * @notice Best-effort Chainlink-only token USD pricing (1e18)
     * @dev    Resolution order:
     *         1) TOKEN/USD
     *         2) TOKEN/ETH × ETH/USD
     *         Special cases:
     *         - asEth/WETH/zero token -> ETH/USD
     * @param  token     Token address
     * @param  asEth     Force treat token as ETH
     * @return price1e18 USD price normalized to 1e18
     * @return updatedAt Timestamp of freshest used feed
     */
    function _tryTokenUsdPrice1e18_CL(address token, bool asEth)
        ...
        returns (uint256 price1e18, uint256 updatedAt)
    {
        // ETH / WETH / forced-as-ETH -> ETH/USD
    ...
    /**
     * @notice Wraps token USD price into OraclePrice struct with staleness check
     * @param  token Token address
    ...
    function _buildLiquidity(address strategy) internal view returns (PoolLiquidityBreakdown memory L) {
        ...
        try adp.tvlUsdt1e18() returns (uint256 tvl) { L.tvlUsdt1e18 = tvl; } catch {}
        try adp.tokenValuesUsdt1e18() returns (uint256[] memory vals) { 
            L.tokenValuesUsdt1e18 = vals; 
        } catch { 
            L.tokenValuesUsdt1e18 = new uint256[](0); 
        }
        ...
        for (uint256 i = 0; i < n; i++) {
            L.oracle[i] = _oraclePrice(L.tokens[i]);
        }
    }
}
