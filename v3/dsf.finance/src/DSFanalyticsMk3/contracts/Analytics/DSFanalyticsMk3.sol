...
struct StrategyRewards {
    ...
    uint256 cvxUsdt;         // 1e6
    uint256 crvUsdt;         // 1e6
    uint256 extraUsdt;       // Extra reward value in USDT native decimals, usually 1e6; 0 for old strategies
}
 ...
 */
contract DSFanalyticsMk3 is Ownable, ReentrancyGuard, AccessControl {
    ...
    /**
     * @notice Deploys DSFanalyticsMk3 and binds DSF core + RewardManager
     ...
     * @return extraUsdtGross extra value in USDT (gross, 1e6)
     ...
     * @return extraUsdtNet   extra value in USDT (net, 1e6)
     */
    ...
    /**
     * @notice Safely reads gross strategy reward balances
     * @dev    Uses a low-level `staticcall` instead of the typed `IStrategy` interface
     *         because old strategies return 4 values, while new strategies may return
     *         5 values including `extraEarningsUSDT`
     *         Supported return formats:
     *         - Old strategies:
     *             (amountIn_cvx, amountIn_crv, cvxEarningsUSDT, crvEarningsUSDT)
     *         - New strategies:
     *             (amountIn_cvx, amountIn_crv, cvxEarningsUSDT, crvEarningsUSDT, extraEarningsUSDT)
     *         If the call fails or returns an unexpected payload size, all values are returned as zero
     * @param  strat Strategy contract address
     * @return aCvx    Gross CVX token amount, in CVX native decimals
     * @return aCrv    Gross CRV token amount, in CRV native decimals
     * @return uCvx    Gross CVX reward value in USDT native decimals, usually 1e6
     * @return uCrv    Gross CRV reward value in USDT native decimals, usually 1e6
     * @return uExtra  Gross extra reward value in USDT native decimals, usually 1e6
     *                 Returns 0 for old strategies without extra rewards
     */
    function _safeGetRewardsGross(address strat)
        internal
        view
        returns (uint256 aCvx, uint256 aCrv, uint256 uCvx, uint256 uCrv, uint256 uExtra)
    {
        return _safeGetRewardsBySignature(strat, "getCVXCRVHoldingsGross()");
    }

    /**
     * @notice Safely reads net strategy reward balances after management fee
     * @dev    Uses a low-level `staticcall` instead of the typed `IStrategy` interface
     *         because old strategies return 4 values, while new strategies may return
     *         5 values including `extraEarningsUSDT`
     *         Supported return formats:
     ...
     *         If the call fails or returns an unexpected payload size, all values are returned as zero
     * @param  strat Strategy contract address
     * @return aCvx    Net CVX token amount, in CVX native decimals
     * @return aCrv    Net CRV token amount, in CRV native decimals
     * @return uCvx    Net CVX reward value in USDT native decimals, usually 1e6
     * @return uCrv    Net CRV reward value in USDT native decimals, usually 1e6
     * @return uExtra  Net extra reward value in USDT native decimals, usually 1e6
     *                 Returns 0 for old strategies without extra rewards
     */
    function _safeGetRewardsNet(address strat)
        ...
        returns (uint256 aCvx, uint256 aCrv, uint256 uCvx, uint256 uCrv, uint256 uExtra)
    {
        return _safeGetRewardsBySignature(strat, "getCVXCRVHoldings()");
    }

    /**
     * @notice Safely reads strategy rewards using a function signature
     * @dev    This helper is backward-compatible with both old and new strategy contracts
     *         It performs a low-level `staticcall` and decodes the response by payload length:
     *         - 160 bytes or more: decodes 5 uint256 values
     *         - 128 bytes or more: decodes 4 uint256 values and sets `uExtra` to zero
     *         - Anything else: returns zeros
     *         This is needed because Solidity typed interfaces cannot safely represent
     *         both old `getCVXCRVHoldings()` return format with 4 values and new format
     *         with 5 values at the same time
     *         Expected signatures:
     *         - `getCVXCRVHoldingsGross()`
     *         - `getCVXCRVHoldings()`
     * @param strat     Strategy contract address
     * @param signature Function signature to call on the strategy
     * @return aCvx    CVX token amount, in CVX native decimals
     * @return aCrv    CRV token amount, in CRV native decimals
     * @return uCvx    CVX reward value in USDT native decimals, usually 1e6
     * @return uCrv    CRV reward value in USDT native decimals, usually 1e6
     * @return uExtra  Extra reward value in USDT native decimals, usually 1e6
     *                 Returns 0 for old strategies without extra rewards
     */
    function _safeGetRewardsBySignature(address strat, string memory signature)
        internal
        view
        returns (uint256 aCvx, uint256 aCrv, uint256 uCvx, uint256 uCrv, uint256 uExtra)
    {
        if (strat == address(0) || strat.code.length == 0) return (0, 0, 0, 0, 0);

        (bool ok, bytes memory data) = strat.staticcall(abi.encodeWithSignature(signature));
        ...
        if (data.length >= 128) {
            (aCvx, aCrv, uCvx, uCrv) = abi.decode(data, (uint256, uint256, uint256, uint256));
            return (aCvx, aCrv, uCvx, uCrv, 0);
        }
