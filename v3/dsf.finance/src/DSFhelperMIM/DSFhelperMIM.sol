...
contract DSFhelperMIM {
     ...
     */
    function _estimate3CRV(uint256 amount, uint128 tokenIndex) internal view returns (uint256) {
     ...
     */
    function _estimateMIM3CRVLP(uint256 amount3CRV) internal view returns (uint256) {
     ...
     * @param stableIndex Index of the coin to withdraw (0 = MIM, 1 = 3CRV)
     * @return withdrawValueInUSD Estimated net value in USD after withdrawal
     */
    function estimateWithdrawalValue(uint256 lpAmount, int128 stableIndex) external view returns (uint256 withdrawValueInUSD) {
        require(lpAmount > 0, "DSF: Zero LP amount");
        require(stableIndex == 0 || stableIndex == 1, "DSF: Invalid coin index");

        ICurveMetaPool poolmim3crv = ICurveMetaPool(MIM_3CRV_POOL);
        uint256 underlyingAmount = poolmim3crv.calc_withdraw_one_coin(lpAmount, stableIndex);
    ...
    /**
     * @notice Returns the estimated deposit value (in USD) for provided amounts of each stablecoin
     * @param daiAmount Amount of DAI (18 decimals)
     * @param usdcAmount Amount of USDC (6 decimals)
     * @param usdtAmount Amount of USDT (6 decimals)
     * @return daiValue Estimated value of DAI deposit
     * @return usdcValue Estimated value of USDC deposit
     * @return usdtValue Estimated value of USDT deposit
     */
    function estimateStablecoinValues(
        ...
        daiValue = daiAmount > 0 ? estimateByIndex(daiAmount, 0) : 0;     // DAI
        usdcValue = usdcAmount > 0 ? estimateByIndex(usdcAmount, 1) : 0;  // USDC
        usdtValue = usdtAmount > 0 ? estimateByIndex(usdtAmount, 2) : 0;  // USDT
    }

    /**
     * @notice Estimates stablecoin return after full roundtrip (deposit → LP → withdraw)
     *         for each stablecoin using custom input amounts.
     * @param daiAmount Amount of DAI to simulate (18 decimals)
     * @param usdcAmount Amount of USDC to simulate (6 decimals)
     * @param usdtAmount Amount of USDT to simulate (6 decimals)
     * @return daiReturned    DAI returned after full roundtrip
     * @return usdcReturned   USDC returned after full roundtrip
     * @return usdtReturned   USDT returned after full roundtrip
     */
    function estimateRoundTripReturns(
        uint256 daiAmount,
        uint256 usdcAmount,
        uint256 usdtAmount
    ) external view returns (
        ...
        daiReturned = daiAmount > 0 ? this.estimateNetRoundTripByIndex(daiAmount, 0) : 0;    // DAI
        usdcReturned = usdcAmount > 0 ? this.estimateNetRoundTripByIndex(usdcAmount, 1) : 0; // USDC
        usdtReturned = usdtAmount > 0 ? this.estimateNetRoundTripByIndex(usdtAmount, 2) : 0; // USDT
    }
