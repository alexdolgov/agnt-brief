 ...
 *      - for 18 decimals (DAI): multiplier = 1
 *      - for 6 decimals (USDC/USDT): multiplier = 1e12
 *    Normalization to 1e18 is done as: amountNorm1e18 = amountTokenUnits * multiplier
 *
 * 3) Default test amount for efficiency calculations is 50_000 tokens in token units:
 *      - DAI:  50_000e18
 *      - USDC: 50_000e6
 *      - USDT: 50_000e6
 *    If user balance is smaller than default, the test amount becomes min(balance, default).
 *
 * 4) Pool semantics:
...
contract DSFuserDataMk2_Aggregated {
    ...
    // Test amount for calculating roundTripEfficiency (in token units, including decimals):
    // DAI: 50_000e18
    // USDC/USDT: 50_000e6
    uint256 public constant TEST_AMOUNT = 50_000;
    ...
    /**
     * @notice Returns default test amount in token units for efficiency calculations.
     * @dev    TEST_AMOUNT is expressed as "whole tokens" (e.g. 50_000).
     *         DSF provides `decimalsMultipliers(i) = 10^(18 - decimals)`.
     *
     *         Mapping by multiplier:
     *           - m == 1   -> 18 decimals token (DAI)  -> 50_000 * 1e18
     *           - m != 1   -> 6 decimals token (USDC/USDT) -> 50_000 * 1e6
     *
     *         Note: this assumes DSF uses only 18-dec (DAI) and 6-dec (USDC/USDT).
     *
     * @param  dsf DSF LP contract (source of decimalsMultipliers).
     * @param  tokenIndex Token index: 0=DAI, 1=USDC, 2=USDT.
     * @return amount Default test amount in token units (with decimals).
     */
    function _defaultTestAmount(IDSFLP dsf, uint128 tokenIndex) internal view returns (uint256) {
        uint256 m = dsf.decimalsMultipliers(tokenIndex); // 10^(18-decimals)
        return (m == 1) ? (TEST_AMOUNT * 1e18) : (TEST_AMOUNT * 1e6);
    }
     ...
     *           defaultAmount = _defaultTestAmount(dsf, i)   // 50_000 tokens (with decimals)
     *           testAmount    = (userBalanceToken > 0) ? min(userBalanceToken, defaultAmount) : defaultAmount
     *
        ...
        returns (uint256[3] memory roundTripEff1e18)
    {
        ...
        if (pidD == 0) {
            // pid=0: helper.estimateNetRoundTripByIndex(amount,i) -> returnedAmount in token units
            for (uint128 i = 0; i < 3; i++) {
                uint256 m = dsf.decimalsMultipliers(i);
                uint256 defaultAmount = _defaultTestAmount(dsf, i);
                uint256 testAmount = b[i] > 0 ? _min(b[i], defaultAmount) : defaultAmount;
                ...
                // if suddenly amountNorm1e18 == 0 (theoretically), return 0
