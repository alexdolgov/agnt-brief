...
contract DSFuserDataMk2_Aggregated {
    ...
    function getUserData(address user) external view returns (
        ...
        pendingDeposits = dsf.pendingDeposits(user);

        IDSFLP.PendingWithdrawal memory pw = dsf.pendingWithdrawals(user);
        pendingWithdrawal.lpShares = pw.lpShares;
        pendingWithdrawal.tokenAmounts = pw.tokenAmounts;
        
        if (pw.lpShares > 0) {
            // try/catch to never break analytics if withdraw strategy misconfigured
            try this.getWithdrawalAmountsByLp(pw.lpShares) returns (uint256[3] memory est) {
                pendingWithdrawal.estimatedWithdrawals = est;
            } catch {
                // keep zeros
            }
        }
    ...
    function getUserPending(address user)
        ...
        returns (
            uint256[3] memory pendingDeposits,   // [DAI, USDC, USDT]
            PendingWithdrawalView memory pendingWithdrawal
        )
    {
        IDSFLP dsf = IDSFLP(dsfLpContract);
