 ...
 *
 * @title DSFuserDataMk3_Aggregated
 * @author DSF.Finance
...
contract DSFuserDataMk3_Aggregated {
    ...
    /// @notice poolId => blacklisted (true = skip in analytics)
    ...
    event PoolBlacklisted(uint256 indexed pid, bool status);
    ...
    constructor(address _dsfLpContract, address _dsfHelperContract, address _pool0Oracle, uint256[] memory _initialBlacklistedPids) {
        ...
        for (uint256 i = 0; i < _initialBlacklistedPids.length; i++) {
            uint256 pid = _initialBlacklistedPids[i];
            isPoolBlacklisted[pid] = true;
            emit PoolBlacklisted(pid, true);
        }
    }
    ...
    function setPoolBlacklisted(uint256 pid, bool status) external onlyOwner {
        isPoolBlacklisted[pid] = status;
        emit PoolBlacklisted(pid, status);
    }

    function setPoolsBlacklisted(uint256[] calldata pids, bool status) external onlyOwner {
        for (uint256 i = 0; i < pids.length; i++) {
            isPoolBlacklisted[pids[i]] = status;
            emit PoolBlacklisted(pids[i], status);
        }
    }
}
