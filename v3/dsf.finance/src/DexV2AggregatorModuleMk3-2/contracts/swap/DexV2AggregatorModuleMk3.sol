...
contract DexV2AggregatorModuleMk3 is Ownable, ReentrancyGuard {
    ...
    // Ethereum mainnet
    address public constant CURVE3POOL_ADDR = 0xbEbc44782C7dB0a1A60Cb6fe97d0b483032FF1C7;

    /// @notice Curve 3Pool instance (DAI/USDC/USDT) used for stable-to-stable fast-path swaps (mainnet 3pool)
    ICurve3Pool public constant CURVE3POOL = ICurve3Pool(CURVE3POOL_ADDR);
    ...
    ) external nonReentrant returns (uint[] memory amounts)
    {
        ...
        if (_isCurve3Direct(path)) {
            ...
            emit SwapExecuted(DexKind.CURVE3, tokenIn, tokenOut, address(CURVE3POOL), to, amountIn, out, minOutUsed);
            return am;
        }
    ...
    function swapExactTokensForTokensWithReport(
        ...
        if (_isCurve3Direct(path)) {
            ...
            (oracleOut, oracleOk) = _oracleExpectedOutRaw(tokenIn, tokenOut, amountIn);
            if (oracleOk) {
            ...
            uint256 minOutLocal = amountOutMin;
            if (minOutLocal == 0) {
                minOutLocal = (outQuote * (10_000 - defaultSlippageBpsCurve3)) / 10_000;
            } else {
                require(outQuote >= minOutLocal, "quote < minOut");
            }
            ...
            uint256 balBefore = IERC20(tokenOut).balanceOf(address(this));
            CURVE3POOL.exchange(i, j, amountIn, minOutLocal);
            uint256 out = IERC20(tokenOut).balanceOf(address(this)) - balBefore;

            require(out >= minOutLocal, "minOut not met");
            IERC20(tokenOut).safeTransfer(to, out);
            ...
            emit SwapExecuted(DexKind.CURVE3, tokenIn, tokenOut, address(CURVE3POOL), to, amountIn, out, minOutLocal);

            return (amounts, quotedOutGross, minOutUsed);
        }

        // ───────────────────── Default: V2 routers (report) ─────────────────────
        (
            DexKind dex1, address router1, uint256 out1,
            DexKind dex2, address router2, uint256 out2
        ) = _quoteTop2(amountIn, path);
        ...
        uint256 oracleOutTotal = 0;
        bool oracleOkTotal = false;
        
        (oracleOutTotal, oracleOkTotal) = _oracleExpectedOutRaw(tokenIn, tokenOut, amountIn);
        ...
        if (!doSplit) {
            ...
            uint256 minOutLocal = amountOutMin;
            if (minOutLocal == 0) {
                minOutLocal = (out1 * (10_000 - defaultSlippageBpsV2)) / 10_000;
            } else {
            ...
            emit SwapExecuted(dex1, tokenIn, tokenOut, router1, to, amountIn, amounts[amounts.length - 1], minOutLocal);
            return (amounts, quotedOutGross, minOutUsed);
        }
        ...
        // Re-quote per leg to reduce race-condition risk
        uint256 qA = _safeQuote(IUniswapV2RouterLike(router1), amountA, path);
        uint256 qB = _safeQuote(IUniswapV2RouterLike(router2), amountB, path);
        require(qA > 0 && qB > 0, "re-quote failed");
        ...
        if (amountA > 0) {
            ...
            require(amA.length >= 2 && amA[amA.length - 1] >= minOutA, "legA minOut not met");
            outLegA = amA[amA.length - 1];
            emit SwapExecuted(dex1, tokenIn, tokenOut, router1, to, amountA, outLegA, minOutA);
        }
        ...
        if (amountB > 0) {
            ...
            require(amB.length >= 2 && amB[amB.length - 1] >= minOutB, "legB minOut not met");
            outLegB = amB[amB.length - 1];
            emit SwapExecuted(dex2, tokenIn, tokenOut, router2, to, amountB, outLegB, minOutB);
        }
