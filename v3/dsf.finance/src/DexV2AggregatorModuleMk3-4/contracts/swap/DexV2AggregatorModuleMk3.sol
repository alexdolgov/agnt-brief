...
/**
 * @title  DexV2AggregatorModuleMk3 (UniV2/SushiV2 + optional Curve3 fast-path + Chainlink guards)
...
contract DexV2AggregatorModuleMk3 is Ownable, ReentrancyGuard {
    ...
    /// @notice Router configuration for a V2-compatible DEX venue
    /// @dev    When `enabled == false` the router is ignored for quoting and swaps.
    ///         A zero `router` address is also treated as "disabled" by the quoting logic.
    struct RouterCfg {
     ...
     *             * returns [amountIn, curveOut] if Curve quote exists and (when enabled) passes oracle guard
     *             * otherwise falls back to V2 routers quoting
     *         - For V2 routers:
     *             * selects the best quote among enabled routers
     *             * if oracle guards are enabled and oracle price is available & fresh, returns empty array
     *               when the best DEX quote is worse than oracle by more than `maxDeviationBps`
     *         - Returns empty array if no enabled router can quote or quoting fails
     *
    ...
    function swapExactTokensForTokensWithReport(
        ...
        if (_isCurve3Direct(path)) {
            ...
            emit SwapExecuted(DexKind.NONE, tokenIn, tokenOut, address(curve3Pool), to, amountIn, out, minOutLocal);
        ...
        if (!doSplit) {
            if (useOracleGuards && oracleOkTotal) {
                bool pass = _passesOracleGuard(out1, oracleOutTotal);
                emit OracleGuardCheck(tokenIn, tokenOut, amountIn, oracleOutTotal, out1, pass);
                require(pass, "oracle guard");
            }
