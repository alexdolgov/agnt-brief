...
/**
 * @title  DexV2AggregatorModuleMk4 (UniV2/SushiV2 + Curve3 direct stable path + Chainlink guard)
...
contract DexV2AggregatorModuleMk4 is Ownable, ReentrancyGuard {
    ...
    function getAmountsOut(uint amountIn, address[] calldata path) external view returns (uint[] memory amounts) {
        ...
        if (_isRlusdToUsdtRoute(path)) {
            ...
            if (ok && !_passesOracleGuard(usdtOut, oracleOut)) return new uint[](0);
    ...
    function testSwapPyusdToUsdtViaUsdc(
        uint256 amountIn,
        uint256 amountOutMin,
        address to
    ) external nonReentrant returns (uint[] memory amounts) {
        ...
        try CURVE_PYUSD_USDC_POOL.get_dy(0, 1, amountIn) returns (uint256 out) {
            ...
            revert("pyusd/usdc no quote");
        }

        require(usdcQuote > 0, "pyusd/usdc no quote");

        uint256 usdtQuote = _safeCurve3Quote(1, 2, usdcQuote);
        require(usdtQuote > 0, "usdc/usdt no quote");

        uint256 minOutUsed = _resolveMinOut(
            usdtQuote,
            amountOutMin,
            defaultSlippageBpsCurve3
        );
    ...
    function testSwapPyusdToUsdtViaCrvUsd(
        uint256 amountIn,
        uint256 amountOutMin,
        address to
    ) external nonReentrant returns (uint[] memory amounts) {
        ...
        try CURVE_PYUSD_CRVUSD_POOL.get_dy(0, 1, amountIn) returns (uint256 out) {
            ...
            revert("pyusd/crvusd no quote");
        }

        require(crvUsdQuote > 0, "pyusd/crvusd no quote");

        uint256 usdtQuote;

        try CURVE_CRVUSD_USDT_POOL.get_dy(1, 0, crvUsdQuote) returns (uint256 out) {
            usdtQuote = out;
        } catch {
            revert("crvusd/usdt no quote");
        }

        require(usdtQuote > 0, "crvusd/usdt no quote");
        ...
        uint256 crvUsdOut = IERC20(CRVUSD_ADDRESS).balanceOf(address(this)) - crvUsdBefore;
        require(crvUsdOut > 0, "no crvusd out");

        uint256 usdtBefore = IERC20(USDT_ADDRESS).balanceOf(address(this));

        _approveMax(CRVUSD_ADDRESS, CURVE_CRVUSD_USDT_POOL_ADDR);
        CURVE_CRVUSD_USDT_POOL.exchange(1, 0, crvUsdOut, minOutUsed);
    ...
    function testSwapRlusdToUsdtViaUsdc(
        uint256 amountIn,
        uint256 amountOutMin,
        address to
    ) external nonReentrant returns (uint[] memory amounts) {
