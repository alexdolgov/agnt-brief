...
import {ICurvePoolStableSwapNG} from "../interfaces/ICurvePoolStableSwapNG_128.sol";

/**
 * @title  DexV2AggregatorModuleMk4 (UniV2/SushiV2 + Curve3 + RLUSD/PYUSD direct stable path + Chainlink guard)
 ...
 *         per exact path, and applies Chainlink Feed Registry guard when oracle data is available & fresh; otherwise guard is skipped.
 *
 * @dev
 * Core features:
 * - Venue selection:
 *    • Quotes the supported V2 routers and selects the best (and 2nd best for splitting).
 *    • If `path.length == 2` and both tokens are Curve 3Pool stables (DAI/USDC/USDT),
 *      the module uses Curve 3Pool as the stable-to-stable path when eligible (no V2 fallback for direct 3Pool pairs).
 *
 * - Oracle guard (always-on, best-effort):
 *    • If an oracle price is available & fresh:
 *        - `getAmountsOut` returns an empty array when best output is worse than oracle by > `maxDeviationBps`.
 *        - swaps revert with "oracle guard" when expected execution is worse than oracle by > `maxDeviationBps`.
...
/**
 * @title  DexV2AggregatorModuleMk4
 * @notice Aggregates UniV2/SushiV2 quotes, can auto-split execution across top-2 venues,
 *         and applies a best-effort Chainlink Feed Registry guard when oracle data is available & fresh.
 */
contract DexV2AggregatorModuleMk4 is Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20;

    /* ───────────────────── Types ───────────────────── */

    /// @notice Supported DEX kinds
    enum DexKind { NONE, UNIV2, SUSHIV2, CURVE3 }

    /// @notice Per-path split configuration
    struct SplitCfg {
        bool enabled;       // enable split for this exact path
    ...
    /// @notice Chainlink Feed Registry (Ethereum mainnet)
    ...
    address public constant PYUSD_ADDRESS                = 0x6c3ea9036406852006290770BEdFcAbA0e23A0e8;
    ...
    address public constant CURVE_3POOL_ADDR             = 0xbEbc44782C7dB0a1A60Cb6fe97d0b483032FF1C7;

    address public constant CURVE_RLUSD_USDC_POOL_ADDR   = 0xD001aE433f254283FeCE51d4ACcE8c53263aa186;
    address public constant CURVE_PYUSD_USDC_POOL_ADDR   = 0x383E6b4437b59fff47B619CBA855CA29342A8559;
    address public constant CURVE_PYUSD_CRVUSD_POOL_ADDR = 0x625E92624Bc2D88619ACCc1788365A69767f6200;
    address public constant CURVE_CRVUSD_USDT_POOL_ADDR  = 0x390f3595bCa2Df7d23783dFd126427CCeb997BF4;

    /// @notice Curve 3Pool instance (DAI/USDC/USDT) used for stable-to-stable fast-path swaps (mainnet 3pool)
    ICurve3Pool public constant CURVE3POOL = ICurve3Pool(CURVE_3POOL_ADDR);

    ICurvePoolStableSwapNG public constant CURVE_RLUSD_USDC_POOL   = ICurvePoolStableSwapNG(CURVE_RLUSD_USDC_POOL_ADDR);
    ICurvePoolStableSwapNG public constant CURVE_PYUSD_CRVUSD_POOL = ICurvePoolStableSwapNG(CURVE_PYUSD_CRVUSD_POOL_ADDR);
    ICurvePoolStableSwapNG public constant CURVE_CRVUSD_USDT_POOL  = ICurvePoolStableSwapNG(CURVE_CRVUSD_USDT_POOL_ADDR);
    ICurvePoolStableSwapNG public constant CURVE_PYUSD_USDC_POOL   = ICurvePoolStableSwapNG(CURVE_PYUSD_USDC_POOL_ADDR);
    
    /* ───────────────────── V2 Routers constants ───────────────────── */

    // Ethereum mainnet
    address public constant UNISWAP_V2_ROUTER_ADDR       = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    address public constant SUSHISWAP_V2_ROUTER_ADDR     = 0xd9e1cE17f2641f24aE83637ab66a2cca9C378B9F;

    IUniswapV2RouterLike public constant UNIV2_ROUTER    = IUniswapV2RouterLike(UNISWAP_V2_ROUTER_ADDR);
    IUniswapV2RouterLike public constant SUSHI_ROUTER    = IUniswapV2RouterLike(SUSHISWAP_V2_ROUTER_ADDR);
    ...
    /// @notice Oracle config update (registry is constant; here we only emit the values that can change)
    event OracleConfigUpdated(uint16 maxDeviationBps, uint256 stalenessSeconds);
    ...
    /**
     * @notice Initializes the module owner.
     */
    ...
    /**
     * @notice Set oracle configuration (deviation, staleness)
     ...
     */
    function setOracleConfig(uint16 deviationBps, uint256 maxStaleness) external onlyOwner {
        require(deviationBps <= 10_000, "deviation too high");
        maxDeviationBps = deviationBps;
        stalenessSeconds = maxStaleness;
        emit OracleConfigUpdated(maxDeviationBps, stalenessSeconds);
    }
    ...
    /**
     * @notice V2-compatible `getAmountsOut` with best-effort oracle guard filtering
     * @dev    Behavior:
     *         - If `path` is a direct Curve 3Pool stable->stable swap (length == 2):
     *              if path.length == 2 and both tokens are 3Pool stables, uses Curve 3Pool quote only; 
     *              returns empty array if Curve quote fails (no fallback)
     *         - For V2 routers:
     *              selects the best quote among supported routers
     *              if oracle data is available & fresh, returns empty array when the best DEX quote
     *              is worse than oracle by more than `maxDeviationBps`
     *         - Returns empty array if no supported router can quote or quoting fails
    ...
    function getAmountsOut(uint amountIn, address[] calldata path) external view returns (uint[] memory amounts) {
        ...
        // ────────────────── Curve RLUSD_USDC fast-path ───────────────────
        if (_isRlusdToUsdtRoute(path)) {
            (uint256 usdcOut, uint256 usdtOut) = _quoteRlusdToUsdt(amountIn);
            if (usdcOut == 0 || usdtOut == 0) return new uint[](0);

            (uint256 rlusdOracleOut, bool rlusdOracleOk) = _oracleExpectedOutRaw(RLUSD_ADDRESS, USDT_ADDRESS, amountIn);
            if (rlusdOracleOk && !_passesOracleGuard(usdtOut, rlusdOracleOut)) return new uint[](0);

            uint[] memory am = new uint[](3);
            am[0] = amountIn;
            am[1] = usdcOut;
            am[2] = usdtOut;
            return am;
        ...
        if (_isPyusdToUsdtRoute(path)) {
            (, uint256 midOut, uint256 usdtOut) = _quotePyusdToUsdt(amountIn);
            if (midOut == 0 || usdtOut == 0) return new uint[](0);

            (uint256 pyusdOracleOut, bool pyusdOracleOk) = _oracleExpectedOutRaw(PYUSD_ADDRESS, USDT_ADDRESS, amountIn);
            if (pyusdOracleOk && !_passesOracleGuard(usdtOut, pyusdOracleOut)) return new uint[](0);
        ...
        if (_isCurve3Direct(path)) {
            ...
            uint out = _safeCurve3Quote(i, j, amountIn);
            if (out == 0) return new uint[](0);

            (uint256 curve3OracleOut, bool curve3OracleOk) = _oracleExpectedOutRaw(path[0], path[1], amountIn);
            if (curve3OracleOk && !_passesOracleGuard(out, curve3OracleOut)) return new uint[](0);
        ...
        if (bestDex == DexKind.NONE || bestRouter == address(0) || bestOut == 0) return new uint[](0);
    ...
    /**
     * @notice Execute an exact-in swap with optional auto-split and best-effort oracle guard
     * @dev    High-level flow:
     ...
     *              - if oracle data is available & fresh, reverts if Curve quote is worse than oracle
     *                by more than `maxDeviationBps`
     *              - transfers final tokenOut to `to`
     *
     *         2) Otherwise (V2 routers Uni/Sushi):
     *              - finds top-1 and top-2 router quotes among supported venues
     ...
     *              - if oracle data is available & fresh, reverts if the expected DEX output
     *                (single quote or sum of re-quotes in split) is worse than oracle by more than `maxDeviationBps`
    ...
    function swapExactTokensForTokens(
        ...
        (amounts,,) = _swapExactTokensForTokensInternal(
            amountIn,
            amountOutMin,
            path,
            to,
    ...
    /**
     * @notice Internal dispatcher for all supported exact-input swap routes
     * @dev    Route priority:
     *         - RLUSD -> USDC -> USDT through Curve RLUSD/USDC + Curve 3Pool
     *         - PYUSD -> USDT through the best available Curve route:
     *           either PYUSD -> USDC -> USDT or PYUSD -> crvUSD -> USDT
     *         - Direct Curve 3Pool stable swap for DAI / USDC / USDT pairs
     *         - Default UniV2 / SushiV2 route with optional split execution
     * @param  amountIn        Exact input amount in raw decimals of `path[0]`
     * @param  amountOutMin    Minimum final output amount in raw decimals of final token
     * @param  path            Swap path
     * @param  to              Recipient of output tokens
     * @param  deadline        Unix timestamp used by V2 router swaps
     * @return amounts         Swap result amounts
     * @return quotedOutGross  Gross quote before slippage deduction
     * @return minOutUsed      Actual minOut enforced during execution
     */
    function _swapExactTokensForTokensInternal(
        ...
        if (_isRlusdToUsdtRoute(path)) {
            return _swapRlusdToUsdt(amountIn, amountOutMin, to);
        }

        if (_isPyusdToUsdtRoute(path)) {
            return _swapPyusdToUsdt(amountIn, amountOutMin, to);
        }

        if (_isCurve3Direct(path)) {
            return _swapCurve3Direct(amountIn, amountOutMin, path, to);
        }

        return _swapV2(amountIn, amountOutMin, path, to, deadline);
    }

    /**
     * @notice Executes a direct Curve 3Pool stable-to-stable swap
     * @dev    Supports only direct 2-token paths between DAI, USDC and USDT
     *         Token indexes are resolved via `_curve3Index`.
     *         The function pulls `tokenIn` from the caller, approves Curve 3Pool,
     *         executes `exchange`, verifies `minOutUsed`, and transfers `tokenOut` to `to`
     * @param  amountIn        Exact input amount in raw decimals of `path[0]`
     * @param  amountOutMin    Minimum acceptable output amount in raw decimals of `path[1]`
     * @param  path            Direct Curve 3Pool path, length must be 2
     * @param  to              Recipient of output tokens
     * @return amounts         Two-element array: [amountIn, actualOut]
     * @return quotedOutGross  Curve 3Pool quote before slippage deduction
     * @return minOutUsed      Actual minOut enforced during execution
     */
    function _swapCurve3Direct(
        uint256 amountIn,
        ...
        quotedOutGross = outQuote;

        (uint256 oracleOut, bool ok) = _oracleExpectedOutRaw(tokenIn, tokenOut, amountIn);
        if (ok) {
            bool pass = _passesOracleGuard(outQuote, oracleOut);
            emit OracleGuardCheck(tokenIn, tokenOut, amountIn, oracleOut, outQuote, pass);
            require(pass, "oracle guard");
        }

        minOutUsed = _resolveMinOut(outQuote, amountOutMin, defaultSlippageBpsCurve3);

        IERC20(tokenIn).safeTransferFrom(msg.sender, address(this), amountIn);
        _approveMax(tokenIn, CURVE_3POOL_ADDR);
    ...
    /**
     * @notice Executes RLUSD -> USDC -> USDT through Curve pools
     * @dev    Route composition:
     *         - RLUSD -> USDC through Curve RLUSD/USDC pool
     *         - USDC -> USDT through Curve 3Pool
     *         RLUSD uses 18 decimals
     *         USDC and USDT use 6 decimals
     *         Therefore `amountIn` is expected in RLUSD raw units,
     *         while `amountOutMin` is expected in USDT raw units
     * @param  amountIn        Exact RLUSD input amount in 18 decimals
     * @param  amountOutMin    Minimum final USDT amount in 6 decimals
     * @param  to              Recipient of USDT
     * @return amounts         Three-element array: [RLUSD in, USDC out, USDT out]
     * @return quotedOutGross  Final USDT quote before slippage deduction
     * @return minOutUsed      Actual USDT minOut enforced during execution
     */
    function _swapRlusdToUsdt(
        uint256 amountIn,
        uint256 amountOutMin,
        address to
    ) internal returns (
        ...
        (uint256 usdcQuote, uint256 usdtQuote) = _quoteRlusdToUsdt(amountIn);
        require(usdcQuote > 0, "rlusd/usdc no quote");
        require(usdtQuote > 0, "usdc/usdt no quote");

        quotedOutGross = usdtQuote;

        (uint256 oracleOut, bool ok) = _oracleExpectedOutRaw(RLUSD_ADDRESS, USDT_ADDRESS, amountIn);
        if (ok) {
            bool pass = _passesOracleGuard(usdtQuote, oracleOut);
            emit OracleGuardCheck(RLUSD_ADDRESS, USDT_ADDRESS, amountIn, oracleOut, usdtQuote, pass);
            require(pass, "oracle guard");
        }

        minOutUsed = _resolveMinOut(usdtQuote, amountOutMin, defaultSlippageBpsCurve3);

        IERC20(RLUSD_ADDRESS).safeTransferFrom(msg.sender, address(this), amountIn);

        uint256 usdcBefore = IERC20(USDC_ADDRESS).balanceOf(address(this));

        _approveMax(RLUSD_ADDRESS, CURVE_RLUSD_USDC_POOL_ADDR);
        CURVE_RLUSD_USDC_POOL.exchange(1, 0, amountIn, 0);

        uint256 usdcOut = IERC20(USDC_ADDRESS).balanceOf(address(this)) - usdcBefore;
        require(usdcOut > 0, "no usdc out");

        uint256 usdtBefore = IERC20(USDT_ADDRESS).balanceOf(address(this));

        _approveMax(USDC_ADDRESS, CURVE_3POOL_ADDR);
        CURVE3POOL.exchange(1, 2, usdcOut, minOutUsed);

        uint256 usdtOut = IERC20(USDT_ADDRESS).balanceOf(address(this)) - usdtBefore;
        require(usdtOut >= minOutUsed, "minOut not met");

        IERC20(USDT_ADDRESS).safeTransfer(to, usdtOut);
    ...
    /**
     * @notice Executes PYUSD -> USDT using the best available Curve route
     * @dev    The function first quotes both supported PYUSD routes:
     *         - PYUSD -> USDC -> USDT
     *         - PYUSD -> crvUSD -> USDT
     *         It then executes the route with the higher final USDT output
     *         PYUSD uses 6 decimals
     *         USDC and USDT use 6 decimals
     *         crvUSD uses 18 decimals
     *
     *         `amountOutMin` must always be expressed in final USDT raw units
     * @param  amountIn        Exact PYUSD input amount in 6 decimals
     * @param  amountOutMin    Minimum final USDT amount in 6 decimals
     * @param  to              Recipient of USDT
     * @return amounts         Three-element array: [PYUSD in, selected mid out, USDT out]
     * @return quotedOutGross  Final USDT quote of the selected route before slippage deduction
     * @return minOutUsed      Actual USDT minOut enforced during execution
     */
    function _swapPyusdToUsdt(
        uint256 amountIn,
        uint256 amountOutMin,
        address to
    ) internal returns (
        ...
        (bool viaUsdc, uint256 midQuote, uint256 usdtQuote) = _quotePyusdToUsdt(amountIn);
        require(midQuote > 0, "pyusd route no mid");
        require(usdtQuote > 0, "pyusd route no quote");

        quotedOutGross = usdtQuote;

        (uint256 oracleOut, bool ok) = _oracleExpectedOutRaw(PYUSD_ADDRESS, USDT_ADDRESS, amountIn);
        if (ok) {
            bool pass = _passesOracleGuard(usdtQuote, oracleOut);
            emit OracleGuardCheck(PYUSD_ADDRESS, USDT_ADDRESS, amountIn, oracleOut, usdtQuote, pass);
            require(pass, "oracle guard");
        }

        minOutUsed = _resolveMinOut(usdtQuote, amountOutMin, defaultSlippageBpsCurve3);

        IERC20(PYUSD_ADDRESS).safeTransferFrom(msg.sender, address(this), amountIn);
        ...
        if (viaUsdc) {
            ...
            _approveMax(PYUSD_ADDRESS, CURVE_PYUSD_USDC_POOL_ADDR);
            CURVE_PYUSD_USDC_POOL.exchange(0, 1, amountIn, 0);

            midOut = IERC20(USDC_ADDRESS).balanceOf(address(this)) - usdcBefore;
            require(midOut > 0, "no usdc out");

            uint256 usdtBefore = IERC20(USDT_ADDRESS).balanceOf(address(this));

            _approveMax(USDC_ADDRESS, CURVE_3POOL_ADDR);
            CURVE3POOL.exchange(1, 2, midOut, minOutUsed);
        ...
        uint256 crvUsdBefore = IERC20(CRVUSD_ADDRESS).balanceOf(address(this));

        _approveMax(PYUSD_ADDRESS, CURVE_PYUSD_CRVUSD_POOL_ADDR);
        CURVE_PYUSD_CRVUSD_POOL.exchange(0, 1, amountIn, 0);

        midOut = IERC20(CRVUSD_ADDRESS).balanceOf(address(this)) - crvUsdBefore;
        require(midOut > 0, "no crvusd out");

        uint256 usdtBefore2 = IERC20(USDT_ADDRESS).balanceOf(address(this));

        _approveMax(CRVUSD_ADDRESS, CURVE_CRVUSD_USDT_POOL_ADDR);
        CURVE_CRVUSD_USDT_POOL.exchange(1, 0, midOut, minOutUsed);

        uint256 usdtOut2 = IERC20(USDT_ADDRESS).balanceOf(address(this)) - usdtBefore2;
        require(usdtOut2 >= minOutUsed, "minOut not met");

        IERC20(USDT_ADDRESS).safeTransfer(to, usdtOut2);
    ...
    /**
     * @notice Executes a default V2-style swap through UniV2 or SushiV2
     * @dev    Quotes both supported V2 routers and selects the best route
     *         If split configuration exists for the exact path and the amount exceeds
     *         the configured threshold, execution may be split between the top-2 routers
     *         Oracle guard is applied when fresh oracle data is available
     *         If `amountOutMin == 0`, default V2 slippage is applied to the router quote
     * @param  amountIn        Exact input amount in raw decimals of `path[0]`
     * @param  amountOutMin    Minimum final output amount in raw decimals of final token
     * @param  path            V2 swap path, length >= 2
     * @param  to              Recipient of output tokens
     * @param  deadline        Unix timestamp after which router swap must revert
     * @return amounts         Router amounts array, or [amountIn, totalOut] for split execution
     * @return quotedOutGross  Best single-router quote before slippage deduction
     * @return minOutUsed      Actual minOut enforced during execution
     */
    function _swapV2(
        uint256 amountIn,
        ...
        if (!doSplit) {
            ...
            minOutUsed = _resolveMinOut(
                out1,
                amountOutMin,
                defaultSlippageBpsV2
            );
            ...
            require(
                amounts.length >= 2 && amounts[amounts.length - 1] >= minOutUsed,
                "minOut not met"
            );
        ...
        if (amountOutMin == 0) {
            minOutA = (qA * (10_000 - defaultSlippageBpsV2)) / 10_000;
            minOutB = (qB * (10_000 - defaultSlippageBpsV2)) / 10_000;
        ...
        if (amountA > 0) {
            ...
            require(
                amA.length >= 2 && amA[amA.length - 1] >= minOutA,
                "legA minOut not met"
            );
        ...
        if (amountB > 0) {
            ...
            require(
                amB.length >= 2 && amB[amB.length - 1] >= minOutB,
                "legB minOut not met"
            );
        ...
        if (amountOutMin > 0) {
            require(totalOut >= amountOutMin, "total minOut not met");
        }
    ...
    /**
     * @notice Resolves the final minimum output amount used for swap execution
     * @dev    If `amountOutMin` is provided by the caller, it is used directly
     *         after checking that the current quote is not lower
     *         If `amountOutMin == 0`, the function applies `slippageBps`
     *         to the provided quote
     * @param  quote        Current gross quote before slippage deduction
     * @param  amountOutMin User-provided minimum output amount, or 0 for auto minOut
     * @param  slippageBps  Slippage tolerance in basis points
     * @return minOut Final minimum output amount used by the swap
     */
    function _resolveMinOut(
        uint256 quote,
        uint256 amountOutMin,
        uint16 slippageBps
    ) internal pure returns (uint256 minOut) {
        if (amountOutMin > 0) {
            require(quote >= amountOutMin, "quote < minOut");
            return amountOutMin;
        }
    ...
    /**
     * @notice Find the best single venue among supported routers
     * @param  amountIn    Input amount
     * @param  path        Swap path
    ...
    {
        uint256 u = _safeQuote(UNIV2_ROUTER, amountIn, path);
        uint256 s = _safeQuote(SUSHI_ROUTER, amountIn, path);

        if (u == 0 && s == 0) return (DexKind.NONE, address(0), 0);

        if (u >= s) return (DexKind.UNIV2, UNISWAP_V2_ROUTER_ADDR, u);
        return (DexKind.SUSHIV2, SUSHISWAP_V2_ROUTER_ADDR, s);
    }

    /**
     * @notice Find top-1 and top-2 venues (if any) among supported routers
     * @param  amountIn Input amount
     * @param  path     Swap path
        ...
        )
    {
        ...
        if (u == 0) {
            return (DexKind.SUSHIV2, SUSHISWAP_V2_ROUTER_ADDR, s, DexKind.NONE, address(0), 0);
        }
        if (s == 0) {
            return (DexKind.UNIV2, UNISWAP_V2_ROUTER_ADDR, u, DexKind.NONE, address(0), 0);
        }

        // both are → sort
        if (u >= s) return (DexKind.UNIV2, UNISWAP_V2_ROUTER_ADDR, u, DexKind.SUSHIV2, SUSHISWAP_V2_ROUTER_ADDR, s);
        return (DexKind.SUSHIV2, SUSHISWAP_V2_ROUTER_ADDR, s, DexKind.UNIV2, UNISWAP_V2_ROUTER_ADDR, u);

    }

    /* ───────────────────── Internal: Curve3 safe quote ───────────────────── */

    /**
     * @notice Safely query Curve 3Pool quote for a stable-to-stable swap
     * @dev    Wraps `CURVE3POOL.get_dy(i, j, dx)` in try/catch.
     *         Returns 0 if:
     *           - dx == 0
     *           - the pool call reverts
     * @param  i   Curve 3Pool index of tokenIn
     * @param  j   Curve 3Pool index of tokenOut
     * @param  dx  Input amount (raw decimals of tokenIn)
     * @return out Quoted output amount (raw decimals of tokenOut) or 0 on failure
     */
    function _safeCurve3Quote(int128 i, int128 j, uint256 dx) internal view returns (uint256 out) {
        if (dx == 0) return 0;
        try CURVE3POOL.get_dy(i, j, dx) returns (uint256 dy) {
            return dy;
        } catch {
            return 0;
        }
    ...
    function _pairPrice1e18(address base, address quote) internal view returns (uint256 price1e18, uint256 updatedAt) {
        address reg = FEED_REGISTRY;

        try IFeedRegistry(reg).latestRoundData(base, quote) returns (uint80, int256 answer, uint256, uint256 upd, uint80) {
            updatedAt = upd;
            if (answer <= 0) return (0, updatedAt);

            uint8 dec = IFeedRegistry(reg).decimals(base, quote);
            uint256 u = uint256(answer);

            if (dec < 18) {
                uint8 diff = uint8(18 - dec);
                price1e18 = u * _safePow10(diff);
            } else if (dec > 18) {
                uint8 diff = uint8(dec - 18);
                uint256 div = _safePow10(diff);
                if (div == 0) return (0, updatedAt);
                price1e18 = u / div;
            } else {
                price1e18 = u;
            }
     ...
     * @return ok        True iff oracle prices were available and passed staleness checks (if configured)
     */
    ...
    /**
     * @notice Checks whether path should use the RLUSD -> USDC -> USDT special route
     * @dev    The route is detected by input token RLUSD and final token USDT
     *         The middle token is not used for execution logic in the current version,
     *         but the intended route is RLUSD -> USDC -> USDT
     * @param  path Swap path provided by caller
     * @return True if path matches the RLUSD special route
     */
    function _isRlusdToUsdtRoute(address[] calldata path) internal pure returns (bool) {
        return (
            path.length == 3 &&
            path[0] == RLUSD_ADDRESS &&
            path[2] == USDT_ADDRESS
        );
    }

    /**
     * @notice Checks whether path should use the PYUSD -> best middle token -> USDT route
     * @dev    The route is detected by input token PYUSD and final token USDT
     *         The middle token is treated as a routing hint only
     *         Actual execution compares PYUSD -> USDC -> USDT and
     *         PYUSD -> crvUSD -> USDT, then selects the better route
     * @param  path Swap path provided by caller
     * @return True if path matches the PYUSD special route
     */
    function _isPyusdToUsdtRoute(address[] calldata path) internal pure returns (bool) {
        return (
            path.length == 3 &&
    ...
    /**
     * @notice Quotes RLUSD -> USDC -> USDT
     * @dev    Quote flow:
     *         - Curve RLUSD/USDC pool: RLUSD index 1 -> USDC index 0
     *         - Curve 3Pool: USDC index 1 -> USDT index 2
     * @param  amountIn RLUSD input amount in 18 decimals
     * @return usdcOut  Intermediate USDC output in 6 decimals
     * @return usdtOut  Final USDT output in 6 decimals
     */
    function _quoteRlusdToUsdt(uint256 amountIn)
        internal
        view
        returns (uint256 usdcOut, uint256 usdtOut)
    {
        if (amountIn == 0) return (0, 0);

        try CURVE_RLUSD_USDC_POOL.get_dy(1, 0, amountIn) returns (uint256 out) {
            usdcOut = out;
        } catch {
            return (0, 0);
        }
        ...
        usdtOut = _safeCurve3Quote(1, 2, usdcOut);
    }

    /**
     * @notice Quotes PYUSD -> USDT through both supported Curve routes and returns the best
     * @dev    Compared routes:
     *         - PYUSD -> USDC -> USDT
     *         - PYUSD -> crvUSD -> USDT
     *         The returned `midOut` corresponds to the selected route:
     *         USDC amount if `viaUsdc == true`,
     *         crvUSD amount if `viaUsdc == false`
     * @param  amountIn PYUSD input amount in 6 decimals
     * @return viaUsdc True if PYUSD -> USDC -> USDT is selected
     * @return midOut  Intermediate token output amount
     * @return usdtOut Final USDT output in 6 decimals
     */    
    function _quotePyusdToUsdt(uint256 amountIn)
        internal
        view
        returns (
            bool viaUsdc,
        ...
        )
    {
        ...
        uint256 usdcOut = 0;
        uint256 usdtViaUsdc = 0;

        try CURVE_PYUSD_USDC_POOL.get_dy(0, 1, amountIn) returns (uint256 out) {
            usdcOut = out;
        } catch {}

        if (usdcOut > 0) {
            usdtViaUsdc = _safeCurve3Quote(1, 2, usdcOut);
        }

        // ───────────── PYUSD -> crvUSD -> USDT ─────────────

        uint256 crvUsdOut = 0;
        uint256 usdtViaCrvUsd = 0;

        try CURVE_PYUSD_CRVUSD_POOL.get_dy(0, 1, amountIn) returns (uint256 out) {
            crvUsdOut = out;
        } catch {}

        if (crvUsdOut > 0) {
            try CURVE_CRVUSD_USDT_POOL.get_dy(1, 0, crvUsdOut) returns (uint256 out) {
                usdtViaCrvUsd = out;
            } catch {}
        }
        ...
        if (usdtViaUsdc == 0 && usdtViaCrvUsd == 0) {
            return (false, 0, 0);
        }

        if (usdtViaUsdc >= usdtViaCrvUsd) {
            return (true, usdcOut, usdtViaUsdc);
        }

        return (false, crvUsdOut, usdtViaCrvUsd);
    }
