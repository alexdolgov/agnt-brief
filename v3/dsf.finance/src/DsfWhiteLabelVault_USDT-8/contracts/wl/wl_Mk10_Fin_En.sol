 ...
 *   kind:
 *     0 = deposit      (amountUSDT = NET deposit in USDT)
 *     1 = withdrawal   (amountUSDT = NET withdrawal in USDT)
 *     2 = OUT transfer (from → to, amountUSDT = NET USDT-value of WL shares leaving `from`)
 *     3 = IN transfer  (to ← from, amountUSDT = NET USDT-value of WL shares arriving to `to`)
 *
 *   mode:
 *     0 = Fast         (direct deposit/withdraw/transfer)
 *     1 = Smart        (via smart request)
 *
 *   status:
 *     0 = Pending      (Request created, waiting for completion)
 *     1 = Completed    (Executed)
 *     2 = Cancelled    (Smart request cancelled)
 *
 *   tokenIndex:
 *     0..2  = token index of the pool (DAI/USDC/USDT)
 *     255   = TOKEN_INDEX_NONE — not applicable / USDT-only accounting
 *
 *   amountUSDT:
 *     - for Fast and Completed smart operations — NET amount in USDT
 *     - for Pending smart deposits — 0 (the real token amount is stored in amountToken)
 *
 *   amountToken:
 *     - "raw" token amount of tokenIndex (e.g., how much USDT was deposited or withdrawn)
 *     - for transfers or USDT-only operations, this = 0
 *
 *   otherParty:
 *     - for transfer OUT/IN — the counterparty address (to / from)
 *     - for other operations — address(0)
 *
 *   requestId:
 *     - 0 = no smart request (Fast / transfer)
 *     - >0 = id of the smart request (shared across Pending/Completed/Cancelled)
 *
...
contract DsfWhiteLabelVault_USDT is ERC20, Ownable, Pausable, ReentrancyGuard {
    ...
    // User transaction history (always in USDT + optionally in token)
    struct TxRecord {
        ...
        uint8   tokenIndex; // 0..2 = DAI/USDC/USDT, 255 = NONE / not applicable
        uint64  timestamp;  // block.timestamp
        uint64  requestId;  // 0 = no smart request, >0 = request ID
        uint256 amountUSDT; // NET amount in USDT at operation time
        uint256 amountToken;// amount in tokenIndex (raw units), if applicable
        address otherParty; // counterparty for transfers, else address(0)
    }
    ...
    // Smart-deposit / withdraw события
        ...
        userCooldown
    {
        ...
        // 3) Mintim WL shares proportional to DSF LP
        ...
        if (addedBodyUSDT > 0) {
            ...
            _userTxHistory[msg.sender].push(
                TxRecord({
                    ...
                    mode: 0,
                    status: 1,
                    tokenIndex: depTokenIndex,
                    timestamp: uint64(block.timestamp),
                    requestId: 0,
                    amountUSDT: addedBodyUSDT,
                    amountToken: depAmountToken,
            ...
            if (transferNetUSDT > 0) {
                ...
                _userTxHistory[from].push(
                    TxRecord({
                        ...
                        mode: 0,
                        status: 1,
                        tokenIndex: settleTokenIndex, // calculated in settle token (default USDT)
                        timestamp: uint64(block.timestamp),
                        requestId: 0,
                        amountUSDT: transferNetUSDT,
                        amountToken: 0,
                        otherParty: to
                    })
                );
    ...
    function _completeSmartDepositSingle(address user) internal {
        ...
        // 2) Mintim WL shares to the user
        uint256 tsBefore = totalSupply();
        uint256 wlMint;
        if (tsBefore == 0) {
            wlMint = lpGained;
        ...
        userCooldown
    {
        ...
        _userTxHistory[msg.sender].push(
            TxRecord({
                ...
                timestamp: uint64(block.timestamp),
                requestId: requestId,
                amountUSDT: previewNowUSDT,
                amountToken: previewTokenAmount,
                otherParty: address(0)
            })
        );
