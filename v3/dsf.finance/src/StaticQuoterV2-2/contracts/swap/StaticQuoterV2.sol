...
contract StaticQuoterV2 is IQuoterV2 {
    ...
    /**
     * @notice  Converts sqrtPriceX96 to the nearest tick.
     * @param   sqrtX96 sqrt(P) in Q64.96 format
     * @return  tick corresponding to sqrt(P)
     */
    function sqrtX96ToTick(uint160 sqrtX96) external pure returns (int24) {
        return TickMath.getTickAtSqrtRatio(sqrtX96);
    }

    /**
     * @notice  Decodes sqrt(P) into ticks and prices (token1/token0 and vice versa) taking into account decimals.
     * @dev     Address pairs are submitted in the same way as they were used in the calculation of sqrtAfter; the order is not important.
     * @param   tokenA address of one token in the pair
     * @param   tokenB address of the second token in the pair
     * @param   sqrtAfter sqrt(P) after swap (Q64.96)
     * @return  tickAfter        tick corresponding to sqrtAfter
     * @return  price1Per0_E18   price of token1 for 1 token0 on a scale of 1e18
     * @return  price0Per1_E18   price of token0 for 1 token1 on a scale of 1e18
     * @return  token0           actual token0 of the pair (smaller address)
     * @return  token1           actual token1 of the pair (larger address)
     */
    function decodeSqrtAfter(
        address tokenA,
        address tokenB,
        uint160 sqrtAfter
    )
        ...
        returns (
            int24 tickAfter,
            uint256 price1Per0_E18,
            uint256 price0Per1_E18,
            address token0,
            address token1
        )
    {
        // let's organize the addresses — just like in the pool
        (token0, token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);

        // tick
        tickAfter = TickMath.getTickAtSqrtRatio(sqrtAfter);

        // decimals
        uint8 d0 = IERC20Metadata(token0).decimals();
        uint8 d1 = IERC20Metadata(token1).decimals();

        // price = (sqrt^2 / 2^192) * 10^(d1-d0), return to 1e18
        // carefully calculate the scale: 1e18 * 10^(d1-d0) / 10^(max(0, d0-d1))
        uint256 sqrt = uint256(sqrtAfter);
        uint256 q192 = 1 << 192;
        uint256 sqrt2 = FullMath.mulDiv(sqrt, sqrt, 1); // sqrt^2, secure in 256 bits

        // let's collect scaling (num / den) to avoid overflows
        uint256 num = 1e18;
        uint256 den = 1;

        if (d1 >= d0) {
            unchecked {
                // multiply num by 10^(d1-d0)
                for (uint8 i = 0; i < d1 - d0; i++) num *= 10;
            }
        } else {
            unchecked {
                // multiply den by 10^(d0-d1)
                for (uint8 i = 0; i < d0 - d1; i++) den *= 10;
            }
        }

        // first, divide by 2^192, then by den — both divisions are exact mulDiv
        uint256 tmp = FullMath.mulDiv(sqrt2, num, q192);
        price1Per0_E18 = den == 1 ? tmp : FullMath.mulDiv(tmp, 1, den);

        // price0Per1_E18 = (1e36) / price1Per0_E18  (to also return to 1e18)
        // use mulDiv for accuracy: (1e36 + price1Per0_E18/2) // rounding? — leave without rounding down
        // 1e36 may overflow 256 with direct multiplication, therefore MulDiv:
        // price0 = (1e18 * 1e18) / price1
        price0Per1_E18 = FullMath.mulDiv(1e18, 1e18, price1Per0_E18);
    }
