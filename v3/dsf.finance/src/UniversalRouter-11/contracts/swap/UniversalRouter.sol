...
contract UniversalRouter is Ownable, ReentrancyGuard {
     ...
     * @return splitAmountOut     Best split quote between two absolute tops (0 if split does not improve).
     * @return splitPercentA      Share for route A (in percent, 0–100) for split (0 if split is not applicable).
     * @return balanceIn          wallet balance in tokens (ETH, if tokenIn==address(0))
     * @return allowance_In       tokenIn allowance for spender (0 for ETH)
     * @return balanceOut         wallet balance in tokens (ETH, if tokenOut==address(0))
     * @return allowance_Out      tokenOut allowance for spender (0 for ETH)
    ...
    function getBestRouteSuper(
        ...
        uint256 balanceIn, uint256 allowance_In,
        uint256 balanceOut, uint256 allowance_Out,
        ...
        (balanceIn, allowance_In) = _safeBalanceAndAllowance(tokenIn, wallet, spender);
        (balanceOut, allowance_Out) = _safeBalanceAndAllowance(tokenOut, wallet, spender);
