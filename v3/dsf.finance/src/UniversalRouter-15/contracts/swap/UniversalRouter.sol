...
contract UniversalRouter is Ownable, ReentrancyGuard {
    ...
    function _oracleExpectedOut(
        ...
        (uint256 pIn,  uint256 updIn)  = _tryTokenUsdPrice1e18(tokenIn,  tokenIn == WETH || tokenIn == address(0));
        (uint256 pOut, uint256 updOut) = _tryTokenUsdPrice1e18(tokenOut, tokenOut == WETH || tokenOut == address(0));
        ...
        uint256 usdValue = (amountIn * pIn) / tenIn;
        if (usdValue == 0) return (0, false, minUpd);

        expectedOut = (usdValue * tenOut) / pOut;
        ok = expectedOut > 0;
    }
    ...
    function _enforceOracleGuard(
        ...
        if (!ok) return; // no valid price — guard “silent”
        uint256 minAllowed = (expOut * (10_000 - oracleMaxDeviationBps)) / 10_000;
        ...
        returns (uint256 price1e18, uint256 updatedAt)
    {
        ...
        uint256 tenPow = (decs == 0) ? 1e18 : _safePow10(decs);
     ...
     * @return netOut       Total amount after fees (transferred to the `to` address)
     */
    function swapExactTokensForTokens(
        uint amountIn,
        uint amountOutMin,
        ...
        address to,
        uint deadline
    ) external nonReentrant returns (uint netOut) {
        ...
        (uint[] memory amts, uint256 quotedOut, uint256 minOutUsed) = IDexV2AggregatorModule(agg).swapExactTokensForTokensWithReport(
            amountIn,
            amountOutMin,
            path,
            address(this),
        ...
        require(amts.length >= 2, "UR: bad amounts");
        uint256 grossOut = amts[amts.length - 1];
        require(grossOut >= minOutUsed, "UR: slippage");


        // 3) We cut commissions and send to the user
        netOut = _distributeTokenWithFees(tokenOut, to, grossOut, quotedOut, minOutUsed);
        ...
        returns (uint256 netOut)
    {
        ...
        if (useOracleGuard) {
            _enforceOracleGuard(tokenIn, tokenOut, amountIn, amountOut);
        }
    ...
    function swapRouteExactETHForTokens(
        ...
        if (useOracleGuard) {
            ...
            _enforceOracleGuard(WETH, tokenOut, msg.value, amountOut);
        }
    ...
    function swapRouteExactTokensForETH(
        ...
        if (useOracleGuard) {
            _enforceOracleGuard(tokenIn, WETH, amountIn, outWeth);
        }
    ...
    function swapSplit(
        ...
        require(totalOut >= minAmountOut, "UR: slippage");

        if (useOracleGuard) {
            _enforceOracleGuard(A.tokenIn, A.tokenOut, A.amountIn, totalOut);
        }
    ...
    function swapSplitExactETHForTokens(
        ...
        if (useOracleGuard) {
            _enforceOracleGuard(WETH, A.tokenOut, msg.value, grossOut);
        }
    ...
    function swapSplitExactTokensForETH(
        ...
        if (useOracleGuard) {
            _enforceOracleGuard(A.tokenIn, WETH, A.amountIn, totalWeth);
        }
