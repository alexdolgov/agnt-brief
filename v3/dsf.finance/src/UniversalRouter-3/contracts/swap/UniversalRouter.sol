...
interface ITwapGuardV3 {
    // RefMode: 0=MEDIAN_TWAP, 1=MAX_LIQUIDITY, 2=BEST_TWAP_QUOTE
    function getTwapLowerViaWeth(
        ...
        uint8 mode
    ) external view returns (uint256 lower);
}
...
interface IDexV2AggregatorModule {
    ...
    function swapExactTokensForTokensWithReport(
        uint amountIn,
        uint amountOutMin,          // can be transferred 0 — the aggregator will calculate it itself
        address[] calldata path,
        address to,
        uint deadline
    ) external returns (
        uint[] memory amounts,      // as in V2
        uint256 quotedOutGross,     // baseline before slippage (best GROSS rate)
        uint256 minOutUsed          // what minOut was actually applied (taking into account defaultSlippageBps / split)
    );
}
...
contract UniversalRouter is Ownable, ReentrancyGuard {
    ...
    // Optional: DSF Dex V2 Aggregator (UniV2/Sushi/Pancake aggregator with split)
    address public dexV2Aggregator;

    // TwapGuardV3 module (on-chain TWAP reference via UniswapV3)
    address public twapGuard; // TwapGuardV3 address
    ...
    // Maximum permissible deterioration to expected (Chainlink) result, bps
    uint16 public oracleMaxDeviationBps = 800;               // 8% by default
    // Quote “freshness” limit (both sides must be no older than)
    uint32 public oracleStalenessSeconds = 1 hours;

    // TWAP fallback defaults
    uint32 public twapSeconds    = 60;  // 60s
    uint16 public minLiqRatioBps = 500; // 5%
    uint8  public mode           = 1;   // MAX_LIQUIDITY (или 2)
    ...
    event DexV2AggregatorUpdated(address indexed oldAgg, address indexed newAgg);
    event OracleGuardUpdated(uint16 maxDeviationBps, uint32 stalenessSeconds);
    event TwapGuardUpdated(address indexed oldGuard, address indexed newGuard);
    ...
    /**
     * @notice Sets a new Chainlink Feed Registry address
     * @dev    Validates non-zero and that `reg` is a contract. Does not emit a dedicated event.
     * @param  reg Address of the Feed Registry to use
     */
    ...
    function setOracleGuard(uint16 maxDeviationBps, uint32 stalenessSeconds) external onlyOwner {
        require(maxDeviationBps <= 10_000, "UR: bad dev");
        oracleMaxDeviationBps   = maxDeviationBps;
        oracleStalenessSeconds  = stalenessSeconds;
        emit OracleGuardUpdated(maxDeviationBps, stalenessSeconds);
    }

    /**
     * @notice Sets/updates TwapGuardV3 module address (optional).
     * @param guard  New TwapGuardV3 address or 0x0 to disable
     */
    function setTwapGuard(address guard) external onlyOwner {
        address old = twapGuard;

        if (guard != address(0)) {
            require(guard.code.length > 0, "UR: twapGuard not a contract");
        }

        twapGuard = guard;
        emit TwapGuardUpdated(old, guard);
    }
    ...
    /**
     * @notice Sets the address of the DexV2 aggregator (UniV2/Sushi/Pancake... aggregator with split).
     * @param agg  New aggregator address or 0x0 to reset/disable
     */
    function setDexV2Aggregator(address agg) external onlyOwner {
        address old = dexV2Aggregator;
        if (agg != address(0)) {
            require(agg.code.length > 0, "UR: agg not a contract");
        }
        dexV2Aggregator = agg;
        emit DexV2AggregatorUpdated(old, agg);
    }
    ...
    function setFeeRecipient(address _recipient) external onlyOwner {
        require(_recipient != address(0), "UR: bad feeRecipient");
    ...
    function _addModule(address module) internal {
        ...
        if (module.code.length == 0) revert NotAModule();
    ...
    /**
     * @notice Tries to read {decimals()} from a token; returns 18 for zero-address; 18 if unreadable
     * @dev    Uses a low-level staticcall to tolerate non-standard ERC-20 implementations
     * @param  token Token address (or address(0) treated as ETH)
     * @return decs  Detected decimals (18 for ETH; 18 default if unreadable)
     */
    function _tryDecimals(address token) internal view returns (uint8 decs) {
        if (token == address(0)) return 18;
        (bool ok, bytes memory data) = token.staticcall(abi.encodeWithSelector(bytes4(0x313ce567)));
        if (ok && data.length >= 32) {
            uint256 d = abi.decode(data, (uint256));
            return d > 255 ? uint8(255) : uint8(d);
        }
        // if unsuccessful, we consider 18 to be conservative
        return 18;
    }

    /**
     * @notice Compute expected amountOut via Chainlink USD prices
     * @dev    Uses P_in / P_out with decimals handling and checks staleness against {oracleStalenessSeconds}
     *         Returns ok = false if any price missing or stale; does not revert
     * @param  tokenIn     Input token address
     * @param  tokenOut    Output token address
     * @param  amountIn    Input amount (tokenIn units)
     * @return expectedOut Expected out in tokenOut units
     * @return ok          Whether the expectation is valid (both prices present & fresh)
     * @return minUpd      Minimum of the two feed timestamps used
     */
    function _oracleExpectedOut(
        address tokenIn,
        address tokenOut,
        uint256 amountIn
    ) internal view returns (uint256 expectedOut, bool ok, uint256 minUpd) {
        if (amountIn == 0) return (0, false, 0);

        // read USD prices (1e18); for WETH/ETH, force asEth=true
        (uint256 pIn,  uint256 updIn)  = _tryTokenUsdPrice1e18_CL(tokenIn,  tokenIn == WETH || tokenIn == address(0));
        (uint256 pOut, uint256 updOut) = _tryTokenUsdPrice1e18_CL(tokenOut, tokenOut == WETH || tokenOut == address(0));

        if (pIn == 0 || pOut == 0) return (0, false, 0);

        // checking “freshness”
        minUpd = updIn < updOut ? updIn : updOut;
        if (minUpd == 0 || block.timestamp - minUpd > oracleStalenessSeconds) {
            // stale — we believe that guard is not applicable, but we do not drop the swap
            return (0, false, minUpd);
        }

        uint8 dIn  = _tryDecimals(tokenIn);
        uint8 dOut = _tryDecimals(tokenOut);

        // usd = amountIn * pIn / 10^dIn
        // out = usd * 10^dOut / pOut
        // => out = amountIn * pIn * 10^dOut / (10^dIn * pOut)
        uint256 tenIn  = _safePow10(dIn);
        uint256 tenOut = _safePow10(dOut);

        // accurately: ((amountIn * pIn) / 10^dIn) * 10^dOut / pOut
        uint256 usdValue = _mulDiv(amountIn, pIn, tenIn);
        if (usdValue == 0) return (0, false, minUpd);

        expectedOut = _mulDiv(usdValue, tenOut, pOut);
        ok = expectedOut > 0;
    }

    function _mulDiv(uint256 a, uint256 b, uint256 d) internal pure returns (uint256) {
    // OZ Math.mulDiv — precision multiplication/division without overflow
        return Math.mulDiv(a, b, d);
    }

    /**
     * @notice Enforces the oracle guard by comparing actualOut with a minimum derived from USD prices
     * @dev    If prices are missing/stale the guard is skipped; otherwise requires actualOut >= minAllowed
     * @param  tokenIn   Input token
     * @param  tokenOut  Output token
     * @param  amountIn  Input amount
     * @param  actualOut Actual swap output to validate
     */
    function _enforceOracleGuard(
        ...
        require(maxDevBps <= 10_000, "UR: bad maxDevBps");
        if (maxDevBps == 0) maxDevBps = oracleMaxDeviationBps;
       
        // 1) Chainlink-only oracle guard
        (uint256 expOut, bool ok, ) = _oracleExpectedOut(tokenIn, tokenOut, amountIn);
        if (ok) {
            uint256 minAllowed = (expOut * (10_000 - maxDevBps)) / 10_000;
            require(actualOut >= minAllowed, "UR: oracle guard");
            return; 
        }

        // 2) fallback: UniswapV3 TWAP lower bound via WETH
        uint256 lower = _twapLowerViaWeth(
            tokenIn,
            tokenOut,
            amountIn,
            maxDevBps
        ...
        if (lower > 0) {
            require(actualOut >= lower, "UR: TWAP guard");
        }
        // else: guard not applicable -> do nothing
    }

    /**
     * @notice Returns net amount after applying fixed swap fee (feeBpsSwap)
     * @dev    Does NOT include positive slippage fee (depends on runtime)
     */
    function _netAfterSwapFee(uint256 gross) internal view returns (uint256 net) {
        if (gross == 0) return 0;
        // if the recipient or commission is not set to 0, we return the gross amount
        if (feeRecipient == address(0) || feeBpsSwap == 0) return gross;
        unchecked {
            uint256 fee = (gross * feeBpsSwap) / 10_000;
            if (fee > gross) fee = gross; // we insure against overflows/rounding
            return gross - fee;
        }
    }

    /**
     * @notice Returns the exit quote of the last token on the V2 route via the UniswapV2 public router.
     * @param  amountIn Input token amount.
     * @param  path     Path of the form [tokenIn, ..., tokenOut]; must be ≥ 2 in length.
     * @return          Expected amountOut for the last element of `path`, or 0 on error/illiquidity.
     */
        ...
        returns (uint256)
    {
        ...
        try V2_ROUTER.getAmountsOut(amountIn, path) returns (uint256[] memory amounts) {
            ...
            return 0; // no liquidity/pairs — simply 0
        }
    }

    /**
     * @notice Quick estimate of the TOKEN price in USDT via the USDT→WETH→TOKEN (V2 router) connection.
     * @param token       Address of the token for which the output is estimated.
     * @param amountUsdt  Amount of USDT at the input (e.g., 1_000_000 for 1 USDT with 6 digits).
     * @return            Expected output in TOKEN (in its minimum units) or 0.
     */
        ...
        returns (uint256 price1e18, uint256 updatedAt)
    {
        ...
        try IFeedRegistry(reg).latestRoundData(base, quote) returns (
            ...
            uint8 dec;
            try IFeedRegistry(reg).decimals(base, quote) returns (uint8 d) {
                dec = d;
            } catch {
                return (0, upd);
            }
            ...
            if (dec < 18) {
                ...
                // overflow-guard: if u * mul would overflow => return (0, upd)
        ...
        returns (uint256 price1e18, uint256 updatedAt)
    {
        ...
        if (tokenOutRaw == 0) return (0, 0);

        uint256 tenPow = (decs == 0) ? 1e18 : _safePow10(uint8(decs));
        price1e18 = (usdtUsd1e18 * tenPow) / tokenOutRaw;
        return (price1e18, updU);
    }

    /**
     * @notice Chainlink-only: token USD price (1e18) via Feed Registry. No DEX fallback.
     * @dev    Resolution:
     *         - asEth OR token==0 OR token==WETH: ETH/USD
     *         - else TOKEN/USD
     *         - else TOKEN/ETH × ETH/USD
     */
    function _tryTokenUsdPrice1e18_CL(address token, bool asEth)
        ...
        returns (uint256 price1e18, uint256 updatedAt)
    {
        // ETH or forced-as-ETH or WETH -> ETH/USD
        if (asEth || token == address(0) || token == WETH) {
            (price1e18, updatedAt) = _pairPrice1e18(DENOM_ETH, DENOM_USD);
    ...
    /**
     * @notice Safely computes 10**{decs} without risking overflow
     * @dev    The maximum exponent that fits in uint256 is 77, since 10**78 > 2**256
     *         Values above 77 are clamped down to 77
     * @param  decs  Requested number of decimals (e.g. ERC-20 decimals)
     * @return power 10 raised to the safe exponent, i.e. 10**min(decs,77)
     */
    function _safePow10(uint8 decs) internal pure returns (uint256) {
        // 10**78 no longer fits in uint256; 10**77 < 2**256
        uint8 safeDecs = decs > 77 ? 77 : decs;
        unchecked { 
            return 10 ** uint256(safeDecs);
        }
    }

    function _twapLowerViaWeth(
        address tokenIn,
        address tokenOut,
        uint256 amountIn,
        uint16  maxDeviationBps
    ) internal view returns (uint256 outLower) {
        address tg = twapGuard;
        if (tg == address(0) || tg.code.length == 0) return 0;
        if (amountIn == 0) return 0;

        // TwapGuardV3 has _toUint128(amountIn) and it REVERTS if amountIn > uint128.
        // For safety, we simply return 0 (fallback does not trigger).
        if (amountIn > type(uint128).max) return 0;

        // We don't let TwapGuardV3 reversals (NoValidPools, TwapSecondsZero, etc.) overload the entire router.
        // We return 0 and let the upper level decide what to do next.
        try ITwapGuardV3(tg).getTwapLowerViaWeth(
            tokenIn,
            tokenOut,
            amountIn,
            twapSeconds,
    ...
    /**
     * @notice Checks whether OracleGuard reference pricing is supported for a token.
     * @dev    This function DOES NOT enforce any protection and does not validate swap execution.
     *         It only verifies that a fresh Chainlink-based USD price can be obtained for the token.
     *
     *         Resolution order (Chainlink-only, no DEX fallback):
     *         1) ETH/USD                — if token is ETH, WETH, or asEth=true
     *         2) TOKEN/USD              — direct feed
     *         3) TOKEN/ETH × ETH/USD    — composed feed
     *
     *         The price is considered valid only if:
     *         - price != 0
     *         - updatedAt != 0
     *         - block.timestamp - updatedAt <= oracleStalenessSeconds
     *
     * @param  token   Token address being checked
     * @param  asEth   If true, token is treated as ETH for pricing purposes (UI/route context)
     *
     * @return supported  True if a fresh Chainlink-based USD reference price is available
     */
    function supportsOracleGuardToken(address token, bool asEth)
        public
        view
        returns (bool)
    {
        (uint256 p, uint256 upd) = _tryTokenUsdPrice1e18_CL(token, asEth);
        if (p == 0 || upd == 0) return false;
        if (block.timestamp - upd > oracleStalenessSeconds) return false;
        return true;
    }

    /**
     * @notice Checks whether OracleGuard reference pricing is supported for a token pair.
     * @dev    Returns true only if BOTH tokenIn and tokenOut have valid, non-stale
     *         Chainlink-based USD reference prices.
     *
     *         This function is intended for:
     *         - UI capability checks
     *         - route/quote metadata
     *
     *         It does NOT imply that OracleGuard is enforced on-chain.
     *
     * @param  tokenIn     Input token address
     * @param  tokenOut    Output token address
     * @param  asEthIn     Treat tokenIn as ETH for pricing purposes
     * @param  asEthOut    Treat tokenOut as ETH for pricing purposes
     *
     * @return supported  True if OracleGuard reference pricing is available for both tokens
     */
    function supportsOracleGuardPair(
        address tokenIn,
        address tokenOut,
        bool asEthIn,
        bool asEthOut
    ) public view returns (bool) {
        return supportsOracleGuardToken(tokenIn, asEthIn)
            && supportsOracleGuardToken(tokenOut, asEthOut);
    }
     ...
     * @return decimals_       Token decimals (18 for ETH; 0 if unreadable)
     * @return ethBalance      Native ETH balance of {wallet} (wei)
     */
        ...
        )
    {
        ...
        // 3) Always return native ETH balance of the wallet
    ...
    function getBestRoute(
        ...
        best1HopRouteTop1 = best.top1Hop.payload; amountOut1HopTop1 = _netAfterSwapFee(best.top1Hop.amountOut);
        best2HopRouteTop1 = best.top2Hop.payload; amountOut2HopTop1 = _netAfterSwapFee(best.top2Hop.amountOut);
        best1HopRouteTop2 = best.second1Hop.payload; amountOut1HopTop2 = _netAfterSwapFee(best.second1Hop.amountOut);
        best2HopRouteTop2 = best.second2Hop.payload; amountOut2HopTop2 = _netAfterSwapFee(best.second2Hop.amountOut);
        ...
        if (top2Overall.amountOut > 0 && keccak256(top1Overall.payload) != keccak256(top2Overall.payload)) {
            ...
            uint256 splitAmountOutNet = _netAfterSwapFee(splitAmountOut);

            // If split provides no improvement, do not return it,
            // since the best will be either T1 or T2 (T1.amountOut >= T2.amountOut).
            if (splitAmountOutNet <= _netAfterSwapFee(top1Overall.amountOut)) {
                 splitAmountOut = 0;
                 splitPercentA = 0;
            } else {
                splitAmountOut = splitAmountOutNet;
            }
        } else {
     ...
     * @return updatedAtOut         Timestamp of the last price update for tokenOut
     *
     * @return ethBalance           {wallet} balance ETH 
     *
     * @return oracleGuardSupported OracleGuard support flag
     */
    function getBestRouteSuper(
        ...
        oracleGuardSupported = supportsOracleGuardPair(tokenIn, tokenOut, asEthIn, asEthOut);
    }
    ...
    /**
     * @notice Proxies the `getAmountsOut` quote to the connected DexV2 aggregator.
     * @param  amountIn   Input token amount
     * @param  path       Path of the form [tokenIn, ..., tokenOut]
     * @return amountsNet Array of amounts at each step of the path (as in UniswapV2), generated by the aggregator
     */
    ...
    {
        require(dexV2Aggregator != address(0), "UR: V2 agg not set");

        amountsNet = IDexV2AggregatorModule(dexV2Aggregator).getAmountsOut(amountIn, path); // GROSS
        
        if (amountsNet.length >= 2) {
            amountsNet[amountsNet.length - 1] = _netAfterSwapFee(amountsNet[amountsNet.length - 1]);
        }
    }
    ...
    /**
     * @notice Performs a Token→Token swap via a connected DexV2 aggregator (Uni/Sushi, etc.)
     * @param  amountIn     Input token amount (taken from msg.sender)
     * @param  amountOutMin Minimum acceptable output (slippage)
     * @param  path         Route of the form [tokenIn, ..., tokenOut]
     * @param  to           Recipient of the final token
     * @param  deadline     Transaction deadline for the aggregator (as in UniswapV2)
     * @return amounts      Total amount after fees (transferred to the `to` address)
     */
    function swapExactTokensForTokens(
        uint amountIn,
        uint amountOutMin,
        ...
        address agg = dexV2Aggregator;
        require(agg != address(0), "UR: V2 agg not set");
        require(path.length >= 2, "UR: path too short");
        require(amountIn > 0, "UR: zero in");

        address tokenIn  = path[0];
        address tokenOut = path[path.length - 1];

        // 1) We take the tokens and approve the aggregator
        IERC20(tokenIn).safeTransferFrom(msg.sender, address(this), amountIn);
        _smartApprove(tokenIn, agg, amountIn);

        // 2) We use the user's minOut directly (the aggregator inside can also split)
        uint256 quotedOut;
        uint256 minOutUsed;
        (amounts, quotedOut, minOutUsed) = IDexV2AggregatorModule(agg).swapExactTokensForTokensWithReport(
            amountIn,
            amountOutMin,
            path,
            address(this),
        ...
        require(amounts.length >= 2, "UR: bad amounts");
        uint256 grossOut = amounts[amounts.length - 1];
        require(grossOut >= minOutUsed, "UR: slippage");

        // 3) We cut commissions and send to the user
        uint256 netOut = _distributeTokenWithFees(tokenOut, to, grossOut, quotedOut, minOutUsed);

        amounts[amounts.length - 1] = netOut;

        emit SwapExecuted(agg, msg.sender, to, tokenIn, tokenOut, amountIn, netOut, quotedOut);
     ...
     * @param  useOracleGuard    If true, validate result against oracle guard thresholds
     * @param  maxDevBps         Slippage
        ...
        returns (uint256 netOut)
    {
        ...
        if (useOracleGuard) {
            _enforceOracleGuard(tokenIn, tokenOut, amountIn, amountOut, maxDevBps);
        }
        ...
        require(netOut >= minAmountOut, "UR: slippage");
    ...
    function swapRouteExactETHForTokens(
        ...
        if (useOracleGuard) {
            // tokenIn = WETH; amountIn = msg.value
            _enforceOracleGuard(WETH, tokenOut, msg.value, amountOut, maxDevBps);
        }
    ...
    function swapRouteExactTokensForETH(
        ...
        if (useOracleGuard) {
            _enforceOracleGuard(tokenIn, WETH, amountIn, outWeth, maxDevBps);
        }

        netEthOut = _distributeETHWithFees(to, outWeth, quotedOut, minAmountOut);
        require(netEthOut >= minAmountOut, "UR: slippage");
    ...
    function swapSplit(
        ...
        if (useOracleGuard) {
            _enforceOracleGuard(A.tokenIn, A.tokenOut, A.amountIn, totalOut, maxDevBps);
        }

        uint256 quotedTotal = (A.quoted * percentA) / 100 + (B.quoted * (uint16(100 - percentA))) / 100;

        // Commission + payment to user
        netOut = _distributeTokenWithFees(A.tokenOut, to, totalOut, quotedTotal, minAmountOut);
        require(netOut >= minAmountOut, "UR: slippage");
    ...
    function swapSplitExactETHForTokens(
        ...
        if (useOracleGuard) {
            _enforceOracleGuard(WETH, A.tokenOut, msg.value, grossOut, maxDevBps);
        }
        ...
        require(netOut >= minTotalOut, "UR: slippage");
    ...
    function swapSplitExactTokensForETH(
        ...
        if (useOracleGuard) {
            _enforceOracleGuard(A.tokenIn, WETH, A.amountIn, totalWeth, maxDevBps);
        }
        ...
        require(netEthOut >= minTotalEthOut, "UR: slippage");
