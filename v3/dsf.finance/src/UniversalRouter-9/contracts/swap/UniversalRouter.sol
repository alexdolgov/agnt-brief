...
contract UniversalRouter is Ownable, ReentrancyGuard {
     ...
     * @return amounts  Array of amounts at each step of the path (as in UniswapV2), generated by the aggregator
     */
        ...
        returns (uint[] memory amounts)
    {
        ...
        return IDexV2AggregatorModule(dexV2Aggregator).getAmountsOut(amountIn, path);
    }
    ...
    function swapExactTokensForTokens(
        ...
        uint[] memory amts = IDexV2AggregatorModule(agg).swapExactTokensForTokens(
            amountIn,
            amountOutMin,
            path,
            address(this),
        ...
        require(grossOut >= amountOutMin, "UR: slippage");

        // 3) We cut commissions and send to the user
        // We take quotedOut as “waiting”: if necessary, you can call getAmountsOut in advance and pass last()
        uint256 quotedOut = grossOut; // conservative (zero positive part)
        netOut = _distributeTokenWithFees(tokenOut, to, grossOut, quotedOut, amountOutMin);
        ...
        returns (uint256 netOut)
    {
        ...
        LegDecoded memory d      = _decodeRouteStructCallData(payload);
        address module           = d.module;
        uint256 quotedOut        = d.quoted;
        address tokenIn          = d.tokenIn;
        address tokenOut         = d.tokenOut;
        uint256 amountIn         = d.amountIn;
        bytes[] memory routeData = d.route;
