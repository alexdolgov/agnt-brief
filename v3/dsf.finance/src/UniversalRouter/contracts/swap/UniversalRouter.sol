...
contract UniversalRouter is Ownable, ReentrancyGuard {
    ...
    uint256 public maxFeedDelay = 1 hours;
    ...
    function setFeedRegistry(address reg) external onlyOwner { feedRegistry = reg; }
    
    function setMaxFeedDelay(uint256 seconds_) external onlyOwner { maxFeedDelay = seconds_; }
    ...
    /**
     * @notice Attempts to obtain the price of 1 token in USD via Chainlink Feed Registry and normalizes it to 1e18.
     * @dev    Does NOT revert: if there is no registry/feed or the response is incorrect, it will return ok=false and a price of 0.
     *         The freshness of the price is not checked here — this is done by the calling function (compares updatedAt with maxFeedDelay).
     *         ETH is passed as address(0) and mapped internally to Chainlink DENOM_ETH.
     * @param  token     Token address (or address(0) for ETH).
     * @return ok        True if the price is received and > 0 (without checking for staleness).
     * @return price1e18 Price of 1 token in USD, scaled to 1e18 (0 if ok=false).
     * @return updatedAt Timestamp of the last feed update (unix seconds).
     */
    function _tryTokenUsdPrice1e18(address token, bool asEth)
        internal
        view
        returns (bool ok, uint256 price1e18, uint256 updatedAt)
    {
        address reg = feedRegistry;
        if (reg == address(0)) return (false, 0, 0);
        ...
        if (answer <= 0) return (false, 0, upd);
        ...
        return (true, price1e18, upd);
    }

    /* ──────────────────────────────────── Read ───────────────────────────────── */

    /**
     * @notice Soft retrieval of the price of 1 token in USD (normalized to 1e18) via Chainlink Feed Registry.
     * @dev    Returns ok=false and 0-price if the feed is missing or outdated (updatedAt is older than maxFeedDelay).
     *         ETH is passed as address(0).
     * @param  token           Token address (or address(0) for ETH).
     * @return ok              True if the price is valid and not stale.
     * @return usdPerToken1e18 Price of 1 token in USD in 1e18 (0 if ok=false).
     * @return updatedAt       Timestamp of the last price update in Chainlink.
     */
    function tryTokenUsdPrice1e18(address token, bool asEth)
        external
        view
        returns (bool ok, uint256 usdPerToken1e18, uint256 updatedAt)
    {
        (ok, usdPerToken1e18, updatedAt) = _tryTokenUsdPrice1e18(token, asEth);
        if (!ok) return (false, 0, updatedAt);
        if (block.timestamp - updatedAt > maxFeedDelay) return (false, 0, updatedAt);
        return (true, usdPerToken1e18, updatedAt);
    }
    ...
    /**
     * @notice Returns (balance, allow) and the price of 1 token in USD (1e18) with a validity flag.
     * @dev    ETH = address(0). The USD price is taken from Chainlink Feed Registry/_tryTokenUsdPrice1e18.
     * @param  token           ERC-20 token address (or address(0) for ETH)
     * @param  wallet          Address of the owner of the funds
     ...
     * @return hasUsd          True if the price exists and has not expired (<= maxFeedDelay)
     * @return usdPerToken1e18 Price of 1 token in USD, normalized to 1e18 (0 if hasUsd=false)
     * @return updatedAt       Time of the last price update from the chainlink
     */
        ...
        )
    {
        ...
        (hasUsd, usdPerToken1e18, updatedAt) = _tryTokenUsdPrice1e18(token, asEth);

        // 3) checking the “freshness” of the price
        if (!hasUsd || block.timestamp - updatedAt > maxFeedDelay) {
            hasUsd = false;
            usdPerToken1e18 = 0;
        }
    }
     ...
     * @return balance            wallet balance in tokens (ETH, if token==address(0))
     * @return allowance_         token allowance for spender (0 for ETH)
     * @return hasUsdIn           true if the price is available and has not expired (<= maxFeedDelay)
     ...
     * @return hasUsdOut          true if the price is available and has not expired (<= maxFeedDelay)
    ...
    function getBestRouteSuper(
        ...
        (hasUsdIn, usdPerToken1e18In, updatedAtIn) = _tryTokenUsdPrice1e18(tokenIn, asEthIn);
        (hasUsdOut, usdPerToken1e18Out, updatedAtOut) = _tryTokenUsdPrice1e18(tokenOut, asEthOut);

        // 3) checking the “freshness” of the price
        if (!hasUsdIn || block.timestamp - updatedAtIn > maxFeedDelay) {
            hasUsdIn = false;
            usdPerToken1e18In = 0;
        }
        if (!hasUsdOut || block.timestamp - updatedAtOut > maxFeedDelay) {
            hasUsdOut = false;
            usdPerToken1e18Out = 0;
        }
    }
