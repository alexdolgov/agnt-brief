...
// File: ElephantNFTGov.sol


/*

    Elephant Money NFT Governance
...
//Sends a percentage of the Elephant Treasury to an address
contract AprForwardingTreasuryStrategy is ElephantCore {
    using SafeMath for uint256;
   
    uint256 public liquidityThreshold = 100e18;
    
    uint256 public constant apr_precision = 1e7;
    uint256 public daily_apr = 275; //1% for the year 
    uint256 public lastSweep;

    address public destination;

    event UpdateDailyAPR(uint oldApr, uint newApr);
    event UpdateDestination(address oldDest, address newDest);
    event Sweep(address destination, uint amount);
    ...
    constructor ()  ElephantCore() {
     ...
     /// @dev Update the daily APR of the strategy
    function updateDailyAPR(uint apr) onlyOwner external {
        require(apr >= 137 && apr <= 550, "Daily APR out of range 137 - 550 equivalent 0.5 - 2% annual APR");
    ...
    function updateDestination(address _destination) onlyOwner external {
        require(destination != address(0) && destination != _destination, "destination not set");

        emit UpdateDestination(destination, _destination);
        destination = _destination;
    }

    //Estimate daily amounts
    function dailyEstimate() public view returns (uint coreAmount, uint collateralAmount) {
        //Calculate daily drip 

        uint256 coreTreasuryBalance = coreToken.balanceOf(address(coreTreasury));

        //What is the amount per day?
        coreAmount = coreTreasuryBalance.mul(daily_apr).div(apr_precision);
    ...
    function sweep() isRunning nonReentrant public returns (uint coreAmount, uint collateralAmount) {
        ...
        if (collateralAmount > liquidityThreshold){
            ...
            coreToken.approve(destination, coreAmount);

            IFund(destination).fund(coreAmount);

            lastSweep = block.timestamp;

            emit Sweep(destination, coreAmount);
        }
