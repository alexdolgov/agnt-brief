...
contract BakeryAdapter is IDODOAdapter {
    using SafeMath for uint;

    //fromToken == token0
    function sellBase(address to, address pool, bytes memory) external override {
        address baseToken = IBakery(pool).token0();
        (uint reserveIn, uint reserveOut,) = IBakery(pool).getReserves();
        ...
        IBakery(pool).swap(0, receiveQuoteAmount, to);
    }

    //fromToken == token1
    function sellQuote(address to, address pool, bytes memory) external override {
        address quoteToken = IBakery(pool).token1();
        (uint reserveOut, uint reserveIn,) = IBakery(pool).getReserves();
        require(reserveIn > 0 && reserveOut > 0, 'UniAdapter: INSUFFICIENT_LIQUIDITY');
        ...
        IBakery(pool).swap(receiveBaseAmount, 0, to);
    }
}