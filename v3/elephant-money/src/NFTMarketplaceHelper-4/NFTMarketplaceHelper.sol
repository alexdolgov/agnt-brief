...
contract NFTMarketplaceHelper is ReentrancyGuard, Ownable, ERC721Holder {
    ...
    function getTokenInfo(uint256[] memory tokenIds) external view returns (TokenInfo[] memory results){
        ...
        for (uint i = 0; i < tokenIds.length; i++ ) {
            tokenId = tokenIds[i];
            (rare, hue, sat, lum, score, weight, uri) = _tokenInfo(tokenId);
            results[i] = TokenInfo(tokenId, rare, hue, sat, lum, score, weight, uri);
        } 

    }

    function _tokenInfo(uint256 _tokenId) internal view returns (uint rare, uint hue, uint sat, uint lum, uint score, uint weight, string memory uri)  {

         (rare, hue, sat, lum, score) = tracker.getInfo(_tokenId);

        weight = 1e18 + (score * staking.rarityWeight());

        uri = nft.tokenURI(_tokenId);
...
contract NFTSalesTracker is Whitelist {
    ...
    function getPrice(uint lookback) external view returns (uint averagePrice, uint lastPrice) {
        
        require(lookback <= totalSold, "lookback greater than sales");
        uint spend;

        lastPrice = sales[totalSold - 1].price;

        for (uint i=totalSold - 1; lookback > 0; i--) {
            spend += sales[i].price;
            lookback--;
        } 
