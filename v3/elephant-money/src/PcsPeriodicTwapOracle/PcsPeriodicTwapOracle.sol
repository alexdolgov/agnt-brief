...
// File: PcsPeriodicTwapOracle.sol
...
contract PcsPeriodicTwapOracle is Whitelist{
    ...
    event UpdatePeriodSize(uint old_period_size, uint period_size);

    constructor(address factory_, uint periodSize_) Ownable() public {
    ...
    function updatePeriodSize(uint periodSize_) external onlyOwner {
        require(periodSize_ > 1 minutes, 'periodSize must be greater than 60 seconds');
    ...
    function updateAll() public {
        ...
        for (uint i; i < registry.length; i++){
            ...
            update(pair.token0(), pair.token1());
        }
    }
    ...
    function consult(address tokenIn, uint amountIn, address tokenOut) public view returns (uint amountOut) {
        ...
        Observation storage firstObservation = pairObservation[pair];
        
        require(firstObservation.timestamp > 0 , 'PcsPeriodicOracle: PAIR_UNINITIALIZED'); 
    ...
    function update(address tokenA, address tokenB) private returns (address pair, uint timestamp) {
