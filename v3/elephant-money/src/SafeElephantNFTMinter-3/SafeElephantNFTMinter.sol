...
contract AddressRegistry {
    ...
    address public constant backedAddress = address(0xdd325C38b12903B727D16961e61333f4871A70E0); //TRUNK
    address public constant routerAddress = address(0x10ED43C718714eb63d5aA57B78B54704E256024E);  
    address public constant oracleAddress = address(0xb9394B2dD11115721D93A6f05215f81c54893861);
    address public constant chainlinkBNBAddress = address(0x0567F2323251f0Aab15c8dFb1967E4e8A7D42aeE); //BNB/USD Price
    address public constant nftAddress = address(0xb92afeDC8f8618BE4198fbE5d97adB7C60aB3198); //NFT
...
contract SafeElephantNFTMinter is Ownable, ReentrancyGuard {
    ...
    //After construction needs to be added to the whitelist of the NFT and oracle
    constructor ()  Ownable() {
        ...
        //setup tokens
        nft = IElephantNFT(registry.nftAddress());
    ...
    function buyForTreasury() nonReentrant external payable {
        ...
        uint[] memory output =  oracle.consultAmountsOut(msg.value, path);

        uint minimum =  output[1] * slippage / 1000;


        //buy immediately and send to the treasury
        collateralRouter.swapExactETHForTokensSupportingFeeOnTransferTokens{value:msg.value}(minimum, path, registry.coreTreasuryAddress(), block.timestamp);

        //stats
        deposited += msg.value;
        busd_value_deposited += estimateCollateralAmount(msg.value); //cheapest way to calculate APR denominator
        txs++;
