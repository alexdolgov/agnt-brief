...
contract AddressRegistry {
    ...
    address public constant wrappedAddress = address(0x241F21dF529C05289A00dAfeCEa10139A287cDCa); //WELEPHANT 2.0
...
interface IWElephant {
    ...
    function wrap(uint256 elephantAmount) external returns (uint256 welephantAmount, uint256 fee);
...
contract WElephantRouterProxy is Ownable {
    ...
    function swapExactETHForWrapped(
        ...
        (wrappedAmount,) = welephant.wrap(totalAmount);
    ...
    function bestPath(uint wethAmount) public view returns (address[] memory path, uint directAmount, uint indirectAmount, uint estimateAmount) {
        ...
        bool isIndirect = indirectAmount > directAmount;
        path = isIndirect ? indirect : direct;
        estimateAmount = isIndirect ? indirectAmount : directAmount;
