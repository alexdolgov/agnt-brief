...
contract AddressRegistry {
    ...
    address public constant wrappedAddress = address(0xA546d072BF7656D10bFedACD4B93a5358C4D8E47); //WELEPHANT
...
interface IWElephant {

    function wrap(uint256 elephantAmount) external;
    function unwrap(uint256 welephantAmount) external;
...
contract WElephantRouterProxy is Ownable {
    ...
    IERC20 immutable private wrappedToken;
    IWElephant immutable private welephant;
    ...
    constructor () public Ownable() {
        ...
        wrappedToken = IERC20(registry.wrappedAddress());
    
        welephant = IWElephant(registry.wrappedAddress());
      
    }

    receive() payable external {}

    function swapExactETHForWrapped(
        uint amountOutMin,
        address to
    ) external payable returns (uint totalAmount, uint wrappedAmount) {
        ...
        //approve received tokens for wrapping
        coreToken.approve(registry.wrappedAddress(), totalAmount);

        (wrappedAmount,) = welephant.previewWrap(totalAmount);

        welephant.wrap(totalAmount);

        //Send wrapped tokens 
        wrappedToken.transfer(to, wrappedAmount);
