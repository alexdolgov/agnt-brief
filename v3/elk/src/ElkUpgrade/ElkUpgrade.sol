...
contract ElkUpgrade is ReentrancyGuard, Ownable {
    using SafeMath for uint256;
    ...
    constructor(address fromToken_, address toToken_, bool upgradeStatus_) public {
        fromToken = IERC20(fromToken_);
        toToken = IERC20(toToken_);
        upgradeStatus = upgradeStatus_;
        emit UpgradeStatusChanged(upgradeStatus);
    }

    /* ========== MUTATIVE FUNCTIONS ========== */

    function upgrade(uint256 amount) external nonReentrant {
        require(amount > 0, "ElkUpgrade::upgrade: cannot upgrade 0!");
        require(upgradeStatus, "ElkUpgrade::upgrade: currently not accepting upgrades!");
        require(fromToken.balanceOf(msg.sender) >= amount, "ElkUpgrade::upgrade: not enough balance in sender's wallet!");
        require(toToken.balanceOf(address(this)) >= amount, "ElkUpgrade::upgrade: not enough balance in upgrade contract wallet!");
        fromToken.safeTransferFrom(msg.sender, address(this), amount);
        toToken.safeTransfer(msg.sender, amount);
        emit Upgraded(msg.sender, amount);
    }
    ...
    event UpgradeStatusChanged(bool status);
