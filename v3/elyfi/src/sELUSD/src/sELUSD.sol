...
import {Silo} from "src/utils/Silo.sol";
...
/// @title sELUSD
/// @notice Staked ELUSD token with cooldown, vesting, and blacklisting features
contract sELUSD is ERC4626, ERC20Permit, SingleAdminAccessControl, Pausable {
    ...
    /*//////////////////////////////////////////////////////////////
                             INITIALIZATION
    //////////////////////////////////////////////////////////////*/
    constructor(address _ELUSD, address _owner)
        ERC20("Staked ELUSD", "sELUSD")
        ERC4626(IERC20(_ELUSD))
        ERC20Permit("Staked ELUSD")
    {
        _grantRole(DEFAULT_ADMIN_ROLE, _owner);
        silo = new Silo(address(this), _ELUSD);
        _setVestingPeriod(8 hours);
        _setCooldownDuration(0);
    }
    ...
    function redistributeLockedAmount(address from, address to) external onlyRole(DEFAULT_ADMIN_ROLE) {
        if (hasRole(FULL_RESTRICTED_STAKER_ROLE, from) && !hasRole(FULL_RESTRICTED_STAKER_ROLE, to)) {
            ...
            // asset is ELUSD
            uint256 elusdToVest = previewRedeem(amountToDistribute);
            _burn(from, amountToDistribute);
            // to address of address(0) enables burning
            if (to == address(0)) {
                _updateVestingAmount(elusdToVest);
            } else {
    ...
    /// @notice Returns the amount of ELUSD tokens that are vested in the contract.
    function totalAssets() public view override returns (uint256) {
        return IERC20(asset()).balanceOf(address(this)) - getUnvestedAmount();
    }

    /// @notice Returns the amount of ELUSD tokens that are unvested in the contract.
