...
import { ensureNonzeroAddress, ensureNonzeroValue } from "../../lib/validators.sol";
import { IERC20Metadata } from "@openzeppelin/contracts/token/ERC20/extensions/IERC20Metadata.sol";
import { CappedOracle } from "./CappedOracle.sol";

/**
 * @title CappedCorrelatedTokenOracle
 * @notice This oracle fetches the price of a token that is correlated to another token.
 */
abstract contract CappedCorrelatedTokenOracle is CappedOracle {
    ...
    function getUncappedPrice(address asset) internal view override returns (uint256) {
        ...
        IERC20Metadata token2 = IERC20Metadata(UNDERLYING_TOKEN);
        uint256 decimalUnderlying = token2.decimals();

        uint256 decimalDelta = decimals - decimalUnderlying; //tofix if decimals < decimalUnderlying

        return (underlyingAmount * underlyingUSDPrice) / ((10 ** decimals)*(10**decimalDelta));
    }
