/*
	...
	./ElitenessFarmland.sol

	Based on ThickALMGaugeEquivalentFarmland

	Changes:
	- Support Deposits of Fee-on-Transfer Tokens
...
contract ElitenessFarmland is ReentrancyGuard {
    using SafeMath for uint256;
    ...
    */
    ...
    function name() external view returns (string memory) {
        return string(abi.encodePacked("Staked ", stake.name()));
    }

    function symbol() external view returns (string memory) {
        return string(abi.encodePacked("stk", stake.symbol()));
    }
    ...
    function deposit(uint256 amount) public nonReentrant notPaused updateReward(msg.sender) {
        ...
        {
        	uint _sbb = stake.balanceOf(address(this));
        	stake.safeTransferFrom(msg.sender, address(this), amount);
        	amount = stake.balanceOf(address(this)) - _sbb;
        }
    ...
    function withdraw(uint256 amount) public nonReentrant updateReward(msg.sender) {
        ...
        //_claimFees();
        require( _totalSupply > 0, "0t"); // Cannot withdraw 0
    }
    ...
    function notifyRewardAmount(address _rewardsToken, uint256 _reward) external nonReentrant updateReward(address(0)) {
        ...
        require(_reward>rewardData[_rewardsToken].rewardsDuration, "rr0");
    ...
    function _claimFees() internal returns (uint claimed0, uint claimed1)  {
        if(tradeFeesCollector==address(0)){
        	return (0,0);
        }
    ...
    */
    ...
    function tvl() public view returns (uint _tvl) {
        ...
        for(uint i; i<rewardTokens.length; i++) {
            ...
            _tvl += ( ( IERC20(rewardTokens[i]).balanceOf(address(this)) - (rewardTokens[i]==address(stake)?_totalSupply:0) ) * _pt) / (10**IERC20(rewardTokens[i]).decimals());
        }
    }
