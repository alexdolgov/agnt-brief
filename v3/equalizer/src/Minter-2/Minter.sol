...
contract Minter is Initializable {
    ...
    function update_period() external returns (uint) {
        ...
        if (block.timestamp >= _period + WEEK && owner == address(0)) { // only trigger if new week
            ...
            uint _equalWftmEmissions = (POOL2_RATE * weekly) / PRECISION;
            ...
            if(pool2 != address(0)) {
            	_equal.approve(pool2, _equalWftmEmissions);
            	IGauge(pool2).notifyRewardAmount(address(_equal), _equalWftmEmissions);
            }
