...
abstract contract Enumerable {
    ...
    function allTokensOf(address _user) public view returns (uint256[] memory) {
        uint _bal = balanceOf(_user);
        uint[] memory _ids = new uint[](_bal);
        for(uint i; i<_bal; i++) {
    ...
    function ownerOf(uint256 _id) public view returns (address) {
        require(_ownerOfToken[_id] != address(0), "Enumerable: address zero is not a valid owner");
        return _ownerOfToken[_id];
    }
    ...
    function _addTokenToOwnerEnumeration(address to, uint256 tokenId) private {
        ...
        require(_ownerOfToken[tokenId] == address(0), "cant overwrite owner");
    ...
    function _removeTokenFromOwnerEnumeration(address from, uint256 tokenId) private {
        ...
        require(_ownerOfToken[tokenId] == from, "cant override owner");
