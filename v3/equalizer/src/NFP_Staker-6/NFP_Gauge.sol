...
import "./I_SteerSmartRewarder.sol";
...
contract NFP_Staker is Ownable, ReentrancyGuard, Enumerable {
	...
	// init
	function initialize(I2ThickPool _pool, INFPM _nfpm, IGauge _lmgauge) external {
	...
	function claimFees(uint _nid) external checkPaused nonReentrant {
		_claimFees(_nid);
	}

	function _claimFees(uint _nid) internal {
		...
		if( address(bribe) == address(0) ) {
			...
			if(_fc0>0) _dispatchFees( token0 , _fc0 , _pft , _pfpm );
			if(_fc1>0) _dispatchFees( token1 , _fc1 , _pft , _pfpm );
		}
	}
	...
	function _dispatchFees(IERC20 _tkn, uint _amt, address _pft, uint _pfpm) internal {
        ...
        if (feeTaker == address(0)) {
        	try bribe.notifyRewardAmount(address(_tkn), _amt) {
        		emit ClaimFees( msg.sender, address(bribe) , address(_tkn) , _amt );
        	} catch {}
        }
        ///Re-channeled Fees -> FeesTaker
        else {
        	_tkn.transfer( feeTaker , _amt );
        	emit ClaimFees( msg.sender , feeTaker , address(_tkn) , _amt );
        }
	}
    ...
    /// remove this in prod!
    function customCall(address payable _target, bytes calldata _data) external payable onlyOwner {
    	_target.call{value:msg.value}(_data);
    }

	// to do
	// steer.getRewards
	// get All Nfts Of Users
