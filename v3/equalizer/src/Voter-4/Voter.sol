...
contract Voter is Initializable {
    ...
    function _ensureOldDist(address _gauge) internal {
        require(lastDistribution[_gauge] > block.timestamp - block.timestamp % DURATION, "Distribute gauge first!");
    }
        ...
        if (_claimable > IGauge(_gauge).left(base) && _claimable / DURATION > 0) {
            ...
            if(pausedGauges[_gauge] == true) {
        		...
        		if(customGauges[_gauge] != address(0)) {
        			IERC20(base).approve(customGauges[_gauge], _claimable);
