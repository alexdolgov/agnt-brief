...
*/
/**
 *Submitted for verification at SonicScan.org on 2025-01-20
*/
/**
...
interface IGauge {
    ...
    function rewardData(address _token) external view returns(Reward memory _rewardData);
}
...
contract Voter is Initializable {
    ...
    function distribute(address _gauge) public lock {
        ...
        */
		...
		// For DEFAULT GAUGE : NO CUSTOM GAUGE
		if(customGauges[_gauge] == address(0)) {
			if(_claimable > IGauge(_gauge).left(base) && _claimable / IGauge(_gauge).rewardData(base).rewardsDuration > 0) {
        		uint _now = block.timestamp;
        		distributions[ _now - (_now % 7 days) ][ _gauge ] += _claimable;
            	claimable[_gauge] = 0;
            	if(pausedGauges[_gauge] == true) {
				...
				//;// skip deleting :: claimable[_gauge] = 0;
			}
		}
		// NOTE: ELSE IF CUSTOM GAUGE Exists!
		else {
			if(_claimable > IGauge(customGauges[_gauge]).left(base) && _claimable / IGauge(customGauges[_gauge]).rewardData(base).rewardsDuration> 0) {
        		uint _now = block.timestamp;
        		distributions[ _now - (_now % 7 days) ][ _gauge ] += _claimable; // stats are recorded agains og gauge slot
            	claimable[_gauge] = 0; // stats are recorded agains og gauge slot
            	if(pausedGauges[_gauge] == true) {
            		IERC20(base).transfer(ms, _claimable);
        			emit DistributeReward(msg.sender, ms, _claimable);
            	}
