...
contract VotingEscrow is Initializable, IERC721Upgradeable, IERC721MetadataUpgradeable, IVotesUpgradeable {
        ...
        if (_isContract(_to)) {
            ...
            try IERC721ReceiverUpgradeable(_to).onERC721Received(msg.sender, _from, _tokenId, _data) returns (bytes4 response) {
                if (response != IERC721ReceiverUpgradeable(_to).onERC721Received.selector) {
                    revert("veNFT: ERC721Receiver rejected tokens");
                }
            } catch (bytes memory reason) {
                if (reason.length == 0) {
                    revert("veNFT: transfer to non ERC721Receiver implementer");
                } else {
                    assembly {
    ...
    function _create_lock(uint _value, uint _lock_duration, address _to) internal returns (uint) {
        ...
        // if `_to` is a contract, implement ERC721Receiver hook
        if (_isContract(_to)) {
            // Throws if transfer destination is a contract which does not implement 'onERC721Received'
            try IERC721ReceiverUpgradeable(_to).onERC721Received(msg.sender, address(0), _tokenId, "") returns (bytes4 response) {
                if (response != IERC721ReceiverUpgradeable(_to).onERC721Received.selector) {
                    revert("veNFT: ERC721Receiver rejected tokens");
                }
            } catch (bytes memory reason) {
