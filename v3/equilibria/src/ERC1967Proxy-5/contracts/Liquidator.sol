...
contract Liquidator is UUPSUpgradeable, AccessControlUpgradeable, IFlash {
    ...
    function onFlash(bytes calldata data) external {
        ...
        require(
            ...
            tokens.length == routers.length &&
            tokens.length == calls.length,
            InvalidInputLengths()
        );
    ...
    function withdraw(IERC20 token, uint256 value) onlyRole(WITHDRAWER_ROLE) external {
        token.safeTransfer(msg.sender, value);
        emit Withdrawn(msg.sender, token, value);
    }

    /// @notice Set address of the RfqEngine contract.
    function setRfqEngine(IRfqEngine _rfqEngine) onlyRole(DEFAULT_ADMIN_ROLE) external {
        rfqEngine = _rfqEngine;
