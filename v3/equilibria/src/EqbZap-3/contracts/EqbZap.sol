...
contract EqbZap is OwnableUpgradeable {
    ...
    function zapInV3SinglePt(
        ...
        address pendleRouterV3 = eqbConfig.getContract(EqbConstants.PENDLE_ROUTER_V3);
        (address market, , , ) = booster.poolInfo(_pid);
        (, address PT, ) = IPMarket(market).readTokens();
        _transferIn(PT, msg.sender, _netPtIn);
        _approveTokenIfNeeded(PT, pendleRouterV3, _netPtIn);
        (uint256 netLpOut, ) = IPendleRouterV3(pendleRouterV3).addLiquiditySinglePt(address(this), market, _netPtIn, _minLpOut, _guessPtSwapToSy, _limit);
        _deposit(_pid, netLpOut, _stake);
    }
    ...
    function zapInV3SingleToken(
        ...
        (address market, , , ) = booster.poolInfo(_pid);
        _transferIn(_input.tokenIn, msg.sender, _input.netTokenIn);
        _approveTokenIfNeeded(_input.tokenIn, pendleRouterV3, _input.netTokenIn);
        (uint256 netLpOut, , ) = IPendleRouterV3(pendleRouterV3)
            .addLiquiditySingleToken{
            value: _input.tokenIn == NATIVE ? _input.netTokenIn : 0
        }(address(this), market, _minLpOut, _guessPtReceivedFromSy, _input, _limit);
        _deposit(_pid, netLpOut, _stake);
    }
    ...
    function zapInV3SingleTokenKeepYt(
        ...
        _transferIn(_input.tokenIn, msg.sender, _input.netTokenIn);
        _approveTokenIfNeeded(_input.tokenIn, pendleRouterV3, _input.netTokenIn);
        (uint256 netLpOut, uint256 netYtOut, , ) = IPendleRouterV3(pendleRouterV3)
    ...
    function _withdraw(
        ...
        (, address token, address rewardPool, ) = booster
            .poolInfo(_pid);
