...
contract GaugeV3 is IGaugeV3, Initializable {
    ...
    function _getReward(
        ...
        if (_reward > 0) {
            ...
            bool antiSybilEnabled = IVoter(voter).isAntiSybilEnabled();
            bool positionIsValid = validateReward(owner, index, tickLower, tickUpper);

            address _receiver;
            if (!antiSybilEnabled || positionIsValid) {
                // standard payout
                _receiver = receiver;  
            } else {
                // sybil slashed to r33 vault
                _receiver = address(IAccessHub(IVoter(voter).accessHub()).r33());  
            }
            IERC20(token).safeTransfer(_receiver, _reward);
            emit ClaimRewards(period, _positionHash, _receiver, token, _reward);
        }
    }
