...
contract AccessHub is IAccessHub, Initializable, AccessControlEnumerableUpgradeable {
    ...
    function initialize(InitParams calldata params) external initializer {
        ...
        nfpManager = INonfungiblePositionManager(params.nfpManager);
    ...
    function compoundR33() external onlyRole(SWAP_FEE_SETTER) {
        // cache original operators
        address xRamOperator = xRam.operator();
        address r33Operator = r33.operator();

        // temporarily make AccessHub the operator of both r33 and xRam
        r33.transferOperator(address(this));
        xRam.migrateOperator(address(this));

        // transfer r33 tokens from r33 contract to AccessHub
        uint256 r33Balance = r33.balanceOf(address(r33));
        r33.rescue(address(r33), r33Balance);
        // Cast to IERC4626 to call redeem
        uint256 xRamAmount = IERC4626(address(r33)).redeem(r33Balance, address(this), address(this));
        xRam.operatorRedeem(xRamAmount);

        // transfer REX to r33 and compound
        IERC20 _rex = IERC20(xRam.REX());
        _rex.transfer(address(r33), xRamAmount); // xRamAmount 1:1 _rex
        r33.compound();

        // transfer operator roles back to the original addresses
        r33.transferOperator(r33Operator);
        xRam.migrateOperator(xRamOperator);
    }
    ...
    function _tryUnwrapLP(address token) internal returns (bool isLP, address tokenA, address tokenB) {
        address LEGACY_ROUTER = 0x32dB39c56C171b4c96e974dDeDe8E42498929c54;
    ...
    function addNfpManagersVoter(address _nfpManager) external onlyMultisig {
        voter.addNfpManager(_nfpManager);
    }
    ...
    function syncClGaugesBatch(uint256 startIndex, uint256 endIndex) external onlyMultisig {
        ...
        for (uint256 i = startIndex; i < endIndex; i++) {
            if (voter.isClGauge(allGauges[i])) {
                try IGaugeV3(allGauges[i]).syncRewardValidator() {} catch {}
                try IGaugeV3(allGauges[i]).syncNfpManagers() {} catch {}
            }
        }
    }
    
    function syncAllClGauges() external onlyMultisig {
        address[] memory allGauges = voter.getAllGauges();
