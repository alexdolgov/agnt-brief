...
contract GaugeV3 is IGaugeV3, Initializable {
    ...
    EnumerableSet.AddressSet internal authorizedNfpManagers;
    ...
    function _isValidNfpManager(address manager) internal view returns (bool) {
        return authorizedNfpManagers.contains(manager);
    }
    ...
    function getAuthorizedNfpManagers() external view returns (address[] memory) {
        return authorizedNfpManagers.values();
    }
    ...
    function getPeriodReward(uint256 period, address[] calldata tokens, uint256 tokenId, address receiver)
        ...
        override
        lock
        onlyAuthorized(address(nfpManager))
    {
        require(period <= _blockTimestamp() / WEEK, Errors.CANT_CLAIM_FUTURE());
        INonfungiblePositionManager _nfpManager = nfpManager;
    ...
    /// @inheritdoc IGaugeV3
    function getReward(uint256 tokenId, address[] memory tokens) public lock onlyAuthorized(address(nfpManager)) {
        INonfungiblePositionManager _nfpManager = nfpManager;
    ...
    /// @inheritdoc IGaugeV3
    function getRewardForOwner(uint256 tokenId, address[] memory tokens) external lock onlyAuthorizedOrVoter {
    ...
    ) external lock onlyAuthorized(nfpManagerAddress) {
        // Only voter can call this function (from VoterRewardClaimers)
    ...
    function _getReward(
        ...
        if (_reward > 0) {
            ...
            if (address(rewardValidator) != address(0) && 
                rewardValidator.validateReward(owner, receiver, _positionHash, tx.origin, index, tickLower, tickUpper)) {
                // slashed to r33
                receiver = address(IAccessHub(IVoter(voter).accessHub()).r33());
            }
    ...
    function validateReward(
        ...
        (uint32 lastTimestamp,) = pool.positionLastRewarderCheckpoint(owner, index, tickLower, tickUpper);
        // lastTimestamp (uint32) gets auto-promoted to uint256 for the math
        uint256 elapsedSinceLastCheckpoint = _blockTimestamp() - lastTimestamp;

        return elapsedSinceLastCheckpoint > IVoter(voter).timeThresholdForRewarder();
    }
    ...
    /// @inheritdoc IGaugeV3
    function syncRewardValidator() public {
        rewardValidator = IRewardValidator(IVoter(voter).rewardValidator());
    }
    
    /// @inheritdoc IGaugeV3
    function syncNfpManagers() public {
        // clear existing
        while (authorizedNfpManagers.length() > 0) {
            authorizedNfpManagers.remove(authorizedNfpManagers.at(0));
        }
        
        // get managers from voter
        address[] memory managers = IVoter(voter).getAllNfpManagers();
        for (uint i = 0; i < managers.length; i++) {
            authorizedNfpManagers.add(managers[i]);
        }
    }
}
