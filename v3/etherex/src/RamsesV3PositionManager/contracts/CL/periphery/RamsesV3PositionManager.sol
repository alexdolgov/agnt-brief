...
import {IRamsesV3PositionManager} from "./interfaces/IRamsesV3PositionManager.sol";
...
contract RamsesV3PositionManager is
    Multicall,
    ...
    IRamsesV3PositionManager
{
    ...
    constructor(
        ...
        address _accessHub
    ) ERC721("Etherex V3 Non Fungible Position", "REX-V3-NFP") PeripheryImmutableState(_deployer, _WETH9) {
    ...
    function _tryClaimRewards(uint256 tokenId, PoolAddress.PoolKey memory poolKey) private {
        if (voter != address(0)) {
            ...
            if (gauge != address(0)) {
                ...
                try IGaugeV3(gauge).getRewardForOwner(tokenId, rewardTokens) {
                } catch {
                }
            }
        }
    ...
    function getReward(uint256 tokenId, address[] calldata tokens) external payable isAuthorizedForToken(tokenId) {
