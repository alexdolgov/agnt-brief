...
contract Voter is IVoter, ReentrancyGuard, Initializable {
    ...
    function initialize(InitializationParams memory inputs) external reinitializer(2) {
        ...
        // initialize the authorizedNfpManagers set with the initial nfpManager
        if (inputs.nfpManager != address(0)) {
            $.authorizedNfpManagers.add(inputs.nfpManager);
        }
    ...
    function setNfpManager(address _nfpManager) external onlyGovernance {
        ...
        VoterStorage.VoterState storage $ = VoterStorage.getStorage();
        if (!$.authorizedNfpManagers.contains(_nfpManager)) {
            $.authorizedNfpManagers.add(_nfpManager);
        }
    }
    ...
    function getAllNfpManagers() external view returns (address[] memory) {
        ...
        if ($.authorizedNfpManagers.length() == 0 && $.nfpManager != address(0)) {
    ...
    /// @param _nfpManager The NFP manager address to add
    function addNfpManager(address _nfpManager) external onlyGovernance {
        VoterStorage.VoterState storage $ = VoterStorage.getStorage();
        require(_nfpManager != address(0), "Zero address");
        require($.authorizedNfpManagers.add(_nfpManager), "Already added");
    }

    /// @notice Remove an NFP manager from the whitelist
    /// @param _nfpManager The NFP manager address to remove
    function removeNfpManager(address _nfpManager) external onlyGovernance {
        VoterStorage.VoterState storage $ = VoterStorage.getStorage();
        require($.authorizedNfpManagers.remove(_nfpManager), "Not found");
    }
            ...
            if ($.isClGauge[_gauge]) {
                if ($.voterOwnsFactory) {
                    if ($.isAlive[_gauge]) {
                        ...
                        /// @dev set the feeProtocol to 5
                        /// @dev review for fresh deployment
                        bytes memory data = abi.encodeWithSignature("setPoolFeeProtocol(address,uint8)", pool, 5);

                        (bool success,) = address($.clFactory).call(data);
    ...
    function batchDistributeByIndex(uint256 startIndex, uint256 endIndex) external {
        ...
        for (uint256 i = startIndex; i < endIndex; ++i) {
            ...
            (bool _sucess,) = address(this).call(data);
            _sucess; // to suppress unused variable warning
        }
    }
