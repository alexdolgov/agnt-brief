...
abstract contract NFTBond is NFT, CollateralizedBondGranter, LiquidityRequester, PausableUpgradeable {
    ...
    */
    ...
    function _bondTokenURI(uint256 tokenId) private view returns (string memory) {
        ...
        string memory dataJSON = string.concat(
            '{'
                ...
                '"atributes": [',
                _setAtribute('Principal', string.concat(bond.principal._decimalString(18, false), ' USD')),',',
                _setAtribute('Collateral', string.concat(calculateCollateralBondAmount(bond.principal)._decimalString(18, false), ' Ethix')),',',
                _setAtribute('APY', (bond.interest*366 days/1e16)._decimalString(2, true)),',',
                _setAtribute('Maturity', string.concat((bond.maturity*1e2/30 days)._decimalString(2, false), ' Months')),',',
                _setAtribute('Maturity Unix Timestamp', (bond.mintingDate + bond.maturity).toString()),
                ']'
            '}'
        );
    ...
    function _setAtribute(string memory _name, string memory _value) private pure returns (string memory) {
