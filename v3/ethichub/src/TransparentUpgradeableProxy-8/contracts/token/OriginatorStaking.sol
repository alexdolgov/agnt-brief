...
contract OriginatorStaking is Initializable, StakingRewards, BaseTokenUpgradeable, IStaking, IProjectFundedRewards, IOriginatorManager {
    ...
    /// @notice IReserve that receives the liquidated stake on DEFAULT, to compensate investors
    /// @dev Declared last to preserve the storage layout of already-deployed proxies on upgrade
    IReserve public COLLATERAL_RESERVE;

    event StateChange(uint256 state);

    event CollateralReserveSet(address indexed collateralReserve);
    ...
    function initialize(
        ...
        COLLATERAL_RESERVE = _collateralReserve;
    ...
    /**
     * @notice Set the collateral reserve that receives the liquidated stake on DEFAULT.
     * Needed to configure already-deployed proxies that were initialized before this field
     * existed. Only governance.
     * @param _collateralReserve IReserve
     */
    function setCollateralReserve(IReserve _collateralReserve) external override onlyGovernance {
        require(address(_collateralReserve) != address(0), 'INVALID_COLLATERAL_RESERVE');
        COLLATERAL_RESERVE = _collateralReserve;
        emit CollateralReserveSet(address(_collateralReserve));
    }
    ...
    /**
     * @notice Function to declare as DEFAULT, or to sweep leftover stake of a contract that is
     * already in DEFAULT (e.g. originators defaulted under the old logic that still hold tokens).
     * From STAKING it records the defaulted amount and moves to DEFAULT; then (in either case)
     * it sweeps the remaining stake to the collateral reserve.
     **/
    function declareDefault() external override onlyGovernance {
        require(
            state == OriginatorStakingState.STAKING || state == OriginatorStakingState.DEFAULT,
            'ONLY_ON_STAKING_OR_DEFAULT'
        );
        if (state == OriginatorStakingState.STAKING) {
            require(block.timestamp >= DEFAULT_DATE, 'DEFAULT_DATE_NOT_REACHED');
            defaultedAmount = IERC20Upgradeable(STAKED_TOKEN).balanceOf(address(this));
            _endDistributionIfNeeded();
    ...
    function _checkRedeemEligibilityState() internal view returns (bool) {
        // On DEFAULT the whole stake is swept to the collateral reserve, so there is
        // nothing left to redeem: redeeming is only allowed once staking has ended.
        return state == OriginatorStakingState.STAKING_END;
    }
    ...
    /**
     * @notice Subtract all tokens of the contract on DEFAULT and send them
     * to the collateral reserve. Internal: only reachable from declareDefault.
     */
    function _liquidateStake() internal onlyOnDefaultState {
        uint256 balance = IERC20Upgradeable(STAKED_TOKEN).balanceOf(address(this));
        IERC20Upgradeable(STAKED_TOKEN).safeTransfer(address(COLLATERAL_RESERVE), balance);
    }
}
