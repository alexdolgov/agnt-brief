...
import { VoteUnchanged } from "./errors/VoteErrors.sol";
...
contract EthosVote is AccessControl, Common, UUPSUpgradeable {
  ...
  /**
   * @notice Idempotent upvote — safe to retry without toggling.
   * Reverts with VoteUnchanged if already an active upvote (saves gas via simulation).
  ...
  function upvoteFor(
    ...
    _idempotentVote(targetContract, targetId, true, false);
  }

  /**
   * @notice Idempotent downvote — safe to retry without toggling.
   * Reverts with VoteUnchanged if already an active downvote (saves gas via simulation).
   * @param targetContract Target contract address. Must implement IVotable.
   * @param targetId Target id.
   */
  function downvoteFor(
    address targetContract,
    uint256 targetId
  ) external whenNotPaused {
    _idempotentVote(targetContract, targetId, false, false);
  }

  /**
   * @notice Idempotent archive — removes the active vote if one exists.
   * Reverts with VoteUnchanged if already archived or never voted (saves gas via simulation).
   * @param targetContract Target contract address. Must implement IVotable.
   * @param targetId Target id.
   */
  function archiveVote(
    address targetContract,
    uint256 targetId
  ) external whenNotPaused {
    _idempotentVote(targetContract, targetId, false, true);
  }
  ...
  /**
   * @dev Unified idempotent vote handler. Supports vote and archive operations.
   * Reverts with VoteUnchanged when the requested state already matches.
   * @param targetContract Target contract address.
   * @param targetId Target id.
   * @param isUpvote Desired vote direction (ignored when isArchive is true).
   * @param isArchive If true, archive the existing vote instead of voting.
   */
  function _idempotentVote(
    address targetContract,
    uint256 targetId,
    bool isUpvote,
    bool isArchive
    ...
    // Archive and vote paths both need to check for prior votes
    bool hasVoted = hasVotedFor(voter, targetContract, targetId);

    if (isArchive) {
      // Archive path: revert if nothing to archive
      if (!hasVoted) {
        revert VoteUnchanged(targetContract, targetId);
      }
      ...
      // Archive the active vote
      VotesGeneral storage vg = votesGeneralByContractByTargetId[targetContract][targetId];
      vote.isUpvote ? vg.upvotesCount-- : vg.downvotesCount--;
      vote.isArchived = true;

      emit VoteChanged(true, vote.isUpvote, voter, targetContract, targetId, voteIndex);
      return;
    }

    // Vote path
    if (!hasVoted) {
      // No prior vote — record it
      _recordVote(voter, targetContract, targetId, isUpvote);
      return;
    }

    {
      ...
      if (!vote.isArchived && vote.isUpvote == isUpvote) {
        // Active vote in the same direction — revert
        revert VoteUnchanged(targetContract, targetId);
      }
    }

    // All other cases: archived (same or opposite), or active opposite direction
    _modifyVote(targetContract, targetId, voter, isUpvote);
  }
