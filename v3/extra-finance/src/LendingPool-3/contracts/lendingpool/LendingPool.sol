...
contract LendingPool is ILendingPool, Ownable, Payments, ReentrancyGuard {
    ...
    constructor(
        ...
        require(_vaultFactory != address(0), Errors.VL_ADDRESS_CANNOT_ZERO);
        require(_treasury != address(0), Errors.VL_ADDRESS_CANNOT_ZERO);
    ...
    function _deposit(
        ...
        require(
            ...
            Errors.VL_ETOKEN_AMOUNT_TOO_SMALL
        );
        if (
            IExtraInterestBearingToken(reserve.eTokenAddress).totalSupply() == 0
        ) {
    ...
    function getPositionStatus(
        ...
        for (uint256 i = 0; i < reserveIdArr.length; i++) {
            ...
            statusArr[i].eTokenStaked = IERC20(
    ...
    function getCurrentDebt(
        uint256[] calldata debtIds
    )
        external
        view
        returns (
            uint256[] memory currentDebts,
            uint256[] memory latestBorrowingIndex
        )
    {
        currentDebts = new uint256[](debtIds.length);
        latestBorrowingIndex = new uint256[](debtIds.length);

        for (uint256 i = 0; i < debtIds.length; i++) {
            DataTypes.DebtPositionData storage debtPosition = debtPositions[
                debtIds[i]
            ];
            DataTypes.ReserveData storage reserve = reserves[
                debtPosition.reserveId
            ...
            latestBorrowingIndex[i] = reserve.latestBorrowingIndex();
            currentDebts[i] = debtPosition
                .borrowed
                .mul(latestBorrowingIndex[i])
                .div(debtPosition.borrowedIndex);
        }
    }
    ...
    function setVaultFactory(address _vaultFactory) public onlyOwner {
        require(_vaultFactory != address(0), Errors.VL_ADDRESS_CANNOT_ZERO);
        vaultFactory = _vaultFactory;
        emit SetVaultFactory(_vaultFactory);
    }
    ...
    function rescueETH(address _to, uint256 _amount) external onlyOwner {
