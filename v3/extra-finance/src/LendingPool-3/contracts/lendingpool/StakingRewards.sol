// SPDX-License-Identifier: MIT
pragma solidity ^0.8.0;

import "../external/openzeppelin/contracts/access/Ownable.sol";
import "../external/openzeppelin/contracts/token/ERC20/IERC20.sol";
import "../external/openzeppelin/contracts/token/ERC20/ERC20.sol";
import "../external/openzeppelin/contracts/token/ERC20/utils/SafeERC20.sol";

import "../external/openzeppelin/contracts/utils/math/Math.sol";
import "../external/openzeppelin/contracts/utils/math/SafeMath.sol";
import "../interfaces/IExtraInterestBearingToken.sol";
import "../interfaces/IStakingRewards.sol";

contract StakingRewards is Ownable, IStakingRewards, ERC20 {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    uint256 public constant REWARD_RATE_PRECISION = 1e18;

    IERC20 public immutable stakedToken;
    address public immutable lendingPool;
    address public rewardsClaimer;

    address[] public rewardTokens;

    mapping(address => bool) public isRewardsPaused;

    mapping(address => bool) public inRewardsTokenList;

    mapping(address => Reward) public rewardData;

    mapping(address => mapping(address => uint256))
        public userRewardPerTokenPaid;
    mapping(address => mapping(address => uint256)) public userRewardsClaimable;

    uint internal _unlocked = 1;

    modifier nonReentrant() {
        require(_unlocked == 1, "reentrant call");
        _unlocked = 2;
        _;
        _unlocked = 1;
    }

    modifier onlyLendingPool() {
        require(lendingPool == msg.sender);
        _;
    }

    modifier onlyRewardsClaimer() {
        require(rewardsClaimer == msg.sender);
        _;
    }

    modifier updateReward(address user) {
        for (uint i; i < rewardTokens.length; i++) {
            address rewardToken = rewardTokens[i];

            uint256 curRewardPerToken = calcRewardPerToken(rewardToken);

            if (
                curRewardPerToken > rewardData[rewardToken].rewardPerTokenStored
            ) {
                rewardData[rewardToken]
                    .rewardPerTokenStored = curRewardPerToken;
            }

            rewardData[rewardToken].lastUpdateTime = block.timestamp;

            if (user != address(0)) {
                userRewardsClaimable[user][rewardToken] = earned(
                    user,
                    rewardToken,
                    curRewardPerToken
                );

                userRewardPerTokenPaid[user][rewardToken] = curRewardPerToken;
            }
        }
        _;
    }

    /// @notice This contract must be create in lendingPool's `initReserve()`
    constructor(
        address _stakingToken
    )
        ERC20(
            string(
                abi.encodePacked(
                    ERC20(_stakingToken).name(),
                    "(ExtraFi Interest Bearing Token With Staking Rewards)"
                )
            ),
            string(abi.encodePacked("st", ERC20(_stakingToken).symbol()))
        )
    {
        stakedToken = IERC20(_stakingToken);
        lendingPool = msg.sender;
    }

    function decimals() public view override returns (uint8) {
        return ERC20(address(stakedToken)).decimals();
    }

    function setRewardsPaused(
        address rewardToken,
        bool paused
    ) external onlyOwner {
        isRewardsPaused[rewardToken] = paused;
        emit RewardsPaused(rewardToken, paused);
    }

    function setRewardsClaimer(address _rewardsClaimer) external onlyOwner {
        require(_rewardsClaimer != address(0), "invalid rewardsClaimer");
        emit RewardsClaimerSet(rewardsClaimer, _rewardsClaimer);
        rewardsClaimer = _rewardsClaimer;
    }

    function setReward(
        address rewardToken,
        uint256 endTime,
        uint256 totalRewards
    ) public onlyOwner nonReentrant updateReward(address(0)) {
        require(!isRewardsPaused[rewardToken], "rewards paused!");
        require(rewardData[rewardToken].endTime < block.timestamp, "not end");
        require(block.timestamp < endTime, "invalid end");

        if (!inRewardsTokenList[rewardToken]) {
            rewardTokens.push(rewardToken);
            inRewardsTokenList[rewardToken] = true;
        }

        rewardData[rewardToken].endTime = endTime;
        rewardData[rewardToken].rewardRate =
            (totalRewards * REWARD_RATE_PRECISION) /
            (endTime - block.timestamp);

        rewardData[rewardToken].lastUpdateTime = block.timestamp;

        IERC20(rewardToken).safeTransferFrom(
            msg.sender,
            address(this),
            totalRewards
        );

        emit RewardsSet(rewardToken, block.timestamp, endTime, totalRewards);
    }

    function calcRewardPerToken(
        address rewardToken
    ) public view returns (uint) {
        uint256 curRewardPerToken = rewardData[rewardToken]
            .rewardPerTokenStored;
        uint256 rewardRate = rewardData[rewardToken].rewardRate;

        uint256 totalStaked = totalSupply();

        uint256 lastUpdateTime = rewardData[rewardToken].lastUpdateTime;
        uint256 endTime = rewardData[rewardToken].endTime;

        if (
            rewardRate == 0 ||
            totalStaked == 0 ||
            block.timestamp == lastUpdateTime ||
            endTime <= lastUpdateTime
        ) {
            // 0 staked or rewards already ended
            return curRewardPerToken;
        }

        uint256 durationEnd = Math.min(endTime, block.timestamp);

        uint256 dt = durationEnd - lastUpdateTime;

        return curRewardPerToken + (rewardRate * dt) / totalStaked;
    }

    function earned(
        address user,
        address rewardToken
    ) public view returns (uint) {
        uint256 curRewardPerToken = calcRewardPerToken(rewardToken);

        return earned(user, rewardToken, curRewardPerToken);
    }

    function earned(
        address user,
        address rewardToken,
        uint256 curRewardPerToken
    ) public view returns (uint) {
        uint256 d = curRewardPerToken -
            userRewardPerTokenPaid[user][rewardToken];

        return
            (balanceOf(user) * d) /
            REWARD_RATE_PRECISION +
            userRewardsClaimable[user][rewardToken];
    }

    /**
     * @dev Stake `amount` of assets to this contract
     * @param amount The amount of assets to be staked
     * @param onBehalfOf The address that will receive the staked position, same as msg.sender if the user
     *   wants to receive them on his own wallet.
     **/
    function stake(
        uint amount,
        address onBehalfOf
    ) external nonReentrant updateReward(onBehalfOf) {
        require(amount > 0, "amount = 0");

        stakedToken.safeTransferFrom(msg.sender, address(this), amount);

        // balanceOf[onBehalfOf] += amount;
        _mint(onBehalfOf, amount);

        emit Staked(msg.sender, onBehalfOf, amount);
    }

    /**
     * @dev Withdraw `amount` of staked assets
     * @param amount The amount of assets to withdraw
     * @param to The address that will receive the staked assets, same as msg.sender if the user
     *   wants to receive them on his own wallet.
     **/
    function withdraw(
        uint amount,
        address to
    ) external nonReentrant updateReward(msg.sender) {
        require(amount > 0, "amount = 0");

        // balanceOf[msg.sender] -= amount;
        _burn(msg.sender, amount);

        require(stakedToken.transfer(to, amount), "transfer failed");

        emit Withdraw(msg.sender, to, amount);
    }

    /**
     * @dev Withdraw `amount` of staked assets called by lendingPool
     * only lendingPool can call this function in `unstakeAndWithdraw()` of LendingPool
     * @param amount The amount of assets to withdraw
     * @param user The user of the staked position
     * @param to The address that will receive the staked assets, same as msg.sender if the user
     *   wants to receive them on his own wallet.
     **/
    function withdrawByLendingPool(
        uint amount,
        address user,
        address to
    ) external onlyLendingPool nonReentrant updateReward(user) {
        require(amount > 0, "amount = 0");

        // balanceOf[user] -= amount;
        _burn(user, amount);

        require(stakedToken.transfer(to, amount), "transfer falied");

        emit Withdraw(user, to, amount);
    }

    function claim() external nonReentrant updateReward(msg.sender) {
        for (uint i = 0; i < rewardTokens.length; i++) {
            address rewardToken = rewardTokens[i];
            if (isRewardsPaused[rewardToken]) {
                continue;
            }
            uint256 claimable = userRewardsClaimable[msg.sender][rewardToken];
            if (claimable > 0) {
                userRewardsClaimable[msg.sender][rewardToken] = 0;
                require(
                    IERC20(rewardToken).transfer(msg.sender, claimable),
                    "transfer failed"
                );
                emit RewardPaid(msg.sender, rewardToken, claimable);
            }
        }
    }

    function claim(
        address[] calldata rewards
    ) external nonReentrant updateReward(msg.sender) {
        for (uint i = 0; i < rewards.length; i++) {
            address rewardToken = rewards[i];
            if (isRewardsPaused[rewardToken]) {
                continue;
            }
            uint256 claimable = userRewardsClaimable[msg.sender][rewardToken];
            if (claimable > 0) {
                userRewardsClaimable[msg.sender][rewardToken] = 0;
                require(
                    IERC20(rewardToken).transfer(msg.sender, claimable),
                    "transfer failed"
                );
                emit RewardPaid(msg.sender, rewardToken, claimable);
            }
        }
    }

    function claimByRewardsClaimer(
        address[] calldata rewards,
        address user
    ) external nonReentrant onlyRewardsClaimer updateReward(user) {
        for (uint i = 0; i < rewards.length; i++) {
            address rewardToken = rewards[i];
            if (isRewardsPaused[rewardToken]) {
                continue;
            }
            uint256 claimable = userRewardsClaimable[user][rewardToken];
            if (claimable > 0) {
                userRewardsClaimable[user][rewardToken] = 0;
                require(
                    IERC20(rewardToken).transfer(user, claimable),
                    "transfer failed"
                );
                emit RewardPaid(user, rewardToken, claimable);
            }
        }
    }

    function update() external updateReward(address(0)) onlyOwner {}

    function rewardsTokenListLength() external view returns (uint256) {
        return rewardTokens.length;
    }

    function _transfer(
        address from,
        address to,
        uint256 amount
    ) internal override {
        revert("steToken is not transferable");
    }
}
