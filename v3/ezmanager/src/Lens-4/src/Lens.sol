...
contract Lens is Ownable {
    ...
    function getUserPositionDetailsPaginated(
        ...
        for (uint256 i = 0; i < outLen; ) {
            ...
            try core.getPositionDetails(key) returns (ICLCore.PositionDetails memory p) {
                positions[count] = LensPosition({
                    key: key,
                    pool: p.pool,
                    token0: p.token0,
                    token1: p.token1,
                ...
                // Skip positions that fail to decode to avoid bubbling reverts.
            }
