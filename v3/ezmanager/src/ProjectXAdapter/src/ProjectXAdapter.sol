...
contract ProjectXAdapter is Ownable, ReentrancyGuard, Pausable, ICLDexAdapter {
    ...
    function warmApprovals(address[] calldata tokens) external {
        if (msg.sender != owner() && msg.sender != manager) revert NotOwnerOrManager();
        uint256 len = tokens.length;
        for (uint256 i; i < len; ++i) {
            address token = tokens[i];
            if (token == address(0)) revert ZeroAddress();
            _approveIfBelow(IERC20(token), address(NPM), 1);
            _approveIfBelow(IERC20(token), address(ROUTER), 1);
        }
    }
        ...
        returns (uint256 tokenId, uint256 used0, uint256 used1, uint256 leftoverUSDC, uint256 remainingLossUSDCOut)
    {
        ...
        _approveIfBelow(IERC20(token0), address(NPM), finalBal0);
        _approveIfBelow(IERC20(token1), address(NPM), finalBal1);
        ...
        returns (uint256 used0, uint256 used1, uint256 leftoverUSDC, uint256 remainingLossUSDCOut)
    {
        ...
        _approveIfBelow(IERC20(t0), address(NPM), fbal0);
        _approveIfBelow(IERC20(t1), address(NPM), fbal1);
    ...
    function _swapExactInSingleHop(
        ...
        _approveIfBelow(IERC20(tokenIn), address(ROUTER), amountIn);
    ...
    function _approveIfBelow(IERC20 token, address spender, uint256 amount) internal {
        if (amount == 0) return;
