...
interface IStakedFF {
    ...
    // Events
    event RewardsReceived(uint256 amount, uint256 totalSupply, uint256 timestamp);
    event CooldownDurationUpdated(uint24 previousDuration, uint24 newDuration);
    event VestingPeriodUpdated(uint32 previousPeriod, uint32 newPeriod);
    event LockedAmountRedistributed(address indexed from, bool sharesBurned, uint256 amount, uint256 siloAmount);
