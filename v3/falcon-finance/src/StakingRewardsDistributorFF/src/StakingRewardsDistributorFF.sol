...
import "src/IStakedFF.sol";

/**
 * @title StakingRewardsDistributorFF
 * @notice This contract manages and distributes FF staking rewards with emergency controls and authorization management
 */
contract StakingRewardsDistributorFF is AccessControl, ReentrancyGuard, Pausable {
    ...
    IStakedFF public immutable STAKING_VAULT;
    /// @notice Falcon Finance token
    IERC20 public immutable FF_TOKEN;
    ...
    constructor(IStakedFF _stakingVault, IERC20 _ff, address _admin, address _operator) {
        if (address(_stakingVault) == address(0)) {
            revert InvalidZeroAddress();
        }
        if (address(_ff) == address(0)) {
            revert InvalidZeroAddress();
        }
        if (_admin == address(0)) {
            revert InvalidZeroAddress();
    ...
    function transferInRewards(uint256 _rewardsAmount) external whenNotPaused onlyRole(OPERATOR_ROLE) {
        FF_TOKEN.approve(address(STAKING_VAULT), _rewardsAmount);
