 ...
 */
contract BitVMBridgeV3 is IBitVMBridge, IInvest, AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    mapping(bytes32 inclusionProofKey => bool isMinted) private minted;
    ...
    /// @notice Vault manager contract interface
    ...
    function getInclusionProofKey(bytes32 _txid, uint256 _txIndex) public pure returns (bytes32) {
        return keccak256(abi.encode(_txid, _txIndex));
    }

    function getMinted(bytes32 _txid, uint256 _txIndex) public view returns (bool) {
        return minted[getInclusionProofKey(_txid, _txIndex)];
    }
    ...
    /// @param vaultAction Vault action
    ...
    function calculateIntentHash(
        ...
        hash = sha256(abi.encodePacked(userBtcAddress, amount, vaultAddress, vaultAction, chainName, intentType));
    }
    ...
    function mint(Peg[] calldata pegs) public onlyRole(BUSINESS_ROLE) nonReentrant {
        ...
        for (uint256 i = 0; i < pegs.length; i++) {
            ...
            // Compute inclusion proof key once
            bytes32 inclusionProofKey = getInclusionProofKey(peg.inclusionProof.txId, peg.inclusionProof.txIndex);

            // Skip if already processed
            if (minted[inclusionProofKey]) {
                continue;
            }
    ...
    function mint_for_invest(Invest[] calldata invests) public onlyRole(BUSINESS_ROLE) nonReentrant {
        ...
        if (address(vaultManager) == address(0)) revert InvalidVaultManager();
        ...
        for (uint256 i = 0; i < invests.length; i++) {
            ...
            // Compute inclusion proof key once
            bytes32 inclusionProofKey = getInclusionProofKey(invest.inclusionProof.txId, invest.inclusionProof.txIndex);
            ...
            bytes32 intentHash = calculateIntentHash(
                invest.userBitcoinAddress, invest.value, invest.vault, invest.vaultExtra, invest.intentType
            );
        ...
        if (totalAmount > 0) {
            ...
            uint256 currentAllowance = IERC20(token).allowance(address(this), address(vaultManager));
            if (currentAllowance < totalAmount) {
                SafeERC20.safeIncreaseAllowance(IERC20(token), address(vaultManager), totalAmount - currentAllowance);
            }
        }
        ...
        for (uint256 i = 0; i < invests.length; i++) {
            ...
            vaultManager.deposit(
                invest.vault, invest.vaultExtra, invest.value, invest.userBitcoinAddress, invest.pubX, invest.pubY
            );

            emit InvestMinted(
                invest.userBitcoinAddress, invest.inclusionProof.txId, invest.value, invest.vault, invest.vaultExtra
            );
        }
    }
    ...
    function withdrawByLP(
        ...
        IERC20(address(btcPeg)).transferFrom(msg.sender, address(this), _value);
        emit WithdrawByLP(msg.sender, _withdraw_id, _btc_addr, _fee_rate, _value, _lp_id);
    }
    ...
    function claimLPWithdraw(LPClaimInfo[] calldata _lp_claim_infos) external nonReentrant onlyRole(BUSINESS_ROLE) {
        for (uint256 i = 0; i < _lp_claim_infos.length; i++) {
            ...
            IERC20(address(btcPeg)).transfer(lpInfo.evm_addr, lpWithdraw.withdraw_amount);
    ...
    function refundLPWithdraw(uint256 _withdraw_id, address _receiver) external nonReentrant onlyRole(BUSINESS_ROLE) {
        ...
        IERC20(address(btcPeg)).transfer(_receiver, lpWithdraw.withdraw_amount);
    ...
    /// @notice Set the address of the vault manager contract
