...
/// @title ExpiryUtils
/// @notice Utility library for formatting expiry timestamps into human-readable strings
/// @dev Converts Unix timestamps to RFC2822-style date strings (e.g., "15JAN2025") for token naming
library ExpiryUtils {
    struct Date {
        uint16 year;
        uint8 month;
    ...
    /**
     * @notice Concatenates a Fira token name/symbol, a yield token name/symbol,
     *         and an expiry, using a delimiter (usually "-" or " ").
     * @param _bt The Fira token name/symbol.
     * @param _ct The coupon token name/symbol.
     ...
     *
     */
    function concat(string memory _bt, string memory _ct, uint256 _expiry, string memory _delimiter)
        internal
        pure
        returns (string memory result)
    {
        result = string(abi.encodePacked(_bt, _delimiter, _ct, _delimiter, toRFC2822String(_expiry)));
    }
