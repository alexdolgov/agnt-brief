...
import {AmountIsZero, NullContract, NotAllowlisted} from "src/errors.sol";
...
/// @title   Usd0 contract
/// @notice  Manages the USD0 token, including minting, burning, and transfers with allowlist checks.
/// @dev     Inherits from Allowlist for allowlist functionality and implements IUsd0 for USD0-specific logic.
...
contract Usd0 is ERC20PausableUpgradeable, ERC20PermitUpgradeable, IUsd0 {
    ...
    /// @dev    Callable only when the contract is not paused and to/from is allowlisted.
    ...
    /// @notice Hook that ensures token transfers are not made from or to not allowlisted addresses.
        ...
        override(ERC20PausableUpgradeable, ERC20Upgradeable)
    {
        ...
        if (from != address(0) && !$.registryAccess.hasRole(ALLOWLISTED, from)) {
            revert NotAllowlisted(from);
        }
        if (to != address(0) && !$.registryAccess.hasRole(ALLOWLISTED, to)) {
