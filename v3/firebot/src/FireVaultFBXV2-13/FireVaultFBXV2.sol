...
contract FireVaultFBXV2 is ERC20, ERC20Burnable, ERC20Permit {
    ...
    constructor() ERC20("FireVault FBX", "fireFBX") ERC20Permit("FireVault FBX") {
        _mint(msg.sender, 10000 * 1e18); // Airdrop for current fireFBX owners <<TESTING to calculate before deployment>>
    }
    ...
    function totalEPValuation() public view returns (uint256) {
        return EPValuation() * EP.stakedEP(address(this));
    }
    ...
    function fireFBXPrice() public view returns (uint256) {
        return totalAssetsValuation() / totalSupply() * 1e18;
    }
    
    function claimRewardsAndBalance() public {
        uint256 claimed = EP.claimFBXRewards();
        if (claimed > 0) {
            _mint(DAO_MULTISIG, ((claimed / 10) * 1e18) / fireFBXPrice());
            uint256 depositedFBXValuation = totalFBXBalance();
            uint256 depositedEPValuation = totalEPValuation();
            if (depositedFBXValuation > depositedEPValuation) {
                // The vault needs more EP
                uint256 spendableFBX = (depositedFBXValuation - depositedEPValuation) / 2;
                uint256 toSpend = 2 * claimed;
                if (spendableFBX < toSpend) {
                    toSpend = spendableFBX;
                }
