...
interface IEP {
    function dailyFBXEmissionPerEP() external view returns (uint256);
...
interface ISUSHI_ROUTER {
    function getAmountsOut(uint256 amountIn, address[2] memory path) external view returns (uint256[] memory amounts);
    function swapExactTokensForTokens(uint256 amountIn, uint256 amountOutMin, address[2] calldata path, address to, uint256 deadline) external returns (uint256[] memory amounts);
}
...
contract FireVaultFBXV2 is ERC20, ERC20Burnable, ERC20Permit {
    ...
    address public constant EP_CONTRACT_ADDRESS = 0xF91d9905b2e33022B44373a80A7F959742202dCb; // <<TESTING to set before deployment>>
    address[2] public SUSHI_PATH = [FBX_CONTRACT_ADDRESS, EP_CONTRACT_ADDRESS];
    ...
    ISUSHI_ROUTER public constant SUSHI_ROUTER = ISUSHI_ROUTER(SUSHI_ROUTER_ADDRESS);

    uint256 public constant REWARD_VALUATION_MULTIPLIER = 270;

    constructor() ERC20("FireVault FBX", "fireFBX") ERC20Permit("FireVault FBX") {
        _mint(msg.sender, 1e18 * 10000); // Airdrop for current fireFBX owners <<TESTING to calculate before deployment>>
    }

    function EPValuation() public view returns (uint256) {
        return REWARD_VALUATION_MULTIPLIER * EP.dailyFBXEmissionPerEP();
    }
    ...
    function fireFBXPrice() public view returns (uint256) {
        return 1e18 * totalAssetsValuation() / totalSupply();
    }

    // testing -----------------------------------------------------------------------------------------------------------------------------
    function sushiEPPrice0(uint256 amountIn) public view returns (uint256) {
        return SUSHI_ROUTER.getAmountsOut(amountIn, SUSHI_PATH)[0];
    }

    function sushiEPPrice1(uint256 amountIn) public view returns (uint256) {
        return SUSHI_ROUTER.getAmountsOut(amountIn, SUSHI_PATH)[1];
    }

    function buyOnSushi(uint256 toSpend) public {
        uint256 buyableAmount = SUSHI_ROUTER.getAmountsOut(toSpend, SUSHI_PATH)[1];
        SAFE_FBX.approve(SUSHI_ROUTER_ADDRESS, toSpend);
        SUSHI_ROUTER.swapExactTokensForTokens(toSpend, buyableAmount * 995 / 1000, SUSHI_PATH, address(this), block.timestamp + 1);
    }

    function mintSomeEP(uint256 toSpend) public {
    ...
    function claimRewardsAndBalance() public {
        uint256 toSpend = EP.claimFBXRewards();
        if (toSpend > 0) {
            _mint(DAO_MULTISIG, 1e18 * toSpend / 10 / fireFBXPrice());
            if (totalFBXBalance() > totalEPValuation()) {
                // The vault needs more EP
                uint256 buyableAmount = SUSHI_ROUTER.getAmountsOut(toSpend, SUSHI_PATH)[1];
