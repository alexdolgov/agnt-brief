...
contract FireVaultFBXV2 is ERC20, ERC20Burnable, ERC20Permit {
    ...
    function getAmountOutMin(address _tokenIn, address _tokenOut, uint256 _amount) public view returns (uint256) {
		...
		path[0] = _tokenIn;
		path[1] = _tokenOut;
		uint256[] memory amountOutMins = SUSHI_ROUTER.getAmountsOut(_amount, path);
    ...
    function getAmountOutMin2(address router, address _tokenIn, address _tokenOut, uint256 _amount) public view returns (uint256) {
		...
		path[0] = _tokenIn;
		path[1] = _tokenOut;
		uint256[] memory amountOutMins = ISushiRouter(router).getAmountsOut(_amount, path);
		return amountOutMins[path.length -1];
	}
    
    function sushiEPPrice0(uint256 amountIn) public view returns (uint256) {
        return SUSHI_ROUTER.getAmountsOut(amountIn, sushiPath)[0];
    }

    function sushiEPPrice1(uint256 amountIn) public view returns (uint256) {
        return SUSHI_ROUTER.getAmountsOut(amountIn, sushiPath)[1];
    }
