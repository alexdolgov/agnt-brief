...
contract Issuer is Ownable {
    ...
    IERC20 constant DCI = IERC20(0xc91953E110EbB0039859304A0d1b64f8450763fC);
    IERC20 constant WSTETH = IERC20(0x03b54A6e9a984069379fae1a4fC4dBAE93B3bCCD);
    ...
    function getMidPrice() public view returns (uint256) {
        ...
        require(c > 0, "Median is zero");
        return c;
    }

    function getFreshOracleCount() public view returns (uint256 count) {
        address[5] memory src = [ORACLE_ADDR_1, ORACLE_ADDR_2, ORACLE_ADDR_3, ORACLE_ADDR_4, ORACLE_ADDR_5];
        for (uint256 i = 0; i < 5; i++) {
    ...
    function issue(uint256 amountIn, uint256 minAmountOut, uint256 deadline) external returns (uint256 amountOut) {
        require(block.timestamp <= deadline, "Order expired");
        require(getFreshOracleCount() == 5, "Oracles not up to date.");

        uint256 askPrice = getMidPrice();
        amountOut = (amountIn * 1e18) / askPrice;

        require(amountOut >= minAmountOut, "Slippage too high");

        WSTETH.safeTransferFrom(msg.sender, address(this), amountIn);
        DCI.safeTransfer(msg.sender, amountOut);
    }

    function redeem(uint256 amountIn, uint256 minAmountOut, uint256 deadline) external returns (uint256 amountOut) {
        require(block.timestamp <= deadline, "Order expired");
        require(getFreshOracleCount() == 5, "Oracles not up to date.");

        uint256 bidPrice = (getMidPrice() * 997) / 1000; // -0.30%
        amountOut = (amountIn * bidPrice) / 1e18;

        require(amountOut >= minAmountOut, "Slippage too high");

        DCI.safeTransferFrom(msg.sender, address(this), amountIn);
        WSTETH.safeTransfer(msg.sender, amountOut);
    }

    // Owner-only withdrawal to owner.
    function withdraw(uint256 dciAmount, uint256 wstethAmount) external onlyOwner {
        address to = owner();
        if (dciAmount > 0) DCI.safeTransfer(to, dciAmount);
        if (wstethAmount > 0) WSTETH.safeTransfer(to, wstethAmount);
    }
}