...
contract FlapTaxTokenV2 is Initializable, ERC20PermitUpgradeable, OwnableUpgradeable, IFlapTaxTokenV2 {
    ...
    function initialize(InitParams memory params) external override initializer {
        ...
        // dividendContract is optional but recommended
    ...
    function finalizeMigration() external override onlyOwner {
        ...
        if (PoolState(currentPoolState.state) == PoolState.Migrating) {
            ...
            currentPoolState.antiFarmerExpirationTime = uint64(block.timestamp + antiFarmerDuration);
            poolState = currentPoolState;
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        PackedPoolState memory currentPoolState = poolState;
        _liquidateTax(to, currentPoolState);
    ...
    /// @param currentPoolState The current pool state in memory
    function _liquidateTax(address to, PackedPoolState memory currentPoolState) internal {
        ...
        PoolState currentState = PoolState(currentPoolState.state);
        if (
            (currentState == PoolState.TaxEnforced || currentState == PoolState.TaxEnforcedAntiFarmer) // only when tax is enforced
            ...
            if (
                taxAmount > 0 // Tax to liquidate
                    ...
                    ) // Enough tax to liquidate or last liquidation
            ) {
    ...
    function _adjustLiquidationThreshold(uint256 taxAmount, uint256 outputAmount) internal {
        ...
        PackedPoolState memory currentPoolState = poolState;
        uint256 expectedOutputAmount = (outputAmount * currentPoolState.liquidationThreshold) / taxAmount;
        if (expectedOutputAmount > liqExpectedOutputAmount) {
            currentPoolState.liquidationThreshold = uint96((currentPoolState.liquidationThreshold * 99) / 100); // Reduce by 1%
            if (currentPoolState.liquidationThreshold < MIN_LIQ_THRESHOLD) {
                currentPoolState.liquidationThreshold = uint96(MIN_LIQ_THRESHOLD);
            }
            poolState = currentPoolState;
        }
    }
    ...
    function _afterTokenTransfer(address from, address to, uint256 amount) internal override {
        ...
        if (dividendContract != address(0)) {
            // Helper function to check if address should be excluded from dividend updates
            ...
            // update 'from' (skip excluded addresses)
            if (!shouldSkipFrom) {
            ...
            // update 'to' (skip excluded addresses)
            if (!shouldSkipTo) {
