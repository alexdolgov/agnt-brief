...
contract FlapTaxToken is Initializable, ERC20PermitUpgradeable, OwnableUpgradeable, IFlapTaxToken {
    ...
    struct ConstructorParams {
        ...
        /// @param TAX_DURATION The duration of the tax in seconds
        uint256 TAX_DURATION;
    ...
    uint256 public immutable TAX_DURATION;
    ...
    constructor(ConstructorParams memory params) {
        ...
        TAX_DURATION = params.TAX_DURATION;
    ...
    function _liquidateTax(address to) internal {
        ...
        if (
            ...
            // State transition
            if (block.timestamp > taxExpirationTime && state != PoolState.TaxFree) {
            ...
            if (
                ...
                if (QUOTE_TOKEN == WETH) {
                    ...
                    path[0] = address(this);
                    path[1] = WETH;
                    beforeBalance = taxSplitter.balance;
                    try IUniswapRouter02(PCS_V2_ROUTER).swapExactTokensForETHSupportingFeeOnTransferTokens(
                        taxAmount, 0, path, taxSplitter, block.timestamp
                    ) {
                        afterBalance = taxSplitter.balance;
                        outputAmount = afterBalance - beforeBalance;
                    ...
                    path[0] = address(this);
                    path[1] = QUOTE_TOKEN;
                    beforeBalance = ERC20PermitUpgradeable(QUOTE_TOKEN).balanceOf(taxSplitter);
                    try IUniswapRouter02(PCS_V2_ROUTER).swapExactTokensForTokensSupportingFeeOnTransferTokens(
                        taxAmount, 0, path, taxSplitter, block.timestamp
                    ) {
                        afterBalance = ERC20PermitUpgradeable(QUOTE_TOKEN).balanceOf(taxSplitter);
                        outputAmount = afterBalance - beforeBalance;
                        _adjustLiquidationThreshold(taxAmount, outputAmount);

                        {
                            if (ERC20PermitUpgradeable(WETH).balanceOf(address(this)) > 0) {
                                // We intentionally transfer 1 wei WBNB to the smart router
                                // to allow it to unwrap weth and call the fallback function of the splitter to trigger the split
                                ERC20PermitUpgradeable(WETH).transfer(PCS_SMART_ROUTER, 1);
                            }

                            // then call smart router's unwrapWETH9 function
                            ISmartRouter(PCS_SMART_ROUTER).unwrapWETH9(0, taxSplitter); // unwrap WETH and send to taxSplitter
                        }
                    } catch (bytes memory reason) {
                        emit TaxLiquidationError(reason);
                        _plainTransfer(address(this), taxSplitter, taxAmount);
