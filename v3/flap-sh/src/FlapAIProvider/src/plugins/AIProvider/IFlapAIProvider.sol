...
interface IFlapAIProvider {
    ...
    /// @notice Reverts when `retryUndelivered` is called but the consumer address has no contract code.
    /// @dev On the real EVM a low-level CALL to an EOA always returns success=true with empty
    ///      returndata, so the callback would silently appear to succeed without actually executing
    ///      any logic. We detect this case via extcodesize and revert instead of silently
    ///      marking the request FULFILLED with no real delivery.
    /// @param consumer The consumer address that has no code.
