...
contract TaxProcessor is TaxProcessorBase {
        ...
        returns (uint256 totalQuoteReceived)
    {
        ...
        if (totalToSwap > 0) {
            ...
            if (quoteReceived > 0) {
                if (fee > 0) {
                    feeQuoteBalance += (quoteReceived * fee) / totalToSwap;
                }
                if (commission > 0) {
                    commissionQuoteBalance += (quoteReceived * commission) / totalToSwap;
                }
                if (market > 0) {
                    marketQuoteBalance += (quoteReceived * market) / totalToSwap;
                }
                if (dividend > 0) {
                    pendingDividendQuoteTokenBalance += (quoteReceived * dividend) / totalToSwap;
                }
                if (lpTaxToSwap > 0) {
                    lpQuoteBalance += (quoteReceived * lpTaxToSwap) / totalToSwap;
                }
            }
        }
    }
