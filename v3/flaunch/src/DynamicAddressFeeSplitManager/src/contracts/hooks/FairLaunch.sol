...
contract FairLaunch is AccessControl {
    ...
    function fillFromPosition(
        ...
        if (tokensOut > info.supply) {
            ...
            // If we have received an `ethIn` value of 0, then we need to revert the transaction as
            // a rounding exploit has very likely occurred. Any non-zero number will be handled by
            // an additional Uniswap V4 swap.
            if (ethIn == 0) {
                revert InvalidFairLaunchBuy();
            }
