...
abstract contract SupportsCreatorTokens {
    ...
    /**
     * Makes batched creator fee claims for specific tokens.
     *
     * @param _flaunchTokens The creator tokens to claim against
     *
     * @return creatorAvailableClaim_ The amount claimed by the creator
     */
    function _creatorClaim(ITreasuryManager.FlaunchToken[] memory _flaunchTokens) internal returns (uint creatorAvailableClaim_) {
        uint flaunchTokenLength = _flaunchTokens.length;
        for (uint i; i < flaunchTokenLength; ++i) {
            creatorAvailableClaim_ += _creatorClaim(_flaunchTokens[i]);
        }
    }
