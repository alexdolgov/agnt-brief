...
contract TrustedSignerFeeCalculator is IFeeCalculator, Ownable, ReentrancyGuard {
    ...
    function trackSwap(
        ...
        // PoolId would be.
        (bytes32 messageHash, bytes memory signature) = _isPremine(poolId)
     ...
     */
    function _isPremine(PoolId _poolId) internal view returns (bool isPremine_) {
        FairLaunch.FairLaunchInfo memory info = IPositionManager(positionManager).fairLaunch().fairLaunchInfo(_poolId);
