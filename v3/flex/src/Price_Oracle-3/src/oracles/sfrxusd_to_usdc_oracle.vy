...
# Decimals
_ONE_SHARE: immutable(uint256)  # 10^collateral_decimals (one whole sfrxUSD share)
...
def __init__():
    ...
    # Precompute one-share unit
    _ONE_SHARE = 10 ** _COLLATERAL_TOKEN_DECIMALS
...
def get_price(scaled: bool = True) -> uint256:
    ...
    pps: uint256 = staticcall IERC4626(_COLLATERAL_TOKEN.address).convertToAssets(_ONE_SHARE)
