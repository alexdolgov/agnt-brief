...
contract FlockMiniPoolV2 is Ownable {
    ...
    function delegate(uint256 _amount) external {
        ...
        require(config.getGmFlockToken().transferFrom(delegator, address(this), _amount), "Transfer failed");
    ...
    function undelegate(uint256 _amount) external {
        address delegator = msg.sender;
        require(lastStakeTimestamp[delegator] +  config.getPoolManager().getDelegationCoolDown() < block.timestamp, "You can't undelegate before cooldown");
        require(delegationOf(delegator).amount >= _amount, "User has insufficient token delegated");
        require(config.getGmFlockToken().transfer(delegator, _amount), "transfer failed");
        addPastTemporalDelegation(delegator, delegationMap[delegator]);
    ...
    function claimRewards() external {
        address delegator = msg.sender;
        require(lastStakeTimestamp[delegator] + config.getPoolManager().getClaimCoolDown() < block.timestamp, "You can't claim before cooldown");
    ...
    function claimable(address _delegator) public view returns (uint256) {
        uint256 rewards = config.getFlockToken().balanceOf(address(this));
