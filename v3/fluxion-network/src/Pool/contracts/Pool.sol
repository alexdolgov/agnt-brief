...
contract Pool is IPool, ERC20Permit, ReentrancyGuard {
    ...
    /// @notice Set the poolFees address (only callable by factory during initialization)
    function setPoolFees(address _poolFees) external {
        if (msg.sender != factory) revert NotFactory();
        if (poolFees != address(0)) revert PoolFeesAlreadySet();
        poolFees = _poolFees;
    }
    ...
    function claimFees() external returns (uint256 claimed0, uint256 claimed1) {
        ...
        if (claimed0 > 0 || claimed1 > 0) {
            ...
            IPoolFeesUpgradeable(poolFees).claimFeesFor(sender, claimed0, claimed1);
    ...
    function _update0(uint256 amountToPoolFees, uint256 amountToTeam) internal {
        // Only update on this pool if there is a fee
        if (amountToPoolFees == 0 && amountToTeam == 0) return;
        IERC20(token0).safeTransfer(poolFees, amountToPoolFees); // transfer the fees out to PoolFees
        IERC20(token0).safeTransfer(IPoolFactory(factory).team(), amountToTeam); // transfer the fees out to Team
        uint256 _ratio = ((amountToPoolFees + amountToTeam) * 1e18) / totalSupply(); // 1e18 adjustment is removed during claim
        if (_ratio > 0) {
            index0 += _ratio;
        }
    ...
    function _update1(uint256 amountToPoolFees, uint256 amountToTeam) internal {
        // Only update on this pool if there is a fee
        if (amountToPoolFees == 0 && amountToTeam == 0) return;
        IERC20(token1).safeTransfer(poolFees, amountToPoolFees);
        IERC20(token1).safeTransfer(IPoolFactory(factory).team(), amountToTeam);
        uint256 _ratio = ((amountToPoolFees + amountToTeam) * 1e18) / totalSupply();
        if (_ratio > 0) {
            index1 += _ratio;
        }
    ...
    function swap(uint256 amount0Out, uint256 amount1Out, address to, bytes calldata data) external nonReentrant {
        ...
        if (amount0In == 0 && amount1In == 0) revert InsufficientInputAmount();
        {
            ...
            (address _token0, address _token1) = (token0, token1);
            if (amount0In > 0) _update0((amount0In * IPoolFactory(factory).getFee(address(this), stable)) / 10000, (amount0In * IPoolFactory(factory).getTeamFee(address(this), stable)) / 10000); // accrue fees for token0 and move them out of pool
            if (amount1In > 0) _update1((amount1In * IPoolFactory(factory).getFee(address(this), stable)) / 10000, (amount1In * IPoolFactory(factory).getTeamFee(address(this), stable)) / 10000); // accrue fees for token1 and move them out of pool
    ...
    function getAmountOut(uint256 amountIn, address tokenIn) external view returns (uint256) {
        (uint256 _reserve0, uint256 _reserve1) = (reserve0, reserve1);
        amountIn = amountIn - (amountIn * IPoolFactory(factory).getFee(address(this), stable)) / 10000 - (amountIn * IPoolFactory(factory).getTeamFee(address(this), stable)) / 10000; // remove fee from amount received
