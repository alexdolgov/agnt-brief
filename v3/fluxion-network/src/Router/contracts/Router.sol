...
contract Router is IRouter, ERC2771Context {
    ...
    function getAmountsIn(uint256 amountOut, Route[] memory routes) public view returns (uint256[] memory amounts) {
        ...
        // Calculate a reasonable upper bound based on expected maximum slippage
        // Use a very large multiplier to account for:
        // 1. Different token decimals (e.g., 18 decimals vs 6 decimals = 1e12 difference)
        // 2. Fees and slippage
        // 3. Stable pool math complexities
        uint256 low = 1;
        uint256 high = amountOut * 1e15; // Very large upper bound to handle decimal differences

        // For safety, cap the search space to avoid extremely large numbers
        if (high < amountOut) high = type(uint256).max / 2; // Handle overflow

        uint256 tolerance = amountOut / 10000; // 0.01% tolerance
        if (tolerance == 0) tolerance = 1; // Minimum tolerance of 1 wei
        uint256 maxIterations = 256; // Increase iterations for larger search space

        uint256 bestAmountIn = 0;
        uint256 iteration = 0;

        // Binary search to find the required input amount
        while (low <= high && iteration < maxIterations) {
            iteration++;
            uint256 mid = low + (high - low) / 2; // Avoid overflow

            // Get the output amount for this input
            uint256[] memory outputs = getAmountsOut(mid, routes);
            uint256 actualOut = outputs[outputs.length - 1];

            // Check if we found a close enough match
            if (actualOut >= amountOut) {
                bestAmountIn = mid;
                // If we're within tolerance, we can stop
                if (actualOut - amountOut <= tolerance) {
                    break;
                }
                // Try a smaller input (but avoid underflow)
                if (mid > 0) {
                    high = mid - 1;
                } else {
                    break;
                ...
                // Output too small, need larger input
                low = mid + 1;
            }
        }
        ...
        amounts = getAmountsOut(bestAmountIn, routes);
