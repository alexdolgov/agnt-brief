...
///    - `spendToken`/`spendAmount` are accounted against `tokenAllowance` (decremented per call).
...
/// - Domain: name = "FT SessionManager", version = "1".
/// - Types (exact strings):
///   - "AssetLimit(address token,uint256 limit)"
///   - "Session(address owner,address delegate,uint48 validAfter,uint48 validUntil,uint32 maxCalls,uint16 maxFeeBps,AssetLimit[] limits,bytes32 salt)"
...
/// - `limits` is order-sensitive and is encoded as `keccak256(abi.encodePacked(keccak256(AssetLimit[0]), ...))`.
...
interface ISessionManager {
    ...
    /// @notice Remaining per-session per-asset allowance.
    /// @dev `type(uint256).max` matches ERC-20 allowance semantics: "infinite" allowance (no decrement).
    function tokenAllowance(bytes32 sessionId, address token) external view returns (uint256);
