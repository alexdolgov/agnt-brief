...
interface ICircuitBreakerV2 {
    ...
    /// @notice Get the default rate limiter configuration
    function getDefaultConfig() external view returns (LimiterConfig memory);

    /// @notice Get the rate limiter configuration for a specific asset (falls back to default if unset)
    function getConfig(address asset) external view returns (LimiterConfig memory);
