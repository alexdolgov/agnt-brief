...
abstract contract JumpIRMBase is IIRM {
    error JumpIRMBaseInvalidOptimalUtilization();
    ...
    function borrowAPR(
        ...
        returns (uint256)
    {
        if (utilWad > Constants.WAD) utilWad = Constants.WAD;

        uint256 uOpt = uOptWad();
        if (uOpt == 0 || uOpt > Constants.WAD) revert JumpIRMBaseInvalidOptimalUtilization();
        uint256 r0 = r0Wad();
        uint256 s1 = s1Wad();
        uint256 s2 = s2Wad();
        ...
        if (utilWad <= uOpt) {
            ...
            uint256 den = (Constants.WAD - uOpt);
 ...
 *  U* = 0.85, r0 = 1.5%, s1 = 10%, s2 = 80%, rMax = 91.5%
 *  =========================
 */
 ...
 *  U* = 0.78, r0 = 1.0%, s1 = 6%, s2 = 60%, rMax = 67%
 *  =========================
 */
 ...
 *  U* = 0.65, r0 = 6%, s1 = 20%, s2 = 150%, rMax = 176%
 *  =========================
 */
