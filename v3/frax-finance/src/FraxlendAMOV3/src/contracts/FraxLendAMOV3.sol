...
contract FraxlendAMOV3 is Ownable {
  ...
  modifier approvedBorrowPair(address _pair) {
    require(
      borrowPairsMaxCollateral[_pair] > 0,
      "Pair not approved for borrow"
    );
    _;
  }
  ...
  modifier borrowOnLTV(address _pair) {
    ...
    if (_borrowShare > 0) {
      uint256 _borrowAmount = _fraxlendPair.toBorrowAmount(
        _borrowShare,
        false,
        true
      );
      uint256 _collateralAmount = _fraxlendPair.userCollateralBalance(
        address(this)
      );
      require(_EXCHANGE_PRECISION > 0, "EXCHANGE_PRECISION is zero.");
      require(_collateralAmount > 0, "Collateral amount is zero.");
      uint256 _ltv = (((_borrowAmount * _exchangeRate) / _EXCHANGE_PRECISION) *
        _LTV_PRECISION) / _collateralAmount;
  ...
  event SetBorrowPair(
    ...
    uint256 _maxCollateralAllocation,
    uint256 _maxLTV
  );
  ...
  /// @notice the ```UtilizationTargeted``` event fires when a given util rate is targeted
  /// @param _pairAddress The pair address
  /// @param _targetedUtilization The utilization targeted (Denominated in 1e5)
  event UtilizationTargeted(address _pairAddress, uint256 _targetedUtilization);
  ...
  function previewUpdateExchangeRate(
    address _pairAddress
  ) public view returns (uint256 _exchangeRate) {
  ...
  function showAllocations()
    public
    view
    returns (uint256[4] memory _allocations)
  {
  ...
  function showPairAccounting(
    address _pairAddress
  ) public view returns (uint256[5] memory _allocations) {
    ...
    if (
      _totalCollateral > 0 && _totalAssetAmount > 0 && _totalBorrowAmount > 0
    ) {
      uint256 _borrowedLTV = (((_totalBorrowAmount * _exchangeRate) /
        _EXCHANGE_PRECISION) * _LTV_PRECISION) / _totalCollateral;
      if (_borrowedLTV > 0) {
        _allocations[3] =
          ((((_totalBorrowAmount * _LTV_PRECISION) / _borrowedLTV)) *
            _LTV_PRECISION) /
          _totalAssetAmount;
      } else {
        _allocations[3] = 0;
  ...
  function showBorrowPairAccounting(
    address _pairAddress
  ) public view returns (uint256[4] memory _allocations) {
    ...
    returns (uint256 fraxValE18, uint256 collatValE18)
  {
    ...
    collatValE18 =
      (fraxValE18 * FRAX.global_collateral_ratio()) /
      (PRICE_PRECISION);
  }

  /// @notice Backwards compatibility
  /// @return FRAX minted balance of the FraxlendAMO
  function mintedBalance() public view virtual returns (int256) {
  ...
  /// @param _pairAddress Address of FraxlendPair
  function accrueInterestFraxlendPair(
    address _pairAddress
  ...
  function setPair(
    ...
    require(
      address(IFraxlendPairV3(_pairAddress).asset()) == address(FRAX),
      "Pair asset is not FRAX"
    );
  ...
  function setBorrowPair(
    ...
    require(
      address(IFraxlendPairV3(_pairAddress).collateralContract()) ==
        address(FRAX),
      "Pair collateral is not FRAX"
    );
  ...
  function withdrawFromPair(
    address _pairAddress,
    uint256 _shares
  ) public onlyByOwnerOperator returns (uint256 _amountWithdrawn) {
    IFraxlendPairV3 _fraxlendPair = IFraxlendPairV3(_pairAddress);

    // Calculate current amount balance
    uint256 _currentBalanceShares = _fraxlendPair.balanceOf(address(this));
    uint256 _currentBalanceAmount = _fraxlendPair.toAssetAmount(
      _currentBalanceShares,
      false,
      true
    );

    // Withdraw amount
    _amountWithdrawn = _fraxlendPair.redeem(
      _shares,
      address(this),
      address(this)
    );
    ...
    if (pairsMintedFrax[_pairAddress] < _currentBalanceAmount) {
      ...
      if (_profit > _amountWithdrawn) {
        ...
        pairsProfitTaken[_pairAddress] =
          pairsProfitTaken[_pairAddress] +
          _profit;
        pairsMintedFrax[_pairAddress] =
          pairsMintedFrax[_pairAddress] -
          (_amountWithdrawn - _profit);
      }
    } else {
      pairsMintedFrax[_pairAddress] =
        pairsMintedFrax[_pairAddress] -
  ...
  function withdrawMaxFromPair(
    ...
    uint256 _availableShares = _fraxlendPair.toAssetShares(
      _availableAmount,
      false,
      false
    );
  ...
  function openBorrowPosition(
    ...
    public
    approvedBorrowPair(_pairAddress)
    borrowOnBudget(_pairAddress)
    borrowOnLTV(_pairAddress)
    onlyByOwnerOperator
  {
    ...
    FRAX.approve(_pairAddress, _fraxAmount);
    uint256 _shares = fraxlendPair.borrowAsset(
      _borrowAmount,
      _fraxAmount,
      address(this)
    );
  ...
  {
    IFraxlendPairV3 _fraxlendPair = IFraxlendPairV3(_pairAddress);
    _amountAssetOut = _fraxlendPair.repayAssetWithCollateral(
      _swapperAddress,
      _collateralToSwap,
      _amountAssetOutMin,
      _path
    );
    uint256 _sharesOut = _fraxlendPair.toBorrowShares(
      _amountAssetOut,
      false,
      false
    );
  ...
  /* ============================================ IR Utilization Functions ================================================= */

  /// @notice Function to either increase or decrease (open/close a loan) in order to bring a pair to a target utilization
  /// @param _pairAddress The address of the pair to modify
  /// @param targetUtil The utilization that is being targeted for the pair
  /// @dev This function assumes that the AMO has sufficient collateral/debt to meet the targeted utilization
  function modifyPairUtilizationBorrowRepay(
    address _pairAddress,
    uint256 targetUtil
  ) public {
    IFraxlendPairV3 _fraxlendPair = IFraxlendPairV3(_pairAddress);
    (uint256 totalAsset, ) = _fraxlendPair.totalAsset();
    (uint256 totalBorrow, ) = _fraxlendPair.totalBorrow();
    uint256 currentUtil = (totalBorrow * 1e5) / totalAsset;
    if (currentUtil < targetUtil) {
      // Solve for amount to borrow
      uint256 toBorrow = ((targetUtil * totalAsset) / 1e5) - totalBorrow;
      uint256 maxBorrow = (((_fraxlendPair.userCollateralBalance(
        address(this)
      ) * borrowPairsMaxLTV[_pairAddress]) / 1e5) * 1e18) /
        previewUpdateExchangeRate(_pairAddress);
      if (toBorrow > maxBorrow) toBorrow = maxBorrow;

      /// Implement Borrow
      openBorrowPosition(_pairAddress, 0, toBorrow);
    } else if (currentUtil > targetUtil) {
      // Solve for the amount to repay
      uint256 toRepay = totalBorrow - ((targetUtil * totalAsset) / 1e5);
      uint256 maxToRepay = _fraxlendPair.toBorrowAmount(
        _fraxlendPair.userBorrowShares(address(this)),
        true,
        true
      );
      if (toRepay > maxToRepay) toRepay = maxToRepay;

      /// Implement Repayment
      uint256 sharesToRepay = _fraxlendPair.toBorrowShares(
        toRepay,
        false,
        true
      );

      repayBorrowPosition(_pairAddress, sharesToRepay);
    }
    /// @notice default do nothing: target == current
    emit UtilizationTargeted(_pairAddress, targetUtil);
  }

  /// @notice Function to either increase or decrease (deposit/withdraw) in order to bring a pair to a target utilization
  /// @param _pairAddress The address of the pair to modify
  /// @param targetUtil The utilization that is being targeted for the pair
  /// @dev If `toWithdraw` | `toDeposit` amount is gt allocation, they will be set to maximum allowed
  function modifyPairUtilization(
    address _pairAddress,
    uint256 targetUtil
  ) public {
    IFraxlendPairV3 _fraxlendPair = IFraxlendPairV3(_pairAddress);
    ...
    if (currentUtil < targetUtil) {
      // Solve for amount to withdraw
      uint256 toWithdraw = totalAsset - ((totalBorrow * 1e5) / targetUtil);
      uint256 allocationCurrent = pairsMintedFrax[_pairAddress];
      if (toWithdraw > allocationCurrent) toWithdraw = allocationCurrent;
      uint256 sharesToWithdraw = _fraxlendPair.toAssetShares(
        toWithdraw,
        false,
        true
      );
      ...
      /// Implement Withdraw
      withdrawFromPair(_pairAddress, sharesToWithdraw);
    } else if (currentUtil > targetUtil) {
      // Solve for the amount to deposit
      uint256 toDeposit = ((totalBorrow * 1e5) / targetUtil) - totalAsset;
      uint256 allocationRemaining = pairsMaxAllocation[_pairAddress] -
        pairsMintedFrax[_pairAddress];
      if (toDeposit > allocationRemaining) toDeposit = allocationRemaining;

      /// Implement Depoist
      depositToPair(_pairAddress, toDeposit);
    }
    /// @notice default do nothing: target == current
    emit UtilizationTargeted(_pairAddress, targetUtil);
  }

  /// @notice Function to modify the utilization of all pools registered with the AMO
  /// @param targetUtil The utilization that is to be targeted for all pairs
  function modifyPairUtilizationAllPairs(uint256 targetUtil) public {
    address[] memory _pairsArray = pairsArray;
    for (uint256 i = 0; i < _pairsArray.length; ++i) {
      modifyPairUtilization(_pairsArray[i], targetUtil);
    }
  }
  ...
  function burnFRAX(uint256 _fraxAmount) public virtual onlyOwner {
  ...
  function setFraxlendPairDeployer(
    address _newFraxlendPairDeployerAddress
  ) external onlyOwner {
    emit SetFraxlendPairDeployer(
      address(fraxlendPairDeployer),
      _newFraxlendPairDeployerAddress
    );
    fraxlendPairDeployer = IFraxlendPairDeployer(
      _newFraxlendPairDeployerAddress
    );
  }
