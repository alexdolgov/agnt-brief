...
contract CErc721NoBorrowDelegator is CTokenInterface, CErc721Interface, CErc721NoBorrowInterface, CDelegatorInterface {
    error Unauthorized();
     ...
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function redeemUnderlyingNfts(uint[] memory nftIds) external override returns (uint) {
        bytes memory data = delegateToImplementation(abi.encodeWithSignature("redeemUnderlyingNfts(uint256[])", nftIds));
        return abi.decode(data, (uint));
    }
    ...
    /**
     * @notice Gets the amount of NFTs held by a user
     * @return uint The number of NFTs held by a user
     */
    function getNFTsHeldByUser(address user) external view returns (uint[] memory) {
        bytes memory data = delegateToViewImplementation(abi.encodeWithSignature("getNFTsHeldByUser(address)", user));
        return abi.decode(data, (uint[]));
    }
