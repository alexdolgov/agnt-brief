...
error ExceedsCurrentCap(uint256 mintCount);
error CapTooHigh();
error FeeTooHigh();

contract NFTMinter is Upgradeable2Step, IERC721Receiver {

    event NFTsMinted(address indexed user, uint256 nftCount);
    event NFTsBurned(address indexed user, uint256 nftCount);
    event CurrentCapUpdated(uint256 oldMintCurrentCap, uint256 newMintCurrentCap);
    event BurnFeeUpdated(uint256 oldBurnFee, uint256 newBurnFee);

    address public constant BURN_ADDRESS = address(0xdead);
    uint256 public constant MAX_BURN_FEE = 2000;
    uint256 public constant FEE_PERCENT_DENOMINATOR = 10000;
    uint256 public constant MINT_TOTAL_CAP = 3500;
    uint256 public constant EXCHANGE_RATE = 210000e18;

    EIP20Interface public immutable fungToken;
    FungifyNFT public immutable nftContract;

    uint256 public burnFee;
    uint256 public mintCurrentCap;
    ...
    constructor(EIP20Interface fungToken_, FungifyNFT nftContract_) {
        fungToken = fungToken_;
        nftContract = nftContract_;
    }

    function mintNFT(uint256 mintCount) public {
        if (mintCount == 0) {
            revert InvalidDepositAmount();
        }
        if (!whitelistDisabled && !whitelist[msg.sender]) {
            revert Unauthorized();
        }
        ...
        uint256 fungAmount = mintCount * EXCHANGE_RATE;
        ...
        totalMints = totalMints + mintCount;

        emit NFTsMinted(msg.sender, mintCount);
    }

    function burnNFT(uint[] calldata nftIds) public {
        
        nftContract.burn(nftIds, msg.sender);

        uint256 fungAmount = nftIds.length * EXCHANGE_RATE;

        if (burnFee != 0) {
            uint256 burnAmount = fungAmount * burnFee / FEE_PERCENT_DENOMINATOR;
            fungAmount = fungAmount - burnAmount;
            fungToken.transfer(
                BURN_ADDRESS,
                burnAmount
            );
        ...
        totalBurns = totalBurns + nftIds.length;

        emit NFTsBurned(msg.sender, nftIds.length);
    }

    function setCurrentCap(uint256 _mintCurrentCap) external onlyOwner {
    ...
    function setWhitelistDisabled(bool _isDisabled) external onlyOwner {
        whitelistDisabled = _isDisabled;
    }

    function setUsersWhitelisted(address[] memory users, bool _isInWhitelist) external onlyOwner {
        for(uint i; i < users.length;) {
            whitelist[users[i]] = _isInWhitelist;
            unchecked { i++; }
        }
    }
