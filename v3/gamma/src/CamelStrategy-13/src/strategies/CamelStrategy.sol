...
contract CamelStrategy is ILiquidityStrategy {
    ...
    function _processWeightsNoCarpet(
        ...
        (int24 leftPeak, int24 rightPeak, uint256 sigma) = _calculatePeakParams(
            params.centerTick,
            params.ticksLeft,
            params.ticksRight
        );
        ...
        for (uint256 i = 0; i < numRanges; ) {
            baseWeights[i] = _calculateCamelWeight(
                params,
                int24((int256(params.lowerTicks[i]) + int256(params.upperTicks[i])) / 2),
                leftPeak,
                rightPeak
            );
    ...
    function _processWeightsWithCarpetPhase1(
        ...
        (uint256[] memory adjustedWeights, uint256 totalAdjustedWeight) = _calculateCarpetedWeights(
            params,
            startIdx,
            endIdx,
            leftPeak,
    ...
    function _calculateCamelWeight(
        DensityParams memory params,
        int24 posCenter,
        int24 leftPeak,
        int24 rightPeak
        ...
        uint256 leftHumpWeight = _calculateHumpWeight(
            posCenter,
            leftPeak,
            minimum,
            params.centerTick,
        ...
        uint256 rightHumpWeight = _calculateHumpWeight(
            posCenter,
            rightPeak,
            params.centerTick,
            maximum,
    ...
    function _calculatePeakParams(
        int24 centerTick,
        uint24 ticksLeft,
        uint24 ticksRight
    ) private pure returns (int24 leftPeak, int24 rightPeak, uint256 sigma) {
