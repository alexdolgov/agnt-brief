...
contract CamelStrategy is ILiquidityStrategy {
    ...
    function _processWeightsNoCarpet(uint256[] memory weights, DensityParams memory params) private pure {
        ...
        // This ensures symmetric distance calculation for left and right ranges
        ...
        uint256 effectiveTicksRight;
        {
            ...
            // Measure right span from centerRangeUpper for symmetry with left side
        ...
        (int24 leftPeak, int24 rightPeak,) =
            _calculatePeakParamsClamped(params.centerTick, centerRangeUpper, effectiveTicksLeft, effectiveTicksRight);
        ...
        for (uint256 i = 0; i < numRanges;) {
            baseWeights[i] = _calculateCamelWeightFixed(
                params, i, leftPeak, rightPeak, centerRangeUpper, effectiveTicksLeft, effectiveTicksRight
            );
    ...
    function _processWeightsWithCarpetPhase1(uint256[] memory weights, DensityParams memory params) private pure {
        ...
        PeakParams memory peak = _getClampedParamsForCarpet(params, startIdx, endIdx);

        // Calculate base and adjusted weights
        (uint256[] memory adjustedWeights, uint256 totalAdjustedWeight) =
            _calculateCarpetedWeightsFixed(params, startIdx, endIdx, peak);
    ...
    function _getClampedParamsForCarpet(DensityParams memory params, uint256 startIdx, uint256 endIdx)
        private
        pure
        returns (PeakParams memory peak)
    {
        // Find the center range (contains centerTick) to get its upper bound
        // This ensures symmetric distance calculation for left and right ranges
        peak.centerRangeUpper = params.centerTick; // fallback
        for (uint256 i = startIdx; i < endIdx;) {
            if (params.lowerTicks[i] <= params.centerTick && params.centerTick < params.upperTicks[i]) {
                peak.centerRangeUpper = params.upperTicks[i];
                break;
            }
            unchecked { ++i; }
        }
        ...
        uint256 actualLeftSpan = leftSpan > 0 ? uint256(leftSpan) : 0;
        peak.effectiveTicksLeft = actualLeftSpan < uint256(params.ticksLeft) ? actualLeftSpan : uint256(params.ticksLeft);

        // Measure right span from centerRangeUpper for symmetry with left side
        int256 rightSpan = int256(params.upperTicks[endIdx - 1]) - int256(peak.centerRangeUpper);
        uint256 actualRightSpan = rightSpan > 0 ? uint256(rightSpan) : 0;
        peak.effectiveTicksRight = actualRightSpan < uint256(params.ticksRight) ? actualRightSpan : uint256(params.ticksRight);

        if (peak.effectiveTicksLeft == 0) peak.effectiveTicksLeft = 1;
        if (peak.effectiveTicksRight == 0) peak.effectiveTicksRight = 1;

        // Calculate peak parameters using clamped values
        (peak.leftPeak, peak.rightPeak,) = _calculatePeakParamsClamped(params.centerTick, peak.centerRangeUpper, peak.effectiveTicksLeft, peak.effectiveTicksRight);
    }
    ...
    function _calculatePeakParamsClamped(int24 centerTick, int24 centerRangeUpper, uint256 effectiveTicksLeft, uint256 effectiveTicksRight)
        private
        pure
        returns (int24 leftPeak, int24 rightPeak, uint256 sigma)
    {
        ...
        // Left peak is relative to centerTick (lower bound of center range)
        leftPeak = centerTick - int24(uint24(leftOffset));
        // Right peak is relative to centerRangeUpper (upper bound of center range) for symmetry
        rightPeak = centerRangeUpper + int24(uint24(rightOffset));
    ...
    function _calculateCamelWeightFixed(
        ...
        // Use scoped blocks to reduce stack depth
        bool isLeftOfCenter;
        int24 posCenter;
        {
        ...
        // Calculate and return the appropriate hump weight
        if (isLeftOfCenter) {
            int24 minimum = params.centerTick - int24(uint24(effectiveTicksLeft));
            uint256 leftHumpWeight = _calculateHumpWeight(posCenter, leftPeak, minimum, params.centerTick, true);
            return leftHumpWeight + LiquidityDistributionLibrary.HALF_WAD;
        } else {
            int24 maximum = centerRangeUpper + int24(uint24(effectiveTicksRight));
            uint256 rightHumpWeight = _calculateHumpWeight(posCenter, rightPeak, params.centerTick, maximum, false);
            return rightHumpWeight + LiquidityDistributionLibrary.HALF_WAD;
        }
    }
    ...
    function _calculateCarpetedWeightsFixed(
        ...
        for (uint256 i = startIdx; i < endIdx;) {
            baseWeights[i] = _calculateCamelWeightFixed(
                params, i, peak.leftPeak, peak.rightPeak, peak.centerRangeUpper, peak.effectiveTicksLeft, peak.effectiveTicksRight
            );
