...
/**
 * @title CamelStrategy
 * @notice Camel (double-peaked) distribution with a center-owned valley band
 * @dev Key features:
 *      1. Dynamic width targeting ~20 ranges
 *      2. Two-sided requests generate one band that owns centerTick
 *      3. Camel weights keep a single valley point at centerTick
 */
contract CamelStrategy is ILiquidityStrategy {
    ...
    function _processWeightsNoCarpet(uint256[] memory weights, DensityParams memory params) private pure {
        ...
        int24 minimum = params.lowerTicks[0];
        int24 maximum = params.upperTicks[numRanges - 1];
        ...
        for (uint256 i = 0; i < numRanges;) {
            baseWeights[i] = _calculateCamelWeightFixed(
                params.lowerTicks[i], params.upperTicks[i], params.centerTick, minimum, maximum, leftPeak, rightPeak
            );
    ...
    function _processWeightsWithCarpetPhase1(uint256[] memory weights, DensityParams memory params) private pure {
        ...
        (uint256[] memory adjustedWeights, uint256 totalAdjustedWeight) = _calculateCarpetedWeightsFixed(
            params, startIdx, endIdx, leftPeak, rightPeak, effectiveTicksLeft, effectiveTicksRight
        );
    ...
    function _calculateCamelWeightFixed(
        ...
        if (StrategyGridLibrary.containsCenterTick(lowerTick, upperTick, centerTick)) {
            uint256 centerWeight = _calculateHumpWeight(centerTick, rightPeak, centerTick, maximum, false);
            return centerWeight + LiquidityDistributionLibrary.HALF_WAD;
        }

        int256 centerOffsetX2 = StrategyGridLibrary.midpointOffsetX2(lowerTick, upperTick, centerTick);
        bool isLeftOfCenter = centerOffsetX2 < 0;

        // Left hump Gaussian (centered at leftPeak)
        uint256 leftHumpWeight = _calculateHumpWeightFixed(lowerTick, upperTick, leftPeak, minimum, centerTick);

        // Right hump Gaussian (centered at rightPeak)
        uint256 rightHumpWeight = _calculateHumpWeightFixed(lowerTick, upperTick, rightPeak, centerTick, maximum);
    ...
    function _calculateHumpWeightFixed(
        int24 lowerTick,
        int24 upperTick,
        int24 peakCenter,
        int24 boundaryLeft,
        ...
        int256 peakOffsetX2 = StrategyGridLibrary.midpointOffsetX2(lowerTick, upperTick, peakCenter);
        uint256 absDistanceX2 = peakOffsetX2 < 0 ? uint256(-peakOffsetX2) : uint256(peakOffsetX2);

        uint256 sigma = peakOffsetX2 < 0
            ? uint256(int256(peakCenter - boundaryLeft)) / 3
            : uint256(int256(boundaryRight - peakCenter)) / 3;

        if (sigma == 0) sigma = 1;

        uint256 sigmaX2 = 2 * sigma;
        if (absDistanceX2 < sigmaX2 * 6) {
            int256 normalizedDist = int256((absDistanceX2 * 1e18) / sigmaX2);
            int256 exponent = -(normalizedDist * normalizedDist) / 2e18;
            return LiquidityDistributionLibrary.exp(exponent);
        }
    ...
    function _calculateCarpetedWeightsFixed(
        ...
        uint256[] memory baseWeights = new uint256[](numRanges);
        uint256 totalBaseWeight;
        int24 minimum = params.lowerTicks[startIdx];
        int24 maximum = params.upperTicks[endIdx - 1];
    ...
    function getDescription() external pure override returns (string memory) {
        return
        "Camel distribution (double-peaked) with a single center valley, optional full-range floor, and weight preferences";
    }
}
