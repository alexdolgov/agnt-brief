...
contract ExponentialStrategy is ILiquidityStrategy {
    ...
    function _processWeightsWithCarpetPhase1(uint256[] memory weights, DensityParams memory params) private pure {
        ...
        (uint256 lambdaLeft, uint256 lambdaRight, int24 centerMidpoint) = _calculateClampedLambda(params, startIdx, endIdx);

        (uint256[] memory adjustedWeights, uint256 totalAdjustedWeight) =
            _calculateExponentialWeights(params, startIdx, endIdx, lambdaLeft, lambdaRight, centerMidpoint);
    ...
    function _calculateClampedLambda(DensityParams memory params, uint256 startIdx, uint256 endIdx)
        private
        pure
        returns (uint256 lambdaLeft, uint256 lambdaRight, int24 centerMidpoint)
    {
        // Find the center range to get BOTH bounds for symmetric distance calculation
        ...
        // Calculate centerMidpoint for symmetric distance calculation
        centerMidpoint = (centerRangeLower + centerRangeUpper) / 2;
