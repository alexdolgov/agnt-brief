 ...
 * @notice Unified Gaussian distribution strategy supporting all options
...
contract GaussianStrategy is ILiquidityStrategy {
    ...
    uint256 public constant STANDARD_DEVIATION = 3e18; // Standard deviation in tick units (scaled by 1e18)
    ...
    /**
     * @notice Generate ranges with optional carpet support
     */
    ...
    function _processWeightsNoCarpet(uint256[] memory weights, DensityParams memory params) private pure {
        ...
        // Calculate sigma based on range (matching Python: sigma = range/3)
        uint256 sigmaLeft = uint256(params.ticksLeft) / 3;
        uint256 sigmaRight = uint256(params.ticksRight) / 3;
        ...
        for (uint256 i = 0; i < numRanges;) {
            ...
            uint256 sigma = distance < 0 ? sigmaLeft : sigmaRight;
