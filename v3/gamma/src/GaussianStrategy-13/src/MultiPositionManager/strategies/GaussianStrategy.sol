...
contract GaussianStrategy is ILiquidityStrategy {
    ...
    function _processWeightsWithCarpetPhase1(uint256[] memory weights, DensityParams memory params) private pure {
        ...
        (uint256[] memory adjustedWeights, uint256 totalAdjustedWeight) =
            _calculateGaussianWeights(params, startIdx, endIdx);
    ...
    function _calculateGaussianWeights(DensityParams memory params, uint256 startIdx, uint256 endIdx)
        private
        pure
        returns (uint256[] memory adjustedWeights, uint256 totalAdjustedWeight)
    {
        uint256 numRanges = params.lowerTicks.length;
        uint256[] memory baseWeights = new uint256[](numRanges);
        uint256 totalBaseWeight;

        // Find center bounds and calculate sigmas using helper to avoid stack depth issues
        (int24 centerMidpoint, uint256 sigmaLeft, uint256 sigmaRight) = _findCenterAndSigmas(params, startIdx, endIdx);

        // Cache arrays locally to reduce stack depth
        int24[] memory lowers = params.lowerTicks;
        int24[] memory uppers = params.upperTicks;

        // Calculate base Gaussian weights for main ranges
        for (uint256 i = startIdx; i < endIdx;) {
            ...
            // This ensures symmetric weights for both small and large ticksLeft/ticksRight
            {
                int24 rangeMidpoint = (lowers[i] + uppers[i]) / 2;
                int256 dist = int256(rangeMidpoint) - int256(centerMidpoint);
                absDistance = dist < 0 ? uint256(-dist) : uint256(dist);
                isLeftOfCenter = rangeMidpoint < centerMidpoint;
            }
    ...
    /// @dev Helper to find center midpoint and calculate sigmas, reducing stack depth in caller
    function _findCenterAndSigmas(DensityParams memory params, uint256 startIdx, uint256 endIdx)
        private
        pure
        returns (int24 centerMidpoint, uint256 sigmaLeft, uint256 sigmaRight)
    {
        // Find the center range (contains centerTick) to get BOTH bounds
        int24 cLower = params.centerTick; // fallback
        int24 cUpper = params.centerTick; // fallback
        for (uint256 i = startIdx; i < endIdx;) {
            if (params.lowerTicks[i] <= params.centerTick && params.centerTick < params.upperTicks[i]) {
                cLower = params.lowerTicks[i];
                cUpper = params.upperTicks[i];
                break;
            }
            unchecked { ++i; }
        }
        centerMidpoint = (cLower + cUpper) / 2;

        // Calculate CLAMPED sigma based on actual range bounds (symmetric)
        int256 leftSpan = int256(cLower) - int256(params.lowerTicks[startIdx]);
        uint256 actualLeftSpan = leftSpan > 0 ? uint256(leftSpan) : 0;
        sigmaLeft = (actualLeftSpan < uint256(params.ticksLeft) ? actualLeftSpan : uint256(params.ticksLeft)) / 3;

        int256 rightSpan = int256(params.upperTicks[endIdx - 1]) - int256(cUpper);
        uint256 actualRightSpan = rightSpan > 0 ? uint256(rightSpan) : 0;
        sigmaRight = (actualRightSpan < uint256(params.ticksRight) ? actualRightSpan : uint256(params.ticksRight)) / 3;

        // Ensure non-zero sigmas
        if (sigmaLeft == 0) sigmaLeft = 1;
        if (sigmaRight == 0) sigmaRight = 1;
    }
    ...
    function getDescription() external pure override returns (string memory) {
        return "Gaussian distribution with optional carpet and weight preferences";
    }
}
