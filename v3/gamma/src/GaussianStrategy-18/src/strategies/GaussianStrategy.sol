...
contract GaussianStrategy is ILiquidityStrategy {
    ...
    function _processWeightsNoCarpet(
        ...
        for (uint256 i = 0; i < numRanges; i++) {
            ...
            uint256 weight;
            if (absDistance < sigma * 6) { // 6-sigma covers 99.9997% of distribution
    ...
    function _processWeightsWithCarpetPhase1(
        ...
        (uint256[] memory adjustedWeights, uint256 totalAdjustedWeight) = _calculateGaussianWeights(
            params,
            startIdx,
            endIdx
        );
    ...
    function _calculateGaussianWeights(
        DensityParams memory params,
        uint256 startIdx,
        uint256 endIdx
