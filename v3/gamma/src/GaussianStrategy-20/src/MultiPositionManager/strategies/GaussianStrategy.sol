...
contract GaussianStrategy is ILiquidityStrategy {
    ...
    function _processWeightsNoCarpet(uint256[] memory weights, DensityParams memory params) private pure {
        ...
        for (uint256 i = 0; i < numRanges;) {
            // Calculate distance from center using direct method
            // If range contains centerTick, distance is 0 (peak weight)
            int256 distance;
            bool isLeftOfCenter;
        ...
        returns (uint256[] memory adjustedWeights, uint256 totalAdjustedWeight)
    {
        ...
        // Find centerRangeUpper and calculate sigmas in one block to reduce stack depth
        ...
        uint256 sigmaRight;
        {
            ...
            // Calculate CLAMPED sigma based on actual range bounds
