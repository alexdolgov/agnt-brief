...
contract Hypervisor is IAlgebraMintCallback, ERC20Permit, ReentrancyGuard {
   ...
   event Deposit(
    ...
    /// @param _pool Kim pool for which liquidity is managed
    ...
    function deposit(
        ...
        if (total != 0) {
          ...
          shares = FullMath.mulDiv(shares, total, (pool0PricedInToken1 + pool1));
            ...
            if (directDeposit) {
                _handleDirectDeposit(deposit0, deposit1, inMin);
            }
        }
        _mint(to, shares);
    ...
    function _handleDirectDeposit(
        ...
        // First try base position with deposit amounts
        ...
        // Calculate remaining amounts after base position
        uint256 remainingToken0 = deposit0;
        uint256 remainingToken1 = deposit1;
        if (liquidity > 0) {
            (uint256 used0, uint256 used1) = _amountsForLiquidity(baseLower, baseUpper, liquidity);
            remainingToken0 = deposit0 > used0 ? deposit0 - used0 : 0;
            remainingToken1 = deposit1 > used1 ? deposit1 - used1 : 0;
        }

        // Try limit position with remaining amounts
    ...
    /// @notice Internal function to collect and distribute fees from both base and limit positions
    /// @return baseLiquidity Liquidity from base position
    /// @return limitLiquidity Liquidity from limit position
    function _zeroBurn() internal returns(uint128 baseLiquidity, uint128 limitLiquidity) {
        ...
        if(baseLiquidity > 0) {
            pool.burn(baseLower, baseUpper, 0, new bytes(0));
            (uint256 owed0Base, uint256 owed1Base) = pool.collect(address(this), baseLower, baseUpper, type(uint128).max, type(uint128).max);
            totalOwed0 = totalOwed0.add(owed0Base);
            totalOwed1 = totalOwed1.add(owed1Base);
        }
        
        // Handle limit position independently
        (limitLiquidity, ,) = _position(limitLower, limitUpper);
        if(limitLiquidity > 0) {
            pool.burn(limitLower, limitUpper, 0, new bytes(0));
            (uint256 owed0Limit, uint256 owed1Limit) = pool.collect(address(this), limitLower, limitUpper, type(uint128).max, type(uint128).max);
            totalOwed0 = totalOwed0.add(owed0Limit);
            totalOwed1 = totalOwed1.add(owed1Limit);
        }
        
        // Emit event and transfer fees if any were collected
        emit ZeroBurn(fee, totalOwed0, totalOwed1);
        
        if (totalOwed0.div(fee) > 0 && token0.balanceOf(address(this)) > 0) {
            token0.safeTransfer(feeRecipient, totalOwed0.div(fee));
        }
        if (totalOwed1.div(fee) > 0 && token1.balanceOf(address(this)) > 0) {
            token1.safeTransfer(feeRecipient, totalOwed1.div(fee));
        }
    }

    /// @notice External function to collect and distribute fees from both positions
    /// @dev Can only be called by whitelisted address
    /// @return baseLiquidity Liquidity from base position
    /// @return limitLiquidity Liquidity from limit position
    function zeroBurn() external returns(uint128 baseLiquidity, uint128 limitLiquidity) {
        require(msg.sender == whitelistedAddress, "WHE");
        return _zeroBurn();
    }

    function _zeroBurnSingle(int24 tickLower, int24 tickUpper) internal returns(uint128 liquidity) {
      /// update fees for inclusion
      (liquidity, ,) = _position(tickLower, tickUpper);
      if(liquidity > 0) {
        pool.burn(tickLower, tickUpper, 0, new bytes(0));
    ...
    function pullLiquidity(
        ...
        _zeroBurnSingle(tickLower, tickUpper);
    ...
    function rebalance(
        ...
        /// Withdraw all liquidity and collect all fees from  pool
    ...
    function _burnLiquidity(
        ...
        if (liquidity > 0) {
            /// Burn liquidity
            (uint256 owed0, uint256 owed1) = pool.burn(tickLower, tickUpper, liquidity, new bytes(0));
    ...
    /// @notice Callback function of pool mint
