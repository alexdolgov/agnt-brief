...
import {MinFloorLogic} from "./MinFloorLogic.sol";
...
library RebalanceLogic {
    ...
    function _updateStrategyParams(SharedStructs.ManagerStorage storage s, StrategyContext memory ctx) internal {
    ...
    function _calculateLiquiditiesFromWeights(
        ...
        // Allocation-based approach (works for explicit + proportional weights)
        ...
        scaleAllocations(data, remaining0, remaining1, useAssetWeights);

        // Step 3: Fix current range allocation and redistribute excess
        if (data.hasCurrentRange && useAssetWeights) {
            fixCurrentRangeAndRedistribute(data, baseRanges, sqrtPriceX96);
        }

        // Step 4: Apply per-range minimum floors and renormalize
        floorInfo = _applyRangeMinFloorsExternal(data, baseRanges, sqrtPriceX96, floorInfo, remaining0, remaining1);

        // Step 5: Mint all positions with corrected allocations
    ...
    function _reserveFloorLiquidity(
        ...
        int24 tickSpacing,
        bool useCarpet
    ) private pure returns (RebalanceTypes.FloorReserveInfo memory info, uint256 remaining0, uint256 remaining1) {
        remaining0 = total0;
        remaining1 = total1;
    ...
    function _computeFloorReservation(
        ...
        IMultiPositionManager.Range[] memory baseRanges,
        uint256 floorIdx
    ) private pure returns (RebalanceTypes.FloorReserveInfo memory info) {
        info.index = floorIdx;
        ...
        RebalanceTypes.FloorReserveInfo memory candidate = _tryOneSidedFloor(ctx, baseRanges, floorIdx);
        if (candidate.active) {
            info.active = true;
    ...
    function _applyRangeMinFloorsExternal(
        AllocationData memory data,
        IMultiPositionManager.Range[] memory baseRanges,
        uint160 sqrtPriceX96,
        RebalanceTypes.FloorReserveInfo memory floorInfo,
        ...
        (uint256[] memory updated0, uint256[] memory updated1, RebalanceTypes.FloorReserveInfo memory updatedFloor) =
            MinFloorLogic.applyRangeMinFloors(
                data.token0Allocations,
                data.token1Allocations,
                data.currentTick,
                baseRanges,
                sqrtPriceX96,
                floorInfo,
        ...
        data.token0Allocations = updated0;
        data.token1Allocations = updated1;
        return updatedFloor;
    }
