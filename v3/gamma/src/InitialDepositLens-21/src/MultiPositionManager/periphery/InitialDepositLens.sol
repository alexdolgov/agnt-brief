...
contract InitialDepositLens {
        ...
        )
    {
        ...
        if (params.limitWidth > 0) {
            ...
            (ranges, liquidities, ) = SimpleLensInMin.addLimitPositionsAndCalculateInMin(ranges, liquidities, limitParams);
        }

        // 5. Build preview and inMin from the generated ranges/liquidities
        (inMin, preview) = _buildPreviewFromRangesAndLiquidities(ranges, liquidities, params, ctx);
    ...
    function _generateRangesAndLiquiditiesWithSqrtPrice(
        ...
        // Block 3: Calculate weights and liquidities
        {
            ...
            RebalanceLogic._calculateLiquiditiesFromWeights(
                liquidities, weights, baseRanges, amount0, amount1, sqrtPriceX96, useAssetWeights
            );
        }
    }
