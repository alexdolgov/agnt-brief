...
library SimpleLensInMin {
     ...
     */
    function _calculateOutMinForShares(
        MultiPositionManager manager,
        uint256 shares,
        uint256 maxSlippage
    ...
    function _calculateInMinFromExistingData(
        ...
        for (uint256 i = 0; i < rangesLength; i++) {
            ...
            (uint256 amount0For, uint256 amount1For) = LiquidityAmounts.getAmountsForLiquidity(
                sqrtPriceX96,
                sqrtPriceLower,
                sqrtPriceUpper,
                positionLiquidity
    ...
    function _estimatePostSwapAmounts(
        ...
        (uint256 weight0, uint256 weight1) = _calculateTargetWeights(
            params,
            poolKey,
            sqrtPriceX96,
            currentTick
     ...
     */
    function calculateRangesFromStrategy(
        RangeCalculationParams memory params
    ...
    /**
     * @notice Public function to get all outMin and inMin for rebalance
     */
    ...
    function _buildPreviewFromRangesAndLiquidities(
        ...
        _populateExpectedPositions(poolManager, poolKey, ranges, liquidities, ctx.sqrtPriceX96, preview);
        ...
        returns (uint256 unusedAmount0, uint256 unusedAmount1)
    {
        ...
        unusedAmount0 = addr0 == address(0) ?
            managerAddr.balance :
            IERC20(addr0).balanceOf(managerAddr);
        unusedAmount1 = addr1 == address(0) ?
            managerAddr.balance :
            IERC20(addr1).balanceOf(managerAddr);
    }
    ...
    function simulateRebalance(
        ...
        _populateExpectedPositions(poolManager, poolKey, allRanges, allLiquidities, ctx.sqrtPriceX96, preview);
    }
    ...
    function previewWithdrawCustomInternal(
        ...
        // Get current total amounts, supply, and position count
        ...
        // Calculate shares to burn using WithdrawLogic (same logic as actual withdrawal)
        ...
        (data.unusedAmount0, data.unusedAmount1) = getUnusedBalances(manager, manager.poolKey());

        // Check if balance + fees sufficient (PATH 1/2) or need to burn positions (PATH 3)
        if ((data.unusedAmount0 + data.totalFee0) >= amount0Desired && (data.unusedAmount1 + data.totalFee1) >= amount1Desired) {
            // PATH 1/2: Balance sufficient - delegate to helper
            _handlePath1And2(
                result,
                manager,
                data,
    ...
    function _handlePath1And2(
        ...
        (result.outMin, result.outMinForRebalance) = createZeroOutMinArrays(
            data.totalPositions
        );
    ...
    function _handlePath3WithRebalance(
        ...
        result.positionSharesBurned = WithdrawLogic.calculatePositionSharesToBurnForSimpleLens(
            address(manager),
            amount0Desired,
            amount1Desired
        );
        ...
        result.outMinForRebalance = createOutMinForRebalanceArray(
            result.isFullBurn,
            data.totalPositions
        );
