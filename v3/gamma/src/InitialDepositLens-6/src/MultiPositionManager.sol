  ...
  Multicall
{
  ...
  ) external payable onlyOwnerOrRelayerOrFactory {
    if (s.basePositionsLength > 0) {
      poolManager.unlock(abi.encode(IMultiPositionManager.Action.ZERO_BURN, ""));
    }
  ...
  ) nonReentrant external onlyOwnerOrRelayerOrFactory returns (uint256 amount0, uint256 amount1) {
  ...
  function withdrawCustom(
    ...
    uint256[2][] memory outMin
  ) external nonReentrant onlyOwnerOrRelayerOrFactory returns (uint256 amount0Out, uint256 amount1Out, uint256 sharesBurned) {
  ...
  function claimFee() external {
    if (msg.sender == owner() || s.relayers[msg.sender]) {
      ...
      poolManager.unlock(
        abi.encode(IMultiPositionManager.Action.CLAIM_FEE, abi.encode(owner()))
      );
