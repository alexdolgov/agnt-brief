...
contract LimitOrderLens is Ownable {
    ...
    error MinAmountNotMet(Currency currency, uint256 amount, uint256 minAmount);
    ...
    // /// @notice Get the minimum amount required for a specific currency
    // /// @param currency The currency to check
    // /// @return The minimum order amount for the currency
    // function getMinAmount(Currency currency) external view returns (uint256) {
    //     return limitOrderManager.minAmount(currency);
    // }
    ...
    function getUserPositionCount(
        ...
        return limitOrderManager.getUserPositionCount(user, poolId);
    }
    ...
    function getUserPositions(address user) internal view returns (
        ...
        for (uint256 i = 0; i < poolCount; i++) {
            ...
            LimitOrderManager.PositionInfo[] memory positions = getAllUserPositionsForPool(user, poolId);
    ...
    //     require(index < poolIdBytes.length());
    ...
    function _getPopulatedTicksInRange(
        ...
        for (int16 wordPos = int16((startTick / tickSpacing) >> 8); wordPos <= int16((endTick / tickSpacing) >> 8); wordPos++) {
    ...
    function _createBasicPositionInfo(
        ...
        positions[index].orderType = "Limit";
    }
    ...
    /// @notice Helper function to calculate and validate order amounts
    function _calculateOrderAmounts(
        uint256 totalAmount,
        uint256 totalOrders,
        uint256 sizeSkew,
        uint256 minAmountRequired,
        Currency currency
    ) internal pure returns (OrderStruct[] memory orders) {
        orders = new OrderStruct[](totalOrders);
        uint256 totalAmountUsed;
        
        for (uint256 i = 0; i < totalOrders;) {
            orders[i].amount = (i == totalOrders - 1) ? 
                totalAmount - totalAmountUsed : 
                _calculateOrderSize(totalAmount, totalOrders, sizeSkew, i + 1);
            
            totalAmountUsed += orders[i].amount;
            
            if (minAmountRequired > 0 && orders[i].amount < minAmountRequired) {
                revert MinAmountNotMet(currency, orders[i].amount, minAmountRequired);
            }
            unchecked { i++; }
        }
    }
    ...
    function verifyOrderSizes(
        ...
        {
            Currency currency = isToken0 ? poolIdToKey[poolId].currency0 : poolIdToKey[poolId].currency1;
            uint256 minAmountRequired = limitOrderManager.minAmount(currency);
            uint256 totalAmountUsed;
    ...
    function calculateOrderSizes(
        ...
        for (uint256 i = 0; i < totalOrders;) {
            if (i == totalOrders - 1) {
                ...
                orderSizes[i] = _calculateOrderSize(
                    totalAmount,
                    totalOrders,
                    sizeSkew,
                    i + 1
