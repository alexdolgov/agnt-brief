...
import {PositionManagement} from "./PositionManagement.sol";
...
contract LimitOrderLens is Ownable, AccessControl {
    ...
    constructor(address _limitOrderManagerAddr) Ownable(msg.sender) {
        ...
        // Grant _owner the role which can grant POOL_FACTORY_ROLE role to another
        _grantRole(DEFAULT_ADMIN_ROLE, msg.sender);
        // Grant _owner the role which can add or remove PoolId
        _grantRole(POOL_FACTORY_ROLE, msg.sender);
    }
    ...
    function addPoolId(PoolId poolId, PoolKey calldata key) external onlyRole(POOL_FACTORY_ROLE) {
