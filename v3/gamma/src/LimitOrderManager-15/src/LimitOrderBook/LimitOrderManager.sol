...
contract LimitOrderManager is ILimitOrderManager, IUnlockCallback, AccessControl, ReentrancyGuard, Pausable {
    ...
    // Hard disable flag. When set, factory-triggered enableHook calls are ignored.
    mapping(address => bool) public isHookHardDisabled;
...
// =========== Create Order Functions ===========
    ...
    function _createOrder(
        ...
        require(Currency.unwrap(key.currency0) < Currency.unwrap(key.currency1), "InvalidPoolOrdering");
        require(!key.currency1.isAddressZero(), "InvalidPoolOrdering");
...
// =========== Admin Functions ===========
    ...
    function enableHook(address _hook) external {
        ...
        bool isFactory = msg.sender == orderBookFactoryAddr;
        require(isAdmin || isFactory, "UnauthorizedAdminOrOrderBookFactoryNotSet");
        require(_hook != address(0), "ZeroAddress");

        // Emergency hard-disable cannot be overridden by factory-triggered calls.
        if (isFactory && isHookHardDisabled[_hook]) {
            return;
        }

        // Admin can explicitly recover a hard-disabled hook.
        if (isAdmin && isHookHardDisabled[_hook]) {
            isHookHardDisabled[_hook] = false;
        }
        isHook[_hook] = true;
    }
