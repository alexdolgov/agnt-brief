...
library RebalanceLogic {
    ...
    function rebalanceSwap(
        ...
        // 1. Burn all existing positions
        (uint256 amount0, uint256 amount1) = _burnForSwap(s, poolManager, outMin, totalSupply);

        // 2. Execute swap as specified in swapData (trusts off-chain calculation)
        (amount0, amount1) = _executeProvidedSwap(s, aggregator, swapData, minAmountOut, amount0, amount1);

        // 3. Process rebalance with new token amounts
    ...
    function _extractSwapParamsFromCalldata(
        ...
        // Extract tokenIn address and amount from common positions
        // This is heuristic-based but works for most aggregators
        // Common patterns:
        // - ODOS: tokenIn at offset 0x04, amount at offset 0x24
        // - ParaSwap: similar structure
        // - 1inch: similar structure
        address tokenIn;
