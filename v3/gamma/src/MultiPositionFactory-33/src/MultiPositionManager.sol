  ...
  Multicall
{
   ...
   * @param directDeposit If true, adds liquidity directly to existing positions proportionally
   * @param inMin Minimum amounts for each base position (for slippage protection)
   * @return shares Number of shares to mint
   * @return deposit0 Quantity of token0 to be deposited
   * @return deposit1 Quantity of token1 to be deposited
   */
  function deposit(
  ...
  ) external payable nonReentrant onlyOwner returns (
    uint256 shares,
    uint256 deposit0,
    uint256 deposit1
  ) {
    ...
    // Interactions: Transfer tokens and potentially add liquidity
    _transferIn(from, s.currency0, deposit0);
    _transferIn(from, s.currency1, deposit1);

    emit Deposit(from, to, shares, deposit0, deposit1);

    // If direct deposit is enabled, add liquidity to positions
    if (directDeposit && s.basePositionsLength > 0) {
      // Get amounts for positions
      (uint256 amount0ForPositions, uint256 amount1ForPositions) = DepositLogic.getDirectDepositAmounts(
        s,
        poolManager,
        deposit0,
        deposit1,
      ...
      if (amount0ForPositions > 0 || amount1ForPositions > 0) {
        bytes memory params = abi.encode(amount0ForPositions, amount1ForPositions, inMin);
        poolManager.unlock(abi.encode(IMultiPositionManager.Action.DIRECT_DEPOSIT, params));
      }
    }
  }
  ...
  function rebalance(
    ...
    uint256[2][] memory outMin,
    uint256[2][] memory inMin
  ) nonReentrant public onlyOwnerOrRebalancer {
   ...
   * @param outMin Minimum output amounts for withdrawals
   * @param inMin Minimum input amounts for new positions (slippage protection)
   * @dev Burns all positions, swaps to target ratio, then rebalances with new amounts
   */
  ...
  function _executeActionWithoutUnlock(
    ...
    if (selector == IMultiPositionManager.Action.WITHDRAW) {
      ...
      if (caller == owner()) {
        ...
        (uint256 totalFee0, uint256 totalFee1) = WithdrawLogic.zeroBurnAllWithoutUnlock(s, poolManager);

        // After zeroBurnAll, treasury portion is minted as ERC-6909 to contract
        // The owner's portion creates negative deltas that are settled by _closePair
        _closePair();

        // If there are fees, transfer owner's portion
        // After _closePair, owner's fees are in contract as ETH or ERC20
        if (s.fee > 0) {
          ...
          // Transfer owner's portion (now in contract after _closePair)
          if (ownerFee0 > 0) {
      ...
      WithdrawLogic.claimFee(poolManager, s.factory, s.currency0);
      WithdrawLogic.claimFee(poolManager, s.factory, s.currency1);
      ...
      (uint256 amount0Out, uint256 amount1Out, uint256 remainingToken0, uint256 remainingToken1) =
        WithdrawLogic.processWithdrawCustomInCallback(
          s,
          poolManager,
          amount0Desired,
          amount1Desired,
      ...
      // Return values - deltas will be closed by _unlockCallback after this returns
