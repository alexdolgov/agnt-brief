...
library RebalanceLogic {
     ...
     * @dev Uses Python approach: allocate, fix current range, redistribute excess, then mint
     */
    function _calculateLiquiditiesFromWeights(
        ...
        _calculateInitialAllocations(data, baseRanges, weights, sqrtPriceX96);

        // Step 2: Scale allocations proportionally to available tokens
        _scaleAllocations(data, total0, total1);

        // Step 3: Fix current range allocation and redistribute excess
        if (data.hasCurrentRange) {
            _fixCurrentRangeAndRedistribute(data, baseRanges, sqrtPriceX96);
        }

        // Step 4: Mint all positions with corrected allocations
        _mintFromAllocations(liquidities, data, baseRanges, sqrtPriceX96);
    }

    /**
     * @notice Calculate initial token allocations based on weights
     */
    function _calculateInitialAllocations(
        AllocationData memory data,
        IMultiPositionManager.Range[] memory baseRanges,
        uint256[] memory weights,
        uint160 sqrtPriceX96
    ...
    function _scaleAllocations(
        ...
        // Scale all allocations to match available tokens
     ...
     */
    function _fixCurrentRangeAndRedistribute(
        AllocationData memory data,
        IMultiPositionManager.Range[] memory baseRanges,
        uint160 sqrtPriceX96
    ) private pure {
        ExcessData memory excess = _calculateCurrentRangeExcess(
            data,
        ...
        if (excess.excessToken0 > 0) {
            _redistributeToken0(data, baseRanges, excess.excessToken0);
        }
        if (excess.excessToken1 > 0) {
            _redistributeToken1(data, baseRanges, excess.excessToken1);
        }
    }

    function _calculateCurrentRangeExcess(
        AllocationData memory data,
        IMultiPositionManager.Range memory range,
        uint160 sqrtPriceX96
    ) private pure returns (ExcessData memory excess) {
    ...
    /**
     * @notice Helper to calculate liquidity for token0
     */
    function _getLiquidityForAmount0(
        ...
        if (sqrtPriceX96 >= sqrtPriceUpperX96) return 0;
        uint256 intermediate = FullMath.mulDiv(sqrtPriceUpperX96, sqrtPriceX96, FixedPoint96.Q96);
        return uint128(FullMath.mulDiv(amount0, intermediate, sqrtPriceUpperX96 - sqrtPriceX96));
    }

    /**
     * @notice Helper to calculate liquidity for token1
     */
    function _getLiquidityForAmount1(
        ...
        if (sqrtPriceX96 <= sqrtPriceLowerX96) return 0;
        return uint128(FullMath.mulDiv(amount1, FixedPoint96.Q96, sqrtPriceX96 - sqrtPriceLowerX96));
    }

    /**
     * @notice Helper to calculate liquidity from amounts
     */
    function _getLiquidityForAmounts(
        ...
        if (sqrtPriceX96 <= sqrtPriceLowerX96) {
            ...
            return _getLiquidityForAmount0(sqrtPriceLowerX96, sqrtPriceUpperX96, amount0);
            ...
            return _getLiquidityForAmount1(sqrtPriceLowerX96, sqrtPriceUpperX96, amount1);
        }
    }
