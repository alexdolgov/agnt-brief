...
contract PoolFactory is AccessControl {
    ...
    function _prepareMintParams(
        ...
        // Build params directly with inline tick calculations
        params.poolKey = poolKey;
        params.tickLowerFull = (TickMath.MIN_TICK / tickSpacing) * tickSpacing;
        params.tickUpperFull = (TickMath.MAX_TICK / tickSpacing) * tickSpacing;
        ...
        // Calculate full range liquidity
        params.fullRangeLiquidity = _calculateOptimalLiquidity(
        ...
        // Calculate concentrated liquidity if needed
        if (hasConcentratedPosition) {
            params.concentratedLiquidity = _calculateOptimalLiquidity(
                concentratedTickLower,
                concentratedTickUpper,
                sqrtPriceX96,
                concentratedAmount0Max,
    ...
    function _calculateOptimalLiquidity(
        ...
        if (amount0Max > 0 && amount1Max > 0) {
            // Calculate liquidity from both tokens and use the minimum
            (uint128 liquidity0, ) = calculatePositionAmounts(
                tickLower, tickUpper, sqrtPriceX96, amount0Max, true
            );
            (uint128 liquidity1, ) = calculatePositionAmounts(
                tickLower, tickUpper, sqrtPriceX96, amount1Max, false
            );
            // Use the minimum liquidity to ensure both token amounts are sufficient
            return liquidity0 < liquidity1 ? liquidity0 : liquidity1;
        } else if (amount0Max > 0) {
            (uint128 liquidity, ) = calculatePositionAmounts(
                tickLower, tickUpper, sqrtPriceX96, amount0Max, true
            );
            return liquidity;
        } else if (amount1Max > 0) {
            (uint128 liquidity, ) = calculatePositionAmounts(
                tickLower, tickUpper, sqrtPriceX96, amount1Max, false
            );
            return liquidity;
