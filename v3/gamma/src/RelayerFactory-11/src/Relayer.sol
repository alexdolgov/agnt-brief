...
contract Relayer is IRelayer, ReentrancyGuard {
    ...
    function executeCompoundSwap(
        ...
        // Effects: Update rebalance count (compound is a form of rebalancing)
        unchecked {
            ++rebalanceCount;
        }
        lastRebalanceTime = block.timestamp;
        ...
        emit AutomatedCompoundExecuted(
            outOfPositionRatio,
            compoundSwapParams.outOfPositionRatioThreshold,
            address(manager)
        );

        // Calculate gas used and reimburse caller
        // Create a dummy status for gas reimbursement (using swap flag)
        RebalanceTriggerStatus memory status = RebalanceTriggerStatus({
            baseTickTrigger: false,
            baseRatioTrigger: false,
            limitTickTrigger: false,
            limitRatioTrigger: false,
            outOfPositionTrigger: true,  // compound swap is triggered by out of position ratio
            anyTriggerMet: true
        });
        _reimburseGas(gasBefore, status, true);
    }
     ...
     */
    function getRebalanceParams() external view override returns (
        TriggerConfig memory,
        StrategyParams memory,
        VolatilityParams memory
    ) {
    ...
    function _validateWithdrawalParamsStruct(WithdrawalParams memory params) private pure {
        ...
        if (params.withdrawAll && (params.withdrawToken0Only || params.withdrawToken1Only)) {
        ...
        if (!params.withdrawAll &&
            !params.withdrawToken0Only &&
            !params.withdrawToken1Only &&
            params.pool0RatioThreshold == 0 &&
            params.pool1RatioThreshold == 0) {
            revert InvalidWithdrawalParams();
        }
    }
    ...
    function executeWithdrawal(uint256[2][] memory outMin) external override nonReentrant {
        ...
        // Determine withdrawal type and execute
        if (withdrawalParams.withdrawAll || (withdrawalParams.withdrawToken0Only && withdrawalParams.withdrawToken1Only)) {
        ...
        emit AutomatedWithdrawalExecuted(
            pool0Triggered,
            pool1Triggered,
            address(manager)
        );
    ...
    function _validateWithdrawalParams() private view {
        ...
        if (!withdrawalParams.withdrawAll &&
            !withdrawalParams.withdrawToken0Only &&
            !withdrawalParams.withdrawToken1Only &&
            withdrawalParams.pool0RatioThreshold == 0 &&
            withdrawalParams.pool1RatioThreshold == 0) {
            revert InvalidWithdrawalParams();
        }
    }
