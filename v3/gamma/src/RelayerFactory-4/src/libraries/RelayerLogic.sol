...
library RelayerLogic {
    ...
    function checkTriggers(
        ...
        status = _checkTickTriggers(manager, triggerConfig, strategyParams, isProportional, status);

        // Check ratio-based triggers if not proportional and not circuit broken
        if (!isProportional && (status.baseTickTrigger || !_isCircuitBroken(manager, triggerConfig))) {
            status = _checkRatioTriggers(manager, triggerConfig, strategyParams, status);
        }

        status.anyTriggerMet = status.baseTickTrigger
            || status.baseRatioTrigger
            || status.limitTickTrigger
            || status.limitRatioTrigger
            || status.outOfPositionTrigger;
    }

    function _isCircuitBroken(
        IMultiPositionManager manager,
        IRelayer.TriggerConfig memory triggerConfig
    ...
    function _checkTickTriggers(
        ...
        if (triggerConfig.baseLowerTrigger != 0 || triggerConfig.baseUpperTrigger != 0) {
            ...
            if (tickDelta > 0) {
                ...
                triggered = (triggerConfig.baseUpperTrigger != 0 && uint256(int256(tickDelta)) >= triggerConfig.baseUpperTrigger);
            } else if (tickDelta < 0) {
            ...
            status.baseTickTrigger = triggered && (triggerConfig.maxDeltaTicks == 0 || absDelta <= triggerConfig.maxDeltaTicks);
        }
    ...
    function _checkRatioTriggers(
        ...
        bool needRatios = (strategyParams.isBaseRatio && (triggerConfig.baseMinRatio != 0 || triggerConfig.baseMaxRatio != 0)) ||
            triggerConfig.limitMinRatio != 0 ||
            triggerConfig.limitDeltaTicks != 0 ||
            triggerConfig.outOfPositionThreshold != 0;

        if (!needRatios) return status;

        (, , , , uint256 outOfPositionRatio, , uint256 limitRatio, uint256 base0Ratio, , uint256 limit0Ratio, uint256 limit1Ratio, ) = manager.getRatios();
        ...
        if (triggerConfig.limitMinRatio != 0 && limitRatio > triggerConfig.limitThreshold) {
            (, int24 centerTick, , , , , , , ) = manager.lastStrategyParams();
            status.limitRatioTrigger = checkLimitRatioTrigger(manager, centerTick, limit0Ratio, limit1Ratio, triggerConfig.limitMinRatio);
        }
     ...
     */
    function _checkLimitTickTrigger(
        IMultiPositionManager manager,
        IRelayer.TriggerConfig memory triggerConfig
    ) private view returns (bool triggered) {
        ...
        if (limitLength == 2 &&
            positionData[ranges.length - 2].liquidity > positionData[ranges.length - 1].liquidity &&
        ...
        if (triggered && triggerConfig.maxDeltaTicks != 0) {
            ...
            triggered = (tickDelta >= 0 ? uint256(int256(tickDelta)) : uint256(int256(-tickDelta))) <= triggerConfig.maxDeltaTicks;
        }
    }
    ...
    function constructRebalanceParams(
        ...
        bool useSentinelValue = status.baseTickTrigger
            || status.baseRatioTrigger
