  ...
  Multicall
{
  ...
  error InvalidAction();
  error TicksLeftExceedsBoundary(uint24 ticksLeft, int24 center, int24 minUsableTick);
  error TicksRightExceedsBoundary(uint24 ticksRight, int24 center, int24 maxUsableTick);
   ...
   */
  function compound(uint256[2][] calldata inMin) external payable onlyOwnerOrFactory {
  ...
  /**
   * @notice Withdraw function callable only by relayer role, sends to owner
   * @dev Relayer can call this to withdraw all of owner's shares and send tokens to owner
   * @param outMin Minimum output amounts for slippage protection
   * @param withdrawToWallet Whether to transfer tokens and burn shares
  ...
  function withdrawRelay(
    uint256[2][] memory outMin,
    bool withdrawToWallet
  ) external nonReentrant onlyOwnerOrRelayerOrFactory returns (uint256 amount0, uint256 amount1) {
    ...
    (amount0, amount1) = WithdrawLogic.processWithdraw(
      ...
      poolManager,
      shares,
      owner(),  // to = owner (hardcoded)
      outMin,
      totalSupply(),
      owner(),  // balance check against owner
      withdrawToWallet
    );

    if (withdrawToWallet) {
      _burn(owner(), shares);  // burn from owner's balance
    }
  }
  ...
  function rebalance(
    ...
    // Validate tick boundaries to prevent extreme tick ranges (scoped to reduce stack depth)
    {
      int24 tickSpacing = s.poolKey.tickSpacing;
      int24 minUsableTick = (-887272 / tickSpacing) * tickSpacing;
      int24 maxUsableTick = (887272 / tickSpacing) * tickSpacing;

      // Resolve center tick (handle CENTER_AT_CURRENT_TICK flag)
      int24 center = params.center;
      if (center == CENTER_AT_CURRENT_TICK) {
        (, center, , ) = poolManager.getSlot0(s.poolId);
      }

      // Check if ticksLeft/ticksRight would exceed usable tick boundaries
      int256 lowerBound = int256(center) - int256(uint256(params.tLeft));
      int256 upperBound = int256(center) + int256(uint256(params.tRight));

      if (lowerBound < int256(minUsableTick)) {
        revert TicksLeftExceedsBoundary(params.tLeft, center, minUsableTick);
      }
      if (upperBound > int256(maxUsableTick)) {
        revert TicksRightExceedsBoundary(params.tRight, center, maxUsableTick);
      }
    }
  ...
  function rebalanceSwap(
    ...
    // Validate tick boundaries to prevent extreme tick ranges (scoped to reduce stack depth)
    {
      ...
      // Resolve center tick (handle CENTER_AT_CURRENT_TICK flag)
      int24 center = params.rebalanceParams.center;
      if (center == CENTER_AT_CURRENT_TICK) {
        (, center, , ) = poolManager.getSlot0(s.poolId);
      }

      // Check if ticksLeft/ticksRight would exceed usable tick boundaries
      int256 lowerBound = int256(center) - int256(uint256(params.rebalanceParams.tLeft));
      int256 upperBound = int256(center) + int256(uint256(params.rebalanceParams.tRight));

      if (lowerBound < int256(minUsableTick)) {
        revert TicksLeftExceedsBoundary(params.rebalanceParams.tLeft, center, minUsableTick);
      }
      if (upperBound > int256(maxUsableTick)) {
        revert TicksRightExceedsBoundary(params.rebalanceParams.tRight, center, maxUsableTick);
      }
    }
  ...
  function getTotalValuesInOneToken() external view returns (
    uint256 totalValueInToken0,
