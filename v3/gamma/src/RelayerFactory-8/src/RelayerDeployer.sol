...
contract RelayerDeployer {
    ...
    function deploy(
        ...
        bytes32 salt = _computeSalt(mpm, owner);
        return address(new Relayer{salt: salt}(
            mpm,
            factory,
            owner,
            triggerConfig,
    ...
    /**
     * @notice Internal function to compute salt from mpm and owner
     * @param mpm The MultiPositionManager address
     * @param owner The owner address
     * @return The computed salt
     */
    function _computeSalt(address mpm, address owner) private pure returns (bytes32) {
        return keccak256(abi.encodePacked(mpm, owner));
    }
}
