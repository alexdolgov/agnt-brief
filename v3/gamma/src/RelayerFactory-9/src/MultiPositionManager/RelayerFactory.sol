...
contract RelayerFactory is IRelayerFactory, Ownable {
     ...
     * @param _relayerDeployer Predeployed RelayerDeployer authorized for this factory
     */
    constructor(address _owner, IMultiPositionFactory _multiPositionFactory, address _relayerDeployer)
        Ownable(_owner)
    {
        if (_owner == address(0)) revert InvalidAddress();
        if (address(_multiPositionFactory) == address(0)) revert InvalidAddress();
        if (_relayerDeployer == address(0) || _relayerDeployer.code.length == 0) revert InvalidAddress();
        multiPositionFactory = _multiPositionFactory;

        RelayerDeployer existingDeployer = RelayerDeployer(_relayerDeployer);
        if (existingDeployer.authorizedFactory() != address(this)) revert InvalidAddress();
