...
contract MultiPositionFactory is IMultiPositionFactory, Ownable, Multicall {
    ...
    // Strategy requires limit position (strategy address -> required)
    mapping(address => bool) public override requiresLimitPosition;
    ...
    /**
     * @notice Set whether a strategy requires a limit position (i.e. no proportional-only rebalances)
     * @param strategy Strategy contract address
     * @param required True to require limit position, false to allow proportional
     */
    function setRequiresLimitPosition(address strategy, bool required) external override onlyOwner {
        if (strategy == address(0)) revert InvalidAddress();
        requiresLimitPosition[strategy] = required;
        emit RequiresLimitPositionSet(strategy, required);
    }
