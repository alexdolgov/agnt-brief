...
library SimpleLensLibrary {
        ...
        returns (uint256[2][] memory inMin, SimpleLensRatioUtils.PositionStats[] memory expectedPositions)
    {
        ...
        expectedPositions = _buildExpectedPositions(manager, ranges, liquidities, sqrtPriceX96);

        // Calculate inMin
        inMin = _calculateInMinArray(expectedPositions, maxSlippage);
    }
        ...
        returns (IMultiPositionManager.Range[] memory ranges, uint128[] memory liquidities, uint160 sqrtPriceX96)
    {
        ...
        // Resolve center tick
        int24 resolvedCenterTick = rebalanceParams.center;
        ...
        ctx.weightsAreProportional = (rebalanceParams.weight0 == 0 && rebalanceParams.weight1 == 0);
    ...
    function _buildExpectedPositions(
        ...
        for (uint256 i = 0; i < ranges.length; i++) {
            expectedPositions[i] = _calculatePositionStats(ranges[i], liquidities[i], sqrtPriceX96, poolKey);
        }
    }
     ...
     * @param poolKey The pool key (for creating Position struct)
