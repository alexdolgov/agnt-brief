...
library SimpleLensInMin {
    ...
    function _estimatePostSwapAmounts(
        ...
        // Get pool state and resolve weights
        (uint256 weight0, uint256 weight1, uint160 sqrtPriceX96) = _getWeightsAndPrice(params);
        ...
        // Apply conservative 10% reduction to account for 0x routing differences
        newAmount0 = FullMath.mulDiv(newAmount0, 9000, 10000);
        newAmount1 = FullMath.mulDiv(newAmount1, 9000, 10000);
    }

    /**
     * @notice Get target weights and current price
     */
