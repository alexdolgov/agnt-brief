...
library SimpleLensRatioUtils {
    ...
    function calculateDirectDepositInMin(
        ...
        for (uint256 i = 0; i < ranges.length; i++) {
            ...
            if (totalToken0InPositions > 0) {
                uint256 amount0ForPosition = FullMath.mulDiv(amount0ForPositions, positionData[i].amount0, totalToken0InPositions);
                inMin[i][0] = FullMath.mulDiv(amount0ForPosition, slippageFactor, 10000);
            }

            if (totalToken1InPositions > 0) {
                uint256 amount1ForPosition = FullMath.mulDiv(amount1ForPositions, positionData[i].amount1, totalToken1InPositions);
    ...
    function getAmountsForDepositAndCompound(
        ...
        // Calculate final amounts and swap params in scoped block
        {
            ...
            uint256 weight0;
            uint256 weight1;
            (finalAmount0, finalAmount1, swapToken0, swapAmount, weight0, weight1) = _getCompoundSwapAmounts(
                manager,
                deposit0,
                deposit1,
                needsSwap
