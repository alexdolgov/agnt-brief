...
library SimpleLensInMin {
    ...
    function _calculateInMinInternal(
        ...
        // Block 3: Prepare density params
        {
            ...
            // For proportional weights, calculate from post-swap amounts (matches RebalanceLogic)
    ...
    function _estimatePostSwapAmounts(
        ...
        // For proportional weights, we need to estimate swap first, then calculate weights from post-swap amounts
        // This matches RebalanceLogic which calls calculateWeightsFromAmounts
        if (params.weight0 == 0 && params.weight1 == 0) {
            // First estimate with 50/50 weights to get approximate swap
            ...
            // For explicit weights, calculate optimal swap directly
            (bool swapToken0, uint256 swapAmount) = RebalanceLogic.calculateOptimalSwap(
                amount0,
                amount1,
                sqrtPriceX96,
