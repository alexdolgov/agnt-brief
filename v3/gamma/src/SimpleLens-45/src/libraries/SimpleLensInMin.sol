...
library SimpleLensInMin {
    ...
    function _calculateInMinInternal(
        ...
        // Block 1: Get amounts and apply swap adjustment
        (calcData.amount0, calcData.amount1) = _getAdjustedAmountsForSwap(params);
    ...
    /**
     * @notice Get adjusted amounts after accounting for swap
     * @dev Helper function to avoid stack too deep
     */
    function _getAdjustedAmountsForSwap(
        InMinRebalanceParams memory params
    ) private view returns (uint256 amount0, uint256 amount1) {
        (amount0, amount1) = _getTotalAmountsForRebalance(params.manager);

        if (!params.swap) {
            return (amount0, amount1);
        }

        // Get optimal swap calculation
        IPoolManager pm = params.manager.poolManager();
        SimpleLensRatioUtils.SwapParams memory swapCalc = SimpleLensRatioUtils.calculateOptimalSwapForRebalance(
            params.manager,
        ...
        // Get current price
        (uint160 sqrtPriceX96Local, , , ) = pm.getSlot0(params.manager.poolKey().toId());

        // Estimate swap output and adjust amounts
        if (swapCalc.swapToken0) {
            // Swapping token0 → token1
            uint256 estimatedOutput = FullMath.mulDiv(
                swapCalc.swapAmount,
                FullMath.mulDiv(sqrtPriceX96Local, sqrtPriceX96Local, FixedPoint96.Q96),
                FixedPoint96.Q96
            );
            amount0 -= swapCalc.swapAmount;
            amount1 += estimatedOutput;
        } else {
            ...
            amount1 -= swapCalc.swapAmount;
            amount0 += estimatedOutput;
        }
    ...
    function calculateLiquidities(
        ...
        // Use FullMath.mulDiv for proper rounding instead of truncation
        for (uint i = 0; i < rangesLength; i++) {
            liquidities[i] = uint128(FullMath.mulDiv(totalLiquidity, weights[i], 1e18));
        }
    }
