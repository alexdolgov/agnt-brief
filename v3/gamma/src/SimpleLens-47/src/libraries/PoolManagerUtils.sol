...
library PoolManagerUtils {
  ...
  function mintLiquidities(
    ...
    positionCount = _mintLimitPositions(
      poolManager,
      poolKey,
      limitRanges,
      positionData,
  ...
  function _mintLiquidityForAmounts(
    ...
    if (liquidity != 0) {
      ...
      if (
        actualAmount0 < inMin[0] ||
        actualAmount1 < inMin[1]
      ) {
        revert SlippageExceeded();
      }
    }

    return IMultiPositionManager.PositionData({
      liquidity: liquidity,
  ...
  function calculateOutMinForRebalance(
    ...
    for (uint i = 0; i < totalPositionsLength; ) {
      (uint256 amount0, uint256 amount1) = getAmountsForLiquidity(
        poolManager,
        poolKey,
        ranges[i],
