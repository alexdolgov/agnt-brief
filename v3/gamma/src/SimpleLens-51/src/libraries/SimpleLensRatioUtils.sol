...
library SimpleLensRatioUtils {
    ...
    function getInMinForCompound(
        ...
        (finalAmount0, finalAmount1, swapToken0, swapAmount) = _getCompoundSwapAmounts(
            manager,
            deposit0,
            deposit1,
            needsSwap
    ...
    function getAmountsForDepositAndCompoundSwap(
        ...
        if (manager.basePositionsLength() == 0) {
            return (inMin, false, 0, expectedPositions);
        }
        ...
        expectedPositions = calculateExpectedPositionsAfterDeposit(
            manager,
            ranges,
            positionData,
            finalAmount0,
    ...
    function _getCompoundSwapAmounts(
        MultiPositionManager manager,
        uint256 deposit0,
        uint256 deposit1,
        bool needsSwap
    ) private view returns (uint256 finalAmount0, uint256 finalAmount1, bool swapToken0, uint256 swapAmount) {
        // Calculate total available
        (uint256 totalAvailable0, uint256 totalAvailable1) = _calculateTotalAvailable(manager, deposit0, deposit1);

        if (!needsSwap) {
            return (totalAvailable0, totalAvailable1, false, 0);
        }
        ...
        (swapToken0, swapAmount) = _calculateOptimalSwap(
            totalAvailable0,
            totalAvailable1,
            sqrtPriceX96,
            weight0,
        ...
        (finalAmount0, finalAmount1) = applySwap(
            totalAvailable0,
            totalAvailable1,
            swapToken0,
            swapAmount,
    ...
    function _calculateTotalAvailable(
        MultiPositionManager manager,
        uint256 deposit0,
        uint256 deposit1
    ) private view returns (uint256 totalAvailable0, uint256 totalAvailable1) {
    ...
    function _getCompoundInMin(
        ...
        return calculateDirectDepositInMin(
            manager,
            finalAmount0,
            finalAmount1,
            maxSlippageBps,
     ...
     */
    function calculateWeightsFromPositions(
        MultiPositionManager manager,
        uint160 sqrtPriceX96
    ) public view returns (uint256 weight0, uint256 weight1) {
    ...
    function applySwap(
        ...
        uint256 swapAmount,
        uint160 sqrtPriceX96
    ) public pure returns (uint256 finalAmount0, uint256 finalAmount1) {
        if (swapAmount == 0) {
    ...
    function _calculateOptimalSwap(
        ...
        return RebalanceLogic.calculateOptimalSwap(
            amount0,
            amount1,
            sqrtPriceX96,
            weight0,
    ...
    function calculateExpectedPositionsAfterDeposit(
        ...
        expectedPositions = new PositionStats[](ranges.length);

        DepositCalculationContext memory ctx;
        PoolKey memory poolKey = manager.poolKey();
        IPoolManager pm = manager.poolManager();
        ...
        if (ctx.totalToken0InPositions == 0 && ctx.totalToken1InPositions > 0) {
            ...
            (ctx.amount0ForPositions, ctx.amount1ForPositions) = DepositRatioLib.getRatioAmounts(
                ctx.totalToken0InPositions,
                ctx.totalToken1InPositions,
                deposit0,
                deposit1
            );
        }
        ...
        for (uint256 i = 0; i < ranges.length; i++) {
            ...
            expectedPositions[i] = buildPositionStats(ctx, ranges[i], positionData[i]);
        }
    }
    ...
    function calculateOptimalSwapForRebalance(
        ...
        // Get amounts after burning
        (uint256 amount0, uint256 amount1, , ) = manager.getTotalAmounts();

        // Get current pool state
        PoolKey memory poolKey = manager.poolKey();
        (uint160 sqrtPriceX96, int24 currentTick, , ) = poolManager.getSlot0(poolKey.toId());

        // Resolve center tick if needed
    ...
    /**
     * @notice Calculate swap details for initial deposit
     */
    function calculateSwapDetails(
        uint256 amount0,
        uint256 amount1,
        uint256 weight0Input,
        uint256 weight1Input,
        address strategyAddress,
        ...
        if (weight0Input == 0 && weight1Input == 0) {
            (swapParams.weight0, swapParams.weight1) = RebalanceLogic.calculateWeightsFromStrategy(
            ...
            swapParams.weight0 = weight0Input;
            swapParams.weight1 = weight1Input;
        }
    ...
    function extractWeightsFromStrategy(
        ...
        // Call strategy
        return ILiquidityStrategy(params.strategyAddr).calculateDensities(
            lowerTicks,
            upperTicks,
            params.currentTick,
            params.resolvedCenter,
            params.tLeft,
            params.tRight,
            params.w0,
            params.w1,
    ...
    function buildPositionStats(
        ...
        {
            // Calculate additional amounts in scoped block
            ...
            // Get current amounts
            (uint256 currentAmount0, uint256 currentAmount1) = PoolManagerUtils.getAmountsForLiquidity(
                ctx.pm,
                ctx.poolKey,
                range,
                uint128(posData.liquidity)
            );
        ...
        // Build stats in separate block
        {
            ...
            stats = PositionStats({
                ...
                valueInToken1: newAmount1 + FullMath.mulDiv(
                    newAmount0,
                    uint256(ctx.sqrtPriceX96) * uint256(ctx.sqrtPriceX96),
                    1 << 192
                )
            });
        }
    ...
    function calculateLimitRanges(
        ...
        lowerLimit.lowerTick = roundToTickSpacing(baseTick - limitWidth, tickSpacing, true);
        ...
        upperLimit.upperTick = roundToTickSpacing(baseTick + tickSpacing + limitWidth, tickSpacing, false);
