 ...
 *      those are disabled in MultiPositionManager
 *
 * Key constraints:
...
contract SimpleLens {
    ...
    {
        return LensRatioUtils.getPositionStats(manager);
    }
     ...
     */
    function getTotalAmounts(MultiPositionManager manager)
        external
        view
    ...
    function previewRebalance(
        ...
        returns (
            ...
            LensRatioUtils.PositionStats[] memory expectedPositions
        )
    {
        ...
        LensInMin.RebalanceMinParams memory params = LensInMin.RebalanceMinParams({
            manager: manager,
        ...
        (outMin, inMin) = LensInMin.getOutMinAndInMinForRebalance(params);
        ...
        LensInMin.RebalancePreview memory preview =
            LensInMin.simulateRebalance(manager, total0, total1, rebalanceParams);
    ...
    function _calculateSwapParamsForRebalance(
        ...
        uint256 total0,
        uint256 total1
    ) private view returns (LensRatioUtils.SwapParams memory swapParams) {
        swapParams = LensRatioUtils.calculateOptimalSwapForRebalance(
            manager,
    ...
    function _simulateRebalanceWithSwap(
        ...
        uint256 total1,
        LensRatioUtils.SwapParams memory swapParams
    ) private view returns (LensRatioUtils.PositionStats[] memory expectedPositions) {
        IPoolManager poolManager = manager.poolManager();
        PoolKey memory poolKey = manager.poolKey();
        (uint160 sqrtPriceX96,,,) = poolManager.getSlot0(poolKey.toId());

        (uint256 finalAmount0, uint256 finalAmount1) = LensRatioUtils.applySwap(
        ...
        LensInMin.RebalancePreview memory preview =
            LensInMin.simulateRebalance(manager, finalAmount0, finalAmount1, swapRebalanceParams);
        expectedPositions = preview.expectedPositions;
    }
     ...
     */
    function getAmountsForExactRatioDeposit(MultiPositionManager manager, bool isToken0, uint256 amount)
        external
        view
        returns (uint256 otherAmount)
    {
        return LensRatioUtils.getAmountsForDeposit(manager, isToken0, amount);
    }
    ...
    function validateRebalanceParams(
        ...
        MultiPositionFactory factory = MultiPositionFactory(manager.unilaunchFactory());
     ...
     */
    function getSwapLockStatus(MultiPositionManager manager)
        external
        view
