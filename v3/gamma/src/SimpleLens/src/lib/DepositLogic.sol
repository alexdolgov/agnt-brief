...
library DepositLogic {
    ...
    /**
     * @notice Process DIRECT_DEPOSIT action in callback
     * @dev This is called from unlockCallback when action is DIRECT_DEPOSIT
     * @param s Storage struct
     * @param poolManager Pool manager contract
     * @param params Encoded parameters (amount0, amount1, inMin)
     * @return Empty bytes (no return value needed)
     */
    function processDirectDepositInCallback(
        SharedStructs.ManagerStorage storage s,
        IPoolManager poolManager,
        bytes memory params
    ) external returns (bytes memory) {
        // Decode parameters
        (uint256 amount0, uint256 amount1, uint256[2][] memory inMin) =
    ...
    function calculateDirectDepositAmounts(
        ...
        uint256 amount0ToDistribute;
        uint256 amount1ToDistribute;

        if (totalToken0InPositions == 0) {
            // Only token1 in positions
            amount0ToDistribute = 0;
            amount1ToDistribute = params.amount1;
        } else if (totalToken1InPositions == 0) {
            // Only token0 in positions
            amount0ToDistribute = params.amount0;
            amount1ToDistribute = 0;
        } else {
            // Both tokens in positions - use cross product to maintain positions' ratio
            uint256 cross = Math.min(
                params.amount0 * totalToken1InPositions,
                params.amount1 * totalToken0InPositions
            );
            if (cross > 0) {
                amount0ToDistribute = (cross - 1) / totalToken1InPositions + 1;
                amount1ToDistribute = (cross - 1) / totalToken0InPositions + 1;
                amount0ToDistribute = Math.min(amount0ToDistribute, params.amount0);
                amount1ToDistribute = Math.min(amount1ToDistribute, params.amount1);
            }
        }
    ...
    /**
     * @notice Process compound with swap
     * @param s Storage struct
     * @param poolManager Pool manager contract
     * @param params CompoundSwapParams with aggregator and slippage details
     * @param msgValue ETH sent with transaction
     */
    function processCompoundSwap(
        SharedStructs.ManagerStorage storage s,
        IPoolManager poolManager,
        IMultiPositionManager.CompoundSwapParams memory params,
        uint256 msgValue
    ) external {
        if (s.basePositionsLength == 0) return;
        ...
        // Step 2: Get idle balances
        uint256 idle0Before = s.currency0.balanceOfSelf();
        uint256 idle1Before = s.currency1.balanceOfSelf();

        if (idle0Before == 0 && idle1Before == 0) return;

        // Step 3: Execute swap via aggregator
        if (params.aggregator != address(0) && params.swapData.length > 0) {
            (bool success,) = params.aggregator.call{value: msgValue}(params.swapData);
            if (!success) revert SwapFailed();

            // Verify minimum output
            uint256 idle0After = s.currency0.balanceOfSelf();
            uint256 idle1After = s.currency1.balanceOfSelf();

            // Check that we received at least minAmountOut of the output token
            uint256 outputReceived = idle0After > idle0Before
                ? idle0After - idle0Before
                : idle1After - idle1Before;
            if (outputReceived < params.minAmountOut) revert InsufficientSwapOutput();
        }

        // Step 4: Add all idle balance to positions
        uint256 finalIdle0 = s.currency0.balanceOfSelf();
        uint256 finalIdle1 = s.currency1.balanceOfSelf();

        if (finalIdle0 > 0 || finalIdle1 > 0) {
            // Calculate distribution amounts and add liquidity
            (uint256[] memory amounts0, uint256[] memory amounts1) = calculateDirectDepositAmounts(
                s,
                poolManager,
            ...
            addLiquidityToPositions(s, poolManager, amounts0, amounts1, params.inMin);
        }
    }
}