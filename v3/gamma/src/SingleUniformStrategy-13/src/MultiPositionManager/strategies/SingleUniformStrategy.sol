...
contract SingleUniformStrategy is ILiquidityStrategy {
        ...
        returns (int24[] memory lowerTicks, int24[] memory upperTicks)
    {
        ...
        if (useCarpet) {
            return _generateRangesWithCarpet(leftBound, rightBound, tickSpacing);
        } else {
            return _generateStandardRanges(leftBound, rightBound);
        }
    }
    ...
    function _generateRangesWithCarpet(int24 leftBound, int24 rightBound, int24 tickSpacing)
        private
        pure
        returns (int24[] memory lowerTicks, int24[] memory upperTicks)
    {
    ...
    function _processWeightsWithCarpetPhase1(uint256[] memory weights, DensityParams memory params) private pure {
        ...
        (uint256[] memory adjustedWeights, uint256 totalAdjustedWeight) =
            _calculateUniformWeights(params, endIdx, leftBound, rightBound);
