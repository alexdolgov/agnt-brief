...
contract SingleUniformStrategy is ILiquidityStrategy {
        ...
        returns (int24[] memory lowerTicks, int24[] memory upperTicks)
    {
        ...
        centerTick = _roundDownTick(centerTick, tickSpacing);
        ...
        if (tempLeft < int256(minUsable)) {
            leftBound = minUsable;
        } else {
            leftBound = _roundDownTick(int24(tempLeft), tickSpacing);
        }

        int256 tempRight = int256(centerTick) + int256(uint256(ticksRight));
        if (tempRight > int256(maxUsable)) {
            rightBound = maxUsable;
        } else {
            rightBound = _roundUpTick(int24(tempRight), tickSpacing);
        }
    ...
    function _generateStandardRanges(int24 leftBound, int24 rightBound)
        ...
        returns (int24[] memory lowerTicks, int24[] memory upperTicks)
    {
        // SingleUniform always creates one position covering the snapped range.
