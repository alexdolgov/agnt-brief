...
import {ILiquidityStrategy} from "../MultiPositionManager/strategies/ILiquidityStrategy.sol";
...
contract SuperchainLBPStrategy is LBPStrategyBasic, ISuperchainLBPStrategy {
    ...
    /// @notice The pool type (REGULAR, DYNAMIC, or VOLATILITY)
    ...
    constructor(
        ...
        if (uint8(_poolType) > uint8(PoolType.VOLATILITY)) revert InvalidPoolType();
    ...
    function _deployPoolAndMPM(MigrationData memory data) private returns (address mpm) {
        ...
        // Calculate inMin array length dynamically from strategy using actual tick from price
        uint256[2][] memory inMin = _calculateInMinArray(data.sqrtPriceX96);
        ...
        mpm = LBPDeploymentLib.callFactoryFunction(
            orderBookFactory, poolType, data, currency0, currency1, deposit0Desired, deposit1Desired, ethValue, inMin, ctx
        );
    }
        ...
        private view returns (uint128 tokenAmount, uint128 currencyAmount)
    {
        ...
        // Include leftover currency if one-sided params exist
        currencyAmount = data.leftoverCurrency > 0 && data.hasOneSidedParams
    ...
    /// @notice Calculates inMin array dynamically from strategy
    /// @param sqrtPriceX96 The sqrt price to calculate the current tick from
    /// @return inMin Array of minimum amounts for slippage protection (all [0,0] = disabled)
    /// @dev inMin only covers BASE positions - limit positions are always minted with no slippage protection
    function _calculateInMinArray(uint160 sqrtPriceX96) private view returns (uint256[2][] memory inMin) {
        // Convert sqrtPriceX96 to tick using TickMath library
        int24 currentTick = TickMath.getTickAtSqrtPrice(sqrtPriceX96);

        // Resolve center tick with proper floor-division alignment
        // This matches InitialDepositLens logic for SENTINEL_CENTER_TICK
        int24 resolvedCenterTick;
        if (rebalanceParams.center == type(int24).max) {
            ...
            int24 compressed = rebalanceParams.center / poolTickSpacing;
            if (rebalanceParams.center < 0 && rebalanceParams.center % poolTickSpacing != 0) compressed--;
            resolvedCenterTick = compressed * poolTickSpacing;
        }

        // Get base range count from strategy using resolved center tick
        (int24[] memory lowerTicks,) = ILiquidityStrategy(rebalanceParams.strategy).generateRanges(
            resolvedCenterTick,
        ...
        // Create inMin array ONLY for base positions (limit positions don't use slippage protection)
        // All elements [0,0] for no slippage protection
        inMin = new uint256[2][](baseRangeCount);
    ...
    function _approveTokens(Currency currency0, Currency currency1, uint256 amount0, uint256 amount1) private {
        if (!currency0.isAddressZero()) {
            IERC20(Currency.unwrap(currency0)).approve(address(orderBookFactory), amount0);
        }
        if (!currency1.isAddressZero()) {
            IERC20(Currency.unwrap(currency1)).approve(address(orderBookFactory), amount1);
        }
    }
