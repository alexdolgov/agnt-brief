...
contract SuperchainLBPStrategy is LBPStrategyBasic, ISuperchainLBPStrategy {
    ...
    function sweepToken() external override(ILBPStrategyBase, LBPStrategyBasic) {
    ...
    /// @inheritdoc ILBPStrategyBase
    function sweepCurrency() external override(ILBPStrategyBase, LBPStrategyBasic) {
        if (_getBlockNumberish() < sweepBlock) revert SweepNotAllowed(sweepBlock, _getBlockNumberish());
    ...
    function _transferAssetsAndExecutePlan(MigrationData memory data, bytes memory) internal override {
        ...
        uint160 strikePriceX96 = data.sqrtPriceX96;
        if (_currencyIsCurrency0()) {
            // invert sqrt price so strike is in terms of currency per token
            strikePriceX96 = uint160(FullMath.mulDiv(uint256(1) << 192, 1, strikePriceX96));
        }
        callOptionFactory.setStrike(strikePriceX96);
