...
contract OrderBookFactory is AccessControl {
    ...
    function createVolatilityDynamicFeePoolWithManager(
        ...
        // Scoped block to limit stack depth
        {
            ...
            if (hookAddr == address(0)) {
                ...
                (bytes32 minedSalt,) = computeSaltForVolatilityDynamicFee(
                    owner,
                    address(this)
                );
    ...
    /// @notice Deploy a volatility limit order hook for a creator using a pre-computed salt
    ...
    function deployVolatilityLimitOrderHookWithSalt(address hookOwner, bytes32 salt)
        external
        returns (address hookAddr)
    {
        ...
        hookAddr = creatorVolatilityDynamicLimitOrderHooks[hookOwner];
        if (hookAddr != address(0)) {
            return hookAddr;
        }

        // Deploy the hook with provided salt
        hookAddr = _deployVolatilityDynamicFeeLimitOrderHook(salt, hookOwner);

        // Verify deployed address has correct hook flags
        // VolatilityDynamicFeeLimitOrderHook requires: BEFORE_SWAP, AFTER_SWAP, BEFORE_INITIALIZE
        ...
        creatorVolatilityDynamicLimitOrderHooks[hookOwner] = hookAddr;
    ...
    /// @notice Deploy a volatility dynamic fee hook (fee-only, no limit orders) using a pre-computed salt
    /// @dev Salt must be computed off-chain using computeSaltForVolatilityDynamicFee().
    ///      This avoids expensive on-chain HookMiner.find() during pool creation.
    /// @param hookOwner The owner of the hook
    /// @param salt The pre-computed salt from HookMiner
    /// @return hookAddr The deployed hook address
    function deployVolatilityDynamicFeeHookWithSalt(address hookOwner, bytes32 salt)
        external
        returns (address hookAddr)
    {
        if (hookOwner == address(0)) revert ZeroAddress();
        ...
        // If hook already exists, return it (idempotent)
        hookAddr = creatorVolatilityDynamicFeeHooks[hookOwner];
        if (hookAddr != address(0)) {
            return hookAddr;
        }
        ...
        // Verify deployed address has correct hook flags
        // VolatilityDynamicFeeHook requires: BEFORE_SWAP, AFTER_SWAP (no BEFORE_INITIALIZE for fee-only)
        uint160 hookFlags = uint160(hookAddr) & Hooks.ALL_HOOK_MASK;
        ...
        // Store for future lookups
        creatorVolatilityDynamicFeeHooks[hookOwner] = hookAddr;
    ...
    function computePoolIdForVolatilityLimitOrder(
        ...
        if (hookAddr == address(0)) {
            ...
            (, hookAddr) = computeSaltForVolatilityDynamicLimitOrder(
                hookOwner,
                address(limitOrderManager),
                address(this)
            );
        ...
        returns (address hookAddr, bool wasReserved)
    {
        ...
        bytes32 poolId = computePoolIdForVolatilityLimitOrder(owner, params.currency0, params.currency1, params.tickSpacing);
        ...
        if (hookAddr == address(0)) {
            // Compute the proper salt using HookMiner to ensure valid hook flags
            (bytes32 minedSalt,) = computeSaltForVolatilityDynamicLimitOrder(
                owner,
                address(limitOrderManager),
                address(this)
            );
            hookAddr = _deployVolatilityDynamicFeeLimitOrderHook(minedSalt, owner);
