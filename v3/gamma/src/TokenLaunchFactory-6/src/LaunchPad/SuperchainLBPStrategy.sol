...
contract SuperchainLBPStrategy is LBPStrategyBasic, ISuperchainLBPStrategy {
    ...
    /// @notice Caller-bound hook salt for hook deployment (base hookSalt mixed with callerSalt)
    ...
    function _deployPoolAndMPM(MigrationData memory data) private returns (PoolKey memory poolKey, address mpm) {
        ...
        LBPDeploymentLib.DeploymentContext memory ctx = LBPDeploymentLib.DeploymentContext({
            ...
            hookSalt: hookSalt,
            callerSalt: callerSalt
        });

        (poolKey, mpm) = LBPDeploymentLib.callFactoryFunction(
