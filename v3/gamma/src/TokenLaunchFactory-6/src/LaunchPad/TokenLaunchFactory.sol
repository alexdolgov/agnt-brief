...
import {IERC20Metadata} from "@openzeppelin-latest/contracts/token/ERC20/extensions/IERC20Metadata.sol";
...
contract TokenLaunchFactory is AccessControl {
    ...
    function launchPresaleWithExistingToken(
        ...
        // Validate token decimals (CCA requires >= 6 to avoid rounding errors)
        if (IERC20Metadata(token).decimals() < 6) revert TokenDecimalsTooLow();

        // Validate auction supply doesn't exceed CCA max (1e30)
        if ((uint256(launchAmount) * migratorParams.tokenSplitToAuction) / 1e7 > 1e30) revert AuctionSupplyTooLarge();

        // Collect creation fee
        _collectCreationFee();

        // Build params and launch presale (useTransferFrom = true since caller provides tokens)
        ...
        (strategy, auction) = _launchPresale(token, params, true);
        ...
        returns (address token, address strategy, address auction)
    {
        ...
        // Validate token decimals (CCA requires >= 6 to avoid rounding errors)
        if (params.decimals < 6) revert TokenDecimalsTooLow();

        // Validate auction supply doesn't exceed CCA max (1e30)
        if ((uint256(params.launchAmount) * params.migratorParams.tokenSplitToAuction) / 1e7 > 1e30) {
            revert AuctionSupplyTooLarge();
        }
        ...
        // Launch presale (useTransferFrom = false since this contract holds the tokens)
        LaunchParams memory launchParams = LaunchParams({
            launchAmount: params.launchAmount,
            migratorParams: params.migratorParams,
            auctionParams: params.auctionParams,
            rebalanceParams: params.rebalanceParams,
            mpmName: params.mpmName,
            salt: params.salt,
    ...
    function _launchPresale(
        ...
        bytes memory configData = abi.encode(
            params.migratorParams, params.rebalanceParams, params.mpmName, params.auctionParams, params.hookSalt
        );
        Distribution memory distribution =
            Distribution({strategy: address(strategyFactory), amount: params.launchAmount, configData: configData});

        // Bind launch salt to the initiating user so copied salts cannot collide across users.
        bytes32 userBoundSalt = _getUserBoundLaunchSalt(msg.sender, params.salt);

        // Deploy strategy via LiquidityLauncher
        IDistributionContract distributionContract =
            liquidityLauncher.distributeToken(token, distribution, false, userBoundSalt);
    ...
    /// @notice Derives a launch salt bound to the initiating user
    function _getUserBoundLaunchSalt(address caller, bytes32 salt) internal pure returns (bytes32) {
        return keccak256(abi.encode(caller, salt));
    }

    /// @notice Derives the callerSalt seen by SuperchainLBPStrategyFactory via LiquidityLauncher
    function _getEffectiveCallerSalt(address caller, bytes32 salt) internal view returns (bytes32) {
        bytes32 userBoundSalt = _getUserBoundLaunchSalt(caller, salt);
        return keccak256(abi.encode(address(this), userBoundSalt));
    }
    ...
    /// @notice Computes a caller-bound hook salt seed for a planned launch
    /// @dev Call this off-chain with the same caller and salt that will be used for launch.
    function computeHookSalt(
        address hookOwner,
        address caller,
        bytes32 salt
    ) external view returns (bytes32 hookSalt, address hookAddress) {
        return _computeHookSalt(hookOwner, caller, salt);
    }

    /// @notice Legacy helper: computes hook salt for msg.sender with zero launch salt
    function computeHookSalt(address hookOwner) external view returns (bytes32 hookSalt, address hookAddress) {
        return _computeHookSalt(hookOwner, msg.sender, bytes32(0));
    }

    function _computeHookSalt(
        address hookOwner,
        address caller,
        bytes32 salt
    ) internal view returns (bytes32 hookSalt, address hookAddress) {
        OrderBookFactory orderBookFactory = strategyFactory.orderBookFactory();
        bytes32 effectiveCallerSalt = _getEffectiveCallerSalt(caller, salt);
        (hookSalt,, hookAddress) = orderBookFactory.computeCallerBoundSaltForVolatilityDynamicLimitOrder(
            hookOwner,
            effectiveCallerSalt,
