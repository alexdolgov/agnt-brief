...
contract CallOptionFactory is AccessControl {
    ...
    event OptionCreatedV2(
        address indexed option, address indexed beneficiary, uint256 amount, uint256 vestingCliff, uint256 vestingEnd
    );
    ...
    {
        uint256 vestingCliff = block.timestamp + MIN_CLIFF;
        return _createOption(beneficiary, amount, vestingCliff, vestingEnd);
    }

    function createOption(address beneficiary, uint256 amount, uint256 vestingCliff, uint256 vestingEnd)
        external
        onlyRole(SCHEDULER_ROLE)
        returns (address option)
    {
    ...
    function _createOption(address beneficiary, uint256 amount, uint256 vestingCliff, uint256 vestingEnd)
        internal
        returns (address option)
    {
        ...
        option = address(
            new CallOption(token, beneficiary, amount, buybackAddress, treasury, address(this), vestingCliff, vestingEnd)
        );
        ...
        emit OptionCreatedV2(option, beneficiary, amount, vestingCliff, vestingEnd);
    }
    ...
    function createOptions(
        ...
        for (uint256 i = 0; i < count; i++) {
            ...
            address option = _createOption(beneficiary, amount, vestingCliff, vestingEnd);
            options[i] = option;
        }
    }
    ...
    function createOptions(
        ...
        uint256 count = beneficiaries.length;
        if (count == 0) revert InvalidAmount();
        if (amounts.length != count || vestingCliffs.length != count || vestingEnds.length != count) revert InvalidAmount();

        options = new address[](count);
        for (uint256 i = 0; i < count; i++) {
            options[i] = _createOption(beneficiaries[i], amounts[i], vestingCliffs[i], vestingEnds[i]);
        }
    }
