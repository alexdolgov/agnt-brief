...
contract TokenLaunchFactoryV2 is AccessControl {
    ...
    uint64 public blocksPerDay = 86_400;
    uint64 private constant MAX_DURATION_DAYS = 7;
    uint64 private constant MEDIUM_DURATION_DAYS = 2;
    uint64 private constant LONG_DURATION_DAYS = 4;
    ...
    uint128 public requiredCurrencyRaised = 0.001 ether;
    address public defaultRebalanceStrategy;

    struct AuctionStepCalc {
        uint256 earlyMps;
        uint256 middleMps;
        uint256 lateMps;
        uint40 phase1Blocks;
        uint40 phase2Blocks;
        uint40 phase3Blocks;
        uint24 phase1MpsPerBlock;
        uint24 phase2MpsPerBlock;
        uint24 phase3BaseMps;
        uint40 phase3Remainder;
    }
    ...
    {
        uint64 minDurationBlocks = blocksPerDay;
        uint64 maxDurationBlocks = blocksPerDay * MAX_DURATION_DAYS;
    ...
    function _buildAuctionSteps(uint64 durationBlocks) internal view returns (bytes memory steps) {
        AuctionStepCalc memory calc = _computeAuctionStepCalc(durationBlocks);

        if (calc.phase3Remainder == 0) {
            steps = abi.encodePacked(
                calc.phase1MpsPerBlock, calc.phase1Blocks,
                calc.phase2MpsPerBlock, calc.phase2Blocks,
                calc.phase3BaseMps, calc.phase3Blocks
            );
        } else {
            uint40 phase3aBlocks = calc.phase3Remainder;
            uint40 phase3bBlocks = calc.phase3Blocks - calc.phase3Remainder;

            // Split the last phase so total mps matches MPS_TOTAL exactly.
            steps = abi.encodePacked(
                calc.phase1MpsPerBlock, calc.phase1Blocks,
                calc.phase2MpsPerBlock, calc.phase2Blocks,
                uint24(calc.phase3BaseMps + 1), phase3aBlocks,
                calc.phase3BaseMps, phase3bBlocks
            );
        }
    }

    function _computeAuctionStepCalc(uint64 durationBlocks) internal view returns (AuctionStepCalc memory calc) {
        uint64 mediumDurationThreshold = blocksPerDay * MEDIUM_DURATION_DAYS;
        uint64 longDurationThreshold = blocksPerDay * LONG_DURATION_DAYS;

        if (durationBlocks < mediumDurationThreshold) {
            calc.earlyMps = (MPS_TOTAL * 33) / 100;
            calc.middleMps = (MPS_TOTAL * 34) / 100;
        } else if (durationBlocks < longDurationThreshold) {
            calc.earlyMps = (MPS_TOTAL * 20) / 100;
            calc.middleMps = (MPS_TOTAL * 55) / 100;
        } else {
            calc.earlyMps = (MPS_TOTAL * 10) / 100;
            calc.middleMps = (MPS_TOTAL * 50) / 100;
        }

        calc.lateMps = MPS_TOTAL - calc.earlyMps - calc.middleMps;

        calc.phase1Blocks = uint40(durationBlocks / 3);
        calc.phase2Blocks = uint40(durationBlocks / 3);
        calc.phase3Blocks = uint40(durationBlocks - calc.phase1Blocks - calc.phase2Blocks);

        calc.phase1MpsPerBlock = uint24(calc.earlyMps / calc.phase1Blocks);
        calc.phase2MpsPerBlock = uint24(calc.middleMps / calc.phase2Blocks);

        uint256 phase1Distributed = uint256(calc.phase1MpsPerBlock) * calc.phase1Blocks;
        uint256 phase2Distributed = uint256(calc.phase2MpsPerBlock) * calc.phase2Blocks;
        uint256 phase3TotalMps = calc.lateMps;
        phase3TotalMps += calc.earlyMps - phase1Distributed;
        phase3TotalMps += calc.middleMps - phase2Distributed;

        calc.phase3BaseMps = uint24(phase3TotalMps / calc.phase3Blocks);
        calc.phase3Remainder = uint40(phase3TotalMps % calc.phase3Blocks);
    }
    ...
    /// @dev Uses address(this) as creator since TokenLaunchFactoryV2 is the deploying contract
    ...
    function setBlocksPerDay(uint64 newBlocksPerDay) external onlyRole(DEFAULT_ADMIN_ROLE) {
        if (newBlocksPerDay == 0) revert InvalidBlocksPerDay();
        if (newBlocksPerDay > type(uint64).max / MAX_DURATION_DAYS) revert InvalidBlocksPerDay();
