...
/// @notice Parameters for deploying a token and launching presale (V3)
/// @dev `currency` is the auction currency (address(0) for native ETH).
///      `requiredCurrencyRaised` is denominated in the chosen currency's smallest units.
///      Set `requiredCurrencyRaised = 0` to use the factory default for that currency.
struct DeployAndLaunchParamsV3 {
    string name;
    string symbol;
    UERC20Metadata metadata;
    bytes32 salt;
    bytes32 hookSalt; // Pre-computed salt for early hook deployment
    uint64 durationBlocks; // Auction duration in blocks
    address currency; // Auction currency (address(0) for native)
    uint128 requiredCurrencyRaised; // 0 = use default
}

struct AuctionCurrencyParams {
    address currency;
...
contract TokenLaunchFactoryV3 is AccessControl {
    ...
    // ============ Auction Currency Config ============

    /// @notice Whitelist for allowed auction currencies (address(0) = native)
    mapping(address => bool) public isAuctionCurrencyWhitelisted;

    /// @notice Optional per-currency default required raise (0 = fallback to `requiredCurrencyRaised`)
    mapping(address => uint128) public defaultRequiredCurrencyRaisedByCurrency;

    /// @notice Optional per-currency tick spacing override (0 = fallback to `auctionTickSpacing`)
    mapping(address => uint256) public auctionTickSpacingByCurrency;

    /// @notice Optional per-currency floor price override (0 = fallback to `auctionFloorPrice`)
    mapping(address => uint256) public auctionFloorPriceByCurrency;
    ...
    /// @notice Error thrown when the selected auction currency is not whitelisted
    ...
    event AuctionCurrencyConfigUpdated(
        address indexed currency,
        bool whitelisted,
        uint128 defaultRequiredCurrencyRaised,
        uint256 tickSpacing,
        uint256 floorPrice
    );
    ...
    constructor(
        ...
        // Default to allowing native currency auctions (address(0)).
        isAuctionCurrencyWhitelisted[address(0)] = true;
    }
    ...
    /// @param params The DeployAndLaunchParamsV3 struct containing all deployment parameters
    ...
    function deployTokenAndLaunchPresale(DeployAndLaunchParamsV3 memory params)
        external
        payable
        returns (address token, address strategy, address auction)
    {
        AuctionCurrencyParams memory auctionCurrencyParams =
            _resolveAuctionCurrencyParams(params.currency, params.requiredCurrencyRaised);
    ...
    function _deployTokenAndAux(DeployAndLaunchParamsV3 memory params, LaunchAmounts memory amounts)
        internal
    ...
    function _finalizeNewLaunch(
        ...
        address auction,
        AuxContracts memory aux,
        DeployAndLaunchParamsV3 memory params,
        LaunchAmounts memory amounts,
        LaunchConfig memory config
    ) internal {
    ...
    function _resolveAuctionCurrencyParams(address currency, uint128 requestedRequiredCurrencyRaised)
        internal
        view
        returns (AuctionCurrencyParams memory params)
    {
        if (!isAuctionCurrencyWhitelisted[currency]) revert CurrencyNotWhitelisted(currency);

        uint256 resolvedTickSpacing = auctionTickSpacingByCurrency[currency];
        if (resolvedTickSpacing == 0) resolvedTickSpacing = auctionTickSpacing;

        uint256 resolvedFloorPrice = auctionFloorPriceByCurrency[currency];
        if (resolvedFloorPrice == 0) resolvedFloorPrice = auctionFloorPrice;

        uint128 resolvedRequiredCurrencyRaised = requestedRequiredCurrencyRaised;
        if (resolvedRequiredCurrencyRaised == 0) {
            resolvedRequiredCurrencyRaised = defaultRequiredCurrencyRaisedByCurrency[currency];
            if (resolvedRequiredCurrencyRaised == 0) resolvedRequiredCurrencyRaised = requiredCurrencyRaised;
        }

        if (resolvedRequiredCurrencyRaised == 0) revert InvalidAmount();
        if (resolvedTickSpacing == 0 || resolvedFloorPrice == 0) revert InvalidAmount();
        // CCA TickStorage requires `floorPrice % tickSpacing == 0`.
        if (resolvedFloorPrice % resolvedTickSpacing != 0) revert InvalidAmount();

        params.currency = currency;
        params.requiredCurrencyRaised = resolvedRequiredCurrencyRaised;
        params.tickSpacing = resolvedTickSpacing;
        params.floorPrice = resolvedFloorPrice;
    }
    ...
    {
        uint64 bph = blocksPerHour;
        if (durationBlocks < bph || durationBlocks > bph * MAX_DURATION_HOURS) revert InvalidDuration();
        uint64 startBlock = uint64(block.number);
        uint64 endBlock = startBlock + durationBlocks;
        address currency = auctionCurrencyParams.currency;

        MigratorParameters memory migratorParams;
        migratorParams.migrationBlock = endBlock + migrationDelayBlocks;
        migratorParams.currency = currency;
        migratorParams.poolLPFee = poolLpFee;
        migratorParams.poolTickSpacing = poolTickSpacing;
        migratorParams.tokenSplit = tokenSplitToAuction;
        migratorParams.initializerFactory = address(0);
        migratorParams.positionRecipient = creator;
        migratorParams.sweepBlock = endBlock + sweepDelayBlocks;
        migratorParams.operator = creator;
        migratorParams.maxCurrencyAmountForLP = type(uint128).max;
        ...
        address tokensRecipient = _computeMpmAddress(token, currency, creator, mpmName);
        bytes memory auctionParams = _buildAuctionParams(
            tokensRecipient,
            startBlock,
            endBlock,
            auctionCurrencyParams
        ...
        rebalanceParams.strategy = defaultRebalanceStrategy;
        rebalanceParams.center = 0;
        rebalanceParams.tLeft = 2_000_000;
        rebalanceParams.tRight = 2_000_000;
        rebalanceParams.limitWidth = 2_000_000;
        rebalanceParams.weight0 = 0.5e18;
        rebalanceParams.weight1 = 0.5e18;
        rebalanceParams.useCarpet = true;

        config.migratorParams = migratorParams;
        config.auctionParams = auctionParams;
        config.rebalanceParams = rebalanceParams;
        config.mpmName = mpmName;
    }
        ...
        returns (bytes memory)
    {
        ...
        AuctionParameters memory params = AuctionParameters({
            currency: auctionCurrencyParams.currency,
            tokensRecipient: tokensRecipient,
            fundsRecipient: ActionConstants.MSG_SENDER,
            startBlock: startBlock,
            endBlock: endBlock,
            claimBlock: endBlock + claimDelayBlocks,
            tickSpacing: auctionCurrencyParams.tickSpacing,
            validationHook: address(0),
            floorPrice: auctionCurrencyParams.floorPrice,
            requiredCurrencyRaised: auctionCurrencyParams.requiredCurrencyRaised,
    ...
    function _computeMpmAddress(address token, address currency, address managerOwner, string memory mpmName)
        internal
        view
        returns (address)
    {
        OrderBookFactory orderBookFactory = OrderBookFactory(address(strategyFactory.orderBookFactory()));
        PoolKey memory poolKey = _buildPoolKey(token, currency, orderBookFactory);
        return orderBookFactory.multiPositionFactory().computeAddress(poolKey, managerOwner, mpmName);
    }

    function _buildPoolKey(address token, address currency, OrderBookFactory orderBookFactory)
        internal
        view
        returns (PoolKey memory poolKey)
    {
        if (token == currency) revert InvalidAmount();
        address currency0 = currency < token ? currency : token;
        address currency1 = currency < token ? token : currency;
        poolKey = PoolKey({
    ...
    function _registerLaunch(LaunchRegistration memory params) internal {
        ...
        // Populate predicted pool key + MPM address
        {
            ...
            PoolKey memory poolKey = _buildPoolKey(params.token, params.migratorParams.currency, orderBookFactory);
    ...
    /// @notice Configure an auction currency (whitelist + optional per-currency defaults)
    /// @dev `tickSpacing`/`floorPrice`/`defaultRequiredCurrencyRaised` are optional overrides.
    ///      Set any of them to 0 to inherit the factory-level defaults.
    function setAuctionCurrencyConfig(
        address currency,
        bool whitelisted,
        uint128 defaultRequiredCurrencyRaised,
        uint256 tickSpacing,
        ...
        if (whitelisted && currency != address(0) && currency.code.length == 0) revert InvalidAmount();
        if (tickSpacing != 0 && floorPrice != 0 && floorPrice % tickSpacing != 0) revert InvalidAmount();

        // If only one override is provided, validate the effective (inherited) pair.
        if (whitelisted) {
            uint256 effectiveTickSpacing = tickSpacing != 0 ? tickSpacing : auctionTickSpacing;
            uint256 effectiveFloorPrice = floorPrice != 0 ? floorPrice : auctionFloorPrice;
            if (effectiveTickSpacing == 0 || effectiveFloorPrice == 0) revert InvalidAmount();
            if (effectiveFloorPrice % effectiveTickSpacing != 0) revert InvalidAmount();
        }

        isAuctionCurrencyWhitelisted[currency] = whitelisted;
        defaultRequiredCurrencyRaisedByCurrency[currency] = defaultRequiredCurrencyRaised;
        auctionTickSpacingByCurrency[currency] = tickSpacing;
        auctionFloorPriceByCurrency[currency] = floorPrice;

        emit AuctionCurrencyConfigUpdated(currency, whitelisted, defaultRequiredCurrencyRaised, tickSpacing, floorPrice);
    }
