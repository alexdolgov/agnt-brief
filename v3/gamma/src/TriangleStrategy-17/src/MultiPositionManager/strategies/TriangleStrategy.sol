...
contract TriangleStrategy is ILiquidityStrategy {
    ...
    function _processWeightsNoCarpet(uint256[] memory weights, DensityParams memory params) private pure {
        ...
        // Using centerMidpoint ensures symmetric distance calculation for left and right ranges
        ...
        // Use centerMidpoint as the symmetric reference point for all calculations
        ...
        uint256 effectiveTicksRight;
        {
            // Measure left span from centerRangeLower (symmetric with right)
            ...
            // Measure right span from centerRangeUpper (symmetric with left)
        ...
        // Use symmetric edges based on centerMidpoint
        int24 leftEdge = centerMidpoint - int24(uint24(effectiveTicksLeft));
        int24 rightEdge = centerMidpoint + int24(uint24(effectiveTicksRight));
        ...
        returns (TriangleBounds memory bounds)
    {
        ...
        // Measure left span from centerRangeLower (symmetric with right)
        int256 leftSpan = int256(centerRangeLower) - int256(params.lowerTicks[startIdx]);
        uint256 actualLeftSpan = leftSpan > 0 ? uint256(leftSpan) : 0;
        ...
        // Use symmetric edges based on centerMidpoint
        bounds.leftEdge = centerMidpoint - int24(uint24(bounds.effectiveTicksLeft));
        bounds.rightEdge = centerMidpoint + int24(uint24(bounds.effectiveTicksRight));
    }
