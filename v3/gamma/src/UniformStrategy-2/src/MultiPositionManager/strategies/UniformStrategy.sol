 ...
 * @notice Uniform distribution with outward-snapped range generation
 * @dev Key features:
 *      1. Dynamic width targeting ~20 ranges
 *      2. Requested bounds snap outward from centerTick for positive/negative parity
 *      3. Equal weights across the generated base grid
 */
contract UniformStrategy is ILiquidityStrategy {
     ...
     * @dev Builds an outward-snapped grid directly from the requested bounds
     */
    ...
    function _generateRanges(int24 centerTick, uint24 ticksLeft, uint24 ticksRight, int24 tickSpacing, bool useCarpet)
        ...
        returns (int24[] memory lowerTicks, int24[] memory upperTicks)
    {
        StrategyGridLibrary.Grid memory grid =
            StrategyGridLibrary.buildGrid(centerTick, ticksLeft, ticksRight, tickSpacing);

        (int24[] memory baseLowers, int24[] memory baseUppers) =
            StrategyGridLibrary.generateEqualWidthRanges(grid.leftBound, grid.rightBound, grid.width);
