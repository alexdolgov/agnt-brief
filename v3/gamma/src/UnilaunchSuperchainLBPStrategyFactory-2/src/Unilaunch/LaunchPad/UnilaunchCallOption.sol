...
contract UnilaunchCallOption {
    ...
    function exercise(uint256 tokenAmount) external payable {
        if (msg.sender != team) revert Unauthorized();
        if (!IUnilaunchCallOptionFactory(factory).strikeSet()) revert StrikeNotSet();
        if (tokenAmount == 0) revert InvalidAmount();
        if (tokenAmount > getExercisableAmount()) revert InvalidAmount();

        uint160 strikePriceX96 = IUnilaunchCallOptionFactory(factory).strikePriceX96();
