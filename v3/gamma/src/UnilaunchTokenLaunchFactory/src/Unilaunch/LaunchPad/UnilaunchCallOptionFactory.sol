...
import {UnilaunchCallOption} from "./UnilaunchCallOption.sol";

/// @title UnilaunchCallOptionFactory
/// @notice Holds option allocation and deploys per-member call options sharing a global strike.
contract UnilaunchCallOptionFactory is AccessControl {
        ...
        returns (address option)
    {
        ...
        option = address(
            new UnilaunchCallOption(token, beneficiary, amount, buybackAddress, treasury, address(this), vestingEnd)
        );
