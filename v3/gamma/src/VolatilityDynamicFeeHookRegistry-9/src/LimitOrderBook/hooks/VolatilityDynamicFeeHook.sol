...
/// @notice Hook with fixed base fee + surge fee on CAP events
...
contract VolatilityDynamicFeeHook is BaseHook, Ownable {
    ...
    function getHookPermissions() public pure override returns (Hooks.Permissions memory) {
        return Hooks.Permissions({
            ...
            beforeDonate: false,
            afterDonate: false,
            beforeSwapReturnDelta: true,  // Enable to collect fees via BeforeSwapDelta
    ...
    function _afterInitialize(
        ...
        // Only initialize if this is a managed pool
        if (managedPools[poolId]) {
            // Oracle initialization happens in registerPool via enableOracleForPool
            // Nothing to do here - oracle already initialized
        }
    ...
    function _afterSwap(
        ...
        if (wasCapped && !surgeStates[poolId].isActive) {
