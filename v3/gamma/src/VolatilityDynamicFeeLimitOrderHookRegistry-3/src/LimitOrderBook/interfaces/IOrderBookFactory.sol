...
interface IOrderBookFactory {
    ...
    function deployVolatilityLimitOrderHookWithSalt(address hookOwner, bytes32 salt) external returns (address hookAddr);

    /// @notice Deploy a volatility dynamic fee hook (fee-only) using a pre-computed salt
    /// @dev Salt must be computed off-chain using computeSaltForVolatilityDynamicFee().
    /// @param hookOwner The owner of the hook
    /// @param salt The pre-computed salt from HookMiner
    /// @return hookAddr The deployed hook address
    function deployVolatilityDynamicFeeHookWithSalt(address hookOwner, bytes32 salt) external returns (address hookAddr);
}