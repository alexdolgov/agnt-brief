...
/// out when needed
///
/// Registered guardians call the submit function and trigger vault operations. The vault may run pre- and post- submit
/// hooks and revert if a guardian is using an unauthorized operation. Authorized operations are configured in an off
/// chain merkle tree and guardians need to provide a merkle proof for each operation. In addition to validating
/// operation targets (the contract and function being called), the merkle tree can maintain custom per-operation hooks
/// which extract specific parts of the calldata for validation or even perform (possibly stateful) validation during
/// the submit call
contract BaseVault is IBaseVault, Pausable, CallbackHandler, ReentrancyGuardTransient, Auth2Step, IERC721Receiver {
    ...
    /// @notice Address of the hooks contract for vault-level operations
    IVaultHooks public hooks;

    /// @notice Mapping of each guardian address to their merkle root (or merkle root provider)
    ...
    constructor() Pausable() Auth2Step(msg.sender, Authority(address(0))) {
        ...
        // Effects: set the initial guardian, its root, and vault-level submit hooks
        _setGuardianRoot(params.guardian, params.root);
        IVaultHooks hooks_ = params.hooks;
    ...
    function submit(bytes calldata data) external whenNotPaused nonReentrant {
        ...
        // Interactions: get the actual merkle root hash, possibly from IMerkleRootProvider
        root = _getRoot(root);

        address vaultHooks = address(hooks);
        // Requirements + Interactions: call the before submit hooks if defined
        _beforeSubmitHooks(vaultHooks, data);
        ...
        (Approval[] memory approvals, uint256 approvalsLength) = _executeSubmit(root, reader, end, false);
        ...
        _afterSubmitHooks(vaultHooks, data);
    }

    /// @inheritdoc IBaseVault
    function setGuardianRoot(address guardian, bytes32 root) external virtual requiresAuth {
        // Requirements + Effects: set the guardian root or provider
    ...
    /// @inheritdoc IBaseVault
    function setHooks(IVaultHooks newHooks) external virtual requiresAuth {
        // Requirements + Effects: set the vault hooks address
        _setHooks(newHooks);
    }
    ...
    function _handleCallback(bytes32 root, uint256 cursor) internal virtual override {
        ...
        (Approval[] memory approvals, uint256 approvalsLength) = _executeSubmit(root, reader, end, true);
    ...
    /// @return hasCallback Whether the guardian expects a callback for this operation
    /// @return newCursor Updated cursor position
    function _processExpectedCallback(CalldataReader reader, bytes32 root) internal returns (CalldataReader, bool) {
        bool hasCallback;
    ...
    /// @param vaultHooks Address of the vault hooks contract
    /// @dev Vault hooks passed as an argument to reduce storage loading
    function _beforeSubmitHooks(address vaultHooks, bytes calldata data) internal {
        if (_hasBeforeHooks(vaultHooks)) {
            // Interactions: call the before submit hooks
            (bool success, bytes memory result) =
                vaultHooks.call(abi.encodeWithSelector(IVaultHooks.beforeSubmit.selector, data, msg.sender));
    ...
    /// @param vaultHooks Address of the vault hooks contract
    /// @dev Vault hooks passed as an argument to reduce storage loading
    function _afterSubmitHooks(address vaultHooks, bytes calldata data) internal {
        if (_hasAfterHooks(vaultHooks)) {
            // Interactions: call the after submit hooks
            (bool success, bytes memory result) =
                vaultHooks.call(abi.encodeWithSelector(IVaultHooks.afterSubmit.selector, data, msg.sender));
        ...
        returns (bytes memory result)
    {
        if (_hasBeforeHooks(operationHooks)) {
            ...
            (bool success, bytes memory returnData) = operationHooks.call(data);
            // Requirements: check that the hooks call succeeded
            require(success, Aera__BeforeOperationHooksFailed(i, returnData));

            // Requirements: check that the return data length is a multiple of 32
            require(returnData.length % WORD_SIZE == 0, Aera__InvalidBeforeOperationHooksReturnDataLength());

            // Replace all instances of the vault address with a sentinel value
    ...
    function _executeSubmit(bytes32 root, CalldataReader reader, CalldataReader end, bool isCalledFromCallback)
        internal
        returns (Approval[] memory approvals, uint256 approvalsLength)
    {
        ...
        for (uint256 i = 0; i < operationsLength; ++i) {
            ...
            if (isStaticCall) {
                ...
                (reader, ctx.hasCallback) = _processExpectedCallback(reader, root);
                ...
                if (!isCalledFromCallback && ctx.hasCallback) {
    ...
    /// which doesn't allow using both a custom pre-hooks and a configurable pre-hooks
        ...
        returns (CalldataReader, bytes memory, uint256, address)
    {
        ...
        uint256 calldafaOffsetsCount = hooksConfigFlag & CONFIGURABLE_HOOKS_LENGTH_MASK;
        ...
        if (calldafaOffsetsCount != 0) {
            ...
            if (_hasBeforeHooks(operationHooks)) {
                revert Aera__BeforeOperationHooksWithConfigurableHooks();
            }
    ...
    function _setHooks(IVaultHooks hooksAddress) internal {
    ...
    /// @notice Replace the vault address with a sentinel value in the given memory fragment
    /// @param data The memory fragment to replace the vault address in
    /// @dev This allows merkle trees to be reused across different vaults
    ...
    /// @notice Check if the stored value is a merkle root (vs IMerkleRootProvider address )
    ...
    /// @dev Check if hooks needs to be called before submit
    /// @param hooks_ Hooks address to check
    /// @return True if hooks needs to be called before submit
    function _hasBeforeHooks(address hooks_) internal pure returns (bool) {
        ...
        return uint160(hooks_) & BEFORE_SUBMIT_MASK != 0;
    }

    /// @dev Check if hooks needs to be called after submit
    /// @param hooks_ Hooks address to check
    /// @return True if hooks needs to be called after submit
    function _hasAfterHooks(address hooks_) internal pure returns (bool) {
        ...
        return uint160(hooks_) & AFTER_SUBMIT_MASK != 0;
    }
}
