...
/// The fee calculator is responsible for calculating the assets under management and performance fees for the vault but
...
abstract contract FeeVault is IFeeVault, BaseVault {
    ...
    function setFees(uint16 aumFee, uint16 performanceFee) external requiresAuth {
        // Interactions: set the new vault fee rates
    ...
    function claimFees() external onlyFeeRecipient {
        ...
        (uint256 feeRecipientFees, uint256 protocolFees, address protocolFeeRecipient) = FEE_CALCULATOR.claimFees();
    ...
    function claimProtocolFees() external {
        ...
        (uint256 protocolFees, address protocolFeeRecipient) = FEE_CALCULATOR.claimProtocolFees();
