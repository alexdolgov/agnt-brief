...
abstract contract RewardsManager is BaseController, VaultManager {
    ...
    /**
     * @notice Thrown when the total share supply changes during a sell rewards operation
     */
    error Reward_ShareSupplyChanged();
    /**
     * @notice Thrown when the actual received amount does not match the expected amount after a swap
     */
    error Reward_IncorrectToAmount();
        ...
        returns (uint256 assets)
    {
        ...
        uint256 originalVaultBalance = IERC20(vaultAsset).balanceOf(vault);
        ...
        require(originalShareSupply == _share.totalSupply(), Reward_ShareSupplyChanged());
        require(IERC20(vaultAsset).balanceOf(vault) - originalVaultBalance == assets, Reward_IncorrectToAmount());
