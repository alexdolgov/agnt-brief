...
contract GIBXSwapFee is SafeOwnable, Initializable {
    ...
    function initialize(address addressA_, address addressB_, IGIBXRouter router_, IGIBXFactory factory_, address WETH_, address destroyToken_, address USDT_, address receiver_, address caller_) external initializer {
        require(address(addressA_) != address(0), "illegal vault address");
        require(address(addressB_) != address(0), "illegal vault address");
    ...
    /*
    function setAddressA(address newAddressA) external onlyOwner {
        require(newAddressA != address(0), "addressA is zero");
        addressA = newAddressA;
    }

    function setAddressB(address newAddressB) external onlyOwner {
        require(newAddressB != address(0), "addressB is zero");
        addressB = newAddressB;
    }
    */
    function transferToVault(address token, uint balance) internal returns (uint balanceRemained) {
        uint balanceUsed = balance.mul(2).div(3); //2/3
        balanceRemained = balance.sub(balanceUsed);
        uint balanceA = balanceUsed.mul(85).div(100); //85%
        uint balanceB = balanceUsed.sub(balanceA); //15%
        SafeERC20.safeTransfer(IERC20(token), addressA, balanceA);
        SafeERC20.safeTransfer(IERC20(token), addressB, balanceB);
    }
