...
contract Token is Ownable, ERC20Permit {
    ...
    constructor(string memory name_, string memory symbol_) Ownable() ERC20(name_,symbol_) ERC20Permit(name_) {
        team = new TokenVesting(msg.sender, block.timestamp + 180 days, 0 days, 730 days);
        _mint(address(team), 1_200_000 * 1e18);

        // Initial Liquidity
        _mint(msg.sender, 250_000 * 1e18);
        // Airdrop
        _mint(msg.sender, 300_000 * 1e18);
        // Protocol Growth
        _mint(msg.sender, 1_900_000 * 1e18);
        // Rewards + Bribes
        _mint(msg.sender, 6_000_000 * 1e18);
        // LGE Participants
        _mint(msg.sender, 350_000 * 1e18);
    }
}
