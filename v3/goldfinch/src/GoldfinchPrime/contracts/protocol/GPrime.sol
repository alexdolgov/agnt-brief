  ...
  ReentrancyGuardUpgradeable
{
  ...
  /// @notice Total returns recorded to date, used for off-chain share price calculation delta
  uint256 public totalReturnsRecordedToDate;
  ...
  function _usdcToGPRIME(uint256 amount) public pure returns (uint256) {
  ...
  */
   ...
   */
  function updateSharePrice(uint256 newSharePrice) public onlyRole(MANAGER_ROLE) {
  ...
  /**
   * @notice Set the total returns recorded to date
   * @param _newTotalReturns The new value for totalReturnsRecordedToDate
   */
  function setTotalReturnsRecordedToDate(uint256 _newTotalReturns) public onlyRole(MANAGER_ROLE) {
    // Consider if _newTotalReturns can be 0 or if it should always be > 0.
    // For now, allowing 0 as it might be a reset or initial state.
    totalReturnsRecordedToDate = _newTotalReturns;
  }

  /**
   * @notice Set both the share price and total returns recorded to date
   * @param _newSharePrice The new share price
   * @param _newTotalReturns The new value for totalReturnsRecordedToDate
   */
  function setSharePriceAndTotalReturns(
    uint256 _newSharePrice,
    uint256 _newTotalReturns
  ) external onlyRole(MANAGER_ROLE) {
    updateSharePrice(_newSharePrice);
    setTotalReturnsRecordedToDate(_newTotalReturns);
  }
   ...
   * @return Value in USDC (in atomic units)
   */
