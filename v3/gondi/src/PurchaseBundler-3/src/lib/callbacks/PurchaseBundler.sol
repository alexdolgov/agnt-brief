    ...
    Pausable
{
    ...
    function executeOperation(
        ...
        for (uint256 i; i < args.loanExecutionData.length; ++i) {
            _checkMultiSourceLoanSelector(args.loanExecutionData[i]);
        }
        ...
        _storeMsgSender
    {
        ...
        if (args.target == address(this)) {
            ...
            if (args.target == address(_multiSourceLoan)) {
                _checkMultiSourceLoanSelector(args.executionCalldata);
            }
    ...
    function _checkMultiSourceLoanSelector(bytes memory data) private pure {
        bytes4 sel;
        assembly {
            sel := mload(add(data, 32))
        }
        if (
            data.length <= 4 || sel != IMultiSourceLoan.emitLoan.selector
                && sel != IMultiSourceLoan.refinanceFull.selector && sel != IMultiSourceLoan.refinancePartial.selector
                && sel != IMultiSourceLoan.refinanceFromLoanExecutionData.selector
                && sel != IMultiSourceLoan.repayLoan.selector
        ) revert InvalidExecutionData();
    }
    ...
    function _buy(bytes[] calldata executionData) private returns (uint256[] memory) {
        ...
        for (uint256 i; i < total;) {
            _checkMultiSourceLoanSelector(executionData[i]);
