...
contract QuoteHelperPancakeV2 {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    function quoteValue(address router, uint256 amountsIn, address[] memory tradePath) public view returns (uint256){
    ...
    function quoteLPValue(address router, address pairAddress, uint256 lpAmount, address quoteToken, address[] memory token0Path, address[] memory token1Path) public view returns (uint256) {
        if (lpAmount == 0) return 0;

        (address token0Address, uint256 total0, address token1Address, uint256 total1) = getLPConstituents(pairAddress, lpAmount);
        ...
        if (token0Address != quoteToken) {
            value0 = quoteValue(router, total0, token0Path);
        }

        uint256 value1 = total1;
        if (token1Address != quoteToken) {
            value1 = quoteValue(router, total1, token1Path);
        }
