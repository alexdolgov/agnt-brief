...
contract StrategyPancakeLPV2 is Ownable, ReentrancyGuard, Pausable {
    ...
    function withdrawBUSD(address _userAddress, uint256 shares) public onlyOwner nonReentrant returns (uint256, uint256){
        ...
        (uint256 amount0, uint256 amount1) = RouterHelperV2.removeLiquidity(router, pairAddress, withdrawnAmount);

        if (amount0 > 0 && token0Address != busdAddress) {
            RouterHelperV2.swapTokens(router, amount0, token0Address, busdAddress, token0ToBusdPath);
        }

        if (amount1 > 0 && token1Address != busdAddress) {
            RouterHelperV2.swapTokens(router, amount1, token1Address, busdAddress, token1ToBusdPath);
        }
    ...
    function _addLiquidity(address inputToken, uint256 inputAmount, address[] memory to0Path, address[] memory to1Path) internal returns (uint256){
        ...
        if (inputToken != token0Address) {
            ...
            token0Amt = RouterHelperV2.swapTokens(router, halfAmount, inputToken, token0Address, to0Path);
        }

        uint256 token1Amt = halfAmount;
        if (inputToken != token1Address) {
            // Swap half earned to token1
            token1Amt = RouterHelperV2.swapTokens(router, halfAmount, inputToken, token1Address, to1Path);
        }

        // Get want tokens, ie. add liquidity
        if (token0Amt > 0 && token1Amt > 0) {
            (uint256 amount0, uint256 amount1, uint256 liquidity) =
            RouterHelperV2.addLiquidity(router, token0Address, token1Address, token0Amt, token1Amt);
            return liquidity;
        }
        return 0;
    }
    ...
    function convertDustToEarned() public whenNotPaused {
        ...
        if (token0Address != earnedAddress && token0Amt > 0) {
            RouterHelperV2.swapTokens(router, token0Amt, token0Address, earnedAddress, token0ToEarnedPath);
        }

        // Converts token1 dust (if any) to earned tokens
        uint256 token1Amt = IERC20(token1Address).balanceOf(address(this));
        if (token1Address != earnedAddress && token1Amt > 0) {
            RouterHelperV2.swapTokens(router, token1Amt, token1Address, earnedAddress, token1ToEarnedPath);
        }
    }
