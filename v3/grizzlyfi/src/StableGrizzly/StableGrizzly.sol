...
abstract contract StableGrizzlyStrategy is Initializable, StableBaseConfig {
    ...
    function grizzlyStrategyDeposit(uint256 amount, address to) internal {
        updateUserMask(to);
        participantData[to].amount += amount;
    ...
    function grizzlyStrategyWithdraw(uint256 amount, address to) internal {
        require(amount > 0, "TZ");
        require(amount <= getGrizzlyStrategyBalance(to), "SD");
    ...
    function updateUserMask(address to) internal {
        updateRoundMasks();

        participantData[to].pendingHoney +=
            ((honeyRoundMask - participantData[to].honeyMask) *
                participantData[to].amount) /
            DECIMAL_OFFSET;

        participantData[to].honeyMask = honeyRoundMask;

        participantData[to].pendingLp +=
            ((lpRoundMask - participantData[to].lpMask) *
                participantData[to].amount) /
            DECIMAL_OFFSET;

        participantData[to].lpMask = lpRoundMask;

        participantData[to].pendingAdditionalHoney +=
            ((additionalHoneyRoundMask -
                participantData[to].additionalHoneyMask) *
                participantData[to].amount) /
            DECIMAL_OFFSET;

        participantData[to].additionalHoneyMask = additionalHoneyRoundMask;
    }
    ...
    function grizzlyStrategyClaimHoney() external returns (uint256) {
        return grizzlyStrategyClaimHoneyInternal(msg.sender);
    }

    function grizzlyStrategyClaimHoneyInternal(address to)
        internal
        returns (uint256)
    {
        isNotPaused();
        updateRoundMasks();
        uint256 pendingHoney = participantData[to].pendingHoney +
            ((honeyRoundMask - participantData[to].honeyMask) *
                participantData[to].amount) /
            DECIMAL_OFFSET;
        ...
        if (pendingHoney > 0) {
            participantData[to].pendingHoney = 0;
        ...
        emit GrizzlyStrategyClaimHoneyEvent(to, pendingHoney);
        return pendingHoney;
    }
    ...
    {
        return grizzlyStrategyClaimLPInternal(msg.sender);
    }

    function grizzlyStrategyClaimLPInternal(address to)
        internal
        returns (uint256 claimedHoney, uint256 claimedBnb)
    {
        isNotPaused();
        updateRoundMasks();
        uint256 pendingLp = participantData[to].pendingLp +
            ((lpRoundMask - participantData[to].lpMask) *
                participantData[to].amount) /
            DECIMAL_OFFSET;
        ...
        uint256 pendingAdditionalHoney = participantData[to]
        ...
        if (pendingLp > 0 || pendingAdditionalHoney > 0) {
            participantData[to].pendingLp = 0;
            participantData[to].pendingAdditionalHoney = 0;
        ...
        emit GrizzlyStrategyClaimLpEvent(to, _claimedHoney, _claimedBnb);
    ...
    function getGrizzlyStrategyBalance(address to)
        public
        view
        returns (uint256)
    {
        return participantData[to].amount;
    }
    ...
    function getGrizzlyStrategyStakedHoney(address to)
        public
        view
        returns (uint256)
    {
        if (participantData[to].honeyMask == 0 || grizzlyStrategyDeposits == 0)
            return 0;
        ...
        return
            participantData[to].pendingHoney +
            ((currentHoneyRoundMask - participantData[to].honeyMask) *
                participantData[to].amount) /
            DECIMAL_OFFSET;
    }
    ...
    function getGrizzlyStrategyLpRewards(address to)
        external
        view
        returns (uint256)
    {
        if (participantData[to].lpMask == 0 || grizzlyStrategyDeposits == 0)
            return 0;
        ...
        return
            participantData[to].pendingLp +
            ((currentLpRoundMask - participantData[to].lpMask) *
                participantData[to].amount) /
            DECIMAL_OFFSET;
    }
...
abstract contract StableStableCoinStrategy is Initializable, StableBaseConfig {
    ...
    /// @param amount The desired deposit amount for an investor
    function stablecoinStrategyDeposit(uint256 amount, address to) internal {
        uint256 currentBalance = getStablecoinStrategyBalance(to);
        uint256 currentAmount = participantData[to].amount;

        participantData[to].rewardMask = roundMask;
        participantData[to].amount = currentBalance + amount;
        participantData[to].totalReinvested += currentBalance - currentAmount;
    ...
    /// @dev The current round mask for rewards is updated before the deposit to have a clean state
    /// @param amount The desired withdraw amount for an investor
    function stablecoinStrategyWithdraw(uint256 amount, address to) internal {
        require(amount > 0, "TZ");
    ...
    function getStablecoinStrategyBalance(address to)
        public
        view
        returns (uint256)
    {
        if (participantData[to].rewardMask == 0) return 0;

        return
            participantData[to].amount +
            ((roundMask - participantData[to].rewardMask) *
                participantData[to].amount) /
            DECIMAL_OFFSET;
    }
...
abstract contract StableStandardStrategy is Initializable, StableBaseConfig {
    ...
    /// @param amount The desired deposit amount for an investor
    function standardStrategyDeposit(uint256 amount, address to) internal {
        updateStandardRewardMask(to);
        uint256 currentDeposit = getStandardStrategyBalance(to);
        uint256 currentAmount = participantData[to].amount;
        ...
        participantData[to].amount = currentDeposit + amount;
        participantData[to].lpMask = lpRoundMask;
        participantData[to].totalReinvested += currentDeposit - currentAmount;
    }

    /// @notice Withdraws the desired amount for a standard strategy investor
    /// @dev Pending lp rewards are rewarded and the investors rewardMask is set again to the current roundMask
    /// @param amount The desired withdraw amount for an investor
    function standardStrategyWithdraw(uint256 amount, address to) internal {
        require(amount > 0, "TZ");
    ...
    function getStandardStrategyBalance(address to)
        public
        view
        returns (uint256)
    {
        if (participantData[to].lpMask == 0) return 0;
    ...
    function standardStrategyClaimHoney() external returns (uint256) {
        return standardStrategyClaimHoneyInternal(msg.sender);
    }

    function standardStrategyClaimHoneyInternal(address to)
        internal
        returns (uint256)
    {
        isNotPaused();
        updateStandardRewardMask(to);
        uint256 pendingRewards = participantData[to].pendingRewards;
        participantData[to].pendingRewards = 0;
        IERC20Upgradeable(address(HoneyToken)).safeTransfer(to, pendingRewards);
        emit StandardStrategyClaimHoneyEvent(to, pendingRewards);
    ...
    function getStandardStrategyHoneyRewards(address to)
        public
        view
        returns (uint256)
    {
        ...
        return
            participantData[to].pendingRewards +
            ((honeyRoundMask - participantData[to].rewardMask) *
                participantData[to].amount) /
            DECIMAL_OFFSET;
    }

    /// @notice Updates the standard strategy honey rewards mask
    function updateStandardRewardMask(address to) private {
        uint256 currentRewardBalance = getStandardStrategyHoneyRewards(to);
        participantData[to].pendingRewards = currentRewardBalance;
        participantData[to].rewardMask = honeyRoundMask;
    }
    ...
    IStableGrizzly
{
    ...
    function withdrawToLp(
        ...
        require(deadline > block.timestamp, "DE");
        DEX.checkSlippage(fromToken, toToken, amountIn, amountOut, slippage);
        uint256 amountWithdrawn = _withdraw(to, amount);
    ...
    function _deposit(
        ...
        uint256 lpValue = amount;

        if (userStrategy[to] == Strategy.STANDARD) {
            standardStrategyDeposit(lpValue, to);
        } else if (userStrategy[to] == Strategy.GRIZZLY) {
            grizzlyStrategyDeposit(lpValue, to);
        } else {
            stablecoinStrategyDeposit(lpValue, to);
        }

        StakingContract.deposit(PoolID, lpValue);

        Referral.referralDeposit(lpValue, to, referralGiver);
        emit DepositEvent(to, lpValue, userStrategy[to]);
        return lpValue;
    }
    ...
    function _withdraw(address to, uint256 amount) internal returns (uint256) {
        _stakeRewards();
        uint256 withdawAmount;

        if (userStrategy[to] == Strategy.STANDARD) {
            standardStrategyWithdraw(amount, to);
            standardStrategyClaimHoneyInternal(to);
        } else if (userStrategy[to] == Strategy.GRIZZLY) {
            grizzlyStrategyWithdraw(amount, to);
            grizzlyStrategyClaimHoneyInternal(to);
            grizzlyStrategyClaimLPInternal(to);
        } else {
            stablecoinStrategyWithdraw(amount, to);
        }
        ...
        Referral.referralWithdraw(amount, to);
        emit WithdrawEvent(to, amount, userStrategy[to]);
    ...
    function changeStrategy(
        ...
        if (userStrategy[msg.sender] == Strategy.STANDARD) {
            currentDeposits = getStandardStrategyBalance(msg.sender);
            if (currentDeposits > 0) {
                standardStrategyWithdraw(currentDeposits, msg.sender);
                standardStrategyClaimHoneyInternal(msg.sender);
            }
        } else if (userStrategy[msg.sender] == Strategy.GRIZZLY) {
            currentDeposits = getGrizzlyStrategyBalance(msg.sender);
            if (currentDeposits > 0) {
                grizzlyStrategyWithdraw(currentDeposits, msg.sender);
                grizzlyStrategyClaimHoneyInternal(msg.sender);
                grizzlyStrategyClaimLPInternal(msg.sender);
            }
        } else {
            currentDeposits = getStablecoinStrategyBalance(msg.sender);
            if (currentDeposits > 0) {
                stablecoinStrategyWithdraw(currentDeposits, msg.sender);
            }
        }

        if (currentDeposits > 0) {
            if (toStrategy == Strategy.STANDARD)
                standardStrategyDeposit(currentDeposits, msg.sender);
            else if (toStrategy == Strategy.GRIZZLY)
                grizzlyStrategyDeposit(currentDeposits, msg.sender);
            else stablecoinStrategyDeposit(currentDeposits, msg.sender);
        }
        ...
        )
    {
        ...
        if (currentStrategy == Strategy.GRIZZLY) {
            deposited = getGrizzlyStrategyBalance(msg.sender);
            balance = deposited;
            totalReinvested = 0;
            (earnedHoney, earnedBnb) = grizzlyStrategyClaimLPInternal(
                msg.sender
            );
            stakedHoney = getGrizzlyStrategyStakedHoney(msg.sender);
            ...
            earnedHoney = getStandardStrategyHoneyRewards(msg.sender);
            earnedBnb = 0;
            stakedHoney = 0;
