...
contract VaultUtils is IVaultUtils, Governable {
    ...
    function getMaxAmountIn(
        ...
        uint256 maxUsdgAmount = vault.maxUsdgAmounts(_tokenIn);

        if (maxUsdgAmount != 0) {
            if (maxUsdgAmount < vault.usdgAmounts(_tokenIn)) {
                return 0;
            }

            uint256 maxAmountIn = maxUsdgAmount.sub(vault.usdgAmounts(_tokenIn));
