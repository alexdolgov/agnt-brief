 ...
 *
 * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 * * * * * Aerarium Finance : Multi-Reward Shadow-farm * * * * *
 * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 *
 * End-users can Stake their vAMM-AERA/USDC Hermes Liquidity Positon Tokens
 * to earn multiple reward tokens added by Aerarium Treasury.
 *
 * Simultaneously, Aerarium Treasury earns HERMES from the Hermes Protocol Gauge.
 *
 */
...
contract MultiRewards is ReentrancyGuard, Pausable {
    ...
    address public TvlGuru = 0x50Dcc6cb1B2d6965c42d98a2b07629c57a6be895;
    ...
    function withdraw(uint256 amount) external nonReentrant notPaused updateReward(msg.sender) {
    	_withdraw(amount);
    }
	...
	function aprs() public view returns(uint[] memory _aprs) {
		( , bytes memory _tt) = address(TvlGuru).staticcall(TvlPriceFeed[address(stakingToken)]);
		(uint _pt) = abi.decode(_tt, (uint));
		uint _tvl = (_totalSupply * _pt) / 1e18;
