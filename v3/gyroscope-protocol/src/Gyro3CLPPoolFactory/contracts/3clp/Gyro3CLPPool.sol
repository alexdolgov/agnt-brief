...
contract Gyro3CLPPool is ExtensibleBaseWeightedPool, CappedLiquidity, LocallyPausable {
    ...
    // Note: These obviously don't include rate-scaling. Use _scalingFactor() or _scalingFactors()
    // to get the factor including rate scaling.
    ...
    IRateProvider public immutable rateProvider2;
        ...
        LocallyPausable(params.config.pauseManager)
    {
        ...
        address[] memory rateProviders = params.config.rateProviders;
        _grequire(tokens.length == 3, Gyro3CLPPoolErrors.TOKENS_LENGTH_MUST_BE_3);
        _grequire(rateProviders.length == 3, Gyro3CLPPoolErrors.TOKENS_LENGTH_MUST_BE_3);
        ...
        rateProvider0 = IRateProvider(rateProviders[0]);
        rateProvider1 = IRateProvider(rateProviders[1]);
        rateProvider2 = IRateProvider(rateProviders[2]);
    ...
    function _scalingFactor(IERC20 token) internal view virtual override returns (uint256) {
        if (token == _token0) {
            return _combineScalingFactors(_scalingFactor0, rateProvider0);
        } else if (token == _token1) {
            return _combineScalingFactors(_scalingFactor1, rateProvider1);
        } else if (token == _token2) {
            return _combineScalingFactors(_scalingFactor2, rateProvider2);
        } else {
    ...
    function _scalingFactors() internal view virtual override returns (uint256[] memory) {
        ...
        scalingFactors[0] = _combineScalingFactors(_scalingFactor0, rateProvider0);
        scalingFactors[1] = _combineScalingFactors(_scalingFactor1, rateProvider1);
        scalingFactors[2] = _combineScalingFactors(_scalingFactor2, rateProvider2);

        return scalingFactors;
    }

    /// @dev Combine a (static) decimal scaling factor with a (dynamic) rate-scaling factor from a
    // rateprovider (that may be address(0) for no-op)
    function _combineScalingFactors(uint256 decimalScalingFactor, IRateProvider rateProvider) internal view returns (uint256 scalingFactor) {
        scalingFactor = decimalScalingFactor;
        if (address(rateProvider) != address(0)) {
            scalingFactor = scalingFactor.mulDown(rateProvider.getRate());
        }
    }
    ...
    function _getThirdToken(IERC20 x, IERC20 y) internal view returns (IERC20 tokenOther, uint256 scalingFactorOther) {
        ...
        if (x == _token0) {
            if (y == _token1) return (_token2, _combineScalingFactors(_scalingFactor2, rateProvider2));
            if (y != _token2) _grequire(false, Gyro3CLPPoolErrors.TOKENS_NOT_AMONG_POOL_TOKENS);
            return (_token1, _combineScalingFactors(_scalingFactor1, rateProvider1));
        }
        if (!(x == _token1 && y == _token2)) _grequire(false, Gyro3CLPPoolErrors.TOKENS_NOT_AMONG_POOL_TOKENS);
        return (_token0, _combineScalingFactors(_scalingFactor0, rateProvider0));
    }
    ...
    function _getAllBalances() private view returns (uint256[] memory balances) {
        ...
        balances[0] = _getScaledTokenBalance(_token0, scalingFactors[0]);
        balances[1] = _getScaledTokenBalance(_token1, scalingFactors[1]);
        balances[2] = _getScaledTokenBalance(_token2, scalingFactors[2]);
        return balances;
    }
     ...
     * When rateProvider{0,1,2} is provided, the returned price corresponds to tokens *before* rate
     * scaling.
     */
    function getPrices() external view returns (uint256 spotPrice0, uint256 spotPrice1) {
        ...
        (spotPrice0, spotPrice1) = Gyro3CLPMath._calcSpotPrice01in2(balances, virtualOffset);
        spotPrice0 = _adjustPrice(spotPrice0, 0);
        spotPrice1 = _adjustPrice(spotPrice1, 1);
    }

    /// @dev Adjust the rate-scaled ("inner") spot price to yield the non-rate-scaled ("outer") spot price.
    // baseTokenIx: 0 for the price of token0 in units of token2; 1 for token1 in units of token2.
    function _adjustPrice(uint256 spotPrice, uint8 baseTokenIx) internal view returns (uint256) {
        if (baseTokenIx == 0) {
            if (address(rateProvider0) != address(0)) spotPrice = spotPrice.mulDown(rateProvider0.getRate());
        } else if (baseTokenIx == 1) {
            if (address(rateProvider1) != address(0)) spotPrice = spotPrice.mulDown(rateProvider1.getRate());
        } else {
            assert(false);
        }
        if (address(rateProvider2) != address(0)) spotPrice = spotPrice.divDown(rateProvider2.getRate());
        return spotPrice;
    }

    /// @notice Convenience function to fetch the two rates used for scaling the three tokens, as of rateProvider{0,1,2}.
        ...
        )
    {
        ...
        rate2 = address(rateProvider2) != address(0) ? rateProvider2.getRate() : GyroFixedPoint.ONE;
    }
