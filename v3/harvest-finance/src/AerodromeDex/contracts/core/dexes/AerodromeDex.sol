...
contract AerodromeDex is Ownable, ILiquidityDex, AerodromeDexStorage {
    ...
    function doSwap(
        ...
        for (uint256 idx = 0; idx < _path.length-1; ) {
            IRouter.Route memory route;
            route.from = _path[idx];
            route.to = _path[idx+1];
            route.stable = stable(_path[idx], _path[idx+1]);
            route.factory = factory(_path[idx], _path[idx+1]);
            routes[idx] = route;
            unchecked {
                ++idx;
            }
