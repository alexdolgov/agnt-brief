...
contract SeamlessFoldStrategy is BaseUpgradeableStrategy {
  ...
  */
  ...
  function _redeemWithFlashloan(uint256 amount, uint256 borrowTargetFactorNumerator) internal {
    ...
    if (borrowDiff > balancerBalance) {
      _redeemNoFlash(amount, currentSupplied(), borrowed, aToken(), debtToken(), factorDenominator(), borrowTargetFactorNumerator);
    } else if (borrowDiff > 0){
      address[] memory tokens = new address[](1);
      uint256[] memory amounts = new uint256[](1);
      bytes memory userData = abi.encode(0);
  ...
  function _redeemNoFlash(uint256 amount, uint256 supplied, uint256 borrowed, address _aToken, address _debtToken, uint256 _denom, uint256 _borrowNum) internal {
    address _underlying = underlying();
    uint256 newBorrowTarget;
    {
