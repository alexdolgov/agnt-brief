...
contract Moonwell2AssetFoldStrategy is BaseUpgradeableStrategy {
  ...
  function getSupplyPriceInBorrow() public view returns (uint256) {
    return MoonwellViewer(address(0x8ccCD2467adD6053c8273042ac742Eb438444389)).getPrice(supplyMToken(), borrowMToken());
  }

  function getBorrowPriceInSupply() public view returns (uint256) {
    return MoonwellViewer(address(0x8ccCD2467adD6053c8273042ac742Eb438444389)).getPrice(borrowMToken(), supplyMToken());
  }
  ...
  function positionHealth() public view returns (uint256) {
    return MoonwellViewer(address(0x8ccCD2467adD6053c8273042ac742Eb438444389)).getPositionHealth(supplyMToken(), borrowMToken(), collateralFactorNumerator());
  }
  ...
  function checker() external view returns (bool canExec, bytes memory execPayload) {
    canExec = positionHealth() < targetHealth().mul(99).div(100);
  ...
  */
  ...
  function withdrawToVault(uint256 amountUnderlying) public restricted {
    ...
    uint256 positionChange = positionBalanceBefore.sub(currentBalance());
    if (positionChange > toRedeem) {
      amountUnderlying = amountUnderlying.sub(positionChange.sub(toRedeem));
    }
  ...
  */
  ...
  function _depositWithFlashloan() internal {
    ...
    uint256 borrowDiff;
    if (positionHealth() < targetHealth().mul(99).div(100)) {
      _redeemPartial(0);
      borrowDiff = 0;
    } else if (positionHealth() < targetHealth().mul(101).div(100)) {
      borrowDiff = 0;
    } else {
      uint256 balance = currentBalance();
    ...
    if (borrowDiff > balancerBalance) {
      // uint256 supplied = MTokenInterface(_supplyMToken).balanceOfUnderlying(address(this));
      // _depositNoFlash(supplied, borrowed, borrowDiff, _supplyMToken, _borrowMToken);
      revert("No flash available");
    } else if (borrowDiff > 0) {
      address[] memory tokens = new address[](1);
      uint256[] memory amounts = new uint256[](1);
      bytes memory userData = abi.encode(0);
      tokens[0] = borrowAsset();
      amounts[0] = borrowDiff.mul(1000).div(1002);
  ...
  function _redeemWithFlashloan(uint256 amount, uint256 _borrowTargetFactorNumerator) internal {
    ...
    if (borrowDiffInUnderlying > balancerBalance) {
      ...
      bytes memory userData = abi.encode(0);
      tokens[0] = underlying();
      amounts[0] = borrowDiffInUnderlying.mul(1002).div(1000);
  ...
  function receiveFlashLoan(IERC20[] memory tokens, uint256[] memory amounts, uint256[] memory feeAmounts, bytes memory /*userData*/) external {
    ...
    if (makingFlashDeposit){
      address _underlying = underlying();
      _swap(token, _underlying, amounts[0]);
  ...
  function _swap(address from, address to, uint256 amount) internal {
    ...
    uint256 minOut = from == underlying() ? 
      amount.mul(getSupplyPriceInBorrow()).div(1e18).mul(1000).div(1005) : 
      amount.mul(getBorrowPriceInSupply()).div(1e18).mul(1000).div(1005);
  ...
  // function _depositNoFlash(uint256 supplied, uint256 borrowed, uint256 borrowDiff, address _supplyMToken, address _borrowMToken) internal {
  //   address _underlying = underlying();
  //   uint256 borrowTarget = borrowed.add(borrowDiff);
  ...
  //       MTokenInterface(_borrowMToken).getCash()
  //     );
  //     _borrow(Math.min(wantBorrow, maxBorrow));
  //     _swap(borrowAsset(), _underlying, IERC20(borrowAsset()).balanceOf(address(this)));
  //     uint256 underlyingBalance = IERC20(_underlying).balanceOf(address(this));
  //     if (underlyingBalance > 0) {
  //       _supply(underlyingBalance);
  //     }
  //     //update parameters
  //     borrowed = MTokenInterface(_borrowMToken).borrowBalanceCurrent(address(this));
