...
/**
 * @title Hemi HemiLock Contract
 * @dev Lock contract for Hemi token.
 */
contract HemiLock is AccessControl {
    using SafeERC20 for IERC20Metadata;

    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");

    IERC20Metadata public immutable hemi;
     ...
     * @param hemi_ Hemi token address
     * @param owner_ Owner address
     */
    constructor(address hemi_, address owner_) {
        if (hemi_ == address(0)) revert AddressIsZero();
        if (owner_ == address(0)) revert AddressIsZero();

        _grantRole(DEFAULT_ADMIN_ROLE, owner_);
        hemi = IERC20Metadata(hemi_);
    }
    ...
    function mint(address to_, uint256 amount_) external onlyRole(MINTER_ROLE) {
        ...
        if (hemi.balanceOf(address(this)) < amount_) revert InsufficientBalance();

        hemi.safeTransfer(to_, amount_);
    }
}
