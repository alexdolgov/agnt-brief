...
/*
 * This contract is used to distribute dividends to users that allocated xHENAI here
 *
 ...
 * This distribution slot is the source of the dividends distribution to xHENAI allocators during the current cycle
 *
 * This transfer from the pending slot to the distribution slot is based on cycleDividendsPercent and CYCLE_PERIOD_SECONDS
 *
 */
contract Dividends is Ownable, ReentrancyGuard, IXHENAITokenUsage, IDividends {
  ...
  address public immutable xHENAIToken; // xHENAIToken contract

  mapping(address => uint256) public usersAllocation; // User's xHENAI allocation
  uint256 public totalAllocation; // Contract's total xHENAI allocation
  ...
  constructor(address xHENAIToken_, uint256 startTime_) {
    require(xHENAIToken_ != address(0), "zero address");
    xHENAIToken = xHENAIToken_;
  ...
  /**
   * @dev Checks if caller is the xHENAIToken contract
   */
  modifier xHENAITokenOnly() {
    require(msg.sender == xHENAIToken, "xHENAITokenOnly: caller should be XHENAIToken");
    _;
  }
  ...
  /**
   * Allocates "userAddress" user's "amount" of xHENAI to this dividends contract
   *
   * Can only be called by xHENAIToken contract, which is trusted to verify amounts
   * "data" is only here for compatibility reasons (IXHENAITokenUsage)
   */
  function allocate(address userAddress, uint256 amount, bytes calldata /*data*/) external override nonReentrant xHENAITokenOnly {
  ...
  /**
   * Deallocates "userAddress" user's "amount" of xHENAI allocation from this dividends contract
   *
   * Can only be called by xHENAIToken contract, which is trusted to verify amounts
   * "data" is only here for compatibility reasons (IXHENAITokenUsage)
   */
  function deallocate(address userAddress, uint256 amount, bytes calldata /*data*/) external override nonReentrant xHENAITokenOnly {
  ...
  */
  ...
  function _updateDividendsInfo(address token) internal {
    ...
    // if no xHENAI is allocated or initial distribution has not started yet
  ...
  function _harvestDividends(address token) internal {
    ...
    uint256 userXHENAIAllocation = usersAllocation[msg.sender];
    uint256 pending = user.pendingDividends.add(
      userXHENAIAllocation.mul(accDividendsPerShare).div(1e18).sub(user.rewardDebt)
    );

    user.pendingDividends = 0;
    user.rewardDebt = userXHENAIAllocation.mul(accDividendsPerShare).div(1e18);
