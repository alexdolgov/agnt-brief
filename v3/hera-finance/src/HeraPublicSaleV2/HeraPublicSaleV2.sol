...
contract HeraPublicSaleV2 is ReentrancyGuard, Ownable {
    ...
    {
        address[] memory _userAddress = new address[](
            allParticipantAddresses.length
        );
        uint256[] memory _userBusdAmount = new uint256[](
            allParticipantAddresses.length
        );
        uint256[] memory _userTokenAmount = new uint256[](
            allParticipantAddresses.length
        );
        uint256[] memory _userClaimRound = new uint256[](
            allParticipantAddresses.length
        );
        uint256[] memory _claimedValue = new uint256[](
            allParticipantAddresses.length
        );
        uint256[] memory _lastClaimDate = new uint256[](
            allParticipantAddresses.length
        );

        for (uint256 i = 0; i < allParticipantAddresses.length; i++) {
            investorData storage investor = _investorData[
                allParticipantAddresses[i]
            ];
            _userAddress[i] = allParticipantAddresses[i];
            _userBusdAmount[i] = investor.totalBuyingAmountUsd;
            _userTokenAmount[i] = investor.totalBuyingAmountToken;
            _userClaimRound[i] = investor.claimRound;
            _claimedValue[i] = investor.claimedValue;
            _lastClaimDate[i] = investor.lastClaimDate;
        }
    ...
    function buyTokens(uint256 _value) external nonReentrant isContractActive {
        ...
        require(
            maxBuyValue >= investor.totalBuyingAmountUsd.add(_value),
            "User limit exceeded"
        );
        ...
        require(
            hardcap >= totalSoldAmountBNB.add(_value),
            "hardcap value exceed"
        );
        require(
            busdToken.transferFrom(msg.sender, address(this), _value),
            "bad transfer"
        );
    ...
    function changeMaxBuyAmount(uint256 _amount)
        external
        nonReentrant
        onlyOwner
    {
    ...
    function claimTokens() external nonReentrant {
        ...
        require(
            ...
            "bad claim round date"
        );
        uint256 tcl = investor.claimedValue.add(
