...
contract FairAuction is Ownable, ReentrancyGuard {
    ...
    mapping(address => bool) public isEarlyInvestor; // early investor info
    ...
    address public weth = 0x75cb093E4D61d2A2e65D8e0BBb01DE8d89b53481;
    ...
    event AddEarlyInvestors(address[] accounts, uint256[] amounts);
    ...
    modifier isSaleActive() {
        ...
        if (address(PROJECT_TOKEN_2) != address(0)) {
            require(
                PROJECT_TOKEN_2.balanceOf(address(this)) >=
                    MAX_PROJECT_TOKENS_2_TO_DISTRIBUTE,
    ...
    modifier isClaimable() {
        ...
        require(forceClaimable, "isClaimable: cannot claim");
        _;
    }
    ...
    function projectTokens2ToDistribute() public view returns (uint256) {
        ...
        if (MIN_TOTAL_RAISED_FOR_MAX_PROJECT_TOKEN > totalRaised) {
            return
                MAX_PROJECT_TOKENS_2_TO_DISTRIBUTE.mul(totalRaised).div(
                    MIN_TOTAL_RAISED_FOR_MAX_PROJECT_TOKEN
                );
        }
    ...
    function getExpectedClaimAmount(
        ...
        public
        view
        returns (uint256 projectTokenAmount, uint256 projectToken2Amount)
    {
        if (totalAllocation == 0) return (0, 0);

        UserInfo memory user = userInfo[account];
        projectTokenAmount = user
            .allocation
            .mul(projectTokensToDistribute())
            .div(totalAllocation);
        projectToken2Amount = user
            .allocation
            .mul(projectTokens2ToDistribute())
            .div(totalAllocation);
    }
    ...
    function _buy(uint256 amount) internal {
        ...
        if (whitelistOnly) {
            ...
            uint256 userWalletCap = CAP_PER_WALLET > user.whitelistCap
                ...
                : user.whitelistCap;
            require(
                user.contribution.add(amount) <= userWalletCap,
    ...
    function addEarlyInvestors(
        address[] calldata accounts,
        uint256[] calldata amounts
    ) external onlyOwner {
        require(
            accounts.length == amounts.length,
            "addEarlyInvestors: invalid input"
        );

        for (uint256 i = 0; i < accounts.length; ++i) {
            // check if the investor already added
            if (isEarlyInvestor[accounts[i]]) {
                continue;
            }

            // sign wallet as added
            isEarlyInvestor[accounts[i]] = true;

            UserInfo storage user = userInfo[accounts[i]];
            // update raised amounts
            user.contribution = user.contribution.add(amounts[i]);
            totalRaised = totalRaised.add(amounts[i]);

            // update allocations
            user.allocation = user.allocation.add(amounts[i]);
            totalAllocation = totalAllocation.add(amounts[i]);

            emit AddEarlyInvestors(accounts, amounts);
        }
    }
    ...
    function withdrawUnsoldTokens() external onlyOwner {
        ...
        if (totalToken2Sold > 0)
            PROJECT_TOKEN_2.transfer(
                msg.sender,
                MAX_PROJECT_TOKENS_2_TO_DISTRIBUTE.sub(totalToken2Sold)
            );
    }
