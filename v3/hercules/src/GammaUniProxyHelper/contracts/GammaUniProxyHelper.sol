...
contract GammaUniProxyHelper is Ownable, ReentrancyGuard {
  ...
  address public gammaUniProxy = 0x0d8F8eB720F4E3C1Bcaa50C78339f796CD4a380F;
  ...
  function getDepositAmount(
    address pos, address token, uint256 deposit_
  ) external view returns (uint256 amountStart, uint256 amountEnd) {
    return IUniProxy(gammaUniProxy).getDepositAmount(pos, token, deposit_);
  }
}