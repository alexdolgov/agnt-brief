...
import "../plugins/interfaces/IXGrailTokenUsage.sol";


/*
 * xMetis Token Contract
 * It's non-transferable, except from/to whitelisted addresses
 * It can be converted back to METIS through a vesting process
 * This contract is made to receive xMetis deposits from users in order to allocate them to Usages (plugins) contracts
 */
contract xMetisToken is Ownable, ReentrancyGuard, ERC20("Metis escrowed token", "xMETIS") {
  using Address for address;
  using SafeMath for uint256;
  using EnumerableSet for EnumerableSet.AddressSet;

  struct xMetisBalance {
    uint256 allocatedAmount; // Amount of xMetis allocated to a Usage
    uint256 redeemingAmount; // Total amount of xMetis currently being redeemed
  }

  struct RedeemInfo {
    uint256 metisAmount; // METIS amount to receive when vesting has ended
    uint256 xMetisAmount; // xMetis amount to redeem
    uint256 endTime;
    IXGrailTokenUsage dividendsAddress;
    uint256 dividendsAllocation; // Share of redeeming xMetis to allocate to the Dividends Usage contract
  }

  IXGrailTokenUsage public dividendsAddress; // Camelot dividends contract

  EnumerableSet.AddressSet private _transferWhitelist; // addresses allowed to send/receive xMetis

  mapping(address => mapping(address => uint256)) public usageApprovals; // Usage approvals to allocate xMetis
  mapping(address => mapping(address => uint256)) public usageAllocations; // Active xMetis allocations to usages

  uint256 public constant MAX_DEALLOCATION_FEE = 200; // 2%
  mapping(address => uint256) public usagesDeallocationFee; // Fee paid when deallocating xMetis
  ...
  // Adjusted dividends rewards for redeeming xMetis
  uint256 public redeemDividendsAdjustment = 0; // 50%

  mapping(address => xMetisBalance) public xMetisBalances; // User's xMetis balances
  mapping(address => RedeemInfo[]) public userRedeems; // User's redeeming instances
  
  address public treasuryAddress;

  constructor(address _treasuryAddress, address _converter) {
    treasuryAddress = _treasuryAddress;
    _transferWhitelist.add(address(this));
    _transferWhitelist.add(_converter);
  }
  ...
  event Redeem(address indexed userAddress, uint256 xMetisAmount, uint256 metisAmount, uint256 duration);
  event FinalizeRedeem(address indexed userAddress, uint256 xMetisAmount, uint256 metisAmount);
  event CancelRedeem(address indexed userAddress, uint256 xMetisAmount);
  ...
  event UpdateTreasuryAddress(address indexed newTreasuryAddress);
  ...
  /*
   * @dev Returns user's xMetis balances
   */
  function getxMetisBalance(address userAddress) external view returns (uint256 allocatedAmount, uint256 redeemingAmount) {
    xMetisBalance storage balance = xMetisBalances[userAddress];
    return (balance.allocatedAmount, balance.redeemingAmount);
  }

  /*
   * @dev returns redeemable METIS for "amount" of xMetis vested for "duration" seconds
   */
  function getMetisByVestingDuration(uint256 amount, uint256 duration) public view returns (uint256) {
   ...
   */
  function getUserRedeem(address userAddress, uint256 redeemIndex) external view validateRedeem(userAddress, redeemIndex) returns (uint256 metisAmount, uint256 xMetisAmount, uint256 endTime, address dividendsContract, uint256 dividendsAllocation) {
    RedeemInfo storage _redeem = userRedeems[userAddress][redeemIndex];
    return (_redeem.metisAmount, _redeem.xMetisAmount, _redeem.endTime, address(_redeem.dividendsAddress), _redeem.dividendsAllocation);
  }

  /**
   * @dev returns approved xMetis to allocate from "userAddress" to "usageAddress"
   */
  ...
  /**
   * @dev returns allocated xMetis from "userAddress" to "usageAddress"
   */
  ...
  /**
   * @dev returns if "account" is allowed to send/receive xMetis
   */
  ...
  function updateTransferWhitelist(address account, bool add) external onlyOwner {
    require(account != address(this), "updateTransferWhitelist: Cannot remove xMetis from whitelist");
  ...
  /**
   * @dev Approves "usage" address to get allocations up to "amount" of xMetis from msg.sender
   */
  ...
  /**
   * @dev Convert caller's "amount" of METIS to xMETIS
   */
  function convert(uint256 amount) external payable nonReentrant {
    _convert(amount, msg.sender);
  }

  /**
   * @dev Convert caller's "amount" of METIS to xMETIS to "to" address
   */
  function convertTo(uint256 amount, address to) external payable nonReentrant {
    require(address(msg.sender).isContract(), "convertTo: not allowed");
    _convert(amount, to);
  }
  /**
   * @dev Initiates redeem process (xMetis to METIS)
   *
   * Handles dividends' compensation allocation during the vesting process if needed
   */
  function redeem(uint256 xMetisAmount, uint256 duration) external nonReentrant {
    require(xMetisAmount > 0, "redeem: xMetisAmount cannot be null");
    require(duration >= minRedeemDuration, "redeem: duration too low");

    _transfer(msg.sender, address(this), xMetisAmount);
    xMetisBalance storage balance = xMetisBalances[msg.sender];

    // get corresponding METIS amount
    uint256 metisAmount = getMetisByVestingDuration(xMetisAmount, duration);
    emit Redeem(msg.sender, xMetisAmount, metisAmount, duration);

    // if redeeming is not immediate, go through vesting process
    if(duration > 0) {
      // add to SBT total
      balance.redeemingAmount = balance.redeemingAmount.add(xMetisAmount);

      // handle dividends during the vesting process
      uint256 dividendsAllocation = xMetisAmount.mul(redeemDividendsAdjustment).div(100);
      ...
      // add redeeming entry
      userRedeems[msg.sender].push(RedeemInfo(metisAmount, xMetisAmount, _currentBlockTimestamp().add(duration), dividendsAddress, dividendsAllocation));
    } else {
      // immediately redeem for METIS
      _finalizeRedeem(msg.sender, xMetisAmount, metisAmount);
    }
  }
  ...
  function finalizeRedeem(uint256 redeemIndex) external nonReentrant validateRedeem(msg.sender, redeemIndex) {
    ...
    balance.redeemingAmount = balance.redeemingAmount.sub(_redeem.xMetisAmount);
    _finalizeRedeem(msg.sender, _redeem.xMetisAmount, _redeem.metisAmount);
  ...
  function cancelRedeem(uint256 redeemIndex) external nonReentrant validateRedeem(msg.sender, redeemIndex) {
    ...
    // make redeeming xMetis available again
    balance.redeemingAmount = balance.redeemingAmount.sub(_redeem.xMetisAmount);
    _transfer(address(this), msg.sender, _redeem.xMetisAmount);
    ...
    emit CancelRedeem(msg.sender, _redeem.xMetisAmount);
  ...
  /**
   * @dev Allocates caller's "amount" of available xMetis to "usageAddress" contract
   *
  ...
  function allocate(address usageAddress, uint256 amount, bytes calldata usageData) external nonReentrant {
    ...
    // allocates xMetis to usageContract
    IXGrailTokenUsage(usageAddress).allocate(msg.sender, amount, usageData);
  }

  /**
   * @dev Allocates "amount" of available xMetis from "userAddress" to caller (ie usage contract)
   *
   * Caller must have an allocation approval for the required xMetis xMetis from "userAddress"
   */
  function allocateFromUsage(address userAddress, uint256 amount) external nonReentrant {
    _allocate(userAddress, msg.sender, amount);
  }

  /**
   * @dev Deallocates caller's "amount" of available xMetis from "usageAddress" contract
   *
   * args specific to usage contract must be passed into "usageData"
   */
  function deallocate(address usageAddress, uint256 amount, bytes calldata usageData) external nonReentrant {
    ...
    // deallocate xMetis into usageContract
    IXGrailTokenUsage(usageAddress).deallocate(msg.sender, amount, usageData);
  }

  /**
   * @dev Deallocates "amount" of allocated xMetis belonging to "userAddress" from caller (ie usage contract)
   *
   * Caller can only deallocate xMetis from itself
   */
  function deallocateFromUsage(address userAddress, uint256 amount) external nonReentrant {
  ...
  /********************************************************/
    ...
    /**
   * @dev Convert caller's "amount" of METIS into xMETIS to "to"
   */
  function _convert(uint256 amount, address to) internal {
    require(amount != 0, "convert: amount cannot be null");
    require(msg.value == amount, "convert: wrong amount");
  ...
  /**
   * @dev Finalizes the redeeming process for "userAddress" by transferring him "metisAmount" and removing "xMetisAmount" from supply
   *
   * Any vesting check should be ran before calling this
   */
  function _finalizeRedeem(address userAddress, uint256 xMetisAmount, uint256 metisAmount) internal {
    uint256 metisExcess = xMetisAmount.sub(metisAmount);

    payable(userAddress).transfer(metisAmount);
    payable(treasuryAddress).transfer(metisExcess);
    _burn(address(this), xMetisAmount);

    emit FinalizeRedeem(userAddress, xMetisAmount, metisAmount);
  }

  /**
   * @dev Allocates "userAddress" user's "amount" of available xMetis to "usageAddress" contract
   *
   */
  function _allocate(address userAddress, address usageAddress, uint256 amount) internal {
    ...
    uint256 approvedxMetis = usageApprovals[userAddress][usageAddress];
    require(approvedxMetis >= amount, "allocate: non authorized amount");

    // remove allocated amount from usage's approved amount
    usageApprovals[userAddress][usageAddress] = approvedxMetis.sub(amount);
  ...
  /**
   * @dev Deallocates "amount" of available xMetis to "usageAddress" contract
   *
   * args specific to usage contract must be passed into "usageData"
   */
  function _deallocate(address userAddress, address usageAddress, uint256 amount) internal {
    ...
    // check if there is enough allocated xMetis to this usage to deallocate
    ...
    payable(treasuryAddress).transfer(deallocationFeeAmount);
