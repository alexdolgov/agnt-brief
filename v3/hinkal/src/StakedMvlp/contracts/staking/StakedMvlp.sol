...
contract StakedMvlp {
    ...
    function balanceOf(address _account) external view returns (uint256) {
        return IRewardTracker(feeMvlpTracker).depositBalances(_account, mvlp);
    }

    function totalSupply() external view returns (uint256) {
        return IERC20(stakedMvlpTracker).totalSupply();
    }
    ...
    function _transfer(address _sender, address _recipient, uint256 _amount) private {
        ...
        require(
            mvlpManager.lastAddedAt(_sender).add(mvlpManager.cooldownDuration()) <= block.timestamp,
            "StakedMvlp: cooldown duration not yet passed"
        );
