...
library UniswapV3SwapLibrary {
    ...
    function _swapByInchRoute(SwapModule self, CompoundSwap memory compoundSwap, RevertParams memory revertParams) public {
        ...
        if (compoundSwap.srcToken == address(self.baseToken())) {
            uint256 baseAmountIn = baseBalanceBefore - baseBalanceAfter;
            uint256 sideAmountOut = sideBalanceAfter - sideBalanceBefore;

            uint256 amountOutMin = self.usdToSide(self.baseToUsd(baseAmountIn / 10000 * (10000 - self.allowedSlippageBp())));
            require(sideAmountOut >= amountOutMin, "Swapped amount less than min amount");

            if (revertParams.targetBalancePrice > 0) {
                if (revertParams.isReverse) {
                    uint256 rate = FullMathUni.mulDiv(baseAmountIn * self.sideDecimals(), revertParams.poolDecimals, sideAmountOut * self.baseDecimals());
                    require(revertParams.targetBalancePrice > rate || !revertParams.isRevertWhenBadRate, "bad rate for swap");
                } else {
                    uint256 rate = FullMathUni.mulDiv(sideAmountOut * self.baseDecimals(), revertParams.poolDecimals, baseAmountIn * self.sideDecimals());
                    require(revertParams.targetBalancePrice < rate || !revertParams.isRevertWhenBadRate, "bad rate for swap");
                }
            }

            emit InchConversion(address(self.baseToken()), address(self.sideToken()), baseAmountIn, sideAmountOut);
        } else {
            uint256 sideAmountIn = sideBalanceBefore - sideBalanceAfter;
            uint256 baseAmountOut = baseBalanceAfter - baseBalanceBefore;

            uint256 amountOutMin = self.usdToBase(self.sideToUsd(sideAmountIn / 10000 * (10000 - self.allowedSlippageBp())));
            require(baseAmountOut >= amountOutMin, "Swapped amount less than min amount");

            if (revertParams.targetBalancePrice > 0) {
                if (revertParams.isReverse) {
                    uint256 rate = FullMathUni.mulDiv(baseAmountOut * self.sideDecimals(), revertParams.poolDecimals, sideAmountIn * self.baseDecimals());
                    require(revertParams.targetBalancePrice < rate || !revertParams.isRevertWhenBadRate, "bad rate for swap");
                } else {
                    uint256 rate = FullMathUni.mulDiv(sideAmountIn * self.baseDecimals(), revertParams.poolDecimals, baseAmountOut * self.sideDecimals());
                    require(revertParams.targetBalancePrice > rate || !revertParams.isRevertWhenBadRate, "bad rate for swap");
                }
            }

            emit InchConversion(address(self.sideToken()), address(self.baseToken()), sideAmountIn, baseAmountOut);
        }
    }
    ...
    function _swapSideToBase(SwapModule self, uint256 delta) public {
        ...
        if (self.sideToUsd(swapSideAmount) <= 10 ** 3) {
            return;
        }
    ...
    function _swapBaseToSide(SwapModule self, uint256 delta) public {
        ...
        if (self.baseToUsd(swapBaseAmount) <= 10 ** 3) {
            return;
        }
