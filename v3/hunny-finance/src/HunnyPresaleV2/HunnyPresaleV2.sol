...
// Root file: contracts/hunny/HunnyPresaleV2.sol
...
interface IHunnyPresale {
    function endTime() external view returns(uint);
    function usersLength() external view returns(uint256);
    function users(uint256 userId) external view returns(address);
    function balanceOf(address account) external view returns(uint);
}

contract HunnyPresaleV2 is Ownable {
    using SafeMath for uint256;
    ...
    mapping(address => uint256) public noopBalances;
    address[] public noopUsers;

    constructor(address _presale, uint _totalBalance) public {
        presale = IHunnyPresale(_presale);

        // include total balance of noops whitelist deposit
        totalBalance = _totalBalance.add(3000000000000000000000000);

        // balance of addresses who directly sent BNB to presale address
        setNoopBalance(0x6eaBB6aa644a76765458b3a084820bBb65A0C778, 60000000000000000000000);  // 15 BNB
        setNoopBalance(0x253eF98454d0C57E6C3E063b4d190E981b53E6a4, 80000000000000000000000);  // 20 BNB
        setNoopBalance(0xEB382283eA0EaF1cFA91454C66160c1dcB26331b, 280399368400000000000000); // 70.09984209 BNB
        setNoopBalance(0x3b6E6C9ff79CA9C5848229d95F6F110d2Bc2268d, 161199368400000000000000); // 40.29984209 BNB
        setNoopBalance(0xc85584eA7C9db6E1f5bdb536aF4F858eE4D56AC0, 121199368400000000000000); // 30.29984209 BNB
        setNoopBalance(0x95D81FE1afCAE89de159142Fcb749b2FE2769F72, 161599368400000000000000); // 40.39984209 BNB
        setNoopBalance(0xdc50EB964e8D8BCACBCf86289d188Cb9dA29A2eF, 80000000000000000000000);  // 20 BNB
        setNoopBalance(0x77CAf519078Ca71Ac7E1F592A8C55f9D5460d50e, 80000000000000000000000);  // 20 BNB
        setNoopBalance(0x3A6968224FeFcA80025994805F7cCcCE13480a5B, 120000000000000000000000); // 30 BNB
        setNoopBalance(0x34f3f50e9F576bD27b9EfC2d65c9DD74Db5a3d21, 80000000000000000000000);  // 20 BNB
        setNoopBalance(0x6BCd7D0B0b874b79592c356D51847d4852dEB10b, 280000000000000000000000); // 70 BNB
        setNoopBalance(0x6d9a8BfAC1bECE4b3B47730CC824a52812E34c3F, 120000000000000000000000); // 30 BNB
        setNoopBalance(0xD0e500c4fB2e03fae75C1C37cAe211C096c8e719, 220000000000000000000000); // 55 BNB
        setNoopBalance(0xF628FA04Ae606530ef9EcAa79586b8d338Db94B8, 200000000000000000000000); // 50 BNB
        setNoopBalance(0x4C9343d92E9001fd4786E2A11029483a8f38D4AB, 400000000000000000000000); // 100 BNB
        setNoopBalance(0x2f2bCA81b98c33ef5a65155fa0C6Df7d1e7D3D86, 80000000000000000000000);  // 20 BNB
        setNoopBalance(0xcB17Aa0c15089027f756c913302693b28A825A97, 80000000000000000000000);  // 20 BNB
        setNoopBalance(0x7fa889B83dE30a17Cd739588Ad765b5C1658fA33, 240000000000000000000000); // 60 BNB
        setNoopBalance(0x6e7E602373404d4492e8D906557F07Bb6192a6Cd, 153200000000000000000000); // 38.3 BNB
        setNoopBalance(0xA219D4A74205084D7F76Aa4468FeF7d865DF9133, 2402526560000000000000);   // 0.60063164 BNB
    }

    // after the presale
    // dev withdraw all BNB from presale contract and direct send BNB to this contract
    receive() payable external {}

    function setNoopBalance(address account, uint256 amount) internal {
        noopBalances[account] = amount;

        noopUsers.push(account);
    }
    ...
    function distributeTokens(uint256 _pid, uint fromIndex, uint toIndex) public onlyOwner {
        ...
        if (toIndex > usersLength() || toIndex == 0) {
            toIndex = usersLength();
        }

        for(uint i=fromIndex; i<toIndex; i++) {
            address user = presale.users(i);
    ...
    function distributeTokensWhiteList(uint256 _pid) public onlyOwner {
        address lpToken = flipToken();
        require(lpToken != address(0), 'not set flip');
        require(masterChef != address (0), 'not set masterChef');
        require(stakingRewards != address(0), 'not set stakingRewards');
        ...
        for(uint i=0; i<noopUsers.length; i++) {
            address user = noopUsers[i];
            uint share = shareOf(user);
    ...
    function shareOf(address _user) private view returns(uint256) {
        if (noopBalances[_user] > 0) {
            return noopBalances[_user].mul(1e18).div(totalBalance);
        } else {
            return presale.balanceOf(_user).mul(1e18).div(totalBalance);
        }
    }
