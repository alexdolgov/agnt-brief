...
contract VoterV3 is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    modifier onlyNewEpoch(uint256 _tokenId) {
        ...
        if (block.timestamp >= HybraTimeLibrary.epochVoteEnd(block.timestamp)) revert("VOTE_WINDOW_ENDED");
        _;
    }
   ...
   function getPoolVotes(uint256 tokenId) external view returns (address[] memory) {
