...
contract DnCoreWriterVault is AccessControlUpgradeable {
    ...
    error DnCoreWriter__WithdrawalQueueNotSet();
    ...
    event WithdrawalQueueSet(address indexed withdrawalQueue);
    ...
    function releaseTokensForWithdrawals(address token, uint256 amount) external onlyRole(WITHDRAWAL_ROLE) {
        if (withdrawalQueue == address(0)) {
            revert DnCoreWriter__WithdrawalQueueNotSet();
        }
        ...
        if (token == HYPE) {
            payable(withdrawalQueue).transfer(amount);
        } else {
     ...
     * @dev Only callable by addresses with ADMIN_ROLE
     * @param _withdrawalQueue The address of the withdrawal queue
     */
    function setWithdrawalQueue(address _withdrawalQueue) external onlyRole(ADMIN_ROLE) {
