...
import { IHyperswapV3MintCallback } from "./hyperswap/callback/IHyperswapV3MintCallback.sol";
import { IHyperswapV3SwapCallback } from "./hyperswap/callback/IHyperswapV3SwapCallback.sol";
import { IHyperswapV3Pool } from "./hyperswap/IHyperswapV3Pool.sol";
 ...
 */
contract ICHIVault is IICHIVault, IHyperswapV3MintCallback, IHyperswapV3SwapCallback, ERC20, ReentrancyGuard, Ownable {
    ...
    */
    constructor(
        ...
        require((_allowToken0 && !_allowToken1) ||
                (_allowToken1 && !_allowToken0), "IVF.createICHIVault: must be single sided");

        ichiVaultFactory = msg.sender;
        pool = _pool;
        token0 = IHyperswapV3Pool(_pool).token0();
        token1 = IHyperswapV3Pool(_pool).token1();
        fee = IHyperswapV3Pool(_pool).fee();
        ...
        tickSpacing = IHyperswapV3Pool(_pool).tickSpacing();
    ...
    function validateDepositAndSyncFees(
        ...
        if (baseLiquidity > 0) {
            (uint burn0, uint burn1) = IHyperswapV3Pool(pool).burn(baseLower, baseUpper, 0);
            require(burn0 == 0 && burn1 == 0, "IV.deposit: unexpected burn (1)");
        }

        (uint128 limitLiquidity, , ) = _position(limitLower, limitUpper);
        if (limitLiquidity > 0) {
            (uint burn0, uint burn1) = IHyperswapV3Pool(pool).burn(limitLower, limitUpper, 0);
    ...
    */
    function rebalance(
        ...
        (uint128 baseLiquidity, , ) = _position(baseLower, baseUpper);
        if (baseLiquidity > 0) {
            IHyperswapV3Pool(pool).burn(baseLower, baseUpper, 0);
        }
        (uint128 limitLiquidity, , ) = _position(limitLower, limitUpper);
        if (limitLiquidity > 0) {
            IHyperswapV3Pool(pool).burn(limitLower, limitUpper, 0);
        }
    ...
    function _mintLiquidity(
        ...
        if (liquidity > 0) {
            (amount0, amount1) = IHyperswapV3Pool(pool).mint(
                address(this),
                tickLower,
                tickUpper,
                liquidity,
    ...
    */
    ...
    function _burnAnyLiquidity(
        ...
        // Burn liquidity
        (uint256 owed0, uint256 owed1) = IHyperswapV3Pool(pool).burn(tickLower, tickUpper, liquidity);

        // Collect amount owed
        uint128 collect0 = collectAll ? type(uint128).max : _uint128Safe(owed0);
        uint128 collect1 = collectAll ? type(uint128).max : _uint128Safe(owed1);
        if (collect0 > 0 || collect1 > 0) {
            (amount0, amount1) = IHyperswapV3Pool(pool).collect(to, tickLower, tickUpper, collect0, collect1);
        }
    }
    ...
    */
    ...
    function _position(
        ...
        (liquidity, , , tokensOwed0, tokensOwed1) = IHyperswapV3Pool(pool).positions(positionKey);
    }
     ...
     */
    function hyperswapV3MintCallback(uint256 amount0, uint256 amount1, bytes calldata data) external override {
    ...
    function checkHysteresis() private view returns (bool) {
        (, , uint16 observationIndex, , , , ) = IHyperswapV3Pool(pool).slot0();
        (uint32 blockTimestamp, , , ) = IHyperswapV3Pool(pool).observations(observationIndex);
    ...
    */
    ...
    function _amountsForLiquidity(
        ...
        (uint160 sqrtRatioX96, , , , , , ) = IHyperswapV3Pool(pool).slot0();
        return
    ...
    function currentTick() public view override returns (int24 tick) {
        (, int24 tick_, , , , , bool unlocked_) = IHyperswapV3Pool(pool).slot0();
