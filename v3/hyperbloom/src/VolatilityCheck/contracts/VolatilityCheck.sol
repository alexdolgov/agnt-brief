...
contract VolatilityCheck {
    ...
    function currentVolatility(address vault) external view returns (uint256 volatility) {
        ...
        address basePluginForPool = _getBasePluginFromPool(pool);
        ...
        uint256 twapPrice = UV3Math.getQuoteAtTick(
            int24(UV3Math.consult(basePluginForPool, twapPeriod)),
            uint128(PRECISION),
            token0,
            token1
        );
        ...
        uint256 auxTwapPrice = auxTwapPeriod > 0
            ? UV3Math.getQuoteAtTick(
                int24(UV3Math.consult(basePluginForPool, auxTwapPeriod)),
                uint128(PRECISION),
                token0,
                token1
            )
        ...
        volatility = max(max(spotTwapDelta, spotAuxTwapDelta), twapAuxTwapDelta).mul(PERCENT).div(PRECISION);
    }
    ...
    function _getBasePluginFromPool(address pool) private view returns (address basePlugin) {
