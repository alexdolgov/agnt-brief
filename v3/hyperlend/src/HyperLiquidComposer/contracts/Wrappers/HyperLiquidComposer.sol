...
contract HyperLiquidComposer is IOAppComposer {
    ...
    constructor(address _endpoint, address _oApp, address _hlNativeTransfer) {
        require(_endpoint != address(0), "Zero address");
        require(_oApp != address(0), "Zero address");
        require(_hlNativeTransfer != address(0), "Zero address");

        HL_NATIVE_TRANSFER = _hlNativeTransfer;
