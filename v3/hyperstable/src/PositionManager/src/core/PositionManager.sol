...
contract PositionManager is UUPSUpgradeable, OwnableUpgradeable, AccessControlUpgradeable, IPositionManager {
        ...
        returns (uint8)
    {
        ...
        emit VaultRegistered(index, _vaultAddress, data.asset, _mcr, _minDebt, data.debtCap, data.interestIndex);
    ...
    function deposit(uint8 _index, uint256 _amountToDeposit, uint256 _minSharesReceived) external {
        _isValidVault(_index);

        PositionManagerStorage.Layout storage $ = PositionManagerStorage.layout();

        VaultData storage vaultData = $.vaults[_index];
        PositionData storage positionData = $.positions[_index][msg.sender];

        if (positionData.owner == address(0)) {
            positionData.owner = msg.sender;
        }
        ...
        emit Deposit(_index, msg.sender, _amountToDeposit);

        emit VaultUpdated(_index, newVaultCollateralSnapshot, vaultData.debtSnapshot, vaultData.interestIndex);
        emit PositionUpdated(
            _index, msg.sender, newCollateralSnapshot, positionData.debtSnapshot, positionData.interestIndex
        );
    }

    function withdraw(uint8 _index, uint256 _amountToWithdraw, uint256 _minAssetRedeemed) external {
        _isValidVault(_index);
        ...
        emit Withdraw(_index, msg.sender, _amountToWithdraw);
        emit VaultUpdated(_index, newVaultCollateralSnapshot, vaultData.debtSnapshot, vaultData.interestIndex);
        emit PositionUpdated(
            _index, msg.sender, newCollateralSnapshot, positionData.debtSnapshot, positionData.interestIndex
        );
    ...
    function borrow(uint8 _index, uint256 _amountToBorrow) external {
        _isValidVault(_index);
        ...
        emit Borrow(_index, msg.sender, _amountToBorrow);

        emit VaultUpdated(_index, vaultData.collateralSnapshot, newVaultDebtSnapshot, vaultData.interestIndex);
        emit PositionUpdated(
            _index, msg.sender, positionData.collateralSnapshot, newDebtSnapshot, positionData.interestIndex
        );
    }

    function repay(uint8 _index, uint256 _amountToRepay) external {
        _isValidVault(_index);
        ...
        emit Repay(_index, msg.sender, _amountToRepay);
