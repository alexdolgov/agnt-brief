...
@license MIT
*/
contract TokenRewardsDistributor is IRewardsDistributor, SystemEpoch {
    ...
    constructor(address _voting_escrow, address _rewardToken) {
        uint256 _t = block.timestamp / EPOCH * EPOCH;
        start_time = _t;
        last_token_time = _t;
        time_cursor = _t;
        token = _rewardToken;
        ...
        require(IERC20(_rewardToken).approve(_voting_escrow, type(uint256).max));
    }
    ...
    function _checkpoint_token() internal {
        ...
        uint256 this_week = t / EPOCH * EPOCH;
        uint256 next_week = 0;

        for (uint256 i = 0; i < 20; i++) {
            next_week = this_week + EPOCH;
    ...
    function _checkpoint_total_supply() internal {
        ...
        uint256 rounded_timestamp = block.timestamp / EPOCH * EPOCH;
        ...
        for (uint256 i = 0; i < 20; i++) {
            if (t >= rounded_timestamp) {
                ...
                int256 currentSupply = int256(pt.bias - pt.slope * dt);
                ve_supply[t] = currentSupply < 0 ? 0 : uint256(currentSupply);
            }
            t += EPOCH;
        }
        time_cursor = t;
    ...
    function _claim(uint256 _tokenId, address ve, uint256 _last_token_time) internal returns (uint256) {
        ...
        if (week_cursor == 0) week_cursor = (user_point.ts + EPOCH - 1) / EPOCH * EPOCH;
        ...
        for (uint256 i = 0; i < 50; i++) {
            ...
            if (week_cursor >= user_point.ts && user_epoch <= max_user_epoch) {
                ...
                uint256 balance_of = int256(old_user_point.bias - dt * old_user_point.slope) < 0
                    ? 0
                    : uint256(int256(old_user_point.bias - dt * old_user_point.slope));
        ...
        user_epoch = FixedPointMathLib.min(max_user_epoch, user_epoch - 1);
    ...
    function _claimable(uint256 _tokenId, address ve, uint256 _last_token_time) internal view returns (uint256) {
        ...
        if (week_cursor == 0) {
            week_cursor = (user_point.ts + EPOCH - 1) / EPOCH * EPOCH;
        }
        ...
        for (uint256 i = 0; i < 50; i++) {
            ...
            if (week_cursor >= user_point.ts && user_epoch <= max_user_epoch) {
                ...
                int128 dt = int128(int256(week_cursor - old_user_point.ts));
                uint256 balance_of =
                    FixedPointMathLib.max(uint256(int256(old_user_point.bias - dt * old_user_point.slope)), 0);
                if (balance_of == 0 && user_epoch > max_user_epoch) break;
                if (balance_of != 0) {
                    to_distribute += balance_of * tokens_per_week[week_cursor] / ve_supply[week_cursor];
                }
    ...
    function claimable(uint256 _tokenId) external view returns (uint256) {
        uint256 _last_token_time = last_token_time / EPOCH * EPOCH;
    ...
    function claim(uint256 _tokenId) external returns (uint256) {
        ...
        if (amount != 0) {
            address owner = IVotingEscrow(voting_escrow).ownerOf(_tokenId);
    ...
    function claim_many(uint256[] memory _tokenIds) external returns (bool) {
        ...
        uint256 total = 0;
        IVotingEscrow ve = IVotingEscrow(_voting_escrow);
