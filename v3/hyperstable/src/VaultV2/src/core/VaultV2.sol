...
contract VaultV2 is ERC4626, OwnableRoles, IVault {
    ...
    uint8 private _assetDecimalOffset;

    event Take(address indexed to, uint256 amount);
    event NewPriceFeed(address oldPriceFeed, address newPriceFeed);

    constructor(address _asset, address _priceFeed, string memory _name, string memory _symbol) ERC4626(ERC20(_asset)) ERC20(_name, _symbol) {
