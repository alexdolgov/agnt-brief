    ...
    ***********************/
    ...
    function trade(
        ...
        if (optionLeg.amount < 0) {
            ...
            // trader buys existing oToken from the pool
            ...
            // send oToken to trader
            IERC20(oTokens[0]).safeTransfer(
                msg.sender,
                uint256(optionLeg.amount)
            );

            // update notional exposure
            if (optionLeg.isPut) {
                notionalExposure[order.underlying][true] -= uint256(
                    optionLeg.amount
                );
            } else {
                notionalExposure[order.underlying][false] -= uint256(
                    optionLeg.amount
                );
            }
        }
