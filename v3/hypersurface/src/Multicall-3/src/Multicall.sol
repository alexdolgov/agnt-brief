 ...
 */
contract Multicall is Multicall3, Initializable, IMulticall {
    // Contracts
    ITradingStorage public storageT;
    IExtendedPairInfos public pairInfos;
    IExtendedPairStorage public pairsStorage;
    IExtendedTrading public trading;
    ...
    /**
     * @notice Constructor to disable initialization on implementation contract
     */
    constructor() {
     ...
     * @dev This function is used to set the addresses of the storage and pair infos contracts
     */
    function initialize(
        address _storage,
        address _pairInfos,
        ...
        )
    {
        ...
        uint longMultiplier;
        uint shortMultiplier;

        // Get utilization threshold and borrow fees multiplier
        uint utilizationThreshold = pairsStorage.pairUtililizationThreshold(pairIndex);
        uint borrowFeesMultiplier = pairsStorage.pairBorrowFeesMultiplier(pairIndex);
        uint skewThreshold = pairsStorage.pairSkewThreshold(pairIndex);

        // If utilization ratio is less than utilization threshold, use borrow fees multiplier
        // Else, use the skew ratio to calculate the multipliers
        if (blendedUtilizationRatio <= utilizationThreshold) {
            longMultiplier = borrowFeesMultiplier;
            shortMultiplier = borrowFeesMultiplier;
        } else {
            ...
            longMultiplier =
                (precision * (blendedUtilizationRatio * skewRatio)) /
