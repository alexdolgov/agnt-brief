...
contract PairInfos is Initializable, IPairInfos {
     ...
     * @param pairIndex The index of the trading pair
     * @param value The value of the rollover fee per block
     */
    function setRolloverFeePerBlockPByKeeper(uint pairIndex, uint value) external onlyKeeper {
   ...
   /**
    ...
    function getTradeRolloverFee(
        ...
        bool isPnl = pairsStorage.getPosType(trader, pairIndex, index);

        TradeInitialAccFees memory t = tradeInitialAccFees[trader][pairIndex][index];
        if (!t.openedAfterUpdate || isPnl) {
            return 0;
        }
        ...
        (uint minBorrowFee, uint maxBorrowFee) = minMaxRollOverFee(trader, pairIndex, index, collateral, leverage);
    ...
    */
    function minMaxRollOverFee(
        ...
        minBorrowFee = pairsStorage.minBorrowFee(pairIndex) * timeSinceOpen * collateral.mul(leverage) / (_PRECISION * 365 days); 
        maxBorrowFee = pairsStorage.maxBorrowFee(pairIndex) * timeSinceOpen * collateral.mul(leverage) / (_PRECISION * 365 days); 
    }
