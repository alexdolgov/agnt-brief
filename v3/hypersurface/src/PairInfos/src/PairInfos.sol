...
contract PairInfos is Initializable, IPairInfos {
   ...
   /**
    ...
    function getTradeRolloverFee(
        ...
        uint minBorrowFee = minRollOverFee(trader, pairIndex, index, collateral, leverage);
        return minBorrowFee >=  borrowFee ? minBorrowFee : borrowFee;
    }

    /**
    @notice Compute the returns minimum roll over Fee for a trade based on its open Timestamp
     */
    function minRollOverFee(
        address trader,
        uint pairIndex,
        uint index,
        uint collateral,
        ...
        if(openTimestamp ==  0) return 0;

        uint minBorrowFee= pairsStorage.minBorrowFee(pairIndex);
        uint timeSinceOpen = block.timestamp - openTimestamp;

        return minBorrowFee * timeSinceOpen * collateral.mul(leverage) / (_PRECISION * 365 days); 
    }
