...
contract PairStorage is Initializable, IPairStorage {
   ...
   /** 
    ...
    /**
     * @notice Calculate the fee for opening a d position on a trading pair based on skew
     * @param _pairIndex The index of the trading pair.
    ...
    * @notice Resets Position type
    ...
    @param buy Long/Short
    @param pairIndex Index of Asset 
    @param isPnl Bool to identify if trade is pnl based
     */
    function correctSl(uint openPrice, uint leverage, uint sl, bool buy, uint pairIndex, bool isPnl) external view override returns (uint) {
        ...
        bool positiveSL = buy ? sl > openPrice : sl < openPrice;

        if(isPnl && sl !=0 && !positiveSL){
            uint minSlP = addtionalPairParams[pairIndex].minSlP;
            uint minSLDiff =((openPrice * uint(minSlP)).div(leverage)) / 100;
            uint minSl = buy ? openPrice - minSLDiff : openPrice + minSLDiff;
            if (minSlP > 0) {
                return buy ? sl < minSl ? sl : minSl : sl > minSl ? sl : minSl;
            }
        }
        return sl;
    }
