    ...
    */
    ...
    function executeMarketOrders(uint[] calldata orderId, bytes[] calldata priceUpdateData, IPriceAggregator.PriceSourcing _priceSourcing) external payable onlyOperator{
        ...
        for(uint i = 0; i< orderId.length; i++){
            storageT.priceAggregator().fulfill{value: msg.value}(orderId[i], priceUpdateData, _priceSourcing, 0);
        }
    }
    /** 
    ...
    function executeMarketOrders(
        ...
        for(uint i = 0; i< orderId.length; i++){
            storageT.priceAggregator().fulfill{value: msg.value}(orderId[i], priceUpdateData, _priceSourcing, spreadP);
        }
    }
    ...
    function _executeLimitOrder(        
        ...
        IPriceAggregator agg = storageT.priceAggregator();
        IExecute executor = agg.executions();

        // Determine order type before creating struct
        IPriceAggregator.OrderType orderType;
        {
            bool isPnl = agg.pairsStorage().getPosType(_trader, _pairIndex, _index);
        ...
        uint orderId = agg.getPrice(_pairIndex, orderType);
        ...
        agg.fulfill{value: msg.value}(orderId, priceUpdateData, _priceSourcing, spreadP);
    }
    /** 
