...
contract TradingStorage is Initializable, ITradingStorage {
    ...
    function unregisterPendingMarketOrder(uint _id, bool _open) external override onlyTrading {
        ...
        require(_order.block > block.number - 20, "ORDER_EXPIRED");
    ...
    function applyReferralAndPnlFee(
        ...
        if (_isPnl) {
            (lpFee, referrerRebate) = _applyPnlFee(_pairIndex, _collateral, _percentProfit);
        } else {
            (lpFee, referrerRebate) = _applyReferralFee(_trader, _fees, _leveragedPosition, _pairIndex);
        }
    }
    
    function _applyPnlFee(
        uint _pairIndex,
        uint _collateral,
        int _percentProfit
    ) internal view returns (uint feeAfterRebate, uint referrerRebate) {
    ...
    function _applyReferralFee(
        address _trader,
        uint _fees,
        uint _leveragedPosition,
        uint _pairIndex
    ) internal returns (uint feeAfterRebate, uint referrerRebate) {
        (uint feeAfter, address referrer, uint rebate) = referral.traderReferralDiscount(_trader, _fees);

        rebates[referrer] += rebate;
        ...
        emit TradeReferred(
            ...
            _leveragedPosition,
            feeAfter,
            _fees,              // “Need to take care of this change in Backend” — unchanged
            rebate,
            _pairIndex
        );

        return (feeAfter, rebate);
    }
