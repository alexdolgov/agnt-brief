...
interface IReferral {
    ...
    struct TraderDiscount {
        uint fixedFeeDiscountBps;
        uint pnlFeeDiscountBps;
    }
    ...
    event SetTraderDiscount(uint256 fixedFeeDiscountBps, uint256 pnlFeeDiscountBps);
    event SetDiscountKeeper(address keeper, bool status);
    ...
    event Discounted(address trader, uint256 feeDiscount, uint256 traderFeePostDiscount, uint256 discountBps, bool isPnl);
    ...
    function traderReferralDiscount(address _account, uint _feeBips, bool _isPnl) external returns (uint, address, uint);
