...
interface IVaultManagerExtended is IVaultManager {
    ...
    function junior() external view returns (address);
}
...
contract Tranche is ERC4626Upgradeable {
    ...
    IVaultManagerExtended public vaultManager;
    address public veTranche; // unused variable kept for storage compatibility
    ...
    event FeeTransferredToVM( uint256 fee, bool isDuringDeposit); 
    event Migrated(address indexed owner, address indexed toTranche, uint256 shares, uint256 assets);
    ...
    modifier onlyKeeper(){
        require(msg.sender == address(vaultManager.keeper()),"KEEPER_ONLY");
        _;
    }

    modifier onlyTranche(){
        require(msg.sender == address(vaultManager.junior()),"TRANCHE_ONLY");
        _;
    }
    ...
    function initialize(address __asset, address _vaultManager, string memory trancheName, string memory tranchSymbol) external initializer {
        vaultManager = IVaultManagerExtended(_vaultManager);
    ...
    function utilizationRatio() public view returns (uint256) {
        return (totalReserved * _PRECISION * 100) / 
                vaultManager.adjustedBalance(super.totalAssets());
    }
    ...
    function hasLiquidity(uint256 _reserveAmount) public view returns (bool) {
        return vaultManager.adjustedBalance(super.totalAssets()) > (_reserveAmount + totalReserved);
    }
    ...
    function _reserveBalance(uint256 amount) internal {
        require(vaultManager.adjustedBalance(super.totalAssets()) >= amount + totalReserved, "RESERVE_AMOUNT_EXCEEDS_AVAILABLE");
    ...
    function _deposit(address caller, address receiver, uint256 assets, uint256 shares) internal virtual override {
        ...
        if (fee > 0) {
            ...
            emit FeeTransferredToVM(fee, true);
        }
    }
    ...
    function maxRedeem(address owner) public view virtual override returns (uint256) {
        int maxAssets = int(vaultManager.adjustedBalance(super.totalAssets())) - int((totalReserved * _PRECISION * 100)/withdrawThreshold) ;
    ...
    /** --------------------------------- LP unification methods --------------------------------------------------------- */

    /**
    * @notice Method to be during LP unification to release all reserved OI
    * @notice This would let all the withdrawals and migration go through without hitting the utilization ratio.
     */
    function swallowReservedOI() external onlyTranche returns(uint256){
        swallowedOI = totalReserved;
        emit BalanceReleased(totalReserved);
        totalReserved = 0;
        return swallowedOI;
    }
    /**
    * @notice Method to be used during LP unification to revert the swallowed OI back to this tranche's total reserved.
    * @notice This would let the tranche go back to normal operations after unification is done
     */
    function unSwallowReservedOI() external onlyTranche returns(uint256) {
        totalReserved += swallowedOI;
        emit BalanceReserved(swallowedOI);
        swallowedOI = 0;
        return totalReserved;
    }
    /**
    * @notice Method to be used during LP unification to set the reserved OI to a specific amount
    * @notice Method used to undo in case things went south
     */
    function updateOIReserved(uint256 amount) external onlyGov{
        totalReserved = amount;
        emit BalanceReserved(amount);
    }

    /**
    * @notice Method to migrate all shares from this tranche to another tranche
    * @notice This assumes that fees would be off and totalReserved would be 0
    * @param _owner The address of the owner whose shares are to be migrated.
    * @param _tranche The address of the tranche to which shares are to be migrated.
     */
    function migrate(address _owner, address _tranche) external onlyKeeper{

        uint totalShares = balanceOf(_owner);
        require(totalShares > 0, "NO_SHARES");

        uint256 assets = previewRedeem(totalShares);

        SafeERC20.safeApprove(ERC20(asset()), _tranche , assets);
        ITranche(_tranche).deposit(assets, _owner);
        _burn(_owner, totalShares);
        emit TokenTransferred(_owner, address(0), totalShares, assets);

        emit Migrated(_owner, _tranche, totalShares, assets);
        emit Withdraw(_msgSender(), address(0), _owner, assets, totalShares);
    }

    /**
    * @notice Method to migrate all shares from this tranche to another tranche on behalf of multiple users
    * @notice This assumes that fees would be off and totalReserved would be 0
    * @param owners The address of the owners whose shares are to be migrated.
    * @param _tranche The address of the tranche to which shares are to be migrated.    
     */
    function migrateOnBehalf(address[] calldata owners, address _tranche) external onlyKeeper {
        for (uint i = 0; i < owners.length; i++) {

            address _owner = owners[i];
            uint totalShares = balanceOf(_owner);
            if(totalShares == 0) revert("NO_SHARES");
