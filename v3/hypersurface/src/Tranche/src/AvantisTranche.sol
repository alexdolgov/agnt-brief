...
interface ITrancheExtended is ITranche {
    function swallowReservedOI() external returns (uint256);
    function unSwallowReservedOI() external returns (uint256);
}

interface IVaultManagerExtended is IVaultManager {
    function keeper() external view returns (address);
    function senior() external view returns (address);
}
...
contract Tranche is ERC4626Upgradeable {
    ...
    event FeeTransferredToVM( uint256 fee, bool isDuringDeposit); // Confirm with Zellic if event can be added in upgraded contract
    ...
    function balancingFee(uint256 assets, bool isDeposit) public view returns (uint256) {
        return 0; // Since tranches are being merged. There is no rebalancing fee
    }
    ...
    /**
    * @notice Swallows the reserved OI from a senior tranche into this tranche's total reserved.
    */
    function swallowReservedOI(address _tranche) external onlyGov{
        uint swallowedOI = ITrancheExtended(_tranche).swallowReservedOI();
    ...
    /**
    * @notice Reverts the swallowed OI back to the senior tranche. To be used in case we are reverting Phase Two
    */
    function unSwallowReservedOI(address _tranche) external onlyGov {
        uint256 oiToUnSwallow = ITrancheExtended(_tranche).unSwallowReservedOI();
        totalReserved -= oiToUnSwallow;
        emit BalanceReleased(oiToUnSwallow);
    }

    /**
    * @notice Method to migrate some shares from this tranche to another tranche
    * @param _owner The address of the owner whose shares are to be migrated back to senior tranche
    * @param _shares The number of shares to migrate.
     */
    function migrate(address _owner, uint256 _shares) external onlyKeeper {

        uint totalShares = _shares;
        require(totalShares > 0, "NO_SHARES");

        feesOn = false;
        uint256 assets = previewRedeem(totalShares);

        SafeERC20.safeApprove(ERC20(asset()), address(vaultManager.senior()) , assets);
        ITranche(address(vaultManager.senior())).deposit(assets, _owner);
