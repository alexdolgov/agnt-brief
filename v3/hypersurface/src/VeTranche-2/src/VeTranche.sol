...
contract VeTranche is ERC721Upgradeable, ReentrancyGuardUpgradeable, IVeTranche {
    ...
    modifier onlyKeeper() {
        require(msg.sender == vaultManager.keeper() , "KEEPER_ONLY");
        _;
    }
    ...
    /**
     * @notice Unlocks a specific tokenId on behalf of the owner. Only callable by governance.
     * @param tokenId The ID of the token to unlock.
     */
    function unlockOnBehalf(uint256 tokenId) public nonReentrant onlyGov {
        require(tokensByTokenId[tokenId] > 0, "NOTHING_TO_UNLOCK");
        uint256 fee = checkUnlockFee(tokenId);

        _claimRewards(tokenId);

        tranche.transfer(_ownerOf(tokenId), tokensByTokenId[tokenId] - fee);
        tranche.transfer(address(vaultManager), fee);      
        emit Unlocked(tokenId, _ownerOf(tokenId), tokensByTokenId[tokenId], fee);
    ...
    /**
    @notice Force unlocks multiple tokens. 
    @param _tokenIds The IDs of the tokens to unlock.
     */
    function forceUnlocks(uint256[] memory _tokenIds) public onlyKeeper nonReentrant {
        for(uint i = 0; i < _tokenIds.length; i++){
            require(tokensByTokenId[_tokenIds[i]] > 0, "NOTHING_TO_UNLOCK");

            _claimRewards(_tokenIds[i]);
            tranche.transfer(_ownerOf(_tokenIds[i]), tokensByTokenId[_tokenIds[i]] );
            emit Unlocked(_tokenIds[i], _ownerOf(_tokenIds[i]), tokensByTokenId[_tokenIds[i]], 0);
            
            _burn(_tokenIds[i]);

            totalLockPoints -= tokensByTokenId[_tokenIds[i]]* lockMultiplierByTokenId[_tokenIds[i]];

            delete tokensByTokenId[_tokenIds[i]];
            delete rewardsByTokenId[_tokenIds[i]];
            delete lockTimeByTokenId[_tokenIds[i]];
            delete lockStartTimeByTokenId[_tokenIds[i]];
            delete lockMultiplierByTokenId[_tokenIds[i]];
            delete lastSharePoint[_tokenIds[i]];
        }
    }

    /**
    @notice Sweeps any ERC20 token held by the contract to a specified recipient.
     */
    function sweep(address token, address recipient) external onlyGov {
