...
interface IStakingDistributor { function stakeFor(address user, uint256 amount) external; }
...
contract PermalockVault is ReentrancyGuard, Ownable, Pausable, IERC721Receiver {
    ...
    constructor(
        ...
        // Allow executeNFTAction -> veAERO calls (merge, transfer, extend, etc.)
    ...
    /// @notice Can be toggled any time.
    ...
    function _mintLIQWithCapSplit(address user, uint256 liqToUser) private {
        ...
        // user mint + treasury top-up must fit in remaining
        // totalOut = liqToUser * (BPS_BASE + TREASURY_LIQ_BPS) / BPS_BASE
        ...
        if (primaryNFT == 0 || !_isNFTValid(primaryNFT)) {
            ...
            for (uint256 j = 0; j < len; j++) {
                ...
                if (cand != 0 && isManaged[cand] && _isNFTValid(cand)) {
                    primaryNFT = cand;
                    lastRebaseTime = block.timestamp;
                    additionalNFTs[j] = additionalNFTs[len - 1];
                    additionalNFTs.pop();
                    break;
                }
            }
