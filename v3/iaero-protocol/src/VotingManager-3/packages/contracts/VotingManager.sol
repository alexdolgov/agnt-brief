...
contract VotingManager is Ownable, ReentrancyGuard, Pausable {
    ...
    function addPool(address pool) external onlyOwner {
        ...
        if (gauge == address(0)) revert PoolNotWhitelisted(); // must have a gauge

        // Best-effort whitelist check; ignored if method missing
        try IVoter(voter).isWhitelisted(pool) returns (bool wh) {
            if (!wh) revert PoolNotWhitelisted();
        } catch { /* no-op if function absent */ }
