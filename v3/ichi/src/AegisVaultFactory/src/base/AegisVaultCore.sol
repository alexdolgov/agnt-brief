...
///     ZAM - Zero amount
///     ZBL - Zero balance
///     ICH - ICHI vault deposits closed/disabled
abstract contract AegisVaultCore is IAegisVault, ReentrancyGuard, AccessControl, Pausable {
    ...
    // Absolute tolerance (in raw token units) used for deposit rounding checks.
    // This is not intended to be governance-tunable: it is purely an allowance for integer division truncation.
    uint256 private constant MAX_DELTA = 10;
    ...
    /// @inheritdoc IAegisVaultState
    uint256 public override minDepositAegisShares;
    ...
    /// @inheritdoc IAegisVaultOwnerActions
    function setMinDepositAegisShares(uint256 _minDepositAegisShares) external override {
        _onlyAdmin();
        minDepositAegisShares = _minDepositAegisShares;
        emit SetMinDepositAegisShares(msg.sender, _minDepositAegisShares);
    }
        ...
        returns (uint256 aegisShares)
    {
        ...
        require(aegisShares >= minDepositAegisShares, "MDS");
        require(aegisShares >= minimumAegisShares, "SET");
    ...
    function checkUpkeep(bytes calldata checkData) external view override returns (bool upkeepNeeded, bytes memory performData) {
        // Don't trigger upkeep if ICHI vaults have deposits disabled
        if (!UV3Math.areICHIDepositsAllowed(depositVault, targetVault, isInverted)) {
    ...
    function _rebalance(uint256 rebalancePct) private {
        ...
        // check ICHI vaults accept deposits before attempting
        _checkICHIDepositsAllowed();
        ...
        returns (uint256 aegisShares)
    {
        ...
        UV3Math.DepositCtx memory ctx =
            UV3Math.prepareDepositCtx(depositVault, targetVault, depositSharesAmount, depositSpotPrice, targetSpotPrice);
        ...
        uint256 _maxDelta = MAX_DELTA;

        // "Meaningfully up" beyond tolerance on either token.
        bool token0Gt = ctx.userContributionToTotal0 > ctx.userDepositAmount0.add(_maxDelta);
        bool token1Gt = ctx.userContributionToTotal1 > ctx.userDepositAmount1.add(_maxDelta);

        // For the "down" leg we use a strict comparison: if one token is meaningfully up (> MAX_DELTA),
        // the other must be strictly down (even by 1 wei) to represent a real implied swap.
        bool isBalanced = (token0Gt && ctx.userContributionToTotal1 < ctx.userDepositAmount1)
            || (token1Gt && ctx.userContributionToTotal0 < ctx.userDepositAmount0);

        // NOTE: any invariant checks should never happen, but we enforce them nonetheless for safety guarantees
        /// @dev invariant check for guaranteed safety.
        /// - If implied swap happened: one token up, the other down (isBalanced)
        /// - Or both sides are within MAX_DELTA (isApproxEqual)
        /// - Otherwise: user cannot be up on either token beyond MAX_DELTA
        require(isBalanced || isApproxEqual || (!token0Gt && !token1Gt), "IMB");
        ...
        if (!isApproxEqual && isBalanced && doCheckImpliedSlippage) {
            ctx.userValueContributionU18 =
                UV3Math.applyImpliedSlippageAdjustment(ctx, token0Gt, targetSpotPrice, targetVault, quoter, token0, token1);
        }
    ...
    function _withdrawFromVault(IICHIVault ichiVault, uint256 ichiShares, address to)
        private
    ...
    function _isApproxEq(uint256 a, uint256 b) private pure returns (bool) {
    ...
    /// @dev Reverts if deposits are disabled on either ICHI vault
    function _checkICHIDepositsAllowed() private view {
        require(UV3Math.areICHIDepositsAllowed(depositVault, targetVault, isInverted), "ICH");
    }
}
