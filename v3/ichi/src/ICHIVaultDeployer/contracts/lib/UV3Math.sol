...
import { LiquidityAmounts } from
    "@velodrome-finance/concentrated-liquidity/contracts/periphery/libraries/LiquidityAmounts.sol";
import { OracleLibrary } from
    "@velodrome-finance/concentrated-liquidity/contracts/periphery/libraries/OracleLibrary.sol";
...
library UV3Math {
    ...
    function checkPriceManipulation(
        ...
        uint256 delta =
            (price > twap) ? price.sub(twap).mul(PRECISION).div(price) : twap.sub(price).mul(PRECISION).div(twap);
     ...
     */
    function getConservativePrice(uint256 spot, uint256 twap, uint256 auxTwap, bool isPool, uint32 auxTwapPeriod)
        public
        pure
        returns (uint256)
    {
     ...
     */
    function getPositionAmounts(uint256 positionId, address pool, address ichiVaultFactory)
        public
        view
        returns (uint128 liquidity, uint256 amount0, uint256 amount1)
    {
        ...
        (,,,,, int24 tickLower, int24 tickUpper, uint128 positionLiquidity,,, uint128 tokensOwed0, uint128 tokensOwed1)
        ...
        (amount0, amount1) = getAmountsForLiquidity(
            sqrtRatioX96, getSqrtRatioAtTick(tickLower), getSqrtRatioAtTick(tickUpper), positionLiquidity
        );
     ...
     * @return amountOut equivalent amount in _tokenOut
     */
    function fetchTwap(address _pool, address _tokenIn, address _tokenOut, uint32 _twapPeriod, uint256 _amountIn)
        public
        view
        returns (uint256 amountOut)
    {
    ...
    function collectRewards(
        ...
        if (lastGauge == NULL_ADDRESS) return; // No gauge, no rewards to collect
        ...
        uint256 rewarded = 0;
        address rewardToken = ILeafCLGauge(lastGauge).rewardToken();
        ...
        if (basePositionId != 0 && !isPositionOwnedByCaller(nftManager, basePositionId)) {
            try ILeafCLGauge(lastGauge).getReward(basePositionId) {
                rewarded = 1;
            } catch { }
        }
        if (limitPositionId != 0 && !isPositionOwnedByCaller(nftManager, limitPositionId)) {
            try ILeafCLGauge(lastGauge).getReward(limitPositionId) {
                rewarded = 1;
            } catch { }
        }
        ...
        if (farmingContract != NULL_ADDRESS && rewarded > 0) {
            ...
            if (token0 == rewardToken || token1 == rewardToken) {
                ...
                rewardAmount = balanceAfter > balanceBefore ? balanceAfter - balanceBefore : 0;
            } else {
                rewardAmount = IERC20(rewardToken).balanceOf(address(this));
            }

            if (rewardAmount > 0) {
                uint256 effectiveBaseFee = IICHIVault(address(this)).getEffectiveBaseFee();

                if (effectiveBaseFee > 0) {
                    // Calculate and distribute fees, then send remainder to farming contract
                    _distributeReward(ichiVaultFactory, rewardToken, rewardAmount, farmingContract, effectiveBaseFee);
                } else {
                    // No fee, send all to farming contract
    ...
    /**
     * @notice Helper function to distribute rewards with base fee applied
     * @param ichiVaultFactory Factory address
     * @param rewardToken Reward token address
     * @param rewardAmount Total reward amount
     * @param farmingContract Farming contract address
    ...
    function _distributeReward(
        ...
        uint256 feeAmount = rewardAmount.mul(baseFee).div(PRECISION);
        uint256 netReward = rewardAmount.sub(feeAmount);

        if (feeAmount > 0) {
            address affiliate = IICHIVault(address(this)).affiliate();
            uint256 baseFeeSplit =
                (affiliate == NULL_ADDRESS) ? PRECISION : IICHIVaultFactory(ichiVaultFactory).baseFeeSplit();

            uint256 toRecipient = feeAmount.mul(baseFeeSplit).div(PRECISION);

            IERC20(rewardToken).safeTransfer(IICHIVaultFactory(ichiVaultFactory).feeRecipient(), toRecipient);

            if (toRecipient < feeAmount) {
                IERC20(rewardToken).safeTransfer(affiliate, feeAmount.sub(toRecipient));
            }
        }

        if (netReward > 0) {
            IERC20(rewardToken).safeTransfer(farmingContract, netReward);
        }
    }
