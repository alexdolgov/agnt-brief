...
contract ICHIVaultFactory is IICHIVaultFactory, ReentrancyGuard, Ownable {
     ...
     */
    constructor(address _uniswapV3Factory, address _nftManager, string memory _ammName) {
        require(_uniswapV3Factory != NULL_ADDRESS && _nftManager != NULL_ADDRESS, "IVF.constructor: zero address");
     ...
     * @param tickSpacing tickSpacing setting of the Velodrome pool.
     *  @return ichiVault Address of the newly created ICHIVault.
     */
    function createICHIVault(address tokenA, bool allowTokenA, address tokenB, bool allowTokenB, int24 tickSpacing)
        external
        override
        nonReentrant
        returns (address ichiVault)
    {
        ...
        uint24 fee = ICLFactory(uniswapV3Factory).tickSpacingToFee(tickSpacing);
        require(fee != 0, "IVF.createICHIVault: fee incorrect");
        address pool = ICLFactory(uniswapV3Factory).getPool(tokenA, tokenB, tickSpacing);

        require(pool != NULL_ADDRESS, "IVF.createICHIVault: pool must exist");

        (,,,, uint16 observationCardinalityNext,) = ICLPool(pool).slot0();
        ...
        ichiVault = ICHIVaultDeployer.createICHIVault(
            pool, token0, allowToken0, token1, allowToken1, tickSpacing, DEFAULT_TWAP_PERIOD
        );
     ...
     * @param tickSpacing the uniswap v3 fee
