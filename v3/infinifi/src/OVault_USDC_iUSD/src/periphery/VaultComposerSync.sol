...
// This file is a fork from LayerZero VaultComposerSync.sol, with some minor modifications listed below :
// - Remove check that IOFT(SHARE_OFT).token() == VAULT in _initializeShareToken()
// - Remove check that IOFT(ASSET_OFT).token() == VAULT.asset() in _initializeAssetToken()
// These are only removing checks from the constructor, allowing more overrides in the logic.
...
contract VaultComposerSync is IVaultComposerSync, ReentrancyGuard {
    ...
    function _sendRemote(address _oft, SendParam memory _sendParam, address _refundAddress, uint256 _msgValue)
        ...
        virtual
    {
        IOFT(_oft).send{value: _msgValue}(
            _sendParam, MessagingFee({nativeFee: _msgValue, lzTokenFee: 0}), _refundAddress
        );
    }
