// SPDX-License-Identifier: MIT
pragma solidity 0.8.28;

import {IERC20} from "@openzeppelin/contracts/token/ERC20/IERC20.sol";
import {SafeERC20} from "@openzeppelin/contracts/token/ERC20/utils/SafeERC20.sol";
import {FixedPointMathLib} from "@solmate/src/utils/FixedPointMathLib.sol";
import {VaultComposerSync} from "@periphery/VaultComposerSync.sol";

import {GatewayLib} from "@libraries/GatewayLib.sol";
import {IInfiniFiGateway} from "@interfaces/IInfiniFiGateway.sol";

/// @title OVault Composer for USDC<->siUSD
/// @notice This is an OVault composer that allows minting and redeeming of USDC<->siUSD across
/// different chains using LayerZero's OFT protocol.
/// see https://docs.layerzero.network/v2/developers/evm/ovault/overview
contract OVault_USDC_siUSD is VaultComposerSync {
    using SafeERC20 for IERC20;
    using GatewayLib for IInfiniFiGateway;
    using FixedPointMathLib for uint256;

    IInfiniFiGateway public immutable gateway;
    IERC20 public immutable usdc;
    IERC20 public immutable iUSD;
    IERC20 public immutable siUSD;

    /// @dev set address(this) as the VaultComposerSync vault, and override/define
    /// the functions needed on the vault on this contract. This allows this composer to implement
    /// custom logic for minting and redeeming, without expecting a valid ERC4626 vault anywhere.
    /// This is needed because iusd isn't an ERC4626 vault of usdc, and we need to implement custom logic
    /// to perform minting, redeeming, and preview functions.
    constructor(address _gateway, address _usdcOFT, address _siUSDOFT)
        VaultComposerSync(address(this), _usdcOFT, _siUSDOFT)
    {
        gateway = IInfiniFiGateway(_gateway);
        usdc = IERC20(gateway.usdc());
        iUSD = IERC20(gateway.receiptToken());
        siUSD = IERC20(gateway.stakedToken());

        // Approve gateway to spend USDC, iUSD, and siUSD
        usdc.forceApprove(_gateway, type(uint256).max);
        iUSD.forceApprove(_gateway, type(uint256).max);
        siUSD.forceApprove(_gateway, type(uint256).max);
    }

    function decimals() public pure returns (uint8) {
        return 18; // siUSD decimals
    }

    function asset() public view returns (address) {
        return address(usdc);
    }

    function maxRedeem(address _who) public view returns (uint256) {
        return gateway.receiptToAsset(gateway.stakedToReceipt(siUSD.balanceOf(_who)));
    }

    function previewRedeem(uint256 _siUSDAmount) public view returns (uint256) {
        return gateway.receiptToAsset(gateway.stakedToReceipt(_siUSDAmount));
    }

    function maxDeposit(address) public pure returns (uint256) {
        return type(uint256).max;
    }

    function previewDeposit(uint256 _usdcAmount) public view returns (uint256) {
        return gateway.receiptToStaked(gateway.assetToReceipt(_usdcAmount));
    }

    function _deposit(bytes32, uint256 _usdcAmount) internal override returns (uint256) {
        uint256 balanceBefore = siUSD.balanceOf(address(this));
        gateway.mintAndStake(address(this), _usdcAmount);
        return siUSD.balanceOf(address(this)) - balanceBefore;
    }

    function _redeem(bytes32, uint256 _siUSDAmount) internal override returns (uint256) {
        uint256 usdcBalanceBefore = usdc.balanceOf(address(this));
        uint256 iUSDBalanceBefore = iUSD.balanceOf(address(this));
        gateway.unstake(address(this), _siUSDAmount);
        uint256 iUSDReceived = iUSD.balanceOf(address(this)) - iUSDBalanceBefore;
        gateway.redeem(address(this), iUSDReceived, gateway.receiptToAsset(iUSDReceived));
        return usdc.balanceOf(address(this)) - usdcBalanceBefore;
    }
}
