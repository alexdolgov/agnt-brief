...
contract MultiAccount is IMultiAccount, Initializable, PausableUpgradeable, AccessControlUpgradeable {
	 ...
	 * @param state Just for compatibility.
	 */
	function delegateAccess(address account, address target, bytes4 selector, bool state) external onlyOwner(account, msg.sender) {
		...
		require( state == true, "Use revoke to functions to revoke Access.");
	...
	function addAccountDelegateAccessAndCall(string memory name, address target, bytes4[] memory selector, bytes[] memory _callDatas) external returns( address account ) {
	...
	event SetAccountsAdmin( address oldAccountAdmin, address newAccountAdmin);
