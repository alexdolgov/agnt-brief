...
contract AeroFarmer {
    ...
    IGauge public constant dolaGauge = IGauge(0xCCff5627cd544b4cBb7d048139C1A6b6Bde67885); 
    IERC20 public constant LP_TOKEN = IERC20(0xf213F2D02837012dC0236cC105061e121bB03e37);
    address public constant aeroTokenAddr = 0x940181a94A35A4569E4529A3CDfB74e38FD98631;
    ...
    IERC20 public constant nUSDC = IERC20(0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913);  // native USDC
    ...
    function claimAeroRewards() external {
        ...
        IERC20(aeroTokenAddr).transfer(treasury, IERC20(aeroTokenAddr).balanceOf(address(this)));
    }
     ...
     */
    function withdrawToL1BaseFed(uint dolaAmount) external onlyRole(l1Chair, l2Chair) {
     ...
     */
    function withdrawToL1BaseFedNative(uint dolaAmount, uint usdcAmount) external onlyRole(l1Chair, l2Chair) {
        if (dolaAmount > DOLA.balanceOf(address(this))) revert NotEnoughTokens();
    ...
    function withdrawToL1BaseFedNative(uint usdcAmount) external onlyRole(l1Chair, l2Chair) {
        if (usdcAmount > nUSDC.balanceOf(address(this))) revert NotEnoughTokens();
     ...
     * @param usdcAmount Amount of USDC to withdraw and send to L1 BaseFed
     */
    function withdrawToL1BaseFedBridged(uint usdcAmount) external onlyRole(l1Chair, l2Chair) {
     ...
     * @dev l1Chair address should be set to the address of L1 AeroFarmerMessenger if it is being used
     ...
     */
    function changeBaseFed(address newBaseFed_) external onlyRole(gov, address(0)) {
        baseFed = newBaseFed_;
    }
}
