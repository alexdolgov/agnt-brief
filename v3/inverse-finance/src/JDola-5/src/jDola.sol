...
contract JDola is ERC4626 {
    ...
    //10000 = 1 DBR per sDOLA max. At 0.05$ per DBR, the initial params sets the max DBR price to 0.5$
    uint public maxDbrDolaRatioBps = 10 * 10000;
    ...
    function buyDbr(uint exactDolaIn, uint exactDbrOut, address to) external updateReserves {
        ...
        require(dbrReserve * 10_000 / dolaReserve <= maxDbrDolaRatioBps, "Above max ratio");
        donate(exactDolaIn);
     ...
     * @param amount Amount of sDOLA needed to repay bad debt
     * @return `amount` of available sDOLA
     */
    ...
    /**
     * @dev Sets the max Dbr/sDola ratio
     * @param _maxDbrDolaRatioBps The max Dbr/Dola ratio in bps
     */
    function setMaxDbrDolaRatioBps(uint _maxDbrDolaRatioBps) external onlyOperator {
        maxDbrDolaRatioBps = _maxDbrDolaRatioBps;
        emit SetMaxDbrDolaRatioBps(_maxDbrDolaRatioBps);
    }
    ...
    event SetMaxDbrDolaRatioBps(uint newBps);
