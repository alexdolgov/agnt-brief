...
contract JrDolaUSDFeed {
    ...
    /**
     * @dev Get the jrDola/USD price by using the jrDola/DOLA price and the DOLA/USD price
     */
    function _getPrice() internal view returns (int256) {
        int256 dolaPerJrDola = jrDolaToDolaFeed.latestAnswer();
        int256 dolaUsdPrice = dolaUsdFeed.latestAnswer();

        return dolaPerJrDola * dolaUsdPrice / 1e18;
    }
}
