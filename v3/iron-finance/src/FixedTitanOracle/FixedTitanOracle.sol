...
// File contracts/oracle/FixedTitanOracle.sol

pragma solidity 0.8.4;

contract FixedTitanOracle is Ownable, IOracle {
    ...
    function consult() external view override returns (uint256) {
        ...
        uint256 _price = (_priceTokenToUsd * _priceShareToToken) / PRICE_PRECISION;
        if (_price == 0) {
            _price = 1; // make it a minimum price as the current titan
        }
        return _price;
    }
