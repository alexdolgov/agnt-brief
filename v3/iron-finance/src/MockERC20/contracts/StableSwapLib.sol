...
library StableSwapLib {
    ...
    function removeLiquidity(
        ...
        self.lpToken.burn(msg.sender, lpAmount);
    ...
    function removeLiquidityImbalance(
        ...
        D1 = get_D(_xp(newBalances, self.tokenMultipliers), amp);
        uint256 burnAmount = ((D0 - D1) * totalSupply) / D0;
        assert(burnAmount > 0);
        burnAmount += 1; //In case of rounding errors - make it unfavorable for the "attacker"
        require(burnAmount <= maxBurnAmount, "> slippage");

        self.lpToken.burn(msg.sender, burnAmount);

        for (uint256 i = 0; i < nCoins; i++) {
            if (amounts[i] != 0) {
                self.pooledTokens[i].safeTransfer(address(this), amounts[i]);
            }
        }
