...
// File contracts/libs/SpotPriceGetter.sol

pragma solidity 0.8.4;

contract SpotPriceGetter is Ownable {
    address public lpPar;
    ...
    function consult() external view returns (uint256) {
        IUniswapLP pair = IUniswapLP(lpPar);
        ...
        if (token == _token0) {
            _price = (_reserve1 * _priceBnbUsd) / _reserve0;
        } else {
            _price = (_reserve0 * _priceBnbUsd) / _reserve1;
        }
        return _price;
    }
    ...
    function setlpPar(address _lpPar) external onlyOwner {
        lpPar = _lpPar;
    }
}