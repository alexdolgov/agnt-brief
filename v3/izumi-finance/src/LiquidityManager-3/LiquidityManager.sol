...
// File contracts/core/interfaces/ILorenzoExFactory.sol

// Original license: SPDX_License_Identifier: BUSL-1.1
pragma solidity ^0.8.4;

interface ILorenzoExFactory {

    /// @notice emit when successfuly create a new pool (calling LorenzoExFactory#newPool)
...
abstract contract Base {
    /// @notice address of LorenzoExFactory
    ...
    /// @param _factory address of LorenzoExFactory
    ...
    function pool(address tokenX, address tokenY, uint24 fee) public view returns(address) {
        return ILorenzoExFactory(factory).pool(tokenX, tokenY, fee);
    }
...
// File contracts/core/interfaces/ILorenzoExCallback.sol

// Original license: SPDX_License_Identifier: BUSL-1.1
pragma solidity ^0.8.4;

interface ILorenzoExMintCallback {

    /// @notice Called to msg.sender in LorenzoExPool#mint call
    /// @param x Amount of tokenX need to pay from miner
    /// @param y Amount of tokenY need to pay from miner
    /// @param data Any data passed through by the msg.sender via the LorenzoExPool#mint call
...
interface ILorenzoExCallback {

    /// @notice Called to msg.sender in LorenzoExPool#swapY2X(DesireX) call
    /// @param x Amount of tokenX trader will acquire
    /// @param y Amount of tokenY trader will pay
    /// @param data Any dadta passed though by the msg.sender via the LorenzoExPool#swapY2X(DesireX) call
    ...
    /// @notice Called to msg.sender in LorenzoExPool#swapX2Y(DesireY) call
    /// @param x Amount of tokenX trader will pay
    /// @param y Amount of tokenY trader will require
    /// @param data Any dadta passed though by the msg.sender via the LorenzoExPool#swapX2Y(DesireY) call
...
interface ILorenzoExAddLimOrderCallback {

    /// @notice Called to msg.sender in LorenzoExPool#addLimOrderWithX(Y) call
    /// @param x Amount of tokenX seller will pay
    /// @param y Amount of tokenY seller will pay
    /// @param data Any dadta passed though by the msg.sender via the LorenzoExPool#addLimOrderWithX(Y) call
...
// File contracts/core/interfaces/ILorenzoExPool.sol

// Original license: SPDX_License_Identifier: BUSL-1.1
pragma solidity ^0.8.4;

interface ILorenzoExPool {
    ...
    /// @dev The caller needs to implement a ILorenzoExPool#flashCallback callback function
...
contract LiquidityManager is Ownable, Base, ERC721Enumerable, ILorenzoExMintCallback {
    ...
    // callback data passed through LorenzoExPool#mint to the callback
    struct MintCallbackData {
        ...
        // fee amount of swap
        uint24 fee;
        // address to pay tokenX and tokenY to LorenzoExPool
        address payer;
    }
    ...
    /// @param factory address of LorenzoExFactory
    /// @param weth address of WETH token
    constructor(
        address factory,
        address weth
    ) ERC721("LorenzoEx Liquidity NFT", "LORENZOEX-LIQUIDITY-NFT") Base(factory, weth) {
    }
    ...
    function createPool(address tokenX, address tokenY, uint24 fee, int24 initialPoint) external returns (address) {
        ...
        address pool = ILorenzoExFactory(factory).pool(tokenX, tokenY, fee);
        if (pool == address(0)) {
            pool = ILorenzoExFactory(factory).newPool(tokenX, tokenY, fee, initialPoint);
            return pool;
        }
        return pool;
    }
    ...
    function getLastFeeScale(address pool, bytes32 key) private view returns(uint256, uint256) {
        ...
        (, uint256 lastFeeScaleX_128, uint256 lastFeeScaleY_128, , ) = ILorenzoExPool(pool).liquidity(
            key
        );
    ...
    function _addLiquidity(MintParam memory mp) private returns(
        ...
        int24 currPt;
        uint160 sqrtPrice_96;
        pool = ILorenzoExFactory(factory).pool(mp.tokenX, mp.tokenY, mp.fee);
        uint160 sqrtRate_96 = ILorenzoExPool(pool).sqrtRate_96();
        ...
        (amountX, amountY) = ILorenzoExPool(pool).mint(address(this), mp.pl, mp.pr, liquidity, 
    ...
    function addLiquidity(
        ...
        int24 currPt;
        uint160 sqrtPrice_96;
        address pool = ILorenzoExFactory(factory).pool(poolMeta.tokenX, poolMeta.tokenY, poolMeta.fee);
        // uint160 sqrtRate_96 = ILorenzoExPool(pool).sqrtRate_96();
        require(pool != address(0), "P0");
        (sqrtPrice_96, currPt) = getPoolPrice(pool);
        liquidityDelta = MintMath.computeLiquidity(
            ...
            sqrtPrice_96,
            // sqrtRate_96
            ILorenzoExPool(pool).sqrtRate_96()
        );
        require(int128(liquid.liquidity) == int256(uint256(liquid.liquidity)), "LO");
        uint128 newLiquidity = liquidityDelta + liquid.liquidity;
        (amountX, amountY) = ILorenzoExPool(pool).mint(address(this), liquid.leftPt, liquid.rightPt, liquidityDelta, 
    ...
    function decLiquidity(
        ...
        (amountX, amountY) = ILorenzoExPool(pool).burn(liquidity.leftPt, liquidity.rightPt, liquidDelta);
    ...
    function collect(
        ...
        require(pool != address(0), "P0");
        if (liquidity.liquidity > 0) {
            ILorenzoExPool(pool).burn(liquidity.leftPt, liquidity.rightPt, 0);
        ...
        (amountX, amountY) = ILorenzoExPool(pool).collect(recipient, liquidity.leftPt, liquidity.rightPt, amountXLim, amountYLim);
