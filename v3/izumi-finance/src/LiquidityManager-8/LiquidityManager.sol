...
// File contracts/core/interfaces/IBigBangSwapFactory.sol

// Original license: SPDX_License_Identifier: BUSL-1.1
pragma solidity ^0.8.4;

interface IBigBangSwapFactory {

    /// @notice emit when successfuly create a new pool (calling BigBangSwapFactory#newPool)
...
abstract contract Base {
    /// @notice address of BigBangSwapFactory
    ...
    /// @param _factory address of BigBangSwapFactory
    ...
    function pool(address tokenX, address tokenY, uint24 fee) public view returns(address) {
        return IBigBangSwapFactory(factory).pool(tokenX, tokenY, fee);
    }
...
// File contracts/libraries/MintMath.sol
...
// File contracts/core/interfaces/IBigBangSwapCallback.sol

// Original license: SPDX_License_Identifier: BUSL-1.1
pragma solidity ^0.8.4;

interface IBigBangSwapMintCallback {

    /// @notice Called to msg.sender in BigBangSwapPool#mint call
    /// @param x Amount of tokenX need to pay from miner
    /// @param y Amount of tokenY need to pay from miner
    /// @param data Any data passed through by the msg.sender via the BigBangSwapPool#mint call
...
interface IBigBangSwapCallback {

    /// @notice Called to msg.sender in BigBangSwapPool#swapY2X(DesireX) call
    /// @param x Amount of tokenX trader will acquire
    /// @param y Amount of tokenY trader will pay
    /// @param data Any dadta passed though by the msg.sender via the BigBangSwapPool#swapY2X(DesireX) call
    ...
    /// @notice Called to msg.sender in BigBangSwapPool#swapX2Y(DesireY) call
    /// @param x Amount of tokenX trader will pay
    /// @param y Amount of tokenY trader will require
    /// @param data Any dadta passed though by the msg.sender via the BigBangSwapPool#swapX2Y(DesireY) call
...
interface IBigBangSwapAddLimOrderCallback {

    /// @notice Called to msg.sender in BigBangSwapPool#addLimOrderWithX(Y) call
    /// @param x Amount of tokenX seller will pay
    /// @param y Amount of tokenY seller will pay
    /// @param data Any dadta passed though by the msg.sender via the BigBangSwapPool#addLimOrderWithX(Y) call
...
// File contracts/core/interfaces/IBigBangSwapPool.sol

// Original license: SPDX_License_Identifier: BUSL-1.1
pragma solidity ^0.8.4;

interface IBigBangSwapPool {
    ...
    /// @dev The caller needs to implement a IBigBangSwapPool#flashCallback callback function
...
contract LiquidityManager is Ownable, Base, ERC721Enumerable, IBigBangSwapMintCallback {
    ...
    // callback data passed through BigBangSwapPool#mint to the callback
    struct MintCallbackData {
        ...
        // fee amount of swap
        uint24 fee;
        // address to pay tokenX and tokenY to BigBangSwapPool
        address payer;
    }
    ...
    /// @param factory address of BigBangSwapFactory
    /// @param weth address of WETH token
    constructor(
        address factory,
        address weth
    ) ERC721("BigBangSwap Liquidity NFT", "BIGBANGSWAP-LIQUIDITY-NFT") Base(factory, weth) {
    }
    ...
    function createPool(address tokenX, address tokenY, uint24 fee, int24 initialPoint) external returns (address) {
        ...
        address pool = IBigBangSwapFactory(factory).pool(tokenX, tokenY, fee);
        if (pool == address(0)) {
            pool = IBigBangSwapFactory(factory).newPool(tokenX, tokenY, fee, initialPoint);
            return pool;
        }
        return pool;
    }
    ...
    function getLastFeeScale(address pool, bytes32 key) private view returns(uint256, uint256) {
        ...
        (, uint256 lastFeeScaleX_128, uint256 lastFeeScaleY_128, , ) = IBigBangSwapPool(pool).liquidity(
            key
        );
    ...
    function _addLiquidity(MintParam memory mp) private returns(
        ...
        int24 currPt;
        uint160 sqrtPrice_96;
        pool = IBigBangSwapFactory(factory).pool(mp.tokenX, mp.tokenY, mp.fee);
        uint160 sqrtRate_96 = IBigBangSwapPool(pool).sqrtRate_96();
        ...
        (amountX, amountY) = IBigBangSwapPool(pool).mint(address(this), mp.pl, mp.pr, liquidity, 
    ...
    function addLiquidity(
        ...
        int24 currPt;
        uint160 sqrtPrice_96;
        address pool = IBigBangSwapFactory(factory).pool(poolMeta.tokenX, poolMeta.tokenY, poolMeta.fee);
        // uint160 sqrtRate_96 = IBigBangSwapPool(pool).sqrtRate_96();
        require(pool != address(0), "P0");
        (sqrtPrice_96, currPt) = getPoolPrice(pool);
        liquidityDelta = MintMath.computeLiquidity(
            ...
            sqrtPrice_96,
            // sqrtRate_96
            IBigBangSwapPool(pool).sqrtRate_96()
        );
        require(int128(liquid.liquidity) == int256(uint256(liquid.liquidity)), "LO");
        uint128 newLiquidity = liquidityDelta + liquid.liquidity;
        (amountX, amountY) = IBigBangSwapPool(pool).mint(address(this), liquid.leftPt, liquid.rightPt, liquidityDelta, 
    ...
    function decLiquidity(
        ...
        (amountX, amountY) = IBigBangSwapPool(pool).burn(liquidity.leftPt, liquidity.rightPt, liquidDelta);
    ...
    function collect(
        ...
        require(pool != address(0), "P0");
        if (liquidity.liquidity > 0) {
            IBigBangSwapPool(pool).burn(liquidity.leftPt, liquidity.rightPt, 0);
        ...
        (amountX, amountY) = IBigBangSwapPool(pool).collect(recipient, liquidity.leftPt, liquidity.rightPt, amountXLim, amountYLim);
