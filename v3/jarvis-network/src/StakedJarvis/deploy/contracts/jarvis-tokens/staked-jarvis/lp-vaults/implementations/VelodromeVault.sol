...
contract VelodromeVault is ILPVaultImplementation {
    ...
    )
  {
    ...
    if (
      ...
      (
        ...
        lpTokenOut
      ) = IVelodromeRouter(moduleArgs.routerAddress).addLiquidityETH{
        value: msg.value
      }(
      ...
      // refund ETH
      uint256 amountInput = isTokenANative
        ? cache.amountInTokenA
        : cache.amountInTokenB;
      if (amountInput < msg.value) {
        uint256 refundAmountEth = msg.value - amountInput;
      ...
      // refund token ERC20
      amountInput = isTokenANative
        ? cache.amountInTokenB
        : cache.amountInTokenA;
      jarvisAmount = amountInput;
      if (amountInput < cache.nonNativeInputAmount) {
        uint256 refundAmount = cache.nonNativeInputAmount - amountInput;
      ...
      (
        ...
        cache.amountInTokenB,
        lpTokenOut
      ) = IVelodromeRouter(moduleArgs.routerAddress).addLiquidity(
        cache.tokenA,
        cache.tokenB,
        IPair(stakeArgs.pair).stable(),
    ...
    )
  {
    ...
    if (
      ...
      bool isTokenANative = cache.tokenA == NATIVE;
      (amountsOut[0], amountsOut[1]) = IVelodromeRouter(
        moduleArgs.routerAddress
      ).removeLiquidityETH(
        isTokenANative ? cache.tokenB : cache.tokenA,
  ...
  function _revertWrongPool(address routerAddr, address pair) internal view {
    IVelodromeRouter router = IVelodromeRouter(routerAddr);
    require(router.isPair(pair), 'Wrong pool');
  }
}
