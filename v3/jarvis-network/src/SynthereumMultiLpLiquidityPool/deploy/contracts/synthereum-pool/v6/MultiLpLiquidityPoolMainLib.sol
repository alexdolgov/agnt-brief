...
library SynthereumMultiLpLiquidityPoolMainLib {
  ...
  struct PositionLPInfoArgs {
    ...
    uint256 tokensValue;
    uint256 maxCapacity;
    uint256 lpInterestsShare;
    uint256 totalInterestsShare;
    uint8 decimals;
  }
  ...
  function setOvercollateralization(
    ...
    (
      uint256 poolInterest,
      uint256 collateralDeposited
    ) = SynthereumMultiLpLiquidityPoolLib._getUpdatedInterest(
      SynthereumMultiLpLiquidityPoolLib._getLendingManager(_finder)
    );
  ...
  function transferToLendingManager(
    ...
    ) = SynthereumMultiLpLiquidityPoolLib._getUpdatedInterest(lendingManager);
  ...
  function positionLPInfo(
    ...
    for (uint256 j = 0; j < positionsCache.length; j++) {
      ...
      if (positionsCache[j].lp == _lp) {
        ...
        info.capacity = SynthereumMultiLpLiquidityPoolLib._calculateCapacity(
          lpPosition,
          positionLPInfoArgs.price,
          positionLPInfoArgs.decimals
        );
        ...
        info.utilization = lpPosition.actualCollateralAmount != 0
          ...
          ? PreciseUnitMath.PRECISE_UNIT
          : 0;
        positionLPInfoArgs.totalCapacity += info.capacity;
        (
        ...
        info.redeemShares = positionLPInfoArgs.totalSynthTokens != 0
          ...
          : 0;
        positionLPInfoArgs.lpInterestsShare =
          SynthereumMultiLpLiquidityPoolLib._calculateNumberOfTokens(
            lpPosition.actualCollateralAmount,
            positionLPInfoArgs.price,
            positionLPInfoArgs.decimals
          ) +
          lpPosition.tokensCollateralized;
        positionLPInfoArgs.totalInterestsShare += positionLPInfoArgs
          .lpInterestsShare;
      } else {
        ...
        positionLPInfoArgs.totalInterestsShare +=
          SynthereumMultiLpLiquidityPoolLib._calculateNumberOfTokens(
            lpPosition.actualCollateralAmount,
            positionLPInfoArgs.price,
            positionLPInfoArgs.decimals
    ...
    info.mintShares = positionLPInfoArgs.totalCapacity != 0
      ? info.capacity.div(positionLPInfoArgs.totalCapacity)
      : 0;
    info.interestShares = positionLPInfoArgs.totalInterestsShare != 0
      ? positionLPInfoArgs.lpInterestsShare.div(
        positionLPInfoArgs.totalInterestsShare
      )
      : 0;
    return info;
