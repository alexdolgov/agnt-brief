...
library SynthereumMultiLpLiquidityPoolLib {
  ...
  /**
   * @notice Calculate and returns interest generated by the pool from the last update triggered by this function
   ...
   */
  function _getUpdatedInterest(ILendingManager _lendingManager)
    internal
    returns (uint256 poolInterests, uint256 collateralDeposited)
  {
    ILendingManager.ReturnValues memory lendingValues = _lendingManager
      .updateAccumulatedInterest();
    poolInterests = lendingValues.poolInterest;
    collateralDeposited = lendingValues.prevTotalCollateral;
  }
    ...
    returns (uint256 prevTotalLPsCollateral, uint256 mostFundedIndex)
  {
    ...
    uint256[] memory interestsShares = new uint256[](lpNumbers);
    ...
    if (totalInterestShares > 0) {
      for (uint256 j = 0; j < lpNumbers; j++) {
        interestsAmount = _totalInterests.mul(interestsShares[j]).div(
          totalInterestShares
        );
        _positionsCache[j].lpPosition.actualCollateralAmount += interestsAmount;
        remainingInterest -= interestsAmount;
      }
      _positionsCache[mostFundedIndex]
        .lpPosition
        .actualCollateralAmount += remainingInterest;
    }
  }
   ...
   * @param _collateralDecimals Decimals of the collateral token
   * @param _positionsCache Temporary memory cache containing LPs positions
   * @param _interestsShares Array to be populated with the interest shares of every LP
   * @return totalInterestShares Sum of all the LP's interest shares
   * @return totalLPsCollateral Sum of all the LP's collaterals
   * @return mostFundedIndex Index in the positionsCache of the LP collateralizing more money
   */
  function _calculateInterestShares(
    ...
    uint8 _collateralDecimals,
    PositionCache[] memory _positionsCache,
    uint256[] memory _interestsShares
  )
    internal
    view
    returns (
    ...
    )
  {
    ...
    ISynthereumMultiLpLiquidityPool.LPPosition memory lpPosition;
    BestShare memory bestShare;

    for (uint256 j = 0; j < _positionsCache.length; j++) {
      lp = _storageParams.activeLPs.at(j);

      lpPosition = _storageParams.lpPositions[lp];
      interestsShare =
        ...
        lpPosition.tokensCollateralized;
      _interestsShares[j] = interestsShare;
      totalInterestShares += interestsShare;
      _positionsCache[j] = PositionCache(lp, lpPosition);
      totalLPsCollateral += lpPosition.actualCollateralAmount;
      bestShare = interestsShare <= bestShare.share
        ? bestShare
        : BestShare(interestsShare, j);
    }
    mostFundedIndex = bestShare.index;
  }
