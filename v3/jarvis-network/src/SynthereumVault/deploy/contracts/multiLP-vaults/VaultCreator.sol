...
contract SynthereumVaultCreator {
  ...
  {
    uint8 version = decodeParams(encodedParams);
    require(version == 2, 'Wrong version');
    encodedCall = abi.encodeWithSelector(IVault.reinitialize.selector);
  }

  /**
   * @notice Decodes constructor parameters into proper types
   * @param encodedParams ABI encoded parameters for constructor
   * @return vesrion of the vault
  ...
  function decodeParams(bytes memory encodedParams)
    ...
    returns (uint8)
  {
    return abi.decode(encodedParams, (uint8));
  }
