...
import { BlastGas } from "contracts/juice/periphery/BlastGas.sol";
...
contract JuiceAccount is InternalAccount, BlastGas {
    ...
    function initialize(address owner_) public virtual override initializer {
        ...
        address _protocolGovernor = ProtocolModule(address(_manager)).getProtocolGovernor();
        __BlastGas_init(_protocolGovernor);

        // Configure Blast Yield
        IERC20Rebasing(address(asset)).configure(YieldMode.CLAIMABLE);
    ...
    function claimYield() external {
        uint256 claimableYield = IERC20Rebasing(address(asset)).getClaimableAmount(address(this));
        address protocolFeeCollector = IProtocolGovernor(ProtocolModule(address(_manager)).getProtocolGovernor())
            .getAddress(GovernorLib.FEE_COLLECTOR);

        if (protocolFeeCollector == address(0)) {
            revert Errors.InvalidParams();
        }

        IERC20Rebasing(address(asset)).claim(protocolFeeCollector, claimableYield);
    }
}
