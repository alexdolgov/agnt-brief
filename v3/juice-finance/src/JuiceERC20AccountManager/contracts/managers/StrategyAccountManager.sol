...
abstract contract StrategyAccountManager is AccountManager, StrategyAccountManagerEvents {
    ...
    /// @notice Add only mapping for tracking all strategies that have been approved
    mapping(address => bool) public liquidatableStrategies;
        ...
        returns (uint256 shares)
    {
        ...
        // TODO: See if check borrow is required here.
    ...
    /// @dev Caller should be the sub-account contract.
    function _strategyWithdrawal(address caller, address owner, address strategy, uint256, bool didRepay) internal {
        ...
        if (didRepay) {
            ...
            if (lastHealthFactor > currentHealthFactor && getAccountHealth(caller).debtAmount > 0) {
    ...
    {
        if (!liquidatableStrategies[strategy]) {
            revert Errors.StrategyNotLiquidatable();
        }
    ...
    function updateStrategyApproval(address strategy, bool approval) external onlyOwner {
        ...
        if (!liquidatableStrategies[strategy] && approval) {
            liquidatableStrategies[strategy] = approval;
        }
