...
abstract contract StrategyVault is IStrategyVault, Context, ERC20, ProtocolModule, Pausable, StrategyVaultEvents {
        ...
        returns (uint256 receivedShares)
    {
        ...
        receivedShares = _deposit(assets, data, recipient);
    }
    ...
    function withdraw(uint256 shares, bytes memory data) external payable virtual returns (uint256 receivedAmount) {
        ...
        _totalBaseDeposit = _totalBaseDeposit.zeroFloorSub(receivedAmount);

        if (balanceOf(msg.sender) > 0) {
            _baseDepositAmounts[msg.sender] = _baseDepositAmounts[msg.sender].zeroFloorSub(receivedAmount);
        } else {
            _baseDepositAmounts[msg.sender] = 0;
        }
    }
        ...
        returns (uint256 receivedAssets)
    {
        ...
        // Encode minAmount and ignore data if it is empty.
        if (keccak256(data) == keccak256("")) {
            receivedAssets = _withdraw(msg.sender, totalShares, abi.encode(minAmountAfterSlippage), receiver);
        } else {
