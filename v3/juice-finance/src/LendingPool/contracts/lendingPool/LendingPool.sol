...
import { AccountLib } from "contracts/libraries/accounts/AccountLib.sol";
...
contract LendingPool is Pausable, ILendingPool, IFlashLoanLender, ProtocolModule, LendingPoolEvents, ReentrancyGuard {
    ...
    function setMinimumOpenBorrow(uint256 minimumOpenBorrow) external onlyOwner {
        ...
        emit MinimumBorrowUpdated(minimumOpenBorrow);
    }
        ...
        returns (uint256)
    {
        ...
        // Enforce deposit cap after amount has been added to reserve and interest has been accrued
        if (depositCap != type(uint256).max && getTotalSupply() > depositCap) {
    ...
    function borrow(
        ...
        returns (uint256)
    {
        if (amount > reserve.assetBalance) {
    ...
    function repay(
        ...
        returns (uint256)
    {
        return _repay(amount, msg.sender, onBehalfOf, onBehalfOf);
    }
    ...
    function repay(
        ...
        returns (uint256)
    {
        return _repay(amount, msg.sender, onBehalfOf, from);
    }
    ...
    function _repay(uint256 amount, address sender, address borrower, address from) internal returns (uint256) {
        _beforeAction();
        ...
        bool isLiquidatableAccount = false;

        // Check if it is an Account Manager facilitating a repay for a liquidatable account.
        if (_contractExists(sender)) {
            try IAccountManager(sender).getAccountLiquidationStatus(borrower) returns (
                AccountLib.LiquidationStatus memory status
            ) {
                isLiquidatableAccount = status.isLiquidating;
            } catch { }
        }

        if (!isLiquidatableAccount && remainingDebt > 0 && remainingDebt < _minimumOpenBorrow) {
            revert Errors.InvalidMinimumOpenBorrow();
        }
