    ...
    IBeacon
{
    ...
    function allowlistRoute(address v1Vault, address v2Vault) external onlyRolesOrOwner(Constants.MANAGER_ROLE) {
        _self().allowlistedRoutes[v1Vault][v2Vault] = true;
    }

    function denylistRoute(address v1Vault, address v2Vault) external onlyRolesOrOwner(Constants.MANAGER_ROLE) {
        _self().allowlistedRoutes[v1Vault][v2Vault] = false;
    }
    ...
    function migrateToV2(IVault v1Vault, IVault2 v2Vault, uint256 oldShares, uint256 minNewShares)
        ...
        whenNotPaused
    {
        if (!_self().allowlistedRoutes[address(v1Vault)][address(v2Vault)]) {
            revert RouteNotAllowlisted();
        }
    ...
    function allowlistedRoutes(address v1Vault, address v2Vault) public view returns (bool) {
        return _self().allowlistedRoutes[v1Vault][v2Vault];
    }
