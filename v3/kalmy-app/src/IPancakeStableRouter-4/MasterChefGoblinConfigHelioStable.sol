...
// File: contracts/MasterChefGoblinConfigHelioStable.sol
...
contract MasterChefGoblinConfigHelioStable is Ownable, GoblinConfig {
    ...
    function isStable(address _goblin) public view returns (bool) {
        IMasterChefGoblin goblin = IMasterChefGoblin(_goblin);
        IPancakeStableRouter router = IPancakeStableRouter(goblin.router());
        // 1. Check that price is in the acceptable range
        address fToken = goblin.fToken();
        address baseToken = goblin.baseToken();
        (address token0, address token1) = sortTokens(fToken, baseToken);
        (uint256 price, uint256 lastUpdate) = oracle.getPrice(token0, token1);
        require(lastUpdate >= now - 7 days, "price too stale");
        uint256 lpPrice = router.get_virtual_price();
        uint256 maxPriceDiff = goblins[address(goblin)].maxPriceDiff;
