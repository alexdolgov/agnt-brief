...
contract Partnership is RoutingManagement {
    ...
    */
    ...
    constructor() public {
        Partner memory partner = Partner(msg.sender, 10, "KALM"); // 0.1%
        partners[0] = partner;
        emit UpdatePartner(0, msg.sender, 10, "KALM");
    }
...
contract KalmarTokenPriviledge is Partnership {
    uint256 public eligibleAmount = 10 ether; // 10 KALM
...
contract KalmarSwap is KalmarTokenPriviledge, WhitelistFeeOnTransfer, ReentrancyGuard {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
