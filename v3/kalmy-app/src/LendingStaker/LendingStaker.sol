...
// File: contracts/LendingStaker.sol
...
contract LendingStaker is IStakingRewards, Ownable , ReentrancyGuard {
    using SafeMath for uint256;
    ...
    constructor(
        ...
        rewardVesting = IRewardVesting(_vest);

        rewardsDuration = 1 days;

        IERC20(_rewardToken).safeApprove(address(rewardVesting), uint256(-1));
    ...
    function _claim(uint256 _amount, address user) internal {

      /* if(claimLock[user] <= 0){
        IERC20(REWARD_TOKEN).safeApprove(address(rewardVesting), uint256(-1));
      } */
      rewardVesting.stake(_amount, user);
      /* claimLock[user] = claimLock[user].add(_amount); */
