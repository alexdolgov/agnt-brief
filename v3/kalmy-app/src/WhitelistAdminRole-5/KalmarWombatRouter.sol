...
// File: contracts/interfaces/IWombat.sol
...
// File: contracts/routes/KalmarWombatRouter.sol
...
contract KalmarWombatRouter is IKalmarTradingRoute, WhitelistedRole, ReentrancyGuard {
    using SafeERC20 for IERC20;
    using SafeMath for uint256;

    // curve-mim-usdt-usdc
    IWombat public constant wombatPool = IWombat(0x312Bc7eAAF93f1C60Dc5AfC115FcCDE161055fb0);
        ...
        returns(uint256 _destAmount)
    {
        ...
        _src.safeApprove(address(wombatPool), _srcAmount);
        (uint256 actualToAmount,) = wombatPool.swap(
          address(_src),
          address(_dest),
          _srcAmount,
          0,
        ...
        returns(uint256 _destAmount)
    {
        ...
        (uint256 potentialOutcome,) = wombatPool.quotePotentialSwap(address(_src), address(_dest), int256(_srcAmount));
    ...
    function _isAvailableToken(
        ...
        returns (bool)
    {
        address[] memory list = wombatPool.getTokens();
