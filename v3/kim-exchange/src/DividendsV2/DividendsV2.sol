...
/*
 * This contract is used to distribute dividends to users that allocated xKim here
 *
 ...
 * This distribution slot is the source of the dividends distribution to xKim allocators during the current cycle
 *
    ...
    IDividendsV2
{
    ...
    address public immutable xKimToken; // xKimToken contract

    mapping(address => uint256) public usersAllocation; // User's xKim allocation
    ...
    constructor(address xKimToken_, uint256 startTime_) {
        require(xKimToken_ != address(0), "zero address");
    ...
    /**
     * Allocates "userAddress" user's "amount" of xKim to this dividends contract
     *
     * Can only be called by xKimToken contract, which is trusted to verify amounts
     * "data" is only here for compatibility reasons (IXKimTokenUsage)
     */
    function allocate(
        address userAddress,
        uint256 amount,
    ...
    /**
     * Deallocates "userAddress" user's "amount" of xKim allocation from this dividends contract
     *
     * Can only be called by xKimToken contract, which is trusted to verify amounts
     * "data" is only here for compatibility reasons (IXKimTokenUsage)
     */
    ...
    function withdraw(
        ...
        require(success, "DividendsV2SFS::withdraw: withdraw failed");
    ...
    function _updateDividendsInfo(address token) internal {
        ...
        // if no xKim is allocated or initial distribution has not started yet
        if (
            totalAllocation == 0 ||
    ...
    function _harvestDividends(address token) internal {
        ...
        uint256 userXKimAllocation = usersAllocation[msg.sender];
        uint256 pending = user.pendingDividends.add(
            userXKimAllocation.mul(accDividendsPerShare).div(1e18).sub(
                user.rewardDebt
            )
        );

        user.pendingDividends = 0;
        user.rewardDebt = userXKimAllocation.mul(accDividendsPerShare).div(
            1e18
        );
