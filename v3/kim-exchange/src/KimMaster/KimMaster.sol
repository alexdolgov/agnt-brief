...
/*
 * This contract centralizes Kim's yield incentives distribution.
 * Pools that should receive those incentives are defined here, along with their allocation.
 * All rewards are claimed from the Kim Token contract.
 */
contract KimMaster is Ownable, IKimMaster {
    ...
    IKimToken private immutable _kimToken; // Address of the Kim token contract
    ...
    constructor(IKimToken kimToken_, uint256 startTime_) {
        require(
            address(kimToken_) != address(0),
            "KimMaster: kimToken cannot be set to zero address"
        );
        require(
            _currentBlockTimestamp() < startTime_ &&
                startTime_ >= kimToken_.lastEmissionTime(),
            "KimMaster: invalid startTime"
        );
        _kimToken = kimToken_;
        startTime = startTime_; // Must be set with the same time as KimToken emission start
    }
    ...
    event SetYieldBooster(
        address previousYieldBooster,
        address newYieldBooster
    );
    event PoolUpdated(
    ...
    modifier validatePool(address poolAddress) {
        require(
            _pools.contains(poolAddress),
            "validatePool: pool does not exist"
        );
        _;
    }
    ...
    /*
     * @dev Returns KimToken address
     */
    function kimToken() external view override returns (address) {
        return address(_kimToken);
    }

    /*
     * @dev Returns KimToken's emission rate (allocated to this contract)
     */
    function emissionRate() public view returns (uint256) {
        return _kimToken.masterEmissionRate();
    }
     ...
     */
    function getActivePoolAddressByIndex(
        uint256 index
    ) external view returns (address) {
        ...
        )
    {
        ...
        if (totalAllocPoint == 0) {
            ...
            poolEmissionRate = emissionRate().mul(allocPoint).div(
                totalAllocPoint
            );
        }
    }
    ...
    function add(
        ...
        uint256 lastRewardTime = currentBlockTimestamp > startTime
    ...
    function withdraw(
        ...
        require(success, "KimMasterSFS::withdraw: withdraw failed");
    ...
    function _safeRewardsTransfer(
        ...
        uint256 kimBalance = _kimToken.balanceOf(address(this));

        if (amount > kimBalance) {
            amount = kimBalance;
        }

        _kimToken.safeTransfer(to, amount);
        return amount;
    }
    ...
    function _updatePool(address poolAddress) internal {
        ...
        if (allocPoint > 0 && INFTPool(poolAddress).hasDeposits()) {
            // calculate how much Kim rewards are expected to be received for this pool
            uint256 rewards = currentBlockTimestamp
                .sub(lastRewardTime)
                .mul(emissionRate())
                .mul(allocPoint)
                .div(totalAllocPoint); // nbSeconds

            // claim expected rewards from the token
            // use returns effective minted amount instead of expected amount
            (rewards) = _kimToken.claimMasterRewards(rewards);
