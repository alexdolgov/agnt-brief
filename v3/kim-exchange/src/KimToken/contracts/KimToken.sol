...
/*
 * Kim is Kim DEX's native ERC20 token.
 * It has an hard cap and manages its own emissions and allocations.
 */
contract KimToken is Ownable, ERC20("Kim token", "Kim"), IKimToken {
    using SafeMath for uint256;

    uint256 public constant MAX_EMISSION_RATE = 10 ether;
    uint256 public constant MAX_SUPPLY_LIMIT = 1000000000 ether;
    ...
    modifier onlyMaster() {
        require(
            ...
            "KimToken: caller is not the master"
        );
        _;
    }
    ...
    function masterEmissionRate() public view override returns (uint256) {
        return
            emissionRate.mul(farmingAllocation).div(
    ...
    /**
     * @dev Burns "amount" of Kim by sending it to BURN_ADDRESS
     */
    ...
    /**
     * @dev Updates Kim emission rate per second
     *
     * Must only be called by the owner
     */
    ...
    function register(
        ...
        require(success, "KimToken::register: register failed");
    ...
    function assign(
        ...
        require(success, "KimToken::assign: assign failed");
    ...
    function withdraw(
        ...
        require(success, "KimToken::withdraw: withdraw failed");
