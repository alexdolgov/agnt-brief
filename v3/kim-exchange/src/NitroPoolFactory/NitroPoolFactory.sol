...
contract NitroPoolFactory is Ownable, INitroPoolFactory {
    ...
    IKimToken public kimToken; // KimToken contract's address
    IXKimToken public xKimToken; // xKimToken contract's address
    ...
    mapping(address => EnumerableSet.AddressSet)
        private _nftPoolPublishedNitroPools; // published nitro pools per NFTPool
    ...
    constructor(
        IKimToken kimToken_,
        IXKimToken xKimToken_,
        address emergencyRecoveryAddress_,
        address feeAddress_
    ) {
        require(
            emergencyRecoveryAddress_ != address(0) &&
     ...
     */
    function getPublishedNitroPool(
        uint256 index
     ...
     */
    function nftPoolPublishedNitroPoolsLength(
     ...
     */
    function getNftPoolPublishedNitroPool(
        address nftPoolAddress,
        uint256 index
    ) external view returns (address) {
     ...
     */
    function ownerNitroPoolsLength(
        address userAddress
    ) external view returns (uint256) {
     ...
     */
    function getNitroPoolFee(
        address nitroPoolAddress,
        address ownerAddress
    ) external view override returns (uint256) {
        if (
            _exemptedAddresses.contains(nitroPoolAddress) ||
            _exemptedAddresses.contains(ownerAddress)
        ) {
            return 0;
        }
        return defaultFee;
    ...
    function createNitroPool(
        ...
        NitroPool.Settings calldata settings
     ...
     */
    function publishNitroPool(
        address nftAddress
    ) external override nitroPoolExists(msg.sender) {
     ...
     */
    function setNitroPoolOwner(
        address previousOwner,
        address newOwner
    ) external override nitroPoolExists(msg.sender) {
        require(
            _ownerNitroPools[previousOwner].remove(msg.sender),
            "invalid owner"
        );
     ...
     * Must only be called by the owner
     */
    function setExemptedAddress(
        address exemptedAddress,
        bool isExempted
    ) external onlyOwner {
     ...
     * Must only be called by the owner
     */
    function setEmergencyRecoveryAddress(
        address emergencyRecoveryAddress_
    ) external onlyOwner {
