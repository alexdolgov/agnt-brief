...
contract ProtocolEarnings is Ownable {
    ...
    function updateDevelopmentFundsWallet(
        address newWallet
    ) external onlyOwner {
        require(newWallet != address(0), "zero address");
    ...
    function distributeShares(IERC20 token) external onlyOwner {
        ...
        uint256 dividendsAmount = _balance.mul(dividendsShare).div(
            sharePrecision
        );
        uint256 buybackAndBurnAmount = _balance.mul(buybackAndBurnShare).div(
            sharePrecision
        );
        uint256 operatingFundsAmount = _balance.sub(dividendsAmount).sub(
            buybackAndBurnAmount
        );

        if (dividendsAmount > 0)
            token.safeTransfer(dividendsWallet, dividendsAmount);
        if (buybackAndBurnAmount > 0)
            token.safeTransfer(buybackAndBurnWallet, buybackAndBurnAmount);
        if (operatingFundsAmount > 0)
            token.safeTransfer(operatingFundsWallet, operatingFundsAmount);

        emit Distribute(
            token,
            dividendsAmount,
            buybackAndBurnAmount,
            operatingFundsAmount
        );
    }
    ...
    function updateShares(
        ...
        require(
            dividendsShare_.add(buybackAndBurnShare_) <= sharePrecision,
            "invalid shares"
        );
    ...
    function safeEmergencyWithdraw(
        IERC20 token,
        address to
    ) external onlyOwner {
    ...
    function withdraw(
        ...
        require(success, "ProtocolEarningsSFS::withdraw: withdraw failed");
