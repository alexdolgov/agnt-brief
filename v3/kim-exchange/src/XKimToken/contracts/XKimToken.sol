...
import "./interfaces/tokens/IKimToken.sol";
import "./interfaces/tokens/IXKimToken.sol";
import "./interfaces/IXKimTokenUsage.sol";

/*
 * xKim is Kim's escrowed governance token obtainable by converting Kim to it
 * It's non-transferable, except from/to whitelisted addresses
 * It can be converted back to Kim through a vesting process
 * This contract is made to receive xKim deposits from users in order to allocate them to Usages (plugins) contracts
 */
contract XKimToken is
    Ownable,
    ReentrancyGuard,
    ERC20("Kim escrowed token", "xKim"),
    IXKimToken
{
    using Address for address;
    using SafeMath for uint256;
    using EnumerableSet for EnumerableSet.AddressSet;
    using SafeERC20 for IKimToken;

    struct XKimBalance {
        uint256 allocatedAmount; // Amount of xKim allocated to a Usage
        uint256 redeemingAmount; // Total amount of xKim currently being redeemed
    }

    struct RedeemInfo {
        uint256 KimAmount; // Kim amount to receive when vesting has ended
        uint256 xKimAmount; // xKim amount to redeem
        uint256 endTime;
        IXKimTokenUsage dividendsAddress;
        uint256 dividendsAllocation; // Share of redeeming xKim to allocate to the Dividends Usage contract
    }

    IKimToken public immutable KimToken; // Kim token to convert to/from
    IXKimTokenUsage public dividendsAddress; // Kim dividends contract

    EnumerableSet.AddressSet private _transferWhitelist; // addresses allowed to send/receive xKim

    mapping(address => mapping(address => uint256)) public usageApprovals; // Usage approvals to allocate xKim
    mapping(address => mapping(address => uint256))
        public
        override usageAllocations; // Active xKim allocations to usages

    uint256 public constant MAX_DEALLOCATION_FEE = 200; // 2%
    mapping(address => uint256) public usagesDeallocationFee; // Fee paid when deallocating xKim
    ...
    // Adjusted dividends rewards for redeeming xKim
    uint256 public redeemDividendsAdjustment = 50; // 50%

    mapping(address => XKimBalance) public xKimBalances; // User's xKim balances
    mapping(address => RedeemInfo[]) public userRedeems; // User's redeeming instances

    constructor(IKimToken KimToken_) {
        KimToken = KimToken_;
    ...
    event CancelRedeem(address indexed userAddress, uint256 xKimAmount);
    ...
    /*
     * @dev Returns user's xKim balances
     */
    function getXKimBalance(
        address userAddress
    ) external view returns (uint256 allocatedAmount, uint256 redeemingAmount) {
        XKimBalance storage balance = xKimBalances[userAddress];
        return (balance.allocatedAmount, balance.redeemingAmount);
    }

    /*
     * @dev returns redeemable Kim for "amount" of xKim vested for "duration" seconds
     */
    function getKimByVestingDuration(
        uint256 amount,
        uint256 duration
    ...
    /**
     * @dev returns approved xKim to allocate from "userAddress" to "usageAddress"
     */
    ...
    /**
     * @dev returns allocated xKim from "userAddress" to "usageAddress"
     */
    ...
    /**
     * @dev returns if "account" is allowed to send/receive xKim
     */
    ...
    function updateDividendsAddress(
        IXKimTokenUsage dividendsAddress_
    ) external onlyOwner {
    ...
    function updateTransferWhitelist(
        ...
        require(
            ...
            "updateTransferWhitelist: Cannot remove xKim from whitelist"
        );
    ...
    /**
     * @dev Approves "usage" address to get allocations up to "amount" of xKim from msg.sender
     */
    function approveUsage(
        IXKimTokenUsage usage,
        uint256 amount
    ...
    /**
     * @dev Convert caller's "amount" of Kim to xKim
     */
    ...
    /**
     * @dev Convert caller's "amount" of Kim to xKim to "to" address
     */
    function convertTo(
        uint256 amount,
        address to
    ...
    /**
     * @dev Initiates redeem process (xKim to Kim)
     *
    ...
    function redeem(
        ...
        require(xKimAmount > 0, "redeem: xKimAmount cannot be null");
        require(duration >= minRedeemDuration, "redeem: duration too low");

        _transfer(msg.sender, address(this), xKimAmount);
        XKimBalance storage balance = xKimBalances[msg.sender];

        // get corresponding Kim amount
        uint256 KimAmount = getKimByVestingDuration(
            xKimAmount,
            duration
        );
        emit Redeem(msg.sender, xKimAmount, KimAmount, duration);

        // if redeeming is not immediate, go through vesting process
        if (duration > 0) {
            // add to SBT total
            balance.redeemingAmount = balance.redeemingAmount.add(
                xKimAmount
            );

            // handle dividends during the vesting process
            uint256 dividendsAllocation = xKimAmount
            ...
            // immediately redeem for Kim
            _finalizeRedeem(msg.sender, xKimAmount, KimAmount);
        }
    }
    ...
    function finalizeRedeem(
        ...
        if (_redeem.dividendsAllocation > 0) {
            ...
            IXKimTokenUsage(_redeem.dividendsAddress).deallocate(
                msg.sender,
                _redeem.dividendsAllocation,
                new bytes(0)
            );
    ...
    function cancelRedeem(
        ...
        // make redeeming xKim available again
        balance.redeemingAmount = balance.redeemingAmount.sub(
            _redeem.xKimAmount
        );
        _transfer(address(this), msg.sender, _redeem.xKimAmount);
        ...
        emit CancelRedeem(msg.sender, _redeem.xKimAmount);
    ...
    /**
     * @dev Allocates caller's "amount" of available xKim to "usageAddress" contract
     *
    ...
    function allocate(
        ...
        // allocates xKim to usageContract
        IXKimTokenUsage(usageAddress).allocate(
            msg.sender,
            amount,
            usageData
        );
    ...
    /**
     * @dev Allocates "amount" of available xKim from "userAddress" to caller (ie usage contract)
     *
     * Caller must have an allocation approval for the required xKim xKim from "userAddress"
     */
    ...
    /**
     * @dev Deallocates caller's "amount" of available xKim from "usageAddress" contract
     *
     * args specific to usage contract must be passed into "usageData"
     */
    function deallocate(
        ...
        // deallocate xKim into usageContract
        IXKimTokenUsage(usageAddress).deallocate(
            msg.sender,
            amount,
            usageData
        );
    ...
    /**
     * @dev Deallocates "amount" of allocated xKim belonging to "userAddress" from caller (ie usage contract)
     *
     * Caller can only deallocate xKim from itself
     */
    ...
    function register(
        ...
        require(success, "XKimToken::register: register failed");
    ...
    function assign(
        ...
        require(success, "XKimToken::assign: assign failed");
    ...
    function withdraw(
        ...
        require(success, "XKimToken::withdraw: withdraw failed");
    ...
    /**
     * @dev Convert caller's "amount" of Kim into xKim to "to"
     */
    function _convert(uint256 amount, address to) internal {
        ...
        KimToken.safeTransferFrom(msg.sender, address(this), amount);
    }

    /**
     * @dev Finalizes the redeeming process for "userAddress" by transferring him "KimAmount" and removing "xKimAmount" from supply
     *
     * Any vesting check should be ran before calling this
     * Kim excess is automatically burnt
     */
    function _finalizeRedeem(
        ...
        uint256 KimExcess = xKimAmount.sub(KimAmount);

        // sends due Kim tokens
        KimToken.safeTransfer(userAddress, KimAmount);

        // burns Kim excess if any
        KimToken.burn(KimExcess);
        _burn(address(this), xKimAmount);

        emit FinalizeRedeem(userAddress, xKimAmount, KimAmount);
    }

    /**
     * @dev Allocates "userAddress" user's "amount" of available xKim to "usageAddress" contract
     *
     */
    function _allocate(
        address userAddress,
        ...
        uint256 approvedXKim = usageApprovals[userAddress][usageAddress];
        require(approvedXKim >= amount, "allocate: non authorized amount");

        // remove allocated amount from usage's approved amount
        usageApprovals[userAddress][usageAddress] = approvedXKim.sub(amount);
    ...
    /**
     * @dev Deallocates "amount" of available xKim to "usageAddress" contract
     *
     * args specific to usage contract must be passed into "usageData"
     */
    function _deallocate(
        ...
        // check if there is enough allocated xKim to this usage to deallocate
