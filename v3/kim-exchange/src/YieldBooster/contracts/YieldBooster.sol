...
/*
 * This contract is a xKim Usage (plugin) that can boost kpNFTs' yield (staking positions on NFTPools) when it
 * receives allocations from the xKimToken contract
 */
contract YieldBooster is
    Ownable,
    ReentrancyGuard,
    IXKimTokenUsage,
    IYieldBooster
{
    ...
    IXKimToken public immutable xKimToken; // XKimToken contract
    ...
    mapping(address => mapping(address => EnumerableSet.UintSet))
        private _usersPositions;
    // User's position xKim total allocation
    // userAddress => poolAddress => tokenId => totalAllocation
    mapping(address => mapping(address => mapping(uint256 => uint256)))
        public usersPositionsAllocation;

    mapping(address => uint256) private _usersTotalAllocation; // User's xKim total allocation
    mapping(address => uint256) private _poolsTotalAllocation; // Pool's total allocation
    uint256 public totalAllocation; // Contract's total xKim allocation

    bool public forcedDeallocationStatus; // Authorize users to forcibly deallocate everything

    constructor(IXKimToken xKimToken_) {
    ...
    /**
     * @dev Checks if caller is the XKimToken contract
     */
    modifier xKimTokenOnly() {
        require(
            msg.sender == address(xKimToken),
            "xKimTokenOnly: caller should be xKimToken"
        );
        _;
    }
    ...
    /**
     * @dev Returns total xKim allocated to this contract by "userAddress"
     */
    function getUserTotalAllocation(
        address userAddress
    ) external view returns (uint256) {
    ...
    /**
     * @dev Returns total xKim allocated to this contract by "poolAddress"
     */
    function getPoolTotalAllocation(
        address poolAddress
    ) external view returns (uint256) {
    ...
    /**
     * @dev Returns allocated xKim to "tokenId" kpNFT from "poolAddress" NFTPool by "userAddress"
     */
    ...
    function setTotalAllocationFloor(uint256 floor) external onlyOwner {
        require(
            floor > 0 && floor <= MAX_TOTAL_ALLOCATION_FLOOR,
            "setTotalAllocationFloor: invalid floor"
        );
    ...
    /**
     * Allocates "userAddress" user's "amount" of xKim to this usage contract
     * "data" should contain tokenId and poolAddress
     *
     * Can only be called by XKimToken contract, which is trusted to verify amounts
     */
    function allocate(
        address userAddress,
        uint256 amount,
        bytes calldata data
    ) external override nonReentrant xKimTokenOnly {
        (address poolAddress, uint256 tokenId) = abi.decode(
            data,
            (address, uint256)
        );
        _allocate(userAddress, poolAddress, tokenId, amount);

        // allocated xKim is added (as boost points) to kpNFT
        INFTPool(poolAddress).boost(tokenId, amount);
    }

    /**
     * Deallocates "userAddress" user's "amount" of xKim from this usage contract
     * "data" should contain tokenId and poolAddress
     *
     * Can only be called by XKimToken contract, which is trusted to verify amounts
     */
    function deallocate(
        ...
        // should only be called if kpNFT has not been burned, to avoid having stuck xKim on it
        if (INFTPool(poolAddress).exists(tokenId)) {
            // allocated xKim is removed (as boost points) from the kpNFT
     ...
     * The pool should remove the allocated xKim (boost points) from its own kpNFT when calling this function
     */
    function deallocateAllFromPool(
        address userAddress,
        uint256 tokenId
    ) external override nonReentrant {
        uint256 amount = usersPositionsAllocation[userAddress][msg.sender][
            tokenId
        ];
        _deallocate(userAddress, msg.sender, tokenId, amount);

        // update user's xKim allocations balance
        xKimToken.deallocateFromUsage(userAddress, amount);
    }

    /**
     * Deallocates msg.sender's "amount" of xKim from XKimToken, without adjusting this contract allocations balances
     *
    ...
    function forceDeallocate() external nonReentrant {
        ...
        uint256 amount = xKimToken.usageAllocations(msg.sender, address(this));

        // update user's xKim allocations balance
        xKimToken.deallocateFromUsage(msg.sender, amount);
    }
    ...
    function withdraw(
        ...
        require(success, "YieldBoosterSFS::withdraw: withdraw failed");
    ...
    /**
     * @dev Returns multiplier that should be applied to a kpNFT based on its boost points (allocated xKim)
     *
     ...
     * 1% of the pool's allocated xKim
     *
    ...
    function _getMultiplier(
        ...
        if (totalAllocation_ < totalAllocationFloor)
            totalAllocation_ = totalAllocationFloor;
        if (
            totalAllocation_ == 0 ||
            lpAmount == 0 ||
            userAllocation == 0 ||
        ...
        uint256 multiplier = userAllocation
            .mul(totalLpSupply)
            .mul(maxBoostMultiplier)
            .div(lpAmount.mul(totalAllocation_));
        return Math.min(multiplier, maxBoostMultiplier);
    }

    /**
     * @dev Allocates "userAddress" user's "amount" of xKim to "tokenId" kpNFT of "poolAddress" pool
     */
    function _allocate(
        address userAddress,
        address poolAddress,
        ...
        _usersTotalAllocation[userAddress] = _usersTotalAllocation[userAddress]
            .add(amount);
        usersPositionsAllocation[userAddress][poolAddress][
            tokenId
        ] = usersPositionsAllocation[userAddress][poolAddress][tokenId].add(
            amount
        );
        _usersPositions[userAddress][poolAddress].add(tokenId);
        totalAllocation = totalAllocation.add(amount);
        _poolsTotalAllocation[poolAddress] = _poolsTotalAllocation[poolAddress]
            .add(amount);
    ...
    /**
     * @dev Deallocates "userAddress" user's "amount" of xKim allocated to "tokenId" kpNFT of "poolAddress" pool
     */
    function _deallocate(
        ...
        uint256 userPositionAllocation = usersPositionsAllocation[userAddress][
            poolAddress
        ][tokenId];
        require(
            userPositionAllocation >= amount,
            "deallocate: not enough allocated xKim"
        );
        ...
        usersPositionsAllocation[userAddress][poolAddress][
            tokenId
        ] = userPositionAllocation.sub(amount);
