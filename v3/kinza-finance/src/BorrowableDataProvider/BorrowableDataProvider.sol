...
contract BorrowableDataProvider {
  ...
  function getUserMaxBorrowable(address user, address asset) public view returns(uint256){
        ...
        bool isNotBorrowable = isAssetBlacklistedFromUserCollaterals(user, asset);
        if (isNotBorrowable) {
            return 0;
        }
  ...
  function isAssetBlacklistedFromUserCollaterals(address user, address asset) public view returns(bool) {
    IPool pool = IPool(provider.getPool());
    DataTypes.UserConfigurationMap memory userConfig = pool.getUserConfiguration(user);
    // check if the bitmap for the to-borrow asset is 0 or not 
    // for each enabled collatera
    uint256 reservesCount = pool.getReservesList().length;
    for (uint16 i; i < reservesCount; i++) {
        address collateral = pool.getReserveAddressById(i);
        if (userConfig.isUsingAsCollateral(i) && !pool.getReserveBorrowable(collateral ,asset)) {
            return true;
        }
    }
    return false;
