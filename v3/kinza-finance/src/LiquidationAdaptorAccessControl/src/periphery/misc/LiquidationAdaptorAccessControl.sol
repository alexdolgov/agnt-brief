...
contract LiquidationAdaptorAccessControl is Ownable {
    ...
    function liquidateWithFlashLoan(IPool pool, address liquidated, address collateral, address debtToken, uint256 debtAmount, ROUTE route, bytes memory customPath) external onlyOwner {
        IPoolDataProvider dataProvider = IPoolDataProvider(provider.getPoolDataProvider());
    ...
    bytes memory params
        ...
        {
            (address pool, address collateralAsset, address liquidatedUser, address liquidator, ROUTE route, bytes memory customPath) = 
        ...
        IPool(inputs.pool).liquidationCall(
