...
// make use of chainlijk ETH/rETH price feed and chainlink ETH feed
contract rETHPriceAdaptor is Ownable, AggregatorInterface {
    ...
    IChainlinkFeed public constant rETHOracle = IChainlinkFeed(0x536218f9E9Eb48863970252233c8F271f554C2d0);
    ...
    function latestAnswer() external view returns (int256) {
        int256 ethPrice = ETH_TO_BASE();
         try rETHOracle.latestAnswer() returns (int256 ratio) {
    ...
    function latestRound() external view returns (uint256) {
        (,,,,uint80 answeredInRound) = rETHOracle.latestRoundData();
        return uint256(answeredInRound);
    }

    // return the redStone rate * latestETHPrice
    function getAnswer(uint256 roundId) external view returns (int256) {
        (,int256 answer,,,) = rETHOracle.getRoundData(uint80(roundId));
        int256 ethPrice = ETH_TO_BASE();
        return ethPrice * answer / 1e8;
    }

    function getTimestamp(uint256 roundId) external view returns (uint256) {
        (,,,uint256 updatedAt,) = rETHOracle.getRoundData(uint80(roundId));
        return updatedAt;
    }
    ...
    function _shouldFallbackToTwap(int256 price) internal view returns(bool) {
        ...
        if (maxFallbackThreshold == 0) {
            // should take multiple years for ezETH to accumulate 30% yield from eth PoS (1.1 at the point of deployment)
            threshold = 1e8 * 130 / 100;
        } else {
