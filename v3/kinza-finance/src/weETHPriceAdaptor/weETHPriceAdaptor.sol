...
// make use of ETH/weETH price feed and chainlink ETH feed
contract weETHPriceAdaptor is Ownable, AggregatorInterface {
    ...
    IOracleFeed public constant weETHOracle = IOracleFeed(0x5c9C449BbC9a6075A2c061dF312a35fd1E05fF22);
    ...
    function latestAnswer() external view returns (int256) {
        int256 ethPrice = ETH_TO_BASE();
         try weETHOracle.latestAnswer() returns (int256 ratio) {
            if (_shouldFallbackToTwap(ratio)) {
                return getTWAP() * ethPrice / 1e8;
            }
            return ratio * ethPrice / 1e18 ;
        } catch {
            return getTWAP() * ethPrice / 1e8;
        }
    }
    ...
    function latestTimestamp() external view returns (uint256) {
            (,,,uint256 updatedAt,) = weETHOracle.latestRoundData();
            return updatedAt;
    }

    function latestRound() external view returns (uint256) {
        (,,,,uint80 answeredInRound) = weETHOracle.latestRoundData();
        return uint256(answeredInRound);
    }

    // return the redStone rate * latestETHPrice
    function getAnswer(uint256 roundId) external view returns (int256) {
        (,int256 answer,,,) = weETHOracle.getRoundData(uint80(roundId));
        int256 ethPrice = ETH_TO_BASE();
        return ethPrice * answer / 1e18;
    }

    function getTimestamp(uint256 roundId) external view returns (uint256) {
        (,,,uint256 updatedAt,) = weETHOracle.getRoundData(uint80(roundId));
        return updatedAt;
    }
    ...
    function _shouldFallbackToTwap(int256 price) internal view returns(bool) {
        ...
        if (maxFallbackThreshold == 0) {
            // should take multiple years for weETH to accumulate 20% yield from eth PoS
            threshold = 1e18 * 120 / 100;
        } else {
