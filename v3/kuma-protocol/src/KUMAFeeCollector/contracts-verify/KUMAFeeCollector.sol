...
/**
 * @title KUMA Fee Collector
 * @author MIMO Labs
 * @dev The fee collector for the KUMA protocol that collects fees for the KUMADAO on bond sales to KUMASwap
 */
contract KUMAFeeCollector is IKUMAFeeCollector, UUPSUpgradeable, Initializable {
    ...
    function initialize(IKUMAAddressProvider KUMAAddressProvider, bytes4 currency, bytes32 issuer, uint32 term)
        external
        initializer
    {
    ...
    /**
     * @notice Adds a payee that receives collected income
     ...
     */
    function addPayee(address payee, uint256 share) external onlyManager {
    ...
    /**
     * @notice Removes a payee from the payee list
     * @dev Will update totalShares and therefore increase the relative share of all other payees
     * @dev Will release existing fees before the update
     * @param payee The address of the payee to remove
     */
    function removePayee(address payee) external onlyManager {
        if (!_payees.contains(payee)) {
    ...
    /**
     * @notice Updates the payees and payee shares of this contract
     * @dev Will release existing fees before the update
     * @dev newPayees and newShares must have the same length
     * @param newPayees Array of  new payees
     * @param newShares Array of shares for each new payee
     */
    function changePayees(address[] calldata newPayees, uint256[] calldata newShares) external onlyManager {
    ...
    /**
     * @return The address provider of the KUMA protocol
     */
    ...
    /**
     * @return The risk category of this FeeCollector
     */
    ...
    /**
     * @notice Internal helper function to release an available income to all payees.
     * @dev Uses totalShares and _shares to calculate correct amount to release to each payee.
    ...
    /**
     * @return Total shares owned by all payees
     */
     ...
     * @return Number of shares a specific payee holds
     */
