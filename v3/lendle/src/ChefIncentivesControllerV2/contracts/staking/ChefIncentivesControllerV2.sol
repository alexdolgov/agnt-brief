...
contract ChefIncentivesControllerV2 is OwnableUpgradable {
    ...
    if (amount > 0) {
      ...
      if (pending > 0) {
        if(incentivesTreasury == address(0)) {
          ...
          uint256 claimAmount = userBaseClaimable[_user].add(pending);
          userBaseClaimable[_user] = 0;
          _mint(_user, claimAmount);
        }
      }
    }
