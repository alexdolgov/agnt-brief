...
import {IStakingConfigurator} from '../interfaces/IStakingConfigurator.sol';
import {Errors} from '../protocol/libraries/helpers/Errors.sol';

contract StakingConfigurator is VersionedInitializable, IStakingConfigurator {
  ...
  function getMultiFeeDistribution() external override view returns (address) {
    return getAddress(MULTI_FEE_DISTRIBUTION);
  }

  function setMultiFeeDistributionImpl(address impl, bytes calldata initParams) external override onlyPoolAdmin {
    _updateImpl(MULTI_FEE_DISTRIBUTION, impl, initParams);
    emit MultiFeeDistributionUpdated(impl);
  }

  function getChefIncentivesController() external override view returns (address) {
    return getAddress(CHEF_INCENTIVES_CONTROLLER);
  }

  function setChefIncentivesControllerImpl(address impl, bytes calldata initParams) external override onlyPoolAdmin {
  ...
  function setMasterChefImpl(address impl, bytes calldata initParams) external override onlyPoolAdmin {
    _updateImpl(MASTER_CHEF, impl, initParams);
    emit MasterChefUpdated(impl);
  }

  function getProtocolRevenueDistribution() external override view returns (address) {
    return getAddress(PROTOCOL_REVENUE_DISTRIBUTION);
  }

  function setProtocolRevenueDistributionImpl(address impl, bytes calldata initParams) external override onlyPoolAdmin {
