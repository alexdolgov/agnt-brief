...
contract StakingManager is Authorizable, Modifiable, IStakingManager {
    ...
    function rewardIntegralFor(
        uint256 _id,
        address _user
    ) external view returns (uint256 _rewardIntegral) {
    ...
    function claimableReward(
        uint256 _id,
        address _user
    ) external view returns (uint256 _claimableReward) {
    ...
    function stake(address _account, uint256 _wad) external {
        ...
        for (uint256 _i = 0; _i < rewards; _i++) {
            ...
            if (_rewardType.isActive) {
                ...
                emit StakingManagerRewardPoolStaked(
                    _account,
                    _i,
                    _rewardType.rewardPool,
                    _wad
                );
            }
        }
    ...
    function withdraw() external {
        ...
        if (
            block.timestamp - _existingWithdrawal.timestamp <
            _params.cooldownPeriod
        ) {
    ...
    function emergencyWithdrawReward(
        ...
        emit StakingManagerEmergencyRewardWithdrawal(
            _rescueReceiver,
    ...
    function _calcRewardIntegral(
        ...
        uint256 _contractTokenBalance = IERC20(_rewardType.rewardToken)
        ...
        if (_currentRewardBalance > _rewardType.rewardRemaining) {
            uint256 _newRewards = _currentRewardBalance -
                _rewardType.rewardRemaining;
            if (_supply > 0) {
        ...
        for (uint256 _i = 0; _i < _accounts.length; _i++) {
            ...
            uint256 _userIntegral = _rewardType.rewardIntegralFor[
                _accounts[_i]
            ];
            ...
            if (_rewardType.rewardIntegral > _userIntegral) {
                _rewardAccrued =
                    (_userBalance *
                        (_rewardType.rewardIntegral - _userIntegral)) /
                    WAD;
            }

            if (_isClaim) {
                uint256 _claimablePreviously = _rewardType.claimableReward[
                    _accounts[_i]
                ];
                uint256 _totalReceivable = _claimablePreviously +
                    _rewardAccrued;
                ...
                if (_totalReceivable > 0) {
                    ...
                    _rewardType.rewardIntegralFor[_accounts[_i]] = _rewardType
                        .rewardIntegral; // Update integral
                    ...
                    // Transfer rewards
                    IERC20(_rewardType.rewardToken).safeTransfer(
                        _accounts[_i + 1],
                        _totalReceivable
                    );

                    emit StakingManagerRewardPaid(
                        _accounts[_i],
                        _rewardType.rewardToken,
                        _totalReceivable,
                        _accounts[_i + 1]
                    );
                } else {
