...
library CalculatorHelper {
    ...
    function calculateFee(
        ...
        // Derived curve params (13/15 and 11/15), all in bps
        uint256 baseEffBps = (2 * feeRateBps) / 15;   // B - (13/15)B = (2/15)B
        uint256 buyRateBps = (13 * feeRateBps) / 15;
        uint256 feeBps = side == Side.BUY
            ? baseEffBps + (buyRateBps * m) / price
            : baseEffBps + ((11 * feeRateBps) * m) / (15 * ONE);
