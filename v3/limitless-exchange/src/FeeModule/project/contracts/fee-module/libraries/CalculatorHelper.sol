...
library CalculatorHelper {
    ...
    // SELL curve params: 0.14 + 1.44 * P(1-P)
    uint256 internal constant SELL_A_NUM   = 14;
    uint256 internal constant SELL_C_NUM   = 144;
    uint256 internal constant SELL_DENOM   = 100;
    ...
    function calculateFee(
        ...
        if (feeRateBps == 0 || outcomeTokens == 0) return 0;

        uint256 price = _calculatePrice(makerAmount, takerAmount, side);
        if (price == 0 || price > ONE) return 0;

        if (side == Side.BUY) {
            uint256 m = min(price, ONE - price);

            uint256 baseEffBps = (2 * feeRateBps) / 15;
            uint256 buyRateBps = (13 * feeRateBps) / 15;

            uint256 feeBps = baseEffBps + (buyRateBps * m) / price;

            return (feeBps * outcomeTokens) / BPS_DIVISOR;
        }

        uint256 term = (price * (ONE - price)) / ONE;

        uint256 effSellBps =
            feeRateBps *
            (SELL_A_NUM + (SELL_C_NUM * term) / ONE) /
            SELL_DENOM;

        if (effSellBps == 0) return 0;

        uint256 collateralValue = outcomeTokens * price / ONE;

        return (collateralValue * effSellBps) / BPS_DIVISOR;
    }
