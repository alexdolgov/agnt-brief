...
contract ERC1155 is ERC165, IERC1155 {
  ...
  function safeTransferFrom(address from, address to, uint256 id, uint256 value, bytes calldata data) external {
    ...
    require(from == msg.sender || _operatorApprovals[from][msg.sender] == true, 'ERC1155: need operator approval for 3rd party transfers.');
  ...
  function _doSafeTransferAcceptanceCheck(address operator, address from, address to, uint256 id, uint256 value, bytes memory data) internal {
    if (to.isContract()) {
      require(
        IERC1155TokenReceiver(to).onERC1155Received(operator, from, id, value, data) == IERC1155TokenReceiver(to).onERC1155Received.selector,
