...
import {Fixed192x64Math} from '../utils/Fixed192x64Math.sol';
...
contract LMSRMarketMaker is MarketMaker {
  ...
  function calcMarginalPrice(uint8 outcomeTokenIndex) public view returns (uint price) {
  ...
  ) private view returns (uint sum, int offset, uint outcomeExpTerm) {
