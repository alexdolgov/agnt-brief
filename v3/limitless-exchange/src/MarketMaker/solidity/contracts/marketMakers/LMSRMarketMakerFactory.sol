...
contract LMSRMarketMakerFactory is ConstructedCloneFactory, LMSRMarketMakerData {
  ...
  function cloneConstructor(bytes calldata consData) external {
    (ConditionalTokens _pmSystem, IERC20 _collateralToken, bytes32[] memory _conditionIds, uint64 _fee, Whitelist _whitelist) = abi.decode(
      consData,
  ...
  function createLMSRMarketMaker(
    ...
    lmsrMarketMaker = LMSRMarketMaker(
      createClone(address(implementationMaster), abi.encode(pmSystem, collateralToken, conditionIds, fee, whitelist))
    );
