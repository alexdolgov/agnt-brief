...
import {SignedSafeMath} from '../utils/SignedSafeMath.sol';
...
contract MarketMaker is Ownable, ERC1155TokenReceiver {
  ...
  function changeFunding(int fundingChange) public onlyOwner atStage(Stage.Paused) {
  ...
  function trade(int[] memory outcomeTokenAmounts, int collateralLimit) public atStage(Stage.Running) onlyWhitelisted returns (int netCost) {
    ...
    int fees;
    if (outcomeTokenNetCost < 0) fees = int(calcMarketFee(uint(-outcomeTokenNetCost)));
