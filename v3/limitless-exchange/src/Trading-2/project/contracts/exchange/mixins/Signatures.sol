...
abstract contract Signatures is ISignatures, PolyFactoryHelper {
    ...
    function isValidSignature(
        ...
        if (signatureType == SignatureType.EOA) {
            ...
            return verifyERC1271Signature(associated, structHash, signature);
        } else {
            // POLY_PROXY
    ...
    /// @notice Verifies an ERC1271 signature
    /// Verifies that:
    /// 1) the signature is valid
    /// 2) the signer is the same as the associated address
    /// @param signer      - Address of the signer сontract
    /// @param structHash  - Hash of the struct being verified
    /// @param signature   - Signature to be verified
    function verifyERC1271Signature(address signer, bytes32 structHash, bytes memory signature)
        internal
        view
        returns (bool)
    {
        return IERC1271(signer).isValidSignature(structHash, signature) == bytes4(keccak256("isValidSignature(bytes32,bytes)"));
    }
}
