...
contract LiquidHundredChef is OwnableUpgradeable {
    ...
    event LogUpdatePool(uint256 indexed pid, uint256 lastRewardTime, uint256 lpSupply, uint256 accTokenPerShare);
    ...
    function add(
        ...
        poolInfo.push(PoolInfo({allocPoint: allocPoint, lastRewardTime: block.timestamp, accTokenPerShare: 0}));
    ...
    function harvest(uint256 pid, address to) public {
        ...
        ISecondRewarder _rewarder = rewarder[pid];
        if (address(_rewarder) != address(0)) {
            _rewarder.onReward(pid, msg.sender, to, _pendingToken, user.amount);
        }
