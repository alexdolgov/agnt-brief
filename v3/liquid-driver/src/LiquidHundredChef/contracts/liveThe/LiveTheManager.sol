...
contract LiveTheManager is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function rebase() external onlyOwner {
        ...
        ILiveTheFeeManager(feeManager).distributeFees();
        ILiveThe(liveThe).updateSupplyAtCurrentEpoch(
            ILiveThe(liveThe).totalSupply()
        );
    ...
    function capAvailableToDeposit(uint256 _tokenId) public view returns (bool isAvailable) {
        uint256 _supply = ILiveThe(liveThe).supplyAtCurrentEpoch();
        uint256 _supplyLimit = _supply.mul(supplyPercentageLimit).div(MULTIPLIER);
        uint256 _toMint = IVeThe(veThe).balanceOfNFT(_tokenId)
