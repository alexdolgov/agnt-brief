...
contract Voter is OwnableUpgradeable {
    ...
    address public spiritGaugeProxy;
    ...
    mapping(uint256 => uint256) public totalVotedWeight;
    mapping(address => mapping(uint256 => uint256)) public votedWeight;
    ...
    function initialize(
        ...
        address _spiritGaugeProxy
    ) public initializer {
        __Ownable_init();
        xLQDR = _xLQDR;
        linSpiritStrategy = _linSpiritStrategy;
        spiritGaugeProxy = _spiritGaugeProxy;
        WEEK = 7 days;
        __NAME__ = _name;
    }
    ...
    function _revote(address _owner) internal {
        ...
        uint256 _voteWeight = IERC20(xLQDR).balanceOf(msg.sender);
        _voteWeight = _voteWeight.add(delegationAmount[_thisWeek][msg.sender]);
        _vote(_owner, _tokens, _weights, _voteWeight);
    }
    ...
    function vote(address[] memory _tokens, uint256[] memory _weights) public {
        ...
        uint256 _thisWeek = thisWeek();
        _voteWeight = _voteWeight.add(delegationAmount[_thisWeek][msg.sender]);
        _vote(msg.sender, _tokens, _weights, _voteWeight);
    }

    function _vote(address _owner, address[] memory _tokens, uint256[] memory _weights, uint256 _voteWeight) internal {
        uint256 _tokenCnt = _tokens.length;
        uint256 _thisWeek = thisWeek();
        ...
        if (_owner != address(0)) {
            ...
            totalVotedWeight[_thisWeek] = totalVotedWeight[_thisWeek].add(_voteWeight);
            votedWeight[_owner][_thisWeek] = _voteWeight;
        }
    }
    ...
    function voteInSpirit(
        address[] memory _protocolVotingTokens, 
        uint256[] memory _protocolVotingWeights
    ) external restricted {
        uint256 _thisWeek = thisWeek();
        uint256 _protocolVoteWeight = totalVotedWeight[_thisWeek].div(3).mul(7);

        _vote(address(0), _protocolVotingTokens, _protocolVotingWeights, _protocolVoteWeight);
        ...
        ILinSpiritStrategy(linSpiritStrategy).voteV2(spiritGaugeProxy, _tokens, _weights);
    }
