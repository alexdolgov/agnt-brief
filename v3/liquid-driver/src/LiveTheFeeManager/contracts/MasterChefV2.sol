...
contract MasterChefV2 is OwnableUpgradeable {
    ...
    function withdrawAllFromStrategy(uint256 _pid, IStrategy _strategy) external onlyOwner {
        _withdrawAllFromStrategy(_pid, _strategy);
    }
    ...
    function lqdrPerBlock() public view returns (uint256 amount) {
        amount = uint256(MASTER_CHEF.lqdrPerBlock())
