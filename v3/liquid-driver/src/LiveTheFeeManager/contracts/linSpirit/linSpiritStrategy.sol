...
contract linSpiritStrategy is OwnableUpgradeable {
    ...
    mapping(address => bool) public isBoostStrategy;
    ...
    function voteV2(address proxy, address[] calldata _tokenVote, uint256[] calldata _weights) external onlyOwner {
        ISpiritGaugeProxy(proxy).vote(_tokenVote, _weights);
    }
    ...
    function claimBribes(address proxy, address[] memory _bribes, address _user) external onlyOwner {
        ISpiritGaugeProxy(proxy).claimBribes(_bribes, _user);
    }
    ...
    function claimVotingFees(
        ...
        address token1, 
        address to
    ) external onlyOwner returns (uint claimed0, uint claimed1) {
        (claimed0, claimed1) = ISpiritGauge(gauge).claimVotingFees();
