...
MAX_EPOCHS: constant(uint256) = 256
...
interface IPxCvx:
    ...
    def getEpochRedeemedSnapshotRewards(
        account: address, epoch: uint256
    ) -> uint256: view
        ...
        if user_bal > 0:
            ...
            for j: uint256 in range(MAX_REWARDS):
                ...
                if (bitmap >> j) & 1 == 0 and snap_amts[j] > 0:
                    indexes.append(j)
            if len(indexes) > 0:
                extcall IPirexCvx(PIREX_CVX).redeemSnapshotRewards(
                    epoch, indexes, receiver
                )

        r_bal: uint256 = staticcall IRPxCvx(RPX_CVX).balanceOf(self, snapshot_id)
        if r_bal > 0:
