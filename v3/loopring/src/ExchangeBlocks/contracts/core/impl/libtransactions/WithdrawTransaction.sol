...
library WithdrawTransaction {
    ...
    function process(
        ...
        WithdrawalAuxiliaryData memory auxData = abi.decode(
            auxiliaryData,
            (WithdrawalAuxiliaryData)
        );
        ...
        withdrawal.maxFee = auxData.maxFee == 0
            ? withdrawal.fee
            : auxData.maxFee;
        ...
        require(
            withdrawal.from == address(0) || withdrawal.to != address(0),
        ...
        if (withdrawal.withdrawalType == 0) {
            ...
            // Validate
            require(
                ctx.timestamp < withdrawal.validUntil,
                "WITHDRAWAL_EXPIRED"
            );
            require(
                withdrawal.fee <= withdrawal.maxFee,
            ...
            withdrawal.withdrawalType == 2 || withdrawal.withdrawalType == 3
        ) {
            ...
            require(
                withdrawal.extraData.length == 0,
                "AUXILIARY_DATA_NOT_ALLOWED"
            );

            ExchangeData.ForcedWithdrawal memory forcedWithdrawal = S
                .pendingForcedWithdrawals[withdrawal.fromAccountID][
                    withdrawal.tokenID
                ];
            ...
            if (forcedWithdrawal.timestamp != 0) {
                if (withdrawal.withdrawalType == 2) {
                    require(
                        withdrawal.from == forcedWithdrawal.owner,
                        "INCONSISENT_OWNER"
                    );
                } else {
                    //withdrawal.withdrawalType == 3
                    require(
                        withdrawal.from != forcedWithdrawal.owner,
                        "INCONSISENT_OWNER"
                    );
                ...
                delete S.pendingForcedWithdrawals[withdrawal.fromAccountID][
                    withdrawal.tokenID
                ];
        ...
        address recipient = S.withdrawalRecipient[withdrawal.from][
            withdrawal.to
        ][withdrawal.tokenID][withdrawal.amount][withdrawal.storageID];
        if (recipient != address(0)) {
            ...
            S.withdrawalRecipient[withdrawal.from][withdrawal.to][
                withdrawal.tokenID
            ][withdrawal.amount][withdrawal.storageID] = withdrawal.to;
        }

        // Validate gas provided
        require(
            auxData.gasLimit >= withdrawal.minGas,
            "OUT_OF_GAS_FOR_WITHDRAWAL"
        );
    ...
    function readTx(
        ...
        require(
            data.toUint8Unsafe(_offset) ==
                uint8(ExchangeData.TransactionType.WITHDRAWAL),
            "INVALID_TX_TYPE"
        );
        _offset += 1;
    ...
    function hashOnchainData(
        ...
        // Only the 20 MSB are used, which is still 80-bit of security, which is more
        // than enough, especially when combined with validUntil.
        return bytes20(keccak256(abi.encodePacked(minGas, to, extraData)));
    }
}
