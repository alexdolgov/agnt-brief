...
library ExchangeNFT {
    ...
    function withdraw(
        ...
        if (nftType == ExchangeData.NftType.ERC1155) {
            try
                IERC1155(token).safeTransferFrom{gas: gasLimit}(
                    address(this),
                    to,
                    nftID,
                    amount,
            ...
            try
                IERC721(token).safeTransferFrom{gas: gasLimit}(
                    address(this),
                    to,
                    nftID,
                    extraData
    ...
    function mintFromL2(
        ...
        try
            IL2MintableNFT(token).mintFromL2{gas: gasLimit}(
                to,
                nftID,
                amount,
                minter,
    ...
    function isTokenAddressAllowed(
        ...
        return
            (token != address(this) && token != address(S.depositContract)) &&
            token.isContract();
    }
}
