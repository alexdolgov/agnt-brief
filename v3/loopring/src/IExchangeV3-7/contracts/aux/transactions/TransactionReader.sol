...
library TransactionReader {
    ...
    function readDeposit(
        ...
        uint txIdx,
        bytes memory txData
    ) internal pure returns (DepositTransaction.Deposit memory deposit) {
    ...
    function readWithdrawal(
        ...
        uint txIdx,
        bytes memory txData
    ) internal pure returns (WithdrawTransaction.Withdrawal memory withdrawal) {
        _block.readTx(txIdx, txData);
    ...
    function readAmmUpdate(
        ...
        uint txIdx,
        bytes memory txData
    ) internal pure returns (AmmUpdateTransaction.AmmUpdate memory ammUpdate) {
        _block.readTx(txIdx, txData);
    ...
    function readTransfer(
        ...
        uint txIdx,
        bytes memory txData
    ) internal pure returns (TransferTransaction.Transfer memory transfer) {
        _block.readTx(txIdx, txData);
    ...
    function readSignatureVerification(
        ...
        pure
        returns (
            SignatureVerificationTransaction.SignatureVerification
                memory verification
        )
    {
        _block.readTx(txIdx, txData);
