        ...
        if (header.numConditionalTransactions > 0) {
            ...
            for (uint i = 0; i < block_auxiliaryData.length; i++) {
                ...
                assembly {
                    ...
                    txIndex := mload(
                        add(add(32, block_auxiliaryData), auxOffset)
                    )
                    approved := mload(
                        add(add(64, block_auxiliaryData), auxOffset)
                    )
                    // Load `data` (pos 2)
                    let auxDataOffset := mload(
                        add(add(96, block_auxiliaryData), auxOffset)
                    )
                    auxData := add(
                        add(32, block_auxiliaryData),
                        add(auxOffset, auxDataOffset)
                    )
                }
                ctx.txIndex = txIndex;
                ...
                ExchangeData.TransactionType txType = ExchangeData
                    .TransactionType(txData.toUint8(0));
                uint txDataOffset = 0;
