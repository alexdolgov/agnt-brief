...
interface IAirdropClaim {
    function setUserInfo(address _who, address _to, uint256 _amount, uint16 veShare) external returns(bool status);
}
...
contract MerkleTree {
    ...
    /// @param veShare share of ve token (1000 = 100%)
    /// @param proof merkle proof to prove address and amount are in tree
    function claim(address to, uint256 amount, uint16 veShare, bytes32[] calldata proof) external {

        // check claim is started
        require(init, 'not started');
        require(veShare <= 1000, 'veShare > 1000');
        ...
        bytes32 leaf = keccak256(bytes.concat(keccak256(abi.encode(_userToCheck, amount, veShare))));
        bool isValidLeaf = MerkleProof.verify(proof, merkleRoot, leaf);
        if (!isValidLeaf) revert NotInMerkle(_userToCheck,amount);

        // Mint tokens to msg.sender
        bool _status = IAirdropClaim(airdropClaim).setUserInfo(msg.sender, to, amount, veShare);
