...
contract LudwigTosser is Ownable {
    ...
    function ludwigToss(address[] calldata recipients, uint256 amountEach) external {
        ...
        for (uint256 i = 0; i < recipients.length; i++) {
            ludwig.transfer(recipients[i], amountEach);
        }
        
        emit LudwigTossed(recipients, amountEach);
    }

    function withdrawLudwig() external onlyOwner {
        uint256 balance = ludwig.balanceOf(address(this));
        require(balance != 0, NoLudwigToWithdraw());
        ludwig.transfer(owner(), balance);
    }
    
    function withdrawS() external onlyOwner {
        uint256 balance = address(this).balance;
        require(balance != 0, NoSToWithdraw());
        (bool success,) = payable(owner()).call{value: balance}("");
        require(success, STransferFailed());
    }
