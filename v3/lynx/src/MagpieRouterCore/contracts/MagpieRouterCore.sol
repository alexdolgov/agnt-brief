...
import {IMagpieRouterCore} from "./interfaces/IMagpieRouterCore.sol";
...
/// @notice DO NOT approve tokens to this contract. It is not safe. All the approvals should be given to MagpieRouter and not MagpieRouterCore.
contract MagpieRouterCore is IMagpieRouterCore, Ownable2Step, Pausable {
    ...
    /// @dev See {IMagpieRouterCore-updateBridge}
    ...
    /// @dev See {IMagpieRouterCore-pause}
    function pause() public onlyOwner whenNotPaused {
        _pause();
    }

    /// @dev See {IMagpieRouterCore-unpause}
    function unpause() public onlyOwner whenPaused {
        _unpause();
    }

    /// @dev See {IMagpieRouterCore-multicall}
    ...
    function swap(SwapData memory swapData, address fromAddress) private returns (uint256 amountOut, uint256 gasUsed) {
    ...
    /// @dev See {IMagpieRouterCore-swapWithoutSignature}
    function swapWithoutSignature(
        bytes calldata
    ) external payable onlyWhitelisted returns (uint256 amountOut, uint256 gasUsed) {
        SwapData memory swapData = LibRouter.getData();
        (amountOut, gasUsed) = swap(swapData, msg.sender);
    }
