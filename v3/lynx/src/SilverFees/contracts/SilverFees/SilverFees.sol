...
contract SilverFees is AutomateTaskCreator, Ownable2Step {
	...
	event FeesBalanceCappedToZero(address indexed token, uint256 totalBalance, uint256 reservedAmount);
	...
	function executeFeesManagement() public onlyDedicatedMsgSender {
		...
		// Balance
		uint256 balance = _availableBalanceForFees();
	...
	/**
	 * @dev Returns available balance for fee distribution (total balance minus reserved amounts).
	 * If reserved > total (accounting desync), resets current token redistribution and returns total so execution has funds.
	 */
	function _availableBalanceForFees() internal returns (uint256 balance) {
		if (feesManagementData.swapToWrappedToken) {
			uint256 total = tokenAmount(address(wrappedToken));
			uint256 reserved = feesManagementData.redistributionData.wrappedTokenWeeklyGiveawayAmount + feesManagementData.redistributionData.wrappedTokenFlareAmount + feesManagementData.redistributionData.wrappedTokenSnatchAmount;
			if (total > reserved) {
				return total - reserved;
			} else {
				if (reserved > 0) {
					emit FeesBalanceCappedToZero(address(wrappedToken), total, reserved);
					_resetCurrentTokenRedistribution();
				}
				return total;
			}
		} else {
			uint256 total = tokenAmount(address(silverToken));
			uint256 reserved = feesManagementData.redistributionData.agWeeklyGiveawayAmount + feesManagementData.redistributionData.agFlareAmount + feesManagementData.redistributionData.agSnatchAmount;
			if (total > reserved) {
				return total - reserved;
			} else {
				if (reserved > 0) {
					emit FeesBalanceCappedToZero(address(silverToken), total, reserved);
					_resetCurrentTokenRedistribution();
				}
				return total;
			}
	...
	/**
	 * @dev Resets redistribution data for the current token (wrapped when swapToWrappedToken, else $AG).
	 */
	function _resetCurrentTokenRedistribution() internal {
		if (feesManagementData.swapToWrappedToken) {
