...
contract SilverLswManager is Ownable2Step, TimelockProtection {
	...
	constructor(
		...
		taskManager = SilverLswTaskManager(payable(_taskManager));
	}

	// ============ Sync System Functions ============

	function syncLswSystem() public onlyTaskManager {
		////testing
	...
	function executeSyncGaugesRewards() public onlyTaskManager {
		require(lswSystemData.isLswLocked, "LSW is not locked");
	...
	function startBreakLiquidityAndCompound() public onlyTaskManager {
		lswSystemData.taskIds.breakLiquidityAndCompoundTaskId = bytes32(0);
	...
	function setGaugeExecutor(address _poolGauge, address _executor) external {
		require(msg.sender == address(taskManager), "Not authorized");
