...
abstract contract LycheeAdapter is Maintainable {
    using SafeERC20 for IERC20;

    event LycheeAdapterSwap(address indexed _tokenFrom, address indexed _tokenTo, uint256 _amountIn, uint256 _amountOut);
    ...
    function recoverERC20(address _tokenAddress, uint256 _tokenAmount) external onlyMaintainer {
        require(_tokenAmount > 0, "LycheeAdapter: Nothing to recover");
        IERC20(_tokenAddress).safeTransfer(msg.sender, _tokenAmount);
        emit Recovered(_tokenAddress, _tokenAmount);
    }

    function recoverNATIVE(uint256 _amount) external onlyMaintainer {
        require(_amount > 0, "LycheeAdapter: Nothing to recover");
    ...
    function swap(
        ...
        emit LycheeAdapterSwap(_fromToken, _toToken, _amountIn, _amountOut);
    }
