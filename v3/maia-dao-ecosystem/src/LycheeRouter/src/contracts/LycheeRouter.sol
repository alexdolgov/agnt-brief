...
import "./interface/ILycheeRouter.sol";
import "./interface/IAdapter.sol";
import "./interface/IERC20.sol";
import "./interface/IWETH.sol";
import "./lib/SafeERC20.sol";
import "./lib/Maintainable.sol";
import "./lib/LycheeViewUtils.sol";
import "./lib/Recoverable.sol";
import "./lib/SafeERC20.sol";


contract LycheeRouter is Maintainable, Recoverable, ILycheeRouter {
    using SafeERC20 for IERC20;
    using OfferUtils for Offer;

    address public immutable WNATIVE;
    address public constant NATIVE = address(0);
    string public constant NAME = "LycheeRouter";
    ...
    function _applyFee(uint256 _amountIn, uint256 _fee) internal view returns (uint256) {
        require(_fee >= MIN_FEE, "LycheeRouter: Insufficient fee");
    ...
    function findBestPathWithGas(
        ...
        require(_maxSteps > 0 && _maxSteps < 5, "LycheeRouter: Invalid max-steps");
    ...
    function getGasPriceInExitTkn(uint256 _gasPrice, address _tokenOut) internal view returns (uint256 price) {
        // Avoid low-liquidity price appreciation (https://github.com/m4r0z/lychee-aggregator/issues/20)
    ...
    function _swapNoSplit(
        ...
        require(amounts[amounts.length - 1] >= _trade.amountOut, "LycheeRouter: Insufficient output amount");
        for (uint256 i = 0; i < _trade.adapters.length; i++) {
        ...
        emit LycheeSwap(_trade.path[0], _trade.path[_trade.path.length - 1], _trade.amountIn, amounts[amounts.length - 1]);
    ...
    function swapNoSplitFromNATIVE(
        ...
        require(_trade.path[0] == WNATIVE, "LycheeRouter: Path needs to begin with WNATIVE");
    ...
    function swapNoSplitToNATIVE(
        ...
        require(_trade.path[_trade.path.length - 1] == WNATIVE, "LycheeRouter: Path needs to end with WNATIVE");
