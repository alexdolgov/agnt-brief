...
contract TimeBondDepository is Ownable {
    ...
    /**
     *  @notice get asset price from DIA
     */
    function assetPrice() public view returns (int) {
        (int price, ) = priceFeed.getValue("ETH/USD");
        return price;
    }
   ...
   /**
    ...
    function standardizedDebtRatio() external view returns ( uint ) {
        return debtRatio().mul( uint( assetPrice() ) )/ 1e8; // ETH feed is 8 decimals
    }
