...
contract ve is IERC721, IERC721Metadata {
    ...
    string constant public name = "veHERMES";
    string constant public symbol = "veHERMES";
    ...
    /// @param _to Address to deposit
    function _create_lock_minter(uint _value, uint _lock_duration, address _to) internal returns (uint) {
        require(block.timestamp < 1647043200, "Only allowed before first distribution");
        uint unlock_time = (block.timestamp + _lock_duration) / WEEK * WEEK - WEEK; // Locktime is rounded down to last week
    ...
    /// @param _lock_duration Number of seconds to lock tokens for (rounded down to last week)
    /// @param _to Address to deposit
    function create_lock_for_minter(uint _value, uint _lock_duration, address _to) external nonreentrant returns (uint) {
