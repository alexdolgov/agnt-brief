...
contract ReaderV2 is Governable {
    ...
    struct PositionTokenInfo {
        ...
        address[] indexTokens;
        uint256[] indexTokenMaxPrices;
        uint256[] indexTokenMinPrices;
    }
    ...
    function getPositions(
        address _vault,
        address _account,
        PositionTokenInfo memory positionTokenInfo,
        bool[] memory _isLong
    ) public view returns (uint256[] memory) {
        uint256[] memory amounts = new uint256[](
            positionTokenInfo.collateralTokens.length * POSITION_PROPS_LENGTH
        );

        for (uint256 i = 0; i < positionTokenInfo.collateralTokens.length; i++) {
            {
                (
                    uint256 size,
                    uint256 collateral,
                    ...
                    uint256 lastIncreasedTime
                        ...
                        _account,
                        positionTokenInfo.collateralTokens[i],
                        positionTokenInfo.indexTokens[i],
                        _isLong[i]
                    );
            ...
            if (averagePrice > 0) {
                (bool hasProfit, uint256 delta) = getDelta(
                    _vault,
                    positionTokenInfo.indexTokens[i],
                    positionTokenInfo.indexTokenMaxPrices[i],
                    positionTokenInfo.indexTokenMinPrices[i],
                    size,
                    averagePrice,
                    _isLong[i],
                    lastIncreasedTime
    ...
    function getDelta(
        address _vault,
        address _indexToken,
        uint256 _indexTokenMaxPrice,
        uint256 _indexTokenMinPrice,
        uint256 _size,
        uint256 _averagePrice,
        bool _isLong,
        uint256 _lastIncreasedTime
    ) public view returns (bool, uint256) {
        require(_averagePrice > 0, "ReaderV2: minus averagePrice");
        uint256 price = _isLong ? _indexTokenMinPrice : _indexTokenMaxPrice;
        ...
        uint256 minBps = block.timestamp >
            _lastIncreasedTime.add(IVault(_vault).minProfitTime())
            ? 0
            : IVault(_vault).minProfitBasisPoints(_indexToken);
