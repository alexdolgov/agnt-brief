...
import "../oracle/interfaces/IPythFlags.sol";
import "../amm/interfaces/IPancakePair.sol";

pragma solidity 0.6.12;

contract VaultPriceFeedV3 is IVaultPriceFeed {
    ...
    address private constant FLAG_MANTA_SEQ_OFFLINE =
        address(
            bytes20(
                bytes32(uint256(keccak256("pyth.flags.manta-seq-offline")) - 1)
            )
        );

    address public gov;
    address public pythFlags;

    bool public isPriceEnabled = false;
    ...
    mapping(address => bool) public isPriceHandler;
    ...
    modifier onlyPriceHandler() {
        require(
            isPriceHandler[msg.sender],
            "VaultPriceFeed: ONLY PRICE HANDLER"
        );
        _;
    }

    modifier priceEnabled() {
        require(isPriceEnabled, "VaultPriceFeed: PRICE IS NOT ENABLED");
        _;
    }
    ...
    function setPriceHandler(address _handler, bool active) external onlyGov {
        isPriceHandler[_handler] = active;
    }

    function setPythFlags(address _pythFlags) external onlyGov {
        pythFlags = _pythFlags;
    }
    ...
    function setIsPriceEnabled(bool _isEnabled) external onlyPriceHandler {
        isPriceEnabled = _isEnabled;
    }
    ...
    function getPriceV1(
        ...
        bool _maximise,
        bool _includeAmmPrice
    ) public view priceEnabled returns (uint256) {
    ...
    function getLatestPrimaryPrice(
        ...
        uint256 price = priceFeed.getMedianPrice(_token);
