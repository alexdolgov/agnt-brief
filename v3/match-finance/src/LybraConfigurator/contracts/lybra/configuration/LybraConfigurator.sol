...
contract LybraConfigurator is Initializable {
    ...
    event RedeemLockedEUSDWithPeUSD(address indexed redeemer, uint256 peUSDAmount, uint256 eUSDShares);
    ...
    function redeemLockedEUSDWithPeUSD(uint256 amount) external onlyRole(ADMIN) {
        require(amount > 0, "Amount must be greater than 0");
        require(peUSD.balanceOf(msg.sender) >= amount, "Insufficient peUSD balance");
        peUSD.transferFrom(msg.sender, address(this), amount);
        peUSD.convertToEUSD(amount);
        uint256 eUSDReceived = EUSD.sharesOf(address(this));
        EUSD.transferShares(msg.sender, eUSDReceived);
        emit RedeemLockedEUSDWithPeUSD(msg.sender, amount, eUSDReceived);
    }
