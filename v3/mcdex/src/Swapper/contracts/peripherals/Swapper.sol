...
contract Swapper is AccessControlEnumerableUpgradeable, ISwapper, IErrors {
    ...
    function swapAndTransfer(
        ...
        require(tokenIn != address(0) && tokenOut != address(0), "Swapper::INVALID_TOKEN");
        // no swap needed
        if (tokenOut == tokenIn) {
        ...
        if (_isSUSDCEnabled() && tokenOut == susdc) {
            actualTokenOut = usdc;
            actualMinAmountOut = IERC4626(susdc).convertToAssets(minAmountOut);
        }
    ...
    function _redeemSUSDC(uint256 susdcAmount) internal returns (uint256 usdcAmount) {
        IERC20Upgradeable(susdc).safeApprove(susdc, susdcAmount);
    ...
    function _depositUSDC(uint256 usdcAmount) internal returns (uint256 susdcAmount) {
        IERC20Upgradeable(usdc).safeApprove(susdc, usdcAmount);
