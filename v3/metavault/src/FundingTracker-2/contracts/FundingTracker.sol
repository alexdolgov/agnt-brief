...
contract FundingTracker is Governable {
    ...
    function setLastUpdated(address _asset, bytes10 _market, uint256 _timestamp) external onlyGov {
        lastUpdated[_asset][_market] = _timestamp;
    }
    ...
    /// @param _asset Asset address, e.g. address(0) for ETH
    /// @param _market Market, e.g. "0x4554482D555344000000" is bytes10 equivalent of "ETH-USD" 
    /// @param _fundingTracker funding tracker value of given asset and market
    function setFundingTracker(
        address _asset,
        bytes10 _market,
        int256 _fundingTracker
    ) external onlyGov {
        fundingTrackers[_asset][_market] = _fundingTracker;
    }
