...
contract LBHooksLens {
    ...
    function getMetroPerSecond(uint256 pid) public view returns (uint256) {
        return _masterChef.getMetroPerSecondForPid(pid);
    }
        ...
        returns (Parameters memory parameters)
    {
        if (hooks != address(0)) {
            ...
            if (parameters.hooksType == ILBHooksManager.LBHooksType.MCRewarder) {
                ...
                parameters.moePerSecond = getMetroPerSecond(parameters.pid);
            } else {
