...
import {TokenHelper, IERC20} from "lb-rewarder/library/TokenHelper.sol";
...
abstract contract LBHooksBaseRewarder is LBBaseHooks, Ownable2StepUpgradeable, Clone, ILBHooksBaseRewarder {
    ...
    function _isAuthorizedCaller(address user) internal view virtual returns (bool) {
        // If the user is blacklisted, the caller can only be the LB Hooks Manager
        // claiming rewards is then only allowed if the LB Hooks Manager is the caller
        if (ILBHooksManager(_lbHooksManager).isBlacklisted(user)) {
           return address(_lbHooksManager) == msg.sender;
        }
    ...
    function _claim(address user, uint256[] memory ids, uint256 rewards) internal virtual {
        ...
        if (ILBHooksManager(_lbHooksManager).isBlacklisted(user)) {
            // If the user is blacklisted, the rewards are sent to the treasury
            address treasury = ILBHooksManager(_lbHooksManager).getTreasury();
            TokenHelper.safeTransfer(_getRewardToken(), treasury, rewards);
        } else {
    ...
    function _beforeMint(address, address to, bytes32[] calldata liquidityConfigs, bytes32) internal virtual override {
        if (ILBHooksManager(_lbHooksManager).isBlacklisted(to)) {
            revert ILBHooksManager.LBHooksManager__BlacklistedAddress();
        }
