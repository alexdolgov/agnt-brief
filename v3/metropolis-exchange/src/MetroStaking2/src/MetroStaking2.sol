 ...
 * @title Metro Staking
 * @author BlueLabs
 * @notice This pool allows users to stake token (METRO) and earn rewards (e.g USDC). Rewards get distributed on a daily/weekly basis.
    ...
    ERC721EnumerableUpgradeable
{
    ...
    // last balance of bonus reward token
    uint256 private _lastBonusRewardBalance;

    // The bonus reward token
    IERC20 private _bonusRewardToken;

    uint256 public _accBonusRewardsPerShare; // Accumulated bonus rewards per share, times PRECISION_FACTOR.
    mapping(uint256 => uint256) internal _bonusRewardDebts; // Bonus reward debt per position

    uint256[6] __gap;

    modifier onlyOwnerOrProxyAdmin() {
        if (msg.sender != owner() && msg.sender != ERC1967Utils.getAdmin()) {
            revert IMetroStaking_Unauthorized();
        }
        _;
    }
    ...
    constructor(IERC20 stakedToken, IERC20 rewardToken) {
        ...
        if (address(stakedToken) == address(0)) revert IMetroStaking_ZeroAddress();
        if (address(rewardToken) == address(0)) revert IMetroStaking_ZeroAddress();

        _stakedToken = stakedToken;
        _rewardToken = rewardToken;

        uint256 decimalsRewardToken = uint256(IERC20Metadata(address(_rewardToken)).decimals());
        if (decimalsRewardToken > 30) revert IMetroStaking_TooMuchTokenDecimals();
    ...
    function initializeBonusRewardToken(IERC20 bonusRewardToken) external onlyOwnerOrProxyAdmin reinitializer(4) {
        if (address(bonusRewardToken) == address(0)) revert IMetroStaking_ZeroAddress();
        if (address(bonusRewardToken) == address(_rewardToken)) revert IMetroStaking_SameAddress();

        _bonusRewardToken = bonusRewardToken;
        _lastBonusRewardBalance = _availableTokenRewardBalance(bonusRewardToken);
    }
    ...
    /**
     * @dev Returns the bonus reward token
     */
    function getBonusRewardToken() external view returns (IERC20) {
        return _bonusRewardToken;
    }
    ...
    function pendingRewards(uint256 tokenId) external view override returns (uint256) {
        ...
        uint256 rewardBalance = _availableRewardBalance();

        uint256 lastRewardBalance = _lastRewardBalance;

        // recompute accRewardsPerShare if not up to date
        if (rewardBalance > lastRewardBalance && _stakedSupplyWithMultiplier > 0) {
    ...
    /**
     * @dev Returns pending bonus rewards for a position
     */
    function pendingBonusRewards(uint256 tokenId) external view returns (uint256) {
        IERC20 bonusRewardToken = _bonusRewardToken;
        if (address(bonusRewardToken) == address(0)) {
            return 0;
        }

        StakingPosition storage position = _stakingPositions[tokenId];

        uint256 accBonusRewardsPerShare = _accBonusRewardsPerShare;
        uint256 rewardBalance = _availableTokenRewardBalance(bonusRewardToken);
        uint256 lastBonusRewardBalance = _lastBonusRewardBalance;

        if (rewardBalance > lastBonusRewardBalance && _stakedSupplyWithMultiplier > 0) {
            uint256 accruedReward = rewardBalance - lastBonusRewardBalance;
            accBonusRewardsPerShare =
                accBonusRewardsPerShare + ((accruedReward * PRECISION_FACTOR) / _stakedSupplyWithMultiplier);
        }

        return position.amountWithMultiplier * accBonusRewardsPerShare / PRECISION_FACTOR - _bonusRewardDebts[tokenId];
    }
    ...
    function setLockMultiplierSettings(uint256 maxLockDuration, uint256 maxLockMultiplier) external onlyOwner {
        if (maxLockMultiplier > _maxGlobalMultiplier) revert IMetroStaking_MaxLockMultiplierTooHigh();
        if (maxLockMultiplier > MAX_LOCK_MULTIPLIER_LIMIT) revert IMetroStaking_MaxLockMultiplierTooHigh();
     ...
     * Must only be called by the owner
     *
     * @notice Enabling emergency unlock allows all positions to be withdrawn regardless of lock time.
     * However, existing positions retain their stored lock multipliers and continue to earn rewards
     * at their pre-emergency weight until they interact with the contract (harvest, withdraw, etc.).
     * New positions and relocks will receive a zero multiplier. This is by design — the emergency
     * unlock is intended to release locks, not to retroactively rebalance reward distribution.
     */
    ...
    function createPosition(uint256 amount, uint256 lockDuration) external override nonReentrant {
        ...
        if (isUnlocked()) {
            if (lockDuration > 0) revert IMetroStaking_LocksDisabled();
        }

        // check for the minimum lock duration
        if (lockDuration < _minimumLockDuration) revert IMetroStaking_InvalidLockDuration();
        ...
        if (amount == 0) revert IMetroStaking_ZeroAmount();
        ...
        _bonusRewardDebts[currentTokenId] = amountWithMultiplier * (_accBonusRewardsPerShare) / (PRECISION_FACTOR);
    ...
    function addToPosition(uint256 tokenId, uint256 amountToAdd) external override nonReentrant {
        ...
        if (isUnlocked()) {
            revert IMetroStaking_LocksDisabled();
        }

        _checkOwnerOf(tokenId);

        // addToPosition: amount cannot be null
        if (amountToAdd == 0) revert IMetroStaking_ZeroAmount();
        ...
        _updateBoostMultiplierInfoAndRewardDebt(tokenId, position);
    ...
    function harvestPosition(uint256 tokenId) external override nonReentrant {
        ...
        _updateBoostMultiplierInfoAndRewardDebt(tokenId, _stakingPositions[tokenId]);
    }
    ...
    function renewLockPosition(uint256 tokenId) external nonReentrant {
        ...
        _updatePool();
        _lockPosition(tokenId, _stakingPositions[tokenId].initialLockDuration, false);
    }
    ...
    function emergencyWithdraw(uint256 tokenId) external override nonReentrant {
        ...
        if ((position.startLockTime + position.lockDuration) > block.timestamp && !isUnlocked()) {
            revert IMetroStaking_PositionStillLocked();
        }
        ...
        // redistribute the rewards to the pool
        {
            ...
            uint256 pendingBonus =
                position.amountWithMultiplier * _accBonusRewardsPerShare / PRECISION_FACTOR - _bonusRewardDebts[tokenId];
            _lastBonusRewardBalance = _lastBonusRewardBalance - pendingBonus;
        }
    ...
    function _updatePool() internal {
        _updateMainRewardPool();
        _updateBonusRewardPool();
    }

    /**
     * @dev Updates primary reward state of this pool to be up-to-date
     */
    function _updateMainRewardPool() internal {
        uint256 accRewardsPerShare = _accRewardsPerShare;
        uint256 rewardBalance = _availableRewardBalance();
        uint256 lastRewardBalance = _lastRewardBalance;
    ...
    /**
     * @dev Updates bonus reward state of this pool to be up-to-date
     */
    function _updateBonusRewardPool() internal {
        IERC20 bonusRewardToken = _bonusRewardToken;
        if (address(bonusRewardToken) == address(0)) {
            return;
        }
        ...
        uint256 accruedReward = rewardBalance - lastBonusRewardBalance;
        _accBonusRewardsPerShare =
            accBonusRewardsPerShare + ((accruedReward * PRECISION_FACTOR) / (_stakedSupplyWithMultiplier));
        _lastBonusRewardBalance = rewardBalance;
    }
    ...
    function _destroyPosition(uint256 tokenId) internal {
        ...
        delete _bonusRewardDebts[tokenId];
    ...
    function _withdrawFromPosition(address nftOwner, uint256 tokenId, uint256 amountToWithdraw) internal {
        ...
        if (amountToWithdraw == 0) revert IMetroStaking_ZeroAmount();
        ...
        if (position.amount < amountToWithdraw) revert IMetroStaking_AmountTooHigh();
     ...
     */
    function _updateBoostMultiplierInfoAndRewardDebt(uint256 tokenId, StakingPosition storage position) internal {
        ...
        _bonusRewardDebts[tokenId] = amountWithMultiplier * _accBonusRewardsPerShare / PRECISION_FACTOR;
    }
    ...
    function _harvestPosition(uint256 tokenId, address to) internal {
        require(to != address(this), "MetroStaking: cannot harvest to this contract");
        ...
        uint256 pendingBonus =
            position.amountWithMultiplier * _accBonusRewardsPerShare / PRECISION_FACTOR - _bonusRewardDebts[tokenId];
        if (pendingBonus > 0) {
            _safeBonusRewardTransfer(to, pendingBonus);
        }
    ...
    function _lockPosition(uint256 tokenId, uint256 lockDuration, bool resetInitial) internal {
        if (isUnlocked()) revert IMetroStaking_LocksDisabled();
        ...
        if (endTime > currentBlockTimestamp) {
            if (lockDuration == 0) revert IMetroStaking_InvalidLockDuration();
            if (lockDuration < (endTime - currentBlockTimestamp)) revert IMetroStaking_InvalidLockDuration();
        }
        ...
        if (resetInitial) {
            if (lockDuration <= position.initialLockDuration) revert IMetroStaking_InvalidLockDuration();
    ...
    /**
     * @notice Safe bonus token transfer function
     * @param _to The address that will receive `_amount` `bonusRewardToken`
     * @param _amount The amount to send to `_to`
     */
    function _safeBonusRewardTransfer(address _to, uint256 _amount) internal {
        IERC20 bonusRewardToken = _bonusRewardToken;
        uint256 rewardBalance = _availableTokenRewardBalance(bonusRewardToken);

        if (_amount > rewardBalance) {
            _lastBonusRewardBalance = _lastBonusRewardBalance - rewardBalance;
            bonusRewardToken.safeTransfer(_to, rewardBalance);
        } else {
            _lastBonusRewardBalance = _lastBonusRewardBalance - _amount;
            bonusRewardToken.safeTransfer(_to, _amount);
        }
    }

    /**
     * @dev Returns currently available reward inventory.
     * If reward and stake token are the same asset, staked principal is excluded.
     */
    function _availableRewardBalance() internal view returns (uint256 rewardBalance) {
        return _availableTokenRewardBalance(_rewardToken);
    }

    /**
     * @dev Returns currently available reward inventory for the provided token.
     * If the token is also the staked asset, staked principal is excluded.
     */
    function _availableTokenRewardBalance(IERC20 rewardToken) internal view returns (uint256 rewardBalance) {
        rewardBalance = rewardToken.balanceOf(address(this));

        if (address(rewardToken) != address(_stakedToken)) {
            return rewardBalance;
        }

        if (rewardBalance <= _stakedSupply) {
            return 0;
        }

        return rewardBalance - _stakedSupply;
    }
        ...
        returns (address)
    {
        ...
        if (from != address(0) && to != address(0)) {
            revert IMetroStaking_TransferNotAllowed();
        }
    ...
    function _checkOwnerOf(uint256 tokenId) internal view {
        ...
        if (ERC721Upgradeable.ownerOf(tokenId) != msg.sender) revert IMetroStaking_NotOwner();
    }
