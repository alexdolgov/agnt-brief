...
contract Router is IRouter02 {
    ...
    function _swap(
        ...
        for (uint256 i; i < path.length - 1; i++) {
            ...
            IPair(PairLibrary.pairFor(factory, input, output)).swap(amount0Out, amount1Out, to, new bytes(0));
        }
    }
    ...
    function swapExactTokensForTokens(
        ...
        TransferHelper.safeTransferFrom(path[0], msg.sender, PairLibrary.pairFor(factory, path[0], path[1]), amounts[0]);
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint256 i; i < path.length - 1; i++) {
            ...
            uint256 amountOutput;
            {
                ...
                amountOutput = PairLibrary.getAmountOut(amountInput, reserveInput, reserveOutput, pair.feeAmount());
            }
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        TransferHelper.safeTransferFrom(path[0], msg.sender, PairLibrary.pairFor(factory, path[0], path[1]), amountIn);
        uint256 balanceBefore = IERC20(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        require(IERC20(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >= amountOutMin, "Router: INSUFFICIENT_OUTPUT_AMOUNT");
    }
    ...
    function getAmountOut(
        ...
        return PairLibrary.getAmountOut(amountIn, reserveIn, reserveOut, feeAmount);
    }
    ...
    function getAmountIn(
        ...
        return PairLibrary.getAmountIn(amountOut, reserveIn, reserveOut, feeAmount);
    }
