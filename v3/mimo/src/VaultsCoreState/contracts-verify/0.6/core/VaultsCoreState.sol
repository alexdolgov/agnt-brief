...
contract VaultsCoreState is IVaultsCoreState {
  ...
  IBridgeableToken public bridgeableToken;
  ...
  constructor(IAddressProvider _addresses, IBridgeableToken _bridgeableToken) public {
    require(address(_addresses) != address(0));
    require(address(_bridgeableToken) != address(0));
    a = _addresses;
    bridgeableToken = _bridgeableToken;
  }
  ...
  **/
  function availableIncome() public view override returns (uint256) {
    return a.vaultsData().debt().sub(a.stablex().totalSupply().sub(bridgeableToken.getPrincipalTokenAmountMinted()));
  }
  ...
  **/
  ...
  /**
    Update the bridgeable token.
    @param _bridgeableToken the new bridgeable token.
  **/
  function updateBridgeableToken(IBridgeableToken _bridgeableToken) public onlyManager {
    require(address(_bridgeableToken) != address(0));
    bridgeableToken = _bridgeableToken;
    emit BridgeableTokenUpdated(address(_bridgeableToken));
  }
