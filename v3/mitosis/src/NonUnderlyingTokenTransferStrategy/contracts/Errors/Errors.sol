...
/// @dev Error when output amount is less than expected
error InsufficientOutputAmount(uint256 expected, uint256 actual);

/// @dev Error when swap direction is not allowed
error SwapNotAllowed();

/// @dev Error when strategy configuration changes the totalUnderlying
