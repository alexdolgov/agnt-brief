    ...
    OperatorDelegatorStorageV12
{
    ...
    /**
     * @notice  reinitializing the OperatorDelegator to track consolidated validators amount, reinitialize with version 4
     * @dev     permissioned call (onlyOperatorDelegatorAdmin), can only reinitialize once
     * @param   _totalConsolidatedAmount The amount of ETH already consolidated
    //  */
    function reinitialize(
        uint256 _totalConsolidatedAmount
    ) external onlyOperatorDelegatorAdmin reinitializer(4) {
        // reset queued shares delta for completed withdrawals
        totalConsolidatedAmount = _totalConsolidatedAmount;
    }
    ...
    /// @dev Sets the address to delegate tokens to in EigenLayer
    /// @dev Can be used to redelegate, It will automatically track the queued withdrawals
    function setDelegateAddress(
        ...
        // If already delegated, then redelegate
        if (delegationManager.delegatedTo(address(this)) != address(0)) {
            // redelegate through Delegation manager
            bytes32[] memory _withdrawalRoots = delegationManager.redelegate(
                delegateAddress,
                approverSignatureAndExpiry,
                approverSalt
            );
            // Tracks undelegated withdrawals
            OperatorDelegatorLib.trackUndelegateQueuedWithdrawals(
                _withdrawalRoots,
                delegationManager,
                queuedWithdrawal,
                queuedShares,
            ...
            // if not delegated, then delegate
    ...
    function completeQueuedWithdrawal(
        ...
        // Reduce Queued Shares
        OperatorDelegatorLib.reduceQueuedShares(
            withdrawal,
            tokens,
            delegationManager,
            queuedShares,
    ...
    ) external payable onlyNativeEthRestakeAdmin {
        totalConsolidatedAmount += OperatorDelegatorLib.getTotalConsolidationAmount(
            requests,
            eigenPod
        );
    ...
    function emergencyTrackMissedCheckpoint(
        ...
        _updateTotalBeaconChainExitBalance(_totalBeaconChainExitBalance);
    ...
    function verifyCheckpointProofs(
        ...
        if (eigenPod.lastCheckpointTimestamp() == currentCheckpointTimestamp) {
            ...
            // record total beacon chain exit balance
            _updateTotalBeaconChainExitBalance(totalBeaconChainExitBalanceGwei * GWEI_TO_WEI);
    ...
    function _updateTotalBeaconChainExitBalance(uint256 newExitBalance) internal {
        // add new exit balance to totalBeaconChainExitBalance
        totalBeaconChainExitBalance += newExitBalance;
        // Deduct consolidated amount from totalBeaconChainExitBalance
        if (totalConsolidatedAmount > 0) {
            if (totalConsolidatedAmount > totalBeaconChainExitBalance) {
                totalConsolidatedAmount -= totalBeaconChainExitBalance;
                totalBeaconChainExitBalance = 0;
            } else {
                totalBeaconChainExitBalance -= totalConsolidatedAmount;
