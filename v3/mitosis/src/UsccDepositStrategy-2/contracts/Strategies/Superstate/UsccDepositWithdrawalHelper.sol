...
contract UsccDepositWithdrawalHelper {
    ...
    constructor(IERC20 _usdcToken, IERC20 _usccToken, address _managedVaultAddress) {
    ...
    function startUsdcDeposit(
        uint256 usdcAmount,
        uint256 expectedUsccAmount
    ) external onlyManagedVault {
        ...
        usdcToken.safeTransferFrom(managedVaultAddress, superstateDepositAddress, usdcAmount);
    ...
    function setSuperstateDepositWithdrawAddresses(
        ...
        if (superstateDepositAddress != address(0) || superstateWithdrawAddress != address(0))
            revert SuperstateAddressAlreadySet();
