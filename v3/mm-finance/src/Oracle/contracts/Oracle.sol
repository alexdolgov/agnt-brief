...
contract Oracle {
    ...
    function sortTokens(address tokenA, address tokenB) public pure returns (address token0, address token1) {
        require(tokenA != tokenB, 'MeerkatFactory: IDENTICAL_ADDRESSES');
        (token0, token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), 'MeerkatFactory: ZERO_ADDRESS');
    }
    ...
    function update(address tokenA, address tokenB) external {
        require(msg.sender == priceUpdater, 'MeerkatOracle: Price can update only price updater address');
        address pair = pairFor(tokenA, tokenB);

        Observation storage observation = pairObservations[pair];
        uint timeElapsed = block.timestamp - observation.timestamp;
        require(timeElapsed >= CYCLE, 'MeerkatOracle: PERIOD_NOT_ELAPSED');
        (uint price0Cumulative, uint price1Cumulative,) = MeerkatOracleLibrary.currentCumulativePrices(pair);
