...
import { IMoneyFiStartegyUpgradeableAllBridge } from "../interfaces/IMoneyFiStartegyUpgradeableAllBridge.sol";

contract MoneyFiStartegyUpgradeableAllBridge is
    MoneyFiStartegyUpgradeableBase,
    UUPSUpgradeable,
    IMoneyFiStartegyUpgradeableAllBridge
{
    ...
    function initialize(address admin_, address asset_, address router_, address allBridgeAddress_) public initializer {
        __DefaultAccessControlEnumerable_init(admin_);
        _MoneyFiStartegyUpgradeableBase_init(IERC20(asset_), router_);
    ...
    function totalAssets() public view override returns (uint256) {
        if (address(allBridgePool) != address(0)) {
            ...
            // uint256 actualUnderlyingAssetBalance = strategyLpBalance;
    ...
    /// @dev Internal hook triggered after withdraw
    ...
    function withdrawEmergency() public onlyAdmin {
        ...
        ERC20(address(ASSET)).safeTransfer(msg.sender, ASSET.balanceOf(address(this)));
    }
}
