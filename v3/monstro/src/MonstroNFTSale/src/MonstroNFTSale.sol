...
// Interface for ERC20 tokens that support the permit function (EIP-2612)
 ...
 * Now supports gasless transactions via meta-transactions with USDC permit.
 */
contract MonstroNFTSale is Ownable, EIP712 {
    ...
    IERC20Permit private usdcPermit;
    ...
    /**
     * @dev Gasless version of mintAndInvest that uses USDC permit
     * This function only requires a single signature (the USDC permit)
     */
    function mintAndInvestWithPermit(
        uint256 typeId,
        uint256 baseAmount,
        address user,
        uint256 deadline,
    ...
    /**
     * @dev Gasless version of addOnFunds that uses USDC permit
     * This function only requires a single signature (the USDC permit)
     */
    function addOnFundsWithPermit(
        uint256 tokenId,
        uint256 addOnAmount,
        address user,
        uint256 deadline,
    ...
    /**
     * @dev Returns the current USDC nonce for a user (for permit signatures)
     */
    function getUsdcNonce(address user) external view returns (uint256) {
        return usdcPermit.nonces(user);
    }
