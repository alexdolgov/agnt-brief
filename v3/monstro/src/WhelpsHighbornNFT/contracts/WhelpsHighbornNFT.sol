...
/**
 * @title WhelpsHighbornNFT
 * @dev ERC721-compatible NFT contract with 2000 implicit tokens, holder tracking, batch burning, 
 ...
 */
contract WhelpsHighbornNFT {
    // Immutable deployer address, set at construction
    address public immutable owner;

    // Fixed total supply of 2000 tokens (IDs 0–1999)
    uint256 public constant TOTAL_SUPPLY = 2000;

    // Private storage for token metadata
    string private _name;        // Token name (e.g., "Whelps: Highborn")
    string private _symbol;      // Token symbol (e.g., "HBORNNFT")
    string private _baseTokenURI;// Base URI for token metadata (e.g., "https://meta.monstro.fun/highborn/")
     ...
     * @param name_ Token name (e.g., "Whelps: Highborn")
     * @param symbol_ Token symbol (e.g., "HBORNNFT")
     * @param initialBaseURI Base URI (e.g., "https://meta.monstro.fun/highborn/")
     */
    constructor(string memory name_, string memory symbol_, string memory initialBaseURI) {
        ...
        _balances[msg.sender] = TOTAL_SUPPLY;  // Implicitly assign all 2000 tokens to deployer
    ...
    /**
     * @dev Returns the fixed total supply (2000).
     */
