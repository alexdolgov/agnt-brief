...
/**
 * @title WhelpsNFT
 * @dev ERC721-compatible NFT contract with 7777 implicit tokens, holder tracking, batch burning, 
...
contract WhelpsNFT {
    ...
    // Fixed total supply of 7777 tokens (IDs 0–7776)
    uint256 public constant TOTAL_SUPPLY = 7777;

    // Private storage for token metadata
    string private _name;        // Token name (e.g., "Whelps")
    string private _symbol;      // Token symbol (e.g., "WLPSNFT")
    string private _baseTokenURI;// Base URI for token metadata (e.g., "https://meta.monstro.fun/whelps/")
     ...
     * @param name_ Token name (e.g., "Whelps")
     * @param symbol_ Token symbol (e.g., "WLPSNFT")
     * @param initialBaseURI Base URI (e.g., "https://meta.monstro.fun/whelps/")
     */
    constructor(string memory name_, string memory symbol_, string memory initialBaseURI) {
        ...
        _balances[msg.sender] = TOTAL_SUPPLY;  // Implicitly assign all 7777 tokens to deployer
    ...
    /**
     * @dev Returns the fixed total supply (7777).
     */
