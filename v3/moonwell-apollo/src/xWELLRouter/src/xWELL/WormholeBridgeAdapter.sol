    ...
    WormholeTrustedSender
{
    ...
    /// @notice V6 upgrade: fix quoterAddress to point at the off-chain quoter operator's signer EOA.
    /// @dev    The router's quoterContract mapping is keyed by the signer EOA
    /// @param _quoterSigner off-chain quoter operator's signing key
    function initializeV6(address _quoterSigner) external reinitializer(6) {
        require(_quoterSigner != address(0), "WormholeBridge: zero address");
        require(
            address(executorQuoterRouter) != address(0),
            "WormholeBridge: V5 not initialized"
        );
        quoterAddress = _quoterSigner;
    }
