...
contract TermEthPlusPriceFeedV1 is AggregatorV3Interface {
    ...
    IEthPlus immutable ethPlus = IEthPlus(referenceToken);
    constructor(
        AggregatorV3Interface ethPriceFeed_,
        IBasketHandler basketHandler_,
        IAssetRegistry assetRegistry_
    ) {
        require(
            address(ethPriceFeed_) != address(0),
            "ethPriceFeed_ cannot be 0"
        );
        ethPriceFeed = ethPriceFeed_;
        require(
            address(basketHandler_) != address(0),
            "basketHandler_ cannot be 0"
        );
        basketHandler = basketHandler_;
        require(
            address(assetRegistry_) != address(0),
            "assetRegistry_ cannot be 0"
        );
    ...
    function description() external pure returns (string memory) {
        return "Term Finance Eth+ price feed";
    }
        ...
        )
    {
        ...
        (address[] memory erc20s, uint256[] memory amounts) = basketHandler
            .quote(baskets, IBasketHandler.RoundingMode.FLOOR);
        // Summation LOOP:
    ...
    function _scaleDownBasket(
        uint192 amtRToken
    ) internal view returns (uint192 amtBaskets) {
        uint192 basketsNeeded = ethPlus.basketsNeeded();
        uint256 totalSupply = ethPlus.totalSupply();
        amtBaskets = _safeWrap(
            _mulDiv256(basketsNeeded, amtRToken, totalSupply)
        );
    }
