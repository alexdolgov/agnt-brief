...
/// clip.sol -- rwaUSD auction module 2.0
...
contract Clipper {
    ...
    address public vow; // Recipient of rwaUSD raised in auctions
    ...
    struct Sale {
        ...
        uint256 tab; // rwaUSD to raise       [rad]
    ...
    //
    // Auctions will not collect more rwaUSD than their assigned rwaUSD target,`tab`;
    // thus, if `amt` would cost more rwaUSD than `tab` at the current price, the
    // amount of collateral purchased will instead be just enough to collect `tab` rwaUSD.
    //
    // To avoid partial purchases resulting in very small leftover auctions that will
    // never be cleared, any partial purchase must leave at least `Clipper.chost`
    // remaining rwaUSD target. `chost` is an asynchronously updated value equal to
    ...
    function take(
        ...
        uint256 max, // Maximum acceptable price (rwaUSD / collateral) [ray]
        ...
        isStopped(3)
    {
...

        {
            ...
            // rwaUSD needed to buy a slice of this sale
            owe = mul(slice, price);

            // Don't collect more than tab of rwaUSD
            if (owe > tab) {
            ...
            // Get rwaUSD from caller
            vat.move(msg.sender, vow, owe);

            // Removes rwaUSD out for liquidation from accumulator
        ...
        emit Take(msg.sender, id, max, price, owe, tab, lot, usr);
    }
