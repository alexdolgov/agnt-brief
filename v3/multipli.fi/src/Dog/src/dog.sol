...
/// dog.sol -- rwaUSD liquidation module 2.0
...
contract Dog {
    ...
    struct Ilk {
        ...
        uint256 hole; // Max rwaUSD needed to cover debt+fees of active auctions per ilk [rad]
        uint256 dirt; // Amt rwaUSD needed to cover debt+fees of active auctions per ilk [rad]
    }
    ...
    uint256 public Hole; // Max rwaUSD needed to cover debt+fees of active auctions [rad]
    uint256 public Dirt; // Amt rwaUSD needed to cover debt+fees of active auctions [rad]
    ...
    //
    // Liquidate a Vault and start a Dutch auction to sell its collateral for rwaUSD.
    //
    // The third argument is the address that will receive the liquidation reward, if any.
    //
    // The entire Vault will be liquidated except when the target amount of rwaUSD to be raised in
    ...
    // outstanding rwaUSD target. The one exception is if the resulting auction would likely
        ...
        uint256 dust;
        {
            ...
            if (art > dart) {
                if (mul(art - dart, rate) < dust) {
                    ...
                    // the extra amount of target rwaUSD over the limits intended is not of economic concern.
                    dart = art;
                } else {
