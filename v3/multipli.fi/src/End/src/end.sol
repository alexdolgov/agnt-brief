...
/*
    ...
    the final rwaUSD / collateral price. In particular, we need to determine

      a. `gap`, the collateral shortfall per collateral type by
         considering under-collateralised CDPs.

      b. `debt`, the outstanding rwaUSD supply after including system
         surplus / deficit
    ...
    We determine (b) by processing ongoing rwaUSD generating processes,
    i.e. auctions. We need to ensure that auctions will not generate any
    further rwaUSD income.
    ...
    4a. i) `wait`: set the cooldown period to be at least as long as the
           ...
           auction dynamics due to the now varying price of rwaUSD.

       ii) `skip`: cancel all ongoing auctions and seize the collateral.

           This allows for faster processing at the expense of more
           processing calls. This option allows rwaUSD holders to retrieve
    ...
    So the way to ensure the protocol will not receive extra rwaUSD income is:
    ...
    6. `thaw()`:
       ...
       - fixes the total outstanding supply of rwaUSD
    ...
    type and rwaUSD holders can now turn their rwaUSD into collateral. Each
    unit rwaUSD can claim a fixed basket of collateral.

    rwaUSD holders must first `pack` some rwaUSD into a `bag`. Once packed,
    rwaUSD cannot be unpacked and is not transferrable. More rwaUSD can be
    added to a bag later.

    8. `pack(wad)`:
        - put some rwaUSD into a bag in preparation for `cash`

    Finally, collateral can be obtained with `cash`. The bigger the bag,
    the more collateral can be released.

    9. `cash(ilk, wad)`:
        - exchange some rwaUSD from your bag for gems from a specific ilk
...
contract End {
    ...
    uint256 public debt; // Total outstanding rwaUSD following processing [rad]
    ...
    function thaw() external {
        ...
        require(vat.rwaUSD(address(vow)) == 0, "End/surplus-not-zero");
