...
contract DssCdpManager {
    ...
    event NewCdp(address usr, address indexed own, uint256 indexed cdp, address indexed cdpAddress);
    event Give(address indexed src, address indexed dst, uint256 cdp, address indexed cdpAddress);
    event CdpAllow(address indexed own, address indexed usr, uint256 cdp, address indexed cdpAddress, uint256 ok);
    event UrnAllow(address indexed usr, address indexed dst, uint256 ok);
    ...
    function cdpAllow(uint256 cdp, address usr, uint256 ok) public cdpAllowed(cdp) {
        ...
        emit CdpAllow(owns[cdp], usr, cdp, urns[cdp], ok);
    }
    ...
    function urnAllow(address usr, uint256 ok) public {
        ...
        emit UrnAllow(msg.sender, usr, ok);
    }
    ...
    function open(bytes32 ilk, address usr) public returns (uint256) {
        ...
        emit NewCdp(msg.sender, usr, cdpi, urns[cdpi]);
        return cdpi;
    }
    ...
    function give(uint256 cdp, address dst) public cdpAllowed(cdp) {
        ...
        emit Give(src, dst, cdp, urns[cdp]);
    }

    // Frob the cdp keeping the generated rwaUSD or collateral freed in the cdp urn address.
    ...
    // Transfer wad amount of rwaUSD from the cdp address to a dst address.
