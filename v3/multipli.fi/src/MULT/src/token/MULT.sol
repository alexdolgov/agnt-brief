...
contract MULT is
    ...
    AccessControlDefaultAdminRulesUpgradeable
{
    error Mult__MaxSupplyExceeded(uint256 supplyAfterMint);
    error Mult__InvalidRecipient(address recipient);
    error Mult__AccountFrozen(address account);
    error Mult__AccountNotFrozen(address account);
    error Mult__TransferFromFailed(address src, address dst, uint256 amt);
    ...
    /// @custom:storage-location multipli.storage.MULT
    struct MultStorage {
    ...
    // keccak256(abi.encode(uint256(keccak256("multipli.storage.MULT")) - 1)) & ~bytes32(uint256(0xff));
    bytes32 private constant MULT_STORAGE_LOCATION = 0xde240803044831f1942ae9c9f21b913a3d55d5924b05c74e535e917f47c5d100;

    // solhint-disable-next-line chainlink-solidity/explicit-returns
    function _getMultStorage() private pure returns (MultStorage storage $) {
        assembly {
            $.slot := MULT_STORAGE_LOCATION
        }
    }
    ...
    function initialize(
        ...
        __Mult_init(name, symbol, decimals_, maxSupply_, preMint, defaultAdmin, defaultUpgrader);
    }

    function __Mult_init(
        string memory name,
        string memory symbol,
        uint8 decimals_,
        uint256 maxSupply_,
        ...
        __Mult_init_unchained(decimals_, maxSupply_, preMint, defaultAdmin, defaultUpgrader);
    }

    function __Mult_init_unchained(
        uint8 decimals_,
        uint256 maxSupply_,
        uint256 preMint,
        address defaultAdmin,
        ...
        MultStorage storage $ = _getMultStorage();
        ...
        if (preMint != 0) {
            if (maxSupply_ != 0 && preMint > maxSupply_) {
                revert Mult__MaxSupplyExceeded(preMint);
            }
    ...
    function decimals() public view virtual override returns (uint8) {
        return _getMultStorage().decimals;
    }

    /// @dev Returns the max supply of the token, 0 if unlimited.
    function maxSupply() public view virtual returns (uint256) {
        return _getMultStorage().maxSupply;
    }

    /// @dev Function required by Flapper contract
    function move(address src, address dst, uint256 amount) external {
        bool success = transferFrom(src, dst, amount);
        if (!success) {
            revert Mult__TransferFromFailed(src, dst, amount);
        }
    }

    /// @dev Function required by DSChief contract
    function pull(address src, uint256 amount) external {
        bool success = transferFrom(src, msg.sender, amount);
        if (!success) {
            revert Mult__TransferFromFailed(src, msg.sender, amount);
        }
    }

    /// @dev Function required by DSChief contract
    function push(address dst, uint256 amount) external {
        bool success = transfer(dst, amount);
        if (!success) {
            revert Mult__TransferFromFailed(msg.sender, dst, amount);
        }
    }
    ...
    function mint(address account, uint256 amount) external override onlyRole(MINTER_ROLE) {
        uint256 _maxSupply = _getMultStorage().maxSupply;
        uint256 _totalSupply = totalSupply();

        if (_maxSupply != 0 && _totalSupply + amount > _maxSupply) {
            revert Mult__MaxSupplyExceeded(_totalSupply + amount);
        }
    ...
    function getCCIPAdmin() external view returns (address) {
        return _getMultStorage().ccipAdmin;
    }
    ...
    function freeze(address account) public onlyRole(FREEZER_ROLE) {
        if (account == address(0)) revert Mult__InvalidRecipient(account);
        if (account == address(this)) revert Mult__InvalidRecipient(account);

        MultStorage storage $ = _getMultStorage();
        if ($.isFrozen[account]) revert Mult__AccountFrozen(account);
    ...
    function unfreeze(address account) public onlyRole(FREEZER_ROLE) {
        MultStorage storage $ = _getMultStorage();
        if (!$.isFrozen[account]) revert Mult__AccountNotFrozen(account);
    ...
    function isFrozen(address account) public view returns (bool) {
        return _getMultStorage().isFrozen[account];
    }
    ...
    function _update(address from, address to, uint256 value) internal virtual override {
        _requireNotPaused();
        if (to == address(this)) revert Mult__InvalidRecipient(to);

        MultStorage storage $ = _getMultStorage();
        if ($.isFrozen[from]) revert Mult__AccountFrozen(from);
        if ($.isFrozen[to]) revert Mult__AccountFrozen(to);
    ...
    function _approve(address owner, address spender, uint256 value, bool emitEvent) internal virtual override {
        _requireNotPaused();
        if (spender == address(this)) revert Mult__InvalidRecipient(spender);

        MultStorage storage $ = _getMultStorage();
        if ($.isFrozen[owner]) revert Mult__AccountFrozen(owner);
        if ($.isFrozen[spender]) revert Mult__AccountFrozen(spender);
