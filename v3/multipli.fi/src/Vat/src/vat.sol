...
contract Vat {
    ...
    event Hope(address indexed u, address indexed v);
    event Nope(address indexed u, address indexed v);
    ...
    event Frob(bytes32 indexed ilk, address indexed u, address v, address w, int256 dink, int256 dart, uint256 rate);
    ...
    event Heal(uint256 rad, address indexed u, uint256 vice, uint256 sin, uint256 rwaUSD, uint256 debt);
    event Suck(address indexed u, address indexed v, uint256 rad, uint256 vice, uint256 debt);
    ...
    mapping(address => uint256) public rwaUSD; // [rad]
    mapping(address => uint256) public sin; // [rad]

    uint256 public debt; // Total rwaUSD Issued    [rad]
    uint256 public vice; // Total Unbacked rwaUSD  [rad]
    ...
    function move(address src, address dst, uint256 rad) external {
        require(wish(src, msg.sender), "Vat/not-allowed");
        rwaUSD[src] = sub(rwaUSD[src], rad);
        rwaUSD[dst] = add(rwaUSD[dst], rad);
    ...
    function frob(bytes32 i, address u, address v, address w, int256 dink, int256 dart) external {
        ...
        rwaUSD[w] = add(rwaUSD[w], dtab);

        urns[i][u] = urn;
        ilks[i] = ilk;
        emit Frob(i, u, v, w, dink, dart, ilk.rate);
    }
    ...
    function grab(bytes32 i, address u, address v, address w, int256 dink, int256 dart) external auth {
        ...
        vice = sub(vice, dtab);
        emit Grab(i, u, v, w, dink, dart, vice, sin[w]);
    }
    ...
    function heal(uint256 rad) external {
        ...
        sin[u] = sub(sin[u], rad);
        rwaUSD[u] = sub(rwaUSD[u], rad);
        vice = sub(vice, rad);
        debt = sub(debt, rad);
        emit Heal(rad, u, vice, sin[u], rwaUSD[u], debt);
    }

    function suck(address u, address v, uint256 rad) external auth {
        sin[u] = add(sin[u], rad);
        rwaUSD[v] = add(rwaUSD[v], rad);
        vice = add(vice, rad);
        debt = add(debt, rad);
        emit Suck(u, v, rad, vice, debt);
    }
