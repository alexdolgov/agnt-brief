...
contract AquaVault is Comptroller, INativeTreasuryV2, UUPSUpgradeable, EIP712Upgradeable, IAquaVault, AquaVaultStorage {
    ...
    function supportMarket(AquaLpToken aquaLpToken, bool enableBorrow) external onlyAdmin {
        lpTokens[aquaLpToken.underlying()] = aquaLpToken;
        isLpTokenBorrowEnabled[address(aquaLpToken)] = enableBorrow;
        _supportMarket(aquaLpToken);
    }

    function setLpTokenBorrowEnableStatus(AquaLpToken aquaLpToken, bool value) external onlyAdmin {
        isLpTokenBorrowEnabled[address(aquaLpToken)] = value;
    }
    ...
    function borrowAllowed(address cToken, address borrower, uint borrowAmount) public override returns (uint) {
        if (isLpTokenBorrowEnabled[cToken]) {
            revert BorrowNotEnabled();
        }
