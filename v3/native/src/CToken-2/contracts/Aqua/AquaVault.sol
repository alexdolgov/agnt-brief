...
contract AquaVault is Comptroller, INativeTreasuryV2, UUPSUpgradeable, IAquaVault, AquaVaultStorage {
    ...
    function initialize(address aquaVaultSignatureCheck_) public initializer NonZero(aquaVaultSignatureCheck_) {
    ...
    function setAdmin(address newAdmin) external onlyAdmin NonZero(newAdmin) {
        admin = newAdmin;
    }

    /// @notice Called by admin to set new native pool
    /// @param newNativePool The address of the new native pool
    function setNativePool(address newNativePool) external onlyAdmin NonZero(newNativePool) {
    ...
    function setSignatureCheck(address newSignatureCheck) external onlyAdmin NonZero(newSignatureCheck) {        
    ...
    function setSigner(address _signer) external onlyAdmin NonZero(_signer){
        signer = _signer;
    }

    /// @notice Called by admin to set new epoch updater
    /// @param _epochUpdater The address of the new epoch updater
    function setEpochUpdater(address _epochUpdater) external onlyAdmin NonZero(_epochUpdater){
    ...
    function positionEpochUpdate(
        ...
        AquaVaultLogic.positionEpochUpdate(traderPositionUpdate, lpTokenValueUpdate, positions, lpTokens, lastEpochUpdateTimestamp);
        lastEpochUpdateTimestamp = block.timestamp;
