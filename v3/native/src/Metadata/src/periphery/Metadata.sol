...
contract Metadata is Ownable2Step {
    ...
    /// @notice TVL monitor threshold in basis points
    uint256 public tvlMonitorThresholdBps;
    ...
    /// @notice Emitted when TVL monitor threshold is updated
    /// @param oldThreshold The previous TVL monitor threshold in basis points
    /// @param newThreshold The new TVL monitor threshold in basis points
    event TvlMonitorThresholdUpdated(uint256 oldThreshold, uint256 newThreshold);
    ...
    /// @notice Sets the TVL monitor threshold
    /// @param threshold The new TVL monitor threshold in basis points
    function setTvlMonitorThreshold(uint256 threshold) external onlyOwner {
        require(threshold <= 10_000, "threshold exceeds maximum");

        uint256 oldThreshold = tvlMonitorThresholdBps;
        tvlMonitorThresholdBps = threshold;

        emit TvlMonitorThresholdUpdated(oldThreshold, threshold);
    }
