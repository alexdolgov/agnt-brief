...
interface INativeBridge is IQuote {
    ...
    /// @dev SOURCE CHAIN: token = sellerToken, amount = sellerTokenAmount, timestamp = initiate time
    /// @dev DEST CHAIN: token = buyerToken, amount = buyerTokenAmount, timestamp = fill time
    struct Order {
    ...
    /// @dev Used in initiate() when block.timestamp >= initDeadline or initDeadline >= quote.deadlineTimestamp
    ...
    /// @dev Used in initiate() when enabledDestination[destChainId] is false
    error InvalidDestChain();

    /// @notice Thrown when an EIP-712 signature verification fails
    /// @dev Used in refund() when the recovered signer is not authorized
    error InvalidSignature();

    /// @notice Thrown when a quoteId is invalid or already used
    /// @dev Used in initiate() when order already exists, or in refund() when order not in Committed state
    error InvalidQuoteId();

    /// @notice Thrown when token validation fails
    /// @dev Used in initiate() when native ETH sent but sellerToken is not WETH9
    /// @dev Used in _claim() when orders don't all have the same token
    error InvalidToken();

    /// @notice Thrown when attempting to fill an already completed order
    /// @dev Used in fill() when destOrder already exists, or in _claim() when order not in Committed state
