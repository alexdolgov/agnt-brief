    ...
    INativeBridge
{
    ...
    function initiate(
        ...
        if (msg.value > 0) {
            ...
            _initiate(swapper, input, output, dstRouterData);
        } else {
    ...
    function _claim(ClaimData memory claimData) internal {
        ...
        if (claimData.users.length == 0) revert NativeBridgeError(ErrorCodes.InvalidOrder);
        if (claimData.quoteIds.length == 0) revert NativeBridgeError(ErrorCodes.InvalidOrder);
    ...
    function _validateDestinationIntent(INativeRfqPool.RFQTQuote memory dstRouterData) internal view {
        // validation for destination chain
        if (userOrder[dstRouterData.recipient][dstRouterData.quoteId].orderState == OrderState.Completed) {
            revert NativeBridgeError(ErrorCodes.OrderAlreadyCompleted);
        }
