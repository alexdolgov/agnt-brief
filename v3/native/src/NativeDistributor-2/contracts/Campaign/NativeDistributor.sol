...
contract NativeDistributor is Ownable, ReentrancyGuard {
    ...
    /// @param campaign The ID of the campaign
    /// @param token The address of the ERC20 token being withdrawn
    /// @param amount The amount of tokens being withdrawn
    event Withdraw(uint256 campaign, address token, uint256 amount);
    ...
    /// @param amount The amount of tokens being claimed
    function claim(address user, uint256 campaignId, bytes32[] calldata proof, uint256 amount) external nonReentrant {
        ...
        if (hasClaimed[user][campaignId]) revert UserAlreadyClaimed();
        bytes32 leaf = keccak256(bytes.concat(keccak256(abi.encode(user, campaignId, amount))));
        if (!MerkleProof.verify(proof, data.root, leaf)) revert InvalidProof();
        hasClaimed[user][campaignId] = true;
        campaignData[campaignId].amountUnclaimed = campaignData[campaignId].amountUnclaimed - amount;
        IERC20(data.token).safeTransfer(user, amount);
        emit Claim(user, campaignId, amount);
    }
    ...
    /// @param campaignId The ID of the campaign
    /// @param amount The amount of tokens being deposited
    function depositForCampaign(address token, uint256 campaignId, uint256 amount) external onlyOwner {
        if (campaignId == 0) revert NonZero();
    ...
    /// @notice Allows the owner to set the Merkle root for a campaign
    /// @param campaignId The ID of the campaign
    /// @param root The Merkle root being set
    function setRoot(uint256 campaignId, bytes32 root) external onlyOwner {
        if (campaignId == 0) revert NonZero();
