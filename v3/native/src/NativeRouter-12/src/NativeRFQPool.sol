...
contract NativeRFQPool is INativeRFQPool, Ownable2Step, EIP712 {
    ...
    function tradeRFQT(uint256 effectiveSellerTokenAmount, address wNLPIn, RFQTQuote memory quote) external override {
        ...
        // Only calculate time decay slippage if decayExponent > 0
        if (quote.decayExponent > 0 && block.timestamp > quote.decayStartTime) {
            // Calculate time ratio with 1e6 precision: preserves accuracy while preventing overflow risk with exponentiation
            uint256 timeRatio =
                ((block.timestamp - quote.decayStartTime) * 1e6) / (quote.deadlineTimestamp - quote.decayStartTime);

            // Apply exponential decay curve: timeRatio^decayExponent
            uint256 ratioPowered = timeRatio ** quote.decayExponent;
            uint256 scalePowered = 1e6 ** quote.decayExponent;

            // Calculate final slippage in basis points
            uint256 slippageBps =
                (ratioPowered * quote.maxSlippageBps * BPS_DENOMINATOR) / (scalePowered * BPS_DENOMINATOR);

            // Apply time decay slippage to reduce buyer token amount
            buyerTokenAmount = buyerTokenAmount * (BPS_DENOMINATOR - slippageBps) / BPS_DENOMINATOR;
        }
    ...
    function _transferAndCallback(
        ...
        if (wNLPIn != address(0)) {
            require(
                (isWrapToken[wNLPIn] && address(WNLP(wNLPIn).underlying()) == buyerToken), ErrorsLib.InvalidWrapToken()
            );

            _transferFromTreasury(buyerToken, address(this), buyerTokenAmount);

            // Approve WNLP contract to spend underlying tokens
            IERC20(buyerToken).safeApprove(address(wNLPIn), buyerTokenAmount);

            // Deposit underlying tokens and wrap to wnlp tokens for recipient
            WNLP(wNLPIn).depositAndWrapFor(recipient, buyerTokenAmount);
        } else {
            // Direct transfer to recipient without wrapping
