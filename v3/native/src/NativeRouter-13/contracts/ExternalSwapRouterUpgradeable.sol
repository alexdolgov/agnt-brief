            ...
            if (bytes4(fallbackSwapCalldata) == bytes4(0x12aa3caf)) {
                {
                    ...
                    fallbackSwapCalldata = abi.encodeWithSelector(bytes4(fallbackSwapCalldata), executor, desc, permit, data);
                }
            } else if (bytes4(fallbackSwapCalldata) == bytes4(0xe449022e)) {
                {
                    ...
                    fallbackSwapCalldata = abi.encodeWithSelector(bytes4(fallbackSwapCalldata), amount, minReturn, pools);
                }
            } else if (bytes4(fallbackSwapCalldata) == bytes4(0x0502b1c5)) {
                {
                    (address srcToken, uint256 amount, uint256 minReturn, uint256[] memory pools) = abi.decode(
                        functionParams,
                        (address, uint256, uint256, uint256[])
                    );
                    amount = sellerTokenAmount;
                    minReturn = buyerTokenAmount;
                    fallbackSwapCalldata = abi.encodeWithSelector(bytes4(fallbackSwapCalldata), srcToken, amount, minReturn, pools);
                }
            } else if (
