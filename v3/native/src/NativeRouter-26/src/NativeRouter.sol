...
contract NativeRouter is INativeRouter, EIP712, Ownable2Step, Pausable, Multicall, ReentrancyGuardTransient {
    ...
    /// @notice Mapping of all trusted external swap sources (pools, routers)
    mapping(address => bool) public externalSwapSources;
    ...
    function tradeRFQT(
        ...
        require(quote.confidenceExtractedValueN <= quote.deadlineTimestamp, ErrorsLib.InvalidConfidenceFactorN());
        require(quote.confidenceExtractedValueE <= MAX_CONFIDENCE_FACTOR_E, ErrorsLib.InvalidConfidenceFactorE());
        ...
        if (actualSellerAmount > 0) {
            if (actualSellerAmount > quote.sellerTokenAmount) {
                // Upward deviation: max 10%
                uint256 upwardDeviation =
                    (actualSellerAmount - quote.sellerTokenAmount) * 10_000 / quote.sellerTokenAmount;
                require(upwardDeviation < MAX_AMOUNT_UPWARD_DEVIATION_BPS, ErrorsLib.AmountDeviationExceeds());
            } else {
                // Downward deviation: max 50%
                uint256 downwardDeviation =
                    (quote.sellerTokenAmount - actualSellerAmount) * 10_000 / quote.sellerTokenAmount;
                require(downwardDeviation < MAX_AMOUNT_DOWNWARD_DEVIATION_BPS, ErrorsLib.AmountDeviationExceeds());
            }
        ...
        } else if (actualSellerAmount > 0 && quote.amountOutMinimum > 0) {
            // Proportionally adjust amountOutMinimum when actualSellerAmount is provided but actualMinOutputAmount is not
            quote.amountOutMinimum = (quote.amountOutMinimum * effectiveSellerTokenAmount) / quote.sellerTokenAmount;
        }
    ...
    /// @notice Execute an external swap through external liquidity sources
    /// @param params The external swap parameters
    function externalSwap(ExternalSwap calldata params) external payable override nonReentrant whenNotPaused {
        require(block.timestamp <= params.deadlineTimestamp, ErrorsLib.QuoteExpired());
        require(externalSwapSources[params.source], ErrorsLib.InvalidExternalSwapSource());

        _verifyExternalSwap(params);

        bool isBuyerTokenNative = params.buyerToken == address(0);

        // Record recipient's initial balance
        uint256 initialBuyerTokenBalance =
            isBuyerTokenNative ? params.recipient.balance : IERC20(params.buyerToken).balanceOf(params.recipient);

        // Handle sellerToken transfer and approval
        if (params.sellerToken == address(0)) {
            // Native token
            require(msg.value == params.sellerTokenAmount, ErrorsLib.InvalidAmount());
        } else {
            // ERC20 token
            require(msg.value == 0, ErrorsLib.UnexpectedMsgValue());
            IERC20(params.sellerToken).safeTransferFrom(msg.sender, address(this), params.sellerTokenAmount);
            // Approve the spender to use exact amount
            IERC20(params.sellerToken).safeApprove(params.spender, params.sellerTokenAmount);
        }

        // Execute external swap call
        (bool success,) = params.source.call{value: msg.value}(params.callData);

        require(success, ErrorsLib.ExternalCallFailed());

        // Check recipient's final balance
        uint256 finalBuyerTokenBalance =
            isBuyerTokenNative ? params.recipient.balance : IERC20(params.buyerToken).balanceOf(params.recipient);

        uint256 receivedAmount = finalBuyerTokenBalance - initialBuyerTokenBalance;
        require(
            receivedAmount >= params.amountOutMinimum,
            ErrorsLib.NotEnoughAmountOut(receivedAmount, params.amountOutMinimum)
        );

        emit ExternalSwapped(
            params.source,
            params.recipient,
            params.sellerToken,
            params.buyerToken,
            params.sellerTokenAmount,
            receivedAmount,
            params.quoteId
        );
    }
    ...
    /// @notice Updates external swap source whitelist status
    /// @param source The address of the external swap source
    /// @param isActive true to whitelist, false to remove from whitelist
    function setExternalSwapSource(address source, bool isActive) external onlyOwner {
        require(source != address(0), ErrorsLib.ZeroAddress());

        externalSwapSources[source] = isActive;

        emit ExternalSwapSourceUpdated(source, isActive);
    }
    ...
    function _verifyExternalSwap(ExternalSwap calldata params) internal view {
        bytes32 digest = _hashTypedDataV4(
            keccak256(
                abi.encode(
                    EXTERNAL_SWAP_SIGNATURE_HASH,
                    params.source,
                    params.spender,
                    params.recipient,
                    params.sellerToken,
        ...
        address recoveredSigner = ECDSA.recover(digest, params.signature);
