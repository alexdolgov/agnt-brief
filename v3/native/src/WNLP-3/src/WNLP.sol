...
contract WNLP is ERC20, Ownable2Step, ReentrancyGuardTransient {
    ...
    function depositAndWrap(uint256 amount) external nonReentrant returns (uint256) {
        ...
        return _depositAndWrap(msg.sender, msg.sender, amount);
    }
    ...
    /// @param amount Amount of underlying tokens to deposit
    /// @return Amount of wNLP tokens received
    function depositAndWrapFor(address to, uint256 amount) external nonReentrant returns (uint256) {
        require(to != address(0), ErrorsLib.ZeroAddress());
        require(amount > 0, ErrorsLib.ZeroAmount());
        require(trustedOperators[msg.sender], ErrorsLib.NotTrustedOperator());

        return _depositAndWrap(msg.sender, to, amount);
    }
    ...
    function _depositAndWrap(address from, address to, uint256 amount) internal returns (uint256) {
        ...
        // Update the recipient's deposit timestamp for cooldown tracking
        lastDepositTimestamp[to] = block.timestamp;

        // Mint wNLP tokens to the specified recipient
        _mint(to, wNLPAmount);

        emit DepositAndWrapped(from, to, amount, wNLPAmount);
