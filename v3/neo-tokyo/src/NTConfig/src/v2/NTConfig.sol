...
contract NTConfig is OwnableUpgradeable, UUPSUpgradeable {
    ...
    function initialize() external initializer {
        __Ownable_init();
        __UUPSUpgradeable_init(); // this actually does nothing
    }
    ...
    function enlistSecondaryMetadata(
        ...
        } else if (metadata == NTSecondaryComponent.S1_CITIZEN_HAND_MINT) {
            _secondaryMetadataContracts.s1CitizenHand = addr;
        }
    }
    ...
    function _seasonChecker(address addr) internal view returns (NTSeason) {
        if (
            ...
            addr == _secondaryMetadataContracts.s1CitizenHand
        ) {
            return NTSeason.SEASON_1;
        } else if (
    ...
    function _validateCaller(
        ...
        if (
            caller == v2Components.s1Identity ||
            caller == v1Components.s1Identity
        ) {
