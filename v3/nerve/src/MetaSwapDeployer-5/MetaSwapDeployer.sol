...
// File openzeppelin-contracts-3.4/proxy/Clones.sol@v3.4.1
...
// File contracts/meta/MetaSwapDeployer.sol
...
contract MetaSwapDeployer is Ownable {
    ...
    function deploy(deployData memory data) external returns (address) {
        address swapClone = Clones.clone(data.swapAddress);
        IMetaSwap(swapClone).metaInitialize(
            data._pooledTokens,
            data.decimals,
            data.lpTokenName,
            data.lpTokenSymbol,
        ...
        emit NewMetaSwapPool(msg.sender, swapClone, data._pooledTokens);
        return swapClone;
    }
}