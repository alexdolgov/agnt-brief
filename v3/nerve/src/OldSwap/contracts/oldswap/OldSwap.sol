...
import "./OldOwnerPausable.sol";
import "./OldSwapUtils.sol";
 ...
 *
 * @dev Most of the logic is stored as a library `OldSwapUtils` for the sake of reducing contract's
 * deployment size.
 */
contract OldSwap is OldOwnerPausable, ReentrancyGuard {
    using SafeERC20 for IERC20;
    using SafeMath for uint256;
    using MathUtils for uint256;
    using OldSwapUtils for OldSwapUtils.Swap;

    // Struct storing data responsible for automatic market maker functionalities. In order to
    // access this data, this contract uses OldSwapUtils library. For more details, see OldSwapUtils.sol
    OldSwapUtils.Swap public swapStorage;
    ...
    // events replicated from OldSwapUtils to make the ABI easier for dumb
    // clients
    event TokenSwap(
    ...
    constructor(
        ...
        address _devaddr
    ) public OldOwnerPausable() ReentrancyGuard() {
        ...
        for (uint8 i = 0; i < _pooledTokens.length; i++) {
            ...
            require(
                decimals[i] <= OldSwapUtils.POOL_PRECISION_DECIMALS,
                "Token decimals exceeds max"
            );
            precisionMultipliers[i] =
                10 **
                    uint256(OldSwapUtils.POOL_PRECISION_DECIMALS).sub(
                        uint256(decimals[i])
                    );
        ...
        require(_a < OldSwapUtils.MAX_A, "_a exceeds maximum");
        require(_fee < OldSwapUtils.MAX_SWAP_FEE, "_fee exceeds maximum");
        require(
            _adminFee < OldSwapUtils.MAX_ADMIN_FEE,
            "_adminFee exceeds maximum"
        );
        require(
            _withdrawFee < OldSwapUtils.MAX_WITHDRAW_FEE,
            "_withdrawFee exceeds maximum"
        );
        require(
            _depositFee < OldSwapUtils.MAX_DEPOSIT_FEE,
            "_depositFee exceeds maximum"
        );

        // Initialize swapStorage struct
        swapStorage.lpToken = new OldLPToken(
            lpTokenName,
            lpTokenSymbol,
            OldSwapUtils.POOL_PRECISION_DECIMALS
        );
        swapStorage.pooledTokens = _pooledTokens;
        swapStorage.tokenPrecisionMultipliers = precisionMultipliers;
        swapStorage.balances = new uint256[](_pooledTokens.length);
        swapStorage.initialA = _a.mul(OldSwapUtils.A_PRECISION);
        swapStorage.futureA = _a.mul(OldSwapUtils.A_PRECISION);
