 ...
 */
 contract SimpleRewarderPerSec is IRewarder, Ownable, ReentrancyGuard {
    ...
    event DurationUpdated(uint256 newDuration, uint256 newEndTime);
    ...
    constructor(
        ...
        poolInfo = PoolInfo({lastRewardTimestamp: startTime, accTokenPerShare: 0});
    }
    ...
    function _setRewardRate(uint256 _tokenPerSec) internal {
        updatePool();
    ...
    function setRewardRate(uint256 _tokenPerSec) external onlyOwner {
        _setRewardRate(_tokenPerSec);
    }

    function _extendDuration(uint256 _extendedDuration) internal {
        if (block.timestamp <= endTime){
            // previous rewards not finished yet
            endTime = endTime.add(_extendedDuration);
            duration = duration + _extendedDuration;
        } else {
            // previous rewards finished
            endTime = block.timestamp + _extendedDuration;
            duration = _extendedDuration;
        }
        emit DurationUpdated(duration, endTime);
    }

    function extendDuration(uint256 _extendedDuration) public onlyOwner {
        _extendDuration(_extendedDuration);
    }

    function renewRewardRateAndEndTime(uint256 _tokenPerSec, uint256 _extendedDuration) public onlyOwner {
        _setRewardRate(_tokenPerSec);
        _extendDuration(_extendedDuration);
    }
