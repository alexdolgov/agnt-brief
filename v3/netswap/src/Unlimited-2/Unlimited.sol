...
contract Unlimited {
    ...
    function extendDepositDuration(uint256 _newDuration) atPhase(Phase.Deposit) external {
        require(
            msg.sender == Ownable(address(padFactory)).owner(),
            "Unlimited: caller is not PadFactory owner"
        );
        require(_newDuration > DEPOSIT_DURATION, "invalid");
        DEPOSIT_DURATION = _newDuration;
    }

    function extendLaunchTime(uint256 _newLaunchTime) external {
        require(
            msg.sender == Ownable(address(padFactory)).owner(),
            "Unlimited: caller is not PadFactory owner"
        );
        require(_newLaunchTime > launchTime, "invalid");
