...
abstract contract CoreStrategyAaveGrail is BaseStrategy {
    ...
    event DebtRebalance(uint256 indexed debtRatio, uint256 indexed swapAmount, uint256 indexed slippage);
    event CollatRebalance(uint256 indexed collatRatio, uint256 indexed adjAmount);
    ...
    constructor(address _vault, CoreStrategyAaveConfig memory _config) BaseStrategy(_vault) {
    ...
    function prepareReturn(
        ...
        if (_loss >= _profit) {
            ...
            _loss = 0;
            (uint256 insurancePayment, uint256 compensation) = insurance.reportProfit(totalDebt, _profit);
            ...
            if (insurancePayment > 0 && insurancePayment < _profit) {
                SafeERC20.safeTransfer(want, address(insurance), insurancePayment);
            }
        }
    }
    ...
    function setDebtThresholds(uint256 _lower, uint256 _upper, uint256 _rebalancePercent) external onlyAuthorized {
    ...
    function liquidateAllPositionsInternal() internal returns (uint256 _amountFreed, uint256 _loss) {
        _withdrawAllPooled();
        _removeAllLp();
     ...
     */
    function collateralCapReached(uint256 _amount) public view virtual returns (bool) {
    ...
    function _rebalanceCollateralInternal() internal {
        ...
        if (collatRatio > collatTarget) {
            uint256 adjAmount = (shortPos.sub(lendPos.mul(collatTarget).div(BASIS_PRECISION))).mul(BASIS_PRECISION).div(
            ...
            uint256 adjAmount = ((lendPos.mul(collatTarget).div(BASIS_PRECISION)).sub(shortPos))
                .mul(BASIS_PRECISION)
    ...
    function _deploy(uint256 _amount) internal {
        ...
        uint256 borrow = collatTarget.mul(_amount).mul(1e18).div(
            BASIS_PRECISION.mul((collatTarget.mul(lpPrice).div(BASIS_PRECISION).add(oPrice)))
        );
     ...
     */
    function _calcDeployment(uint256 _amount) internal returns (uint256 _lendNeeded, uint256 _borrow) {
        uint256 oPrice = getOraclePrice();
        uint256 lpPrice = getLpPrice();
        ...
        if (Di > Si2) {
            numerator = (collatTarget.mul(_amount).mul(1e18).add(CrPlp.mul(Di.sub(Si2)))).sub(
                oPrice.mul(BASIS_PRECISION).mul(Di)
            );
        } else {
            numerator = (collatTarget.mul(_amount).mul(1e18).sub(CrPlp.mul(Si2.sub(Di)))).sub(
                oPrice.mul(BASIS_PRECISION).mul(Di)
            );
        }

        _borrow = numerator.div(BASIS_PRECISION.mul(oPrice.add(CrPlp.div(BASIS_PRECISION))));
        _lendNeeded = _amount.sub((_borrow.add(Si2).sub(Di)).mul(lpPrice).div(1e18));
    }
    ...
    function _rebalanceDebtInternal() internal {
        ...
        if (debtInShort > balShort) {
            ...
            (swapAmountWant, slippage) = _swapExactShortWant(excessShort.mul(rebalancePercent).div(BASIS_PRECISION));
        }
        _repayDebt();
     ...
     */
    function _withdraw(uint256 _amountNeeded) internal returns (uint256 _liquidatedAmount, uint256 _loss) {
        uint256 balanceWant = balanceOfWant();
        ...
        uint256 stratPercent = _amountNeeded.sub(balanceWant).mul(BASIS_PRECISION).div(balanceDeployed);
        ...
        if (stratPercent > 9500) {
            ...
            if (stratPercent > 500) {
                ...
                if (debtInShort > shortInShort) {
                    ...
                    uint256 swapAmountWant = debt.mul(stratPercent).div(BASIS_PRECISION);
                    _redeemWant(swapAmountWant);
                    slippage = _swapExactWantShort(swapAmountWant);
                } else {
    ...
    function balanceDeployed() public view returns (uint256) {
        return balanceLend().add(balanceLp()).add(balanceShortWantEq()).sub(balanceDebt());
    }
    ...
    function getLpReserves() public view returns (uint256 _wantInLp, uint256 _shortInLp) {
    ...
    function getLpReservesAndFee()
        public
        view
        returns (uint256 _wantInLp, uint256 _shortInLp, uint256 _wantFeePercent, uint256 _shortFeePercent)
    {
    ...
    function convertShortToWantLP(uint256 _amountShort) internal view returns (uint256) {
        (uint256 wantInLp, uint256 shortInLp) = getLpReserves();
        return (_amountShort.mul(wantInLp).div(shortInLp));
    }

    function convertShortToWantOracle(uint256 _amountShort) internal view returns (uint256) {
        return _amountShort.mul(getOraclePrice()).div(1e18);
    }

    function convertWantToShortLP(uint256 _amountWant) internal view returns (uint256) {
        (uint256 wantInLp, uint256 shortInLp) = getLpReserves();
    ...
    function balanceLp() public view returns (uint256) {
        ...
        return balanceLpInShort().mul(wantInLp).mul(2).div(wantShortLP.totalSupply());
    }
    ...
    function _borrowWantEq(uint256 _amount) internal returns (uint256 _borrowamount) {
    ...
    function _removeAllLp() internal {
        ...
        if (_amount > 0) {
            ...
            uint256 amountAMin = _amount.mul(shortLP).mul(slippageAdj).div(BASIS_PRECISION).div(lpIssued);
            uint256 amountBMin = _amount.mul(wantLP).mul(slippageAdj).div(BASIS_PRECISION).div(lpIssued);
     ...
     */
    function _swapExactWantShort(uint256 _amount) internal returns (uint256 slippageWant) {
     ...
     */
    function _swapExactShortWant(uint256 _amountShort) internal returns (uint256 _amountWant, uint256 _slippageWant) {
    ...
    function _swapWantShortExact(uint256 _amountOut) internal returns (uint256 _slippageWant) {
