...
contract StrategyInsurance {
    ...
    event InsurancePayment(uint256 indexed strategyDebt, uint256 indexed harvestProfit, uint256 indexed wantPayment);
    ...
    uint256 public profitTakeRate = 0; // 10% default
    ...
    function _onlyAuthorized() internal {
        require(msg.sender == strategy.strategist() || msg.sender == governance());
    }
    ...
    function setmaximumCompenstionRate(uint256 _maximumCompenstionRate) external {
        _onlyAuthorized();
    ...
    function reportProfit(
        uint256 _totalDebt,
        uint256 _profit
    ) external returns (uint256 _payment, uint256 _compensation) {
        _onlyStrategy();
     ...
     */
    function reportLoss(uint256 _totalDebt, uint256 _loss) external returns (uint256 _compensation) {
        _onlyStrategy();
     ...
     * @return _compensation amount sent back to the strategy.
     */
    function compensate(uint256 _totalDebt) internal returns (uint256 _compensation) {
        uint256 balance = want.balanceOf(address(this));
    ...
    function withdraw() external {
        _onlyGovernance();
        SafeERC20.safeTransfer(want, governance(), want.balanceOf(address(this)));
    }
    ...
    function migrateInsurance(address newInsurance) external {
        _onlyStrategy();
        SafeERC20.safeTransfer(want, newInsurance, want.balanceOf(address(this)));
    }
    ...
    function migrateStrategy(address newStrategy) external {
        _onlyGovernance();
        SafeERC20.safeTransfer(want, StrategyAPIExt(newStrategy).insurance(), want.balanceOf(address(this)));
    }
}
