    ...
    TroveHandler
{
    ...
    function _withdrawTokensFromBridges(uint256 _amountETH, uint256 _amountLUSD) private {
        if (_amountETH > 0) {
            _withdrawTokenFromBridge(l1ETHBridge, address(this), _amountETH);
        }

        if (_amountLUSD > 0) {
            _withdrawTokenFromBridge(l1LUSDBridge, address(this), _amountLUSD);
        }
    }
    ...
    function approveToBridge(uint256 _amount) external onlyOwner {
        IERC20(lusd).approve(l1LUSDBridge, _amount);
    }
