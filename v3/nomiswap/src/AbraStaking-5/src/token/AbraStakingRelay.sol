...
contract AbraStakingRelay is OAppCore, IOAppComposer {
    ...
    VoterV4 voter;
    AbraStaking staking;
    OFTMediator mediator;

    constructor(VoterV4 _voter, OFTMediator _mediator, address _owner)
    ...
    function forward(uint256 lockupIndex, uint32 eid, bytes calldata options) external payable {
        ...
        SendParam memory sendParam = _getSendParam(msg.sender, lockup, eid, options);
        ...
        mediator.send{value: msg.value}(sendParam, fee, msg.sender);
    }
