...
import {AbraStorageLayoutCompatibility} from "src/token/AbraStorageLayoutCompatibility.sol";
...
contract Abra is AbraStorageLayoutCompatibility, ERC20Upgradeable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function makeupEpochFrom(uint32 epoch) external onlyOwner  {
        uint32 _currentEpoch = currentEpoch();
        while (epoch < _currentEpoch) {
            _makeupEpoch(epoch);
            epoch = epoch + WEEK;
        }
    }
    ...
    function _makeupEpoch(uint32 epoch) internal {
