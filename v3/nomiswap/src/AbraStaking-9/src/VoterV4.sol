...
contract VoterV4 is AccessManagedUpgradeable, ReentrancyGuardUpgradeable, UUPSUpgradeable {
    ...
    function _reset(uint256 lockupId) internal {
        ...
        for (uint256 i = 0; i < _poolVoteCnt; i ++) {
            ...
            if (_votes != 0) {
                ...
                IBribe(internalBribes[_ys]).withdraw(uint256(_votes), lockupId);
                IBribe(externalBribes[_ys]).withdraw(uint256(_votes), lockupId);
        ...
        if(lastVoted[lockupId] < _time) _totalWeight = 0;
    ...
    function _vote(AbraStaking.Lockup memory lockup, address[] memory _yieldSources, uint256[] memory _weights) internal {
        ...
        for (uint256 i = 0; i < _poolCnt; i++) {
            ...
            if (isAlive[_gauge]) {
                ...
                IBribe(internalBribes[_ys]).deposit(uint256(_poolWeight), lockup.id);
                IBribe(externalBribes[_ys]).deposit(uint256(_poolWeight), lockup.id);
    ...
    function _distribute(address _gauge) internal {
        ...
        // Is it necessary to check? Why can't we always distribute? Because then anyone could distribute ABRA to gauges 
        // at any time, which would break the current 7-day drip period. Here, distribution can only occur after
        // the `openEpoch` is updated. This ensures that the `voter` has already accepted the emission for this week, 
        // and therefore, this emission will be distributed to gauges and drip for 7 days. If someone adds funds using 
        // `notifyRewardAmount` after the distribution, they won't be able to call `_distribute`.        
