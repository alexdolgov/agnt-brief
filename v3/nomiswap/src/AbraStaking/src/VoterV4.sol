...
contract VoterV4 is AccessManagedUpgradeable, ReentrancyGuardUpgradeable, UUPSUpgradeable {
    ...
    mapping(address => uint256) public killedFrom;               // pool/ys/gauge  => killed from
    ...
    event Kill(address indexed target, uint256 from);
    ...
    error VoterV4_KilledYS(address ys);
    error VoterV4_KilledGauge(address gauge);
    error VoterV4_InvalidTarget(address target);
    ...
    function reset(uint256 lockupIndex) external nonReentrant {
        ...
        // when resetting the votes we don't care about lockup end date, we just check that the lockup exists
        require(lockup.points >= 0, "empty lockup");
        _reset(lockup.id);
        _abstain(lockup.id);
    ...
    function _vote(AbraStaking.Lockup memory lockup, address[] memory _yieldSources, uint256[] memory _weights) internal {
        ...
        for (uint i = 0; i < _poolCnt; i++) {
            ...
            address _gauge = gauges[_ys];
            if (isKilled(_gauge)) {
                revert VoterV4_KilledGauge(_gauge);
            }
            if (isKilled(_ys)) {
                revert VoterV4_KilledYS(_ys);
            }
        ...
        restricted
    {
        ...
        emit YieldSourceAdded(_gauge, _pool, msg.sender, _ys, _internalBribe, _externalBribe);
    }

    function killFromNextEpoch(address _target) external restricted {
        kill(_target, nextEpoch());
    }

    function kill(address _target, uint256 _from) public restricted {
        require(killedFrom[_target] == 0, "Already killed");
        require(_from >= block.timestamp, "Kill in the past");

        if (isGauge[_target] || gauges[_target] != address(0)) {
            killedFrom[_target] = _from;
            emit Kill(_target, _from);
        } else {
            revert VoterV4_InvalidTarget(_target);
        }
    }


    /// @notice claim locking and UPR rewards
    function claimRewards(address[] calldata bribes, bool claimRebaseRewards) external {
        if (claimRebaseRewards) {
            AbraStaking(ve).collectRewards(msg.sender);
        }

        for (uint i = 0; i < bribes.length; i++) {
            IBribe bribe = IBribe(bribes[i]);

            uint rewardTokensLength = bribe.rewardsListLength();
            address[] memory rewardTokens = new address[](rewardTokensLength);

            for (uint j = 0; j < rewardTokensLength; j++) {
                rewardTokens[j] = bribe.rewardTokens(j);
            }

            bribe.getReward(msg.sender, rewardTokens);
        }
    }
    ...
    function _distribute(address _gauge) internal {
        ...
        if(lastDistributionTimestamp < openEpoch) {
            ...
            // distribute only if claimable is > 0, currentEpoch != lastepoch
            if (_claimable > 0) {
                claimable[_gauge] = 0;
    ...
    function isKilled(address target) public view returns (bool) {
        uint256 _killedFrom = killedFrom[target];
        return _killedFrom != 0 && _killedFrom <= block.timestamp;
    }
