...
library AbraStakingLibrary {
    ...
    function split(
        ...
        uint256 amountBefore = lockup.amount;
        for (uint i = 1; i < parts.length; i++) {
            uint128 newAmount = uint128(parts[i] * amountBefore / partsSum);
            if (newAmount == 0) revert AbraStaking_ZeroAmount();

            lockup.amount -= newAmount;
            newPointsSum += calcPoints(newAmount, lockup.end, epoch);

            AbraStaking(address(this)).createLockupAndEmitStake(
                newAmount,
                lockup.end,
                calcPoints(newAmount, lockup.end, epoch),
                staker
            );
        }
