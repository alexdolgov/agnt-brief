...
abstract contract AbstractWrappedPendle is AbstractBaseWrapper, ERC4626Upgradeable {
    ...
    function _convertToShares(uint256 pendleLp, Math.Rounding rounding) internal view override returns (uint256) {
        ...
        if (totalSupply == 0) {
            return pendleLp * _getLpToAssetRate(rounding == Math.Rounding.Ceil);
        }
    ...
    function _getLpToAssetRate(bool optimistic) internal view returns (uint256 assetToLpRatio) {
        ...
        uint currentLpToAssetRate = PMath.divDown(totalPtAndSyAsset, uint(market.totalLp));
    ...
    function _checkImpliedRates() private view returns (uint minLnImpliedRate, uint maxLnImpliedRate) {
        (uint lastLnImpliedRate, uint observationLnImpliedRate) = _getImpliedLnRates();
    ...
    function _getImpliedLnRates() private view returns (uint lastLnImpliedRate, uint observationLnImpliedRate) {
        (,,uint96 storageLnImpliedRate, uint16 observationIndex, uint16 observationCardinality,) = MARKET._storage();
        lastLnImpliedRate = storageLnImpliedRate;
        (uint32 blockTimestamp, uint216 lnImpliedRateCumulative, bool initialized) = MARKET.observations(observationIndex);
        uint16 prevIndex = observationIndex > 0 ? observationIndex - 1: observationCardinality - 1;
        (uint32 prevBlockTimestamp, uint216 prevLnImpliedRateCumulative, bool prevInitialized) = MARKET.observations(prevIndex);
        require(initialized && prevInitialized, "Pendle: OINI");
        observationLnImpliedRate = (lnImpliedRateCumulative - prevLnImpliedRateCumulative) / (blockTimestamp - prevBlockTimestamp);
    }
