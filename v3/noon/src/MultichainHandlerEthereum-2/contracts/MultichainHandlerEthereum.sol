...
contract MultichainHandlerEthereum is IMultichainHandlerEthereum, IHyperlaneReceiver, AccessControl, ReentrancyGuard, Pausable {
    ...
    uint256 public cctpMaxFee;                     // Max CCTP V2 fee in USDC units sender will pay (0 = standard transfers only)
    uint32 public cctpMinFinalityThreshold;        // CCTP V2 finality: 1000 = fast, 2000 = standard (default)
    ...
    constructor(
        ...
        // Default to CCTP V2 standard finality (no fee). Admin can switch to fast (1000) with cctpMaxFee > 0.
        cctpMinFinalityThreshold = 2000;
    ...
    function _handleRedeem(
        ...
        // USN allowance to redeemHandler is set once in setRedeemHandler — see note there
    ...
    function _sendCollateral(
        ...
        if (
            ...
            IERC20(_collateral).forceApprove(address(cctpTokenMessenger), _amount);
            // CCTP V2: destinationCaller=0 (anyone may finalize), maxFee/finalityThreshold are admin-configurable
    ...
    function setRedeemHandler(address _redeemHandler) external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        // Revoke USN allowance from previous redeem handler if any
        address oldRedeemHandler = address(redeemHandler);
        if (oldRedeemHandler != address(0)) {
            IERC20(address(usnToken)).forceApprove(oldRedeemHandler, 0);
        }
        redeemHandler = IRedeemHandlerV2(_redeemHandler);
        // One-time MaxUint256 approval so queued redeems with overlapping allowances don't corrupt each other
        IERC20(address(usnToken)).forceApprove(_redeemHandler, type(uint256).max);
    ...
    /**
     * @notice Configure CCTP V2 transfer parameters
     * @dev Standard transfers: maxFee=0, minFinalityThreshold=2000 (~13min on Ethereum, no fee)
     *      Fast transfers: maxFee>0, minFinalityThreshold=1000 (Circle Iris attestation, ~seconds)
     * @param _maxFee Maximum fee in USDC units (6 decimals) the sender will pay
     * @param _minFinalityThreshold Minimum finality threshold (1000 = fast, 2000 = standard)
     */
    function setCctpTransferParams(
        uint256 _maxFee,
        uint32 _minFinalityThreshold
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        cctpMaxFee = _maxFee;
        cctpMinFinalityThreshold = _minFinalityThreshold;
        emit CctpTransferParamsSet(_maxFee, _minFinalityThreshold);
    }
