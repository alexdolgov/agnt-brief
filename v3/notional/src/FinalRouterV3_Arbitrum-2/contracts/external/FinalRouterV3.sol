...
abstract contract FinalRouterV3 is StorageLayoutV2, UUPSUpgradeable {
    ...
    function withdrawAave() external {
        require(owner == msg.sender, "Unauthorized");
        aavePool().withdraw(currencyIdToAddress(1), uint256(type(uint256).max), address(this));
        aavePool().withdraw(currencyIdToAddress(3), uint256(type(uint256).max), address(this));
        aavePool().withdraw(currencyIdToAddress(8), uint256(type(uint256).max), address(this));
    }
