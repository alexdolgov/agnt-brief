...
interface LandhiveBSPTStakingV04Helpers {
    ...
    function getPropertyAPY(address property) external view returns (uint256);
...
contract LandhiveBSPTStakingV04 is
    OwnableUpgradeable,
    ERC20Upgradeable,
    ReentrancyGuard
{
    string private constant _NAME = "sBlocksquarePropertyTokenV04";
    string private constant _SYMBOL = "sBSPTv04";
    ...
    function _updateBeforeStakeStart(
        ...
        uint256 tempBSPTAmount = (amount * propertyValuation * apy) /
            (_BSPT_TOTAL_SUPPLY * 100);
    ...
    function deposit(address property, uint256 amount) external nonReentrant {
        ...
        uint256 propertyValuation = LandhiveBSPTStakingV04Helpers(_propertyRegistry)
        ...
        uint256 apy = LandhiveBSPTStakingV04Helpers(_propertyRegistry).getPropertyAPY(
            property
        );
        require(apy > 0, "BSPTStaking: Invalid APY");
    ...
    function _withdraw(
        ...
        if (_rewardVesting != address(0)) {
            ...
            require(
                LandhiveBSPTStakingV04Helpers(_rewardVesting).addVestingInfo(
                    property,
                    user,
                    reward
                ),
        ...
        require(
            _rewardToken.transfer(
                LandhiveBSPTStakingV04Helpers(_dataProxy).getCPOfProperty(property),
                feeReward
            ),
    ...
    function sellBSPTToOceanPoint(address property) external nonReentrant {
        ...
        require(
            LandhiveBSPTStakingV04Helpers(_oceanPoint).sellBSPT(
                property,
                user,
                stakedBSPT
            ),
    ...
    /// @notice Calculates amount of sBSPT wallet will recieve for given amount
    /// @param property Address of property
    /// @param wallet Address of user wallet
    /// @param amount Amounft of BSPT
    /// @return sBSPTReceived Amount of sBSPT that would be minted
    function calculatesBSPT(
        address property,
        address wallet,
        uint256 amount
        ...
        uint256 tempBSPTAmount = (amount * propertyValuation * apy) /
            (_BSPT_TOTAL_SUPPLY * 100);
        uint256 amountsbsptToMint = (totalSupply() == 0 ||
            _tbsptBalanceThis == 0)
            ? tempBSPTAmount
            : (tempBSPTAmount * totalSupply()) / _tbsptBalanceThis;
        return amountsbsptToMint;
    }
}
