 ...
 * @dev Interface for a NAV provider that prices Base/Quote swaps using a virtual constant-product AMM centered around an externally supplied anchor price.
 *
 *      - `quoteBuyBase` and `quoteSellBase` are view functions used externally to compute swap outcomes without modifying reserves.
 *      - `executeBuyBase` and `executeSellBase` perform the same math but also update the virtual AMM reserves.
 *
 *      Conventions:
 *        - `anchorPriceWad` is expressed as Quote per Base in 1e18 fixed-point (WAD).
 *        - Returned `execPrice` values are expressed as Quote per Base in the Base asset's decimals.
 */
interface ISecuritizeAmmNavProvider {
    ...
    /**
     * @dev Emitted whenever a trade is recorded, storing market status and anchor price.
     * @param marketStatus     Market status used (0 = closed, 1 = open).
     * @param anchorPriceWad   Anchor price used for the trade (WAD precision).
     */
    event TradeMetadata(
        uint8 marketStatus,
    ...
    /**
     * @dev Emitted when the price scale factor is updated.
     *
     * @param oldScaleFactor Previous scale factor.
     * @param newScaleFactor New scale factor.
     */
    event PriceScaleFactorUpdated(
        uint256 oldScaleFactor,
