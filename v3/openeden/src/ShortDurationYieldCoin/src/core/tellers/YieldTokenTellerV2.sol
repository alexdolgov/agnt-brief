...
contract YieldTokenTellerV2 is OwnableUpgradeable, ReentrancyGuardUpgradeable, UUPSUpgradeable {
    ...
    function _buyPreview(uint256 _amount) internal view virtual returns (uint256 payout, uint256 fee, int256 price) {
        ...
        if (
            updatedY != nowY || updatedM != nowM || updatedD != nowD || currentTime.getHour() > afterHour / 100
     ...
     */
    function _buyFor(address _from, uint256 _amount, address _recipient)
        internal
        virtual
        returns (uint256 payout, int256 price)
    {
     ...
     * @return payout amount of yield token received
     */
    function _buyForWithVerification(address _from, uint256 _amount, address _recipient)
        internal
        virtual
    ...
    function buyWithPermit(
        ...
        if (!authority.doesUserHaveRole(msg.sender, Role.System_FundAdmin) && _from != msg.sender) revert NotPermissioned();
    ...
    function _sellFor(uint256 _amount, address _recipient) internal virtual returns (uint256 payout, int256 price) {
        ...
        ytoken.burn(msg.sender, _amount);
