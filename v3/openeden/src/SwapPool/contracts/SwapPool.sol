    ...
    ISwapPoolEvents
{
    ...
    // Tokens marked as having non-standard transfer behavior (fee-on-transfer, rebasing, etc.)
    // These tokens are explicitly allowed despite not being standard ERC-20
    mapping(address => bool) public allowNonStandardToken;
    ...
    function swapFrom(
        ...
        uint256 minRequired = minAmountInByPair[pairHash];
    ...
    function deposit(address token, address from, uint256 amount) external onlyOwner nonReentrant {
        ValidationLib.validateAddress(token);
        ValidationLib.validateAddress(from);
        ValidationLib.validateAmount(amount);

        // Enforce token policy: cannot deposit tokens explicitly marked unsupported
        if (unsupportedTokens[token]) revert("UNSUPPORTED_TOKEN");

        // Restrict source to caller; use msg.sender directly in transferFrom to satisfy Slither.
        if (from != msg.sender) revert("INVALID_DEPOSIT_SOURCE");

        // Pre-balance for exact-in enforcement across all custody paths
        uint256 beforeBal = _treasuryTokenBalance(token);

        if (contractHeldTreasury || tokenHeldInContract[token]) {
            // When contract holds funds (globally or per-token), transfer directly into this contract.
            IERC20(token).safeTransferFrom(msg.sender, address(this), amount);
        } else {
            // External treasury path (EOA or ITreasury contract)
            IERC20(token).safeTransferFrom(msg.sender, treasury, amount);
        }

        // Enforce exact-in semantics to reject fee-on-transfer/rebasing discrepancies
        uint256 afterBal = _treasuryTokenBalance(token);
        uint256 received = afterBal - beforeBal;
        if (received != amount) revert("FEE_ON_TRANSFER_NOT_SUPPORTED");

        // For EOA treasuries, outbound transfers are bounded by expiry and caps in _treasurySend.
        uint256 newBalance = afterBal;
        emit TokenDeposited(token, amount, newBalance);
        emit TokenDepositedV2(
            token,
            amount,
            newBalance,
    ...
    /// @notice Deposit tokens from an approved source (for multisig use).
    /// @dev Allows owner (multisig) to pull tokens from `from` who must have approved this contract.
    ///      Unlike deposit(), this does NOT require msg.sender == from, enabling multisig deposits.
    /// @param token The ERC20 token address
    /// @param from The address holding tokens (must have approved SwapPool for >= amount)
    /// @param amount The amount to deposit
    // slither-disable-start arbitrary-send-erc20
    // Rationale: onlyOwner; depositFrom is for multisig pulling from pre-approved `from` address
    function depositFrom(
        ...
        // slither-disable-next-line arbitrary-send-erc20 -- onlyOwner; depositFrom is for multisig pulling from pre-approved `from`
        if (contractHeldTreasury || tokenHeldInContract[token]) {
            // When contract holds funds (globally or per-token), transfer directly into this contract.
    ...
    function _executeSwap(
        ...
        if (actualReceived != amount) {
            if (!allowNonStandardToken[fromToken]) revert("NON_STANDARD_TOKEN_NOT_ALLOWED");
            amount = actualReceived; // adjust to actual received for whitelisted non-standard tokens
        }
    ...
    /// @notice Explicitly allow a non-standard token (fee-on-transfer, rebasing, etc.) for inbound swaps only.
    /// @dev Owner-only. WARNING: Breaks 1:1 atomicity - only use for trusted tokens where necessary.
    ///      Outbound transfers still enforce exact delivery to destination.
    /// @param token The token address to configure
    /// @param allowed Whether to allow this token's non-standard behavior
    function setAllowNonStandardToken(address token, bool allowed) external onlyOwner {
        ValidationLib.validateAddress(token);
        allowNonStandardToken[token] = allowed;
        emit NonStandardTokenAllowed(token, allowed);
    }
