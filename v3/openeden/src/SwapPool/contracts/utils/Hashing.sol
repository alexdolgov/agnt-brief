...
library Hashing {
    ...
    /// @dev Direction-dependent pair hash: keccak256(abi.encodePacked(fromToken, toToken)) — 20+20 tight pack.
    /// TOB-FT-SWAPDIFF-3 fix: Order-dependent — directedPairHash(A,B) != directedPairHash(B,A).
    function directedPairHash(
        address fromToken,
        address toToken
    ) internal pure returns (bytes32 out) {
        assembly ("memory-safe") {
            let ptr := mload(0x40)
            mstore(ptr, shl(96, fromToken))
            mstore(add(ptr, 20), shl(96, toToken))
            out := keccak256(ptr, 40)
            mstore(0x40, add(ptr, 64))
        }
    }
