...
contract WordCupCompetition {
    ...
    constructor(address _admin, address _distribute, IERC20 _competitionToken, IERC20 _oleToken, uint32 _exchangeRate) {
        admin = _admin;
    ...
    function setAirdropBatch(address[] memory accounts, uint256[] memory amounts, uint256 totalAmount, uint32 _exchangeRate) external{
        require(msg.sender == distribute, "caller must be distribute");
