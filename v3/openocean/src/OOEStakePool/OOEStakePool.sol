...
contract OOEStakePool is ReentrancyGuard, Ownable {
            ...
            if (lo.state == State.LOCKING) {
                if (lo.timestampLockEnd > block.timestamp) {
                    power =
                        power +
                        lo.OOELockedAmount +
                        ((lo.XOOEBaseAmount - lo.OOELockedAmount) * (block.timestamp - lo.timestampLockStart)) /
                        (lo.timestampLockEnd - lo.timestampLockStart);
                } else {
                    power =
                        power +
                        lo.XOOEBaseAmount +
                        ((block.timestamp - lo.timestampLockEnd) * minimumIncrease * lo.XOOEBaseAmount) /
                        3110400000000;
                }
            }
        }
    ...
    function harvest(uint256[] calldata lockid) external notPause {
        ...
        for (uint256 i = 0; i < locksAmount; i++) {
            ...
            require(lo.locker == msg.sender, "OPENOCEAN_STAKING_POOL_V1:NOT_ORDER_LOCKER");
    ...
    function unlock(uint256[] calldata lockid) external notPause {
        for (uint256 i = 0; i < lockid.length; i++) {
            ...
            require(
                lockDetail[lockid[i]].timestampLockEnd > block.timestamp,
                "OPENOCEAN_STAKING_POOL_V1:THIS_LOCK_WAS_ALREADY_COMPLETED"
            );
    ...
    function _punishment(Lock memory _lock) internal view returns (uint256) {
        uint256 punishmentAmount = (punishmentDegree *
            (_lock.XOOEBaseAmount - _lock.OOELockedAmount) *
            (block.timestamp - _lock.timestampLockStart)) /
            (_lock.timestampLockEnd - _lock.timestampLockStart) /
            5;
    ...
    function _singleLock(Lock memory _lock) internal {
        ...
        require(
            block.timestamp > _lock.timestampLockStart && block.timestamp - _lock.timestampLockStart < 120,
            "OPENOCEAN_STAKING_POOL_V1:WRONG_START_TIME"
        );
        require(lockDetail[_lock.lockId].OOELockedAmount == 0, "OPENOCEAN_STAKING_POOL_V1:LOCK_ALREADY_EXIST");
        require(
            (_lock.timestampLockEnd - _lock.timestampLockStart) == _lock.lockTimeUnit * period,
            "OPENOCEAN_STAKING_POOL_V1:WRONG_ENDTIME"
        );
        ...
        require(
            _lock.XOOEBaseAmount == (_lock.OOELockedAmount * (rates[_lock.lockTimeUnit] + 10000000000)) / 10000000000,
            "OPENOCEAN_STAKING_POOL_V1:NOT_CORRECT_AMOUNT"
        );
