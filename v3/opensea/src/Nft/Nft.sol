...
contract Nft is ERC721Enumerable, ERC721URIStorage, Ownable {
    ...
    constructor(string memory metaBaseURI) 
      ERC721("Hypurr", "HYPURR")
      Ownable(msg.sender)
    {
    ...
    function bulkSafeMint(address to, uint256 tokenIdFrom, uint256 tokenIdTo) public onlyOwner {
        require(tokenIdFrom <= tokenIdTo, "Invalid tokenId range");
    ...
    function bulkTransfer(address[] memory recipients, uint256 tokenIdFrom, uint256 tokenIdTo) public onlyOwner {
        uint256 n = recipients.length;
        require(n == tokenIdTo - tokenIdFrom + 1, "Invalid number of recipients");

        for (uint256 tokenId = tokenIdFrom; tokenId <= tokenIdTo; tokenId++) {
            transferFrom(msg.sender, recipients[tokenId - tokenIdFrom], tokenId);
        }
    }
}