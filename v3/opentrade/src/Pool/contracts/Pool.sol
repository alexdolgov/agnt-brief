...
contract Pool is IPool, ERC20Upgradeable, BeaconImplementation {
  ...
  event WithdrawEarlyRequested(address indexed lender, address indexed loan, uint256 shares, uint256 principal);
  event WithdrawEarlyAccepted(address indexed lender, address indexed loan, uint256 shares, uint256 principal);
...
//////////////////////////////////////////////////////////////*/
  ...
  function rolloverAndFinalizeApr(uint256 _apr) external onlyNotPaused onlyBorrowerManger {
    (activeLoan, requestedLoan, maturedLoan) = PoolLib.executeRolloverAndFinalizeApr(address(this), _apr, _accountings);
  }
  ...
  function loanCreated(address loan) external {
    if (msg.sender != serviceConfiguration().getLoanFactory()) revert InvalidAccess();
  ...
  function approveLoanForPool(address loan) external onlyNotPaused onlyPoolController {
    if (requestedLoan != address(0) || ILoan(loan).pool() != address(this)) revert InvalidLoan();
  ...
  modifier onlyDepositWindow() {
    if (requestedLoan == address(0) || !ILoan(requestedLoan).inDepositWindow()) revert InvalidLoan();

    _;
  }
  modifier onlyPermittedLender() {
    if (!poolAccessControl.isAllowed(msg.sender) || IPoolController(poolController).borrowerManager() == msg.sender)
      revert NotLender();

    _;
  }
  modifier onlyBorrowerManger() {
    if (IPoolController(poolController).borrowerManager() != msg.sender) revert NotBorrowerManager();
  ...
  function initialize(
    ...
    _serviceConfiguration = IServiceConfigurationV3(poolAddressList.serviceConfiguration);
    ...
    poolController = IPoolController(
      IPoolControllerFactory(poolAddressList.poolControllerFactory).createController(
        address(this),
    ...
    withdrawController = IWithdrawController(
      IWithdrawControllerFactory(poolAddressList.withdrawControllerFactory).createController(
        address(this),
   ...
   */
  function serviceConfiguration() public view returns (IServiceConfigurationV3) {
  ...
  function initiateRollover(address loan, address priorLoan) external onlyNotPaused onlyPoolController {
    if (!(loan == requestedLoan && priorLoan == activeLoan)) revert InvalidLoan();

    PoolLib.executeInitiateRollover(loan, priorLoan, address(this), _accountings);
  }

  function completeRolloverNetPayment(address settlingLoan) external onlyNotPaused onlyPoolController {
    if (maturedLoan != settlingLoan) revert InvalidLoan();

    PoolLib.executeCompleteRolloverNetPayment(settlingLoan, address(this), _accountings); //
  ...
  function releaseRolloverRedemption(address owner) external onlyPoolController {
    uint256 shares;
    uint256 assets;

    (shares, assets) = PoolLib.executeReleaseRolloverRedemption(owner, address(this));
  ...
  function liquidityPoolAssets() public view returns (uint256 assets) {
    ...
    return PoolLib.calculateTotalAssets(address(_liquidityAsset), address(this), 0, interest);
  }
  ...
  function reschedule(
    ...
    if (!(activeLoan == loan || requestedLoan == loan)) revert InvalidLoan();
  ...
  function requestRedeem(
    ...
    assets = PoolLib.executeRequestRedeemn(shares, address(this));
  }
  ...
  function convertToSharesAtDeposit(uint256 assets) public view override returns (uint256 shares) {
    if (activeLoan == address(0)) {
      shares = assets;
    } else {
      shares = assets.mul(1e18).div(exchangeRateAtDeposit());
    }
  }
  ...
  function exchangeRateAtDeposit() public view override returns (uint256 _exchangeRateAtDeposit) {
    if (activeLoan != address(0)) {
  ...
  function exchangeRateAtMaturity() public view override returns (uint256 _exchangeRateAtMaturity) {
    if (activeLoan != address(0)) {
      ILoan loan = ILoan(activeLoan);
  ...
  function maxDeposit(address owner) public view override returns (uint256) {
    ...
    return
      PoolLib.calculateMaxDeposit(
        poolController.state() == IPoolLifeCycleState.Active,
        poolController.settings().maxCapacity,
  ...
  {
    if (poolController.state() == IPoolLifeCycleState.DisruptionOrDefault || msg.sender != lender)
      revert PoolInvalidState();

    shares = PoolLib.executeDeposit(address(this), lender, assets, _accountings);
    _mint(lender, shares);
  ...
  function maxWithdraw(address owner) public view override returns (uint256 assets) {
    if (_serviceConfiguration.paused() == true || !isPermittedLender(owner)) {
      return 0;
    }
  ...
  function withdraw(
    ...
    address receiver,
    address owner
  ) public virtual onlyNotPaused onlyPermittedLender returns (uint256 shares) {
  ...
  function withdrawFeeVault(uint256 amount, address receiver) external onlyNotPaused onlyPoolController {
    PoolLib.executewWithdrawFeeVault(amount, receiver, address(this));
  }
  ...
  function redeem(
    ...
    address receiver,
    address owner
  ) public virtual onlyNotPaused onlyPermittedLender returns (uint256 assets) {
    assets = PoolLib.executeRedeem(shares, receiver, owner, address(this));
  ...
  function requestEarlyRedeem(
    uint256 shares
  ) external onlyNotPaused onlyActivatedPool onlyPermittedLender onlyLenderWithPPT {
    uint256 principal = withdrawController.requestEarlyRedeem(msg.sender, shares);
  ...
  ) external override onlyNotPaused onlyBorrowerManger returns (uint256 principal) {
    uint256 shares;
    (shares, principal) = withdrawController.acceptEarlyRedeemRequest(investorAddr);
  ...
  function repayEarlyWithdraw(
    ...
    onlyNotPaused
    onlyBorrowerManger
    returns (uint256 principal, uint256 repayment, uint256 redeemedShares, uint256 fees)
  {
    (repayment, redeemedShares, principal, fees) = PoolLib.executeRepayEarlyWithdraw(
      investorAddr,
      amount,
      address(this),
      _accountings
    ...
    _liquidityAsset.safeTransferFrom(address(this), investorAddr, repayment);
  }
  ...
  function crossChainTransferApproveSource(
    ...
    try withdrawController.crossChainTransferApproveSource(sourceWalletAddr, amount) {
  ...
  function crossChainTransferApproveDestination(
    ...
    try withdrawController.crossChainTransferApproveDestination(destinationWalletAddr, amount) {
  ...
  function crossChainTransferBurnSource(uint256 amount) external override onlyPermittedLender onlyLenderWithPPT {
    try withdrawController.crossChainTransferBurnSource(msg.sender, amount) {
      uint256 shares = PoolLib.executeCrossChainTransferBurnSource(amount, address(this), _accountings);
  ...
  function crossChainTransferMintDestination(
    ...
    try withdrawController.crossChainTransferMintDestination(destinationWalletAddr, amount) {
      uint256 shares = PoolLib.executeCrossChainTransferMintDestination(amount, address(this), _accountings);
  ...
  function crossChainTransferStatus(
    ...
    return withdrawController.crossChainTransferStatus(walletAddr);
  }
}
