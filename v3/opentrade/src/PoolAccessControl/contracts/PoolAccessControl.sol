 ...
 */
contract PoolAccessControl is IPoolAccessControl, BeaconImplementation {
  /**
  ...
  modifier onlyNotPaused() {
    require(!_pool.serviceConfiguration().paused(), "PoolAccessControl: Protocol paused");
    _;
  }
   ...
   */
  function initialize(address pool, address tosAcceptanceRegistry) public initializer {
    require(tosAcceptanceRegistry != address(0), "Pool: invalid ToS registry");
   ...
   */
  function allowParticipant(address addr) external onlyNotPaused onlyPoolAdmin {
   ...
   * Emits an {AllowedParticipantListUpdated} event.
   */
  function removeParticipant(address addr) external onlyNotPaused onlyPoolAdmin {
