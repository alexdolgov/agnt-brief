...
contract WithdrawDepositControllerFlex is IWithdrawDepositControllerFlex, BeaconImplementation {
  ...
  mapping(address => ICrossChainTransferFlexState) private _crossChainTransferAssetsMap;
   ...
   */
  function initialize(address poolAddr_, address vaultFactory, address borrowerWalletAddr_) public initializer {
  ...
  function drawDownToBorrowerWallet(uint256 amount) external onlyPool onlyNotPaused {
  ...
  function deposit(
  ...
  ) external onlyPool returns (uint256 inflows, uint256 shares, uint256 transferInDayTimestamp) {
    IBusinessDayRegistry businessRegisry = IBusinessDayRegistry(_pool.businessDayRegistry());
    inflows = 0;
    uint8 numberOfDays;
    (numberOfDays, transferInDayTimestamp) = businessRegisry.businessDaysToCalendarDays(
    ...
    for (uint8 i = 0; i < numberOfDays; i++) {
      projectedExchangeRate = projectedExchangeRate.mul(_dailyInterestRate + 1e18).div(1e18);
    }
    ...
    if (numberOfDays != 0) {
      lenderTotals_.sharesTransitioningIn = lenderTotals_.sharesTransitioningIn + shares;
      lenderTotals_.assetsTransitioningIn = lenderTotals_.assetsTransitioningIn + assets;
    }
    ...
    if (numberOfDays != 0) {
      ...
      IDailyDepositTotals memory dailyDepositTotal = _dailyDepositTotals[transferInDayTimestamp];
  ...
  function dailyInterestAccural(uint256 currentDay) external override onlyPool returns (uint256 inflows) {
    IDailyDepositTotals memory _dailyDepositTotal = _dailyDepositTotals[currentDay];
    IDailyWithdrawTotals memory _dailyWithdrawTotal = _dailyWithdrawTotals[currentDay];
    ...
    if (_dailyWithdrawTotal.requestedAssets > 0) {
      for (uint i = _withdrawEvents.length; i >= 1; i--) {
        ...
        if (eventIdIsDue[ev.eventId] == false && ev.transferOutDayTimestamp == currentDay) {
          eventIdIsDue[ev.eventId] = true;
          _lenderTotals[ev.lender].sharesDueForWithdraws += ev.requestedShares;
          _lenderTotals[ev.lender].assetsDueForWithdraws += ev.requestedAssets;
        }
      }
    }
    ...
    if (_dailyDepositTotal.depositedAssets > 0) {
      ...
      _poolDepositsTotals.depositedAssets -= _dailyDepositTotal.depositedAssets;
      ...
      for (uint i = _depositEvents.length; i >= 1; i--) {
        ...
        if (ev.transferInDayTimestamp == currentDay) {
          uint256 shareAtCurrentExchangeRate = PoolLibFlex.roundDiv(ev.depositedAssets * 1e18, _pool.exchangeRate());
          if (ev.mintedShares < shareAtCurrentExchangeRate) {
            _pool.topOffSharesDueToExchangeRateDecrease(ev.lender, shareAtCurrentExchangeRate - ev.mintedShares);
          }

          _lenderTotals[ev.lender].sharesTransitioningIn -= ev.mintedShares;
          _lenderTotals[ev.lender].assetsTransitioningIn -= ev.depositedAssets;
   ...
   * @inheritdoc IWithdrawDepositControllerFlex
   */
  function maxRedeemRequest(address lender) external view returns (uint256 maxShares) {
    maxShares =
  ...
  function generateWithdrawEventId() internal returns (uint256) {
    ...
    return uint256(keccak256(abi.encodePacked(block.timestamp, msg.sender, requestNumber)));
  }

  function performRequest(address lender, uint256 shares, uint256 assets) external onlyPool {
    require(this.maxRedeemRequest(lender) >= shares, "Pool: InsufficientBalance");
    require(shares > 0, "Pool: Zero Shares");
    IBusinessDayRegistry businessRegisry = IBusinessDayRegistry(_pool.businessDayRegistry());
    (uint8 numberOfDays, uint256 transferOutDayTimestamp) = businessRegisry.businessDaysToCalendarDays(
      uint64(block.timestamp),
    ...
    _poolWithdrawTotals.requestedShares = _poolWithdrawTotals.requestedShares + shares;

    _lenderTotal.requestedAssets = _lenderTotal.requestedAssets + assets;
    _poolWithdrawTotals.requestedAssets = _poolWithdrawTotals.requestedAssets + assets;
    ...
    IDailyWithdrawTotals memory dailyWithdrawTotal = _dailyWithdrawTotals[transferOutDayTimestamp];

    dailyWithdrawTotal.requestedShares = dailyWithdrawTotal.requestedShares + shares;

    dailyWithdrawTotal.requestedAssets = dailyWithdrawTotal.requestedAssets + assets;
  ...
  function findEventAndRemove(uint256 eventId) internal returns (IPoolLenderWithdrawEvent memory ev) {
    bool isFound = false;
  ...
  function releaseWithdrawal(uint256 eventId) public onlyPool returns (IPoolLenderWithdrawEvent memory ev) {
    bool eventIsDueForId = eventIdIsDue[eventId];
    ev = findEventAndRemove(eventId);

    _dailyWithdrawTotals[ev.transferOutDayTimestamp].requestedAssets -= ev.requestedAssets;
    _dailyWithdrawTotals[ev.transferOutDayTimestamp].requestedShares -= ev.requestedShares;
  ...
  function repayLoans(uint256 assetsToReturnToPool) public onlyPool {
    _borrowerVault.repayLoan(_pool.liquidityAssetAddr(), assetsToReturnToPool);
  }

  function tokenTransfer(address from, address to, uint256 assetAmount) public onlyPool {
  ...
  function crossChainTransferApproveSource(address sourceWalletAddr, uint256 amount) external override onlyPool {
    console.log("WC: crossChainTransferApproveSource: ", sourceWalletAddr, amount);

    if (
      _pool.state() != IPoolLifeCycleStateFlex.Active ||
      _pool.maxWithdrawRequest(sourceWalletAddr) < amount ||
    ...
    ICrossChainTransferFlexState memory transferState;
    ...
    console.log(
      "WC: crossChainTransferApproveSource: ",
      sourceWalletAddr,
      _crossChainTransferAssetsMap[sourceWalletAddr].amount
    );
  }
  function crossChainTransferApproveDestination(
    ...
    IPoolFlex poolContract = IPoolFlex(_pool);
    if (
      poolContract.state() != IPoolLifeCycleStateFlex.Active ||
  ...
  function crossChainTransferBurnSource(address sourceWalletAddr, uint256 amount) external override onlyPool {
    IPoolFlex poolContract = IPoolFlex(_pool);
    console.log("WC: crossChainTransferBurnSource: ", amount, poolContract.maxWithdrawRequest(sourceWalletAddr));
    ...
    console.log("WC: crossChainTransferBurnSource: ", poolContract.maxWithdrawRequest(sourceWalletAddr), amount);
    ...
    if (
      ...
      console.log("WC: crossChainTransferBurnSource:  Revert", amount);
      revert CrossChainWCError();
    }
    delete _crossChainTransferAssetsMap[sourceWalletAddr];
    _lenderTotals[sourceWalletAddr].assetsWithdrawn += amount;
  }
  function crossChainTransferMintDestination(address destinationWalletAddr, uint256 amount) external override onlyPool {
    ...
    _lenderTotals[destinationWalletAddr].assetsDeposited += amount;
  }
}
