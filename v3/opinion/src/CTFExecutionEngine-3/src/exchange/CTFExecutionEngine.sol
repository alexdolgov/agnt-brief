...
contract CTFExecutionEngine is
    ...
    Pausable,
    AssetOperations,
    Hashing("OPINION CTF Exchange", "1"),
    Registry,
    Signatures,
    Trading
{
    ...
    constructor(address _collateral, address _ctf, address _proxyFactory, address _safeFactory, address _treasury)
    ...
    function setReferralManager(address _referralManager) external onlyAdmin {
    ...
    function matchOrders(
        ...
        // Use token-specific minFee if configured, otherwise use global default
        uint256 finalMinFee = (enabled && minFee > 0) ? minFee : _getGlobalDefaultMinFee();
