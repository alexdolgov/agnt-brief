...
contract ConditionalTokens is ERC1155 {
    ...
    mapping(bytes32 => address) public conditionCollateral;

    /// @dev This function prepares a condition by initializing a payout vector associated with the condition.
    /// @param oracle The account assigned to report the result for the prepared condition.
    /// @param questionId An identifier for the question to be answered by the oracle.
    /// @param outcomeSlotCount The number of outcome slots which should be used for this condition. Must not exceed 256.
    function prepareCondition(address oracle, bytes32 questionId, uint outcomeSlotCount, IERC20 collateralToken) external {
        ...
        conditionCollateral[conditionId] = address(collateralToken);
    ...
    function splitPosition(
        ...
        if (freeIndexSet == 0) {
            ...
            if (parentCollectionId == bytes32(0)) {
                SafeTransferLib.safeTransferFrom(ERC20(address(collateralToken)), msg.sender, address(this), amount);
            } else {
                _burn(
                    msg.sender,
