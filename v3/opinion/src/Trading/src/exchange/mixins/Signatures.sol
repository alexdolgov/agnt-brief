...
import { OLABFactoryHelper } from "./OLABFactoryHelper.sol";

/// @title Signatures
/// @notice Maintains logic that defines the various signature types and validates them
abstract contract Signatures is ISignatures, OLABFactoryHelper {
    constructor(address _proxyFactory, address _safeFactory) OLABFactoryHelper(_proxyFactory, _safeFactory) { }
    ...
    ///                           For signature types OLAB_PROXY and OLAB_GNOSIS_SAFE, this is the address of the proxy or the safe
    ...
    function isValidSignature(
        ...
        } else if (signatureType == SignatureType.OLAB_GNOSIS_SAFE) {
            return verifyOLABSafeSignature(signer, associated, structHash, signature);
        } else {
            // OLAB_PROXY
            return verifyOLABProxySignature(signer, associated, structHash, signature);
        }
    }
    ...
    /// @notice Verifies a signature signed by a OLAB proxy wallet
    // Verifies that:
    // 1) the ECDSA signature is valid
    // 2) the Proxy wallet is owned by the signer
    /// @param signer       - Address of the signer
    /// @param proxyWallet  - Address of the OLAB proxy wallet
    /// @param structHash   - Hash of the struct being verified
    /// @param signature    - Signature to be verified
    function verifyOLABProxySignature(address signer, address proxyWallet, bytes32 structHash, bytes memory signature)
        internal
        view
        returns (bool)
    {
        return verifyECDSASignature(signer, structHash, signature) && getOLABProxyWalletAddress(signer) == proxyWallet;
    }

    /// @notice Verifies a signature signed by a OLAB Gnosis safe
    // Verifies that:
    // 1) the ECDSA signature is valid
    ...
    /// @param signature   - Signature to be verified
    function verifyOLABSafeSignature(address signer, address safeAddress, bytes32 hash, bytes memory signature)
        internal
        view
        returns (bool)
    {
