...
contract RedstoneOracleProxy is Ownable, PriceOracle {
    ...
    function setAddresses(
        ...
        address[] calldata redstoneAddresses
    ) external onlyOwner {
        require(
            underlyingAssets.length == redstoneAddresses.length,
            "API3OracleProxy: assets and priceFeedIds length mismatch"
        );
        for (uint i = 0; i < underlyingAssets.length; i++) {
            underlyingToOracleAddress[underlyingAssets[i]] = redstoneAddresses[i];
        }
    }
    ...
    function getUnderlyingPrice(
        ...
        uint8 decimals = IChainlinkInterface(underlyingToOracleAddress[asset]).decimals();
        uint32 expToUse = uint32(36 - decimals); // Chainlink returns values with 8 decimals places
