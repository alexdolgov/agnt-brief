    ...
    ExponentialNoError
{
    ...
    /// @notice Emitted when a new borrow-side ORB speed is calculated for a market
    event OrbBorrowSpeedUpdated(OToken indexed oToken, uint newSpeed);

    /// @notice Emitted when a new supply-side ORB speed is calculated for a market
    event OrbSupplySpeedUpdated(OToken indexed oToken, uint newSpeed);

    /// @notice Emitted when a new ORB speed is set for a contributor
    event ContributorOrbSpeedUpdated(
        address indexed contributor,
        uint newSpeed
    );

    /// @notice Emitted when ORB is distributed to a supplier
    event DistributedSupplierOrb(
    ...
    /// @notice Emitted when ORB is distributed to a borrower
    event DistributedBorrowerOrb(
        OToken indexed oToken,
    ...
    /// @notice Emitted when ORB is granted by admin
    event OrbGranted(address recipient, uint amount);

    /// @notice Emitted when ORB accrued for a user has been manually adjusted.
    event OrbAccruedAdjusted(
        address indexed user,
        uint oldOrbAccrued,
        uint newOrbAccrued
    );

    /// @notice Emitted when ORB receivable for a user has been updated.
    event OrbReceivableUpdated(
        address indexed user,
        uint oldOrbReceivable,
        uint newOrbReceivable
    );

    /// @notice The initial ORB index for a market
    ...
    function mintAllowed(
        ...
        updateOrbSupplyIndex(oToken);
        distributeSupplierOrb(oToken, minter);
    ...
    function redeemAllowed(
        ...
        // Keep the flywheel moving
        updateOrbSupplyIndex(oToken);
        distributeSupplierOrb(oToken, redeemer);
    ...
    function borrowAllowed(
        ...
        updateOrbBorrowIndex(oToken, borrowIndex);
        distributeBorrowerOrb(oToken, borrower, borrowIndex);
    ...
    function seizeAllowed(
        ...
        // Keep the flywheel moving
        updateOrbSupplyIndex(oTokenCollateral);
        distributeSupplierOrb(oTokenCollateral, borrower);
        distributeSupplierOrb(oTokenCollateral, liquidator);
    ...
    function transferAllowed(
        ...
        // Keep the flywheel moving
        updateOrbSupplyIndex(oToken);
        distributeSupplierOrb(oToken, src);
        distributeSupplierOrb(oToken, dst);
    ...
    function fixBadAccruals(
        ...
        for (uint i = 0; i < affectedUsers.length; ++i) {
            ...
            // The case where the user has claimed and received an incorrect amount of ORB.
            // The user has less currently accrued than the amount they incorrectly received.
            if (amountToSubtract > currentAccrual) {
                // Amount of ORB the user owes the protocol
                ...
                // Accounting: record the ORB debt for the user
                compReceivable[user] = newReceivable;

                emit OrbReceivableUpdated(user, oldReceivable, newReceivable);
            ...
            if (amountToSubtract > 0) {
                ...
                emit OrbAccruedAdjusted(user, currentAccrual, newAccrual);
            }
        }
    ...
    /**
     * @notice Set ORB speed for a single market
     * @param oToken The market whose ORB speed to update
     * @param supplySpeed New supply-side ORB speed for market
     * @param borrowSpeed New borrow-side ORB speed for market
     */
    function setOrbSpeedInternal(
        OToken oToken,
        uint supplySpeed,
        uint borrowSpeed
    ) internal {
        ...
        if (compSupplySpeeds[address(oToken)] != supplySpeed) {
            ...
            //  1. ORB accrued properly for the old speed, and
            //  2. ORB accrued at the new speed starts after this block.
            updateOrbSupplyIndex(address(oToken));

            // Update speed and emit event
            compSupplySpeeds[address(oToken)] = supplySpeed;
            emit OrbSupplySpeedUpdated(oToken, supplySpeed);
        }
        ...
        if (compBorrowSpeeds[address(oToken)] != borrowSpeed) {
            ...
            updateOrbBorrowIndex(address(oToken), borrowIndex);

            // Update speed and emit event
            compBorrowSpeeds[address(oToken)] = borrowSpeed;
            emit OrbBorrowSpeedUpdated(oToken, borrowSpeed);
        }
    }

    /**
     * @notice Accrue ORB to the market by updating the supply index
     * @param oToken The market whose supply index to update
     * @dev Index is a cumulative sum of the ORB per oToken accrued.
     */
    function updateOrbSupplyIndex(address oToken) internal {
        CompMarketState storage supplyState = compSupplyState[oToken];
    ...
    /**
     * @notice Accrue ORB to the market by updating the borrow index
     * @param oToken The market whose borrow index to update
     * @dev Index is a cumulative sum of the ORB per oToken accrued.
     */
    function updateOrbBorrowIndex(
        address oToken,
    ...
    /**
     * @notice Calculate ORB accrued by a supplier and possibly transfer it to them
     * @param oToken The market in which the supplier is interacting
     * @param supplier The address of the supplier to distribute ORB to
     */
    function distributeSupplierOrb(address oToken, address supplier) internal {
        // TODO: Don't distribute supplier ORB if the user is not in the supplier market.
        // This check should be as gas efficient as possible as distributeSupplierOrb is called in many places.
        ...
        // Update supplier's index to the current index since we are distributing accrued ORB
        compSupplierIndex[oToken][supplier] = supplyIndex;

        if (supplierIndex == 0 && supplyIndex >= compInitialIndex) {
            // Covers the case where users supplied tokens before the market's supply state index was set.
            // Rewards the user with ORB accrued from the start of when supplier rewards were first
            // set for the market.
            supplierIndex = compInitialIndex;
        }

        // Calculate change in the cumulative sum of the ORB per oToken accrued
        ...
        // Calculate ORB accrued: oTokenAmount * accruedPerOToken
        uint supplierDelta = mul_(supplierTokens, deltaIndex);

        uint supplierAccrued = add_(compAccrued[supplier], supplierDelta);
        compAccrued[supplier] = supplierAccrued;

        emit DistributedSupplierOrb(
            OToken(oToken),
            supplier,
            supplierDelta,
            supplyIndex
    ...
    /**
     * @notice Calculate ORB accrued by a borrower and possibly transfer it to them
     * @dev Borrowers will not begin to accrue until after the first interaction with the protocol.
     * @param oToken The market in which the borrower is interacting
     * @param borrower The address of the borrower to distribute ORB to
     */
    function distributeBorrowerOrb(
        address oToken,
        address borrower,
        Exp memory marketBorrowIndex
    ) internal {
        // TODO: Don't distribute supplier ORB if the user is not in the borrower market.
        // This check should be as gas efficient as possible as distributeBorrowerOrb is called in many places.
        // - We really don't want to call an external contract as that's quite expensive.
        ...
        // Update borrowers's index to the current index since we are distributing accrued ORB
        compBorrowerIndex[oToken][borrower] = borrowIndex;

        if (borrowerIndex == 0 && borrowIndex >= compInitialIndex) {
            // Covers the case where users borrowed tokens before the market's borrow state index was set.
            // Rewards the user with ORB accrued from the start of when borrower rewards were first
            // set for the market.
            borrowerIndex = compInitialIndex;
        }

        // Calculate change in the cumulative sum of the ORB per borrowed unit accrued
        Double memory deltaIndex = Double({
        ...
        // Calculate ORB accrued: oTokenAmount * accruedPerBorrowedUnit
        uint borrowerDelta = mul_(borrowerAmount, deltaIndex);

        uint borrowerAccrued = add_(compAccrued[borrower], borrowerDelta);
        compAccrued[borrower] = borrowerAccrued;

        emit DistributedBorrowerOrb(
            OToken(oToken),
            borrower,
            borrowerDelta,
            borrowIndex
    ...
    /**
     * @notice Calculate additional accrued ORB for a contributor since last accrual
     ...
     */
    function claimOrb(address holder) public {
        return claimOrb(holder, allMarkets);
    }
     ...
     */
    function claimOrb(address holder, OToken[] memory oTokens) public {
        address[] memory holders = new address[](1);
        holders[0] = holder;
        claimOrb(holders, oTokens, true, true);
    }
    ...
    function claimOrb(
        ...
        for (uint i = 0; i < oTokens.length; i++) {
            ...
            if (suppliers == true) {
                updateOrbSupplyIndex(address(oToken));
                for (uint j = 0; j < holders.length; j++) {
                    distributeSupplierOrb(address(oToken), holders[j]);
                }
            }
        }
        for (uint j = 0; j < holders.length; j++) {
            compAccrued[holders[j]] = grantOrbInternal(
                holders[j],
     ...
     */
    function grantOrbInternal(
        address user,
        uint amount
    ) internal returns (uint) {
        OrbitToken comp = OrbitToken(getTokenAddress());
     ...
     * @param amount The amount of tokens to (possibly) transfer
     */
    function _grantOrb(address recipient, uint amount) public {
        require(adminOrInitializing(), "only admin can grant comp");
        uint amountLeft = grantOrbInternal(recipient, amount);
        require(amountLeft == 0, "insufficient comp for grant");
        emit OrbGranted(recipient, amount);
    }
    ...
    function _setOrbSpeeds(
        ...
        require(
            ...
            "SpaceStation::_setOrbSpeeds invalid input"
        );

        for (uint i = 0; i < numTokens; ++i) {
            setOrbSpeedInternal(oTokens[i], supplySpeeds[i], borrowSpeeds[i]);
        }
    }
     ...
     */
    function _setContributorOrbSpeed(
        address contributor,
        uint compSpeed
    ) public {
        ...
        emit ContributorOrbSpeedUpdated(contributor, compSpeed);
    }
    ...
    /**
     * @notice Set the address of the ORB token
     * @param newAddress The new address of the ORB token
     */
