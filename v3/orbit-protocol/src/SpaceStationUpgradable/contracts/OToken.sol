...
import "./External/IBlast.sol";

// Dev
// import "hardhat/console.sol";

/**
 * @title Orbit's OToken Contract
 * @notice Abstract base for OTokens
 * @author Orbit
 */
abstract contract OToken is
    CTokenInterface,
    ExponentialNoError,
    TokenErrorReporter
{
    ...
    // Mainnet Values
    IERC20Rebasing public constant USDBBlast =
        IERC20Rebasing(0x4300000000000000000000000000000000000003);
    IERC20Rebasing public constant WETHBlast =
        IERC20Rebasing(0x4300000000000000000000000000000000000004);
    IBlast public constant BlastContract =
        IBlast(0x4300000000000000000000000000000000000002);
    
    /**
     * @notice Initialize the money market
     * @param spaceStation_ The address of the Space Station Comptroller
     ...
     */
    function initialize(
        ComptrollerInterface spaceStation_,
        ...
        // Set the comptroller
        uint err = _setComptroller(spaceStation_);
        ...
        // Set blast yield configuration
        // BlastContract.configureClaimableYield();
        // BlastContract.configureClaimableGas();
        // WETHBlast.configure(YieldMode.CLAIMABLE);
        // USDBBlast.configure(YieldMode.CLAIMABLE);
    ...
    function liquidateBorrowInternal(
        address borrower,
        uint repayAmount,
        CTokenInterface oTokenCollateral
    ) internal nonReentrant {
        accrueInterest();

        uint error = oTokenCollateral.accrueInterest();
        if (error != NO_ERROR) {
    ...
    function liquidateBorrowFresh(
        ...
        if (address(oTokenCollateral) == address(this)) {
            ...
            require(
                oTokenCollateral.seize(liquidator, borrower, seizeTokens) ==
                    NO_ERROR,
    ...
    function _blastClaimYield(address target) external {
        // Check caller is admin
        if (msg.sender != admin) {
            revert BlastYieldOwnerCheck();
        }
        BlastContract.claimAllYield(address(this), target);
        BlastContract.claimAllGas(address(this), target);
    }

    /**
     * @notice Gets the claimable amount of yield from the contract
     * @dev Admin function to get claimable yield
     * @return (uint256, uint256) of usdb and weth claimable
     */
    function _blastGetClaimableAmount()
        external
        view
        returns (uint256, uint256)
    {
        uint256 usdbAmount = USDBBlast.getClaimableAmount(address(this));
        uint256 wethAmount = WETHBlast.getClaimableAmount(address(this));

        return (usdbAmount, wethAmount);
    }

    /**
     * @notice Claims blast yield and sends it to the DAO
     * @dev Admin function to claim yield
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function _blastClaimYieldAll() external returns (uint) {
        // Check caller is admin
        if (msg.sender != admin) {
            revert BlastYieldOwnerCheck();
        }
        ...
        uint256 usdbAmount = USDBBlast.getClaimableAmount(address(this));
        if (usdbAmount > 0) {
            USDBBlast.claim(address(this), usdbAmount);
            // Transfer funds to DAO
            bool res = USDBBlast.transfer(msg.sender, usdbAmount);
            if (!res) {
                revert BlastTransferFailed();
            }
        }

        uint256 wethAmount = WETHBlast.getClaimableAmount(address(this));
        if (wethAmount > 0) {
            WETHBlast.claim(address(this), wethAmount);
            bool res2 = WETHBlast.transfer(msg.sender, wethAmount);
            if (!res2) {
                revert BlastTransferFailed();
            }
        }
