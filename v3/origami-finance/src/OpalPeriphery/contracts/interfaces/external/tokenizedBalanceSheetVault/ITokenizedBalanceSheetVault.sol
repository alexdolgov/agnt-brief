...
/// @notice
///  Shares in this vault represent a proportional slice of the balance sheet of this vault. The balance sheet adheres
/// to the
...
interface ITokenizedBalanceSheetVault is IERC20, IERC20Metadata {
    ...
    function convertFromToken(
        ...
        uint256 tokenAmount
    ) external view returns (uint256 shares, uint256[] memory assets, uint256[] memory liabilities);
    ...
    /// - MUST NOT account for mint limits like those returned from maxJoinWithShares and should always act as though
    /// the joinWithShares would be accepted, regardless if the user has enough tokens approved, etc.
    /// - MUST be inclusive of join fees. Integrators should be aware of the existence of join fees.
    /// - MUST NOT revert.
    ...
    function joinWithShares(
        ...
        address receiver,
        bytes32 tokensHash
    ) external returns (uint256[] memory assets, uint256[] memory liabilities);

    /// @notice Returns the maximum amount of `tokenAddress` that can be withdrawn from the vault given the owner
    ...
    /// @notice Returns the maximum amount of Vault shares that can be redeemed from the vault given the owner balance
    /// in the vault,
