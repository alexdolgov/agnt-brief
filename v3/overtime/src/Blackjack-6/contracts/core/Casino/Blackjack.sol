        ...
        if (isSpl) {
            ...
            if (ss.activeHand == 1) {
                ...
                if (playerValue > BLACKJACK_TARGET) {
                    // Hand 1 busted — advance to hand 2 without resolving yet
                    ss.activeHand = 2;
                }
                hand.status = HandStatus.PLAYER_TURN;
            } else {
                ...
                if (playerValue > BLACKJACK_TARGET) {
                    // Hand 2 busted. If hand 1 also busted → resolve all (no dealer needed).
                    // Otherwise dealer must play — request 7 more VRF words.
                    (uint8 hand1Value, ) = _handValue(hand.playerCards, hand.playerCardCount);
                    if (hand1Value > BLACKJACK_TARGET) {
                        // Both busted — _resolveSplit short-circuits to PLAYER_BUST on pv>21
