...
contract Pool is VersionedInitializable, PoolStorage, IPool {
    ...
    function claimRefundedGas(address recipient) external onlyPoolAdmin {
        ...
        for (uint256 index = 0; index < assetAmount; index++) {
            address asset = reserves[index];
            DataTypes.ReserveData memory reserveData = _reserves[asset];
            IGasReceiver(reserveData.aTokenAddress).claimRefundedGas(recipient);
            IGasReceiver(reserveData.stableDebtTokenAddress).claimRefundedGas(
                recipient
            );
            IGasReceiver(reserveData.variableDebtTokenAddress).claimRefundedGas(
                recipient
            );
        }
    }
