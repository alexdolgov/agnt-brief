...
import "./interfaces/IFarmBoosterV1.sol";

contract FarmBoosterV2 is Ownable, ReentrancyGuard {
    ...
    /// @notice Farm Booster version one contract.
    IFarmBoosterV1 public immutable FarmBoosterV1;
    ...
    /// @notice User token position initialized flag.
    /// @dev Update userPositionLiquidity from the Farm Booster version one contract.
    /// userPositionLiquidityInitialized[user][tokenId] = true means userPositionLiquidity[user][tokenId] was initialized.
    mapping(address => mapping(uint256 => bool)) public userPositionLiquidityInitialized;

    /// @notice User pool initialized flag.
    /// @dev Update userPoolTotalLiquidity from the Farm Booster version one contract.
    /// userPoolTotalLiquidityInitialized[user][pid] = true means userPoolTotalLiquidity[user][pid] was initialized.
    mapping(address => mapping(uint256 => bool)) public userPoolTotalLiquidityInitialized;
    ...
    /// @param _FarmBoosterV1 Farm Booster version one contract.
    /// @param _cA Limit max boost.
    /// @param _cB Controls difficulties.
    constructor(
        address _VECake,
        IMasterChefV3 _v3,
        IFarmBoosterV1 _FarmBoosterV1,
        uint256 _cA,
        uint256 _cB
    ) {
    ...
    function updateUserPoolLiquidity(
        ...
        // update user total liquidity from Farm Booster version one contract.
        if (userPoolTotalLiquidityInitialized[_user][_pid] == false) {
            userPoolTotalLiquidity[_user][_pid] = FarmBoosterV1.userPoolTotalLiquidity(_user, _pid);
            userPoolTotalLiquidityInitialized[_user][_pid] = true;
        }

        // update user position liquidity from Farm Booster version one contract.
        if (userPositionLiquidityInitialized[_user][_tokenId] == false) {
            userPositionLiquidity[_user][_tokenId] = FarmBoosterV1.userPositionLiquidity(_user, _tokenId);
            userPositionLiquidityInitialized[_user][_tokenId] = true;
        }
    ...
    function getUserMultiplier(uint256 _tokenId) external view returns (uint256) {
        ...
        if (!whiteList[pid]) {
            ...
            uint256 userRealPoolTotalLiquidity = userPoolTotalLiquidity[user][pid];
            uint256 userRealPositionLiquidity = userPositionLiquidity[user][_tokenId];

            // Need to query from Farm Booster version one contract when not initialized.
            if (userPoolTotalLiquidityInitialized[user][pid] == false) {
                userRealPoolTotalLiquidity = FarmBoosterV1.userPoolTotalLiquidity(user, pid);
            }

            if (userPositionLiquidityInitialized[user][_tokenId] == false) {
                userRealPositionLiquidity = FarmBoosterV1.userPositionLiquidity(user, _tokenId);
            }

            uint256 totalLiquidityInPool = userRealPoolTotalLiquidity + liquidity - userRealPositionLiquidity;
