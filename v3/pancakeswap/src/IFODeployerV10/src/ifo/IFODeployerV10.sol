...
contract IFODeployerV10 is Ownable {
    ...
    function createIFO(
        ...
        if (_addresses[0] != NATIVE) require(IERC20(_addresses[0]).totalSupply() >= 0);
        if (_addresses[1] != NATIVE) require(IERC20(_addresses[1]).totalSupply() >= 0);
        ...
        if (_vestingStartTime != 0) {
            if (_vestingStartTime > _startAndEndTimestamps[1] + MAX_BUFFER_SECONDS_VESTING) {
        ...
        bytes32 salt =
            keccak256(abi.encodePacked(_addresses[0], _addresses[1], _addresses[2], _startAndEndTimestamps[0]));
        address ifoAddress;
        ...
        IFOInitializableV10(ifoAddress).initialize(
            _addresses, _startAndEndTimestamps, MAX_BUFFER_SECONDS, _maxPoolId, _vestingStartTime
        );
