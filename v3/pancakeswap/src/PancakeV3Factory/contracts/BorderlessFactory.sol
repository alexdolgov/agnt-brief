...
import './interfaces/IBorderlessFactory.sol';
import "./interfaces/IBorderlessPoolDeployer.sol";
import './interfaces/IBorderlessPool.sol';
...
contract BorderlessFactory is IBorderlessFactory {
    /// @inheritdoc IBorderlessFactory
    ...
    function createPool(
        ...
        pool = IBorderlessPoolDeployer(poolDeployer).deploy(address(this), token0, token1, fee, tickSpacing);
    ...
    function setFeeProtocol(address pool, uint32 feeProtocol0, uint32 feeProtocol1) external override onlyOwner {
        IBorderlessPool(pool).setFeeProtocol(feeProtocol0, feeProtocol1);
    }
    ...
    function collectProtocol(
        ...
        return IBorderlessPool(pool).collectProtocol(recipient, amount0Requested, amount1Requested);
    }

    function setLmPool(address pool, address lmPool) external override onlyOwnerOrLmPoolDeployer {
        IBorderlessPool(pool).setLmPool(lmPool);
    }
}
