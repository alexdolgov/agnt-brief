...
import './interfaces/IPintV3Factory.sol';
import "./interfaces/IPintV3PoolDeployer.sol";
import './interfaces/IPintV3Pool.sol';

/// @title Canonical PintSwap V3 factory
/// @notice Deploys PintSwap V3 pools and manages ownership and control over pool protocol fees
contract PintV3Factory is IPintV3Factory {
    /// @inheritdoc IPintV3Factory
    ...
    function createPool(
        ...
        pool = IPintV3PoolDeployer(poolDeployer).deploy(address(this), token0, token1, fee, tickSpacing);
    ...
    function setFeeProtocol(address pool, uint32 feeProtocol0, uint32 feeProtocol1) external override onlyOwner {
        IPintV3Pool(pool).setFeeProtocol(feeProtocol0, feeProtocol1);
    }
    ...
    function collectProtocol(
        ...
        return IPintV3Pool(pool).collectProtocol(recipient, amount0Requested, amount1Requested);
    }

    function setLmPool(address pool, address lmPool) external override onlyOwnerOrLmPoolDeployer {
        IPintV3Pool(pool).setLmPool(lmPool);
    }
}
