...
import './interfaces/IPlunderV3Factory.sol';
import './interfaces/IPlunderV3PoolDeployer.sol';
import './interfaces/IPlunderV3Pool.sol';

/// @title Canonical PlunderSwap V3 factory
/// @notice Deploys PlunderSwap V3 pools and manages ownership and control over pool protocol fees
contract PlunderV3Factory is IPlunderV3Factory {
    /// @inheritdoc IPlunderV3Factory
    ...
    function createPool(address tokenA, address tokenB, uint24 fee) external override returns (address pool) {
        ...
        pool = IPlunderV3PoolDeployer(poolDeployer).deploy(address(this), token0, token1, fee, tickSpacing);
    ...
    function setFeeProtocol(address pool, uint32 feeProtocol0, uint32 feeProtocol1) external override onlyOwner {
        IPlunderV3Pool(pool).setFeeProtocol(feeProtocol0, feeProtocol1);
    }
    ...
    function collectProtocol(
        ...
        return IPlunderV3Pool(pool).collectProtocol(recipient, amount0Requested, amount1Requested);
    }

    function setLmPool(address pool, address lmPool) external override onlyOwnerOrLmPoolDeployer {
        IPlunderV3Pool(pool).setLmPool(lmPool);
    }
}
