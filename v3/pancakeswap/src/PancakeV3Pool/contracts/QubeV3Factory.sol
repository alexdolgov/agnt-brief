...
/// @title Canonical QubeSwap V3 factory
/// @notice Deploys QubeSwap V3 pools and manages ownership and control over pool protocol fees
contract QubeV3Factory is IQubeV3Factory {
    /// @inheritdoc IQubeV3Factory
    ...
    function createPool(
        ...
        pool = IQubeV3PoolDeployer(poolDeployer).deploy(address(this), token0, token1, fee, tickSpacing);
    ...
    function setFeeProtocol(address pool, uint32 feeProtocol0, uint32 feeProtocol1) external override onlyOwner {
        IQubeV3Pool(pool).setFeeProtocol(feeProtocol0, feeProtocol1);
    }
    ...
    function collectProtocol(
        ...
        return IQubeV3Pool(pool).collectProtocol(recipient, amount0Requested, amount1Requested);
    }

    function setLmPool(address pool, address lmPool) external override onlyOwnerOrLmPoolDeployer {
        IQubeV3Pool(pool).setLmPool(lmPool);
    }
}
