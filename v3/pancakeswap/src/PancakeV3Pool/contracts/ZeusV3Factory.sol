...
import './interfaces/IZeusV3Factory.sol';
import "./interfaces/IZeusV3PoolDeployer.sol";
import './interfaces/IZeusV3Pool.sol';

/// @title Canonical ZeusSwap V3 factory
/// @notice Deploys ZeusSwap V3 pools and manages ownership and control over pool protocol fees
contract ZeusV3Factory is IZeusV3Factory {
    /// @inheritdoc IZeusV3Factory
    ...
    function createPool(
        ...
        pool = IZeusV3PoolDeployer(poolDeployer).deploy(address(this), token0, token1, fee, tickSpacing);
    ...
    function setFeeProtocol(address pool, uint32 feeProtocol0, uint32 feeProtocol1) external override onlyOwner {
        IZeusV3Pool(pool).setFeeProtocol(feeProtocol0, feeProtocol1);
    }
    ...
    function collectProtocol(
        ...
        return IZeusV3Pool(pool).collectProtocol(recipient, amount0Requested, amount1Requested);
    }

    function setLmPool(address pool, address lmPool) external override onlyOwnerOrLmPoolDeployer {
        IZeusV3Pool(pool).setLmPool(lmPool);
    }
}
