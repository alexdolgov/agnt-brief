...
contract SwapRouter is ISwapRouter02, Ownable {
    ...
    function _swap(
        ...
        for (uint256 i; i < path.length - 1; i++) {
            ...
            if (tradingPool != address(0)) {
                ITradingPool(tradingPool).enter(tx.origin, input, output, amountOut);
            }
    ...
    function swapExactETHForTokens(
        ...
        assert(IWBNB(WETH).transfer(SwapLibrary.pairFor(factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
    }
    ...
    function swapExactETHForTokensSupportingFeeOnTransferTokens(
        ...
        assert(IWBNB(WETH).transfer(SwapLibrary.pairFor(factory, path[0], path[1]), amountIn));
        uint256 balanceBefore = IERC20(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        require(
            IERC20(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >= amountOutMin,
