...
contract StakingRewardsV2 is ReentrancyGuard, Ownable {
    ...
    // address that is also allowed to call owner-restricted functions
    ...
    // set the feeCollector address that shares permissions with owner
    function setFeeCollector(address _feeCollector) external onlyOwner {
        feeCollector = _feeCollector;
    }

    // Always needs to update the balance of the contract when calling this method
    function notifyRewardAmount(uint256 reward) external onlyOwnerOrFeeCollector updateReward(address(0)) {
    ...
    function recoverERC20(address tokenAddress, uint256 tokenAmount) external onlyOwnerOrFeeCollector nonReentrant {
        require(tokenAddress != address(stakingToken), "Cannot withdraw the staking token");
        IERC20(tokenAddress).safeTransfer(owner(), tokenAmount);
        emit Recovered(tokenAddress, tokenAmount);
    }

    function setRewardsDuration(uint256 _rewardsDuration) external onlyOwnerOrFeeCollector {
        require(
    ...
    modifier onlyOwnerOrFeeCollector() {
        require(msg.sender == owner() || msg.sender == feeCollector, "Not owner or feeCollector");
        _;
    }
