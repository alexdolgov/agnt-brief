...
contract Paxe is ERC20, Ownable {
 ...
 using SafeERC20 for IERC20;
    ...
    uint256 public constant treasuryBuyFee = 2;
    uint256 public constant treasurySellFee = 2;
    uint256 public totalFees = treasuryBuyFee + treasurySellFee;

    address public treasuryWalletAddress = 0x513624C9569819123b2F7F4D8916f796152BB7e9;
    ...
    event SwapTokensForUSDTFailed(uint256 amount);
    ...
    constructor(address newOwner) ERC20("Paxe Token", "PAXE") {
        transferOwnership(newOwner);
        uint256 totalSupply_ = 100_000_000 * 10 ** 18;
        ...
        address pair = factory.createPair(address(this), USDT);
    ...
    function setSwapTokensAtAmount(uint256 amount) external onlyOwner {
        require(
            amount > totalSupply() / 10 ** 6 && amount <= totalSupply() / 10 ** 4,
            "Amount must be between 0.0001% - 0.01 of total supply"
        );
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        ...
        require(automatedMarketMakerPairs[pair] != value, "Pair is already set to this address.");
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (
            canSwap && !localSwapping && !automatedMarketMakerPairs[from] && from != owner() && to != owner()
                && localTotalFees > 0
        ) {
            swap();
        }

        bool takeFee = !localSwapping && !_isExcludedFromFees[from] && !_isExcludedFromFees[to] && localTotalFees > 0;
        if (takeFee) {
            localTotalFees = 0;
            if (automatedMarketMakerPairs[from] && to != address(uniswapV2Router)) {
                localTotalFees = treasuryBuyFee;
            }

            if (automatedMarketMakerPairs[to] && from != address(uniswapV2Router)) {
                localTotalFees = treasurySellFee;
            }
    ...
    function _transferOwnership(address newOwner) internal override {
        ...
        _isExcludedFromFees[oldOwner] = false;
        _isExcludedFromFees[newOwner] = true;

        super._transferOwnership(newOwner);

        emit IncludeInFees(oldOwner);
        emit ExcludeFromFees(newOwner);
    }

    function swap() private lockTheSwap {
        uint256 amount = swapTokensAtAmount;

        swapTokensForUSDT(amount, treasuryWalletAddress);
    }

    function swapTokensForUSDT(uint256 tokenAmount, address destination) private {
        ...
        try uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount, 0, path, destination, block.timestamp
        ) {} catch {
            emit SwapTokensForUSDTFailed(tokenAmount);
        }
    }
}