...
interface IErrors {
    ...
    /**
     * @dev Error thrown when the msg.sender is not the signer wallet.
     */
    error OnlySigner();
    ...
    /**
     * @dev Thrown when the tokens paired by the PAXE Oracle and the SUSD Oracle do not match.
     */
...
// File src/interfaces/IEvents.sol
...
interface IEvents {
    ...
    /**
     * @dev Emits an event when the special referrer is updated.
     */
    ...
    /**
     * @dev Emits an event when the signer wallet is updated.
     */
    event SignatureSignerUpdated(address indexed oldSignerWallet, address indexed newSignerWallet);
...
contract SUSDLiquidFarming is Ownable, IErrors, IEvents {
    ...
    address public immutable mainFarming;
    ...
    constructor(
        ...
        if (_SUSD == address(0)) revert ZeroAddressNotAllowed();
        if (_paxeTreasury == address(0)) revert ZeroAddressNotAllowed();
        if (_mainFarming == address(0)) revert ZeroAddressNotAllowed();
     ...
     *
     * Emits a {SetBurnDepositedPAXE} event.
     */
    ...
    /**
     * @dev Sets the signer wallet.
     * Callable only by the owner.
     *
     * Emits a {SignatureSignerUpdated} event.
     */
    function setSignatureSigner(address newWallet) external onlyOwner {
        address tmpWallet = signatureSigner;
        signatureSigner = newWallet;
        emit SignatureSignerUpdated(tmpWallet, signatureSigner);
    }

    /**
     * @dev Stake an equal value of SUSD and PAXE tokens, use refCode.
     */
    function deposit(uint256 susdAmount, uint256 paxeAmount, string memory refCode) external payable {
        _deposit(susdAmount, paxeAmount, refCode);
    }

    /**
     * @dev Stake an equal value of SUSD and PAXE tokens, mint pPAXE and record the staking information.
     *
     * Emits a {Stake} event.
     */
    function _deposit(uint256 susdAmount, uint256 paxeAmount, string memory refCode) internal updateOracle {
        if (msg.value != STAKE_FEE) revert InvalidEtherValue(STAKE_FEE);
        if (susdAmount == 0 || paxeAmount == 0) revert InsufficientStakeAmount();

        uint256 susdValue = susdAmount;
        if (susdAmount / 10 ** DOLLAR_DECIMALS < MINIMUM_STAKE_AMOUNT) revert InsufficientStakeAmount();

        uint256 paxeTokensNeeded = getTokenAmount(address(PAXE), susdValue);
        if (paxeAmount < paxeTokensNeeded) revert InsufficientPaxeTokens();

        SUSD.safeTransferFrom(msg.sender, DEAD_ADDRESS, susdAmount);

        if (burnDepositedPAXE) {
            PAXE.safeTransferFrom(msg.sender, DEAD_ADDRESS, paxeTokensNeeded);
        } else {
            PAXE.safeTransferFrom(msg.sender, address(mainFarming), paxeTokensNeeded);
        }
        payable(paxeTreasury).transfer(STAKE_FEE);

        pPAXE.mint(msg.sender, paxeTokensNeeded);

        uint256 stakeValue = susdValue + getDollarValue(address(PAXE), paxeTokensNeeded);
