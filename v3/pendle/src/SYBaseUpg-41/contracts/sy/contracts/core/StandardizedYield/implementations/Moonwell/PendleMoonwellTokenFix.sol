...
contract PendleMoonwellTokenFix is SYBaseWithRewardsUpg, IPTokenWithSupplyCap {
    ...
    function adjustLastBalance() onlyOwner external {
        rewardState[USDC].lastBalance = PMath.Uint128(_selfBalance(USDC));
    }
    ...
    function _redeem(
        ...
        if (tokenOut == yieldToken) {
            ...
            uint256 preBalance = _selfBalance(underlying);
            uint256 errCode;
            if ((errCode = IMErc20(yieldToken).redeem(amountSharesToRedeem)) != NO_ERROR) {
                revert MoonwellRedeemError(errCode);
            }
            _transferOut(underlying, receiver, amountTokenOut = _selfBalance(underlying) - preBalance);
