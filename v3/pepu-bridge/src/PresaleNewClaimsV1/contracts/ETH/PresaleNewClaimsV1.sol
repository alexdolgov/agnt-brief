...
contract PresaleNewClaimsV1 is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable {
   ...
   * @param _claimStart epoch claim start time // can be set to 0 if claim time is unknown
   * @param _saleToken sale token contract address
   */
  function initialize(uint256 _claimStart, address _saleToken) external initializer {
    ...
    emit ClaimStartUpdated(0, _claimStart, block.timestamp);
  }
