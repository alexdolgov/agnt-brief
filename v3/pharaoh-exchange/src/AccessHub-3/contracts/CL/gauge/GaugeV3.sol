...
contract GaugeV3 is IGaugeV3, Initializable {
    ...
    function initialize(
        ...
        address phar = IVoter(_voter).ram();
        address xrex = IVoter(_voter).xRam();
        ...
        rewards.push(token1);
        rewards.push(xrex);
        (isReward[token0], isReward[token1], isReward[xrex]) = (true, true, true);
        
        /// @dev if token0 and token1 aren't shadow add phar in the records (prevents duplicates)
        if (token0 != phar && token1 != phar) {
            rewards.push(phar);
            isReward[phar] = true;
        }
