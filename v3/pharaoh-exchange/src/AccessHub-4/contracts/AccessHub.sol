...
contract AccessHub is IAccessHub, Initializable, AccessControlEnumerableUpgradeable {
 ...
 // @inheritdoc IAccessHub
    ...
    function adjustEmissionsInMinter(int256 _basisPointsChange) external onlyRole(PROTOCOL_OPERATOR) {
        minter.adjustEmissions(_basisPointsChange);
    }
    ...
    function addRewardToAllGauges(address _reward) external onlyRole(PROTOCOL_OPERATOR) {
        ...
        for (uint256 i = 0; i < allGauges.length; i++) {
            // Only add to CL gauges (V3), skip legacy gauges
            if (voter.isClGauge(allGauges[i])) {
                try IGaugeV3(allGauges[i]).addRewards(_reward) {} catch {}
            }
        }
    }
    ...
    function removeRewardFromAllGauges(address _reward) external onlyRole(PROTOCOL_OPERATOR) {
        address[] memory allGauges = voter.getAllGauges();
        for (uint256 i = 0; i < allGauges.length; i++) {
            // Only remove from CL gauges (V3), skip legacy gauges
            if (voter.isClGauge(allGauges[i])) {
                try IGaugeV3(allGauges[i]).removeRewards(_reward) {} catch {}
            }
        }
    }
    ...
    function setOperatorAutoVault(address _operator) external onlyRole(PROTOCOL_OPERATOR) {
    ...
    /// @notice rescues full balance of token and wipes its input budgets in AutoVault
    /// @param _token the token address to rescue and wipe
    function rescueAndWipeAutoVault(address _token) external onlyMultisig {
        autoVault.rescueAndWipe(_token);
    ...
    /// @notice sets the legacy router address for LP unwrapping in AutoVault
    /// @param _legacyRouter the legacy router address
    function setLegacyRouterAutoVault(address _legacyRouter) external onlyMultisig {
        autoVault.setLegacyRouter(_legacyRouter);
    }
}
