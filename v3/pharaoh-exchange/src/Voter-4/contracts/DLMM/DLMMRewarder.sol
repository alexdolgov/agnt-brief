...
contract DLMMRewarder is DLMMBaseHooks, Ownable2Step {
    ...
    IDLMMPool internal immutable dlmmPool;
    ...
    constructor(IDLMMPool _dlmmPool, IERC20 _rewardToken, address initialOwner) Ownable(initialOwner) {
        dlmmPool = _dlmmPool;
    ...
    function _pendingRewardForId(
        ...
        if (accRewardsPerShareX64 > userAccRewardsPerShareX64) {
            ...
            pendingRewards = (accRewardsPerShareX64 - userAccRewardsPerShareX64).mulShiftRoundDown(
                pair.balanceOf(user, id), OFFSET_PRECISION
            );
        }
    }
    ...
    function _updateUser(address user, uint256[] memory ids) internal {
        ...
        for (uint256 i; i < ids.length; ++i) {
            ...
            if (accRewardsPerShareX64 > userAccRewardsPerShareX64) {
                uint256 userRewards = (accRewardsPerShareX64 - userAccRewardsPerShareX64).mulShiftRoundDown(
                    pair.balanceOf(user, id), OFFSET_PRECISION
                );
