...
/// @title DLMM Hook Base
/// @notice Base contract for DLMM pool hooks
/// Inherited by any contract that wants to implement hooks on DLMM pools
abstract contract DLMMBaseHooks is IDLMMHooks {
    ...
    /// @dev Returns the DLMM pool contract
    /// @return The DLMM pool contract
    function getLBPair() external view override returns (IDLMMPool) {
        return _getLBPair();
    }
    ...
    /// @dev Returns the DLMM pool contract
    function _getLBPair() internal view virtual returns (IDLMMPool);
