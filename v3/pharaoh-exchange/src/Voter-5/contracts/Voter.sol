...
contract Voter is IVoter, ReentrancyGuard, Initializable {
    ...
    function distribute(address _gauge) public nonReentrant {
        ...
        if (_lastDistro != currentPeriod) {
            ...
            if ($.isClGauge[_gauge]) {
                if ($.voterOwnsFactory) {
                    /// @dev read current pool fee split from the factory
                    uint24 currentSplit = IRamsesV3Factory($.clFactory).poolFeeProtocol(pool);
                    /// @dev if governance set a custom split via AccessHub, leave it untouched across epochs
                    if (
                        currentSplit == 1_000_000 ||
                        currentSplit == 50_000 ||
                    ...
                    /// @dev else: custom governance-set split — preserved across distribute() calls
                }
                address poolV3 = pool;
                /// @dev try/catch since some tokens (e.g. avUSD) revert on dust transfers
                try IFeeCollector(IRamsesV3Factory($.clFactory).feeCollector()).collectProtocolFees(poolV3) {} catch {}
            }
