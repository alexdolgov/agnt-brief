...
import "./interfaces/IIndexRouterV2.sol";
...
contract IndexRouterV2 is IIndexRouterV2, UUPSUpgradeable, ReentrancyGuardUpgradeable {
    ...
    ) external override nonReentrant isValidIndex(_params.index) returns (uint) {
    ...
    ) public override nonReentrant isValidIndex(_params.index) returns (uint) {
    ...
    /// @inheritdoc IIndexRouterV2
    function mintSwapValue(
        MintSwapValueParams calldata _params
    ) external payable override nonReentrant isValidIndex(_params.index) returns (uint) {
    ...
    /// @inheritdoc IIndexRouterV2
    function burnWithAmounts(
        BurnParams calldata _params
    ) external override nonReentrant isValidIndex(_params.index) returns (uint[] memory _amounts) {
        IERC20(_params.index).safeTransferFrom(msg.sender, _params.index, _params.amount);
    ...
    /// @inheritdoc IIndexRouterV2
    function burnSwapValue(
        BurnSwapParams calldata _params
    ) public override nonReentrant isValidIndex(_params.index) returns (uint wethBalance) {
    ...
    function _mint(
        ...
        for (uint i; i < quotesCount; i++) {
            ...
            _safeApprove(_inputToken, _quotes[i].allowanceTarget, _amountInInputToken);
        ...
        for (uint i; i < totalAssetsCount; ) {
            ...
            if (inputAssetBalanceBefore > 0 && address(inputAsset) != _params.outputAsset) {
                ...
                // otherwise exchange the asset for the desired output asset
                    ...
                    _safeApprove(address(inputAsset), _params.quotes[i].allowanceTarget, inputAssetBalanceBefore);
    ...
    function _safeApprove(address _token, address _spender, uint _requiredAllowance) internal {
