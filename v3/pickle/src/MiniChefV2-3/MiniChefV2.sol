...
contract MiniChefV2 is BoringOwnable, BoringBatchable {
        ...
        returns (uint256 pending)
    {
        ...
        if (block.timestamp > pool.lastRewardTime && lpSupply != 0) {
            ...
            uint256 pickleReward = time.mul(picklePerSecond).mul(
                pool.allocPoint
            ) / totalAllocPoint;
            accPicklePerShare = accPicklePerShare.add(
                pickleReward.mul(ACC_PICKLE_PRECISION) / lpSupply
            );
        }
        pending = int256(
            user.amount.mul(accPicklePerShare) / ACC_PICKLE_PRECISION
        ).sub(user.rewardDebt)
        .toUInt256();
    }
        ...
        if (block.timestamp > pool.lastRewardTime) {
            ...
            if (lpSupply > 0) {
                ...
                pool.accPicklePerShare = pool.accPicklePerShare.add(
                    (pickleReward.mul(ACC_PICKLE_PRECISION) / lpSupply).to128()
                );
            }
    ...
    function deposit(
        ...
        user.rewardDebt = user.rewardDebt.add(
            int256(amount.mul(pool.accPicklePerShare) / ACC_PICKLE_PRECISION)
        );
    ...
    function harvest(uint256 pid, address to) public {
        PoolInfo memory pool = updatePool(pid);
        UserInfo storage user = userInfo[pid][msg.sender];
        int256 accumulatedPickle = int256(
            user.amount.mul(pool.accPicklePerShare) / ACC_PICKLE_PRECISION
        );
        uint256 _pendingPickle = accumulatedPickle
        .sub(user.rewardDebt)
        .toUInt256();
    ...
    function withdrawAndHarvest(
        ...
        // Effects
        user.rewardDebt = accumulatedPickle.sub(
            int256(amount.mul(pool.accPicklePerShare) / ACC_PICKLE_PRECISION)
        );
        user.amount = user.amount.sub(amount);
