...
interface IMultipleMerkleDistributor {
    ...
    /// @notice event is triggered whenever a token is withdrawn by owner
...
/// Adapted from https://optimistic.etherscan.io/address/0x49b7e13922c46f403b18cfdcdc729dd5de614ea0
/// @title PIKA MultipleMerkleDistributor
...
contract MultipleMerkleDistributor is IMultipleMerkleDistributor, Owned {
    ...
    /// @notice token to be distributed (esPIKA)
    ...
    mapping(uint256 => mapping(uint256 => uint256)) public claimedBitMaps;

    /// @notice set addresses for deployed esPIKA.
    ...
    /// @notice set a merkle root for the distribution epoch
    /// @param _merkleRoot: new merkle root
    /// @param _distributionEpoch: distribution epoch
    function updateMerkleRoot(bytes32 _merkleRoot, uint256 _distributionEpoch) external onlyOwner {
        merkleRoots[_distributionEpoch] = _merkleRoot;
        emit MerkleRootAdded(_distributionEpoch);
    }
    ...
    /// @notice attempt to claim as `account` and esPIKA for `account`
    /// @param index: used for merkle tree managment and verification
    /// @param account: address used for escrow entry
    /// @param amount: $esPIKA amount to be claimed
    /// @param merkleProof: off-chain generated proof of merkle tree inclusion
    /// @param epoch: distribution index to check
    function claim(
        uint256 index,
        ...
        // mark it claimed and send the token to account
