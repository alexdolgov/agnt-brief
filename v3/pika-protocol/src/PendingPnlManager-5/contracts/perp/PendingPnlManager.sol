...
contract PendingPnlManager {
    ...
    function getPnlChangeAndPrice(uint256 productId) public view returns(int256,uint256) {
        ...
        return (PerpLib._getPnl(openInterestLong > openInterestShort, lastPrices[productId], 1e8, oiDelta, price) - cumulativeFundingPayment[productId] -
    ...
    function getPnlChange(uint256 productId) public view returns(int256,int256) {
        (address productToken,,,,uint256 openInterestLong,uint256 openInterestShort,,) = IPikaPerp(pikaPerp).getProduct(productId);
        if (productToken == address(0)) {
            return (0, 0);
        }
        ...
        PerpLib._getFundingPayment(fundingManager, openInterestLong > openInterestShort, productId, 1e8, oiDelta, lastFundings[productId]));
    }
