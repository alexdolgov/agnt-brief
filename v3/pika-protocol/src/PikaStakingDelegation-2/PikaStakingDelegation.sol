...
contract PikaStakingDelegation {
    ...
    /// @dev Get the effective staking balance for the addresses for fee discount calculation. 
    /// If the address is a delegate address, return its delegator's staking balance;
    /// If the address is not a delegate address but is a delegator address, return 0 staking balance for the address.
    /// If the address is not a delegate address and is not delegator address, return the staking balance for the address.
    function getEffectiveBalances(address[] memory _addresses) external view returns(uint256[] memory) {
        uint256 length = _addresses.length;
        uint256[] memory balances = new uint256[](length); 
        for (uint256 i = 0; i < length; i++) {
            address userAddress = delegators[_addresses[i]] == address(0) ? _addresses[i] : delegators[_addresses[i]];
            if (userAddress == _addresses[i] && delegates[userAddress] != address(0)) {
                balances[i] == 0;
                continue;
            } 
            uint256 balance = IPikaStaking(pikaStaking).balanceOf(userAddress);
