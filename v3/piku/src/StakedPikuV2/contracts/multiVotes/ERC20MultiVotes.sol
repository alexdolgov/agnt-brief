...
import {IERC20MultiVotes} from "contracts/interfaces/IERC20MultiVotes.sol";

/// @title ERC20 Multi-Delegation Voting contract
abstract contract ERC20MultiVotes is ERC20Permit, Ownable, IERC20MultiVotes {
    ...
    function delegateBySig(address delegatee, uint256 nonce, uint256 expiry, uint8 v, bytes32 r, bytes32 s) public {
        ...
        address signer = ecrecover(
            keccak256(
                abi.encodePacked(
                    "\x19\x01", _domainSeparatorV4(), keccak256(abi.encode(DELEGATION_TYPEHASH, delegatee, nonce, expiry))
                )
            ),
            v,
            r,
