...
contract BuybackBabyToken is IERC20Extended, Auth {
    ...
    struct Args {
        ...
        address rewardToken;
        address router;
        uint256[5] feeSettings;
        address serviceFeeReceiver;
        uint256 serviceFee;
        address tokenRegistry;
    }

    constructor(Args memory args) payable Auth(msg.sender) {
        require(args.tokenRegistry != address(0), "TokenRegistry: required");
        _name = args.name;
        _symbol = args.symbol;
        _totalSupply = args.totalSupply;

        rewardToken = args.rewardToken;
        router = IUniswapV2Router02(args.router);
        pair = IUniswapV2Factory(router.factory()).createPair(
            address(this),
            router.WETH()
        );
        distributor = new DividendDistributor(args.rewardToken, args.router);

        _initializeFees(args.feeSettings);
        ...
        ITokenRegistry(args.tokenRegistry).register(
            msg.sender,
            TokenType.buybackBaby,
            VERSION
        );

        payable(args.serviceFeeReceiver).transfer(args.serviceFee);
    }
