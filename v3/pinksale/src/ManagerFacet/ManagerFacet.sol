...
// Root file: contracts/pink-degen/facets/ManagerFacet.sol

pragma solidity ^0.8.0;

// import "@openzeppelin/contracts/utils/structs/EnumerableSet.sol";
// import {LibDiamond} from "contracts/pink-degen/libraries/LibDiamond.sol";
// import {DegenBase, DegenPoolInfo, DegenPoolConfig, DegenMasterConfig} from "contracts/pink-degen/libraries/LibDegenAppStorage.sol";
// import {LibDegen} from "contracts/pink-degen/libraries/LibDegen.sol";

contract ManagerFacet is DegenBase {
    using EnumerableSet for EnumerableSet.AddressSet;
    ...
    function getMasterConfig()
        external
        view
        returns (DegenMasterConfig memory)
    {
    ...
    function setPoolConfig(
        uint256 index,
        DegenPoolConfig memory poolConfig
    ) external onlyAdmin {
    ...
    function getPoolConfig(
        uint256 index
    ) external view returns (DegenPoolConfig memory) {
        DegenPoolConfig memory config = s.poolConfigMapping[index];
        require(
            config.initialVirtualBaseReserve > 0,
            "PinkDegen: Invalid config"
        );
        return config;
    }
    ...
    function getAllPoolConfigs()
        external
        view
        returns (DegenPoolConfig[] memory)
    {
        uint256 count = s.poolConfigIndexes.length();
        DegenPoolConfig[] memory configs = new DegenPoolConfig[](count);
    ...
    function setPoolState(address token, uint8 state) external onlyOperator {
        DegenPoolInfo storage poolInfo = s.poolInfoMapping[token];
        require(poolInfo.token != address(0), "PinkDegen: Invalid pool");
    ...
    function getPoolAt(
        uint256 index
    ) external view returns (DegenPoolInfo memory) {
        require(
            s.tokens.at(index) != address(0),
            "PinkPump: Invalid token index"
        );
    ...
    ) public view returns (DegenPoolInfo memory) {
        DegenPoolInfo memory poolInfo = s.poolInfoMapping[tokenAddress];
        require(
            poolInfo.token == tokenAddress,
            "PinkPump: Invalid token address"
        );
        return poolInfo;
    }
