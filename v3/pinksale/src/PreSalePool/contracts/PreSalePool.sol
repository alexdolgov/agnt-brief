...
import "./extensions/MLandWhitelist.sol";

contract PreSalePool is Ownable, ReentrancyGuard, Pausable, MLandWhitelist {
    ...
    fallback() external payable {
        revert("POOL::NOT_ACCEPT_FALLBACK");
    }
    ...
    function changeWallet(address _newWallet) external onlyOwner {
        emit WalletChanged(fundingWallet, _newWallet);
    ...
    function changeSaleToken(address _token) external onlyOwner {
        require(_token != address(0), "POOL::INVALID_ADDRESS");
        token = IERC20(_token);
    ...
    function buyTokenByEtherWithPermission(
        ...
        require(
            ...
            "POOL::NOT_ENOUGH_TOKENS_FOR_SALE"
        );

        require(
            tokens >= _minAmount ||
                userPurchased[_candidate] + tokens >= _minAmount,
            "POOL::MIN_AMOUNT_UNREACHED"
        );
        require(
            userPurchased[_candidate] + tokens <= _maxAmount,
        ...
        investedAmountOf[address(0)][_candidate] =
            investedAmountOf[address(0)][_candidate] +
            weiAmount;
    ...
    function buyTokenByTokenWithPermission(
        ...
        investedAmountOf[_token][_candidate] =
            investedAmountOf[address(0)][_candidate] +
            _amount;
    ...
    function claimTokens(
        ...
        uint256 maxClaimAmount = userPurchased[_candidate] -
     ...
     */
    function _preValidatePurchase(uint256 _weiAmount) private pure {
     ...
     * @param _to Address performing the token purchase
     * @param _tokenAmount Number of tokens to be emitted
     */
    function _deliverTokens(address _to, uint256 _tokenAmount) private {
        require(
     ...
     */
    function _forwardFunds(uint256 _value) private {
        address payable wallet = payable(fundingWallet);
        (bool success, ) = wallet.call{ value: _value }("");
        require(success, "POOL::WALLET_safeTransfer_FAILED");
    }

    /**
     * @dev Determines how Token is stored/forwarded on purchases.
     */
    function _forwardTokenFunds(address _token, uint256 _amount) private {
    ...
    function _validPurchase() private view returns (bool) {
