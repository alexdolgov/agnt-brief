...
// Dependency file: contracts/PinkswapPair.sol
...
// import './interfaces/IPinkswapFactory.sol';
// import './PinkswapPair.sol';

contract PinkswapFactory is IPinkswapFactory {
    bytes32 public constant INIT_CODE_PAIR_HASH = keccak256(abi.encodePacked(type(PinkswapPair).creationCode));
    ...
    function createPair(address tokenA, address tokenB) external returns (address pair) {
        require(tokenA != tokenB, 'Pinkswap: IDENTICAL_ADDRESSES');
        (address token0, address token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), 'Pinkswap: ZERO_ADDRESS');
        require(getPair[token0][token1] == address(0), 'Pinkswap: PAIR_EXISTS'); // single check is sufficient
        bytes memory bytecode = type(PinkswapPair).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(token0, token1));
        assembly {
            pair := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        IPinkswapPair(pair).initialize(token0, token1);
    ...
    function setFeeTo(address _feeTo) external {
        require(msg.sender == feeToSetter, 'Pinkswap: FORBIDDEN');
        feeTo = _feeTo;
    }
