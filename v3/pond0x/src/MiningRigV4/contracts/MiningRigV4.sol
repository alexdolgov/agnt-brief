...
contract MiningRigV4 is ReentrancyGuard, IActiveChecker, MiningClaims, MathHelpers, RigTokenBoosterManager, ExecutorManager {
    ...
    constructor(
        ...
        porkContract.approve(address(nonfungiblePositionManager), type(uint256).max);
        porkContract.approve(address(uniswapRouter), type(uint256).max);
        porkContract.approve(address(spawnManager), type(uint256).max);
    }
    ...
    function createInitialLP(uint256 pepeAmount, uint256 wethAmount) external onlyExecutor() {
        ...
        require(porkContract.transferFrom(msg.sender, address(this), pepeAmount), "Could not transfer pepe");

        (,,uint256 addedPepe, uint256 addedWeth) = _mintLiquidityPosition(pepeAmount, wethAmount);

        if (addedPepe < pepeAmount) {
            require(porkContract.transfer(msg.sender, pepeAmount - addedPepe), "Could not return pepe");
        }
    ...
    function _remint() internal {
        ...
        uint256 spawnAmount = porkContract.balanceOf(address(this));
        spawnManager.createSpawn(spawnAmount);
        porkContract.transfer(address(spawnManager), spawnAmount);
    }
    ...
    function _mine(
        ...
        uint256 recievedPepe = _swapTokens(wethContract, porkContract, toConvertWeth);
    ...
    function addExecutor(address _toAdd) onlyExecutor external override {
        porkContract.approve(_toAdd, type(uint256).max);
    ...
    function removeExecutor(address _toRemove) onlyExecutor external override {
        porkContract.approve(_toRemove, 0);
