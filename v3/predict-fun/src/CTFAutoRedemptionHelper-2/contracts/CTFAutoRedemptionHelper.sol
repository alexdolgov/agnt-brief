...
contract CTFAutoRedemptionHelper is AccessControl, ERC1155Holder {
    ...
    /**
     * @notice Emitted when positions are redeemed on behalf of a user
     * @param redeemer The user whose positions were redeemed
     * @param isYieldBearing Whether the CTF is yield-bearing
     * @param isNegRisk Whether the CTF is Neg Risk
     * @param conditionIds The condition IDs that were redeemed
     * @param indexSets The index sets for each condition
     * @param amounts The position token amounts redeemed per condition
     * @param payouts The collateral payout amounts per condition
     */
    event CTFAutoRedemptionHelper__PayoutRedemption(
    ...
    function _processRedemption(RedemptionRequest calldata redemptionRequest) private {
        ...
        uint256[] memory positionIds = _getPositionIds(
            _getCtfCollateral(redemptionRequest.isYieldBearing, redemptionRequest.isNegRisk),
            conditionIds,
            indexSets
        );

        IConditionalTokens ctf = _getCTF(redemptionRequest.isYieldBearing, redemptionRequest.isNegRisk);

        if (ctf.isApprovedForAll(redemptionRequest.user, address(this))) {
            uint256[] memory balances = _getBalancesAndTransfer(
                ctf,
                positionIds,
                redemptionRequest.user,
                address(this)
            );
            (uint256[] memory payouts, uint256 totalPayout) = _redeemPositions(redemptionRequest, balances);
            // 0 payout redemptions don't transfer collateral
            if (totalPayout > 0) {
                COLLATERAL.safeTransfer(redemptionRequest.user, totalPayout);
            }

            emit CTFAutoRedemptionHelper__PayoutRedemption(
                redemptionRequest.user,
                redemptionRequest.isYieldBearing,
                redemptionRequest.isNegRisk,
                conditionIds,
                indexSets,
                balances,
                payouts
            ...
            emit CTFAutoRedemptionHelper__ContractNotApprovedForAll(redemptionRequest.user, address(ctf));
            return;
        }
    }
    ...
    function _redeemPositions(
        RedemptionRequest calldata redemptionRequest,
        uint256[] memory balances
    ) private returns (uint256[] memory payouts, uint256 totalPayout) {
        bytes32[] calldata conditionIds = redemptionRequest.conditionIds;
        uint256 conditionsCount = conditionIds.length;

        payouts = new uint256[](conditionsCount);
        uint256 currentCollateralBalance = COLLATERAL.balanceOf(address(this));
        ...
        if (redemptionRequest.isNegRisk) {
            ...
            for (uint256 j; j < conditionsCount; j++) {
                ...
                (payouts[j], currentCollateralBalance) = _updatePayout(currentCollateralBalance);
                totalPayout += payouts[j];
            }
        } else {
    ...
    ) private view returns (uint256 payout, uint256 newCollateralBalance) {
        newCollateralBalance = COLLATERAL.balanceOf(address(this));
        payout = newCollateralBalance - currentCollateralBalance;
    }
