...
contract Controllers {
    ...
    // BxdIndex Address.
    address public immutable bxdIndex;

    constructor(address _bxdIndex) {
        bxdIndex = _bxdIndex;
    }
    ...
    /**
    * @dev Throws if the sender not is Master Address from BxdIndex
    * or Enabled Chief.
    */
    modifier isChief {
        require(chief[msg.sender] || msg.sender == IndexInterface(bxdIndex).master(), "not-an-chief");
        _;
    }
    ...
    */
    function toggleChief(address _chiefAddress) external {
        require(msg.sender == IndexInterface(bxdIndex).master(), "toggleChief: not-master");
...
contract BxdConnectorsV2 is Controllers {
    ...
    constructor(address _bxdIndex) public Controllers(_bxdIndex) {}
