...
import {PausableInternal} from "@solidstate/contracts/security/pausable/Pausable.sol";
...
contract Staker is ReentrancyGuard, PausableInternal, Context {
    ...
    function burnAndRedeem(uint256 tokenId) public nonReentrant whenNotPaused returns (uint256 nftStaked) {
        ...
        require(nftData.lockedData.initialUnlockTimestamp <= block.timestamp, TokenLocked());
    ...
    function lockNFT(uint256 tokenId) external whenNotPaused {
        ...
        if (lockedData.initialUnlockTimestamp > 0) {
            lockedData.initialUnlockTimestamp = 0;
            lockedData.initialVestingAmount = 0;
        }
    ...
    function unlockNFT(uint256 tokenId) external whenNotPaused {
        ...
        lockedData.unlockTimestamp = 0;
        lockedData.lockedAmount = 0;

        // Remove the token from the locked tokens list
        uint256[] storage lockedTokens = $.lockedTokens;
    ...
    function withdraw(uint256 tokenId, uint256 amount) public nonReentrant whenNotPaused returns (uint256 amountToWithdraw) {
        ...
        if (lockedData.initialUnlockTimestamp > block.timestamp) {
            require(nftData.staked - lockedData.initialVestingAmount >= amount, NotEnoughStaked());
        }
    ...
    function withdrawSurplus(uint256 tokenId, uint256 amount) public nonReentrant whenNotPaused {
        ONFTStakerStorage.Layout storage $ = ONFTStakerStorage.layout();
        StakerStorage storage s = $.stakerData;
        ...
        uint256 level = checkLevel(nftData.staked);

        require(level == 20, NotEnoughStaked());

        require(erc721.ownerOf(tokenId) == sender || sender == address(this), InvalidTokenOwner());

        require(nftData.staked - s.stakedNeededForMaxLevel >= amount, NotEnoughStaked());
        ...
        uint256 weightBefore = calcWeight(nftData.staked, nftData.nftMultiplier + level);

        nftData.staked -= amount;

        uint256 weightAfter = calcWeight(nftData.staked, nftData.nftMultiplier + level);
        ...
        emit WithdrawStaked(tokenId, amount, 0);
    }
    ...
    function getWithdrawFee() external view returns (uint256 withdrawFee) {
        withdrawFee = ONFTStakerStorage.layout().stakerData.withdrawFee;
    }

    function getNFTData(uint256 tokenId) external view returns (NftData memory nftData) {
        StakerStorage storage s = ONFTStakerStorage.layout().stakerData;
        nftData = s.nftData[tokenId];
    }

    function getStakerRewardsTotalAmount() external view returns (uint256 rewardsAmount) {
        ONFTStakerStorage.Layout storage $ = ONFTStakerStorage.layout();
        StakerStorage storage s = $.stakerData;

        rewardsAmount = s.rewardsData.rewardsAmount;
    }

    function getStaked(uint256 tokenId) external view returns (uint256 staked) {
        StakerStorage storage s = ONFTStakerStorage.layout().stakerData;
        NftData storage nftData = s.nftData[tokenId];
        staked = nftData.staked;
    }

    function getStakerRewardsClaimedAmount(uint256 tokenId) external view returns (uint256 claimedAmount) {
        ONFTStakerStorage.Layout storage $ = ONFTStakerStorage.layout();
        StakerStorage storage s = $.stakerData;

        claimedAmount = s.rewardsData.lastClaimedAmount[tokenId];
    }

    function getAmountToLevelUp(uint256 tokenId) external view returns (uint256 amountToLevelUp) {
        ONFTStakerStorage.Layout storage $ = ONFTStakerStorage.layout();
        StakerStorage storage s = $.stakerData;

        NftData storage nftData = s.nftData[tokenId];

        uint256 currentLevel = checkLevel(nftData.staked);

        if (currentLevel == 20) {
            return 0; // Already at max level
        }

        uint256 stakedNeededForNextLevel;
        for (uint256 i; i <= currentLevel; i++) {
            stakedNeededForNextLevel += s.levelStakedNeeded[i];
        }

        amountToLevelUp = stakedNeededForNextLevel - nftData.staked;
    }
    ...
    function getTokenLevel(uint256 tokenId) external view returns (uint256 level) {
        ...
        level = checkLevel(nftData.staked);
    }
    ...
    function getTokenRarity(uint256 tokenId) external view returns (uint256 rarity) {
        ...
        rarity = nftData.nftMultiplier;
    }

    function getLockedTokens() external view returns (uint256[] memory lockedTokens) {
        ONFTStakerStorage.Layout storage $ = ONFTStakerStorage.layout();
    ...
    function getTotalLockedTokens() external view returns (uint256 totalLockedTokens) {
        ...
        totalLockedTokens = $.lockedTokens.length;
    }

    function getLockedData(uint256 tokenId) external view returns (uint256 lockTimestamp, uint256 unlockTimestamp, bool isLocked) {
        ONFTStakerStorage.Layout storage $ = ONFTStakerStorage.layout();
        StakerStorage storage s = $.stakerData;
        ...
        lockTimestamp = lockedData.unlockTimestamp - s.minLockedTime;
        unlockTimestamp = lockedData.unlockTimestamp;
        isLocked = lockedData.isCurrentLocked;
    }
}
