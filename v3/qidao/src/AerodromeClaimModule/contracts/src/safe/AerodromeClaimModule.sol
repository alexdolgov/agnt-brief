...
/// @dev No voting task — handled by vote-poker. Delta-based transfer sends earned tokens to revenue manager.
contract AerodromeClaimModule {
    ...
    /// @notice Address to receive claimed reward tokens
    ...
    /// @dev Unique set of all claimable tokens to delta-transfer to revenue manager
    address[] private _transferTokens;
    ...
    error InvalidRevenueManager();
    error NoTransferTokens();
    ...
    /// @param _revenueManager Address to receive claimed reward tokens
    ...
    /// @param transferTokens_ Deduplicated set of all claimable tokens to delta-transfer
    constructor(
        address _safe,
        address _vault,
        ...
        address[] memory transferTokens_
    ) {
        if (_safe == address(0)) revert InvalidSafe();
        if (_vault == address(0)) revert InvalidVault();
        if (_rewardsDistributor == address(0)) revert InvalidRewardsDistributor();
        if (_revenueManager == address(0)) revert InvalidRevenueManager();
        if (transferTokens_.length == 0) revert NoTransferTokens();
        ...
        // Copy transfer tokens to storage
        for (uint256 i = 0; i < transferTokens_.length; i++) {
            _transferTokens.push(transferTokens_[i]);
        }
    }
    ...
    function _executeTasks() private {
        ...
        _executeTransferDeltas(balancesBefore);
    }
    ...
    function _snapshotBalances() private view returns (uint256[] memory) {
        uint256[] memory balances = new uint256[](_transferTokens.length);
        for (uint256 i = 0; i < _transferTokens.length; i++) {
            balances[i] = IERC20(_transferTokens[i]).balanceOf(safe);
        }
        return balances;
    }

    function _executeTransferDeltas(uint256[] memory balancesBefore) private {
        for (uint256 i = 0; i < _transferTokens.length; i++) {
            uint256 balanceAfter = IERC20(_transferTokens[i]).balanceOf(safe);
            if (balanceAfter <= balancesBefore[i]) continue;

            uint256 earned = balanceAfter - balancesBefore[i];
            bytes memory data = abi.encodeCall(IERC20.transfer, (revenueManager, earned));
            // Task IDs start at 4 (after ClaimRebases=0, ClaimFees=1, ClaimIncentives=2, DepositBribe=3)
            _executeTask(4 + i, "Transfer", _transferTokens[i], data);
        }
    }
    ...
    /// @notice Get the transfer tokens array
    function getTransferTokens() external view returns (address[] memory) {
        return _transferTokens;
    }

    /// @notice Get the number of tasks (4 base + transfer tokens)
    function getTaskCount() external view returns (uint256) {
        return 4 + _transferTokens.length;
    }
