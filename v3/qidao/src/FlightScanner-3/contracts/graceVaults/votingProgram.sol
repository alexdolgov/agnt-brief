...
contract FlightScanner {
     ...
     */
    function getVotingSplit(uint256 tokenId) external view returns (
        uint256 userSplitBps,
        uint256 globalSplitBps,
        uint256 lockEndTime,
        uint256 lockDuration
    ) {
        IVotingEscrow collateral = IVotingEscrow(IGraceVault(address(this)).collateral());
     ...
     * @return lockDuration The remaining lock duration in seconds
     */
    function getVotingSplitByVault(uint256 vaultId) external view returns (
        uint256 userSplitBps,
        uint256 globalSplitBps,
        uint256 lockEndTime,
        uint256 lockDuration
    ) {
        IGraceVault graceVault = IGraceVault(address(this));
        ...
        return this.getVotingSplit(tokenId);
    }
}
