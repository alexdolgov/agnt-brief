...
contract FlightScanner {
    ...
    function poke(uint256 tokenId) external {
        ...
        // Scope for voting parameters and calculations to free stack space
        {
            ...
            IVotingEscrow.LockedBalance memory lockedBalance = contracts.collateral.locked(tokenId);
            
            if (lockedBalance.isPermanent) {
                // Permanent locks get maximum discount (50% user weight)
                discountPercentage = 500;
            } else {
                uint256 lockEndTime = lockedBalance.end;
                ...
                discountPercentage = lockDurationInSeconds > 0 ? (lockDurationInSeconds * 500) / maxLockDuration : 0;
    ...
    function getVotingSplit(address graceVaultAddress, uint256 tokenId) external view returns (
        ...
        IVotingEscrow.LockedBalance memory lockedBalance = collateral.locked(tokenId);
        
        if (lockedBalance.isPermanent) {
            // Permanent locks get maximum user split (50%)
            userSplitBps = 500;
            lockEndTime = 0; // Permanent locks have end = 0
            lockDuration = type(uint256).max; // Indicate permanent lock with max duration
        } else {
            lockEndTime = lockedBalance.end;
