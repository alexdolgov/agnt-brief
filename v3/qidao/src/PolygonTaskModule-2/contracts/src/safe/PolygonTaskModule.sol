...
contract PolygonTaskModule {
    ...
    /// @notice Cooldown duration between collections (1 day)
    uint256 public constant COLLECT_COOLDOWN = 1 days;
    ...
    /// @notice Timestamp of last collection execution
    uint256 public lastCollectTimestamp;
    ...
    event Collected(address indexed caller);
    ...
    error CollectCooldownNotExpired(uint256 remainingTime);
    ...
    /// @notice Collect interest and PSM fees only (no Curve deposit)
    /// @dev Enforces a 1-day cooldown between collections
    /// @dev Does NOT reset lastPokeTimestamp - weekly poke() handles deposits
    function collectOnly() external {
        if (paused) revert ModulePaused();

        // Cooldown check (skip if first collection)
        if (lastCollectTimestamp != 0) {
            uint256 nextAllowedCollect = lastCollectTimestamp + COLLECT_COOLDOWN;
            if (block.timestamp < nextAllowedCollect) {
                revert CollectCooldownNotExpired(nextAllowedCollect - block.timestamp);
            }
        }

        _executeCollectionOnly();

        lastCollectTimestamp = block.timestamp;
        emit Collected(msg.sender);
    }

    /// @notice Internal function to execute collection tasks only (no deposit)
    function _executeCollectionOnly() private {
    ...
    /// @notice Check if collectOnly can be called
    function canCollect() external view returns (bool) {
        if (paused) return false;
        if (lastCollectTimestamp == 0) return true;
        return block.timestamp >= lastCollectTimestamp + COLLECT_COOLDOWN;
    }

    /// @notice Get seconds until next collection is allowed
    function timeUntilNextCollect() external view returns (uint256) {
        if (lastCollectTimestamp == 0) return 0;
        uint256 nextAllowedCollect = lastCollectTimestamp + COLLECT_COOLDOWN;
        if (block.timestamp >= nextAllowedCollect) return 0;
        return nextAllowedCollect - block.timestamp;
    }
