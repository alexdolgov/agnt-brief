...
// File contracts/IPriceSourceAll.sol
...
contract fixedVault is ReentrancyGuard, VaultNFTv4 {
    ...
    modifier frontExists(uint256 vaultID) {
        require(_exists(vaultID) || vaultID == treasury, "not a front end");
        _;
    }
        ...
        returns (uint256, uint256)
    {
        ...
        if (lastInterest[vaultID] != 0 && iR > 0) {
            ...
            //oneYear was removed from here to
            uint256 feeAccrued = (((iR * debt) * timeDelta) / 31556952) / 10000;
            fee = feeAccrued;
    ...
    function calculateFee(
        ...
        if (promoFee < 10000) {
            _fee = ((amount * fee * getTokenPriceSource() * promoFee) /
                (getEthPriceSource() * 1e8));
        } else {
            _fee =
                (amount * fee * getTokenPriceSource()) /
                (getEthPriceSource() * 1e4);
        }
    ...
    onlyVaultOwner(vaultID) 
    {
        ...
        uint256 _openingFee = calculateFee(openingFee, newDebt, _front);
        ...
        vaultExists(vaultID)
    {
        ...
        //TODO: if there is no debt, this will give divide by zero error
        uint256 collateralPercentage = collateralValueTimes100 / (debtValue);
...
// File contracts/stableQiVault.sol
...
contract stableQiVault is fixedVault, Ownable {
    ...
    event UpdateClosingFee(uint256 newFee);
    event UpdateOpeningFee(uint256 newFee);

    event WithdrawInterest(uint256 earned);
    event UpdatedTreasury(uint256 newTreasury);
    ...
    event AddFrontEnd(uint256 promoter);
    event RemoveFrontEnd(uint256 promoter);
    event UpdateFrontEnd(uint256 promoter, uint256 newFee);
    ...
    function setFees(uint256 _admin, uint256 _ref) external onlyOwner {
        require((_admin+_ref)==10000, "setFees: must equal 10000.");
        adminFee=_admin;
        refFee=_ref;
    ...
    function setOpeningFee(uint256 _openingFee) external onlyOwner {
        require(_openingFee > 0 && _openingFee <= 500, "setOpeningFee: cannot be more than 5%");
        openingFee = _openingFee;
        // emit event
        emit UpdateOpeningFee(openingFee);
    }

    function setClosingFee(uint256 _closingFee) external onlyOwner {
        require(_closingFee > 0 && _closingFee <= 500, "setClosingFee: cannot be more than 5%");
        closingFee = _closingFee;
        // emit event
        emit UpdateClosingFee(closingFee);
    }

    function setTreasury(uint256 _treasury) external onlyOwner {
        require(_exists(_treasury), "setTreasury: Vault does not exist");
        treasury = _treasury;
        emit UpdatedTreasury(treasury);
    }

    function addFrontEnd(uint256 _promoter) external onlyOwner {
        require(_exists(_promoter), "addFrontEnd: Vault does not exist");    
        require(promoter[_promoter] == 0, "addFrontEnd: already added");
        promoter[_promoter] = 10000;
        emit AddFrontEnd(_promoter);
    }

    function updateFrontEnd(uint256 _promoter, uint256 cashback) external onlyVaultOwner(_promoter) {
        require(_exists(_promoter), "updateFrontEnd: Vault does not exist");    
        require(promoter[_promoter] != 0, "updateFrontEnd: promoter not added");
        require(cashback > 0 && cashback <= 10000, "updateFrontEnd: cannot be 0");
        promoter[_promoter] = cashback;
        emit UpdateFrontEnd(_promoter, cashback);
    }

    function removeFrontEnd(uint256 _promoter) external onlyOwner {
        ...
        emit RemoveFrontEnd(_promoter);
    }

    function withdrawInterest() public onlyOwner {

        uint256 adm_fee = maiDebt*adminFee / 10000;
        uint256 ref_fee = maiDebt*refFee / 10000;

        require((adm_fee+ref_fee) <= maiDebt, "withdrawInterest: fee withdrawn cannot be bigger than the fee earned.");

        // Transfer
        mai.transfer(ref, ref_fee);
