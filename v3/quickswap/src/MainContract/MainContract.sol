...
interface ISwapContract{
    ...
    function numOfThisBlockchain() external view returns(uint128);
    function blockchainRouter() external view returns(address);
    function RubicAddresses(uint256) external view returns(bytes32);
    function existingOtherBlockchain(uint256) external view returns(bool);
    function feeAmountOfBlockchain(uint256) external view returns(uint256);
    function blockchainCryptoFee(uint256) external view returns(uint256);
    function swapTokensToOtherBlockchain(swapToParams memory params) external payable;
}
