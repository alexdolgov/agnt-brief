...
contract ManagerContract is Storage{
    mapping(address => uint256) internal providerFee;
    mapping(address => uint256) internal amountOfProvider;

    function getProviderFee(address _provider) external view returns(uint){
        return providerFee[_provider];
    }

    function getAmountOfProvider(address _provider) external view returns(uint){
        return amountOfProvider[_provider];
    }

    function setProviderFee(address _provider, uint _fee) external {
        require(hasRole(OWNER_ROLE, msg.sender), 'Manager: not owner');
        require(_fee <= 100000, 'Manager: fee too high');

        providerFee[_provider] = _fee;
    }

    function collectProviderFee() external{
        uint amount = amountOfProvider[msg.sender];
        require(amount > 0, 'Manager: amount is zero');

        amountOfProvider[msg.sender] = 0;
