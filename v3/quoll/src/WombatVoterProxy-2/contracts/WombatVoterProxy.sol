...
contract WombatVoterProxy is IWombatVoterProxy, OwnableUpgradeable {
    ...
    function _getBribeRewardTokens(
        address _bribesContract
    ...
    function withdrawAllByOwner(
        ...
        for (uint256 i = 0; i < _pids.length; i++) {
            ...
            (address token, , , , , , ) = masterWombat.poolInfo(_pid);
            uint256 poolAmount = balanceOfPool(_pid);
            if (poolAmount > 0) {
                masterWombat.withdraw(_pid, poolAmount);
                emit Withdrawn(_pid, poolAmount);
            }
            uint256 balance = IERC20(token).balanceOf(address(this));
    ...
    function withdrawAllV2ByOwner(
        address[] calldata _masterWombats,
        uint256[] calldata _pids,
        address to
    ) external onlyOwner {
        require(_masterWombats.length == _pids.length, "Length mismatch");
        for (uint256 i = 0; i < _pids.length; i++) {
            address _masterWombat = _masterWombats[i];
            uint256 _pid = _pids[i];
            address token = getLpTokenV2(_masterWombat, _pid);

            uint256 amount = balanceOfPoolV2(_masterWombat, _pid);

            IMasterWombatV3(_masterWombat).withdraw(_pid, amount);

            emit WithdrawnV2(_masterWombat, _pid, amount);
    ...
    function withdrawLPsByOwner(address[] calldata lpTokens, address to) external onlyOwner {
        for (uint256 i = 0; i < lpTokens.length; i++) {
            uint256 balance = IERC20(lpTokens[i]).balanceOf((address(this)));
            if (balance > 0) IERC20(lpTokens[i]).safeTransfer(to, balance);
        }
    }
}
