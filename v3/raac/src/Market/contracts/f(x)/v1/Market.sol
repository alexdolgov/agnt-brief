...
contract Market is AccessControlUpgradeable, ReentrancyGuardUpgradeable, IFxMarket {
  ...
  /// @notice Pause or unpause mintBoth in system stability mode.
  /// @param status The new status for mintBoth.
  event PauseMintBothInSystemStabilityMode(bool status);
  ...
  bytes32 public constant EMERGENCY_DAO_ROLE = keccak256("EMERGENCY_DAO_ROLE");
    ...
    /// @notice The role for permissioned mint/redeem operator.
  ...
  /// @notice Whether the mintBoth is paused in system stability mode.
  bool public mintBothInSystemStabilityModePaused;
  ...
  modifier onlyEmergencyDAO() {
    ...
    /// @notice Ensures that the caller has the manager role.
  ...
  ) external override nonReentrant onlyManager returns (uint256 _fTokenMinted, uint256 _xTokenMinted) {
  ...
  function mintFToken(
    ...
    address _recipient,
    uint256 _minFTokenMinted
  ) external override nonReentrant onlyManager returns (uint256 _fTokenMinted) {
    require(!mintPaused, "mint is paused");
    ...
    emit Mint(msg.sender, _recipient, _baseIn, _fTokenMinted, 0, 0);
  }

  /// @inheritdoc IFxMarket
  function mintFTokenWithoutBaseToken(
    ...
    require(_targetCollateralRatio >= marketConfig.stabilityRatio, "target collateral ratio too low");
    ...
    _fTokenMinted = _treasury.mintFTokenWithoutBaseToken(_targetCollateralRatio, _recipient);
    require(_fTokenMinted >= _minFTokenMinted, "insufficient fToken output");

    emit Mint(msg.sender, _recipient, 0, _fTokenMinted, 0, 0);
  }
  ...
  function mintXToken(
    ...
    address _recipient,
    uint256 _minXTokenMinted
  ) external override nonReentrant onlyManager returns (uint256 _xTokenMinted, uint256 _bonus) {
    require(!mintPaused, "mint is paused");
    ...
    require(_xTokenMinted >= _minXTokenMinted, "insufficient xToken output");
// Optional Bonus- onl if reservePool is set
    if (reservePool !=address(0)){
    uint256 _bonusAmount;
      if (_baseIn < _maxBaseInBeforeSystemStabilityMode) {
        _bonusAmount = _baseIn;
      } else {
        _bonusAmount = _maxBaseInBeforeSystemStabilityMode;
      }
      if (_bonusAmount > 0) {
        try IFxReservePool(reservePool).requestBonus(baseToken, _recipient, _bonusAmount) returns (uint256 result) {
          _bonus = result;
        } catch {
          // If bonus request fails, continue without bonus
          _bonus = 0;
        }
      }
    }
    ...
    emit Mint(msg.sender, _recipient, _baseIn, 0, _xTokenMinted, 0);
  }
  ...
  function addBaseToken(
    ...
    address _recipient,
    uint256 _minXTokenMinted
  ) external override nonReentrant onlyManager returns (uint256 _xTokenMinted) {
    require(!mintPaused, "mint is paused");
    ...
    (uint256 _maxBaseInBeforeSystemStabilityMode, ) = _treasury.maxMintableXToken(
      _marketConfig.stabilityRatio
    );
    _maxBaseInBeforeSystemStabilityMode = _treasury.convertToWrapped(_maxBaseInBeforeSystemStabilityMode);

    // bound the amount of base token - use full amount
    if (_baseIn  > _maxBaseInBeforeSystemStabilityMode ) {
      _baseIn = _maxBaseInBeforeSystemStabilityMode;
    }
  ...
  /// @inheritdoc IFxMarket
  function donateBaseToken(uint256 _baseIn) external override nonReentrant onlyManager {
    IFxTreasury _treasury = IFxTreasury(treasury);
    
    IERC20Upgradeable(baseToken).safeTransferFrom(msg.sender, address(_treasury), _baseIn);
    _treasury.donateBaseToken(
      _treasury.convertToUnwrapped(_baseIn)
    );

    emit DonateBaseToken(msg.sender, _baseIn);
  }
  ...
  function redeem(
    ...
    address _recipient,
    uint256 _minBaseOut
  ) external override nonReentrant onlyManager returns (uint256 _baseOut, uint256 _bonus) {
   ...
   // Optional bonus for fToken redemption - only if reservePool is set
    if (_fTokenIn > 0 && reservePool != address(0)) {
      uint256 _bonusAmount;
      if (_baseOut < _maxBaseOut) {
        _bonusAmount = _baseOut;
      } else {
        _bonusAmount = _maxBaseOut;
      }
      // No fee deduction from bonus
      if (_bonusAmount > 0) {
        try IFxReservePool(reservePool).requestBonus(baseToken, _recipient, _bonusAmount) returns (uint256 result) {
          _bonus = result;
        } catch {
    ...
    emit Redeem(msg.sender, _recipient, _fTokenIn, _xTokenIn, _baseOut, 0);
  }
  ...
  function pauseMint(bool _status) external onlyEmergencyDAO {
    ...
    /// @notice Add a new manager for permissioned operations.
  /// @param _manager The address to grant manager role.
  function grantManagerRole(address _manager) external onlyAdmin {
    grantRole(MANAGER_ROLE, _manager);
  }

    /// @notice Remove a manager for permissioned operations.
  /// @param _manager The address to revoke manager role.
  function revokeManagerRole(address _manager) external onlyAdmin {
  ...
  /// @notice Pause mintBoth in system stability mode
  /// @param _status The pause status.
  function pauseMintBothInSystemStabilityMode(bool _status) external onlyEmergencyDAO {
    mintBothInSystemStabilityModePaused = _status;
    emit PauseMintBothInSystemStabilityMode(_status);
  }
