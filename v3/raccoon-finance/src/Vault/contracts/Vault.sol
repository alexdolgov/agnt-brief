...
contract Vault is Ownable, ReentrancyGuard, AntiFlashload {
    ...
    function getMultiplier(uint256 _from, uint256 _to) public view returns (uint256) {
        if (_to <= bonusEndBlock) {
            ...
            return bonusEndBlock.sub(_from).mul(3).add(_to.sub(bonusEndBlock).mul(4));
        } else if (_from <= bonusEndBlock && _to > bonusEndBlock1 && _to <= bonusEndBlock2) {
            return bonusEndBlock.sub(_from).mul(3).add(bonusEndBlock1.sub(bonusEndBlock).mul(4)).add(_to.sub(bonusEndBlock1).mul(2));
        } else if (_from <= bonusEndBlock && _to > bonusEndBlock2) {
            return bonusEndBlock.sub(_from).mul(3).add(bonusEndBlock1.sub(bonusEndBlock).mul(4)).add(bonusEndBlock2.sub(bonusEndBlock1).mul(2)).add(_to.sub(bonusEndBlock2));
        } else if (_from >= bonusEndBlock && _to <= bonusEndBlock1) {
            return _to.sub(_from).mul(4);
        } else if (_from > bonusEndBlock && _to > bonusEndBlock1 && _to <= bonusEndBlock2) {
            return bonusEndBlock1.sub(_from).mul(4).add(_to.sub(bonusEndBlock1).mul(2));
        } else if (_from > bonusEndBlock && _to > bonusEndBlock2) {
            return bonusEndBlock1.sub(_from).mul(4).add(bonusEndBlock2.sub(bonusEndBlock1).mul(2)).add(_to.sub(bonusEndBlock2));
    ...
    function setFlashloadBlk(uint _flashloadBlk)
        public
        onlyOwner
    {
