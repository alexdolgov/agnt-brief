...
contract Leverager is Ownable {
	...
	/// @notice Emitted when lockZap is updated
	...
	/**
	 * @notice Sets lockZap address
	 * @param _lockZap address
	 */
	function setLockZap(ILockZap _lockZap) external onlyOwner {
		if (address(_lockZap) == address(0)) revert AddressZero();
		lockZap = _lockZap;
		emit LockZapUpdated(address(_lockZap));
	}
	...
	*/
	function setLendingPool(ILendingPool _lendingPool) external onlyOwner {
		if (address(_lendingPool) == address(0)) revert AddressZero();
		lendingPool = _lendingPool;
