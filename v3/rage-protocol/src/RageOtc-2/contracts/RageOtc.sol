...
import {IRageTreasury} from "./IRageTreasury.sol";
...
contract RageOtc is ReentrancyGuard {
  ...
  IRageTreasury private constant RAGE_TREASURY = IRageTreasury(0x94e29AF27dc811406e1F5dEd721E42e160AdAE30);
  ...
  function otcProcess(RageStructs.OtcOverview memory overview, IERC20 inputToken) private {
    ...
    // send rage tokens to user via treasury
    require(RAGE_TREASURY.requestTransfer(overview.wallet, overview.outputAmount), "rageTransferFailed");
  ...
  function getRageBalance() internal view returns(uint256) {
    return RAGE_TREASURY.getAvailableFor(address(this));
  }
}
