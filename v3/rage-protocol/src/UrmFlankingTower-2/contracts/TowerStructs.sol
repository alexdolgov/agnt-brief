...
library TowerStructs {
  ...
  struct OtcConfig {
    ...
    uint256 bonusMin;         // minimum amount of bonus in percent (min 1, max 5, default 1)
    ...
    uint32 twap;              // TWAP duration in seconds for oracle price calculations (60-3600)
  }
  ...
  // RagePoolState: internal state tracking for the UrmRagePool (URM/RAGE position). No config struct: UrmRagePool has no setConfigs
  struct RagePoolState {
    bool nftLocked;            // true once the URM/RAGE NFT has been transferred into the pool
    bool closed;               // true once closeTower has wound down the position; permanently disables sync
    address urmRagePoolToken0; // cached token0 of the URM/RAGE pool, for URM vs RAGE direction
    uint256 lastSync;          // timestamp of the last fortressSync that deployed liquidity
    uint256 syncCount;         // number of fortressSync calls that increased liquidity
    uint256 totalUrmIncrease;  // cumulative URM deployed into the NFT position (18 dec)
    uint256 totalRageIncrease; // cumulative RAGE deployed into the NFT position (18 dec)
    uint256 lastCollectFees;   // timestamp of the last fee collection
    uint256 totalUrmFees;      // cumulative URM paid to the Fortress via collectFees (18 dec)
  }

  // RagePoolGetState: complete read-only snapshot of the UrmRagePool returned by getState()
  struct RagePoolGetState {
    uint256 version;          // contract version identifier
    address automator;        // current automator address with config privileges
    uint256 nftId;            // the managed URM/RAGE position id
    uint256 nftUrm;           // URM currently held inside the NFT position (18 dec)
    uint256 nftRage;          // RAGE currently held inside the NFT position (18 dec)
    uint256 urmBalance;       // URM held directly by the pool, not yet deployed (18 dec)
    uint256 rageBalance;      // RAGE held directly by the pool, awaiting deployment (18 dec)
    TowerStructs.RagePoolState state; // current internal state
  }
}
