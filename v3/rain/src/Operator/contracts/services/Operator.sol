...
import { OracleInterface } from "@chainlink/contracts/src/v0.8/operatorforwarder/interfaces/OracleInterface.sol";
...
contract Operator is AuthorizedReceiver, ConfirmedOwner, LinkTokenReceiver, OperatorInterface, IWithdrawal {
  ...
  function setLink(address link) public onlyOwner {
    i_linkToken = LinkTokenInterface(link); // external but already deployed and unalterable
  }
  ...
  function whitslistConsumer(address _consumer) public onlyOwner {
    require(_consumer != address(0), "Cannot whitelist zero address");
    isWhitelisted[_consumer] = true;
  }
  ...
  function distributeFunds(address payable[] calldata receivers, uint256[] calldata amounts) external payable onlyOwner {
  ...
  modifier validateCallbackAddress(address callbackAddress) {
    ...
    require(isWhitelisted[callbackAddress], "callbackAddress is not whitelisted");
    _;
  }
}
