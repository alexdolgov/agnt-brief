...
contract LQDXDistributor is Ownable, ReentrancyGuard {
  using SafeERC20 for IERC20;

  uint256 public coreDecimal = 10000_0000;
  uint256 public baseUnit = 30 days;
  uint256 public TGE = 1712613600; // 11:00 PM, Apr 18th, 2024
  address public LQDX;
  ...
  mapping(string => uint256) public tgePerRole;
  ...
  constructor (address _LQDX, address _relayer) {
    ...
    LQDX = _LQDX;
    relayer = _relayer;
    roles["Zealy"] = Role(6000_0000, 0, 1);       //  60%  0 1
    tgePerRole["Zealy"] = 1712613600;             //  11:00 PM, Apr 18th, 2024
  }
  ...
  function getAvailableTime(string memory role, address user) public view returns(uint256) {
    ...
    if (userRole[role][user].lastIssue == 0) {
      ...
      if (userRole[role][user].lastIssue < cliffTs) return cliffTs + baseUnit;
      uint256 usedPeriod = (userRole[role][user].lastIssue - cliffTs) / baseUnit;
  ...
  function setRole(string memory role, uint256 percent, uint256 cliff, uint256 vesting) public onlyOwner {
    roles[role] = Role(percent, cliff, vesting);
  }

  function setRoleTGE(string memory role, uint256 tge) public onlyOwner {
    tgePerRole[role] = tge;
  }
