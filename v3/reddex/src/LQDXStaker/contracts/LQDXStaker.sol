...
contract LQDXStaker is Ownable, ReentrancyGuard {
  ...
  constructor(address _treasury, address[] memory _stakingTokens, address[] memory _rewardTokens) {
    treasury = _treasury;
    require(_stakingTokens.length == _rewardTokens.length, "Wrong Parameters");
    uint256 len = _stakingTokens.length;
    for (uint256 x = 0; x<len; x++) {
      rewardToken[_stakingTokens[x]] = _rewardTokens[x];
    }
  }
  ...
  function stake(address account, address token, uint256 amount) public payable {
  ...
  function unstake(address account, address token, uint256 amount) public {
    ...
    if (withdrawFee > 0) {
      ...
      if (token != address(0)) {
        ...
        require(success, "LQDXStaker Failed unstake");
      }
    }
  }
  ...
  function claim(address account, address token) public nonReentrant {
    ...
    if (amount > 0) {
      address pmRewardToken = rewardToken[token];
      uint256 balance = 0;
      if (pmRewardToken != address(0)) {
        balance = IERC20(pmRewardToken).balanceOf(address(this));
      }
      else {
        balance = address(this).balance;
      }
      if (token == pmRewardToken) {
      ...
      if (balance >= amount) {
        if (rewardFee > 0) {
          amount = _cutFee(pmRewardToken, rewardFee, amount);
        }
        if (pmRewardToken != address(0)) {
          IERC20(pmRewardToken).safeTransfer(account, amount);
        }
        else {
          (bool success, ) = payable(account).call{value: amount}("");
          require(success, "LQDXStaker Failed claim");
        }
        emit ClaimLiquidXReward(account, pmRewardToken, amount);
        userInfo[account][token].debtReward = 0;
      }
      else {
        if (balance > 0) {
          if (rewardFee > 0) {
            balance = _cutFee(pmRewardToken, rewardFee, balance);
          }
          if (pmRewardToken != address(0)) {
            IERC20(pmRewardToken).safeTransfer(account, balance);
          }
          else {
            (bool success, ) = payable(account).call{value: balance}("");
            require(success, "LQDXStaker Failed claim");
          }
          emit ClaimLiquidXReward(account, pmRewardToken, balance);
        }
  ...
  function setRewardToken(address _stakingToken, address _reward) public onlyOwner() {
    rewardToken[_stakingToken] = _reward;
  }

  function withdrawToken(address token, address target, uint256 amount) public onlyOwner {
    if (token == address(0)) {
      (bool success, ) = payable(target).call{value: amount}("");
      require(success, "LQDXStaker: Failed withdraw");
    }
    else {
    ...
    if (_amount > 0) {
      ...
      if (fee > 0) {
        if (_token == address(0)) {
          ...
          require(success, "LQDXStaker Failed cut fee");
        }
        else {
