...
contract Mutilcall is Ownable {
  ...
  function execute(address[2] memory contracts, bytes[2] memory params, address[] calldata tokens, uint256[] calldata amounts) public payable {
    ...
    for (uint256 x = 0; x < len; x++) {
      ...
      _approveTokenIfNeeded(tokens[x], contracts[0], amounts[x]);
    }
    (bool ok, ) = address(contracts[0]).call{value: msg.value}(params[0]);
    if (!ok) {
      revert DelegatecallFailed();
    }
    (ok, ) = address(contracts[1]).call(params[1]);
    if (!ok) {
      revert DelegatecallFailed();
    }
  }
