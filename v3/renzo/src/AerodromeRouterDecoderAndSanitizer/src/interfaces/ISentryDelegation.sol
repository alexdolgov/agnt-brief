...
interface ISentryDelegation {
    ...
    /// @notice The canonical token whitelist registry pinned at construction.
    /// @dev    Read by affected decoders (Aerodrome, CoW, Morpho Blue) inside
    ///         `decode()` via `ISentryDelegation(caller).tokenWhitelist()`.
    function tokenWhitelist() external view returns (ITokenWhitelistRegistry);
}
