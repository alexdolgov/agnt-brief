    ...
    LEZyVaultStorageV1
{
    ...
    modifier onlyMinter() {
        if (!roleManager.isMinter(_msgSender())) revert NotMinter();
        _;
    }
     ...
     * @param receiver The address that will receive the minted shares
     */
    function mintShares(uint256 _shares, address receiver) external onlyMinter {
        // mint shares to withdraw queue
    ...
    function getPositionInfo() external returns (PositionInfo[] memory positionInfo) {
        positionInfo = new PositionInfo[](allowedDelegateStrategy.length());
        // get underlyingValue calldata
        bytes memory underlyingPayload = _getUnderlyingValueCalldata();
        for (uint256 i = 0; i < allowedDelegateStrategy.length(); ) {
            positionInfo[i] = PositionInfo(
                allowedDelegateStrategy.at(i),
                abi.decode(
    ...
    function _getUnderlyingValueCalldata() internal view returns (bytes memory) {
        return abi.encodeWithSelector(IDelegateStrategy.underlyingValue.selector, asset());
    }
