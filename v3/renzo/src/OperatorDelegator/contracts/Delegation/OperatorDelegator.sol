    ...
    OperatorDelegatorStorageV3
{
    ...
    receive() external payable nonReentrant {
        // If a payment comes in from the delayed withdrawal router,
        // refund the gas spent by the admin (tx.origin)
        uint256 gasRefunded;
        if (
            msg.sender == address(eigenPod.delayedWithdrawalRouter()) &&
            adminGasSpentInWei[tx.origin] > 0
        ) {
            gasRefunded = _refundGas();
        }

        // If no funds left, return
        if (msg.value - gasRefunded == 0) {
            return;
        }

        // Forward remaining balance to the deposit queue
        address destination = address(restakeManager.depositQueue());
        (bool success, ) = destination.call{ value: msg.value - gasRefunded }("");
        if (!success) revert TransferFailed();
