...
abstract contract Transformer is Ownable2Step, Constants {
    ...
    function _validateOwner(IAerodromePositionManager aerodromePositionManager, uint256 tokenId, address vault)
        internal
    {
        ...
        if (vault != address(0)) {
            ...
            owner = aerodromePositionManager.ownerOf(tokenId);
        }
    ...
    function _validateCaller(IAerodromePositionManager aerodromePositionManager, uint256 tokenId) internal view {
        if (vaults[msg.sender]) {
            ...
            address owner = aerodromePositionManager.ownerOf(tokenId);
