...
/// @title v3Utils v1.1
/// @notice Utility functions for Aerodrome V2 positions
...
contract V3Utils is Transformer, Swapper, IERC721Receiver {
    ...
    /// @notice Parameters for returning leftover tokens
        ...
        returns (uint256 tokenId, uint128 liquidity, uint256 added0, uint256 added1)
    {
        ...
        // return leftovers
        LeftoverParams memory lp = LeftoverParams(
            params.recipient,
        ...
        returns (uint128 liquidity, uint256 added0, uint256 added1)
    {
        ...
        IAerodromePositionManager.IncreaseLiquidityParams memory increaseLiquidityParams = IAerodromePositionManager
    ...
    function _decreaseLiquidity(
        ...
        if (liquidity != 0) {
            (amount0, amount1) = nonfungiblePositionManager.decreaseLiquidity(
                IAerodromePositionManager.DecreaseLiquidityParams(
    ...
    // collects specified amount of fees from aerodrome position
        ...
        returns (uint256 amount0, uint256 amount1)
    {
        ...
        (amount0, amount1) = nonfungiblePositionManager.collect(
            IAerodromePositionManager.CollectParams(
    ...
    /// @notice Returns leftover tokens to recipient
    /// @param p Parameters containing recipient, tokens, amounts and unwrap flag
    function _returnLeftoverTokens(LeftoverParams memory p) internal {
        uint256 left0 = p.total0 - p.added0;
        uint256 left1 = p.total1 - p.added1;
        if (left0 != 0) _transferToken(p.recipient, IERC20(p.token0), left0, p.unwrap);
        if (left1 != 0) _transferToken(p.recipient, IERC20(p.token1), left1, p.unwrap);
    }
