...
contract AddressRegistryController is Ownable {
    ...
    function transferRegistryOwnership(address account) external onlyOwner {
        bytes32 DEFAULT_ADMIN_ROLE = 0x0;

        //Grant admin power to new owner which is needed to modify breakers/pausers
        AccessControl(address(registry)).grantRole(DEFAULT_ADMIN_ROLE, account);
        
        //transfer ownership
        Ownable(address(registry)).transferOwnership(account);

        //Renounce admin powers
        AccessControl(address(registry)).renounceRole(DEFAULT_ADMIN_ROLE, address(this));
    }
