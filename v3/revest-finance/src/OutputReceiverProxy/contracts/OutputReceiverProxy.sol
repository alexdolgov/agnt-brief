...
contract OutputReceiverProxy is Ownable, IOutputReceiverV3, ERC165 {
     ...
     */
    function receiveRevestOutput(uint256 fnftId, address, address payable recipient, uint256 quantity)
        external
        override
    {
     ...
     */
    function triggerOutputReceiverUpdate(uint256 fnftId, bytes memory) external override {
        require(FNFT_HANDLER.getBalance(msg.sender, fnftId) > 0, "ER010");
        IResonate(resonate).claimInterest(fnftId, msg.sender);
    }

    function handleFNFTRemaps(uint256 fnftId, uint256[] memory newFNFTIds, address caller, bool cleanup) external {}
    function handleTimelockExtensions(uint256 fnftId, uint256 expiration, address caller) external override {}
    ...
    function getValue(uint256 fnftId) external view override returns (uint256) {
        ...
        uint256 index = resonateContract.fnftIdToIndex(fnftId);
        (uint256 principalId,,, bytes32 poolId) = resonateContract.activated(index);
        if (fnftId == principalId) {
            (,,,,,, uint256 packetSize) = resonateContract.pools(poolId);
            return packetSize;
        } else {
            (uint256 accruedInterest,) = IResonateHelper(resonateContract.RESONATE_HELPER()).calculateInterest(fnftId);
            return accruedInterest;
        }
    }

    function getAsset(uint256 fnftId) external view override returns (address asset) {
        IResonate resonateContract = IResonate(resonate);
        uint256 index = resonateContract.fnftIdToIndex(fnftId);
    ...
    function getOutputDisplayValues(uint256 fnftId) external view override returns (bytes memory output) {
