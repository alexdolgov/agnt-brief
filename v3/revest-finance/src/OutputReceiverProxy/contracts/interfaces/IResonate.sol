...
interface IResonate {
    ...
    event EnqueueProvider(
        bytes32 indexed poolId, address indexed addr, uint64 indexed position, bool shouldFarm, Order order
    );
    event EnqueueConsumer(
        bytes32 indexed poolId, address indexed addr, uint64 indexed position, bool shouldFarm, Order order
    );

    event DequeueProvider(
        bytes32 indexed poolId, address indexed dequeuer, address indexed owner, uint64 position, Order order
    );
    event DequeueConsumer(
        bytes32 indexed poolId, address indexed dequeuer, address indexed owner, uint64 position, Order order
    );

    event OracleRegistered(address indexed vaultAsset, address indexed paymentAsset, address indexed oracleDispatch);

    event VaultAdapterRegistered(
        address indexed underlyingVault, address indexed vaultAdapter, address indexed vaultAsset
    );
    ...
    event BatchInterestClaimed(
        bytes32 indexed poolId, uint256[] fnftIds, address indexed claimer, uint256 amountInterest
    );
    ...
    function residuals(uint256 fnftId) external view returns (uint256 residual);
    ...
    function providerQueue(bytes32 poolId, uint256 providerHead)
        external
        view
        returns (uint256 packetsRemaining, uint256 depositedShares, bytes32 owner);
    function consumerQueue(bytes32 poolId, uint256 consumerHead)
        external
        view
        returns (uint256 packetsRemaining, uint256 depositedShares, bytes32 owner);
    function activated(uint256 fnftId)
        external
        view
        returns (uint256 principalId, uint256 sharesPerPacket, uint256 startingSharesPerPacket, bytes32 poolId);
    ...
    function fnftIdToIndex(uint256 fnftId) external view returns (uint256 index);
    function REGISTRY_ADDRESS() external view returns (address registry);

    function receiveRevestOutput(uint256 fnftId, address, address payable owner, uint256 quantity) external;

    function claimInterest(uint256 fnftId, address recipient) external;
}
