...
contract PriceProvider is IPriceProvider, Ownable {
    ...
    function getValueOfAsset(address token, address quote) external view override returns (uint256 safePrice) {
        ...
        uint256 tokenPriceToEth = IPriceOracle(tokenOracle).getSafePrice(token);
        uint256 quotePriceToEth = IPriceOracle(quoteOracle).getSafePrice(quote);
        // Prices should always be in 1E18 precision
        safePrice = PRECISION * tokenPriceToEth / quotePriceToEth;

        uint256 tokenDecimals = IERC20Detailed(token).decimals();
        uint256 quoteDecimals = IERC20Detailed(quote).decimals();
