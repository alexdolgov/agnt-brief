...
contract Resonate is IResonate, ReentrancyGuard {
    ...
    uint256 private constant DENOM = 100;
    ...
    mapping(uint256 => uint256) public fnftIdToIndex;

    /// Contains all activated orders
    mapping(uint256 => Active) public override activated;

    /// Mapping to residual interest for an interest-bearing FNFT
    mapping(uint256 => uint256) public override residuals;

    /// Map poolIds to their respective configs
    mapping(bytes32 => PoolConfig) public override pools;

    /// Provider queue
    mapping(bytes32 => mapping(uint256 => Order)) public override providerQueue;

    /// Consumer queue
    mapping(bytes32 => mapping(uint256 => Order)) public override consumerQueue;
    ...
    constructor(
        ...
        require(
            _router != address(0) && _proxyOutputReceiver != address(0) && _proxyAddressLock != address(0)
                && _resonateHelper != address(0) && _smartWalletWhitelist != address(0) && _priceProvider != address(0)
    ...
    function createPool(
        ...
        address vaultAsset;
        {
            ...
            require(
                (lockupPeriod >= MIN_LOCKUP && additionalRate == 0) || (additionalRate > 0 && lockupPeriod == 0),
                "ER008"
            );
        ...
        emit PoolCreated(
            ...
            lockupPeriod,
            packetSize,
            lockupPeriod > 0 && additionalRate == 0,
            poolName,
            msg.sender
        );
    }
     ...
     */
    function submitConsumer(bytes32 poolId, uint256 amount, bool shouldFarm) external nonReentrant {
        ...
        if (hasCounterparty) {
            ...
            uint256 currentExchange =
                pool.asset != vaultAsset ? PRICE_PROVIDER.getValueOfAsset(vaultAsset, pool.asset) : 0;
            ...
            while (hasCounterparty && consumerOrder.packetsRemaining > 0) {
                ...
                if (pool.asset != vaultAsset) {
                    uint256 previousExchange = producerOrder.depositedShares;
                    if (currentExchange != previousExchange) {
                        // This will almost always be true
                        uint256 maxPacketNumber = (producerOrder.packetsRemaining * previousExchange) / currentExchange; // 5
                        uint256 amountToRefund;
                        if (consumerOrder.packetsRemaining >= maxPacketNumber) {
                            if (currentExchange > previousExchange) {
                                // Position is partially or fully insolvent
                                amountToRefund = (
                                    ((pool.rate * pool.packetSize) / PRECISION)
                                        * (
                        ...
                        if (maxPacketNumber == 0) {
                            ...
                            amountToRefund = (
                                ((pool.rate * pool.packetSize * producerOrder.packetsRemaining) / PRECISION)
                                    * previousExchange
                            ) / PRECISION;
                ...
                if (producerOrder.owner.toAddress() == address(0)) {
                    ...
                    uint256 digestAmt;
                    {
                        uint256 consumerAmt = consumerOrder.packetsRemaining;
                        uint256 producerAmt = producerOrder.packetsRemaining;
     ...
     */
    function submitProducer(bytes32 poolId, uint256 amount, bool shouldFarm) external nonReentrant {
        PoolConfig memory pool = pools[poolId];
        require(amount > MIN_DEPOSIT, "ER043");
        require(_validRecipient(), "ER034");
     ...
     */
    function modifyExistingOrder(bytes32 poolId, uint112 amount, uint64 position, bool isProvider)
        external
        nonReentrant
    {
        ...
        uint256 amountTokens =
            isProvider ? (amount * pool.packetSize * pool.rate) / PRECISION : amount * pool.packetSize;
        ...
        if (order.depositedShares > 0 && (!isProvider || !isCrossAsset)) {
            ...
            uint256 tokensReceived = _getWalletForPool(poolId).withdrawFromVault(
            ...
            if (isCrossAsset && isProvider) {
                ...
                uint256 producerPacket =
                    (((pool.rate * pool.packetSize) / PRECISION) * order.depositedShares) / PRECISION;
     ...
     */
    function batchClaimInterest(uint256[][] memory fnftIds, address recipient) public nonReentrant {
        // Outer array is an array of all FNFTs segregated by pool
        // Inner array is array of FNFTs to claim interest on
        uint256 numberPools = fnftIds.length;
        require(numberPools > 0, "ER003");

        // for each pool
        for (uint256 i; i < numberPools; ++i) {
            // save the list of ids for the pool
            uint256[] memory fnftsByPool = fnftIds[i];
            ...
            uint256 len = fnftsByPool.length;
            for (uint256 j; j < len; ++j) {
                {
                    Active memory active = activated[fnftIdToIndex[fnftsByPool[j]]];
                    require(active.poolId == poolId, "ER039");
                    // save the individual id
                    uint256 fnftId = fnftsByPool[j];
                    require(
                        msg.sender == PROXY_OUTPUT_RECEIVER || FNFT_HANDLER.getBalance(msg.sender, fnftId) > 0, "ER010"
                    );
                    require(fnftId == active.principalId + 1, "ER009");
                    uint256 prinPackets = FNFT_HANDLER.getSupply(active.principalId);
                    require(prinPackets > 0, "ER016");
                    uint256 oldShares = active.sharesPerPacket * prinPackets;
                    uint256 newShares = vault.previewWithdraw(pool.packetSize * prinPackets) * PRECISION;
                    require(oldShares > newShares, "ER040"); // Shouldn't pass FNFTs into this method that aren't owed interest
                    {
                        ...
                        uint256 sharesRedeemed = oldShares - newShares;
            ...
            uint256 interest = _getWalletForFNFT(poolId).redeemShares(
                pool.adapter,
                address(this),
                totalSharesToRedeem / PRECISION // recover extra precision
            );
            uint256 fee = (interest * FEE) / DENOM;
     ...
     */
    function claimInterest(uint256 fnftId, address recipient) external override {
        uint256[][] memory fnftIds = new uint256[][](1);
        uint256[] memory fnftIDi = new uint256[](1);
        fnftIDi[0] = fnftId;
        fnftIds[0] = fnftIDi;
     ...
     */
    function receiveRevestOutput(uint256 fnftId, address, address payable tokenHolder, uint256 quantity)
        external
        override
        nonReentrant
    {
        ...
        if (fnftId == active.principalId) {
            ...
            uint256 amountUnderlying = quantity * pool.packetSize;
            ...
            uint256 residual = _getWalletForFNFT(active.poolId).reclaimPrincipal(
                vaultAdapter,
                tokenHolder,
                amountUnderlying,
            ...
            emit WithdrawERC20OutputReceiver(
                tokenHolder, IERC4626(vaultAdapter).asset(), amountUnderlying, fnftId, "0x0"
            );
        } else {
            ...
            uint256 claimPerPacket;
            uint256 interest;

            {
                uint256 residual = residuals[fnftId];
                if (residual > 0) {
                    residuals[fnftId] = 0;
                }
                uint256 amountUnderlying = prinPackets * pool.packetSize;

                uint256 totalShares = (active.sharesPerPacket * prinPackets) / PRECISION; // Recover our extra PRECISION here

                // NB: Violation of checks-effects-interaction. Acceptable with nonReentrant
                (interest, claimPerPacket) = _getWalletForFNFT(active.poolId).reclaimInterestAndResidual(
                    pool.adapter, address(this), amountUnderlying, totalShares, residual
                );
            }
        ...
        // Clean up mappings
        uint256 index = fnftIdToIndex[active.principalId];
        if (prinPackets == 0) {
     ...
     */
    function _activateCapital(ParamPacker memory packer) private returns (uint256 principalId) {
        ...
        uint256 amountFromConsumer = packer.quantityPackets * packer.pool.packetSize;
        uint256 amountToConsumer = packer.isCrossAsset
            ? (((amountFromConsumer * packer.pool.rate) / PRECISION) * packer.currentExchangeRate) / PRECISION
        ...
        if (packer.isProducerNew) {
            {
                ...
                uint256 fee = (amountToConsumer * FEE) / DENOM;
                ...
                principalId = _createFNFTs(
                    packer.quantityPackets, packer.poolId, consumerOwner, packer.producerOrder.owner.toAddress()
                );
            }
            ...
            // Order was previously farming
            {
                ...
                if (packer.consumerOrder.depositedShares > 0) {
                    ...
                    (uint256 depositedShares, uint256 interest) = IPoolWallet(_getAddressForPool(packer.poolId))
                    ...
                    shares = IPoolWallet(_getAddressForPool(packer.poolId)).depositAndTransfer(
                        amountFromConsumer, packer.adapter, _getAddressForFNFT(packer.poolId)
                    );
                }
            ...
            // Prepare the desired FNFTs
            principalId = _createFNFTs(
                packer.quantityPackets, packer.poolId, packer.consumerOrder.owner.toAddress(), producerOwner
            );
            {
                Active storage active = activated[principalId];
                // We add an extra PRECISION to this to avoid losing data
                uint256 shares = (vault.deposit(amountFromConsumer, _getAddressForFNFT(packer.poolId)) * PRECISION)
            ...
            if (packer.producerOrder.depositedShares > 0 && !packer.isCrossAsset) {
                uint256 interest = IPoolWallet(_getAddressForPool(packer.poolId)).activateExistingProducerPosition(
                    amountToConsumer,
                ...
                uint256 fee = (amountToConsumer * FEE) / DENOM;
                IPoolWallet(_getAddressForPool(packer.poolId)).withdraw(
                    amountToConsumer, fee, packer.pool.asset, msg.sender, DEV_ADDRESS
                );
            }

            // Included to comply with IOutputReceiverV3 standard
            emit DepositERC20OutputReceiver(
                packer.consumerOrder.owner.toAddress(), vaultAsset, amountFromConsumer, principalId, "0x0"
            );
        }
     ...
     */
    function _createFNFTs(uint256 quantityPackets, bytes32 poolId, address consumerOwner, address producerOwner)
        private
        returns (uint256 principalId)
    {
        PoolConfig memory pool = pools[poolId];
        ...
        if (pool.addInterestRate != 0) {
            ...
            // Mint Type 0
            principalId =
                _getRevest().mintTimeLock(block.timestamp + pool.lockupPeriod, recipients, quantities, fnftConfig);
        }
    ...
    function _enqueue(
        ...
        if (shouldFarm) {
            ...
            order.depositedShares = (IERC4626(vaultAdapter).deposit(amount, _getAddressForPool(poolId)) * PRECISION)
