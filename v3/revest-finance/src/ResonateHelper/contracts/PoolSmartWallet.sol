...
contract PoolSmartWallet is IPoolWallet, ReentrancyGuard {
     ...
     */
    function withdraw(uint256 value, uint256 fee, address token, address recipient, address devWallet)
        external
        override
        onlyMaster
        nonReentrant
    ...
    function withdrawFromVault(uint256 shares, address receiver, address vault)
        external
        override
        onlyMaster
        nonReentrant
    ...
    ) external override onlyMaster nonReentrant returns (uint256 shares, uint256 interest) {
        IERC4626 vault = IERC4626(vaultAdapter);
        uint256 totalValueOfShares = vault.previewRedeem(totalShares);
    ...
    function activateExistingProducerPosition(
        ...
        address devWallet,
        address vaultAdapter
    ) external override onlyMaster nonReentrant returns (uint256 interest) {
        IERC4626 vault = IERC4626(vaultAdapter);
        IERC20 asset = IERC20(vault.asset());

        // Edge-case handling for withdrawal fees
        uint256 assets = vault.redeem(totalShares, address(this), address(this));
    ...
    function _checkApproval(uint256 amountUnderlying, address token, address vaultAdapter) private {
        if (IERC20(token).allowance(address(this), vaultAdapter) < amountUnderlying) {
            IERC20(token).safeApprove(vaultAdapter, 0);
            IERC20(token).safeApprove(vaultAdapter, type(uint256).max);
        }
    }
}
