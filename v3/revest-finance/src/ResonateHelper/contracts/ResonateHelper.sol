...
contract ResonateHelper is IResonateHelper, AccessControl {
     ...
     */
    function sandwichSnapshot(bytes32 poolId, uint256 amount, bool isWithdrawal)
        external
        override
        onlySandwichBot
        glassUnbroken
     ...
     */
    function proxyCall(bytes32 poolId, address[] memory targets, uint256[] memory values, bytes[] memory calldatas)
        external
        onlySandwichBot
        glassUnbroken
    {
     ...
     */
    function calculateInterest(uint256 fnftId)
        external
        view
        override
        returns (uint256 interest, uint256 interestAfterFee)
    {
        IResonate resonate = IResonate(owner);
        uint256 index = resonate.fnftIdToIndex(fnftId);

        (uint256 principalId, uint256 sharesPerPacket,, bytes32 poolId) = resonate.activated(index);
        (,, address vaultAdapter,,,, uint256 packetSize) = resonate.pools(poolId);
        IERC4626 vault = IERC4626(vaultAdapter);
        uint256 prinPackets =
            IFNFTHandler(IAddressRegistry(resonate.REGISTRY_ADDRESS()).getRevestFNFT()).getSupply(principalId);
        {
            uint256 assetShares = vault.previewWithdraw(prinPackets * packetSize); // shares for current principal
            uint256 totalShares = sharesPerPacket * prinPackets / PRECISION; // Shares you own, with recovered PRECISION
    ...
    function nextInQueue(bytes32 poolId, bool isProvider)
        external
        view
        override
        returns (IResonate.Order memory order)
    {
        IResonate resonate = IResonate(owner);
        ...
        if (isProvider) {
            (uint256 a, uint256 b, bytes32 c) = resonate.providerQueue(poolId, queue.providerHead);
            order = IResonate.Order(a, b, c);
        } else {
            (uint256 a, uint256 b, bytes32 c) = resonate.consumerQueue(poolId, queue.consumerHead);
            order = IResonate.Order(a, b, c);
        }
    }
