...
contract ResonateSmartWallet is ISmartWallet, ReentrancyGuard {
    ...
    function reclaimInterestAndResidual(
        ...
        uint256 totalShares,
        uint256 residual
    ) external override onlyMaster nonReentrant returns (uint256 interest, uint256 sharesRedeemed) {
        uint256 shareSum = totalShares + residual;
        IERC4626 vault = IERC4626(vaultAdapter);

        uint256 underlyingShares = vault.previewWithdraw(amountUnderlying);
    ...
    function reclaimPrincipal(
        ...
        uint256 totalShares,
        bool leaveResidual
    ) external onlyMaster nonReentrant returns (uint256 residualShares) {
        IERC4626 vault = IERC4626(vaultAdapter);
        if (leaveResidual) {
    ...
    function proxyCall(
        ...
        uint256 preBalVaultToken = IERC20(vaultAdapter).balanceOf(address(this));
        uint256 preBalToken = IERC20(token).balanceOf(address(this));
        uint256 preVaultBalToken = IERC20(token).balanceOf(vault);
        uint256 prePreviewRedeem = IERC4626(vaultAdapter).previewRedeem(PRECISION);
     ...
     */
    function withdrawOrDeposit(address vaultAdapter, uint256 amount, bool isWithdrawal)
        external
        override
        onlyMaster
        nonReentrant
