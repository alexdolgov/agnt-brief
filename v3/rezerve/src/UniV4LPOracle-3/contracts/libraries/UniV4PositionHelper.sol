...
contract UniV4PositionHelper {
        ...
        returns (uint256 amount0, uint256 amount1, address token0, address token1)
    {
        ...
        if (sqrtPriceX96 <= sqrtPriceLowerX96) {
            ...
            amount0 = uint256(liquidity) * (sqrtPriceUpperX96 - sqrtPriceLowerX96) / sqrtPriceUpperX96;
            amount1 = 0;
        } else if (sqrtPriceX96 >= sqrtPriceUpperX96) {
            // Current price is above the range, only token1
            amount0 = 0;
            amount1 = uint256(liquidity) * (sqrtPriceUpperX96 - sqrtPriceLowerX96) / FixedPoint96.Q96;
        } else {
            // Current price is within the range
            amount0 = uint256(liquidity) * (sqrtPriceUpperX96 - sqrtPriceX96) / sqrtPriceUpperX96;
            amount1 = uint256(liquidity) * (sqrtPriceX96 - sqrtPriceLowerX96) / FixedPoint96.Q96;
        }
    }
}
