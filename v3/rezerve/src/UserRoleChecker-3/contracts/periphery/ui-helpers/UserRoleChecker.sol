...
contract UserRoleChecker {
     ...
     */
    function checkIfHolding(address user, IERC20 token, address[] memory forwarders) public view returns (bool isHolding, uint256 totalUsdValue, uint256 rzrValue) {
        (,uint256 spotPrice,) = spotOracle.getPriceForAmount(1e18);
        uint256 balance = token.balanceOf(user);

        for (uint256 j = 0; j < forwarders.length; j++) {
            balance += token.balanceOf(forwarders[j]);
        }

        (uint256 rzrAmount, uint256 usdAmount,) = spotOracle.getPriceForAmount(balance);
        totalUsdValue += usdAmount + (rzrAmount * spotPrice);
        rzrValue += rzrAmount;
     ...
     * @return rzrValue The total RZR value of the tokens
     */
    function checkIfHoldingMultiple(address user, IERC20[] memory tokens, address[][] memory forwarders) public view returns (bool isHolding, uint256 totalUsdValue, uint256 rzrValue) {
