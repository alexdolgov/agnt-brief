    ...
    IFlashLoanLoopingFactory
{
    ...
    function estimateCollateralRatio(
        ...
        uint256 sellCreditMarketAmount = estimateSellCreditMarketAmount(size, user, targetLeveragePercent);
        ...
        uint256 initialCollateralInCash =
            collateralTokenAmountToDebtTokenAmount(size, size.data().collateralToken.balanceOf(user));
        uint256 initialDebt = size.data().debtToken.balanceOf(user);
