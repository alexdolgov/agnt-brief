    ...
    IFlashLoanLoopingFactory
{
    ...
    /// @param sellCreditMarketParamsArray Parameters for the sell credit market operation
    ...
    function expectedTargetLeveragePercentForLoop(
        ...
        uint256 slippagePercent,
        SellCreditMarketParams[] calldata sellCreditMarketParamsArray
    ) public view returns (uint256) {
        if (!sizeFactory().isMarket(address(size))) {
            revert Errors.INVALID_MARKET(address(size));
        }
        ...
        uint256 totalCreditAmountIn = 0;
        for (uint256 i = 0; i < sellCreditMarketParamsArray.length; i++) {
            totalCreditAmountIn += size.getSellCreditMarketSwapData(sellCreditMarketParamsArray[i]).creditAmountIn;
        }

        // Calculate total expected debt in collateral terms (D)
        // D = current debt + credit amount from selling on Size
        uint256 D = size.debtTokenAmountToCollateralTokenAmount(debtBalance + totalCreditAmountIn);
