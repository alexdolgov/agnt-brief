    ...
    IFlashLoanLoopingFactory
{
    ...
    function computeAddress(address onBehalfOf, uint256 expiresAt, bytes32 extraSalt) external view returns (address) {
        bytes32 salt = _salt(onBehalfOf, expiresAt, extraSalt);
    ...
    function _salt(address onBehalfOf, uint256 expiresAt, bytes32 extraSalt) private pure returns (bytes32) {
        return keccak256(abi.encode(onBehalfOf, expiresAt, extraSalt));
    }
