...
abstract contract DexSwapUpgradeable is Initializable {
    ...
    function _swap(SwapParams[] memory swapParamsArray) internal {
        for (uint256 i = 0; i < swapParamsArray.length; i++) {
            ...
            if (swapParams.method == SwapMethod.UniswapV2) {
                _swapUniswapV2(abi.decode(swapParams.data, (UniswapV2Params)));
            } else if (swapParams.method == SwapMethod.UniswapV3) {
                _swapUniswapV3(abi.decode(swapParams.data, (UniswapV3Params)));
    ...
    function _swapUniswapV2(UniswapV2Params memory params) private {
    ...
    function _swapUniswapV3(UniswapV3Params memory params) private {
        uint256 amountIn = IERC20(params.tokenIn).balanceOf(address(this));
