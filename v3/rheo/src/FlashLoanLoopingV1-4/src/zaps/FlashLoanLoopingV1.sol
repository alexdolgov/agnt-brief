...
string constant FLASH_LOAN_LOOPING_VERSION = "v1.2";
...
contract FlashLoanLoopingV1 is MulticallUpgradeable, MorphoFlashLoanReceiverBaseUpgradeable, OwnableUpgradeable {
    ...
    /// @dev If `swapTarget` is null, no swap is performed, and expect `tokenIn` to be the `collateral token`
    function zapIn(ZapInParams memory zapInParams) external onlyOwner factoryNotPaused {
    ...
    /// @notice Flash loans borrow tokens, swaps to collateral token, deposits collateral token into `sizeMarket`, sells credit market, withdraws borrow tokens, and repays the flash loan
    /// @dev Users must first `setAuthorization` with `SELL_CREDIT_MARKET` to this contract
    function loop(LoopParamsV1 memory loopParams) external onlyOwner factoryNotPaused {
