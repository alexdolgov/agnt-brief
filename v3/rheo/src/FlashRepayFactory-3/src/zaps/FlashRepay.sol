    ...
    OwnableUpgradeable
{
    ...
    function repayWithCollateral(ISize market, uint256 debtPositionId, SwapParams[] calldata swapParamsArray)
        external
        onlyOwner
    {
    ...
    function _flashLoanCallback(address, uint256, bytes memory params) internal override {
        ...
        uint256 collateralAmount = _withdrawableCollateralAmount(operationParams.market, owner());
    ...
    function _withdrawableCollateralAmount(ISize market, address user) internal view returns (uint256) {
        ...
        uint256 withdrawableDebtAmountInCollateralAmount =
            _withdrawableDebtAmountInCollateralAmount(market, debtBalance);
        if (debtBalance == 0) {
    ...
    function _withdrawableDebtAmountInCollateralAmount(ISize market, uint256 debtTokenAmount)
        internal
        view
        returns (uint256 collateralTokenAmount)
    {
        IPriceFeed priceFeed = IPriceFeed(market.oracle().priceFeed);
        collateralTokenAmount = Math.mulDivDown(
            debtTokenAmount * 10 ** priceFeed.decimals(),
