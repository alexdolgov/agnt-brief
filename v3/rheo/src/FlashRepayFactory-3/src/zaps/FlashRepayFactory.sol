...
import {FlashRepay} from "src/zaps/FlashRepay.sol";
    ...
    IFlashRepayFactory
{
    ...
    struct FlashRepayFactoryStorage {
        ...
        FlashRepay _flashRepayImplementation;
        IMorpho _morpho;
    ...
    function deploy(address onBehalfOf, bytes32 extraSalt) public whenNotPaused returns (FlashRepay flashRepay) {
        bytes32 salt = _salt(onBehalfOf, extraSalt);
        flashRepay = FlashRepay(Clones.cloneDeterministic(address(flashRepayImplementation()), salt, 0));
        flashRepay.initialize(this, onBehalfOf);
    }

    function deploy(bytes32 extraSalt) external /*whenNotPaused*/ returns (FlashRepay flashRepay) {
    ...
    function flashRepayImplementation() public view override returns (FlashRepay) {
        FlashRepayFactoryStorage storage $ = _getFlashRepayFactoryStorage();
