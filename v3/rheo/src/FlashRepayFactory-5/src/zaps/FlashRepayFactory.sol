    ...
    IFlashRepayFactory
{
    ...
    function withdrawableCollateralAmount(ISize market, address user) public view returns (uint256) {
        return withdrawableCollateralAmount(market, user, 0);
    }

    function withdrawableCollateralAmount(ISize market, address user, uint256 reducedDebt)
        public
        view
        returns (uint256)
    {
        ...
        UserView memory userView = market.getUserView(user);
        uint256 cr = market.collateralRatio(user);
        uint256 crOpening = market.riskConfig().crOpening;
        uint256 debtBalance = MathUtils.saturatingSub(userView.debtBalance, reducedDebt);
        ...
        if (debtBalance == 0) {
            return userView.collateralTokenBalance;
        } else {
