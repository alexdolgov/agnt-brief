...
interface IFlashRepayFactory {
    function sizeFactory() external view returns (ISizeFactory);
    function flashRepayImplementation() external view returns (IFlashRepay);
    ...
    function withdrawableCollateralAmount(ISize market, address user) external view returns (uint256);
}
