...
contract MixedRouteQuoterV1 is IMixedRouteQuoterV1, IUniswapV3SwapCallback, PeripheryImmutableState {
    ...
    address public immutable ringFactoryV2;
    ...
    // ring mask:  1 << 22 = 010000000000000000000000 = decimal value 4194304
    uint24 private constant flagBitmask = 8388608;
    uint24 private constant flagRingBitmask = 4194304;
    ...
    constructor(
        address _factory,
        address _factoryV2,
        address _ringFactoryV2,
        address _WETH9
    ) PeripheryImmutableState(_factory, _WETH9) {
        factoryV2 = _factoryV2;
        ringFactoryV2 = _ringFactoryV2;
    }
    ...
    /// @dev Given an amountIn, fetch the reserves of the Ring V2 pair and get the amountOut
    function getRingPairAmountOut(
        uint256 amountIn,
        address tokenIn,
        address tokenOut
    ) private view returns (uint256) {
        (uint256 reserveIn, uint256 reserveOut) = RingV2Library.getReserves(ringFactoryV2, tokenIn, tokenOut);
        return RingV2Library.getAmountOut(amountIn, reserveIn, reserveOut);
    }
    ...
    /// @dev Fetch an exactIn quote for a Ring V2 pair on chain
    function quoteExactInputSingleRingV2(QuoteExactInputSingleV2Params memory params)
        public
        view
        override
        returns (uint256 amountOut)
    {
        amountOut = getRingPairAmountOut(params.amountIn, params.tokenIn, params.tokenOut);
    }
        ...
        )
    {
        ...
        while (true) {
            ...
            } else if (fee & flagRingBitmask != 0) {
                amountIn = quoteExactInputSingleRingV2(
                    QuoteExactInputSingleV2Params({tokenIn: tokenIn, tokenOut: tokenOut, amountIn: amountIn})
                );
            } else {
