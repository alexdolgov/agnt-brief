...
import {SwapIncentive} from './SwapIncentive.sol';
...
abstract contract V2SwapRouter is IV2SwapRouter, UniswapImmutables, Permit2Payments {
    ...
    function setPermittedAccount(address permittedAccount, bool enabled) external override {
        if (msg.sender != IUniswapV2Factory(UNISWAP_V2_FACTORY).feeToSetter()) revert V2Forbidden();

        getPermittedAccount[permittedAccount] = enabled;
    }

    function setIncentiveContract(address incentive) external override {
        if (!getPermittedAccount[msg.sender]) revert V2Forbidden();
        ...
        unchecked {
            ...
            for (uint256 i; i < finalPairIndex; i++) {
                ...
                if (incentiveContract != address(0)) {
                    IIncentive(incentiveContract).incentivize(input, amountInput, output, amountOutput);
                }
    ...
    function fewV2SwapExactInput(
        ...
        if (
            ...
            if (amountIn > 0) {
                ERC20(token).safeApprove(path[0], amountIn);
