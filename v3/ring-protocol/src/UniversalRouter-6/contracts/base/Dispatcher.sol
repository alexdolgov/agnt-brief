...
import {FewV2SwapRouter} from '../modules/uniswap/fewV2/V2SwapRouter.sol';
...
abstract contract Dispatcher is Payments, V2SwapRouter, FewV2SwapRouter,V3SwapRouter, V4SwapRouter, V3ToV4Migrator, Lock {
    ...
    function dispatch(bytes1 commandType, bytes calldata inputs) internal returns (bool success, bytes memory output) {
        ...
        if (command < Commands.EXECUTE_SUB_PLAN) {
            ...
            if (command == Commands.EXECUTE_SUB_PLAN) {
                ...
                address[] calldata path = inputs.toAddressArray(3);
                address payer = payerIsUser ? msgSender() : address(this);
                fewV2SwapExactInput(map(recipient), amountIn, amountOutMin, path, payer);
                ...
                address[] calldata path = inputs.toAddressArray(3);
                address payer = payerIsUser ? msgSender() : address(this);
                fewV2SwapExactOutput(map(recipient), amountOut, amountInMax, path, payer);
