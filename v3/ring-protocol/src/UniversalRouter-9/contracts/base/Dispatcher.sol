...
import {RingSwapRouter} from '../modules/uniswap/ring/RingSwapRouter.sol';
...
abstract contract Dispatcher is NFTImmutables, Payments, V2SwapRouter, V3SwapRouter, RingSwapRouter, Callbacks, LockAndMsgSender {
    ...
    function dispatch(bytes1 commandType, bytes calldata inputs) internal returns (bool success, bytes memory output) {
        ...
        if (command < Commands.FOURTH_IF_BOUNDARY) {
            ...
            if (command == Commands.SEAPORT_V1_4) {
                ...
                address[] calldata path = inputs.toAddressArray(3);
                address payer = payerIsUser ? lockedBy : address(this);
                fewV2SwapExactInput(map(recipient), amountIn, amountOutMin, path, payer, isOriginalOutput);
                ...
                address[] calldata path = inputs.toAddressArray(3);
                address payer = payerIsUser ? lockedBy : address(this);
                fewV2SwapExactOutput(map(recipient), amountOut, amountInMax, path, payer, isOriginalOutput);
                ...
                address[] calldata path = inputs.toAddressArray(3);
                address payer = payerIsUser ? lockedBy : address(this);
                ringV2SwapExactInput(map(recipient), amountIn, amountOutMin, path, payer, isOriginalOutput);
                ...
                address[] calldata path = inputs.toAddressArray(3);
                address payer = payerIsUser ? lockedBy : address(this);
                ringV2SwapExactOutput(map(recipient), amountOut, amountInMax, path, payer, isOriginalOutput);
            } else {
