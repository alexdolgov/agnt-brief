...
contract RcktMasterChef is ReentrancyGuard,Ownable {
    // remember to change for mainnet deploy
    address constant _trustedForwarder = 0x7C2B04D63FAEF69668e30d911EB48A78140Ab2EB; //TRUSTED FORWARDER
    ...
    function add(uint256 _allocPoint, IERC20 _lpToken, uint16 _depositFeeBP, uint256 _harvestInterval, bool _withUpdate) public onlyOperator {
    ...
    // Update the given pool's Rckt allocation point and deposit fee. Can only be called by the owner.
    ...
    // Deposit LP tokens to MasterChef for Rckt allocation.
    ...
    function safeSolarTransfer(address _to, uint256 _amount) internal {
        if (solar.balanceOf(address(this)) > totalSolarInPools) {
            //RcktBal = total Rckt in RcktDistributor - total Rckt in Rckt pools, this will make sure that RcktDistributor never transfer rewards from deposited Rckt pools
