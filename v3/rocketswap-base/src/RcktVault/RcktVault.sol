...
interface IRcktERC20 is IERC20 {
...
contract RcktVault is ReentrancyGuard,Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. Rckt to distribute per block.
        uint256 lastRewardBlock; // Last block number that Rckt distribution occurs.
        uint256 accSolarPerShare; // Accumulated Rckt per share, times 1e12. See below.
    ...
    address public constant _trustedForwarder = 0x7C2B04D63FAEF69668e30d911EB48A78140Ab2EB; //Trusted forwarder
    ...
    // The block number when Rckt mining starts.
    uint256 public startBlock;

    // Total locked up rewards
    uint256 public totalLockedUpRewards;

    // Total Rckt in Rckt Pools (can be multiple pools)
    ...
    constructor(){
        ...
        solar = IRcktERC20(0x6653dD4B92a0e5Bf8ae570A98906d9D6fD2eEc09);
        solarPerBlock = 0.01875 ether;
        devAddress = 0x25FD48E004b1f0148D7f562763Db6706c00dB7f7;
        feeAddress = 0x25FD48E004b1f0148D7f562763Db6706c00dB7f7;
    ...
    function add(uint256 _allocPoint,IERC20 _lpToken,uint16 _depositFeeBP,uint256 _harvestInterval,uint256 _lockupDuration,bool _withUpdate) public onlyOwner {
    ...
    // View function to see pending Rckt on frontend.
    ...
    function userLockedUntil(uint256 _pid, address _user) public view returns (uint256)
    {
        UserInfo storage user = userInfo[_pid][_user];
        PoolInfo storage pool = poolInfo[_pid];
    ...
    // View function to see if user can harvest Rckt.
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 solarReward = multiplier.mul(solarPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        solar.mint(devAddress, solarReward.mul(5).div(100));
        solar.mint(address(this), solarReward);
        pool.accSolarPerShare = pool.accSolarPerShare.add(solarReward.mul(1e12).div(pool.totalLp));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to RcktVault for Rckt allocation
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        require(block.number >= startBlock,"RcktVault: cannot deposit before farming start");
    ...
    function withdraw(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        require(block.timestamp > user.lastInteraction + pool.lockupDuration,"Withdraw: you cannot withdraw yet");
    ...
    // Safe Rckt transfer function, just in case if rounding error causes pool do not have enough Rckt.
    ...
    function updateEmissionRate(uint256 _rcktPerBlock) public onlyOperator {
        massUpdatePools();
        emit EmissionRateUpdated(msg.sender, solarPerBlock, _rcktPerBlock);
        solarPerBlock = _rcktPerBlock;
    }
    ...
    function enableMetaTxns() public onlyOperator {
        require(!metaTxnsEnabled,"RcktVault: meta transactions are already enabled" );
    ...
    function disableMetaTxns() public onlyOperator {
        require(metaTxnsEnabled,"RcktVault: meta transactions are already disabled");
        metaTxnsEnabled = false;
        emit MetaTxnsDisabled(_msgSender());
    }

    function setRcktPerBlock(uint256 _token) public onlyOperator{
        solarPerBlock = _token;
    }
}