...
library LibFeeManager {
    ...
    bytes32 constant FEE_MANAGER_STORAGE_POSITION = keccak256("fee.manager.storage");
    ...
    function removeFeeConfig(uint16 index) internal {
        ...
        require(fms.feeConfigPairs[index].length == 0, "LibFeeManager: Cannot remove the last fee config");
    ...
    // revenue = feeAmount - broker - dao - lpPool;
    function chargeFee(address token, uint256 feeAmount, uint24 broker) internal returns (uint24 brokerId, uint256 brokerAmount, uint256 daoAmount, uint256 lpPoolAmount){
        FeeManagerStorage storage fms = feeManagerStorage();
        IFeeManager.FeeDetail storage detail = fms.feeDetails[token];
        detail.total += feeAmount;

        (brokerAmount, brokerId, daoAmount, lpPoolAmount) = LibBrokerManager.updateBrokerCommission(token, feeAmount, broker);
        detail.brokerAmount += brokerAmount;

        if (daoAmount > 0) {
            // The buyback address prefers to receive wrapped tokens since LPs are composed of wrapped tokens, for example: WBNB-ABC LP.
            IERC20(token).safeTransfer(fms.daoRepurchase, daoAmount);
            detail.daoAmount += daoAmount;
        }

        if (lpPoolAmount > 0) {
            IVault(address(this)).increase(token, lpPoolAmount);
            detail.lpPoolAmount += lpPoolAmount;
        }

        uint256 revenue = feeAmount - brokerAmount - daoAmount - lpPoolAmount;
        if (revenue > 0) {
            LibBrokerManager.Commission storage c = fms.revenues[token];
            c.total += revenue;
            c.pending += revenue;
        }
        return (brokerId, brokerAmount, daoAmount, lpPoolAmount);
    }
}
