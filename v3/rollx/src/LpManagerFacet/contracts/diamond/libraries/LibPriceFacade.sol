...
library LibPriceFacade {
    ...
    bytes32 constant PRICE_FACADE_POSITION = keccak256("price.facade.storage");
    ...
    function getPrice(address token) internal view returns (uint256) {
        (uint64 price, ) = getPriceFromCacheOrOracle(token);
        return price;

        // TODO: use oracle when update frequetly
    ...
    function requestPriceCallback(bytes32 requestId, uint64 price) internal {
        ...
        require(ids.length > 0, string(abi.encodePacked("Non existed requestId")));
        
        (uint64 beforePrice, uint40 updatedAt) = getPriceFromCacheOrOracle(pfs, pendingPrice.token);
        // set beforePrice if not existed
        if (beforePrice == 0 && updatedAt == 0) {
            beforePrice = price;
        ...
        if (updatedAt > 0 && block.timestamp > updatedAt + pfs.maxDelay) {
            emit PriceRejected(msg.sender, requestId, pendingPrice.token, price, beforePrice, updatedAt);
            return;
        }
        ...
        /** comment this line, because the oracle is not updated frequently enough */
        ...
        for (UC i = ZERO; i < uc(ids.length); i = i + ONE) {
            ...
            if (idInfo.requestType == RequestType.OPEN) {
               ...
               ITradingOpen(address(this)).marketTradeCallback(idInfo.id, upperPrice, lowerPrice);
            } else if (idInfo.requestType == RequestType.CLOSE) {
                // ITradingClose(address(this)).closeTradeCallback(idInfo.id, upperPrice, lowerPrice);
    ...
    function getPriceFromCacheOrOracle(PriceFacadeStorage storage pfs, address token) internal view returns (uint64, uint40) {
        ...
        (uint256 price, int32 expo, uint256 oracleUpdatedAt) = LibChainlinkPrice.getPriceFromChainlink(token);
        ...
        // return (cachePrice.price, cachePrice.timestamp);
    }
}
