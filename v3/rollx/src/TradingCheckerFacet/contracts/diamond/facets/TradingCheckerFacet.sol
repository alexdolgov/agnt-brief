...
contract TradingCheckerFacet is ITradingChecker {
    ...
    function checkLimitOrderTp(LimitOrder calldata order) external view override {
    ...
    function openLimitOrderCheck(IBook.OpenDataInput calldata data) external view override {
        ...
        require(tc.limitOrder, "Limit order is temporarily disabled");

        (uint marketPrice,) = IPriceFacade(address(this)).getPriceFromCacheOrOracle(data.pairBase);
        require(marketPrice > 0, "No access to current market effective prices");
        ...
        require(
            ...
            "This limit order will be filled immediately"
        );

        // price * qty * 10^18 / 10^(8+10) = price * qty
        uint notionalUsd = uint256(data.price) * data.qty;
        require(notionalUsd >= tc.minNotionalUsd, "Position is too small");
        ...
        require(notionalUsd <= lms[lms.length - 1].notionalUsd, "Position is too large");
        ...
        require(amountInUsd > openFeeUsd + tc.executionFeeUsd, "The amount is not able to cover fee");
        ...
        require(
            ...
            "Exceeds the maximum leverage allowed for the position"
        );

        require(
            checkSl(data.isLong, data.stopLoss, data.price),
            "StopLoss is not in the valid range"
        );
    }
    ...
    function _buildExecuteLimitOrderCheckTuple(LimitOrder memory order) private view returns (ExecuteLimitOrderCheckTuple memory) {
    ...
    function _buildMarketTradeCallbackResult(
        ...
        return abi.encode(
            uint96(openFeeUsd * (10 ** tuple.token.decimals) / (1e10 * tuple.token.price)),
            uint96(tuple.tc.executionFeeUsd * (10 ** tuple.token.decimals) / (1e10 * tuple.token.price)),       // possible divide by zero
