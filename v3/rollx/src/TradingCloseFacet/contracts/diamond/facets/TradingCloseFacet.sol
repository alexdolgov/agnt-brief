...
contract TradingCloseFacet is ITradingClose, OnlySelf {
    ...
    function executeTpSlOrLiq(TpSlOrLiq[] calldata arr) external override {
        LibAccessControlEnumerable.checkRole(Constants.KEEPER_ROLE);
        LibTrading.TradingStorage storage ts = LibTrading.tradingStorage();
        for (uint256 i; i < arr.length;) {
            TpSlOrLiq calldata t = arr[i];
            _executeTpSlOrLiq(ts, t.tradeHash, t.price, t.executionType);
            unchecked {++i;}
        }
    }

    function executeTpSlOrLiq(TpSlOrLiqPyth[] calldata params) external override {
        LibAccessControlEnumerable.checkRole(Constants.KEEPER_ROLE);
        LibTrading.TradingStorage storage ts = LibTrading.tradingStorage();
        for (uint256 i; i < params.length;) {
            TpSlOrLiqPyth calldata t = params[i];
            _executeTpSlOrLiq(ts, t.tradeHash, t.priceId.getPythPrice(t.updateData), t.executionType);
            unchecked {++i;}
        }
    }

    function _executeTpSlOrLiq(
        LibTrading.TradingStorage storage ts, bytes32 tradeHash, uint64 price, ExecutionType executionType
    ) private {
        ITrading.OpenTrade storage ot = ts.openTrades[tradeHash];
        if (ot.margin == 0) {
            return;
        }
        (bool available, uint64 upper, uint64 lower) = IPriceFacade(address(this)).confirmTriggerPrice(ot.pairBase, price);
        if (!available) {
            emit ExecuteCloseRejected(ot.user, tradeHash, executionType, price, 0);
            return;
        }
        uint64 marketPrice;
        if (ot.isLong) {
        ...
        uint256 closePrice = ITradingCore(address(this)).slippagePrice(ot.pairBase, marketPrice, ot.qty, !ot.isLong);
        if (executionType == ExecutionType.TP) {
            if ((ot.isLong && marketPrice < ot.takeProfit) || (!ot.isLong && marketPrice > ot.takeProfit)) {
                emit ExecuteCloseRejected(ot.user, tradeHash, executionType, uint64(closePrice), marketPrice);
                return;
            }
            _executeTp(ts, ot, tradeHash, marketPrice, closePrice);
        } else if (executionType == ExecutionType.SL) {
            if ((ot.isLong && marketPrice > ot.stopLoss) || (!ot.isLong && marketPrice < ot.stopLoss)) {
                emit ExecuteCloseRejected(ot.user, tradeHash, executionType, uint64(closePrice), marketPrice);
                return;
            }
            _executeSl(ts, ot, tradeHash, marketPrice, closePrice);
        } else {
            _executeLiq(ts, ot, tradeHash, marketPrice, closePrice);
        }
    }
    ...
    function _executeLiq(
        ...
        if (!needLiq) {
            emit ExecuteCloseRejected(ot.user, tradeHash, ExecutionType.LIQ, uint64(closePrice), marketPrice);
            return;
        }

        ITradingCore(address(this)).updatePairPositionInfo(ot.pairBase, ot.entryPrice, marketPrice, ot.qty, ot.isLong, false);
        (closeFee, holdingFee) = _settleForLiqTrade(ts, ot, tradeHash, pnl, fundingFee, closeFee, holdingFee);
