...
contract TradingCoreFacet is ITradingCore, ITradingCoreError, OnlySelf {
    ...
    /**
     * Trigger funding fee collection if not balanced
     */
    ...
    function lpUnrealizedPnlUsdToken(address targetToken) external view override returns (int256 totalUsd, int256 tokenUsd) {
        ...
        for (UC i = ZERO; i < uc(ts.openTradeTokenIns.length); i = i + ONE) {
            ...
            if (ts.openTradeAmountIns[token] > 0) {
                ...
                // all convert to USD, this token's percentage
        ...
        // total unrealized USD,  total * percentage
        return (totalUsd, totalUsd * int256(tokenMarginUsd) / int256(totalMarginUsd));
    }

    // return total unrealized USD from rolldex, including all pairs
    function lpUnrealizedPnlTotalUsd() public view returns (int256 unrealizedPnlUsd) {
        LibTradingCore.TradingCoreStorage storage tcs = LibTradingCore.tradingCoreStorage();
