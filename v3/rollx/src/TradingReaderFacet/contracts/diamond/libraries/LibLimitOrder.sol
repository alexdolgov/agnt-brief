...
library LibLimitOrder {
    ...
    function check(LimitOrder storage order) internal view {
        require(order.amountIn > 0, "Order does not exist");
        require(order.user == msg.sender, "Limit order can only be updated by yourself");
    }
    ...
    function openLimitOrder(IBook.OpenDataInput memory data) internal returns (bytes32 orderHash) {
        ...
        LimitOrder memory order = LimitOrder(
            user, uint32(orderHashes.length), data.price, data.pairBase, data.amountIn, data.tokenIn, 
            data.isLong, data.qty, data.stopLoss, data.takeProfit, data.broker, uint40(block.timestamp)
        );
        // create order hash
