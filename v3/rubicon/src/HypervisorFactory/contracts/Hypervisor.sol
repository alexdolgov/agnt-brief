...
//   File:     contracts/Hypervisor.sol  (Hypervisor v1.3)
// Rubicon CLMM adaptation:
//   - `IUniswapV3Pool`          -> `IRubiconV3Pool`          (ABI-compatible rename fork)
//   - `IUniswapV3MintCallback`  -> `IRubiconV3MintCallback`  (ABI-compatible rename fork)
//   - `pool` state variable type changed accordingly.
//   - The callback function selector is preserved (`uniswapV3MintCallback(uint256,uint256,bytes)`)
//     because Rubicon CLMM kept the Uniswap V3 selector for bytecode compatibility
//     (see IRubiconV3MintCallback).
//   - All other logic, events, storage layout, and external/public ABI are byte-for-byte
...
import "./rubicon-clmm/interfaces/callback/IRubiconV3MintCallback.sol";
import "./rubicon-clmm/interfaces/IRubiconV3Pool.sol";
import "@uniswap/v3-core/contracts/libraries/TickMath.sol";
import "@uniswap/v3-core/contracts/libraries/FullMath.sol";
import "@uniswap/v3-periphery/contracts/libraries/LiquidityAmounts.sol";

/// @title Hypervisor v1.3 — Rubicon CLMM fork
/// @notice A Uniswap V2-like interface with fungible liquidity to a Rubicon V3 (Uniswap V3 fork) pool,
/// which allows for arbitrary liquidity provision: one-sided, lop-sided, and balanced.
contract Hypervisor is IRubiconV3MintCallback, ERC20Permit, ReentrancyGuard {
    using SafeERC20 for IERC20;
    using SafeMath for uint256;
    using SignedSafeMath for int256;

    IRubiconV3Pool public pool;
   ...
   event Deposit(
    ...
    /// @param _pool Rubicon V3 pool for which liquidity is managed
    ...
    function withdraw(
        ...
        /// Withdraw liquidity from Rubicon pool
    ...
    function rebalance(
        ...
        /// Withdraw all liquidity and collect all fees from Rubicon pool
    ...
    /// @notice Callback function of Rubicon V3 pool mint.
    /// @dev The selector name is preserved as `uniswapV3MintCallback` to match Rubicon CLMM's
    /// IRubiconV3MintCallback interface, which kept the Uniswap V3 selector for bytecode compatibility.
    ...
    /// @return tick Rubicon pool's current price tick
