...
contract Oracle is Ownable {
     ...
     * @param _timestamps array of original price submission timestamps (preserves dispute period finalization state)
     */
    function migrateOracle(
        ...
        require(_expiries.length == _timestamps.length, "Oracle: invalid migration data");

        for (uint256 i; i < _expiries.length; i++) {
            storedPrice[_asset][_expiries[i]] = Price(_prices[i], _timestamps[i]);
        }
    }
    ...
    function setDisputer(address _disputer) external onlyOwner {
        require(_disputer != address(0), "Oracle: cannot set disputer to address(0)");
        disputer = _disputer;
    ...
    function disputeExpiryPrice(
        ...
        require(_price > 0, "Oracle: disputed price cannot be zero");
    ...
    function setExpiryPrice(
        ...
        require(_price > 0, "Oracle: expiry price cannot be zero");
