...
contract ControllerLogic is Initializable, OwnableUpgradeSafe, ReentrancyGuardUpgradeSafe {
        ...
        if (typeVault == 2) {
            ...
            if (settleMem.receivingAssetBalance == 0) {
                ...
                settleMem.strikeCount = otoken.isPut() ? 
                    _convertToE18(settleMem.underlying, settleMem.receivingAssetBalance).div(1e10) :
                    _convertToE18(settleMem.strike, settleMem.receivingAssetBalance).div(strikePrice.mul(1e2));
    ...
    function _convertToE18(address _asset, uint256 _amount) internal view returns (uint256 convertedAmount) {
        // assumes asset decimals <= 18
        return _amount.mul(10**(18 - uint256(ERC20Interface(_asset).decimals())));
    }
}
