...
contract LiquidityPool is ILiquidityPool, ERC20Upgradeable, UUPSUpgradeable, AccessControl, ReentrancyGuardUpgradeable, PausableUpgradeable {
	...
	/**
	 * @notice write a number of options that already exist
	 * @param optionSeries the series detail of the option - strike in e8
	 * @param seriesAddress the series address of the oToken
	 * @param amount the number of options to write - in e8 (oToken decimals)
	...
	function handlerWriteOption( 
		...
		return
			_writeOption(
				...
				seriesAddress,
				amount, // number of options in e8
				optionRegistry,
				premium,
				recipient
			);
	 ...
	 * @return the number of options written in e8
	 */
	function _writeOption(
		...
		// Calculate collateral needed from the number of options
		uint256 collateral;
		{
			uint8 collateralDec = ERC20(collateralAsset).decimals();
			if (optionSeries.isPut) {
				// puts: collateral = ceil(options * strike price / 1e16)
				// Ceiling division ensures Opyn's 1e27 fixed-point check always passes.
				uint256 numerator = amount * uint256(optionSeries.strike) * (10 ** collateralDec);
				collateral = (numerator == 0) ? 0 : (numerator + 1e16 - 1) / 1e16;
			} else {
				// calls: 1 oToken = 1 collateral unit (with decimal adjustment)
				if (collateralDec >= 8) {
					collateral = amount * (10 ** (collateralDec - 8));
				} else {
					uint256 divisor = 10 ** (8 - collateralDec);
					collateral = (amount + divisor - 1) / divisor;
				}
			}
		}
		...
		if (collateral + saleFee > getBalance(collateralAsset)) {
		...
		IERC20(collateralAsset).forceApprove(address(optionRegistry), collateral);
		uint256 otokenBalanceBefore = IERC20(seriesAddress).balanceOf(address(this));
		(, uint256 collateralUsed) = optionRegistry.open(seriesAddress, amount, collateral);
	...
	function _buybackOption(
		...
		// Update written amounts tracking (tracks options in e8)
		if (writtenAmounts[seriesAddress] > amount) {
			writtenAmounts[seriesAddress] -= amount;
		} else {
