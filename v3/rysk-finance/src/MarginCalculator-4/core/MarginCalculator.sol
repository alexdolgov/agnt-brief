...
contract MarginCalculator is Ownable {
        ...
        returns (FPI.FixedPointInt memory, FPI.FixedPointInt memory)
    {
        ...
        if (now < otokenDetails.otokenExpiry) {
            ...
            FPI.FixedPointInt memory collateralValue = _convertAmountOnExpiryPrice(
                valueInStrike,
            ...
            // This stops redemptions having access to more collateral than is in the system.
            // This does not apply to puts since strikeAsset == collateralAsset for puts.
            if (collateralValue.isGreaterThan(FPI.fromScaledUint(1, 0)) && !otokenDetails.isPut){
                collateralValue = FPI.fromScaledUint(1, 0);
            }
            return (collateralAmount, collateralValue);
        }
    }
