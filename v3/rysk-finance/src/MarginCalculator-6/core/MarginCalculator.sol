...
contract MarginCalculator is Ownable {
    ...
    function getExpiredPayoutRate(address _otoken) external view returns (uint256) {
        ...
        // because LST collateralised calls are not liquidatable, there is a chance they can become undercollateralised
        // In the case of an LST depeg is it possible for the payout > 1 collateral token.
        // A safe way to resolve is to set payout to 1 collateral token per option.
        // This stops redemptions having access to more collateral than is in the system.
        // This does not apply to puts since strikeAsset == collateralAsset for puts.
        if (cashValueInCollateral.isGreaterThan(FPI.fromScaledUint(1, 0)) && !isPut){
        ...
        if (now < otokenDetails.otokenExpiry) {
            ...
            if (_vaultDetails.vaultType == 1) {
                ...
                if (otokenDetails.isPut) {
                    ...
                    return (collateralAmount, underlyingNeeded);
                }
            }
        } else {
            ...
            FPI.FixedPointInt memory collateralNeeded = _convertAmountOnExpiryPrice(
                valueInStrike,
            ...
            // because LST collateralised calls are not liquidatable, there is a chance they can become undercollateralised
            // In the case of an LST depeg is it possible for collateralNeeded > collateralAmount
            // A safe way to resolve is to set payout to 1 collateral token per option.
            // This stops redemptions having access to more collateral than is in the system.
            // It also ensures any overcollateralisation of the vault is safe from being claimed as payout by expired options.
            // This does not apply to puts since strikeAsset == collateralAsset for puts.
            if (collateralNeeded.isGreaterThan(shortAmount) && !otokenDetails.isPut){
                collateralNeeded = shortAmount;
            }
