 ...
 *  @notice All amounts in e8 (oToken decimals) for both writes and buybacks
 */
contract PremiumOptionHandler is UUPSUpgradeable, AccessControl, ReentrancyGuardUpgradeable, EIP712Upgradeable {
	 ...
	 * @param _amount the number of options in e8 (oToken decimals) for both writes and buybacks
	 * @param _price the price per unit - in e18
	...
	function createOrder(
		...
		if (_amount == 0) {
			revert("PremiumOptionHandler: Invalid amount");
		}
		if (_price == 0) {
	...
	function _executeWrite(Types.Order memory order) internal {
		// amount is in e8 (oToken decimals), price is per option in e18
		uint256 premium = _calculatePremium(order.amount, order.price);
	...
	function _executeBuyback(Types.Order memory order) internal {
		// For buybacks: amount is in e8 (oToken decimals), price is in e18
		uint256 premium = _calculatePremium(order.amount, order.price);
	...
	function createAndExecuteOrderWithSignature(
		...
		if (_isBuyback) {
			...
			uint256 premium = _calculatePremium(_amount, _price);
	...
	/**
	 * @notice calculate premium in collateral decimals from amount (e8) and price (e18)
	 * @dev amount(e8) * price(e18) = e26 intermediate. Scale to collateralDec:
	 *      collateralDec <= 26: divide by 10^(26 - collateralDec) with ceiling
	 *      collateralDec >  26: multiply by 10^(collateralDec - 26), exact
	 * @param _amount option amount in e8 (must be > 0)
	 * @param _price price per option in e18 (must be > 0)
	 * @return premium in collateral decimals
	 */
	function _calculatePremium(uint256 _amount, uint256 _price) internal view returns (uint256) {
		uint256 numerator = _amount * _price;
		uint8 collateralDec = ERC20(collateralAsset).decimals();
		if (collateralDec <= 26) {
			uint256 divisor = 10 ** (26 - collateralDec);
			return (numerator + divisor - 1) / divisor;
		} else {
			return numerator * (10 ** (collateralDec - 26));
		}
	}
