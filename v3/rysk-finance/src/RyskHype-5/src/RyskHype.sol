...
contract RyskHype is EIP712Upgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
  ...
  function setAssetApproval(address _asset, bool _approval) external {
    _checkOwner();
    if (_approval) {
      ERC20Interface(_asset).approve(address(marginPool), type(uint256).max);
    } else {
      ERC20Interface(_asset).approve(address(marginPool), 0);
    }
  }
  ...
  /**
	...
	function getOrDeployOtoken(Otoken memory otoken) internal {
      ...
      if (ERC20(otokenFromFactory).allowance(address(this), address(mmarket)) == 0) {ERC20(otokenFromFactory).approve(address(mmarket), type(uint256).max);}
			return;
		}
    ...
    ERC20(otokenCreated).approve(address(mmarket), type(uint256).max);
	}
  ...
  function executeOperation(
    ...
    // ====== hyperswap tx ========
    uint256 repayAmount = premium + amount;

    (address collateral, address underlying, address strike,, uint256 expiry,,) = IOtoken(args.asset).getOtokenDetails();

    ERC20Interface(collateral).approve(swapRouter, amountInMaximum);

    ISwapRouterHype.ExactOutputParams memory swapParams = ISwapRouterHype.ExactOutputParams({
			path: swapRoute,
			recipient: address(this),
      deadline: block.timestamp,
			amountOut: repayAmount - ERC20Interface(strike).balanceOf(address(this)),
    ...
    uint256 userProfitCollateral = ERC20Interface(collateral).balanceOf(address(this));
    ERC20Interface(collateral).transfer(redeemer, userProfitCollateral);

    // ====== Hyperlend flash loan repayment ========
		ERC20Interface(strike).approve(flashLoanPool, repayAmount);
