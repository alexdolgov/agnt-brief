...
contract RyskHype is EIP712Upgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
  ...
  function ingresso_settle(Actions.ActionArgs[] memory actions) external nonReentrant {
    ...
    for (uint256 i = 0; i < len; i++ ){
      if(msg.sender != _operator) {
        require(actions[i].owner == msg.sender, "only vault owner can settle");
      } else {
  ...
  function ingresso_newUserPosition(bytes calldata payload) external nonReentrant {
    ...
    require(totalPremium > sellerConfirmation.gasFee, "premium must cover gas fee");
    ...
    if(fee == 0 && sellerConfirmation.gasFee == 0) {
      operations = new MMarketOperations.Operation[](3); // no fee or gas fee
    } else if (fee > 0 && sellerConfirmation.gasFee > 0) {
      operations = new MMarketOperations.Operation[](5); // both fee and gas fee are > 0
    } else {
      operations = new MMarketOperations.Operation[](4); // either fee or gas fee is > 0
    }
