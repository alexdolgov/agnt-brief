...
contract RyskHype is EIP712Upgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
  ...
  function ingresso_newUserPosition(bytes calldata payload) external nonReentrant {
    ...
    // 7. CONDITIONAL Operations.Withdraw - handle fee payment if buyer is not rysk
  ...
  function ingresso_OTCTrade(bytes calldata payload) external nonReentrant {
    ...
    if(!checkSignature(otcTrade.user1, digest, sig)) {
    ...
    operations[0] = MMarketOperations.Operation(
      ...
      otcTrade.asset2, // asset 2 not used
      otcTrade.amount1, // amount 1 in asset decimals
      otcTrade.amount2, // amount 2 not used
      bytes('')
    );
