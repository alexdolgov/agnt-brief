...
library Parser {
  ...
  enum OperationType {
	...
	/// @dev Parses 360-byte payload into Quote + Confirmation + sigs for both
    ...
    )
	{
		...
		// 16 (fee)
		// = 360 bytes total
		require(payload.length == 360, "Invalid payload length");
		...
		assembly {
			...
			mstore(add(cPtr, 0x200), mload(add(payload, 344))) // gasFee
