...
contract ApexMakerV2 is AccessControl, ReentrancyGuard {
    ...
    address public immutable xApexStaking; // xApex staking contract, if applicable
    ...
    constructor(
        address apex_,
        IxBIFKN314 xBIFKN314_,
        address treasury_,
        address xApexStaking_
    ) {
        apex = apex_;
        xApex = xBIFKN314_;
        AVAX = 0xB31f66AA3C1e785363F0875A1B74E27b85FD66c7;
        treasury = treasury_;
        xApexStaking = xApexStaking_;
    ...
    function distributeNativeRewards() external onlyRole(APEX_MAKER_ROLE) {
        uint256 balance = nativeBalance();

        if (balance == 0) return;

        if (xApexStaking == address(0)) revert InvalidAddress(xApexStaking);

        (bool success, ) = payable(xApexStaking).call{value: balance}(
            new bytes(0)
        );
        if (!success) revert FailedToSendNativeCurrency(xApexStaking, balance);
    }
