/**
 *Submitted for verification at snowscan.xyz on 2024-12-12
*/
...
library events {
    ...
    event PlayerBet(uint256 indexed gameId, address indexed playerWallet, string playerUserName, uint256 indexed amountBet);
    event EpochStarted(uint256 indexed epochId, structs.GamePoints indexed epochPoints);
}
...
contract KovinJeckpot is Ownable, VRFConsumerBaseV2 {
    ...
    mapping(uint256 => structs.GameStats) public gameStats;
    mapping(uint256 => structs.PVPStats) public pvpStats;
    ...
    constructor(uint64 subscriptionId) VRFConsumerBaseV2(vrfCoordinator) {
        ...
        game++;
        gameStats[game].state = GameState.STARTED;
    }

    function getDailyWager(address user) public view returns (uint256) {
        return dailyWagers[returnDay(block.timestamp)][user]/10**18;
    }

    function getWeeklyWager(address user) public view returns (uint256) {
        return weeklyWagers[returnWeek(block.timestamp)][user]/10**18;
    }
    
    function getEpochWager(address user) public view returns (uint256) {
        return epochWagers[epoch][user]/10**18;
    }
    ...
    function getPlayersForGame(uint256 gameId) public view returns (address[] memory) {
        return gameStats[gameId].players;
    }
    ...
    function getPlayersForLastGame() public view returns (address[] memory, uint256[] memory, string[] memory) {
        address[] memory players = gameStats[game].players;
    ...
    function getGame(address user) public view returns (uint256 currentGame, uint256 lastGame, structs.GameStats memory lastGameStats, structs.GameStats memory currentGameStats, address[] memory playersWallets, uint256[] memory playersAmounts, string[] memory playersUsernames, structs.PlayerInfos memory playerInfos) {
        currentGame = game;
	    lastGame = game-1;
        lastGameStats = gameStats[lastGame];
	    currentGameStats = gameStats[currentGame];
    ...
    function getWinner(uint256 winAmount) internal view returns(address winner, string memory winnerUsername){
        ...
        for(uint256 i=0; i<gameStats[game].players.length; i++){
            uint256 amountWageredByPlayer = plInfo[gameStats[game].players[i]].amountPlayed;
            totalAmount += amountWageredByPlayer;
            if(winAmount <= totalAmount){
                return (gameStats[game].players[i], plInfo[gameStats[game].players[i]].username);
            }
        }
    }
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords) internal override {
        if(_vrfRequestType[requestId] == VRFRequestType.Game){
			require(gameStats[game].state == GameState.VRF_REQUESTED && gameStats[game].requestId == requestId, "GAME VRF error");
			uint256 totWag = gameStats[game].totalWager;
		
			uint256 winAmount = (randomWords[0] % totWag) + 1;
			gameStats[game].random = winAmount;
			(address winner, string memory winnerUsername) = getWinner(winAmount);
			gameStats[game].winner = winner;
			gameStats[game].winnerUsername = winnerUsername;
			plInfo[winner].points += epPoint[epoch].winPoints;
			plInfo[winner].totalWin += totWag;
			gameStats[game].state = GameState.FINISHED;
            ...
            require(pvpStats[PVPId].state == GameState.VRF_REQUESTED && pvpStats[PVPId].requestId == requestId, "PVP VRF error");
            uint256 totWag = pvpStats[PVPId].totalWager;
            uint256 winAmount = (randomWords[0] % 2);
            pvpStats[PVPId].random = winAmount;
            if(winAmount == 0){
                pvpStats[PVPId].winner = pvpStats[PVPId].walletA;
                pvpStats[PVPId].winnerUsername = pvpStats[PVPId].nameA;
            }
            else{
                pvpStats[PVPId].winner = pvpStats[PVPId].walletB;
                pvpStats[PVPId].winnerUsername = pvpStats[PVPId].nameB;
            }
            pvpStats[PVPId].state = GameState.FINISHED;
            plInfo[pvpStats[PVPId].winner].points += epPoint[epoch].winPoints;
            plInfo[pvpStats[PVPId].winner].totalWin += totWag;
            pvpReservedAmount -= totWag;
            IERC20(KOVIN).transfer(pvpStats[PVPId].winner, totWag);
            emit events.PVPEnded(PVPId, pvpStats[PVPId].winner, pvpStats[PVPId].winnerUsername, totWag);
        }
    ...
    function enterGame(uint256 amount, string memory userName) public payable {
        ...
        if(gameStats[game].players.length == 1){
            gameStats[game].startTime = block.timestamp;
            gameStats[game].gameDuration = gameDuration;
        ...
        require(gameStats[game].state == GameState.STARTED, "GAME not started");
        require(block.timestamp <= gameStats[game].startTime + gameDuration || gameStats[game].startTime == 0, "GAME ended");
        ...
        //UPDATE game Infos
        gameStats[game].totalWager += gameAmount;
        if(!isPlayerInGame[game][msg.sender]){
            isPlayerInGame[game][msg.sender] = true;
            gameStats[game].players.push(msg.sender);
        }
        checkPlayerInEpoch(msg.sender);
        emit events.PlayerBet(game, msg.sender, userName, gameAmount);
    ...
    function finishGame() public payable returns (uint256 requestId) {
        checkEOA();
        require(msg.value == taxTx, "TAX error");
        require(gameStats[game].state != GameState.VRF_REQUESTED, "VRF already");
        require(gameStats[game].state == GameState.STARTED && gameStats[game].startTime != 0 && block.timestamp > gameStats[game].startTime + gameDuration, "GAME not ended");
        
        if(gameStats[game].players.length > 0){
        ...
        // Will revert if subscription is not set and funded.
            ...
            gameStats[game].requestId = requestId;
            gameStats[game].state = GameState.VRF_REQUESTED;
    ...
    function enterPVP(address userA, address userB, string memory nameA, string memory nameB, uint256 wag) public payable returns (uint256 PVPId) {
        ...
        //UPDATE game Infos
        pvp++;
        pvpStats[pvp].walletA = userA;
        pvpStats[pvp].walletB = userB;
        pvpStats[pvp].nameA = nameA;
        pvpStats[pvp].nameB = nameB;
        pvpStats[pvp].wager = amountCommitted;
        pvpStats[pvp].totalWager = gameAmount;
        pvpStats[pvp].state = GameState.STARTED;
        pvpStats[pvp].startTime = block.timestamp;
    ...
    function acceptPVP(uint256 PVPId) public payable returns (uint256 requestId){
        checkEOA();
        require(pvpStats[PVPId].state == GameState.STARTED, "PVP state");
        require(block.timestamp <= pvpStats[PVPId].startTime + gameDuration, "Too late");
        require(msg.sender == pvpStats[PVPId].walletB, "PVP not user B");
        checkAllowanceAndBalance(msg.sender, pvpStats[PVPId].wager);
        require(msg.value == (taxTx*150)/100, "TAX error");
        IERC20(KOVIN).transferFrom(msg.sender, address(this), pvpStats[PVPId].wager);
        uint256 amountCommitted = pvpStats[PVPId].wager;
        uint256 holdAmount = IERC20(KOVIN).balanceOf(msg.sender);
        //UPDATE player Infos
        plInfo[msg.sender].username = pvpStats[PVPId].nameB;
        pvpReservedAmount -= amountCommitted;
        ...
        plInfo[pvpStats[PVPId].walletA].volume += amountCommitted;
        plInfo[pvpStats[PVPId].walletB].volume += amountCommitted;
        recordWagers(pvpStats[PVPId].walletA, amountCommitted);
        recordWagers(pvpStats[PVPId].walletB, amountCommitted);
        plInfo[pvpStats[PVPId].walletA].points += getPointsForWager(amountCommitted, plInfo[pvpStats[PVPId].walletA].lastHoldAmount);
        plInfo[pvpStats[PVPId].walletB].points += getPointsForWager(amountCommitted, holdAmount);
        checkPlayerInEpoch(pvpStats[PVPId].walletA);
        checkPlayerInEpoch(pvpStats[PVPId].walletB);
        IERC20(KOVIN).transfer(burnAddress, burnAmount);
        IERC20(KOVIN).transfer(marketingAddress, marketingAmount);
        //UPDATE game Infos
        pvpStats[PVPId].totalWager = gameAmount;
        pvpReservedAmount += gameAmount;
        emit events.PlayerBet(PVPId, pvpStats[PVPId].walletA, pvpStats[PVPId].nameA, gameAmount/2);
        emit events.PlayerBet(PVPId, pvpStats[PVPId].walletB, pvpStats[PVPId].nameB, gameAmount/2);
        //VRF
        requestId = COORDINATOR.requestRandomWords(
                s_keyHash,
                s_subscriptionId,
        ...
        pvpStats[PVPId].state = GameState.VRF_REQUESTED;
        pvpStats[PVPId].requestId = requestId;
    ...
    function cancellationPVP(uint256 PVPId) internal {
        checkEOA();
        require(pvpStats[PVPId].state == GameState.STARTED, "PVP state");
        IERC20(KOVIN).transfer(pvpStats[PVPId].walletA, pvpStats[PVPId].wager);
        pvpReservedAmount -= pvpStats[PVPId].wager;
        pvpStats[PVPId].wager = 0;
        pvpStats[PVPId].totalWager = 0;
        pvpStats[PVPId].state = GameState.FINISHED;
    }
    ...
    function cancelPVP(uint256 PVPId) public {
        require(block.timestamp > pvpStats[PVPId].startTime + gameDuration, "PVP cancel early");
        cancellationPVP(PVPId);
    }
    ...
    function withdrawToken(address token) public onlyOwner {
        if(token == KOVIN){
            IERC20(token).transfer(msg.sender, IERC20(token).balanceOf(address(this))-gameStats[game].totalWager-pvpReservedAmount);
        }
        else{
