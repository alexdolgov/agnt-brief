...
contract SalvorGovernanceToken is ERC20, ERC20Snapshot, Ownable {
    bool public minted;
    uint public constant maxSupply = 160000000000000000000000000;

    constructor(string memory tokenName, string memory symbol) ERC20(tokenName, symbol) {
        minted = false;
    }

    function startMint(address[] memory receivers, uint256[] memory values) external onlyOwner {
        ...
        for (uint i = 0; i < receivers.length; i++) {
            totalMintedAmount += values[i];
            require(totalMintedAmount <= maxSupply, "Maximum token supply reached. Cannot mint any more tokens.");
