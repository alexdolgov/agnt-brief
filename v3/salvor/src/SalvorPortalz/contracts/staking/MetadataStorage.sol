...
contract MetadataStorage is Ownable, Multicall {
    ...
    function metadata(address contractAddress, uint256 tokenId) external view returns (Metadata memory) {
        return _metadata[contractAddress][tokenId];
    }

    function rarityReward(address contractAddress, uint256 tokenId) external view returns (uint256) {
        return _metadata[contractAddress][tokenId].rarityReward;
    }

    function attributes(address contractAddress, uint256 tokenId) external view returns (uint8[MAX_ATTRIBUTES] memory) {
    ...
    /**
     ...
     @param attribute type of the attribute (eg: 0 -> Background).
    ...
    */
    function hasAttribute(
        ...
        return _metadata[contractAddress][tokenId].attributes[attribute] == variant;
    }
    ...
    function updateMetadata(address tokenContract, uint256[] memory ids, Metadata[] memory data) external onlyOwner {
