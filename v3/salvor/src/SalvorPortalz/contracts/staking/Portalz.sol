...
abstract contract Portalz is VRFConsumerBaseV2, AccessControl, Pausable, ERC1155Holder {
     ...
     */
    function areNFTsAllowed(Tokens.ERC165Token[] calldata tokens) external view virtual returns (bool) {
    ...
    */
     ...
     */
    mapping(uint16 => BonusERC1155Reward) public bonusRewardz;
    ...
    function drawBonusReward(
        ...
        if (startTime < block.timestamp && bonusRewardzLength > 0) {
            uint256 period = (block.timestamp - startTime) > (maxPeriod() * 1 days)
                ? (maxPeriod() * 1 days)
                : (block.timestamp - startTime);
            uint256 fullDaysPeriod = period / 1 days;
    ...
    /**
     @notice Callback from Chainlink VRF. Checks and sends Bonus Rewards to the user claiming from staking.
     @dev This is the basic implementation of the method which allows multiple bonus rewards if very lucky.
     The method can be override in specific portals to have a different logic, for example allowing only 1 bonus reward per claim.
     @param _requestId VRF requestId
     @param _randomWords VRF random numbers
    */
