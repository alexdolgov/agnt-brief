...
contract StrategyHyperliquidSTETH is IStrategy, IERC3156FlashBorrower, OwnableUpgradeable, ParaSwapCaller {
    ...
    address public constant CCTP_FORWARDER = 0xb21D281DEdb17AE5B501F6AA8256fe38C4e45757;
    ...
    function _bridge(address _token, uint256 _amount, address _receiver) internal {
        ...
        bytes32 recieverBytes_ = bytes32(abi.encode(CCTP_FORWARDER));

        uint256 maxFee = getStorage().bridgeMaxFee;
        ...
        ITokenMessenger(getStorage().circleBridge).depositForBurnWithHook(
            _amount, uint32(19), recieverBytes_, USDC, recieverBytes_, maxFee, 2000, hookData
        );
    }
    ...
    function swap(address _srcToken, address _dstToken, uint256 _amount, uint256 _minOut, bytes memory _swapData)
        external
        onlyRebalancer
    {
        if ((_srcToken == USDC && _dstToken == WSTETH) || (_srcToken == WSTETH && _dstToken == USDC)) {
    ...
    function transferToVault(address _token, uint256 _amount) external onlyRebalancer {
        ...
        IERC20(STETH).safeTransfer(s.vault, stETHAmount_);
    ...
    function callFlashLoan(address _token, uint256 _amount, bytes calldata _data) external onlyRebalancer {
        bytes memory dataBytes_ = abi.encode(uint256(0), this.onFlashLoan.selector, _data);
        IFlashloanHelper(getStorage().flashLoanHelper).flashLoan(
            IERC3156FlashBorrower(address(this)), _token, _amount, dataBytes_
        );
    }
    ...
    function onFlashLoan(address _initiator, address _token, uint256 _amount, uint256 _fee, bytes calldata _data)
        ...
        returns (bytes32)
    {
        if (_initiator != address(this) || msg.sender != getStorage().flashLoanHelper) {
