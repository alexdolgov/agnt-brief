...
abstract contract StrategyFactory is OwnableUpgradeable {
    ...
    function createStrategy(address _impl, bytes calldata _initBytes, uint256 _positionLimit) external onlyOwner {
        if (_positionLimit == 0 || _positionLimit > MAX_POSITION_LIMIT) revert CustomErrors.InvalidLimit();
    ...
    function removeStrategy(address _strategy) external onlyOwner {
        if (IStrategy(_strategy).getNetAssets() > 0) revert CustomErrors.UnSupportedOperation();
    ...
    function updateStrategyLimit(uint256 _offset, uint256 _newPositionLimit) external onlyOwner {
        if (_newPositionLimit == 0 || _newPositionLimit > MAX_POSITION_LIMIT) revert CustomErrors.InvalidLimit();
