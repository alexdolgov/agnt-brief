    ...
    ReentrancyGuardUpgradeable
{
    ...
    modifier onlyRebalancer() {
        if (msg.sender != vaultParams.rebalancer) revert CustomErrors.CallerNotRebalancer();
        _;
    }
    ...
    modifier whenPriceUpdated() {
        if (vaultParams.maxPriceUpdatePeriod < block.timestamp - vaultState.lastUpdatePriceTime) {
            revert CustomErrors.PriceNotUpdated();
        }
        _;
    }
    ...
    function initialize(bytes calldata _initBytes) external initializer {
        ...
        if (params_.underlyingToken == address(0)) revert CustomErrors.InvalidUnderlyingToken();
        if (params_.rebalancer == address(0)) revert CustomErrors.InvalidRebalancer();
        if (params_.admin == address(0)) revert CustomErrors.InvalidAdmin();
        if (params_.feeReceiver == address(0)) revert CustomErrors.InvalidFeeReceiver();
        if (params_.marketCapacity < MIN_MARKET_CAPACITY) revert CustomErrors.MarketCapacityTooLow();
        if (params_.managementFeeRate > MAX_MANAGEMENT_FEE_RATE) revert CustomErrors.ManagementFeeRateTooHigh();
        if (params_.managementFeeClaimPeriod < MIN_MANAGEMENT_FEE_CLAIM_PERIOD) {
            revert CustomErrors.ManagementFeeClaimPeriodTooShort();
        }
        if (params_.maxPriceUpdatePeriod > MAX_PRICE_UPDATED_PERIOD) revert CustomErrors.PriceUpdatePeriodTooLong();
        if (params_.revenueRate > MAX_REVENUE_FEE_RATE) revert CustomErrors.RevenueFeeRateTooHigh();
        if (params_.exitFeeRate > MAX_EXIT_FEE_RATE) revert CustomErrors.ExitFeeRateTooHigh();
        ...
        for (uint256 i; i < tokens_.length; i++) {
            if (tokens_[i] == address(0)) revert CustomErrors.InvalidUnderlyingToken();
    ...
    function updateMarketCapacity(uint256 _newCapacityLimit) external onlyOwner {
        if (_newCapacityLimit <= vaultParams.marketCapacity) revert CustomErrors.UnSupportedOperation();
    ...
    function updateManagementFee(uint256 _newManagementFeeRate) external onlyOwner {
        if (_newManagementFeeRate > MAX_MANAGEMENT_FEE_RATE) revert CustomErrors.ManagementFeeRateTooHigh();
    ...
    function updateMaxPriceUpdatePeriod(uint256 _newMaxPriceUpdatePeriod) external onlyOwner {
        if (_newMaxPriceUpdatePeriod > MAX_PRICE_UPDATED_PERIOD) revert CustomErrors.PriceUpdatePeriodTooLong();
    ...
    function updateRevenueRate(uint256 _newRevenueRate) external onlyOwner {
        if (_newRevenueRate > MAX_REVENUE_FEE_RATE) revert CustomErrors.RevenueFeeRateTooHigh();
    ...
    function updateExitFeeRate(uint256 _newExitFeeRate) external onlyOwner {
        if (_newExitFeeRate > MAX_EXIT_FEE_RATE) revert CustomErrors.ExitFeeRateTooHigh();
    ...
    function updateRedeemOperator(address _newRedeemOperator) external onlyOwner {
        if (_newRedeemOperator == address(0)) revert CustomErrors.InvalidRedeemOperator();
    ...
    function addToken(address _newToken) external onlyOwner {
        if (_newToken == address(0)) revert CustomErrors.InvalidUnderlyingToken();
    ...
    function updateExchangePrice() external onlyRebalancer returns (uint256 newExchangePrice, uint256 newRevenue) {
        ...
        if (newExchangePrice > vaultState.revenueExchangePrice) {
            ...
            if (vaultState.exchangePrice - oldExchangePrice_ > oldExchangePrice_ * MAX_PRICE_CHANGE_RATE / 1e4) {
                revert CustomErrors.IncorrectState();
            }
    ...
    function transferToStrategy(address _token, uint256 _amount, uint256 _strategyIndex) external {
        ...
        if (_strategyIndex == 0) {
            if (caller_ != owner() && caller_ != vaultParams.rebalancer) revert CustomErrors.InvalidOperator();
        } else {
            if (caller_ != owner()) revert CustomErrors.InvalidOperator();
        }
        address strategyAddress_ = strategyAddress(_strategyIndex);
        IERC20(_token).safeIncreaseAllowance(strategyAddress_, _amount);
        if (!IStrategy(strategyAddress_).onTransferIn(_token, _amount)) revert CustomErrors.IncorrectState();
    ...
    {
        if (!tokens.contains(_token)) revert CustomErrors.InvalidAsset();
        if (msg.sender != vaultParams.redeemOperator) revert CustomErrors.UnSupportedOperation();
        if (vaultState.lastUpdatePriceTime != block.timestamp) revert CustomErrors.PriceNotUpdated();
    ...
    function withdraw(uint256, address, address) public virtual override returns (uint256) {
        ...
        revert CustomErrors.NotSupportedYet();
    }
    ...
    function requestRedeem(uint256 _shares, address _token) external virtual nonReentrant whenNotPaused {
        if (_shares == 0) revert CustomErrors.WithdrawZero();
    ...
    function collectManagementFee() external {
        if (msg.sender != vaultParams.feeReceiver) revert CustomErrors.InvalidFeeReceiver();
        uint256 nowTime_ = block.timestamp;
        if (nowTime_ - vaultState.lastClaimMngFeeTime < vaultParams.managementFeeClaimPeriod) {
            revert CustomErrors.InvalidClaimTime();
        }
    ...
    function pause() external {
        if (msg.sender != owner() && msg.sender != vaultParams.rebalancer) revert CustomErrors.UnSupportedOperation();
        _pause();
    }
