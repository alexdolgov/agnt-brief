...
contract Vault is VaultYieldBasic, Constants {
    ...
    event UpdateInstantFee(uint256 oldInstantFee, uint256 newInstantFee);
    event UpdateDepositPaused(bool newDepositPaused);
    ...
    function updateInstantFee(uint256 _newInstantFee) external onlyOwner {
        emit UpdateInstantFee(instantFee, _newInstantFee);
        instantFee = _newInstantFee;
    }

    function updateDepositPaused(bool _isDepositPaused) external onlyOwner {
        depositPaused = _isDepositPaused;
        emit UpdateDepositPaused(_isDepositPaused);
    }
}
