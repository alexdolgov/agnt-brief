...
import "./interfaces/ILBPairV21.sol";
...
contract JoeV21LegacyDexLens is PendingOwnable, IJoeDexLens {
    ...
    function _getPriceFromV2(address _pairAddress, address _token) private view returns (uint256 price) {
        ILBPairV21 pair = ILBPairV21(_pairAddress);

        uint256 activeID = pair.getActiveId();
        uint256 priceScaled = _ROUTER_V2.getPriceFromId(ILBPair(_pairAddress), uint24(activeID));
    ...
    function _getTokens(DataFeed calldata _dataFeed) private view returns (address tokenA, address tokenB) {
        if (_dataFeed.dfType == dfType.V1) {
            ...
            ILBPairV21 pair = ILBPairV21(_dataFeed.dfAddress);
