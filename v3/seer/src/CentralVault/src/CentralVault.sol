...
import {CVPositions} from "./CVPositions.sol";
...
///      1. enterPosition: user deposits collateral → Router splits into one unit of each raw Seer outcome
///         token per collateral unit → vault mints CVPositions (ERC1155) for the user's weighted share of
///         each positionable outcome; raw surplus stays in this contract as the "socialized pool".
///      2. redeem: user returns CVPositions → vault unwraps to raw outcome tokens, adds a proportional
///         share of its surplus (the bonus) → Router redeems the combined raw amount → collateral is
///         sent to the user.
///      Users never hold raw Seer outcome tokens; only CVPositions can redeem through this vault.
...
contract CentralVault is ICentralVault {
    ...
    constructor(IRouter _router, IReality _reality) {
        ...
        reality = _reality;
        cvPositions = ICVPositions(address(new CVPositions(address(this))));
    }
    ...
    ///      3. Pull collateral and split via Router (vault receives `amount` of every raw outcome token).
    ///      4. Mint CVPositions for each user share; vault balance retains the raw surplus (unwrapped).
    ///      5. Mint the full INVALID position token amount to the user (not part of positioning).
        ...
        external
    {
        ...
        // For each positionable outcome i: user gets CVPositions for amount * weights[i] / WEIGHT_SCALE;
        ...
        for (uint256 i = 0; i < numOutcomes; i++) {
            ...
            if (userKeeps > 0) {
                ...
                require(token.approve(address(cvPositions), userKeeps), "Outcome approve failed");
                cvPositions.mint(market, i, msg.sender, userKeeps);
            }
        }

        // INVALID_RESULT: always the last slot; never split via weights — user receives 100% as CVPositions.
        (IERC20 invalidToken,) = market.wrappedOutcome(slotCount - 1);
        require(invalidToken.approve(address(cvPositions), amount), "Invalid outcome approve failed");
        cvPositions.mint(market, slotCount - 1, msg.sender, amount);
    ...
    ///      2. Burn user's CVPositions and release raw underlying into this contract.
    ///      3. Redeem (user amount + bonus) per outcome through the Router.
    ///      4. Forward all collateral received to the user.
    /// @param amounts Length must equal outcomeSlotCount; entry i is the CVPosition amount the user
    ///                redeems for outcome i (0 if not redeeming that slot). Typically includes INVALID at
    ...
    function redeem(IERC20 collateralToken, IMarket market, uint256[] calldata amounts) external {
        ...
        for (uint256 i = 0; i < slotCount; i++) {
            if (amounts[i] > 0) {
                cvPositions.releasePositions(market, i, msg.sender, amounts[i]);
            }
        }
    ...
    ///         where positionCirculating_i = cvPositions.totalSupply(tokenId_i).
    /// @dev Skips outcomes with zero payout numerator (losing side after resolution) or zero
    ///      position supply (avoids division by zero). Bonus is only paid on redeemable outcomes.
    ...
    /// @inheritdoc ICentralVault
    /// @notice CV position tokens held by users for a given outcome (excludes external Seer mints).
    function positionCirculating(IMarket market, uint256 outcomeIndex) public view returns (uint256) {
        return cvPositions.totalSupply(positionTokenId(market, outcomeIndex));
    }

    /// @inheritdoc ICentralVault
    function positionTokenId(IMarket market, uint256 outcomeIndex) public view returns (uint256) {
        return cvPositions.tokenId(market, outcomeIndex);
    }
