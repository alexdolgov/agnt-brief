// SPDX-License-Identifier: MIT
pragma solidity 0.8.20;

import {IMarket} from "./IMarket.sol";

/// @title IParimutuelPositions
/// @notice ParimutuelVault-specific API for parimutuel position tokens (ERC1155 surface from solmate ERC1155).
interface IParimutuelPositions {
    /// @notice Vault authorized to mint and release position tokens.
    function parimutuelVault() external view returns (address);

    /// @notice ERC1155 id for a deadline-scoped position on a market outcome slot.
    function tokenId(IMarket market, uint256 outcomeIndex, uint32 entryDeadline) external pure returns (uint256);

    /// @notice Circulating supply of position tokens for a given ERC1155 id.
    function totalSupply(uint256 id) external view returns (uint256);

    /// @notice Pull underlying from `vault`, escrow it, and mint position tokens to `to`.
    function mint(IMarket market, uint256 outcomeIndex, uint32 entryDeadline, address to, uint256 amount) external;

    /// @notice Burn position tokens from `from` and release underlying to the vault.
    function releasePositions(IMarket market, uint256 outcomeIndex, uint32 entryDeadline, address from, uint256 amount)
        external;
}
