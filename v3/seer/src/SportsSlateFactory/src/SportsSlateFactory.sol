...
import {PickemStrings} from "./libs/PickemStrings.sol";

/// @title SportsSlateFactory
/// @notice Creates Seer sports markets per match and registers a slate via PickemSlateFactory.
contract SportsSlateFactory is ISportsSlateFactory {
    uint256 public constant MAX_MATCHES = 32;
    ...
    IPickemSlateFactory public immutable pickemSlateFactory;
    ...
    constructor(IMarketFactory seerFactory_, IPickemSlateFactory pickemSlateFactory_) {
        seerFactory = seerFactory_;
        pickemSlateFactory = pickemSlateFactory_;
    }
        ...
        returns (uint256 slateId, address[] memory markets)
    {
        ...
        if (matchCount > MAX_MATCHES) revert TooManyMatches();
        ...
        slateId = pickemSlateFactory.createSlate(
            IPickemSlateFactory.CreateSlateParams({
                collateral: IERC20(seerFactory.collateralToken()),
                markets: markets
            })
        );

        emit SportsSlateCreated(slateId, params.kind, params.tournament, markets, msg.sender);
    }
    ...
    {
        string memory question = PickemStrings.formatQuestion(
        ...
        if (params.kind == MarketKind.Categorical) {
            ...
            marketParams.outcomes = new string[](0);
            marketParams.tokenNames = new string[](0);
