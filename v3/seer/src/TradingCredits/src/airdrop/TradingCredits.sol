...
contract TradingCredits {
    ...
    mapping(address => uint256) public creditsBalance; // The amount of credits assigned to each address.
    mapping(address => bool) public whitelistedContracts; // Whitelist of contracts that can be called.
    mapping(address => bool) public whitelistedOutputTokens; // Whitelist of output tokens that can be acquired.
    ...
    /// @dev Add or remove output tokens from the whitelist.
    /// @param _tokens The addresses of the output tokens to modify.
    /// @param _whitelisted The whitelist status for each token (true to add, false to remove).
    function setWhitelistedOutputTokens(address[] memory _tokens, bool[] memory _whitelisted) external onlyGovernor {
        require(_tokens.length == _whitelisted.length, "Arrays length mismatch");
        for (uint256 i; i < _tokens.length; ++i) {
            whitelistedOutputTokens[_tokens[i]] = _whitelisted[i];
        }
    }

    /// @dev Check if a user can spend a specific amount of credits for a specific output token.
    /// @param _user The address of the user to check.
    /// @param _amount The amount of credits to check.
    /// @param _outputToken The output token that the user wants to acquire.
    /// @return True if the user can spend the specified amount of credits.
    function canSpendCredits(address _user, uint256 _amount, address _outputToken) external view returns (bool) {
        ...
        // Check if output token is whitelisted
        if (!whitelistedOutputTokens[_outputToken]) {
            return false;
        }
    ...
    /// @param outputToken The token that the user is buying (must be whitelisted and have increased balance after swap).
    ...
    function execute(address to, bytes calldata data, uint256 amount, ERC20 outputToken) external {
        ...
        require(whitelistedOutputTokens[address(outputToken)], "Output token not whitelisted");
