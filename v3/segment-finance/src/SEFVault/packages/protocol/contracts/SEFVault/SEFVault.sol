...
interface ISEFVaultProxy {
    function _acceptImplementation() external returns (uint);

    function admin() external returns (address);
}

contract SEFVault is ISEFVault, SEFVaultStorage, ECDSA, AccessControlledV5 {
    ...
    event StoreUpdated(address oldSef, address oldStore, address newSef, address newStore);
    ...
    function add(
        ...
        require(address(sefStore) != address(0), "Store contract address is empty");
        ...
        ISEFStore(sefStore).setRewardToken(_rewardToken, true);
    ...
    function setRewardAmountPerBlock(address _rewardToken, uint256 _rewardAmount) external {
        ...
        require(ISEFStore(sefStore).rewardTokens(_rewardToken), "Invalid reward token");
        massUpdatePools(_rewardToken);
    ...
    /**
     * @notice Deposit SEFVault for SEF allocation
    ...
    function deposit(address _rewardToken, uint256 _pid, uint256 _amount) external nonReentrant isActive {
        ...
        if (address(pool.token) == sefAddress) {
            _moveDelegates(address(0), delegates[msg.sender], safe96(_amount, "SEFVault::deposit: votes overflow"));
        }
    ...
    function claim(address _account, address _rewardToken, uint256 _pid) external nonReentrant isActive {
        ...
        if (user.amount > 0) {
            ...
            ISEFVaultDistributor(pool.rewardsDistributor).claimAll(_account);
        }
    }
    ...
    /**
     * @notice Execute withdrawal to SEFVault for SEF allocation
     * @param _rewardToken The Reward Token Address
     * @param _pid The Pool Index
     */
    ...
    /**
     * @notice Request withdrawal to SEFVault for SEF allocation
     * @param _rewardToken The Reward Token Address
     * @param _pid The Pool Index
    ...
    function requestWithdrawal(address _rewardToken, uint256 _pid, uint256 _amount) external nonReentrant isActive {
        ...
        if (address(pool.token) == sefAddress) {
            _moveDelegates(
                ...
                address(0),
                safe96(_amount, "SEFVault::requestWithdrawal: votes overflow")
            );
        }
    ...
    /**
     * @notice View function to see pending SEFs on frontend
     * @param _rewardToken Reward token address
     * @param _pid Pool index
     ...
     * @param _user User address
     */
    function pendingDistributorRewards(address _rewardToken, uint256 _pid, address _user) external view returns (TSEFVaultDistributorReward[] memory) {
        _ensureValidPool(_rewardToken, _pid);
        ...
        if (pool.rewardsDistributor == address(0)) {
            return new TSEFVaultDistributorReward[](0);
        }
        return ISEFVaultDistributor(pool.rewardsDistributor).pendingRewards(_user, userSupply, totalSupply);
    }
    ...
    function _updateDistributorAccountChange (address _user, address _rewardToken, uint256 _pid, uint256 amountChange, bool isDeposited) internal {
        ...
        if (isDeposited) {
            ...
            ISEFVaultDistributor(pool.rewardsDistributor).deposited(_user, prevUserSupply, userSupply, totalSupply);
        } else {
            uint256 prevUserSupply = userSupply + amountChange;
            ISEFVaultDistributor(pool.rewardsDistributor).withdrawn(_user, prevUserSupply, userSupply, totalSupply);
        }
    ...
    function _updateDistributor (address _rewardToken, uint256 _pid) internal {
        ...
        uint256 totalSupply = pool.totalSupply - totalPendingWithdrawals[_rewardToken][_pid];
        ISEFVaultDistributor(pool.rewardsDistributor).update(totalSupply);
    }
    ...
    /**
     * @notice Get the SEF stake balance of an account (excluding the pending withdrawals)
     * @param account The address of the account to check
     * @return The balance that user staked
     */
    function getStakeAmount(address account) internal view returns (uint96) {
        require(sefAddress != address(0), "SEFVault::getStakeAmount: sef address is not set");

        PoolInfo[] storage poolInfo = poolInfos[sefAddress];

        uint256 length = poolInfo.length;
        for (uint256 pid = 0; pid < length; ++pid) {
            if (address(poolInfo[pid].token) == address(sefAddress)) {
                UserInfo storage user = userInfos[sefAddress][pid][account];
                return safe96(user.amount.sub(user.pendingWithdrawals), "SEFVault::getStakeAmount: votes overflow");
            }
        }
        return uint96(0);
    }
    ...
    /**
     * @notice Public method to get the SEF stake balance of an account (excluding the pending withdrawals)
     * @param account The address of the account to check
     * @return The balance that user staked
     */
    ...
    function delegateBySig(
        ...
        bytes32 domainSeparator = keccak256(
            abi.encode(DOMAIN_TYPEHASH, keccak256(bytes("SEFVault")), getChainId(), address(this))
        );
        bytes32 structHash = keccak256(abi.encode(DELEGATION_TYPEHASH, delegatee, nonce, expiry));
        bytes32 digest = keccak256(abi.encodePacked("\x19\x01", domainSeparator, structHash));
        address signatory = ECDSA.recover(digest, v, r, s);
        require(nonce == nonces[signatory]++, "SEFVault::delegateBySig: invalid nonce");
        require(block.timestamp <= expiry, "SEFVault::delegateBySig: signature expired");
    ...
    function _moveDelegates(address srcRep, address dstRep, uint96 amount) internal {
        if (srcRep != dstRep && amount > 0) {
            if (srcRep != address(0)) {
                ...
                uint96 srcRepNew = sub96(srcRepOld, amount, "SEFVault::_moveVotes: vote amount underflows");
            ...
            if (dstRep != address(0)) {
                ...
                uint96 dstRepNew = add96(dstRepOld, amount, "SEFVault::_moveVotes: vote amount overflows");
    ...
    function _writeCheckpoint(address delegatee, uint32 nCheckpoints, uint96 oldVotes, uint96 newVotes) internal {
        uint32 blockNumber = safe32(block.number, "SEFVault::_writeCheckpoint: block number exceeds 32 bits");
    ...
    /**
     * @notice Determine the sef stake balance for an account
     * @param account The address of the account to check
     * @param blockNumber The block number to get the vote balance at
     * @return The balance that user staked
     */
    function getPriorVotes(address account, uint256 blockNumber) external view returns (uint96) {
        require(blockNumber < block.number, "SEFVault::getPriorVotes: not yet determined");
    ...
    function _become(ISEFVaultProxy sefVaultProxy) external {
        require(msg.sender == sefVaultProxy.admin(), "only proxy admin can change brains");
        require(sefVaultProxy._acceptImplementation() == 0, "change not authorized");
    }

    function setSefStore(address _sef, address _sefStore) external onlyAdmin {
        _ensureNonzeroAddress(_sef);
        _ensureNonzeroAddress(_sefStore);

        address oldSefContract = sefAddress;
        address oldStore = sefStore;
        require(oldSefContract == address(0), "already initialized");
        ...
        emit StoreUpdated(oldSefContract, oldStore, _sef, _sefStore);
    }
    ...
    function _transferReward(address rewardToken, address userAddress, uint256 amount) internal {
        address sefStore_ = sefStore;
        uint256 storeBalance = IBEP20(rewardToken).balanceOf(sefStore_);
        ...
        if (fullAmount <= storeBalance) {
            ...
            ISEFStore(sefStore_).safeRewardTransfer(rewardToken, userAddress, fullAmount);
            return;
        }
        ...
        ISEFStore(sefStore_).safeRewardTransfer(rewardToken, userAddress, storeBalance);
    }
    ...
    /**
     * @dev Adjust totalSupply of the pools to be equal to balanceOf SEF
     */
    function adjustTotalSupplies (address[] memory rewardTokens, uint256[] memory amounts) external onlyAdmin {
        ...
        uint currentBalance = IBEP20(sefAddress).balanceOf(address(this));
