...
contract SEFVault is SEFVaultStorage, ECDSA, AccessControlledV5 {
    ...
    function _updatePool(address _rewardToken, uint256 _pid) internal {
        ...
        if (pool.totalSupply == 0) {
            // just in case to ensure we track the balance
            pool.totalSupply = IBEP20(pool.token).balanceOf(address(this));
        }
