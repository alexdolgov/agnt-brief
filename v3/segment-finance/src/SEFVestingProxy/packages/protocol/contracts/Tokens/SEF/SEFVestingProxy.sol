...
contract SEFVestingProxy is SEFVestingAdminStorage {
    /**
    ...
    /**
     * @notice Emitted when pendingImplementation is accepted, which means SEFVesting implementation is updated
     */
    ...
    constructor(
        ...
        address _sefAddress
    ) nonZeroAddress(implementation_) nonZeroAddress(_sefAddress) {
        ...
        delegateTo(implementation_, abi.encodeWithSignature("initialize(address)", _sefAddress));
    }
    ...
    function _setImplementation(address implementation_) public {
        require(msg.sender == admin, "SEFVestingProxy::_setImplementation: admin only");
        require(implementation_ != address(0), "SEFVestingProxy::_setImplementation: invalid implementation address");
    ...
    /**
     * @notice Accepts new implementation of SRT Vault. msg.sender must be pendingImplementation
