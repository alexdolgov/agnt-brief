...
error SupplyError(address supplier, address seToken, uint256 errorCode);

///@notice Error indicating that repaying to given market failed.
error RepayError(address repayer, address seToken, uint256 errorCode);
...
///@notice Error thrown when invalid seToken address is passed to swap router
error SeTokenNotListed(address seToken);

///@notice Error thrown when invalid underlying is passed as per given seToken
error SeTokenUnderlyingInvalid(address underlying);
