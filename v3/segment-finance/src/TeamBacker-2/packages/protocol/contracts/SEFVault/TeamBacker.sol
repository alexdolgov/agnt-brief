...
contract TeamBacker is Ownable2StepUpgradeable {
    ...
    function distribute(address recipient) external onlyOwner {
        ...
        swapToSEF(usdtBalance);

        uint sefBalance = IERC20(SEF).balanceOf(address(this));
        IERC20(SEF).transfer(recipient, sefBalance);
        emit BackerTransfer(recipient, sefBalance);
    }

    function swapTokensToUSDT () internal  {
        ...
        for (uint256 i = 0; i < tokens.length; i++) {
            ...
            uint amountOut = swapToUSDTFromV2(tokens[i], amount);
    ...
    function swapToUSDTFromV2 (address token, uint256 amount) internal returns (uint256){
    ...
    function swapToSEF (uint256 amount) internal {
        ...
        THENA_ROUTER.swapExactTokensForTokens(amount, 0, path, address(this), block.timestamp);
