...
contract TeamBacker is Ownable2StepUpgradeable {
    ...
    function distribute(address recipient, TExpectedSwap[] memory swaps, uint256 sefUsdtPrice) external onlyOwner {

        swapTokensToUSDT(swaps);

        uint usdtBalance = IERC20(USDT).balanceOf(address(this));
        swapToSEF(usdtBalance, sefUsdtPrice);
    ...
    function swapToSEF (uint256 amount, uint256 sefUsdtPrice) internal {
        ...
        uint minAmountOut = amount * 1 ether / sefUsdtPrice * 95 / 100;
