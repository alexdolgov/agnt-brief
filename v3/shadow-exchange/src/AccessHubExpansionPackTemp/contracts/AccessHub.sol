...
contract AccessHub is IAccessHub0, Initializable, AccessControlEnumerableUpgradeable {
    ...
    function preX33UpdateHook(address user) external {
        ...
        require(msg.sender == x33Operator || hasRole(PROTOCOL_OPERATOR, msg.sender), Errors.NOT_X33_ADAPTER());

        // poke votes on voter
        _voter.poke(user);

        // allow xShadow to be transfered to/from x33
        address[] memory exemptionList = new address[](1);
...
contract AccessHubExpansionPack1 is IAccessHub1, AccessHubExpansionPack {
    ...
    function rebaseX33() external {
        ...
        uint256 xShadowAmount = IERC4626(address(_x33.operator())).redeem(x33Balance, address(this), address(this));
    ...
    /// @inheritdoc IAccessHub1
    function flashExpand(address _flashExpansion, bytes memory data) external onlyRole(PROTOCOL_OPERATOR) {
