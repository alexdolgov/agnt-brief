...
contract Gauge is IGauge, ReentrancyGuard {
    ...
    mapping(address token => uint256 dust) public dustForReward;
    ...
    constructor(address _stake, address _voter) {
        ...
        tokenWhitelists.add(tempVoter.emissionsToken());
    ...
    function notifyRewardAmount(
        ...
        if (block.timestamp >= _rewardData[token].periodFinish) {
            ...
            dustForReward[token] =
                amount -
                ((_rewardData[token].rewardRate * DURATION) / PRECISION);
        } else {
