...
// File: contracts/MigrationToken2.sol
...
contract MigrationToken2 is CheckOper, ReentrancyGuard {

    using SafeMath for uint256;

    uint256 internal constant MAX = 1e4;
    uint256 public conversion = 1e5;
    IERC20 public from;
    IERC20 public to;

    constructor(address oper, address from_, address to_) CheckOper(oper) {
        from = IERC20(from_);
        to = IERC20(to_);
    }

    function convert(uint256 amount) external nonReentrant {
        uint balanceBefore = from.balanceOf(address(this));
        TransferHelper.safeTransferFrom(address(from), msg.sender, address(this), amount);
        uint balanceAfter = from.balanceOf(address(this));
        require(balanceAfter >= balanceBefore, "TOKEN_TRANSFER_IN_OVERFLOW");
        amount = (balanceAfter - balanceBefore).mul(conversion).div(MAX);
        TransferHelper.safeTransfer(address(to), msg.sender, amount);
    }

    function setFrom(address from_, address to_) external onlyOperator {
        from = IERC20(from_);
        to = IERC20(to_);
    }

    function setConversion(uint256 conversion_) external onlyOperator {
