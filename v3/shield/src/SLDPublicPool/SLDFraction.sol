...
library Fraction {
        ...
        returns (fractionNumber memory)
    {
        ...
        fractionNumber memory t = mul(xNum, fractionNumber(2316419, 1e7)); // p=2316419  t = 1 / (1 + p * x)
        t.numerator += t.denominator;
        t = invert(t); // t = 1 / (1 + p * x)
        xNum = mul(xNum, xNum);
        xNum.denominator *= -2; // -pow(x, 2) / 2
        xNum = exp(xNum); // pow(e, -pow(x, 2) / 2)
        xNum = mul(fractionNumber(39894228040143267793, 1e20), xNum); //39894228040143267793 = 1/sqrt(2π)
        ...
        returns (fractionNumber memory)
    {
        ...
        if (intValue > 0x386bfdba29) {
            fractionNumber memory _x1 = fractionNumber(
                _x.numerator / 2,
                _x.denominator
            );
            fractionNumber memory _x2 = fractionNumber(
                _x.numerator - _x1.numerator,
                _x.denominator
            );
