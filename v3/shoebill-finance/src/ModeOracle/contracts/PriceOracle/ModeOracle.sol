...
contract ModeOracle is PriceOracle, OwnableUpgradeable {
    ...
    // upgrade redstone
    IPriceCalculator public redStoneOracleCalculator; // fallback oracle

    mapping(address => address) underlyingFeed;
    ...
    function setRedstoneOracleCalculator(address _redStoneOracleCalculator) external onlyOwner {
        redStoneOracleCalculator = IPriceCalculator(_redStoneOracleCalculator);
    }

    function setUnderlyingFeed(address cToken, address feed) external onlyOwner {
        underlyingFeed[cToken] = feed;
    }
    ...
    function getRedstonePrice(CToken cToken) public view returns (uint256) {
        address asset = underlyingFeed[address(cToken)];

        return redStoneOracleCalculator.priceOf(asset);
    }

    function getPythPrice(CToken cToken) public view returns (uint256) {
    ...
    function getPrice(CToken cToken) public view override returns (uint256) {
        uint256 price = getRedstonePrice(cToken);
