...
contract BasicLens {
    function rewardsAccrued(ComptrollerLensInterface comptroller, address account)
        external
        returns (address[] memory rewardTokens, uint256[] memory accrued)
    {
        ...
        rewardTokens = RewardDistributor(externalRewardDistributor).getRewardTokens();
    ...
    function getBalancesInternal(address[] memory tokens, address account)
        internal
        view
