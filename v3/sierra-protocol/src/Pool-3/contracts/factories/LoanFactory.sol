...
contract LoanFactory is ILoanFactory, BeaconProxyFactory {
  ...
  function createLoan(
    ...
    require(msg.sender == IPool(pool).borrowerManagerAddr(), "LoanFactory: Caller not borrowerManager");
    require(borrower == IPool(pool).borrowerManagerAddr(), "LoanFactory: Caller not borrowerManager");
