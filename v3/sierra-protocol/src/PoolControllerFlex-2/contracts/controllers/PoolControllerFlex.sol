...
contract PoolControllerFlex is IPoolControllerFlex, BeaconImplementation {
  ...
  modifier onlyNotPaused() {
    require(IServiceConfigurationV5(serviceConfiguration).paused() == false, "Pool: Protocol paused");
    _;
  }
  ...
  modifier onlyPermittedAdmin() {
    require(IServiceConfigurationV5(serviceConfiguration).isPoolAdmin(msg.sender), "Pool: Only Pool Admin Allowed");
    _;
  }
  ...
  modifier atInitializedOrActiveState() {
    ...
    require(
      _currentState == IPoolLifeCycleStateFlex.Active || _currentState == IPoolLifeCycleStateFlex.Initialized,
  ...
  modifier atActiveOrClosedState() {
    ...
    require(
      _currentState == IPoolLifeCycleStateFlex.Active || _currentState == IPoolLifeCycleStateFlex.Closed,
  ...
  function updatePoolSettings(IPoolConfigurableSettingsFlex memory poolSettings_) external onlyAdmin {
    emit PoolSettingsUpdated();
    _settings = poolSettings_;
  }

  function updatePoolAdmin(address newAdmin_) external  {
    require(IServiceConfigurationV5(serviceConfiguration).isOperator(msg.sender) || admin == msg.sender, "Pool:Pool Admin or Operator Allowed");
    require(IServiceConfigurationV5(serviceConfiguration).isPoolAdmin(newAdmin_), "Pool: Only Pool Admin Allowed");
    admin = newAdmin_;
  }
  ...
  function _setState(IPoolLifeCycleStateFlex newState) internal {
    if (_state != newState) {
      if (newState == IPoolLifeCycleStateFlex.Active && pool.activatedAt() == 0) {
        pool.onActivated();
      }
