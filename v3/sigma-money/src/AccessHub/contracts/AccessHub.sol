    ...
    AccessControlEnumerableUpgradeable
{
    ...
    function initialize(InitParams calldata params) external initializer {
        /// @dev zero address checks (N4 fix)
        require(params.owner != address(0), ZERO_ADDRESS());
        require(params.operator != address(0), ZERO_ADDRESS());
        require(params.voter != address(0), ZERO_ADDRESS());
        require(params.gaugeEmission != address(0), ZERO_ADDRESS());
        require(params.xShadow != address(0), ZERO_ADDRESS());
        require(params.voteModule != address(0), ZERO_ADDRESS());
        require(params.sigmaGaugeFactory != address(0), ZERO_ADDRESS());
        require(params.veFunderGaugeFactory != address(0), ZERO_ADDRESS());
        require(params.sigmaFeeDistributorFactory != address(0), ZERO_ADDRESS());
    ...
    function setCooldownExemption(
        ...
        require(_candidates.length == _exempt.length, IVoter.LENGTH_MISMATCH());
    ...
    function setNewOwner(address _owner) external onlyOwner {
        ...
        /// @dev transfer AccessControl roles from old owner to new owner (N3 fix)
        _revokeRole(DEFAULT_ADMIN_ROLE, oldOwner);
        _revokeRole(PROTOCOL_OPERATOR, oldOwner);
        _grantRole(DEFAULT_ADMIN_ROLE, _owner);
        _grantRole(PROTOCOL_OPERATOR, _owner);
    ...
    function setNewOperator(address _operator) external onlyOwner {
        ...
        _revokeRole(PROTOCOL_OPERATOR, oldOperator);
    ...
    function upgradeContract(
        ...
        UUPSUpgradeable(_proxy).upgradeToAndCall(_newImplementation, _data);
    }
}
