  ...
  IFxUSDBasePool
{
  ...
  function updateRedeemCoolDownPeriod(uint256 newPeriod) external onlyRole(OPERATOR_ROLE) {
    _updateRedeemCoolDownPeriod(newPeriod);
  }

  function updateInstantRedeemFeeRatio(uint256 newRatio) external onlyRole(OPERATOR_ROLE) {
