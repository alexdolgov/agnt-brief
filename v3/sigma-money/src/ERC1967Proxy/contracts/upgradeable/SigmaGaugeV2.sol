...
/**
 * @title SigmaGaugeV2
 * @notice Upgradeable version of the SigmaGauge contract using UUPS proxy pattern
 * @dev Implements Requirements 5.1, 5.2, 5.3
 * 
 * Storage Layout:
 ...
 * 
 * Key difference from GaugeV2:
 * - SigmaGaugeV2 only whitelists shadow and xShadow tokens (no pool token0/token1)
 * - Used by SigmaGaugeFactory for Sigma-specific gauge creation
 */
contract SigmaGaugeV2 is IGauge, Initializable, ReentrancyGuardUpgradeable, UUPSUpgradeable {
    using EnumerableSet for EnumerableSet.AddressSet;

    // ============ Storage Variables (Order Preserved from Original SigmaGauge.sol) ============
    // Requirements 5.2, 5.3: Storage layout must be preserved exactly
    ...
    // Requirement 5.2: Constructor logic moved to initialize function

    /**
     * @notice Initializes the SigmaGaugeV2 contract
     * @dev Can only be called once due to initializer modifier
     * @dev Unlike GaugeV2, this only whitelists shadow and xShadow (no pool tokens)
     * @param _stake Address of the LP token to stake
     * @param _voter Address of the voter contract
     */
    function initialize(address _stake, address _voter) external initializer {
