...
/**
 * @title XShadowV2
 * @notice Upgradeable version of the XShadow contract using UUPS proxy pattern
 * @dev Implements Requirements 11.1, 11.2, 11.3, 11.4, 11.5, 11.6
 * 
 * Storage Layout:
 * - Inherits from ERC20Upgradeable (uses storage slots for balances, allowances, totalSupply, name, symbol)
 * - Inherits from PausableUpgradeable (uses storage slot for _paused)
 ...
 * 
 * Changes from XShadow.sol:
 * - ERC20 -> ERC20Upgradeable
 * - Pausable -> PausableUpgradeable
 * - Added UUPSUpgradeable inheritance
 * - immutable MINTER -> regular storage variable MINTER
 * - immutable ACCESS_HUB -> regular storage variable ACCESS_HUB
 * - immutable SHADOW -> regular storage variable SHADOW
 * - Constructor logic moved to initialize function
 * - Added _authorizeUpgrade for accessHub-only upgrades
 * - Added 50 storage gap slots
 */
contract XShadowV2 is Initializable, ERC20Upgradeable, PausableUpgradeable, UUPSUpgradeable, IXShadow {
    using EnumerableSet for EnumerableSet.AddressSet;

    // ============ Storage Variables (Order Preserved from Original XShadow.sol) ============
    // Requirements 11.4, 11.5, 11.6: Storage layout must be preserved exactly
    ...
    /// @inheritdoc IXShadow
    /// @dev Changed from immutable to regular storage variable (Requirement 11.2)
    address public MINTER;
    /// @inheritdoc IXShadow
    /// @dev Changed from immutable to regular storage variable (Requirement 11.2)
    address public ACCESS_HUB;

    /// @inheritdoc IXShadow
    address public VOTE_MODULE;
    ...
    // Requirement 11.3: Constructor logic moved to initialize function

    /**
     * @notice Initializes the XShadowV2 contract
    ...
    function initialize(
        ...
        __ERC20_init("xSigma", "xSigma");
        __Pausable_init();
        __UUPSUpgradeable_init();

        require(_shadow != address(0), ZERO());
        require(_operator != address(0), ZERO());
        require(_accessHub != address(0), ZERO());
        require(_minter != address(0), ZERO());
    ...
    // ============ Rebase Start Function ============

    function startRebase(address _voteModule, address _voter) external onlyGovernance {
        VOTE_MODULE = _voteModule;
        exempt.add(VOTE_MODULE);
        exemptTo.add(VOTE_MODULE);
        VOTER = IVoter(_voter);
        exempt.add(_voter);
        lastDistributedPeriod = VOTER.getPeriod();
        rebaseStarted = true;
    }
    ...
    function _update(
        ...
        if (_isExempted(from, to)) {
            _u = 1;
        } else if (rebaseStarted) {
            if (VOTER.isGauge(from) || VOTER.isFeeDistributor(from)){
