...
contract GaugeEmission is IGaugeEmission, Initializable, UUPSUpgradeable {
    ...
    function initialize(address _accessHub) external initializer {
        require(_accessHub != address(0), INVALID_CONTRACT());
        __UUPSUpgradeable_init();
        accessHub = _accessHub;
    }

    /// @dev UUPS upgrade authorization — only AccessHub (governance)
    ...
    /// @inheritdoc IGaugeEmission
    function startEmissions(address _voter) external onlyGovernance {
