...
contract LongPositionOperateFacet is MorphoFlashLoanFacetBase {
  ...
  function openOrAddPosition(
    ...
    positionId = IPoolManager(pool).operate(pool, positionId, int256(amountIn), int256(bnbUSDAmount));
  ...
  function _closeOrRemove(address pool, uint256 positionId, uint256 bnbUSDAmount, bytes calldata data) internal {
    ...
    // swap collateral token to other token
