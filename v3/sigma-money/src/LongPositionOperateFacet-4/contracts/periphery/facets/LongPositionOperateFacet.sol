...
contract LongPositionOperateFacet is MorphoFlashLoanFacetBase {
  ...
  /// @param paramsOut The parameters to convert bnbUSD to target token.
  ...
  function openOrAddPosition(
    ...
    uint256 collAmount = LibRouter.transferInAndConvert(params, IPool(pool).collateralToken());
    (bytes32 miscData, uint256 bnbUSDAmount) = abi.decode(data, (bytes32, uint256));

    if (positionId != 0) {
      IERC721(pool).transferFrom(msg.sender, address(this), positionId);
    }
    LibRouter.approve(IPool(pool).collateralToken(), poolManager, collAmount);
    positionId = IPoolManager(pool).operate(pool, positionId, int256(collAmount), int256(bnbUSDAmount));
    _checkPositionDebtRatio(pool, positionId, miscData);
    IERC721(pool).transferFrom(address(this), msg.sender, positionId);

    emit OpenOrAddPosition(pool, positionId, msg.sender, collAmount, bnbUSDAmount);

    // convert bnbUSD to other token
    LibRouter.convertAndTransferOut(paramsOut, bnbUSD, bnbUSDAmount, msg.sender);
  }
  ...
  function closeOrRemovePosition(
    ...
    // swapt source token to bnbUSD
    uint256 bnbUSDAmount = LibRouter.transferInAndConvert(params, bnbUSD);

    address collateralToken = IPool(pool).collateralToken();
    (bytes32 miscData, uint256 collAmount) = abi.decode(data, (bytes32, uint256));
    ...
    if (bnbUSDAmount >= maxBnbUSD) {
      ...
      IPoolManager(pool).operate(pool, positionId, -int256(collAmount), -int256(bnbUSDAmount));
      _checkPositionDebtRatio(pool, positionId, miscData);
    }
    IERC721(pool).transferFrom(address(this), msg.sender, positionId);

    emit CloseOrRemovePosition(pool, positionId, msg.sender, collAmount, bnbUSDAmount);

    // convert collateral token to other token
    LibRouter.convertAndTransferOut(
      paramsOut,
      collateralToken,
    ...
    // transfer extra bnbUSD to revenue pool
    LibRouter.refundERC20(bnbUSD, LibRouter.routerStorage().revenuePool);
  }
