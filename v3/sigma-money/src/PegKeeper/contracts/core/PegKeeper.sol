...
contract PegKeeper is AccessControlUpgradeable, IPegKeeper {
  ...
  function updateConverter(address newConverter) external onlyRole(OPERATOR_ROLE) {
    _updateConverter(newConverter);
  }

  /// @notice Update the address of curve pool.
  /// @param newPool The address of curve pool.
  function updateCurvePool(address newPool) external onlyRole(OPERATOR_ROLE) {
  ...
  function updatePriceThreshold(uint256 newThreshold) external onlyRole(OPERATOR_ROLE) {
