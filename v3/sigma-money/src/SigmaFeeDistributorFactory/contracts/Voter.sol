...
contract Voter is IVoter, Initializable, ReentrancyGuardUpgradeable, UUPSUpgradeable {
    ...
    /// @dev Stores preallocation percentage for each gauge (SigmaGauge or VeFunderGauge) (using BASIS, max 1_000_000 = 100%)
    mapping(address gauge => uint256 preallocationBps) public gaugePreallocation;
    ...
    function initialize(
        ...
        /// @dev initialize inherited upgradeable contracts
    ...
    /**
     * @notice Initializes the Voter contract with accessHub parameter
     * @dev Use this for fresh deployments of the upgradeable contract
     * @param _accessHub Address of the access hub for permission control
     * @param _shadow Address of the SHADOW token
     * @param _veFunderGaugeFactory Address of the VeFunder gauge factory
     * @param _sigmaGaugeFactory Address of the Sigma gauge factory
     * @param _sigmaFeeDistributorFactory Address of the Sigma fee distributor factory
     * @param _gaugeEmission Address of the gaugeEmission contract
     * @param _msig Address of the multisig/governor
     * @param _xShadow Address of the xShadow token
     * @param _voteModule Address of the vote module
     * @param _maxTotalSigmaGaugePreallocation Maximum total preallocation for sigma gauges
     * @param _maxVeFunderGaugeCap Maximum cap for VeFunder gauges
     */
    ...
    /// @inheritdoc IVoter
    /// @notice Fixed poke function - only works for users who voted in the current period
    function poke(address user) external {
        ...
        /// @dev KEY FIX: only poke users who voted in the current period
        /// @dev prevents cross-period vote manipulation
        if (_lastVoted != period) return;
        ...
        /// @dev reset the votes since user voted in current period
        _reset(user);
    ...
    function killGauge(address _gauge) public onlyGovernance {
        ...
        for (uint256 period = _lastDistro; period <= currentPeriod; ++period) {
            ...
            if (!gaugePeriodDistributed[_gauge][period]) {
                ...
                /// @dev prevent gaugePeriodDistributed being marked true when the gaugeEmission hasn't updated yet
    ...
    function reviveGauge(address _gauge) public onlyGovernance {
        ...
        require(
            _getCurrentTotalGaugePreallocation() <= maxTotalSigmaGaugePreallocation,
    ...
    function createSigmaGauge(address _pool, uint256 _preallocationBps) external onlyGovernance returns (address) {
        ...
        /// @dev check total preallocation limit
        uint256 currentTotalPreallocation = _getCurrentTotalGaugePreallocation();
        require(
            currentTotalPreallocation + _preallocationBps <= maxTotalSigmaGaugePreallocation,
            TOTAL_PREALLOCATION_EXCEEDS_MAX()
        );
        
        /// @dev create the feeDist via factory from the feeDistributor
        ...
        gaugePreallocation[_gauge] = _preallocationBps;
    ...
    function createVeFunderGauge(address _receiver, uint256 _maxEmission, address _pool, uint256 _preallocationBps) external onlyGovernance returns (address) {
        /// @dev ensure there is no gauge for the pool
        require(
            gaugeForPool[_pool] == address(0),
            ACTIVE_GAUGE(gaugeForPool[_pool])
        ...
        /// @dev set preallocation percentage for this ve funder gauge
        gaugePreallocation[_gauge] = _preallocationBps;
        /// @dev set the last distribution as the current period
        lastDistro[_gauge] = getPeriod();
        /// @dev emit the gauge creation event
    ...
    /// @dev Sets preallocation percentage for a SigmaGauge or VeFunderGauge
    function setGaugePreallocation(
        address _gauge,
        uint256 _preallocationBps
    ) external onlyGovernance {
        require(gauges.contains(_gauge), GAUGE_INACTIVE(_gauge));
        require(isSigmaGauge[_gauge] || isVeFunderGauge[_gauge], NOT_PREALLOCATABLE_GAUGE());
        
        uint256 oldPreallocation = gaugePreallocation[_gauge];
        ...
        emit GaugePreallocationChanged(_gauge, oldPreallocation, _preallocationBps);
    }
    ...
    function notifyRewardAmount(uint256 amount) external {
        /// @dev gate to gaugeEmission which prevents bricking distribution
        require(msg.sender == gaugeEmission, NOT_AUTHORIZED(msg.sender));
    ...
    function distribute(address _gauge) public nonReentrant {
        ...
        IGaugeEmission(gaugeEmission).updatePeriod();
        /// @dev fetch the last distribution
        uint256 _lastDistro = lastDistro[_gauge];
        /// @dev fetch the current period
        uint256 currentPeriod = getPeriod();
    ...
    function _claimablePerPeriod(
        ...
        // Check if this gauge has preallocation (SigmaGauge or VeFunderGauge)
        if ((isSigmaGauge[gauge] || isVeFunderGauge[gauge]) && gaugePreallocation[gauge] > 0) {
            preallocationAmount = (totalReward * gaugePreallocation[gauge]) / BASIS;
        }
    ...
    /// @dev calculates total preallocation for all SigmaGauges and VeFunderGauges in a period
    function _getTotalPreallocationForPeriod(uint256 period) internal view returns (uint256) {
        ...
        for (uint256 i = 0; i < gaugesLength; i++) {
            ...
            if (isAlive[gauge] && (isSigmaGauge[gauge] || isVeFunderGauge[gauge])) {
                totalPreallocation += (totalReward * gaugePreallocation[gauge]) / BASIS;
            }
        }
    ...
    /// @dev calculates current total preallocation for all active gauges (SigmaGauges and VeFunderGauges)
    function _getCurrentTotalGaugePreallocation() internal view returns (uint256) {
        uint256 totalPreallocation = 0;
        uint256 gaugesLength = gauges.length();
        ...
        for (uint256 i = 0; i < gaugesLength; i++) {
            address gauge = gauges.at(i);
            if (isAlive[gauge] && (isSigmaGauge[gauge] || isVeFunderGauge[gauge])) {
                totalPreallocation += gaugePreallocation[gauge];
            }
        }
