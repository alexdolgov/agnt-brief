...
contract Voter is IVoter, ReentrancyGuard, Initializable {
    ...
    /// @dev backward compatibility: returns the preallocation for a gauge (renamed from sigmaGaugePreallocation)
    function sigmaGaugePreallocation(address _gauge) external view returns (uint256) {
        return gaugePreallocation[_gauge];
    }
}
