...
contract DynaLiquidityRegistry is AccessControl {
	...
	error MissingDynaLiquidityAddress();
	...
	function requireRecord(address dynaLiquidityAddress) internal view {
		if (dynaLiquidityList.length == 0 || dynaLiquidityList[liquidityIndex[dynaLiquidityAddress]].liquidity != dynaLiquidityAddress) revert NotRegistered();
	}
	...
	function registerDynaLiquidity(address dynaLiquidityAddress, DynaLiquidityType dynaLiquidityType) external onlyRole(REGISTRY_ADMIN) {
		if (isRegistered[dynaLiquidityAddress]) revert AlreadyRegistered();
		if (dynaLiquidityAddress == address(0)) revert MissingDynaLiquidityAddress();
		DynaLiquidityRecord memory record = DynaLiquidityRecord({liquidity: dynaLiquidityAddress, dynaLiquidityType: dynaLiquidityType, active: false});
		isRegistered[dynaLiquidityAddress] = true;
		liquidityIndex[dynaLiquidityAddress] = dynaLiquidityList.length;
		address poolTokenAddress = DynaLiquidityAPI(dynaLiquidityAddress).pool();
