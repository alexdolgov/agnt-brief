...
contract DynaRouterRegistry is IDynaRouterRegistryAPI, AccessControl {
	...
	event UpdatedPairRoutes(address tokenIn, address tokenOut, Route[] newRoutes);
	event UpdatedDirectionalPairRoutes(address tokenIn, address tokenOut, Route[] newRoutes);
	...
	/**
	 * @notice Sets the bi directional routes to use for a specific token pair
	 * @dev Only DEFAULT_ADMIN_ROLE role can call this
	 * @param tokenIn The address of the token
	 * @param tokenOut The address of the token
	 * @param newRoutes The addresses of the routers
	 */
	function setPairRoutes(address tokenIn, address tokenOut, Route[] memory newRoutes) external onlyRole(DEFAULT_ADMIN_ROLE) {
		delete pairRoutes[tokenIn][tokenOut];
		delete pairRoutes[tokenOut][tokenIn];

		for (uint i = 0; i < newRoutes.length; i++) {
			pairRoutes[tokenIn][tokenOut].push(newRoutes[i]);
			pairRoutes[tokenOut][tokenIn].push(newRoutes[i]);
		}

		emit UpdatedPairRoutes(tokenIn, tokenOut, newRoutes);
	}

	/**
	 * @notice Sets the directional routes to use for a specific token pair
	 * @dev Only DEFAULT_ADMIN_ROLE role can call this
	 * @param tokenIn The address of the token
	 * @param tokenOut The address of the token
	 * @param newRoutes The addresses of the routers
	 */
	function setDirectionalPairRoutes(address tokenIn, address tokenOut, Route[] memory newRoutes) external onlyRole(DEFAULT_ADMIN_ROLE) {
		delete pairRoutes[tokenIn][tokenOut];
		...
		emit UpdatedDirectionalPairRoutes(tokenIn, tokenOut, newRoutes);
	}
