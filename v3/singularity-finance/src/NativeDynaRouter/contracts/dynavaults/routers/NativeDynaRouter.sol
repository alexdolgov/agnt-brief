 ...
 */
contract NativeDynaRouter is INativeDynaRouterAPI, AccessControl {
	...
	function initialize(address wrappedNativeAddress, address metaRouterAddress) external onlyRole(DEFAULT_ADMIN_ROLE) {
		WRAPPED_NATIVE.isNotAlreadyInitialized();
...
<	 */
	 ...
	 */
	function spenderAllowance(address token) external view override returns (uint256 allowed) {
	...
	function previewSwap(
		...
		if (tokenIn == USE_NATIVE && tokenOut != WRAPPED_NATIVE) {
			(amountOut, swapRouter, routerSwapData) = IMetaDynaRouterAPI(metaRouter).previewSwap(WRAPPED_NATIVE, amountIn, tokenOut);
		...
		if (tokenIn != WRAPPED_NATIVE && tokenOut == USE_NATIVE) {
			(amountOut, swapRouter, routerSwapData) = IMetaDynaRouterAPI(metaRouter).previewSwap(tokenIn, amountIn, WRAPPED_NATIVE);
			swapData = _encodeSwapData(amountOut, swapRouter, routerSwapData);
			return (amountOut, router, swapData);
		}
	}
	 ...
	 */
	function swap(address tokenIn, uint256 amountIn, address tokenOut, uint256 minAmountOut, address to, bytes32[] memory swapData) external payable override {
		// wrap native
		if (tokenIn == USE_NATIVE && tokenOut == WRAPPED_NATIVE) {
		...
		// unwrap native
		if (tokenIn == WRAPPED_NATIVE && tokenOut == USE_NATIVE) {
			IWETH(WRAPPED_NATIVE).transferFrom(msg.sender, address(this), amountIn);
		...
		if (tokenIn == USE_NATIVE && tokenOut != WRAPPED_NATIVE) {
			...
			_approveTokenIfNeeded(WRAPPED_NATIVE, metaRouter, amountIn);
			uint256 tokenBalanceBefore = IERC20(tokenOut).balanceOf(address(this));
			IMetaDynaRouterAPI(metaRouter).swap(WRAPPED_NATIVE, amountIn, tokenOut, minAmountOutRouter, tokenRouter, address(this), routerSwapData);
		...
		if (tokenIn != WRAPPED_NATIVE && tokenOut == USE_NATIVE) {
			...
			IMetaDynaRouterAPI(metaRouter).swap(tokenIn, amountIn, WRAPPED_NATIVE, minAmountOutRouter, tokenRouter, address(this), routerSwapData);
