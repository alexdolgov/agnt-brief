 ...
 */
abstract contract BaseDynaRouter is DynaRouterAPI, AccessControl {
	 ...
	 * @param selectedRouter The router used
	 * @param token The token to check
	 * @return spender The address of the spender
	 * @return allowed The amount of token allowed to spend
	 */
	function spenderAllowance(address selectedRouter, address token) external view virtual override returns (address spender, uint256 allowed) {
		spender = DynaRouterAPI(selectedRouter).getSpender();
