...
library VaultStrategiesLib {
	...
	function addStrategy(address strategy, uint256 minDebtPerHarvest, uint256 maxDebtPerHarvest, uint256 performanceFee) external {
		...
		StrategyStorage storage _storage = strategyStorage();
		require(_storage.strategies[strategy].activation == 0, "ERR_DUPLICATE");
		address _want = DynaStrategyAPI(strategy).want();
	...
	function migrateStrategy(address oldVersion, address newVersion) external {
		...
		require(strategyOldVersion.want == DynaStrategyAPI(newVersion).want(), "!want");
		...
		DynaStrategyAPI(oldVersion).migrate(newVersion);
	...
	function increaseStrategyDebtRatioByInvestmentAmount(address strategy, uint256 amount) internal {
		...
		uint256 strategy_debtLimit = FixedPointMathLib.fullMulDiv(_storage.strategies[strategy].debtRatio, tokenTotalAssets, MAX_DEBT_RATIO);
		uint256 newStrategyDebtRatio = FixedPointMathLib.fullMulDiv(strategy_debtLimit + amount, MAX_DEBT_RATIO, tokenTotalAssets);
		// update debt ratio
	...
	function decreaseStrategyDebtRatioByInvestmentAmount(address strategy, uint256 amount, uint256 givenTokenTotalAssets) internal {
		...
		uint256 strategy_debtLimit = Math.max(
			FixedPointMathLib.fullMulDiv(_storage.strategies[strategy].debtRatio, givenTokenTotalAssets, MAX_DEBT_RATIO),
			amount
		);
		// update debt ratio
		_setNewStrategyDebtRatio(strategy, ((strategy_debtLimit - amount) * MAX_DEBT_RATIO) / givenTokenTotalAssets);
	}
	...
	function debtOutstanding(address strategy) public view returns (uint256) {
		...
		uint256 strategy_debtLimit = (_storage.strategies[strategy].debtRatio * VaultTokensLib.totalAssets(_storage.strategies[strategy].want)) /
			MAX_DEBT_RATIO;
		//
		uint256 strategy_totalDebt = _storage.strategies[strategy].totalDebt;
	...
	function creditAvailable(address strategy) public view returns (uint256) {
		StrategyStorage storage _storage = strategyStorage();
		if (_storage.emergencyShutdown) {
			return 0;
		}
		address _want = _storage.strategies[strategy].want;

		uint256 vault_tokenDebt = VaultTokensLib.tokenDebt(_want);
		uint256 vault_totalAssets = VaultTokensLib.tokenIdle(_want) + vault_tokenDebt;
		uint256 vault_debtLimit = FixedPointMathLib.fullMulDiv(_storage.tokenDebtRatio[_want], vault_totalAssets, MAX_DEBT_RATIO);

		uint256 strategy_debtLimit = FixedPointMathLib.fullMulDiv(_storage.strategies[strategy].debtRatio, vault_totalAssets, MAX_DEBT_RATIO);
		uint256 strategy_totalDebt = _storage.strategies[strategy].totalDebt;

		uint256 strategy_minDebtPerharvest = _storage.strategies[strategy].minDebtPerHarvest;
		uint256 strategy_maxDebtPerHarvest = _storage.strategies[strategy].maxDebtPerHarvest;

		// Exhausted credit line
		if (strategy_debtLimit <= strategy_totalDebt || vault_debtLimit <= vault_tokenDebt) {
			return 0;
		}
		// Start with debt limit left for the Strategy
		uint256 available = strategy_debtLimit - strategy_totalDebt;
		// Adjust by the global debt limit left
		available = Math.min(available, vault_debtLimit - vault_tokenDebt);
	...
	function registerLoss(address strategyAddress, uint256 loss) external {
		...
		if (_storage.strategies[strategyAddress].debtRatio != 0) {
			...
			uint256 newDebtRatio = ((strategyTotalDebt - loss) * MAX_DEBT_RATIO) / (VaultTokensLib.totalAssets(_want) - loss);
	...
	function calcFees(
...

		{
			...
			uint256 delegatedAssets = DynaStrategyAPI(strategyAddress).delegatedAssets();
	...
	function ethToWant(uint256 amount, address wethAddress, address strategyAddress) external view returns (uint256 value) {
		...
		(uint256 price, ) = IReferenceAssetOracle(DynaVaultAPI(VaultGovernanceLib.vault()).referenceAssetOracle()).getPrice(wethAddress, _want);
