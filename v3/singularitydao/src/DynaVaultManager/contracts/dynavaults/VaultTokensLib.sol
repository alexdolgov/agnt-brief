...
library VaultTokensLib {
	...
	uint8 private constant MAXIMUM_MAXTOKENS = 25;
	uint8 private constant MAXIMUM_STRATEGIES = 12;
	uint256 private constant PRECISION = 10 ** 18;

	uint256 internal constant MAX_BPS = 1e18;

	/** @dev Struct to store token-specific statistics and parameters
	...
	/** @dev Struct to store token-related storage variables
	...
	/** @dev Returns a struct storage pointer for accessing the state variables
	...
	/** @dev Initializes the token storage with the first token
	...
	function initializeTokens(address depositToken) external {
		...
		require(_storage.tokens.length == 0, "ERR_REINIT");
	...
	/** @dev Returns the index of a token in the tokens array
	 * @param tokenAddress Address of the token
	 * @return uint256 Index of the token
	 */
	function tokenIndex(address tokenAddress) external view returns (uint256) {
		TokenStorage storage _storage = tokenStorage();
		uint256 index = _storage.tokenIndex[tokenAddress];

		if (index != 0 || tokenAddress == _storage.tokens[0]) {
			return index;
		}
	...
	/** @dev Returns the address of the main deposit token
	...
	/** @dev Returns the number of tokens in the vault
	...
	/** @dev Returns the idle and debt amounts for a given token
	 * @param tokenAddress Address of the token
	...
	/** @dev Returns the deposit debt for a given token
	 * @param tokenAddress Address of the token
	...
	/** @dev Returns the target deposit debt ratio for a given token
	 * @param tokenAddress Address of the token
	...
	/** @dev Returns the amount of idle tokens for a given token
	 * @param tokenAddress Address of the token
	...
	/** @dev Increases the amount of idle tokens for a given token
	 * @param tokenAddress Address of the token
	...
	/** @dev Decreases the amount of idle tokens for a given token
	 * @param tokenAddress Address of the token
	...
	function decreaseTokenIdle(address tokenAddress, uint256 amount) external {
		...
		if (tokenIdleAmount > amount) {
			unchecked {
				_amount = tokenIdleAmount - amount;
			}
		} else {
			_amount = 0;
		...
		_storage.tokenStats[tokenAddress].tokenIdle = _amount;
	}

	/** @dev Returns the amount of token debt for a given token
	 * @param tokenAddress Address of the token
	...
	/** @dev Increases the amount of token debt for a given token
	 * @param tokenAddress Address of the token
	 * @param amount Amount to increase by
	 */
	...
	/** @dev Decreases the amount of token debt for a given token
	 * @param tokenAddress Address of the token
	 * @param amount Amount to decrease by
	 */
	function decreaseTokenDebt(address tokenAddress, uint256 amount) external {
		...
		if (tokenDebtAmount > amount) {
			...
			unchecked {
				newAmount = tokenDebtAmount - amount;
			}
		}

		// assign tokenDebt value
		_storage.tokenStats[tokenAddress].tokenDebt = newAmount;
	}

	/** @dev Reports profit, loss, and balance for a given token
	...
	function reportProfitLossBalance(
		...
		if (lossInReferenceAsset != 0) {
			...
			if (tokenAddress != _storage.tokens[0]) {
				_storage.reserveDebt -= deltaDebt;
			}
		}
	}

	/** @dev Calculates the value for a given token amount in reference asset denomination
	 * @param tokenAddress Address of the token
	...
	function tokenReferenceValue(address tokenAddress, uint256 assets) public view returns (uint256 referenceValue) {
		(referenceValue, ) = IReferenceAssetOracle(DynaVaultAPI(VaultGovernanceLib.vault()).referenceAssetOracle()).tokenReferenceValue(tokenAddress, assets);
	}

	/** @dev Returns the address of the reference asset
	 * @return address Address of the reference asset
	 */
	function referenceAsset() public view returns (address) {
		return IReferenceAssetOracle(DynaVaultAPI(VaultGovernanceLib.vault()).referenceAssetOracle()).referenceAsset();
	}
	...
	/** @dev Internal function to update the last report for a given token
	 * @param tokenAddress Address of the token
	 */
	...
	/** @dev Returns an array of TokenStats for all tokens in the vault
	...
	/** @dev Reports the idle amount for a given token
	 * @param tokenAddress Address of the token
	 * @param assets Amount of assets to report as idle
	 */
	function reportedTokenIdle(address tokenAddress, uint256 assets) external {
		tokenStorage().tokenStats[tokenAddress].tokenIdle = assets;
	}

	/** @dev Deposits idle tokens for a given token
	 * @param tokenAddress Address of the token
	...
	/** @dev Handles depositDebt accounting when idle tokens are withdrawn
	 * @param tokenAddress Address of the token
	...
	/**
	 * @dev Checks if the given token is valid (either it's the deposit token or a registered reserve token)
	...
	function requireValidToken(address tokenAddress) private view {
		TokenStorage storage _storage = tokenStorage();
		require(_storage.tokenIndex[tokenAddress] != 0 || _storage.tokens[0] == tokenAddress, "ERR_INVALID_TOKEN");
	}
	 ...
	 * This value is configurable because gaslimits might differ per blockchain.
	 */
	function setMaxTokens(uint256 maxTokens) external {
		VaultGovernanceLib.onlyGovernance();
		TokenStorage storage _storage = tokenStorage();
		require(maxTokens <= MAXIMUM_MAXTOKENS && maxTokens >= _storage.tokens.length, "ERR_MAX_TOKENS");
		_storage.maxTokens = maxTokens;
	}

	/**
	 * @dev Checks if a token exists in the vault's token list
	...
	function addReserveToken(address tokenAddress) external {
		...
		TokenStorage storage _storage = tokenStorage();
		require(!tokenExists(tokenAddress), "ERR_DUPLICATE");
		require(_storage.tokens.length < _storage.maxTokens, "ERR_MAX_TOKENS");
		...
		DynaVaultAPI(VaultGovernanceLib.vault()).approveAddedToken(tokenAddress);
	}
	...
	function removeReserveToken(address tokenAddress) external {
		...
		TokenStorage storage _storage = tokenStorage();
		TokenStats storage _stats = _storage.tokenStats[tokenAddress];
		require(_storage.tokenIndex[tokenAddress] != 0, "ERR_REMOVE");
		require(_stats.tokenIdle == 0, "ERR_TOKEN_IDLE");
		require(_stats.tokenDebt == 0, "ERR_TOKEN_DEBT");
		require(_stats.depositDebt == 0, "ERR_DEPOSIT_DEBT");
		uint256 index = _storage.tokenIndex[tokenAddress];
	 ...
	 */
	function totalAssets(address tokenAddress) external view returns (uint256) {
		TokenStats storage _stats = tokenStorage().tokenStats[tokenAddress];
		return _stats.tokenIdle + _stats.tokenDebt;
	}

	/**
	 * @dev Increases the deposit debt for the deposit token
	...
	/**
	 * @dev Decreases the deposit debt for the deposit token
	...
	/**
	 * @dev Updates debt after a token swap
	...
	/**
	 * @dev Adds a strategy to the token strategies list for a given token
	 * @param tokenAddress Address of the token
	...
	/**
	 * @dev Replaces a strategy in the token strategies list for a given token
	 * @param tokenAddress Address of the token
	 * @param oldStrategy Address of the strategy to replace
	...
	/**
	 * @dev Removes a strategy from the token strategies list for a given token
	 * @param tokenAddress Address of the token
	...
	/**
	 * @dev Returns the total deposit debt
	 * @return uint256 Total deposit debt (idle + debt + reserve debt)
	 */
	function totalDepositDebt() external view returns (uint256) {
		TokenStorage storage _storage = tokenStorage();
		address depositToken = _storage.tokens[0];
		return _storage.tokenStats[depositToken].tokenIdle + _storage.tokenStats[depositToken].tokenDebt + _storage.reserveDebt;
	}
}
