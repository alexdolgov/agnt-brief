...
interface VaultManagerAPI {
	...
	function simulateUnlockedFundsRatio(VaultSimulatorAPI.VaultSnapshot memory snapshot) external view returns (uint256);

	function simulatedFreeFunds(VaultSimulatorAPI.VaultSnapshot memory snapshot) external view returns (uint256);

	function takeStrategiesSnapshot(address tokenAddress) external view returns (VaultSimulatorAPI.VaultStrategySnapshot[] memory snapshots);
}
