...
interface VaultSimulatorAPI {
	...
	function simulatedMint(uint256 sharesNotIncludingFees, VaultSimulatorAPI.VaultSnapshot memory snapshot) external view returns (uint256 assetsIncludingFees);

	function simulatedWithdraw(uint256 assets, VaultSimulatorAPI.VaultSnapshot memory snapshot) external view returns (uint256 shares);
