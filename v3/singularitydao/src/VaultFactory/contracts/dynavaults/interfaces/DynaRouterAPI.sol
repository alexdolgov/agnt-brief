 ...
 * 3. call spenderAllowance using the returned selected router by previewswap and the input token whic returns:
 *    - address which is going to spend your input tokens when you execute the swap
 *    - current allowed amount of input tokens that can be spend by the given spender
 * 4. call to increases allowance of the spender, when the current allowed amount is less than the amount you want to swap
