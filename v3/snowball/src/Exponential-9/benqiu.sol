...
contract StrategyBenqiEth is StrategyBase, Exponential {
    ...
    function getSuppliedView() public view returns (uint256) {
        (, uint256 qiTokenBal, , uint256 exchangeRate) = IQiToken(qieth)
    ...
    function getBorrowedView() public view returns (uint256) {
        return IQiToken(qieth).borrowBalanceStored(address(this));
    }
    ...
    function getMarketColFactor() public view returns (uint256) {
        (, uint256 colFactor) = IComptroller(comptroller).markets(qieth);
    ...
    */
    ...
    function getSupplied() public returns (uint256) {
        return IQiToken(qieth).balanceOfUnderlying(address(this));
    }

    function getBorrowed() public returns (uint256) {
        return IQiToken(qieth).borrowBalanceCurrent(address(this));
    }
    ...
    function leverageUntil(uint256 _supplyAmount) public onlyKeepers {
        ...
        while (supplied < _supplyAmount) {
            ...
            IQiToken(qieth).borrow(_borrowAndSupply);
            deposit();
    ...
    function deleverageUntil(uint256 _supplyAmount) public onlyKeepers {
        ...
        do {
            ...
            require(
                IQiToken(qieth).redeemUnderlying(_redeemAndRepay) == 0,
                "!redeem"
            );
            IERC20(eth).safeApprove(qieth, 0);
            IERC20(eth).safeApprove(qieth, _redeemAndRepay);
            require(IQiToken(qieth).repayBorrow(_redeemAndRepay) == 0, "!repay");
    ...
    function deposit() public override {
        uint256 _want = IERC20(want).balanceOf(address(this));
        if (_want > 0) {
            IERC20(want).safeApprove(qieth, 0);
            IERC20(want).safeApprove(qieth, _want);
            require(IQiToken(qieth).mint(_want) == 0, "!deposit");
        }
    }
        ...
        returns (uint256)
    {
        ...
        if (_want < _amount) {
            ...
            require(IQiToken(qieth).getCash() >= _redeem, "!cash-liquidity");
            ...
            // Redeems underlying
            require(IQiToken(qieth).redeemUnderlying(_redeem) == 0, "!redeem");
        }
