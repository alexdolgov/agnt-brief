...
abstract contract StrategyEcdFarmBase is StrategyJoeBase {
    ...
    address public constant ecdPtp_staking = 0xc9AA91645C3a400246B9D16c8d648F5dcEC6d1c8; 
    uint256 poolId; 
    ...
    function balanceOfPool() public view override returns (uint256) {
        (uint256 amount, ) = IMasterChef(ecdPtp_staking).userInfo(
            poolId, 
            address(this)
        ); 
        return amount;
    ...
    function getHarvestable() external view returns (uint256) {
       return IMasterChef(ecdPtp_staking).pendingEcd(poolId, address(this));
    ...
    function deposit() public override {
        uint256 _want = IERC20(want).balanceOf(address(this));
        if (_want > 0) {
            IERC20(want).approve(ecdPtp_staking, 0);
            IERC20(want).approve(ecdPtp_staking, _want);
            IMasterChef(ecdPtp_staking).deposit(poolId, _want);
        }
    }

    /// @notice unstakes ecdptp back into unstaked ecdptp
    function _withdrawSome(uint256 _amount)
        internal
        override
        returns (uint256)
    {
      IMasterChef(ecdPtp_staking).withdraw(poolId, _amount);
      return _amount;
    }
...
// File contracts/strategies/echidna/strategy-ecd-ecdPtp.sol
