...
// File contracts/strategies/bases/strategy-kyber-base.sol

/// @notice This is a kyber base contract for KyberSwap
abstract contract StrategyKyberBase {
    ...
    address public constant knc = 0x39fC9e94Caeacb435842FADeDeCB783589F50f5f;
    address public constant qi = 0x8729438EB15e2C8B576fCc6AeCdA6A148776C0F5;

    // Dex
    address public constant kyberRouter = 0x8Efa5A9AD6D594Cf76830267077B78cE0Bc5A5F8; 
    address public constant kyberFactory = 0x10908C875D865C66f271F5d3949848971c9595C9;
    ...
    function bestPoolForSwap(IERC20 _from, IERC20 _to) internal view returns(address){
        address[] memory poolsPath = IKyberFactory(kyberFactory).getPools(_from, _to);

        address bestPool;
        uint256 highestKLast = 0;
        uint256 bestIndex = 0;
        for (uint i = 0; i < poolsPath.length; i++) {
            uint256 currentKLast = IDMMPool(poolsPath[i]).kLast();
    ...
    function _swapToken(
        ...
        if (_from == wavax || _to == wavax) {
            ...
            poolsPath[0] = bestPoolForSwap(IERC20(_from), IERC20(_to)); 
            ...
            poolsPath = new address[](2); 
            poolsPath[0] = bestPoolForSwap(IERC20(_from), IERC20(wavax));
            poolsPath[1] = bestPoolForSwap(IERC20(wavax), IERC20(_to));
        }

        IERC20(_from).safeApprove(kyberRouter, 0); 
        IERC20(_from).safeApprove(kyberRouter, _amount); 

        IKyber(kyberRouter).swapExactTokensForTokens(
            _amount,
            0,
            poolsPath,
            path,
...
// File contracts/strategies/bases/strategy-kyber-farm-base.sol
...
// File contracts/strategies/kyber/strategy-ky-savax-avax.sol
