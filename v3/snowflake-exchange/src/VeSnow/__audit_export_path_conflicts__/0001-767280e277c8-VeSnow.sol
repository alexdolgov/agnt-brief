...
import './interfaces/ISnowflakeNFT.sol';

/// @title VeSnow
/// @notice Snowflake Venom: the staking contract for SNOW, as well as the token used for governance.
/// Note Venom does not seem to hurt the Snowflake, it only makes it stronger.
/// Allows depositing/withdraw of snow and staking/unstaking ERC721.
/// Here are the rules of the game:
/// If you stake snow, you generate veSnow at the current `generationRate` until you reach `maxCap`
/// If you unstake any amount of snow, you loose all of your veSnow.
    ...
    IVeSnow
{
    ...
    struct UserInfo {
        uint256 amount; // snow staked by user
        uint256 lastRelease; // time of last veSnow claim or first deposit if user has not claimed yet
        // the id of the currently staked nft
        // important: the id is offset by +1 to handle tokenID = 0
        uint256 stakedNftId;
    }

    /// @notice the snow token
    IERC20 public snow;

    /// @notice the masterSnowflake contract
    IMasterSnowflake public masterSnowflake;

    /// @notice the NFT contract
    ISnowflakeNFT public nft;

    /// @dev Magic value for onERC721Received
    /// Equals to bytes4(keccak256("onERC721Received(address,address,uint256,bytes)"))
    bytes4 private constant ERC721_RECEIVED = 0x150b7a02;

    /// @notice max veSnow to staked snow ratio
    /// Note if user has 10 snow staked, they can only have a max of 10 * maxCap veSnow in balance
    uint256 public maxCap;

    /// @notice the rate of veSnow generated per second, per snow staked
    ...
    /// @dev contract addresses are by default unable to stake snow, they must be previously whitelisted to stake snow
    ...
    function initialize(
        ...
        IERC20 _snow,
        IMasterSnowflake _masterSnowflake,
        ISnowflakeNFT _nft
    ) public initializer {
        require(address(_masterSnowflake) != address(0), 'zero address');
        require(address(_snow) != address(0), 'zero address');
        ...
        // set generationRate (veSnow per sec per snow staked)
        ...
        // set master snowflake
        masterSnowflake = _masterSnowflake;
    ...
    /// @param _masterSnowflake the new masterSnowflake address
    function setMasterSnowflake(IMasterSnowflake _masterSnowflake) external onlyOwner {
        require(address(_masterSnowflake) != address(0), 'zero address');
        masterSnowflake = _masterSnowflake;
    }
    ...
    function setNftAddress(ISnowflakeNFT _nft) external onlyOwner {
    ...
    /// @notice checks wether user _addr has snow staked
    /// @param _addr the user address to check
    /// @return true if the user has snow in stake, false otherwise
    function isUser(address _addr) public view override returns (bool) {
        return users[_addr].amount > 0;
    }

    /// @notice returns staked amount of snow for user
    /// @param _addr the user address to check
    /// @return staked amount of snow
    function getStakedSnow(address _addr) external view override returns (uint256) {
    ...
    /// @notice deposits SNOW into contract
    /// @param _amount the amount of snow to deposit
    function deposit(uint256 _amount) external override nonReentrant whenNotPaused {
        ...
        if (isUser(msg.sender)) {
            // if user exists, first, claim his veSNOW
            _claim(msg.sender);
        ...
        // Request Snow from user
        snow.safeTransferFrom(msg.sender, address(this), _amount);
    }
    ...
    /// @notice claims accumulated veSNOW
    ...
    /// @notice Calculate the amount of veSNOW that can be claimed by user
    /// @param _addr the address to check
    /// @return amount of veSNOW that can be claimed by user
    ...
    function _claimable(address _addr) private view returns (uint256) {
        ...
        // get user's veSNOW balance
        uint256 userVeSnowBalance = balanceOf(_addr);

        // user veSNOW balance cannot go above user.amount * maxCap
        uint256 maxVeSnowCap = user.amount * maxCap;

        // first, check that user hasn't reached the max limit yet
        if (userVeSnowBalance < maxVeSnowCap) {
            // then, check if pending amount will make user balance overpass maximum amount
            if ((userVeSnowBalance + pending) > maxVeSnowCap) {
                return maxVeSnowCap - userVeSnowBalance;
            } else {
                return pending;
            }
        }
    ...
    /// @notice withdraws staked snow
    /// @param _amount the amount of snow to unstake
    /// Note Beware! you will loose all of your veSNOW if you unstake any amount of snow!
    function withdraw(uint256 _amount) external override nonReentrant whenNotPaused {
        ...
        // update his balance before burning or sending back snow
        users[msg.sender].amount -= _amount;

        // get user veSNOW balance that must be burned
        uint256 userVeSnowBalance = balanceOf(msg.sender);

        _burn(msg.sender, userVeSnowBalance);

        // send back the staked snow
        snow.safeTransfer(msg.sender, _amount);
    }

    /// @notice hook called after token operation mint/burn
    /// @dev updates masterSnowflake
    /// @param _account the account being affected
    /// @param _newBalance the newVeSnowBalance of the user
    function _afterTokenOperation(address _account, uint256 _newBalance) internal override {
        masterSnowflake.updateFactor(_account, _newBalance);
    }

    /// @notice This function is called when users stake NFTs
    /// When Snowflake NFT sent via safeTransferFrom(), we regard this action as staking the NFT
    ...
    function onERC721Received(
        ...
        require(msg.sender == address(nft), 'only snowflake NFT can be received');
    ...
    function getVotes(address _account) external view virtual override returns (uint256) {
        uint256 veSnowBalance = balanceOf(_account);

        // check that user has more than voting treshold of maxCap and has snow in stake
        if (veSnowBalance * invVoteThreshold > users[_account].amount * maxCap && isUser(_account)) {
            return veSnowBalance;
        } else {
            return 0;
        }
    }
