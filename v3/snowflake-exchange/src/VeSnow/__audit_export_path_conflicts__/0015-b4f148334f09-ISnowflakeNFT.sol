...
interface ISnowflakeNFT is IERC721Enumerable, IERC2981Royalties {
    struct Snowflake {
        uint32 power;
        uint16 level;
        uint16 score;
    ...
    function getSnowflakeDetails(uint256 tokenId)
        external
        view
        returns (
            uint16 level,
    ...
    function snowflakeesLength() external view returns (uint256);
    ...
    event SnowflakeCreation(uint256 from, uint256 length);
