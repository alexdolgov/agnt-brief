...
contract RewardDistributor is RewardDistributorStorage, Exponential {
    ...
    function claimReward(uint8 rewardType, address payable holder, CToken[] memory cTokens) public {
        claimReward(rewardType, holder, cTokens, true, true);
    }

    /**
     * @notice Claim all COMP/ETH  accrued by the holders
     * @param rewardType  0 = COMP, 1 = ETH
     * @param holder The addresses to claim COMP/ETH for
     * @param cTokens The list of markets to claim COMP/ETH in
    ...
    function claimReward(
        ...
        for (uint256 i = 0; i < cTokens.length; i++) {
            ...
            if (borrowers == true) {
                Exp memory borrowIndex = Exp({mantissa: cToken.borrowIndex()});
                updateRewardBorrowIndex(rewardType, address(cToken), borrowIndex);
                distributeBorrowerReward(rewardType, address(cToken), holder, borrowIndex);
                claims += rewardAccruedBorrow[rewardType][address(cToken)][holder];
                rewardAccruedBorrow[rewardType][address(cToken)][holder] = 0;
            }
            if (suppliers == true) {
                updateRewardSupplyIndex(rewardType, address(cToken));
                distributeSupplierReward(rewardType, address(cToken), holder);
                claims += rewardAccruedSupply[rewardType][address(cToken)][holder];
                rewardAccruedSupply[rewardType][address(cToken)][holder] = 0;
            }
        }

        EIP20Interface reward = EIP20Interface(rewardAddresses[rewardType]);
        reward.transfer(holder, claims);
    }
