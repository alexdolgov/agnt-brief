...
contract AsyncVault is IERC7540, SyncVault {
     ...
     * @param assets The amount of assets requested by the user.
     */
    function decreaseDepositRequest(uint256 assets) external whenClosed whenNotPaused {
     ...
     * @param shares The amount of shares requested by the user.
     */
    function decreaseRedeemRequest(uint256 shares) external whenClosed whenNotPaused {
        address owner = _msgSender();
     ...
     * the vault.
     */
    function open(uint256 assetReturned) external override onlyOwner whenNotPaused whenClosed {
    ...
    function settle(uint256 newSavedBalance) external onlyOwner whenNotPaused whenClosed {
        (uint256 _lastSavedBalance, uint256 totalSupply) = _settle(newSavedBalance);
    ...
    function claimableRedeemRequest(address owner) external view returns (uint256) {
        ...
        return
            isCurrentEpoch(lastRequestId) ? 0 : epochs[lastRequestId].redeemRequestBalance[owner];
    }
    ...
    function claimableDepositRequest(address owner) external view returns (uint256 assets) {
        ...
        return
            isCurrentEpoch(lastRequestId) ? 0 : epochs[lastRequestId].depositRequestBalance[owner];
    }
     ...
     */
    function _settle(uint256 newSavedBalance) internal returns (uint256, uint256) {
        (
            uint256 assetsToOwner,
            uint256 assetsToVault,
            ,
        ...
        if (settleValues.pendingDeposit > settleValues.assetsToWithdraw) {
            _asset.safeTransferFrom(address(pendingSilo), owner(), assetsToOwner);
    ...
    function _checkMaxDrawdown(uint256 _lastSavedBalance, uint256 newSavedBalance) internal view {
        if (
            newSavedBalance <
            _lastSavedBalance.mulDiv(BPS_DIVIDER - _maxDrawdown, BPS_DIVIDER, Math.Rounding.Ceil)
        ) {
