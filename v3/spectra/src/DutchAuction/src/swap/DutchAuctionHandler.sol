...
contract DutchAuction is BaseConditionalOrder {
    ...
    function getTradeableOrder(
        ...
        uint256 bucketBuyAmount = data.startBuyAmount -
            (bucket * data.stepDiscount * data.startBuyAmount) /
            10000;
         ...
         */
        if (
            data.buyToken.balanceOf(GPv2Order.actualReceiver(order, owner)) >=
            data.buyTokenBalance + bucketBuyAmount
        ) {
    ...
    function _validateData(Data memory data) internal pure {
        ...
        if (data.numSteps * data.stepDiscount >= 10000) {
            revert OrderNotValid(ERR_MAX_TOTAL_DISCOUNT);
        }
    }
}
