    ...
    ReentrancyGuardUpgradeable
{
    ...
    function requestDeposit(
        ...
        if (!(owner == msg.sender || _getWrapperStorage().isOperator[owner][msg.sender])) {
    ...
    // checks if the requestId is the current epoch of amphor vault used by the wrapper
    function _isCurrentEpoch(uint256 requestId) internal view returns (bool) {
        return requestId == IAmphor(getInfraVault()).epochId();
    }

    // updates the internal epoch ID to match the infra vault's current epoch ID
    ...
    //Once per epoch the wrapper can claim the pending deposits from Amphor
    ...
    //Once per epoch the wrapper can claim the pending redeem requests from Amphor
