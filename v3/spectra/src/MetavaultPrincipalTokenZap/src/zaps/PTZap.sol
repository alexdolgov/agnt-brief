...
contract MetavaultPrincipalTokenZap {
    ...
    function redeemPTsInIBTs(
        ...
        if (isUnwrap) {
            uint256 unwrappedIBTs = ISpectra4626Wrapper(ibt).unwrap(
                IERC20(ibt).balanceOf(address(this)),
                msg.sender,
                address(this)
            );
            if (unwrappedIBTs < minTokenOut) {
                revert InsufficientIBTs(unwrappedIBTs, minTokenOut);
            } else {
                if (IERC20(ibt).balanceOf(address(this)) < minTokenOut) {
                    revert InsufficientIBTs(IERC20(ibt).balanceOf(address(this)), minTokenOut);
                }
                IERC20(ibt).transfer(msg.sender, IERC20(ibt).balanceOf(address(this)));
            }
        }
    }
