...
contract MetavaultsRegistry is IMetavaultsRegistry, Initializable, AccessManagedUpgradeable {
    ...
    function registerMetavault(address metavault, address[] memory markets) external restricted {
        if (metavault == address(0)) revert ZeroAddress();
    ...
    function allowBridgePath(
        ...
        _allowBridgePath(metavault, tokenIn, tokenOut, dstChainId, bridge);
    }

    /**
     * @dev See {IMetavaultsRegistry-allowBridgePathAsMetavault}.
     */
    function allowBridgePathAsMetavault(
        address tokenIn,
        address tokenOut,
        uint32 dstChainId,
        address bridge
    ) external {
        _allowBridgePath(msg.sender, tokenIn, tokenOut, dstChainId, bridge);
    }
    ...
    function removeBridgePathByIndex(
        ...
        if (index >= config.bridgePaths.length) {
            revert BridgePathIndexOutOfBounds(index, config.bridgePaths.length);
        }

        BridgePath memory path = config.bridgePaths[index];
        bytes32 pathKey = _getBridgePathKey(path.tokenIn, path.tokenOut, path.dstChainId, path.bridge);

        _removeBridgePathAtIndex(config, index);
        delete config.pathExists[pathKey];

        emit BridgePathRemoved(metavault, path.tokenIn, path.tokenOut, path.dstChainId, path.bridge);
    }
    ...
    function _allowBridgePath(
        address metavault,
        address tokenIn,
        address tokenOut,
        uint32 dstChainId,
        address bridge
    ) internal onlyRegistered(metavault) {
