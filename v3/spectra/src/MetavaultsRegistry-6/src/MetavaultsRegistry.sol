...
contract MetavaultsRegistry is IMetavaultsRegistry, Initializable, AccessManagedUpgradeable {
     ...
     */
    function allowBridgePath(address metavault, address tokenIn, address tokenOut,uint32 dstChainId, address bridge) external restricted onlyRegistered(metavault) {
        if (tokenIn == address(0) || tokenOut == address(0) || bridge == address(0)) revert ZeroAddress();
        // check if path is registered on this metavault
        MetavaultBridgeConfig storage config = _getMvBridgeStorage().metavaultBridgeConfigs[metavault];
        bytes32 key = keccak256(abi.encode(tokenIn, tokenOut, dstChainId));
        EnumerableSet.AddressSet storage bridges = config.bridgePaths[key];
        if (!bridges.add(bridge)) {
            revert BridgePathAlreadyAllowed(tokenIn, tokenOut, dstChainId, bridge);
        }

        emit BridgePathAllowed(tokenIn, tokenOut, dstChainId, bridge);
    }
    ...
    */
    function removeBridgePath(
        ...
        bytes32 key = keccak256(abi.encode(tokenIn, tokenOut, dstChainId));
        EnumerableSet.AddressSet storage bridges = config.bridgePaths[key];
        if (!bridges.remove(bridge)) {
            revert BridgePathNotAllowed(tokenIn, tokenOut, dstChainId, bridge);
        }
        emit BridgePathRemoved(tokenIn, tokenOut, dstChainId, bridge);
    }
    ...
    /**
     * @dev See {IMetavaultsRegistry-isPathAllowed}.
     */
    function isPathAllowed(
        ...
        return _getMvBridgeStorage().metavaultBridgeConfigs[metavault].bridgePaths[keccak256(abi.encode(tokenIn, tokenOut, dstChainId))].contains(bridge);
    }
