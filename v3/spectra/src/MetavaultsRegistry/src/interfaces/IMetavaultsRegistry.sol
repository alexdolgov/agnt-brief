...
interface IMetavaultsRegistry {
    ...
    error BridgePathAlreadyRegistered(
        address metavault,
        address tokenIn,
        address tokenOut,
        uint32 dstChainId,
    ...
    error BridgePathNotRegistered(address metavault, address tokenIn, address tokenOut, uint32 dstChainId, address bridge);

    error IndexOutOfBounds(uint256 index, uint256 length);

    error ChainIndexOutOfBounds(uint256 index, uint256 length);

    error BridgePathIndexOutOfBounds(uint256 index, uint256 length);
    ...
    struct MetavaultBridgeConfig {
        ...
        mapping(bytes32 => bool) pathExists;
        uint256 pathCount;
    }


    /**
    * @notice Elements defining bridge path the admin can allow
     */
    struct BridgePath {
        address tokenIn;
        address tokenOut;
    ...
    /**
     * @notice allow a bridge path from this chain to a destination chain for a specific token
     * @notice This function is used by a metavault.
     * @param tokenIn Address of the tokenIn to allow bridging
     * @param tokenOut Address of the tokenOut to bridge
     * @param dstChainId ID of the destination chain
     * @param bridge Address of the bridge contract
     ...
     */
    function removeBridgePathByIndex(address metavault, uint256 index) external;
    ...
    /**
    * @param metavault Address of the metavault
    * @return BridgePath Array of bridge paths that are allowed 
     */
    function getBridgePaths(
        address metavault
    ) external view returns (BridgePath[] memory);

    /**
    * @param metavault address of the metavault
    * @return number of paths registered for this metavault  
     */
