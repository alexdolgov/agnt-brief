 ...
 */
contract FDFPair is Initializable, ReentrancyGuardUpgradeable, IFDFPair, AccessControlUpgradeable, EIP712Upgradeable, UUPSUpgradeable, IERC1155Receiver, ERC165 {
    ...
    function _executeBuyTokens(
        ...
        (uint256[] memory currencySpent, uint256[] memory feeAmounts) = _swapCurrencyForPlayerTokens(
            _playerTokenIds, _playerTokenAmountsToBuy, _maxCurrencySpend, _recipient == address(0x0) ? msg.sender : _recipient);
    ...
    function _swapCurrencyForPlayerTokens(
        ...
        console.log("Transfering Player tokens to buyer");
            
            // Update EMA price to track buy activity
            uint256 buyFeeAmount = feeManager.getBuyFeeAndUpdateEMA(playerTokenId, buyPriceForOneToken);
        ...
        if (totalCurrencyRefund > 0) {
            currencyToken.safeTransfer(msg.sender, totalCurrencyRefund);
        }
    ...
    function _swapPlayerTokensForCurrency(
        ...
        for (uint256 i = 0; i < numberOfPlayerIds; i++) {
            ...
            // Tokens are already in the contract, so we need to calculate the original price.
            sellVars.currentPlayerTokenReserve = sellVars.currentPlayerTokenReserve - sellVars.saleAmount;
    ...
    function _calculateBuyPriceForTokens(
        ...
        (uint256 buyFeeRate, uint8 feeType) = feeManager.getBuyFeeSimulated();
