...
import {IPlayerV2} from "../../interfaces/upgrades/IPlayerV2.sol";
import {IERC20} from "@openzeppelin/contracts/token/ERC20/IERC20.sol";
import {SafeERC20} from "@openzeppelin/contracts/token/ERC20/utils/SafeERC20.sol";
import {IFeeManager} from "../../interfaces/IFeeManager.sol";
import {IFDFPair} from "../../interfaces/IFDFPair.sol";
import {IFDFPairV2} from "../../interfaces/upgrades/IFDFPairV2.sol";
import {ECDSA} from "@openzeppelin/contracts/utils/cryptography/ECDSA.sol";
import {Strings} from "@openzeppelin/contracts/utils/Strings.sol";

/**
 * @title PlayerV2 - V2 upgrade for Player contract
 * @notice Changes requestSupply function to be protected by MINTER_ROLE instead of SUPPLY_MANAGER
 */
contract PlayerV2 is Initializable, ERC1155Upgradeable, ERC1155HolderUpgradeable, AccessControlUpgradeable, UUPSUpgradeable, EIP712Upgradeable, IPlayerV2 {  
    ...
    bytes32 public constant SWAP_TYPEHASH = keccak256(
        "SwapTokens(address from,uint256[] playerTokenIdsIn,uint256[] playerTokenAmountsIn,uint256[] playerTokenIdsOut,address recepient,uint256 deadline,uint256 nonce)"
    );
    ...
    // Storage gap for future upgrades (reduced by 2 for upgradeVersion and swapsEnabledFrom)
    ...
    function initializeV2() external onlyRole(GOVERNOR_ROLE) {
        require(upgradeVersion == 0, "V2 already initialized");
        upgradeVersion = 2;
    }
    ...
    function swapTokens(IFDFPairV2.SwapTokensParams calldata params) external {
        ...
        // Validate swap parameters and signature
        _validateSwapTokens(params);

        // Update latest used nonce for user
        usedNonces[msg.sender] = params.nonce;

        // Generate payload and trigger transfer to FDFPair (re-write to set the recipient to the user)
        ...
        _safeBatchTransferFrom(
            ...
            abi.encode(IFDFPairV2(fdfPair).SWAP_TOKENS_SIG(), payload)
        );
    }
    ...
    function _verifySignature(bytes32 structHash, bytes calldata signature) internal view {
        ...
        address recoveredSigner = ECDSA.recover(keccak256(abi.encodePacked("\x19\x01", _domainSeparatorV4(), structHash)), signature);
     ...
     * @param params SwapTokensParams struct containing all swap parameters.
     */
    function _validateSwapTokens(IFDFPairV2.SwapTokensParams calldata params) internal view {
        if (params.playerTokenIdsIn.length != 1 || params.playerTokenIdsOut.length != 1) revert ArrayLengthMismatch(); // Current implementation only supports single token swap.
        if (params.playerTokenIdsIn.length != params.playerTokenAmountsIn.length) revert ArrayLengthMismatch();
        if (params.playerTokenIdsOut.length != params.playerTokenAmountsOut.length) revert ArrayLengthMismatch();
        ...
        // Check if all input players are sellable
        for (uint256 i = 0; i < params.playerTokenIdsIn.length;) {
            if (!isSellable(params.playerTokenIdsIn[i])) revert PlayerNotSellable();
            unchecked { i++; }
        }

        // Check if all output players are buyable
        for (uint256 i = 0; i < params.playerTokenIdsOut.length;) {
            if (!isBuyable(params.playerTokenIdsOut[i])) revert PlayerNotBuyable();
            unchecked { i++; }
        }

        // Create EIP-712 structured hash for swap operation
        bytes32 structHash = keccak256(abi.encode(
            SWAP_TYPEHASH,
            msg.sender,
    ...
    function supportsInterface(bytes4 _interfaceId) public view virtual override(ERC1155Upgradeable, ERC1155HolderUpgradeable, AccessControlUpgradeable, IERC165) returns (bool) {
        return _interfaceId == type(IPlayerV2).interfaceId || super.supportsInterface(_interfaceId);
    }
