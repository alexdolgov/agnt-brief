...
contract SquadOFT is OFTWithFee, Pausable {
    ...
    constructor(
        ...
        address _blastPointAddress
    ) OFTWithFee("SquadSwap", "SQUAD", 8, _lzEndpoint) {
        IBlastPoints(_blastPointAddress).configurePointsOperator(
            _pointsOperator
        );
    ...
    function claimAllGas(address recipient) external payable onlyOwner {
        BLAST.claimAllGas(address(this), recipient);
    }

    function claimMaxGas(address recipient) external payable onlyOwner {
        BLAST.claimMaxGas(address(this), recipient);
    }

    function readGas() public view returns (uint256) {
        (, uint256 etherBalance, , ) = BLAST.readGasParams(address(this));
        return etherBalance;
    }
}
