...
import {BalancerLocker, BalancerProtocol, BalancerStrategy} from "@address-book/src/BalancerEthereum.sol";
...
contract BalancerAccumulator is AccumulatorDelegableMultiToken, SafeModule {
    ...
    constructor(address _gauge, address _locker, address _governance, address _gateway)
        AccumulatorDelegableMultiToken(
            ...
            BalancerStrategy.VE_BOOST_DELEGATION, // veBoostDelegation
            1e18
        )
        SafeModule(_gateway)
    {
    ...
    /// @dev For delegatable tokens (BAL), only the portion from feeReceiver.split() is shared
    ///      with the delegation contract. The portion from FeeDistributor goes 100% to gauge.
    /// @param _tokenReward token to notify
    /// @param _amount amount to notify (unused, balance is recalculated)
    function _notifyReward(address _tokenReward, uint256 _amount) internal virtual override {
        // Track balance before split to isolate FeeDistributor vs feeReceiver portions
        uint256 balanceBeforeSplit = IERC20(_tokenReward).balanceOf(address(this));
        ...
        // Share delegation ONLY on the portion from feeReceiver.split(), not FeeDistributor
        // FeeDistributor rewards (protocol fees) go 100% to gauge
        // feeReceiver rewards (bribes/votemarket) are shared with boost delegators
        if (_isDelegatableToken(_tokenReward)) {
            uint256 fromSplit = _amount - balanceBeforeSplit;
            if (fromSplit > 0) {
                _amount -= _shareWithDelegationAmount(_tokenReward, fromSplit);
            }
        }
        if (_amount == 0) return;
    ...
    function _execute_claimMultipleRewards() internal returns (uint256[] memory claimed) {
        ...
        bytes memory returnData = _executeTransaction(
            FEE_DISTRIBUTOR, abi.encodeWithSelector(IFeeDistributor.claimTokens.selector, locker, rewardTokens)
        );

        // When locker != gateway, the return data is wrapped as (bool, bytes) from locker.execute()
        // We need to unwrap it before decoding the actual claim return data
        if (locker != GATEWAY) {
            (, returnData) = abi.decode(returnData, (bool, bytes));
        }
