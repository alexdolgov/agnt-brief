...
contract ProtocolContext {
    ...
    /// @dev Accountant is passed directly to avoid chicken-and-egg deployment issues with ProtocolController timelock
    ...
    /// @param _accountant The accountant contract address for this protocol
    ...
    /// @custom:throws ZeroAddress If protocol controller, gateway, or accountant is zero
    /// @custom:throws InvalidProtocolId If protocol ID is empty
    constructor(bytes4 _protocolId, address _protocolController, address _accountant, address _locker, address _gateway) {
        require(_protocolController != address(0) && _gateway != address(0) && _accountant != address(0), ZeroAddress());
        require(_protocolId != bytes4(0), InvalidProtocolId());

        GATEWAY = _gateway;
        PROTOCOL_ID = _protocolId;
        ACCOUNTANT = _accountant;
        REWARD_TOKEN = IAccountant(_accountant).REWARD_TOKEN();
    ...
    function _executeTransaction(address target, bytes memory data) internal returns (bool success) {
        if (LOCKER == GATEWAY) {
            ...
            // Decode inner success from Locker.execute() return value (bool success_, bytes memory result_)
            if (success && returnData.length >= 32) {
                (success,) = abi.decode(returnData, (bool, bytes));
            }
        }
    }
        ...
        returns (bool success, bytes memory returnData)
    {
        if (LOCKER == GATEWAY) {
            ...
            // Decode inner success and return data from Locker.execute() return value
            if (success && returnData.length >= 32) {
