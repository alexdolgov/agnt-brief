...
contract BotMarket is Auth, Multicallable {
    ...
    function getBalancerLiquidityQuote(uint256 underlyingAmount) internal returns (uint256 _liquidityQuote) {
        address[] memory tokens = new address[](2);
        tokens[0] = BAL;
        tokens[1] = WETH;
    ...
    function getBalancerSwapQuote(uint256 underlyingAmount) internal returns (uint256) {
        SingleSwap memory singleSwap = SingleSwap({
            poolId: SD_BAL_80BAL_20WETH_POOL_ID,
            kind: SwapKind.GIVEN_IN,
            assetIn: B_80BAL_20WETH,
    ...
    function getCurveQuote(address pool, int128 x, int128 y, uint256 amount) internal view returns (uint256 _quote) {
