...
contract CakePlatform is Owned, ReentrancyGuard {
    ...
    function _claim(address _user, address _recipient, uint256 _bountyId) internal notKilled returns (uint256 amount) {
        ...
        // Using current week since Gauge voting still relies on WEEK rounding for calculations
        // `canUserClaim` prevent claiming if we are not in the week = active period
        uint256 currentWeek = getCurrentPeriod();
        ...
        lastUserClaim[_user][_bountyId] = currentWeek;

        // Voting Power = userSlope * dt
        // with dt = lock_end - period.
        uint256 _bias = _getAddrBias(userSlope.slope, userSlope.end, currentWeek); // we are in the week after the voting period (active period)
        ...
        emit Claimed(_user, bounty.rewardToken, _bountyId, amount, feeAmount, currentWeek);
    }
    ...
    function claimable(address user, uint256 bountyId) external view returns (uint256 amount) {
        return _activeClaimable(user, bountyId);
    }
    ...
    /// @notice Return the current week based on Gauge Voting rounding. (Gauge Voting contract still relies on 1 week for maths)
