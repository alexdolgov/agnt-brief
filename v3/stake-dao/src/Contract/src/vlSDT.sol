...
import {Core} from "@address-book/src/SDEthereum.sol";
...
contract vlSDT is ERC20, Ownable2Step, ReentrancyGuard {
    ...
    address public constant SDT = Core.SDT;
    address public constant VE_SDT = Core.VESDT;
    ...
    /// @notice Unstake configuration with 2-step delay timelock
    ...
    /// @notice Unstake nonce (for unique IDs) and delay configuration with 2-step timelock
    UnstakeConfig public unstakeConfig;
    ...
    ///      The unstake nonce is set to 1 to avoid costly zero-to-nonzero storage write for the first unstaker
    ...
    constructor(address _owner, address _boostRegistry) ERC20("Vote Locked SDT", "vlSDT") Ownable(_owner) {
        require(_boostRegistry != address(0), BoostRegistryCannotBeZeroAddress());
        BOOST_REGISTRY = _boostRegistry;
        unstakeConfig = UnstakeConfig({nonce: 1, delay: uint32(8 weeks), pendingDelay: 0, effectiveTimestamp: 0});
    ...
    function unstake(uint256 amount) external returns (uint256 id) {
        ...
        UnstakeConfig storage config = unstakeConfig;
        id = (uint256(config.nonce++) << 160) | uint256(uint160(msg.sender));

        uint48 deadline = uint48(block.timestamp + uint256(config.delay));
        unstakeRequests[id] =
        ...
        returns (uint256 penaltyAmount, uint256 penaltyBps, uint256 userReceives)
    {
        ...
        // penaltyBps is the theoretical decay rate (informational, for frontends)
        penaltyBps = (uint256(maxPenaltyBps) * remainingTime) / totalDelay;

        // penaltyAmount is the actual charge, computed with full precision via mulDiv
        // to eliminate the zero-penalty window from the old two-step formula
        uint256 amount = uint256(request.amount);
        penaltyAmount = Math.mulDiv(amount, uint256(maxPenaltyBps) * remainingTime, totalDelay * 10_000);
    ...
    /**
     * @notice Schedule an update to the unstake delay
     * @dev Gives users PENALTY_UPDATE_DELAY time before the new delay takes effect.
     *      Can be called again to override a pending update (resets the cooldown).
     * @param _newUnstakeDelay New unstake delay in seconds
     */
    function scheduleUnstakeDelayUpdate(uint256 _newUnstakeDelay) external onlyOwner {
        uint32 newDelay = SafeCast.toUint32(_newUnstakeDelay);
        uint48 effectiveAt = uint48(block.timestamp + PENALTY_UPDATE_DELAY);

        UnstakeConfig storage config = unstakeConfig;
        config.pendingDelay = newDelay;
        config.effectiveTimestamp = effectiveAt;

        emit UnstakeDelayUpdateScheduled(uint256(config.delay), _newUnstakeDelay, uint256(effectiveAt));
    }

    /**
     * @notice Apply a pending unstake delay update
     * @dev Can be called by anyone after the effective timestamp.
     *      Reverts if no pending update or if grace period hasn't elapsed.
     */
    function applyUnstakeDelayUpdate() external {
        UnstakeConfig storage config = unstakeConfig;

        uint256 effectiveTs = uint256(config.effectiveTimestamp);
        require(effectiveTs != 0, NoPendingUpdate());
        require(block.timestamp >= effectiveTs, UpdateNotEffectiveYet());

        uint32 oldDelay = config.delay;
        uint32 newDelay = config.pendingDelay;

        config.delay = newDelay;
        config.pendingDelay = 0;
        config.effectiveTimestamp = 0;

        emit UnstakeDelayUpdateApplied(uint256(oldDelay), uint256(newDelay));
    }
