...
contract GaugeVotingCalc is Ownable {
    ...
    function _getTotalCappedPercent(uint256 gaugeTotalWeight) internal view returns (uint256 gaugeTotalCappedPercent) {
        ...
        for (uint256 i = 0; i < IGaugeVoting(gaugeVotingAddress).gaugeCount(); i++) {
            ...
            ( , , uint256 chainId, address pairAddress, , uint256 maxVoteCap) = IGaugeVoting(gaugeVotingAddress).gauges(i);
            uint256 weight = IGaugeVoting(gaugeVotingAddress).getGaugeWeight(pairAddress, chainId, false);
    ...
    function _getTotalFinalWeights(uint256 gaugeTotalWeight, uint256 gaugeTotalCappedPercent) internal view returns (uint256 gaugeTotalFinalWeights) {
    ...
    function _getGaugeWeightDetails(uint256 _gaugeId, uint256 _gaugeTotalWeight, uint256 _gaugeTotalCappedPercent, uint256 _gaugeTotalFinalWeights) internal view returns (
        uint256 gaugeWeight,
        ...
        ( , , uint256 chainId, address pairAddress, , uint256 maxVoteCap) = IGaugeVoting(gaugeVotingAddress).gauges(_gaugeId);
        ...
        if (_gaugeTotalFinalWeights == 0) {
            gaugeTotalFinalWeights = gaugeTotalFinalWeights = _getTotalFinalWeights(gaugeTotalWeight, gaugeTotalCappedPercent);
        } else {
    ...
    function getGaugeWeightDetails(uint256 _gaugeId) external view returns (
        uint256 gaugeWeight,
        uint256 gaugeTotalWeight,
        uint256 gaugeRawPercent,
        uint256 gaugeCappedPercent,
    ...
    function getGaugeWeightMass(address _gaugeAddr, uint256 _chainId, bool _inCap, uint256 _gaugeTotalWeight, uint256 _gaugeTotalCappedPercent, uint256 _gaugeTotalFinalWeights) public view returns (uint256) {
        require (_gaugeTotalWeight * _gaugeTotalCappedPercent * _gaugeTotalFinalWeights > 0, "missing total params");

        bytes32 gaugeHash = keccak256(abi.encodePacked(_gaugeAddr, _chainId));
        uint256 gaugeIdx = (IGaugeVoting(gaugeVotingAddress).gaugeIndex_(gaugeHash) - 1);
        (uint256 gaugeWeight, , , , uint256 gaugeInCapWeight, , ) = _getGaugeWeightDetails(gaugeIdx, _gaugeTotalWeight, _gaugeTotalCappedPercent, _gaugeTotalFinalWeights);
