...
pragma solidity <0.9.0 =0.8.28 >=0.8.0 ^0.8.20 ^0.8.24 ^0.8.7;
...
// src/lending/interfaces/IChainlinkFeed.sol
...
// src/lending/interfaces/IOracle.sol
...
// node_modules/@stake-dao/strategies/src/interfaces/ICurvePool.sol
...
// src/lending/oracles/BaseOracle.sol
...
// src/lending/oracles/CurveCryptoswapOracle.sol
...
// src/lending/oracles/CurveStableswapOracle.sol
...
// src/lending/interfaces/ILendingFactory.sol
...
// src/lending/factories/MorphoMarketFactory.sol
...
contract MorphoMarketFactory is ILendingFactory {
    ...
    /// @dev The seed is forfeited to the dead address!!! The most important factor is the utilization
    ///      rate to reach the expected utilization rate. The amount of tokens supplied doesn't matter.
    ///      Only supply a small amount (equivalent to $1 USD, for example) to prevent the
    ///      zero utilization rate decay while forfeiting a negligible amount of tokens.
    ...
    function create(
        ...
        if (MORPHO_BLUE.market(id).lastUpdate == 0) {
        ...
        emit LendingMarketDeployed(address(MORPHO_BLUE), address(collateral), address(loan), address(oracle), lltv, irm);

        // 3. Pre-seed the market if needed
        if (initialLoanSupply != 0) {
            _preSeedMarket(initialLoanSupply, morphoMarketParams);
        }
    }

    /// @notice Pre-seeds the market with the given amount of loan token. The seeded amount is sent to a dead address!
    ...
    function _preSeedMarket(uint256 initialLoanSupply, MarketParams memory morphoMarketParams) internal {
        ...
        uint256 collateralToSupply = Math.mulDiv(
            Math.mulDiv(
                borrowAmount,
                10 ** IOracle(morphoMarketParams.oracle).ORACLE_SCALING_EXPONENT(),
        ...
        IStrategyWrapper(morphoMarketParams.collateralToken).depositShares(collateralToSupply, address(this));
    ...
    /// @return _ The scaling exponent expected by the protocol for the oracle to use to scale the price.
    ///           In the case of Morpho Blue, it is 36.
    function protocolScalingExponent() external pure returns (uint256) {
        return 36;
    }
...
// src/lending/factories/CurveLendingMarketFactory.sol
...
/// @author Stake DAO
/// @custom:contact contact@stakedao.org
contract CurveLendingMarketFactory is Ownable {
    ...
    error FailedToReunceOwnership();

    /// @dev The parameters for creating a Curve oracle (both stableswap and cryptoswap).
    struct OracleParams {
        address quoteAsset;
        address quoteAssetFeed;
        uint256 quoteAssetFeedHeartbeat;
    ...
    function deploy(
        ...
        address curvePool = rewardVault.asset();
        IOracle oracle = _deployOracle(curvePool, oracleType, oracleParams, protocolScalingExponent);

        // 3. Create the lending market
        data = _createLendingMarket(collateral, oracle, oracleParams.quoteAsset, lendingFactory, marketParams);

        // 4. Renounce ownership of the collateral making it permissionless
        (bool success,) = address(collateral).call(abi.encodeWithSignature("renounceOwnership()"));
        require(success, FailedToReunceOwnership());
    ...
    function _deployOracle(
        ...
        if (oracleType == OracleType.CRYPTOSWAP) {
            oracle = new CurveCryptoswapOracle(
                curvePool,
                oracleParams.quoteAsset,
                oracleParams.quoteAssetFeed,
                oracleParams.quoteAssetFeedHeartbeat,
...
// src/lending/factories/MonolithicCurveMorphoMarketFactory.sol

interface IMonolithicCurveMorphoMarketFactory {
    function create(
        IRewardVault rewardVault,
        address quoteAsset,
        address quoteAssetFeed,
        uint256 quoteAssetFeedHeartbeat,
        address[] calldata token0ToUsdFeeds,
        uint256[] calldata token0ToUsdHeartbeats,
        address irm,
        uint256 lltv,
        uint256 initialSupply
    ) external returns (IStrategyWrapper collateral, IOracle oracle, bytes memory data);
}

contract MonolithicCurveMorphoMarketFactory is IMonolithicCurveMorphoMarketFactory, Ownable2Step {
    /// @dev Whether the factory is disabled
    bool public isFactoryDisabled;

    /// @dev The address of the Curve lending market factory
    CurveLendingMarketFactory public immutable CURVE_LENDING_MARKET_FACTORY;

    /// @dev The address of the Morpho market factory
    MorphoMarketFactory public immutable MORPHO_MARKET_FACTORY;

    /// @dev The address of the Morpho lending protocol
    address private immutable MORPHO_BLUE;

    /// @dev Thrown when the given address is zero.
    error AddressZero();

    /// @dev Thrown when the factory is disabled.
    error FactoryDisabled();

    /// @dev The address of the Morpho market factory
    constructor(address _curveLendingMarketFactory, address _morphoMarketFactory, address _admin) Ownable(_admin) {
        require(_curveLendingMarketFactory != address(0), AddressZero());
        require(_morphoMarketFactory != address(0), AddressZero());
        require(_admin != address(0), AddressZero());

        CURVE_LENDING_MARKET_FACTORY = CurveLendingMarketFactory(_curveLendingMarketFactory);
        MORPHO_MARKET_FACTORY = MorphoMarketFactory(_morphoMarketFactory);
        MORPHO_BLUE = MORPHO_MARKET_FACTORY.protocol();
    }
    ...
    modifier whenFactoryIsEnabled() {
        require(!isFactoryDisabled, FactoryDisabled());
        _;
    }
    ...
    /// @notice Creates a Curve lending market for a given reward vault
    /// @param rewardVault The reward vault to use for the market
    /// @param quoteAsset The asset to use for the market
    /// @param quoteAssetFeed The feed to use for the market
    /// @param quoteAssetFeedHeartbeat The heartbeat to use for the market
    /// @param token0ToUsdFeeds The feeds to use for the market
    /// @param token0ToUsdHeartbeats The heartbeats to use for the market
    /// @param irm The IRM to use for the market
    /// @param lltv The LLTV to use for the market
    /// @param initialSupply The initial supply to use for the market. Can be 0.
    /// @return collateral The collateral token
    /// @return oracle The oracle
    /// @return data The data returned by the lending protocol after creating the market
    function create(
        IRewardVault rewardVault,
        address quoteAsset,
        address quoteAssetFeed,
        ...
        uint256 lltv,
        uint256 initialSupply
    ) external whenFactoryIsEnabled returns (IStrategyWrapper collateral, IOracle oracle, bytes memory data) {
        // 1. deploy the collateral
        collateral = new MorphoStrategyWrapper(rewardVault, MORPHO_BLUE, address(CURVE_LENDING_MARKET_FACTORY), "", "");

        CurveLendingMarketFactory.OracleParams memory oracleParams = CurveLendingMarketFactory.OracleParams(
            quoteAsset, quoteAssetFeed, quoteAssetFeedHeartbeat, token0ToUsdFeeds, token0ToUsdHeartbeats
        );

        CurveLendingMarketFactory.MarketParams memory marketParams =
            CurveLendingMarketFactory.MarketParams(irm, lltv, initialSupply);

        // 2. call the curve lending market factory to create the market
        (, oracle, data) = CURVE_LENDING_MARKET_FACTORY.deploy(
            collateral, OracleType.STABLESWAP, oracleParams, marketParams, MORPHO_MARKET_FACTORY
        );
    ...
    function toggleFactory(bool value) external onlyOwner {
        isFactoryDisabled = value;
    }
}