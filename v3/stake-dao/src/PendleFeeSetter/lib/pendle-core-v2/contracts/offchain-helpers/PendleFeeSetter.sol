...
import {IPMarketV3, MarketState} from "../interfaces/IPMarketV3.sol";
import {IPMarketFactoryV3} from "../interfaces/IPMarketFactoryV3.sol";
...
contract PendleFeeSetter is BoringOwnableUpgradeableV2 {
    ...
    function setFees(address[] memory markets) external onlyOwnerOrSetter {
        ...
        for (uint256 i = 0; i < n; ++i) {
            (, , IPYieldToken yt) = IPMarketV3(markets[i]).readTokens();
            ...
            calls[i].target = IPMarketV3(markets[i]).factory();
            calls[i].callData = abi.encodeCall(
                IPMarketFactoryV3.setOverriddenFee,
    ...
    function _calcFee(
        ...
        uint256 marketLnFee = IPMarketV3(market).getNonOverrideLnFeeRateRoot();
        MarketState memory state = IPMarketV3(market).readState(router);
        ...
        )
    {
        ...
        for (uint256 i = 0; i < n; ++i) {
            MarketState memory state = IPMarketV3(markets[i]).readState(router);
