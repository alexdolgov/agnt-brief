...
contract PriceLens0 is Owned {
    ...
    function _getPriceFromV3(address poolAddress, address token) private view returns (uint256 price) {
        ...
        if (sqrtPriceX96 == 0 || uint256(sqrtPriceX96) > MAX_U128) revert UnknowPrice();
        ...
        uint256 p = FullMath.mulDiv(
            (uint256(sqrtPriceX96) * uint256(sqrtPriceX96)), 10 ** (decimals0 + PRECISION), 2 ** (96 * 2)
        );
        p = FullMath.mulDiv(p, 10 ** DECIMALS, 10 ** (PRECISION + decimals1));
    ...
    function _getPriceFromSS(address swapAddress, address token) private view returns (uint256 price) {
        ...
        if (isToken0) {
            ...
            uint256 dy = ss.get_dy(1, 0, 10 ** decimals1);
