...
import {AccessManagedUpgradeable, IAccessManager} from "@openzeppelin-upgradeable/contracts/access/manager/AccessManagedUpgradeable.sol";

/*
 * @title Curve Fee Distribution modified for ve(3,3) emissions
 * @author Modified from  Curve Finance, andrecronje
 * @author Modified from velodrome.finance (https://github.com/velodrome-finance/contracts)
 * @author spectra.finance
 * @license MIT
 */
contract RewardsDistributor is IRewardsDistributor, Initializable, AccessManagedUpgradeable {
    ...
    function initialize(address _initialAuthority, address _ve, address _governanceRegistry) external initializer {
        ...
        token = _token;
        IERC20(_token).forceApprove(_ve, type(uint256).max);
    ...
    /// @inheritdoc IRewardsDistributor
    function checkpointToken() external restricted {
        _checkpointToken();
    }
