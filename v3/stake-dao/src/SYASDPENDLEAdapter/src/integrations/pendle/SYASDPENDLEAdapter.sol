...
contract SYASDPENDLEAdapter is IStandardizedYieldAdapter {
    ...
    function previewConvertToDeposit(address, /*tokenIn*/ uint256 amountIn) external view returns (uint256) {
        return _calculatePivotTokensMinted(amountIn);
    }

    function convertToDeposit(address, /*tokenIn*/ uint256 amountTokenIn) external returns (uint256 amountOut) {
        amountOut = _calculatePivotTokensMinted(amountTokenIn);
        IDepositor(PendleLocker.DEPOSITOR).deposit(amountTokenIn, true, false, msg.sender);
    }

    function _calculatePivotTokensMinted(uint256 amountTokenIn) internal view returns (uint256) {
        return amountTokenIn + IDepositor(PendleLocker.DEPOSITOR).incentiveToken();
    }
