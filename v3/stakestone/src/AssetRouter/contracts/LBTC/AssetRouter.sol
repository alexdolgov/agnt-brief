...
import {IGMPBascule} from "../bascule/interfaces/IGMPBascule.sol";
    ...
    ReentrancyGuardUpgradeable
{
    ...
    function changeDepositMinAmount(
        address token,
        uint256 _depositMinAmount
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        _changeDepositMinAmount(token, _depositMinAmount);
    }
    ...
    function depositMinAmount(address token) external view returns (uint256) {
        AssetRouterStorage storage $ = _getAssetRouterStorage();
    ...
    function bascule() external view override returns (IGMPBascule) {
    ...
    function _deposit(
        ...
        AssetRouterStorage storage $ = _getAssetRouterStorage();
        if (amount < $.depositMinAmount) revert AssertRouter_AmountTooSmall();
        if (
            _getRouteType(
    ...
    /**
     * @dev Checks that the mint was validated by the Bascule drawbridge.
     * @param $ LBTC storage.
     */
    function _confirmMint(
        AssetRouterStorage storage $,
        uint256 nonce,
        address recipient,
        address toToken,
        ...
        IGMPBascule bascule_ = $.bascule;
        if (address(bascule_) != address(0)) {
            bascule_.validateMint(
                IGMPBascule.Message(nonce, recipient, toToken, amount)
            );
        }
    }
    ...
    function _changeNativeToken(address newValue) internal {
        ...
        delete $.routes[key];
        _revokeRole(CALLER_ROLE, prevValue);
    ...
    function _changeDepositMinAmount(address token, uint256 newVal) internal {
        AssetRouterStorage storage $ = _getAssetRouterStorage();
        if ($.nativeToken != token) revert AssetRouter_WrongNativeToken();
        emit AssetRouter_DepositMinAmountChanged(
            token,
            $.depositMinAmount,
            newVal
        );
        $.depositMinAmount = newVal;
    }
