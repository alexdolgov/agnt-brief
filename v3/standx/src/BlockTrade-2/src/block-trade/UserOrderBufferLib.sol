...
library UserOrderBufferLib {
    ...
    function _addOrder(
        ...
        for (uint8 i = 0; i < USER_ORDER_BUFFER_SIZE; i++) {
            ...
            OrderLib.Order storage orderInfo = OrderMapLib.get(orderMap, userOrderId);
            if (!OrderLib._isOrderActive(orderInfo)) {
                availableSlot = i;
                break;
            }
            BlockLib.Block storage blockInfo = BlockMapLib.get(blockMap, orderInfo.blockId);
            if (!BlockLib._isBlockOpen(blockInfo)) {
                availableSlot = i;
                break;
            }
        }
