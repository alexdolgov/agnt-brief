...
library BlockLib {
    ...
    function _initBlock(
        ...
        blockInfo.makerFee = orderMsg.makerFee;
        blockInfo.takerFee = orderMsg.takerFee;

        BlockMapLib.set(blockMap, blockId, blockInfo);
        OrderLib._initOrder(orderMap, blockId, orderMsg);
        return (
    ...
    function _isBlockActive(
        Block storage blockInfo
    ) internal view returns (bool) {
        return
            blockInfo.expiredAt > block.timestamp &&
            _isBlockStatusActive(blockInfo);
    }

    function _isBlockStatusActive(
        Block storage blockInfo
    ) internal view returns (bool) {
        if (blockInfo.status == BlockTradeLib.BlockStatus.Open) {
            return true;
        }

        bool isMatched = blockInfo.status ==
            BlockTradeLib.BlockStatus.OnchainMatched;
        bool hasRightPolicy = blockInfo.policy ==
            BlockTradeLib.MatchPolicy.IssueRight ||
            blockInfo.policy == BlockTradeLib.MatchPolicy.SubscribeRight;

        return isMatched && hasRightPolicy;
    }
