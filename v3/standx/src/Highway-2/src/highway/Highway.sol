    ...
    UUPSUpgradeable
{
    ...
    function reqRemoteCall(
        ...
        require(
            uint256(message.nonce) > callerInfo.lastNonce,
        ...
        if (availableSlot < HighwayCallerLib.HIGHWAY_MAX_MSG_BUFFER) {
            callerInfo.withdrawBuffer[availableSlot] = HighwayLib
                .WithdrawMessage(
                    HighwayLib.MessageStatus.Pending,
                    block.timestamp,
                    0,
    ...
    function _ackRemoteCall(
        ...
        HighwayLib.WithdrawMessage storage pendingMsg = caller.withdrawBuffer[
            messageIndex
        ];
        ...
        if (newStatus == HighwayLib.MessageStatus.Pending) {
            ...
            require(
                pendingMsg.message.token == state.dusd,
                HighwayLib.InvalidAddress()
            );
    ...
    function getCallerInfo(
        ...
        HighwayCallerLib.Caller memory callerInfo = HighwayCallerLib.initCaller(
            callerAddress
        );
        return callerInfo;
    }
    ...
    function updateCaller(
        ...
        HighwayCallerLib.Caller storage callerInfo = HighwayCallerLib
            .getOrInitCaller(callerInfoMap, callerInfo.caller);
        require(callerInfo.enabled == false, HighwayLib.CallerDisabled());
        HighwayCallerLib.updateCaller(callerInfoMap, callerInfo);
    }
