...
// File: contracts/farm/FarmRewardLock.sol
...
contract PrivateSaleLock is Context, Ownable {
    ...
    function addUserPrivateSaleAmount(address user, uint256 amount) onlyOwner external returns (bool) {
        sbf.safeTransferFrom(_msgSender(), address(this), amount);
    ...
    function emergencyWithdraw() onlyOwner public {
        sbf.safeTransfer(_msgSender(), sbf.balanceOf(address(this)));
    }
}