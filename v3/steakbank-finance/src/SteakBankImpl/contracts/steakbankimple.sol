...
contract SteakBankImpl is Context, Initializable, ReentrancyGuard {
    ...
    function claimVault(uint256 amount) onlyAdmin external {
        IVault(unstakeVault).claimBNB(amount, msg.sender);
    }

    function mintLBNB(uint256 amount) onlyAdmin external {
        IMintBurnToken(LBNB).mintTo(msg.sender, amount);
    }
