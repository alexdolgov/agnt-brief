...
contract IDOLocker is OwnableUpgradeable, ReentrancyGuardUpgradeable {
  ...
  struct PoolInfo {
    ...
    uint256 saleStartTimestamp; // Sale Start Time
    uint256 saleEndTimestamp; // Sale End Time
  }
  ...
  function unlock(uint256 _pid) external nonReentrant notContract {
    ...
    if (block.timestamp >= pool.unlockTimestamp) {
      ...
      bool isDuringSale = block.timestamp >= pool.saleStartTimestamp && block.timestamp <= pool.saleEndTimestamp;
      require(!isDuringSale, "Cannot early unlock during sale");
  ...
  function update(
    ...
    poolInfo[_pid].saleStartTimestamp = _saleStartTimestamp;
    poolInfo[_pid].saleEndTimestamp = _saleEndTimestamp;
  }
