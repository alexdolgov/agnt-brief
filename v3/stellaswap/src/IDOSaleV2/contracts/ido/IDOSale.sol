...
contract IDOSaleV2 is OwnableUpgradeable, ReentrancyGuardUpgradeable {
  ...
  uint256 feeBase; // All fees should be quoted in 10,000 format e.g 1500 = 15%
  ...
  function initializeSale(
    ...
    uint256 tokenPrice, // How much tokens will I get if I give 1 base Token, ATH / USDC == 5 / $
    uint256 amount, // 250K
    uint256 totalBaseAmount, // 50K
    uint256 liqPercentage, // 0%
    uint256 listingRate, // 4 / $
    uint256 startTime, //
    uint256 endTime, //
    uint256 sId
  ) external onlyOwner {
  ...
  function initializeTokens() external onlyPresaleOwner {
    ...
    require(
      STELLA_SETTINGS.stellaTokenFees != 0,
      "SETTINGS ARE NOT INITIALIZED YET"
    );
    uint256 tokensToMove = SALE_INFO.AMOUNT +
      ((SALE_INFO.AMOUNT * STELLA_SETTINGS.stellaTokenFees) / feeBase);
  ...
  function userDeposit(uint256 _amount) external {
    ...
    if (STELLA_SETTINGS.IS_WHITELIST) {
      require(
        WHITELIST.contains(msg.sender) || WINNERS.contains(msg.sender),
        "NOT WHITELISTED"
      );
    }
  ...
  function ownerWithdrawTokens() external onlyPresaleOwner {
    require(presaleStatus() == 5, "NOT CANCELLED");
    uint256 tokenFees = (SALE_INFO.AMOUNT * STELLA_SETTINGS.stellaTokenFees) /
      feeBase;
    TransferHelper.safeTransfer(
      address(SALE_INFO.S_TOKEN),
      msg.sender,
  ...
  function ownerClaimUnderflow() external onlyPresaleOwner {
    require(
      presaleStatus() == 2 || presaleStatus() == 3 || presaleStatus() == 4,
      "Cannot claim yet"
    );
    require(
      SALE_STATUS.TOTAL_TOKENS_SOLD != SALE_INFO.AMOUNT,
      "Sale is fully subscribed"
    );
    uint256 remainig = SALE_INFO.AMOUNT - SALE_STATUS.TOTAL_TOKENS_SOLD;
    TransferHelper.safeTransfer(
      address(SALE_INFO.S_TOKEN),
      msg.sender,
      remainig
  ...
  function sweepFunds() external onlyOwner {
    ...
    uint256 baseFees = (SALE_STATUS.TOTAL_BASE_COLLECTED *
      STELLA_SETTINGS.stellaBaseFees) / feeBase;
    uint256 tokenFees = (SALE_INFO.AMOUNT * STELLA_SETTINGS.stellaTokenFees) /
      feeBase;
  ...
  function _getUserMaxCap(address _userAddr)
    internal
    view
    returns (uint256, uint256)
  {
    ...
    // functionality to support winners;
    if (WINNERS.contains(_userAddr)) {
      XStellaTiers memory _tier = tiers[0];
      maxCap = _tier.maxCap;
    } else {
  ...
  function viewOnly() external view returns (uint256) {
    return feeBase;
  }

  function editWinners(address[] memory _users, bool _add) external onlyOwner {
    if (_add) {
      for (uint256 i = 0; i < _users.length; i++) {
        WINNERS.add(_users[i]);
      }
    } else {
      for (uint256 i = 0; i < _users.length; i++) {
        WINNERS.remove(_users[i]);
      }
    }
  }

  function editTier(uint256 index, XStellaTiers memory _tier)
    external
    onlyOwner
  {
    tiers[index] = _tier;
  ...
  function getWinnersUsersLength() external view returns (uint256) {
    return WINNERS.length();
  }

  function getUserWinnerStatus(address _user) external view returns (bool) {
    return WINNERS.contains(_user);
  }
