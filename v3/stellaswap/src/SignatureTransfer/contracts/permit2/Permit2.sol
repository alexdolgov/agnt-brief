...
contract SignatureTransfer is ISignatureTransfer, EIP712 {
    ...
    function _useUnorderedNonce(address from, uint256 nonce) internal {
        ...
        uint256 flipped = nonceBitmap[from][wordPos];
        if (flipped & bit == 0) revert InvalidNonce();
    }

    function getNonce(address from, uint256 nonce) public view returns (uint256) {
        if(nonceBitmap[from][nonce] > 0) { return 0; } //error cond
        uint256 shifted = nonce << 8; //move into the area of wordPos, get out of bitPos
        uint one = shifted ^ 1; //flips the 1st bit of bitPos
        return one;
    }
}
