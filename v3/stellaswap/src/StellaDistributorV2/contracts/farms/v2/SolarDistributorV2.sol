...
import "./IStellaPair.sol";

contract StellaDistributorV2 is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 lastRewardTimestamp; // Last block number that Stella distribution occurs.
        uint256 accStellaPerShare; // Accumulated Stella per share, times 1e18. See below.
    ...
    IBoringERC20 public stella;

    // Stella tokens created per second
    uint256 public stellaPerSec;
    ...
    // The timestamp when Stella mining starts.
    ...
    constructor(
        IBoringERC20 _stella,
        uint256 _stellaPerSec,
        ...
        )
    {
        ...
        if (block.timestamp > pool.lastRewardTimestamp && lpSupply != 0) {
            ...
            accStellaPerShare += (
                ((stellaReward * ACC_TOKEN_PRECISION) / lpSupply)
            );
        }

        uint256 pendingStella = (((user.amount * accStellaPerShare) /
        ...
        addresses[0] = address(stella);
        symbols[0] = IBoringERC20(stella).safeSymbol();
        decimals[0] = IBoringERC20(stella).safeDecimals();
        amounts[0] = pendingStella;
        ...
        )
    {
        ...
        rewardsPerSec[0] =
            (pool.allocPoint * stellaPerSec * lpPercent) /
            totalAllocPoint /
            total;
    ...
    function _updatePool(uint256 _pid) internal validatePoolByPid(_pid) {
        ...
        uint256 stellaReward = ((multiplier * stellaPerSec) * pool.allocPoint) /
            totalAllocPoint;
        ...
        stella.mint(teamAddress, (stellaReward * teamPercent) / total);
        stella.mint(treasuryAddress, (stellaReward * treasuryPercent) / total);
        stella.mint(investorAddress, (stellaReward * investorPercent) / total);
        stella.mint(address(this), (stellaReward * lpPercent) / total);

        pool.accStellaPerShare +=
            (stellaReward * ACC_TOKEN_PRECISION * lpPercent) /
            pool.totalLp /
            total;
    ...
    function depositWithPermit(
        ...
        PoolInfo storage pool = poolInfo[pid];
        IStellaPair pair = IStellaPair(address(pool.lpToken));
        pair.permit(msg.sender, address(this), amount, deadline, v, r, s);
        _deposit(pid, amount);
    }

    // Deposit tokens for Stella allocation.
        ...
        validatePoolByPid(_pid)
    {
        ...
        if (_amount > 0) {
            ...
            if (address(pool.lpToken) == address(stella)) {
                totalStellaInPools += _amount;
            }
        }
        user.rewardDebt =
            (user.amount * pool.accStellaPerShare) /
            ACC_TOKEN_PRECISION;
        ...
        for (
            ...
            pool.rewarders[rewarderId].onStellaReward(
                _pid,
                msg.sender,
                user.amount
            );
    ...
    function emergencyWithdraw(uint256 _pid) public nonReentrant {
        ...
        for (
            ...
            pool.rewarders[rewarderId].onStellaReward(_pid, msg.sender, 0);
        }

        if (address(pool.lpToken) == address(stella)) {
            totalStellaInPools -= amount;
        }
    ...
    function payOrLockupPendingStella(uint256 _pid) internal {
        ...
        uint256 pending = ((user.amount * pool.accStellaPerShare) /
        ...
        if (canHarvest(_pid, msg.sender)) {
            if (pending > 0 || user.rewardLockedUp > 0) {
                ...
                // send rewards
                safeStellaTransfer(msg.sender, pendingRewards);
            }
        } else if (pending > 0) {
    ...
    function safeStellaTransfer(address _to, uint256 _amount) internal {
        if (stella.balanceOf(address(this)) > totalStellaInPools) {
            ...
            uint256 stellaBal = stella.balanceOf(address(this)) -
                totalStellaInPools;
            if (_amount >= stellaBal) {
                stella.safeTransfer(_to, stellaBal);
            } else if (_amount > 0) {
                stella.safeTransfer(_to, _amount);
            }
        }
    }

    function updateEmissionRate(uint256 _stellaPerSec) public onlyOwner {
        _massUpdatePools();

        emit EmissionRateUpdated(msg.sender, stellaPerSec, _stellaPerSec);
