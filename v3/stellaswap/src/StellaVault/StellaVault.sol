...
// File contracts/vault/StellaVault.sol
...
contract StellaVault is Ownable, ReentrancyGuard {
    ...
    mapping(uint256 => mapping(address => TimedStake)) timeStakeInfo;
    ...
    function set(
        ...
        poolInfo[_pid].lockDownDuration = _lockDownDuration;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        if (_amount > 0) {
            ...
            TimedStake storage _timedStake = timeStakeInfo[_pid][msg.sender];
            _timedStake.stakes[block.timestamp] = _amount;
            _timedStake.stakeTimes.push(block.timestamp);
    ...
    function withdraw(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        require(withdrawableAmount(_pid, msg.sender) >= _amount,"not enough withdrawable balance");
        if (_amount > 0) {
    ...
    // calcuate amount that can be withdrawn
    function withdrawableAmount(uint256 _pid, address account)public view returns(uint256){
        PoolInfo storage pool = poolInfo[_pid];
        uint256 amount = 0;
        TimedStake storage _timedStake = timeStakeInfo[_pid][account];
        
        for (uint8 index = 0; index < _timedStake.stakeTimes.length; index++) {
            uint256 key = _timedStake.stakeTimes[index];
            if (block.timestamp.sub(key) > pool.lockDownDuration){
