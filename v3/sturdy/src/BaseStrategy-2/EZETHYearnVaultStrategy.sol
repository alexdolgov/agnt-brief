...
contract EZETHYearnVaultStrategy is BaseStrategy {
    ...
    AggregatorV3Interface public immutable EZETH_ORACLE;
    ...
    // swap path between asset and ezETH
    ...
    /**
     *  @dev Fallback function, only accept ETH from WETH
     */
    receive() external payable {
        if (msg.sender != address(asset)) revert INVALID_CONFIGURATION();
    }
    ...
    constructor(
        ...
        EZETH = vault.asset();
        EZETH_ORACLE = AggregatorV3Interface(_vaultAssetOracle);
    ...
    function _deployFunds(uint256 _amount) internal override {
        ...
        if (thresholdMet) {
            // swap to ezETH from asset
            amount = _swapToEZETH(_amount);
            // deposit ezETH to yearn vault
            ERC20(EZETH).safeApprove(address(vault), 0);
            ERC20(EZETH).safeApprove(address(vault), amount);
    ...
    function _freeFunds(uint256 _amount) internal override {
        ...
        if (block.timestamp >= timeToUnlock) {
            uint256 ezETHAmount = _getEzETHAmount(_amount, false);
            vault.redeem(
                Math.min(vault.convertToShares(ezETHAmount), vault.balanceOf(address(this))),
                address(this),
                address(this)
            );
            // swap from ezETH to asset
            _swapFromEZETH(IERC20(EZETH).balanceOf(address(this)));
        } else {
        ...
        returns (uint256 _totalAssets)
    {
        ...
        // swap to ezETH from asset
        _swapToEZETH(_totalAssets);
        // deposit to yearn vault
        uint256 amount = IERC20(EZETH).balanceOf(address(this));
        ERC20(EZETH).safeApprove(address(vault), 0);
        ERC20(EZETH).safeApprove(address(vault), amount);
        vault.deposit(amount, address(this));
    }
    ...
    /**
     * @dev get the ezETH amount from asset amount based on the slippage
     * @param _amount the asset amount
     * @return the amount of ezETH
     */
    function _getEzETHAmount(uint256 _amount, bool _applySlippage) internal view returns (uint256) {
        (, int256 _answer, , uint256 _updatedAt, ) = EZETH_ORACLE.latestRoundData();     // ezETH/WETH
    ...
    /**
     * @dev get the asset amount from ezETH amount based on the slippage
     * @param _ezETHAmount the ezETH amount
     * @return the amount of asset
     */
    function _getAmount(uint256 _ezETHAmount, bool _applySlippage) internal view returns (uint256) {
        (, int256 _answer, , uint256 _updatedAt, ) = EZETH_ORACLE.latestRoundData();     // ezETH/WETH
        // If data is stale or negative, set bad data to true and return
        if (_answer <= 0 || (block.timestamp - _updatedAt > MAX_ORACLE_DELAY)) {
            revert CHAINLINK_BAD_PRICE();
        ...
        uint256 _slippage = _applySlippage ? slippage : 0;
        return _ezETHAmount * uint256(_answer) * (PERCENTAGE_FACTOR - _slippage) / 1e18 / PERCENTAGE_FACTOR;
    }

    /**
     * @dev swap from ezETH to asset
     * @param _ezETHAmount the ezETH amount
     */
    function _swapFromEZETH(uint256 _ezETHAmount) internal returns (uint256) {
        IBaseLeverage.BiDirectSwapInfo memory _swapInfo = swapInfo;

        if (_swapInfo.pathLength == 0) revert INVALID_CONFIGURATION();

        uint256 amount = _ezETHAmount;
        ...
        if (amount < _getAmount(_ezETHAmount, true)) revert SLIPPAGE_LOW();
        
        return amount;
    }

    /**
     * @dev swap from asset to ezETH
     * @param _amount the asset amount
     */
    function _swapToEZETH(uint256 _amount) internal returns (uint256) {
        IBaseLeverage.BiDirectSwapInfo memory _swapInfo = swapInfo;
        ...
        if (amount < _getEzETHAmount(_amount, true)) revert SLIPPAGE_LOW();
    ...
    function _emergencyWithdraw(uint256 _amount) internal override {
        ...
        uint256 shares = vault.convertToShares(ezETHAmount);
    ...
    function setSwapInfo(IBaseLeverage.BiDirectSwapInfo memory _swapInfo) external onlyManagement {
        ...
        if (_swapInfo.paths[_swapInfo.pathLength - 1].swapTo != EZETH) revert INVALID_CONFIGURATION();
        if (_swapInfo.reversePaths[0].swapFrom != EZETH) revert INVALID_CONFIGURATION();
