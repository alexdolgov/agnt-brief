...
contract AGETHYearnVaultStrategy is BaseStrategy {
    using SafeERC20 for ERC20;
    
    IRSETHAdapter private constant RSETHAdapter = 
        IRSETHAdapter(0xbf28C9FCb12A97441488f9C68FaA49811a98688a);

    uint256 private constant PERCENTAGE_FACTOR = 100_00;

    IERC4626 public immutable vault;
    address public immutable AGETH;
    AggregatorV3Interface public immutable AGETH_ORACLE;
    ...
    // oracle delay time
    uint256 public MAX_ORACLE_DELAY;

    // False if we use deposit and True if we use swap
    bool public mode;

    // swap and deposit slippage to get min amount
    ...
    // swap path between asset and agETH
    IBaseLeverage.BiDirectSwapInfo swapInfo;
    ...
    constructor(
        ...
        AGETH = vault.asset();
        AGETH_ORACLE = AggregatorV3Interface(_vaultAssetOracle);
        ...
        slippage = _slippage;
        // Approve the pair
        ERC20(_asset).safeApprove(address(vault), type(uint256).max);
    }
    ...
    function _deployFunds(uint256 _amount) internal override {
        ...
        if (thresholdMet) {
            if (mode == false) {
                // deposit asset and get agETH
                amount = _depositToAGETH(_amount);
            } else {
                // swap to agETH from asset
                amount = _swapToAGETH(_amount);
            }
            // deposit agETH to yearn vault
            ERC20(AGETH).safeApprove(address(vault), 0);
            ERC20(AGETH).safeApprove(address(vault), amount);
    ...
    function _freeFunds(uint256 _amount) internal override {
        ...
        if (block.timestamp >= timeToUnlock) {
            uint256 agETHAmount = _getAgETHAmount(_amount, false);
            vault.redeem(
                Math.min(vault.convertToShares(agETHAmount), vault.balanceOf(address(this))),
                address(this),
                address(this)
            );
            // swap from agETH to asset
            _swapFromAGETH(IERC20(AGETH).balanceOf(address(this)));
        } else {
        ...
        returns (uint256 _totalAssets)
    {
        ...
        // swap to agETH from asset
        _swapToAGETH(_totalAssets);
        // deposit to yearn vault
        uint256 amount = IERC20(AGETH).balanceOf(address(this));
        ERC20(AGETH).safeApprove(address(vault), 0);
        ERC20(AGETH).safeApprove(address(vault), amount);
        vault.deposit(amount, address(this));
    }
    ...
    /**
     * @dev deposit asset and get agETH
     * @param _amount the asset amount
     * @return the amount of agETH
     */
    function _depositToAGETH(uint256 _amount) internal returns (uint256) {
        asset.safeApprove(address(RSETHAdapter), 0);
        asset.safeApprove(address(RSETHAdapter), _amount);

        RSETHAdapter.getRSETHWithERC20(address(asset), _amount, "");

        return IERC20(AGETH).balanceOf(address(this));
    }

    /**
     * @dev get the agETH amount from asset amount based on the slippage
     * @param _amount the asset amount
     * @return the amount of agETH
     */
    function _getAgETHAmount(uint256 _amount, bool _applySlippage) internal view returns (uint256) {
        (, int256 _answer, , uint256 _updatedAt, ) = AGETH_ORACLE.latestRoundData();     // agETH/WETH
        // If data is stale or negative, set bad data to true and return
        if (_answer <= 0 || (block.timestamp - _updatedAt > MAX_ORACLE_DELAY)) {
            revert CHAINLINK_BAD_PRICE();
        }

        uint256 _slippage = _applySlippage ? slippage : 0;
        return _amount * 1e18 * (PERCENTAGE_FACTOR - _slippage) / uint256(_answer) / PERCENTAGE_FACTOR;
    }

    /**
     * @dev get the asset amount from agETH amount based on the slippage
     * @param _agETHAmount the agETH amount
     * @return the amount of asset
     */
    function _getAmount(uint256 _agETHAmount, bool _applySlippage) internal view returns (uint256) {
        (, int256 _answer, , uint256 _updatedAt, ) = AGETH_ORACLE.latestRoundData();     // agETH/WETH
        // If data is stale or negative, set bad data to true and return
        if (_answer <= 0 || (block.timestamp - _updatedAt > MAX_ORACLE_DELAY)) {
            revert CHAINLINK_BAD_PRICE();
        ...
        uint256 _slippage = _applySlippage ? slippage : 0;
        return _agETHAmount * uint256(_answer) * (PERCENTAGE_FACTOR - _slippage) / 1e18 / PERCENTAGE_FACTOR;
    }

    /**
     * @dev swap from agETH to asset
     * @param _agETHAmount the agETH amount
     */
    function _swapFromAGETH(uint256 _agETHAmount) internal returns (uint256) {
        IBaseLeverage.BiDirectSwapInfo memory _swapInfo = swapInfo;

        if (_swapInfo.pathLength == 0) revert INVALID_CONFIGURATION();

        uint256 amount = _agETHAmount;
        if (amount == 0) return 0;

        for (uint256 i; i < _swapInfo.pathLength; ++i) {
            if (_swapInfo.reversePaths[i].swapType == IBaseLeverage.SwapType.NONE) continue;

            amount = _swapByPath(amount, _swapInfo.reversePaths[i], false);
        }

        if (amount < _getAmount(_agETHAmount, true)) revert SLIPPAGE_LOW();
        
        return amount;
    }

    /**
     * @dev swap from asset to agETH
     * @param _amount the asset amount
     */
    function _swapToAGETH(uint256 _amount) internal returns (uint256) {
        IBaseLeverage.BiDirectSwapInfo memory _swapInfo = swapInfo;
        ...
        for (uint256 i; i < _swapInfo.pathLength; ++i) {
            if (_swapInfo.paths[i].swapType == IBaseLeverage.SwapType.NONE) continue;

            amount = _swapByPath(amount, _swapInfo.paths[i], false);
        }

        if (amount < _getAgETHAmount(_amount, true)) revert SLIPPAGE_LOW();
        
        return amount;
    }

    /**
     * @dev swap based on the path
     */
    function _swapByPath(
        ...
        if (poolCount == 0) revert INVALID_CONFIGURATION();
        ...
        if (_path.swapType == IBaseLeverage.SwapType.BALANCER) {
            ...
            return BalancerswapAdapter.swapExactTokensForTokens(
                _path.swapFrom,
                _path.swapTo,
                _fromAmount,
                path,
        ...
        if (_path.swapType == IBaseLeverage.SwapType.UNISWAP) {
            ...
            return UniswapAdapter.swapExactTokensForTokens(
                _path.swapFrom,
                _path.swapTo,
                _fromAmount,
                path,
        ...
        // Curve Swap
        return CurveswapAdapter.swapExactTokensForTokens(
            _path.swapFrom,
            _path.swapTo,
            _fromAmount,
    ...
    function _emergencyWithdraw(uint256 _amount) internal override {
        ...
        uint256 shares = vault.convertToShares(agETHAmount);

        shares = Math.min(shares, vault.balanceOf(address(this)));
    ...
    // set the deposit/swap way between asset and weETHs.
    function setMaxOracleDelay(uint256 _maxOracleDelay) external onlyManagement {
        MAX_ORACLE_DELAY = _maxOracleDelay;
    }

    // set the deposit/swap way between asset and agETH.
    function setMode(bool _mode) external onlyManagement {
        mode = _mode;
    }

    // Set the slippage percent 1% = 100.
    function setSlippage(uint256 _slippage) external onlyManagement {
        slippage = _slippage;
    }

    // Set the swap path
    function setSwapInfo(IBaseLeverage.BiDirectSwapInfo memory _swapInfo) external onlyManagement {
        if (_swapInfo.pathLength == 0) revert INVALID_CONFIGURATION();
        if (_swapInfo.paths[0].swapFrom != address(asset)) revert INVALID_CONFIGURATION();
        if (_swapInfo.paths[_swapInfo.pathLength - 1].swapTo != AGETH) revert INVALID_CONFIGURATION();
        if (_swapInfo.reversePaths[0].swapFrom != AGETH) revert INVALID_CONFIGURATION();
        if (_swapInfo.reversePaths[_swapInfo.pathLength - 1].swapTo != address(asset)) revert INVALID_CONFIGURATION();

        for (uint256 i; i < _swapInfo.pathLength; ++i) {
            swapInfo.paths[i] = _swapInfo.paths[i];
            swapInfo.reversePaths[i] = _swapInfo.reversePaths[i];
        }
        swapInfo.pathLength = _swapInfo.pathLength;
    }
}