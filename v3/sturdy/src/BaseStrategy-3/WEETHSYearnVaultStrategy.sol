...
contract WEETHSYearnVaultStrategy is BaseStrategy {
    using SafeERC20 for ERC20;

    ITellerWithMultiAssetSupport private constant TellerWithMultiAssetSupport = 
        ITellerWithMultiAssetSupport(0x99dE9e5a3eC2750a6983C8732E6e795A35e7B861);
    
    uint256 private constant PERCENTAGE_FACTOR = 100_00;

    IERC4626 public immutable vault;
    address public immutable WEETHS;
    AggregatorV3Interface public immutable WEETHS_ORACLE;
    ...
    // swap path between asset and weETHs
    ...
    constructor(
        ...
        WEETHS = vault.asset();
        WEETHS_ORACLE = AggregatorV3Interface(_vaultAssetOracle);
    ...
    function _deployFunds(uint256 _amount) internal override {
        ...
        if (thresholdMet) {
            if (mode == false) {
                // deposit asset and get weETHs
                amount = _depositToWEETHS(_amount);
            } else {
                // swap to weETHs from asset
                amount = _swapToWEETHS(_amount);
            }
            // deposit weETHs to yearn vault
            ERC20(WEETHS).safeApprove(address(vault), 0);
            ERC20(WEETHS).safeApprove(address(vault), amount);
    ...
    function _freeFunds(uint256 _amount) internal override {
        ...
        if (block.timestamp >= timeToUnlock) {
            uint256 weETHsAmount = _getWeETHsAmount(_amount, false);
            vault.redeem(
                Math.min(vault.convertToShares(weETHsAmount), vault.balanceOf(address(this))),
                address(this),
                address(this)
            );
            // swap from weETHs to asset
            _swapFromWEETHS(IERC20(WEETHS).balanceOf(address(this)));
        } else {
        ...
        returns (uint256 _totalAssets)
    {
        ...
        // swap to weETHs from asset
        _swapToWEETHS(_totalAssets);
        // deposit to yearn vault
        uint256 amount = IERC20(WEETHS).balanceOf(address(this));
        ERC20(WEETHS).safeApprove(address(vault), 0);
        ERC20(WEETHS).safeApprove(address(vault), amount);
        vault.deposit(amount, address(this));
    }
    ...
    /**
     * @dev deposit asset and get weETHs
     * @param _amount the asset amount
     * @return the amount of weETHs
     */
    function _depositToWEETHS(uint256 _amount) internal returns (uint256) {
        asset.safeApprove(WEETHS, 0);
        asset.safeApprove(WEETHS, _amount);

        TellerWithMultiAssetSupport.deposit(address(asset), _amount, _getWeETHsAmount(_amount, true));

        return IERC20(WEETHS).balanceOf(address(this));
    }

    /**
     * @dev get the weETHs amount from asset amount based on the slippage
     * @param _amount the asset amount
     * @return the amount of weETHs
     */
    function _getWeETHsAmount(uint256 _amount, bool _applySlippage) internal view returns (uint256) {
        (, int256 _answer, , uint256 _updatedAt, ) = WEETHS_ORACLE.latestRoundData();     // weETHs/WETH
    ...
    /**
     * @dev get the asset amount from weETHs amount based on the slippage
     * @param _weETHsAmount the weETHs amount
     * @return the amount of asset
     */
    function _getAmount(uint256 _weETHsAmount, bool _applySlippage) internal view returns (uint256) {
        (, int256 _answer, , uint256 _updatedAt, ) = WEETHS_ORACLE.latestRoundData();     // weETHs/WETH
        // If data is stale or negative, set bad data to true and return
        if (_answer <= 0 || (block.timestamp - _updatedAt > MAX_ORACLE_DELAY)) {
            revert CHAINLINK_BAD_PRICE();
        ...
        uint256 _slippage = _applySlippage ? slippage : 0;
        return _weETHsAmount * uint256(_answer) * (PERCENTAGE_FACTOR - _slippage) / 1e18 / PERCENTAGE_FACTOR;
    }

    /**
     * @dev swap from weETHs to asset
     * @param _weETHsAmount the weETHs amount
     */
    function _swapFromWEETHS(uint256 _weETHsAmount) internal returns (uint256) {
        IBaseLeverage.BiDirectSwapInfo memory _swapInfo = swapInfo;

        if (_swapInfo.pathLength == 0) revert INVALID_CONFIGURATION();

        uint256 amount = _weETHsAmount;
        ...
        if (amount < _getAmount(_weETHsAmount, true)) revert SLIPPAGE_LOW();
        
        return amount;
    }

    /**
     * @dev swap from asset to weETHs
     * @param _amount the asset amount
     */
    function _swapToWEETHS(uint256 _amount) internal returns (uint256) {
        IBaseLeverage.BiDirectSwapInfo memory _swapInfo = swapInfo;
        ...
        if (amount < _getWeETHsAmount(_amount, true)) revert SLIPPAGE_LOW();
    ...
    function _emergencyWithdraw(uint256 _amount) internal override {
        ...
        uint256 shares = vault.convertToShares(weETHsAmount);
    ...
    function setSwapInfo(IBaseLeverage.BiDirectSwapInfo memory _swapInfo) external onlyManagement {
        ...
        if (_swapInfo.paths[_swapInfo.pathLength - 1].swapTo != WEETHS) revert INVALID_CONFIGURATION();
        if (_swapInfo.reversePaths[0].swapFrom != WEETHS) revert INVALID_CONFIGURATION();
