...
/**
    @title Sturdy Gauge
 */
contract Gauge is Ownable, ERC20 {
    using SafeERC20 for IERC20;

    uint256 private constant REWARD_DURATION = 1 weeks;

    IERC20 public immutable STRDY;
    ...
    event RewardClaimed(address indexed receiver, uint256 strdyAmount);
    event Stake(address receiver, uint256 amount);
    event Unstake(uint256 amount);

    error CallerIsNotPool();
    error CallerIsNotVault();
    error EmissionIDAlreadyRegistered();
    error IncorrectEmissionIDCount();
    error CanOnlyFetchOncePerWeek();

    constructor(address _pool, address _vault, address _strdy) ERC20(
        string(
            abi.encodePacked(
                IERC20Metadata(_pool).name(),
                " Gauge"
            )
        ), 
        string(
            abi.encodePacked(
                IERC20Metadata(_pool).symbol(),
                "-G"
            )
        )
    ) {
        pool = _pool;
        vault = ISTRDYVault(_vault);
        STRDY = IERC20(_strdy);
    ...
    function initialize() external {
        STRDY.safeApprove(address(vault), type(uint256).max);
    }

    function notifyRegisteredId(uint256[] memory assignedIds) external returns (bool) {
        if (msg.sender != address(vault)) revert CallerIsNotVault();
        if (emissionId != 0) revert EmissionIDAlreadyRegistered();
        if (assignedIds.length != 1) revert IncorrectEmissionIDCount();
    ...
    function stake(address _receiver, uint256 _amount) external {
        uint256 balance = balanceOf(_receiver);
        uint256 supply = totalSupply();

        IERC20(pool).safeTransferFrom(msg.sender, address(this), _amount);

        _mint(_receiver, _amount);

        _updateIntegrals(_receiver, balance, supply);
        if (_checkPeriodFinish()) {
            _fetchRewards();
        }
    ...
    function unstake(uint256 _amount) external {
        ...
        IERC20(pool).safeTransfer(msg.sender, _amount);
    ...
    function fetchRewards() external {
        if (!_checkPeriodFinish()) {
            revert CanOnlyFetchOncePerWeek();
        }
    ...
    function claimReward(address receiver) external returns (uint256 strdyAmount) {
        strdyAmount = _claimReward(msg.sender);
        vault.transferAllocatedTokens(msg.sender, receiver, strdyAmount);

        emit RewardClaimed(receiver, strdyAmount);
    }

    function vaultClaimReward(address claimant, address receiver) external returns (uint256) {
        require(msg.sender == address(vault));
        uint128 strdyAmount = _claimReward(claimant);

        emit RewardClaimed(claimant, strdyAmount);
        return strdyAmount;
    }
    ...
    function claimableReward(
        address account,
        uint256
    ) external view returns (uint256 strdyAmount) {
        ...
        strdyAmount = storedPendingReward[account] + ((balance * (integral - integralFor)) / 1e18);
    }

    function decimals() public view override returns (uint8) {
        return IERC20Metadata(pool).decimals();
    }

    function _checkPeriodFinish() internal view returns (bool) {
        return (block.timestamp / 1 weeks >= periodFinish / 1 weeks);
    }
    ...
    function _transfer(address _from, address _to, uint256 _amount) internal override {
        ...
        _updateIntegrals(_from, fromBalance, supply);
        _updateIntegrals(_to, toBalance, supply);
    }

    function _fetchRewards() internal {
        uint256 strdyAmount;
        uint256 id = emissionId;
        if (id > 0) (strdyAmount, ) = vault.allocateNewEmissions(id);

        uint256 _periodFinish = periodFinish;
        if (block.timestamp < _periodFinish) {
            uint256 remaining = _periodFinish - block.timestamp;
            strdyAmount += remaining * rewardRate;
        }

        rewardRate = uint128(strdyAmount / REWARD_DURATION);
    ...
    function _claimReward(address claimant) internal returns (uint128 amounts) {
    ...
    function _pushExcessEmissions(uint256 newAmount) internal {
        if (vault.lockWeeks() > 0) {
            ...
            vault.increaseUnallocatedSupply(STRDY.balanceOf(address(this)));
        }
    }
}