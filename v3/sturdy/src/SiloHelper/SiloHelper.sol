...
contract SiloHelper is ReentrancyGuard {
    using SafeERC20 for IERC20;

    error AG_INVALID_CONFIGURATION();
    error AG_REPAY_FAILED();

    /**
     * @dev Repay asset and withdraw collateral from `silo`
     * - Caller is anyone
     * @param _silo The silo address.
     * @param _repayAmount The repaying amount.
     * @param _collateralAmount The collateral amount to withdraw.
     * @param _asset The lending asset address.
    ...
    function repayAndWithdrawCollateral(
        ...
        if (_asset != ISturdyPair(_silo).asset()) revert AG_INVALID_CONFIGURATION();
        if (_repayAmount == 0) revert AG_INVALID_CONFIGURATION();
        if (_collateralAmount == 0) revert AG_INVALID_CONFIGURATION();

        IERC20(_asset).safeTransferFrom(msg.sender, address(this), _repayAmount);

        // repay
        ISturdyPair(_silo).addInterest(false);
        uint256 borrowShares =  ISturdyPair(_silo).toBorrowShares(_repayAmount, false, false);

        IERC20(_asset).safeApprove(_silo, 0);
        IERC20(_asset).safeApprove(_silo, _repayAmount);
        uint256 paybackAmount = ISturdyPair(_silo).repayAsset(borrowShares, msg.sender);
        if (paybackAmount == 0) revert AG_REPAY_FAILED();

        // withdraw collateral
        ISturdyPair(_silo).removeCollateralFrom(_collateralAmount, msg.sender, msg.sender);
    }
}