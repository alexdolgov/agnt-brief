...
contract SturdyLeverage is Ownable, BaseLeverage {
  ...
  function _enterPositionWithFlashloan(
    ...
    if (pair.asset() == _borrowAsset) {
      //borrow
      _borrow(_params.silo, _borrowedAmount + _fee, _params.user);
    } else {
      ...
      //borrow
      _borrow(_params.silo, borrowAmount, _params.user);
  ...
  function _afterLeverageWithFlashloan(
    ...
    IERC20(asset).safeApprove(_params.silo, 0);
    IERC20(asset).safeApprove(_params.silo, assetAmount);
    ISturdyPair(_params.silo).deposit(assetAmount, _params.user);
  }
