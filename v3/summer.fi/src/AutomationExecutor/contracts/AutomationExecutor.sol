...
contract AutomationExecutor {
    ...
    constructor(IAutomationBot _bot, IWETH _weth, IServiceRegistry _serviceRegistry) {
        require(address(_bot) != address(0), "executor/invalid-bot");
        require(address(_weth) != address(0), "executor/invalid-weth");
        require(address(_serviceRegistry) != address(0), "executor/invalid-service-registry");
        address uniswapV3RouterAddress = _serviceRegistry.getRegisteredService(UNISWAP_ROUTER_KEY);
        require(uniswapV3RouterAddress != address(0), "executor/invalid-uniswap-v3-router-address");
        address uniswapV3FactoryAddress = _serviceRegistry.getRegisteredService(
            UNISWAP_FACTORY_KEY
        );
        require(
            uniswapV3FactoryAddress != address(0),
            "executor/invalid-uniswap-v3-factory-address"
        );
