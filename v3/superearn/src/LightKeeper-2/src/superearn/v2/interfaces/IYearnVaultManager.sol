...
interface IYearnVaultManager {
    ...
    /// @notice Emitted when a strategy's deposit limit is updated
    /// @param vault The managed Yearn vault
    /// @param strategy The strategy whose limit was updated
    /// @param limit New absolute asset cap (0 means unlimited)
    event DepositLimitSet(address indexed vault, address indexed strategy, uint256 limit);
    ...
    /**
     * @notice Set the deposit limit for a single strategy
     * @dev A value of 0 means UNLIMITED. Values are interpreted in the vault's
     *      underlying asset units (NOT BPS).
     * @param strategy Address of the strategy to set the limit for
     * @param limit Maximum absolute asset amount allowed to be allocated to the strategy
     */
    function setDepositLimit(address strategy, uint256 limit) external;
    ...
    /**
     * @notice Set the withdrawal queue order for the vault
     * @dev Wrapper for IVault.setWithdrawalQueue
     * @param queue Array of strategy addresses in withdrawal order
     */
    function setWithdrawalQueue(address[20] memory queue) external;

    /**
     * @notice Add a strategy to the end of the withdrawal queue
     * @dev Wrapper for IVault.addStrategyToQueue
     * @param strategy Address of the strategy to add
     */
    function addStrategyToQueue(address strategy) external;

    /**
     * @notice Remove a strategy from the withdrawal queue
     * @dev Wrapper for IVault.removeStrategyFromQueue
    ...
    /**
     * @notice Get the per-strategy deposit limit (max asset allocation)
     * @dev A return value of 0 means UNLIMITED (no cap is applied to the strategy).
     * @param strategy Address of the strategy
     * @return Maximum absolute asset amount allowed to be allocated to the strategy
     */
    function getDepositLimit(address strategy) external view returns (uint256);
