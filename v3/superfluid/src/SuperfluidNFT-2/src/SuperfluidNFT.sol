...
contract SuperfluidNFT is ERC721Upgradeable, OwnableUpgradeable {
    ...
    /// @notice Whether linear price scaling is enabled (price increases by mintPrice per token minted)
    bool public linearPriceScaling;
    ...
    /**
     * @notice Sets the linear price scaling flag
     * @dev Only callable by the owner. When enabled, each token costs mintPrice * (tokenNumber),
     *      e.g. if mintPrice=0.1 ETH, token #1 costs 0.1, #2 costs 0.2, #3 costs 0.3, etc.
     * @param _linearPriceScaling Whether linear price scaling is enabled
     */
    function setLinearPriceScaling(bool _linearPriceScaling) external onlyOwner {
        linearPriceScaling = _linearPriceScaling;
    }
    ...
    /**
     * @notice Returns the total cost for `user` to mint `quantity` more tokens
     * @param user The address minting (price is tracked per-user, not globally)
     * @param quantity The number of tokens to mint
     * @return totalCost The total cost in wei
     */
    function getCurrentMintCost(address user, uint256 quantity) external view returns (uint256 totalCost) {
        totalCost = _getCurrentMintCost(user, quantity);
    }

    /**
     * @notice Calculates the total cost for `user` to mint `quantity` more tokens
     * @dev When linearPriceScaling is enabled, the n-th token a user mints costs mintPrice * n.
     *      For `quantity` tokens starting at the user's (tokensMinted[user]+1)-th mint:
     *      totalCost = mintPrice * quantity * (2*tokensMinted[user] + quantity + 1) / 2
     *      Scaling is per-user: other users' mints don't affect this user's price.
     * @param user The address minting
     * @param quantity The number of tokens to mint
     * @return totalCost The total cost in wei
     */
    function _getCurrentMintCost(address user, uint256 quantity) internal view returns (uint256 totalCost) {
        if (!linearPriceScaling) {
            return mintPrice * quantity;
        }
        uint256 u = tokensMinted[user];
        totalCost = mintPrice * quantity * (2 * u + quantity + 1) / 2;
    }
        ...
        view
    {
        ...
        // Verify that the payment is correct (price is per-user under linear scaling)
        if (msg.value != _getCurrentMintCost(msg.sender, quantity)) revert INSUFFICIENT_PAYMENT();
