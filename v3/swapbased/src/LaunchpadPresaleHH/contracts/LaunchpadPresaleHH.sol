...
contract LaunchpadPresaleHH is Ownable, ReentrancyGuard {
    ...
    /****** Token Details ******/
    IERC20Extended public TKN = IERC20Extended(address(0x486C3fb721c7faA426D4E68D7769b4427F86a7d9)); // HVR
    IERC20Extended public raisedTKN;

    /****** Sale Raise Details ******/
    uint256 public TKNPerRaisedTKN = 6000000000000000000; // X TKN = 1 raisedTKN
    ...
    event Deposit(address indexed _presaler, uint256 amount);
    event claimTKNtoken(address indexed _presaler, uint256 _amount);
    ...
    function claimPhase1() external nonReentrant {  
        require(block.timestamp >= presaleEnd, "not claimable yet");
        UserInfo storage user = userInfo[msg.sender];
        require(user.claimedPhase1 == false, "claimedPhase1");
        uint256 amountPublic = user.pendingTotalTokensPublic;
        uint256 TKNamount = amountPublic;
        user.pendingTotalTokensPublic = user.pendingTotalTokensPublic;
        user.claimedPhase1 = true;
        safeTKNTransfer(msg.sender, TKNamount);
        emit claimTKNtoken(msg.sender, TKNamount);
    }

    function withdrawLiquidityRaised() external nonReentrant {
        require(msg.sender == owner(), "Must be the funds receiver");
        require(block.timestamp >= presaleEnd + 1 hours);
        uint256 balanceOfRaisedToken = raisedTKN.balanceOf(address(this));
        safeRaisedTKNTransfer(owner(), balanceOfRaisedToken);
    }
    ...
    function getUnsoldTokens(address _to, uint256 _amount) external {
        require(msg.sender == owner(), "Must be the funds receiver");
        require(block.timestamp > presaleEnd + 1 days);
