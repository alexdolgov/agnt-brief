...
contract LaunchpadPresaleWag is Ownable, ReentrancyGuard {
    ...
    /****** Token Details ******/
    IERC20Extended public TKN = IERC20Extended(address(0x2a0304bb9Dd8F50ec04c483ddAA8d586EAe0A2ff)); // WAG
    IERC20Extended public raisedTKN;

    /****** Sale Raise Details ******/
    uint256 public TKNPerRaisedTKN = 14285714285714285714; // X TKN = 1 raisedTKN
    uint256 public totalRaised; // total raised so far
    uint256 public presaleSoftcap; // private presale hardcap (lower than hardcap)
    ...
    constructor(
        ...
        uint256 _minCapUserPublic,
        uint256 _maxCapUserPublic,
        ...
        minCapUserPublic = _minCapUserPublic;
        maxCapUserPublic = _maxCapUserPublic;
    ...
    function depositGeneral(uint256 _amount) public nonReentrant {
        ...
        user.pendingTotalTokensPublic = user.amountPublic*TKNPerRaisedTKN.div(10**raisedTKN.decimals());
        totalRaised = totalRaised.add(_amount);
        remainingTokensAvailable = remainingTokensAvailable.sub(_amount);

        raisedTKN.safeTransferFrom(address(msg.sender), address(this), _amount);

        emit Deposit(_amount);
    }

    // Claim tokens after presale ends
    function claimPhase1() external nonReentrant {  
        require(block.timestamp >= presaleEnd, "not claimable yet"); // Claimable after presale (50%)
        UserInfo storage user = userInfo[msg.sender];
        require(user.claimedPhase1 == false, "claimedPhase1");
        uint256 amountPublic = user.pendingTotalTokensPublic;
        uint256 TKNamount = amountPublic.div(2);
        user.pendingTotalTokensPublic = user.pendingTotalTokensPublic.div(2);
        user.claimedPhase1 = true;
        safeTKNTransfer(msg.sender, TKNamount);
        emit claimTKNtoken(TKNamount);
    }

    function claimPhase2() external nonReentrant {
        UserInfo storage user = userInfo[msg.sender];
        require(user.claimedPhase1 == true, "must claim phase 1 first");
        require(block.timestamp >= presaleEnd + 7 days, "not claimable yet"); // Claimable after presale + 7 days (50%)
        require(user.claimedPhase2 == false, "claimedPhase2");
        uint256 TKNamount =  user.pendingTotalTokensPublic;
        user.claimedPhase2 = true;
        safeTKNTransfer(msg.sender, TKNamount);
        emit claimTKNtoken(TKNamount);
    }

    function withdrawLiquidityRaised() external nonReentrant {
        require(msg.sender == fundsReceiver || msg.sender == owner(), "Must be the funds receiver");
        require(block.timestamp >= presaleEnd);
        uint256 balanceOfRaisedToken = raisedTKN.balanceOf(address(this));
        uint256 ownerFee = balanceOfRaisedToken.mul(50).div(1000); // 5% of raisedFunds
        safeRaisedTKNTransfer(owner(), ownerFee);
        safeRaisedTKNTransfer(fundsReceiver, balanceOfRaisedToken.sub(ownerFee));
    }
    ...
    function safeRaisedTKNTransfer(address _to, uint256 _amount) internal {
        uint256 raisedTokenContractBalance = raisedTKN.balanceOf(address(this));
        if (_amount > raisedTokenContractBalance) {
            raisedTKN.transfer(_to, raisedTokenContractBalance);
        } else {
            raisedTKN.transfer(_to, _amount);
        }
    }
    ...
    function getUnsoldTokens(address _to, uint256 _amount) external {
        require(msg.sender == fundsReceiver || msg.sender == owner(), "Must be the funds receiver");
