...
contract LockSBF is Ownable, ReentrancyGuard {
    ITokenBurnable public xSBF;
    ITokenBurnable public SBF;
    ...
    constructor(ITokenBurnable _xSBF, ITokenBurnable _SBF, address _masterChef) {
        xSBF = _xSBF;
        SBF = _SBF;
    ...
    function lockSBF(uint256 amount) public nonReentrant {
        require(SBF.transferFrom(msg.sender, address(this), amount), "Transfer of SBF failed");
        SBF.burn(amount);

        IMasterChef(masterChef).mintRewards(msg.sender, amount); // mints xSBF to user
    }
