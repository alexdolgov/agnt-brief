...
interface IMasterChefCoin {
    function mintRewardsByAddress(address _receiver, uint256 _amount, address _token) external;
}
...
contract MasterChefV3 is INonfungiblePositionManagerStruct, Multicall, Ownable, ReentrancyGuard, Enumerable {
    ...
    function setMasterchefCoin(address _masterchefCoin) external onlyOwner {
        masterchefCoin = _masterchefCoin;
    }
    ...
    function _safeTransfer(address _to, uint256 _amount, uint256 _pid) internal {
        if (_amount > 0) {
            ...
            for (uint256 i = 0; i < rewardsRatio.length; i++) {
                ...
                IMasterChefCoin(masterchefCoin).mintRewardsByAddress(_to, rewardsAmount, rewardsAddresses[i]);
                // treasury emissions
                uint256 treasuryEmissions =  ((rewardsRatio[i] * _amount) / REWARDS_PRECISION) * (ownerFee / REWARDS_PRECISION);
                IMasterChefCoin(masterchefCoin).mintRewardsByAddress(_to, treasuryEmissions, rewardsAddresses[i]);
            }
        }
    }
