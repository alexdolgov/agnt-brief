 ...
 */
contract Synthesis is ISynthesis, RelayRecipientUpgradeable {
    /// ** PUBLIC states **
    ...
    /// @custom:oz-renamed-from mintSerialBTC
    BtcSerial __mintSerialBTC; // deprecated
    /// @custom:oz-renamed-from burnSerialBTC
    BtcSerial __burnSerialBTC; // deprecated
    /// @custom:oz-renamed-from minFeeBTC
    uint256 __minFeeBTC; // deprecated

    /// @custom:oz-renamed-from syntToMintSerialBTC
    mapping(address => BtcSerial) public realToMintSerialBTC;
    /// @custom:oz-renamed-from syntToBurnSerialBTC
    mapping(address => BtcSerial) public realToBurnSerialBTC;
    mapping(address => uint256) public syntToMinFeeBTC;
    ...
    event ClientIdLogBTC(BtcSerial burnSerial, bytes32 indexed clientId);
    ...
    function metaMintSyntheticTokenBTC(
        ...
        // This event should be emitted each time BTC is synthesized
        emit BTCSynthesizeCompleted(
            _metaMintTransaction.externalID,
            _metaMintTransaction.to,
            _metaMintTransaction.amount,
            _metaMintTransaction.serial,
            _metaMintTransaction.stableBridgingFee,
            _metaMintTransaction.tokenReal);

        _metaMintTransaction.amount = _metaMintTransaction.amount - _metaMintTransaction.stableBridgingFee;

        if (_metaMintTransaction.receiveSideCalldata.length == 0) {
            TransferHelper.safeTransfer(
                syntReprAddr,
                _metaMintTransaction.to,
                _metaMintTransaction.amount
        ...
        // metaRouter swap
        try metaRouter.externalCall(
            syntReprAddr,
            _metaMintTransaction.amount,
            _metaMintTransaction.receiveSide,
            _metaMintTransaction.receiveSideCalldata,
            _metaMintTransaction.receiveSideOffset,
            _metaMintTransaction.to
        ) {
        } catch {
            metaRouter.sendTransitToken(syntReprAddr, _metaMintTransaction.to, _metaMintTransaction.amount);
        }
     ...
     * @param _stableBridgingFee Bridging fee on another network
     * @param _amount Number of tokens to burn
     * @param _to The address to which the user wants to receive tokens
     * @param _stoken SyntERC20 token
     */
    function burnSyntheticTokenBTC(
        ...
        address _stoken,
        bytes32 _clientID
    ) external whenNotPaused returns (BtcSerial) {
        require(_amount >= tokenThreshold[_stoken], "Symb: amount under threshold");
        require(syntToMinFeeBTC[_stoken] <= _stableBridgingFee, "Symb: insufficient fee");

        ISyntFabric(fabric).unsynthesize(_msgSender(), _amount, _stoken);
        ISyntFabric(fabric).synthesize(bridge, _stableBridgingFee, _stoken);

        address rtoken = ISyntFabric(fabric).getRealRepresentation(_stoken);
        require(rtoken != address(0), "Symb: incorrect synt");

        realToBurnSerialBTC[rtoken] = realToBurnSerialBTC[rtoken].inc();

        IBridge(bridge).transmitRequestBTC(
            _msgSender(),
            _to,
            _amount - _stableBridgingFee,
            realToBurnSerialBTC[rtoken]
        );
        emit BurnRequestBTC(realToBurnSerialBTC[rtoken], _msgSender(), _to, _amount - _stableBridgingFee, _stableBridgingFee, rtoken);
        emit ClientIdLogBTC(realToBurnSerialBTC[rtoken], _clientID);
        return realToBurnSerialBTC[rtoken];
    }

    function burnSyntheticTokenBTCByAdmin(
        address _user,
        uint256 _stableBridgingFee,
        uint256 _amount,
        bytes calldata _to,
        address _stoken,
        bytes32 _clientID
    ) external whenNotPaused onlyOwner returns (BtcSerial) {
        require(_amount >= tokenThreshold[_stoken], "Symb: amount under threshold");
        require(syntToMinFeeBTC[_stoken] <= _stableBridgingFee, "Symb: insufficient fee");

        ISyntFabric(fabric).unsynthesize(_user, _amount, _stoken);
        ISyntFabric(fabric).synthesize(bridge, _stableBridgingFee, _stoken);
        ...
        emit BurnRequestBTC(realToBurnSerialBTC[rtoken], _user, _to, _amount - _stableBridgingFee, _stableBridgingFee, rtoken);
        emit ClientIdLogBTC(realToBurnSerialBTC[rtoken], _clientID);
        return realToBurnSerialBTC[rtoken];
    }
    ...
    /**
    * @notice Sets minimal fee for btc burn
     * @param _synt to set fee for
     * @param _minFee new fee value (satoshi)
     */
    function setMinFeeBTC(address _synt, uint256 _minFee) external onlyOwner {
        syntToMinFeeBTC[_synt] = _minFee;
        emit SetMinFeeBTC(_minFee);
    }
