...
/**
 * @title MetaRouterExecutorDontApprove
 */
contract MetaRouterExecutorDontApprove is Context {
    ...
    function metaRoute(
        ...
        if (_metarouteTransaction.firstSwapCalldata.length != 0) {
...

            {
                ...
                require(size != 0, "MetaRouterExecutorDontApprove: call for a non-contract account");
            }
            ...
            if (!firstSwapSuccess) {
                revert(RevertMessageParser.getRevertMessage(swapData, "MetaRouterExecutorDontApprove: first swap failed"));
            }
        ...
        if (_metarouteTransaction.secondSwapCalldata.length != 0) {
            ...
            if (!secondSwapSuccess) {
                revert(RevertMessageParser.getRevertMessage(swapData, "MetaRouterExecutorDontApprove: second swap failed"));
            }
        ...
        if (!otherSideCallSuccess) {
            revert(RevertMessageParser.getRevertMessage(data, "MetaRouterExecutorDontApprove: other side call failed"));
        }
    }
    ...
    function returnSwap(
        ...
        if (!success) {
            revert(RevertMessageParser.getRevertMessage(data, "MetaRouterExecutorDontApprove: internal swap failed"));
        }
        ...
        if (!otherSideCallSuccess) {
            revert(RevertMessageParser.getRevertMessage(burnData, "MetaRouterExecutorDontApprove: revertSynthesizeRequest call failed"));
        }
    }
    ...
    function metaMintSwap(
        ...
        if (_metaMintTransaction.secondSwapCalldata.length != 0) {
            ...
            if (!internalSwapSuccess) {
                revert(RevertMessageParser.getRevertMessage(internalSwapData, "MetaRouterExecutorDontApprove: internal swap failed"));
            }
        ...
        if (_metaMintTransaction.finalCalldata.length != 0) {
            ...
            if (!finalSuccess) {
                revert(RevertMessageParser.getRevertMessage(finalData, "MetaRouterExecutorDontApprove: final call failed"));
            }
        }
