...
contract SyntFabric is OwnableUpgradeable, ISyntFabric {
    ...
    * @notice Burns synthetic tokens
    ...
    /**
     * @notice function for deleting representation by admin
     * @param _sToken address of synthetic token
     * @param _chainID Chain id of the network
     */
    function deleteRepresentationByAdmin(
        address _sToken,
        uint256 _chainID
    ) external onlyOwner {
        address rtoken = representationReal[_sToken];
        representationReal[_sToken] = address(0);
        representationSynt[keccak256(abi.encodePacked(rtoken, _chainID))] = address(0);
        emit RepresentationDeleted(_sToken);
    }

    /**
     * @notice function for changing representation by admin
     * @param _sToken address of synthetic token
     * @param _newRtoken address of new real token
     * @param _chainID Chain id of the network
     */
    function setRepresentationByAdmin(
        address _sToken,
        address _newRtoken,
        uint256 _chainID
    ) external onlyOwner {
        require(_newRtoken != address(0), "Symb: rtoken is the zero address");
        require(
            representationReal[_sToken] == address(0x0),
            "Symb: token representation already set"
        );
        representationReal[_sToken] = _newRtoken;
        representationSynt[keccak256(abi.encodePacked(_newRtoken, _chainID))] = _sToken;
        emit RepresentationSet(_sToken, _newRtoken);
    }
