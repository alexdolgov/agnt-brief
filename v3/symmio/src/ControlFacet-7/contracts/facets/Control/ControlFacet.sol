...
contract ControlFacet is Accessibility, Ownable, IControlFacet {
	...
	/// @notice Sets the name of a specific symbol.
	/// @param symbolId The ID of the symbol whose name is to be set.
	/// @param name The new name for the symbol.
	function setSymbolName(uint256 symbolId, string memory name) external onlyRole(LibAccessibility.SYMBOL_MANAGER_ROLE) {
		SymbolStorage.Layout storage symbolLayout = SymbolStorage.layout();
		require(symbolId <= symbolLayout.lastId, "ControlFacet: Invalid id");
		emit SetSymbolName(symbolId, symbolLayout.symbols[symbolId].name, name);
		symbolLayout.symbols[symbolId].name = name;
	}
