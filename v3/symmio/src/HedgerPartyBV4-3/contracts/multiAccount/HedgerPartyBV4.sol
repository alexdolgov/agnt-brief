...
contract HedgerPartyBV4 is HedgerPartyBV3 {
  ...
  function withdrawTo(address _to, uint256 _amount) external override virtual onlyRebalancerContract {
  ...
  function rebalance(address rebalancer, address targetAddress, uint256 amount) external override virtual onlyRebalancer {
  ...
  function isContractWhitelisted(address _addr) private returns (bool) {
    PartyBStorageV4 storage sv4 = storeV4();
  ...
  function isAddressWhitelisted(address _addr) private returns (bool) {
    PartyBStorageV4 storage sv4 = storeV4();
  ...
  function isRebalancerWhitelisted(address _addr) private returns (bool) {
    PartyBStorageV4 storage sv4 = storeV4();
    return _isElementInArray(sv4.rebalancers, _addr);
  }

  function _pushToArray(address[] storage array, address[] calldata items) private {
    for (uint256 i; i < items.length; i++) {
      array.push(items[i]);
    }
  }

  function _isElementInArray(address[] storage array, address item) private view returns (bool) {
