/**
 *Submitted for verification at basescan.org on 2024-08-19
*/
...
// File: contracts/MagSymSymmioPartyB.sol
...
contract MagSymSymmioPartyB is Initializable, PausableUpgradeable, AccessControlUpgradeable {
	...
	constructor(address[] memory admins, address withdrawalAddress_, address symmioAddress_) {
		initialize(admins, withdrawalAddress_, symmioAddress_);
		_disableInitializers();
	}
	 ...
	 */
	function initialize(address[] memory admins, address withdrawalAddress_, address symmioAddress_) public initializer {
		__Pausable_init();
        ...
        for (uint8 i; i < admins.length; i++) {
            ...
            _grantRole(TRUSTED_ROLE, admins[i]);
            _grantRole(MANAGER_ROLE, admins[i]);
        }
	}
	...
	function withdrawERC20(address targetAddress, address token, uint256 amount) external onlyRole(MANAGER_ROLE) {
        require(targetAddress == withdrawalAddress, 'Invalid target withdrawal Address!');
