...
contract MultiAccount is IMultiAccount, Initializable, PausableUpgradeable, AccessControlUpgradeable {
	...
	function createAccounts(AccountCreationParams[] memory params) public whenNotPaused returns (address[] memory accountAddresses) {
		...
		for (uint256 i = 0; i < params.length; i++) {
			...
			// Create the account
			address account = _addAccount(msg.sender, param.name);
			accountAddresses[i] = account;

			// Bind to PartyB if specified
			if (param.partyB != address(0)) {
				accountToPartyBBinding[account] = param.partyB;
	 ...
	 * @notice It is there only for backward compatibility
	 */
	function addAccount(string memory name) external whenNotPaused {
		AccountCreationParams[] memory params = new AccountCreationParams[](1);
		params[0] = AccountCreationParams({ name: name, partyB: address(0), rflCode: "", referrerTokenId: 0 });
		createAccounts(params);
	}
