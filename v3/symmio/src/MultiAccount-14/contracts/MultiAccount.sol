...
contract MultiAccount is IMultiAccount, Initializable, PausableUpgradeable, AccessControlUpgradeable {
	...
	function getVibeAccount(address account) external view returns (VibeAccount memory) {
		for (uint256 i = 0; i < accounts[account].length; i++) {
			if (accounts[account][i].accountAddress == account) {
				return VibeAccount({ accountAddress: account, name: accounts[account][i].name, data: vibeAccountData[account] });
			}
		}
