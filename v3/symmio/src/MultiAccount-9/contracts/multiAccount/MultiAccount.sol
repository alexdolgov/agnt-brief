...
contract MultiAccount is IMultiAccount, Initializable, PausableUpgradeable, AccessControlUpgradeable {
	 ...
	 * @param accountName      Name for both the account.
	 * @param rflCode          Code for RFL nft.
	 * @param referrerTokenId  Token ID of the referring RFL NFT.
	 */
	function addAccountWithReferrer(string memory accountName, string memory rflCode, uint256 referrerTokenId) external whenNotPaused {
		_addAccount(msg.sender, accountName);

		if (referrerTokenId == 0) {
			IRFL(vibeRfl).safeMintFor(msg.sender, rflCode);
		} else {
			IRFL(vibeRfl).safeMintWithReferrerFor(msg.sender, referrerTokenId, rflCode);
		}
	}
	 ...
	 */
	function addAccountWithBinding(string memory name, address _partyB) public whenNotPaused {
	 ...
	 * @param referrerTokenId  Token ID of the referring RFL NFT.
	 */
	function addAccountWithBindingAndReferrer(string memory accountName, string memory rflCode, address _partyB, uint256 referrerTokenId) external whenNotPaused {
		require(_partyB != address(0), "MultiAccount: Zero Address");
		address account = _addAccount(msg.sender, accountName);
		accountToPartyBBinding[account] = _partyB;
		emit BindToPartyB(account, _partyB);
