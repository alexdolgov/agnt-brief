...
/**
 * @title  MultiAccount
 * @notice Manages multiple trading accounts per user on the Symmio protocol.
 *         Users can create sub-accounts, delegate trading permissions, bind accounts
 *         to specific PartyB addresses, and integrate with the RFL referral system.
 *
 ...
 *         • RFL NFT integration for referral tracking
 *         • Pause functionality and access control
 */
...
import { IRFL } from "./interfaces/IRFL.sol";

import { SingleUpnlAndPriceSig } from "./types/MuonTypes.sol";
...
contract MultiAccount is IMultiAccount, Initializable, PausableUpgradeable, AccessControlUpgradeable {
	...
	/**
	 * @notice Grant delegated access to a specific function on a target contract.
	 * @param account   Account address to grant access for.
	 * @param target    Target contract address to delegate to.
	...
	/**
	 * @notice Grant delegated access to multiple function selectors on a target contract.
	 * @param account   Account address to grant access for.
	 * @param target    Target contract address to delegate to.
	...
	/**
	 * @dev Internal function to create a new trading account for a user.
	...
	/**
	 * @notice Create a new trading account with RFL NFT minting.
	 * @param name Unique name for both the account and RFL NFT.
	 */
	function addAccount(string memory name) external whenNotPaused {
		_addAccount(msg.sender, name);
		IRFL(vibeRfl).safeMint(name);
	}

	/**
	 * @notice Create a new trading account with RFL NFT minting and referrer assignment.
	 * @param name             Unique name for both the account and RFL NFT.
	 * @param referrerTokenId  Token ID of the referring RFL NFT.
	 */
	function addAccountWithReferrer(string memory name, uint256 referrerTokenId) external whenNotPaused {
		_addAccount(msg.sender, name);
		IRFL(vibeRfl).safeMintWithReferrer(referrerTokenId, name);
	}

	/**
	 * @notice Create a new trading account and bind it to a specific PartyB.
	...
	/**
	 * @notice Create account with PartyB binding, RFL NFT minting, and referrer assignment.
	 * @param name             Unique name for both the account and RFL NFT.
	 * @param _partyB          PartyB address to bind this account to.
	 * @param referrerTokenId  Token ID of the referring RFL NFT.
	 */
	function addAccountWithBindingAndReferrer(string memory name, address _partyB, uint256 referrerTokenId) external whenNotPaused {
		require(_partyB != address(0), "MultiAccount: Zero Address");
		address account = _addAccount(msg.sender, name);
		accountToPartyBBinding[account] = _partyB;
		IRFL(vibeRfl).safeMintWithReferrer(referrerTokenId, name);
	...
	/**
	 * @notice Deposit collateral tokens into a trading account.
	...
	/**
	 * @notice Deposit collateral tokens and immediately allocate them for trading.
	...
	/**
	 * @notice Withdraw collateral tokens from a trading account to the owner.
	...
	/**
	 * @notice Execute multiple function calls on behalf of a trading account.
	...
	/**
	 * @dev Execute a function call on a trading account with PartyB binding validation.
