...
contract MultiAccount is IMultiAccount, Initializable, PausableUpgradeable, AccessControlUpgradeable {
	 ...
	 */
	function initialize(address admin, bytes memory accountImplementation_) public initializer {
		__Pausable_init();
	 ...
	 */
	function pauseWithdrawalsForAccounts(address[] calldata accountsToPause) external onlyRole(PAUSER_ROLE) {
		for (uint256 i = 0; i < accountsToPause.length; i++) {
			address account = accountsToPause[i];
			require(owners[account] != address(0), "MultiAccount: Account doesn't exist");
	 ...
	 */
	function unpauseWithdrawalsForAccounts(address[] calldata accountsToUnpause) external onlyRole(PAUSER_ROLE) {
	 ...
	 */
	function pauseDepositsForAccounts(address[] calldata accountsToPause) external onlyRole(PAUSER_ROLE) {
		for (uint256 i = 0; i < accountsToPause.length; i++) {
			address account = accountsToPause[i];
			require(owners[account] != address(0), "MultiAccount: Account doesn't exist");
	 ...
	 */
	function unpauseDepositsForAccounts(address[] calldata accountsToUnpause) external onlyRole(PAUSER_ROLE) {
		for (uint256 i = 0; i < accountsToUnpause.length; i++) {
			address account = accountsToUnpause[i];
	 ...
	 */
	function createAccounts(AccountCreationParams[] memory params) public whenNotPaused returns (address[] memory accountAddresses) {
		...
		for (uint256 i = 0; i < params.length; i++) {
			...
			if (bytes(param.rflCode).length > 0) {
				if (param.referrerTokenId == 0) {
					...
					IRFL(vibeRfl).safeMintWithReferrerFor(msg.sender, param.referrerTokenId, param.rflCode);
				}
			}

			vibeAccountData[account] = VibeAccountData({ symmioAddress: param.symmioAddress, boundPartyB: param.partyB });

			if (param.partyB != address(0)) emit BindToPartyB(account, param.partyB);
	...
	function depositForAccount(address account, uint256 amount) external onlyOwner(account, msg.sender) whenNotPaused {
		require(!depositsPaused[account], "MultiAccount: Deposits paused for this account");
	...
	function withdrawFromAccount(address account, uint256 amount) external onlyOwner(account, msg.sender) whenNotPaused {
		require(!withdrawalsPaused[account], "MultiAccount: Withdrawals paused for this account");
	...
	function innerCall(address account, bytes memory _callData) internal {
		...
		if (_callData.length >= 4) {
			...
			if (withdrawalsPaused[account]) {
				require(functionSelector != SELECTOR_DEALLOCATE, "MultiAccount: Withdrawals paused for this account");
			}

			// Check if deposits are paused for this account and if it's a deposit/allocation function
			if (depositsPaused[account]) {
				require(
					functionSelector != SELECTOR_ALLOCATE && functionSelector != SELECTOR_TRANSFER_ALLOCATION,
	 ...
	 */
	function _deployPartyA(address symmioAddress) internal returns (address account) {
	 ...
	 */
	function getVibeAccounts(address user, uint256 start, uint256 size) external view returns (VibeAccount[] memory) {
		uint256 len = size > accounts[user].length - start ? accounts[user].length - start : size;
	 ...
	 */
	function getVibeAccount(address account) external view returns (VibeAccount memory) {
		address owner = owners[account];
		for (uint256 i = 0; i < accounts[owner].length; i++) {
			if (accounts[owner][i].accountAddress == account) {
				return VibeAccount({ accountAddress: account, name: accounts[owner][i].name, data: vibeAccountData[account] });
			}
		}
