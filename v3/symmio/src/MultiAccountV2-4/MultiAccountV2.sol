...
contract MultiAccountV2 is MultiAccount {
   ...
   */
  function addAccountWithReferral(string memory name, address referrer) public whenNotPaused {
   ...
   */
  function addAccountWithReferralAndDepositAndAllocate(string memory name, address referrer, uint256 amount) public whenNotPaused {
    addAccountWithReferral(name, referrer);
    depositAndAllocateForAccount(accounts[msg.sender][accounts[msg.sender].length - 1].accountAddress, amount);
  }
}
