...
library LibMuon {
    ...
    event LogEventBytes(string name, bytes value);
    event LogEventBytes32(string name, bytes32 value);
    event LogEventUint256(string name, uint256 value);
    event LogEventUint8(string name, uint8 value);
    event LogEventAddress(string name, address value);
    ...
    function verifyTSSAndGateway(
        ...
        emit LogEventUint256("Converted hash in verifyTSSAndGateway", uint256(hash));
        emit LogEventUint256("sign.signature in verifyTSSAndGateway", sign.signature);
        emit LogEventAddress("sign.owner in verifyTSSAndGateway", sign.owner);
        emit LogEventAddress("sign.nonce in verifyTSSAndGateway", sign.nonce);
        emit LogEventUint256("MuonStorage.layout().muonPublicKey.x in verifyTSSAndGateway", MuonStorage.layout().muonPublicKey.x);
        emit LogEventUint8("MuonStorage.layout().muonPublicKey.parity in verifyTSSAndGateway", MuonStorage.layout().muonPublicKey.parity);
    ...
    function verifyLiquidationSig(LiquidationSig memory liquidationSig, address partyA) internal {
    ...
    function verifyQuotePrices(QuotePriceSig memory priceSig) internal {
        MuonStorage.Layout storage muonLayout = MuonStorage.layout();
    ...
    function verifyPartyAUpnl(SingleUpnlSig memory upnlSig, address partyA) internal {
        MuonStorage.Layout storage muonLayout = MuonStorage.layout();
        require(
    ...
    function verifyPartyAUpnlAndPrice(
        ...
        emit LogEventUint256("muonLayout.muonAppId in verifyPartyAUpnlAndPrice", muonLayout.muonAppId);
        emit LogEventBytes("upnlSig.reqId of verifyPartyAUpnlAndPrice", upnlSig.reqId);
        emit LogEventBytes32("Hash of verifyPartyAUpnlAndPrice", hash);
        emit LogEventBytes("upnlSig.gatewaySignature of verifyPartyAUpnlAndPrice", upnlSig.gatewaySignature);
        verifyTSSAndGateway(hash, upnlSig.sigs, upnlSig.gatewaySignature);
    }
