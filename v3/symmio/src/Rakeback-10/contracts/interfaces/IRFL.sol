...
interface IRFL {
	...
	function getIndexOfTokenForUser(address user, uint256 tokenId) external view returns (uint256);

	// =========================== Public Mutative Functions ===========================

	function safeMint(string memory code) external returns (uint256);
	function safeMintWithReferrer(uint256 referrerTokenId, string memory code) external returns (uint256);
	...
	function ogMint(address user, string memory code) external returns (uint256);
	function setCollateral(address _collateral) external;
	function setLockThreshold(uint256 newLockThreshold) external;
	function setReferralOpenTimestamp(uint256 newReferralOpenTimestamp) external;
