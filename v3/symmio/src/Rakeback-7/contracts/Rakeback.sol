...
contract Rakeback is AccessControlEnumerableUpgradeable, ReentrancyGuardUpgradeable {
	...
	event RakebackTokenUpdated(address indexed oldAddr, address indexed newAddr);
	 ...
	 */
	function checkSignature(bytes32 ethHash, bytes calldata signature) public view returns (bool) {
		return SignatureChecker.isValidSignatureNow(vibeSigner, ethHash, signature);
	}

	/**
	 * @dev Internal signature verification that reverts on failure.
	 */
	function _checkSignature(bytes32 ethHash, bytes calldata signature) private view {
	 ...
	 */
	function vibeClaimSignatureHash(uint256 amount, uint256 sigTimestamp) public view returns (bytes32 dataHash, bytes32 ethHash) {
		dataHash = keccak256(abi.encode(block.chainid, address(this), amount, sigTimestamp));
	...
	function claimSignatureHash(
		...
		dataHash = keccak256(abi.encode(block.chainid, address(this), tokenId, account, amount, sigTimestamp, claimType));
		ethHash = _ethHash(dataHash);
	}
	...
	function _collectFeesAndVerifyBalance(uint256 withdrawable) private {
		...
		if (currentBalance < withdrawable) {
			...
			for (uint256 i = 0; i < collectorCount; i++) {
				...
				uint256 balanceBefore = IERC20(rakebackToken).balanceOf(address(this));

				// Try to claim from this fee collector
				try IFeeCollector(collector).claimAllFee() {
					uint256 collected = IERC20(rakebackToken).balanceOf(address(this)) - balanceBefore;
					if (collected > 0) {
	...
	function _processClaim(
		...
		if (IRFL(rfl).referrer(tokenId) == 0 && !IRFL(rfl).isOg(tokenId)) revert NotAllowedToClaim();

		(, bytes32 ethHash) = claimSignatureHash(tokenId, account, amount, sigTimestamp, claimType);
		...
		IMultiAccount.VibeAccount memory vibeAccount = IMultiAccount(multiAccount).getVibeAccount(account);

		IERC20(rakebackToken).approve(vibeAccount.data.symmioAddress, withdrawable);
		ISymmio(vibeAccount.data.symmioAddress).depositFor(address(this), withdrawable);

		uint256 amountWith18Decimals = (withdrawable * 1e18) / (10 ** IERC20Metadata(rakebackToken).decimals());
		ISymmio(vibeAccount.data.symmioAddress).internalTransfer(account, amountWith18Decimals);
	 ...
	 */
	function addFeeCollector(address _feeCollector) external onlyRole(SETTER_ROLE) {
		if (_feeCollector == address(0)) revert ZeroAddress();
		if (!feeCollectors.add(_feeCollector)) revert FeeCollectorAlreadyExists();
	 ...
	 */
	function removeFeeCollector(address _feeCollector) external onlyRole(SETTER_ROLE) {
		if (!feeCollectors.remove(_feeCollector)) revert FeeCollectorDoesNotExist();
	 ...
	 */
	function getFeeCollectors() external view returns (address[] memory collectors) {
	...
	function setRakebackToken(address newRakebackToken) external onlyRole(SETTER_ROLE) {
