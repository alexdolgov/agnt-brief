...
library InterestLogic {
    ...
    function isNotInitialized(HubSpokeStructs.AccrualIndices memory indices) public pure returns (bool) {
        return indices.deposited == 0 && indices.borrowed == 0;
    }

    function defaultIndex(uint256 _precision) public pure returns (HubSpokeStructs.AccrualIndices memory) {
        return HubSpokeStructs.AccrualIndices({ deposited: _precision, borrowed: _precision });
    }

    function indexOrDefault(HubSpokeStructs.AccrualIndices memory indices, uint256 _precision) public pure returns (HubSpokeStructs.AccrualIndices memory) {
        if (isNotInitialized(indices)) {
            return defaultIndex(_precision);
        }
        return indices;
    }
    ...
    function getCurrentAccrualIndices(HubSpokeStructs.HubState storage _state, address assetAddress) public view returns (HubSpokeStructs.AccrualIndices memory) {
        uint256 lastActivity = _state.lastActivityBlockTimestamps[assetAddress];
        if (lastActivity == block.timestamp) {
            return indexOrDefault(_state.indices[assetAddress], _state.interestAccrualIndexPrecision);
        } else {
            IAssetRegistry.AssetInfo memory info = _state.assetRegistry.getAssetInfo(assetAddress);
            if (!info.exists) {
            ...
            return getCurrentAccrualIndices(
                info,
                lastActivity,
                _state.totalAssets[assetAddress],
                indexOrDefault(_state.indices[assetAddress], _state.interestAccrualIndexPrecision),
                _state.interestAccrualIndexPrecision
            );
        }
    }
     ...
     * @param _assetInfo The asset info struct from AssetRegistry
    ...
    function getCurrentAccrualIndices(
        IAssetRegistry.AssetInfo memory _assetInfo,
        ...
        if (secondsElapsed != 0 && _globalAssetAmounts.amounts.borrowed != 0 && _globalAssetAmounts.amounts.deposited != 0) {
            IInterestRateCalculator assetCalculator = IInterestRateCalculator(_assetInfo.interestRateCalculator);
    ...
    function getVaultAmounts(HubSpokeStructs.HubState storage _state, address _vaultOwner, address _asset) public view returns (HubSpokeStructs.DenormalizedVaultAmount memory) {
        return applyInterest(_state.vault[_vaultOwner][_asset], getCurrentAccrualIndices(_state, _asset));
    }

    function getGlobalAmounts(HubSpokeStructs.HubState storage _state, address _asset) public view returns (HubSpokeStructs.DenormalizedVaultAmount memory) {
        return applyInterest(_state.totalAssets[_asset], getCurrentAccrualIndices(_state, _asset));
    }
}