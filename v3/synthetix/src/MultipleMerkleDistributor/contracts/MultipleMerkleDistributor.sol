...
contract MultipleMerkleDistributor is IMultipleMerkleDistributor, Owned {
    ...
    function _setClaimed(uint256 index, uint256 epoch) private {
        uint256 claimedWordIndex = index / 256;
        uint256 claimedBitIndex = index % 256;
        claimedBitMaps[epoch][claimedWordIndex] = claimedBitMaps[epoch][claimedWordIndex] | (1 << claimedBitIndex);
    }
    ...
    function claim(
        ...
        require(MerkleProof.verify(merkleProof, merkleRoots[epoch], node), 'MultipleMerkleDistributor: Invalid proof.');
    ...
    function claimMultiple(Claims[] calldata claims) external override {
        ...
        for (uint256 i = 0; i < cacheLength; ) {
            claim(claims[i].index, claims[i].account, claims[i].amount, claims[i].merkleProof, claims[i].epoch);
            unchecked {
                ++i;
            }
        }
