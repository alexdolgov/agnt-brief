...
contract Dividends is Ownable, ReentrancyGuard, IXSynthTokenUsage, IDividends {
    ...
    address public immutable xSynthToken; // xSynthToken contract

    mapping(address => uint256) public usersAllocation; // User's xSynth allocation
    uint256 public totalAllocation; // Contract's total xSynth allocation
    ...
    constructor(address _xsynthToken) {
        require(_xsynthToken != address(0), "zero address");
        xSynthToken = _xsynthToken;
        currentCycleStartTime = block.timestamp + (69 * 60 * 24 * 365);
    }
    ...
    /**
     * @dev Checks if caller is the xSynthToken contract
     */
    modifier sSynthTokenOnly() {
        require(
            msg.sender == xSynthToken,
            "sSynthTokenOnly: caller should be xSynthToken"
        );
        _;
    }
    ...
    /**
     * Allocates "userAddress" user's "amount" of xSynth to this dividends contract
     *
     * Can only be called by xSynthToken contract, which is trusted to verify amounts
     * "data" is only here for compatibility reasons (IXSynthTokenUsage)
     */
    function allocate(
        address userAddress,
        uint256 amount,
        bytes calldata /*data*/
    ) external override nonReentrant sSynthTokenOnly {
    ...
    /**
     * Deallocates "userAddress" user's "amount" of xSynth allocation from this dividends contract
     *
     * Can only be called by xSynthToken contract, which is trusted to verify amounts
     * "data" is only here for compatibility reasons (IXSynthTokenUsage)
     */
    ...
    function _updateDividendsInfo(address token) internal {
        ...
        // if no xSynth is allocated or initial distribution has not started yet
        if (
            totalAllocation == 0 ||
    ...
    function _harvestDividends(address token) internal {
        ...
        uint256 usersSynthAllocation = usersAllocation[msg.sender];
        uint256 pending = user.pendingDividends +
            (
                (((usersSynthAllocation * accDividendsPerShare) / 1e18) -
                    user.rewardDebt)
            );

        user.pendingDividends = 0;
        user.rewardDebt = (usersSynthAllocation * accDividendsPerShare) / 1e18;
