...
contract PTOraclePriceAdapterFactory is IPTOraclePriceAdapterFactory {
    ...
    /* EXTERNAL */
    /// @inheritdoc IPTOraclePriceAdapterFactory
    function createPTOraclePriceAdapter(IPTOracle _ptOracle, address _market, uint32 _duration, bytes32 salt)
        external
        returns (PTOraclePriceAdapter oracle)
    {
        oracle = new PTOraclePriceAdapter{salt: salt}(_ptOracle, _market, _duration);
