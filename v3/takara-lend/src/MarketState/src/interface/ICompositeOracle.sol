...
interface ICompositeOracle {
    ...
    event TTokenConfigUpdate(address indexed tToken, address indexed underlying);
    ...
    function getPrice(TToken tToken) external view returns (uint256);
    function getUnderlyingPrice(TToken tToken) external view returns (uint256);
    ...
    function getAssetAggregators(TToken tToken) external view returns (address[] memory);
}
