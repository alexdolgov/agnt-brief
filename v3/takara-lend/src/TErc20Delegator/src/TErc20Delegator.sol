...
/**
 * @title TErc20Delegator Contract
...
contract TErc20Delegator is TTokenInterface, TErc20Interface, TDelegatorInterface {
     ...
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function liquidateBorrow(address borrower, uint256 repayAmount, TTokenInterface tTokenCollateral)
        external
        override
        returns (uint256)
    {
        bytes memory data = delegateToImplementation(
            abi.encodeWithSignature("liquidateBorrow(address,uint256,address)", borrower, repayAmount, tTokenCollateral)
        );
        return abi.decode(data, (uint256));
    }
