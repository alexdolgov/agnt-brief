...
contract LiquidityManager is AddressAccessor, Pausable {
        ...
        whenNotPaused
    {
        ...
        // todo: require(msg.sender == treasury, "caller is not treasury");
     ...
     */
    function _stakeLPToken() public { // todo: private
    ...
    function collectRewards() external onlyRole(CONTROLLER_ROLE) {
        // If the pool ID is not set, find the pool ID for the current curve pool
        if (!_pidIsSet) {
            _pid = _findPid(curvePool);
            _pidIsSet = true;
        }
     ...
     */
    function _findPid(address forToken) private returns (uint256) {
        ...
        // If no matching pool is found, revert with an error message
        ...
        returns (uint256 usdrBalance, uint256 underlyingBalance)
    {
        ...
        if (total) {
            ...
            if (ownership == 0 && !total) return (0, 0);
        }
        ...
        for (uint256 i = 1; i < nCoins; i++) {
            underlyingBalance +=
                (balances[i] * (10**(18 - decimals[i])) * ownership) /
                1e18;
        }
    }
