...
contract Minter is IMinter, OwnableUpgradeable {
    ...
    function circulating_supply() public view returns (uint256 _circulating) {
        unchecked {
            ...
            _circulating = _pearl.totalSupply() - _pearl.balanceOf(address(_ve)) - _pearl.balanceOf(address(_oldVE));
            if (oldVE != address(0)) {
    ...
    function calculate_rebase(uint256 _weeklyMint) public view returns (uint256) {
        uint256 _veTotal = _pearl.balanceOf(address(_ve)) + _pearl.balanceOf(address(_oldVE));
