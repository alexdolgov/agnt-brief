...
contract RewardsDistributor is Initializable, IRewardsDistributor {
    ...
    function _claimable_debug(uint256 _tokenId, address ve, uint256 _last_token_time) internal view returns (uint256) {
        uint256 user_epoch = 0;
        uint256 to_distribute = 0;
        ...
        if (week_cursor == 0) week_cursor = ((user_point.ts - 1) / EPOCH_DURATION) * EPOCH_DURATION;
        if (week_cursor >= last_token_time) return 0;
        if (week_cursor < _start_time) week_cursor = _start_time;
    ...
    function claimable_debug(uint256 _tokenId, bool _add, bool _alt) external view returns (uint256) {
        uint256 _last_token_time = (last_token_time / EPOCH_DURATION) * EPOCH_DURATION + (_add ? EPOCH_DURATION : 0);
        return _alt ? _claimable_debug(_tokenId, voting_escrow, _last_token_time) : _claimable(_tokenId, voting_escrow, _last_token_time);
    }
