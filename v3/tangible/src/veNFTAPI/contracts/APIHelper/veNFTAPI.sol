...
contract veNFTAPI is Initializable {
    ...
    function getLockedAmountFromAddress(address _user) external view returns (uint128 _totalLockedAmount) {
        for (uint256 _i = ve.balanceOf(_user); _i != 0; ) {
            unchecked {
                --_i;
            }
            uint256 _id = ve.tokenOfOwnerByIndex(_user, _i);
            if (_id != 0) {
                veNFT memory _token =  _getNFTFromId(_id, _user);
                _totalLockedAmount += _token.amount;
            }
        }
    }
