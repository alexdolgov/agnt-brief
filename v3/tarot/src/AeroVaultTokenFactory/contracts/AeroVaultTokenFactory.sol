...
import "./AeroVaultToken.sol";
import "./interfaces/IAeroVaultTokenFactory.sol";

contract AeroVaultTokenFactory is Ownable, IAeroVaultTokenFactory {
        ...
        returns (address vaultToken)
    {
        ...
        bytes memory bytecode = type(AeroVaultToken).creationCode;
        assembly {
            vaultToken := create2(0, add(bytecode, 32), mload(bytecode), _underlying)
        }
        AeroVaultToken(vaultToken)._initialize(
            _underlying,
            optiSwap,
            router,
            voter,
