...
import "./RamsesVaultToken.sol";
import "./interfaces/IRamsesVaultTokenFactory.sol";

contract RamsesVaultTokenFactory is Ownable, IRamsesVaultTokenFactory {
        ...
        returns (address vaultToken)
    {
        ...
        bytes memory bytecode = type(RamsesVaultToken).creationCode;
        assembly {
            vaultToken := create2(0, add(bytecode, 32), mload(bytecode), _underlying)
        }
        RamsesVaultToken(vaultToken)._initialize(
            _underlying,
            optiSwap,
            router,
            voter,
