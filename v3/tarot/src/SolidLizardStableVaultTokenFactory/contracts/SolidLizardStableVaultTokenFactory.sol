...
import "./SolidLizardStableVaultToken.sol";
import "./interfaces/ISolidLizardStableVaultTokenFactory.sol";

contract SolidLizardStableVaultTokenFactory is Ownable, ISolidLizardStableVaultTokenFactory {
        ...
        returns (address vaultToken)
    {
        ...
        bytes memory bytecode = type(SolidLizardStableVaultToken).creationCode;
        assembly {
            vaultToken := create2(0, add(bytecode, 32), mload(bytecode), _underlying)
        }
        SolidLizardStableVaultToken(vaultToken)._initialize(
            _underlying,
            optiSwap,
            router,
            voter,
