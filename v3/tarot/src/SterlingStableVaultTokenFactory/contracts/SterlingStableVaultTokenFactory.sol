...
import "./SterlingStableVaultToken.sol";
import "./interfaces/ISterlingStableVaultTokenFactory.sol";

contract SterlingStableVaultTokenFactory is Ownable, ISterlingStableVaultTokenFactory {
        ...
        returns (address vaultToken)
    {
        ...
        bytes memory bytecode = type(SterlingStableVaultToken).creationCode;
        assembly {
            vaultToken := create2(0, add(bytecode, 32), mload(bytecode), _underlying)
        }
        SterlingStableVaultToken(vaultToken)._initialize(
            _underlying,
            optiSwap,
            router,
            voter,
