...
contract SupplyVaultStrategyV4 is ISupplyVaultStrategy, Ownable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    using BorrowableHelpers for IBorrowable;
    using BorrowableDetailHelpers for BorrowableDetail;

    IFactory[] m_tarotFactoryList;
    mapping(IFactory => bool) m_tarotFactoryEnabled;
    ...
    function updateSupplyVaultInfo(
        ...
        require(isAuthorized[supplyVault], "SupplyVaultStrategyV4: INVALID_VAULT");
    ...
    function _authorize(ISupplyVault supplyVault) private {
        require(!isAuthorized[supplyVault], "SupplyVaultStrategyV4: ALREADY_AUTHORIZED");
    ...
    modifier onlyAuthorized() {
        require(isAuthorized[ISupplyVault(msg.sender)], "SupplyVaultStrategyV4: NOT_AUTHORIZED");

        _;
    }

    function tarotFactoryListLength() external view returns (uint256) {
        return m_tarotFactoryList.length;
    }

    function tarotFactoryListItem(uint256 index) external view returns (IFactory) {
        return m_tarotFactoryList[index];
    }

    function tarotFactoryEnabled(IFactory factory) external view returns (bool) {
        return m_tarotFactoryEnabled[factory];
    }

    function _addTarotFactory(IFactory factory) private {
        require(!m_tarotFactoryEnabled[factory], "SupplyVaultStrategyV4: TAROT_FACTORY_ENABLED");

        m_tarotFactoryEnabled[factory] = true;
        m_tarotFactoryList.push(factory);
    }

    function addTarotFactory(IFactory factory) external onlyOwner {
        _addTarotFactory(factory);
    }

    function addTarotFactories(IFactory[] calldata factoryList) external onlyOwner {
        for (uint256 i = 0; i < factoryList.length; i++) {
            IFactory factory = factoryList[i];
            _addTarotFactory(factory);
        }
    }

    function _indexOfTarotFactory(IFactory factory) private view returns (uint256 index) {
        uint256 count = m_tarotFactoryList.length;
        for (uint256 i = 0; i < count; i++) {
            if (m_tarotFactoryList[i] == factory) {
                return i;
            }
        }
        require(false, "SupplyVaultStrategyV4: TAROT_FACTORY_NOT_FOUND");
    }

    function removeTarotFactory(IFactory factory) external onlyOwner {
        require(m_tarotFactoryEnabled[factory], "SupplyVaultStrategyV4: TAROT_FACTORY_ENABLED");

        uint256 index = _indexOfTarotFactory(factory);
        IFactory last = m_tarotFactoryList[m_tarotFactoryList.length - 1];
        m_tarotFactoryList[index] = last;
        m_tarotFactoryList.pop();
        delete m_tarotFactoryEnabled[factory];
    }
    ...
    function getBorrowable(address _address) external view override onlyAuthorized returns (IBorrowable) {
        ...
        // Treating _address as a UniswapV2Pair, try to get the lending pool from the known factory addresses
        uint256 tarotFactoryCount = m_tarotFactoryList.length;
        for (uint256 i = 0; i < tarotFactoryCount; i++) {
            IFactory tarotFactory = m_tarotFactoryList[i];
            (bool initialized, , , address borrowable0, address borrowable1) = tarotFactory.getLendingPool(_address);
            if (initialized) {
        ...
        require(false, "SupplyVaultStrategyV4: INVALID_BORROWABLE");
    }
    ...
    function _allocate(uint256 amount) private {
        ...
        uint256 numBorrowables = supplyVault.getBorrowablesLength();
        require(numBorrowables > 0, "SupplyVaultStrategyV4: NO_BORROWABLES");
    ...
    function _deallocateFromLowestSupplyRate(
        ...
        require(best.minLoss < type(uint256).max, "SupplyVaultStrategyV4: INSUFFICIENT_CASH");
    ...
    function deallocate(uint256 needAmount) public override onlyAuthorized {
        require(needAmount > 0, "SupplyVaultStrategyV4: ZERO_AMOUNT");
    ...
    function getReallocateInfo(bytes calldata _data) private pure returns (ReallocateInfo memory info) {
        if (_data.length == 0) {
            ...
            require(info.deallocFromBorrowable != info.allocIntoBorrowable, "SupplyVaultStrategyV4: SAME_IN_OUT");
        } else {
            require(false, "SupplyVaultStrategyV4: INVALID_DATA");
        }
    }

    function reallocate(uint256 _underlyingAmount, bytes calldata _data) external override onlyAuthorized {
        require(_underlyingAmount > 0, "SupplyVaultStrategyV4: ZERO_AMOUNT");
