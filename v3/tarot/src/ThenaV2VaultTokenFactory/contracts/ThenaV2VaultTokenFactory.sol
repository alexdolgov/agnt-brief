...
import "./ThenaV2VaultToken.sol";
import "./interfaces/IThenaV2VaultTokenFactory.sol";

contract ThenaV2VaultTokenFactory is Ownable, IThenaV2VaultTokenFactory {
        ...
        returns (address vaultToken)
    {
        ...
        bytes memory bytecode = type(ThenaV2VaultToken).creationCode;
        assembly {
            vaultToken := create2(0, add(bytecode, 32), mload(bytecode), _underlying)
        }
        ThenaV2VaultToken(vaultToken)._initialize(
            _underlying,
            optiSwap,
            router,
            voter,
