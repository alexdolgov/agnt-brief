...
contract Ris3Farm is Ownable {
    ...
    ris3Gov public government = ris3Gov(0xa264eB021F184B80d4EfA864C65988492DA52b13);
    ...
    event taxTransferLog(uint256 amount);
    ...
    //Ris3 stakes for multiple addresses
    function createMultipleStakes(address[] memory _addresses, uint256[] memory _amounts) public {
        require(_amounts.length > 0, "Can not stake empty values");
        require(_addresses.length == _amounts.length, "Addresses and amounts must of same length arrays");
        uint256 cycleStartTime = government.getCycleStartTime();
        uint256 withdrawingStartTime = government.getWithdrawingStartTime();
        require(now > cycleStartTime && now < withdrawingStartTime, "Staking can be done only on first day"); //staking only on first day
        
        uint256 i; uint256 amount; address user;
        for(i=0; i<_addresses.length; i++){
            ...
            //add total stakers
            if (userBalance[user].totalAmount == 0) {
                totalStakers++;
            }
            
            //add value in ris3 token balance for user
            userBalance[user].totalAmount = getUserBalance(user).add(_amount);
            userBalance[user].lastStakingTime = now;
