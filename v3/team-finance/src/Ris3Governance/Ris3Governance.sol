...
contract Ris3Governance is Ownable {
    ...
    IERC20 public ris3 = IERC20(0x7815Ff67055E4DCA8B100c500DF6e2aD9162B795);
    ...
    event governmentVoteLog(address userAddress, uint256 govType, uint256 noOfVotes);
    event electionFinishLog(uint256 cycleNo, uint256 govType, uint256 totalVoteCounts);
    event cycleStartLog(uint256 cycleNo, uint256 blockNumber);
    ...
    constructor () public {
        ...
        emit cycleStartLog(currentCycleNo, block.number);
    }
    ...
    function electGovernment(uint256 govType, uint256 taxRates, uint256 prodRates, uint256 taxPoolUses, address _dictatorAddress) public {
        ...
        //add logs
        emit governmentVoteLog(msg.sender, govType, yourVotes);
    }
    ...
    function finishElection() public {
        ...
        //add logs
        emit electionFinishLog(currentCycleNo, currentGovType, currentVoteCounts);
        
    }
    
    //set laws by dictator
    function dictatorSetLaws(uint256 taxRates, uint256 prodRates, uint256 taxPoolUses) public {
        require(now > govElectionStartTime + electionDuration && now <= govElectionStartTime + electionDuration + ruleDisplayDuration, "Laws can be set between rule display time");
        require(stopElection, "Election have not finished yet");
