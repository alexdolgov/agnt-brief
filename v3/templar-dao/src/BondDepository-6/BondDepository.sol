...
contract BondDepository is Ownable {
    ...
    function deposit( 
        ...
        if (address(this).balance >= _amount) {
            // pay with WMOVR
            IMOVR(principle).deposit{value: _amount}(); // wrap only what is needed to pay
            IMOVR(principle).transfer(treasury, _amount);
        } else {
        ...
        refundMOVR(); //refund user if needed
        return payout; 
    }
    ...
    function refundMOVR() internal {
        if (address(this).balance > 0) safeTransferMOVR(DAO, address(this).balance);
    }
    ...
    function safeTransferMOVR(address to, uint256 value) internal {
