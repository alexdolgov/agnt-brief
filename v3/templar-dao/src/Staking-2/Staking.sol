...
contract Staking is Ownable {
    ...
    modifier onlyDistributor() {
        require(distributor == msg.sender, "Distributor: caller is not the distributor");
        _;
    }

    function updateEpochBlock(uint256 _blockNumber) external onlyDistributor() {
        epoch.endBlock = _blockNumber;
    }

    function setEpochLength(uint256 _length) external onlyDistributor() {
        epoch.length = _length;
    }
}