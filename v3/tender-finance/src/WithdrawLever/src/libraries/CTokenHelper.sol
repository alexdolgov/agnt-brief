...
library CTokenHelper {
  ...
  function getLiquidity(address account) public view returns (uint) {
    (uint err, uint liquidity, uint shortfall) = comptroller.getHypotheticalAccountLiquidity(account, address(0), 0, 0, false);
    require(err == 0 && shortfall == 0, "Error: Failed to fetch liquidity or shortfall detected");
    return liquidity;
