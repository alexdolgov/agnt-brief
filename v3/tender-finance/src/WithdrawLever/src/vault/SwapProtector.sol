...
contract SwapProtector is Ownable {
  ...
  function swap(IERC20 token0, IERC20 token1, uint amount, uint24 maxSlippage) internal returns (uint) {
    uint24 fee = getPairFee(token0, token1);
    return TokenSwap.swap(token0, token1, amount, fee, maxSlippage);
  }
}
