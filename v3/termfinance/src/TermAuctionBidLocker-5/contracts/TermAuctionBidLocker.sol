    ...
    ReentrancyGuardUpgradeable
{
    ...
    modifier onlyAuthenticated(TermAuth calldata termAuth) {
    ...
    function lockBid(
        ...
        onlyAuthenticated(termAuth)
        whenLockingNotPaused
        nonReentrant
        returns (bytes32)
    {
        TermAuctionBid storage bid = _lock(bidSubmission, termAuth.user);
    ...
    {
        if (termAuth.user == referralAddress) {
        ...
        for (uint256 i = 0; i < bidSubmissions.length; ++i) {
            TermAuctionBid storage bid = _lock(
                bidSubmissions[i],
                termAuth.user
            );
            bidIds[i] = bid.id;
    ...
    /// @dev Will revert if either the price does not match the bid price or is greater than the max bid price
    ...
    /// @notice unlockBid unlocks a bid and returns funds to the bidder
    /// @dev Does not run when contract is paused; requires authentication
    /// @param id A bid id
    /// @param termAuth An authentication struct containing the signed bidder wallet address
    function unlockBid(
        bytes32 id,
        TermAuth calldata termAuth
    ) external whenUnlockingNotPaused onlyAuthenticated(termAuth) nonReentrant {
        // solhint-disable-next-line not-rely-on-time
        ...
        _unlock(
            id,
            termAuth.user,
            bids[id].collateralTokens,
            bids[id].collateralAmounts
        );
    }
