    ...
    ReentrancyGuardUpgradeable
{
    ...
    function lockOffer(
        ...
        returns (bytes32)
    {
        TermAuctionOffer storage offer = _lock(offerSubmission, termAuth.user);
    ...
    function unlockOffer(
        ...
        onlyExistingOffer(id)
        onlyOfferor(offers[id].offeror, termAuth.user)
        nonReentrant
    {
        // solhint-disable-next-line not-rely-on-time
    ...
    function unlockOffers(
        ...
        for (uint256 i = 0; i < ids.length; ++i) {
            ...
            if (termAuth.user != offers[ids[i]].offeror) {
                revert OfferNotOwned();
            }
            _unlock(ids[i], termAuth.user);
        }
    }
