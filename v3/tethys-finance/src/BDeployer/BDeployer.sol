...
contract StyxERC20 {
    ...
    function _checkSignature(
        ...
        require(deadline >= block.timestamp, "Styx: EXPIRED");
        bytes32 digest =
            keccak256(
                abi.encodePacked(
                    "\x19\x01",
...
interface IFactory {
	...
	function styxPriceOracle() external view returns (address);
...
contract PoolToken is IPoolToken, StyxERC20 {
    ...
    function _setFactory() external {
        require(factory == address(0), "Styx: FACTORY_ALREADY_SET");
        factory = msg.sender;
    }
        ...
        returns (uint256 mintTokens)
    {
        ...
        require(mintTokens > 0, "Styx: MINT_AMOUNT_ZERO");
        ...
        returns (uint256 redeemAmount)
    {
        ...
        require(redeemAmount > 0, "Styx: REDEEM_AMOUNT_ZERO");
        require(redeemAmount <= totalBalance, "Styx: INSUFFICIENT_CASH");
    ...
    modifier nonReentrant() {
        require(_notEntered, "Styx: REENTERED");
        _notEntered = false;
        _;
        _notEntered = true;
    }
...
contract BAllowance is PoolToken, BStorage {
    ...
    function _checkBorrowAllowance(
        ...
        if (spender != owner && _borrowAllowance != uint256(-1)) {
            require(_borrowAllowance >= value, "Styx: BORROW_NOT_ALLOWED");
...
contract BSetter is PoolToken, BStorage {
    ...
    function _initialize(
        ...
        require(msg.sender == factory, "Styx: UNAUTHORIZED"); // sufficient check
    ...
    function _checkSetting(
        ...
        _checkAdmin();
        require(parameter >= min, "Styx: INVALID_SETTING");
        require(parameter <= max, "Styx: INVALID_SETTING");
    }

    function _checkAdmin() internal view {
        require(msg.sender == IFactory(factory).admin(), "Styx: UNAUTHORIZED");
    }
}
