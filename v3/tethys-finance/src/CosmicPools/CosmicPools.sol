...
// The goal of this farm is to allow a stake xTOKEN earn anything model
// In a flip of a traditional farm, this contract only accepts xTOKEN as the staking token
// Each new pool added is a new reward token, each with its own start times
// end times, and rewards per second.
contract CosmicPools is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 xTokenStakedAmount; // # of xtoken allocated to this pool
    ...
    function pendingReward(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.timestamp > pool.lastRewardTime && pool.xTokenStakedAmount != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardTime, block.timestamp, pool);
            uint256 reward = multiplier * pool.RewardPerSecond;
            accRewardPerShare += (reward * pool.TokenPrecision) / pool.xTokenStakedAmount;
        }
    ...
    function updatePool(uint256 _pid) internal {
        ...
        pool.accRewardPerShare += reward * pool.TokenPrecision / pool.xTokenStakedAmount;
        pool.lastRewardTime = block.timestamp;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) external {
        ...
        user.amount += _amount;
        pool.xTokenStakedAmount += _amount;
    ...
    function withdraw(uint256 _pid, uint256 _amount) external {  
        ...
        safeTransfer(xtoken, address(msg.sender), _amount);
    ...
    function emergencyWithdraw(uint256 _pid) external {
        ...
        pool.xTokenStakedAmount -= user.amount;
        user.amount = 0;
        user.rewardDebt = 0;

        xtoken.safeTransfer(address(msg.sender), oldUserAmount);
    ...
    function recoverWrongTokens(address _tokenAddress) external onlyOwner {
        require(_tokenAddress != address(xtoken), "recoverWrongTokens: Cannot be xtoken");
