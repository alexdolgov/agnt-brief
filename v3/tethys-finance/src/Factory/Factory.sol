...
contract Factory is IFactory {
    ...
    IStyxPriceOracle public styxPriceOracle;
    ...
    constructor(
        ...
        IStyxPriceOracle _styxPriceOracle
    ) public {
        admin = _admin;
        reservesAdmin = _reservesAdmin;
        bDeployer = _bDeployer;
        cDeployer = _cDeployer;
        styxPriceOracle = _styxPriceOracle;
    ...
    function initializeLendingPool(address uniswapV2Pair) external {
        ...
        require(!lPool.initialized, "TMarket: ALREADY_INITIALIZED");

        require(
            lPool.collateral != address(0),
            "TMarket: COLLATERALIZABLE_NOT_CREATED"
        );
        require(
            lPool.borrowable0 != address(0),
            "TMarket: BORROWABLE0_NOT_CREATED"
        );
        require(
            lPool.borrowable1 != address(0),
            "TMarket: BORROWABLE1_NOT_CREATED"
        );

        (, , , , , bool oracleInitialized) =
            styxPriceOracle.getPair(uniswapV2Pair);
        if (!oracleInitialized) styxPriceOracle.initialize(uniswapV2Pair);
    ...
    function _setPendingAdmin(address newPendingAdmin) external {
        require(msg.sender == admin, "TMarket: UNAUTHORIZED");
    ...
    function _acceptAdmin() external {
        require(msg.sender == pendingAdmin, "TMarket: UNAUTHORIZED");
    ...
    function _setReservesPendingAdmin(address newReservesPendingAdmin)
        external
    {
        require(msg.sender == reservesAdmin, "TMarket: UNAUTHORIZED");
    ...
    function _acceptReservesAdmin() external {
        require(msg.sender == reservesPendingAdmin, "TMarket: UNAUTHORIZED");
