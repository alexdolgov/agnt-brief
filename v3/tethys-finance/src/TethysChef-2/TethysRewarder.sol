...
contract TethysChef is IBorrowTracker, Ownable {
	...
	constructor(address _rewardToken, uint256 _rewardPerSec) public {
		require(_rewardPerSec < MAX_REWARD_PER_SEC, "TethysChef: MAX_REWARD_PER_SEC");
...
// File: contracts\TethysRewarder.sol
...
contract TethysRewarder is IRewarder, Ownable {
	using SafeMath for uint256;
	using SafeToken for address;
	...
	address private immutable TETHYS_CHEF;
	...
	constructor(address _rewardToken, uint256 _rewardPerSec, address _TETHYS_CHEF) public {
		rewardToken = _rewardToken;
		rewardPerSec = _rewardPerSec;
		TETHYS_CHEF = _TETHYS_CHEF;
		emit LogSetReward(_rewardPerSec);
	}
	...
	function onReward(address borrowable, address borrower, address to, uint256, uint256 newShares) override external {
		require(msg.sender == TETHYS_CHEF, "Only MC can call this function.");
		PoolInfo memory pool = updatePool(borrowable);
		UserInfo storage user = userInfo[borrowable][borrower];
		uint256 pending;
		if (user.shares > 0) {
