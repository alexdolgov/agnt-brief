...
// TethysMaker is MasterChef's left hand and kinda a wizard. He can make Tethys from pretty much anything!
// This contract handles "serving up" rewards for xTethys holders by trading tokens collected from fees for Tethys.

// T1 - T4: OK
contract TethysMaker is Ownable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    IUniswapV2Factory public immutable factory;

    address public immutable xtethys;
    address private immutable tethys;
    address private immutable metis;
    ...
    modifier onlyAuth() {
        require(isAuth[msg.sender] || anyAuth, "TethysMaker: FORBIDDEN");
        _;
    }
    ...
    function setBridge(address token, address bridge) external onlyOwner {
        // Checks
        require(
            token != tethys && token != metis && token != bridge,
            "TethysMaker: Invalid bridge"
        );
    ...
    modifier onlyEOA() {
        ...
        require(msg.sender == tx.origin, "TethysMaker: must use EOA");
        _;
    }

    // F1 - F10: OK
    // F3: _convert is separate to save gas by only checking the 'onlyEOA' modifier once in case of convertMultiple
    // F6: There is an exploit to add lots of tethys to the xtethys, run convert, then remove the tethys again.
    //     As the size of the tethysxtethys has grown, this requires large amounts of funds and isn't super profitable anymore
    ...
    function _convert(address token0, address token1) internal {
        ...
        if(token0 == token1) {
            ...
            require(address(pair) != address(0), "TethysMaker: Invalid pair");
    ...
    // F1 - F10: OK
    // C1 - C24: OK
    // All safeTransfer, _swap, _totethys, _convertStep: X1 - X5: OK
    function _convertStep(
        address token0,
        address token1,
        uint256 amount0,
        uint256 amount1
    ) internal returns (uint256 tethysOut) {
        // Interactions
        if (token0 == token1) {
            uint256 amount = amount0.add(amount1);
            if (token0 == tethys) {
                IERC20(tethys).safeTransfer(xtethys, amount);
                tethysOut = amount;
            } else if (token0 == metis) {
                tethysOut = _totethys(metis, amount);
            } else {
                address bridge = bridgeFor(token0);
                amount = _swap(token0, bridge, amount, address(this));
                tethysOut = _convertStep(bridge, bridge, amount, 0);
            }
        } else if (token0 == tethys) {
            // eg. tethys - ETH
            IERC20(tethys).safeTransfer(xtethys, amount0);
            tethysOut = _totethys(token1, amount1).add(amount0);
        } else if (token1 == tethys) {
            // eg. USDT - tethys
            IERC20(tethys).safeTransfer(xtethys, amount1);
            tethysOut = _totethys(token0, amount0).add(amount1);
        } else if (token0 == metis) {
            // eg. ETH - USDC
            tethysOut = _totethys(
                metis,
                _swap(token1, metis, amount1, address(this)).add(amount0)
            );
        } else if (token1 == metis) {
            // eg. USDT - ETH
            tethysOut = _totethys(
                metis,
                _swap(token0, metis, amount0, address(this)).add(amount1)
            );
        } else {
            // eg. MIC - USDT
    ...
    function _swap(
        ...
        require(address(pair) != address(0), "TethysMaker: Cannot convert");
    ...
    // F1 - F10: OK
    // C1 - C24: OK
    function _totethys(address token, uint256 amountIn)
        internal
    ...
    {   
        ...
        amountOut = _swap(token, tethys, amount, xtethys);
    }

    function getAmountOut(uint amountIn, uint reserveIn, uint reserveOut) internal pure returns (uint amountOut) {
        require(amountIn > 0, 'TethysMaker: INSUFFICIENT_INPUT_AMOUNT');
        require(reserveIn > 0 && reserveOut > 0, 'TethysMaker: INSUFFICIENT_LIQUIDITY');
