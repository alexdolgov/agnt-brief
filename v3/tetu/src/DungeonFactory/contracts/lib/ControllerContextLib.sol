...
library ControllerContextLib {
  ...
  function getPvpController(ControllerContext memory cc) internal view returns (IPvpController pvpController) {
    if (address(cc.pvpController) == address(0)) {
      cc.pvpController = IPvpController(cc.controller.pvpController());
    }
    return cc.pvpController;
  }
