...
library DungeonLib {
  ...
  function _enter(
    ...
    {
      IPvpController pc = ControllerContextLib.getPvpController(cc);
  ...
  function _getBiomeTax(
    ...
    IPvpController pvpController = ControllerContextLib.getPvpController(cc);
    if (address(pvpController) != address(0)) {
      ...
      if (_guildId != 0) {
        ...
        guildBank = ControllerContextLib.getGuildController(cc).getGuildBank(_guildId);
    ...
    if (amount != 0) {
      if (context.heroPayToken == address(0)) {
        ...
        if (context.taxPercent != 0 && context.guildBank != address(0)) {
          ...
          if (taxAmount != 0) {
            IERC20(token).transfer(context.guildBank, taxAmount);
  ...
  function _claimItem(
    ...
    if (IERC721(token).ownerOf(tokenId) == address(this)) {
      ...
      if (context.heroPayToken == address(0)) {
        ...
        if (context.taxPercent != 0 && context.guildBank != address(0)) {
          toBiomeOwner = ControllerContextLib.getOracle(cc).getRandomNumber(100_000, 0) < context.taxPercent;
        }
        ...
        if (toBiomeOwner) {
          IERC721(token).safeTransferFrom(address(this), context.guildBank, tokenId);
