...
contract StakingManagerV4 is Initializable, OwnableUpgradeable {
     ...
     */
    function harvestAndDeposit() external whenNotPaused {
        require(block.number < endBlock, "Staking has ended");

        updatePoolRewards();
        PoolStaker storage staker = poolStakers[msg.sender];
        uint256 rewardsToDeposit = ((staker.amount *
            accumulatedRewardsPerShare) / REWARDS_PRECISION) -
            staker.rewardDebt;
        require(rewardsToDeposit > 0, "Deposit amount can't be zero");

        // Update current staker
        staker.amount += rewardsToDeposit;
        staker.harvestedRewards += rewardsToDeposit;
        staker.rewardDebt =
            (staker.amount * accumulatedRewardsPerShare) /
            REWARDS_PRECISION;
        staker.stakedTime = block.timestamp;
        ...
        // Update pool
        tokensStaked += rewardsToDeposit;

        // Harvest and deposit tokens
        emit HarvestRewards(msg.sender, rewardsToDeposit);
        emit Deposit(msg.sender, rewardsToDeposit);
    }
