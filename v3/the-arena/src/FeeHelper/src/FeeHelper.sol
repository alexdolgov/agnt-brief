...
contract FeeHelper is Ownable, ReentrancyGuard {
    ...
    event TokenParamsUpdated(
        address indexed token, uint40 maxSupplyNumerator, uint136 amountToKeep, uint32 minSellInterval
    );
    ...
    constructor(address payable _foundationWallet, address payable _arenaFeeWallet) Ownable(msg.sender) {
        ...
        emit DefaultParamsUpdated(
            defaultParams.maxSupplyNumerator, defaultParams.amountToKeep, defaultParams.minSellInterval
        );
    }
    ...
    function setFeeSourceToFeeDestination(address _feeSource, address _feeDestination, string memory _feeSourceName)
        external
        onlyOwner
    {
    ...
    function setDefaultParams(uint40 maxSupplyNumerator, uint136 amountToKeep, uint32 minSellInterval)
        external
        onlyOwner
    {
    ...
    function setTokenParams(
        ...
        uint40 maxSupplyNumerator,
        uint136 amountToKeep,
        uint32 minSellInterval
    ) external onlyOwner {
    ...
    function batchSell(address[] calldata tokenList, bytes calldata aggregatorCalldata)
        external
        onlySellerAdmin
        nonReentrant
    {
        ...
        uint256 wavaxBalanceDiff = IERC20(WAVAX_ADDRESS).balanceOf(address(this)) - wavaxBalance;
    ...
    function approveTokensIfNeededBatch(address[] calldata tokenList) external onlySellerAdmin {
        for (uint256 i = 0; i < tokenList.length; i++) {
            // TODO check if maxApproving causes issues
    ...
    function isApprovalNeededBatch(address[] calldata tokenList)
        external
        view
        returns (bool[] memory isApprovalNeeded)
    {
    ...
    function tokenTotalSupplyBatch(address[] calldata tokenList)
        external
        view
    ...
    function _applyChecksAndUpdates(address[] memory tokenList, uint256[] memory prevTokenBalances)
        internal
        returns (uint256[] memory amountsSold)
    {
        ...
        for (uint256 i = 0; i < tokenList.length; i++) {
            address tokenAddress = tokenList[i];
            require(
                tokenAddress != WAVAX_ADDRESS && tokenAddress != ARENA_TOKEN_ADDRESS,
                "WAVAX and ARENA are not supported"
            );
            ...
            if (previousTokenBalance >= postBalance) {
                // its possible you end up with more tokens due to fee routing etc.
                amountSold = previousTokenBalance - postBalance;
            }
            amountsSold[i] = amountSold;
            uint256 maxSellAmount =
                IERC20(tokenAddress).totalSupply() * effectiveParams.maxSupplyNumerator / MAX_SUPPLY_DENOMINATOR;
            ...
            if (effectiveParams.latestSwapTs != 0) {
                ...
                if (timePassedSinceLastSwap < effectiveParams.minSellInterval) {
                    revert SellIntervalNotElapsed(
                        tokenAddress, effectiveParams.latestSwapTs, effectiveParams.minSellInterval
                    );
                }
            }
            if (amountSold > 0) {
    ...
    function _getEffectiveTokenConfig(address tokenAddress, TokenParams memory _defaultParams)
        internal
        view
        returns (TokenParams memory effectiveParams)
    {
    ...
    receive() external payable {
        if (msg.sender != WAVAX_ADDRESS) {
            ...
            // leave avax on the contract for unidentified sources
        }
    }
