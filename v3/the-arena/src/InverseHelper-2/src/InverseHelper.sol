...
contract InverseHelper {
    ...
    function getPurchaseAmountAndPriceWithGivenAvax(uint256 avaxAmount, uint256 _tokenId) public view returns (uint256 purchaseAmount, uint256 priceInArena, uint256 requiredAvax) {
        ...
        requiredAvax = avaxAmount;
        if(purchaseAmount > maxTokensForSale) {
            purchaseAmount = maxTokensForSale;
        }
        priceInArena = TOKEN_MANAGER.calculateCostWithFees(purchaseAmount / 10 ** 18, _tokenId);
        if(purchaseAmount == maxTokensForSale) {
            requiredAvax = getAvaxInForArenaOut(priceInArena);
        }
        return (purchaseAmount, priceInArena, requiredAvax);
    }

    function getPurchaseAmountAndPriceWithGivenAvaxForNewToken(uint256 a, uint256 curveScaler, uint256 tokenSplit, uint256 creatorFeeBasisPoints, uint256 avaxAmount) public view returns (uint256 purchaseAmount, uint256 price) { 
        uint256 arenaAmount = getArenaAmountForGivenAvax(avaxAmount);
        return getPurchaseAmountAndPriceWithGivenArenaForNewToken(a, curveScaler, tokenSplit, creatorFeeBasisPoints, arenaAmount);

    }

    function getPurchaseAmountAndPriceWithGivenArena(uint256 arenaAmount, uint256 _tokenId) public view returns (uint256 purchaseAmount, uint256 price) {
        uint256 rawCost = invertTotalToRaw(_tokenId, arenaAmount);
        uint256 currentSupply = ITokenTemplate(TOKEN_MANAGER.getTokenParameters(_tokenId).tokenContractAddress).totalSupply() / 10 ** 18;
        purchaseAmount = calculateBuyAmount(rawCost, currentSupply, TOKEN_MANAGER.getTokenParameters(_tokenId).a, TOKEN_MANAGER.getTokenParameters(_tokenId).curveScaler);
        uint256 maxTokensForSale = TOKEN_MANAGER.getMaxTokensForSale(_tokenId);
        ...
        if(purchaseAmount > maxTokensForSale) {
            purchaseAmount = maxTokensForSale;
            price = TOKEN_MANAGER.calculateCostWithFees(purchaseAmount / 10 ** 18 + 10, _tokenId);
            if(price > arenaAmount) {
                price = arenaAmount;
            }
        }
    ...
    function getPurchaseAmountAndPriceWithGivenArenaForNewToken(uint256 a, uint256 curveScaler, uint256 tokenSplit, uint256 creatorFeeBasisPoints, uint256 arenaAmount) public view returns (uint256 purchaseAmount, uint256 price) { 
        ...
        uint256 maxTokensForSale = 10_000_000_000 ether * tokenSplit / 100;
        price = arenaAmount;
        if(purchaseAmount > maxTokensForSale) {
            purchaseAmount = maxTokensForSale;
            // small buffer to avoid rounding errors
            price = TOKEN_MANAGER.calculateCostScaledParametricWithFees(purchaseAmount + 10 ether, 0, a, 0, curveScaler, creatorFeeBasisPoints);
            price = price + uint256(TOKEN_MANAGER.tokenCreationBuyFeeAmount());
            // we made it here with the given Arena amount anyhow, so it should suffice
            if(price > arenaAmount) {
                price = arenaAmount;
            }
        }
    ...
    function getAvaxAmountForSell(uint256 sellAmount, uint256 tokenId) public view returns (uint256 avaxAmount) {   
    ...
    // How much AVAX is REQUIRED to receive `_arenaOut` ARENA
    function getAvaxInForArenaOut(uint256 _arenaOut) public view returns (uint256 avaxIn) {
        address[] memory path = new address[](2);
        path[0] = WAVAX_ADDRESS;   // input
        path[1] = ARENA_ADDRESS;   // output

        uint256[] memory amountsIn = uniswapRouter.getAmountsIn(_arenaOut, path);
        avaxIn = amountsIn[0]; // required AVAX in
    }

    // How much ARENA is REQUIRED to receive `_avaxOut` AVAX
    function getArenaInForAvaxOut(uint256 _avaxOut) public view returns (uint256 arenaIn) {
        address[] memory path = new address[](2);
        path[0] = ARENA_ADDRESS;   // input
        path[1] = WAVAX_ADDRESS;   // output

        uint256[] memory amountsIn = uniswapRouter.getAmountsIn(_avaxOut, path);
        arenaIn = amountsIn[0]; // required ARENA in
    }

    function calculateCostWithFeesInAvax(uint256 purchaseAmountInWei, uint256 _tokenId) public view returns (uint256 priceInAvax) {
        uint256 priceInArena = TOKEN_MANAGER.calculateCostWithFees(purchaseAmountInWei / 10 ** 18, _tokenId);
        priceInAvax = getAvaxInForArenaOut(priceInArena);
    }

    function calculateRewardWithFeesInAvax(uint256 sellAmountInWei, uint256 tokenId) public view returns (uint256 rewardInAvax) {
        uint256 rewardInArena = TOKEN_MANAGER.calculateRewardWithFees(sellAmountInWei / 10 ** 18, tokenId);
        rewardInAvax = getAvaxAmountForGivenArenaAmount(rewardInArena);
    }
