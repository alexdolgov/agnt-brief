...
contract MCHelper {
    ...
    IPoolManager public constant poolManager = IPoolManager(0x06380C0e0912312B5150364B9DC4542BA0DbBc85);
    uint256 private constant Q192 = 1 << 192;
    PoolKey public poolKeyGlobal = PoolKey({
    ...
    ITokenManager public immutable TokenManagerERC20;
    ...
    constructor(address _tokenManager, address _tokenManagerERC20) {
        TokenManager = ITokenManager(_tokenManager);
        TokenManagerERC20 = ITokenManager(_tokenManagerERC20);
        owner = msg.sender;
    ...
    function setPoolKey(uint24 fee, int24 tickSpacing, address hooks) public onlyOwner {
        poolKeyGlobal = PoolKey({
            currency0: Currency.wrap(address(0)),
            currency1: Currency.wrap(address(0)),
            fee: fee,
    ...
    function getTokenData(TokenData[] calldata data) external view returns (Response[] memory response) {
        ...
        for (uint256 i = 0; i < data.length; i++) {
            ...
            if (tokenData.isLp) {
                if(tokenData.lpAddress == address(0) ) { // if lp is not set, use v4 data
                    response[i] = transformV4DataToResponse(tokenData.tokenAddress, pairedTokenAddress, supply, avaxUsdPrice, arenaReserve, avaxReserve, response[i]);
                    continue;
                }                
                uint256 tokenReserve;
    ...
    function getPriceAndMarketCapInPairedToken(address tokenAddress, address pairedTokenAddress, uint256 totalSupply)
     public view returns (uint256 pricePairedTokenPerTokenX1e18, uint256 marketCapPairedToken1e18) {
        bool tokenIsToken0 = tokenAddress < pairedTokenAddress;

        PoolKey memory poolKey = PoolKey({
            currency0: tokenIsToken0 ? Currency.wrap(tokenAddress) : Currency.wrap(pairedTokenAddress),
            currency1: tokenIsToken0 ? Currency.wrap(pairedTokenAddress) : Currency.wrap(tokenAddress),
            fee: poolKeyGlobal.fee,
            tickSpacing: poolKeyGlobal.tickSpacing,
            hooks: poolKeyGlobal.hooks
        });
        PoolId poolId = poolKey.toId();
        // --- 1) Read sqrtPriceX96 from the pool ---
        (uint160 sqrtPriceX96, , , ) = poolManager.getSlot0(poolId);
        uint256 sp = uint256(sqrtPriceX96);

        // priceRaw = token1 / token0 in raw units (since both 18 decimals)
        uint256 priceRaw1e18 = FullMath.mulDiv(
            sp * sp,       // sqrtPriceX96^2
            1e18,          // scale to 1e18
            Q192
        );
        require(priceRaw1e18 != 0, "empty or invalid pool");

        // --- 2) Figure out whether tokenX is token0 or token1 ---
        // v4 sorts currencies by address, so:
        //   currency0 = min(addr0, addr1)
        //   currency1 = max(addr0, addr1)
        // Pool is tokenX / WAVAX, so we can infer ordering from addresses.

        // priceRaw1e18 is:
        //   if tokenIsToken0:  WAVAX per 1 tokenX (already what we want)
        //   else:               tokenX per 1 WAVAX (needs inversion)

        if (tokenIsToken0) {
            // token0 = tokenX, token1 = WAVAX
            // priceRaw1e18 = WAVAX/tokenX → already AVAX per tokenX
            pricePairedTokenPerTokenX1e18 = priceRaw1e18;
        } else {
            // token0 = WAVAX, token1 = tokenX
            // priceRaw1e18 = tokenX/WAVAX → invert to get AVAX/tokenX
            //
            // pricePairedTokenPerTokenX1e18 = 1 / (tokenX/WAVAX)
            //                         = WAVAX/tokenX
            // in 1e18 fixed-point: 1e36 / priceRaw1e18
            pricePairedTokenPerTokenX1e18 = FullMath.mulDiv(1e36, 1, priceRaw1e18);
        }

        marketCapPairedToken1e18 = FullMath.mulDiv(
            totalSupply,
            pricePairedTokenPerTokenX1e18,
            1e18
        );
    }

    function transformV4DataToResponse(address tokenAddress, address pairedTokenAddress, uint256 totalSupply, uint256 avaxUsdPrice, uint256 arenaReserve, uint256 avaxReserve, Response memory responsePrefilled) public view returns (Response memory response) {
       (uint256 pricePairedTokenPerTokenX1e18, uint256 marketCapPairedToken1e18) = getPriceAndMarketCapInPairedToken(tokenAddress, pairedTokenAddress, totalSupply);
        responsePrefilled.avaxReserve = 0; // those fields are irrelevant for v4 data
        responsePrefilled.tokenReserve = 0;
        responsePrefilled.liquidity = 0;
        if(pairedTokenAddress == wavax) {
            responsePrefilled.price = pricePairedTokenPerTokenX1e18;
            responsePrefilled.marketCap = marketCapPairedToken1e18;
            responsePrefilled.marketCapUsd = marketCapPairedToken1e18 * avaxUsdPrice;
        } else { // arena paired
            responsePrefilled.price = pricePairedTokenPerTokenX1e18 * avaxReserve / arenaReserve;
            responsePrefilled.marketCap = marketCapPairedToken1e18 * avaxReserve / arenaReserve;
            responsePrefilled.marketCapUsd = marketCapPairedToken1e18 * avaxUsdPrice * avaxReserve / arenaReserve;
       }
       return responsePrefilled;
    }
}
