...
contract TokenManager is OwnableUpgradeable, UUPSUpgradeable, PausableUpgradeable {
    ...
    function _deployLPManually(uint256 avaxAmount, uint256 tokenAmount, address pairAddress, address tokenAddress)
        internal
        returns (uint256 avaxAmountSentToLP, uint256 tokenAmountSentToLP, uint256 liquidity)
    {
        ...
        liquidity = IPair(pairAddress).mint(LP_TOKEN_VAULT);
    ...
    function _deployLPWithRouter(uint256 avaxAmount, uint256 tokenAmount, address tokenAddress)
        internal
        returns (uint256 avaxAmountSentToLP, uint256 tokenAmountSentToLP, uint256 liquidity)
    {
        ITokenTemplate(tokenAddress).approve(address(uniswapV2Router02), tokenAmount);
        // Add the liquidity
        (tokenAmountSentToLP, avaxAmountSentToLP, liquidity) = uniswapV2Router02.addLiquidityAVAX{value: avaxAmount}(
            tokenAddress,
            tokenAmount,
            1, // slippage is unavoidable
            1, // slippage is unavoidable
            LP_TOKEN_VAULT, // LP tokens can be sent directly to the caller -> burns the LP
    ...
    function _isManualDeployRequired(address tokenAddress) internal view returns (bool) {
        ...
        address pairAddress = uniswapV2Factory.getPair(tokenAddress, WAVAX_ADDRESS);
    ...
    function _createLp(uint256 _tokenId) internal {
        ...
        if (_isManualDeployRequired(paramsCached.tokenContractAddress)) {
            (avaxAmountSentToLP, tokenAmountSentToLP, liquidity) =
                _deployLPManually(avaxAmount, tokenAmount, paramsCached.pairAddress, paramsCached.tokenContractAddress);
        } else {
            // Approve token transfer to cover all possible scenarios
            (avaxAmountSentToLP, tokenAmountSentToLP, liquidity) = _deployLPWithRouter(
                avaxAmount, tokenAmount, paramsCached.tokenContractAddress
            );
        }
        ...
        emit TokenLPCreated(_tokenId, tokenAmountSentToLP, avaxAmountSentToLP, liquidity);
    }
    ...
    function _getPairAddress(address tokenAddress) internal view returns (address pair) {
        ...
        (address token0, address token1) = tokenAddress < wavax ? (tokenAddress, wavax) : (wavax, tokenAddress);
        IUniswapV2Factory uniswapV2Factory = IUniswapV2Factory(factory);
