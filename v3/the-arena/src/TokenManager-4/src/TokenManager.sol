...
contract TokenManager is OwnableUpgradeable, UUPSUpgradeable, PausableUpgradeable {
    ...
    */
    ...
    function _createToken(uint16 a, uint8 b, uint128 curveScaler, uint8 creatorFeeBasisPoints, address tokenCreatorAddress, uint256 tokenSplit, string memory name, string memory symbol, uint256 amount, bool wlLaunch) internal {
        ...
        if (amount > 0) {
            ...
            if(wlLaunch) {
                _buy(amount, tokenIdentifier); // lp creation is handled later on
            }
            else {
