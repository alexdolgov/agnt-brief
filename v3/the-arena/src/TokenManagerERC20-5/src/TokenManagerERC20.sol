...
contract TokenManagerERC20 is OwnableUpgradeable, UUPSUpgradeable, PausableUpgradeable {
    ...
    uint256 public constant INITIAL_TOKEN_ID = 1000000000;
   ...
   /**
    * @notice This is the case for arena is token0, thus everything is positive and the inverted is smaller than the .
    ...
    */
    ...
    function createToken(
        ...
        _createToken(a, b, curveScaler, creatorFeeBasisPoints, tokenCreatorAddress, tokenSplit, name, symbol, amount, false);
    }

    function _createToken(uint32 a, uint8 b, uint128 curveScaler, uint8 creatorFeeBasisPoints, address tokenCreatorAddress, uint256 tokenSplit, string memory name, string memory symbol, uint256 amount, bool wlLaunch) internal {
        ...
        if (amount > 0) {
            ...
            if (wlLaunch) {
                ...
                buyAndCreateLpIfPossible(amount, tokenIdentifier, type(uint256).max); // The arena amount is not relevant here, it will be set to max
            }
        }
    ...
    function createTokenWithWL(
        ...
        _createToken(a, b, curveScaler, creatorFeeBasisPoints, tokenCreatorAddress, tokenSplit, name, symbol, amount, true);
        uint256 tokenId = tokenIdentifier - 1;
        address tokenContractAddress = tokenParams[tokenId].tokenContractAddress;
        if (_isLpTokenThresholdReached(tokenId)) {
            _createLp(tokenId, false);
        }
        ITokenTemplate(tokenContractAddress).setWhitelistedAddresses(whitelist);
        ITokenTemplate(tokenContractAddress).setCreator(tokenParams[tokenId].creatorAddress);
        if (tokenParams[tokenId].lpDeployed) {
            OwnableUpgradeable(tokenContractAddress).renounceOwnership();
        }
    ...
    function _createLp(uint256 _tokenId, bool _renounceOwnership) internal {
        ...
        ITokenTemplate(ARENA_ADDRESS).approve(address(arenaPoolDeployer), arenaAmount);

        v4PoolInitParamsCached.poolInitParams.recipient = LP_TOKEN_VAULT;
        v4PoolInitParamsCached.poolInitParams.hookData = bytes("");
        // invert the starting price, lower and upper tick if arena is token1, 
        if(ARENA_ADDRESS > paramsCached.tokenContractAddress) {
            ...
            v4PoolInitParamsCached.poolInitParams.token1Amount = arenaAmount;
            v4PoolInitParamsCached.poolInitParams.token0 = paramsCached.tokenContractAddress;
            v4PoolInitParamsCached.poolInitParams.token1 = ARENA_ADDRESS;
        }
        else { // wavax is token0, default params
            v4PoolInitParamsCached.poolInitParams.token0Amount = arenaAmount;
            v4PoolInitParamsCached.poolInitParams.token1Amount = tokenAmount;
            v4PoolInitParamsCached.poolInitParams.token0 = ARENA_ADDRESS;
        ...
        emit TokenLPCreated(_tokenId, tokenAmount, arenaAmount, 0);
    }
