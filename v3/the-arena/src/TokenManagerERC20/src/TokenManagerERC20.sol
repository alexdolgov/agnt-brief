...
contract TokenManagerERC20 is OwnableUpgradeable, UUPSUpgradeable, PausableUpgradeable {
    ...
    IInverseHelper public INVERSE_HELPER;
    ...
    function setInverseHelper(address _newInverseHelper) external onlyOwner {
        INVERSE_HELPER = IInverseHelper(_newInverseHelper);
    }
     ...
     * @dev There is no slippage check and this intended.
     */
    function buyAndCreateLpIfPossible(uint256 amount, uint256 _tokenId) public {
    ...
    function buyAndCreateLpIfPossibleWithAvax(uint256 _tokenId, uint256 _amountOutMin) public payable returns (uint256 amountOut) { 
        address[] memory path = new address[](2);
        address WAVAX_ADDRESS = 0xB31f66AA3C1e785363F0875A1B74E27b85FD66c7;
        path[0] = WAVAX_ADDRESS;
        path[1] = ARENA_ADDRESS;
        uint256[] memory amounts = uniswapV2Router02.swapExactAVAXForTokens{value: msg.value}(0, path, msg.sender, block.timestamp);
        uint256 amountInToken = amounts[1];
        uint256 rawCost = INVERSE_HELPER.invertTotalToRaw(protocolFeeBasisPoint, referralFeeBasisPoint, tokenParams[_tokenId].creatorFeeBasisPoints, tokenCreationBuyFeeAmount, amountInToken);
        uint256 currentSupply = ITokenTemplate(tokenParams[_tokenId].tokenContractAddress).totalSupply() / GRANULARITY_SCALER;
        amountOut = INVERSE_HELPER.calculateBuyAmount(rawCost,currentSupply,tokenParams[_tokenId].a, tokenParams[_tokenId].curveScaler);
        if(amountOut < _amountOutMin) {
            revert("Insufficient amount out");
        }
        buyAndCreateLpIfPossible(amountOut, _tokenId);
    }
    ...
    function sellToAvax(uint256 _tokenId, uint256 _amount, uint256 _amountOutAvaxMin) public returns (uint256 avaxAmountOut) {
        address WAVAX_ADDRESS = 0xB31f66AA3C1e785363F0875A1B74E27b85FD66c7;
        uint256 userArenaBalanceBefore = ITokenTemplate(ARENA_ADDRESS).balanceOf(msg.sender);
        sell(_amount, _tokenId);
        uint256 userArenaBalanceAfter = ITokenTemplate(ARENA_ADDRESS).balanceOf(msg.sender);
        uint256 diff = userArenaBalanceAfter - userArenaBalanceBefore;
        ITokenTemplate(ARENA_ADDRESS).approve(address(uniswapV2Router02), diff);
        ITokenTemplate(ARENA_ADDRESS).transferFrom(msg.sender, address(this), diff);
        address[] memory path = new address[](2);
        path[0] = ARENA_ADDRESS;
        path[1] = WAVAX_ADDRESS;
        uint256[] memory amounts = uniswapV2Router02.swapExactTokensForAVAX(diff, _amountOutAvaxMin, path, msg.sender, block.timestamp);
