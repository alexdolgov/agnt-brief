...
import { ICLSwapCallback } from
    "@velodrome-finance/concentrated-liquidity/contracts/core/interfaces/callback/ICLSwapCallback.sol";

import { INonfungiblePositionManager } from
    "@velodrome-finance/concentrated-liquidity/contracts/periphery/interfaces/INonfungiblePositionManager.sol";
...
contract ICHIVault is IICHIVault, ICLSwapCallback, ERC20, ManualReentrancyGuard, Ownable, IERC721Receiver {
    ...
    {
        require(_pool != address(0), "I1");
        // Enforces that only single-sided deposit vaults can be created via the factory
        require((_allowToken0 && !_allowToken1) || (_allowToken1 && !_allowToken0), "I2");
     ...
     */
    function _mintPosition(int24 tickLower, int24 tickUpper, uint256 amount0Desired, uint256 amount1Desired)
        internal
    ...
    /// @param amount0Desired desired amount of token0
    /// @param amount1Desired desired amount of token1
    function _mintBasePosition(int24 _baseLower, int24 _baseUpper, uint256 amount0Desired, uint256 amount1Desired)
        internal
    {
    ...
    /// @param amount0Desired desired amount of token0
    /// @param amount1Desired desired amount of token1
    function _mintLimitPosition(int24 _limitLower, int24 _limitUpper, uint256 amount0Desired, uint256 amount1Desired)
        internal
    {
     ...
     */
    function _getConservativePrice(uint256 spot, uint256 twap, uint256 auxTwap, bool isPool)
        internal
        view
        returns (uint256)
    {
    ...
    function _checkPriceManipulation(uint256 price, uint256 twap, uint256 auxTwap) internal view {
        UV3Math.checkPriceManipulation(price, twap, auxTwap, hysteresis, auxTwapPeriod, pool);
    }
    ...
    function deposit(uint256 deposit0, uint256 deposit1, address to) external override returns (uint256 shares) {
        ...
        require(allowToken0 || deposit0 == 0, "I7");
        require(allowToken1 || deposit1 == 0, "I8");
        require(deposit0 > 0 || deposit1 > 0, "I9");
        require(deposit0 <= deposit0Max && deposit1 <= deposit1Max, "I10");
        require(to != address(0) && to != address(this), "I11");
        ...
        uint256 auxTwap = auxTwapPeriod > 0 ? _fetchTwap(pool, token0, token1, auxTwapPeriod, PRECISION) : twap;
     ...
     */
    function _withdrawFromPosition(uint256 positionId, uint256 shares, uint256 totalSupply, address to)
        internal
        returns (uint256 amount0, uint256 amount1)
    {
        ...
        // Get position info
        (,,,,,,, uint128 positionLiquidity,,, uint128 tokensOwed0, uint128 tokensOwed1) =
            _nftManager().positions(positionId);

        // should not be happening, safety check
        require(tokensOwed0 == 0 && tokensOwed1 == 0, "I12");
        ...
        if (liquidityToDecrease > 0) {
            ...
            // Only collect if there are amounts to collect (prevents revert on dust positions)
    ...
    function withdraw(uint256 shares, address to) external override returns (uint256 amount0, uint256 amount1) {
        ...
        address _lastActiveGauge = lastActiveGauge;
        _preAction(_lastActiveGauge);
        require(shares > 0, "I13");
        require(to != address(0), "I14");

        uint256 _totalSupply = totalSupply();
        require(shares == _totalSupply || _totalSupply >= shares.add(MIN_SHARES), "I15");
        ...
        override
    {
        ...
        require(_baseLower < _baseUpper && _baseLower % tickSpacing_ == 0 && _baseUpper % tickSpacing_ == 0, "I16");
        require(_limitLower < _limitUpper && _limitLower % tickSpacing_ == 0 && _limitUpper % tickSpacing_ == 0, "I17");
        require(_baseLower != _limitLower || _baseUpper != _limitUpper, "I18");
        ...
        _mintLimitPosition(
            _limitLower, _limitUpper, IERC20(token0).balanceOf(address(this)), IERC20(token1).balanceOf(address(this))
        );
        _postAction(_lastActiveGauge);
        _afterNonReentrant();
    }
    ...
    function _distributeFees(uint256 fees0, uint256 fees1) internal {
        ...
        if (baseFee > 0) {
            ...
            uint256 baseFeeSplit =
                (affiliate == address(0)) ? PRECISION : IICHIVaultFactory(ichiVaultFactory).baseFeeSplit();
    ...
    function setBaseFeeOverride(uint256 _baseFee) external override {
        ...
        if (_baseFee != type(uint256).max) {
            require(_baseFee <= PRECISION, "I20");
            uint256 currentAmmFee = IICHIVaultFactory(ichiVaultFactory).ammFee();
            require(_baseFee.add(currentAmmFee) <= PRECISION, "I21");
        }
     ...
     */
    function _getPositionAmounts(uint256 positionId)
        internal
        view
        returns (uint128 liquidity, uint256 amount0, uint256 amount1)
    {
        return UV3Math.getPositionAmounts(positionId, pool, ichiVaultFactory);
    }
    ...
    {
        return UV3Math.fetchSpot(_tokenIn, _tokenOut, _tick, _amountIn);
    }
    ...
    function _fetchTwap(address _pool, address _tokenIn, address _tokenOut, uint32 _twapPeriod, uint256 _amountIn)
        ...
        returns (uint256 amountOut)
    {
        return UV3Math.fetchTwap(_pool, _tokenIn, _tokenOut, _twapPeriod, _amountIn);
    }
    ...
    function _collectRewards(address _lastGauge) internal {
        UV3Math.collectRewards(
            ichiVaultFactory, pool, basePositionId, limitPositionId, farmingContract, token0, token1, _lastGauge
        );
    }
    ...
    function _preAction(address _lastGauge) internal {
        ...
        UV3Math.preAction(ichiVaultFactory, pool, basePositionId, limitPositionId, _lastGauge);
    }
    ...
    function _postAction(address _lastGauge) internal {
        lastActiveGauge = UV3Math.postAction(ichiVaultFactory, pool, basePositionId, limitPositionId, _lastGauge);
    }
