    ...
    IDeltaManager
{
    ...
    */
    ...
    function _checkAndUpdateRedeemLimit(uint256 amount) private {
        ...
        if (amount > redeemLimit - (_currentRedeemAmount + pendingRedemption)) {
    ...
    function _processRedemption(uint256 processLimit) private {
        ...
        if (_redemptionQueue.length > 0) {
            if (_redemptionQueue[0].processing == true) {
                emit PendingRedemptions(_redemptionQueue.length);
            }
            else{
                for (uint256 i = 0; i < _redemptionQueue.length; i++) {
                    if (_redemptionQueue[i].processing == true) {
                        emit PendingRedemptions(_redemptionQueue.length - i);
                        break;
                    }
                }
            }
    ...
    function _transitionEpoch() internal {
            ...
            _currentEpochStartTimestamp) / _epochDuration;
        if (epochDifference > 0) {
           ...
           block.timestamp - (block.timestamp % _epochDuration) + 21600;
