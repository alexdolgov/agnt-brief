...
contract NftPositionLocker is Ownable, IERC721Receiver, INonfungiblePositionManagerStruct {
    ...
    constructor(uint40 lockId_, address positionManagerAddress_, uint40 unlockTime_, address owner_) {
        ...
        nonfungiblePositionManager = INonfungiblePositionManager(positionManagerAddress_);
    ...
    function collectPositionFee() external onlyOwner returns (uint256 amount0, uint256 amount1) {
        if (isUnlocked()) revert Unlocked();
        if (!isDeposited) revert NotDeposited();
        if (isWithdrawn) revert AlreadyWithdrawn();
