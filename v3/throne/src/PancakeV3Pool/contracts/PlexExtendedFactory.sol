...
import './interfaces/IPlexExtendedFactory.sol';
import "./interfaces/IPlexExtendedPoolDeployer.sol";
import './interfaces/IPlexExtendedPool.sol';

/// @title Canonical PlexSwap Extended factory
/// @notice Deploys PlexSwap Extended Pools and manages ownership and control over pool protocol fees
contract PlexExtendedFactory is IPlexExtendedFactory {
    /// @inheritdoc IPlexExtendedFactory
    ...
    function createPool(
        ...
        pool = IPlexExtendedPoolDeployer(poolDeployer).deploy(address(this), token0, token1, fee, tickSpacing);
    ...
    function setFeeProtocol(address pool, uint32 feeProtocol0, uint32 feeProtocol1) external override onlyOwner {
        IPlexExtendedPool(pool).setFeeProtocol(feeProtocol0, feeProtocol1);
    }
    ...
    function collectProtocol(
        ...
        return IPlexExtendedPool(pool).collectProtocol(recipient, amount0Requested, amount1Requested);
    }

    function setLmPool(address pool, address lmPool) external override onlyOwnerOrLmPoolDeployer {
        IPlexExtendedPool(pool).setLmPool(lmPool);
    }
}
