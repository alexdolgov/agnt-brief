...
import '@throneexchange/v3-periphery/contracts/base/BlockTimestamp.sol';
import '@throneexchange/v3-periphery/contracts/libraries/Path.sol';
import '@throneexchange/v3-periphery/contracts/libraries/PoolAddress.sol';
import '@throneexchange/v3-core/contracts/interfaces/IThroneV3Pool.sol';
import '@throneexchange/v3-periphery/contracts/libraries/OracleLibrary.sol';
...
abstract contract OracleSlippage is IOracleSlippage, PeripheryImmutableState, BlockTimestamp {
    ...
    function getPoolAddress(
        ...
        address tokenB,
        uint24 fee
    ) internal view virtual returns (IThroneV3Pool pool) {
        pool = IThroneV3Pool(PoolAddress.computeAddress(deployer, PoolAddress.getPoolKey(tokenA, tokenB, fee)));
    }
    ...
    function getSyntheticTicks(
        ...
        for (uint256 i = 0; i < numPools; i++) {
            ...
            IThroneV3Pool pool = getPoolAddress(tokenIn, tokenOut, fee);
            ...
            if (secondsAgo == 0) {
                ...
                (, currentTick, , , , , ) = IThroneV3Pool(pool).slot0();
            }
