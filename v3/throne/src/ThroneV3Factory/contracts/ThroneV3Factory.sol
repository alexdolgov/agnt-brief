...
/// @title Canonical ThroneExchange V3 factory
/// @notice Deploys ThroneExchange V3 pools and manages ownership and control over pool protocol fees
contract ThroneV3Factory is IThroneV3Factory {
    uint8 public immutable _VERSION = 1;

    /// @inheritdoc IThroneV3Factory
    ...
    function createPool(address tokenA, address tokenB, uint24 fee) external override returns (address pool) {
        ...
        pool = IThroneV3PoolDeployer(poolDeployer).deploy(address(this), token0, token1, fee, tickSpacing);
    ...
    function setFeeProtocol(address pool, uint32 feeProtocol0, uint32 feeProtocol1) external override onlyOwner {
        IThroneV3Pool(pool).setFeeProtocol(feeProtocol0, feeProtocol1);
    }
    ...
    function collectProtocol(
        ...
        return IThroneV3Pool(pool).collectProtocolFee(recipient, amount0Requested, amount1Requested);
    }

    function setLmPool(address pool, address lmPool) external override onlyOwnerOrLmPoolDeployer {
        IThroneV3Pool(pool).setLmPool(lmPool);
    }
}
