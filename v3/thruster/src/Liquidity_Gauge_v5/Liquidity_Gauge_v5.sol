...
relative_weight_cap: public(uint256)


@external
def __init__(_token_admin: address, _gauge_controller: address, _veboost: address):
    """
    @notice Contract constructor
    @param _token_admin TokenAdmin that mints tokens 
    @param _gauge_controller The gauge controller
    @param _veboost The 
    """
    MINTER = TokenAdmin(_token_admin).minter() 
    TOKEN_ADMIN = _token_admin 
    VOTING_ESCROW = Controller(_gauge_controller).voting_escrow()
    GAUGE_CONTROLLER = _gauge_controller
    VEBOOST_PROXY = _veboost
...
def _get_capped_relative_weight(period: uint256) -> uint256:
    ...
    """
    return min(Controller(GAUGE_CONTROLLER).gauge_relative_weight(self, period), self.relative_weight_cap)
...
def _checkpoint(addr: address):
    ...
    new_rate: uint256 = TokenAdmin(TOKEN_ADMIN).rate()
    pending_rate: PendingRate = self.pendingRate
    ...
    if block.timestamp > _period_time:
        ...
        if rate != new_rate and pending_rate.rate != new_rate:
            self.pendingRate = PendingRate({ rate: new_rate, epoch: (block.timestamp + WEEK) / WEEK })
        ...
        for i in range(500):
            ...
            if _working_supply > 0:
                if rate != pending_rate.rate and week_time == pending_rate.epoch * WEEK:
                    # if there is a pending rate update for this week then
                    # write the new rate
                    rate = pending_rate.rate
                    self.inflation_rate = pending_rate.rate 
...
def _set_relative_weight_cap(relative_weight_cap: uint256):
    ...
    self.relative_weight_cap = relative_weight_cap
...
def initialize(_lp_token: address, relative_weight_cap: uint256):
    ...
    self.inflation_rate = TokenAdmin(TOKEN_ADMIN).rate()

    lp_symbol: String[26] = ERC20Extended(_lp_token).symbol()
    name: String[64] = concat("Thruster ", lp_symbol, " Gauge Deposit")

    self.NAME = name
    self.SYMBOL = concat(lp_symbol, "-gauge")
    ...
    self.admin = Controller(GAUGE_CONTROLLER).admin()