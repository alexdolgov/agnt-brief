...
contract FundingWorks is ERC721, Ownable, ReentrancyGuard {
    ...
    /// @notice Whether or not each token has unique metadata
    bool private uniqueTokenMetadata;
    /// @notice Tokens can be traded after the vesting period.
    bool private unlockAfterVesting;  
    ...
    error NotAllowed();
    /// @notice Thrown when the caller has already minted a token
    error AlreadyMinted();
    ...
    constructor(
        ...
        bool _uniqueTokenMetadata,
        bool _unlockAfterVesting,
        ...
        ipfsHash = _ipfsHash;
        uniqueTokenMetadata = _uniqueTokenMetadata;
        unlockAfterVesting = _unlockAfterVesting;
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        if (!_exists(tokenId)) revert TokenDoesNotExist();
        if (uniqueTokenMetadata) {
            return string.concat("ipfs://", ipfsHash, "/", LibString.toString(tokenId));
        }
    ...
    /// @notice Checks if token transfers are allowed
    /// @return True if transfers are allowed, false otherwise
    /// @dev Returns true if unlockAfterVesting is true and vesting period has completed
    function _canTransfer() internal view returns (bool) {
        if (!unlockAfterVesting) return false;
    ...
    function _beforeTokenTransfer(
        ...
        // Allow if _canTransfer is true
        // Reject all other transfers
        if (from != address(0) && to != address(0) && !_canTransfer()) {
            revert NotAllowed();
        }
    ...
    /*                   BLOCKED FUNCTIONS                 */
    /* ™™™™™™™™™™™™™™™™™™™™™™™™™™™™™™™™™™™™™™™™™™™™™™™™™™™ */
    /// @notice Function is blocked unless unlockAfterVesting is true and vesting period completes
    function setApprovalForAll(address, bool) public virtual override {
        if (!_canTransfer()) revert NotAllowed();
    }
