...
/// @title HeftUpgradable - UUPS-upgradable crowdfunding NFT with gifts, hard-date vesting, caller-chosen id-reuse remint, paid-supply cap, post-vesting transfers, and mint-on-behalf (paid + traditional).
...
contract HeftUpgradable is ERC721, Ownable, ReentrancyGuard, Initializable, UUPSUpgradeable {
    ...
    /// @notice Override price for mintTraditional / mintTraditionalOnBehalf only. Zero means "follow mintPrice".
    /// @dev Added in an upgrade — occupies one former __gap slot. Deliberately separate from `mintPrice`
    ///      so it does NOT touch paid mint/remint pricing or any vesting accounting (which is snapshotted
    ///      from `mintPrice` at enableMint and must stay frozen for already-minted NFTs).
    uint256 public traditionalMintPrice;

    /// @notice Additional address (alongside owner and manager) permitted to call `gift`.
    /// @dev Added in an upgrade — occupies one former __gap slot. Zero disables the role.
    ///      Gifting-only: confers no payout, upgrade, or other admin rights.
    ...
    ///      the number of slots consumed (e.g. one `uint256` field => `__gap[47]`). Variable-length
    ...
    event TraditionalMintPriceSet(uint256 previous, uint256 current);
    ...
    /// @dev Allows owner, manager, or the dedicated `minter` address. Used to gate `gift`.
    ///      Confers gifting rights only — no payout, upgrade, or other admin privileges.
    modifier onlyValidGifter() {
        if (msg.sender != owner() && msg.sender != manager && msg.sender != minter) {
            revert NotValidGifter();
        }
        _;
    }
    ...
    /// @notice Set (or clear) the dedicated minter permitted to call `gift`, alongside owner and manager.
    /// @param newMinter The new minter address. Pass address(0) to disable the role.
    /// @dev Gifting-only role; confers no payout, upgrade, or other admin rights.
    function updateMinterAddress(address newMinter) external onlyOwnerOrManager {
    ...
    /// @notice Set the price charged by mintTraditional / mintTraditionalOnBehalf, per token.
    /// @dev Intentionally does NOT touch `mintPrice` and therefore leaves paid mint(), mintOnBehalf(),
    ///      remint(), and all vesting accounting (initialPayoutPerToken / initialLockedPerToken) untouched —
    ///      already-minted vested NFTs are unaffected. Pass 0 to revert to following `mintPrice`.
    function updateMintPrice(uint256 newPrice) external onlyOwnerOrManager {
        uint256 previous = traditionalMintPrice;
        traditionalMintPrice = newPrice;
        emit TraditionalMintPriceSet(previous, newPrice);
    }

    /// @notice The price a traditional mint actually charges: the override if set, otherwise `mintPrice`.
    function effectiveTraditionalMintPrice() public view returns (uint256) {
        return traditionalMintPrice == 0 ? mintPrice : traditionalMintPrice;
    }

    /// @notice Mint gift tokens to specified recipients (one token per address). Owner, manager, or minter only.
    /// @dev Gifts have ids >= GIFT_ID_OFFSET, are non-burnable, soulbound, and excluded from vesting math.
    function gift(address[] calldata recipients) external onlyValidGifter {
    ...
    {
        return _mintTraditional(msg.sender, quantity);
    }

    /// @notice Mint `quantity` gift-style tokens to `recipient`, paid by msg.sender.
    /// @dev Mirrors `mintTraditional` but credits the tokens to `recipient`. No vesting, no time
    ///      gates, ETH forwarded to payoutAddress. Subject to MAX_TOTAL_SUPPLY across all mint paths.
    function mintTraditionalOnBehalf(address recipient, uint256 quantity)
        external
        payable
        nonReentrant
        returns (uint256[] memory tokenIds)
    {
        ...
        return _mintTraditional(recipient, quantity);
    }
    ...
    /// @dev Shared traditional-mint path used by `mintTraditional` and `mintTraditionalOnBehalf`.
    ///      Tokens land in the gift id range (>= GIFT_ID_OFFSET), inheriting gift semantics:
    ///      non-burnable, soulbound until vestingEndDate (or allowTransfers), excluded from vesting math.
    function _mintTraditional(address to, uint256 quantity) internal returns (uint256[] memory tokenIds) {
        if (quantity == 0) revert CannotBeZero();
        if (msg.value != effectiveTraditionalMintPrice() * quantity) revert WrongAmount();
        _requireTotalCap(quantity);
