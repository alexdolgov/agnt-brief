...
contract MaxPainFeeHandler is Ownable, ReentrancyGuard {
    ...
    /// @notice Event emitted when a Max Pain is purchased by the protocol and burned
    event MaxPainProcessed(uint256 cost, uint256 tokenId);
    ...
    /// @notice Buys a Max Pain from the strategy and calls burn(), paying a reward to the caller
    /// @param maxPainId The ID of the Max Pain to buy and burn
    ...
    function buyMaxPain(uint256 maxPainId) external nonReentrant returns (uint256) {
        ...
        // Burn
        IERC721(MAX_PAIN_ADDRESS).burn(maxPainId);

        // Send reward to caller
        SafeTransferLib.forceSafeTransferETH(msg.sender, reward);

        emit MaxPainProcessed(price, maxPainId);
