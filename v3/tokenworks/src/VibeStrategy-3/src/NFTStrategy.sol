...
contract NFTStrategy is Initializable, UUPSUpgradeable, Ownable, ReentrancyGuard, ERC20 {
    ...
    /// @notice Address of the manager who can manage settings
    ...
    struct SeaportListing {
        uint256 salt;
        uint256 proceeds; // Our proceeds after fees (what we receive)
        uint48 startTime;
        uint48 endTime;
        uint8 numFeeRecipients; // Number of fee recipients (0-2)
        ListingFeeRecipient[2] feeRecipients; // Fee recipients with actual amounts
    }
    ...
    /// @notice TokenWorks fee address
    address public constant TOKEN_WORKS = 0x23Ddfb0cc40682ad90BD4269a602141B7e481C5a;
    ...
    /// @notice Restricts function access to owner or manager
    ...
    /// @dev Only callable by owner or manager. Must be between 1.1x (1100) and 10.0x (10000)
    function setPriceMultiplier(uint256 _newMultiplier) external onlyOwnerOrManager {
        if (_newMultiplier < 1100 || _newMultiplier > 10000) revert InvalidMultiplier();
        priceMultiplier = _newMultiplier;
    }

    /// @notice Allows owner or manager to whitelist addresses that can distribute tokens freely
    /// @param distributor Address to whitelist
    /// @param status True to whitelist, false to remove from whitelist
    /// @dev Only callable by owner or manager. Enables fee-free token distribution for whitelisted addresses
    function setDistributor(address distributor, bool status) external onlyOwnerOrManager {
        isDistributor[distributor] = status;
        emit DistributorUpdated(distributor, status);
    }

    /// @notice Updates the manager address
    /// @param _manager New manager address (can be address(0) to remove manager)
    /// @dev Only callable by owner or current manager
    ...
    function syncOrderStatus(uint256[] calldata tokenIds) external nonReentrant {
        ...
        for (uint256 i; i < tokenIds.length; i++) {
            ...
            // technically we should always have set aside 0.5% of proceeds if we are able to sync an order
    ...
    /// @dev Total fees cannot exceed 600 bps (6%). Only callable by owner or manager.
    function setSeaportFeeRecipients(SeaportFeeRecipient[] calldata recipients) external onlyOwnerOrManager {
        ...
        if (totalFeeBps > 600) revert InvalidParams();
    ...
    function _createSeaportListing(uint256 tokenId, uint256 price) internal {
        ...
        ConsiderationItem[] memory consideration = new ConsiderationItem[](2 + numFees);
        SeaportListing storage listing = seaportListings[tokenId];
        
        uint256 twFeeAmount = price * 100 / 10000;
        consideration[1] = ConsiderationItem({
            itemType: ItemType.NATIVE,
            token: address(0),
            identifierOrCriteria: 0,
            startAmount: twFeeAmount,
            endAmount: twFeeAmount,
            recipient: payable(TOKEN_WORKS)
        });

        uint256 totalFeeAmount = twFeeAmount;
        for (uint256 i; i < numFees; i++) {
            uint256 feeAmount = (price * seaportFeeRecipients[i].feeBps) / 10000;
            totalFeeAmount += feeAmount;

            // Build consideration item
            consideration[2 + i] = ConsiderationItem({
                itemType: ItemType.NATIVE,
                token: address(0),
                identifierOrCriteria: 0,
        ...
        OrderParameters memory params = OrderParameters({
            ...
            salt: salt,
            conduitKey: CONDUIT_KEY,
            totalOriginalConsiderationItems: 2 + numFees
        });
    ...
    function _cancelSeaportListingIfExists(uint256 tokenId) internal {
        ...
        uint256 twFeeAmount = nftForSale[tokenId] * 100 / 10000;
        consideration[1] = ConsiderationItem({
            itemType: ItemType.NATIVE,
            token: address(0),
            identifierOrCriteria: 0,
