...
contract DynamicSwapFeeModule is IDynamicFeeModule {
    ...
    function configureDynamicFees(
    ...
    ) external override onlySwapFeeManager {
        uint256 poolsLength = _pools.length;
        require(poolsLength == _feeCaps.length && poolsLength == _scalingFactors.length, 'LMM');
        ...
        for (uint256 i = 0; i < poolsLength; i++) {
            ...
            require(scalingFactor <= MAX_SCALING_FACTOR, 'ISF');
            ...
            emit DynamicFeeConfigured({pool: pool, feeCap: feeCap, scalingFactor: scalingFactor});
        }
    }
    ...
    function _getDynamicFee(address _pool, uint256 _scalingFactor) internal view returns (uint256) {
        (, int24 currentTick, , , , ) = ICLPool(_pool).slot0();
