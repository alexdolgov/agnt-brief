...
contract PositionBurnHelper {
    ...
    function myBurnable() public view returns (uint256 count) {
        count = _burnablePositions(msg.sender);
    }

    function burnablePositions(address _address) public view returns (uint256 count) {
        count = _burnablePositions(_address);
    }


    function _burnablePositions(address _addr) internal view returns(uint256) {
