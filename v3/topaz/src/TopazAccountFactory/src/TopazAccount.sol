...
contract TopazAccount is IERC721Receiver {
    /// @notice Registry of connectors this account may delegatecall. Set once on
    ///         the implementation; shared by every clone (read from impl code).
    ...
    event DelegateExecuted(address indexed connector);
    ...
    ///      `registry` is an immutable baked into the implementation code, so
    ///      every EIP-1167 clone reads the same value.
    ...
    /// @notice Execute an allowlisted connector's logic in this account's context
    ///         via delegatecall (DSA pattern). The connector runs as the account,
    ///         so positions it mints and stakes are owned by the account.
    function executeDelegate(address connector, bytes calldata data)
        external
        payable
        onlyOwner
        nonReentrant
        returns (bytes memory result)
    {
        if (!IConnectorRegistry(registry).isConnector(connector)) {
            revert ConnectorNotAllowed();
        }
        unchecked {
            ++nonce;
        }
        bool success;
        (success, result) = connector.delegatecall(data);
        if (!success) {
            assembly {
                revert(add(result, 0x20), mload(result))
            }
        }
        emit DelegateExecuted(connector);
    }
