...
contract Multicall {
    ...
    address public constant ORACLE = 0x71041dddad3595F9CEd3DcCFBe3D1F4b0a16Bb70;
    ...
    function bribeCardData(address plugin, address account) public view returns (BribeCard memory bribeCard) {
        ...
        for (uint i = 0; i < bribeCard.rewardTokens.length; i++) {
            (uint256 periodFinish, , , ) = IBribe(bribeCard.bribe).rewardData(bribeCard.rewardTokens[i]);
            _rewardsPerToken[i] = (periodFinish < block.timestamp ? 0 : IBribe(bribeCard.bribe).getRewardForDuration(bribeCard.rewardTokens[i]));
        }
