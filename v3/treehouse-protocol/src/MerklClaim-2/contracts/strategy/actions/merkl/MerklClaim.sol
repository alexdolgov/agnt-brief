...
contract MerklClaim is ActionBase {
  ...
  function _claim(
    ...
    uint256[] memory preBalances = new uint256[](tokens.length);

    for (uint i; i < tokens.length; ++i) {
      if (users[i] != address(this)) revert OnlyRecipient();

      if (toTreasury[i]) {
        preBalances[i] = IERC20(tokens[i]).balanceOf(address(this));
      }
    }
    ...
    for (uint i; i < tokens.length; ++i) {
      if (toTreasury[i]) {
        uint256 toTransferAmount = IERC20(tokens[i]).balanceOf(address(this)) - preBalances[i];
        if (toTransferAmount > 0) {
          IERC20(tokens[i]).safeTransfer(TREASURY, toTransferAmount);
        }
      }
    }
