...
interface ISingleLiquidityProvider {
     ...
     * @param timestamp The timestamp of request creation
     */
    struct Request {
        uint256 amountToWithdraw;
        uint256 timestamp;
    ...
    /**
     * @dev Event emitted when unstake request was made
     * @param user The address of user
    ...
    event UpdatedRewardsPerShare(
        uint256 indexed timestamp, uint256 totalStakedTokens, uint256 rewardPerShare
    );
    ...
    /// @dev Error emitted when TREN token is not set
    error TrenTokenNotSetYet();
