    ...
    IERC721Receiver 
{
    ...
    function _isV2Market(address market) internal view returns (bool) {
        try ITruthMarketV2(market).VERSION() returns (string memory version) {
            ...
            return versionBytes.length >= 3 && 
                   versionBytes[0] == bytes1("2") && 
                   versionBytes[1] == bytes1(".") &&
                   versionBytes[2] >= bytes1("0") && 
                   versionBytes[2] <= bytes1("9");
        } catch {
    ...
    function ensurePermit2Approvals(address[] calldata tokens, uint256[] calldata amounts) external onlyRole(Roles.OPERATOR_ROLE) nonReentrant {
    ...
    function mintAndProvideLiquidity(address truthMarket, uint256 paymentTokenAmount) external
        onlyRole(Roles.OPERATOR_ROLE) onlyV2Markets(truthMarket) nonReentrant {
    ...
    function removeMarketLiquidity(address market) external onlyRole(Roles.OPERATOR_ROLE) onlyV2Markets(market) nonReentrant {
    ...
    function _ensureExactPermit2Approval(address token, uint256 exactAmount) internal {
        ...
        (uint160 currentAllowance, uint48 expiration,) = permit2.allowance(address(this), token, address(positionManager));
    ...
    function _addLiquidity(
        ...
        int24 tickLower = _nearestUsableTick(TickMath.MIN_TICK, poolKey.tickSpacing);
        int24 tickUpper = _nearestUsableTick(TickMath.MAX_TICK, poolKey.tickSpacing);
    ...
    function _calculateTickRange(
        ...
        if (min == 0) {
            tick0 = _nearestUsableTick(
                tokenInverse ? TickMath.MAX_TICK : TickMath.MIN_TICK,
                tickSpacing
            );
        } else {
    ...
    function _tickHelperPriceToTick(
        address baseToken,
        address quoteToken,
        uint256 price,
        int24 tickSpacing
