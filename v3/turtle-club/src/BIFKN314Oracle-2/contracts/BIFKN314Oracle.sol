...
contract BIFKN314Oracle {
     ...
     * @param invert If true, returns the inverted price (the price of the other token in terms of the target token).
     * @return price The spot price, scaled to 18 decimals (in WEI).
     *         - If `invert` is false:
     *           - If `targetToken` is the native token, `price` is in terms of the paired token per 1 native token.
     *           - If `targetToken` is the paired token, `price` is in terms of the native token per 1 paired token.
     *         - If `invert` is true:
     *           - If `targetToken` is the native token, `price` is in terms of the native token per 1 paired token.
     *           - If `targetToken` is the paired token, `price` is in terms of the paired token per 1 native token.
    ...
    function getSpotPrice(
        ...
        if (reserve0 == 0 || reserve1 == 0) revert NoReserves(targetToken);

        if (targetToken == native) {
            price = invert
                ? (reserve1 * 1e18) / reserve0 // Paired token in terms of native token
                : (reserve0 * 1e18) / reserve1; // Native token in terms of paired token
