...
contract BIFKN314Oracle {
    ...
    function getSpotPrice(
        ...
        if (reserve0 == 0 || reserve1 == 0) revert NoReserves(token_);

        if (token_ == native) {
            price = (reserve0 * 1e18) / reserve1; // Native token price in terms of paired token (in WEI)
        } else if (token_ == token) {
            price = (reserve1 * 1e18) / reserve0; // Paired token price in terms of native token (in WEI)
        } else {
            revert InvalidToken(token_);
        }
    }
