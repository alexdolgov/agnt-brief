...
contract BIFKN314Router is Ownable(msg.sender), ReentrancyGuard {
     ...
     *   - This function is used to swap native currency (e.g., ETH) to a token.
     *
     * LEGACY_TOKEN_TO_NATIVE_SELECTOR:
     *   - Selector for the `swapTokenToNative(uint256,uint256,uint256)` function.
     *   - This function is used to swap a token to native currency (e.g., ETH).
     *
     ...
     */
    IBIFKN314WrapperFactory public immutable wrapperFactory;
    ...
    constructor(address _wavax, address _wrapperFactory, address _factory) {
        ...
        wrapperFactory = IBIFKN314WrapperFactory(_wrapperFactory);
    ...
    function optimizePath(
        address[] memory path
    ) public view returns (address[] memory optimizedPath) {
        // Handle invalid path length first
        if (path.length < 2) {
            revert InvalidPathLength();
        }

        if (path[0] == path[path.length - 1]) {
            revert InvalidPath(); // Prevents token-to-same-token swaps
        }

        // Cache these values to avoid multiple memory access
        address firstToken = path[0];
        address lastToken = path[path.length - 1];

        // Check for common case 1: Direct [AVAX, Token] path
        if (firstToken == WAVAX) {
            optimizedPath = new address[](2);
            optimizedPath[0] = WAVAX;
            optimizedPath[1] = lastToken;
            return optimizedPath;
        }

        // Check for common case 2: Direct [Token, AVAX] path
        if (lastToken == WAVAX) {
            optimizedPath = new address[](2);
        ...
        // Handle token-to-token path
        if (firstToken != WAVAX && lastToken != WAVAX) {
            optimizedPath = new address[](3);
            optimizedPath[0] = firstToken;
            optimizedPath[1] = WAVAX;
    ...
    function _swapExactTokensForTokens(
        ...
        address inputERC314Token = _getERC314Token(inputToken);
        address outputERC314Token = _getERC314Token(outputToken);
    ...
    function _swapExactAVAXForTokens(
        ...
        // Return excess AVAX if this was called with msg.value
        if (msg.value > avaxToUse) {
            _safeTransferAVAX(msg.sender, msg.value - avaxToUse);
        }
    ...
    function _validateSwapPath(
        ...
        if (inputIsAVAX) {
            if (path[0] != WAVAX) {
                revert InputMustBeAVAX();
            }
        } else {
            if (path[0] == WAVAX) {
                revert InputCannotBeAVAX();
            }
        }

        // Validate output token
        if (outputIsAVAX) {
            if (path[pathLength - 1] != WAVAX) {
                revert PathMustEndWithAVAX();
            }
        } else {
            if (path[pathLength - 1] == WAVAX) {
                revert OutputCannotBeAVAX();
            }
        }
    }

    /**
     * @dev Maps a token to its ERC314 version for use in the router. Returns AVAX as-is,
     * wraps ERC20 to ERC314, or uses native ERC314 directly.
    ...
    function _getERC314Token(address token) internal view returns (address) {
        ...
        if (address(wrapper) != address(0)) {
            return address(wrapper.wrappedToken()); // Wrapped ERC314 token
        }
        revert InvalidToken();
    }
    ...
    function _safeApprove(
        ...
        // Direct approve with explicit error handling
