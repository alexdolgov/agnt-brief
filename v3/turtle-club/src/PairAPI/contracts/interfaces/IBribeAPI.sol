...
interface IBribeAPI {
    ...
    function rewardPerToken(address _token, uint256 ts) external view returns(uint256);
    ...
    function earnedTokenId(address token, uint256 tokenId, uint48 timestamp) external view returns (uint256);
}