...
contract CollateralReserve is AccessControlUpgradeable {
    ...
    function totalGlobalSynthValue() public view returns (uint256 _tgsv) {
        for (uint256 i = 0; i < synthArray.length; i++) {
            if (synthArray[i] != address(0)) {
                uint256 _offset = synthOffset[synthArray[i]];
                uint256 _totalSupply = IERC20(synthArray[i]).totalSupply().sub(
                    _offset
                );
    ...
    function setSynthOffset(address _synth, uint256 _offset) external {
        require(hasRole(MAINTAINER, msg.sender), "Sender is not a maintainer");
        synthOffset[_synth] = _offset;

        emit SetSynthOffset(_synth, _offset);
    }
    ...
    mapping(address => uint256) public synthOffset;
}
