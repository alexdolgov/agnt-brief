...
contract UNCX_ProofOfReservesUniV3 is IUNCX_ProofOfReservesUniV3, Ownable2Step, IERC721Receiver, ReentrancyGuard {
    ...
    constructor(ICountryList _countryList, address payable _autoCollectAddress, address payable _lpFeeReceiver, address payable _collectFeeReceiver) {
        ...
        addOrEditFee("DEFAULT", 100, 0, 0, address(0));
    }
    ...
    */
    ...
    function emitLockEvent (uint256 _lockId) private {
        ...
        INonfungiblePositionManager.Position memory position;
        (,,position.token0,position.token1,position.fee,,,,,,,) = newLock.nftPositionManager.positions(newLock.nft_id);
        emit onLock(
            newLock.lock_id, 
    ...
    */
    function collect (uint256 _lockId, address _recipient, uint128 _amount0Max, uint128 _amount1Max) external override nonReentrant {
        _collect(_lockId, _recipient, _amount0Max, _amount1Max);
    }

    /**
    * @dev Private collect function, wrap this in re-entrancy guard calls
    */
    function _collect (uint256 _lockId, address _recipient, uint128 _amount0Max, uint128 _amount1Max) private {
        ...
        if (userLock.ucf == 0) { // No Protocol fee
            userLock.nftPositionManager.collect(INonfungiblePositionManager.CollectParams(userLock.nft_id, _recipient, _amount0Max, _amount1Max));
        } else { // Protocol fees
            ...
            if (balance0 > 0) {
                uint256 fee0 = balance0 * userLock.ucf / FEE_DENOMINATOR;
                TransferHelper.safeTransfer(_token0, feeTo, fee0);
                TransferHelper.safeTransfer(_token0, remainderTo, balance0 - fee0);
            }
            if (balance1 > 0) {
                uint256 fee1 = balance1 * userLock.ucf / FEE_DENOMINATOR;
                TransferHelper.safeTransfer(_token1, feeTo, fee1);
                TransferHelper.safeTransfer(_token1, remainderTo, balance1 - fee1);
            }
        }
    }
