...
contract AlphaIDO is RefProgram, IStakingLockAgent {
    using SafeERC20 for IERC20;
    ...
    struct VestingParams {
        uint32 freezePeriod;
        uint32 mainPeriod;
        uint256 preFreezeAmount;
        uint256 preMainAmount;
        uint256 postMainAmount;
    }
    ...
    function addProgram(
        ...
        _validateParams(vesting, params);
    ...
    function takeByIndex(uint256 index, uint256 amount) public onlyOwner {
        Program storage program = _programs[index];
        uint32 filledDate = program.params.draw + program.vesting.freezePeriod + program.vesting.mainPeriod;
        require(block.timestamp > filledDate || program.cancelled, 'forbidden');
    ...
    function _validateParams(VestingParams memory vesting, ProgramParams memory params) internal view {
        require(!_tokensInvolved.contains(address(params.token)), '!REPEAT');
        require(
            vesting.preFreezeAmount + vesting.preMainAmount + vesting.postMainAmount <= params.tokenAmount,
            '!AMOUNT'
        );
        require(
    ...
    function _getCumulativeClaimable(uint256 index) internal view returns (uint256 amount) {
        ...
        if (block.timestamp >= program.params.draw + program.vesting.freezePeriod + program.vesting.mainPeriod) {
            return program.params.tokenAmount;
        }
        amount = program.vesting.preFreezeAmount; // (optional) Amount at beginning of freeze period
        if (block.timestamp >= program.params.draw + program.vesting.freezePeriod) {
            amount += program.vesting.preMainAmount; // (optional) Amount at beginning of main vesting period
            uint256 mainVestmentAmount = program.params.tokenAmount - amount - program.vesting.postMainAmount;
            uint256 timePassed = block.timestamp - (program.params.draw + program.vesting.freezePeriod);
            amount += (mainVestmentAmount * timePassed) / program.vesting.mainPeriod;
        }
    }
