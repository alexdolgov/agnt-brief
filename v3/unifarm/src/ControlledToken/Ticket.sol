...
contract ControlledToken is ERC20, ControlledTokenInterface {
    ...
    function controllerBurnFrom(address _operator, address _user, uint256 _amount) external virtual override onlyController {
        if (_operator != _user) {
        uint256 decreasedAllowance = allowance(_user, _operator).sub(_amount);
        _approve(_user, _operator, decreasedAllowance);
        }
        _burn(_user, _amount);
    }

    function changeController(address _address) public onlyOwner(){
        require(_address != address(0),"only valid address");
        controller = TokenControllerInterface(_address);
  ...
  /// @dev Function modifier to ensure that the caller is the controller contract
    ...
    modifier onlyOwner() {
        require(_msgSender() == owner, " ControlledToken/only-Owner");
        _;
    }
...
contract Ticket is ControlledToken {
  ...
  bytes32 constant private TREE_KEY = keccak256("4Play/Ticket");
  uint256 constant private MAX_TREE_LEAVES = 5;

  // Ticket-weighted odds
  SortitionSumTreeFactory.SortitionSumTrees internal sortitionSumTrees;

  constructor() public ControlledToken("4PlayTicket","4PT"){
  ...
  function draw(uint256 randomNumber) public view  returns (address) {
    ...
    if (bound == 0) {
      ...
      uint256 token = randomNumber % bound;
