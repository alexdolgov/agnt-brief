 ...
 */
contract UnionClubPluginV3 is UUPSUpgradeable, AccessControlUpgradeable {
    ...
    function mintMemberNFT(address recipient) external returns (uint) {
        require(invited[recipient], "account not in invited list");
    ...
    function invite(address account) external {
        require(!invited[account], "account already in invited list");
        invited[account] = true;
        require(IClubMemberNFTV3(clubMemberNFT).useInviteCount(msg.sender), "no invite count");
    ...
    function clearBadDebt(uint256 tokenId) external {
        ...
        IClubMemberNFTV3 clubMemberNFTContract = IClubMemberNFTV3(clubMemberNFT);
        require(clubMemberNFTContract.ownerOf(tokenId) == msg.sender, "Not owner");
        require(!uTokenContract.checkIsOverdue(account), "Account is overdue");
    ...
    function deleteMembership(uint256 tokenId) external {
        ...
        require(clubMemberNFTContract.ownerOf(tokenId) == msg.sender, "Not owner");
        (, , uint256 badDebt, , ) = clubMemberNFTContract.nftCreditStatus(tokenId);
        require(!uTokenContract.checkIsOverdue(account) && badDebt == 0, "Account is overdue");
    ...
    function validateUpdate(address from, address to, uint tokenId) external onlyMemberNFT returns (bool) {
        ...
        if (from != address(0)) {
            ...
            require(badDebt == 0, "Sender has badDebt");
            require(active, "Sender not active");
