...
import "./IUnitasMintingEventsV2.sol";

interface IUnitasMintingV2 is IUnitasMintingEventsV2 {
  enum Role {
    Minter,
    Redeemer
  }
  ...
  struct BlockTotals {
    /// @notice usdu minted per block / per asset per block
    uint128 mintedPerBlock;
    /// @notice usdu redeemed per block / per asset per block
    uint128 redeemedPerBlock;
  }

  struct GlobalConfig {
    /// @notice max usdu that can be minted across all assets within a single block.
    uint128 globalMaxMintPerBlock;
    /// @notice max usdu that can be redeemed across all assets within a single block.
