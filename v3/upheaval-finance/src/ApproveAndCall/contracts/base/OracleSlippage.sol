...
import '@upheavalswap/v3-periphery/contracts/base/BlockTimestamp.sol';
import '@upheavalswap/v3-periphery/contracts/libraries/Path.sol';
import '@upheavalswap/v3-periphery/contracts/libraries/PoolAddress.sol';
import '@upheavalswap/v3-core/contracts/interfaces/IUpheavalV3Pool.sol';
import '@upheavalswap/v3-periphery/contracts/libraries/OracleLibrary.sol';
...
abstract contract OracleSlippage is IOracleSlippage, PeripheryImmutableState, BlockTimestamp {
    ...
    function getBlockStartingAndCurrentTick(IUpheavalV3Pool pool)
        internal
        view
    ...
    function getPoolAddress(
        ...
        address tokenB,
        uint24 fee
    ) internal view virtual returns (IUpheavalV3Pool pool) {
        pool = IUpheavalV3Pool(PoolAddress.computeAddress(deployer, PoolAddress.getPoolKey(tokenA, tokenB, fee)));
    }
        ...
        returns (int256 syntheticAverageTick, int256 syntheticCurrentTick)
    {
        ...
        for (uint256 i = 0; i < numPools; i++) {
            ...
            IUpheavalV3Pool pool = getPoolAddress(tokenIn, tokenOut, fee);
            ...
            if (secondsAgo == 0) {
                ...
                (, currentTick, , , , , ) = IUpheavalV3Pool(pool).slot0();
            }
