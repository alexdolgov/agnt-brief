...
/**
 * @title LinearVesting
 * @dev Linear vesting contract for ERC20 tokens
 * Allows the owner to claim tokens linearly over time after a start date
 */
contract LinearVesting is Ownable, ReentrancyGuard {
    ...
    /**
     * @dev Calculate the amount of tokens that can be claimed at the current time
    ...
    /**
     * @dev Calculate the total amount of tokens vested at the current time
    ...
    /**
     * @dev Claim vested tokens (only owner)
     */
    function claimTokens() external onlyOwner nonReentrant onlyAfterStart {
        uint256 claimableAmount = getClaimableAmount();
        require(claimableAmount > 0, "No tokens available for claim");
        
        claimedAmount += claimableAmount;
        token.safeTransfer(owner(), claimableAmount);
        
        emit TokensClaimed(owner(), claimableAmount);
    }
    
    /**
     * @dev Claim a specific amount of vested tokens (only owner)
     * @param amount The amount of tokens to claim
     */
    function claimTokens(uint256 amount) external onlyOwner nonReentrant onlyAfterStart {
        require(amount > 0, "Amount must be greater than 0");
        
        uint256 claimableAmount = getClaimableAmount();
        require(amount <= claimableAmount, "Amount exceeds claimable tokens");
    ...
    function getVestingInfo() external view returns (
        ...
        _vestedAmount = getVestedAmount();
        _claimedAmount = claimedAmount;
        _claimableAmount = getClaimableAmount();
        _remainingAmount = totalAmount - claimedAmount;
        _startTime = startTime;
        _endTime = endTime;
        _isActive = block.timestamp >= startTime && block.timestamp <= endTime;
        _isRevoked = false;
    }
    ...
    /**
     * @dev Calculate vesting progress as a percentage (in basis points)
    ...
    function getVestingProgress() external view returns (uint256) {
        ...
        return (timeElapsed * 10000) / vestingDuration;
    }
}