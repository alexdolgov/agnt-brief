...
/// @title Canonical UpheavalSwap V3 factory
/// @notice Deploys UpheavalSwap V3 pools and manages ownership and control over pool protocol fees
contract UpheavalV3Factory is IUpheavalV3Factory {
    /// @inheritdoc IUpheavalV3Factory
    ...
    function createPool(
        ...
        pool = IUpheavalV3PoolDeployer(poolDeployer).deploy(address(this), token0, token1, fee, tickSpacing);
    ...
    function setFeeProtocol(address pool, uint32 feeProtocol0, uint32 feeProtocol1) external override onlyOwner {
        IUpheavalV3Pool(pool).setFeeProtocol(feeProtocol0, feeProtocol1);
    }
    ...
    function collectProtocol(
        ...
        return IUpheavalV3Pool(pool).collectProtocol(recipient, amount0Requested, amount1Requested);
    }

    function setLmPool(address pool, address lmPool) external override onlyOwnerOrLmPoolDeployer {
        IUpheavalV3Pool(pool).setLmPool(lmPool);
    }
}
