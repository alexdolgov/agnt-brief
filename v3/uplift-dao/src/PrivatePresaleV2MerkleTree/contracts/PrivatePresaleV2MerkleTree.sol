...
import "./interfaces/IPrivatePresaleV2MerkleTree.sol";
import "./libraries/UQ112x112.sol";

/// @title Private Presale contract v2 contract with Merkle tree integration
contract PrivatePresaleV2MerkleTree is IPrivatePresaleV2MerkleTree, UUPSUpgradeable, ReentrancyGuardUpgradeable, ERC165 {
   using SafeERC20 for IERC20;
    ...
    function initialize(InitializeParams calldata _initializeParams) initializer external {
        ...
        require(_initializeParams.endTimestamp > _initializeParams.startTimestamp, "PrivatePresaleV2MerkleTree: INVALID_TIME");
        ...
        updateStakingPower(_buyParams.idsToUpdate)
    {
        ...
        if (!isProofValid && !IAccessControl(registry).hasRole(PRIVATE_PRESALE_BUYER, msg.sender)) {
            ...
            require(stakingDetails.accountDetails.totalStakingPower >= minStakingPower, "PrivatePresaleV2MerkleTree: INVALID_STAKING_POWER");
        }
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override returns (bool) {
        return interfaceId == type(IERC165).interfaceId || interfaceId == type(IPrivatePresaleV2MerkleTree).interfaceId;
    }
    ...
    modifier requireKYC(bytes[] calldata _signatures, address[] calldata _signers) {
        ...
        require(IWhitelist(whitelist).isAddressWhitelisted(data, _signatures, _signers), "PrivatePresaleV2MerkleTree: USER_SHOULD_PASS_KYC");
        _;
    }
