...
interface IHookableLender {
    ...
    function notifyPrincipalIncrease(uint256 delta) external;
}
...
interface IPeerToPeerOpenTermLoan {
    ...
    function proposedPrincipalIncrease() external view returns (uint256);    
...
abstract contract AbstractLender is OwnableLiquidityPool {
    ...
    function acceptPrincipalIncrease(address loanAddr) external nonReentrant ifConfigured onlyLoansOperator {
        ...
        uint256 amount = IPeerToPeerOpenTermLoan(loanAddr).proposedPrincipalIncrease();
        if (amount < 1) revert AssetsAmountRequired();

        SafeERC20.safeApprove(_underlyingAsset, loanAddr, amount);

        IPeerToPeerOpenTermLoan(loanAddr).acceptPrincipalIncrease();

        SafeERC20.safeApprove(_underlyingAsset, loanAddr, 0);
    }
...
abstract contract HookableLender is IHookableLender, AbstractLender {
    ...
    function notifyPrincipalIncrease(uint256 newDelta) external override ifConfigured onlyKnownLoanContract {
        _deployedLoans[msg.sender].activeDelta += newDelta;
        globalLoansAmount += newDelta;
    }
...
contract LendingPoolv3 is LendingPoolv2 {
    constructor() LendingPoolv2() {
    }

    function updateLoansDeployerAddress(address newAddr) external nonReentrant ifConfigured onlyOwner {
        loansDeployerAddress = newAddr;
    }
}