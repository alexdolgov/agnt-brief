...
contract SilentDataRollupERC20 is ExtendedERC20 {
  ...
  function balanceOf(address account) public view override returns (uint256) {
    ...
    require(
      ...
      'SilentDataRollupERC20: Only caller with roles DEFAULT_ADMIN_ROLE or PAUSER_ROLE or VIEWER_ROLE granted or get its own balance.'
    );
  ...
  function isHashRegistered(bytes32 documentHash) public view override returns (bool) {
    ...
    require(
      hasAccess,
      'SilentDataRollupERC20: Only caller with roles DEFAULT_ADMIN_ROLE or PAUSER_ROLE or VIEWER_ROLE granted.'
    );
