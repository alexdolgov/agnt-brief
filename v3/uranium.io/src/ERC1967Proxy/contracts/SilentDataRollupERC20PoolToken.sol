...
import './ERC20PoolToken.sol';

contract SilentDataRollupERC20PoolToken is ERC20PoolToken {
  ...
  function balanceOf(address account) public view override returns (uint256) {
    ...
    require(
      ...
      'SilentDataRollupERC20PoolToken: Only caller with roles DEFAULT_ADMIN_ROLE or PAUSER_ROLE or VIEWER_ROLE granted or get its own balance.'
    );
  ...
  function isHashRegistered(bytes32 documentHash) public view override returns (bool) {
    ...
    require(
      hasAccess,
      'SilentDataRollupERC20PoolToken: Only caller with roles DEFAULT_ADMIN_ROLE or PAUSER_ROLE or VIEWER_ROLE granted.'
    );
