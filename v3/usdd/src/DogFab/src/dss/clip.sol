...
/// clip.sol -- Usdd auction module 2.0
...
contract Clipper {
    ...
    address     public vow;      // Recipient of usdd raised in auctions
    ...
    // 
    // Auctions will not collect more USDD than their assigned USDD target,`tab`;
    // thus, if `amt` would cost more USDD than `tab` at the current price, the
    // amount of collateral purchased will instead be just enough to collect `tab` USDD.
    //
    // To avoid partial purchases resulting in very small leftover auctions that will
    // never be cleared, any partial purchase must leave at least `Clipper.chost`
    // remaining USDD target. `chost` is an asynchronously updated value equal to
    ...
    function take(
        ...
        uint256 max,          // Maximum acceptable price (USDD / collateral) [ray]
...

        {
            ...
            // USDD needed to buy a slice of this sale
            owe = mul(slice, price);

            // Don't collect more than tab of USDD
            if (owe > tab) {
            ...
            // Get USDD from caller
            vat.move(msg.sender, vow, owe);

            // Removes Usdd out for liquidation from accumulator
