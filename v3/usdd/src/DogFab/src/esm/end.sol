...
/*
    ...
    the final usdd / collateral price. In particular, we need to determine

      a. `gap`, the collateral shortfall per collateral type by
         considering under-collateralised CDPs.

      b. `debt`, the outstanding usdd supply after including system
         surplus / deficit
    ...
    We determine (b) by processing ongoing usdd generating processes,
    ...
    So the way to ensure the protocol will not receive extra usdd income is:
    ...
    6. `thaw()`:
       ...
       - fixes the total outstanding supply of usdd
    ...
    type and usdd holders can now turn their usdd into collateral. Each
    unit usdd can claim a fixed basket of collateral.

    Usdd holders must first `pack` some usdd into a `bag`. Once packed,
    usdd cannot be unpacked and is not transferrable. More usdd can be
    added to a bag later.

    8. `pack(wad)`:
        - put some usdd into a bag in preparation for `cash`

    Finally, collateral can be obtained with `cash`. The bigger the bag,
    the more collateral can be released.

    9. `cash(ilk, wad)`:
        - exchange some usdd from your bag for gems from a specific ilk
...
contract End {
    ...
    uint256  public debt;  // Total outstanding usdd following processing [rad]
    ...
    function thaw() external {
        ...
        require(vat.usdd(address(vow)) == 0, "End/surplus-not-zero");
