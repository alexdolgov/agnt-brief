...
contract DssCdpManager {
    ...
    event NewCdp(address indexed usr, address indexed own, uint indexed cdp, bytes32 ilk);
    ...
    function open(
        ...
        emit NewCdp(msg.sender, usr, cdpi, ilk);
        return cdpi;
    }
    ...
    // Frob the cdp keeping the generated USDD or collateral freed in the cdp urn address.
    function frob(
        uint cdp,
        int dink,
        int dart
    ...
    // Transfer wad amount of USDD from the cdp address to a dst address.
    function move(
        uint cdp,
        address dst,
        uint rad
