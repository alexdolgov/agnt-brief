 ...
 *  - The `push()` method swaps entire GEM balance to USDD using PSM.
 *  - The `push(uint256)` method swaps specified amount of GEM to USDD using PSM.
...
contract RwaSwapInputConduit {
    ...
    /// @notice Recipient address for USDD.
    address public to;
    ...
    /**
     * @notice `wad` amount of Usdd was pushed to `to`.
     * @param to Recipient address for USDD.
    ...
    constructor(
        ...
        require(PsmAbstract(_psm).usdd() == _usdd, "RwaSwapInputConduit/wrong-usdd-for-psm");
    ...
    function file(bytes32 what, address data) external auth {
        if (what == "quitTo") {
            ...
            require(PsmAbstract(data).usdd() == address(usdd), "RwaSwapInputConduit/wrong-usdd-for-psm");
            require(
    ...
    /**
     * @notice Swaps the GEM balance of this contract into USDD through the PSM and push it into the recipient address.
    ...
    /**
     * @notice Swaps the specified amount of GEM into USDD through the PSM and push it into the recipient address.
     * @dev `msg.sender` must have received push access through `mate()`.
    ...
    /**
     * @notice Calculates the amount of USDD received for swapping `amt` of GEM.
     * @param amt GEM amount.
     * @return wad Expected USDD amount.
     */
    function expectedUsddWad(uint256 amt) public view returns (uint256 wad) {
    ...
    /**
     * @notice Calculates the required amount of GEM to get `wad` amount of USDD.
    ...
    function _doPush(uint256 amt) internal {
        ...
        psm.sellGem(to, amt);
        emit Push(to, expectedUsddWad(amt));
    }
