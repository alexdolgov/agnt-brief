...
import {GemAbstract} from "../interfaces/ERC/GemAbstract.sol";
 ...
 *  - The `push()` method swaps all USDD balance of the contract to GEM using the PSM.
 *  - The `push(uint256)` method swaps specified amount of USDD to GEM using the PSM.
 *  - Requires USDD, GEM and PSM addresses in the constructor.
 *      - USDD and GEM are immutable, PSM can be replaced as long as it uses the same USDD and GEM.
 *  - The `quit()` method allows moving outstanding USDD balance to `quitTo`. It can be called only by `mate`d addresses.
 *  - The `quit(uint256)` method allows moving the specified amount of USDD balance to `quitTo`. It can be called only by `mate`d addresses.
...
contract RwaSwapOutputConduit {
    ...
    /// @notice USDD token contract address.
    UsddAbstract public immutable usdd;
    /// @dev USDD/GEM resolution difference.
    ...
    address public to;
    /// @notice Destination address for USDD after calling `quit`.
    ...
    /**
     * @notice The conduit outstanding USDD balance was flushed out to `quitTo` address.
     * @param quitTo The quitTo address.
     * @param wad The amount of USDD flushed out.
     */
    ...
    modifier onlyMate() {
        require(may[msg.sender] == 1, "RwaSwapOutputConduit/not-mate");
        _;
    }
     ...
     * @param _usdd USDD contract address.
    ...
    function file(bytes32 what, address data) external auth {
        if (what == "quitTo") {
            ...
            require(PsmAbstract(data).usdd() == address(usdd), "RwaSwapOutputConduit/wrong-usdd-for-psm");
            require(
            ...
            usdd.approve(address(psm), 0);
            // Give unlimited approval to the new PSM
            usdd.approve(data, type(uint256).max);
    ...
    function pick(address who) external {
        require(can[msg.sender] == 1, "RwaSwapOutputConduit/not-operator");
    ...
    /**
     * @notice Swaps the USDD balance of this contract into GEM through the PSM and push it into the recipient address.
     * @dev `msg.sender` must have received push access through `mate()`.
     */
    function push() external onlyMate {
        _doPush(usdd.balanceOf(address(this)));
    }

    /**
     * @notice Swaps the specified amount of USDD into GEM through the PSM and push it to the recipient address.
     * @dev `msg.sender` must have received push access through `mate()`.
     * @param wad USDD amount.
     */
    ...
    /**
     * @notice Flushes out any USDD balance to `quitTo` address.
     * @dev `msg.sender` must have received push access through `mate()`.
     */
    function quit() external onlyMate {
        _doQuit(usdd.balanceOf(address(this)));
    }

    /**
     * @notice Flushes out the specified amount of USDD to the `quitTo` address.
     * @dev `msg.sender` must have received push access through `mate()`.
     * @param wad USDD amount.
     */
    ...
    /**
     * @notice Calculates the amount of GEM received for swapping `wad` of USDD.
     * @param wad USDD amount.
    ...
    /**
     * @notice Calculates the required amount of USDD to get `amt` amount of GEM.
     * @param amt GEM amount.
     * @return wad Required USDD amount.
     */
    function requiredUsddWad(uint256 amt) external view returns (uint256 wad) {
    ...
    function _doPush(uint256 wad) internal {
        ...
        // We might lose some dust here because of rounding errors. I.e.: USDC has 6 dec and USDD has 18.
    ...
    /**
     * @notice Flushes out the specified amount of USDD to `quitTo` address.
     * @param wad The USDD amount.
     */
    function _doQuit(uint256 wad) internal {
        require(quitTo != address(0), "RwaSwapOutputConduit/invalid-quit-to-address");

        usdd.transfer(quitTo, wad);
