 ...
 * Usdd balance from it into the output conduit.
 */
contract RwaUrn2 {
    ...
    /// @notice The adapter to mint/burn Usdd tokens.
    UsddJoinAbstract public usddJoin;
    /// @notice The destination of Usdd drawn from this urn.
    ...
    /**
     * @notice `wad` amount of Usdd was drawn by `usr` into `outputConduit`.
     * @param usr The operator address.
     * @param wad The amount drawn.
     */
    event Draw(address indexed usr, uint256 wad);
    /**
     * @notice `wad` amount of Usdd was repaid by `usr`.
     * @param usr The operator address.
     ...
     * @param jug_ Stability fee management module.
     * @param gemJoin_ GemJoin adapter for the gem in this urn.
     * @param usddJoin_ Adapter to mint/burn Usdd tokens.
     * @param outputConduit_ Destination of Usdd drawn from this urn.
     */
    constructor(
        address vat_,
        address jug_,
        ...
        VatAbstract(vat_).hope(usddJoin_);
    ...
    /**
     * @notice Draws `wad` amount of Usdd from the contract.
    ...
    function draw(uint256 wad) external operator {
        ...
        usddJoin.exit(outputConduit, wad);
        emit Draw(msg.sender, wad);
    }

    /**
     * @notice Repays `wad` amount of Usdd to the contract.
     * @param wad The amount to wipe.
     */
    function wipe(uint256 wad) external {
        usddJoin.join(address(this), wad);
    ...
    /**
     * @notice Flushes out any outstanding Usdd balance to `outputConduit` address.
    ...
    function quit() external {
        ...
        DSTokenAbstract usdd = DSTokenAbstract(usddJoin.usdd());
        uint256 wad = usdd.balanceOf(address(this));

        usdd.transfer(outputConduit, wad);
