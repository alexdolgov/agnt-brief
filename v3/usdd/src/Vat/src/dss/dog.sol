...
/// dog.sol -- Usdd liquidation module 2.0
...
contract Dog {
    ...
    struct Ilk {
        ...
        uint256 hole;  // Max Usdd needed to cover debt+fees of active auctions per ilk [rad]
        uint256 dirt;  // Amt Usdd needed to cover debt+fees of active auctions per ilk [rad]
    }
    ...
    uint256 public Hole;  // Max Usdd needed to cover debt+fees of active auctions [rad]
    uint256 public Dirt;  // Amt Usdd needed to cover debt+fees of active auctions [rad]
    ...
    //
    // Liquidate a Vault and start a Dutch auction to sell its collateral for Usdd.
    //
    // The third argument is the address that will receive the liquidation reward, if any.
    //
    // The entire Vault will be liquidated except when the target amount of Usdd to be raised in
    ...
    // outstanding Usdd target. The one exception is if the resulting auction would likely
        ...
        uint256 dust;
        {
            ...
            if (art > dart) {
                if (mul(art - dart, rate) < dust) {
                    ...
                    // the extra amount of target Usdd over the limits intended is not of economic concern.
                    dart = art;
                } else {
