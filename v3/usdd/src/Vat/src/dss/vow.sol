...
/// vow.sol -- Usdd settlement module
...
contract Vow {
    ...
    function heal(uint rad) external {
        require(rad <= vat.usdd(address(this)), "Vow/insufficient-surplus");
    ...
    function flop() external returns (uint id) {
        ...
        require(vat.usdd(address(this)) == 0, "Vow/surplus-not-zero");
        Ash = add(Ash, sump);
    ...
    function flap() external returns (uint id) {
        require(vat.usdd(address(this)) >= add(add(vat.sin(address(this)), bump), hump), "Vow/insufficient-surplus");
    ...
    function cage() external auth {
        ...
        Sin = 0;
        Ash = 0;
        flapper.cage(vat.usdd(address(flapper)));
        flopper.cage();
        vat.heal(min(vat.usdd(address(this)), vat.sin(address(this))));
        emit Cage();
    }
}