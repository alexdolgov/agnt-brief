...
contract WBTCCSpellBeta {
    ...
    // ── WBTC-C collateral type ────────────────────────────────────────────
    bytes32 public constant ilk = "WBTC-C";

    // ── WBTC-C contract addresses (fill in after deploy_wbtc.s.sol executes)
    address public constant gem     = 0x0eef30749f8B982f5a8277b013114015460666C0;
    address public constant calc    = 0x3c3C54bA56803af4514d904870f343Cda939f69B;
    address public constant clip    = 0xF7326Ba025BD107f9487C349718B25249bA77768;
    address public constant gemjoin = 0xA8E0BAceE4874acFeb22E7BB8E86199010290a6E;
    ...
    uint256 public constant ILK_MAT   = 155 * 10 ** 27 / 100;     // 155% CR (most conservative)
    uint256 public constant ILK_DUTY  = 1000000002071266685321207000; // 6.75%/yr
    ...
    function setup() public {
        ...
        Setter(pip).kiss(clip);   // whitelist clip_c to read from OSM
        ...
        // 7. OSM / Spotter (setOsm with "WBTC-C" is idempotent, safe to call)
        ...
        // 10. ChainLog registration (C-specific keys only; oracle stack set by WBTCASpell)
        ChainLogAbstract cl = ChainLogAbstract(LOG);
        cl.setAddress("MCD_JOIN_WBTC_C",      gemjoin);
        cl.setAddress("MCD_CLIP_WBTC_C",      clip);
        cl.setAddress("MCD_CLIP_CALC_WBTC_C", calc);
        cl.setVersion("1.3.6");
    }
}
