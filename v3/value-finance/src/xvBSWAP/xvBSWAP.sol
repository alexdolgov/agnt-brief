...
contract xvBSWAP is ERC20, ITokenLocker {
    ...
    constructor(
        address _vbswap,
        uint256 __startReleaseBlock,
        uint256 __endReleaseBlock
    ) public ERC20("Locked vSWAP.fi", "xvBSWAP") {
        require(__endReleaseBlock > __startReleaseBlock, "endReleaseBlock < startReleaseBlock");
        vbswap = _vbswap;
        _startReleaseBlock = __startReleaseBlock;
        _endReleaseBlock = __endReleaseBlock;
        governance = msg.sender;
        emit EditLocker(__startReleaseBlock, __endReleaseBlock);
    }
    ...
    function editLocker(uint256 __startReleaseBlock, uint256 __endReleaseBlock) external onlyGovernance {
        require(__startReleaseBlock < __endReleaseBlock, "endReleaseBlock < startReleaseBlock");
        require(__startReleaseBlock > block.number, "late");
    ...
    function lock(address _account, uint256 _amount) external override {
        require(block.number < _startReleaseBlock, "no more lock");
        require(_account != address(0), "no lock to address(0)");
        require(_amount > 0, "zero lock");

        IERC20(vbswap).safeTransferFrom(msg.sender, address(this), _amount);
    ...
    function unlock(uint256 _amount) public override {
        require(block.number > _startReleaseBlock, "still locked");
    ...
    function claimUnlocked() public override {
        ...
        IERC20(vbswap).safeTransfer(msg.sender, _amount);
    ...
    function governanceRecoverUnsupported(
        ...
        require(_token != vbswap || IERC20(vbswap).balanceOf(address(this)).sub(_amount) >= _totalLock, "Not enough locked amount left");
