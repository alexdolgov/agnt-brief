...
import { LibBytesToTypes } from "./LibBytesToTypes.sol";

library LibBytesManipulation {
    ...
    function bytesToAddress(uint256 _offst, bytes memory _input) internal pure returns (address) {
        return LibBytesToTypes.bytesToAddress(_offst, _input);
    }

    function bytesToUint256(uint256 _offst, bytes memory _input) internal pure returns (uint256) {
        return LibBytesToTypes.bytesToUint256(_offst, _input);
    }
}
