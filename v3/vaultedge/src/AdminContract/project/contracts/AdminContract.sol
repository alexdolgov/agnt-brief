...
contract AdminContract is IAdminContract, OwnableUpgradeable, Addresses {
    // Constants --------------------------------------------------------------------------------------------------------
    ...
    uint256 public constant MIN_NET_DEBT_DEFAULT = 200 ether;
    uint256 public constant MINT_CAP_DEFAULT = 1_000 ether; // 1000 debt tokens tokens
    uint256 public constant COLL_GAS_COMPENSATION_DIVISOR_DEFAULT = 200; // dividing by 200 yields 0.5%
    uint256 public constant REDEMPTION_FEE_FLOOR_DEFAULT = 0.005 ether; // 0.5%
    uint256 public constant REDEMPTION_BLOCK_TIMESTAMP_DEFAULT = 0; // never
    ...
    // Fixed APR per-second rate for interest accrual (1e18 precision), stored separately for upgrade safety
    mapping(address => uint256) internal borrowRatePerSecond;
    ...
    function addNewCollateral(
        ...
        uint8 onchainDecimals = IERC20Decimals(_collateral).decimals();
        require(onchainDecimals <= 18, "collateral decimals cannot be above 18");
        ...
        collateralParams[_collateral] = CollateralParams({
            ...
            collGasCompensationDivisor: COLL_GAS_COMPENSATION_DIVISOR_DEFAULT,
    ...
    function setCollateralParameters(
        ...
        setCollGasCompensationDivisor(_collateral, collGasCompensationDivisor);
    ...
    function setIsActive(address _collateral, bool _active) external onlyTimelock {
        ...
        emit CollateralStateChanged(_collateral, _active);
    }
    ...
    /// @notice Sets the divisor for calculating the collateral gas compensation.
    /// @dev The divisor is used to calculate the collateral gas compensation as follows:
    ///      collateral gas compensation = collateral balance / divisor.
    ///      20 = 5%, 200 = 0.5%, 1000 = 0.1%, type(uint256).max = 0%
    ///      For example, if the collateral balance is 1000 and the divisor is 200, the collateral gas compensation will be 5.
    function setCollGasCompensationDivisor(
        address _collateral,
        uint256 collGasCompensationDivisor
    )
        public
        override
        onlyTimelock
        safeCheck("Coll Gas Compensation Divisor", _collateral, collGasCompensationDivisor, 20, type(uint256).max)
    {
        CollateralParams storage collParams = collateralParams[_collateral];
        uint256 oldCollGasCompensationDivisor = collParams.collGasCompensationDivisor;
        collParams.collGasCompensationDivisor = collGasCompensationDivisor;
        emit CollGasCompensationDivisorChanged(oldCollGasCompensationDivisor, collGasCompensationDivisor);
    }

    /// @notice Leaving for backwards compatibility
    /// @dev deprecated in favor of setCollGasCompensationDivisor
    function setPercentDivisor(address _collateral, uint256 percentDivisor) public onlyTimelock {
        setCollGasCompensationDivisor(_collateral, percentDivisor);
    }
    ...
    function getCollGasCompensationDivisor(address _collateral) public view override returns (uint256) {
        return collateralParams[_collateral].collGasCompensationDivisor;
    }

    /// @notice Leaving for backwards compatibility
    /// @dev deprecated in favor of getCollGasCompensationDivisor
    function getPercentDivisor(address _collateral) external view returns (uint256) {
        return getCollGasCompensationDivisor(_collateral);
    }
    ...
    function getTotalAssetDebt(address _asset) external view override returns (uint256) {
        return
            IActivePool(activePool).getDebtTokenBalance(_asset) + IDefaultPool(defaultPool).getDebtTokenBalance(_asset);
    }
