...
contract BorrowerOperations is SharedProtocolBase, ReentrancyGuardUpgradeable, IBorrowerOperations {
    ...
    function openVessel(
        ...
        vars.ICR = MathUtils._computeCR(_assetAmount, vars.compositeDebt, vars.price);
        vars.NICR = MathUtils._computeNominalCR(_assetAmount, vars.compositeDebt);
    ...
    function _adjustVessel(
        ...
        vars.oldICR = MathUtils._computeCR(vars.coll, vars.debt, vars.price);
    ...
    function _activePoolAddColl(address _asset, uint256 _amount) internal {
        /// @dev Enforce alignment so 18-decimal internal amounts map to whole token units for non-18 decimal tokens
        SafetyTransfer.requireAlignedAmount(_asset, _amount);
    ...
    function _getNewICRFromVesselChange(
        ...
        uint256 newICR = MathUtils._computeCR(newColl, newDebt, _price);
        return newICR;
    }
    ...
    function _getNewTCRFromVesselChange(
        ...
        uint256 newTCR = MathUtils._computeCR(totalColl, totalDebt, _price);
        return newTCR;
    }
