...
contract FeeCollector is IFeeCollector, IVersionable, OwnableUpgradeable, Addresses {
     ...
     *
     * ### v1.1.0 (October 2025)
     * - Added configurable refund toggle (`refundsEnabled` flag)
     * - When disabled (default): protocol collects 100% of borrowing fees immediately with no refunds
     * - When enabled: time-based refund mechanism applies (3.85% immediate, 96.15% refundable over ~6 months)
     * - Existing FeeRecords always honored regardless of flag state (user protection guarantee)
     * - Added `setRefundsEnabled(bool)` admin function with `onlyOwner` access control
     * - Added `RefundsEnabledUpdated` event for transparency
     * - Storage layout: added `refundsEnabled` state variable + `__gap[48]` for upgrade safety
     *
     * ### v1.0.0 (Initial Release)
     * - Core fee collection and time-based refund mechanism
     * - Split fee structure: minimum fee (3.85%) + refundable portion (96.15%)
     * - Linear decay over 175 days with 7-day grace period
     * - FeeRecord management for tracking user fee commitments
     */
    ...
    bool public constant routeToStaking = false; // if true, collected fees go to stakers; if false, to the treasury

    bool public refundsEnabled; // if true, time-based fee refunds are enabled; if false, 100% fee collected immediately

    /**
     * @dev This empty reserved space is put in place to allow future versions to add new
     * state variables without shifting down storage in the inheritance chain.
    ...
    function initialize() public initializer {
        __Ownable_init();
        refundsEnabled = false; // Default: refunds disabled (protocol collects 100% fee immediately)
    }
     ...
     *
     * Behavior depends on refundsEnabled flag:
     * - If refundsEnabled = true: Collects minimum fee (3.85%) immediately, creates FeeRecord for remaining amount (96.15%)
     *   with time-based refund eligibility over ~6 months
     * - If refundsEnabled = false: Collects 100% of fee immediately, no FeeRecord created, no refunds available
     *
    ...
    function increaseDebt(
        ...
        if (refundsEnabled) {
            // Refunds enabled: split fee into minimum (collected now) + refundable (FeeRecord)
            ...
            // Refunds disabled: collect entire fee immediately, no FeeRecord
            _collectFee(_borrower, _asset, _feeAmount);
        }
    }
    ...
    function handleRedemptionFee(address _asset, uint256 _amount) external onlyVesselManager {
        if (_routeToStaking()) {
            IStaking(vedgeStaking).increaseFee_Asset(_asset, _amount);
        }
        emit RedemptionFeeCollected(_asset, _amount);
    }

    function getProtocolRevenueDestination() public view override returns (address) {
        return _routeToStaking() ? vedgeStaking : treasuryAddress;
    }

    /**
     * @notice Enable or disable the time-based fee refund mechanism
     * @dev When disabled, 100% of borrowing fees are collected immediately with no refunds
     * @dev Existing FeeRecords are always honored regardless of this flag state
     * @param _enabled True to enable time-based refunds, false to disable (collect 100% immediately)
     */
    function setRefundsEnabled(bool _enabled) external onlyOwner {
        if (refundsEnabled == _enabled) return; // No-op if no change
        refundsEnabled = _enabled;
        emit RefundsEnabledUpdated(_enabled);
    }
    ...
    /**
     * Transfers collected (debt token) fees to either the treasury or the VEDGEStaking contract, depending on a flag.
     */
    function _collectFee(address _borrower, address _asset, uint256 _feeAmount) internal {
        if (_feeAmount != 0) {
            ...
            if (_routeToStaking()) {
                IStaking(vedgeStaking).increaseFee_DebtToken(_feeAmount);
            }
     ...
     */
    function _routeToStaking() internal view virtual returns (bool) {
        return routeToStaking;
    }
