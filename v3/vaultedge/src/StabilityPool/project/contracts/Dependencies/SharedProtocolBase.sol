...
import "../Interfaces/ISharedProtocolBase.sol";
 ...
 * common functions.
 */
abstract contract SharedProtocolBase is ISharedProtocolBase, BaseMath, OwnableUpgradeable, Addresses {
    ...
    function _getCollGasCompensation(address _asset, uint256 _entireColl) internal view returns (uint256) {
        return _entireColl / IAdminContract(adminContract).getCollGasCompensationDivisor(_asset);
    }
    ...
    function _getTCR(address _asset, uint256 _price) internal view returns (uint256 TCR) {
        ...
        TCR = MathUtils._computeCR(entireSystemColl, entireSystemDebt, _price);
    }
