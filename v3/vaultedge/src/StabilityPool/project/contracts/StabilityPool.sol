...
import "./Dependencies/SharedProtocolBase.sol";
 ...
 *
 *
 * --- ISSUANCE TO STABILITY POOL DEPOSITORS ---
 *
 * An issuance event occurs at every deposit operation, and every liquidation.
 *
 * All deposits earn a share of the issued in proportion to the deposit as a share of total deposits.
 *
 * Please see the system Readme for an overview:
 * https://github.com/liquity/dev/blob/main/README.md#lqty-issuance-to-stability-providers
 *
 * We use the same mathematical product-sum approach to track gains for depositors, where 'G' is the sum corresponding to gains.
 * The product P (and snapshot P_t) is re-used, as the ratio P/P_t tracks a deposit's depletion due to liquidations.
 *
 */
contract StabilityPool is ReentrancyGuardUpgradeable, SharedProtocolBase, IStabilityPool {
     ...
     * depositSnapshots are used to allocate VEDGE rewards, calculate compoundedDepositAmount
    ...
    /*
     * Similarly, the sum 'G' is used to calculate VEDGE gains. During it's lifetime, each deposit d_t earns a VEDGE gain of
     *  ( d_t * [G - G_t] )/P_t, where G_t is the depositor's snapshot of G taken at time t when  the deposit was made.
     *
     *  VEDGE reward events occur are triggered by depositor operations (new deposit, topup, withdrawal), and liquidations.
     *  In each case, the VEDGE reward is issued (i.e. G is updated), before other state changes are made.
     */
    mapping(uint128 => mapping(uint128 => uint256)) public epochToScaleToG;

    // Error tracker for the error correction in the VEDGE issuance calculation
    uint256 public lastVEDGEError;
     ...
     * @dev Triggers a VEDGE issuance, based on time passed since the last issuance.
     * The VEDGE issuance is shared between *all* depositors
     * - Sends depositor's accumulated gains (VEDGE, collateral assets) to depositor
    ...
    function provideToSP(uint256 _amount, address[] calldata _assets) external override nonReentrant {
        ...
        // First pay out any VEDGE gains
        _payOutVEDGEGains(msg.sender);
    ...
    function _withdrawFromSP(
        ...
        uint256 debtTokensToWithdraw = MathUtils._min(_amount, compoundedDeposit);
        uint256 loss = initialDeposit - compoundedDeposit; // Needed only for event log
    ...
    // --- VEDGE issuance functions ---

    function _triggerCommunityIssuance() internal {
        if (communityIssuance != address(0)) {
            uint256 tokensIssued = ICommunityIssuance(communityIssuance).issueTokens();
            _updateG(tokensIssued);
        }
    }

    function _updateG(uint256 _VEDGEIssuance) internal {
        uint256 cachedTotalDebtTokenDeposits = totalDebtTokenDeposits; // cached to save an SLOAD
        /*
         * When total deposits is 0, G is not updated. In this case, the VEDGE issued can not be obtained by later
         * depositors - it is missed out on, and remains in the balanceof the CommunityIssuance contract.
         *
         */
        if (cachedTotalDebtTokenDeposits == 0 || _VEDGEIssuance == 0) {
            return;
        }
        uint256 VEDGEPerUnitStaked = _computeVEDGEPerUnitStaked(_VEDGEIssuance, cachedTotalDebtTokenDeposits);
        uint256 marginalVEDGEGain = VEDGEPerUnitStaked * P;
        uint256 newEpochToScaleToG = epochToScaleToG[currentEpoch][currentScale];
        newEpochToScaleToG += marginalVEDGEGain;
        epochToScaleToG[currentEpoch][currentScale] = newEpochToScaleToG;
        emit G_Updated(newEpochToScaleToG, currentEpoch, currentScale);
    }

    function _computeVEDGEPerUnitStaked(uint256 _VEDGEIssuance, uint256 _totalDeposits) internal returns (uint256) {
        /*
         * Calculate the VEDGE-per-unit staked.  Division uses a "feedback" error correction, to keep the
         ...
         */
        uint256 VEDGENumerator = (_VEDGEIssuance * DECIMAL_PRECISION) + lastVEDGEError;
        uint256 VEDGEPerUnitStaked = VEDGENumerator / _totalDeposits;
        lastVEDGEError = VEDGENumerator - (VEDGEPerUnitStaked * _totalDeposits);
        return VEDGEPerUnitStaked;
    }
    ...
    /*
     * Calculate the VEDGE gain earned by a deposit since its last snapshots were taken.
     * Given by the formula:  VEDGE = d0 * (G - G(0))/P(0)
     * where G(0) and P(0) are the depositor's snapshots of the sum G and product P, respectively.
     * d0 is the last recorded deposit value.
     */
    function getDepositorVEDGEGain(address _depositor) public view override returns (uint256) {
        uint256 initialDeposit = deposits[_depositor];
        if (initialDeposit == 0) {
            return 0;
        }
        ...
        Snapshots storage snapshots = depositSnapshots[_depositor];
        return _getVEDGEGainFromSnapshots(initialDeposit, snapshots);
    }

    function _getVEDGEGainFromSnapshots(
        uint256 initialStake,
        Snapshots storage snapshots
    ) internal view returns (uint256) {
        /*
         * Grab the sum 'G' from the epoch at which the stake was made. The VEDGE gain may span up to one scale change.
         * If it does, the second portion of the VEDGE gain is scaled by 1e9.
         * If the gain spans no scale change, the second portion will be 0.
         */
        ...
        uint256 VEDGEGain = (initialStake * (firstPortion + secondPortion)) / P_Snapshot / DECIMAL_PRECISION;
    ...
    function _sendGainsToDepositor(address _to, address[] memory assets, uint256[] memory amounts) internal {
        ...
        for (uint256 i = 0; i < assetsLen; ) {
            ...
            uint256 decimalsCorrectedAmount = SafetyTransfer.decimalsCorrection(asset, amount);
            if (decimalsCorrectedAmount != 0) {
                IERC20Upgradeable(asset).safeTransfer(_to, decimalsCorrectedAmount);
            }
            unchecked {
                i++;
            }
    ...
    function _payOutVEDGEGains(address _depositor) internal {
        if (address(communityIssuance) != address(0)) {
            uint256 depositorVEDGEGain = getDepositorVEDGEGain(_depositor);
            ICommunityIssuance(communityIssuance).sendTokens(_depositor, depositorVEDGEGain);
            emit VEDGEPaidToDepositor(_depositor, depositorVEDGEGain);
        }
    }
