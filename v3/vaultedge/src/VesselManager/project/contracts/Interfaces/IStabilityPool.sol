...
interface IStabilityPool is IDeposit {
    ...
    event VEDGEPaidToDepositor(address indexed _depositor, uint256 _VEDGE);
     ...
     * - _amount is not zero
     * ---
     * - Triggers a VEDGE issuance, based on time passed since the last issuance. The VEDGE issuance is shared between *all* depositors.
     * - Sends depositor's accumulated gains (VEDGE, assets) to depositor
     */
     ...
     * ---
     * - Triggers a VEDGE issuance, based on time passed since the last issuance. The VEDGE issuance is shared between *all* depositors.
     * - Sends all depositor's accumulated gains (VEDGE, assets) to depositor
     ...
     */
    function getDepositorVEDGEGain(address _depositor) external view returns (uint256);
