 ...
 * @custom:changelog
 * - v1.1.0: Rename percentDivisor to collGasCompensationDivisor and add getCollGasCompensationDivisor and setCollGasCompensationDivisor
 */
interface IAdminContract {
    ...
    event CollateralStateChanged(address _collateral, bool _active);
    event MCRChanged(uint256 oldMCR, uint256 newMCR);
    event CCRChanged(uint256 oldCCR, uint256 newCCR);
    event MinNetDebtChanged(uint256 oldMinNet, uint256 newMinNet);
    event CollGasCompensationDivisorChanged(
        uint256 oldCollGasCompensationDivisor,
        uint256 newCollGasCompensationDivisor
    );
    ...
    function setCollGasCompensationDivisor(address _collateral, uint256 collGasCompensationDivisor) external;
    ...
    function getCollGasCompensationDivisor(address _collateral) external view returns (uint256);
