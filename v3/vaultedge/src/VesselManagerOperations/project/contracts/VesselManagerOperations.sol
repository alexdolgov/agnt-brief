...
contract VesselManagerOperations is IVesselManagerOperations, ReentrancyGuardUpgradeable, SharedProtocolBase {
    ...
    function redeemCollateral(
        ...
        // Use the saved total debt tokens token supply value, from before it was reduced by the redemption.
        ...
        while (currentVesselBorrower != address(0) && remainingDebt != 0 && vars.maxIterations-- != 0) {
            ...
            if (currentVesselNetDebt <= remainingDebt) {
                ...
                if (currentVesselNetDebt > IAdminContract(adminContract).getMinNetDebt(vars.asset)) {
                    uint256 maxRedeemableDebt = MathUtils._min(
                        remainingDebt,
                    ...
                    partialRedemptionHintNewICR = MathUtils._computeNominalCR(newColl, compositeDebt);
    ...
    */
    function getApproxHint(
        ...
        diff = MathUtils._getAbsoluteDifference(_CR, IVesselManager(vesselManager).getNominalICR(_asset, hintAddress));
        ...
        while (i < _numTrials) {
            ...
            uint256 currentDiff = MathUtils._getAbsoluteDifference(currentNICR, _CR);
    ...
    function computeNominalCR(uint256 _coll, uint256 _debt) external pure override returns (uint256) {
        return MathUtils._computeNominalCR(_coll, _debt);
    }
    ...
    function _getTotalFromBatchLiquidate_RecoveryMode(
        ...
        for (uint i = 0; i < _vesselArray.length; ) {
            ...
            if (!vars.backToNormalMode) {
                ...
                if (
                    vars.ICR >= IAdminContract(adminContract).getMcr(_asset) && vars.remainingDebtTokenInStabPool == 0
                ) {
                    unchecked {
                        ++i;
                    }
                ...
                uint256 TCR = MathUtils._computeCR(vars.entireSystemColl, vars.entireSystemDebt, _price);
        ...
        returns (uint256 debtToOffset, uint256 collToSendToSP, uint256 debtToRedistribute, uint256 collToRedistribute)
    {
        if (_debtTokenInStabPool != 0) {
             ...
             *
             */
            debtToOffset = MathUtils._min(_debt, _debtTokenInStabPool);
    ...
    function _checkPotentialRecoveryMode(
        ...
        uint256 TCR = MathUtils._computeCR(_entireSystemColl, _entireSystemDebt, _price);
    ...
    // Redeem as much collateral as possible from _borrower's vessel in exchange for debt tokens tokens up to _maxDebtTokenAmount
    function _redeemCollateralFromVessel(
        ...
        singleRedemption.debtLot = MathUtils._min(
            _maxDebtTokenAmount,
        ...
        if (newDebt == IAdminContract(adminContract).getDebtTokenGasCompensation(_asset)) {
            ...
            uint256 newNICR = MathUtils._computeNominalCR(newColl, newDebt);
