...
contract VEIL_200_USDC is MerkleTreeWithHistory, ReentrancyGuard {
    /// @dev The SNARK verifier contract used to verify zero-knowledge proofs
    Verifier public immutable verifier;
    /// @dev The fixed amount of USDC that can be deposited/withdrawn (200 USDC)
    uint256 public denomination = 200 * 10 ** 6;
    ...
    /// @dev The ERC20 token that will be used for deposits and withdrawals
    IERC20 public immutable token;

    /// @dev Amount of ETH that must be deposited for each deposit for gas
    uint256 public gasDeposit = 0.0001 ether;
     ...
     */
    constructor(Verifier _verifier, IHasher _hasher, uint32 _merkleTreeHeight, IERC20 _token)
    ...
    function deposit(bytes32 _commitment) external payable nonReentrant onlyValidator {
        ...
        require(msg.value == gasDeposit, "Gas deposit is required");
    ...
    function _processWithdraw(address _recipient, address _relayer, uint256 _fee, uint256 _refund) internal {
        ...
        (bool success,) = _recipient.call{value: gasDeposit}("");
    ...
    /**
     * @dev Updates the amount of ETH that must be deposited for each deposit for gas
     * @param _newGasDeposit The amount of ETH that must be deposited for each deposit for gas
     */
    function updateGasDeposit(uint256 _newGasDeposit) external onlyVeilDeployer {
        gasDeposit = _newGasDeposit;
    }
}
