 ...
 * @title VeilBTCPool
 * @author Veil Cash
 * @dev Veil Wallet BTC Pool - Called via VeilWalletEntry
 */
contract VeilBTCPool is MerkleTreeWithHistory, ReentrancyGuard, Ownable {
    // @dev Operation-Specific Errors (BTC specific operations)
    error BTCTransferFailed();
    error BTCTransferFromFailed();
    ...
    event BTCWithdrawal(address indexed recipient, uint256 amount);
    event BTCTransfer();
     ...
     * @param _btc BTC token address for the pool
     */
    constructor(IVerifier _verifier2, IVerifier _verifier16, uint32 _levels, address _hasher, IERC20 _btc)
        Ownable(msg.sender)
     ...
     */
    function depositBTC(VeilLib.Proof memory _args, VeilLib.ExtData memory _extData) public onlyValidator {
        // Only allow deposits
        ...
        uint256 btcBalanceBefore = btc.balanceOf(address(this));

        /// Transfer BTC from validator to pool
        bool success = btc.transferFrom(msg.sender, address(this), depositAmount);
        if (!success) revert BTCTransferFromFailed();

        /// Verify BTC was actually transferred
        uint256 btcBalanceAfter = btc.balanceOf(address(this));
        if (btcBalanceAfter != btcBalanceBefore + depositAmount) revert BTCTransferFromFailed();

        _transact(_args, _extData);
    }

    /**
     * @dev Function for BTC withdrawals only.
     */
    function withdrawBTC(VeilLib.Proof memory _args, VeilLib.ExtData memory _extData) public {
        // Only allow withdrawals
        if (_extData.extAmount >= 0) revert VeilLib.OnlyForWithdrawals();

        _transact(_args, _extData);
        emit BTCWithdrawal(_extData.recipient, uint256(-_extData.extAmount));
    }

    /**
     * @dev Main function that allows transfers only + UTXO updates
     */
    function transactBTC(VeilLib.Proof memory _args, VeilLib.ExtData memory _extData) public {
    ...
    function _transact(VeilLib.Proof memory _args, VeilLib.ExtData memory _extData) internal nonReentrant {
        ...
        if (_extData.extAmount < 0) {
            ...
            bool success = btc.transfer(_extData.recipient, withdrawAmount);
            if (!success) revert BTCTransferFailed();
        }
