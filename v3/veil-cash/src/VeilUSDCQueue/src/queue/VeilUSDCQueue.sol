...
import {WaveTracker} from "src/queue/WaveTracker.sol";

/**
 * @title VeilUSDCQueue v1.0.0
 * @author Veil Cash
 * @dev Queue contract for USDC deposits with screening support
 */
contract VeilUSDCQueue is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    /// @notice Events
    ...
    /**
     * @dev Initializes the VeilUSDCQueue contract with core parameters
    ...
    /**
     * @dev Queues a USDC deposit for screening and processing
        ...
        onlyVeilEntry
    {
        ...
        // Transfer USDC from VeilEntry to this contract
    ...
    /**
     * @dev Accepts a specific deposit by nonce and forwards USDC to VeilEntry
        ...
        returns (uint256 shieldAmount, uint256 fee)
    {
        ...
        // Transfer USDC to VeilEntry
        bool transferSuccess = usdc.transfer(veilEntry, deposit.amountIn);
        if (!transferSuccess) revert VeilLib.USDCTransferFailed();
    ...
    /**
     * @dev Rejects a specific deposit by nonce and refunds USDC to the owner
    ...
    function rejectDeposit(uint256 _nonce) external onlyVeilEntry nonReentrant {
        ...
        // Refund USDC to fallback receiver
        bool refundSuccess = usdc.transfer(deposit.fallbackReceiver, deposit.amountIn);
        if (!refundSuccess) revert VeilLib.USDCTransferFailed();
     ...
     * @param _minimumDeposit New minimum deposit amount in USDC units
     */
