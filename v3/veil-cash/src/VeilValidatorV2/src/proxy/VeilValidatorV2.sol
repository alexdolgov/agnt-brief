...
/// @custom:oz-upgrades-from VeilValidator
contract VeilValidatorV2 is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    /// @dev Errors
    ...
    function deposit0005ETH(bytes32 commitment) external payable nonReentrant {
        ...
        if (!_hasVeil(msg.sender)) revert InsufficientVeilTokens();
    ...
    function _hasVeil(address _depositor) internal view returns (bool) {
        if (address(veilToken) == address(0)) return true;
        return veilToken.balanceOf(_depositor) >= veilAmount;
    }
    ...
    /// @dev Sets the amount of VEIL tokens required for deposits
    function setVeilAmount(uint256 _veilAmount) external onlyOwner {
        veilAmount = _veilAmount;
        emit VeilTokenAmountSet(address(veilToken), _veilAmount);
    }
