...
/// @custom:oz-upgrades-from VeilValidatorV5
contract VeilValidatorV6 is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    /// @dev Errors
    ...
    function deposit200USDC(bytes32 commitment) external payable nonReentrant {
        ...
        if (!isAllowedDepositor(msg.sender)) revert NotAllowedToDeposit();
        if (!_hasVeil(msg.sender, 5)) revert InsufficientVeilTokens();
    ...
    /// @dev Sets the address of the rewards tracker contract
    function setUSDCPool(address _usdcToken, address _veil200USDC) external onlyOwner {
        usdcToken = IERC20(_usdcToken);
        veil200USDC = IVeilUSDCPool(_veil200USDC);
    }
}
