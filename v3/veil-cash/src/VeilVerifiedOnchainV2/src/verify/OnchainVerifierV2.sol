...
/// @custom:oz-upgrades-from VeilVerifiedOnchain
contract VeilVerifiedOnchainV2 is Initializable, OwnableUpgradeable {
    /// @notice Events
    ...
    event EtoOSDepositorSet(address indexed _address, string _ethOSId, bool _allowed);
    ...
    /// @dev EtoOS approvals
    mapping(address => VerifyLib.EtoOSInfo) public etoOSDepositors;
    ...
    /// @param _address Address to check verification status
    /// @return bool True if address is verified on EtoOS, false otherwise
    function _isEtoOSVerified(address _address) internal view returns (bool) {
        return etoOSDepositors[_address].isAllowed && etoOSDepositors[_address].expirationTime > block.timestamp;
    }
    ...
    /// @param _address Address of the depositor
    /// @return uint256 The expiration time
    function getEtoOSExpirationTime(address _address) external view returns (uint256) {
        return etoOSDepositors[_address].expirationTime;
    }
    ...
    function getVerificationType(address _address) external view returns (uint256) {
        ...
        if (_isEtoOSVerified(_address)) return 4;
        return 0;
    }
    ...
    /// @param _allowed Whether to allow or disallow the depositor
    function setEtoOSDepositor(address _address, string calldata _ethOSId, bool _allowed) external onlyOperator {
        uint256 expirationTime = block.timestamp + 3 days;
        etoOSDepositors[_address] =
            VerifyLib.EtoOSInfo({isAllowed: _allowed, expirationTime: expirationTime, ethOSId: _ethOSId});

        emit EtoOSDepositorSet(_address, _ethOSId, _allowed);
    }
}
