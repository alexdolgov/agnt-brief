...
/**
 * @title VeilWalletEntry v10.0.0
 * @author Veil Cash
 * @dev Veil Wallet - Entry point for depositing ETH into the Veil pools with seperate queue contract
 */
/// @custom:oz-upgrades-from VeilWalletEntryV9
contract VeilWalletEntryV10 is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    /// @custom:oz-renamed-from veilLimits
    address private __deprecated_veilLimits;

    /// @custom:oz-renamed-from allowedDepositors
    mapping(address => VeilLib.DepositorInfo) private __deprecated_allowedDepositors;
    ...
    /// @custom:oz-renamed-from veilBTCPool
    address private __deprecated_veilBTCPool;
    /// @custom:oz-renamed-from depositBTCEnabled
    bool private __deprecated_depositBTCEnabled;
    /// @custom:oz-renamed-from minimumDepositBTC
    uint256 private __deprecated_minimumDepositBTC;
    /// @custom:oz-renamed-from totalBTCDeposits
    uint256 private __deprecated_totalBTCDeposits;
    /// @custom:oz-renamed-from btc
    address private __deprecated_btc;
    /// @custom:oz-renamed-from btcQueueContract
    address private __deprecated_btcQueueContract;

    /// @dev Forwarder feature flags
    bool public forwarderEnabled;
    ...
    function acceptUSDC(uint256 _nonce, VeilLib.Proof memory _args, VeilLib.ExtData memory _extData)
        ...
        nonReentrant
    {
        if (_extData.extAmount <= 0) revert VeilLib.InvalidExtAmount();
        ...
        if (uint256(_extData.extAmount) != shieldAmount) revert VeilLib.ProofAmountMismatch();
    ...
    // ============================================
    // Direct ETH and USDC Deposit Functions
    // ============================================
     ...
     *      Used by forwarders where msg.sender must be the fallback receiver.
     * @param _depositKey Deposit key used for building zero-knowledge proof
     */
    function queueEthFor(bytes calldata _depositKey) external payable nonReentrant {
        if (!forwarderEnabled) revert VeilLib.ForwarderDisabled();
        if (_depositKey.length != 64) revert VeilLib.InvalidDepositKey();
    ...
    function queueUSDCFor(uint256 _amount, bytes calldata _depositKey) external nonReentrant {
        ...
        bool approveSuccess = usdc.approve(address(usdcQueueContract), _amount);
        if (!approveSuccess) revert VeilLib.USDCTransferFailed();
    ...
    /**
     * @dev Enables or disables the forwarder feature
     */
    function setForwarderEnabled(bool _forwarderEnabled) external onlyOwner {
        forwarderEnabled = _forwarderEnabled;
    }
