...
/**
 * @title VeilWalletEntry v3.0.0
 * @author Veil Cash
 * @dev Veil Wallet - Entry point for depositing ETH into the Veil pools
 */
/// @custom:oz-upgrades-from VeilWalletEntryV2
contract VeilWalletEntryV3 is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    /// @dev OnlyQueueContract modifier
    modifier onlyQueueContract() {
        if (msg.sender != address(queueContract)) revert OnlyQueueContractAllowed();
        _;
    }
    ...
    event QueueContractUpdated(address indexed newQueueContract);
    ...
    /// @dev Process next ETH deposit from queue (FIFO order, atomic operation)
    function processETH(VeilLib.Proof memory _args, VeilLib.ExtData memory _extData) external nonReentrant {
        if (msg.sender != operator) revert OnlyOperatorAllowed();

        /// @dev Process next deposit in queue (marks as completed atomically)
        (, uint256 shieldAmount, uint256 fee) = queueContract.processNextDeposit();

        /// @dev Check rate limits -- save as the queue contract is the depositor
        _checkLimits(address(queueContract), shieldAmount);
        ...
        /// @dev Emit the deposit event (using processed index for tracking)
        emit DepositedETH(tx.origin, shieldAmount);
    }
    /**
    ...
    /**
     * @dev Function for setting the deposit queue contract address
     */
    function setQueueContract(address _queueContract) external onlyOwner {
        queueContract = IVeilDepositQueue(_queueContract);
        emit QueueContractUpdated(_queueContract);
    }

    /**
     * @dev Function for setting the operator
     */
