...
contract SwapFacet is VaultStorage, IFacet {
    ...
    function initializeFacet() external {
        _setFunction(SwapFacet.notifyInitialSupply.selector, thisImplementation);
        _setFunction(SwapFacet.attachBribe.selector, thisImplementation);
        _setFunction(SwapFacet.killBribe.selector, thisImplementation);
        _setFunction(SwapFacet.killGauge.selector, thisImplementation);
        _setFunction(SwapFacet.emissionToken.selector, thisImplementation);
        ...
        internal
    {
        ...
        for (uint256 i = 0; i < ops.length; i++) {
            ...
            if (opType == 0) {
                ...
                int128[] memory deltas = new int128[](opTokenLength);
                for (uint256 j = 0; j < opTokenLength; j++) {
                    int128 delta =
                        (opTokens.u(j).balanceOf(address(this)).toInt256() - balances.u(j).toInt256()).toInt128();
                    require(-delta <= int128(uint128(uint256(opTokenInformations.u(j)))));

                    deltas.u(j, -delta);
                    cumDelta[uint8(uint256(opTokenInformations.u(j) >> (256 - 8)))] += delta;
                }
