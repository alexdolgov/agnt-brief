...
contract BridgeMiddleware is ReentrancyGuard, AccessControlEnumerable {
    ...
    function approveBaseToken(uint256 _amount) external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        address nativeTokenVault = IL1AssetRouter(bridgehub.assetRouter()).nativeTokenVault();

        if (_amount != 0 && baseToken.allowance(address(this), nativeTokenVault) > 0) {
            revert AllowanceNotZero();
        }

        baseToken.approve(nativeTokenVault, _amount);
    }
