...
contract AccessHub is IAccessHub, Initializable, AccessControlEnumerableUpgradeable {
    ...
    /// @inheritdoc IAccessHub
    function setFeeCollectorInFactoryV3(address _newFeeCollector) public onlyRole(PROTOCOL_OPERATOR) {
    ...
    function lpBribeOldGauge(address _pool, address _token, uint256 _amount) external onlyRole(PROTOCOL_OPERATOR) {
        IFeeCollector newFeeCollector = IFeeCollector(0x949E7B2F3F66EAe761a9397346dDbd719F046c0e);
        newFeeCollector.collectProtocolFees(_pool);
        setFeeCollectorInFactoryV3(0xcc0365F8f453C55EA7471C9F89767928c8f8d27F);
        IERC20(_token).approve(voter.gaugeForPool(_pool), _amount);
        IGaugeV3(voter.gaugeForPool(_pool)).notifyRewardAmount(_token, _amount);
        setFeeCollectorInFactoryV3(address(newFeeCollector));
    }
}
