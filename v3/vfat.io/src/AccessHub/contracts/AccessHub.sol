...
contract AccessHub is IAccessHub, Initializable, AccessControlEnumerableUpgradeable {
    ...
    /// @inheritdoc IAccessHub
    function createCLGaugeOveridden(address tokenA, address tokenB, int24 tickSpacing)
        external
        onlyRole(PROTOCOL_OPERATOR)
    {
    ...
    /// @inheritdoc IAccessHub
    function resetGauge(address _user, address _pool, int24 _tickspaceTo) external timelocked {
        address t0;
        address t1;
        int24 ts;
        (t0, t1, ts) =
            (IRamsesV3Pool(_pool).token0(), IRamsesV3Pool(_pool).token1(), IRamsesV3Pool(_pool).tickSpacing());

        voter.setMainTickSpacing(t0, t1, _tickspaceTo);
        voter.reset(_user);
        voter.setMainTickSpacing(t0, t1, ts);
    }
    /**
     * xShadow Functions
     */
