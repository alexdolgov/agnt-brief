...
interface IFarmingCenter {
    ...
    function virtualPoolAddresses(
        address poolAddress
    ) external view returns (address virtualPoolAddresses);
    ...
    /// @param tokenId The ID of the deposit (and token) that is being
    /// transferred
    /// @return eternalIncentiveId The id of eternal incentive that is active
    /// for this NFT
    function deposits(
        uint256 tokenId
    ) external view returns (bytes32 eternalIncentiveId);
    ...
    /// @param virtualPool The virtual pool to be connected, must not be zero
    /// address
    /// @param plugin The Algebra farming plugin
    function connectVirtualPoolToPlugin(
        address virtualPool,
        address plugin
    ) external;
    ...
    /// @dev only farming can do it.
    /// If the specified virtual pool is not connected to the plugin, nothing
    /// will happen
    /// @param virtualPool The virtual pool to be disconnected, must not be zero
    /// address
    /// @param plugin The Algebra farming plugin
    function disconnectVirtualPoolFromPlugin(
        address virtualPool,
        address plugin
    ) external;
    ...
    /// @notice Used to collect reward from eternal farming. Then reward can be
    /// claimed.
    /// @param key The incentive key
    /// @param tokenId The id of position NFT
