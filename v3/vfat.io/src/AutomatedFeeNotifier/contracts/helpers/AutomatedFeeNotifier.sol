...
contract AutomatedFeeNotifier {
    ...
    IMarbleMinter private immutable MARBLE_MINTER;
    ...
    constructor(address _voter, address _collector, address _legacyFactory, address _v3Factory, address _marbleMinter) {
        ...
        MARBLE_MINTER = IMarbleMinter(_marbleMinter);
    }
    ...
    function notify(IShadowV3Pool[] calldata _pools) external {
        for (uint256 i; i < _pools.length; ++i) {
            ...
            if (v == 0) revert("v = 0");
            if (v == 3) {
                COLLECTOR.collectProtocolFees(address(_pools[i]));
            } else {
                IMockPool(poolAddress).mintFee();
                address[] memory singlePool = new address[](1);
                singlePool[0] = poolAddress;
                MARBLE_MINTER.redirectFeesForPairs(singlePool);
            }
        }
    }
    ...
    function blindPush(uint256 _index, uint256 _end) external {
        ...
        for (; _index < _end; ++_index) {
            address pool = targets[_index];
            if (!LEGACY_FACTORY.isPair(pool)) {
                COLLECTOR.collectProtocolFees(address(IShadowV3Pool(pool)));
            } else {
                IMockPool(pool).mintFee();
