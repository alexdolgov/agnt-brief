...
contract IchiConnector is ILiquidityConnector {
    ...
    function addLiquidity(
        ...
        bool allowToken0 = IICHIVault(addLiquidityParams.lpToken).allowToken0();
        bool allowToken1 = IICHIVault(addLiquidityParams.lpToken).allowToken1();

        if (allowToken0 && !allowToken1) {
            IICHIVault(addLiquidityParams.lpToken).deposit(
                addLiquidityParams.desiredAmounts[0], 0, address(this)
            );
        } else if (!allowToken0 && allowToken1) {
            IICHIVault(addLiquidityParams.lpToken).deposit(
