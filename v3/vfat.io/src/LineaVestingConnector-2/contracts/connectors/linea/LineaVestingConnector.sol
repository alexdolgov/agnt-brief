...
struct LineaVestingExtraData {
    ...
    address secondVestingContract; // Optional second vesting contract to claim from
}
...
contract LineaVestingConnector is INftFarmConnector {
    ...
    /// @notice Claims vested tokens from Linea vesting contract(s) and optionally Etherex
    /// gauge rewards
    /// @param position The NFT position containing the primary vesting contract address
    /// @param extraData Encoded LineaVestingExtraData with hasGauge flag, rewardTokens,
    /// and optional secondVestingContract
    /// @dev Calls release() on vesting contract(s) if user has releasable amount,
    ...
    function claim(
        ...
        // Claim LINEA from primary vesting contract if user has releasable amount
        ...
        // Claim LINEA from second vesting contract if specified and user has releasable amount
        if (extra.secondVestingContract != address(0)) {
            ILineaVesting secondVesting = ILineaVesting(extra.secondVestingContract);
            if (secondVesting.releasable(address(this)) > 0) {
                secondVesting.release();
            }
        }
