...
contract MigrationStrategy {
    ...
    function migrate(
        ...
        IOldSickle oldSsickle = IOldSickle(payable(oldSickleAddress));
        if (oldFactory.admins(oldSickleAddress) != msg.sender) {
            revert NotOwner(msg.sender);
        }
        Sickle newSickle =
            Sickle(payable(newFactory.getOrDeploy(msg.sender, referralCode)));
        ...
        oldSsickle.multicall(targets, data, isDelegateCall, values);
    ...
    function migrateMultiple(
        ...
        for (uint256 i = 0; i < migrationInfos.length;) {
            migrate(oldSickleAddress, migrationInfos[i], referralCode);
