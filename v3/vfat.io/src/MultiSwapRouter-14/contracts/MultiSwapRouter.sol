...
contract MultiSwapRouter {
    ...
    function _executeStep(
        ...
        } else if (step.dexType == DexType.Slipstream) {
            return _swapSlipstream(step, amountIn, useETH);
        }
    ...
    /// @dev Slipstream (Aerodrome CL) interface - uses tickSpacing instead of fee
    function _swapSlipstream(
        ...
        (bool success, bytes memory result) = step.router.call{ value: useETH ? amountIn : 0 }(
            abi.encodeWithSelector(
                0xa026383e, // exactInputSingle (Slipstream)
                step.tokenIn,
                step.tokenOut,
                tickSpacing,
                address(this),
