...
contract MultiSwapRouter {
    ...
    function swapSplit(
        ...
        // Transfer total input tokens from caller
        if (msg.value == 0) {
            SafeTransferLib.safeTransferFrom(tokenIn, msg.sender, address(this), totalAmountIn);
        }
        ...
        amountOut = _applyFeeAndTransfer(tokenOut, totalOutput, minAmountOut, recipient);

        emit MultiSwap(msg.sender, tokenIn, tokenOut, totalAmountIn, amountOut);
    }
    ...
    function _executeRoutes(
        ...
        for (uint256 r = 0; r < routes.length; r++) {
            totalOutput += _executeSingleRoute(routes[r], amounts[r], r == 0 && msg.value > 0);
        }
    }
    ...
    function _executeSingleRoute(
        ...
        currentAmount = useETH ? msg.value : amountIn;
        for (uint256 s = 0; s < steps.length; s++) {
            currentAmount = _executeStep(steps[s], currentAmount, useETH && s == 0);
        }
    }
    ...
    function _applyFeeAndTransfer(
        ...
        uint256 fee = (totalOutput * feeBps) / BPS_DENOMINATOR;
    ...
    function _swapAlgebra(
        ...
        if (step.dexData.length >= 32) {
            // Algebra Integral V1.1+ params: tokenIn, tokenOut, deployer, recipient, deadline, amountIn, amountOutMin, limitSqrtPrice
