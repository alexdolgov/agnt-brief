...
contract MultiSwapRouter is ReentrancyGuard {
    ...
    /// @dev ETH balance before a bridge call, used to refund leftover ETH
    ...
    error BridgeAmountExceedsAvailable(uint256 bridgeAmount, uint256 available);
    ...
    /// @param amountIn Amount of input token
    /// @param minSwapOut Minimum output amount from the swap portion
    /// @param bridgeContract Bridge protocol contract (validated against allowlist)
    /// @param bridgeCalldata Pre-encoded calldata for the bridge call
    /// @param bridgeAmount Exact amount the bridge expects to pull
    ...
    /// @dev Simpler entry point for users who already hold the bridgeable token.
    /// @param bridgeContract Bridge protocol contract (validated against allowlist)
    /// @param bridgeCalldata Pre-encoded calldata for the bridge call
    /// @param bridgeAmount Exact amount the bridge expects to pull
        ...
        )
    {
        ...
        if (steps.length > 0) {
            ...
            if (currentAmount < minSwapOut) {
                revert InsufficientOutputAmount(currentAmount, minSwapOut);
            }
        } else {
    ...
    function _executeBridgeAndRefund(
        ...
        // Validate bridge contract against allowlist
        allowlist.requireAllowed(bridgeContract);

        // Cap bridgeAmount to what this call actually deposited (post-fee)
        uint256 available =
            IERC20(bridgeToken).balanceOf(address(this))
                - initialBridgeTokenBalance;
        if (bridgeAmount > available) {
            revert BridgeAmountExceedsAvailable(bridgeAmount, available);
        }

        // Approve bridge contract for exactly bridgeAmount
        _approve(bridgeToken, bridgeContract, bridgeAmount);

        // Record balance before bridge call for consumption check
        uint256 balanceBeforeBridge =
            IERC20(bridgeToken).balanceOf(address(this));

        // Call bridge contract with pre-encoded calldata
        (bool success, bytes memory result) =
            bridgeContract.call{ value: bridgeNativeValue }(bridgeCalldata);
        if (!success) revert BridgeFailed(result);

        uint256 balanceAfterBridge =
            IERC20(bridgeToken).balanceOf(address(this));

        // Verify bridge consumed at least bridgeAmount
        uint256 consumed = balanceBeforeBridge - balanceAfterBridge;
        if (consumed < bridgeAmount) {
        ...
        if (ethBalance > _preCallEthBalance) {
            SafeTransferLib.safeTransferETH(
                recipient, ethBalance - _preCallEthBalance
            );
        }
    }
