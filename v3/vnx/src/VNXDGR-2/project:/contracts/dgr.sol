...
contract VNXDGR is Initializable, OwnableUpgradeable, ERC20PausableUpgradeable, ERC20PermitUpgradeable {
    ...
    event RBACAddressUpdated(address indexed newRbacAddress);
    ...
    */
    function transfer(address _to, uint256 _value) public whenNotPaused override returns (bool) {
        require(_to != _msgSender(), "Useless");
        require(!isFrozen(_to) && !isFrozen(_msgSender()), "Frozen");
        require(_value > 0, "VAL>0!");
        
        uint256 _fee = getFeeFor(_value);
        uint256 _cleanvalue = _value;
        bool  feeReqd = false;

        //  _fee shouldn't be < 0 or >= _value by design, but protect ourselves just in case
        if (_fee > 0 && _fee < _value) {
            _cleanvalue = _value - _fee;
            feeReqd = true;
        }

        //  first transfer _cleanvalue
        super.transfer(_to, _cleanvalue);

        //  next transfer _fee if required
        if (feeReqd) {
    ...
    function transferFrom(address _from, address _to, uint256 _value) public whenNotPaused override returns (bool) {
        ...
        require(!isFrozen(_to) && !isFrozen(_from) && !isFrozen(_msgSender()), "Frozen");
        require(_value > 0, "VAL>0!");
        ...
        //  first transferFrom _cleanvalue
        super.transferFrom(_from, _to, _cleanvalue);

        //  next transferFrom _fee if required
        if (feeReqd) {
            super.transferFrom(_from, feeRecipient, _fee);
    ...
    function approve(address _spender, uint256 _value) public whenNotPaused override returns (bool) {
        require(!isFrozen(_spender) && !isFrozen(_msgSender()), "Frozen");
    ...
    function unfreeze(address _addr) external onlyAssetProtectionRole {
        require(isFrozen(_addr), "Unfrozen");
    ...
    /**
    * @dev Reclaim the DGRs from a specific frozen user address and send them to the wallet of the sender (asset protection address).
    * @param _addr The frozen address to reclaim the tokens from.
    */
    function reclaimTokensFromFrozenAddress(address _addr) external onlyAssetProtectionRole {
        require(isFrozen(_addr), "!Frozen");
   ...
   */
    ...
    */
    function getFeeFor(uint256 _value) public view returns (uint256) {
        //  do not take fee if either feeRate=0 or there's nowhere to send it
    ...
    */
    ...
    /**
     * @dev Update the RBAC address
     * @param _newRbacAddress the new RBAC address
     */
    function updateRBACAddress(address _newRbacAddress) public onlyOwner
    {
        rbacManager = IRBAC(_newRbacAddress);
        emit RBACAddressUpdated(_newRbacAddress);
    }
}
