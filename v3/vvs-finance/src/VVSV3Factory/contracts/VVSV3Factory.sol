...
/// @title Canonical VVS V3 factory
/// @notice Deploys VVS V3 pools and manages ownership and control over pool protocol fees
contract VVSV3Factory is IVVSV3Factory {
    /// @inheritdoc IVVSV3Factory
    address public override owner;
    /// the v3 pool deployer
    address public immutable poolDeployer;
    /// @inheritdoc IVVSV3Factory
    mapping(uint24 => int24) public override feeAmountTickSpacing;
    /// @inheritdoc IVVSV3Factory
    mapping(address => mapping(address => mapping(uint24 => address))) public override getPool;
    /// the liquidity mining deployer
    ...
    function createPool(
        ...
        pool = IVVSV3PoolDeployer(poolDeployer).deploy(address(this), token0, token1, fee, tickSpacing);
    ...
    function setLmPool(address pool, address lmPool) external override onlyOwnerOrLmPoolDeployer {
        IVVSV3Pool(pool).setLmPool(lmPool);
    }
}
