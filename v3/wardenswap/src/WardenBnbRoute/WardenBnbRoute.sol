...
contract WardenBnbRoute is IWardenTradingRoute, WhitelistedRole, ReentrancyGuard {
        ...
        returns(uint256 _destAmount)
    {
        ...
        if (_src == etherERC20 && msg.value > 0) { // ETH => TOKEN
            ...
            uint256[] memory amounts = router.swapExactBNBForTokens.value(msg.value)(
                amountOutMin,
                path,
                msg.sender,
                deadline
            ...
            path[0] = address(_src);
            path[1] = address(wETH);
            uint256[] memory amounts = router.swapExactTokensForBNB(
                _srcAmount,
                amountOutMin,
                path,
                msg.sender,
