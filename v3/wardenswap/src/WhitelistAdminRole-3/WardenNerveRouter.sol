...
contract WardenNerveRouter is IWardenTradingRoute, WhitelistedRole, ReentrancyGuard {
    using SafeERC20 for IERC20;

    INerveSwap public constant nerve = INerveSwap(0x1B3771a66ee31180906972580adE9b81AFc5fCDc);
    ...
    IERC20 public constant usdc = IERC20(0x8AC76a51cc950d9822D68b83fE1Ad97B32Cd580d); // 2
        ...
        returns(uint256 _destAmount)
    {
        ...
        _src.safeApprove(address(nerve), _srcAmount);
        _destAmount = nerve.swap(
            uint8(i),
            uint8(j),
            _srcAmount,
            0,
        ...
        returns(uint256 _destAmount)
    {
        ...
        return nerve.calculateSwap(uint8(i), uint8(j), _srcAmount);
    }
    ...
    {
        require(_src != _dest, "WardenNerveRouter: Destination token can not be source token");
        i = -1;
        j = -1;
        i = _src == busd ? 0 : i;
        i = _src == usdt ? 1 : i;
        ...
        j = _dest == usdt ? 1 : j;
        j = _dest == usdc ? 2 : j;
        require(i != -1 && j != -1, "WardenNerveRouter: Tokens're not supported!");
    }
}