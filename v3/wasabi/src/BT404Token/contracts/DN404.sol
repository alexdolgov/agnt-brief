...
abstract contract DN404 {
    ...
    function _setMarketNFTFeeRate(uint256 feeBips) internal virtual {
        if (feeBips > 10000) revert();
        _getDN404Storage().marketFeeBips = uint16(feeBips);
    }
    ...
    function _calldatacopyOrders(uint256 offset) private pure returns (NFTOrder[] memory orders) {
        ...
        for (uint256 i; i < length;) {
            ...
            // @solidity memory-safe-assembly
            assembly {
                calldatacopy(tmp, offset, 0x40) // Copy array element
