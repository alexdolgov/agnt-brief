    ...
    IWasabiVault, UUPSUpgradeable, OwnableUpgradeable, ERC4626Upgradeable, ReentrancyGuardUpgradeable 
{
    ...
    ///      Slot retained for storage layout compatibility on UUPS upgrade.
    ///      Read live values via `longPool()` / `shortPool()` (which delegate to the manager)
    ///      or `_getPools()` internally.
    /// @custom:oz-renamed-from longPool
    IWasabiPerps public _deprecated_longPool;
    /// @dev DEPRECATED 2026-05-01: see _deprecated_longPool above.
    ...
    function __WasabiVault_init(
        ...
        // _longPool / _shortPool params retained for ABI compatibility with existing
        // deploy scripts. As of 2026-05-01 pool addresses are read live from the manager
        // (IAddressProvider.longPool() / shortPool()), so we no longer write them locally.
        // The manager must have the pool addresses populated before this vault is used —
        // see PerpManager.setLongPool / setShortPool.
    ...
    ///      Pool addresses are sourced live from the manager (IAddressProvider) — a single
    ///      call to PerpManager.setLongPool / setShortPool propagates to every consumer
    ...
    function getPoolAddress(bool _long) external view returns (address) {
        (IWasabiPerps _longPool, IWasabiPerps _shortPool) = _getPools();
        return _long ? address(_longPool) : address(_shortPool);
    }

    /// @notice Returns the live long pool address from the manager.
    ///         Backwards-compatible shim for callers that previously read `vault.longPool()`
    ///         from the now-deprecated local storage.
    ...
    ///         Backwards-compatible shim for callers that previously read `vault.shortPool()`.
    ...
    ///      Two staticcalls — same cost profile as `_getWethAddress()` / `_getFeeReceiver()`.
    ///      Use this on hot paths (`onlyPool`, `getPoolAddress`) to avoid duplicating the
    ///      manager fetch.
