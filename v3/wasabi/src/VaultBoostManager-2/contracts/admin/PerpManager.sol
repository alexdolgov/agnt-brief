...
contract PerpManager is UUPSUpgradeable, AccessManagerUpgradeable, IPerpManager, IAddressProvider, IDebtController {
    ...
    // IAddressProvider state — pool addresses (added 2026-05-01 for centralization).
    // Consumers (vaults, router, partner fee manager, vault boost manager, etc.) read
    // these via _getManager().longPool() / shortPool() instead of holding their own copy.
    // Updated atomically on every consumer by calling setLongPool / setShortPool.
    /// @inheritdoc IAddressProvider
    ...
    function deployVault(address implementation, bytes calldata data) external onlyVaultAdmin returns (address) {
        ...
        // Read shortPool directly from local state instead of routing through wasabiRouter.
        // Post-centralization, wasabiRouter.shortPool() ultimately reads from this same field
        // anyway via _getManager().shortPool(); the indirect call would be a self-staticcall.
        shortPool.addVault(IWasabiVault(vault));
        return vault;
    }
    ...
    /// @inheritdoc IAddressProvider
    /// @notice Pool repointing has high blast radius — any consumer reading pool
    ///         addresses through this manager will see the new value on its next
    ///         call. Operationally, `setLongPool` should be gated by a timelock
    ///         (OZ AccessManager `restricted` modifier or a `TimelockController`
    ///         in front of the admin) so on-chain monitoring has time to react.
    function setLongPool(IWasabiPerps _longPool) external onlyAdmin {
        if (address(_longPool) == address(0)) revert InvalidAddress();
        longPool = _longPool;
    }

    /// @inheritdoc IAddressProvider
    /// @notice See setLongPool — same blast radius and timelock recommendation.
    function setShortPool(IWasabiPerps _shortPool) external onlyAdmin {
        if (address(_shortPool) == address(0)) revert InvalidAddress();
