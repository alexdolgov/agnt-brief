    ...
    OwnableUpgradeable
{
    ...
    function __WasabiRouter_init(
        ...
        // see _getPools() and the longPool()/shortPool() getter shims.
        weth = _weth;
    ...
    function _openPositionInternal(
        ...
        if (_pool != _longPool) {
            ...
            if (_pool != _shortPool) revert InvalidPool();
        }

        // Currency to withdraw from vault for payment - always the quote currency
        address currency = _pool == _longPool
            ? _request.currency
            : _request.targetCurrency;
    ...
    function _withdrawFromVault(
        ...
        IWasabiVault vault = _getManager().shortPool().getVault(_asset);
    ...
    /// @dev Returns both pool addresses from the manager in one helper. Used on hot
    ///      paths (`_openPositionInternal`, `_addCollateralInternal`) to share the cost
    ///      of two staticcalls between both reads.
    ...
    ///         Backwards-compatible shim for callers that previously read `router.longPool()`
