...
// File contracts/interfaces/alpaca/IbBNBToken.sol

pragma solidity 0.6.12;

interface IbBNBToken is IERC20 {
...
// File contracts/interfaces/alpaca/IAlpacaPool.sol
...
// File contracts/adapters/AlpacaBNBVaultAdapter.sol
...
/// @title AlpacaBNBVaultAdapter
///
/// @dev A vault adapter implementation which wraps a alpaca vault.
contract AlpacaBNBVaultAdapter is IVaultAdapterV2 {
  using FixedPointMath for FixedPointMath.uq192x64;
  using SafeERC20 for IDetailedERC20;
  using SafeMath for uint256;

  /// @dev The vault that the adapter is wrapping.
  IbBNBToken public vault;
  ...
  constructor(IbBNBToken _vault, address _admin, IUniswapV2Router02 _uniV2Router, IAlpacaPool _stakingPool, IDetailedERC20 _alpacaToken) public {
    require(address(_vault) != address(0), "AlpacaBNBVaultAdapter: vault address cannot be 0x0.");
    require(_admin != address(0), "AlpacaBNBVaultAdapter: _admin cannot be 0x0.");
  ...
  modifier onlyAdmin() {
    require(admin == msg.sender, "AlpacaBNBVaultAdapter: only admin");
    _;
  }
  ...
  function totalValue() external view override returns (uint256) {
    ...
    (uint256 amount,,,) = stakingPool.userInfo(1,address(this));
  ...
  function deposit(uint256 _amount) external override {
    IWETH wbnb = IWETH(address(vault.token()));
    wbnb.withdraw(_amount);
    vault.deposit{value:_amount}(_amount);
    // stake to pool
    stakingPool.deposit(address(this),1,vault.balanceOf(address(this)));
  ...
  function withdraw(address _recipient, uint256 _amount, bool _isHarvest) external override onlyAdmin {
    ...
    IWETH wbnbToken = IWETH(vault.token());
    // unstake
    stakingPool.withdraw(address(this),1,_tokensToShares(_amount));
    // withdraw
    vault.withdraw(_tokensToShares(_amount));

    wbnbToken.deposit{value:_amount}();

    // sell alpaca if is called from harvest
    if(_isHarvest){

      // withdraw accumulated ibbnb from collector harvest
      if(vault.balanceOf(address(this)) > 0){
        vault.withdraw(vault.balanceOf(address(this)));
      }

      stakingPool.harvest(1);

      address[] memory _pathAlpaca = new address[](2);
    ...
    wbnbToken.transfer(_recipient,wbnbToken.balanceOf(address(this)));
  }
...
// File contracts/adapters/AlpacaBNBVaultAdapterWithIndirection.sol

pragma solidity ^0.6.12;

/// @title AlpacaBNBVaultAdapterWithIndirection
///
/// @dev A vault adapter implementation which wraps a vesper vault.
contract AlpacaBNBVaultAdapterWithIndirection is AlpacaBNBVaultAdapter {
  using SafeERC20 for IDetailedERC20;
  using SafeMath for uint256;

  constructor(IbBNBToken _vault, address _admin, IUniswapV2Router02 _uniV2Router, IAlpacaPool _stakingPool, IDetailedERC20 _alpacaToken)
              AlpacaBNBVaultAdapter(_vault,_admin,_uniV2Router,_stakingPool,_alpacaToken) public {
  }
