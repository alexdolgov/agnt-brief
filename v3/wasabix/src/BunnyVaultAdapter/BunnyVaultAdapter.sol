...
// File contracts/interfaces/bunny/IBunnyVault.sol
...
// File contracts/libraries/pancake/IPancakeRouter02.sol
...
// File contracts/adapters/BunnyVaultAdapter.sol
...
///
/// @dev A vault adapter implementation which wraps a bunny vault.
contract BunnyVaultAdapter is IVaultAdapterV2 {
  using FixedPointMath for FixedPointMath.uq192x64;
  using SafeERC20 for IDetailedERC20;
  using SafeMath for uint256;

  /// @dev The vault that the adapter is wrapping.
  IBunnyVault public vault;
  ...
  constructor(IBunnyVault _vault, address _admin, IPancakeRouter02 _pancakeRouter, IDetailedERC20 _bunnyToken, IDetailedERC20 _wBNBToken) public {
    vault = _vault;
    admin = _admin;
  ...
  modifier onlyAdmin() {
    require(admin == msg.sender, "BunnyVaultAdapter: only admin");
    _;
  }
  ...
  function token() external view override returns (IDetailedERC20) {
    return IDetailedERC20(vault.rewardsToken());
  }
  ...
  function totalValue() external view override returns (uint256) {
    return vault.principalOf(address(this));
  }
  ...
  function withdraw(address _recipient, uint256 _amount, bool _isHarvest) external override onlyAdmin {
    ...
    IDetailedERC20 busdToken = IDetailedERC20(vault.rewardsToken());

    if(_isHarvest){

      //get back earned busd and bunny from vault
      vault.getReward();

      address[] memory _pathBunny = new address[](3);
      _pathBunny[0] = address(bunnyToken);
      _pathBunny[1] = address(wBNBToken);
      _pathBunny[2] = address(busdToken);

      pancakeRouter.swapExactTokensForTokens(bunnyToken.balanceOf(address(this)),
                                           0,
                                           _pathBunny,
                                           address(this),
                                           block.timestamp+800);
    } else {
      // withdraw (will incur 0.5% withdraw fee + 0.01% venus fee when withdraw within 3 days from the last deposit)
      vault.withdrawUnderlying(_amount);
    }
  ...
  function updateApproval() public {
    // busd to vault
    address _token = vault.rewardsToken();
    IDetailedERC20(_token).safeApprove(address(vault), uint256(-1));
    // bunny to swapRouter
    bunnyToken.safeApprove(address(pancakeRouter), uint256(-1));
  }
}