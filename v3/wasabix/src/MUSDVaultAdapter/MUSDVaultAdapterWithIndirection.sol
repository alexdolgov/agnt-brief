...
contract MUSDVaultAdapter is IVaultAdapterV2 {
  ...
  function withdraw(address _recipient, uint256 _amount, bool _isHarvest) external override onlyAdmin {
    ...
    // transfer all the musd in adapter to yum
...
// File contracts/adapters/MUSDVaultAdapterWithIndirection.sol
...
contract MUSDVaultAdapterWithIndirection is MUSDVaultAdapter {
  using SafeERC20 for IDetailedERC20;
  using SafeMath for uint256;

  constructor(IimUSDToken _vault, address _admin, IBalancerVault _balancerVault, IMUSDVault _stakingPool, IDetailedERC20 _mtaToken, IDetailedERC20 _wMaticToken, IDetailedERC20 _usdcToken)
              MUSDVaultAdapter(_vault,_admin,_balancerVault,_stakingPool,_mtaToken,_wMaticToken,_usdcToken) public {
  }
  ...
  function indirectWithdraw(address _recipient, uint256 _amount) external onlyAdmin {
      // unstake imUsd
      stakingPool.withdraw(_tokensToShares(_amount));
      // transfer imUsd to recipient
      vault.transfer(_recipient, _tokensToShares(_amount));
      // claim alpaca token from staking pool
      stakingPool.claimReward();
      // transfer mta to recipient
      mtaToken.transfer(_recipient, mtaToken.balanceOf(address(this)));
      // transfer wMatc to recipient
      wMaticToken.transfer(_recipient, wMaticToken.balanceOf(address(this)));
  }
}