...
// File contracts/libraries/pools/PoolV2.sol
...
/// @title PoolV2
///
/// @dev A library which provides the Pool data struct and associated functions with additional lock data.
library PoolV2 {
  using FixedPointMath for FixedPointMath.uq192x64;
  using PoolV2 for PoolV2.Data;
  using PoolV2 for PoolV2.List;
  using SafeMath for uint256;
  ...
  struct Data {
    ...
    bool needVesting;
    uint256 withdrawFee; // divided by 10000
  ...
  function lastIndex(List storage _self) internal view returns (uint256) {
    ...
    return _length.sub(1, "PoolV2.List: list is empty");
  }
...
// File contracts/libraries/pools/StakeV2.sol
...
/// @title StakeV2
///
/// @dev A library which provides the Stake data struct and associated functions with PoolV2.
library StakeV2 {
  using FixedPointMath for FixedPointMath.uq192x64;
  using PoolV2 for PoolV2.Data;
  using SafeMath for uint256;
  using StakeV2 for StakeV2.Data;
  ...
  function update(Data storage _self, PoolV2.Data storage _pool, PoolV2.Context storage _ctx) internal {
  ...
  function getUpdatedTotalUnclaimed(Data storage _self, PoolV2.Data storage _pool, PoolV2.Context storage _ctx)
    internal view
    returns (uint256)
  {
...
// File contracts/libraries/pools/PowerV2.sol
...
/// @title PowerV2
///
/// @dev A library which provides the Power data struct and associated functions with PoolV2.
library PowerV2 {
  using FixedPointMath for FixedPointMath.uq192x64;
  using PoolV2 for PoolV2.Data;
  using SafeMath for uint256;
  using PowerV2 for PowerV2.Data;
  ...
  function getUpdatedTotalPower(Data storage _self, PoolV2.Data storage _pool, PoolV2.Context storage _ctx)
    internal view
    returns (uint256)
  {
    FixedPointMath.uq192x64 memory _currentAccumulatedWeight = _pool.getUpdatedAccumulatedRewardWeight(_ctx);
...
// File contracts/StakingPoolsV2.sol
...
contract StakingPoolsV2 is ReentrancyGuard {
  using FixedPointMath for FixedPointMath.uq192x64;
  using PoolV2 for PoolV2.Data;
  using PoolV2 for PoolV2.List;
  ...
  constructor(
    ...
    address _governance,
    address _sentinel,
    address _withdrawFeeCollector,
    IRewardVesting _rewardVesting
  ) public {
    require(_governance != address(0), "StakingPoolsV2: governance address cannot be 0x0");
    require(_sentinel != address(0), "StakingPoolsV2: sentinel address cannot be 0x0");
    require(_withdrawFeeCollector != address(0), "StakingPoolsV2: withdrawFee collector address cannot be 0x0");
  ...
  modifier onlyGovernance() {
    require(msg.sender == governance, "StakingPoolsV2: only governance");
    _;
  }
  ...
  function setPendingGovernance(address _pendingGovernance) external onlyGovernance {
    require(_pendingGovernance != address(0), "StakingPoolsV2: pending governance address cannot be 0x0");
  ...
  function acceptGovernance() external {
    require(msg.sender == pendingGovernance, "StakingPoolsV2: only pending governance");
  ...
  function createPool(IERC20 _token, bool _needVesting, uint256 _withdrawFee, uint256 _withdrawLock) external onlyGovernance returns (uint256) {
    require(tokenPoolIds[_token] == 0, "StakingPoolsV2: token already has a pool");

    uint256 _poolId = _pools.length();

    _pools.push(PoolV2.Data({
      token: _token,
      totalDeposited: 0,
      rewardWeight: 0,
  ...
  function setRewardWeights(uint256[] calldata _rewardWeights) external onlyGovernance {
    require(_rewardWeights.length == _pools.length(), "StakingPoolsV2: weights length mismatch");

    _updatePools();

    uint256 _totalRewardWeight = _ctx.totalRewardWeight;
    for (uint256 _poolId = 0; _poolId < _pools.length(); _poolId++) {
      PoolV2.Data storage _pool = _pools.get(_poolId);
  ...
  function deposit(uint256 _poolId, uint256 _depositAmount) external nonReentrant checkIfNewUser(_poolId) {
    ...
    StakeV2.Data storage _stake = _stakes[msg.sender][_poolId];
    PowerV2.Data storage _power = _powers[msg.sender][_poolId];

    _stake.update(_pool, _ctx);
    _power.update(_pool, _ctx);

    _depositedAt[msg.sender][_poolId] = block.timestamp;
  ...
  /// @dev Gets the withdraw fee of a pool which determines how much of the withdraw fee will be collected if withdraw within the lock period.
  ///
  /// @param _poolId the identifier of the pool.
  ///
  /// @return the pool withdraw fee.
  function getPoolWithdrawFee(uint256 _poolId) external view returns (uint256) {
    PoolV2.Data storage _pool = _pools.get(_poolId);
    return _pool.withdrawFee;
  }

    /// @dev Gets the withdraw lock of a pool which determines how long is the lock up period.
  ///
  /// @param _poolId the identifier of the pool.
  ///
  /// @return the pool withdraw lock.
  function getPoolWithdrawLock(uint256 _poolId) external view returns (uint256) {
    PoolV2.Data storage _pool = _pools.get(_poolId);
    return _pool.withdrawLock;
  }
  ...
  function getStakeTotalDeposited(address _account, uint256 _poolId) external view returns (uint256) {
    StakeV2.Data storage _stake = _stakes[_account][_poolId];
  ...
  function getAccumulatedPower(address _account, uint256 _poolId) external view returns (uint256) {
    PowerV2.Data storage _power = _powers[_account][_poolId];
  ...
  function _withdraw(uint256 _poolId, uint256 _withdrawAmount) internal {
    ...
    uint256 finalizedAmount = _withdrawAmount;
    uint256 depositTimestamp = _depositedAt[msg.sender][_poolId];

    if (depositTimestamp.add(_pool.withdrawLock) > block.timestamp) {
      withdrawPenalty = _withdrawAmount.mul(_pool.withdrawFee).div(10000);

      _pool.token.safeTransfer(withdrawFeeCollector, withdrawPenalty);
      finalizedAmount = finalizedAmount.sub(withdrawPenalty);
    }


    _pool.token.safeTransfer(msg.sender, finalizedAmount);

    emit TokensWithdrawn(msg.sender, _poolId, finalizedAmount, withdrawPenalty);
  }
  ...
  function _claim(uint256 _poolId) internal {
    require(!pause, "StakingPoolsV2: emergency pause enabled");
  ...
  function setRewardVesting(IRewardVesting _rewardVesting) external {
    require(pause && (msg.sender == governance || msg.sender == sentinel), "StakingPoolsV2: not paused, or not governance or sentinel");
    rewardVesting = _rewardVesting;
  ...
  function setPause(bool _pause) external {
      require(msg.sender == governance || msg.sender == sentinel, "StakingPoolsV2: !(gov || sentinel)");
      pause = _pause;
      emit PauseUpdated(_pause);
  }

  function setWithdrawFeeCollector(address _newWithdrawFeeCollector) external onlyGovernance {
      require(_newWithdrawFeeCollector != address(0), "StakingPoolsV2: withdrawFeeCollector address cannot be 0x0.");
