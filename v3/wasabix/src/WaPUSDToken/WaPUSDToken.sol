...
// File contracts/WaPUSDToken.sol
...
contract WaPUSDToken is AccessControl, ERC20("Wasabi PUSD", "waPUSD") {
  ...
  modifier onlyWhitelisted() {
    require(whiteList[msg.sender], "waPUSD: YumVault is not whitelisted");
    _;
  }
  ...
  function mint(address _recipient, uint256 _amount) external onlyWhitelisted {
    require(!blacklist[msg.sender], "waPUSD: YumVault is blacklisted.");
    uint256 _total = _amount.add(hasMinted[msg.sender]);
    require(_total <= ceiling[msg.sender],"waPUSD: YumVault's ceiling was breached.");
    require(!paused[msg.sender], "waPUSD: user is currently paused.");
