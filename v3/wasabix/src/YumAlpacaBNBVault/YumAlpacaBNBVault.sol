...
// File contracts/YumAlpacaBNBVault.sol
...
contract YumAlpacaBNBVault is ReentrancyGuard {
  ...
  {
    require(_governance != ZERO_ADDRESS, "YumAlpacaBNBVault: governance address cannot be 0x0.");
    require(_sentinel != ZERO_ADDRESS, "YumAlpacaBNBVault: sentinel address cannot be 0x0.");
    require(_feeCollector != ZERO_ADDRESS, "YumAlpacaBNBVault: feeCollector address cannot be 0x0.");
    ...
    flushActivator = 500 ether;// change for non 18 digit tokens
  ...
  function setPendingGovernance(address _pendingGovernance) external onlyGov {
    require(_pendingGovernance != ZERO_ADDRESS, "YumAlpacaBNBVault: governance address cannot be 0x0.");
  ...
  function setTransmuter(address _transmuter) external onlyGov {
    ...
    require(_transmuter != ZERO_ADDRESS, "YumAlpacaBNBVault: transmuter address cannot be 0x0.");
  ...
  function setRewards(address _rewards) external onlyGov {
    ...
    // transfers to the address because of `safeTransfer` checks.
    require(_rewards != ZERO_ADDRESS, "YumAlpacaBNBVault: rewards address cannot be 0x0.");
  ...
  function setHarvestFee(uint256 _harvestFee) external onlyGov {
    ...
    require(_harvestFee <= PERCENT_RESOLUTION, "YumAlpacaBNBVault: harvest fee above maximum.");
  ...
  function setFeeLevel(uint256[] calldata _feeLevel) external onlyGov {
    require(_feeLevel.length == 9, "YumAlpacaBNBVault: feeLevel length mismatch");
    feeLevel = _feeLevel;
  }

  function setDiscountTable(uint256[] calldata _discountTable) external onlyGov {
    require(_discountTable.length == 9, "YumAlpacaBNBVault: discountTable length mismatch");
  ...
  function setCollateralizationLimit(uint256 _limit) external onlyGov {
    ...
    require(_limit >= MINIMUM_COLLATERALIZATION_LIMIT, "YumAlpacaBNBVault: collateralization limit below minimum.");
    require(_limit <= MAXIMUM_COLLATERALIZATION_LIMIT, "YumAlpacaBNBVault: collateralization limit above maximum.");
  ...
  function initialize(IVaultAdapterV2 _adapter) external onlyGov {
    ...
    require(!initialized, "YumAlpacaBNBVault: already initialized");

    require(transmuter != ZERO_ADDRESS, "YumAlpacaBNBVault: cannot initialize transmuter address to 0x0");
    require(rewards != ZERO_ADDRESS, "YumAlpacaBNBVault: cannot initialize rewards address to 0x0");
  ...
  function mint(uint256 _amount) external nonReentrant noContractAllowed onLinkCheck expectInitialized {
    ...
    if (_totalCredit < _amount) {
      ...
      _cdp.checkHealth(_ctx, "YumAlpacaBNBVault: Loan-to-value ratio breached");
    } else {
  ...
  modifier expectInitialized() {
    require(initialized, "YumAlpacaBNBVault: not initialized.");
    _;
  }
  ...
  ///
  modifier onlyGov() {
    require(msg.sender == governance, "YumAlpacaBNBVault: only governance.");
    _;
  }
  ...
  function _updateActiveVault(IVaultAdapterV2 _adapter) internal {
    require(_adapter != IVaultAdapterV2(ZERO_ADDRESS), "YumAlpacaBNBVault: active vault address cannot be 0x0.");
    require(_adapter.token() == token, "YumAlpacaBNBVault: token mismatch.");
  ...
  function _recallFunds(uint256 _vaultId, uint256 _amount) internal returns (uint256, uint256) {
    require(emergencyExit || msg.sender == governance || _vaultId != _vaults.lastIndex(), "YumAlpacaBNBVault: not an emergency, not governance, and user does not have permission to recall funds from active vault");
