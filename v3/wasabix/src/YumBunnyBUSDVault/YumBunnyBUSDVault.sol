...
// File contracts/libraries/alchemist/BunnyVaultV2.sol
...
library BunnyVaultV2 {
  using BunnyVaultV2 for Data;
  using BunnyVaultV2 for List;
  using SafeERC20 for IDetailedERC20;
  using SafeMath for uint256;
...
// File contracts/YumBunnyBUSDVault.sol
...
contract YumBunnyBUSDVault is  ReentrancyGuard {
  using CDP for CDP.Data;
  using FixedPointMath for FixedPointMath.uq192x64;
  using BunnyVaultV2 for BunnyVaultV2.Data;
  using BunnyVaultV2 for BunnyVaultV2.List;
  ...
  BunnyVaultV2.List private _vaults;
  ...
  {
    require(_governance != ZERO_ADDRESS, "YumBunnyBUSDVault: governance address cannot be 0x0.");
    require(_sentinel != ZERO_ADDRESS, "YumBunnyBUSDVault: sentinel address cannot be 0x0.");
  ...
  function setPendingGovernance(address _pendingGovernance) external onlyGov {
    require(_pendingGovernance != ZERO_ADDRESS, "YumBunnyBUSDVault: governance address cannot be 0x0.");
  ...
  function setTransmuter(address _transmuter) external onlyGov {
    ...
    require(_transmuter != ZERO_ADDRESS, "YumBunnyBUSDVault: transmuter address cannot be 0x0.");
  ...
  function setRewards(address _rewards) external onlyGov {
    ...
    // transfers to the address because of `safeTransfer` checks.
    require(_rewards != ZERO_ADDRESS, "YumBunnyBUSDVault: rewards address cannot be 0x0.");
  ...
  function setHarvestFee(uint256 _harvestFee) external onlyGov {
    ...
    require(_harvestFee <= PERCENT_RESOLUTION, "YumBunnyBUSDVault: harvest fee above maximum.");
  ...
  function setCollateralizationLimit(uint256 _limit) external onlyGov {
    ...
    require(_limit >= MINIMUM_COLLATERALIZATION_LIMIT, "YumBunnyBUSDVault: collateralization limit below minimum.");
    require(_limit <= MAXIMUM_COLLATERALIZATION_LIMIT, "YumBunnyBUSDVault: collateralization limit above maximum.");
  ...
  function initialize(IVaultAdapterV2 _adapter) external onlyGov {
    ...
    require(!initialized, "YumBunnyBUSDVault: already initialized");

    require(transmuter != ZERO_ADDRESS, "YumBunnyBUSDVault: cannot initialize transmuter address to 0x0");
    require(rewards != ZERO_ADDRESS, "YumBunnyBUSDVault: cannot initialize rewards address to 0x0");
  ...
  function harvest(uint256 _vaultId) external expectInitialized noContractAllowed returns (uint256, uint256) {
    ...
    BunnyVaultV2.Data storage _vault = _vaults.get(_vaultId);
  ...
  function flushActiveVault() internal returns (uint256) {
    ...
    BunnyVaultV2.Data storage _activeVault = _vaults.last();
  ...
  function mint(uint256 _amount) external nonReentrant noContractAllowed onLinkCheck expectInitialized {
    ...
    if (_totalCredit < _amount) {
      ...
      _cdp.checkHealth(_ctx, "YumBunnyBUSDVault: Loan-to-value ratio breached");
    } else {
  ...
  modifier expectInitialized() {
    require(initialized, "YumBunnyBUSDVault: not initialized.");
    _;
  }
  ...
  ///
  modifier onlyGov() {
    require(msg.sender == governance, "YumBunnyBUSDVault: only governance.");
    _;
  }
  ...
  function _updateActiveVault(IVaultAdapterV2 _adapter) internal {
    require(_adapter != IVaultAdapterV2(ZERO_ADDRESS), "YumBunnyBUSDVault: active vault address cannot be 0x0.");
    require(_adapter.token() == token, "YumBunnyBUSDVault: token mismatch.");

    _vaults.push(BunnyVaultV2.Data({
      adapter: _adapter,
      totalDeposited: 0
    }));
  ...
  function _recallFunds(uint256 _vaultId, uint256 _amount) internal returns (uint256, uint256) {
    require(emergencyExit || msg.sender == governance || _vaultId != _vaults.lastIndex(), "YumBunnyBUSDVault: not an emergency, not governance, and user does not have permission to recall funds from active vault");
