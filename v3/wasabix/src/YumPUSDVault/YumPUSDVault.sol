...
// File contracts/libraries/alchemist/PolyquityVaultV2.sol
...
library PolyquityVaultV2 {
  using PolyquityVaultV2 for Data;
  using PolyquityVaultV2 for List;
  using SafeERC20 for IDetailedERC20;
  using SafeMath for uint256;
...
// File contracts/YumPUSDVault.sol
...
contract YumPUSDVault is ReentrancyGuard {
  using CDP for CDP.Data;
  using FixedPointMath for FixedPointMath.uq192x64;
  using PolyquityVaultV2 for PolyquityVaultV2.Data;
  using PolyquityVaultV2 for PolyquityVaultV2.List;
  ...
  PolyquityVaultV2.List private _vaults;
  ...
  {
    require(_governance != ZERO_ADDRESS, "YumPUSDVault: governance address cannot be 0x0.");
    require(_sentinel != ZERO_ADDRESS, "YumPUSDVault: sentinel address cannot be 0x0.");
    require(_feeCollector != ZERO_ADDRESS, "YumPUSDVault: feeCollector address cannot be 0x0.");
  ...
  function setPendingGovernance(address _pendingGovernance) external onlyGov {
    require(_pendingGovernance != ZERO_ADDRESS, "YumPUSDVault: governance address cannot be 0x0.");
  ...
  function setTransmuter(address _transmuter) external onlyGov {
    ...
    require(_transmuter != ZERO_ADDRESS, "YumPUSDVault: transmuter address cannot be 0x0.");
  ...
  function setRewards(address _rewards) external onlyGov {
    ...
    // transfers to the address because of `safeTransfer` checks.
    require(_rewards != ZERO_ADDRESS, "YumPUSDVault: rewards address cannot be 0x0.");
  ...
  function setHarvestFee(uint256 _harvestFee) external onlyGov {
    ...
    require(_harvestFee <= PERCENT_RESOLUTION, "YumPUSDVault: harvest fee above maximum.");
  ...
  function setFeeLevel(uint256[] calldata _feeLevel) external onlyGov {
    require(_feeLevel.length == 9, "YumPUSDVault: feeLevel length mismatch");
    feeLevel = _feeLevel;
  }

  function setDiscountTable(uint256[] calldata _discountTable) external onlyGov {
    require(_discountTable.length == 9, "YumPUSDVault: discountTable length mismatch");
  ...
  function setCollateralizationLimit(uint256 _limit) external onlyGov {
    ...
    require(_limit >= MINIMUM_COLLATERALIZATION_LIMIT, "YumPUSDVault: collateralization limit below minimum.");
    require(_limit <= MAXIMUM_COLLATERALIZATION_LIMIT, "YumPUSDVault: collateralization limit above maximum.");
  ...
  function initialize(IVaultAdapterV2 _adapter) external onlyGov {
    ...
    require(!initialized, "YumPUSDVault: already initialized");

    require(transmuter != ZERO_ADDRESS, "YumPUSDVault: cannot initialize transmuter address to 0x0");
    require(rewards != ZERO_ADDRESS, "YumPUSDVault: cannot initialize rewards address to 0x0");
  ...
  function harvest(uint256 _vaultId) external expectInitialized noContractAllowed returns (uint256, uint256) {
    ...
    PolyquityVaultV2.Data storage _vault = _vaults.get(_vaultId);
  ...
  function flushActiveVault() internal returns (uint256) {
    ...
    PolyquityVaultV2.Data storage _activeVault = _vaults.last();
  ...
  function mint(uint256 _amount) external nonReentrant noContractAllowed onLinkCheck expectInitialized {
    ...
    if (_totalCredit < _amount) {
      ...
      _cdp.checkHealth(_ctx, "YumPUSDVault: Loan-to-value ratio breached");
    } else {
  ...
  modifier expectInitialized() {
    require(initialized, "YumPUSDVault: not initialized.");
    _;
  }
  ...
  ///
  modifier onlyGov() {
    require(msg.sender == governance, "YumPUSDVault: only governance.");
    _;
  }
  ...
  function _updateActiveVault(IVaultAdapterV2 _adapter) internal {
    require(_adapter != IVaultAdapterV2(ZERO_ADDRESS), "YumPUSDVault: active vault address cannot be 0x0.");
    require(_adapter.token() == token, "YumPUSDVault: token mismatch.");

    _vaults.push(PolyquityVaultV2.Data({
      adapter: _adapter,
      totalDeposited: 0
    }));
  ...
  function _recallFunds(uint256 _vaultId, uint256 _amount) internal returns (uint256, uint256) {
    require(emergencyExit || msg.sender == governance || _vaultId != _vaults.lastIndex(), "YumPUSDVault: not an emergency, not governance, and user does not have permission to recall funds from active vault");
