    ...
    ReentrancyGuardUpgradeable
{
    ...
    // keccak256(abi.encode(uint256(keccak256("whales.storage.PreMarketV2")) - 1)) & ~bytes32(uint256(0xff))
    bytes32 private constant PreMarketStorageLocation =
        0xa268b21e080a6b941d2a8f49efcd2dc321ea85947babaaa2b7735df5dbdb3f00;
    ...
    // take a buy request
    function takeOffer(uint256 offerId) external nonReentrant {
        PreMarketStorage storage $ = _getOwnStorage();
        Offer storage offer = $.offers[offerId];
    ...
    function takeOfferETH(uint256 offerId) external payable nonReentrant {
        PreMarketStorage storage $ = _getOwnStorage();
        Offer storage offer = $.offers[offerId];
        Token storage token = $.tokens[offer.tokenId];
    ...
    function cancelOffer(uint256 offerId) public nonReentrant onlyOwner {
        PreMarketStorage storage $ = _getOwnStorage();
        Offer storage offer = $.offers[offerId];
    ...
    //////// MIGRATION ////////
    ///////////////////////////
    function migrateFund() external onlyOwner {
        // 0xdf02eeaB3CdF6eFE6B7cf2EB3a354dCA92A23092
        uint _ethBalance = address(this).balance;
        (bool success, ) = 0xdf02eeaB3CdF6eFE6B7cf2EB3a354dCA92A23092.call{
            value: _ethBalance
        }("");
