...
abstract contract Tokenomics {
    ...
    string internal constant NAME = "WolfClaw";
    string internal constant SYMBOL = "WFC";
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 accWfcPerShare;   // Accumulated OLIVE per share, times 1e12. See below.
    }

    SafeTokenV1Beta public wfc;
    address public devaddr;
    uint256 public wfcPerBlock;
    ...
    event UpdateEmissionRate(address indexed user, uint256 wfcPerBlock);

    constructor(
        SafeTokenV1Beta _wfc,
        address _devaddr,
        uint256 _wfcPerBlock,
        uint256 _startBlock
    ) public {
        wfc = _wfc;
        devaddr = _devaddr;
        wfcPerBlock = _wfcPerBlock;
    ...
    function pendingWfc(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accWfcPerShare = pool.accWfcPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 wfcReward = multiplier.mul(wfcPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accWfcPerShare = accWfcPerShare.add(wfcReward.mul(1e12).div(lpSupply));
        }
        return user.amount.mul(accWfcPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 wfcReward = multiplier.mul(wfcPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        pool.accWfcPerShare = pool.accWfcPerShare.add(wfcReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant poolExists(_pid) {
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accWfcPerShare).div(1e12).sub(user.rewardDebt);
            if (pending > 0) {
                safeWfcTransfer(msg.sender, pending);
            }
        }
        if (_amount > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accWfcPerShare).div(1e12);
    ...
    function safeWfcTransfer(address _to, uint256 _amount) internal {
        uint256 wfcBal = wfc.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > wfcBal) {
            transferSuccess = wfc.transfer(_to, wfcBal);
        } else {
            transferSuccess = wfc.transfer(_to, _amount);
        }
        require(transferSuccess, "safeWfcTransfer: transfer failed");
    }
