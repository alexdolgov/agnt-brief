...
contract FXPool is IMinimalSwapInfoPool, BalancerPoolToken, Ownable, Storage, ReentrancyGuard, Pausable {
    ...
    address public poolAdmin = address(0);
    ...
    event FeesCollected(address indexed recipient, uint256 feesCollected);
    event FeesAccrued(uint256 feesCollected);
    event ProtocolFeeShareUpdated(address indexed updater, uint256 newProtocolPercentage);
    event PoolAdminChanged(address indexed newPoolAdmin);
    ...
    modifier isNotInitialized() {
        require(initialized == false, 'FXPool/already-initialized');
        _;
    }

    modifier onlyAdminOrOwner() {
        require(msg.sender == poolAdmin || msg.sender == owner(), 'FXPool/caller-not-admin-or-owner');
        _;
    }
    ...
    function initialize(address[] memory _assets, uint256[] memory _assetWeights) external isNotInitialized onlyOwner {
    ...
    function setParams(
        ...
        require(initialized == true, 'FXPool/not-initialized');
    ...
    // SETTERS
    function setPaused() external onlyAdminOrOwner {
    ...
    function setCap(uint256 _cap) external onlyAdminOrOwner {
    ...
    function setCollectorAddress(address _collectorAddress) external onlyAdminOrOwner {
    ...
    /// @notice Change the pool admin of this pool
    /// @param _poolAdmin new pool admin
    function setPoolAdmin(address _poolAdmin) external onlyOwner {
        poolAdmin = _poolAdmin;
        emit PoolAdminChanged(_poolAdmin);
    }
    ...
    ) external view whenNotPaused returns (uint256, uint256[] memory) {
    ...
    function mintProtocolFees() external onlyAdminOrOwner {
