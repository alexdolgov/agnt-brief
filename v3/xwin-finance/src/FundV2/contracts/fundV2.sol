...
contract FundV2 is xWinStrategy {
    ...
    function _deposit(uint256 amount, uint32 _slippage) internal returns (uint256) {
        ...
        _calcFundFee();
        for (uint256 i = 0; i < targetAddr.length; i++) {
            if (IxWinStrategyInteractor(address(xWinSwap)).isxWinStrategy(targetAddr[i])) {
                xWinStrategyWithFee(targetAddr[i]).collectFundFee();
                if(xWinStrategyWithFee(targetAddr[i]).canCollectPerformanceFee() && xWinStrategyWithFee(targetAddr[i]).performanceFee() > 0)
                {
