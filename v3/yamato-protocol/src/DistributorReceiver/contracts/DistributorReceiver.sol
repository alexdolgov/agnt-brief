...
contract DistributorReceiver is Ownable, CCIPReceiver {
    ...
    /// @param scorerAddress The airdrop address which requests to add the score
    ...
    ) CCIPReceiver(router_) Ownable(initialOwner_) {
    ...
    modifier onlyAirdrop() {
        require(factory.airdrops(msg.sender), "You are not the airdrop.");
        _;
    }

    /// @notice Add a specified amount to the score of a specified user
    /// @dev Expected to be called from airdrop contracts
    /// @param target_ The address of the user who is rewarded
    /// @param amount_ The amount of the score to be added
    function addScore(address target_, uint256 amount_) external onlyAirdrop {
    ...
    function setToken(address token_) external onlyOwner {
        require(address(token) == address(0), "Token address is already set");
        token = IERC20(token_);
    }
