...
contract YieldManager {
    ...
    ) external virtual returns (uint256 _currentRate, uint256 _expectedRate) {}
}
...
contract Allocatin {
    ...
    function shouldProposeNewDebt(
        address _vault
    ) external view returns (bool trigger, bytes memory _calldata) {
        YieldManager.Allocation[] memory _allocation;
        (trigger, _allocation) = getOptimalAllocation(_vault);
        if (trigger) {
            _calldata = abi.encodeCall(
                YieldManager.updateAllocation,
                (_vault, _allocation)
            );
        } else {
            _calldata = bytes("Already Optimal");
        }
    }
    ...
    {
        return getOptimalAllocation(_vault, IVault(_vault).get_default_queue());
    }
        ...
        )
    {
        ...
        _allocation = new YieldManager.Allocation[](_length);
        uint256 fullDebt = IVault(_vault).totalAssets() -
            IVault(_vault).minimum_total_idle();
        uint256 highestRate;
        uint256 highestStrategy;

        address _strategy;
        uint256 currentDebt;
        for (uint256 i = 0; i < _length; ++i) {
            _strategy = _strategies[i];
            currentDebt = IVault(_vault).strategies(_strategy).current_debt;
            uint256 rate = aprOracle.getStrategyApr(
                _strategy,
                int256(fullDebt) - int256(currentDebt)
            );
        ...
        uint256 idx = 0;
        for (uint256 i = 0; i < _length; ++i) {
            if (i != highestStrategy) {
                _allocation[idx] = YieldManager.Allocation({
        ...
        _allocation[_length - 1] = YieldManager.Allocation({
            strategy: _strategies[highestStrategy],
            newDebt: uint96(fullDebt)
        });

        if (
            IVault(_vault)
                .strategies(_strategies[highestStrategy])
                .current_debt != fullDebt
        ) {
            shouldUpdate = true;
        }
    }
