...
contract StrategyAprOracle {
    ...
    function aprAfterDebtChange(
        ...
        address vault = IAjnaRouter(_strategy).vault();
        address compounder = IAjnaRouter(_strategy).compounder();

        uint256 vaultApr = aprOracle.getExpectedApr(vault, _delta);
        uint256 compounderApr = aprOracle.getExpectedApr(compounder, _delta);

        return vaultApr + compounderApr;
    }
