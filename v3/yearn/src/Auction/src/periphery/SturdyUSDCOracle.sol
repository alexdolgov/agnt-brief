...
contract SturdyUSDCOracle is AprOracleBase {
    ...
    ICurvePool internal constant CURVE_TRICRYPTO_USDC_WETH_WBTC =
    ...
    function aprAfterDebtChange(
        ...
        _apr = APR_ORACLE.getExpectedApr(address(_sturdyAggregator), _delta);
        ...
        for (uint256 i; i < _rewardInfo.length; ++i) {
            ...
            uint256 _rewardAmountUsdc = CURVE_TRICRYPTO_USDC_WETH_WBTC
                .get_dy(2, 0, uint256(-_rewardAmountWeth));

            _apr +=
                (_rewardAmountUsdc * WAD * (ONE_YEAR / _rewardPeriod)) /
                uint256(
