...
contract GaugeValidator {
    ...
    uint256 public constant MAX_PROPOSALS_TO_CHECK = 20;
    ...
    /**
     * @notice Get all gauge addresses from active proposals
     * @return gauges Array of gauge addresses
     */
    function getActiveProposalGauges() external view returns (address[] memory gauges) {
        uint256 votesLength = dao.votesLength();
        uint256 startIndex = votesLength > 0 ? votesLength - 1 : 0;
        
        // First, collect all gauges in a dynamic array
        address[] memory tempGauges = new address[](MAX_PROPOSALS_TO_CHECK * 20); // Assume max 20 gauges per proposal
        uint256 gaugeCount = 0;
        ...
        for (uint256 i = startIndex; i >= 0; i--) {
            ...
            address[] memory proposalGauges = parseGaugeAddressesFromBytes(script);
            for (uint256 j = 0; j < proposalGauges.length; j++) {
                tempGauges[gaugeCount] = proposalGauges[j];
                gaugeCount++;
            }
        ...
        address[] memory finalGauges = new address[](gaugeCount);
        for (uint256 i = 0; i < gaugeCount; i++) {
            finalGauges[i] = tempGauges[i];
        }
     ...
     * @return gauges Array of gauge addresses
     */
    function getProposalGauges(uint256 proposalId) public view returns (address[] memory gauges) {
        (
            bool open,
            bool executed,
            uint64 startDate,,,,,,,
        ...
        return parseGaugeAddressesFromBytes(script);
    }

    function validateProposalGauges(uint256 proposalId) public view returns (bool) {
        address[] memory gauges = getProposalGauges(proposalId);
        for (uint256 i = 0; i < gauges.length; i++) {
            (bool isValid, ) = validateGauge(gauges[i]);
            if (!isValid) {
                return false;
            }
        }
