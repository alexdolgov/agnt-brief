...
contract PublicAllocatorTendExecutor is Governance {
    ...
    constructor(address _governance,address _publicAllocator) Governance(_governance) {
        require(_publicAllocator != address(0), "allocator=0");
        publicAllocator = IPublicAllocator(_publicAllocator);
        allowed[_governance] = true;
    }

    function setAllowed(address _address, bool _allowed) external onlyGovernance {
