...
contract USDCToUSDS is BaseHealthCheck, UniswapV3Swapper {
    ...
    address private constant pocket =
            ...
            if (depositLimit > totalDeposits) {
                return
                    _min(
                        _min(
                            _balancePSM(),
                            IVault(vault).maxDeposit(address(this))
                        ) / SCALER,
    ...
    function _deployFunds(uint256 _amount) internal override {
        ...
        IExchange(DAI_USDS_EXCHANGER).daiToUsds(
            address(this),
            _amount * SCALER
        ); //DAI --> USDS 1:1
    ...
    function _vaultsMaxWithdraw() internal view returns (uint256) {
        if (IPSM(PSM).tout() >= maxAcceptableFeeOutPSM) {
            return
                ...
                ) / SCALER; //vault maximum redeemable assets
        } else {
            return
                _min(
    ...
    function _freeFunds(uint256 _amount) internal override {
        ...
        if (feeOut >= maxAcceptableFeeOutPSM) {
            ...
            _swapFrom(
                ...
                address(asset),
                _amount,
                (_amount * (MAX_BPS - swapSlippageBPS)) / MAX_BPS / SCALER
            ); //swap DAI --> USDC through Uniswap (in DAI amount)
        } else {
            IPSM(PSM).buyGem(
                address(this),
                (_amount * WAD) / (WAD + feeOut) / SCALER
            ); // DAI --> USDC 1:1 through PSM (in USDC amount). Need to account for fees that will be added on top.
        }
    }
        ...
        returns (uint256 _totalAssets)
    {
        ...
        if (TokenizedStrategy.isShutdown()) {
            _totalAssets =
                balance +
                IVault(vault).convertToAssets(_balanceVault()) /
                SCALER;
        } else {
