...
interface EtherfiLiquidityPoolInterface {
    function deposit() external payable;
}
...
contract eETHDepositor {
    EtherfiLiquidityPoolInterface public immutable etherfiLiquidityPool;
    IERC20 public immutable eETH;
    WEETH public immutable weETH;

    constructor(EtherfiLiquidityPoolInterface _etherfiLiquidityPool, IERC20 _eETH, WEETH _weETH) {
        etherfiLiquidityPool = _etherfiLiquidityPool;
        eETH = _eETH;
        weETH = _weETH;
        _eETH.approve(address(_weETH), type(uint256).max);
    }

    function deposit() external payable {
        etherfiLiquidityPool.deposit{ value: msg.value }();
        weETH.wrap(eETH.balanceOf(address(this)));
        weETH.transfer(address(msg.sender), weETH.balanceOf(address(this)));
    }
}
