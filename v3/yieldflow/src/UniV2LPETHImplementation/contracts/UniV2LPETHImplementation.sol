...
contract UniV2LPETHImplementation is ReentrancyGuard {
    ...
    external payable nonReentrant {
        ...
        IERC20(IUniV2LPFactory(factoryAddress).getStakingTokenA()).safeApprove(IUniV2LPFactory(factoryAddress).getStakingContract(), amountTokenDesired);
    ...
    function removeLiquidityETH(
        ...
        IERC20(IUniV2LPFactory(factoryAddress).getLPToken()).safeApprove(IUniV2LPFactory(factoryAddress).getStakingContract(), liquidity);
    ...
    function stake(uint amount) public onlyOwner {
        IERC20(IUniV2LPFactory(factoryAddress).getLPToken()).safeApprove(IUniV2LPFactory(factoryAddress).getRewardStakingContract(), amount);
