...
// src/interface/IVaultViewer.sol
// solhint-disable one-contract-per-file

library ERC20Viewer {
    function symbol(IERC20Metadata_0 token) internal view returns (string memory) {
    ...
    function name(IERC20Metadata_0 token) internal view returns (string memory) {
    ...
    function decimals(IERC20Metadata_0 token) internal view returns (uint8) {
...
abstract contract BaseVault is IVault, ERC20PermitUpgradeable, AccessControlUpgradeable, ReentrancyGuardUpgradeable {
     ...
     */
    function decimals() public view virtual override(ERC20Upgradeable, IERC20Metadata_0) returns (uint8) {
    ...
    function convertToShares(uint256 assets) public view virtual returns (uint256 shares) {
        (shares,) = _convertToShares(asset(), assets, Math_0.Rounding.Floor);
    }
    ...
    function convertToAssets(uint256 shares) public view virtual returns (uint256 assets) {
        (assets,) = _convertToAssets(asset(), shares, Math_0.Rounding.Floor);
    }
    ...
    function previewMint(uint256 shares) public view virtual returns (uint256 assets) {
        (assets,) = _convertToAssets(asset(), shares, Math_0.Rounding.Ceil);
    }
    ...
    function previewWithdraw(uint256 assets) public view virtual returns (uint256 shares) {
        ...
        (shares,) = _convertToShares(asset(), assets + fee, Math_0.Rounding.Ceil);
    }
    ...
    function deposit(uint256 assets, address receiver) public virtual nonReentrant returns (uint256) {
        ...
        (uint256 shares, uint256 baseAssets) = _convertToShares(asset(), assets, Math_0.Rounding.Floor);
    ...
    function mint(uint256 shares, address receiver) public virtual nonReentrant returns (uint256) {
        ...
        (uint256 assets, uint256 baseAssets) = _convertToAssets(asset(), shares, Math_0.Rounding.Floor);
        _deposit(asset(), _msgSender(), receiver, assets, shares, baseAssets);
        return assets;
    }
    ...
    function previewDepositAsset(address asset_, uint256 assets) public view virtual returns (uint256 shares) {
        (shares,) = _convertToShares(asset_, assets, Math_0.Rounding.Floor);
    }
        ...
        returns (uint256)
    {
        ...
        (uint256 shares, uint256 baseAssets) = _convertToShares(asset_, assets, Math_0.Rounding.Floor);
    ...
    function _deposit(
        ...
        SafeERC20.safeTransferFrom(IERC20_0(asset_), caller, address(this), assets);
     ...
     */
    function _convertToAssets(address asset_, uint256 shares, Math_0.Rounding rounding)
        internal
        view
        virtual
     ...
     */
    function _convertToShares(address asset_, uint256 assets, Math_0.Rounding rounding)
        internal
        view
        virtual
        returns (uint256, uint256)
    ...
    function _convertAssetToBase(address asset_, uint256 assets) internal view virtual returns (uint256) {
        ...
        return assets.mulDiv(rate, 10 ** (_getAssetStorage().assets[asset_].decimals), Math_0.Rounding.Floor);
    }
    ...
    function _convertBaseToAsset(address asset_, uint256 assets) internal view virtual returns (uint256) {
        if (asset_ == address(0)) revert ZeroAddress();
        uint256 rate = IProvider(provider()).getRate(asset_);
        return assets.mulDiv(10 ** (_getAssetStorage().assets[asset_].decimals), rate, Math_0.Rounding.Floor);
    }
    ...
    function addAsset(address asset_, bool active_) public virtual onlyRole(ASSET_MANAGER_ROLE) {
        _addAsset(asset_, IERC20Metadata_0(asset_).decimals(), active_);
    }
    ...
    function _computeTotalAssets() internal view virtual returns (uint256 totalBaseBalance) {
        ...
        for (uint256 i = 0; i < assetListLength; i++) {
            uint256 balance = IERC20_0(assetList[i]).balanceOf(address(this));
...
contract BaseVaultViewer is Initializable, IVaultViewer {
    ...
    function _getAssets() internal view returns (AssetInfo[] memory assetsInfo) {
        ...
        for (uint256 i = 0; i < assets.length; ++i) {
            balances[i] = IERC20Metadata_0(assets[i]).balanceOf(address(vault));
        }
        ...
        returns (AssetInfo[] memory assetsInfo)
    {
        ...
        for (uint256 i = 0; i < assets.length; ++i) {
            ...
            IERC20Metadata_0 asset = IERC20Metadata_0(assets[i]);

            uint256 assetBalance = balances[i];
            uint256 baseBalance = Math_0.mulDiv(assetBalance, rate, 10 ** assetParams.decimals, Math_0.Rounding.Floor);
    ...
    function getRate() external view returns (uint256) {
        ...
        return Math_0.mulDiv(baseAssetValue, totalAssets, totalSupply);
    }
    ...
    function getStrategies() public view returns (AssetInfo[] memory) {
        ...
        for (uint256 i = 0; i < assetsLength; ++i) {
            if (!isUnderlyingAsset(assets[i])) {
                ...
                balances[j] = IERC20Metadata_0(assets[i]).balanceOf(address(vault));
                j++;
            }
        }
