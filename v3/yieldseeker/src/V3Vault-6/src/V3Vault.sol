...
contract V3Vault is ERC20, Multicall, Ownable2Step, IVault, IERC721Receiver, Constants {
    ...
    function stakePosition(uint256 tokenId) external {
        require(gaugeManager != address(0), "Gauge manager not set");
        require(ownerOf(tokenId) == msg.sender, "Not depositor");
    ...
    function unstakePosition(uint256 tokenId) external {
        ...
        bool isDepositor = ownerOf(tokenId) == msg.sender;
        bool isLiquidation = transformedTokenId == tokenId; // During liquidation, position is being transformed
        
        require(isDepositor || isLiquidation, "Not authorized");
    ...
    // Note: For claiming and compounding rewards, users should interact
    // directly with the GaugeManager contract to save vault contract size
    ...
    function _unstakeForLiquidation(uint256 tokenId) internal {
        if (gaugeManager != address(0)) {
            ...
            address gauge = IGaugeManager(gaugeManager).tokenIdToGauge(tokenId);
            if (gauge != address(0)) {
