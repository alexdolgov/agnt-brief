...
import {V2SwapRouter} from '../modules/zf/V2SwapRouter.sol';
import {V3SwapRouter} from '../modules/zf/V3SwapRouter.sol';
import {StableSwapRouter} from '../modules/zf/StableSwapRouter.sol';
...
import {IZFNFTMarket} from '../interfaces/IZFNFTMarket.sol';
...
abstract contract Dispatcher is Payments, V2SwapRouter, V3SwapRouter, StableSwapRouter, Callbacks, LockAndMsgSender {
    ...
    function dispatch(bytes1 commandType, bytes calldata inputs) internal returns (bool success, bytes memory output) {
        ...
        if (command < Commands.FOURTH_IF_BOUNDARY) {
            ...
            if (command == Commands.EXECUTE_SUB_PLAN) {
            ...
            } else if (command == Commands.ZF_NFT_ETH) {
                ...
                (success, output) = ZF_NFT_MARKET.call{value: value}(
                    abi.encodeWithSelector(IZFNFTMarket.buyTokenUsingETH.selector, collection, tokenId)
                );
                if (!success) output = abi.encodePacked(BuyZFNFTFailed.selector);
            } else if (command == Commands.ZF_NFT_WETH) {
                // equivalent: abi.decode(inputs, (address, uint256, uint256))
                address collection;
                uint256 tokenId;
                uint256 price;
                ...
                IZFNFTMarket(ZF_NFT_MARKET).buyTokenUsingWETH(collection, tokenId, price);
            } else {
