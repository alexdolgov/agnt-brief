...
import {IZFV2Factory} from '../interfaces/IZFV2Factory.sol';
...
library UniversalRouterHelper {
    ...
    function getStableAmountsIn(
        address stableSwapFactory,
        address stableSwapTwoPoolInfo,
        address stableSwapThreePoolInfo,
    ...
    function pairAndReservesFor(address factory, bytes32 initCodeHash, address tokenA, address tokenB)
        private
        view
        returns (address pair, uint256 reserveA, uint256 reserveB, uint256 swapFee)
    {
        address token0;
        (pair, token0) = pairAndToken0For(factory, initCodeHash, tokenA, tokenB);
        (uint256 reserve0, uint256 reserve1, uint256 fee) = IZFV2Pair(pair).getReservesAndParameters();
        ...
        returns (uint256 amount, address pair)
    {
        ...
        for (uint256 i = path.length - 1; i > 0; i--) {
            ...
            uint256 reserveOut;
            uint256 swapFee;
            (pair, reserveIn, reserveOut, swapFee) = pairAndReservesFor(factory, initCodeHash, path[i - 1], path[i]);
            amount = getAmountIn(amount, reserveIn, reserveOut, swapFee);
        }
    }
