...
import {IZFV2Pair} from '../../interfaces/IZFV2Pair.sol';
...
abstract contract V2SwapRouter is RouterImmutables, Permit2Payments {
    ...
    function _v2Swap(address[] calldata path, address recipient, address pair) private {
        ...
        for (uint256 i; i < finalPairIndex; i++) {
            ...
            (uint256 reserve0, uint256 reserve1, uint swapFee) = IZFV2Pair(pair).getReservesAndParameters();
            ...
            (nextPair, token0) = i < penultimatePairIndex
                ? UniversalRouterHelper.pairAndToken0For(
                    ZF_V2_FACTORY, ZF_V2_PAIR_INIT_CODE_HASH, output, path[i + 2]
                )
                : (recipient, address(0));
            if (pair == address(0)) revert V2InvalidTokenAddress();
            IZFV2Pair(pair).swap(amount0Out, amount1Out, nextPair, new bytes(0));
            pair = nextPair;
        }
    }

    /// @notice Performs a ZF v2 exact input swap
    ...
    function v2SwapExactInput(
        ...
        address firstPair =
            UniversalRouterHelper.pairFor(ZF_V2_FACTORY, ZF_V2_PAIR_INIT_CODE_HASH, path[0], path[1]);
    ...
    /// @notice Performs a ZF v2 exact output swap
    /// @param recipient The recipient of the output tokens
    ...
    function v2SwapExactOutput(
        ...
        (uint256 amountIn, address firstPair) =
            UniversalRouterHelper.getAmountInMultihop(ZF_V2_FACTORY, ZF_V2_PAIR_INIT_CODE_HASH, amountOut, path);
