...
import './interfaces/IZeDPOld.sol';
    ...
    AccessManagedUpgradeable
{
    ...
    {
        (uint256 tokenId, uint256 zeusdMinted) = _mint(
            rwaAddress,
            assetAddress,
            amount,
            msg.sender,
    ...
    function rebalanceZeUSD(
        ...
        for (uint256 i = 0; i < allNftTokenIds.length; i++) {
            DataTypes.OldDepositMetadata memory oldMetadata = IZeUSD_CDP(
                $.registry.getContract(Constants.CONTRACT_ZEUSD_CDP_OLD)
