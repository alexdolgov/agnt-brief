...
contract VodkaV1 is OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable, ERC20BurnableUpgradeable {
    ...
    event Withdraw(address indexed user, uint256 amount, uint256 time, uint256 glpAmount,uint256 profits,uint256 glpprice);
    ...
    function getCurrentPosition(
        ...
        return (_convertGLPToUSDC(userShareAfterFee,getGLPPrice(true)), _convertGLPToUSDC(userShares, _userInfo.price));
    }
    ...
    function handleAndCompoundRewards() public returns (uint256) {
        ...
        IRewardRouterV2(strategyAddresses.rewardVault).handleRewards(
            true,
            true,
            true,
            true,
        ...
        if (balanceDiff > 0) {
            (uint256 toOwner,uint256 toWater,uint256 toVodkaUsers) = IGlpRewardHandler(strategyAddresses.glpRewardHandler).getVodkaSplit(balanceDiff);
    ...
    function closePosition(
        ...
        if (!data.inFull) {
            ...
            (waterProfits, data.leverageUserProfits) = _getProfitSplit(data.profits, _userInfo.leverage);
            if (waterProfits > 0) {
                IERC20Upgradeable(strategyAddresses.USDC).safeTransfer(feeConfiguration.waterFeeReceiver, waterProfits);
            }

            toLeverageUser = userShares + data.leverageUserProfits;
            ...
            if (returnedValue < fulldebtValue) {
                ...
                if(toLeverageUser > _userInfo.deposit) {
                    data.leverageUserProfits = toLeverageUser - _userInfo.deposit;
                }
        ...
        emit Withdraw(_user, amountAfterFee, block.timestamp, data.withdrawableShares,data.leverageUserProfits, getGLPPrice(true));
    }
    ...
    function _getProfitSplit(uint256 _profit, uint256 _leverage) internal view returns (uint256, uint256) {
        ...
        uint256 toVodkaUser = _profit - toWater;

        return (toWater, toVodkaUser);
    }
    ...
    function _removeVaultSwapFees(address _token, uint256 _usdgAMount) internal view returns (uint256) {
        ...
        uint256 getVaultRedemptionAmount = IVault(strategyAddresses.Vault).getRedemptionAmount(
            _token,
            _usdgAMount
        );
