...
contract LinkWater is ERC4626Upgradeable, OwnableUpgradeable {
    ...
    modifier onlyLINKGifter() {
        require(allowedToGift[msg.sender], "Not allowed to increment LINK");
        _;
    }
    ...
    event LINKGifterAllowed(address indexed gifter, bool status);
    ...
    function initialize(address _LINK) external initializer {
        require(_LINK != address(0), "ZERO_ADDRESS");
        LINK = _LINK;
        ...
        __Ownable_init();
        __ERC4626_init(IERC20Upgradeable(_LINK));
        __ERC20_init("LINK-WATER", "LINK-WATER");
    }
    ...
    function totalAssetInPrice() external view returns(uint256) {
        ...
        uint256 getPrice = IHandler(vodkaHandler).getLatestData(LINK, false);
    ...
    function allowLINKGifter(address _gifter, bool _status) external onlyOwner zeroAddress(_gifter) {
        allowedToGift[_gifter] = _status;
        emit LINKGifterAllowed(_gifter, _status);
    }
    ...
    /**
     * @notice Allow the VodkaV2 Vault to lend a certain amount of LINK to the protocol.
     * @dev The function allows the VodkaV2 Vault to lend a certain amount of LINK to the protocol. It updates the total debt and total LINK balances accordingly.
     * @param _borrowed The amount of LINK to lend.
    ...
    function lend(uint256 _borrowed, address _receiver) external onlyAllowedVaults returns (bool status) {
        ...
        require(totalAsset > am, "Not enough LINK to lend");

        totalDebt += _borrowed;
        totalAsset -= _borrowed;

        require(getUtilizationRate() <= utilRate, "Leverage ratio too high");
        IERC20(LINK).safeTransfer(_receiver, am);
     ...
     * @dev The function allows the VodkaV2 Vault to repay a certain amount of debt to the protocol. It updates the total debt and total LINK balances accordingly.
     * @param _debtAmount The amount of debt to repay.
     * @param _amountPaid The amount of LINK paid to repay the debt.
    ...
    function repayDebt(uint256 _debtAmount, uint256 _amountPaid) external onlyAllowedVaults returns (bool) {
        IERC20(LINK).safeTransferFrom(msg.sender, address(this), _amountPaid);
     ...
     *      It updates the total LINK balance and sets a timelock for the receiver.
    ...
    function deposit(uint256 _assets, address _receiver) public override noZeroValues(_assets) returns (uint256) {
        IERC20(LINK).transferFrom(msg.sender, address(this), _assets);
    ...
    function _deposit(address caller, address receiver, uint256 assets, uint256 _shares) internal override noZeroValues(assets) {
        ...
        if (totalSupply() == 0) {
            ...
            // LINK decimal is known to be 18
     ...
     *      The function then performs the actual withdrawal by calling the internal `_withdraw` function. It updates the total LINK balance after the withdrawal and returns the amount of shares returned to the user.
     * @param _assets The amount of assets (LINK) to withdraw.
     * @param _receiver The address of the receiver who will receive the corresponding shares.
     * @param _owner The address of the owner who is making the withdrawal.
     * @return The amount of shares returned to the user.
     */
    function withdraw(
        uint256 _assets, // Native (LINK) token amount
        address _receiver,
        address _owner
        ...
        IERC20(LINK).safeTransfer(feeReceiver, feeAmount);
    ...
    function increaseTotalLINK(uint256 _amount) external onlyLINKGifter {
