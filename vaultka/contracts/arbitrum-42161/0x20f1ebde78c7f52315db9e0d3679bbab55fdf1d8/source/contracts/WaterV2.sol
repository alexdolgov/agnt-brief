...
contract WaterV2 is ERC4626Upgradeable, OwnableUpgradeable {
    ...
    address public RadiantLendingPool;
    address public RadiantBearingUSDC;
    address public bearingProfitReceiver;
    uint256 public bearingIncrement;
    ...
    event RadiantConfigChanged(address _radiantLendingPool,address _RadiantBearingUSDC, address _bearingProfitReceiver);
    ...
    function depositRadiantAsset(uint256 _amount) external onlyOwner {
        _radiantDeposit(_amount, false);
        bearingIncrement = IERC20(RadiantBearingUSDC).balanceOf(address(this));
    }

    function setRadiantConfig(
        address _radiantLendingPool,
        address _RadiantBearingUSDC,
        address _bearingProfitReceiver) external onlyOwner {
        require(_radiantLendingPool != address(0) &&
        _RadiantBearingUSDC != address(0) &&
        _bearingProfitReceiver != address(0), "ZERO_ADDRESS");

        RadiantLendingPool = _radiantLendingPool;
        RadiantBearingUSDC = _RadiantBearingUSDC;
        bearingProfitReceiver = _bearingProfitReceiver;
        
        emit RadiantConfigChanged(_radiantLendingPool,_RadiantBearingUSDC,bearingProfitReceiver);
    }
    ...
    */
    function lend(uint256 _borrowed) external onlySakeVault returns (bool status) {
        ...
        console.log("Before extract profit");
        _extractBearingProfit();
        console.log("After extract profit");

        if (IERC20(RadiantBearingUSDC).balanceOf(address(this)) > 0) {
            (uint256 bearingWithdrawn, uint256 USDCWithdrawn) = _radiantWithdraw(_borrowed);
        }
    ...
    function repayDebt(uint256 _debtAmount, uint256 _amountPaid) external onlySakeVault returns (bool) {
        ...
        uint256 bearingAdded = _radiantDeposit(_amountPaid, false);
    ...
    function deposit(uint256 _assets, address _receiver) public override noZeroValues(_assets) returns (uint256) {
        ...
        _radiantDeposit(_assets,true);
        _deposit(_msgSender(), msg.sender, _assets, shares);
        
        totalUSDC += _assets;
        bearingIncrement = IERC20(RadiantBearingUSDC).balanceOf(address(this));
        userTimelock[msg.sender] = block.timestamp + lockTime;
        console.log("BalanceOf Owner",balanceOf(msg.sender));
        console.log("MaxW",maxWithdraw(msg.sender));
    ...
    function withdraw(
        ...
        totalUSDC -= _assets;
        console.log("Before radiant withdraw",_assets);
        _radiantWithdraw(_assets);
    ...
    function _radiantDeposit(uint256 _assets, bool _needsTransfer) internal returns (uint256) {
        console.log("Before USDC deposit to water");
        if (_needsTransfer) {
        ...
        console.log("USDC balance: %s", IERC20Upgradeable(USDC).balanceOf(address(this)));

        uint256 balBefore = IERC20Upgradeable(RadiantBearingUSDC).balanceOf(address(this));
        IERC20Upgradeable(USDC).approve(RadiantLendingPool, _assets);

        console.log("Before deposit");
        IRadiantLendingPool(RadiantLendingPool).deposit(
            USDC,
            _assets,
            address(this),
            0
        ...
        uint256 balAfter = IERC20Upgradeable(RadiantBearingUSDC).balanceOf(address(this));
        uint256 bearingUSDC = balAfter - balBefore;
        
        console.log("bearingUSDC",bearingUSDC);

        return bearingUSDC;
    }

    function _radiantWithdraw(uint256 _assets) internal returns (uint256,uint256) {
        uint256 balBefore = IERC20Upgradeable(USDC).balanceOf(address(this));
        uint256 bearingBalBefore = IERC20Upgradeable(RadiantBearingUSDC).balanceOf(address(this));

        console.log("Before withdraw",_assets);

        IRadiantLendingPool(RadiantLendingPool).withdraw(
            USDC,
            _assets,
            address(this)
        );
        uint256 balAfter = IERC20Upgradeable(USDC).balanceOf(address(this));
        uint256 bearingBalAfter = IERC20Upgradeable(RadiantBearingUSDC).balanceOf(address(this));

        uint256 USDCWithdrawn = balAfter - balBefore;
        console.log("USDCWithdrawn",USDCWithdrawn);

        uint256 bearingUSDCWithdrawn = bearingBalBefore - bearingBalAfter;
        console.log("bearingUSDCWithdrawn",bearingUSDCWithdrawn);

        return (bearingUSDCWithdrawn,USDCWithdrawn);
    }

    function _extractBearingProfit() internal {
        console.log("bearingIncrement",bearingIncrement);
        uint256 bearingBal = IERC20(RadiantBearingUSDC).balanceOf(address(this));
        console.log("bearingBal",bearingBal);

        uint256 bearingProfit = bearingBal - bearingIncrement;
        console.log("bearingProfit",bearingProfit);

        if (bearingProfit > 0) {
            console.log("bearingProfit",bearingProfit);
            _radiantWithdraw(bearingProfit);
            console.log("Before transfer to bearing receiver");
            IERC20(USDC).safeTransfer(bearingProfitReceiver, bearingProfit);
            bearingIncrement = IERC20(RadiantBearingUSDC).balanceOf(address(this));
        }
    }
