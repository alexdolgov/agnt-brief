...
contract SingleStaking is ISingleStaking, Ownable {
    using SafeERC20 for IERC20;
    using EnumerableSet for EnumerableSet.AddressSet;

    // Main Reward Token
    IERC20 public immutable esVKAToken;

    uint256 public constant BOOST_PRECISION = 100 * 1e10;

    uint256 public constant ACC_ESVKA_PRECISION = 1e18;

    IBoosting public immutable boostContract;
    // esVKA tokens created per second.
    //@note since we've preminted all the token, have already done the necessary edit to make it work
    ...
    //@note will be Water POD or Bartender POD
    ...
    event Add(uint256 indexed pid, uint256 allocPoint, IERC20 indexed lpToken, IRewardPerSec indexed rewarder);
    ...
    ///@notice constructor
    ///@param _esVKAToken address of the esVKA token
    ///@param _boosting address of the boosting contract
    ///@param _esVKAPerSec amount of esVKA emitted per second
    ///@param _startTimestamp timestamp when the staking starts
    constructor(IERC20 _esVKAToken, IBoosting _boosting, uint256 _esVKAPerSec, uint256 _startTimestamp) {
    ...
    function add(uint256 _allocPoint, IERC20 _lpToken, IRewardPerSec _rewarder) external onlyOwner {
    ...
    function deposit(uint256 _pid, uint256 _amount) external {
        ...
        if (user.amount > 0) {
            ...
            // multiplier will get updated in settlePendingESVKA
        }

        multiplier = updateBoostMultiplier(msg.sender, _pid);
    ...
    function withdraw(uint256 _pid, uint256 _amount) external {
        ...
        user.rewardDebt = (user.amount * multiplier * pool.accesVKAPerShare) / ACC_ESVKA_PRECISION / BOOST_PRECISION;
        poolInfo[_pid].totalBoostedShare = poolInfo[_pid].totalBoostedShare - (_amount * multiplier) / BOOST_PRECISION;
    ...
    function unstakeAndLiquidate(uint256 _pid, address _user, uint256 _amount) public onlyLeverageVault {
        ...
        multiplier = updateBoostMultiplier(_user, _pid);
