  ...
  ERC20BurnableUpgradeable
{
  ...
  function setMC(
    address _mc,
    uint256 _mcPid
  ) public onlyOwner zeroAddress(_mc) {
    MasterChef = _mc;
    MCPID = _mcPid;
  ...
  function setCloser(
    address _closer,
    bool _allowed
  ) public onlyOwner zeroAddress(_closer) {
  ...
  function updateLiquidatorsRewardPercentage(
  ...
  function getPositionUnlockEpoch(
    uint256 positionID,
    address user,
    bool state
    ...
    if (state) {
      ...
      if (
        currentGainsEpoch == _userInfo.epochUnlock &&
        IOpenTradesPnlFeed(openPNL).nextEpochValuesRequestCount() == 0
      ) {
        canBeUnlocked = true;
      } else {
        canBeUnlocked = false;
      ...
      LiquidationRequests memory liquidationRequest = liquidationRequests[
        positionID
      ];
  ...
  function closePosition(
    ...
    require(
      _userInfo.withdrawalRequested,
      "Whiskey: user has not requested a withdrawal"
    );
    require(
      getPositionUnlockEpoch(positionID, _user, true),
      "Whiskey: position is not unlocked yet"
    );
    require(
      allowedClosers[msg.sender] || msg.sender == _userInfo.user,
      "Whiskey: not allowed to close position"
    );
    ...
    (uint256 currentDTV, , uint256 debtValue) = getUpdatedDebtAndValue(
      positionID,
      _user
    );
    ...
    if (currentDTV >= (9 * DECIMAL) / 10 || returnedAssetInDAI < debtValue) {
      ...
      ILendingVault(lendingVault).repayDebt(
        _userInfo.leverage,
        returnedAssetInDAI
      );
    ...
    emit Withdraw(
      ...
      amountAfterFee,
      block.timestamp,
      _userInfo.closedPositionValue
    );
  }

  function requestLiquidationPosition(
    uint256 positionId,
    address user
  ) external {
  ...
  function liquidatePosition(uint256 liquidationId) external {
    LiquidationRequests storage liquidationRequest = liquidationRequests[
      liquidationId
    ];
    require(!isPositionLiquidated[liquidationId], "Whiskey: Not Liquidatable");
    require(
      liquidationRequest.user != address(0),
      "Whiskey: liquidation request does not exist"
    );
    require(
      getPositionUnlockEpoch(liquidationId, address(this), false),
      "Whiskey: position is not unlocked yet"
    );
    uint256 position = userInfo[liquidationRequest.user][
    ...
    // repay debt
    IERC20Upgradeable(dai).safeApprove(
      lendingVault,
      amountAfterLiquidatorReward
    );
    ILendingVault(lendingVault).repayDebt(
      liquidationRequest.leverage,
      amountAfterLiquidatorReward
    );
  ...
  function makeWithdrawRequestWithAssets(
    ...
    // epoch time are incremented by 1 in ever life cycle.
    uint256 currentGainsEpoch = IGainsVault(gainsVault).currentEpoch();
    if (_userInfo.epochUnlock < currentGainsEpoch) {
      require(
        !_userInfo.withdrawalRequested,
        "Whiskey: user has already requested a withdrawal"
      );
    }

    uint256 userAmountStaked;
    if (MasterChef != address(0)) {
      (userAmountStaked, ) = IMasterChef(MasterChef).userInfo(
        MCPID,
        _userInfo.user
      );
      if (userAmountStaked > 0) {
        IMasterChef(MasterChef).unstakeAndLiquidate(
          MCPID,
          _userInfo.user,
          _userInfo.position
        );
    ...
    if (userAmountStaked == 0 && !_userInfo.withdrawalRequested) {
      _burn(_userInfo.user, _userInfo.position);
    }

    require(
      IOpenTradesPnlFeed(openPNL).nextEpochValuesRequestCount() == 0,
      "Whiskey: gdai next epoch values request count is not 0"
    );
  ...
  function getWithdrawableEpochTime() public view returns (uint256) {
    return
      IGainsVault(gainsVault).currentEpoch() +
      IGainsVault(gainsVault).withdrawEpochsTimelock();
  }
  ...
  function getUtilizationRate() public view returns (uint256) {
    ...
    return
      totalWaterDebt == 0
        ? 0
        : totalWaterDebt.mulDiv(DECIMAL, totalWaterAssets + totalWaterDebt);
  }
  ...
  function transferFrom(
    ...
    require(
      allowedSenders[from] || allowedSenders[to] || allowedSenders[spender],
