...
contract WSOLWater is ERC4626Upgradeable, OwnableUpgradeable {
    ...
    modifier onlyWSOLGifter() {
        require(allowedToGift[msg.sender], "Not allowed to increment WSOL");
        _;
    }
    ...
    event WSOLGifterAllowed(address indexed gifter, bool status);
    ...
    function initialize(address _WSOL) external initializer {
        require(_WSOL != address(0), "ZERO_ADDRESS");
        WSOL = _WSOL;
        ...
        __Ownable_init();
        __ERC4626_init(IERC20Upgradeable(_WSOL));
        __ERC20_init("WSOL-WATER", "WSOL-WATER");
    }
    ...
    function allowWSOLGifter(address _gifter, bool _status) external onlyOwner zeroAddress(_gifter) {
        allowedToGift[_gifter] = _status;
        emit WSOLGifterAllowed(_gifter, _status);
    }
    ...
    /**
     * @notice Allow the VodkaV2 Vault to lend a certain amount of WSOL to the protocol.
     * @dev The function allows the VodkaV2 Vault to lend a certain amount of WSOL to the protocol. It updates the total debt and total WSOL balances accordingly.
     * @param _borrowed The amount of WSOL to lend.
    ...
    function lend(uint256 _borrowed, address _receiver) external onlyAllowedVaults returns (bool status) {
        ...
        require(totalAsset > am, "Not enough WSOL to lend");

        totalDebt += _borrowed;
        totalAsset -= _borrowed;

        require(getUtilizationRate() <= utilRate, "Leverage ratio too high");
        IERC20(WSOL).safeTransfer(_receiver, am);
     ...
     * @dev The function allows the VodkaV2 Vault to repay a certain amount of debt to the protocol. It updates the total debt and total WSOL balances accordingly.
     * @param _debtAmount The amount of debt to repay.
     * @param _amountPaid The amount of WSOL paid to repay the debt.
    ...
    function repayDebt(uint256 _debtAmount, uint256 _amountPaid) external onlyAllowedVaults returns (bool) {
        IERC20(WSOL).safeTransferFrom(msg.sender, address(this), _amountPaid);
     ...
     *      It updates the total WSOL balance and sets a timelock for the receiver.
    ...
    function deposit(uint256 _assets, address _receiver) public override noZeroValues(_assets) returns (uint256) {
        ...
        IERC20(WSOL).transferFrom(msg.sender, address(this), _assets);
    ...
    function _deposit(address caller, address receiver, uint256 assets, uint256 _shares) internal override noZeroValues(assets) {
        ...
        uint256 shares;
        if (totalSupply() == 0) {
            require(assets > 10000, "Not Enough Shares for first mint");
            // WSOL decimal is known to be 9
            uint256 SCALE = 10 ** decimals() / 10 ** 9;
            shares = (assets - 10000) * SCALE;
            _mint(address(this), 10000 * SCALE);
        } else {
            shares = _shares;
        }
     ...
     *      The function then performs the actual withdrawal by calling the internal `_withdraw` function. It updates the total WSOL balance after the withdrawal and returns the amount of shares returned to the user.
     * @param _assets The amount of assets (WSOL) to withdraw.
     * @param _receiver The address of the receiver who will receive the corresponding shares.
     * @param _owner The address of the owner who is making the withdrawal.
     * @return The amount of shares returned to the user.
     */
    function withdraw(
        uint256 _assets, // Native (WSOL) token amount
        address _receiver,
        address _owner
        ...
        IERC20(WSOL).safeTransfer(feeReceiver, feeAmount);
    ...
    function increaseTotalWSOL(uint256 _amount) external onlyWSOLGifter {
        IERC20(WSOL).safeTransferFrom(msg.sender, address(this), _amount);
        totalAsset += _amount;
    }
