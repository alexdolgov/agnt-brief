...
contract Vester3MonthsV2 is IERC20Upgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    uint256 public extraAmountBurned;
    uint256 public accumulatedAmountNotBurned; //amount that was not burned in the current vesting period
    ...
    function setExtraAmountBurned(uint256 _extraAmountBurned) external onlyOwner {
        extraAmountBurned = _extraAmountBurned;
    }
    ...
    function remainingVestingAmount(address _account) public view returns (uint256) {
        ...
        return balance - claimable(_account) * 2;
    }
    ...
    function _updateVesting(address _account) private {
        ...
        _burn(_account, amount * 2);
        cumulativeClaimAmounts[_account] = cumulativeClaimAmounts[_account] + amount;
        //
        ERC20BurnableUpgradeable(esToken).burn(amount * 2);

        if (accumulatedAmountNotBurned < extraAmountBurned) {
            accumulatedAmountNotBurned += amount;
        } else {
            ERC20BurnableUpgradeable(claimableToken).burn(amount);
        }
    }
    ...
    function _getNextClaimableAmount(address _account) private view returns (uint256) {
        ...
        uint256 claimableAmount = (vestedAmount * timeDiff) / VESTING_DURATION / 2;

        if (claimableAmount < balance / 2) {
