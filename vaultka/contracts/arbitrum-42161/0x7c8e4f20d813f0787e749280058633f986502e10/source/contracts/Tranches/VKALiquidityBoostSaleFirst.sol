...
contract VKALiquidityBoostSaleFirst is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    using MathUpgradeable for uint256;
    using SafeCastUpgradeable for uint256;
    using SafeERC20Upgradeable for IERC20Upgradeable;

    uint256 public firstTrancheCap;
    ...
    function initialize(address _vkaToken, address _weth, uint256 _firstTrancheCap) external initializer {
        vkaToken = _vkaToken;
        weth = _weth;
        firstTrancheCap = _firstTrancheCap;
        PRECISION = 10 ** 18;
        __Ownable_init();
    ...
    function withdraw(address to) external onlyOwner {
        ...
        uint256 ethToWithdraw = totalETHDeposited >= firstTrancheCap ? firstTrancheCap : totalETHDeposited;
        ethWithdrawn = true;
    ...
    function deposit(uint256 _amount) public nonReentrant {
        ...
        require(totalETHDeposited + _amount <= firstTrancheCap, "Target amount reached");
