...
import {IDevSrcBridge} from "./interfaces/IDevSrcBridge.sol";
...
contract DevSrcBridge is Initializable, PausableUpgradeable, AccessControlUpgradeable, IDevSrcBridge {
    ...
    function initialize(
        ...
        if (_defaultAdmin == address(0) || _exchange == address(0) || address(_spokePool) == address(0) || _router == address(0))
            revert ZeroAddress();
     ...
     * @param toAddress The address to deposit the tokens to.
     *
     * Requirements:
   ...
   function withdraw(
        ...
        if (signer != account) {
            ...
            if (!isSolanaNative) {
                ...
                bytes32 digest = ECDSAUpgradeable.toTypedDataHash(
                    keccak256(abi.encode(DOMAIN_TYPEHASH, NAME_HASH, VERSION_HASH, router)),
                    structHash
                );
        ...
        emit Withdrawed(account, token, amount, recipient, requestHash);
    }
     ...
     function withdrawSolana(
        ...
        (
            ...
            uint256 amount,
            uint256 chainId
        ) = abi.decode(requestArgs, (uint256, uint256, bool, address, bytes32, address, uint256, uint256));
        if (!supportedTokens[token]) revert UnsupportedToken();
        bytes32 requestHash = keccak256(requestArgs);
        if (completedTransactions[requestHash]) revert AlreadyProcessed();
        if (isDeposit) revert InvalidRequestType();
        ...
        ITokenMessenger(CCTPTokenMessenger).depositForBurn(amount, 5, recipient, token);
        emit WithdrawedSolana(account, token, amount, recipient, requestHash);
    }
