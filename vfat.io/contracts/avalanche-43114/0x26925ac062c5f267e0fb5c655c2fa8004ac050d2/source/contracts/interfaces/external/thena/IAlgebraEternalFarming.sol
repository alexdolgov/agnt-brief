...
/// @notice Allows farming nonfungible liquidity tokens in exchange for reward
/// tokens without locking NFT for incentive time
interface IAlgebraEternalFarming {
    ...
    struct IncentiveParams {
        ...
        uint128 bonusReward; // The amount of bonus reward tokens to be
            // distributed
        uint128 rewardRate; // The rate of reward distribution per second
        uint128 bonusRewardRate; // The rate of bonus reward distribution per
            // second
        uint24 minimalPositionWidth; // The minimal allowed width of position
            // (tickUpper - tickLower)
    }
    ...
    /// @notice Returns hash of 'INCENTIVE_MAKER_ROLE', used as role for
    /// incentive creation
    function INCENTIVE_MAKER_ROLE() external view returns (bytes32);

    /// @notice Returns hash of 'FARMINGS_ADMINISTRATOR_ROLE', used as role for
    /// permissioned actions in farming
    function FARMINGS_ADMINISTRATOR_ROLE() external view returns (bytes32);

    /// @notice The nonfungible position manager with which this farming
    ...
    /// @dev Does not check if the incentive is indeed currently connected to
    /// the Algebra pool or not
    /// @param incentiveId The ID of the incentive computed from its parameters
    ...
    /// @notice Changes `isEmergencyWithdrawActivated`. Users can withdraw
    /// liquidity without any checks if activated.
    /// User cannot enter to farmings if activated.
    /// _Must_ only be used in emergency situations. Farmings may be unusable
    /// after activation.
    ...
    /// according to the last time all farms were updated
    /// @param owner The owner for which the rewards owed are checked
    /// @param rewardToken The token for which to check rewards
    /// @return rewardsOwed The amount of the reward token claimable by the
    /// owner
    function rewards(
        address owner,
        address rewardToken
    ...
    /// @notice Transfers `amountRequested` of accrued `rewardToken` (if
    /// possible) rewards from the contract to the recipient `to`
    /// @param rewardToken The token being distributed as a reward
    /// @param to The address where claimed rewards will be sent to
    /// @param amountRequested The amount of reward tokens to claim. Claims
    /// entire reward amount if set to 0.
    /// @return rewardBalanceBefore The total amount of unclaimed reward
    /// *before* claim
    ...
    /// @notice Calculates the reward amount that will be received for the given
    /// farm
    /// @param key The key of the incentive
    /// @param tokenId The ID of the token
    /// @return reward The reward accrued to the NFT for the given incentive
    /// thus far
    /// @return bonusReward The bonus reward accrued to the NFT for the given
    /// incentive thus far
    ...
    /// @return innerRewardGrowth0 The last saved reward0 growth inside
    /// position,
    ...
    /// @param rewardRate The new rate of main token (token0) distribution per
    /// sec
    /// @param bonusRewardRate The new rate of bonus token (token1) distribution
    /// per sec
    function setRates(
        IncentiveKey memory key,
        uint128 rewardRate,
    ...
    /// @notice Event emitted when a liquidity mining incentive has been stopped
    /// from the outside
    ...
    /// @param incentiveId The ID of the incentive for which rewards were
    /// decreased
    ...
    /// @param minimalAllowedPositionWidth The minimal allowed position width
    /// (tickUpper - tickLower)
    ...
    /// @param newStatus New value of `isEmergencyWithdrawActivated`. Users can
    /// withdraw liquidity without any checks if active.
