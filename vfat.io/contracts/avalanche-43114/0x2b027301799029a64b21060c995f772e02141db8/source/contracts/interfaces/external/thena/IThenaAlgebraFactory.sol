...
interface IThenaAlgebraFactory {
    ...
    function defaultConfigurationForPool()
        external
        view
        returns (uint16 communityFee, int24 tickSpacing, uint16 fee);
    ...
    /// @notice Deterministically computes the custom pool address given the
    /// customDeployer, token0 and token1
    /// @dev The method does not check if such a pool has been created
    ...
    /// @notice Returns the custom pool address for a customDeployer and a given
    ...
    /// @notice Creates a custom pool for the given two tokens using `deployer`
    /// contract
    /// @param deployer The address of plugin deployer, also used for custom
