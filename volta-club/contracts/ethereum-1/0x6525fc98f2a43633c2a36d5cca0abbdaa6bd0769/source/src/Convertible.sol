...
contract Convertible {
    ...
    function writeOff(address borrower) public returns (bool success) {
        ...
        // Conditions: debt > 0, debt < 1e18, has collateral
        if (borrowerDebt == 0 || borrowerDebt >= 1e18 || borrowerCollateral == 0) {
            return false;
        }
        ...
        // Check: debt < collateral value (ignoring collateralFactor)
        uint256 collateralValue = borrowerCollateral * price / (10 ** collateralDecimals);
        if (borrowerDebt >= collateralValue) {
            return false;
        }
