...
contract VaultBoostManager is IVaultBoostManager, UUPSUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    /// @dev DEPRECATED 2026-05-01: short pool moved to PerpManager (IAddressProvider).
    ///      Slot retained for storage layout compatibility on UUPS upgrade.
    ///      Read live value via `shortPool()` (delegates to the manager).
    ...
    ///      The 2nd parameter (short pool) is UNUSED — kept anonymous for ABI
    ///      compatibility. As of 2026-05-01 the short pool is read live from the
    ///      manager (IAddressProvider.shortPool()).
    ...
    function initiateBoost(address token, uint256 amount, uint256 startTimestamp, uint256 duration) external nonReentrant {
        ...
        IWasabiVault vault = _getManager().shortPool().getVault(token);
    ...
    ///         Backwards-compatible shim for callers that previously read `vbm.shortPool()`
