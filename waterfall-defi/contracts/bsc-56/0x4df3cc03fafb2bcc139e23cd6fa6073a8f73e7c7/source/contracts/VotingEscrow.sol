    ...
    Multiplier
{
    ...
    uint256 public MAX_LOCK_TIME = 63113904; // 2 years
    uint256 public MIN_LOCK_TIME = 7889238; // 3 months
    ...
    constructor(address _wtf, address _governor)
        VEWTF("WTF Voting Escrow", "VeWTF")
        Permissions(_governor)
    {
        wtf = IERC20(_wtf);
    }
        ...
        internal
    {
        ...
        if (expired) {
            ...
            require(
                lock.expiryTimestamp.sub(lock.startTimestamp).add(_duration) <=
                    MAX_LOCK_TIME,
                "WTF Voting Escrow: Greater than MAX_LOCK_TIME"
            );
