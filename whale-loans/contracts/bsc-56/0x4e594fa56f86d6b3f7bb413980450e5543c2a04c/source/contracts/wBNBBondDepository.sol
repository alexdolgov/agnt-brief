...
contract WBNBHumpBondDepository is Ownable {
    ...
    enum PARAMETER { VESTING, PAYOUT, NULL, DEBT, BCV }
    /**
    ...
    function setBondTerms ( PARAMETER _parameter, uint _input ) external onlyPolicy() {
        require(_parameter != PARAMETER.NULL, "Not supported for WBNB");
        
        if ( _parameter == PARAMETER.VESTING ) { // 0
            require( _input >= 50000, "Vesting must be longer than 36 hours" );
    ...
    function deposit(
        ...
        ITreasury( treasury ).mint(address(this), payout);
    ...
    function stakeOrSend( address _recipient, bool _stake, uint _amount ) internal returns ( uint ) {
        if ( !_stake ) { // if user does not want to stake
            ...
            IStaking( staking ).stake( _recipient, _amount, true );
        }
        return _amount;
    }
