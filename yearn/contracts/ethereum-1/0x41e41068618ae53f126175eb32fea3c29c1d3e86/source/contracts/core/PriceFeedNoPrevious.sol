...
import "contracts/interfaces/IAggregatorV3Interface.sol";
import "contracts/utils/Address.sol";
import "contracts/dependencies/PrismaMath.sol";
import "contracts/dependencies/PrismaOwnable.sol";

/**
    @title Prisma Multi Token Price Feed Without Previous Answer
    @notice Based on Gravita's PriceFeed:
            ...
            This implementation does not require retrieving the previous answer from the price aggregator.
            To be used only for price feeds which do not support historical prices.

 */
contract PriceFeedNoPrevious is PrismaOwnable {
    struct OracleRecord {
    ...
    function _setOracle(
        ...
        (FeedResponse memory currResponse, ) = _fetchFeedResponse(newFeed, 0);

        if (!_isValidResponse(currResponse)) {
    ...
    function fetchPrice(address _token) public returns (uint256) {
        ...
        if (priceRecord.lastUpdated != block.timestamp) {
            ...
            (FeedResponse memory currResponse, bool updated) = _fetchFeedResponse(
    ...
    function _processFeedResponses(
        ...
        bool isValidResponse = _isValidResponse(_currResponse) &&
    ...
    function _fetchFeedResponse(
        IAggregatorV3Interface oracle,
        uint256 oracleUpdatedTimestamp
    ) internal view returns (FeedResponse memory currResponse, bool updated) {
        currResponse = _fetchCurrentFeedResponse(oracle);
        updated = currResponse.timestamp > oracleUpdatedTimestamp;
    }
