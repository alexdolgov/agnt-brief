...
contract TwapOracle is AprOracleBase {
    uint256 private constant HALF_HOUR = 30 minutes;
    uint256 private constant WEEK = 7 days;
    uint256 private constant YEAR = 365 days;
    uint256 private constant WAD = 1e18;

    mapping(address => address) public uniswapPool;

    constructor() AprOracleBase("Twap APR Oracle", msg.sender) {}
    ...
    {
        address _uniswapPool = uniswapPool[_strategy];
        if (_uniswapPool != address(0)) {
            int256 _uniswapTwapApr = _getAprFromUniswapTWAP(
                _uniswapPool,
                _strategy
            );
            _apr = (_uniswapTwapApr >= 0) ? uint256(_uniswapTwapApr) : 0;
        }

    }

    function getAprFromUniswapTWAP(address _strategy)
        external
        view
        returns (int256)
    {
        return _getAprFromUniswapTWAP(uniswapPool[_strategy], _strategy);
    }

    function setUniswapPool(address _token, address _uniswapPool) external onlyGovernance {
        uniswapPool[_token] = _uniswapPool;
    }

    function _getAprFromUniswapTWAP(address _uniswapPool, address _asset)
        internal
        view
        returns (int256 _apr)
    {
        ...
        uint256 timeMultiplier = (YEAR * WAD) / (startingObservation - HALF_HOUR);
