...
contract SeasolverHelper {
          ...
          uint256 gasLeft)
  {
    ...
    // }
        ...
        return isSelling ? balanceAfter - balanceBefore : balanceBefore - balanceAfter;  
    }
    ...
    function flattenedBalancerJoinPool(
        ...
        bytes memory userData = isSelling ? abi.encode("uint256", "uint256", "uint256", 2, amountGiven, enterIndex)
         : abi.encode('uint256', 'uint256[]', 'uint256', 1, maxAmountsIn,0);
        JoinPoolRequest memory request = JoinPoolRequest(assets, maxAmountsIn, userData, false);
        uint256 balanceBefore = outToken.balanceOf(address(tradeHandler));
    ...
    function flattenedBalancerExitPool(
        ...
        bytes memory userData = isSelling ? abi.encode(['uint256', 'uint256', 'uint256'], 0, amountGiven, exitIndex)
        : abi.encode(['uint256', 'uint256[]', 'uint256'], 2, amountsOut, 2 ** 256 -1);
        ExitPoolRequest memory request = ExitPoolRequest(assets, minAmountsOut, userData, false);
        uint256 balanceBefore = outToken.balanceOf(address(tradeHandler));
