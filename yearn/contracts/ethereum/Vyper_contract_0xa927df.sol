...
def __init__(
    ...
    self.payee = owner
    self.packed_scaling_factor = shift(scaling_factor, 128) | shift(
        scaling_factor, 192
    )
...
def redeem(amount: uint256, recipient: address = msg.sender) -> uint256:
    ...
    tolerance: uint256 = (
        eth_required * SLIPPAGE_TOLERANCE / SLIPPAGE_DENOMINATOR
    )
    if msg.value < (eth_required - tolerance) or msg.value > (
        eth_required + tolerance
    ):
...
def scaling_factor_ramp() -> (uint256, uint256, uint256, uint256):
    ...
    old: int256 = 0
    new: int256 = 0
    ramp_start, ramp_end, old, new = self._unpack_scaling_factor(
        self.packed_scaling_factor
    )
...
@external
def start_ramp(
    new: uint256, duration: uint256 = 604_800, start: uint256 = block.timestamp
):
    """
...
def stop_ramp():
    ...
    assert active
    self.packed_scaling_factor = self._pack_scaling_factor(
        0, 0, scaling_factor, scaling_factor
    )
...
@internal
@pure
def _pack_scaling_factor(
    _ramp_start: uint256, _ramp_end: uint256, _old: int256, _new: int256
) -> uint256:
    assert _ramp_start <= MASK and _ramp_end <= MASK
    assert (
        _old <= convert(MASK, int256)
        and _new > 0
        and _new <= convert(MASK, int256)
    )
    return (
        _ramp_start
        | shift(_ramp_end, 64)
        | shift(convert(_old, uint256), 128)
        | shift(convert(_new, uint256), 192)
    )


@internal
@pure
def _unpack_scaling_factor(
    _packed: uint256,
) -> (uint256, uint256, int256, int256):
    return _packed & MASK, shift(_packed, -64) & MASK, convert(
        shift(_packed, -128) & MASK, int256
    ), convert(shift(_packed, -192), int256)
