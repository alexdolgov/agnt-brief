...
@license GNU AGPLv3
@notice
    Voting contract for weight redistribution. Time is divided in 4 week epochs. In the final week of the epoch, 
    all users are able to vote on the current pool, as well as a 'blank' option, indicating their desire to 
    keep the composition unchanged. A part of the weight of the assets in the pool is redistributed according to 
...
def vote(_votes: DynArray[uint256, 33]):
    ...
    assert len(_votes) <= NUM_ASSETS + 1
