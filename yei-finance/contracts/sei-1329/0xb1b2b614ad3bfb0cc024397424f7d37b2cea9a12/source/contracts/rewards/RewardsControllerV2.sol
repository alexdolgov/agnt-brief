...
contract RewardsControllerV2 is RewardsDistributor, VersionedInitializable, IRewardsController {
  ...
  function _claimRewards(
    ...
    uint8 rewardTokenDecimals = IERC20(reward).decimals();
    if (rewardTokenDecimals < 18) {
      totalRewards = totalRewards / 10**(18 - rewardTokenDecimals);
    } 
  ...
  function _claimAllRewards(
    ...
    for (uint256 i = 0; i < rewardsListLength; i++) {
      ...
      uint8 rewardTokenDecimals = IERC20(rewardsList[i]).decimals();
      if (rewardTokenDecimals < 18) {
        claimedAmounts[i] = claimedAmounts[i] / 10**(18 - rewardTokenDecimals);
      }
