...
import "./Interfaces/IPriceFeed3.sol";
 ...
 */
contract PriceFeed3 is IPriceFeed3, OwnableUpgradeable, UUPSUpgradeable, Addresses {
	// Constants --------------------------------------------------------------------------------------------------------
	...
	function setOracle(
		...
		if (_type == ProviderType.Chainlink && decimals == 0) {
		...
		uint256 price = _fetchOracleScaledPrice(newOracle,_token);
		if (price == 0) {
	...
	function fetchPrice(address _token) public view virtual returns (uint256) {
		...
		uint256 price = _fetchOracleScaledPrice(oracle,_token);
		if (price != 0) {
		...
		price = _fetchOracleScaledPrice(oracle,_token);
		if (price != 0) {
			return oracle.isEthIndexed ? _calcEthIndexedPrice(price) : price;
		}
		revert PriceFeed__InvalidOracleResponseError(_token);
	...
	function _fetchOracleScaledPrice(OracleRecordV2 memory oracle,address _token) internal view returns (uint256) {
		uint256 oraclePrice;
		...
		if (ProviderType.Pyth == oracle.providerType) {
			bytes32 id = pythIds[oracle.oracleAddress][_token];
			(oraclePrice, priceTimestamp,decimals) = _fetchPythOracleResponse(oracle.oracleAddress,id);
		}
		if (oraclePrice != 0 && !_isStalePrice(priceTimestamp, oracle.timeoutSeconds)) {
			return _scalePriceByDigits(oraclePrice, decimals);
		}
		return 0;
	}
	...
	function _fetchPythOracleResponse(
		address pythAddress,
		bytes32 id
	) internal view returns (uint256 price, uint256 timestamp,uint32 decimals) {
		try IPyth(pythAddress).getPriceUnsafe(id) returns (PythStructs.Price memory pyPrice
		) {
			int64 answer = pyPrice.price;
			uint publishTime = pyPrice.publishTime;
			int32 intDecimals = pyPrice.expo;
 		if (price != 0 && publishTime != 0 ) {
				price = uint256(uint64(answer));
				timestamp = publishTime;
				if(intDecimals > 0 ){
				  decimals = uint32(intDecimals);
				}else{
					decimals = uint32(-intDecimals);
				}
			}
	...
	function setPythId(address _oracleAddress,address _token,bytes32 id) external  onlyOwner {
		pythIds[_oracleAddress][_token] = id;
	}
}
