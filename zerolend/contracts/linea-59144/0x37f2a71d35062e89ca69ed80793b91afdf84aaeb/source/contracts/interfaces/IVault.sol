...
interface IVault {
    ...
    function getEqEth(uint256 lsdAmount) external view  returns(uint256 eqEth) ;
    function getEqLsd(uint256 amount) external view  returns(uint256 eqLsd) ;
    function vaultFactory() external view returns (address );
    function vaultGauge() external view returns (address );
