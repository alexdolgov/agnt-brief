...
contract Pair is IPair {
    ...
    function _update0(uint amount) internal {
         // get treasury fee
        address _treasury = PermissionsRegistry(PairFactory(factory).permissionsRegistry()).treasury();
        uint256 _treasuryFee = PairFactory(factory).treasuryFee();
        uint256 _treasuryAmount = amount * _treasuryFee / 10000;
        _safeTransfer(token0, _treasury, _treasuryAmount); // transfer the fees out to PairFees
        amount -= _treasuryAmount;
        ...
        emit Fees(msg.sender, amount+_treasuryAmount, 0);
    }
    ...
    function _update1(uint amount) internal {
        ...
        uint256 _treasuryFee = PairFactory(factory).treasuryFee();
        uint256 _treasuryAmount = amount * _treasuryFee / 10000;
        _safeTransfer(token1, _treasury, _treasuryAmount); // transfer the fees out to PairFees
        amount -= _treasuryAmount;
        ...
        emit Fees(msg.sender, 0,  amount+ _treasuryAmount);
    }
