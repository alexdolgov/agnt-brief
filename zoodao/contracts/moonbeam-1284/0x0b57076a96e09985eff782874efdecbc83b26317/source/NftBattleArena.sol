...
contract NftBattleArena
{
	...
	uint256 public baseStakerReward = 62500 * 10 ** 18;       // 83 333 zoo.
	uint256 public baseVoterReward = 500000 * 10 ** 18;      // 2 000 000 zoo.
...
/*
	...
	function _calculateVotingYTokensExcludingRewards(uint256 votingPositionId) internal view returns(uint256 yTokens)
	{
		...
		for (uint256 i = startEpoch; i < endEpoch; i++)
		{
			...
			{
				yTokens -= daiInvested * 10**6 / rewardsForEpoch[stakingPositionId][i].pricePerShareCoef;
			}
		}
	}*/
	...
	function withdrawDaiFromVoting(uint256 votingPositionId, address voter, address beneficiary, uint256 daiNumber, bool toSwap) public only(nftVotingPosition)
	{
		...
		votingPosition.yTokensNumber -= /*_calculateVotersYTokensExcludingRewards(votingPositionId)*/ shares;// Decreases amount of shares.
	...
	function claimRewardFromVoting(uint256 votingPositionId, address voter, address beneficiary) external only(nftVotingPosition) returns (uint256 daiReward)
	{
		...
		if (battleReward.yTokens >= yTokenReward) // todo: it's kostyl and possible it should to set yTokenReward as Math.min of yTokenReward * 950 / 975 and battleReward.yTokens
		{
	...
	function getPendingVoterReward(uint256 votingPositionId) public view returns (uint256 yTokens, uint256 wells, uint256 glmrs)
	{
		...
		for (uint256 i = startEpoch; i < endEpoch; i++)
		{
			...
			if (totalVotes > 0 && numberOfPlayedPairsInEpoch[i] > 0) // todo: possible need only second condition
			{
				wells += 965 * wellClaimedByEpoch[i] * votingPosition.votes / totalVotes / numberOfPlayedPairsInEpoch[i] / 1000; // 96.5%
				glmrs += 965 * glmrClaimedByEpoch[i] * votingPosition.votes / totalVotes / numberOfPlayedPairsInEpoch[i] / 1000;
			}
		}
